...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         ...
                         ((discoveryType, "args"))((sourceType, "RmanCpp"))((bxdfType, "bxdf")));
...
TF_DEFINE_PRIVATE_TOKENS(_xmlAttributeNames,

                         ((nameAttr, "name"))((typeAttr, "type"))((arraySizeAttr, "arraySize"))(
                             (defaultAttr, "default"))((inputAttr, "input"))((tagAttr, "tag"))(
                             (vstructmemberAttr, "vstructmember"))((sdrDefinitionNameAttr,
                                                                    "sdrDefinitionName")));
...
static std::tuple<TfToken, size_t> _GetTypeName(const NdrTokenMap &attributes);

// Gets a VtValue instance from a string value and a type, array, metadata
// hint
static VtValue _GetVtValue(const std::string &stringValue,
                           TfToken &type,
                           size_t arraySize,
                           const NdrTokenMap &metadata);

// Emits a deprecation warning to the log
static void _OutputDeprecationWarning(const TfToken &attrName,
...
static SdrShaderPropertyUniquePtr _ParseChildElem(const SdrShaderRepresentation &shaderRep,
                                                  bool isOutput,
                                                  pugi::xml_node childElement,
                                                  const std::string &parentPage);

// Creates a SdrShaderProperty with the given inputs
static SdrShaderPropertyUniquePtr _CreateProperty(const SdrShaderRepresentation &entry,
                                                  bool isOutput,
...
static void _InjectParserMetadata(NdrTokenMap &metadata, const TfToken &typeName);

// Recursively builds up the shaderRep as the shader is parsed
static void _Parse(SdrShaderRepresentation &shaderRep,
                   pugi::xml_node parent,
                   const std::string &parentPage);

// For each child under the specified parent, get the value of the
// specified attribute
static NdrStringVec _GetAttributeValuesFromChildren(pugi::xml_node parent, const char *attrName);

// Helper methods to get a value out of the attr map (or the default value
// if the key doesn't exist on the map)
static TfToken _Get(const NdrTokenMap &map, const TfToken &key, TfToken defaultValue);

static int _Get(const NdrTokenMap &map, const TfToken &key, int defaultValue);
...
static bool _LoadXmlDoc(const NdrNodeDiscoveryResult &discoveryResult, xml_document *doc)
{
...
static TfToken _GetSdrContextFromShaderType(const TfToken &shaderType)
{
...
NdrNodeUniquePtr RmanArgsParserPlugin::Parse(const NdrNodeDiscoveryResult &discoveryResult)
{
  ...
  metadata.insert(shaderRepresentation.metadata.begin(), shaderRepresentation.metadata.end());
      ...
      pathElts[pathElts.size() - 2] != "Args")
  {
    ...
    if (pathElts.size() > 1 && pathElts[pathElts.size() - 2] != "Args") {
      TF_DEBUG(NDR_PARSING)
          .Msg(
              "Args file being parsed does not "
                   ...
                   const std::string &parentPage)
{
  ...
  while (childElement) {
    ...
    if (isInput || isOutput) {
      ...
      const NdrTokenMap &propMetadata = shaderRep.properties.back()->GetMetadata();
      if (propMetadata.find(SdrPropertyMetadata->ImplementationName) != propMetadata.end()) {
    ...
    // error, reporting malformed args file.
      ...
      while (attrChild) {
        ...
        shaderRep.metadata.emplace(TfToken(attrChild.name()), attrValue.value());
        if (EQUALS(SdrNodeMetadata->SdrDefinitionNameFallbackPrefix.GetText(), attrChild.name())) {
          ...
          shaderRep.sdrDefinitionNameFallbackPrefix = TfToken(attrValue.value());
        }
        attrChild = attrChild.next_sibling();
      }
    }
    ...
    // ---------------------------------------------------------------------
      ...
      if (parentPage.empty()) {
        ...
        _Parse(shaderRep,
               childElement,
               parentPage + SdrPropertyTokens->PageDelimiter.GetString() + pageName);
      }
    }
    ...
    // <apiSchemasForAttrPruning> is a list of apiSchema names (schemas).
      ...
      while (attrChild) {
        if (EQUALS(apiSchemaAutoApplyToStr, attrChild.name())) {
          NdrStringVec apiSchemas = _GetAttributeValuesFromChildren(attrChild, "name");
          shaderRep.metadata.emplace(TfToken(apiSchemaAutoApplyToStr),
                                     CreateStringFromStringVec(apiSchemas));
        }
        else if (EQUALS(apiSchemaCanOnlyApplyToStr, attrChild.name())) {
          NdrStringVec apiSchemas = _GetAttributeValuesFromChildren(attrChild, "name");
          shaderRep.metadata.emplace(TfToken(apiSchemaCanOnlyApplyToStr),
                                     CreateStringFromStringVec(apiSchemas));
        }
        else if (EQUALS(apiSchemasForAttrPruningStr, attrChild.name())) {
          NdrStringVec apiSchemas = _GetAttributeValuesFromChildren(attrChild, "name");
          shaderRep.metadata.emplace(TfToken(apiSchemasForAttrPruningStr),
                                     CreateStringFromStringVec(apiSchemas));
        }
        else {
          xml_attribute attrValue = attrChild.attribute(valueStr);
...
static SdrShaderPropertyUniquePtr _CreateProperty(const SdrShaderRepresentation &shaderRep,
                                                  bool isOutput,
                                                  NdrTokenMap &attributes,
                                                  NdrStringVec &validConnectionTypes,
                                                  NdrOptionVec &options)
{
  ...
  if (attributes.count(_xmlAttributeNames->sdrDefinitionNameAttr)) {
    ...
    TfToken definitionName = TfToken(
        SdfPath::JoinIdentifier(shaderRep.sdrDefinitionNameFallbackPrefix, propName));
    attributes[SdrPropertyMetadata->ImplementationName] = propName;
    propName = definitionName;
  }
...
static NdrStringVec _GetAttributeValuesFromChildren(xml_node parent, const char *attrName)
{
