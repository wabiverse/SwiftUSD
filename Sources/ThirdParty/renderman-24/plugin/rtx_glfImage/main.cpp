...
template<class T>
static void _ConvertToFloatAndFill(const HioImage::StorageSpec &level,
                                   RtxPlugin::FillRequest &fillReq)
{
  ...
  const T *src = reinterpret_cast<const T *>(level.data) +
                 (startY * level.width + startX) * numImageChannels + (fillReq.channelOffset);
  ...
  if (fillReq.channelOffset == 0 && fillReq.numChannels == numImageChannels) {
    for (int y = startY; y < endY; y++) {
      const T *s = src;
      for (int x = 0; x < numImageChannels * fillReq.tile.size.X; x++, s++, dest++) {
...
int RtxHioImagePlugin::Open(TextureCtx &tCtx)
{
  ...
  // Open HioImage.
  HioImageSharedPtr image = HioImage::OpenForReading(filename, 0, 0, sourceColorSpace);
  if (!image) {
    if (m_msgHandler) {
