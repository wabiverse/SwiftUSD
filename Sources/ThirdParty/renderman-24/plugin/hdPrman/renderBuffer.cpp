...
/*virtual*/
bool HdPrmanRenderBuffer::Allocate(GfVec3i const &dimensions,
                                   HdFormat format,
                                   bool multiSampled)
{
  _Deallocate();
...
void HdPrmanRenderBuffer::Blit(HdFormat format, int width, int height,
                               uint8_t const *data)
{
      ...
      for (unsigned int i = 0; i < _width; ++i)
      {
        ...
        if (convertAsInt)
        {
          _ConvertPixel<int32_t>(
              _format, static_cast<uint8_t *>(&_buffer[(j * _width + i) * dstPixelSize]),
