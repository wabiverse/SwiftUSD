...
{
  HdSceneIndexPluginRegistry::Define<HdPrman_VelocityMotionBlurSceneIndexPlugin>();
}
...
TF_REGISTRY_FUNCTION(HdSceneIndexPlugin)
{
  ...
  HdContainerDataSourceHandle const inputArgs = HdRetainedContainerDataSource::New(
      _tokens->fps, HdRetainedSampledDataSource::New(VtValue(_fps)));
...
class _PrimvarValueDataSource final : public HdSampledDataSource {
 public:
  ...
  _PrimvarValueDataSource(const HdSampledDataSourceHandle &samplesSource,
 ...
 private:
  ...
  std::pair<Time, Time> _GetSamplingInterval(Time startTime, Time endTime) const;

  bool _HasVelocities() const;
  bool _HasAccelerations() const;

  bool _GetSamplesDeformBlur(Time startTime,
                             Time endTime,
                             float blurScale,
                             std::vector<Time> *outSampleTimes);

  bool _GetSamplesVelocityBlur(Time givenStartTime,
                               Time givenEndTime,
                               std::vector<Time> *outSampleTimes);
...
float _GetFps(const HdContainerDataSourceHandle &inputArgs)
{
  ...
  HdSampledDataSourceHandle const source = HdSampledDataSource::Cast(inputArgs->Get(_tokens->fps));
  if (!source) {
    return _fps;
  }
...
VtValue _PrimvarValueDataSource::_GetSourcePointsValue(const Time shutterOffset) const
{
  return _samplesSource->GetValue(shutterOffset);
}

VtValue _PrimvarValueDataSource::_GetSourcePrimvarValue(const HdDataSourceLocator &locator) const
{
  HdSampledDataSourceHandle const source = HdSampledDataSource::Cast(
...
VtValue _PrimvarValueDataSource::_GetSourceVelocitiesValue() const
{
  // Find velocities located on prim at primvars>velocities>primvarValue
  static const HdDataSourceLocator locator(HdTokens->velocities,
                                           HdPrimvarSchemaTokens->primvarValue);
  return _GetSourcePrimvarValue(locator);
}

VtValue _PrimvarValueDataSource::_GetSourceAccelerationsValue() const
{
  // Find velocities located on prim at primvars>velocities>primvarValue
  static const HdDataSourceLocator locator(HdTokens->accelerations,
                                           HdPrimvarSchemaTokens->primvarValue);
  return _GetSourcePrimvarValue(locator);
}
...
int _PrimvarValueDataSource::_GetSourceNonlinearSampleCount() const
{
  ...
  static const HdDataSourceLocator locator(HdTokens->nonlinearSampleCount,
                                           HdPrimvarSchemaTokens->primvarValue);
  const VtValue value = _GetSourcePrimvarValue(locator);
  return value.GetWithDefault<int>(_defaultNonlinearSampleCount);
}

float _PrimvarValueDataSource::_GetSourceBlurScale() const
{
  // Find velocities located on prim at primvars>blurScale>primvarValue
  static const HdDataSourceLocator locator(HdTokens->blurScale,
                                           HdPrimvarSchemaTokens->primvarValue);
  const VtValue value = _GetSourcePrimvarValue(locator);
  return std::fabs(value.GetWithDefault<float>(1.0f));
}

bool _PrimvarValueDataSource::_HasVelocities() const
{
...
bool _PrimvarValueDataSource::_HasAccelerations() const
{
...
VtValue _PrimvarValueDataSource::GetValue(const Time givenShutterOffset)
{
  if (!_samplesSource) {
    return VtValue();
  }
  ...
  const VtVec3fArray &velocitiesArray = velocitiesValue.UncheckedGet<VtVec3fArray>();
  ...
  if (velocitiesArray.size() != num) {
    TF_WARN(
        "Number %zu of velocity vectors does not match number %zu "
        "of points.",
        velocitiesArray.size(),
        num);
  ...
  const VtVec3fArray &accelerationsArray = accelerationsValue.IsHolding<VtVec3fArray>() ?
  ...
  if (!accelerationsArray.empty()) {
    ...
    if (!useAccelerations) {
      ...
      TF_WARN(
          "Number %zu of accleration vectors does not match "
          "number %zu of points - applying velocities but not "
          "accelerations.",
          accelerationsArray.size(),
          num);
    }
  }
  ...
  if (useAccelerations) {
    ...
    for (size_t i = 0; i < num; i++) {
      result[i] += pointsArray[i] + time * velocitiesArray[i] +
...
//
std::pair<HdSampledDataSource::Time, HdSampledDataSource::Time> _PrimvarValueDataSource::
    _GetSamplingInterval(const Time startTime, const Time endTime) const
{
  ...
  _samplesSource->GetContributingSampleTimesForInterval(startTime, endTime, &sampleTimes);
  ...
  const auto iteratorPair = std::minmax_element(sampleTimes.begin(), sampleTimes.end());
  return {*iteratorPair.first, *iteratorPair.second};
}

bool _PrimvarValueDataSource::GetContributingSampleTimesForInterval(
    const Time startTime, const Time endTime, std::vector<Time> *const outSampleTimes)
{
  if (!_samplesSource) {
    return false;
  }
  ...
  if (_HasVelocities()) {
    ...
    return _GetSamplesVelocityBlur(startTime, endTime, outSampleTimes);
  }
  else {
    return _GetSamplesDeformBlur(startTime, endTime, blurScale, outSampleTimes);
  }
}

bool _PrimvarValueDataSource::_GetSamplesDeformBlur(const Time startTime,
                                                    const Time endTime,
                                                    ...
                                                    std::vector<Time> *const outSampleTimes)
{
  ...
  if (!_samplesSource->GetContributingSampleTimesForInterval(
          blurScale * startTime, blurScale * endTime, outSampleTimes))
  {
    return false;
  }
...
bool _PrimvarValueDataSource::_GetSamplesVelocityBlur(Time givenStartTime,
                                                      Time givenEndTime,
                                                      std::vector<Time> *outSampleTimes)
{
  ...
  std::tie(startTime, endTime) = _GetSamplingInterval(givenStartTime, givenEndTime);
  ...
  const size_t nonlinearSampleCount = _GetSourceNonlinearSampleCount();
  ...
  for (size_t i = 0; i < nonlinearSampleCount; ++i) {
    ...
    // and last value.
    outSampleTimes->push_back((float(m - i) / m) * startTime + (float(i) / m) * endTime);
  }
...
class _PointsDataSource final : public HdContainerDataSource {
 public:
  ...
  /// primvarsSource: original source for locator primvars
  /// inputArgs: source from scene index plugin
  _PointsDataSource(const HdContainerDataSourceHandle &pointsSource,
                    const HdContainerDataSourceHandle &primvarsSource,
                    const HdContainerDataSourceHandle &inputArgs)
...
HdDataSourceBaseHandle _PointsDataSource::Get(const TfToken &name)
{
  if (!_pointsSource) {
    return nullptr;
  }
  ...
  if (name == HdPrimvarSchemaTokens->primvarValue) {
    ...
    if (HdSampledDataSourceHandle const primvarValueSource = HdSampledDataSource::Cast(result)) {
      return _PrimvarValueDataSource::New(primvarValueSource, _primvarsSource, _inputArgs);
    }
  }
...
class _PrimvarsDataSource final : public HdContainerDataSource {
 public:
  ...
  /// primvarsSource: original source for locator primvars
  /// inputArgs: source from scene index plugin
  _PrimvarsDataSource(const HdContainerDataSourceHandle &primvarsSource,
                      const HdContainerDataSourceHandle &inputArgs)
...
HdDataSourceBaseHandle _PrimvarsDataSource::Get(const TfToken &name)
{
  if (!_primvarsSource) {
    return nullptr;
  }
  ...
  if (name == HdPrimvarsSchemaTokens->points) {
    ...
    if (HdContainerDataSourceHandle const pointsSource = HdContainerDataSource::Cast(result)) {
      return _PointsDataSource::New(pointsSource, _primvarsSource, _inputArgs);
    }
  }
...
HdDataSourceBaseHandle _PrimDataSource::Get(const TfToken &name)
{
  if (!_primSource) {
    return nullptr;
  }
  ...
  if (name == HdPrimvarsSchemaTokens->primvars) {
    if (HdContainerDataSourceHandle const primvarsSource = HdContainerDataSource::Cast(result)) {
...
class _SceneIndex final : public HdSingleInputFilteringSceneIndexBase {
 public:
  static _SceneIndexRefPtr New(const HdSceneIndexBaseRefPtr &inputSceneIndex,
                               const HdContainerDataSourceHandle &inputArgs)
  {
    return TfCreateRefPtr(new _SceneIndex(inputSceneIndex, inputArgs));
  }

  HdSceneIndexPrim GetPrim(const SdfPath &primPath) const override
  {
    const HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(primPath);
    return {prim.primType, _PrimDataSource::New(prim.dataSource, _inputArgs)};
  }
...
HdDataSourceLocator _GetPrimvarValueLocator(const TfToken &name)
{
  return HdDataSourceLocator(
      HdPrimvarsSchemaTokens->primvars, name, HdPrimvarSchemaTokens->primvarValue);
}

void _SceneIndex::_PrimsDirtied(const HdSceneIndexBase &sender,
                                const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  static const HdDataSourceLocator pointsValueLocator = _GetPrimvarValueLocator(HdTokens->points);
...
HdPrman_VelocityMotionBlurSceneIndexPlugin::HdPrman_VelocityMotionBlurSceneIndexPlugin() = default;

HdSceneIndexBaseRefPtr HdPrman_VelocityMotionBlurSceneIndexPlugin::_AppendSceneIndex(
