...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         (PxrDisplace)(bxdf)(OSL)(omitFromRender)(material)(light)(PrimvarPass));
...
void HdPrmanMaterial::_ResetMaterial(HdPrman_RenderParam *renderParam)
{
                          ...
                          _PathSet *visitedNodes)
{
  ...
  for (const auto &param : node.parameters) {
    ...
    if (propType == SdrPropertyTypes->Struct || propType == SdrPropertyTypes->Vstruct) {
      ...
      RtUString v = HdPrman_ResolveAssetToRtUString(param.second.UncheckedGet<SdfAssetPath>(),
  ...
  if ((*dirtyBits & HdMaterial::DirtyResource) || (*dirtyBits & HdMaterial::DirtyParams)) {
    ...
    if (hdMatVal.IsEmpty()) {
      ...
      _ConvertHdMaterialNetwork2ToRman(
          sceneDelegate, param, id, _materialNetwork, &_materialId, &_displacementId);
    }
    else {
      TF_CODING_ERROR(
