...
void HdPrmanRenderDelegate::_Initialize()
{
  ...
  _settingDescriptors.push_back({std::string("Integrator"),
  ...
  if (TfGetEnvSetting(HD_PRMAN_ENABLE_QUICKINTEGRATE))
  {
    ...
    // to path tracing
    _settingDescriptors.push_back({std::string("Interactive Integrator Timeout (ms)"),
                                   HdPrmanRenderSettingsTokens->interactiveIntegratorTimeout,
                                   VtValue(200)});
  }

  _settingDescriptors.push_back({std::string("Max Samples"),
                                 HdRenderSettingsTokens->convergedSamplesPerPixel,
                                 VtValue(maxSamples)});

  _settingDescriptors.push_back({std::string("Variance Threshold"),
                                 HdRenderSettingsTokens->convergedVariance,
                                 VtValue(pixelVariance)});

  _settingDescriptors.push_back({std::string("Riley variant"),
                                 HdPrmanRenderSettingsTokens->rileyVariant,
                                 VtValue(TfGetenv("RILEY_VARIANT"))});

  _settingDescriptors.push_back({std::string("Disable motion blur"),
...
void HdPrmanRenderDelegate::SetRenderSetting(TfToken const &key,
                                             VtValue const &value)
{
