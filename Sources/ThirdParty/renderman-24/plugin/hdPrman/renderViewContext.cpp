...
void HdPrman_RenderViewContext::CreateRenderView(const HdPrman_RenderViewDesc &desc,
                                                 riley::Riley *const riley)
{
  ...
  for (const RenderOutputDesc &outputDesc : desc.renderOutputDescs) {
    const riley::FilterSize filterWidth = {outputDesc.filterWidth[0], outputDesc.filterWidth[1]};

    _renderOutputIds.push_back(riley->CreateRenderOutput(
  ...
  const riley::Extent rtResolution = {
      static_cast<uint32_t>(desc.resolution[0]), static_cast<uint32_t>(desc.resolution[1]), 1};
  ...
  _renderTargetId = riley->CreateRenderTarget(
      ...
      {static_cast<uint32_t>(_renderOutputIds.size()), _renderOutputIds.data()},
      rtResolution,
      rtWeighted,
      1.0f,
      RtParamList());
  ...
  for (const DisplayDesc &displayDesc : desc.displayDescs) {
    ...
    for (const size_t renderOutputIndex : displayDesc.renderOutputIndices) {
      displayRenderOutputIds.push_back(_renderOutputIds[renderOutputIndex]);
    }
    _displayIds.push_back(riley->CreateDisplay(
        riley::UserId(stats::AddDataLocation(displayDesc.name.CStr()).GetValue()),
        _renderTargetId,
        displayDesc.name,
        displayDesc.driver,
        {static_cast<uint32_t>(displayRenderOutputIds.size()), displayRenderOutputIds.data()},
        displayDesc.params));
  }

  _renderViewId = riley->CreateRenderView(
...
void HdPrman_RenderViewContext::_DestroyRenderView(riley::Riley *const riley)
{
...
void HdPrman_RenderViewContext::SetIntegratorId(const riley::IntegratorId id,
                                                riley::Riley *const riley)
{
  if (_renderViewId == riley::RenderViewId::InvalidId()) {
    return;
  ...
  riley->ModifyRenderView(_renderViewId, nullptr, nullptr, &id, nullptr, nullptr, nullptr);
}

void HdPrman_RenderViewContext::SetResolution(const GfVec2i &resolution, riley::Riley *const riley)
{
  if (_renderTargetId == riley::RenderTargetId::InvalidId()) {
    return;
  }
  ...
  riley->ModifyRenderTarget(_renderTargetId, nullptr, &extent, nullptr, nullptr, nullptr);
}
