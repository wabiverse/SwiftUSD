...
static void _AccumulateSampleTimes(const HdTimeSampleArray<T1, C> &in,
...
RtUString _FixupPrimvarName(const TfToken &name)
{
...
bool _SetPrimvarValue(const RtUString &name,
                      const VtValue &val,
                      const TfToken &role,
                      ...
                      RtParamList &dest)
{
  if (val.IsHolding<VtArray<float>>()) {
    ...
    if (isConstantRate) {
      dest.SetFloatArray(name, reinterpret_cast<const float *>(v.data()), 2 * v.size());
    }
    else {
      dest.SetFloatArray(name, reinterpret_cast<const float *>(v[instanceIndex].data()), 2);
    }
  }
  else if (val.IsHolding<GfVec2f>()) {
    const GfVec2f &v = val.UncheckedGet<GfVec2f>();
    dest.SetFloatArray(name, reinterpret_cast<const float *>(v.data()), 2);
  }
    ...
    if (isConstantRate) {
      ...
      if (role == HdPrimvarRoleTokens->color) {
        dest.SetColorArray(name, reinterpret_cast<const RtColorRGB *>(v.data()), v.size());
      }
      else if (role == HdPrimvarRoleTokens->point) {
        dest.SetPointArray(name, reinterpret_cast<const RtPoint3 *>(v.data()), v.size());
      }
      else if (role == HdPrimvarRoleTokens->normal) {
        dest.SetNormalArray(name, reinterpret_cast<const RtNormal3 *>(v.data()), v.size());
      }
      else if (role == HdPrimvarRoleTokens->vector) {
        dest.SetVectorArray(name, reinterpret_cast<const RtVector3 *>(v.data()), v.size());
      }
      else {
        dest.SetFloatArray(name, reinterpret_cast<const float *>(v.data()), 3 * v.size());
      }
    }
    else {
      ...
      if (role == HdPrimvarRoleTokens->color) {
        ...
        dest.SetFloatArray(name, reinterpret_cast<const float *>(v.data()), 3);
      }
    }
  }
    ...
    if (isConstantRate) {
      dest.SetFloatArray(name, reinterpret_cast<const float *>(v.data()), 4 * v.size());
    }
    else {
      dest.SetFloatArray(name, reinterpret_cast<const float *>(v[instanceIndex].data()), 4);
    }
  }
  else if (val.IsHolding<GfVec4f>()) {
    const GfVec4f &v = val.UncheckedGet<GfVec4f>();
    dest.SetFloatArray(name, reinterpret_cast<const float *>(v.data()), 4);
  }
    ...
    if (isConstantRate) {
      ...
      std::transform(v.begin(), v.end(), std::back_inserter(values), [](const GfMatrix4d &mat) {
        return HdPrman_GfMatrixToRtMatrix(mat);
      });
      dest.SetMatrixArray(name, values.data(), values.size());
    }
    else {
      dest.SetMatrix(name, HdPrman_GfMatrixToRtMatrix(v[instanceIndex]));
    }
  }
    ...
    if (isConstantRate) {
      ...
      values.reserve(v.size());
      std::transform(v.begin(), v.end(), std::back_inserter(values), [](const std::string &str) {
    ...
    if (isConstantRate) {
      VtArray<RtUString> values;
      values.reserve(v.size());
      std::transform(v.begin(), v.end(), std::back_inserter(values), [](const TfToken &tok) {
...
HdDirtyBits HdPrmanInstancer::GetInitialDirtyBitsMask() const
{
  return HdChangeTracker::DirtyTransform | HdChangeTracker::DirtyPrimvar |
         ...
         HdChangeTracker::DirtyCategories | HdChangeTracker::DirtyInstanceIndex;
};
...
void HdPrmanInstancer::Finalize(HdRenderParam *renderParam)
{
...
void HdPrmanInstancer::Populate(HdRenderParam *renderParam,
                                HdDirtyBits *dirtyBits,
...
void HdPrmanInstancer::_SyncCategories(HdDirtyBits *dirtyBits)
{
...
void HdPrmanInstancer::_SyncVisibility(HdDirtyBits *dirtyBits)
{
  HdSceneDelegate *delegate = GetDelegate();
  const SdfPath &id = GetId();
...
void HdPrmanInstancer::_PopulateInstances(
    HdRenderParam *renderParam,
    HdDirtyBits *dirtyBits,
    const SdfPath &hydraPrototypeId,
      ...
      for (size_t j = 0; j < rileyPrototypeIds.size(); ++j) {
        ...
        if (!parentInstancer) {
          ...
          param->ConvertCategoriesToAttributes(
              instancerId, {flats.categories.begin(), flats.categories.end()}, attrs);
        }

        _RileyInstanceId &instIds = _protoMap.get(prototypePrimPath).map[protoId][i];
            ...
            instIds.groupId != groupId)
        {
          ...
          riley->DeleteGeometryInstance(instIds.groupId, instIds.instanceId);
        ...
        if (instIds.instanceId == riley::GeometryInstanceId::InvalidId()) {
          ...
          riley::UserId userId = riley::UserId(stats::AddDataLocation(name.CStr()).GetValue());
          instIds.instanceId = riley->CreateGeometryInstance(
              userId, instIds.groupId, protoId, matId, coordSysList, rileyXform, attrs);
        }
        else if (*dirtyBits) {
          riley->ModifyGeometryInstance(
              instIds.groupId, instIds.instanceId, &matId, &coordSysList, &rileyXform, &attrs);
        }
      }
    }
...
bool HdPrmanInstancer::_RemoveDeadInstances(
    riley::Riley *riley,
    const SdfPath &prototypePrimPath,
...
void HdPrmanInstancer::_ComposePrototypeData(
    ...
    _GfMatrixSA &protoXform)
{
  ...
  auto SetProtoAttrs = [&](const SdfPath &protoPath, RtParamList &attrs, _FlattenData &flats) {
...
void HdPrmanInstancer::_ResizeProtoMap(
    riley::Riley *riley,
    const SdfPath &prototypePrimPath,
    const std::vector<riley::GeometryPrototypeId> &rileyPrototypeIds,
...
bool HdPrmanInstancer::_CleanDisusedGroupIds(HdPrman_RenderParam *param)
{
...
void HdPrmanInstancer::_GetInstancePrimvars(const size_t instanceIndex, RtParamList &attrs)
{
  for (auto entry : _primvarMap) {
    ...
    // win out.
    if (TfStringStartsWith(entry.first.GetString(), "ri:attributes:") && attrs.HasParam(name)) {
      continue;
    }

    if (!_SetPrimvarValue(name, val, primvar.role, isConstantRate, instanceIndex, attrs)) {
      TF_WARN(
          "Unrecognized primvar value type at %s.%s", GetId().GetText(), entry.first.GetText());
    }
  }
}

void HdPrmanInstancer::_GetPrototypePrimvars(const SdfPath &protoPath, RtParamList &attrs)
{
       ...
       i = HdInterpolation(i + 1))
  {
    for (const HdPrimvarDescriptor &primvar : delegate->GetPrimvarDescriptors(protoPath, i)) {
      ...
      if (TfStringStartsWith(primvar.name.GetString(), "ri:attributes") && attrs.HasParam(name)) {
        continue;
      }
...
/* static */
void HdPrmanInstancer::_MultiplyTransforms(const _GfMatrixSA &lhs,
