    ...
    for (size_t i = 0; i < instances.size(); ++i)
    {
      ...
      for (size_t j = 0; j < rileyPrototypeIds.size(); ++j)
      {
        ...
        _RileyInstanceId &instIds = _protoMap.get(prototypePrimPath)
                                        .map[protoId][i];
        if (instIds.instanceId != riley::GeometryInstanceId::InvalidId() && instIds.groupId != groupId)
        {
...
void HdPrmanInstancer::_GetInstancePrimvars(
    const size_t instanceIndex,
    RtParamList &attrs)
{
...
void HdPrmanInstancer::_GetPrototypePrimvars(
    const SdfPath &protoPath,
    RtParamList &attrs)
{
...
/* static */
void HdPrmanInstancer::_MultiplyTransforms(
