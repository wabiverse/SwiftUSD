...
namespace
{
  ...
  RtxHioImagePlugin::RtxHioImagePlugin(RixContext *rixCtx, const char *pluginName) : m_msgHandler(rixCtx ? (RixMessages *)rixCtx->GetRixInterface(k_RixMessages) : nullptr)
  {
  }
      ...
      RtxPlugin::FillRequest &fillReq)
  {
    ...
    const T *src = reinterpret_cast<const T *>(level.data) + (startY * level.width + startX) * numImageChannels + (fillReq.channelOffset);
  ...
  int RtxHioImagePlugin::Fill(TextureCtx &tCtx, FillRequest &fillReq)
  {
    ...
    else
    {
      ...
      const int startY = fillReq.tile.offset.Y * fillReq.tile.size.Y;
      const int endY = startY + fillReq.tile.size.Y;
      char *src = (char *)level.data + (startY * level.width + startX) * bytesPerImagePixel + (fillReq.channelOffset * bytesPerChannel);
