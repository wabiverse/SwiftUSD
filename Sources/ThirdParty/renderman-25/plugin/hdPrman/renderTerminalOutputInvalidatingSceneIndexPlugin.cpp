...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    ((sceneIndexPluginName, "HdPrman_RenderTerminalOutputInvalidatingSceneIndexPlugin"))(
        ...
        (outputsRiSampleFilters, "outputs:ri:sampleFilters"))((outputsRiDisplayFilters,
...
{
  HdSceneIndexPluginRegistry::Define<HdPrman_RenderTerminalOutputInvalidatingSceneIndexPlugin>();
}
...
VtArray<SdfPath> _GetConnectedOutputs(const HdSceneIndexPrim &prim)
{
  const HdContainerDataSourceHandle renderSettingsDs = HdContainerDataSource::Cast(
  ...
  HdContainerDataSourceHandle namespacedSettingsDS = rsSchema.GetNamespacedSettings();
  ...
  for (const auto &outputToken : outputTokens) {
    const HdSampledDataSourceHandle valueDs = HdSampledDataSource::Cast(
        namespacedSettingsDS->Get(outputToken));
    if (!valueDs) {
      continue;
    }
...
class _HdPrmanRenderTerminalOutputFnvalidatingSceneIndex
 ...
 public:
  static _HdPrmanRenderTerminalOutputFnvalidatingSceneIndexRefPtr New(
      const HdSceneIndexBaseRefPtr &inputSceneIndex)
  {
    return TfCreateRefPtr(new _HdPrmanRenderTerminalOutputFnvalidatingSceneIndex(inputSceneIndex));
  }
 ...
 protected:
  _HdPrmanRenderTerminalOutputFnvalidatingSceneIndex(const HdSceneIndexBaseRefPtr &inputSceneIndex)
    ...
    for (auto const &entry : entries) {
      if (entry.primType == HdPrimTypeTokens->renderSettings) {
        ...
        for (auto const &path : _GetConnectedOutputs(prim)) {
          const TfToken outputType = _GetInputSceneIndex()->GetPrim(path).primType;
          if (outputType == HdPrimTypeTokens->integrator) {
            outputsToDirty.emplace_back(path, HdIntegratorSchema::GetDefaultLocator());
          }
          else if (outputType == HdPrimTypeTokens->sampleFilter) {
            outputsToDirty.emplace_back(path, HdSampleFilterSchema::GetDefaultLocator());
          }
          else if (outputType == HdPrimTypeTokens->displayFilter) {
            outputsToDirty.emplace_back(path, HdDisplayFilterSchema::GetDefaultLocator());
          }
        }
      }
    }
                     ...
                     const HdSceneIndexObserver::DirtiedPrimEntries &entries) override
  {
    ...
    HdSceneIndexObserver::DirtiedPrimEntries outputsToDirty;
    for (auto const &entry : entries) {
      if (entry.dirtyLocators.Intersects(HdRenderSettingsSchema::GetNamespacedSettingsLocator())) {
        const HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(entry.primPath);
        for (auto const &path : _GetConnectedOutputs(prim)) {
...
HdSceneIndexBaseRefPtr HdPrman_RenderTerminalOutputInvalidatingSceneIndexPlugin::_AppendSceneIndex(
