...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    ((meshLightLightName, "__meshLight_light"))((meshLightSourceName, "__meshLight_sourceMesh"))(
...
{
  if ((prim.primType == HdPrimTypeTokens->mesh) || (prim.primType == HdPrimTypeTokens->volume)) {
...
bool _HasValidMaterialNetwork(const HdSceneIndexPrim &prim)
{
  HdMaterialSchema matSchema = HdMaterialSchema::GetFromParent(prim.dataSource);
  if (!matSchema.IsDefined()) {
    return false;
  }
  HdContainerDataSourceHandle matDS = matSchema.GetMaterialNetwork(_tokens->renderContext);
  if (!matDS) {
    return false;
  }
...
SdfPath _GetBoundMaterialPath(const HdContainerDataSourceHandle &primDS)
{
#if HD_API_VERSION >= 51
  HdMaterialBindingsSchema materialBindings = HdMaterialBindingsSchema::GetFromParent(primDS);
...
SdfPathVector _GetLightFilterPaths(const HdContainerDataSourceHandle &inputContainer)
{
  if (HdLightSchema lightSchema = HdLightSchema::GetFromParent(inputContainer)) {
...
HdContainerDataSourceHandle _BuildLightShaderDataSource(
    ...
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
  ...
  const TfToken terminalToken = originPrim.primType == HdPrimTypeTokens->volume ?
                                    HdMaterialTerminalTokens->volume :
                                    HdMaterialTerminalTokens->surface;
  const TfToken expectedShader = terminalToken == HdMaterialTerminalTokens->volume ?
                                     _tokens->PxrVolume :
                                     _tokens->PxrSurface;
  const TfToken glowParam = expectedShader == _tokens->PxrVolume ? _tokens->emitColor :
                                                                   _tokens->glowColor;
  ...
  if (_GetMaterialSyncMode(originPrim.dataSource) != UsdLuxTokens->materialGlowTintsLight) {
  ...
  const HdContainerDataSourceHandle &matDS = HdMaterialSchema::GetFromParent(matPrim.dataSource)
                                                 .GetMaterialNetwork(_tokens->renderContext);
  if (!matDS) {
    // could not get material shader network from material prim;
    // return unmodified
    TF_DEBUG(HDPRMAN_MESHLIGHT)
        .Msg(
            "Could not get material shader network "
            "from material prim; shader for %s light <%s> will not be "
            "modified\n",
            originPrim.primType.GetText(),
            originPath.GetText());
    return originalShaderDS;
  }
  ...
  const TfToken &nodeType = srcMatNI.GetNodeType(terminalConn.second.upstreamNodeName);

  if (nodeType != expectedShader) {
    // unsupported node type; return unmodified
    TF_DEBUG(HDPRMAN_MESHLIGHT)
        .Msg(
            "%s terminal upstream node is not "
            "%s; shader for %s light <%s> will not be modified\n",
            terminalToken.GetText(),
            expectedShader.GetText(),
            originPrim.primType.GetText(),
            originPath.GetText());
    return originalShaderDS;
  }
  ...
  const auto lightTC = shaderNI.GetTerminalConnection(HdMaterialTerminalTokens->light);

  // try for material's glow input connection
  const auto glowIC = srcMatNI.GetNodeInputConnection(terminalConn.second.upstreamNodeName,
                                                      glowParam);
  if (!glowIC.empty()) {
    // glow input connection exists; set as textureColor on
    // light terminal's upstream node
    shaderNI.SetNodeInputConnection(
        lightTC.second.upstreamNodeName, _tokens->textureColor, glowIC);
  ...
  const VtValue glowIV = srcMatNI.GetNodeParameterValue(terminalConn.second.upstreamNodeName,
                                                        glowParam);
  if (glowIV.IsHolding<GfVec3f>()) {
    // glow param value exists; set as textureColor on
    // light terminal's upstream node
    shaderNI.SetNodeParameterValue(lightTC.second.upstreamNodeName, _tokens->textureColor, glowIV);
...
HdContainerDataSourceHandle _BuildLightDependenciesDataSource(
    const SdfPath &originPath,
    ...
    const HdContainerDataSourceHandle &bindingSourceDS)
{
  ...
  sources.push_back(
      HdDependencySchema::Builder()
          .SetDependedOnPrimPath(HdRetainedTypedSampledDataSource<SdfPath>::New(bindingSourcePath))
  ...
  for (const SdfPath &filterPath : _GetLightFilterPaths(originDS)) {
    ...
    sources.push_back(
        HdDependencySchema::Builder()
            .SetDependedOnPrimPath(HdRetainedTypedSampledDataSource<SdfPath>::New(filterPath))
    ...
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
  ...
  if (materialSyncMode == UsdLuxTokens->materialGlowTintsLight) {
    ...
    sources.push_back(HdRetainedContainerDataSource::New(
        ...
        _BuildLightShaderDataSource(originPath, originPrim, bindingSourceDS, inputSceneIndex)));
  }
  ...
  sources.push_back(_BuildLightDependenciesDataSource(
      originPath, originPrim.dataSource, bindingSourcePath, bindingSourceDS));
...
HdContainerDataSourceHandle _BuildSourceDependenciesDataSource(const SdfPath originPath)
{
  // Data source locators
...
HdContainerDataSourceHandle _BuildSourceDataSource(const SdfPath &originPath,
                                                   const HdContainerDataSourceHandle &originDS)
{
  std::vector<TfToken> names;
  std::vector<HdDataSourceBaseHandle> sources;
  ...
  return HdOverlayContainerDataSource::New(
      HdRetainedContainerDataSource::New(names.size(), names.data(), sources.data()), originDS);
}

HdContainerDataSourceHandle _BuildMeshDependenciesDataSource(const SdfPath originPath)
{
  // Data source locators
...
/* static */
HdPrmanMeshLightResolvingSceneIndexRefPtr HdPrmanMeshLightResolvingSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
  return TfCreateRefPtr(new HdPrmanMeshLightResolvingSceneIndex(inputSceneIndex));
}
...
HdSceneIndexPrim HdPrmanMeshLightResolvingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  // The origin prim -> no primType (should only be visible to HSB)
  if (_meshLights.count(primPath)) {
    return {TfToken(), _GetInputSceneIndex()->GetPrim(primPath).dataSource};
  }
  ...
  if (_meshLights.count(parentPath) > 0) {
    ...
    if (primPath.GetNameToken() == _tokens->meshLightMeshName && _meshLights.at(parentPath)) {
      return {parentPrim.primType,
              HdOverlayContainerDataSource::New(
                  HdRetainedContainerDataSource::New(HdLightSchemaTokens->light,
    ...
    if (primPath.GetNameToken() == _tokens->meshLightLightName) {
      return {HdPrimTypeTokens->meshLight,
              _BuildLightDataSource(parentPath,
                                    parentPrim,
                                    parentPath,
                                    parentPrim.dataSource,  // materialBinding source
                                    _GetInputSceneIndex())};
    }

    // The source mesh -> "meshLightSourceMesh" or "meshLightSourceVolume"
    if (primPath.GetNameToken() == _tokens->meshLightSourceName) {
      return {parentPrim.primType == HdPrimTypeTokens->volume ?
...
SdfPathVector HdPrmanMeshLightResolvingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
    ...
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (const auto &entry : entries) {
    if ((entry.primType == HdPrimTypeTokens->mesh) || (entry.primType == HdPrimTypeTokens->volume))
    {
      ...
      if (_IsMeshLight(prim) && _HasValidMaterialNetwork(prim)) {
        ...
        // The light prim
        added.emplace_back(entry.primPath.AppendChild(_tokens->meshLightLightName),
                           HdPrimTypeTokens->meshLight);

        // The source mesh (for the light prim)

        added.emplace_back(entry.primPath.AppendChild(_tokens->meshLightSourceName),
        ...
        if (meshVisible) {
          added.emplace_back(entry.primPath.AppendChild(_tokens->meshLightMeshName),
                             entry.primType);
        }
    ...
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  ...
  for (const auto &entry : entries) {
    if (_meshLights.count(entry.primPath)) {
      ...
      // The light prim
      removed.emplace_back(entry.primPath.AppendChild(_tokens->meshLightLightName));

      // The source mesh (for the light prim)
      removed.emplace_back(entry.primPath.AppendChild(_tokens->meshLightSourceName));

      // The stripped-down origin prim
      if (_meshLights.at(entry.primPath)) {
        removed.emplace_back(entry.primPath.AppendChild(_tokens->meshLightMeshName));
      }
    ...
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  static const HdDataSourceLocator materialSyncModeLocator = {HdLightSchemaTokens->light,
  ...
  for (const auto &entry : entries) {
    if (_meshLights.count(entry.primPath) && entry.dirtyLocators.Contains(materialSyncModeLocator))
    {
      ...
      if (visible && (!_meshLights.at(entry.primPath))) {
        ...
        _SendPrimsAdded({{entry.primPath.AppendChild(_tokens->meshLightMeshName), prim.primType}});
      }
