...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    (PxrDisplace)(bxdf)(OSL)(omitFromRender)(material)(surface)((materialid, "__materialid"))(light)(PrimvarPass));
...
HdPrmanMaterial::HdPrmanMaterial(SdfPath const &id)
    : HdMaterial(id), _materialId(riley::MaterialId::InvalidId()), _displacementId(riley::DisplacementId::InvalidId()), _rileyIsInSync(false)
{
  /* NOTHING */
}
    ...
    bool elideDefaults)
{
  ...
  if ((node.nodeTypeId == _tokens->PxrDisplace) && node.parameters.empty() && node.inputConnections.empty())
  {
    return true;
  }
...
void HdPrmanMaterial::SyncToRiley(
    HdSceneDelegate *sceneDelegate,
    riley::Riley *riley)
{
  {
...
void HdPrmanMaterial::_SyncToRileyWithLock(
    HdSceneDelegate *sceneDelegate,
    riley::Riley *riley)
{
