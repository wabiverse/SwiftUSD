...
TF_DEFINE_ENV_SETTING(HD_PRMAN_MATERIALID,
                      true,
                      "Enable __materialid as hash of material network");
static bool _enableMaterialID = TfGetEnvSetting(HD_PRMAN_MATERIALID);

TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         (PxrDisplace)(bxdf)(OSL)(omitFromRender)(material)(surface)((
                             materialid, "__materialid"))(light)(PrimvarPass));
...
HdMaterialNetwork2 const &HdPrmanMaterial::GetMaterialNetwork() const
{
...
void HdPrmanMaterial::_ResetMaterialWithLock(riley::Riley *riley)
{
...
static bool _GetStringAsBool(std::string value, bool defaultValue)
{
  if (value.empty()) {
    return defaultValue;
  }
                          ...
                          bool elideDefaults)
{
  ...
  for (auto const &connEntry : node.inputConnections) {
    for (auto const &e : connEntry.second) {
      ...
      // before
      _ConvertNodes(network, e.upstreamNode, result, visitedNodes, elideDefaults);
    }
  }
  ...
  if ((node.nodeTypeId == _tokens->PxrDisplace) && node.parameters.empty() &&
  ...
  for (const auto &param : node.parameters) {
    ...
    if (propType == SdrPropertyTypes->Struct || propType == SdrPropertyTypes->Vstruct) {
      ...
      RtUString v = HdPrman_Utils::ResolveAssetToRtUString(
          ...
          isLight ? _tokens->light.GetText() : _tokens->material.GetText());
                                                  ...
                                                  std::vector<riley::ShadingNode> *result)
{
  ...
  _PathSet visitedNodes;
  return _ConvertNodes(network, nodePath, result, &visitedNodes, elideDefaults);
}
...
static void _ConvertHdMaterialNetwork2ToRman(HdSceneDelegate *sceneDelegate,
                                             riley::Riley *riley,
                                             SdfPath const &id,
  ...
  for (auto const &terminal : network.terminals) {
            ...
            network, terminal.second.upstreamNode, &nodes))
    {
      ...
      if (_enableMaterialID) {
        ...
        nodes.back().params.SetString(materialid, RtUString(TfStringify(networkHash).c_str()));
      }
        ...
        if (*displacementId == riley::DisplacementId::InvalidId()) {
          ...
          *displacementId = riley->CreateDisplacement(
          ...
          riley::ShadingNetwork const displacement = {static_cast<uint32_t>(nodes.size()),
                                                      &nodes[0]};
          riley::DisplacementResult const result = riley->ModifyDisplacement(
          ...
          if (result == riley::DisplacementResult::k_ResendPrimVars) {
            ...
            for (auto rprimid : index.GetRprimIds()) {
              ...
              if (rprim->GetMaterialId() == id) {
                changeTracker.MarkRprimDirty(rprimid, HdChangeTracker::DirtyPrimvar);
              }
            }
          }
        }
...
void HdPrmanMaterial::SyncToRiley(HdSceneDelegate *sceneDelegate, riley::Riley *riley)
{
  {
...
void HdPrmanMaterial::_SyncToRileyWithLock(HdSceneDelegate *sceneDelegate, riley::Riley *riley)
{
  ...
  if (hdMatVal.IsHolding<HdMaterialNetworkMap>()) {
    ...
    _ConvertHdMaterialNetwork2ToRman(
        sceneDelegate, riley, id, _materialNetwork, &_materialId, &_displacementId);
  }
  else {
    TF_CODING_ERROR(
...
HdMaterialNetwork2 HdPrmanMaterial_GetFallbackSurfaceMaterialNetwork()
{
  ...
  const std::map<SdfPath, HdMaterialNode2> nodes = {
      {
          ...
          HdMaterialNode2{
              ...
              // parameters
              {
                  {TfToken("varname"), VtValue(TfToken("displayColor"))},
                  {TfToken("defaultColor"), VtValue(GfVec3f(0.5, 0.5, 0.5))},
                  {TfToken("type"), VtValue(TfToken("color"))},
              },
          },
      },
      {
          ...
          HdMaterialNode2{
              ...
              // parameters
              {
                  {TfToken("varname"), VtValue(TfToken("displayRoughness"))},
                  {TfToken("defaultFloat"), VtValue(1.0f)},
                  {TfToken("type"), VtValue(TfToken("float"))},
              },
          },
      },
      {
          ...
          HdMaterialNode2{
              ...
              // parameters
              {
                  {TfToken("varname"), VtValue(TfToken("displayOpacity"))},
                  {TfToken("defaultFloat"), VtValue(1.0f)},
                  {TfToken("type"), VtValue(TfToken("float"))},
              },
          },
      },
      {
          ...
          HdMaterialNode2{
              ...
              // parameters
              {
                  {TfToken("varname"), VtValue(TfToken("displayMetallic"))},
                  {TfToken("defaultFloat"), VtValue(0.0f)},
                  {TfToken("type"), VtValue(TfToken("float"))},
              },
          },
      },
          ...
          HdMaterialNode2{
              ...
              // connections
              {
                  {TfToken("diffuseColor"),
                   {{SdfPath("/Primvar_displayColor"), TfToken("resultRGB")}}},
                  {TfToken("roughness"),
                   {{SdfPath("/Primvar_displayRoughness"), TfToken("resultF")}}},
                  {TfToken("metallic"),
                   {{SdfPath("/Primvar_displayMetallic"), TfToken("resultF")}}},
                  {TfToken("opacity"), {{SdfPath("/Primvar_displayOpacity"), TfToken("resultF")}}},
              },
          },
      },
      // PxrSurface (connected to UsdPreviewSurfaceParameters)
      {
          ...
          HdMaterialNode2{
              ...
              // parameters
              {
                  {TfToken("specularModelType"), VtValue(int(1))},
                  {TfToken("diffuseDoubleSided"), VtValue(int(1))},
                  {TfToken("specularDoubleSided"), VtValue(int(1))},
                  {TfToken("specularFaceColor"), VtValue(GfVec3f(0.04))},
                  {TfToken("specularEdgeColor"), VtValue(GfVec3f(1.0))},
              },
              // connections
              {
                  {TfToken("diffuseColor"),
                   {{SdfPath("/UsdPreviewSurfaceParameters"), TfToken("diffuseColorOut")}}},
                  {TfToken("diffuseGain"),
                   {{SdfPath("/UsdPreviewSurfaceParameters"), TfToken("diffuseGainOut")}}},
                  {TfToken("specularFaceColor"),
                   {{SdfPath("/UsdPreviewSurfaceParameters"), TfToken("specularFaceColorOut")}}},
                  {TfToken("specularEdgeColor"),
                   {{SdfPath("/UsdPreviewSurfaceParameters"), TfToken("specularEdgeColorOut")}}},
                  {TfToken("specularRoughness"),
                   {{SdfPath("/UsdPreviewSurfaceParameters"), TfToken("specularRoughnessOut")}}},
                  {TfToken("presence"),
                   {{SdfPath("/Primvar_displayOpacity"), TfToken("resultF")}}},
              },
          },
      },
  };
  ...
  const std::map<TfToken, HdMaterialConnection2> terminals = {
      {TfToken("surface"), HdMaterialConnection2{SdfPath("/PxrSurface"), TfToken("outputName")}},
  };
