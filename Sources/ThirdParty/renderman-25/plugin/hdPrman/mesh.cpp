...
TF_DEFINE_PRIVATE_TOKENS(_tokens, ((displacementBoundSphere, "displacementbound:sphere")));

HdPrman_Mesh::HdPrman_Mesh(SdfPath const &id, const bool isMeshLight)
    : BASE(id), _isMeshLight(isMeshLight)
{
}

bool HdPrman_Mesh::_PrototypeOnly()
{
  return _isMeshLight;
}
...
static VtIntArray _Union(const VtIntArray &a, const VtIntArray &b)
{
  if (a.empty()) {
    ...
    VtIntArray merged;
    std::set_union(
        aCopy.cbegin(), aCopy.cend(), bCopy.cbegin(), bCopy.cend(), std::back_inserter(merged));
    return merged;
  }
}

RtPrimVarList HdPrman_Mesh::_ConvertGeometry(HdPrman_RenderParam *renderParam,
  ...
  *geomSubsets = topology.GetGeomSubsets();
  ...
  if (*primType == RixStr.k_Ri_SubdivisionMesh) {
    ...
    if (!creaseIndices.empty()) {
      const bool weightPerCrease = creaseWeights.size() == creaseLengths.size();
