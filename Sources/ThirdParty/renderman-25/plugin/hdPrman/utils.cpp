...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (primvar));
...
struct _VtValueToRtParamList {
  ...
  {
    return params->SetFloatArray(name, reinterpret_cast<const float *>(v.cdata()), 2 * v.size());
  }
  ...
  bool operator()(const VtArray<GfVec3f> &v)
  {
    if (role == HdPrimvarRoleTokens->color) {
      ...
      return params->SetPointArray(name, reinterpret_cast<const RtPoint3 *>(v.cdata()), v.size());
    }
    else if (role == HdPrimvarRoleTokens->normal) {
      ...
      return params->SetFloatArray(name, reinterpret_cast<const float *>(v.cdata()), 3 * v.size());
    }
  }
  ...
  {
    return params->SetFloatArray(name, reinterpret_cast<const float *>(v.cdata()), 4 * v.size());
  }
  ...
  bool operator()(const VtArray<SdfAssetPath> &v)
  {
    ...
    for (SdfAssetPath const &asset : v) {
      us.push_back(
          HdPrman_Utils::ResolveAssetToRtUString(asset, flipTexture, _tokens->primvar.GetText()));
    }
    return (*this)(us);
  }
...
struct _VtValueToRtPrimVar : _VtValueToRtParamList {
  ...
  _VtValueToRtPrimVar(RtUString const &name_,
      ...
      : _VtValueToRtParamList{name_, role_, primvars_}, detail(detail_), primvars(primvars_)
  {
  }
  ...
  bool operator()(const VtArray<GfVec2f> &v)
  {
    return primvars->SetFloatArrayDetail(
        name, reinterpret_cast<const float *>(v.cdata()), 2, detail);
  }
  bool operator()(const VtArray<GfVec2d> &vd)
  {
    // Convert double->float
  ...
  bool operator()(const VtArray<GfVec3f> &v)
  {
    if (role == HdPrimvarRoleTokens->color) {
      ...
      return primvars->SetPointDetail(name, reinterpret_cast<const RtPoint3 *>(v.cdata()), detail);
    }
    else if (role == HdPrimvarRoleTokens->normal) {
...
bool _IsNativeRenderManFormat(std::string const &path)
{
...
bool SetPrimVarFromVtValue(RtUString const &name,
                           VtValue const &val,
...
RtUString ResolveAssetToRtUString(SdfAssetPath const &asset,
                                  bool flipTexture,
                                  char const *debugNodeType /* = nullptr*/)
{
  ...
  else if (!_IsNativeRenderManFormat(v) && imageRegistry.IsSupportedImageFile(v)) {
...
RtParamList PruneDeprecatedOptions(const RtParamList &options)
{
