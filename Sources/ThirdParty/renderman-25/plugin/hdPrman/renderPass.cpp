...
HdPrman_RenderPass::HdPrman_RenderPass(
    ...
    : HdRenderPass(index, collection), _renderParam(renderParam), _converged(false), _lastRenderedVersion(0), _lastTaskRenderTagsVersion(0), _lastRprimRenderTagVersion(0), _lastRenderSettingsPrimVersion(0), _quickIntegrateTime(0.2f)
{
...
void HdPrman_RenderPass::_Execute(
    ...
    TfTokenVector const &renderTags)
{
  ...
  const HdPrman_RenderSettings *renderSettingsPrim = nullptr;
  {
    ...
    legacySettingsChanged = _renderParam->GetLastLegacySettingsVersion() != currentLegacySettingsVersion;
  ...
  // Commit settings options to riley
  {
    ...
    if (updateLegacyOptions)
    {
      ...
      TF_DEBUG(HDPRMAN_RENDER_SETTINGS).Msg("Setting options from legacy data flow: \n%s", HdPrmanDebugUtil::RtParamListToString(prunedOptions).c_str());
    }
        ...
        (updateLegacyOptions || primSettingsChanged))
    {
      ...
      TF_DEBUG(HDPRMAN_RENDER_SETTINGS).Msg("Setting options from render settings prim %s:\n %s", renderSettingsPrim->GetId().GetText(), HdPrmanDebugUtil::RtParamListToString(prunedOptions).c_str());
    }
  }
...
void HdPrman_RenderPass::_RestartRenderIfNecessary(
