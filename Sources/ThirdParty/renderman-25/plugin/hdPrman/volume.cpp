                        ...
                        RtPrimVarList *primvars)
{
  ...
  if (TfIsFile(volumeAssetPath)) {
    ...
    if (!gridVecPtr) {
      TF_WARN("Failed to retrieve VDB grids from %s.", volumeAssetPath.c_str());
      return;
    }
    ...
    RixStorage *storage = static_cast<RixStorage *>(context->GetRixInterface(k_RixGlobalData));
    if (!storage) {
    ...
    storage->Lock();
    storage->Set(RtUString(key.c_str()),
                 grids,
  ...
  const std::array<RtUString, 4> sa = {RtUString(vdbSource.c_str()),
...
RtPrimVarList HdPrman_Volume::_ConvertGeometry(HdPrman_RenderParam *renderParam,
