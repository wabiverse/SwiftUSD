...
HdPrman_SampleFilter::HdPrman_SampleFilter(SdfPath const &id) : HdSprim(id) {}

void HdPrman_SampleFilter::Finalize(HdRenderParam *renderParam) {}

void HdPrman_SampleFilter::_CreateRmanSampleFilter(HdSceneDelegate *sceneDelegate,
                                                   ...
                                                   HdMaterialNode2 const &sampleFilterNode)
{
  ...
  for (const auto &param : sampleFilterNode.parameters) {
    ...
    HdPrman_Utils::SetParamFromVtValue(RtUString(prop->GetImplementationName().c_str()),
                                       param.second,
                                       prop->GetType(),
                                       &rileyNode.params);
  }
...
void HdPrman_SampleFilter::Sync(HdSceneDelegate *sceneDelegate,
                                ...
                                HdDirtyBits *dirtyBits)
{
  ...
  if (*dirtyBits & HdChangeTracker::DirtyParams) {
    ...
    {
      const VtValue sampleFilterResourceValue = sceneDelegate->Get(id, _tokens->resource);
      ...
      if (sampleFilterResourceValue.IsHolding<HdMaterialNode2>()) {
        ...
        _CreateRmanSampleFilter(sceneDelegate, param, id, sampleFilterNode);
      }
    }
  }
