...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (ND_standard_surface_surfaceshader)(ND_UsdPreviewSurface_surfaceshader)(ND_displacement_float)(ND_image_vector2)(ND_image_vector3)(ND_image_vector4)

    // MaterialX - OSL Adapter Node names
    ((SS_Adapter, "StandardSurfaceParameters"))((USD_Adapter, "UsdPreviewSurfaceParameters"))(
        (Displacement_Adapter, "DisplacementParameters"))

    // HdPrman Terminal Nodes
    (PxrSurface)(PxrDisplace)

    // Texture Coordinate Tokens
    (ND_geompropvalue_vector2)(ND_remap_vector2)(texcoord)(geomprop)(geompropvalue)(in)(inhigh)(inlow)(remap)(vector2)(float2)((
        string_type, "string"))
    ...
    // Wrap Modes
    (black)(clamp)(repeat)(uaddressmode)(vaddressmode)

    // Color Space
    ((cs_raw, "raw"))((cs_auto, "auto")));

static bool _FindGraphAndNodeByName(mx::DocumentPtr const &mxDoc,
                                           ...
                                           std::string const &mxNodeGraphName)
{
  ...
  _FindGraphAndNodeByName(mxDoc, mxNodeGraphName, mxNodeName, &mxNodeGraph, &mxNode);

  if (!mxNodeGraph) {
    TF_WARN("NodeGraph '%s' not found in the mxDoc.", mxNodeGraphName.c_str());
    return mx::EMPTY_STRING;
  }

  if (!mxNode) {
    TF_WARN("Node '%s' not found in '%s' nodeGraph.", mxNodeName.c_str(), mxNodeGraphName.c_str());
    return mx::EMPTY_STRING;
  }
...
static TfToken _GetTerminalShaderType(TfToken const &hdNodeType)
{
  return (hdNodeType == _tokens->ND_displacement_float) ? _tokens->PxrDisplace :
                                                          _tokens->PxrSurface;
}

// Convert terminal MaterialX shader type to corresponding connection name
static TfToken _GetTerminalConnectionName(TfToken const &hdNodeType)
{
  return (hdNodeType == _tokens->ND_displacement_float) ? HdMaterialTerminalTokens->displacement :
...
static bool _HasNode(HdMaterialNetworkInterface *netInterface, TfToken const &nodeName)
{
  return !netInterface->GetNodeType(nodeName).IsEmpty();
}

static void _GatherNodeGraphNodes(HdMaterialNetworkInterface *netInterface,
                                  TfToken const &hdNodeName,
                                  std::set<TfToken> *upstreamNodeNames,
                                  std::set<TfToken> *visitedNodeNames)
{
  TfTokenVector cNames = netInterface->GetNodeInputConnectionNames(hdNodeName);

  // Traverse the upsteam connections to gather the nodeGraph nodes
  for (TfToken const &cName : cNames) {
    auto inputConnections = netInterface->GetNodeInputConnection(hdNodeName, cName);
    ...
    for (auto const &currConnection : inputConnections) {
      ...
      if (!_HasNode(netInterface, upstreamNodeName)) {
        TF_WARN("Unknown material node '%s'", upstreamNodeName.GetText());
        continue;
      }
      ...
      _GatherNodeGraphNodes(netInterface, upstreamNodeName, upstreamNodeNames, visitedNodeNames);
...
static std::string _CompileOslSource(std::string const &name,
...
#  endif
  for (mx::FilePath const &path : searchPaths) {
    ...
    oslArgs.push_back(fullPath.exists() ? "-I" + fullPath.asString() : "-I" + path.asString());
  }
...
#  endif
  ...
  if (oslCompiledSource.empty()) {
    TF_WARN(
        "Unable to compile MaterialX Osl shader for the '%s' "
        "MaterialX node\n",
...
static void _DeleteAllInputConnections(HdMaterialNetworkInterface *netInterface,
...
static void _DeleteAllParameters(HdMaterialNetworkInterface *netInterface, TfToken const &nodeName)
{
  TfTokenVector pNames = netInterface->GetAuthoredNodeParameterNames(nodeName);
...
static void _UpdateNetwork(HdMaterialNetworkInterface *netInterface,
                           ...
                           std::set<TfToken> nodesToRemove)
{
  ...
  TfTokenVector terminalConnectionNames = netInterface->GetNodeInputConnectionNames(
      terminalNodeName);

  for (TfToken const &cName : terminalConnectionNames) {
    auto inputConnections = netInterface->GetNodeInputConnection(terminalNodeName, cName);
    ...
    for (auto const &currConnection : inputConnections) {
      ...
      if (!newNode) {
        ...
        if (sdrNode->GetOutput(outputName)) {
          netInterface->SetNodeInputConnection(
              terminalNodeName, inputName, {{upstreamNodeName, outputName}});
        }
        else {
      ...
      _GatherNodeGraphNodes(netInterface, upstreamNodeName, &nodesToRemove, &visitedNodeNames);
      ...
      std::string const &mxNodeGraphName = nodePath.GetParentPath().GetName();
      ...
      std::string compiledShaderPath = _CompileOslSource(shaderName, oslSource, searchPath);
      ...
      netInterface->SetNodeType(upstreamNodeName, sdrNode->GetIdentifier());
      ...
      if (sdrNode->GetOutput(outputName)) {
        ...
        if (deletePreviousConnection) {
          netInterface->DeleteNodeInputConnection(terminalNodeName, cName);
        }
      }
...
static void _TransformTerminalNode(HdMaterialNetworkInterface *netInterface,
                                   TfToken const &terminalNodeName)
{
  ...
  SdrShaderNodeConstPtr const sdrAdapter = sdrRegistry.GetShaderNodeByIdentifier(adapterType,
                                                                                 {_tokens->OSL});
  SdrShaderNodeConstPtr const sdrShader = sdrRegistry.GetShaderNodeByIdentifier(
      shaderType, {_tokens->RmanCpp});
  if (!sdrAdapter) {
  ...
  if (adapterType != _tokens->USD_Adapter) {
    ...
    TfTokenVector pNames = netInterface->GetAuthoredNodeParameterNames(terminalNodeName);
    for (TfToken const &pName : pNames) {
      TfToken updatedName = _GetUpdatedInputToken(pName);
      if (!updatedName.IsEmpty()) {
        VtValue val = netInterface->GetNodeParameterValue(terminalNodeName, pName);
        netInterface->SetNodeParameterValue(terminalNodeName, updatedName, val);
  ...
  TfToken rmanShaderNodeName = TfToken(terminalNodeName.GetString() + "_" +
                                       shaderType.GetString());
  ...
  for (const auto &inParamName : sdrShader->GetInputNames()) {
    ...
    if (sdrShader->GetShaderInput(inParamName)) {
      ...
      if (sdrAdapter->GetShaderOutput(adapterOutParam)) {
        netInterface->SetNodeInputConnection(
            rmanShaderNodeName, inParamName, {{terminalNodeName, adapterOutParam}});
      }
    }
  }
  ...
  netInterface->SetTerminalConnection(terminalToken, {rmanShaderNodeName, TfToken()});
}

// Get the Hydra equivalent for the given MaterialX input value
static TfToken _GetHdWrapString(TfToken const &hdTextureNodeName, std::string const &mxInputValue)
{
...
static void _GetWrapModes(HdMaterialNetworkInterface *netInterface,
                          ...
                          TfToken *vWrap)
{
  ...
  *vWrap = _tokens->repeat;
  ...
  // For <image> nodes:
  VtValue vUAddrMode = netInterface->GetNodeParameterValue(hdTextureNodeName,
                                                           _tokens->uaddressmode);
  if (!vUAddrMode.IsEmpty()) {
    *uWrap = _GetHdWrapString(hdTextureNodeName, vUAddrMode.UncheckedGet<std::string>());
  }
  VtValue vVAddrMode = netInterface->GetNodeParameterValue(hdTextureNodeName,
                                                           _tokens->vaddressmode);
  if (!vVAddrMode.IsEmpty()) {
    *vWrap = _GetHdWrapString(hdTextureNodeName, vVAddrMode.UncheckedGet<std::string>());
  }
}

static void _GetColorSpace(HdMaterialNetworkInterface *netInterface,
                           TfToken const &hdTextureNodeName,
                           TfToken *colorSpace)
{
  const TfToken nodeType = netInterface->GetNodeType(hdTextureNodeName);
  if (nodeType == _tokens->ND_image_vector2 || nodeType == _tokens->ND_image_vector3 ||
...
static void _UpdateTextureNodes(HdMaterialNetworkInterface *netInterface,
                                ...
                                mx::DocumentPtr const &mxDoc)
{
  for (SdfPath const &texturePath : hdTextureNodePaths) {
    ...
    if (nodeType.IsEmpty()) {
      TF_WARN("Connot find texture node '%s' in material network.", textureNodeName.GetText());
      continue;
    }

    VtValue vFile = netInterface->GetNodeParameterValue(textureNodeName, _tokens->file);
    if (vFile.IsEmpty()) {
      TF_WARN("File path missing for texture node '%s'.", textureNodeName.GetText());
      continue;
    }
    ...
    if (vFile.IsHolding<SdfAssetPath>()) {
      ...
      if (!ext.empty() && ext != "tex") {
        ...
        const std::string pluginName = std::string("RtxHioImage") + ARCH_LIBRARY_SUFFIX;
      ...
      mx::InputPtr texcoordInput = mxTextureNode->getInput(_tokens->texcoord);
      if (!texcoordInput) {
        texcoordInput = mxTextureNode->addInput(_tokens->texcoord, _tokens->vector2);
        const std::string stNodeName = textureNodeName.GetString() + "__texcoord";
        ...
        geompropNode->setInputValue(_tokens->geomprop, primvarName, _tokens->string_type);
        texcoordInput->setConnectedNode(geompropNode);
        geompropNode->setNodeDefString(_tokens->ND_geompropvalue_vector2);
      }
      if (needInvertT) {
        texcoordInput = mxTextureNode->getInput(_tokens->texcoord);
        if (texcoordInput) {
          const std::string remapNodeName = textureNodeName.GetString() + "__remap";
          mx::NodePtr remapNode = mxNodeGraph->addNode(
              _tokens->remap, remapNodeName, _tokens->vector2);
          remapNode->setNodeDefString(_tokens->ND_remap_vector2);
          mx::InputPtr inInput = remapNode->addInput(_tokens->in, _tokens->vector2);
          const mx::FloatVec inhigh = {1, 0};
          const mx::FloatVec inlow = {0, 1};
          remapNode->setInputValue(_tokens->inhigh, inhigh, _tokens->float2);
          remapNode->setInputValue(_tokens->inlow, inlow, _tokens->float2);
...
static void _UpdatePrimvarNodes(HdMaterialNetworkInterface *netInterface,
                                ...
                                mx::DocumentPtr const &mxDoc)
{
  for (SdfPath const &nodePath : hdPrimvarNodePaths) {
    ...
    if (nodeType.IsEmpty()) {
      TF_WARN("Can't find node '%s' in material network.", nodeName.GetText());
      continue;
    }
    ...
    _FindGraphAndNodeByName(
        mxDoc, nodePath.GetParentPath().GetName(), mxNodeName, &mxNodeGraph, &mxNode);

    // Ignore nodes that aren't "texcoord" nodes
    if (!mxNode || mxNode->getCategory() != _tokens->texcoord) {
      continue;
    }
    mx::NodeDefPtr mxNodeDef = mxDoc->getNodeDef(_tokens->ND_geompropvalue_vector2.GetText());
    if (!mxNodeDef) {
      continue;
    }
    ...
    mxNode->setInputValue(
        _tokens->geomprop.GetText(), primvarName, _tokens->string_type.GetText());
  }
}
                      ...
                      std::vector<std::string> *outputErrorMessages)
{
  ...
  static const std::vector<TfToken> supportedTerminalTokens = {
      HdMaterialTerminalTokens->surface, HdMaterialTerminalTokens->displacement};
  ...
  for (auto terminalName : supportedTerminalTokens) {
    ...
    const TfToken terminalNodeType = netInterface->GetNodeType(terminalNodeName);
    ...
    const SdrShaderNodeConstPtr mtlxSdrNode = sdrRegistry.GetShaderNodeByIdentifierAndType(
        terminalNodeType, _tokens->mtlx);
    if (!mtlxSdrNode) {
      return;
    }

    TfTokenVector cNames = netInterface->GetNodeInputConnectionNames(terminalNodeName);
    ...
    if (!cNames.empty()) {
      ...
      mx::DocumentPtr mxDoc = HdMtlxCreateMtlxDocumentFromHdMaterialNetworkInterface(
          netInterface, terminalNodeName, cNames, stdLibraries, &hdMtlxData);
      ...
      _UpdateNetwork(
          netInterface, terminalNodeName, mxDoc, searchPath, nodesToKeep, nodesToRemove);
    }
