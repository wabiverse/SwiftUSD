...
NdrPropertyUniquePtrVec RmanOslParserPlugin::_getNodeProperties(
    const RixShaderQuery *sq, const NdrNodeDiscoveryResult &discoveryResult) const
{
...
NdrTokenMap RmanOslParserPlugin::_getPropertyMetadata(
    const RixShaderParameter *param, const NdrNodeDiscoveryResult &discoveryResult) const
{
  ...
  for (int i = 0; i < param->MetaDataSize(); ++i) {
    ...
    if (entryName == _tokens->vstructMember) {
      ...
      metadata[entryName] = TfStringReplace(std::string(*metaParam->DefaultS()),
