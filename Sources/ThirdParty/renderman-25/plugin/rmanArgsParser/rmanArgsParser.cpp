...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    ((nameAttr, "name"))((typeAttr, "type"))((arraySizeAttr, "arraySize"))((defaultAttr, "default"))((inputAttr, "input"))((tagAttr, "tag"))((vstructmemberAttr, "vstructmember"))((sdrDefinitionNameAttr, "sdrDefinitionName")));
...
{
  static const std::unordered_map<TfToken, TfToken, TfHash> contextMapping({{TfToken("displayfilter"), SdrNodeContext->DisplayFilter},
    ...
    xml_node childElement, const std::string &parentPage)
{
  ...
  if (!parentPage.empty())
  {
    attributes.insert({SdrPropertyMetadata->Page,
                       parentPage});
  }
    ...
    const NdrTokenMap &metadata)
{
           ...
           type == SdrPropertyTypes->Vector)
  {
    ...
    if (!isArray)
    {
      ...
      {
        TF_DEBUG(NDR_PARSING).Msg("float3 default value [%s] has %zd values; should "
                                  "have three.",
                                  stringValue.c_str(), parts.size());
...
_OutputDeprecationWarning(
    ...
    const TfToken &propName)
{
  TF_DEBUG(NDR_PARSING).Msg("Deprecated '%s' attribute on shader [%s] on property [%s]", attrName.GetText(), shaderRep.name.c_str(), propName.GetText());
}
    ...
    NdrOptionVec &options)
{
  ...
  if (typeName.IsEmpty())
  {
    ...
    TF_DEBUG(NDR_PARSING).Msg("Property [%s] doesn't have a valid type. "
  ...
  if (attributes.count(_xmlAttributeNames->tagAttr))
  {
    ...
    attributes.insert({SdrPropertyMetadata->ValidConnectionTypes,
                       attributes.at(_xmlAttributeNames->tagAttr)});
  ...
  if (attributes.count(_xmlAttributeNames->vstructmemberAttr))
  {
    ...
    if (!vstructMember.empty())
    {
      ...
      if (dotPos != std::string::npos)
      {
        ...
        attributes.insert({SdrPropertyMetadata->VstructMemberOf,
                           vstructMember.substr(0, dotPos)});

        // Add member name to attributes
        attributes.insert({SdrPropertyMetadata->VstructMemberName,
                           vstructMember.substr(dotPos + 1)});
      }
      else
      {
        TF_DEBUG(NDR_PARSING).Msg("Bad virtual structure member in %s.%s:%s", shaderRep.name.c_str(), propName.GetText(), vstructMember.c_str());
      }
    }
  }
    ...
    const NdrTokenMap &map, const TfToken &key, int defaultValue)
{
  ...
  if (it != map.end())
  {
    ...
    {
      TF_DEBUG(NDR_PARSING).Msg("Attribute [%s] with string value [%s] "
