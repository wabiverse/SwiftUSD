...
namespace
{
  ...
  UsdSamples::UsdSamples(const SdfPath &primPath, const TfToken &propertyName) : _propPath(primPath.AppendProperty(propertyName)),
                                                                                 _data(NULL)
  {
    _Clear();
  }
  ...
  UsdSamples::UsdSamples(
      ...
      const TfToken &propertyName,
      const SdfAbstractData &data) : _propPath(primPath.AppendProperty(propertyName)),
                                     _data(&data)
  {
    VtValue value;
                  ...
                  "No type name on <%s>", _propPath.GetText()))
    {
      ...
      {
        _typeName =
            SdfSchema::GetInstance().FindType(value.UncheckedGet<TfToken>());
        _Validate();
      }
      else
      {
  ...
  _WriterContext::_WriterContext() : _timeScale(24.0), // Usd is frames, Alembic is seconds.
      ...
      R (DST::*method)(T))
  {
    typedef typename boost::remove_const<
        typename boost::remove_reference<T>::type>::type SampleValueType;
  ...
  */
      ...
      const std::function<OCompoundProperty()> &getParentProperty)
  {
    ...
    if (anyProperties)
    {
      ...
      {
        TfTokenVector names =
            name == UsdAbcPropertyNames->stIndices ? SdfPath::TokenizeIdentifierAsTokens(
  ...
  template <class T>
  static bool _ExtractWithFallback(UsdSamples const &samples, double time,
  ...
  _WriteXform(_PrimWriterContext *context)
  {
    ...
    const SdfValueTypeName &transformValueType = hasXformOpOrder ? SdfValueTypeNames->Matrix4d : SdfValueTypeNames->Matrix4d;
  ...
  _WritePolyMesh(_PrimWriterContext *context)
  {
    ...
    UsdSamples uv = (TfGetEnvSetting(USD_ABC_READ_FLOAT2_AS_UV)) ? (context->ExtractSamples(UsdAbcPropertyNames->st,
    ...
    if (uv.IsEmpty())
    {
      uv = (TfGetEnvSetting(USD_ABC_READ_FLOAT2_AS_UV)) ? (context->ExtractSamples(UsdAbcPropertyNames->uv,
                                                                                   SdfValueTypeNames->TexCoord2fArray,
                                                                                   SdfValueTypeNames->Float2Array))
...
bool UsdAbc_AlembicDataWriter::Open(
