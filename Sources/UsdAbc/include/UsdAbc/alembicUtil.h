...
namespace Alembic
{
  namespace Util
  {
    ...
    {
      template <>
      struct PODTraitsFromType<PXR_NS::GfHalf>
...
#define USD_ABC_PRIM_TYPE_NAMES \
  (BasisCurves)(Camera)(HermiteCurves)(Mesh)(NurbsCurves)(Points)(PolyMesh)(PseudoRoot)(Scope)(Xform)(GeomSubset) /* end */
...
#define USD_ABC_GPRIM_NAMES \
  (primvars)(userProperties)((defaultFamilyName, "materialBind"))((defaultFamilyTypeAttributeName, "subsetFamily:materialBind:familyType")) /* end */
#define USD_ABC_POINTBASED_NAMES \
  ((uv, "primvars:uv"))((uvIndices, "primvars:uv:indices"))((st, "primvars:st"))((stIndices, "primvars:st:indices")) /* end */
...
#define USD_ABC_CUSTOM_METADATA \
  ...
  struct UsdAbc_AlembicType : boost::totally_ordered<UsdAbc_AlembicType>
  {
    ...
    UsdAbc_AlembicType(PlainOldDataType pod_, uint8_t extent_, bool_t array_) : pod(pod_), extent(extent_), array(array_)
    {
      // Do nothing
    }

    // An Alembic property's type.
    UsdAbc_AlembicType(const PropertyHeader &header) : pod(header.getPropertyType() == kCompoundProperty ? kUnknownPOD : header.getDataType().getPod()),
  ...
  public:
    ...
    /// An empty sample.
    _SampleForAlembic() : _numSamples(0),
    ...
    /// error message.
    _SampleForAlembic(const Error &error) : _numSamples(0),
    ...
    template <class T>
    _SampleForAlembic(const std::vector<T> &value) : _numSamples(value.size()),
    ...
    template <class E>
    _SampleForAlembic(const VtValue &value, const E &extractor) : _value(_HolderValue(new _VtValueHolder(value, &_numSamples, extractor)))
    {
      // Do nothing
    }
    ...
    template <class T>
    _SampleForAlembic(const std::shared_ptr<T> &value) : _numSamples(1),
    ...
    template <class T>
    _SampleForAlembic(std::unique_ptr<T[]> &&value, size_t count) : _numSamples(count),
  ...
  private:
    ...
    public:
      ...
      _VtValueHolder(const VtValue &value,
                     size_t *numSamples, const E &extractor) : _value(new VtValue(value)),
