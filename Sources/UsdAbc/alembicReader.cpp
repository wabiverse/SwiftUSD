...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    (transform)((xformOpTransform, "xformOp:transform")));
...
namespace
{
  ...
  _GetUVPropertyName()
  {
    static const TfToken uvUsdAbcPropertyName =
        (TfGetEnvSetting(USD_ABC_WRITE_UV_AS_ST_TEXCOORD2FARRAY)) ? (UsdAbcPropertyNames->st) : (UsdAbcPropertyNames->uv);
  ...
  _GetUVTypeName()
  {
    static const SdfValueTypeName uvTypeName =
        (TfGetEnvSetting(USD_ABC_WRITE_UV_AS_ST_TEXCOORD2FARRAY)) ? (SdfValueTypeNames->TexCoord2fArray) : (SdfValueTypeNames->Float2Array);
    return uvTypeName;
  }
  ...
  AlembicProperty::AlembicProperty(
      const SdfPath &path,
      const std::string &name) : _path(path), _parent(), _name(name)
  {
    // Do nothing
  }
  ...
  AlembicProperty::AlembicProperty(
      ...
      const IObject &parent) : _path(path), _parent(parent.getProperties()), _name(name)
  {
    // Do nothing
  }
  ...
  AlembicProperty::AlembicProperty(
      const SdfPath &path,
      const std::string &name,
      const ICompoundProperty &parent) : _path(path), _parent(parent), _name(name)
  {
    // Do nothing
  }
  ...
  _ReaderContext::_ReaderContext() : _mutex(NULL),
  ...
  _PrimReaderContext::_PrimReaderContext(
      _ReaderContext &context,
      const IObject &prim,
      const SdfPath &path) : _context(context),
                             _prim(prim),
                             _path(path)
  {
  ...
  struct _CopyGeneric
  {
    ...
    _CopyGeneric(const AlembicProperty &object_,
                 SchemaInterpMatching matching = kStrictMatching) : object(object_.Cast<PropertyType>(matching)) {}
  ...
  struct _CopyVisibility : _CopyGeneric<ICharProperty>
  {
                    ...
                    const ISampleSelector &iss) const
    {
      ...
      {
        const std::string authoredValue =
            (vis == kVisibilityVisible ? "kVisibilityVisible" : TfStringify(vis));
  ...
  _ReadUnknown(_PrimReaderContext *context)
  {
    ...
    prim.typeName =
        TfToken(context->GetObject().getMetaData().get(_AmdName(SdfFieldKeys->TypeName)));
    if (prim.typeName.IsEmpty())
    {
  ...
  static _ReaderContext::Ordering
...
UsdAbc_AlembicDataReader::TimeSamples::TimeSamples(
    const std::vector<double> &times) : _times(times)
{
  // Do nothing
}
...
template <class T>
bool UsdAbc_AlembicDataReader::TimeSamples::Bracket(
...
bool UsdAbc_AlembicDataReader::HasField(
    const SdfPath &path,
    const TfToken &fieldName,
...
bool UsdAbc_AlembicDataReader::HasValue(
    const SdfPath &path,
    Index index,
    SdfAbstractDataValue *value) const
{
...
void UsdAbc_AlembicDataReader::VisitSpecs(
    const SdfAbstractData &owner,
    SdfAbstractDataSpecVisitor *visitor) const
{
