...
bool Sdf_PathExpressionEvalBase::_Match(
...
void Sdf_PathExpressionEvalBase::_PatternMatchBase::_Init(
...
bool Sdf_PathExpressionEvalBase::_PatternMatchBase::_Match(
    SdfPath const &path,
    TfFunctionRef<bool(int, SdfPath const &)> runNthPredicate) const {
  ...
  if (!path.HasPrefix(_prefix) || (_isProperty && !path.IsPrimPropertyPath())) {
    return false;
  }
  ...
  struct Segment {
    ...
    bool StartsAt(ComponentIter iter) const { return begin == iter; }
    bool EndsAt(ComponentIter iter) const { return end == iter; }
    size_t GetSize() const { return std::distance(begin, end); }
    ComponentIter begin;
    ComponentIter end;
  };
  ...
  auto checkMatch = [this,
                     &runNthPredicate](Segment const &seg,
