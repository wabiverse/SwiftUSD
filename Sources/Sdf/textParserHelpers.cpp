...
#define ERROR_AND_RETURN_IF_NOT_ALLOWED(context, allowed) \
  { \
    ...
    if (!allow) { \
      SDF_TEXTFILEFORMATPARSER_ERR(context, "%s", allow.GetWhyNot().c_str()); \
      return; \
    } \
  }
...
SdfPermission _GetPermissionFromString(const std::string &str, Sdf_TextParserContext *context)
{
  if (str == "public") {
    ...
    SDF_TEXTFILEFORMATPARSER_ERR(context, "'%s' is not a valid permission constant", str.c_str());
    return SdfPermissionPublic;
  }
}
...
TfEnum _GetDisplayUnitFromString(const std::string &name, Sdf_TextParserContext *context)
{
  ...
  if (unit == TfEnum()) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "'%s' is not a valid display unit", name.c_str());
  }
  return unit;
}
...
void _ValueSetAtomic(Sdf_TextParserContext *context)
{
  if (!context->values.IsRecordingString()) {
    if (context->values.valueIsShaped) {
      SDF_TEXTFILEFORMATPARSER_ERR(context, "Type name has [] for non-shaped value!\n");
      return;
    }
  }
  ...
  if (context->currentValue.IsEmpty()) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "Error parsing simple value: %s", errStr.c_str());
    return;
  }
}
...
void _PrimSetSpecializesListItems(SdfListOpType opType, Sdf_TextParserContext *context)
{
...
void _PrimSetReferenceListItems(SdfListOpType opType, Sdf_TextParserContext *context)
{
...
void _PrimSetVariantSetNamesListItems(SdfListOpType opType, Sdf_TextParserContext *context)
{
...
void _RelationshipSetTargetsList(SdfListOpType opType, Sdf_TextParserContext *context)
{
                   ...
                   Sdf_TextParserContext *context)
{
  ...
  if (!SdfSchema::IsValidRelocatesSourcePath(srcPath)) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "'%s' is not a valid relocates path", srcStr.c_str());
    return;
  }
...
void _AttributeSetConnectionTargetsList(SdfListOpType opType, Sdf_TextParserContext *context)
{
...
void _PrimInitAttribute(const Sdf_ParserHelpers::Value &arg1, Sdf_TextParserContext *context)
{
  ...
  if (!SdfPath::IsValidNamespacedIdentifier(name)) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "'%s' is not a valid attribute name", name.GetText());
  }
                                  ...
                                  Sdf_TextParserContext *context)
{
  ...
  if (!_SetupValue(typeName, context)) {
    SDF_TEXTFILEFORMATPARSER_ERR(
        context, "Unrecognized value typename '%s' for dictionary", typeName.c_str());
  }
}
...
void _ValueSetTuple(Sdf_TextParserContext *context)
{
  ...
  if (context->currentValue == VtValue()) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "Error parsing tuple value: %s", errStr.c_str());
    return;
  }
}
...
void _ValueSetList(Sdf_TextParserContext *context)
{
  if (!context->values.IsRecordingString()) {
    if (!context->values.valueIsShaped) {
      SDF_TEXTFILEFORMATPARSER_ERR(context, "Type name missing [] for shaped value.");
      return;
    }
  }
  ...
  context->currentValue = context->values.ProduceValue(&errStr);
  if (context->currentValue == VtValue()) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "Error parsing shaped value: %s", errStr.c_str());
    return;
  }
}
...
void _PrimInitRelationship(const Sdf_ParserHelpers::Value &arg1, Sdf_TextParserContext *context)
{
  TfToken name(arg1.Get<std::string>());
  if (!SdfPath::IsValidNamespacedIdentifier(name)) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "'%s' is not a valid relationship name", name.GetText());
    return;
  }
...
void _PathSetPrim(const Sdf_ParserHelpers::Value &arg1, Sdf_TextParserContext *context)
{
  ...
  if (!context->savedPath.IsPrimPath()) {
    SDF_TEXTFILEFORMATPARSER_ERR(context, "'%s' is not a valid prim path", pathStr.c_str());
  }
}
                                     ...
                                     Sdf_TextParserContext *context)
{
  ...
  if (!pathValid) {
    SDF_TEXTFILEFORMATPARSER_ERR(
        context, "'%s' is not a valid prim or property scene path", pathStr.c_str());
  }
}
  ...
  if (specDef.IsMetadataField(context->genericMetadataKey)) {
    ...
    if (_IsGenericMetadataListOpType(fieldType)) {
      if (!fieldDef.IsValidListValue(context->currentValue)) {
        SDF_TEXTFILEFORMATPARSER_ERR(
            context, "invalid value for field \"%s\"", context->genericMetadataKey.GetText());
      }
      else {
...
void _RaiseError(Sdf_TextParserContext *context, const char *msg)
{
  ...
  std::string s = TfStringPrintf(
      "%s in <%s> on line %i", msg, context->path.GetText(), errLineNumber);
