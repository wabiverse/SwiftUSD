...
TF_REGISTRY_FUNCTION(TfType) {
  TfType::Define<SdfPrimSpecHandleVector>().Alias(TfType::GetRoot(),
                                                  "SdfPrimSpecHandleVector");
  TfType::Define<SdfVariantSetSpecHandleMap>().Alias(
      TfType::GetRoot(), "map<string, SdfVariantSetSpecHandle>");
}
...
const std::string &SdfPrimSpec::GetName() const { return GetPath().GetName(); }

TfToken SdfPrimSpec::GetNameToken() const { return GetPath().GetNameToken(); }

bool SdfPrimSpec::CanSetName(const std::string &newName,
  ...
  return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CanRename(*this,
                                                           TfToken(newName))
      .IsAllowed(whyNot);
}
...
void SdfPrimSpec::SetNameChildren(
    const SdfPrimSpecHandleVector &nameChildrenSpecs) {
  Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::SetChildren(GetLayer(), GetPath(),
                                                      nameChildrenSpecs);
}
...
SdfPrimSpec::PropertySpecView SdfPrimSpec::GetProperties() const {
  return PropertySpecView(GetLayer(), GetPath(),
                          SdfChildrenKeys->PropertyChildren);
}

void SdfPrimSpec::SetProperties(
    const SdfPropertySpecHandleVector &propertySpecs) {
  if (!_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
    return;
  }

  Sdf_ChildrenUtils<Sdf_PropertyChildPolicy>::SetChildren(GetLayer(), GetPath(),
                                                          propertySpecs);
}

bool SdfPrimSpec::InsertProperty(const SdfPropertySpecHandle &property,
                                 int index) {
  if (!_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
    return false;
  }
...
SdfPrimSpec::RelationshipSpecView SdfPrimSpec::GetRelationships() const {
  return RelationshipSpecView(GetLayer(), GetPath(),
                              SdfChildrenKeys->PropertyChildren);
}

SdfPropertyOrderProxy SdfPrimSpec::GetPropertyOrder() const {
  return SdfGetNameOrderProxy(SdfCreateHandle(this),
...
SDF_DEFINE_TYPED_GET_SET(Specifier, SdfFieldKeys->Specifier, SdfSpecifier,
                         SdfSpecifier)
SDF_DEFINE_TYPED_GET_SET(Permission, SdfFieldKeys->Permission, SdfPermission,
                         SdfPermission)
...
SdfInheritsProxy SdfPrimSpec::GetInheritPathList() const {
  return SdfGetPathEditorProxy(SdfCreateHandle(this),
...
bool SdfPrimSpec::HasPayloads() const { return GetPayloadList().HasKeys(); }
...
SdfReferencesProxy SdfPrimSpec::GetReferenceList() const {
  return SdfGetReferenceEditorProxy(SdfCreateHandle(this),
                                    SdfFieldKeys->References);
}

bool SdfPrimSpec::HasReferences() const { return GetReferenceList().HasKeys(); }
...
SdfPrimSpec::GetVariantNames(const std::string &name) const {
  ...
  TF_FOR_ALL(i, variantNameTokens) { variantNames.push_back(i->GetString()); }
...
SdfVariantSetsProxy SdfPrimSpec::GetVariantSets() const {
  return SdfVariantSetsProxy(
      SdfVariantSetView(GetLayer(), GetPath(),
...
SdfVariantSelectionProxy SdfPrimSpec::GetVariantSelections() const {
  if (!_IsPseudoRoot()) {
    return SdfVariantSelectionProxy(SdfCreateHandle(this),
...
bool Sdf_UncheckedCreatePrimInLayer(SdfLayer *layerPtr,
...
static inline bool Sdf_CanCreatePrimInLayer(SdfLayer *layer,
                                            _AbsPathHelper const &absPath) {
