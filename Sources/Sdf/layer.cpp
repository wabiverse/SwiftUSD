...
TF_REGISTRY_FUNCTION(TfType) { TfType::Define<SdfLayer>(); }
...
TF_MAKE_STATIC_DATA(SdfLayer::DetachedLayerRules, _detachedLayerRules) {
  const std::vector<std::string> includePatterns =
      TfStringSplit(TfGetEnvSetting(SDF_LAYER_INCLUDE_DETACHED), ",");
  ...
  const std::vector<std::string> excludePatterns =
      TfStringSplit(TfGetEnvSetting(SDF_LAYER_EXCLUDE_DETACHED), ",");
...
_CreateDataForFileFormat(const SdfFileFormatConstPtr &fileFormat,
                         const std::string &identifier,
                         const SdfLayer::FileFormatArguments &args) {
  return SdfLayer::IsIncludedByDetachedLayerRules(identifier)
             ? fileFormat->InitDetachedData(args)
             : fileFormat->InitData(args);
}

SdfLayer::SdfLayer(const SdfFileFormatConstPtr &fileFormat,
                   const string &identifier, const string &realPath,
                   const ArAssetInfo &assetInfo,
                   const FileFormatArguments &args, bool validateAuthoring)
    : _self(this), _fileFormat(fileFormat), _fileFormatArgs(args),
      _schema(fileFormat->GetSchema()), _idRegistry(SdfLayerHandle(this)),
      _data(_CreateDataForFileFormat(fileFormat, identifier, args)),
      _stateDelegate(SdfSimpleLayerStateDelegate::New()),
      _lastDirtyState(false), _assetInfo(new Sdf_AssetInfo),
      _mutedLayersRevisionCache(0), _isMutedCache(false),
      _permissionToEdit(true), _permissionToSave(true),
      _validateAuthoring(validateAuthoring ||
                         TfGetEnvSetting<bool>(SDF_LAYER_VALIDATE_AUTHORING)),
  ...
  string layerIdentifier =
      Sdf_IsAnonLayerIdentifier(identifier)
  ...
  _InitializeFromIdentifier(layerIdentifier, realPath, std::string(),
                            assetInfo);
...
SdfLayerRefPtr
SdfLayer::_CreateNewWithFormat(const SdfFileFormatConstPtr &fileFormat,
                               const string &identifier, const string &realPath,
                               const ArAssetInfo &assetInfo,
  ...
  return fileFormat->NewLayer(fileFormat, identifier, realPath, assetInfo,
                              args);
}
...
static bool _ModificationTimesEqual(const VtDictionary &t1,
                                    const VtDictionary &t2) {
  if (t1.size() != t2.size()) {
    return false;
  }
...
SdfLayerRefPtr
SdfLayer::_CreateAnonymousWithFormat(const SdfFileFormatConstPtr &fileFormat,
  ...
  SdfLayerRefPtr layer =
      _CreateNewWithFormat(fileFormat, Sdf_GetAnonLayerIdentifierTemplate(tag),
...
SdfLayerRefPtr SdfLayer::CreateNew(const string &identifier,
                                   const FileFormatArguments &args) {
...
SdfLayerRefPtr SdfLayer::_CreateNew(SdfFileFormatConstPtr fileFormat,
  ...
  if (!Sdf_CanCreateNewLayerWithIdentifier(identifier, &whyNot)) {
    TF_CODING_ERROR("Cannot create new layer '%s': %s", identifier.c_str(),
                    whyNot.c_str());
    return TfNullPtr;
  }
  ...
  if (targetArgIt != finalArgs.end()) {
    absIdentifier =
        Sdf_CreateIdentifier(absIdentifier, FileFormatArguments{*targetArgIt});
  }
  ...
  SdfLayerRefPtr layer;
  {
    ...
    layer = _CreateNewWithFormat(fileFormat, absIdentifier, localPath,
                                 ArAssetInfo(), finalArgs);
...
bool SdfLayer::_ComputeInfoToFindOrOpenLayer(
    const string &identifier, const SdfLayer::FileFormatArguments &args,
    _FindOrOpenLayerInfo *info, bool computeAssetInfo) {
  TRACE_FUNCTION();
  ...
  if (layerArgs.empty()) {
    ...
    TF_FOR_ALL(it, args) { layerArgs[it->first] = it->second; }
  }
  ...
  info->identifier =
      Sdf_CreateIdentifier(info->layerPath, info->fileFormatArgs);
...
/* static */
SdfLayerRefPtr SdfLayer::FindOrOpen(const string &identifier,
                                    const FileFormatArguments &args) {
  TRACE_FUNCTION();
...
/* static */
SdfLayerRefPtr
SdfLayer::FindOrOpenRelativeToLayer(const SdfLayerHandle &anchor,
                                    const string &identifier,
                                    const FileFormatArguments &args) {
  TRACE_FUNCTION();
  ...
  return FindOrOpen(SdfComputeAssetPathRelativeToLayer(anchor, identifier),
                    args);
}
...
SdfLayerRefPtr SdfLayer::OpenAsAnonymous(const std::string &layerPath,
  ...
  SdfLayerRefPtr layer;
  {
    tbb::queuing_rw_mutex::scoped_lock lock(_GetLayerRegistryMutex());
    layer =
        _CreateNewWithFormat(layerInfo.fileFormat,
                             Sdf_GetAnonLayerIdentifierTemplate(tag), string());
...
const SdfSchemaBase &SdfLayer::GetSchema() const { return _schema; }
...
SdfLayer::_ReloadResult SdfLayer::_Reload(bool force) {
  ...
  if (isAnonymous && GetFileFormat()->ShouldSkipAnonymousReload()) {
    ...
    if (!force && !IsDirty() && (resolvedPath == oldResolvedPath) &&
        ...
        (_ModificationTimesEqual(externalAssetTimestamps,
                                 _externalAssetModificationTimes))) {
      return _ReloadSkipped;
    }
...
bool SdfLayer::Reload(bool force) { return _Reload(force) == _ReloadSucceeded; }
...
bool SdfLayer::_Read(const string &identifier,
                     const ArResolvedPath &resolvedPathIn, bool metadataOnly) {
  TRACE_FUNCTION();
  ...
  TF_DEBUG(SDF_LAYER).Msg("SdfLayer::_Read('%s', '%s', metadataOnly=%s)\n",
                          identifier.c_str(),
                          resolvedPathIn.GetPathString().c_str(),
...
/*static*/
SdfLayerHandle SdfLayer::Find(const string &identifier,
                              const FileFormatArguments &args) {
  TRACE_FUNCTION();
...
SdfLayerRefPtr SdfLayer::_Find(const string &identifier,
  ...
  if (SdfLayerRefPtr layer =
          _TryToFindLayer(layerInfo.identifier, layerInfo.resolvedLayerPath,
                          lock, retryAsWriter)) {
    return layer->_WaitForInitializationAndCheckIfSuccessful() ? layer
                                                               : TfNullPtr;
  }
  return TfNullPtr;
}
...
bool SdfLayer::GetBracketingTimeSamples(double time, double *tLower,
                                        double *tUpper) {
...
bool SdfLayer::GetBracketingTimeSamplesForPath(const SdfPath &path, double time,
                                               double *tLower, double *tUpper) {
  return _data->GetBracketingTimeSamplesForPath(path, time, tLower, tUpper);
}

bool SdfLayer::QueryTimeSample(const SdfPath &path, double time,
                               VtValue *value) const {
...
static TfType _GetExpectedTimeSampleValueType(const SdfLayer &layer,
                                              const SdfPath &path) {
...
void SdfLayer::SetTimeSample(const SdfPath &path, double time,
                             const VtValue &value) {
  if (!PermissionToEdit()) {
  ...
  if (value.GetType() == expectedType) {
    ...
    if (castValue.IsEmpty()) {
      TF_CODING_ERROR("Can't set time sample on <%s> to %s: "
                      ...
                      path.GetText(), TfStringify(value).c_str(),
...
void SdfLayer::SetTimeSample(const SdfPath &path, double time,
  ...
  if (TfSafeTypeCompare(value.valueType, expectedType.GetTypeid())) {
    ...
    if (castValue.IsEmpty()) {
      TF_CODING_ERROR("Can't set time sample on <%s> to %s: "
                      "expected a value of type \"%s\"",
                      path.GetText(), TfStringify(tmpValue).c_str(),
                      expectedType.GetTypeName().c_str());
      return;
    }
...
static const VtValue &_GetVtValue(const VtValue &v) { return v; }
...
template <class T>
void SdfLayer::_PrimSetTimeSample(const SdfPath &path, double time,
                                  const T &value, bool useDelegate) {
...
template void SdfLayer::_PrimSetTimeSample(const SdfPath &, double,
                                           const VtValue &, bool);
template void SdfLayer::_PrimSetTimeSample(const SdfPath &, double,
...
void SdfLayer::_InitializeFromIdentifier(const string &identifier,
  ...
  std::unique_ptr<Sdf_AssetInfo> newInfo(Sdf_ComputeAssetInfoFromIdentifier(
      identifier, realPath, assetInfo, fileVersion));
  if (!newInfo)
    return;
...
double SdfLayer::GetTimeCodesPerSecond() const {
  ...
  VtValue value;
  if (HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->TimeCodesPerSecond,
               &value)) {
...
void SdfLayer::ClearTimeCodesPerSecond() {
  return EraseField(SdfPath::AbsoluteRootPath(),
                    SdfFieldKeys->TimeCodesPerSecond);
}
...
SdfSubLayerProxy SdfLayer::GetSubLayerPaths() const {
  return SdfSubLayerProxy(std::make_unique<Sdf_SubLayerListEditor>(_self),
...
bool SdfLayer::_CanGetSpecAtPath(const SdfPath &path, SdfPath *canonicalPath,
...
void SdfLayer::SetPermissionToEdit(bool allow) { _permissionToEdit = allow; }

void SdfLayer::SetPermissionToSave(bool allow) { _permissionToSave = allow; }

static bool _HasObjectAtPath(const SdfLayerHandle &layer, const SdfPath &path) {
  return layer->GetObjectAtPath(path);
}

static bool _CanEdit(const SdfLayerHandle &layer, const SdfNamespaceEdit &edit,
                     std::string *detail) {
  if (edit.currentPath.IsPrimPath()) {
    if (edit.newPath.IsEmpty()) {
      ...
      return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::
          CanRemoveChildForBatchNamespaceEdit(
              ...
              edit.currentPath.GetNameToken(), detail);
    } else {
      // Insert prim child.
      return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::
    ...
    if (edit.newPath.IsEmpty()) {
      ...
      if (SdfAttributeSpecHandle attr =
        ...
        return Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::
            CanMoveChildForBatchNamespaceEdit(
                layer, edit.newPath.GetParentPath(), attr,
                edit.newPath.GetNameToken(), edit.index, detail);
      } else {
  ...
  if (edit.currentPath.IsPrimPath()) {
    if (edit.newPath.IsEmpty()) {
      ...
      Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::MoveChildForBatchNamespaceEdit(
          layer, edit.newPath.GetParentPath(),
          layer->GetPrimAtPath(edit.currentPath), edit.newPath.GetNameToken(),
          edit.index);
    }
  } else {
    if (edit.newPath.IsEmpty()) {
      if (edit.currentPath.IsRelationalAttributePath()) {
        ...
        Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::
            RemoveChildForBatchNamespaceEdit(layer,
...
SdfLayer::CanApply(const SdfBatchNamespaceEdit &edits,
  ...
  if (!edits.Process(NULL, std::bind(&_HasObjectAtPath, _self, ph::_1),
                     std::bind(&_CanEdit, _self, ph::_1, ph::_2), details,
                     !fixBackpointers)) {
...
bool SdfLayer::Apply(const SdfBatchNamespaceEdit &edits) {
  ...
  if (!edits.Process(&final, std::bind(&_HasObjectAtPath, _self, ph::_1),
                     std::bind(&_CanEdit, _self, ph::_1, ph::_2), NULL,
                     !fixBackpointers)) {
    return false;
  }
...
void SdfLayer::RemovePropertyIfHasOnlyRequiredFields(
...
bool SdfLayer::_RemoveInertDFS(SdfPrimSpecHandle prim) {
  ...
  if (!inert) {
    ...
    TF_FOR_ALL(it, removedChildren) { prim->RemoveNameChild(*it); }
    ...
    TF_FOR_ALL(varSetIt, variantSetMap) {
      ...
      TF_FOR_ALL(varIt, variants) { _RemoveInertDFS((*varIt)->GetPrimSpec()); }
    }
  }
...
void SdfLayer::_RemoveInertToRootmost(SdfPrimSpecHandle prim) {
  while (prim && !SdfIsDefiningSpecifier(prim->GetSpecifier()) &&
         prim->IsInert()) {
...
bool SdfLayer::SplitIdentifier(const string &identifier, string *layerPath,
...
const string &SdfLayer::GetIdentifier() const { return _assetInfo->identifier; }
...
void SdfLayer::SetIdentifier(const string &identifier) {
  ...
  if (!Sdf_CanCreateNewLayerWithIdentifier(newLayerPath, &whyNot)) {
    TF_CODING_ERROR("Cannot change identifier to '%s': %s", identifier.c_str(),
                    whyNot.c_str());
    return;
  }
...

  {
    ...
    SdfLayerRefPtr existingLayer =
        _Find(absIdentifier, FileFormatArguments(), lock, retryAsWriter);
    if (existingLayer) {
  ...
  if (oldResolvedPath != newResolvedPath) {
    const ArTimestamp timestamp =
        ArGetResolver().GetModificationTimestamp(newLayerPath, newResolvedPath);
...
string SdfLayer::ComputeAbsolutePath(const string &assetPath) const {
  ...
  return SdfComputeAssetPathRelativeToLayer(SdfCreateNonConstHandle(this),
                                            assetPath);
}

SdfLayer::DetachedLayerRules &SdfLayer::DetachedLayerRules::Include(
...
SdfLayer::DetachedLayerRules &SdfLayer::DetachedLayerRules::Exclude(
    const std::vector<std::string> &patterns) {
...
bool SdfLayer::DetachedLayerRules::IsIncluded(
  ...
  const bool included =
      _includeAll || std::find_if(_include.begin(), _include.end(),
  ...
  const bool excluded = std::find_if(_exclude.begin(), _exclude.end(),
                                     [&layerPath](const std::string &s) {
                                       return TfStringContains(layerPath, s);
                                     }) != _exclude.end();
...
void SdfLayer::SetStateDelegate(
    const SdfLayerStateDelegateBaseRefPtr &delegate) {
...
bool SdfLayer::IsEmpty() const {
  ...
  return GetRootPrims().empty() && GetRootPrimOrder().empty() &&
         GetSubLayerPaths().empty();
}

bool SdfLayer::StreamsData() const { return _GetData()->StreamsData(); }

bool SdfLayer::IsDetached() const { return _GetData()->IsDetached(); }
  ...
  if (prim != prim->GetLayer()->GetPseudoRoot()) {
    ...
    for (const auto &varSetIt : variantSetMap) {
      ...
      for (const SdfVariantSpecHandle &varSpec : variants) {
        _GatherPrimCompositionDependencies(varSpec->GetPrimSpec(),
                                           assetReferences);
      }
    }
  }
...
bool SdfLayer::UpdateExternalReference(const string &oldLayerPath,
...
bool SdfLayer::UpdateCompositionAssetDependency(const string &oldLayerPath,
  ...
  _UpdatePrimCompositionDependencyPaths(GetPseudoRoot(), oldLayerPath,
                                        newLayerPath);
...
_UpdateRefOrPayloadPath(const string &oldLayerPath, const string &newLayerPath,
...
void SdfLayer::_UpdatePrimCompositionDependencyPaths(
    const SdfPrimSpecHandle &prim, const string &oldLayerPath,
    const string &newLayerPath) {
  TF_AXIOM(!oldLayerPath.empty());

  // Prim references
  prim->GetReferenceList().ModifyItemEdits(
      std::bind(&_UpdateRefOrPayloadPath<SdfReference>, oldLayerPath,
                newLayerPath, ph::_1));

  // Prim payloads
  prim->GetPayloadList().ModifyItemEdits(
      std::bind(&_UpdateRefOrPayloadPath<SdfPayload>, oldLayerPath,
                newLayerPath, ph::_1));
...
/* static */
template <class Lock>
SdfLayerRefPtr SdfLayer::_OpenLayerAndUnlockRegistry(
    Lock &lock, const _FindOrOpenLayerInfo &info, bool metadataOnly) {
  ...
  TF_DEBUG(SDF_LAYER).Msg(
      ...
      info.fileFormat ? info.fileFormat->GetFormatId().GetText()
                      : "unknown file format",
  ...
  SdfLayerRefPtr layer = _CreateNewWithFormat(
      info.fileFormat, info.identifier, info.resolvedLayerPath, info.assetInfo,
      info.fileFormatArgs);
  ...
  if (!layer->IsMuted()) {
    ...
    readSuccess =
        layer->_Read(info.identifier, info.resolvedLayerPath, metadataOnly);
    //     });
...
SdfLayer::_GetRequiredFieldDef(const SdfPath &path, const TfToken &fieldName,
                               SdfSpecType specType) const {
...
SdfLayer::_GetRequiredFieldDef(const SdfSchemaBase &schema,
                               const TfToken &fieldName, SdfSpecType specType) {
  if (ARCH_UNLIKELY(schema.IsRequiredFieldName(fieldName))) {
    if (SdfSchema::SpecDefinition const *specDef =
            schema.GetSpecDefinition(specType)) {
      // If this field is required for this spec type, look up the
...
bool SdfLayer::HasField(const SdfPath &path, const TfToken &fieldName,
                        VtValue *value) const {
  SdfSpecType specType;
...
bool SdfLayer::HasFieldDictKey(const SdfPath &path, const TfToken &fieldName,
...
VtValue SdfLayer::GetField(const SdfPath &path,
...
static bool _IsValidFieldForLayer(const SdfLayer &layer, const SdfPath &path,
                                  const TfToken &fieldName) {
  return layer.GetSchema().IsValidFieldForSpec(fieldName,
                                               layer.GetSpecType(path));
}

void SdfLayer::SetField(const SdfPath &path, const TfToken &fieldName,
                        const VtValue &value) {
  if (value.IsEmpty())
...
SdfAbstractDataConstPtr SdfLayer::_GetData() const { return _data; }

SdfAbstractDataRefPtr SdfLayer::_CreateData() const {
  return _CreateDataForFileFormat(GetFileFormat(), GetIdentifier(),
                                  GetFileFormatArguments());
}

void SdfLayer::_SwapData(SdfAbstractDataRefPtr &data) { _data.swap(data); }
...
void SdfLayer::_SetData(const SdfAbstractDataPtr &newData,
  ...
  // Remove specs that no longer exist or whose required fields changed.
  {
    ...
    struct _SpecsToDelete : public SdfAbstractDataSpecVisitor {
      ...
      virtual bool VisitSpec(const SdfAbstractData &oldData,
                             const SdfPath &path) {
  ...
  // Create new specs.
  {
    ...
    struct _SpecsToCreate : public SdfAbstractDataSpecVisitor {
      ...
      virtual bool VisitSpec(const SdfAbstractData &newData,
                             const SdfPath &path) {
      ...
      if (path.IsPrimPath()) {
        ...
        inert =
            (newData->GetAs<SdfSpecifier>(path, SdfFieldKeys->Specifier,
                                          SdfSpecifierOver) ==
             SdfSpecifierOver) &&
            (newData->GetAs<TfToken>(path, SdfFieldKeys->TypeName, TfToken())
                 .IsEmpty());
  ...
  // Update spec fields.
  {
    struct _SpecUpdater : public SdfAbstractDataSpecVisitor {
      _SpecUpdater(SdfLayer *layer_, const SdfSchemaBase &newDataSchema_)
          : layer(layer_), newDataSchema(newDataSchema_) {}
    ...
    if (!updater.unrecognizedFields.empty()) {
      ...
      for (std::pair<TfToken, SdfPath> const &tokenPath :
        ...
        fieldDescrs.push_back(
            TfStringPrintf("'%s' first seen at <%s>", tokenPath.first.GetText(),
...
template <class T>
void SdfLayer::_PrimSetField(const SdfPath &path, const TfToken &fieldName,
                             const T &value, VtValue *oldValuePtr,
                             bool useDelegate) {
  if (useDelegate && TF_VERIFY(_stateDelegate)) {
  ...
  Sdf_ChangeManager::Get().DidChangeField(_self, path, fieldName,
                                          std::move(oldValue), newValue);

  _data->Set(path, fieldName, value);
}

template void SdfLayer::_PrimSetField(const SdfPath &, const TfToken &,
...
void SdfLayer::_PrimPushChild(const SdfPath &parentPath,
                              const TfToken &fieldName, const T &value,
                              bool useDelegate) {
...
template void SdfLayer::_PrimPushChild(const SdfPath &, const TfToken &,
                                       const TfToken &, bool);
template void SdfLayer::_PrimPushChild(const SdfPath &, const TfToken &,
                                       const SdfPath &, bool);

template <class T>
void SdfLayer::_PrimPopChild(const SdfPath &parentPath,
                             const TfToken &fieldName, bool useDelegate) {
  if (useDelegate && TF_VERIFY(_stateDelegate)) {
...
template void SdfLayer::_PrimPopChild<TfToken>(const SdfPath &, const TfToken &,
                                               bool);
template void SdfLayer::_PrimPopChild<SdfPath>(const SdfPath &, const TfToken &,
                                               bool);
...
void SdfLayer::_PrimSetFieldDictValueByKey(const SdfPath &path,
                                           ...
                                           bool useDelegate) {
  if (useDelegate && TF_VERIFY(_stateDelegate)) {
    _stateDelegate->SetFieldDictValueByKey(path, fieldName, keyPath, value,
                                           oldValuePtr);
    return;
  }
...
void SdfLayer::_PrimMoveSpec(const SdfPath &oldPath, const SdfPath &newPath,
                             bool useDelegate) {
  if (useDelegate && TF_VERIFY(_stateDelegate)) {
  ...
  Traverse(oldPath, std::bind(_MoveSpecInternal, _data, &_idRegistry, ph::_1,
                              oldPath, newPath));
}
...
bool SdfLayer::_CreateSpec(const SdfPath &path, SdfSpecType specType,
                           bool inert) {
  if (specType == SdfSpecTypeUnknown) {
    return false;
  }
...
void SdfLayer::_TraverseChildren(const SdfPath &path,
                                 const TraversalFunction &func) {
...
void SdfLayer::_PrimDeleteSpec(const SdfPath &path, bool inert,
                               bool useDelegate) {
  if (useDelegate && TF_VERIFY(_stateDelegate)) {
...
void SdfLayer::_PrimCreateSpec(const SdfPath &path, SdfSpecType specType,
                               bool inert, bool useDelegate) {
  if (useDelegate && TF_VERIFY(_stateDelegate)) {
...
bool SdfLayer::_IsInert(const SdfPath &path, bool ignoreChildren,
  ...
  if (path.IsPrimVariantSelectionPath() &&
    ...
    if (HasField(path, SdfChildrenKeys->VariantChildren, &variants)) {
      for (const TfToken &variant : variants) {
        if (!_IsInertSubtree(parentPath.AppendVariantSelection(
...
bool SdfLayer::_WriteToFile(const string &newFileName, const string &comment,
                            SdfFileFormatConstPtr fileFormat,
  ...
  if (!fileFormat->SupportsWriting()) {
    TF_CODING_ERROR("Cannot save layer @%s@: %s file format does not"
                    "support writing",
                    newFileName.c_str(), fileFormat->GetFormatId().GetText());
    return false;
  }
...
bool SdfLayer::Export(const string &newFileName, const string &comment,
                      const FileFormatArguments &args) const {
  return _WriteToFile(
      newFileName, comment,
...
bool SdfLayer::Save(bool force) const { return _Save(force); }
...
bool SdfLayer::_Save(bool force) const {
  ...
  if (!_WriteToFile(path, std::string(), GetFileFormat(),
                    GetFileFormatArguments()))
    return false;
