...
SdfPath SdfLayer::GetDefaultPrimAsPath() const
{
  ...
  return path.IsPrimPath() ?
             path.IsAbsolutePath() ? path : path.MakeAbsolutePath(SdfPath::AbsoluteRootPath()) :
             SdfPath();
}
...
bool SdfLayer::DetachedLayerRules::IsIncluded(const std::string &identifier) const
{
  ...
  const bool included = _includeAll || std::find_if(_include.begin(),
                                                    _include.end(),
  ...
  const bool excluded = std::find_if(
                            _exclude.begin(), _exclude.end(), [&layerPath](const std::string &s) {
                              return TfStringContains(layerPath, s);
                            }) != _exclude.end();
...
static std::optional<RefOrPayloadType> _UpdateRefOrPayloadPath(
    const string &oldLayerPath, const string &newLayerPath, const RefOrPayloadType &refOrPayload)
{
                                                     ...
                                                     bool metadataOnly)
{
  ...
  // other layers.
  TF_VERIFY(_layerRegistry->Find(layer->GetIdentifier(), layer->GetResolvedPath()) == layer,
            "Could not find %s",
  ...
  if (!info.isAnonymous) {
    ...
    VtValue timestamp(
        ArGetResolver().GetModificationTimestamp(info.layerPath, info.resolvedLayerPath));
...
void SdfLayer::_SetData(const SdfAbstractDataPtr &newData, const SdfSchemaBase *newDataSchema)
{
  ...
  // Remove specs that no longer exist or whose required fields changed.
  {
    ...
    TF_REVERSE_FOR_ALL(i, specsToDelete.paths)
    {
      ...
      _PrimDeleteSpec(
          *i,
          _IsInert(*i, true /*ignoreChildren*/, true /* requiredFieldOnlyPropertiesAreInert */));
    }
  }
...
template void SdfLayer::_PrimSetField(
    const SdfPath &, const TfToken &, const VtValue &, VtValue *, bool);
template void SdfLayer::_PrimSetField(
    const SdfPath &, const TfToken &, const SdfAbstractDataConstValue &, VtValue *, bool);
