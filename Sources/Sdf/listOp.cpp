...
TF_REGISTRY_FUNCTION(TfType) {
  ...
  TfType::Define<SdfReferenceListOp>().Alias(TfType::GetRoot(),
                                             "SdfReferenceListOp");
  TfType::Define<SdfPayloadListOp>().Alias(TfType::GetRoot(),
                                           "SdfPayloadListOp");
  ...
  TfType::Define<SdfUnregisteredValueListOp>().Alias(
      TfType::GetRoot(), "SdfUnregisteredValueListOp");
...
template <typename T> bool SdfListOp<T>::HasItem(const T &item) const {
  ...
  return (std::find(_addedItems.begin(), _addedItems.end(), item) !=
          _addedItems.end()) ||
...
template <typename T>
void SdfListOp<T>::ApplyOperations(ItemVector *vec,
                                   const ApplyCallback &cb) const {
  if (!vec) {
    return;
  }
  ...
  if (IsExplicit()) {
    ...
    if (!cb && ((numToDelete + numToAdd + numToPrepend + numToAppend +
                 numToOrder) == 0)) {
    ...
    _ApplyMap search;
    for (typename _ApplyList::iterator i = result.begin(); i != result.end();
         ++i) {
      search[*i] = i;
    }
...
SdfListOp<T>::ApplyOperations(const SdfListOp<T> &inner) const {
  ...
  if (GetAddedItems().empty() && GetOrderedItems().empty()) {
    ...
    if (inner.GetAddedItems().empty() && inner.GetOrderedItems().empty()) {
      ...
      pre.insert(pre.begin(), GetPrependedItems().begin(),
      ...
      app.insert(app.end(), GetAppendedItems().begin(),
...
static inline void _InsertIfUnique(const ItemType &item, ListType *result,
                                   MapType *search) {
...
template <class ItemType, class ListType, class MapType>
static inline void _RemoveIfPresent(const ItemType &item, ListType *result,
                                    MapType *search) {
...
template <class T>
void SdfListOp<T>::_AddKeys(SdfListOpType op, const ApplyCallback &callback,
...
template <class T>
void SdfListOp<T>::_PrependKeys(SdfListOpType op, const ApplyCallback &callback,
                                _ApplyList *result, _ApplyMap *search) const {
...
template <class T>
void SdfListOp<T>::_AppendKeys(SdfListOpType op, const ApplyCallback &callback,
                               _ApplyList *result, _ApplyMap *search) const {
  const ItemVector &items = GetItems(op);
  if (callback) {
...
template <class T>
void SdfListOp<T>::_DeleteKeys(SdfListOpType op, const ApplyCallback &callback,
                               _ApplyList *result, _ApplyMap *search) const {
  TF_FOR_ALL(i, GetItems(op)) {
    if (callback) {
      if (boost::optional<T> item = callback(op, *i)) {
...
template <class T>
void SdfListOp<T>::_ReorderKeys(SdfListOpType op, const ApplyCallback &callback,
                                _ApplyList *result, _ApplyMap *search) const {
...
bool SdfListOp<T>::ModifyOperations(const ModifyCallback &callback,
  ...
  if (callback) {
    didModify |=
        _ModifyCallbackHelper(callback, &_explicitItems, removeDuplicates);
    didModify |=
        _ModifyCallbackHelper(callback, &_addedItems, removeDuplicates);
    didModify |=
        _ModifyCallbackHelper(callback, &_prependedItems, removeDuplicates);
    didModify |=
        _ModifyCallbackHelper(callback, &_appendedItems, removeDuplicates);
    didModify |=
        _ModifyCallbackHelper(callback, &_deletedItems, removeDuplicates);
    didModify |=
        _ModifyCallbackHelper(callback, &_orderedItems, removeDuplicates);
  }
...
template <typename T>
bool SdfListOp<T>::ReplaceOperations(const SdfListOpType op, size_t index,
                                     size_t n, const ItemVector &newItems) {
  ...
  if (index > itemVector.size()) {
    TF_CODING_ERROR("Invalid start index %zd (size is %zd)", index,
                    itemVector.size());
    return false;
  } else if (index + n > itemVector.size()) {
    TF_CODING_ERROR("Invalid end index %zd (size is %zd)", index + n - 1,
                    itemVector.size());
    return false;
  }
  ...
  if (n == newItems.size()) {
    ...
    itemVector.insert(itemVector.begin() + index, newItems.begin(),
                      newItems.end());
  }
...
template <typename T>
void SdfListOp<T>::ComposeOperations(const SdfListOp<T> &stronger,
                                     SdfListOpType op) {
...
template <typename T>
static void _StreamOutItems(std::ostream &out, const string &itemsName,
                            const std::vector<T> &items, bool *firstItems,
                            bool isExplicitList = false) {
  if (isExplicitList || !items.empty()) {
    out << (*firstItems ? "" : ", ") << itemsName << " Items: [";
    *firstItems = false;
    TF_FOR_ALL(it, items) { out << *it << (it.GetNext() ? ", " : ""); }
    out << "]";
  }
}
...
#define SDF_INSTANTIATE_LIST_OP(ValueType)                                     \
  ...
  template SDF_API std::ostream &operator<<(std::ostream &,                    \
                                            const SdfListOp<ValueType> &)
