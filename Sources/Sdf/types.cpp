...
TF_REGISTRY_FUNCTION(TfType) {
  ...
  // Other.
  TfType::Define<SdfTimeSampleMap>().Alias(TfType::GetRoot(),
                                           "SdfTimeSampleMap");
...
static void _AddToUnitsMaps(_UnitsInfo &info, const TfEnum &unit,
                            const string &unitName, double scale,
...
#define _ADD_UNIT_ENUM(r, category, elem)                                      \
  TF_ADD_ENUM_NAME(BOOST_PP_CAT(Sdf##category##Unit, _SDF_UNIT_TAG(elem)),     \
                   _SDF_UNIT_NAME(elem));

#define _REGISTRY_FUNCTION(r, unused, elem)                                    \
  TF_REGISTRY_FUNCTION_WITH_TAG(TfEnum, _SDF_UNITSLIST_CATEGORY(elem)) {       \
    BOOST_PP_SEQ_FOR_EACH(_ADD_UNIT_ENUM, _SDF_UNITSLIST_CATEGORY(elem),       \
...
#define _ADD_UNIT_TO_MAPS(r, category, elem)                                   \
  _AddToUnitsMaps(*info,                                                       \
                  ...
                  _SDF_UNIT_NAME(elem), _SDF_UNIT_SCALE(elem), #category);

#define _POPULATE_UNIT_MAPS(r, unused, elem)                                   \
  BOOST_PP_SEQ_FOR_EACH(_ADD_UNIT_TO_MAPS, _SDF_UNITSLIST_CATEGORY(elem),      \
                        _SDF_UNITSLIST_TUPLES(elem))
...
#define _REGISTRY_FUNCTION(r, unused, elem)                                    \
  TF_REGISTRY_FUNCTION_WITH_TAG(                                               \
      TfType, BOOST_PP_CAT(Type, _SDF_UNITSLIST_CATEGORY(elem))) {             \
    TfType::Define<_SDF_UNITSLIST_ENUM(elem)>();                               \
  }                                                                            \
  TF_REGISTRY_FUNCTION_WITH_TAG(                                               \
      VtValue, BOOST_PP_CAT(Value, _SDF_UNITSLIST_CATEGORY(elem))) {           \
...
static std::string _GetKeyPathText(std::vector<std::string> const &keyPath) {
  return keyPath.empty() ? std::string()
                         : TfStringPrintf(" under key '%s'",
                                          TfStringJoin(keyPath, ":").c_str());
}

// Add a human-readable error to errMsgs indicating that the passed value does
// not have a valid scene description datatype.
static void _AddInvalidTypeError(char const *prefix, VtValue const &value,
                                 std::vector<std::string> *errMsgs,
                                 std::vector<std::string> const *keyPath) {
  errMsgs->push_back(TfStringPrintf("%s%s%s is not a valid scene description "
                                    "datatype",
                                    prefix,
...
using _ValueVectorToVtArrayFn = bool (*)(VtValue *, std::vector<std::string> *,
...
// vector<VtValue>.
template <class T>
bool _ValueVectorToVtArray(VtValue *value, std::vector<std::string> *errMsgs,
                           std::vector<std::string> const *keyPath) {
...
static bool _ValueVectorToAnyVtArray(VtValue *value,
  ...
  if (SdfValueHasValidType(valVec.front())) {
    return _GetTypedValueVectorToVtArrayFn(valVec.front().GetType())(
        value, errMsgs, keyPath);
  } else {
    _AddInvalidTypeError("first vector/list element ", valVec.front(), errMsgs,
                         keyPath);
    *value = VtValue();
    return false;
  }
...
using _PySeqToVtArrayFn = bool (*)(VtValue *, std::vector<std::string> *,
                                   std::vector<std::string> const *);

template <class T>
bool _PySeqToVtArray(VtValue *value, std::vector<std::string> *errMsgs,
                     std::vector<std::string> const *keyPath) {
  using ElemType = T;
  bool allValid = true;
  TfPyLock lock;
  ...
  for (Py_ssize_t i = 0; i != len; ++i) {
    ...
    if (!h) {
      ...
      errMsgs->push_back(TfStringPrintf(
          "failed to obtain element %s from sequence%s", TfStringify(i).c_str(),
          _GetKeyPathText(*keyPath).c_str()));
      allValid = false;
    }
    ...
    if (!e.check()) {
      errMsgs->push_back(TfStringPrintf(
          ...
          "%s: %s%s to <%s>",
          TfStringify(i).c_str(),
          _GetDiagnosticStringForValue(boost::python::extract<VtValue>(h.get()))
              .c_str(),
          _GetKeyPathText(*keyPath).c_str(),
...
static bool _PyObjToAnyVtArray(VtValue *value,
  ...
  if (!PySequence_Check(obj.ptr())) {
    errMsgs->push_back(
        TfStringPrintf("cannot convert python object as sequence%s",
                       _GetKeyPathText(*keyPath).c_str()));
    *value = VtValue();
    return false;
  }
  ...
  if (SdfValueHasValidType(firstVal)) {
    return _GetTypedPySeqToVtArrayFn(firstVal.GetType())(value, errMsgs,
                                                         keyPath);
  } else {
...
static bool
_ConvertToValidMetadataDictValueInternal(VtValue *value,
                                         std::vector<std::string> *errMsgs,
                                         std::vector<std::string> *keyPath) {
  ...
  if (value->IsHolding<VtDictionary>()) {
    ...
    for (auto &kv : d) {
      ...
      allValid &= _ConvertToValidMetadataDictValueInternal(&kv.second, errMsgs,
                                                           keyPath);
      keyPath->pop_back();
    }
    value->UncheckedSwap(d);
...
bool SdfConvertToValidMetadataDictionary(VtDictionary *dict,
                                         std::string *errMsg) {
  std::vector<std::string> keyPath;
  std::vector<std::string> errMsgs;
  bool allValid = true;
...
std::ostream &VtStreamOut(const SdfVariantSelectionMap &varSelMap,
                          std::ostream &stream) {
...
TfToken Sdf_ValueTypeNamesType::GetSerializationName(
...
TfStaticData<const Sdf_ValueTypeNamesType, Sdf_ValueTypeNamesType::_Init>
    SdfValueTypeNames;
