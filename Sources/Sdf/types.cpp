...
#define _SDF_UNIT_MAX_UNITS_IMPL(seq) BOOST_PP_SEQ_SIZE(seq),
#define _SDF_UNIT_MAX_UNITS_OP(elem)                                           \
  _SDF_UNIT_MAX_UNITS_IMPL(TF_PP_TUPLE_ELEM(1, elem))

constexpr size_t _Sdf_UnitMaxUnits =
    std::max({_SDF_FOR_EACH_UNITS(_SDF_UNIT_MAX_UNITS_OP, _SDF_UNITS)
...
struct _UnitsInfo {
  ...
  std::array<std::array<std::string, _Sdf_UnitMaxUnits>, _Sdf_UnitNumTypes>
      _UnitNameTable;
...
#define _ADD_UNIT_ENUM(r, category, elem)                                      \
  TF_ADD_ENUM_NAME(TF_PP_CAT(Sdf##category##Unit, _SDF_UNIT_TAG(elem)),        \
                   _SDF_UNIT_NAME(elem));
...
#define _ADD_UNIT_TO_MAPS(r, category, elem)                                   \
  _AddToUnitsMaps(*info, TF_PP_CAT(Sdf##category##Unit, _SDF_UNIT_TAG(elem)),  \
...
#define _REGISTRY_FUNCTION(elem)                                               \
  TF_REGISTRY_FUNCTION_WITH_TAG(                                               \
      TfType, TF_PP_CAT(Type, _SDF_UNITSLIST_CATEGORY(elem))) {                \
    TfType::Define<_SDF_UNITSLIST_ENUM(elem)>();                               \
  }                                                                            \
  TF_REGISTRY_FUNCTION_WITH_TAG(                                               \
      VtValue, TF_PP_CAT(Value, _SDF_UNITSLIST_CATEGORY(elem))) {              \
