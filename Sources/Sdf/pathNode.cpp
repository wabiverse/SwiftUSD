...
template<class HashState, class T> inline void TfHashAppend(HashState &h, _ParentAnd<T> const &pat)
{
                                                ...
                                                const Args &...args)
{
  ...
  if (iresult.second ||
      (Table::NodeHandle::IsCounted && (Access::GetRefCount(iresult.first->second).fetch_add(1) &
    ...
    TfFunctionRef<bool()> isValid)
{
  return _FindOrCreate<Sdf_PrimVariantSelectionNode>(
      *_primVarSelNodes, isValid, parent, VariantSelectionType(variantSet, variant));
}
...
Sdf_PathPropNodeHandle Sdf_PathNode ::FindOrCreateRelationalAttribute(
    Sdf_PathNode const *parent, const TfToken &name, TfFunctionRef<bool()> isValid)
{
...
Sdf_PrimPathNode::~Sdf_PrimPathNode()
{
  _Remove(this,
          *_primNodes,
          Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()),
          _name);
}
