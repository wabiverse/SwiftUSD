...
struct Sdf_PathNodePrivateAccess {
  ...
  static inline typename Pool::Handle New(Sdf_PathNode const *parent,
                                          Args const &...args) {
...
inline bool operator==(_ParentAndRef<void> const &x,
                       _ParentAnd<void> const &y) {
  return x.parent == y.parent;
}
inline bool operator==(_ParentAnd<void> const &x,
                       _ParentAndRef<void> const &y) {
  return x.parent == y.parent;
}

inline size_t hash_value(const Sdf_PathNode *p) { return TfHash()(p); }
...
_FindOrCreate(Table &table, const TfFunctionRef<bool()> &isValid,
              const Sdf_PathNode *parent, const Args &...args) {
  ...
  if (iresult.second ||
      (Table::NodeHandle::IsCounted &&
       (Access::GetRefCount(iresult.first->second).fetch_add(1) &
  ...
  return
      typename Table::NodeHandle(iresult.first->second, /* add_ref = */ false);
}

template <class Table, class... Args>
inline void _Remove(const Sdf_PathNode *pathNode, Table &table,
...
Sdf_PathNode::FindOrCreatePrim(Sdf_PathNode const *parent, const TfToken &name,
...
Sdf_PathNode::FindOrCreatePrimProperty(Sdf_PathNode const *parent,
  ...
  return _FindOrCreate<Sdf_PrimPropertyPathNode>(*_primPropertyNodes, isValid,
                                                 nullptr, name);
}
...
Sdf_PathNode::FindOrCreateTarget(Sdf_PathNode const *parent,
                                 ...
                                 TfFunctionRef<bool()> isValid) {
  return _FindOrCreate<Sdf_TargetPathNode>(*_targetNodes, isValid, parent,
                                           targetPath);
}

Sdf_PathPropNodeHandle
Sdf_PathNode ::FindOrCreateRelationalAttribute(Sdf_PathNode const *parent,
                                               const TfToken &name,
                                               TfFunctionRef<bool()> isValid) {
  return _FindOrCreate<Sdf_RelationalAttributePathNode>(*_relAttrNodes, isValid,
                                                        parent, name);
}

Sdf_PathPropNodeHandle
Sdf_PathNode ::FindOrCreateMapper(Sdf_PathNode const *parent,
                                  SdfPath const &targetPath,
                                  TfFunctionRef<bool()> isValid) {
  return _FindOrCreate<Sdf_MapperPathNode>(*_mapperNodes, isValid, parent,
                                           targetPath);
}
...
Sdf_PathNode::FindOrCreateMapperArg(Sdf_PathNode const *parent,
                                    const TfToken &name,
                                    TfFunctionRef<bool()> isValid) {
  return _FindOrCreate<Sdf_MapperArgPathNode>(*_mapperArgNodes, isValid, parent,
                                              name);
}
...
Sdf_PathNode::FindOrCreateExpression(Sdf_PathNode const *parent,
                                     TfFunctionRef<bool()> isValid) {
  return _FindOrCreate<Sdf_ExpressionPathNode>(*_expressionNodes, isValid,
                                               parent);
}

Sdf_PathNode::Sdf_PathNode(bool isAbsolute)
    : _refCount(1), _elementCount(0), _nodeType(RootNode),
...
const TfToken &Sdf_PathNode::GetPathToken(Sdf_PathNode const *primPart,
  ...
  _pathTokenTable->insert(primAccessor,
                          std::make_pair(primPart, _PropToTokenTable()));
...
TfToken Sdf_PathNode::GetPathAsToken(Sdf_PathNode const *primPart,
                                     Sdf_PathNode const *propPart) {
...
TfToken Sdf_PathNode::_CreatePathToken(Sdf_PathNode const *primPart,
                                       Sdf_PathNode const *propPart) {
  _StringBuffer buf;
...
void Sdf_PathNode::_WriteTextToBuffer(Sdf_PathNode const *primPart,
  ...
  Sdf_PathNode const *const root =
      (primPart->IsAbsolutePath() ? Sdf_PathNode::GetAbsoluteRootNode()
...
char const *Sdf_PathNode::GetDebugText(Sdf_PathNode const *primPart,
                                       Sdf_PathNode const *propPart) {
  _DebugBuffer buf;
  _WriteTextToBuffer(primPart, propPart, buf);
  return buf.GetText();
...
static void _GatherChildrenFrom(Sdf_PathNode const *parent, Table const &table,
...
static void _Visit(Sdf_PathNode const *path, Sdf_Stats *stats) {
  ...
  TF_FOR_ALL(child, children) { _Visit(child->get(), stats); }
}
