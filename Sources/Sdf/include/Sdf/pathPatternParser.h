...
struct BracedPredExpr
    : if_must<one<'{'>, OptSpaced<SdfPredicateExpressionParser::PredExpr>, one<'}'>> {};

struct PrimPathWildCard
    : seq<plus<sor<identifier_other, one<'?', '*'>>>,
...
struct PrimPathPatternElem
    : if_then_else<PrimPathPatternElemText, opt<BracedPredExpr>, BracedPredExpr> {};

struct PropPathPatternElem
    : if_then_else<PropPathPatternElemText, opt<BracedPredExpr>, BracedPredExpr> {};
...
template<class Rule> struct PathPatternAction : nothing<Rule> {};

template<> struct PathPatternAction<AbsoluteStart> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
    builder.pattern.SetPrefix(SdfPath::AbsoluteRootPath());
  }
};

template<> struct PathPatternAction<PathPatStretch> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
...
template<> struct PathPatternAction<PrimPathPatternElemText> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
...
template<> struct PathPatternAction<PropPathPatternElemText> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
    builder.curElemText = in.string();
  }
...
template<> struct PathPatternAction<PrimPathPatternElem> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
...
template<> struct PathPatternAction<PropPathPatternElem> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
...
template<> struct PathPatternAction<ReflexiveRelative> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
...
template<> struct PathPatternAction<DotDot> {
  template<class Input> static void apply(Input const &in, PatternBuilder &builder)
  {
