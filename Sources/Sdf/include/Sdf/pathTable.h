...
template<class MappedType> class SdfPathTable {
 ...
 public:
  ...
  template<class ValType, class EntryPtr> class Iterator {
   public:
  ...
  SdfPathTable(SdfPathTable &&other)
      : _buckets(std::move(other._buckets)), _size(other._size), _mask(other._mask)
  {
    other._size = 0;
    other._mask = 0;
  }
  ...
  void ClearInParallel()
  {
    ...
    Sdf_VisitPathTableInParallel(
        reinterpret_cast<void **>(_buckets.data()), _buckets.size(), visitFn);
    _size = 0;
  }
  ...
  template<typename Callback> void ParallelForEach(Callback const &visitFn)
  {
    ...
    Sdf_VisitPathTableInParallel(
        reinterpret_cast<void **>(_buckets.data()), _buckets.size(), lambda);
  }
 ...
 private:
  ...
  {
    return _InsertInTableImpl(value.first,
                              [&value](_Entry *next) { return new _Entry(value, next); });
  }
