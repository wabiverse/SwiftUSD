...
SDF_API
void Sdf_VisitPathTableInParallel(void **, size_t,
...
private:
  ...
  struct _Entry {
    ...
    _Entry(value_type const &value, _Entry *n)
        : value(value), next(n), firstChild(nullptr),
          nextSiblingOrParent(nullptr, false) {}

    _Entry(value_type &&value, _Entry *n)
        : value(std::move(value)), next(n), firstChild(nullptr),
          nextSiblingOrParent(nullptr, false) {}
...
public:
  ...
  class Iterator
      : public boost::iterator_facade<Iterator<ValType, EntryPtr>, ValType,
  ...
  protected:
    ...
    inline void increment() {
      ...
      _entry =
          _entry->firstChild ? _entry->firstChild : GetNextSubtree()._entry;
    }
    ...
    for (const_iterator i = other.begin(), end = other.end(); i != end; ++i) {
      ...
      if (i._entry->nextSiblingOrParent.Get() &&
        ...
        j._entry->nextSiblingOrParent.Set(
            _InsertInTable(i._entry->nextSiblingOrParent.Get()->value)
                .first._entry,
