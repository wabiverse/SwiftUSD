...
public:
  ...
  static bool OpenParensIfNeeded(Sdf_TextOutput &out, bool didParens,
                                 bool multiLine);

  static void CloseParensIfNeeded(Sdf_TextOutput &out, size_t indent,
                                  bool didParens, bool multiLine);

  static void WriteQuotedString(Sdf_TextOutput &out, size_t indent,
                                const std::string &str);

  static void WriteAssetPath(Sdf_TextOutput &out, size_t indent,
                             const std::string &str);

  static void WriteDefaultValue(Sdf_TextOutput &out, size_t indent,
                                VtValue value);

  static void WriteSdfPath(Sdf_TextOutput &out, size_t indent,
                           const SdfPath &path);

  static bool WriteNameVector(Sdf_TextOutput &out, size_t indent,
                              const std::vector<std::string> &vec);
  static bool WriteNameVector(Sdf_TextOutput &out, size_t indent,
                              const std::vector<TfToken> &vec);

  static bool WriteTimeSamples(Sdf_TextOutput &out, size_t indent,
                               const SdfPropertySpec &);

  static bool WriteRelocates(Sdf_TextOutput &out, size_t indent, bool multiLine,
                             const SdfRelocatesMap &reloMap);

  static void WriteDictionary(Sdf_TextOutput &out, size_t indent,
                              bool multiLine, const VtDictionary &dictionary,
                              bool stringValuesOnly = false);

  template <class T>
  static void WriteListOp(Sdf_TextOutput &out, size_t indent,
                          const TfToken &fieldName, const SdfListOp<T> &listOp);

  static void WriteLayerOffset(Sdf_TextOutput &out, size_t indent,
                               bool multiline, const SdfLayerOffset &offset);
...
private:
  ...
  static void _WriteDictionary(Sdf_TextOutput &out, size_t indent,
                               bool multiLine, _OrderedDictionary &dictionary,
...
static inline bool Sdf_WritePrim(const SdfPrimSpec &prim, Sdf_TextOutput &out,
                                 size_t indent);
...
static bool Sdf_WriteVariantSet(const SdfVariantSetSpec &spec,
                                Sdf_TextOutput &out, size_t indent);
...
static bool Sdf_WritePrimPreamble(const SdfPrimSpec &prim, Sdf_TextOutput &out,
                                  size_t indent) {
...
static bool Sdf_WriteIfListOp(Sdf_TextOutput &out, size_t indent,
                              const TfToken &field, const VtValue &value) {
  if (value.IsHolding<ListOpType>()) {
    Sdf_FileIOUtility::WriteListOp(out, indent, field,
                                   value.UncheckedGet<ListOpType>());
    return true;
  }
  return false;
}

static void Sdf_WriteSimpleField(Sdf_TextOutput &out, size_t indent,
  ...
  if (isUnregisteredValue) {
    ...
    if (boxedValue.IsHolding<SdfUnregisteredValueListOp>()) {
      ...
      if (boxedValue.IsHolding<VtDictionary>()) {
        Sdf_FileIOUtility::WriteDictionary(out, indent, true,
                                           boxedValue.Get<VtDictionary>());
...
static bool Sdf_WritePrimMetadata(const SdfPrimSpec &prim, Sdf_TextOutput &out,
                                  size_t indent) {
  ...
  for (TfTokenVector::const_iterator fieldIt = fields.begin();
    ...
    if (field == SdfFieldKeys->Documentation) {
      ...
      const VtValue v = prim.GetField(field);
      if (!Sdf_WriteIfListOp<SdfReferenceListOp>(out, indent + 1,
                                                 TfToken("references"), v)) {
        TF_CODING_ERROR("'%s' field holding unexpected type '%s'",
                        field.GetText(), v.GetTypeName().c_str());
      }
      ...
      if (variantSetNameList.IsExplicit()) {
        // Explicit list
        SdfVariantSetNamesProxy::ListProxy setNames =
            variantSetNameList.GetExplicitItems();
        ...
        // List operations
        SdfVariantSetNamesProxy::ListProxy setNames =
            variantSetNameList.GetDeletedItems();
        if (!setNames.empty()) {
      ...
      const VtValue v = prim.GetField(field);
      if (!Sdf_WriteIfListOp<SdfPathListOp>(out, indent + 1,
                                            TfToken("inherits"), v)) {
        TF_CODING_ERROR("'%s' field holding unexpected type '%s'",
                        field.GetText(), v.GetTypeName().c_str());
      }
      ...
      Sdf_FileIOUtility::WriteRelocates(out, indent + 1, multiLine,
                                        finalRelocates);
...
static bool Sdf_WritePrimProperties(const SdfPrimSpec &prim,
                                    Sdf_TextOutput &out, size_t indent) {
  ...
  for (const SdfPropertySpecHandle &specHandle : properties) {
    ...
    if (specType == SdfSpecTypeAttribute) {
      Sdf_WriteAttribute(
          Sdf_CastAccess::CastSpec<SdfAttributeSpec, SdfPropertySpec>(spec),
          out, indent + 1);
    } else {
      Sdf_WriteRelationship(
          Sdf_CastAccess::CastSpec<SdfRelationshipSpec, SdfPropertySpec>(spec),
          out, indent + 1);
    }
  }
...
static bool Sdf_WritePrimChildren(const SdfPrimSpec &prim, Sdf_TextOutput &out,
                                  size_t indent) {
  bool newline = false;
...
static bool Sdf_WritePrimVariantSets(const SdfPrimSpec &prim,
                                     Sdf_TextOutput &out, size_t indent) {
...
static bool Sdf_WritePrimBody(const SdfPrimSpec &prim, Sdf_TextOutput &out,
                              size_t indent) {
...
static bool Sdf_WriteConnectionStatement(
    Sdf_TextOutput &out, size_t indent,
    ...
    const std::string &nameStr, const SdfAttributeSpec *attrOwner) {
  Sdf_FileIOUtility::Write(out, indent, "%s%s%s %s.connect = ", opStr.c_str(),
                           variabilityStr.c_str(), typeStr.c_str(),
                           nameStr.c_str());
...
static bool Sdf_WriteConnectionList(Sdf_TextOutput &out, size_t indent,
  ...
  if (connList.IsExplicit()) {
    ...
    Sdf_WriteConnectionStatement(out, indent, vec, "", variabilityStr, typeStr,
                                 nameStr, attrOwner);
  } else {
    SdfConnectionsProxy::ListProxy vec = connList.GetDeletedItems();
    if (!vec.empty()) {
      Sdf_WriteConnectionStatement(out, indent, vec, "delete ", variabilityStr,
                                   typeStr, nameStr, NULL);
    }
    vec = connList.GetAddedItems();
    if (!vec.empty()) {
      Sdf_WriteConnectionStatement(out, indent, vec, "add ", variabilityStr,
                                   typeStr, nameStr, attrOwner);
    }
    vec = connList.GetPrependedItems();
    if (!vec.empty()) {
      Sdf_WriteConnectionStatement(out, indent, vec, "prepend ", variabilityStr,
                                   typeStr, nameStr, attrOwner);
    }
    vec = connList.GetAppendedItems();
    if (!vec.empty()) {
      Sdf_WriteConnectionStatement(out, indent, vec, "append ", variabilityStr,
                                   typeStr, nameStr, attrOwner);
    }
    vec = connList.GetOrderedItems();
    if (!vec.empty()) {
      Sdf_WriteConnectionStatement(out, indent, vec, "reorder ", variabilityStr,
                                   typeStr, nameStr, NULL);
    }
  }
  return true;
...
static inline bool Sdf_WriteAttribute(const SdfAttributeSpec &attr,
  ...
  if (hasInfo || hasDefault || hasCustomDeclaration ||
    ...
    Sdf_FileIOUtility::Write(
        out, indent, "%s%s%s %s", (hasCustomDeclaration ? "custom " : ""),
    ...
    for (TfTokenVector::const_iterator fieldIt = fields.begin();
      ...
      if (field == SdfFieldKeys->Documentation) {
        ...
        Sdf_FileIOUtility::Write(
            out, multiLine ? indent + 1 : 0, "permission = %s%s",
        ...
        Sdf_FileIOUtility::Write(
            out, multiLine ? indent + 1 : 0, "displayUnit = %s%s",
  ...
  if (hasConnections) {
    Sdf_WriteConnectionList(out, indent, attr.GetConnectionPathList(),
                            variabilityStr, typeName, attr.GetName(), &attr);
  }
...
static bool
Sdf_WriteRelationshipTargetList(const SdfRelationshipSpec &rel,
...
static inline bool Sdf_WriteRelationship(const SdfRelationshipSpec &rel,
  ...
  if (hasTargets) {
    ...
    hasExplicitTargets =
        targetPathList.IsExplicit() && targetPathList.HasKeys();
  ...
  std::string varyingStr =
      isVarying ? "varying " : ""; // the space in "varying " is required...
  ...
  if (hasInfo || (hasTargets && hasExplicitTargets) ||
    ...
    if (hasCustom) {
      ...
      Sdf_FileIOUtility::Write(out, indent, "%srel %s", varyingStr.c_str(),
                               rel.GetName().c_str());
    }

    if (hasTargets && hasExplicitTargets) {
      SdfTargetsProxy targetPathList = rel.GetTargetPathList();
      SdfTargetsProxy::ListProxy targetPaths =
          targetPathList.GetExplicitItems();
      if (targetPaths.size() == 0) {
        Sdf_FileIOUtility::Write(out, 0, " = None");
      } else {
        // Write explicit targets
        Sdf_WriteRelationshipTargetList(rel, targetPaths, out, indent,
                                        Sdf_WriteFlagAttributes |
                                            Sdf_WriteFlagNoLastNewline);
      }
    }
  ...
  if (hasTargets && hasTargetListOps) {
    ...
    if (!targetPaths.empty()) {
      Sdf_FileIOUtility::Write(out, indent, "delete %srel %s",
                               varyingStr.c_str(), rel.GetName().c_str());
      Sdf_WriteRelationshipTargetList(rel, targetPaths, out, indent,
                                      Sdf_WriteFlagDefault);
    }

    // Write added targets
    targetPaths = targetPathList.GetAddedItems();
    if (!targetPaths.empty()) {
      Sdf_FileIOUtility::Write(out, indent, "add %srel %s", varyingStr.c_str(),
                               rel.GetName().c_str());
      Sdf_WriteRelationshipTargetList(rel, targetPaths, out, indent,
                                      Sdf_WriteFlagAttributes);
    }
  ...
  if (hasDefaultValue) {
    ...
    if (!value.IsEmpty()) {
      Sdf_FileIOUtility::Write(out, indent,
                               "%srel %s.default = ", varyingStr.c_str(),
                               rel.GetName().c_str());
