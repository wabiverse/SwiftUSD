...
public:
  ...
  Sdf_VectorListEditor(const SdfSpecHandle &owner, const TfToken &field,
                       SdfListOpType op,
  ...
  virtual void ApplyList(SdfListOpType op,
...
template <class TP, class FST>
bool Sdf_VectorListEditor<TP, FST>::ReplaceEdits(
  ...
  if (!fieldListOp.ReplaceOperations(op, index, n,
                                     _GetTypePolicy().Canonicalize(elems))) {
    return false;
  }
...
template <class TP, class FST>
void Sdf_VectorListEditor<TP, FST>::ApplyList(SdfListOpType op,
