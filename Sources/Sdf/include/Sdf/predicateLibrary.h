...
public:
  ...
  constexpr SdfPredicateFunctionResult() : _value(false), _constancy(MayVary) {}

  /// Construct with \p value and \p constancy.
  explicit SdfPredicateFunctionResult(bool value, Constancy constancy = MayVary)
...
private:
  ...
  static bool _CheckNamesAndDefaultsWithSignature(
    ...
    static_assert(std::is_convertible<typename Traits::ReturnType, bool>::value,
                  "");
  ...
  _TryBindArgs(ParamsTuple &params,
    ...
    // _TryBindOne N times.
    int unused[] = {0, (_TryBindOne(I, N, std::get<I>(params), bound, args,
  ...
  template <class Fn> struct _OverloadBinder : _OverloadBinderBase {
    ...
    TryCreate(Fn &&fn, NamesAndDefaults const &nd) {
      auto ret = std::unique_ptr<_OverloadBinder>(new _OverloadBinder(fn, nd));
  ...
  private:
    ...
    std::function<SdfPredicateFunctionResult(DomainType)> _Bind(
