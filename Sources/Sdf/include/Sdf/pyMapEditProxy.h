...
private:
  ...
  public:
    _Iterator(const boost::python::object &object)
        : _object(object), _owner(boost::python::extract<const Type &>(object)),
          _cur(_owner.begin()), _end(_owner.end()) {
      // Do nothing
    }
  ...
  static std::string _GetName() {
    std::string name =
        "MapEditProxy_" + ArchGetDemangled<typename Type::Type>();
  ...
  static void _Update(Type &x, const std::vector<pair_type> &values) {
    ...
    TF_FOR_ALL(i, values) { x[i->first] = i->second; }
  }
  ...
  static void _UpdateList(Type &x, const boost::python::list &pairs) {
    ...
    for (int i = 0, n = len(pairs); i != n; ++i) {
      values.push_back(pair_type(extract<key_type>(pairs[i][0])(),
