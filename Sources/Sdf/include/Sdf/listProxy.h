...
/// insertion sequence).
///
template<class _TypePolicy> class SdfListProxy {
 public:
 ...
 private:
  ...
  template<class Owner, class GetItem> class _Iterator {
    class _PtrProxy {
     public:
     ...
     private:
      ...
      explicit _PtrProxy(
          std::add_const_t<std::add_lvalue_reference_t<typename GetItem::result_type>> result)
          : _result(result)
      {
      }
...
template<typename T> struct Tf_ShouldIterateOverCopy<SdfListProxy<T>> : std::true_type {};
