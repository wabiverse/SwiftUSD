...
static void _GetFieldNames(const SdfLayerHandle &layer, const SdfPath &path,
...
static void _ProcessChildren(
    const TfToken &childrenField, const VtValue &srcChildrenValue,
    const VtValue &dstChildrenValue, const SdfLayerHandle &srcLayer,
    const SdfPath &srcPath, bool childrenInSrc, const SdfLayerHandle &dstLayer,
    const SdfPath &dstPath, bool childrenInDst, _CopyStack *copyStack) {
  ...
  if (childrenInDst) {
    ...
    for (const auto &oldDstChild :
      ...
      if (std::find(dstChildren.begin(), dstChildren.end(), oldDstChild) ==
          dstChildren.end()) {
...
static void _ProcessChildField(
    const TfToken &childField, const SdfLayerHandle &srcLayer,
    const SdfPath &srcPath, bool childrenInSrc, const SdfLayerHandle &dstLayer,
    const SdfPath &dstPath, bool childrenInDst,
  ...
  if (!shouldCopyChildren(childField, srcLayer, srcPath, childrenInSrc,
                          dstLayer, dstPath, childrenInDst, &srcChildrenToCopy,
                          &dstChildrenToCopy)) {
    return;
  }
  ...
  if (childField == SdfChildrenKeys->ConnectionChildren) {
    _ProcessChildren<Sdf_AttributeConnectionChildPolicy>(
        childField, srcChildren, dstChildren, srcLayer, srcPath, childrenInSrc,
...
template <class ChildPolicy>
static void _DoAddNewSpec(const SdfLayerHandle &destLayer,
                          const _SpecDataEntry &specData) {
  Sdf_ChildrenUtils<ChildPolicy>::CreateSpec(destLayer, specData.dstPath,
                                             specData.specType);
}

static void _DoAddNewPrimSpec(const SdfLayerHandle &destLayer,
                              const _SpecDataEntry &specData) {
...
template <class ChildPolicy>
static void _DoAddNewPropertySpec(const SdfLayerHandle &destLayer,
                                  const _SpecDataEntry &specData) {
...
static void _AddNewSpecToLayer(const SdfLayerHandle &destLayer,
                               const _SpecDataEntry &specData) {
...
template <class ChildPolicy>
static void _DoRemoveSpec(const SdfLayerHandle &dstLayer,
                          const SdfPath &dstPath) {
  Sdf_ChildrenUtils<ChildPolicy>::RemoveChild(
      dstLayer, ChildPolicy::GetParentPath(dstPath),
      ChildPolicy::GetFieldValue(dstPath));
}

static void _RemoveSpecFromLayer(const SdfLayerHandle &dstLayer,
                                 const SdfPath &dstPath) {
...
static void _AddFieldValueToCopy(SdfSpecType specType, const TfToken &field,
  ...
  if (shouldCopyValue(specType, field, srcLayer, srcPath, fieldInSrc, dstLayer,
                      dstPath, fieldInDst, &value)) {
  ...
  while (!copyStack.empty()) {
    ...
    if (copyingPrimToVariant || copyingVariantToPrim) {
      ...
      copyEntry.dataToCopy.erase(
          std::remove_if(copyEntry.dataToCopy.begin(),
      ...
      if (copyingPrimToVariant) {
        ...
        _ForEachField(
            srcFields, dstFields,
            [&](const TfToken &field, bool fieldInSrc, bool fieldInDst) {
              _AddFieldValueToCopy(specType, field, srcLayer, srcPrimPath,
                                   fieldInSrc, dstLayer, toCopy.dstPath,
                                   fieldInDst, shouldCopyValueFn,
                                   &copyEntry.dataToCopy);
            });
    ...
    for (const _FieldValuePair &fieldValue : copyEntry.dataToCopy) {
      if (fieldValue.second.IsHolding<SdfCopySpecsValueEdit>()) {
        ...
        dstLayer->SetField(copyEntry.dstPath, fieldValue.first,
                           fieldValue.second);
      }
    }
    ...
    _ForEachField(srcChildrenFields, dstChildrenFields,
                  [&](const TfToken &field, bool fieldInSrc, bool fieldInDst) {
                    _ProcessChildField(field, srcLayer, toCopy.srcPath,
                                       fieldInSrc, dstLayer, toCopy.dstPath,
                                       fieldInDst, shouldCopyChildrenFn,
                                       &copyStack);
                  });
  }
...
template <class T>
static T _FixInternalSubrootPaths(const T &ref, const SdfPath &srcPrefix,
  ...
  if (fieldInSrc) {
    if (field == SdfFieldKeys->ConnectionPaths ||
      ...
      if (srcLayer->HasField(srcPath, field, &refListOp)) {
        ...
        refListOp.ModifyOperations(std::bind(
            &_FixInternalSubrootPaths<SdfReference>, std::placeholders::_1,
      ...
      if (srcLayer->HasField(srcPath, field, &payloadListOp)) {
        ...
        payloadListOp.ModifyOperations(std::bind(
            &_FixInternalSubrootPaths<SdfPayload>, std::placeholders::_1,
            std::cref(srcPrefix), std::cref(dstPrefix)));
...
bool SdfCopySpec(const SdfLayerHandle &srcLayer, const SdfPath &srcPath,
  ...
  return SdfCopySpec(srcLayer, srcPath, dstLayer, dstPath,
                     /* shouldCopyValueFn = */
                     std::bind(SdfShouldCopyValue, std::cref(srcPath),
                               std::cref(dstPath), ph::_1, ph::_2, ph::_3,
                               ph::_4, ph::_5, ph::_6, ph::_7, ph::_8, ph::_9),
                     /* shouldCopyChildrenFn = */
                     std::bind(SdfShouldCopyChildren, std::cref(srcPath),
                               std::cref(dstPath), ph::_1, ph::_2, ph::_3,
                               ph::_4, ph::_5, ph::_6, ph::_7, ph::_8, ph::_9));
}
