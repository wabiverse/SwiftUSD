...
SdfPayload::SdfPayload(const std::string &assetPath, const SdfPath &primPath,
...
std::ostream &operator<<(std::ostream &out, const SdfPayload &payload) {
  return out << "SdfPayload(" << payload.GetAssetPath() << ", "
             << payload.GetPrimPath() << ", " << payload.GetLayerOffset()
             << ")";
}
