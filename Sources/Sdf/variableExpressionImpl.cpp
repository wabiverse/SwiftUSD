...
template<class T>
using IsSupportedListType = std::integral_constant<bool,
...
template<class... Results> static std::vector<std::string> _CombineErrors(Results *...results)
{
...
std::pair<EvalResult, bool> EvalContext::GetVariable(const std::string &var)
{
  ...
  if (std::find(_variableStack.begin(), _variableStack.end(), var) != _variableStack.end()) {
    ...
    return {EvalResult::Error({TfStringPrintf(
                ...
                TfStringJoin(formattedVars.begin(), formattedVars.end(), ", ").c_str(),
                var.c_str())}),
            true};
  }
  ...
  if (GetValueType(*value) == ValueType::Unknown) {
    return {EvalResult::Error({TfStringPrintf("Variable '%s' has unsupported type %s",
                                              var.c_str(),
  ...
  if (value->IsHolding<std::string>()) {
    ...
    if (Sdf_IsVariableExpression(strValue)) {
      ...
      if (subExpr.expression) {
        ...
        TfScoped<> popStack([this]() { _variableStack.pop_back(); });
...
template<class T> ConstantNode<T>::ConstantNode(T value) : _value(value) {}

template<class T> EvalResult ConstantNode<T>::Evaluate(EvalContext *ctx) const
{
 ...
 public:
  ...
  typename std::enable_if_t<IsSupportedScalarType<T>::value, bool> operator()(T v)
  {
    if (list.IsEmpty()) {
      ...
      list.UncheckedMutate<VtArray<T>>([&v](VtArray<T> &arr) { arr.push_back(std::move(v)); });
      return true;
    }
  ...
  for (size_t i = 0; i < _elements.size(); ++i) {
    ...
    if (!VtVisitValue(r.value, visitor)) {
      errors.push_back(TfStringPrintf("Unexpected value of type %s in list at element %zu",
                                      GetValueTypeName(r.value).c_str(),
                                      i));
    }
  }
...
template<class FunctionNode> static std::string _FormatFunctionError(const std::string &err)
{
                             ...
                             const std::unique_ptr<Node> &elseValue)
{
  ...
  if (elseValue) {
    const bool valuesHaveDifferentTypes = ifValueResult.value.GetType() !=
                                          elseValueResult.value.GetType();
    const bool neitherValueIsNone = !ifValueResult.value.IsEmpty() &&
                                    !elseValueResult.value.IsEmpty();
...
If3Node::If3Node(std::unique_ptr<Node> &&condition,
...
template<class T, template<typename> class Comparator> EvalResult _Compare(const T &x, const T &y)
{
...
template<> EvalResult _Compare<std::equal_to>(const VtValue &x, const VtValue &y)
{
  return EvalResult::Value(true);
}

template<> EvalResult _Compare<std::not_equal_to>(const VtValue &x, const VtValue &y)
{
  return EvalResult::Value(false);
}

template<template<typename> class Comparator> class _ComparisonVisitor {
 public:
  ...
  {
    return EvalResult::Error(
        {_FormatFunctionError<ComparisonNode<Comparator>>("Unsupported type for comparison")});
  }
...
EvalResult ComparisonNode<Comparator>::Evaluate(EvalContext *ctx) const
{
  ...
  if (x.value.GetType() != y.value.GetType()) {
    return EvalResult::Error({_FormatFunctionError<ComparisonNode<Comparator>>(
        TfStringPrintf("Cannot compare values of type %s and %s",
...
#define DEFINE_COMPARISON_NODE(ComparatorType, FunctionName) \
  ...
  template<> const char *ComparisonNode<ComparatorType>::GetFunctionName() \
  { \
    return FunctionName; \
  }
...
EvalResult LogicalNode<Operator>::Evaluate(EvalContext *ctx) const
{
  ...
  for (size_t i = 0; i < _conditions.size(); ++i) {
    ...
    if (_CollectErrors(&errors, &condition)) {
      ...
      errors.push_back(_FormatFunctionError<LogicalNode<Operator>>(TfStringPrintf(
          "Invalid type %s for argument %zu", GetValueTypeName(condition.value).c_str(), i)));
    }
    else {
      if (result.IsEmpty()) {
        ...
        result = Operator<bool>()(result.UncheckedGet<bool>(),
                                  condition.value.UncheckedGet<bool>());
      }
    }
  };

  return errors.empty() ? EvalResult::Value(std::move(result)) :
                          EvalResult::Error(std::move(errors));
}

#define DEFINE_LOGICAL_NODE(LogicType, FunctionName) \
  template class LogicalNode<LogicType>; \
  template<> const char *LogicalNode<LogicType>::GetFunctionName() \
  { \
    return FunctionName; \
  }
...
EvalResult LogicalNotNode::Evaluate(EvalContext *ctx) const
{
  ...
  if (!condition.errors.empty()) {
    ...
    return EvalResult::Error({_FormatFunctionError<LogicalNotNode>(TfStringPrintf(
        "Invalid type %s for argument", GetValueTypeName(condition.value).c_str()))});
  }
...
class _ContainsVisitor {
 public:
  ...
  std::enable_if_t<IsSupportedListType<ListType>::value, EvalResult> operator()(
      const ListType &searchIn) const
  {
    ...
    return EvalResult::Value(std::find(searchIn.begin(),
                                       searchIn.end(),
...
class _AtVisitor {
 ...
 private:
  static EvalResult _Error(const std::string &err)
  {
...
EvalResult DefinedNode::Evaluate(EvalContext *ctx) const
{
  ...
  for (size_t i = 0; i < _nodes.size(); ++i) {
    ...
    if (_CollectErrors(&errors, &varName)) {
      ...
      errors.push_back(_FormatFunctionError<DefinedNode>(TfStringPrintf(
          "Invalid type %s for argument %zu", GetValueTypeName(varName.value).c_str(), i)));
    }
    else {
