...
Sdf_Children<ChildPolicy>::Sdf_Children(const Sdf_Children<ChildPolicy> &other)
    : _layer(other._layer), _parentPath(other._parentPath),
      _childrenKey(other._childrenKey), _keyPolicy(other._keyPolicy),
...
Sdf_Children<ChildPolicy>::Sdf_Children(const SdfLayerHandle &layer,
    ...
    : _layer(layer), _parentPath(parentPath), _childrenKey(childrenKey),
      _keyPolicy(keyPolicy), _childNamesValid(false) {}
...
template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::IsEqualTo(
    const Sdf_Children<ChildPolicy> &other) const {
...
template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::Copy(const std::vector<ValueType> &values,
  ...
  return Sdf_ChildrenUtils<ChildPolicy>::SetChildren(_layer, _parentPath,
                                                     values);
}

template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::Insert(const ValueType &value, size_t index,
                                       const std::string &type) {
  _childNamesValid = false;
...
template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::Erase(const KeyType &key,
                                      const std::string &type) {
  _childNamesValid = false;
  ...
  return Sdf_ChildrenUtils<ChildPolicy>::RemoveChild(_layer, _parentPath,
                                                     expectedKey);
}
...
void Sdf_Children<ChildPolicy>::_UpdateChildNames() const {
  ...
  if (_layer) {
    _childNames =
        _layer->GetFieldAs<std::vector<FieldType>>(_parentPath, _childrenKey);
  } else {
    _childNames.clear();
  }
}
