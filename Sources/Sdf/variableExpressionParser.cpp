...
class ParserContext {
 public:
  ...
  {
    if (CreatorType *existingCreator = dynamic_cast<CreatorType *>(_nodeCreator.get())) {
...
struct ExpressionBody : sor<Variable, DoubleQuotedString, SingleQuotedString> {};
...
Sdf_VariableExpressionParserResult Sdf_ParseVariableExpression(const std::string &expr)
{
  ...
  try {
    const bool parseSuccess = TfDebug::IsEnabled(SDF_VARIABLE_EXPRESSION_PARSING) ?
                                  pegtl::parse<Expression, Action, pegtl::trace<Errors>::control>(
                                      in, context) :
    ...
    return {
        ...
        {TfStringPrintf("%s at character %zu",
                        e.what(),
                        e.positions.empty() ? expr.size() : e.positions.front().byte_in_line)}};
  }
