...
struct QuotedStringBody
    : star<sor<QuotedStringVariable, QuotedStringChars<QuoteChar>>> {};

template <char QuoteChar>
struct QuotedString
    : if_must<QuotedStringStart<QuoteChar>, QuotedStringBody<QuoteChar>,
...
Sdf_ParseVariableExpression(const std::string &expr) {
  ...
  try {
    ...
    return {nullptr,
            ...
            {TfStringPrintf("%s at character %zu", e.what(),
                            e.positions.empty()
                                ? expr.size()
                                : e.positions.front().byte_in_line)}};
  }
