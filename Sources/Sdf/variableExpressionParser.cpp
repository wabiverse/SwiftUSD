...
template<class Type> class ConstantNodeCreator : public NodeCreator {
 public:
  std::unique_ptr<Impl::Node> CreateNode(std::string *errMsg) override
  {
...
class FunctionNodeCreator : public NodeCreator {
 ...
 private:
                                          ...
                                          std::tuple<> *)
  {
    if (*matchedFunctionName) {
      *errMsg = TfStringPrintf(
          "Function '%s' does not take %zu arguments.", functionName.c_str(), functionArgs.size());
    }
    else {
  ...
  std::enable_if_t<NodeType::IsVariadic, std::unique_ptr<Impl::Node>> _CreateNodeHelper(
      std::string *errMsg)
  {
    if (functionArgs.size() < NodeType::MinNumArgs) {
      *errMsg = TfStringPrintf("Function '%s' requires at least %zu arguments.",
                               functionName.c_str(),
                               NodeType::MinNumArgs);
      return nullptr;
    }
...
class ParserContext {
 public:
  ...
  template<class CreatorType, class... Args> CreatorType *PushNodeCreator(const Args &...args)
  {
  ...
  template<class CreatorType> CreatorType *GetExistingNodeCreator()
  {
    if (!_nodeStack.empty()) {
  ...
  template<class CreatorType> CreatorType *GetNodeCreator()
  {
...
template<char QuoteChar> struct QuotedStringEscapedChar : one<'`', '$', '\\', QuoteChar> {};
...
template<class Base> struct FunctionArgumentWrapper : public Base {};
...
template<char QuoteChar> struct Action<QuotedStringChars<QuoteChar>> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
    context.GetNodeCreator<StringNodeCreator>()->parts.push_back(
        {in.string(), /* isVar = */ false});
  }
};

template<> struct Action<QuotedStringVariable::Name> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
    context.GetNodeCreator<StringNodeCreator>()->parts.push_back(
        {in.string(), /* isVar = */ true});
  }
};

template<char QuoteChar> struct Action<QuotedStringStart<QuoteChar>> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
...
template<> struct Action<Variable::Name> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
...
template<> struct Action<BooleanTrue> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
...
template<> struct Action<BooleanFalse> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
...
template<> struct Action<ListStart> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
...
template<> struct Action<ListElement> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
    std::string errMsg;
...
template<> struct Action<FunctionName> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
...
template<> struct Action<FunctionArgument> {
  template<typename ActionInput> static void apply(const ActionInput &in, ParserContext &context)
  {
    std::string errMsg;
...
template<typename Rule> struct Errors : public normal<Rule> {
...
#define MATCH_ERROR(rule, msg) template<> const std::string Errors<rule>::errorMsg = msg;
