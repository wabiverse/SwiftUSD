...
SdfPathExpression SdfPathExpression::MakeOp(Op op,
...
SdfPathExpression SdfPathExpression::ReplacePrefix(SdfPath const &oldPrefix,
    ...
    TfFunctionRef<SdfPathExpression(ExpressionReference const &)> resolve) &&
{
  ...
  auto patternIdent = [&stack](PathPattern const &pattern) { stack.push_back(MakeAtom(pattern)); };
...
std::string SdfPathExpression::GetText() const
{
  ...
  auto printPathPattern = [&result](PathPattern const &pattern) { result += pattern.GetText(); };
...
struct BracedPredExpr : if_must<one<'{'>, OptSpaced<EmbeddedPredExpr>, one<'}'>> {};

struct PathWildCard : plus<sor<identifier_other, one<'[', ']', '!', '-', '?', '*'>>> {};

struct PathPatternElemText : PathWildCard {};

struct PathPatternElem : if_then_else<PathPatternElemText, opt<BracedPredExpr>, BracedPredExpr> {};
...
template<> struct PathExprAction<AbsoluteStart> {
  template<class Input> static void apply(Input const &in, Sdf_PathExprBuilder &builder)
  {
...
template<> struct PathExprAction<EmbeddedPredExpr> {
  template<class Input> static void apply(Input const &in, Sdf_PathExprBuilder &builder)
  {
    builder.GetPatternBuilder().curPredExpr = SdfPredicateExpression(in.string());
  }
};
...
template<> struct PathExprAction<ReflexiveRelative> {
  template<class Input> static void apply(Input const &in, Sdf_PathExprBuilder &builder)
  {
    builder.GetPatternBuilder().pattern.SetPrefix(SdfPath::ReflexiveRelativePath());
  }
};
...
struct RelExpressionRefPath
    : seq<opt<DotDots>, if_then_else<one<'/'>, ExpressionRefPathAndName, ExpressionRefName>> {};
...
template<> struct PathExprAction<PathSetUnion> : PathExprOpAction<SdfPathExpression::Union> {};
template<>
                         ...
                         std::string *errMsgOut)
{
  ...
  try {
    parse<must<PathExpr, eolf>, PathExprAction /*, tracer*/>(
        string_input<>{inputStr, parseContext.empty() ? "<input>" : parseContext.c_str()},
        builder);
    if (expr) {
      *expr = builder.Finish();
    }
