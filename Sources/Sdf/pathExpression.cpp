...
SdfPathExpression const &SdfPathExpression::EveryDescendant() {
  static SdfPathExpression const *theEveryDescendant =
...
SdfPathExpression SdfPathExpression::MakeOp(Op op, SdfPathExpression &&left,
...
void SdfPathExpression::WalkWithOpStack(
...
SdfPathExpression::PathPattern::PathPattern()
    : _prefix(SdfPath::ReflexiveRelativePath()), _isProperty(false) {}
...
void SdfPathExpression::PathPattern::AppendChild(std::string const &text) {
  return AppendChild(text, {});
}

void SdfPathExpression::PathPattern ::AppendChild(
    std::string const &text, SdfPredicateExpression const &predExpr) {
  return AppendChild(text, SdfPredicateExpression(predExpr));
}

void SdfPathExpression::PathPattern::AppendChild(
    std::string const &text, SdfPredicateExpression &&predExpr) {
  if (_isProperty) {
...
void SdfPathExpression::PathPattern::AppendProperty(std::string const &text) {
  return AppendProperty(text, {});
}

void SdfPathExpression::PathPattern::AppendProperty(
    std::string const &text, SdfPredicateExpression const &predExpr) {
...
void SdfPathExpression::PathPattern::SetPrefix(SdfPath &&p) {
...
std::string SdfPathExpression::PathPattern::GetText() const {
  std::string result;
  ...
  for (size_t i = 0, end = _components.size(); i != end; ++i) {
    if (_components[i].text.empty() && _components[i].predicateIndex == -1) {
...
struct Sdf_PathExprBuilder {
  ...
  Sdf_PathPatternBuilder &GetPatternBuilder() { return _patternBuilder; }
...
private:
  struct _Stack {
    ...
    void PushExpressionRef(SdfPath &&path, std::string &&name) {
      exprStack.push_back(
          SdfPathExpression::MakeAtom({std::move(path), std::move(name)}));
    }
  ...
  private:
    void _Reduce() {
      ...
      if (op == SdfPathExpression::Complement) {
        ...
        exprStack.pop_back();
        exprStack.push_back(
            SdfPathExpression::MakeOp(op, std::move(left), std::move(right)));
      }
    }
...
struct PathPatternElems
    : seq<LookaheadList<PrimPathPatternElem, PathPatSep>, opt<PathPatStretch>,
...
struct PathPattern
    : sor<if_must<AbsoluteStart, AbsPathPattern>,
          seq<DotDots,
              if_then_else<PathPatSep, opt<PathPatternElems>, success>>,
          PathPatternElems,
...
template <class Rule> struct PathExprAction : nothing<Rule> {};

template <> struct PathExprAction<AbsoluteStart> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
    builder.GetPatternBuilder().pattern.SetPrefix(SdfPath::AbsoluteRootPath());
  }
};

template <> struct PathExprAction<PathPatStretch> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<EmbeddedPredExpr> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<PathPatternElemText> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<PrimPathPatternElem> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<PropPathPatternElem> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
    auto &pb = builder.GetPatternBuilder();
...
template <> struct PathExprAction<ReflexiveRelative> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<DotDot> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<PathPattern> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
    builder.PushPattern();
  }
...
struct RelExpressionRefPath
    : seq<opt<DotDots>,
          if_then_else<one<'/'>, ExpressionRefPathAndName, ExpressionRefName>> {
};
...
template <> struct PathExprAction<WeakerRef> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
template <> struct PathExprAction<ExpressionRefPath> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
...
struct PathExprAtom
    : sor<ExpressionReference, PathPattern,
...
struct PathOperator
    : sor<OptSpaced<PathSetUnion>, OptSpaced<PathSetIntersection>,
          OptSpaced<PathSetDifference>, PathSetImpliedUnion> {};

struct PathExpr : LookaheadList<PathFactor, PathOperator> {};

////////////////////////////////////////////////////////////////////////

template <SdfPathExpression::Op op> struct PathExprOpAction {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
    builder.PushOp(op);
  }
...
template <>
struct PathExprAction<PathSetComplement>
    : PathExprOpAction<SdfPathExpression::Complement> {};
template <>
struct PathExprAction<PathSetImpliedUnion>
    : PathExprOpAction<SdfPathExpression::ImpliedUnion> {};
template <>
struct PathExprAction<PathSetUnion>
    : PathExprOpAction<SdfPathExpression::Union> {};
template <>
struct PathExprAction<PathSetIntersection>
    : PathExprOpAction<SdfPathExpression::Intersection> {};
template <>
struct PathExprAction<PathSetDifference>
    : PathExprOpAction<SdfPathExpression::Difference> {};

template <> struct PathExprAction<PathExprOpenGroup> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
    builder.OpenGroup();
  }
...
template <> struct PathExprAction<PathExprCloseGroup> {
  template <class Input>
  static void apply(Input const &in, Sdf_PathExprBuilder &builder) {
    builder.CloseGroup();
  }
...
bool ParsePathExpression(std::string const &inputStr,
                         std::string const &parseContext,
                         SdfPathExpression *expr, std::string *errMsgOut) {
