  ...
  if (elementType != UsdGeomTokens->edge) {
    ...
    if (_GetEdgesFromPrim(geom, time, edgesOnPrim)) {
      ...
      for (const auto &subset : subsets) {
        ...
        std::copy(subsetEdges.begin(), subsetEdges.end(), std::back_inserter(edgesInFamily));
      }

      struct cmpEdge e;
      std::vector<GfVec2i> unassignedEdges;
      std::set_difference(edgesOnPrim.begin(),
                          edgesOnPrim.end(),
                          edgesInFamily.begin(),
                          edgesInFamily.end(),
                          std::inserter(unassignedEdges, unassignedEdges.begin()),
                          e);
                                   ...
                                   std::string *const reason)
{
  ...
  const std::vector<UsdGeomSubset> familySubsets = UsdGeomSubset::GetGeomSubsets(
      geom, elementType, familyName);
  ...
  const size_t earliestTimeElementCount = _GetElementCountAtTime(
      geom, elementType, UsdTimeCode::EarliestTime(), &isElementCountTimeVarying);
  ...
  for (const UsdTimeCode &t : allTimeCodes) {
    ...
    if (elementType != UsdGeomTokens->edge) {
        ...
        if (!familyIsRestricted) {
          ...
          for (const GfVec2i &edge : subsetIndices) {
            if (!edgesInFamily.insert(edge).second) {
              ...
              if (reason) {
                *reason += TfStringPrintf(
                    "Found duplicate edge index (%d, %d) "
                    "in GeomSubset at path <%s> at time %s.\n",
                    edge[0],
                    edge[1],
                    subset.GetPath().GetText(),
      ...
      if (_GetEdgesFromPrim(geom, t, edgesOnPrim)) {
        struct cmpEdge e;
        bool hasValidEdges = std::includes(
            edgesOnPrim.begin(), edgesOnPrim.end(), edgesInFamily.begin(), edgesInFamily.end(), e);
