...
UsdGeomBBoxCache::UsdGeomBBoxCache(
    ...
    : _time(time), _includedPurposes(includedPurposes), _ctmCache(time), _useExtentsHint(useExtentsHint), _ignoreVisibility(ignoreVisibility)
{
}

UsdGeomBBoxCache::UsdGeomBBoxCache(UsdGeomBBoxCache const &other)
    : _time(other._time), _baseTime(other._baseTime), _includedPurposes(other._includedPurposes), _ctmCache(other._ctmCache), _bboxCache(other._bboxCache), _useExtentsHint(other._useExtentsHint)
{
}
...
bool UsdGeomBBoxCache::_ComputePointInstanceBoundsHelper(
...
bool UsdGeomBBoxCache::ComputePointInstanceWorldBounds(
...
bool UsdGeomBBoxCache::ComputePointInstanceRelativeBounds(
    const UsdGeomPointInstancer &instancer,
    int64_t const *instanceIdBegin,
    size_t numIds,
    const UsdPrim &relativeToAncestorPrim,
...
bool UsdGeomBBoxCache::ComputePointInstanceLocalBounds(
    const UsdGeomPointInstancer &instancer,
    int64_t const *instanceIdBegin,
    size_t numIds,
    GfBBox3d *result)
{
  ...
  return _ComputePointInstanceBoundsHelper(
      ...
      _ctmCache.GetLocalTransformation(
          instancer.GetPrim(), &resetsXformStack),
      result);
}
...
void UsdGeomBBoxCache::_ComputePurposeInfo(
    ...
    if (parentPrim && parentPrim.GetPath() != SdfPath::AbsoluteRootPath())
    {
      ...
      if (parentEntry)
      {
        ...
        else
        {
          ...
          TF_DEBUG(USDGEOM_BBOX).Msg("[BBox Cache] Computing purpose for <%s> before purpose"
...
bool UsdGeomBBoxCache::_ShouldPruneChildren(const UsdPrim &prim,
...
bool UsdGeomBBoxCache::_Resolve(
    const UsdPrim &prim,
...
bool UsdGeomBBoxCache::_GetBBoxFromExtentsHint(
                                    ...
                                    const GfMatrix4d &inverseComponentCtm)
{
  ...
  if (!entry->isVarying)
  {
    ...
    entry->isVarying =
        (xformable && xformable.TransformMightBeTimeVarying()) || (extentQuery && _IsVarying(extentQuery)) || (visQuery && _IsVarying(visQuery));
  }
  ...
  else
  {
    ...
    const GfMatrix4d &inverseEnclosingComponentCtm =
        _IsComponentOrSubComponent(prim) ? xfCache.GetLocalToWorldTransform(prim).GetInverse() : inverseComponentCtm;
  ...
  TF_DEBUG(USDGEOM_BBOX).Msg("[BBox Cache] resolved value: %s %s "
                             "(varying: %s)\n",
                             primContext.ToString().c_str(), TfStringify(_GetCombinedBBoxForIncludedPurposes(*bboxes)).c_str(), entry->isVarying ? "true" : "false");
}
