...
VtIntArray UsdGeomBasisCurves::ComputeSegmentCounts(const UsdTimeCode &timeCode) const
{
  ...
  if (!GetCurveVertexCountsAttr().Get(&curveVertexCounts, timeCode)) {
    TF_WARN(
        "CurveVertexCounts could not be read from prim, "
  ...
  if (!GetTypeAttr().Get(&type, timeCode)) {
    TF_WARN(
        "Curve type could not be read from prim, "
        "cannot compute segment counts.");
    return VtIntArray();
  }
  ...
  if (!GetWrapAttr().Get(&wrap, timeCode)) {
    TF_WARN(
        "Curve wrap could not be read from prim, "
        "cannot compute segment counts.");
    return VtIntArray();
  }
  ...
  if (!GetBasisAttr().Get(&basis, timeCode)) {
    TF_WARN(
        "Curve basis could not be read from prim, "
        "cannot compute segment counts.");
    return VtIntArray();
  }
  ...
  if (type == UsdGeomTokens->linear) {
    if (wrap == UsdGeomTokens->periodic) {
      ...
      std::transform(curveVertexCounts.cbegin(),
                     curveVertexCounts.cend(),
                     segmentCounts.begin(),
                     [](int n) { return n - 1; });
      isValid = true;
    }
    ...
    if (basis == UsdGeomTokens->bezier) {
      ...
      if (wrap == UsdGeomTokens->periodic) {
        ...
        std::transform(curveVertexCounts.cbegin(),
                       curveVertexCounts.cend(),
                       segmentCounts.begin(),
                       [vstep](int n) { return (n - 4) / vstep + 1; });
        isValid = true;
      }
    }
