...
#include "PXRBaseArch/env.h"
#include "PXRBaseArch/error.h"
#include "PXRBaseArch/export.h"
#include "PXRBaseArch/pxrerrno.h"
#include <pxr/pxrns.h>
...
#include <Winsock2.h>
#endif
#include "PXRBaseArch/fileSystem.h"
#include "PXRBaseArch/pxrinttypes.h"
#include "PXRBaseArch/symbols.h"
...
void Arch_ProgInfo::SetProgramInfoForErrors(const std::string &key,
                                            const std::string &value) {
...
void Arch_LogInfo::SetExtraLogInfoForErrors(
...
static void _atexitCallback() { ArchLogSessionInfo(); }
...
static bool _MakeArgv(const char *dstArgv[], size_t maxDstArgs, const char *cmd,
...
#if defined(ARCH_OS_LINUX)
static int nonLockingLinux__execve(const char *file, char *const argv[],
                                   char *const envp[]) {
  /*
...
#if defined(ARCH_CPU_ARM)
  {
    ...
    __asm__ __volatile__("svc 0"
                         : "=r"(__file_result)
                         : "r"(__num_execve), "r"(__file_result), "r"(__argv),
                           "r"(__envp)
                         : "memory");
...
static int _LogStackTraceForPid(bool isFatal, const char *logfile,
                                const char *reason) {
  ...
  const char *const substitutions[4][2] = {{"$pid", pidBuffer},
                                           {"$log", logfile},
                                           {"$time", timeBuffer},
                                           {"$reason", reason}};
 ...
 */
ARCH_HIDDEN
void Arch_SetAppLaunchTime() { _appLaunchTime = time(NULL); }
 ...
 */
void ArchSetFatalStackLogging(bool flag) { _shouldLogStackToDb = flag; }
 ...
 * This function can be called from python.
 */
bool ArchGetFatalStackLogging() { return _shouldLogStackToDb; }

void ArchSetProgramInfoForErrors(const std::string &key,
                                 const std::string &value) {
...
void ArchSetLogSession(const char *command, const char *const argv[],
...
bool ArchIsAppCrashing() { return _isCrashing; }
 ...
 */
static void _ArchLogProcessStateHelper(bool isFatal, const char *reason,
 ...
 */
void ArchLogStackTrace(const std::string &reason, bool fatal,
...
void ArchLogStackTrace(const std::string &progname, const std::string &reason,
  ...
  string tmpFile;
  int fd = ArchMakeTmpFile(ArchStringPrintf("%s_%s", stackTracePrefix,
                                            ArchGetProgramNameForErrors()),
                           &tmpFile);
...
static void _LogStackTraceToOutputIterator(OutputIterator oi, size_t maxDepth,
                                           bool addEndl) {
  /* Could use tmpnam but we're trying to be minimalist here. */
  char logfile[1024];
 ...
 */
void ArchPrintStackTrace(ostream &oss, const std::string &programName,
                         const std::string &reason) {
  oss << "==============================================================\n"
  ...
  _LogStackTraceToOutputIterator(ostream_iterator<string>(oss),
                                 numeric_limits<size_t>::max(), true);
...
void ArchGetStackFrames(size_t maxDepth, size_t skip,
                        vector<uintptr_t> *frames) {
...
static _Unwind_Reason_Code Arch_unwindcb(struct _Unwind_Context *ctx,
                                         void *data) {
...
static std::string Arch_DefaultStackTraceCallback(uintptr_t address) {
  ...
  if (ArchGetAddressInfo(reinterpret_cast<void *>(address - 1), &objectPath,
                         &baseAddress, &symbolName, &symbolAddress) &&
      symbolAddress) {
...
static vector<string> Arch_GetStackTrace(const vector<uintptr_t> &frames,
  ...
  for (size_t i = 0; i < frames.size(); i++) {
    ...
    rv.push_back(ArchStringPrintf(" #%-3i 0x%016lx in %s", n++, frames[i],
                                  symbolic.c_str()));
  }
