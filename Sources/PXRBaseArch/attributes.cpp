...
MachO::SectionInfo MachO::find(const char *segname,
...
static std::vector<Arch_ConstructorEntry>
GetConstructorEntries(const struct mach_header *mh, intptr_t slide,
...
ImageNT::SectionInfo ImageNT::Find(const char *sectname) const {
  ...
  for (WORD i = 0; i != ntHeader->FileHeader.NumberOfSections; ++i) {
    ...
    if (strncmp(reinterpret_cast<const char *>(section.Name), sectname,
