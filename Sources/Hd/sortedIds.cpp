...
struct Hd_SortedIds::_UpdateImpl {
  ...
  bool Insert(SdfPathVector *insertsPtr, SdfPath const &id)
  {
    ...
    if (_insertsBloom.IsFull() || (_removesBloom.MightContain(id) && _Find(_removes, id))) {
      return false;
    }
  ...
  bool Remove(SdfPathVector *insertsPtr, SdfPath const &id)
  {
    ...
    if (_removesBloom.IsFull() || (_insertsBloom.MightContain(id) && _Find(inserts, id))) {
      return false;
    }
...
Hd_SortedIds &Hd_SortedIds::operator=(Hd_SortedIds const &other)
{
  _ids = other._ids;
  _edits = other._edits;
  _mode = other._mode;
  _updater.reset(other._updater ? new _UpdateImpl{*other._updater} : nullptr);
  return *this;
}
...
static inline OutIter _SetDifference(Iter1 f1, Iter1 l1, Iter2 f2, Iter2 l2, OutIter out)
{
  while (f1 != l1 && f2 != l2) {
    if (*f1 < *f2) {
      *out = *f1;
...
void Hd_SortedIds::_Sort()
{
  ...
  if (removing) {
    ...
    auto removeBegin = lower_bound(_ids.begin(), _ids.end(), _edits.front());
    ...
    if (_edits.size() == 1) {
      ...
      auto removeEnd = upper_bound(_ids.begin(), _ids.end(), _edits.back());
      ...
      if (removeRangeSize / BinarySearchFrac > _edits.size()) {
        ...
        // Take the difference.
        auto newRemoveEnd = _SetDifference(make_move_iterator(removeBegin),
    ...
    if (_edits.size() == 1) {
      ...
      auto addEnd = upper_bound(_ids.begin(), _ids.end(), _edits.back());
