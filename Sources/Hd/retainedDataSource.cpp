...
class Hd_MappedRetainedContainerDataSource : public HdRetainedContainerDataSource {
 ...
 private:
  ...
  TfDenseHashMap<TfToken, HdDataSourceBaseHandle, TfToken::HashFunctor, std::equal_to<TfToken>, T>
      _values;
};
...
HdRetainedContainerDataSource::Handle HdRetainedContainerDataSource::New(
    const TfToken &name1, const HdDataSourceBaseHandle &value1)
{
  TfToken names[] = {name1};
...
HdRetainedSmallVectorDataSource::HdRetainedSmallVectorDataSource(
    const size_t count, const HdDataSourceBaseHandle *values)
    : _values(count)
{
  for (size_t i = 0; i < count; ++i) {
    if (values[i]) {
...
struct Hd_CreateTypedRetainedDataSourceVisitor {
  template<class T> HdSampledDataSource::Handle operator()(T const &obj) const
  {
