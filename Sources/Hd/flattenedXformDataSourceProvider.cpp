    ...
    const Context &ctx) const
{
  ...
  if (inputMatrixDataSource && parentMatrixDataSource) {
    const GfMatrix4d parentMatrix = parentMatrixDataSource->GetTypedValue(0.0f);
    const GfMatrix4d inputMatrix = inputMatrixDataSource->GetTypedValue(0.0f);

    return HdXformSchema::Builder()
        .SetMatrix(HdRetainedTypedSampledDataSource<GfMatrix4d>::New(inputMatrix * parentMatrix))
        .Build();
  }
