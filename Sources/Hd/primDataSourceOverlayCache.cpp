...
void HdPrimDataSourceOverlayCache::HandlePrimsAdded(
    const HdSceneIndexObserver::AddedPrimEntries &entries, const HdSceneIndexBaseRefPtr &source)
{
  TRACE_FUNCTION();
  ...
  for (const auto &entry : entries) {
    ...
    if (prim.dataSource) {
      ...
      prim.dataSource = _HdPrimDataSourceOverlay::New(
          inputDataSource, parentOverlay, shared_from_this());
    }
  }
}
  ...
  if (TF_VERIFY(cache)) {
    ...
    for (const auto &overlay : cache->_overlayTopology) {
      ...
      for (const auto &loc : overlay.second.onPrim) {
        if (namesAtStart.find(loc.GetFirstElement()) != namesAtStart.end()) {
          names.push_back(overlay.first);
          sortMe = true;
          break;
        }
    ...
    const TfToken &name)
{
  ...
  if (TF_VERIFY(cache)) {
    ...
    if (topoIt != cache->_overlayTopology.end()) {
      ...
      HdDataSourceBaseHandle ds = cache->_ComputeOverlayDataSource(
          name, _inputDataSource, _parentOverlayDataSource);
