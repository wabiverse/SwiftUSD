    ...
    HdSceneIndexObserver::AddedPrimEntries *outputAddedEntries)
{
  ...
  for (const HdSceneIndexObserver::RenamedPrimEntry &renamedEntry : renamedEntries) {
    if (renamedEntry.oldPrimPath != renamedEntry.newPrimPath) {
      ...
      while (!workQueue.empty()) {
        ...
        workQueue.insert(workQueue.end(), childPaths.rbegin(), childPaths.rend());
      }
    }
  }
}
    ...
    HdSceneIndexObserver *observer)
{
  ...
  ConvertPrimsRenamedToRemovedAndAdded(sender, renamedEntries, &removed, &added);
