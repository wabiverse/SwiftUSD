  ...
  for (auto const &iter : hdNetworkMap.map) {
    ...
    for (const HdMaterialNode &node : hdNetwork.nodes) {
      ...
      for (const auto &p : node.parameters) {
        ...
        const std::pair<std::string, bool> res = SdfPath::StripPrefixNamespace(
            p.first, HdMaterialNodeParameterSchemaTokens->colorSpace);
    ...
    const HdMaterialNetworkMap &hdNetworkMap)
{
  ...
  HdDataSourceBaseHandle network = ConvertHdMaterialNetworkToHdMaterialNetworkSchema(hdNetworkMap);
