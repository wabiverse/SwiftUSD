...
static void _FillAddedChildEntriesRecursively(WorkDispatcher *dispatcher,
                                              ...
                                              _AddedPrimEntryQueue *queue)
{
  for (SdfPath childPath : inputSceneIndex->GetChildPrimPaths(parentPath)) {
    ...
    const TfToken resolvedPrimType = mergingSceneIndex->GetPrim(childPath).primType;

    queue->emplace(childPath, resolvedPrimType);

    dispatcher->Run([=]() {
      _FillAddedChildEntriesRecursively(
          dispatcher, mergingSceneIndex, inputSceneIndex, childPath, queue);
    });
  }
}

static bool _Contains(const SdfPath &path, const SdfPathVector &v)
{
  return std::find(v.begin(), v.end(), path) != v.end();
}

static bool _HasPrim(HdSceneIndexBase *const sceneIndex, const SdfPath &path)
{
                                        ...
                                        const SdfPath &activeInputSceneRoot)
{
  ...
  // Add entries for input scene
  {
    ...
    // Old scene indices might have a prim of different type at the given path,
    // so we need to query the merging scene index itself here.
    queue.emplace(activeInputSceneRoot, GetPrim(activeInputSceneRoot).primType);

    WorkDispatcher dispatcher;
    _FillAddedChildEntriesRecursively(&dispatcher, this, inputScene, activeInputSceneRoot, &queue);
    dispatcher.Wait();
                                      ...
                                      const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
    ...
    for (const _InputEntry &inputEntry : _inputs) {
      ...
      const TfToken primType = get_pointer(inputEntry.sceneIndex) == &sender ?
                                   entry.primType :
    ...
    if (resolvedPrimType != entry.primType) {
      if (filteredEntries.empty()) {
        ...
        for (const HdSceneIndexObserver::AddedPrimEntry &origEntry : entries) {
          if (&origEntry == &entry) {
            break;
          }
  ...
  for (const HdSceneIndexObserver::RemovedPrimEntry &entry : entries) {
    ...
    if (primFullyRemoved) {
      ...
      for (const SdfPath &descendantPath :
           HdSceneIndexPrimView(HdMergingSceneIndexRefPtr(this), entry.primPath))
      {
        addedEntries.emplace_back(descendantPath, GetPrim(descendantPath).primType);
      }
    }
  }
...
void HdMergingSceneIndex::_Observer::PrimsRenamed(const HdSceneIndexBase &sender,
