...
bool HdAovHasDepthSemantic(TfToken const &aovName)
{
  ...
  // case folding [A-Z]
  return TfStringEndsWith(TfStringToLowerAscii(aovName.GetString()), HdAovTokens->depth);
}
...
bool HdAovHasDepthStencilSemantic(TfToken const &aovName)
{
  ...
  return TfStringEndsWith(TfStringToLowerAscii(aovName.GetString()),
...
HdParsedAovToken::HdParsedAovToken(TfToken const &aovName)
    : isPrimvar(false), isLpe(false), isShader(false)
{
