...
class _PrimLevelWrappingDataSource : public HdContainerDataSource {
 public:
  ...
  // invalidated.
  bool PrimDirtied(const _DataSourceLocatorSetVector &relativeDirtyLocators);

 private:
  _PrimLevelWrappingDataSource(const HdFlatteningSceneIndex &flatteningSceneIndex,
                               const SdfPath &primPath,
                               const HdContainerDataSourceHandle &inputDataSource)
      : _flatteningSceneIndex(flatteningSceneIndex),
        _primPath(primPath),
        _inputDataSource(inputDataSource),
        _computedDataSources(_flatteningSceneIndex.GetFlattenedDataSourceNames().size())
  {
  }
  ...
  TfSmallVector<HdDataSourceBaseAtomicHandle, _smallVectorSize> _computedDataSources;
};

HD_DECLARE_DATASOURCE_HANDLES(_PrimLevelWrappingDataSource);

bool _PrimLevelWrappingDataSource::PrimDirtied(
    const _DataSourceLocatorSetVector &relativeDirtyLocators)
{
  ...
  for (size_t i = 0; i < relativeDirtyLocators.size(); i++) {
    ...
    HdDataSourceBaseHandle const ds = HdDataSourceBase::AtomicLoad(dsAtomicHandle);
    if (!ds) {
      continue;
    }
    if (!relativeDirtyLocators[i].Contains(HdDataSourceLocator::EmptyLocator())) {
      if (HdInvalidatableContainerDataSourceHandle const invalidatableDs =
              HdInvalidatableContainerDataSource::Cast(ds))
      {
        anyDirtied |= invalidatableDs->Invalidate(relativeDirtyLocators[i]);
        continue;
      }
    }
...
void _Insert(const TfTokenVector &vec, TfTokenVector *const result)
{
  if (vec.size() > 31) {
...
TfTokenVector _PrimLevelWrappingDataSource::GetNames()
{
  if (!_inputDataSource) {
...
HdDataSourceBaseHandle _PrimLevelWrappingDataSource::Get(const TfToken &name)
{
  const TfTokenVector &dataSourceNames = _flatteningSceneIndex.GetFlattenedDataSourceNames();
  ...
  for (size_t i = 0; i < dataSourceNames.size(); i++) {
    ...
    HdDataSourceBaseAtomicHandle &dsAtomicHandle = _computedDataSources[i];
    if (HdDataSourceBaseHandle const computedDs = HdDataSourceBase::AtomicLoad(dsAtomicHandle)) {
      return HdContainerDataSource::Cast(computedDs);
    }
    const HdFlattenedDataSourceProvider::Context ctx(
        _flatteningSceneIndex, _primPath, name, _inputDataSource);
    HdDataSourceBaseHandle flattenedDs = providers[i]->GetFlattenedDataSource(ctx);
    if (!flattenedDs) {
    ...
    // returned.
    if (HdDataSourceBase::AtomicCompareExchange(dsAtomicHandle, existingDs, flattenedDs)) {
...
HdFlatteningSceneIndex::HdFlatteningSceneIndex(HdSceneIndexBaseRefPtr const &inputScene,
    ...
    : HdSingleInputFilteringSceneIndexBase(inputScene)
{
  ...
  for (const TfToken &name : inputArgs->GetNames()) {
    using DataSource = HdTypedSampledDataSource<HdFlattenedDataSourceProviderSharedPtr>;
    DataSource::Handle const ds = DataSource::Cast(inputArgs->Get(name));
    if (!ds) {
      continue;
    }
    HdFlattenedDataSourceProviderSharedPtr const provider = ds->GetTypedValue(0.0f);
    if (!provider) {
      continue;
    }
    _dataSourceNames.push_back(name);
    _dataSourceProviders.push_back(provider);
    _dataSourceLocatorSet.insert(HdDataSourceLocator(name));
    _relativeDataSourceLocators.push_back(HdDataSourceLocatorSet::UniversalSet());
  }
}
...
HdSceneIndexPrim HdFlatteningSceneIndex::GetPrim(const SdfPath &primPath) const
{
  ...
  prim.dataSource = _PrimLevelWrappingDataSource::New(*this, primPath, prim.dataSource);
                                         ...
                                         const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
    _DirtyHierarchy(
        entry.primPath, _relativeDataSourceLocators, _dataSourceLocatorSet, &dirtyEntries);
  }
...
void HdFlatteningSceneIndex::_PrimDirtied(
    ...
    HdSceneIndexObserver::DirtiedPrimEntries *const dirtyEntries)
{
  ...
  _DataSourceLocatorSetVector relativeDirtyLocators(_dataSourceNames.size());
  ...
  for (size_t i = 0; i < _dataSourceNames.size(); i++) {
    ...
    HdDataSourceLocatorSet &relativeDirtyLocatorSet = relativeDirtyLocators[i];
    ...
    for (const HdDataSourceLocator &dirtyLocator : entry.dirtyLocators.Intersection(locator)) {
      relativeDirtyLocatorSet.insert(dirtyLocator.RemoveFirstElement());
    }
    // Let provider expand locators.
    HdFlattenedDataSourceProviderSharedPtr const &provider = _dataSourceProviders[i];
    provider->ComputeDirtyLocatorsForDescendants(&relativeDirtyLocatorSet);
    if (relativeDirtyLocatorSet.Contains(HdDataSourceLocator::EmptyLocator())) {
    ...
    for (const HdDataSourceLocator &relativeDirtyLocator : relativeDirtyLocatorSet) {
      dirtyLocators.insert(locator.Append(relativeDirtyLocator));
    }
  }

  if (!dirtyLocators.IsEmpty()) {
    _DirtyHierarchy(entry.primPath, relativeDirtyLocators, dirtyLocators, dirtyEntries);
  }
...
void HdFlatteningSceneIndex::_ConsolidateRecentPrims()
{
