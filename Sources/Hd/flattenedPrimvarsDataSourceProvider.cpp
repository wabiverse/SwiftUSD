...
bool _IsConstantPrimvar(HdContainerDataSourceHandle const &primvarDataSource)
{
  ...
  HdTokenDataSourceHandle const interpolationSource = primvarSchema.GetInterpolation();
...
{
  return locator.GetElementCount() < 2 ||
...
class _PrimvarsDataSource : public HdInvalidatableContainerDataSource {
 ...
 private:
  ...
  using _NameToPrimvarDataSource =
      tbb::concurrent_hash_map<TfToken, HdDataSourceBaseAtomicHandle, _TokenHashCompare>;
  // Cached data sources.
  //
...
_PrimvarsDataSource::_PrimvarsDataSource(HdContainerDataSourceHandle const &primvarsDataSource,
                                         Handle const &parentDataSource)
    : _primvarsDataSource(primvarsDataSource), _parentDataSource(parentDataSource)
{
}

std::shared_ptr<std::set<TfToken>> _PrimvarsDataSource::_GetConstantPrimvarNames()
{
  std::shared_ptr<std::set<TfToken>> result = std::atomic_load(&_constantPrimvarNames);

  if (!result) {
    // Cache miss
    result = std::make_shared<std::set<TfToken>>(_GetConstantPrimvarNamesUncached());
...
std::set<TfToken> _PrimvarsDataSource::_GetConstantPrimvarNamesUncached()
{
  ...
  if (_primvarsDataSource) {
    for (const TfToken &name : _primvarsDataSource->GetNames()) {
      if (_IsConstantPrimvar(HdContainerDataSource::Cast(_primvarsDataSource->Get(name)))) {
        result.insert(name);
      }
    }
  }
...
TfTokenVector _PrimvarsDataSource::GetNames()
{
  ...
  // primvar source.
  std::set<TfToken> constantPrimvars = *(_parentDataSource->_GetConstantPrimvarNames());
  ...
  result.insert(result.end(), constantPrimvars.begin(), constantPrimvars.end());

  return result;
}

HdContainerDataSourceHandle _PrimvarsDataSource::_GetUncached(const TfToken &name)
{
  // Check whether this prim has this primvar.
  if (_primvarsDataSource) {
    if (HdContainerDataSourceHandle const result = HdContainerDataSource::Cast(
  ...
  // constant.
  if (_parentDataSource) {
    HdContainerDataSourceHandle const result = HdContainerDataSource::Cast(
...
HdDataSourceBaseHandle _PrimvarsDataSource::Get(const TfToken &name)
{
  ...
  if (HdDataSourceBaseHandle const ds = HdDataSourceBase::AtomicLoad(a->second)) {
    // Cache hit.
  ...
  if (result) {
    ...
    HdDataSourceBase::AtomicStore(a->second, HdRetainedTypedSampledDataSource<bool>::New(false));
  }
  return result;
}

bool _PrimvarsDataSource::Invalidate(const HdDataSourceLocatorSet &locators)
{
...
HdContainerDataSourceHandle HdFlattenedPrimvarsDataSourceProvider::GetFlattenedDataSource(
    const Context &ctx) const
{
  return _PrimvarsDataSource::New(
      ctx.GetInputDataSource(),
      _PrimvarsDataSource::Cast(ctx.GetFlattenedDataSourceFromParentPrim()));
}

void HdFlattenedPrimvarsDataSourceProvider::ComputeDirtyLocatorsForDescendants(
