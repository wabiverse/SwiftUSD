...
HdContainerDataSourceEditor &HdContainerDataSourceEditor::Set(
    const HdDataSourceLocator &locator, const HdDataSourceBaseHandle &dataSource)
{
  if (locator.IsEmpty()) {
    // exception?
    return (*this);
...
HdContainerDataSourceEditor &HdContainerDataSourceEditor::Overlay(
    const HdDataSourceLocator &locator, const HdContainerDataSourceHandle &dataSource)
{
  if (locator.IsEmpty()) {
    // exception?
    return (*this);
...
HdContainerDataSourceHandle HdContainerDataSourceEditor::Finish()
{
  if (_initialContainer) {
    ...
    if (_directContainerSets.empty()) {
      return HdOverlayContainerDataSource::New(_NodeContainerDataSource::New(_root),
                                               _initialContainer);
    }
    else {
  ...
  if (_node) {
    ...
    if (auto container = HdContainerDataSource::Cast(entry.dataSource)) {
      if (!entry.childNode) {
        ...
        return HdOverlayContainerDataSource::New(_NodeContainerDataSource::New(entry.childNode),
                                                 container);
      }
    }
