...
void _ConvertDoubleToFloat(VtValue *value, HdTupleType *tupleType)
{
...
template<typename DBL, typename FLT>
void _ConvertDoubleToFloatArray(VtValue *value, HdTupleType *tupleType)
{
...
void HdVtBufferSource::_SetValue(const VtValue &v, int arraySize, bool allowDoubles)
{
  _value = v;
  ...
  if (_value.IsHolding<bool>()) {
    ...
    if (_value.IsArrayValued()) {
      if (_value.IsHolding<VtDoubleArray>()) {
        ...
        _ConvertDoubleToFloatArray<GfVec2d, GfVec2f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<VtVec3dArray>()) {
        _ConvertDoubleToFloatArray<GfVec3d, GfVec3f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<VtVec4dArray>()) {
        _ConvertDoubleToFloatArray<GfVec4d, GfVec4f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<VtMatrix2dArray>()) {
        _ConvertDoubleToFloatArray<GfMatrix2d, GfMatrix2f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<VtMatrix3dArray>()) {
        _ConvertDoubleToFloatArray<GfMatrix3d, GfMatrix3f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<VtMatrix4dArray>()) {
        _ConvertDoubleToFloatArray<GfMatrix4d, GfMatrix4f>(&_value, &_tupleType);
      }
    }
    else {
      if (_value.IsHolding<double>()) {
        ...
        _ConvertDoubleToFloat<GfMatrix2d, GfMatrix2f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<GfMatrix3d>()) {
        _ConvertDoubleToFloat<GfMatrix3d, GfMatrix3f>(&_value, &_tupleType);
      }
      else if (_value.IsHolding<GfMatrix4d>()) {
        _ConvertDoubleToFloat<GfMatrix4d, GfMatrix4f>(&_value, &_tupleType);
      }
    }
  }
...
HdVtBufferSource::HdVtBufferSource(TfToken const &name,
    ...
    : _name(name)
{
  bool const resolvedAllowDoubles = allowDoubles && GetDefaultMatrixType() == HdTypeDoubleMat4;
