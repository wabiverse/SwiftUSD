...
HdDataSourceLocator::HdDataSourceLocator(
    const TfToken &t1, const TfToken &t2, const TfToken &t3, const TfToken &t4, const TfToken &t5)
                                                       ...
                                                       const HdDataSourceLocator &newPrefix) const
{
  ...
  result._tokens.insert(
      result._tokens.end(), _tokens.begin() + oldPrefix.GetElementCount(), _tokens.end());
...
std::ostream &operator<<(std::ostream &out, const HdDataSourceLocator &self)
{
...
void HdDataSourceLocatorSet::_Normalize()
{
  if (_locators.size() < 2) {
    return;
  }
...
const HdDataSourceLocatorSet &HdDataSourceLocatorSet::UniversalSet()
{
  static const HdDataSourceLocatorSet &result{HdDataSourceLocator::EmptyLocator()};
  return result;
}
...
void HdDataSourceLocatorSet::insert(HdDataSourceLocatorSet &&locatorSet)
{
  if (_locators.empty()) {
...
HdDataSourceLocatorSet::const_iterator HdDataSourceLocatorSet::_FirstIntersection(
    const HdDataSourceLocator &locator) const
{
  ...
  if (_locators.size() < _binarySearchCutoff) {
    for (const_iterator it = _locators.begin(); it != _locators.end(); ++it) {
  ...
  const auto it = std::lower_bound(
      _locators.begin(), _locators.end(), locator, _LessThanNotPrefix);
...
bool HdDataSourceLocatorSet::Contains(const HdDataSourceLocator &locator) const
{
...
HdDataSourceLocatorSet HdDataSourceLocatorSet::ReplacePrefix(
    const HdDataSourceLocator &oldPrefix, const HdDataSourceLocator &newPrefix) const
{
  ...
  auto it = std::lower_bound(_locators.begin(), _locators.end(), oldPrefix);
  ...
  if (it != _locators.end()) {
    ...
    if (it->HasPrefix(oldPrefix)) {
      ...
      auto lowerIt = locators.begin() + std::distance(_locators.begin(), it);
      ...
      if (*lowerIt == oldPrefix) {
        ...
        *lowerIt = newPrefix;
        ...
        auto upperIt = std::lower_bound(
            ...
            locators.end(),
            oldPrefix,
            [](const HdDataSourceLocator &elem, const HdDataSourceLocator &prefix) {
...
HdDataSourceLocatorSet::IntersectionIterator &HdDataSourceLocatorSet::IntersectionIterator::
operator++()
{
  _isFirst = false;
  ++_iterator;
...
HdDataSourceLocatorSet::IntersectionView HdDataSourceLocatorSet::Intersection(
    const HdDataSourceLocator &locator) const
{
  return IntersectionView(IntersectionIterator(
                              /* isFirst = */ true, _FirstIntersection(locator), end(), locator),
                          IntersectionIterator(
                              /* isFirst = */ false, end(), end(), locator));
}

std::ostream &operator<<(std::ostream &out, const HdDataSourceLocatorSet &self)
{
  out << "{ ";
