...
static std::vector<HdSampledDataSource::Time> _Union(
    const std::vector<HdSampledDataSource::Time> &a,
...
bool HdGetMergedContributingSampleTimesForInterval(
    const size_t count,
    ...
    std::vector<HdSampledDataSource::Time> *const outSampleTimes)
{
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (!inputDataSources[i]->GetContributingSampleTimesForInterval(startTime, endTime, &times)) {
      continue;
    }
    if (times.empty()) {
      continue;
