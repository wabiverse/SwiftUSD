...
using _SToBMap =
    std::unordered_map<TfToken, HdDirtyBitsTranslator::LocatorSetToDirtyBitsFnc, TfHash>;

using _BToSMap =
    std::unordered_map<TfToken, HdDirtyBitsTranslator::DirtyBitsToLocatorSetFnc, TfHash>;
                                                       ...
                                                       HdDataSourceLocatorSet *set)
{
  ...
  if (primType == HdPrimTypeTokens->material) {
    ...
    if (bits & HdCoordSys::DirtyName) {
      static const HdDataSourceLocator locator = HdCoordSysSchema::GetDefaultLocator().Append(
                                                              ...
                                                              HdDataSourceLocatorSet const &set)
{
  ...
  if (primType == HdPrimTypeTokens->material) {
    ...
    static const HdDataSourceLocator nameLocator = HdCoordSysSchema::GetDefaultLocator().Append(
        HdCoordSysSchemaTokens->name);
                                                              ...
                                                              HdDataSourceLocatorSet const &set)
{
  ...
  if (primType == HdPrimTypeTokens->renderBuffer) {
    ...
    if (_FindLocator(HdRenderSettingsSchema::GetRenderProductsLocator(), end, &it)) {
      bits |= HdRenderSettings::DirtyRenderProducts;
    }
    if (_FindLocator(HdRenderSettingsSchema::GetIncludedPurposesLocator(), end, &it)) {
      bits |= HdRenderSettings::DirtyIncludedPurposes;
    }
    if (_FindLocator(HdRenderSettingsSchema::GetMaterialBindingPurposesLocator(), end, &it)) {
...
/*static*/
void HdDirtyBitsTranslator::RegisterTranslatorsForCustomSprimType(TfToken const &primType,
