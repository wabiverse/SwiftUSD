...
class HdDependencyForwardingSceneIndex : public HdSingleInputFilteringSceneIndexBase {
 public:
  static HdDependencyForwardingSceneIndexRefPtr New(HdSceneIndexBaseRefPtr inputScene)
  {
    return TfCreateRefPtr(new HdDependencyForwardingSceneIndex(inputScene));
  }
 ...
 private:
  ...
  using _LocatorsEntryMap =
      tbb::concurrent_unordered_map<TfToken, _LocatorsEntry, TfToken::HashFunctor>;
  ...
  using _AffectedPrimsDependencyMap =
      tbb::concurrent_unordered_map<SdfPath, _AffectedPrimDependencyEntry, SdfPath::Hash>;

  // Top-level map keyed by paths of depended-on paths
  using _DependedOnPrimsAffectedPrimsMap =
      tbb::concurrent_unordered_map<SdfPath, _AffectedPrimsDependencyMap, SdfPath::Hash>;
  ...
  using _AffectedPrimToDependsOnPathsEntryMap =
      tbb::concurrent_unordered_map<SdfPath, _AffectedPrimToDependsOnPathsEntry, SdfPath::Hash>;
  ...
  //       GetPrim -- which is defined as const within HdSceneIndexBase.
  //       This is in service of lazy population goals.
  mutable _AffectedPrimToDependsOnPathsEntryMap _affectedPrimToDependsOnPathsMap;
  ...
  struct _VisitedNode {
    ...
    template<class HashState> friend void TfHashAppend(HashState &h, _VisitedNode const &myObj)
    {
  ...
  using _VisitedNodeSet = TfDenseHashSet<_VisitedNode, _VisitedNode::HashFunctor>;

  // impl for PrimDirtied which handles propogation of PrimDirtied notices
  // for affected prims/dataSources.
  void _PrimDirtied(const SdfPath &primPath,
 ...
 public:
  ...
  HD_API
  void RemoveDeletedEntries(SdfPathVector *removedAffectedPrimPaths = nullptr,
                            SdfPathVector *removedDependedOnPrimPaths = nullptr);
};

inline size_t HdDependencyForwardingSceneIndex::_VisitedNode::Hash() const
{
  return TfHash()(*this);
}
