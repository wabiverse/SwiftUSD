...
class HdRenderIndex final {
 public:
  ...
  // ---------------------------------------------------------------------- //
  HD_API
  void InsertSceneIndex(const HdSceneIndexBaseRefPtr &inputScene,
                        SdfPath const &scenePathPrefix,
                        bool needsPrefixing = true);

  HD_API
  void RemoveSceneIndex(const HdSceneIndexBaseRefPtr &inputScene);
 ...
 private:
  ...
  using HdTaskCreateFnc = std::function<HdTaskSharedPtr(HdSceneDelegate *, SdfPath const &)>;
