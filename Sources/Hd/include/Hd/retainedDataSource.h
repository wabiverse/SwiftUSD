...
HdRetainedTypedMultisampledDataSource<T>::HdRetainedTypedMultisampledDataSource(
    size_t count, HdSampledDataSource::Time *sampleTimes, T *sampleValues)
{
  ...
  for (size_t i = 0, e = _sampledValues.size(); i < e; ++i) {
    ...
    if (sampleTime > shutterOffset) {
      ...
      if (i < 1) {
        ...
        // tie, use the earlier.
        const HdSampledDataSource::Time previousDelta = shutterOffset -
                                                        _sampledValues[i - 1].first;
...
class HdRetainedSmallVectorDataSource : public HdVectorDataSource {
 public:
  ...
  HD_API
  HdRetainedSmallVectorDataSource(size_t count, const HdDataSourceBaseHandle *values);
