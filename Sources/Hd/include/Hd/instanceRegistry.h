...
template<typename VALUE> class HdInstanceRegistry {
 public:
  ...
  HdInstanceRegistry(const HdInstanceRegistry &other)
      : _dictionary(other._dictionary), _registryMutex()  // mutex is not copied
  {
  }
  ...
  template<typename Callback> size_t GarbageCollect(Callback &&callback, int recycleCount = 0);
...
size_t HdInstanceRegistry<VALUE>::GarbageCollect(Callback &&callback, int recycleCount)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
