...
struct HdPrimvarDescriptor {
  ...
  HdPrimvarDescriptor()
      : interpolation(HdInterpolationConstant), role(HdPrimvarRoleTokens->none), indexed(false)
  {
  }
...
struct HdModelDrawMode {
  ...
  HdModelDrawMode(TfToken const &drawMode_,
      ...
      : drawMode(drawMode_),
        applyDrawMode(applyDrawMode_),
        drawModeColor(drawModeColor_),
        cardGeometry(cardGeometry_),
        cardTextureXPos(cardTextureXPos_),
        cardTextureYPos(cardTextureYPos_),
        cardTextureZPos(cardTextureZPos_),
        cardTextureXNeg(cardTextureXNeg_),
        cardTextureYNeg(cardTextureYNeg_),
        cardTextureZNeg(cardTextureZNeg_)
  {
  }

  bool operator==(HdModelDrawMode const &rhs) const
  {
    return drawMode == rhs.drawMode && applyDrawMode == rhs.applyDrawMode &&
           drawModeColor == rhs.drawModeColor && cardGeometry == rhs.cardGeometry &&
           cardTextureXPos == rhs.cardTextureXPos && cardTextureYPos == rhs.cardTextureYPos &&
           cardTextureZPos == rhs.cardTextureZPos && cardTextureXNeg == rhs.cardTextureXNeg &&
           cardTextureYNeg == rhs.cardTextureYNeg && cardTextureZNeg == rhs.cardTextureZNeg;
  }
  bool operator!=(HdModelDrawMode const &rhs) const
  {
    return !(*this == rhs);
...
class HdSceneDelegate {
 public:
  ...
  virtual HdExtComputationPrimvarDescriptorVector GetExtComputationPrimvarDescriptors(
      SdfPath const &id, HdInterpolation interpolationMode);
...
template<unsigned int CAPACITY>
void HdSceneDelegate::SampleTransform(SdfPath const &id,
                                      HdTimeSampleArray<GfMatrix4d, CAPACITY> *sa)
{
...
void HdSceneDelegate::SampleTransform(SdfPath const &id,
                                      ...
                                      HdTimeSampleArray<GfMatrix4d, CAPACITY> *sa)
{
  size_t authoredSamples = SampleTransform(
      id, startTime, endTime, CAPACITY, sa->times.data(), sa->values.data());
  if (authoredSamples > CAPACITY) {
    sa->Resize(authoredSamples);
    size_t authoredSamplesSecondAttempt = SampleTransform(
        id, startTime, endTime, authoredSamples, sa->times.data(), sa->values.data());
    // Number of samples should be consisntent through multiple
    // invokations of the sampling function.
    TF_VERIFY(authoredSamples == authoredSamplesSecondAttempt);
  }
...
template<unsigned int CAPACITY>
void HdSceneDelegate::SampleInstancerTransform(SdfPath const &instancerId,
                                               HdTimeSampleArray<GfMatrix4d, CAPACITY> *sa)
{
...
{
  size_t authoredSamples = SampleInstancerTransform(
      instancerId, startTime, endTime, CAPACITY, sa->times.data(), sa->values.data());
  if (authoredSamples > CAPACITY) {
    sa->Resize(authoredSamples);
    size_t authoredSamplesSecondAttempt = SampleInstancerTransform(
        instancerId, startTime, endTime, authoredSamples, sa->times.data(), sa->values.data());
    // Number of samples should be consisntent through multiple
    // invokations of the sampling function.
    TF_VERIFY(authoredSamples == authoredSamplesSecondAttempt);
  }
...
void HdSceneDelegate::SamplePrimvar(SdfPath const &id,
                                    ...
                                    HdTimeSampleArray<VtValue, CAPACITY> *sa)
{
  size_t authoredSamples = SamplePrimvar(
      id, key, startTime, endTime, CAPACITY, sa->times.data(), sa->values.data());
  if (authoredSamples > CAPACITY) {
    sa->Resize(authoredSamples);
    size_t authoredSamplesSecondAttempt = SamplePrimvar(
        id, key, startTime, endTime, authoredSamples, sa->times.data(), sa->values.data());
    // Number of samples should be consistent through multiple
    // invocations of the sampling function.
    TF_VERIFY(authoredSamples == authoredSamplesSecondAttempt);
  }
...
template<unsigned int CAPACITY>
void HdSceneDelegate::SampleExtComputationInput(SdfPath const &computationId,
                                                TfToken const &input,
                                                HdTimeSampleArray<VtValue, CAPACITY> *sa)
{
...
{
  size_t authoredSamples = SampleExtComputationInput(
      computationId, input, startTime, endTime, CAPACITY, sa->times.data(), sa->values.data());
