...
TF_DEFINE_ENV_SETTING(HD_USE_ENCAPSULATING_SCENE_INDICES,
                      false,
...
class _EncapsulatingSceneIndexBase : public HdSceneIndexBase {
 public:
  _EncapsulatingSceneIndexBase(std::vector<HdSceneIndexBaseRefPtr> inputScenes) {}
};

class _FilteringEncapsulatingSceneIndexBase : public HdFilteringSceneIndexBase {
 public:
  _FilteringEncapsulatingSceneIndexBase(std::vector<HdSceneIndexBaseRefPtr> inputScenes)
...
class _EncapsulatingSceneIndex : public Base, public HdEncapsulatingSceneIndexBase {
 public:
  ...
  static TfRefPtr<This> New(const std::vector<HdSceneIndexBaseRefPtr> &inputScenes,
                            HdSceneIndexBaseRefPtr const &encapsulatedScene)
  {
    return TfCreateRefPtr(new _EncapsulatingSceneIndex(inputScenes, encapsulatedScene));
  }
 ...
 private:
  _EncapsulatingSceneIndex(const std::vector<HdSceneIndexBaseRefPtr> &inputScenes,
                           HdSceneIndexBaseRefPtr const &encapsulatedScene)
      : Base(inputScenes), _encapsulatedScene(encapsulatedScene), _observer(this)
  {
  ...
  class _Observer : public HdSceneIndexObserver {
   public:
    _Observer(This *owner) : _owner(owner) {}
...
void _RecurseInputScenes(HdSceneIndexBasePtr const &sceneIndex,
                         ...
                         _SceneIndexSet *const externalScenes)
{
  ...
  auto const filteringSceneIndex = TfDynamic_cast<HdFilteringSceneIndexBasePtr>(sceneIndex);
  if (!filteringSceneIndex) {
    return;
  }

  for (HdSceneIndexBasePtr const &inputScene : filteringSceneIndex->GetInputScenes()) {
    _RecurseInputScenes(inputScene, givenInputScenes, allInputScenes, externalScenes);
  }
}

void _ValidateInputScenesCanBeReached(const std::vector<HdSceneIndexBaseRefPtr> &inputScenes,
                                      HdSceneIndexBaseRefPtr const &encapsulatedScene)
{
  TRACE_FUNCTION();

  const _SceneIndexSet givenInputScenes(inputScenes.begin(), inputScenes.end());

  _SceneIndexSet allInputScenes;
  _SceneIndexSet externalScenes;

  _RecurseInputScenes(encapsulatedScene, givenInputScenes, &allInputScenes, &externalScenes);
    ...
    HdSceneIndexBaseRefPtr const &encapsulatedScene)
{
  ...
  if (inputScenes.empty()) {
    ...
    return _EncapsulatingSceneIndex<Base>::New(inputScenes, encapsulatedScene);
  }
  else {
    // Raise coding error if the input scenes cannot be
    // reached from the encapsulated scene.
    _ValidateInputScenesCanBeReached(inputScenes, encapsulatedScene);
