...
class UsdviewqHydraObserver {
 public:
  ...
  USDVIEWQ_API
  static std::vector<std::string> GetRegisteredSceneIndexNames();
  ...
  USDVIEWQ_API
  std::vector<std::string> GetInputDisplayNames(const IndexList &inputIndices);
  ...
  /// python.
  struct NoticeEntry {
    NoticeEntry(const HdSceneIndexObserver::AddedPrimEntries &entries) : added(entries) {}

    NoticeEntry(const HdSceneIndexObserver::RemovedPrimEntries &entries) : removed(entries) {}

    NoticeEntry(const HdSceneIndexObserver::DirtiedPrimEntries &entries) : dirtied(entries) {}
