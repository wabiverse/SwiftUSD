...
class _PrimDataSource final : public HdContainerDataSource {
 public:
  ...
  TfTokenVector GetNames() override
  {
    ...
    if (_si->GetEnabled()) {
      // Filter out binding
      names.erase(std::remove(names.begin(), names.end(), _si->GetBindingToken()), names.end());
    }
    return names;
  }
...
template<typename T> T _Get(HdContainerDataSourceHandle const &container, const TfToken &token)
{
  if (!container) {
    return {};
  }
  using DataSource = HdTypedSampledDataSource<T>;
  typename DataSource::Handle const ds = DataSource::Cast(container->Get(token));
  if (!ds) {
    return {};
  }
...
// static
HdsiPrimTypePruningSceneIndexRefPtr HdsiPrimTypePruningSceneIndex::New(
    HdSceneIndexBaseRefPtr const &inputSceneIndex, HdContainerDataSourceHandle const &inputArgs)
{
  return TfCreateRefPtr(new HdsiPrimTypePruningSceneIndex(inputSceneIndex, inputArgs));
}

bool HdsiPrimTypePruningSceneIndex::_PruneType(const TfToken &primType) const
{
...
bool HdsiPrimTypePruningSceneIndex::_PrunePath(const SdfPath &path) const
{
...
bool HdsiPrimTypePruningSceneIndex::GetEnabled() const
{
  return _enabled;
}

void HdsiPrimTypePruningSceneIndex::SetEnabled(const bool enabled)
{
  if (_enabled == enabled) {
    return;
  }
...
HdSceneIndexPrim HdsiPrimTypePruningSceneIndex::GetPrim(const SdfPath &primPath) const
{
...
SdfPathVector HdsiPrimTypePruningSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
...
void HdsiPrimTypePruningSceneIndex::_PrimsAdded(
...
void HdsiPrimTypePruningSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  _SendPrimsRemoved(entries);
}

void HdsiPrimTypePruningSceneIndex::_PrimsDirtied(
...
HdsiPrimTypePruningSceneIndex::HdsiPrimTypePruningSceneIndex(
    ...
    : HdSingleInputFilteringSceneIndexBase(inputSceneIndex),
      _primTypes(_Get<TfTokenVector>(inputArgs, HdsiPrimTypePruningSceneIndexTokens->primTypes)),
      _bindingToken(_Get<TfToken>(inputArgs, HdsiPrimTypePruningSceneIndexTokens->bindingToken)),
      _doNotPruneNonPrimPaths(
          _Get<bool>(inputArgs, HdsiPrimTypePruningSceneIndexTokens->doNotPruneNonPrimPaths)),
      _enabled(false)
{
  if (_primTypes.empty()) {
    TF_CODING_ERROR("Empty prim types given to HdsiPrimTypePruningSceneIndex");
  }
}

HdsiPrimTypePruningSceneIndex::~HdsiPrimTypePruningSceneIndex() = default;
