...
const HdDataSourceLocator curveVertexCountsSourceLocator =
    HdNurbsCurvesSchema::GetDefaultLocator().Append(HdNurbsCurvesSchemaTokens->curveVertexCounts);

// Data source for __dependencies.
//
// Only dependency is curveVertexCounts which we computed for basisCurves from
// nurbsCurves.
HdDataSourceBaseHandle _ComputeDependenciesDataSource(const SdfPath &primPath)
{
  ...
  static HdLocatorDataSourceHandle const dependedOnLocatorDataSource =
      HdRetainedTypedSampledDataSource<HdDataSourceLocator>::New(curveVertexCountsSourceLocator);
...
class _BasisCurvesTopologyDataSource : public HdContainerDataSource {
 public:
  ...
  {
    static const TfTokenVector names = {HdBasisCurvesTopologySchemaTokens->curveVertexCounts,
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdBasisCurvesTopologySchemaTokens->curveVertexCounts) {
      return HdContainerDataSource::Get(_primDataSource, curveVertexCountsSourceLocator);
    }

    if (name == HdBasisCurvesTopologySchemaTokens->basis ||
        name == HdBasisCurvesTopologySchemaTokens->type)
    {
      static const HdDataSourceBaseHandle ds = HdRetainedTypedSampledDataSource<TfToken>::New(
          HdTokens->linear);
      return ds;
    }
 ...
 private:
  _BasisCurvesTopologyDataSource(const HdContainerDataSourceHandle &primDataSource)
...
{
  static HdDataSourceBaseHandle const nurbsCurvesDataSource = HdBlockDataSource::New();
  HdDataSourceBaseHandle const basisCurvesDataSource = HdRetainedContainerDataSource::New(
      HdBasisCurvesTopologySchemaTokens->topology,
      _BasisCurvesTopologyDataSource::New(primDataSource));
  HdDataSourceBaseHandle const dependenciesDataSource = _ComputeDependenciesDataSource(primPath);

  HdContainerDataSourceHandle sources[] = {
      HdRetainedContainerDataSource::New(HdNurbsCurvesSchemaTokens->nurbsCurves,
                                         nurbsCurvesDataSource,
                                         HdBasisCurvesSchemaTokens->basisCurves,
                                         basisCurvesDataSource,
...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (basisCurvesToMesh));
...
std::pair<HdIntDataSourceHandle, HdIntDataSourceHandle> _GetUVVertexCountDataSources(
...
std::pair<int, int> _GetUVVertexCounts(const HdContainerDataSourceHandle &primDataSource,
                                       const HdSampledDataSource::Time shutterOffset)
{
  ...
  return {sources.first ? sources.first->GetTypedValue(shutterOffset) : 0,
...
bool _GetMergedContributingSampleTimesForIntervalForTopology(
    const HdContainerDataSourceHandle &primDataSource,
...
class _FaceVertexCountsDataSource : public HdIntArrayDataSource {
 public:
  ...
  {
    return _GetMergedContributingSampleTimesForIntervalForTopology(
        _primDataSource, startTime, endTime, outSampleTimes);
  }

 private:
  _FaceVertexCountsDataSource(const HdContainerDataSourceHandle &primDataSource)
      : _primDataSource(primDataSource)
  {
  }
...
class _FaceVertexIndicesDataSource : public HdIntArrayDataSource {
 ...
 private:
  _FaceVertexIndicesDataSource(const HdContainerDataSourceHandle &primDataSource)
      : _primDataSource(primDataSource)
  {
  }
...
class _MeshTopologyDataSource : public HdContainerDataSource {
 public:
  ...
  TfTokenVector GetNames() override
  {
    static TfTokenVector names = {HdMeshTopologySchemaTokens->faceVertexCounts,
 ...
 private:
  _MeshTopologyDataSource(const HdContainerDataSourceHandle &primDataSource)
      : _primDataSource(primDataSource)
  {
  }
...
{
  HdContainerDataSourceHandle const topologyDs = _MeshTopologyDataSource::New(primDataSource);
  ...
  static const HdDataSourceLocator doubleSidedLocator =
      HdNurbsPatchSchema::GetDefaultLocator().Append(HdNurbsPatchSchemaTokens->doubleSided);
  HdBoolDataSourceHandle const doubleSidedDs = HdBoolDataSource::Cast(
...
HdContainerDataSourceHandle _ComputePrimDataSource(
    const SdfPath &primPath, const HdContainerDataSourceHandle &primDataSource)
{
  static HdDataSourceBaseHandle const nurbsPatchDataSource = HdBlockDataSource::New();
  HdDataSourceBaseHandle const meshDataSource = _ComputeMeshDataSource(primDataSource);
  HdDataSourceBaseHandle const dependenciesDataSource = _ComputeDependenciesDataSource(primPath);

  HdContainerDataSourceHandle sources[] = {
      HdRetainedContainerDataSource::New(HdNurbsPatchSchemaTokens->nurbsPatch,
                                         nurbsPatchDataSource,
...
HdsiNurbsApproximatingSceneIndexRefPtr HdsiNurbsApproximatingSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
  return TfCreateRefPtr(new HdsiNurbsApproximatingSceneIndex(inputSceneIndex));
}
...
HdSceneIndexPrim HdsiNurbsApproximatingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  const HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(primPath);
  if (prim.primType == HdPrimTypeTokens->nurbsCurves) {
    return {HdPrimTypeTokens->basisCurves,
            _NurbsCurvesToBasisCurves::_ComputePrimDataSource(primPath, prim.dataSource)};
  }
  if (prim.primType == HdPrimTypeTokens->nurbsPatch) {
    return {HdPrimTypeTokens->mesh,
            _NurbsPatchToMesh::_ComputePrimDataSource(primPath, prim.dataSource)};
  }

  return prim;
}

SdfPathVector HdsiNurbsApproximatingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void HdsiNurbsApproximatingSceneIndex::_PrimsAdded(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (size_t i = 0; i < entries.size(); i++) {
    ...
    if (primType == HdPrimTypeTokens->nurbsCurves || primType == HdPrimTypeTokens->nurbsPatch) {
      indices.push_back(i);
    }
  }
...
void HdsiNurbsApproximatingSceneIndex::_PrimsRemoved(
...
void HdsiNurbsApproximatingSceneIndex::_PrimsDirtied(
