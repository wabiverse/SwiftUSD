...
HdsiPrimTypeNoticeBatchingSceneIndex::PrimTypePriorityFunctor::~PrimTypePriorityFunctor() =
    default;

namespace {

using _PriorityFunctorHandle = HdsiPrimTypeNoticeBatchingSceneIndex::PrimTypePriorityFunctorHandle;

_PriorityFunctorHandle _GetPrimTypePriorityFunctor(HdContainerDataSourceHandle const &inputArgs)
{
  if (!inputArgs) {
    return nullptr;
  }
  ...
  static const TfToken &key = HdsiPrimTypeNoticeBatchingSceneIndexTokens->primTypePriorityFunctor;
...
size_t _GetNumPriorities(_PriorityFunctorHandle const &h)
{
  if (!h) {
    return 1;
  }
      ...
      _populated(false)
{
  ...
  for (const SdfPath &path : HdSceneIndexPrimView(_GetInputSceneIndex())) {
    _addedOrDirtiedPrims[path] = _PrimAddedEntry{_GetInputSceneIndex()->GetPrim(path).primType};
  }
}

HdsiPrimTypeNoticeBatchingSceneIndex::~HdsiPrimTypeNoticeBatchingSceneIndex() = default;

HdSceneIndexPrim HdsiPrimTypeNoticeBatchingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  if (!_populated) {
    return {};
  }
...
SdfPathVector HdsiPrimTypeNoticeBatchingSceneIndex::GetChildPrimPaths(
...
void HdsiPrimTypeNoticeBatchingSceneIndex::_PrimsAdded(
...
void HdsiPrimTypeNoticeBatchingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
    ...
    _PrimAddedOrDirtiedEntry &addedOrDirtiedEntry = _addedOrDirtiedPrims[entry.primPath];

    // Prim added entry is stronger than prim dirty entry.
    // That is, if we already had a prim added entry for this path,
    // we do nothing.

    if (_PrimDirtiedEntry *const dirtiedEntry = std::get_if<_PrimDirtiedEntry>(
            &addedOrDirtiedEntry))
    {
...
void HdsiPrimTypeNoticeBatchingSceneIndex::_RemovePathFromAddedOrDirtiedPrims(const SdfPath &path)
{
...
void HdsiPrimTypeNoticeBatchingSceneIndex::_AddPathToRemovedPrims(const SdfPath &path)
{
...
void HdsiPrimTypeNoticeBatchingSceneIndex::_PrimsRemoved(
...
size_t HdsiPrimTypeNoticeBatchingSceneIndex::_GetPriority(const TfToken &primType) const
{
  if (!_primTypePriorityFunctor) {
    return 0;
  }

  const size_t priority = _primTypePriorityFunctor->GetPriorityForPrimType(primType);

  if (priority >= _numPriorities) {
    TF_CODING_ERROR("Priority %zu for prim type %s exceeds _numPriorities %zu\n",
                    priority,
                    primType.GetText(),
                    _numPriorities);
...
void HdsiPrimTypeNoticeBatchingSceneIndex::Flush()
{
  TRACE_FUNCTION();
  ...
  if (_IsObserved()) {
    ...
    if (!_addedOrDirtiedPrims.empty()) {
      ...
      std::vector<HdSceneIndexObserver::AddedPrimEntries> addedEntries(_numPriorities);
      std::vector<HdSceneIndexObserver::DirtiedPrimEntries> dirtiedEntries(_numPriorities);
      ...
      for (const auto &pathAndEntry : _addedOrDirtiedPrims) {
        ...
        if (const _PrimAddedEntry *const addedEntry = std::get_if<_PrimAddedEntry>(&entry)) {
          // Prim type from added entry
          const TfToken &primType = addedEntry->primType;
          const size_t priority = _GetPriority(primType);
          addedEntries[priority].push_back({path, primType});
        }
        else {
          const _PrimDirtiedEntry &dirtiedEntry = std::get<_PrimDirtiedEntry>(entry);
          // Prim type needs to be pulled from input scene.
          const TfToken primType = _GetInputSceneIndex()->GetPrim(path).primType;
          const size_t priority = _GetPriority(primType);
          dirtiedEntries[priority].push_back({path, dirtiedEntry.dirtyLocators});
        }
      }
