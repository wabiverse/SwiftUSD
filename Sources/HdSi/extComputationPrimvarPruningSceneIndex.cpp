...
class _ExtComputationContext {
 public:
  _ExtComputationContext(const HdSceneIndexBaseRefPtr &si) : _si(si) {}
  ...
  VtValue GetComputedValue(const TfToken &primvarName,
                           const SdfPath &sourceCompId,
                           const TfToken &compOutputName,
                           const Time &shutterOffset)
  {
    TokenValuePairs result = _ExecuteComputationNetwork(sourceCompId, shutterOffset);
    ...
    TF_WARN("Did not find computed value for primvar %s", primvarName.GetText());

    return VtValue();
  }

  bool GetContributingSampleTimesForInterval(const SdfPath &sourceCompId,
                                             const Time startTime,
                                             const Time endTime,
                                             std::vector<Time> *outSampleTimes)
  {
    return _GetSampleTimesFromComputationNetwork(sourceCompId, startTime, endTime, outSampleTimes);
  }

 private:
  using _ComputationDependencyMap = std::unordered_map<SdfPath, SdfPathVector, SdfPath::Hash>;
  ...
  static SdfPathVector _GetInputComputationPaths(HdVectorDataSourceHandle inputCompsDs)
  {
    ...
    for (size_t i = 0; i < inputCompsDs->GetNumElements(); i++) {
      ...
      if (HdContainerDataSourceHandle c = HdContainerDataSource::Cast(inputCompsDs->GetElement(i)))
      {
  ...
  void _GatherComputationSources(const SdfPath &sourceCompId,
                                 ...
                                 _ComputationDependencyMap *compDepMap)
  {
    ...
    while (!compsQueue.empty()) {
      ...
      if (HdExtComputationSchema curCompSchema = HdExtComputationSchema::GetFromParent(
              prim.dataSource))
      {
        ...
        entry = _GetInputComputationPaths(curCompSchema.GetInputComputations());
  ...
  static TokenPairs _GetComputationInputAndSourceOutputNames(HdVectorDataSourceHandle inputCompsDs)
  {
    if (!ARCH_UNLIKELY(inputCompsDs)) {
      return TokenPairs();
    }
    ...
    for (size_t i = 0; i < inputCompsDs->GetNumElements(); i++) {
      ...
      if (HdContainerDataSourceHandle c = HdContainerDataSource::Cast(inputCompsDs->GetElement(i)))
      {
        ...
        if (h1 && h2) {
          result.emplace_back(std::make_pair(h1->GetTypedValue(0.0f), h2->GetTypedValue(0.0f)));
        }
      }
    }
  ...
  static TfTokenVector _GetComputationOutputNames(HdVectorDataSourceHandle outputsDs)
  {
    ...
    for (size_t i = 0; i < outputsDs->GetNumElements(); i++) {
      ...
      if (HdContainerDataSourceHandle c = HdContainerDataSource::Cast(outputsDs->GetElement(i))) {
  ...
  bool _GetSampleTimesFromComputationNetwork(const SdfPath &sourceCompId,
                                             const Time &startTime,
                                             const Time &endTime,
                                             std::vector<Time> *outSampleTimes)
  {
  ...
  // token value pairs.
  //
  TokenValuePairs _ExecuteComputationNetwork(const SdfPath &sourceCompId,
                                             const Time &shutterOffset)
  {
    ...
    SdfPathVector orderedComps;
    {
      ...
      while (!compsQueue.empty()) {
        ...
        while (it != compDepMap.end()) {
          ...
          deps.erase(std::remove(deps.begin(), deps.end(), indComp), deps.end());
    ...
    //
    {
      using _ValueStore = std::unordered_map<TfToken, VtValue, TfToken::HashFunctor>;
      ...
      for (SdfPath const &compId : orderedComps) {
        ...
        HdContainerDataSourceHandle inputsDs = cs.GetInputValues();
        for (TfToken const &name : inputsDs->GetNames()) {

          if (HdSampledDataSourceHandle sds = HdSampledDataSource::Cast(inputsDs->Get(name))) {
        ...
        // ... (attribute) inputs
        for (TfToken const &name : inputsDs->GetNames()) {
          if (ARCH_UNLIKELY(valueStore.find(name) == valueStore.end())) {
            TF_WARN(
                "Couldn't find input %s for computation "
                "%s.",
                name.GetText(),
                compId.GetText());
            continue;
        ...
        TokenPairs inputAndSourceOutputNames = _GetComputationInputAndSourceOutputNames(
        ...
        for (const auto &pair : inputAndSourceOutputNames) {
          ...
          if (ARCH_UNLIKELY(valueStore.find(sourceCompOutputName) == valueStore.end())) {

            TF_WARN(
                "Couldn't find input %s (sourceCompOutput %s) "
                "for computation %s.",
                inputName.GetText(),
          ...
          executionContext.SetInputValue(inputName, valueStore.at(sourceCompOutputName));
        }
        ...
        if (HdExtComputationCallbackDataSourceHandle callbackDs =
                HdExtComputationCallbackDataSource::Cast(cs.GetCpuCallback()))
        {

          callbackDs->Invoke(&executionContext);
        }
        else {
          TF_WARN("Could not find CPU callback data source for %s", compId.GetText());
          continue;
        }
        ...
        if (executionContext.HasComputationError()) {
          ...
          TfTokenVector outputNames = _GetComputationOutputNames(outputsDs);
          ...
          for (const auto &name : outputNames) {
            ...
            if (!executionContext.GetOutputValue(name, &value)) {
              TF_WARN(
                  "Did not find computed output %s for "
                  "computation %s.\n",
                  name.GetText(),
                  compId.GetText());
            }
...
class _SampledExtCompPrimvarDataSource : public HdSampledDataSource {
 public:
  ...
  _SampledExtCompPrimvarDataSource(const HdContainerDataSourceHandle &extCompPrimvarDataSource,
                                   const TfToken &extCompPrimvarName,
                                   _ExtComputationContextSharedPtr ctx)
      : _input(extCompPrimvarDataSource), _primvarName(extCompPrimvarName), _ctx(ctx)
  {
  ...
  VtValue GetValue(Time shutterOffset) override
  {
    ...
    if (h1 && h2) {
      ...
      return _ctx->GetComputedValue(_primvarName, sourceComp, outputName, shutterOffset);
    }
...
class _ExtCompPrimvarDataSource : public HdContainerDataSource {
 public:
  ...
  _ExtCompPrimvarDataSource(const HdContainerDataSourceHandle &extCompPrimvarDataSource,
                            const TfToken &extCompPrimvarName,
                            _ExtComputationContextSharedPtr ctx)
      : _input(extCompPrimvarDataSource), _primvarName(extCompPrimvarName), _ctx(ctx)
  {
  ...
  TfTokenVector GetNames() override
  {
    static const TfTokenVector names = {HdPrimvarSchemaTokens->primvarValue,
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdPrimvarSchemaTokens->interpolation || name == HdPrimvarSchemaTokens->role) {

      return _input->Get(name);
    }

    if (name == HdPrimvarSchemaTokens->primvarValue) {
      return _SampledExtCompPrimvarDataSource::New(_input, _primvarName, _ctx);
    }
...
class _PrimvarsDataSource : public HdContainerDataSource {
 public:
  ...
  _PrimvarsDataSource(const HdContainerDataSourceHandle &primvarsDs,
        ...
        _ctx(std::make_shared<_ExtComputationContext>(si))
  {
    ...
    if (ARCH_UNLIKELY(!extCompPrimvarsDs)) {
      TF_CODING_ERROR(
          "Invalid container data source input provided for "
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (HdContainerDataSourceHandle c = HdContainerDataSource::Cast(_extCompPrimvarsDs->Get(name)))
    {
...
class _PrimDataSource : public HdContainerDataSource {
 public:
  ...
  _PrimDataSource(const HdContainerDataSourceHandle &input, const HdSceneIndexBaseRefPtr &si)
      : _input(input), _si(si)
  {
  ...
  TfTokenVector GetNames() override
  {
    ...
    if (ARCH_UNLIKELY(std::find(names.begin(),
                                names.end(),
                                HdExtComputationPrimvarsSchemaTokens->extComputationPrimvars) !=
                          names.end() &&
                      std::find(names.begin(), names.end(), HdPrimvarsSchemaTokens->primvars) ==
                          names.end()))
    {
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdPrimvarsSchemaTokens->primvars) {
      ...
      if (HdContainerDataSourceHandle c = HdContainerDataSource::Cast(
              _input->Get(HdExtComputationPrimvarsSchemaTokens->extComputationPrimvars)))
      {

        HdExtComputationPrimvarsSchema es(c);
        if (!(es.GetExtComputationPrimvarNames().empty())) {

          HdContainerDataSourceHandle primvarsDs = HdContainerDataSource::Cast(result);

          HdContainerDataSourceHandle &extCompPrimvarsDs = c;

          return _PrimvarsDataSource::New(primvarsDs, extCompPrimvarsDs, _si);
        }
      }
    }

    // Suppress computed primvars.
    if (name == HdExtComputationPrimvarsSchemaTokens->extComputationPrimvars) {
...
/* static */
HdSiExtComputationPrimvarPruningSceneIndexRefPtr HdSiExtComputationPrimvarPruningSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
  return TfCreateRefPtr(new HdSiExtComputationPrimvarPruningSceneIndex(inputSceneIndex));
}

HdSiExtComputationPrimvarPruningSceneIndex::HdSiExtComputationPrimvarPruningSceneIndex(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
    : HdSingleInputFilteringSceneIndexBase(inputSceneIndex)
{
}

HdSceneIndexPrim HdSiExtComputationPrimvarPruningSceneIndex::GetPrim(const SdfPath &primPath) const
{
  HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(primPath);

  const bool isRprim = prim.primType == HdPrimTypeTokens->mesh ||
                       prim.primType == HdPrimTypeTokens->basisCurves ||
                       prim.primType == HdPrimTypeTokens->points;

  if (isRprim && prim.dataSource) {
    prim.dataSource = _PrimDataSource::New(prim.dataSource, _GetInputSceneIndex());
  }

  return prim;
}

SdfPathVector HdSiExtComputationPrimvarPruningSceneIndex::GetChildPrimPaths(
    const SdfPath &primPath) const
{
  return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void HdSiExtComputationPrimvarPruningSceneIndex::_PrimsAdded(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  _SendPrimsAdded(entries);
}

void HdSiExtComputationPrimvarPruningSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  _SendPrimsRemoved(entries);
}

void HdSiExtComputationPrimvarPruningSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  for (const auto &entry : entries) {
    ...
    // authored primvars.
    editedEntries.emplace_back(
        entry.primPath,
        locators.ReplacePrefix(HdExtComputationPrimvarsSchema::GetDefaultLocator(),
