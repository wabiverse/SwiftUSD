...
TF_DEFINE_PRIVATE_TOKENS(_tokens, ((coordSysPrimName, "__coordSys"))(xformDependency));
...
//
bool _IgnoreBinding(const SdfPath &targetedPrimPath)
{
...
SdfPath _PathForCoordSysPrim(const SdfPath &targetedPrimPath, const TfToken &name)
{
  const TfToken propName(SdfPath::JoinIdentifier(TfTokenVector{_tokens->coordSysPrimName, name}));
...
class _CoordSysPrimDataSource : public HdContainerDataSource {
 public:
  ...
  {
    static const TfTokenVector result = {HdCoordSysSchema::GetSchemaToken(),
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdCoordSysSchema::GetSchemaToken()) {
      return HdCoordSysSchema::Builder()
          .SetName(HdRetainedTypedSampledDataSource<TfToken>::New(_name))
          .Build();
    }
    if (name == HdXformSchema::GetSchemaToken()) {
      HdContainerDataSourceHandle const primSource = _inputScene->GetPrim(_primPath).dataSource;
      if (!primSource) {
        return nullptr;
      }
    ...
    if (name == HdDependenciesSchema::GetSchemaToken()) {
      ...
      return HdRetainedContainerDataSource::New(
          ...
          HdDependencySchema::Builder()
              .SetDependedOnPrimPath(HdRetainedTypedSampledDataSource<SdfPath>::New(_primPath))
 ...
 private:
  _CoordSysPrimDataSource(const HdSceneIndexBaseRefPtr &inputScene,
                          ...
                          const TfToken &name)
      : _inputScene(inputScene), _primPath(primPath), _name(name)
  {
  }
...
class _CoordSysBindingDataSource : public HdContainerDataSource {
 ...
 private:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    HdPathDataSourceHandle const ds = HdPathDataSource::Cast(_inputSource->Get(name));
    if (!ds) {
      return nullptr;
    }
...
class _PrimDataSource : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdCoordSysBindingSchema::GetSchemaToken()) {
      HdContainerDataSourceHandle const containerDs = HdContainerDataSource::Cast(ds);
      if (!containerDs) {
        return nullptr;
      }
 ...
 private:
  _PrimDataSource(const HdContainerDataSourceHandle &inputSource) : _inputSource(inputSource) {}

  HdContainerDataSourceHandle const _inputSource;
};

HdSceneIndexObserver::AddedPrimEntries _ToAddedPrimEntries(const SdfPathSet &paths)
{
...
HdSceneIndexObserver::RemovedPrimEntries _ToRemovedPrimEntries(const SdfPathSet &paths)
{
...
HdsiCoordSysPrimSceneIndex::HdsiCoordSysPrimSceneIndex(HdSceneIndexBaseRefPtr const &inputScene)
...
HdContainerDataSourceHandle HdsiCoordSysPrimSceneIndex::_GetCoordSysPrimSource(
    const SdfPath &primPath) const
{
  ...
  return _CoordSysPrimDataSource::New(_GetInputSceneIndex(), parentPrimPath, coordSysName);
}

HdSceneIndexPrim HdsiCoordSysPrimSceneIndex::GetPrim(const SdfPath &primPath) const
{
  if (HdContainerDataSourceHandle const coordSysPrimSource = _GetCoordSysPrimSource(primPath)) {
...
SdfPathVector HdsiCoordSysPrimSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
...
void HdsiCoordSysPrimSceneIndex::_AddBindingsForPrim(const SdfPath &primPath,
                                                     SdfPathSet *const addedCoordSysPrims)
{
  HdCoordSysBindingSchema schema = HdCoordSysBindingSchema::GetFromParent(
  ...
  for (const TfToken &name : schema.GetContainer()->GetNames()) {
    ...
    size_t &refCount = _targetedPrimToNameToRefCount[targetedPrimPath][name];
    if (refCount == 0) {
      if (addedCoordSysPrims) {
        addedCoordSysPrims->insert(_PathForCoordSysPrim(targetedPrimPath, name));
      }
    }
    refCount++;
...
void HdsiCoordSysPrimSceneIndex::_RemoveBindings(const _Bindings &bindings,
                                                 SdfPathSet *const removedCoordSysPrims)
{
  for (const _Binding &binding : bindings) {
    ...
    if (removedCoordSysPrims) {
      removedCoordSysPrims->insert(_PathForCoordSysPrim(binding.path, binding.name));
    }
...
void HdsiCoordSysPrimSceneIndex::_RemoveBindingsForPrim(const SdfPath &primPath,
                                                        SdfPathSet *const removedCoordSysPrims)
{
...
void HdsiCoordSysPrimSceneIndex::_RemoveBindingsForSubtree(const SdfPath &primPath,
                                                           SdfPathSet *const removedCoordSysPrims)
{
...
void HdsiCoordSysPrimSceneIndex::_PrimsAdded(const HdSceneIndexBase &sender,
                                             const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
    _RemoveBindingsForPrim(entry.primPath, isObserved ? &removedCoordSysPrims : nullptr);
    _AddBindingsForPrim(entry.primPath, isObserved ? &addedCoordSysPrims : nullptr);
  }
...
void HdsiCoordSysPrimSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
    if (entry.dirtyLocators.Intersects(HdCoordSysBindingSchema::GetDefaultLocator())) {
      _RemoveBindingsForPrim(entry.primPath, isObserved ? &removedCoordSysPrims : nullptr);
      _AddBindingsForPrim(entry.primPath, isObserved ? &addedCoordSysPrims : nullptr);
    }
  }
...
void HdsiCoordSysPrimSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  ...
  if (!_primToBindings.empty()) {
    for (const HdSceneIndexObserver::RemovedPrimEntry &entry : entries) {
      _RemoveBindingsForSubtree(entry.primPath, isObserved ? &removedCoordSysPrims : nullptr);
    }
  }
