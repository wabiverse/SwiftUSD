...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (fps));
...
bool _PrimTypeUsesVelocity(const TfToken &primType)
{
  static const TfToken::Set types{HdPrimTypeTokens->points,
...
bool _PrimvarAffectedByVelocity(const TfToken &primvar)
{
  static const TfToken::Set primvars{HdPrimvarsSchemaTokens->points,
...
template<typename T>
VtValue _ApplyAngularVelocities(const VtArray<T> &rotations,
                                ...
                                const HdSampledDataSource::Time scaledTime)
{
  ...
  for (size_t i = 0; i < rotations.size(); ++i) {
    ...
    rotation *= GfRotation(velocities[i], scaledTime * velocities[i].GetLength());
    result[i] = T(rotation.GetQuat());
  }
  return VtValue(result);
}

VtValue _ApplyAngularVelocities(const VtValue &rotations,
                                const VtVec3fArray &velocities,
                                const HdSampledDataSource::Time scaledTime)
{
  if (rotations.IsHolding<VtQuathArray>()) {
    return _ApplyAngularVelocities(rotations.UncheckedGet<VtQuathArray>(), velocities, scaledTime);
  }
  if (rotations.IsHolding<VtQuatfArray>()) {
    return _ApplyAngularVelocities(rotations.UncheckedGet<VtQuatfArray>(), velocities, scaledTime);
  }
...
class _VelocityHelper {
 public:
  ...
  _VelocityHelper(const TfToken &name,
 ...
 protected:
  bool _GetContributingSampleTimesForInterval(Time startTime,
                                              Time endTime,
  ...
  VtValue _GetValue(Time shutterOffset)
  {
    VtVec3fArray velocities;
    VtValue sourceVal;
    Time sampleTime;
    if (!_VelocityMotionValidForCurrentFrame(&sourceVal, &velocities, &sampleTime)) {
    ...
    if (useAccelerations) {
      ...
      for (size_t i = 0; i < positions.size(); ++i) {
        result[i] = positions[i] + scaledTime * velocities[i] + timeSqrHalf * accelerations[i];
      }
    }
    else {
      for (size_t i = 0; i < positions.size(); ++i) {
        result[i] = positions[i] + scaledTime * velocities[i];
      }
    }
    return VtValue(result);
  }
 ...
 private:
  ...
  VtVec3fArray _GetAccelerations(const Time sampleTime) const
  {
    static const VtVec3fArray empty{};
    static const HdDataSourceLocator accelerationsLocator{HdPrimvarsSchema::GetSchemaToken(),
    ...
    if (!accelerationsDs->GetContributingSampleTimesForInterval(0.0, 0.0, &times)) {
  ...
  /// value is false.
  bool _VelocityMotionValidForCurrentFrame(VtValue *srcValue = nullptr,
                                           VtVec3fArray *velocities = nullptr,
                                           Time *outSampleTime = nullptr) const
  {
    const HdDataSourceLocator velocitiesLocator{HdPrimvarsSchema::GetSchemaToken(),
                                                _name == HdInstancerTokens->instanceRotations ?
                                                    HdTokens->angularVelocities :
                                                    HdTokens->velocities,
                                                HdPrimvarSchemaTokens->primvarValue};
    const HdSampledDataSourceHandle velocitiesDs = HdSampledDataSource::Cast(
        HdContainerDataSource::Get(_primSource, velocitiesLocator));
    if (!velocitiesDs) {
      // velocities not present
      return false;
    }
    std::vector<std::vector<Time>> times(2);
    if (!_source->GetContributingSampleTimesForInterval(0.0, 0.0, &times[0])) {
      // source has constant value across all time; no velocity motion
      return false;
    }
    if (!velocitiesDs->GetContributingSampleTimesForInterval(0.0, 0.0, &times[1])) {
    ...
    if (_name == HdInstancerTokens->instanceRotations) {
      if (!(sourceVal.IsHolding<VtQuathArray>() || sourceVal.IsHolding<VtQuatfArray>())) {
  ...
  int _GetNonlinearSampleCount()
  {
    static const int defaultValue = 3;  // From UsdGeomMotionAPI
    static const HdDataSourceLocator locator = {HdPrimvarsSchema::GetSchemaToken(),
                                                HdTokens->nonlinearSampleCount,
                                                HdPrimvarSchemaTokens->primvarValue};
    const auto ds = HdSampledDataSource::Cast(HdContainerDataSource::Get(_primSource, locator));
    if (!ds) {
      return defaultValue;
    }
...
class _UntypedValueDataSource final : public HdSampledDataSource, private _VelocityHelper {
 public:
  using Time = HdSampledDataSource::Time;

  HD_DECLARE_DATASOURCE(_UntypedValueDataSource);

  _UntypedValueDataSource(const TfToken &name,
                          const HdSampledDataSourceHandle &source,
                          const SdfPath &primPath,
                          const HdContainerDataSourceHandle &primSource,
                          const HdContainerDataSourceHandle &inputArgs)
  ...
  {
    return _GetContributingSampleTimesForInterval(startTime, endTime, outSampleTimes);
  }
};
...
template<typename T>
class _TypedValueDataSource final : public HdTypedSampledDataSource<T>, private _VelocityHelper {
 public:
  using Time = HdSampledDataSource::Time;

  HD_DECLARE_DATASOURCE_ABSTRACT(_TypedValueDataSource<T>);

  _TypedValueDataSource(const TfToken &name,
                        const HdSampledDataSourceHandle &source,
                        const SdfPath &primPath,
                        const HdContainerDataSourceHandle &primSource,
                        const HdContainerDataSourceHandle &inputArgs)
  ...
  static typename _TypedValueDataSource<T>::Handle New(
      const TfToken &name,
      const HdSampledDataSourceHandle &source,
      const SdfPath &primPath,
      const HdContainerDataSourceHandle &primSource,
      const HdContainerDataSourceHandle &inputArgs)
  {
    return _TypedValueDataSource<T>::Handle(
        new _TypedValueDataSource<T>(name, source, primPath, primSource, inputArgs));
  }
};
...
struct _PrimvarSourceTypeVisitor {
  ...
  template<typename T> HdDataSourceBaseHandle operator()(const T &)
  {
    return _TypedValueDataSource<T>::New(name, source, primPath, primSource, inputArgs);
  }

  HdDataSourceBaseHandle operator()(const VtValue &)
  {
    return _UntypedValueDataSource::New(name, source, primPath, primSource, inputArgs);
  }
};
...
class _PrimvarDataSource final : public HdContainerDataSource {
 public:
  ...
  _PrimvarDataSource(const TfToken &name,
    ...
    if (ds && name == HdPrimvarSchemaTokens->primvarValue) {
      if (const auto source = HdSampledDataSource::Cast(ds)) {
        ...
        return VtVisitValue(
            ...
            _PrimvarSourceTypeVisitor{_name, source, _primPath, _primSource, _inputArgs});
      }
    }
    return ds;
  }
...
class _PrimvarsDataSource final : public HdContainerDataSource {
 public:
  ...
  _PrimvarsDataSource(const HdContainerDataSourceHandle &source,
                      const SdfPath &primPath,
                      const HdContainerDataSourceHandle &primSource,
                      const HdContainerDataSourceHandle &inputArgs)
      : _source(source), _primPath(primPath), _primSource(primSource), _inputArgs(inputArgs)
  {
  }
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (ds && _PrimvarAffectedByVelocity(name)) {
      return _PrimvarDataSource::New(
          name, HdContainerDataSource::Cast(ds), _primPath, _primSource, _inputArgs);
    }
    return ds;
  }
...
class _PrimDataSource final : public HdContainerDataSource {
 public:
  ...
  _PrimDataSource(const SdfPath &primPath,
                  const HdContainerDataSourceHandle &primSource,
                  const HdContainerDataSourceHandle &inputArgs)
      : _primPath(primPath), _primSource(primSource), _inputArgs(inputArgs)
  {
  }
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (ds && name == HdPrimvarsSchema::GetSchemaToken()) {
      return _PrimvarsDataSource::New(
          HdContainerDataSource::Cast(ds), _primPath, _primSource, _inputArgs);
    }
    return ds;
  }
...
HdsiVelocityMotionResolvingSceneIndexRefPtr HdsiVelocityMotionResolvingSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex, const HdContainerDataSourceHandle &inputArgs)
{
  return TfCreateRefPtr(new HdsiVelocityMotionResolvingSceneIndex(inputSceneIndex, inputArgs));
}
...
HdSceneIndexPrim HdsiVelocityMotionResolvingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(primPath);
  if (_PrimTypeUsesVelocity(prim.primType)) {
    prim.dataSource = _PrimDataSource::New(primPath, prim.dataSource, _inputArgs);
  }
  return prim;
}

SdfPathVector HdsiVelocityMotionResolvingSceneIndex::GetChildPrimPaths(
    const SdfPath &primPath) const
{
  return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void HdsiVelocityMotionResolvingSceneIndex::_PrimsAdded(
    const HdSceneIndexBase & /*sender*/, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  return _SendPrimsAdded(entries);
}

void HdsiVelocityMotionResolvingSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase & /*sender*/, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  return _SendPrimsRemoved(entries);
}

void HdsiVelocityMotionResolvingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase & /*sender*/, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  static const HdDataSourceLocatorSet positionsAffectingLocators{
      HdPrimvarsSchema::GetDefaultLocator().Append(HdTokens->velocities),
      HdPrimvarsSchema::GetDefaultLocator().Append(HdTokens->accelerations),
      HdPrimvarsSchema::GetDefaultLocator().Append(HdTokens->nonlinearSampleCount)};
  ...
  static const HdDataSourceLocatorSet rotationsAffectingLocators{
      HdPrimvarsSchema::GetDefaultLocator().Append(HdTokens->angularVelocities),
      HdPrimvarsSchema::GetDefaultLocator().Append(HdTokens->nonlinearSampleCount)};
