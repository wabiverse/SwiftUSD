...
TF_DEBUG_CODES(HDSI_LIGHT_LINK_COLLECTION_CACHE, HDSI_LIGHT_LINK_VERBOSE);
...
TF_DEFINE_PUBLIC_TOKENS(HdsiLightLinkingSceneIndexTokens, HDSI_LIGHT_LINKING_SCENE_INDEX_TOKENS);

TF_DEFINE_PRIVATE_TOKENS(_tokens, ((groupPrefix, "group_")));

static bool _LogDebugMsg()
{
...
struct _Cache {
 public:
  ...
  _Cache(const HdSceneIndexBaseRefPtr &inputSceneIndex) : _si(inputSceneIndex) {}

  // Updates tables and adds invalidation notices when processing a new
  // collection or when its expression has changed.
  //
  void UpdateCollection(const SdfPath &primPath,
                        ...
                        HdSceneIndexObserver::DirtiedPrimEntries *dirtiedEntries)
  {
    ...
    const SdfPath collectionPath = _MakeCollectionPath(primPath, collectionName);

    // Have we seen this collection before?
    const auto pathIdEntry = _collectionPathToId.find(collectionPath);
    const bool collectionExists = (pathIdEntry != _collectionPathToId.end());
    ...
    if (collectionExists) {
      ...
      if (curExpr == expr) {
        TF_DEBUG(HDSI_LIGHT_LINK_VERBOSE)
            .Msg(
    ...
    if (IsTrivial(expr)) {
      ...
      TF_DEBUG(HDSI_LIGHT_LINK_VERBOSE)
          .Msg("* UpdateCollection -- Expression for %s is trivial.\n", collectionPath.GetText());
    ...
    if (isNewExpr) {
      ...
      if (_LogDebugMsg()) {
        TfDebug::Helper().Msg("* UpdateCollection -- Using shared ID %s for collection %s.\n",
                              id.GetText(),
                              collectionPath.GetText());
      }
    }
  ...
  void RemoveCollection(const SdfPath &primPath,
                        const TfToken &collectionName,
                        HdSceneIndexObserver::DirtiedPrimEntries *dirtiedEntries)
  {
    const SdfPath collectionPath = _MakeCollectionPath(primPath, collectionName);
  ...
  //
  TfTokenVector ComputeCategoriesForPrimPath(const SdfPath &primPath) const
  {
    TRACE_FUNCTION();
  ...
  //
  bool GetCategoryIdForLightLinkingCollection(const SdfPath &primPath,
                                              const TfToken &collectionName,
                                              TfToken *id)
  {
  ...
  static bool IsTrivial(const SdfPathExpression &expr)
  {
    ...
    return expr == SdfPathExpression::Everything() || expr == everythingButProperties;
  }
 ...
 private:
  ...
  //       is being removed.
  //
  void _RemoveCollection(const SdfPath &collectionPath,
                         HdSceneIndexObserver::DirtiedPrimEntries *dirtiedEntries)
  {
    const auto pathIdEntry = _collectionPathToId.find(collectionPath);
    if (pathIdEntry == _collectionPathToId.end()) {
    ...
    if (sharingId) {
      ...
      //
      if (_LogDebugMsg()) {
        TfDebug::Helper().Msg("   * Removing cache entries referencing Id (%s).\n", id.GetText());
      }
  ...
  TfToken _GetNewCategoryId()
  {
    std::string strId = _tokens->groupPrefix.GetString() + std::to_string(_groupIdx++);

    // We expect the number of unique expressions to be in the 100s,
    // not 2^64.
    if (_groupIdx == 0) {
      TF_CODING_ERROR("Overflow detected when computing the category ID.\n");
    }
    return TfToken(strId);
  }

  void _InvalidateCategoriesOnTargets(const SdfPathVector &targets,
                                      HdSceneIndexObserver::DirtiedPrimEntries *dirtiedEntries)
  {
    if (_LogDebugMsg()) {
      TfDebug::Helper().Msg("   * Invalidating categories on %zu targets ....\n", targets.size());
    }

    for (const auto targetPath : targets) {
      dirtiedEntries->push_back(HdSceneIndexObserver::DirtiedPrimEntry(
          targetPath, HdCategoriesSchema::GetDefaultLocator()));
    }
  }

  void _InvalidateLight(const SdfPath &primPath,
                        const TfToken &collectionName,
                        HdSceneIndexObserver::DirtiedPrimEntries *dirtiedEntries)
  {
    if (_LogDebugMsg()) {
      TfDebug::Helper().Msg("   * Invalidating category ID for %s:%s....\n",
                            primPath.GetText(),
                            collectionName.GetText());
    }

    dirtiedEntries->push_back(HdSceneIndexObserver::DirtiedPrimEntry(
        primPath, HdLightSchema::GetDefaultLocator().Append(collectionName)));
  }

  SdfPathVector _ComputeAllMatches(const _Eval &eval)
  {
    constexpr auto matchKind = HdCollectionExpressionEvaluator::MatchAll;

    SdfPathVector resultVec;
    // XXX This doesn't support instancing.
    eval.PopulateMatches(SdfPath::AbsoluteRootPath(), matchKind, &resultVec);

    return resultVec;
  }

  HdCollectionExpressionEvaluator _MakePathExpressionEvaluator(const SdfPathExpression &expr) const
  {
    ...
    //
    return HdCollectionExpressionEvaluator(_si, expr, HdGetCollectionPredicateLibrary());
  }

  static SdfPath _MakeCollectionPath(const SdfPath &primPath, const TfToken &collectionName)
  {
 ...
 private:
  ...
  //
  using _ExprToCategoryIdAndEvalMap = std::unordered_map<_Expr, _CategoryIdAndEval, TfHash>;
  ...
  //
  using _CollectionPathToCategoryIdMap = std::unordered_map<SdfPath, _CategoryId, TfHash>;
  ...
  //
  using _CategoryIdToCollectionPathsMap = std::unordered_map<_CategoryId, SdfPathSet, TfHash>;
...
const TfTokenVector &_GetAllLinkingCollectionNames()
{
  static const TfTokenVector names = {
      HdTokens->lightLink, HdTokens->shadowLink, HdTokens->filterLink};
...
const TfTokenVector &_GetLightLinkingSchemaTokens()
{
  static const TfTokenVector names = {
      HdTokens->lightLink,
      HdTokens->shadowLink,
...
//
VtArray<TfToken> _GetPrimTypes(const HdContainerDataSourceHandle &inputArgs,
                               ...
                               const VtArray<TfToken> &fallback)
{
  ...
  if (HdTokenArrayDataSourceHandle tokenArrayHandle = HdTokenArrayDataSource::Cast(
          inputArgs->Get(argName)))
  {
...
bool _Contains(const VtArray<TfToken> &tokens, const TfToken &key)
{
  return std::find(tokens.cbegin(), tokens.cend(), key) != tokens.cend();
}

bool _Contains(const TfTokenVector &tokens, const TfToken &key)
{
  return std::find(tokens.cbegin(), tokens.cend(), key) != tokens.cend();
}

void _AddIfNecessary(const TfToken &token, TfTokenVector *tokens)
{
...
//
HdContainerDataSourceHandle _BuildCategoriesDataSource(const _CacheSharedPtr &cache,
                                                       const SdfPath &primPath)
{
  const TfTokenVector categories = cache->ComputeCategoriesForPrimPath(primPath);

  if (categories.empty()) {
    return nullptr;
  }

  return HdCategoriesSchema::BuildRetained(categories.size(),
                                           categories.data(),
                                           /*excludedNameCount = */ 0,
...
class _GprimDataSource : public HdContainerDataSource {
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdCategoriesSchemaTokens->categories) {
      ...
      return HdOverlayContainerDataSource::OverlayedContainerDataSources(
          _BuildCategoriesDataSource(_cache, _primPath), HdContainerDataSource::Cast(result));
    }
 ...
 private:
  _GprimDataSource(const HdContainerDataSourceHandle &primContainer,
                   const SdfPath &primPath,
                   const _CacheSharedPtr &cache)
      : _inputPrimDs(primContainer), _primPath(primPath), _cache(cache)
  {
  }
...
class _InstancerPrimDataSource : public HdContainerDataSource {
 public:
  ...
  {
    TfTokenVector names = _inputPrimDs->GetNames();
    _AddIfNecessary(HdInstanceCategoriesSchemaTokens->instanceCategories, &names);
    return names;
  }
 ...
 private:
  _InstancerPrimDataSource(const HdContainerDataSourceHandle &primContainer,
                           const SdfPath &primPath,
                           const _CacheSharedPtr &cache)
      : _inputPrimDs(primContainer), _primPath(primPath), _cache(cache)
  {
...
class _LightDataSource : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (_Contains(schemaTokens, name)) {
      ...
      const TfToken &collectionName = name == HdTokens->lightFilterLink ? HdTokens->filterLink :
                                                                          name;
      ...
      //
      const auto collectionsSchema = HdCollectionsSchema::GetFromParent(_primDs);

      if (collectionsSchema.GetCollection(collectionName)) {
        TfToken id;
        if (_cache->GetCategoryIdForLightLinkingCollection(_primPath, collectionName, &id)) {

          return HdRetainedTypedSampledDataSource<TfToken>::New(id);
        }

        static const auto trivialIdDs = HdRetainedTypedSampledDataSource<TfToken>::New(TfToken());
        return trivialIdDs;
      }
    }
 ...
 private:
  _LightDataSource(const HdContainerDataSourceHandle &primContainer,
                   const HdContainerDataSourceHandle &lightContainer,
                   const SdfPath &primPath,
                   const _CacheSharedPtr &cache)
      : _primDs(primContainer), _lightDs(lightContainer), _primPath(primPath), _cache(cache)
  {
...
class _LightPrimDataSource : public HdContainerDataSource {
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdLightSchemaTokens->light) {
      return _LightDataSource::New(
          _inputPrimDs, HdContainerDataSource::Cast(result), _primPath, _cache);
    }
 ...
 private:
  _LightPrimDataSource(const HdContainerDataSourceHandle &primContainer,
                       const SdfPath &primPath,
                       const _CacheSharedPtr &cache)
      : _inputPrimDs(primContainer), _primPath(primPath), _cache(cache)
  {
...
static const VtArray<TfToken> GEOMETRY_PRIM_TYPES = {HdRprimTypeTokens->allTokens.begin(),
...
static const VtArray<TfToken> LIGHT_FILTER_PRIM_TYPES = {
    HdLightFilterTypeTokens->allTokens.begin(), HdLightFilterTypeTokens->allTokens.end()};

HdSceneIndexBaseRefPtr HdsiLightLinkingSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex, const HdContainerDataSourceHandle &inputArgs)
{
  HdSceneIndexBaseRefPtr sceneIndex = TfCreateRefPtr(
...
HdsiLightLinkingSceneIndex::HdsiLightLinkingSceneIndex(
    ...
    : HdSingleInputFilteringSceneIndexBase(inputSceneIndex),
      ...
      _lightPrimTypes(_GetPrimTypes(
          inputArgs, HdsiLightLinkingSceneIndexTokens->lightPrimTypes, LIGHT_PRIM_TYPES)),
      _lightFilterPrimTypes(_GetPrimTypes(inputArgs,
                                          HdsiLightLinkingSceneIndexTokens->lightFilterPrimTypes,
                                          LIGHT_FILTER_PRIM_TYPES)),
      _geometryPrimTypes(_GetPrimTypes(
          inputArgs, HdsiLightLinkingSceneIndexTokens->geometryPrimTypes, GEOMETRY_PRIM_TYPES))
{
}

HdSceneIndexPrim HdsiLightLinkingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  TRACE_FUNCTION();
  ...
  if (prim.dataSource) {
    if (_IsGeometry(prim.primType)) {
      prim.dataSource = _GprimDataSource::New(prim.dataSource, primPath, _cache);
    }
    else if (prim.primType == HdPrimTypeTokens->instancer) {
      prim.dataSource = _InstancerPrimDataSource::New(prim.dataSource, primPath, _cache);
    }
    else if (_IsLight(prim.primType) || _IsLightFilter(prim.primType)) {

      prim.dataSource = _LightPrimDataSource::New(prim.dataSource, primPath, _cache);
    }
  }
...
SdfPathVector HdsiLightLinkingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  // This scene index doesn't change the topology.
  return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void HdsiLightLinkingSceneIndex::_PrimsAdded(const HdSceneIndexBase &sender,
  ...
  for (const auto &entry : entries) {
    if (_IsLight(entry.primType)) {
      ...
      _ProcessAddedLightOrFilter(
          entry, {HdTokens->lightLink, HdTokens->shadowLink}, &dirtiedEntries);
    }
    else if (_IsLightFilter(entry.primType)) {
      // Update internal tracking.
      _lightAndFilterPrimPaths.insert(entry.primPath);

      _ProcessAddedLightOrFilter(entry, {HdTokens->filterLink}, &dirtiedEntries);
    }
  }
...
void HdsiLightLinkingSceneIndex::_ProcessAddedLightOrFilter(
    const HdSceneIndexObserver::AddedPrimEntry &entry,
    const TfTokenVector &collectionNames,
    HdSceneIndexObserver::DirtiedPrimEntries *dirtiedEntries)
{
  TF_DEBUG(HDSI_LIGHT_LINK_VERBOSE)
      .Msg("Processing added notice for %s.\n", entry.primPath.GetText());

  const HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(entry.primPath);

  HdCollectionsSchema collectionsSchema = HdCollectionsSchema::GetFromParent(prim.dataSource);

  for (const TfToken &colName : collectionNames) {

    HdCollectionSchema colSchema = collectionsSchema.GetCollection(colName);
    ...
    if (const auto exprDs = colSchema.GetMembershipExpression()) {
      ...
      if (HdsiLightLinkingSceneIndex_Impl::_Cache::IsTrivial(expr)) {
        ...
        TF_DEBUG(HDSI_LIGHT_LINK_VERBOSE)
            .Msg("   ... %s:%s is trivial. Nothing to do.\n",
                 entry.primPath.GetText(),
                 colName.GetText());
      ...
      _cache->UpdateCollection(entry.primPath, colName, expr, dirtiedEntries);
    }
  }
}

void HdsiLightLinkingSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  ...
  for (const auto &entry : entries) {
    ...
    const auto itRange = SdfPathFindPrefixedRange(
        _lightAndFilterPrimPaths.begin(), _lightAndFilterPrimPaths.end(), entry.primPath);
    ...
    TF_DEBUG(HDSI_LIGHT_LINK_VERBOSE)
        .Msg("Processing removed notice for %s.\n", entry.primPath.GetText());
    ...
    for (auto it = begin; it != end; ++it) {
      ...
      for (const TfToken &colName : _GetAllLinkingCollectionNames()) {
        _cache->RemoveCollection(trackedPrimPath, colName, &dirtiedEntries);
      }
    }
...
void HdsiLightLinkingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  static const HdDataSourceLocatorSet collectionLocators = {
      ...
      HdCollectionsSchema::GetDefaultLocator().Append(HdTokens->shadowLink),
      HdCollectionsSchema::GetDefaultLocator().Append(HdTokens->filterLink),
  };
  ...
  for (const auto &entry : entries) {
    ...
    if (_lightAndFilterPrimPaths.find(primPath) == _lightAndFilterPrimPaths.end()) {
      continue;
    }
    ...
    for (const auto &locator : collectionLocators) {
      ...
      HdCollectionSchema colSchema = collectionsSchema.GetCollection(collectionName);
      if (!colSchema) {
        continue;
      }
      ...
      if (const auto exprDs = colSchema.GetMembershipExpression()) {
        ...
        const SdfPathExpression expr = exprDs->GetTypedValue(0.0);

        _cache->UpdateCollection(primPath, collectionName, expr, &newEntries);
      }
      else {
...
bool HdsiLightLinkingSceneIndex::_IsLight(const TfToken &primType) const
{
  return _Contains(_lightPrimTypes, primType);
}

bool HdsiLightLinkingSceneIndex::_IsLightFilter(const TfToken &primType) const
{
  return _Contains(_lightFilterPrimTypes, primType);
}

bool HdsiLightLinkingSceneIndex::_IsGeometry(const TfToken &primType) const
{
