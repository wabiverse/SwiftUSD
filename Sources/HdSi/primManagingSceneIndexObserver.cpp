...
HdsiPrimManagingSceneIndexObserver::PrimBase::~PrimBase() = default;

HdsiPrimManagingSceneIndexObserver::PrimFactoryBase::~PrimFactoryBase() = default;

static HdsiPrimManagingSceneIndexObserver::PrimFactoryBaseHandle _GetPrimFactory(
    HdContainerDataSourceHandle const &inputArgs)
{
  ...
  using DataSource =
      HdTypedSampledDataSource<HdsiPrimManagingSceneIndexObserver::PrimFactoryBaseHandle>;
  typename DataSource::Handle const ds = DataSource::Cast(
      inputArgs->Get(HdsiPrimManagingSceneIndexObserverTokens->primFactory));
  if (!ds) {
    return nullptr;
  }
...
HdsiPrimManagingSceneIndexObserver::HdsiPrimManagingSceneIndexObserver(
    HdSceneIndexBaseRefPtr const &sceneIndex, HdContainerDataSourceHandle const &inputArgs)
    : _sceneIndex(sceneIndex), _primFactory(_GetPrimFactory(inputArgs))
{
  if (!sceneIndex) {
    return;
  }
...
HdsiPrimManagingSceneIndexObserver::~HdsiPrimManagingSceneIndexObserver() = default;

const HdsiPrimManagingSceneIndexObserver::PrimBaseHandle &HdsiPrimManagingSceneIndexObserver::
    GetPrim(const SdfPath &primPath) const
{
