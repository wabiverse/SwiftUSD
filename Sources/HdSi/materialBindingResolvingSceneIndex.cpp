...
class _MaterialBindingsDataSource final : public HdContainerDataSource {
 public:
  ...
  _MaterialBindingsDataSource(const HdContainerDataSourceHandle &input,
                              const TfTokenVector &purposePriorityOrder,
                              const TfToken &dstPurpose)
      : _input(input), _purposePriorityOrder(purposePriorityOrder), _dstPurpose(dstPurpose)
  {
  }
 ...
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdMaterialBindingsSchema::GetSchemaToken()) {
      if (auto materialBindingContainer = HdContainerDataSource::Cast(data)) {
...
// static
HdsiMaterialBindingResolvingSceneIndexRefPtr HdsiMaterialBindingResolvingSceneIndex::New(
...
HdSceneIndexPrim HdsiMaterialBindingResolvingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  if (auto input = _GetInputSceneIndex()) {
    HdSceneIndexPrim prim = input->GetPrim(primPath);
    if (prim.dataSource) {
      prim.dataSource = _PrimDataSource::New(prim.dataSource, _purposePriorityOrder, _dstPurpose);
    }
    return prim;
  }
...
SdfPathVector HdsiMaterialBindingResolvingSceneIndex::GetChildPrimPaths(
...
HdsiMaterialBindingResolvingSceneIndex::~HdsiMaterialBindingResolvingSceneIndex() = default;
