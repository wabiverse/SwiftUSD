...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    (materialPrimvarTransfer_dep)(materialPrimvarTransfer_primvarsToBinding)(materialPrimvarTransfer_primvarsToMaterial));
...
class _PrimDataSource final : public HdContainerDataSource {
 public:
  ...
  _PrimDataSource(const HdSceneIndexBaseRefPtr &inputScene,
                  const HdContainerDataSourceHandle &inputDs)
      : _inputScene(inputScene), _inputDs(inputDs)
  {
  }
    ...
    if (_inputDs) {
      ...
      if (HdMaterialBindingsSchema::GetFromParent(_inputDs)) {
        for (const TfToken &name :
             {HdPrimvarsSchema::GetSchemaToken(), HdDependenciesSchema::GetSchemaToken()})
        {
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdDependenciesSchema::GetSchemaToken()) {
      if (HdPathDataSourceHandle pathDs = _GetMaterialBindingPath()) {
        ...
        HdContainerDataSourceHandle depsDs = HdRetainedContainerDataSource::New(
            ...
            HdDependencySchema::Builder()
                .SetDependedOnPrimPath(nullptr)  // self
                .SetDependedOnDataSourceLocator(materialBindingsLocDs)
                .SetAffectedDataSourceLocator(primvarsToMaterialLocDs)
                .Build());

        if (HdContainerDataSourceHandle inputResultContainer = HdContainerDataSource::Cast(
                inputResult))
        {
          return HdOverlayContainerDataSource::New(depsDs, inputResultContainer);
        }
        else {
          return depsDs;
        }
      ...
      if (HdPathDataSourceHandle pathDs = _GetMaterialBindingPath()) {
        ...
        if (HdPrimvarsSchema pvSchema = HdPrimvarsSchema::GetFromParent(prim.dataSource)) {
          if (HdContainerDataSourceHandle inputResultContainer = HdContainerDataSource::Cast(
                  inputResult))
          {
            // local primvars have stronger opinion
            return HdOverlayContainerDataSource::New(inputResultContainer,
                                                     pvSchema.GetContainer());
          }
          else {
 ...
 private:
  ...
  {
    HdMaterialBindingsSchema materialBindings = HdMaterialBindingsSchema::GetFromParent(_inputDs);
...
HdsiMaterialPrimvarTransferSceneIndex::~HdsiMaterialPrimvarTransferSceneIndex() = default;

HdsiMaterialPrimvarTransferSceneIndexRefPtr HdsiMaterialPrimvarTransferSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
  return TfCreateRefPtr(new HdsiMaterialPrimvarTransferSceneIndex(inputSceneIndex));
}

HdSceneIndexPrim HdsiMaterialPrimvarTransferSceneIndex::GetPrim(const SdfPath &primPath) const
{
...
SdfPathVector HdsiMaterialPrimvarTransferSceneIndex::GetChildPrimPaths(
