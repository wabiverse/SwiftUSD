...
exr_result_t exr_add_part(exr_context_t ctxt,
                          const char *partname,
                          exr_storage_t type,
                          int *new_index)
{
  ...
  switch (type) {
    ...
    default:
      ...
      return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid storage type %d for new part", (int)type));
  }

  rv = exr_attr_list_add_static_name(
      ctxt, &(part->attributes), EXR_REQ_TYPE_STR, EXR_ATTR_STRING, 0, NULL, &(part->type));
  ...
  if (partname && partname[0] != '\0') {
    ...
    rv = exr_attr_list_add_static_name(
        ctxt, &(part->attributes), EXR_REQ_NAME_STR, EXR_ATTR_STRING, 0, NULL, &(part->name));
  ...
  {
    rv = exr_attr_list_add_static_name(
        ctxt, &(part->attributes), EXR_REQ_VERSION_STR, EXR_ATTR_INT, 0, NULL, &(part->version));
    if (rv == EXR_ERR_SUCCESS)
      part->version->i = 1;
  ...
  if (rv == EXR_ERR_SUCCESS) {
    if (pctxt->num_parts > 1)
      pctxt->is_multipart = 1;

    if (!pctxt->has_nonimage_data && pctxt->num_parts == 1 && type == EXR_STORAGE_TILED)
...
exr_result_t exr_get_tile_levels(exr_const_context_t ctxt,
                                 int part_index,
                                 int *levelsx,
                                 int *levelsy)
{
  ...
  if (part->storage_mode == EXR_STORAGE_TILED || part->storage_mode == EXR_STORAGE_DEEP_TILED) {
        ...
        !part->tile_level_tile_count_x || !part->tile_level_tile_count_y)
    {
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(
          pctxt->print_error(pctxt, EXR_ERR_MISSING_REQ_ATTR, "Tile data missing or corrupt"));
    }
...
exr_result_t exr_get_tile_sizes(exr_const_context_t ctxt,
                                int part_index,
                                int levelx,
                                int levely,
                                int32_t *tilew,
...
exr_result_t exr_get_level_sizes(
    exr_const_context_t ctxt, int part_index, int levelx, int levely, int32_t *levw, int32_t *levh)
{
  EXR_PROMOTE_CONST_CONTEXT_AND_PART_OR_ERROR(ctxt, part_index);
...
exr_result_t exr_get_chunk_count(exr_const_context_t ctxt, int part_index, int32_t *out)
{
  ...
  if (part->dataWindow) {
    if (part->storage_mode == EXR_STORAGE_TILED || part->storage_mode == EXR_STORAGE_DEEP_TILED) {
      ...
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(
          pctxt->report_error(pctxt, EXR_ERR_MISSING_REQ_ATTR, "Tile data missing or corrupt"));
    }
             ...
             part->storage_mode == EXR_STORAGE_DEEP_SCANLINE)
    {
      ...
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->report_error(
          pctxt, EXR_ERR_MISSING_REQ_ATTR, "Missing scanline chunk compression information"));
    }
  }

  return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->report_error(
      pctxt, EXR_ERR_MISSING_REQ_ATTR, "Missing data window for chunk information"));
}
...
exr_result_t exr_get_dwa_compression_level(exr_const_context_t ctxt, int part_index, float *level)
{
  float l;
  EXR_PROMOTE_CONST_CONTEXT_AND_PART_OR_ERROR(ctxt, part_index);
...
exr_result_t exr_set_dwa_compression_level(exr_context_t ctxt, int part_index, float level)
{
  ...
  if (level > 0.f && level <= 100.f) {
    ...
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid dwa quality level specified"));
  }
