...
EXR_INTERNAL internal_exr_unpack_fn internal_exr_match_decode(exr_decode_pipeline_t *decode,
                                                              int isdeep,
                                                              int chanstofill,
                                                              int chanstounpack,
                                                              int sametype,
...
EXR_INTERNAL internal_exr_pack_fn internal_exr_match_encode(exr_encode_pipeline_t *encode,
                                                            int isdeep);

EXR_INTERNAL exr_result_t
internal_coding_fill_channel_info(exr_coding_channel_info_t **channels,
                                  int16_t *num_chans,
...
EXR_INTERNAL exr_result_t
internal_coding_update_channel_info(exr_coding_channel_info_t *channels,
                                    int16_t num_chans,
                                    const exr_chunk_info_t *cinfo,
                                    const struct _internal_exr_context *pctxt,
                                    const struct _internal_exr_part *part);

EXR_INTERNAL exr_result_t internal_validate_next_chunk(exr_encode_pipeline_t *encode,
                                                       const struct _internal_exr_context *pctxt,
                                                       const struct _internal_exr_part *part);

/**************************************/

EXR_INTERNAL exr_result_t internal_encode_free_buffer(exr_encode_pipeline_t *encode,
                                                      exr_transcoding_pipeline_buffer_id_t bufid,
                                                      void **buf,
                                                      size_t *sz);

EXR_INTERNAL exr_result_t internal_encode_alloc_buffer(exr_encode_pipeline_t *encode,
                                                       exr_transcoding_pipeline_buffer_id_t bufid,
                                                       void **buf,
                                                       size_t *cursz,
                                                       size_t newsz);

EXR_INTERNAL exr_result_t internal_decode_free_buffer(exr_decode_pipeline_t *decode,
                                                      exr_transcoding_pipeline_buffer_id_t bufid,
                                                      void **buf,
                                                      size_t *sz);

EXR_INTERNAL exr_result_t internal_decode_alloc_buffer(exr_decode_pipeline_t *decode,
                                                       exr_transcoding_pipeline_buffer_id_t bufid,
                                                       void **buf,
                                                       size_t *cursz,
                                                       size_t newsz);
...
static inline uint32_t half_to_float_int(uint16_t hv)
{
  union {
    uint32_t i;
    float f;
...
static inline uint16_t float_to_half(float fv)
{
#ifdef IMATH_HALF_SAFE_FOR_C
...
static inline uint16_t float_to_half_int(uint32_t fiv)
{
  union {
    uint32_t i;
    float f;
...
static inline uint32_t half_to_uint(uint16_t hv)
{
...
static inline uint32_t float_to_uint(float fv)
{
  if (fv < 0.f || isnan(fv))
    return 0;
  if (isinf(fv) || fv > (float)(UINT32_MAX))
    return UINT32_MAX;
  return (uint32_t)(fv);
}

static inline uint32_t float_to_uint_int(uint32_t fint)
{
  union {
    uint32_t i;
    float f;
...
static inline uint16_t uint_to_half(uint32_t ui)
{
  if (ui > 65504)
    return 0x7c00;
...
static inline float uint_to_float(uint32_t ui)
{
  return (float)ui;
}

static inline uint32_t uint_to_float_int(uint32_t ui)
{
  union {
    uint32_t i;
    float f;
