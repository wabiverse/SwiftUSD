...
static inline void Classifier_destroy(void (*free_fn)(void *), Classifier *p)
{
  if (p->_suffix && !p->_stringStatic)
    free_fn(EXR_CONST_CAST(void *, p->_suffix));
}

static exr_result_t Classifier_read(void *(*alloc_fn)(size_t),
                                    Classifier *out,
                                    const uint8_t **ptr,
                                    size_t *size)
{
...
static inline int Classifier_match(const Classifier *me,
                                   const char *suffix,
                                   const exr_pixel_type_t type)
{
  if (me->_type != type)
    return DWA_CLASSIFIER_FALSE;
#ifdef _MSC_VER
  if (me->_caseInsensitive)
    return _stricmp(suffix, me->_suffix) == 0;
#else
  if (me->_caseInsensitive)
    return strcasecmp(suffix, me->_suffix) == 0;
#endif

  return strcmp(suffix, me->_suffix) == 0;
}

static inline uint64_t Classifier_size(const Classifier *me)
{
  return strlen(me->_suffix) + 1 + 2 * sizeof(uint8_t);
}

static inline uint64_t Classifier_write(const Classifier *me, uint8_t **ptr)
{
  uint8_t *outptr = *ptr;
  uint8_t value = 0;
...
static inline const char *Classifier_find_suffix(const char *channel_name)
{
