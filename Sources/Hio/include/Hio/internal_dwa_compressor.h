...
static exr_result_t DwaCompressor_construct(DwaCompressor *me,
...
static exr_result_t DwaCompressor_uncompress(DwaCompressor *me,
                                             const uint8_t *inPtr,
                                             uint64_t iSize,
                                             void *uncompressed_data,
                                             uint64_t uncompressed_size);

static exr_result_t DwaCompressor_initializeBuffers(DwaCompressor *me, size_t *);

static exr_result_t DwaCompressor_writeRelevantChannelRules(DwaCompressor *me,
                                                            uint8_t **outPtr,
                                                            uint64_t nAvail,
                                                            uint64_t *nWritten);
static exr_result_t DwaCompressor_readChannelRules(DwaCompressor *me,
                                                   const uint8_t **inPtr,
                                                   uint64_t *nAvail,
...
exr_result_t DwaCompressor_construct(DwaCompressor *me,
                                     AcCompression acCompression,
                                     exr_encode_pipeline_t *encode,
                                     exr_decode_pipeline_t *decode)
{
  ...
  if (encode) {
    ...
    me->_channelData = internal_exr_alloc_aligned(me->alloc_fn,
                                                  &(me->_channel_mem),
                                                  sizeof(ChannelData) *
                                                      (size_t)encode->channel_count,
                                                  _SSE_ALIGNMENT);
    if (!me->_channelData)
...
static void DwaCompressor_destroy(DwaCompressor *me)
{
  if (me->_packedAcBuffer)
    me->free_fn(me->_packedAcBuffer);
  if (me->_packedDcBuffer)
    me->free_fn(me->_packedDcBuffer);
  if (me->_rleBuffer)
    me->free_fn(me->_rleBuffer);
  ...
  if (me->_cscChannelSets)
    me->free_fn(me->_cscChannelSets);
...
exr_result_t DwaCompressor_compress(DwaCompressor *me)
{
  ...
  rv = internal_encode_alloc_buffer(me->_encode,
...
#define OBIDX(x) (uint64_t *)(sizes + x)
  ...
  if (rv == EXR_ERR_SUCCESS && fileVersion >= 2) {
    rv = DwaCompressor_writeRelevantChannelRules(me, &outPtr, nAvail, &nWritten);
  }
  ...
  *acCompression = me->_acCompression;
  ...
  for (int y = me->_min[1]; y <= me->_max[1]; ++y) {
    for (int c = 0; c < me->_numChannels; ++c) {
      ...
      rv = DctCoderChannelData_push_row(me->alloc_fn, me->free_fn, &(cd->_dctData), inDataPtr);
      if (rv != EXR_ERR_SUCCESS)
        return rv;
  ...
  for (int csc = 0; csc < me->_numCscChannelSets; ++csc) {
    ...
    rv = LossyDctEncoderCsc_construct(&enc,
    ...
    switch (cd->compression) {
      case LOSSY_DCT:
        ...
        //
        {
          ...
          rv = LossyDctEncoder_construct(&enc,
                                         me->_dwaCompressionLevel / 100000.f,
                                         &(cd->_dctData),
                                         packedAcEnd,
                                         packedDcEnd,
      ...
      case RLE: {
        ...
        for (size_t y = 0; y < dcd->_size; ++y) {
          ...
          for (int x = 0; x < pchan->width; ++x) {
            for (int byte = 0; byte < pchan->bytes_per_element; ++byte) {
      ...
      case UNKNOWN:
        ...
        {
          size_t scanlineSize = (size_t)pchan->width * (size_t)pchan->bytes_per_element;
          DctCoderChannelData *dcd = &(cd->_dctData);
          for (size_t y = 0; y < dcd->_size; ++y) {
            memcpy(cd->planarUncBufferEnd, dcd->_rows[y], scanlineSize);
  ...
  if (*unknownUncompressedSize > 0) {
    ...
    rv = exr_compress_buffer(me->_encode->context,
  ...
  if (*totalAcUncompressedCount > 0) {
    switch (me->_acCompression) {
      case STATIC_HUFFMAN: {
        size_t outDataSize = outBufferSize - (size_t)((uintptr_t)outDataPtr - (uintptr_t)sizes);

        rv = internal_huf_compress(acCompressedSize,
                                   outDataPtr,
                                   outDataSize,
        ...
        if (rv != EXR_ERR_SUCCESS) {
          if (rv == EXR_ERR_ARGUMENT_OUT_OF_RANGE) {
            memcpy(me->_encode->compressed_buffer,
      ...
      case DEFLATE: {
        size_t sourceLen = *totalAcUncompressedCount * sizeof(uint16_t);
        size_t destLen;
  ...
  if (*rleRawSize > 0) {
    ...
    *rleUncompressedSize = internal_rle_compress(
        me->_rleBuffer, me->_rleBufferSize, me->_planarUncBuffer[RLE], *rleRawSize);
  ...
  dataBytes = (uintptr_t)outDataPtr - (uintptr_t)me->_encode->compressed_buffer;
  if (nWritten != dataBytes) {
    return EXR_ERR_CORRUPT_CHUNK;
  }

  if (nWritten >= me->_encode->packed_bytes) {
    memcpy(me->_encode->compressed_buffer, me->_encode->packed_buffer, me->_encode->packed_bytes);
...
exr_result_t DwaCompressor_uncompress(DwaCompressor *me,
                                      const uint8_t *inPtr,
                                      uint64_t iSize,
                                      void *uncompressed_data,
                                      uint64_t uncompressed_size)
{
  ...
  compressedSize = unknownCompressedSize + acCompressedSize + dcCompressedSize + rleCompressedSize;
  ...
  if (iSize < (headerSize + compressedSize) || iSize < unknownCompressedSize ||
      iSize < acCompressedSize || iSize < dcCompressedSize || iSize < rleCompressedSize)
  {
    return EXR_ERR_CORRUPT_CHUNK;
  }

  if ((int64_t)unknownUncompressedSize < 0 || (int64_t)unknownCompressedSize < 0 ||
      (int64_t)acCompressedSize < 0 || (int64_t)dcCompressedSize < 0 ||
      (int64_t)rleCompressedSize < 0 || (int64_t)rleUncompressedSize < 0 ||
      (int64_t)rleRawSize < 0 || (int64_t)totalAcUncompressedCount < 0 ||
  ...
  if (version < 2) {
    ...
    uint64_t ruleSize;
    rv = DwaCompressor_readChannelRules(me, &dataPtr, &dataLeft, &ruleSize);
  ...
  compressedAcBuf = compressedUnknownBuf + (ptrdiff_t)(unknownCompressedSize);
  ...
  if (compressedUnknownBuf > dataPtrEnd || dataPtr > compressedAcBuf ||
      compressedAcBuf > dataPtrEnd || dataPtr > compressedDcBuf || compressedDcBuf > dataPtrEnd ||
      dataPtr > compressedRleBuf || compressedRleBuf > dataPtrEnd ||
  ...
  if (unknownCompressedSize > 0) {
    ...
    if (EXR_ERR_SUCCESS != exr_uncompress_buffer(me->_decode->context,
  ...
  if (acCompressedSize > 0) {
    if (!me->_packedAcBuffer ||
        totalAcUncompressedCount * sizeof(uint16_t) > me->_packedAcBufferSize)
    {
      return EXR_ERR_CORRUPT_CHUNK;
    }
    ...
    switch (acCompression) {
      case STATIC_HUFFMAN:
        rv = internal_huf_decompress(me->_decode,
                                     compressedAcBuf,
                                     acCompressedSize,
      ...
      case DEFLATE: {
        ...
        rv = exr_uncompress_buffer(me->_decode->context,
                                   compressedAcBuf,
                                   acCompressedSize,
                                   me->_packedAcBuffer,
  ...
  if (dcCompressedSize > 0) {
    ...
    rv = internal_decode_alloc_buffer(me->_decode,
                                      EXR_TRANSCODE_BUFFER_SCRATCH1,
                                      &(me->_decode->scratch_buffer_1),
                                      &(me->_decode->scratch_alloc_size_1),
                                      uncompBytes);
  ...
  if (rleRawSize > 0) {
    ...
    if (rleUncompressedSize > me->_rleBufferSize || rleRawSize > me->_planarUncBufferSize[RLE]) {
      return EXR_ERR_CORRUPT_CHUNK;
    }
    ...
    if (internal_rle_decompress(me->_planarUncBuffer[RLE],
                                rleRawSize,
  ...
  for (int y = me->_min[1]; y <= me->_max[1]; ++y) {
    for (int c = 0; c < me->_numChannels; ++c) {
      ...
      rv = DctCoderChannelData_push_row(me->alloc_fn, me->free_fn, &(cd->_dctData), outBufferEnd);
      if (rv != EXR_ERR_SUCCESS)
        return rv;
  ...
  for (int csc = 0; csc < me->_numCscChannelSets; ++csc) {
    ...
    rv = LossyDctDecoderCsc_construct(&decoder,
    ...
    switch (cd->compression) {
      case LOSSY_DCT:
...

        {
          ...
          rv = LossyDctDecoder_construct(&decoder,
                                         dcddata,
                                         packedAcBufferEnd,
                                         packedAcBufferEnd +
                                             totalAcUncompressedCount * sizeof(uint16_t),
          ...
          packedAcBufferEnd += (size_t)decoder._packedAcCount * sizeof(uint16_t);

          packedDcBufferEnd += (size_t)decoder._packedDcCount * sizeof(uint16_t);
      ...
      case UNKNOWN:
...

        {
          ...
          for (int y = me->_min[1]; y <= me->_max[1]; ++y) {
            ...
            //
            if ((cd->planarUncBufferEnd + (size_t)(dstScanlineSize)) >
                (me->_planarUncBuffer[UNKNOWN] + me->_planarUncBufferSize[UNKNOWN]))
            {
              return EXR_ERR_CORRUPT_CHUNK;
            }
...
exr_result_t DwaCompressor_initializeBuffers(DwaCompressor *me, size_t *bufferSize)
{
  ...
  uint64_t maxLossyDctAcSize = (uint64_t)(ceilf((float)me->_numScanLines / 8.0f)) *
                               (uint64_t)(ceilf((float)(me->_max[0] - me->_min[0] + 1) / 8.0f)) *
                               63 * sizeof(uint16_t);

  uint64_t maxLossyDctDcSize = (uint64_t)(ceilf((float)me->_numScanLines / 8.0f)) *
                               (uint64_t)(ceilf((float)(me->_max[0] - me->_min[0] + 1) / 8.0f)) *
                               sizeof(uint16_t);

  uint64_t pixelCount = (uint64_t)(me->_numScanLines) * (uint64_t)(me->_max[0] - me->_min[0] + 1);
  ...
  for (int c = 0; c < me->_numChannels; ++c) {
    ...
    switch (me->_channelData[c].compression) {
      case LOSSY_DCT:
        ...
        maxOutBufferSize += std_max(2lu * maxLossyDctAcSize + 65536lu,
  ...
  maxOutBufferSize += exr_compress_max_buffer_size(maxLossyDctDcSize * numLossyDctChans);
  ...
  *bufferSize = maxOutBufferSize;
  ...
  if (planarUncBufferSize[UNKNOWN] > 0) {
    planarUncBufferSize[UNKNOWN] = exr_compress_max_buffer_size(planarUncBufferSize[UNKNOWN]);
  }
...
exr_result_t DwaCompressor_writeRelevantChannelRules(DwaCompressor *me,
                                                     uint8_t **outPtr,
                                                     uint64_t nAvail,
                                                     uint64_t *nWritten)
{
  ...
  for (size_t i = 0; i < me->_channelRuleCount; ++i) {
    for (int c = 0; c < me->_numChannels; ++c) {
      ...
      if (Classifier_match(&(me->_channelRules[i]), suffix, (exr_pixel_type_t)curc->data_type)) {
        if (nAvail < (*nWritten + nOut + Classifier_size(&(me->_channelRules[i]))))
          return EXR_ERR_OUT_OF_MEMORY;
...
exr_result_t DwaCompressor_readChannelRules(DwaCompressor *me,
                                            const uint8_t **inPtr,
                                            uint64_t *nAvail,
                                            uint64_t *outRuleSize)
{
    ...
    if (rv == EXR_ERR_SUCCESS) {
      ...
      if (me->_channelRules) {
        ...
        for (size_t i = 0; i < nRules; ++i) {
          Classifier_read(me->alloc_fn, &(me->_channelRules[i]), &readPtr, &dataSize);
        }
      }
      else
...
exr_result_t DwaCompressor_classifyChannels(DwaCompressor *me)
{
  ...
  me->_cscChannelSets = me->alloc_fn(sizeof(CscChannelSet) * (size_t)me->_numChannels);
  if (!me->_cscChannelSets)
    return EXR_ERR_OUT_OF_MEMORY;
  ...
  //
  prefixMap = me->alloc_fn(sizeof(CscPrefixMapItem) * (size_t)me->_numChannels);
  if (!prefixMap)
    return EXR_ERR_OUT_OF_MEMORY;
  ...
  for (int c = 0; c < me->_numChannels; ++c) {
    ...
    CscPrefixMapItem *mapi = CscPrefixMap_find(
        prefixMap, me->_numChannels, curc->channel_name, (size_t)(curc->channel_name - suffix));
  ...
  for (int c = 0; c < me->_numChannels; ++c) {
    ...
    if ((redc->x_samples != grnc->x_samples) || (redc->x_samples != bluc->x_samples) ||
        (grnc->x_samples != bluc->x_samples) || (redc->y_samples != grnc->y_samples) ||
        (redc->y_samples != bluc->y_samples) || (grnc->y_samples != bluc->y_samples))
    {
      continue;
    }
...
exr_result_t DwaCompressor_setupChannelData(DwaCompressor *me)
{
  ...
  for (int c = 0; c < me->_numChannels; ++c) {
    ...
    uncSize = (size_t)curc->width * (size_t)curc->height * (size_t)curc->bytes_per_element;
    ...
    for (int byte = 1; byte < curc->bytes_per_element; ++byte) {
      cd->planarUncRle[byte] = cd->planarUncRle[byte - 1] + curc->width * curc->height;
    ...
    if (cd->compression == LOSSY_DCT) {
      ...
      planarUncBuffer[cd->compression] += uncSize;
    }
  }
