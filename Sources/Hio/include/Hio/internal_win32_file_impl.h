...
static exr_result_t print_error_helper(struct _internal_exr_context *pf,
                                       ...
                                       const char *msg)
{
  ...
  bufsz = (lstrlen((LPCTSTR)lpMsgBuf) + strlen(msg) + 20); /* extra for format string */

  lpDisplayBuf = (LPVOID)LocalAlloc(LMEM_ZEROINIT, bufsz * sizeof(TCHAR));

  if (FAILED(StringCchPrintf(
          (LPTSTR)lpDisplayBuf, bufsz, TEXT("%s: (%" PRId64 ") %s"), msg, (int64_t)dw, lpMsgBuf)))
  {
    return pf->print_error(pf, EXR_ERR_OUT_OF_MEMORY, "Unable to format message print");
  }

  if (error_cb)
    error_cb((exr_const_context_t)pf, errcode, "%s", (const char *)lpDisplayBuf);
  else
...
static exr_result_t print_error(struct _internal_exr_context *pf,
                                exr_result_t errcode,
                                const char *msg)
{
...
static exr_result_t send_error(struct _internal_exr_context *pf,
                               exr_result_t errcode,
                               exr_stream_error_func_ptr_t error_cb,
                               const char *msg)
{
...
static wchar_t *widen_filename(struct _internal_exr_context *file, const char *fn)
{
...
static void default_shutdown(exr_const_context_t c, void *userdata, int failed)
{
...
static exr_result_t finalize_write(struct _internal_exr_context *pf, int failed)
{
  ...
  if (!failed && pf->tmp_filename.str) {
    ...
    if (!res)
      return print_error(
          pf, EXR_ERR_FILE_ACCESS, "Unable to rename temporary file to final destination");
  }
...
static int64_t default_read_func(exr_const_context_t ctxt,
                                 void *userdata,
                                 void *buffer,
                                 uint64_t sz,
                                 uint64_t offset,
                                 exr_stream_error_func_ptr_t error_cb)
{
  ...
  if (!fh) {
    if (error_cb)
      error_cb(ctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid file handle pointer");
    return retsz;
  }
  ...
  if (fd == INVALID_HANDLE_VALUE) {
    if (error_cb)
      error_cb(ctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid file descriptor");
    return retsz;
  }

  if (sz > (uint64_t)(INT32_MAX)) {
    if (error_cb)
      error_cb(ctxt, EXR_ERR_INVALID_ARGUMENT, "Read request too large for win32 api");
    return retsz;
  }
  ...
  if (ReadFile(fd, buffer, (DWORD)sz, &nread, &overlap)) {
    ...
    if (dw != ERROR_HANDLE_EOF) {
      print_error_helper(
          EXR_CTXT(ctxt), EXR_ERR_READ_IO, dw, error_cb, "Unable to read requested data");
    }
    else {
      retsz = nread;
    }
...
static int64_t default_write_func(exr_const_context_t ctxt,
                                  void *userdata,
                                  const void *buffer,
                                  uint64_t sz,
                                  uint64_t offset,
                                  exr_stream_error_func_ptr_t error_cb)
{
  ...
  if (WriteFile(fd, buffer, (DWORD)sz, &nwrote, &overlap)) {
    ...
    DWORD dw = GetLastError();
    print_error_helper(
        EXR_CTXT(ctxt), EXR_ERR_READ_IO, dw, error_cb, "Unable to write requested data");
  }
...
static exr_result_t default_init_read_file(struct _internal_exr_context *file)
{
  wchar_t *wcFn = NULL;
  HANDLE fd;
  ...
  if (wcFn) {
    ...
    fd = CreateFile2(wcFn, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, NULL);
#else
    fd = CreateFileW(wcFn,
                     GENERIC_READ,
                     FILE_SHARE_READ,
...
static exr_result_t default_init_write_file(struct _internal_exr_context *file)
{
  wchar_t *wcFn = NULL;
  struct _internal_exr_filehandle *fh = file->user_data;
  HANDLE fd;
...
static int64_t default_query_size_func(exr_const_context_t ctxt, void *userdata)
{
  struct _internal_exr_filehandle *fh = userdata;
  int64_t sz = -1;
...
static exr_result_t make_temp_filename(struct _internal_exr_context *ret)
{
  ...
  if (nwr >= 32)
    return ret->report_error(
        ret, EXR_ERR_INVALID_ARGUMENT, "Invalid assumption in temporary filename");
