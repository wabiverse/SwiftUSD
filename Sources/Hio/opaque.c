...
int exr_attr_opaquedata_init(exr_context_t ctxt, exr_attr_opaquedata_t *u, size_t b)
{
  ...
  if (!u)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid reference to opaque data object to initialize");
...
exr_result_t exr_attr_opaquedata_create(exr_context_t ctxt,
...
exr_result_t exr_attr_opaquedata_copy(exr_context_t ctxt,
                                      ...
                                      const exr_attr_opaquedata_t *srcud)
{
  ...
  if (srcud->packed_data)
    return exr_attr_opaquedata_create(ctxt, ud, (size_t)srcud->size, srcud->packed_data);
  rv = exr_attr_opaquedata_init(ctxt, ud, 0);
  if (rv == EXR_ERR_SUCCESS)
    rv = exr_attr_opaquedata_set_unpacked(ctxt, ud, srcud->unpacked_data, srcud->unpacked_size);
  return rv;
}

/**************************************/

exr_result_t exr_attr_opaquedata_unpack(exr_context_t ctxt,
                                        exr_attr_opaquedata_t *u,
                                        int32_t *sz,
                                        void **unpacked)
{
  ...
  if (u->unpacked_data) {
    if (sz)
      *sz = u->unpacked_size;
    if (unpacked)
      *unpacked = u->unpacked_data;
    return EXR_ERR_SUCCESS;
  }

  if (!u->unpack_func_ptr)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "No unpack provider specified for opaque data");
  rv = u->unpack_func_ptr(ctxt, u->packed_data, u->size, &(tmpusz), &(tmpuptr));
  if (rv == EXR_ERR_SUCCESS) {
...
exr_result_t exr_attr_opaquedata_pack(exr_context_t ctxt,
                                      exr_attr_opaquedata_t *u,
                                      int32_t *sz,
                                      void **packed)
{
  ...
  if (u->packed_data) {
    if (sz)
      *sz = u->size;
    if (packed)
      *packed = u->packed_data;
    return EXR_ERR_SUCCESS;
  }

  if (!u->pack_func_ptr)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "No pack provider specified for opaque data");

  rv = u->pack_func_ptr(ctxt, u->unpacked_data, u->unpacked_size, &nsize, NULL);
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->print_error(pctxt,
                              rv,
  ...
  if (nsize > 0) {
    ...
    rv = u->pack_func_ptr(ctxt, u->unpacked_data, u->unpacked_size, &nsize, tmpptr);
    if (rv != EXR_ERR_SUCCESS) {
...
exr_result_t exr_attr_opaquedata_set_unpacked(exr_context_t ctxt,
                                              exr_attr_opaquedata_t *u,
                                              void *unpacked,
                                              int32_t sz)
{
  ...
  if (sz < 0)
    return pctxt->print_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Opaque data given invalid negative size (%d)", sz);
  ...
  if (u->packed_data) {
    if (u->packed_alloc_size > 0)
      pctxt->free_fn(u->packed_data);
    u->packed_data = NULL;
    u->size = 0;
    u->packed_alloc_size = 0;
  }
...
exr_result_t exr_attr_opaquedata_set_packed(exr_context_t ctxt,
                                            exr_attr_opaquedata_t *u,
                                            const void *packed,
                                            int32_t sz)
{
  ...
  if (packed)
    memcpy((void *)u->packed_data, packed, (size_t)sz);
