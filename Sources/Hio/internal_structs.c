...
static void default_error_handler(exr_const_context_t ctxt, exr_result_t code, const char *msg)
{
...
#  endif
  ...
  if (pctxt) {
    if (pctxt->filename.str)
      fprintf(
          stderr, "%s: (%s) %s\n", pctxt->filename.str, exr_get_error_code_as_string(code), msg);
    else
      fprintf(stderr,
              "Context 0x%p: (%s) %s\n",
              (const void *)ctxt,
...
static exr_result_t dispatch_error(const struct _internal_exr_context *pctxt,
                                   exr_result_t code,
                                   const char *msg)
{
...
static exr_result_t dispatch_standard_error(const struct _internal_exr_context *pctxt,
                                            exr_result_t code)
{
...
static exr_result_t dispatch_print_error(const struct _internal_exr_context *pctxt,
                                         exr_result_t code,
                                         const char *msg,
  ...
  va_start(fmtargs, msg);
  {
    ...
    if (nwrit >= 256) {
      ...
      if (heapbuf) {
        ...
        dispatch_error(pctxt, code, "Unable to allocate temporary memory");
    }
    else
...
static void internal_exr_destroy_part(struct _internal_exr_context *ctxt,
                                      struct _internal_exr_part *cur)
{
  ...
  if (cur->tile_level_tile_count_x)
    dofree(cur->tile_level_tile_count_x);

#if defined(_MSC_VER)
  ctable = (uint64_t *)InterlockedOr64((int64_t volatile *)&(cur->chunk_table), 0);
  cur->chunk_table = 0;
#else
...
static void internal_exr_destroy_parts(struct _internal_exr_context *ctxt)
{
  exr_memory_free_func_t dofree = ctxt->free_fn;
...
exr_result_t internal_exr_add_part(struct _internal_exr_context *f,
                                   ...
                                   int *new_index)
{
  ...
  if (ncount == 1) {
    ...
    nptrs = f->alloc_fn(sizeof(struct _internal_exr_part *) * (size_t)ncount);
    if (!nptrs) {
      f->free_fn(part);
      return f->standard_error(f, EXR_ERR_OUT_OF_MEMORY);
    }
...
void internal_exr_revert_add_part(struct _internal_exr_context *ctxt,
                                  struct _internal_exr_part **outpart,
                                  int *new_index)
{
...
exr_result_t internal_exr_context_restore_handlers(struct _internal_exr_context *ctxt,
                                                   exr_result_t rv)
{
...
exr_result_t internal_exr_alloc_context(struct _internal_exr_context **out,
                                        ...
                                        size_t default_size)
{
  ...
  *out = NULL;
  ...
  memptr = (initializers->alloc_fn)(sizeof(struct _internal_exr_context) + extra_data);
  if (memptr) {
    ...
    if (initializers->read_fn || initializers->write_fn)
      ...
      ret->user_data = (((uint8_t *)memptr) + sizeof(struct _internal_exr_context));
    ...
    if (ret->max_image_w > 0 && gmaxw > 0 && ret->max_image_w && ret->max_image_w > gmaxw)
      ret->max_image_w = gmaxw;
    ...
    if (ret->max_image_h > 0 && gmaxh > 0 && ret->max_image_h && ret->max_image_h > gmaxh)
      ret->max_image_h = gmaxh;
    ...
    if (ret->max_tile_w > 0 && gmaxw > 0 && ret->max_tile_w && ret->max_tile_w > gmaxw)
      ret->max_tile_w = gmaxw;
    ...
    if (ret->max_tile_h > 0 && gmaxh > 0 && ret->max_tile_h && ret->max_tile_h > gmaxh)
      ret->max_tile_h = gmaxh;
    ...
    if (initializers->flags & EXR_CONTEXT_FLAG_SILENT_HEADER_PARSE)
      ret->silent_header = 1;
    ret->disable_chunk_reconstruct = (initializers->flags &
...
#  endif
    ...
    *out = ret;
    ...
    (initializers->error_handler_fn)(
        NULL, EXR_ERR_OUT_OF_MEMORY, exr_get_default_error_message(EXR_ERR_OUT_OF_MEMORY));
...
void internal_exr_update_default_handlers(exr_context_initializer_t *inits)
{
  ...
  if (!inits->alloc_fn)
    inits->alloc_fn = &internal_exr_alloc;
  if (!inits->free_fn)
    inits->free_fn = &internal_exr_free;
}
