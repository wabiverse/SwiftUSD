...
exr_result_t exr_attr_string_init(exr_context_t ctxt, exr_attr_string_t *s, int32_t len)
{
  ...
  if (!s)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid reference to string object to initialize");
...
exr_result_t exr_attr_string_init_static_with_length(exr_context_t ctxt,
                                                     exr_attr_string_t *s,
                                                     const char *v,
                                                     int32_t len)
{
  ...
  if (!v)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid static string argument to initialize");
...
exr_result_t exr_attr_string_init_static(exr_context_t ctxt, exr_attr_string_t *s, const char *v)
{
  size_t fulllen = 0;
  int32_t length = 0;
  ...
  if (v) {
    ...
    if (fulllen >= (size_t)INT32_MAX) {
      INTERN_EXR_PROMOTE_CONTEXT_OR_ERROR(ctxt);
      return pctxt->report_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid string too long for attribute");
    }
...
exr_result_t exr_attr_string_create_with_length(exr_context_t ctxt,
                                                exr_attr_string_t *s,
                                                const char *d,
                                                int32_t len)
{
  ...
  if (!s)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid (NULL) arguments to string create with length");
...
exr_result_t exr_attr_string_create(exr_context_t ctxt, exr_attr_string_t *s, const char *d)
{
  size_t fulllen = 0;
  int32_t len = 0;
  if (d) {
...
exr_result_t exr_attr_string_set_with_length(exr_context_t ctxt,
                                             exr_attr_string_t *s,
                                             const char *d,
                                             int32_t len)
{
  ...
  if (!s)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid string argument to string set");
...
exr_result_t exr_attr_string_set(exr_context_t ctxt, exr_attr_string_t *s, const char *d)
{
  size_t fulllen = 0;
  int32_t len = 0;
  if (d) {
