...
static exr_result_t silent_error(const struct _internal_exr_context *pctxt,
                                 exr_result_t code,
                                 const char *msg)
{
  (void)pctxt;
...
static exr_result_t silent_standard_error(const struct _internal_exr_context *pctxt,
                                          exr_result_t code)
{
  (void)pctxt;
  return code;
...
static exr_result_t silent_print_error(const struct _internal_exr_context *pctxt,
                                       exr_result_t code,
                                       const char *msg,
                                       ...)
{
...
struct _internal_exr_seq_scratch {
  ...
  exr_result_t (*sequential_read)(struct _internal_exr_seq_scratch *, void *, uint64_t);
  exr_result_t (*sequential_skip)(struct _internal_exr_seq_scratch *, int32_t);

  struct _internal_exr_context *ctxt;
};

static inline int scratch_attr_too_big(struct _internal_exr_seq_scratch *scr,
                                       int32_t attrsz,
                                       int64_t fsize)
{
...
static exr_result_t scratch_seq_read(struct _internal_exr_seq_scratch *scr, void *buf, uint64_t sz)
{
  uint8_t *outbuf = buf;
  uint64_t nCopied = 0;
  uint64_t notdone = sz;
  ...
  while (notdone > 0) {
    if (scr->navail > 0) {
      ...
      rv = scr->ctxt->do_read(
          scr->ctxt, outbuf, nToRead, &(scr->fileoff), &nread, EXR_MUST_READ_ALL);
      if (nread > 0) {
      ...
      if (nread > 0) {
        ...
        if (nread == 0)
          rv = scr->ctxt->report_error(
              scr->ctxt, EXR_ERR_READ_IO, "End of file attempting to read header");
        break;
      }
    }
  }
...
static exr_result_t scratch_seq_skip(struct _internal_exr_seq_scratch *scr, int32_t sz)
{
  uint64_t nCopied = 0;
...
static exr_result_t priv_init_scratch(struct _internal_exr_context *ctxt,
...
static void priv_destroy_scratch(struct _internal_exr_seq_scratch *scr)
{
...
static exr_result_t check_bad_attrsz(struct _internal_exr_context *ctxt,
...
static exr_result_t read_text(struct _internal_exr_context *ctxt,
                              char text[256],
                              int32_t *outlen,
                              int32_t maxlen,
                              struct _internal_exr_seq_scratch *scratch,
...
static exr_result_t extract_attr_chlist(struct _internal_exr_context *ctxt,
                                        struct _internal_exr_seq_scratch *scratch,
                                        ...
                                        int32_t attrsz)
{
  ...
  while (rv == EXR_ERR_SUCCESS && attrsz > 0) {
    ...
    rv = exr_attr_chlist_add_with_length((exr_context_t)ctxt,
                                         attrdata,
                                         chname,
                                         chlen,
...
static exr_result_t extract_attr_uint8(struct _internal_exr_context *ctxt,
                                       struct _internal_exr_seq_scratch *scratch,
                                       uint8_t *attrdata,
                                       const char *aname,
                                       const char *tname,
                                       ...
                                       uint8_t maxval)
{
  ...
  if (scratch->sequential_read(scratch, attrdata, sizeof(uint8_t)))
    return ctxt->print_error(ctxt, EXR_ERR_READ_IO, "Unable to read '%s' %s data", aname, tname);
...
static exr_result_t extract_attr_64bit(struct _internal_exr_context *ctxt,
                                       struct _internal_exr_seq_scratch *scratch,
                                       void *attrdata,
                                       const char *aname,
                                       const char *tname,
                                       ...
                                       int32_t num)
{
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(ctxt, rv, "Unable to read '%s' %s data", aname, tname);
...
static exr_result_t extract_attr_32bit(struct _internal_exr_context *ctxt,
                                       struct _internal_exr_seq_scratch *scratch,
                                       void *attrdata,
                                       const char *aname,
                                       const char *tname,
...
static exr_result_t extract_attr_float_vector(struct _internal_exr_context *ctxt,
                                              struct _internal_exr_seq_scratch *scratch,
...
{
  int32_t n = 0;
  exr_result_t rv = check_bad_attrsz(ctxt, scratch, attrsz, (int)sizeof(float), aname, tname, &n);
...
static exr_result_t extract_attr_string(struct _internal_exr_context *ctxt,
                                        struct _internal_exr_seq_scratch *scratch,
                                        ...
                                        char *strptr)
{
  exr_result_t rv = scratch->sequential_read(scratch, (void *)strptr, (uint64_t)attrsz);
  ...
  return exr_attr_string_init_static_with_length((exr_context_t)ctxt, attrdata, strptr, attrsz);
}

/**************************************/

static exr_result_t extract_attr_string_vector(struct _internal_exr_context *ctxt,
                                               struct _internal_exr_seq_scratch *scratch,
                                               ...
                                               int32_t attrsz)
{
  ...
  while (pulled < attrsz) {
    ...
    if (rv != EXR_ERR_SUCCESS) {
      rv = ctxt->print_error(ctxt, rv, "Attribute '%s': Unable to read string length", aname);
    ...
    if ((nstr + 1) >= nalloced) {
      nalloced *= 2;
      nlist = ctxt->alloc_fn((size_t)(nalloced) * sizeof(exr_attr_string_t));
      if (nlist == NULL) {
        rv = ctxt->standard_error(ctxt, EXR_ERR_OUT_OF_MEMORY);
        goto extract_string_vector_fail;
      }
    ...
    *nlist = nil;
    ...
    rv = scratch->sequential_read(scratch, EXR_CONST_CAST(void *, nlist->str), (uint64_t)nlen);
    if (rv != EXR_ERR_SUCCESS) {
      rv = ctxt->print_error(
          ctxt, rv, "Attribute '%s': Unable to read string of length (%d)", aname, nlen);
      goto extract_string_vector_fail;
    }
...
static exr_result_t extract_attr_tiledesc(struct _internal_exr_context *ctxt,
                                          struct _internal_exr_seq_scratch *scratch,
...
static exr_result_t extract_attr_opaque(struct _internal_exr_context *ctxt,
                                        struct _internal_exr_seq_scratch *scratch,
                                        ...
                                        int32_t attrsz)
{
  ...
  rv = exr_attr_opaquedata_init((exr_context_t)ctxt, attrdata, (uint64_t)attrsz);
  if (rv != EXR_ERR_SUCCESS)
    return rv;

  rv = scratch->sequential_read(scratch, (void *)attrdata->packed_data, (uint64_t)attrsz);
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_opaquedata_destroy((exr_context_t)ctxt, attrdata);
    return ctxt->print_error(ctxt,
                             EXR_ERR_READ_IO,
...
static exr_result_t extract_attr_preview(struct _internal_exr_context *ctxt,
                                         struct _internal_exr_seq_scratch *scratch,
                                         ...
                                         int32_t attrsz)
{
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(ctxt, rv, "Attribute '%s': Unable to read preview sizes", aname);
  ...
  if (bytes > 0) {
    ...
    if (rv != EXR_ERR_SUCCESS) {
      exr_attr_preview_destroy((exr_context_t)ctxt, attrdata);
      return ctxt->print_error(
          ctxt, rv, "Attribute '%s': Unable to read preview data (%d bytes)", aname, attrsz);
    }
  }
...
static exr_result_t check_populate_channels(struct _internal_exr_context *ctxt,
                                            ...
                                            int32_t attrsz)
{
  ...
  if (curpart->channels) {
    ...
    return ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "Duplicate copy of required attribute 'channels' encountered");
  }
  ...
  rv = extract_attr_chlist(ctxt, scratch, &(tmpchans), EXR_REQ_CHANNELS_STR, tname, attrsz);
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_chlist_destroy((exr_context_t)ctxt, &(tmpchans));
    return rv;
  }

  rv = exr_attr_list_add_static_name((exr_context_t)ctxt,
                                     &(curpart->attributes),
  ...
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_chlist_destroy((exr_context_t)ctxt, &tmpchans);
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'chlist'", EXR_REQ_CHANNELS_STR);
  }
...
static exr_result_t check_populate_compression(struct _internal_exr_context *ctxt,
                                               struct _internal_exr_part *curpart,
                                               struct _internal_exr_seq_scratch *scratch,
                                               const char *tname,
                                               int32_t attrsz)
{
  ...
  rv = extract_attr_uint8(
      ctxt, scratch, &data, EXR_REQ_COMP_STR, tname, attrsz, (uint8_t)EXR_COMPRESSION_LAST_TYPE);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'compression'", EXR_REQ_COMP_STR);
...
static exr_result_t check_populate_dataWindow(struct _internal_exr_context *ctxt,
                                              struct _internal_exr_part *curpart,
                                              struct _internal_exr_seq_scratch *scratch,
                                              const char *tname,
                                              int32_t attrsz)
{
  ...
  rv = extract_attr_32bit(ctxt, scratch, &(tmpdata), EXR_REQ_DATA_STR, tname, attrsz, 4);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'box2i'", EXR_REQ_DATA_STR);
...
static exr_result_t check_populate_displayWindow(struct _internal_exr_context *ctxt,
                                                 struct _internal_exr_part *curpart,
                                                 struct _internal_exr_seq_scratch *scratch,
                                                 const char *tname,
                                                 int32_t attrsz)
{
  ...
  rv = extract_attr_32bit(ctxt, scratch, &(tmpdata), EXR_REQ_DISP_STR, tname, attrsz, 4);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'box2i'", EXR_REQ_DISP_STR);
...
static exr_result_t check_populate_lineOrder(struct _internal_exr_context *ctxt,
                                             struct _internal_exr_part *curpart,
                                             struct _internal_exr_seq_scratch *scratch,
                                             const char *tname,
                                             int32_t attrsz)
{
  ...
  rv = extract_attr_uint8(
      ctxt, scratch, &data, EXR_REQ_LO_STR, tname, attrsz, (uint8_t)EXR_LINEORDER_LAST_TYPE);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'lineOrder'", EXR_REQ_LO_STR);
...
static exr_result_t check_populate_pixelAspectRatio(struct _internal_exr_context *ctxt,
                                                    struct _internal_exr_part *curpart,
                                                    struct _internal_exr_seq_scratch *scratch,
                                                    const char *tname,
                                                    int32_t attrsz)
{
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Attribute '%s': Unable to read data (%d bytes)", EXR_REQ_PAR_STR, attrsz);
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'float'", EXR_REQ_PAR_STR);
...
static exr_result_t check_populate_screenWindowCenter(struct _internal_exr_context *ctxt,
                                                      struct _internal_exr_part *curpart,
                                                      struct _internal_exr_seq_scratch *scratch,
                                                      const char *tname,
                                                      int32_t attrsz)
{
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Attribute '%s': Unable to read data (%d bytes)", EXR_REQ_SCR_WC_STR, attrsz);
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'v2f'", EXR_REQ_SCR_WC_STR);
...
static exr_result_t check_populate_screenWindowWidth(struct _internal_exr_context *ctxt,
                                                     struct _internal_exr_part *curpart,
                                                     struct _internal_exr_seq_scratch *scratch,
                                                     const char *tname,
                                                     int32_t attrsz)
{
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Attribute '%s': Unable to read data (%d bytes)", EXR_REQ_SCR_WW_STR, attrsz);
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'float'", EXR_REQ_SCR_WW_STR);
...
static exr_result_t check_populate_tiles(struct _internal_exr_context *ctxt,
                                         struct _internal_exr_part *curpart,
                                         struct _internal_exr_seq_scratch *scratch,
                                         const char *tname,
                                         int32_t attrsz)
{
  ...
  if (curpart->tiles) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "Duplicate copy of required attribute 'tiles' encountered");
  }

  if (0 != strcmp(tname, "tiledesc")) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_ATTR_TYPE_MISMATCH, "Required attribute 'tiles': Invalid type '%s'", tname);
  }
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'tiledesc'", EXR_REQ_TILES_STR);
...
static exr_result_t check_populate_name(struct _internal_exr_context *ctxt,
                                        struct _internal_exr_part *curpart,
                                        struct _internal_exr_seq_scratch *scratch,
                                        const char *tname,
                                        int32_t attrsz)
{
  ...
  rv = check_bad_attrsz(ctxt, scratch, attrsz, 1, EXR_REQ_NAME_STR, tname, &n);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (curpart->name) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "Duplicate copy of required attribute 'name' encountered");
  }

  if (0 != strcmp(tname, "string")) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_ATTR_TYPE_MISMATCH, "attribute 'name': Invalid type '%s'", tname);
  }
  ...
  if (rv != EXR_ERR_SUCCESS) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'string'", EXR_REQ_NAME_STR);
  }

  rv = scratch->sequential_read(scratch, outstr, (uint64_t)attrsz);
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_list_remove((exr_context_t)ctxt, &(curpart->attributes), curpart->name);
    curpart->name = NULL;
  ...
  rv = exr_attr_string_init_static_with_length(
      (exr_context_t)ctxt, curpart->name->string, (const char *)outstr, attrsz);
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_list_remove((exr_context_t)ctxt, &(curpart->attributes), curpart->name);
    curpart->name = NULL;
    return ctxt->report_error(ctxt, rv, "Unable to read 'name' data");
...
static exr_result_t check_populate_type(struct _internal_exr_context *ctxt,
                                        struct _internal_exr_part *curpart,
                                        struct _internal_exr_seq_scratch *scratch,
                                        const char *tname,
                                        int32_t attrsz)
{
  ...
  rv = check_bad_attrsz(ctxt, scratch, attrsz, 1, EXR_REQ_TYPE_STR, tname, &n);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (curpart->type) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "Duplicate copy of required attribute 'type' encountered");
  }

  if (0 != strcmp(tname, "string")) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_ATTR_TYPE_MISMATCH, "Required attribute 'type': Invalid type '%s'", tname);
  }
  ...
  if (rv != EXR_ERR_SUCCESS) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'string'", EXR_REQ_TYPE_STR);
  }

  rv = scratch->sequential_read(scratch, outstr, (uint64_t)attrsz);
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_list_remove((exr_context_t)ctxt, &(curpart->attributes), curpart->type);
    curpart->type = NULL;
    return ctxt->report_error(ctxt, rv, "Unable to read 'name' data");
  }
  outstr[attrsz] = '\0';
  ...
  rv = exr_attr_string_init_static_with_length(
      (exr_context_t)ctxt, curpart->type->string, (const char *)outstr, attrsz);
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_list_remove((exr_context_t)ctxt, &(curpart->attributes), curpart->type);
    curpart->type = NULL;
    return ctxt->report_error(ctxt, rv, "Unable to read 'name' data");
  ...
  if (strcmp((const char *)outstr, "scanlineimage") == 0)
    ...
    rv = ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "attribute 'type': Invalid type string '%s'", outstr);
    exr_attr_list_remove((exr_context_t)ctxt, &(curpart->attributes), curpart->type);
    curpart->type = NULL;
  }
...
static exr_result_t check_populate_version(struct _internal_exr_context *ctxt,
                                           struct _internal_exr_part *curpart,
                                           struct _internal_exr_seq_scratch *scratch,
                                           const char *tname,
                                           int32_t attrsz)
{
  ...
  if (curpart->version) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "Duplicate copy of required attribute 'version' encountered");
  }

  if (0 != strcmp(tname, "int")) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_ATTR_TYPE_MISMATCH, "attribute 'version': Invalid type '%s'", tname);
  }

  if (attrsz != sizeof(int32_t)) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_INVALID_ATTR, "attribute 'version': Invalid size %d (exp 4)", attrsz);
  }
  ...
  if (attrsz != 1)
    return ctxt->print_error(ctxt, EXR_ERR_INVALID_ATTR, "Invalid version %d: expect 1", attrsz);
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'int'", EXR_REQ_VERSION_STR);
...
static exr_result_t check_populate_chunk_count(struct _internal_exr_context *ctxt,
                                               struct _internal_exr_part *curpart,
                                               struct _internal_exr_seq_scratch *scratch,
                                               const char *tname,
                                               int32_t attrsz)
{
  ...
  if (0 != strcmp(tname, "int")) {
    scratch->sequential_skip(scratch, attrsz);
    return ctxt->print_error(
        ctxt, EXR_ERR_ATTR_TYPE_MISMATCH, "attribute 'chunkCount': Invalid type '%s'", tname);
  }
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type 'int'", EXR_REQ_CHUNK_COUNT_STR);
...
static exr_result_t check_req_attr(struct _internal_exr_context *ctxt,
                                   struct _internal_exr_part *curpart,
                                   struct _internal_exr_seq_scratch *scratch,
                                   const char *aname,
                                   const char *tname,
  ...
  switch (aname[0]) {
    case 'c':
      if (0 == strcmp(aname, EXR_REQ_CHANNELS_STR))
        return check_populate_channels(ctxt, curpart, scratch, tname, attrsz);
      if (0 == strcmp(aname, EXR_REQ_COMP_STR))
        return check_populate_compression(ctxt, curpart, scratch, tname, attrsz);
      if (0 == strcmp(aname, EXR_REQ_CHUNK_COUNT_STR))
        return check_populate_chunk_count(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 'd':
      if (0 == strcmp(aname, EXR_REQ_DATA_STR))
        return check_populate_dataWindow(ctxt, curpart, scratch, tname, attrsz);
      if (0 == strcmp(aname, EXR_REQ_DISP_STR))
        return check_populate_displayWindow(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 'l':
      if (0 == strcmp(aname, EXR_REQ_LO_STR))
        return check_populate_lineOrder(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 'n':
      if (0 == strcmp(aname, EXR_REQ_NAME_STR))
        return check_populate_name(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 'p':
      if (0 == strcmp(aname, EXR_REQ_PAR_STR))
        return check_populate_pixelAspectRatio(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 's':
      if (0 == strcmp(aname, EXR_REQ_SCR_WC_STR))
        return check_populate_screenWindowCenter(ctxt, curpart, scratch, tname, attrsz);
      if (0 == strcmp(aname, EXR_REQ_SCR_WW_STR))
        return check_populate_screenWindowWidth(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 't':
      if (0 == strcmp(aname, EXR_REQ_TILES_STR))
        return check_populate_tiles(ctxt, curpart, scratch, tname, attrsz);
      if (0 == strcmp(aname, EXR_REQ_TYPE_STR))
        return check_populate_type(ctxt, curpart, scratch, tname, attrsz);
      break;
    case 'v':
      if (0 == strcmp(aname, EXR_REQ_VERSION_STR))
        return check_populate_version(ctxt, curpart, scratch, tname, attrsz);
      break;
    default:
      break;
  }
...
static exr_result_t pull_attr(struct _internal_exr_context *ctxt,
                              struct _internal_exr_part *curpart,
                              uint8_t init_byte,
                              struct _internal_exr_seq_scratch *scratch)
{
  ...
  if (namelen == 0)
    return ctxt->report_error(
        ctxt, EXR_ERR_FILE_BAD_HEADER, "Invalid empty string encountered parsing attribute name");
  ...
  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to read attribute size for attribute '%s', type '%s'", name, type);
  attrsz = (int32_t)one_to_native32((uint32_t)attrsz);

  rv = check_req_attr(ctxt, curpart, scratch, name, type, attrsz);
  if (rv != EXR_ERR_UNKNOWN)
    return rv;
  ...
  if (!strcmp(type, "string")) {
    ...
    rv = exr_attr_list_add((exr_context_t)ctxt,
                           &(curpart->attributes),
                           name,
                           EXR_ATTR_STRING,
                           n + 1,
    ...
    rv = exr_attr_list_add_by_type(
        (exr_context_t)ctxt, &(curpart->attributes), name, type, 0, NULL, &nattr);
  }

  if (rv != EXR_ERR_SUCCESS)
    return ctxt->print_error(
        ctxt, rv, "Unable to initialize attribute '%s', type '%s'", name, type);

  switch (nattr->type) {
    case EXR_ATTR_BOX2I:
      rv = extract_attr_32bit(ctxt, scratch, nattr->box2i, name, type, attrsz, 4);
      break;
    case EXR_ATTR_BOX2F:
      rv = extract_attr_32bit(ctxt, scratch, nattr->box2f, name, type, attrsz, 4);
      break;
    case EXR_ATTR_CHLIST:
      rv = extract_attr_chlist(ctxt, scratch, nattr->chlist, name, type, attrsz);
      break;
    case EXR_ATTR_CHROMATICITIES:
      rv = extract_attr_32bit(ctxt, scratch, nattr->chromaticities, name, type, attrsz, 8);
      break;
    case EXR_ATTR_COMPRESSION:
      rv = extract_attr_uint8(
          ctxt, scratch, &(nattr->uc), name, type, attrsz, (uint8_t)EXR_COMPRESSION_LAST_TYPE);
      break;
    case EXR_ATTR_ENVMAP:
      rv = extract_attr_uint8(
          ctxt, scratch, &(nattr->uc), name, type, attrsz, (uint8_t)EXR_ENVMAP_LAST_TYPE);
      break;
    case EXR_ATTR_LINEORDER:
      rv = extract_attr_uint8(
          ctxt, scratch, &(nattr->uc), name, type, attrsz, (uint8_t)EXR_LINEORDER_LAST_TYPE);
      break;
    case EXR_ATTR_DOUBLE:
      rv = extract_attr_64bit(ctxt, scratch, &(nattr->d), name, type, attrsz, 1);
      break;
    case EXR_ATTR_FLOAT:
      rv = extract_attr_32bit(ctxt, scratch, &(nattr->f), name, type, attrsz, 1);
      break;
    case EXR_ATTR_FLOAT_VECTOR:
      rv = extract_attr_float_vector(ctxt, scratch, nattr->floatvector, name, type, attrsz);
      break;
    case EXR_ATTR_INT:
      rv = extract_attr_32bit(ctxt, scratch, &(nattr->i), name, type, attrsz, 1);
      break;
    case EXR_ATTR_KEYCODE:
      rv = extract_attr_32bit(ctxt, scratch, nattr->keycode, name, type, attrsz, 7);
      break;
    case EXR_ATTR_M33F:
      rv = extract_attr_32bit(ctxt, scratch, nattr->m33f->m, name, type, attrsz, 9);
      break;
    case EXR_ATTR_M33D:
      rv = extract_attr_64bit(ctxt, scratch, nattr->m33d->m, name, type, attrsz, 9);
      break;
    case EXR_ATTR_M44F:
      rv = extract_attr_32bit(ctxt, scratch, nattr->m44f->m, name, type, attrsz, 16);
      break;
    case EXR_ATTR_M44D:
      rv = extract_attr_64bit(ctxt, scratch, nattr->m44d->m, name, type, attrsz, 16);
      break;
    case EXR_ATTR_PREVIEW:
      rv = extract_attr_preview(ctxt, scratch, nattr->preview, name, type, attrsz);
      break;
    case EXR_ATTR_RATIONAL:
      rv = extract_attr_32bit(ctxt, scratch, nattr->rational, name, type, attrsz, 2);
      break;
    case EXR_ATTR_STRING:
      rv = extract_attr_string(ctxt, scratch, nattr->string, name, type, attrsz, (char *)strptr);
      break;
    case EXR_ATTR_STRING_VECTOR:
      rv = extract_attr_string_vector(ctxt, scratch, nattr->stringvector, name, type, attrsz);
      break;
    case EXR_ATTR_TILEDESC:
      rv = extract_attr_tiledesc(ctxt, scratch, nattr->tiledesc, name, type, attrsz);
      break;
    case EXR_ATTR_TIMECODE:
      rv = extract_attr_32bit(ctxt, scratch, nattr->timecode, name, type, attrsz, 2);
      break;
    case EXR_ATTR_V2I:
      rv = extract_attr_32bit(ctxt, scratch, nattr->v2i->arr, name, type, attrsz, 2);
      break;
    case EXR_ATTR_V2F:
      rv = extract_attr_32bit(ctxt, scratch, nattr->v2f->arr, name, type, attrsz, 2);
      break;
    case EXR_ATTR_V2D:
      rv = extract_attr_64bit(ctxt, scratch, nattr->v2d->arr, name, type, attrsz, 2);
      break;
    case EXR_ATTR_V3I:
      rv = extract_attr_32bit(ctxt, scratch, nattr->v3i->arr, name, type, attrsz, 3);
      break;
    case EXR_ATTR_V3F:
      rv = extract_attr_32bit(ctxt, scratch, nattr->v3f->arr, name, type, attrsz, 3);
      break;
    case EXR_ATTR_V3D:
      rv = extract_attr_64bit(ctxt, scratch, nattr->v3d->arr, name, type, attrsz, 3);
      break;
    case EXR_ATTR_OPAQUE:
      rv = extract_attr_opaque(ctxt, scratch, nattr->opaque, name, type, attrsz);
      break;
    case EXR_ATTR_UNKNOWN:
    case EXR_ATTR_LAST_KNOWN_TYPE:
    default:
      rv = ctxt->print_error(
          ctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid type '%s' for attribute '%s'", type, name);
      break;
  }
  if (rv != EXR_ERR_SUCCESS) {
    exr_attr_list_remove((exr_context_t)ctxt, &(curpart->attributes), nattr);
  }
...
static int64_t calc_level_size(int64_t mind,
                               int64_t maxd,
                               int level,
...
exr_result_t internal_exr_compute_tile_information(struct _internal_exr_context *ctxt,
                                                   struct _internal_exr_part *curpart,
                                                   int rebuild)
{
  exr_result_t rv = EXR_ERR_SUCCESS;
  ...
  if (curpart->tile_level_tile_count_x == NULL) {
    ...
    levcntX = (int32_t *)ctxt->alloc_fn(2 * (size_t)(numX + numY) * sizeof(int32_t));
    if (levcntX == NULL)
      return ctxt->standard_error(ctxt, EXR_ERR_OUT_OF_MEMORY);
    levszX = levcntX + numX;
    levcntY = levszX + numX;
    ...
    for (int32_t l = 0; l < numX; ++l) {
      int64_t sx = calc_level_size(dw.min.x, dw.max.x, l, EXR_GET_TILE_ROUND_MODE((*tiledesc)));
    ...
    for (int32_t l = 0; l < numY; ++l) {
      int64_t sy = calc_level_size(dw.min.y, dw.max.y, l, EXR_GET_TILE_ROUND_MODE((*tiledesc)));
...
int32_t internal_exr_compute_chunk_offset_size(struct _internal_exr_part *curpart)
{
  int32_t retval = 0;
  const exr_attr_box2i_t dw = curpart->data_window;
...
static exr_result_t update_chunk_offsets(struct _internal_exr_context *ctxt,
                                         struct _internal_exr_seq_scratch *scratch)
{
  ...
  ctxt->parts[0]->chunk_table_offset = scratch->fileoff - (uint64_t)scratch->navail;
  ...
  for (int p = 0; p < ctxt->num_parts; ++p) {
    ...
    if (prevpart != curpart)
      curpart->chunk_table_offset = prevpart->chunk_table_offset +
...
static exr_result_t read_magic_and_flags(struct _internal_exr_context *ctxt,
                                         uint32_t *outflags,
                                         uint64_t *initpos)
{
  ...
  rv = ctxt->do_read(
      ctxt, magic_and_version, sizeof(uint32_t) * 2, &fileoff, &nread, EXR_MUST_READ_ALL);
  if (rv != EXR_ERR_SUCCESS) {
    ctxt->report_error(ctxt, EXR_ERR_READ_IO, "Unable to read magic and version flags");
    return rv;
  }
...
exr_result_t internal_exr_parse_header(struct _internal_exr_context *ctxt)
{
  ...
  if (!curpart) {
    rv = ctxt->report_error(ctxt, EXR_ERR_INVALID_ARGUMENT, "Error during file initialization");
    priv_destroy_scratch(&scratch);
    return internal_exr_context_restore_handlers(ctxt, rv);
  }
  ...
  if (ctxt->strict_header) {
    ctxt->max_name_length = (flags & EXR_LONG_NAMES_FLAG) ? EXR_LONGNAME_MAXLEN :
  ...
  if (ctxt->is_singlepart_tiled) {
    if (ctxt->has_nonimage_data || ctxt->is_multipart) {
      if (ctxt->strict_header) {
        rv = ctxt->print_error(ctxt,
                               EXR_ERR_FILE_BAD_HEADER,
                               "Invalid combination of version flags: single part found, but also "
                               "marked as deep (%d) or multipart (%d)",
  ...
  do {
    ...
    if (rv != EXR_ERR_SUCCESS) {
      rv = ctxt->report_error(ctxt, EXR_ERR_FILE_BAD_HEADER, "Unable to extract header byte");
      priv_destroy_scratch(&scratch);
      return internal_exr_context_restore_handlers(ctxt, rv);
    }
    ...
    if (next_byte == '\0') {
      ...
      if (rv != EXR_ERR_SUCCESS) {
        rv = ctxt->report_error(
            ctxt, EXR_ERR_FILE_BAD_HEADER, "Unable to go to next part definition");
        priv_destroy_scratch(&scratch);
        return internal_exr_context_restore_handlers(ctxt, rv);
      }
  ...
  if (rv == EXR_ERR_SUCCESS) {
    rv = update_chunk_offsets(ctxt, &scratch);
  }
