...
exr_result_t internal_coding_fill_channel_info(exr_coding_channel_info_t **channels,
                                               int16_t *num_chans,
                                               ...
                                               const struct _internal_exr_part *part)
{
  ...
  if (chans <= 5) {
    chanfill = builtinextras;
  }
  else {
    chanfill = pctxt->alloc_fn((size_t)(chans) * sizeof(exr_coding_channel_info_t));
    if (chanfill == NULL)
      return pctxt->standard_error(pctxt, EXR_ERR_OUT_OF_MEMORY);
    memset(chanfill, 0, (size_t)(chans) * sizeof(exr_coding_channel_info_t));
  }
  ...
  for (int c = 0; c < chans; ++c) {
    ...
    decc->height = compute_sampled_height(cinfo->height, curc->y_sampling, cinfo->start_y);
    decc->width = compute_sampled_width(cinfo->width, curc->x_sampling, cinfo->start_x);
                                                 ...
                                                 const struct _internal_exr_part *part)
{
  ...
  for (int c = 0; c < chans; ++c) {
    ...
    ccic->height = compute_sampled_height(cinfo->height, curc->y_sampling, cinfo->start_y);
    ccic->width = compute_sampled_width(cinfo->width, curc->x_sampling, cinfo->start_x);
...
exr_result_t internal_encode_free_buffer(exr_encode_pipeline_t *encode,
  ...
  if (curbuf) {
    if (cursz > 0) {
      if (encode->free_fn)
        ...
        EXR_PROMOTE_CONST_CONTEXT_OR_ERROR_NO_PART_NO_LOCK(encode->context, encode->part_index);
...
exr_result_t internal_encode_alloc_buffer(exr_encode_pipeline_t *encode,
                                          exr_transcoding_pipeline_buffer_id_t bufid,
                                          void **buf,
                                          size_t *cursz,
                                          size_t newsz)
  ...
  if (!curbuf || *cursz < newsz) {
    ...
    if (curbuf == NULL) {
      ...
      return pctxt->print_error(
          pctxt, EXR_ERR_OUT_OF_MEMORY, "Unable to allocate %" PRIu64 " bytes", (uint64_t)newsz);
    }
...
exr_result_t internal_decode_free_buffer(exr_decode_pipeline_t *decode,
                                         exr_transcoding_pipeline_buffer_id_t bufid,
                                         void **buf,
                                         size_t *sz)
{
  ...
  if (curbuf) {
    if (cursz > 0) {
      if (decode->free_fn)
        ...
        EXR_PROMOTE_CONST_CONTEXT_OR_ERROR_NO_PART_NO_LOCK(decode->context, decode->part_index);
...
exr_result_t internal_decode_alloc_buffer(exr_decode_pipeline_t *decode,
                                          exr_transcoding_pipeline_buffer_id_t bufid,
                                          void **buf,
                                          size_t *cursz,
                                          size_t newsz)
