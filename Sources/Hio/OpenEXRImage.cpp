...
class Hio_OpenEXRImage final : public HioImage {
 ...
 public:
  ...
  const VtDictionary &GetMetadata() const
  {
    return _metadata;
  }
...
{
  return _exrReader.channelCount * static_cast<int>(HioGetDataSizeOfType(GetFormat()));
}
...
// but writing is not.
int64_t exr_AssetRead_Func(exr_const_context_t ctxt,
                           void *userdata,
                           void *buffer,
                           uint64_t sz,
                           uint64_t offset,
                           exr_stream_error_func_ptr_t error_cb)
{
  ...
  if (!self || !self->Asset() || !buffer || !sz) {
    if (error_cb) {
      error_cb(ctxt, EXR_ERR_INVALID_ARGUMENT, "%s", "Invalid arguments to read callback");
    }
    return -1;
  }
...
template<typename T> class ImageProcessor {
 public:
  ...
  static void FlipImage(T *buffer, int width, int height, int channelCount)
  {
    ...
    for (int y = 0; y < height / 2; ++y) {
      std::swap_ranges(buffer + y * width * channelCount,
  ...
  static void HalfToFloat(
      GfHalf *buffer, float *outBuffer, int width, int height, int channelCount)
  {
    if (!buffer || !outBuffer) {
      return;
    }
  ...
  static void FloatToHalf(
      float *buffer, GfHalf *outBuffer, int width, int height, int channelCount)
  {
    if (!buffer || !outBuffer) {
      return;
    }
...
bool Hio_OpenEXRImage::ReadCropped(int const cropTop,
                                   int const cropBottom,
                                   int const cropLeft,
                                   int const cropRight,
                                   StorageSpec const &storage)
{
  ...
  if (!resizing) {
    ...
    if (inputIsHalf && outputIsHalf) {
      memcpy(reinterpret_cast<void *>(storage.data), halfInputBuffer.data(), outSize);
    }
    else if (inputIsFloat && outputIsFloat) {
      memcpy(reinterpret_cast<void *>(storage.data), floatInputBuffer.data(), outSize);
    }
  ...
  if (inputIsHalf) {
    ImageProcessor<uint16_t>::HalfToFloat(
        &halfInputBuffer[0], &floatInputBuffer[0], fileWidth, fileHeight, fileChannelCount);
    inputIsFloat = true;
    inputIsHalf = false;
  }
...
bool Hio_OpenEXRImage::GetSamplerMetadata(HioAddressDimension dim, HioAddressMode *param) const
{
  if (!param)
    return false;
  ...
  switch (_exrReader.wrapMode) {
    ...
    case nanoexr_WrapModeRepeat:
      *param = HioAddressModeRepeat;
      break;
    case nanoexr_WrapModeMirrorRepeat:
      *param = HioAddressModeMirrorRepeat;
      break;
                                       ...
                                       bool /*suppressErrors*/)
{
  ...
  int rv = nanoexr_read_header(
      &_exrReader, exr_AssetRead_Func, _AttributeReadCallback, this, _subimage);
  if (rv != 0) {
    TF_DIAGNOSTIC_WARNING("Cannot open image \"%s\" for reading, %s",
                          filename.c_str(),
                          nanoexr_get_error_code_as_string(rv));
    return false;
  }

  if (_exrReader.numMipLevels <= mip) {
    TF_DIAGNOSTIC_WARNING("In image \"%s\" mip level %d does not exist", filename.c_str(), mip);
    return false;
  }
...
void Hio_OpenEXRImage::_AttributeWriteCallback(void *self_, exr_context_t exr)
{
  ...
  for (const std::pair<std::string, VtValue> m : *self->_callbackDict) {
    ...
    if (value.IsHolding<std::string>()) {
      nanoexr_attr_set_string(exr, self->_subimage, key.c_str(), value.Get<std::string>().c_str());
    }
    else if (value.IsHolding<char>()) {
      nanoexr_attr_set_int(exr, self->_subimage, key.c_str(), value.Get<char>());
    }
    else if (value.IsHolding<unsigned char>()) {
      nanoexr_attr_set_int(exr, self->_subimage, key.c_str(), value.Get<unsigned char>());
    }
    else if (value.IsHolding<int>()) {
      nanoexr_attr_set_int(exr, self->_subimage, key.c_str(), value.Get<int>());
    }
    else if (value.IsHolding<unsigned int>()) {
      nanoexr_attr_set_int(exr, self->_subimage, key.c_str(), value.Get<unsigned int>());
    }
    else if (value.IsHolding<float>()) {
      nanoexr_attr_set_float(exr, self->_subimage, key.c_str(), value.Get<float>());
    }
    else if (value.IsHolding<double>()) {
      nanoexr_attr_set_double(exr, self->_subimage, key.c_str(), value.Get<double>());
    }
    else if (value.IsHolding<GfMatrix4f>()) {
      nanoexr_attr_set_m44f(exr, self->_subimage, key.c_str(), value.Get<GfMatrix4f>().GetArray());
    }
      ...
      if (isWorldToNDC(key) || isWorldToCamera(key)) {
        ...
        nanoexr_attr_set_m44f(exr, self->_subimage, key.c_str(), mf.GetArray());
      }
      else {
        nanoexr_attr_set_m44d(
            exr, self->_subimage, key.c_str(), value.Get<GfMatrix4d>().GetArray());
      }
    }
  }
}
...
bool Hio_OpenEXRImage::Write(StorageSpec const &storage, VtDictionary const &metadata)
{
  ...
  if (type == HioTypeUnsignedByte) {
    ...
    rv = nanoexr_write_exr(_filename.c_str(),
                           _AttributeWriteCallback,
                           this,
                           storage.width,
                           storage.height,
                           storage.flipped,
