...
TF_DEFINE_PUBLIC_TOKENS(HioGlslfxResourceLayoutTokens, HIO_GLSLFX_RESOURCE_LAYOUT_TOKENS);
...
TfToken _Token(InputValue const &input)
{
  return TfToken(input.GetWithDefault(HioGlslfxResourceLayoutTokens->unknown.GetString()));
}

InputValueVector _GetInputValueVector(InputValue const &input)
{
  return input.GetWithDefault(InputValueVector());
}

// Check fpr specific storage and interpolation qualifiers.
bool _IsMemberQualifier(TfToken const &input)
{
...
MemberVector _ParseMembers(InputValueVector const &input, int fromElement)
{
  MemberVector result;
...
bool _ParseValue(InputValueVector const &input, Element *element)
{
  if (input.size() != 3 && input.size() != 4)
    return false;
  if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->inValue) {
    *element = Element(InOut::STAGE_IN,
                       Kind::VALUE,
    ...
    *element = Element(InOut::STAGE_OUT,
                       Kind::VALUE,
                       /*dataType=*/_Token(input[1]),
                       /*name=*/_Token(input[2]));
    if (input.size() == 4) {
...
bool _ParseValueArray(InputValueVector const &input, Element *element)
{
  if (input.size() != 4)
    return false;
  ...
  else if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->outValueArray) {
    *element = Element(InOut::STAGE_OUT,
                       Kind::VALUE,
                       /*dataType=*/_Token(input[1]),
                       /*name=*/_Token(input[2]),
...
//          ["vec3", "Neye"]
//      ]
bool _ParseBlock(InputValueVector const &input, Element *element)
{
  if (input.size() < 4)
    return false;
...
//          ["vec3", "Neye"]
//      ]
bool _ParseBlockArray(InputValueVector const &input, Element *element)
{
  if (input.size() < 5)
    return false;
  ...
  else if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->outBlockArray) {
    *element = Element(InOut::STAGE_OUT,
                       Kind::BLOCK,
                       /*dataType=*/HioGlslfxResourceLayoutTokens->block,
                       /*name=*/_Token(input[2]),
...
bool _ParseQualifier(InputValueVector const &input, Element *element)
{
  if (input.size() != 2)
    return false;
  if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->inValue) {
...
//          ["int",  "resetPass"]
//      ]
bool _ParseUniformBlock(InputValueVector const &input, Element *element)
{
  if (input.size() < 4)
    return false;
  if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->uniformBlock) {
    *element = Element(InOut::NONE,
                       Kind::UNIFORM_BLOCK_CONSTANT_PARAMS,
...
//          ["uint", "drawCommands", "[]"]
//      ]
bool _ParseBuffer(InputValueVector const &input, Element *element)
{
  if (input.size() < 4)
    return false;
  if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->bufferReadOnly) {
    *element = Element(InOut::NONE,
                       Kind::BUFFER_READ_ONLY,
  ...
  else if (_Token(input[0]) == HioGlslfxResourceLayoutTokens->bufferReadWrite) {
    *element = Element(InOut::NONE,
...
void _ParsePerStageLayout(ElementVector *result, VtValue const &perStageLayout)
{
  ...
  for (VtValue const &perSnippet : perSnippetVector) {
    ...
    for (VtValue const &perDecl : perDeclVector) {
      ...
      if (_ParseValue(input, &element) || _ParseValueArray(input, &element) ||
          _ParseBlock(input, &element) || _ParseBlockArray(input, &element) ||
          _ParseQualifier(input, &element) || _ParseUniformBlock(input, &element) ||
...
/* static */
void HioGlslfxResourceLayout::ParseLayout(ElementVector *result,
