...
static exr_result_t update_pack_unpack_ptrs(exr_decode_pipeline_t *decode)
{
  exr_result_t rv;
  exr_storage_t stortype = ((exr_storage_t)decode->chunk.type);

  if (stortype == EXR_STORAGE_DEEP_SCANLINE || stortype == EXR_STORAGE_DEEP_TILED) {
    size_t sampsize = (((size_t)decode->chunk.width) * ((size_t)decode->chunk.height));
    ...
    if (decode->chunk.sample_count_table_size == sampsize) {
      internal_decode_free_buffer(decode,
    ...
    if (rv != EXR_ERR_SUCCESS || (decode->decode_flags & EXR_DECODE_SAMPLE_DATA_ONLY) != 0)
      return rv;
  }
...
static exr_result_t read_uncompressed_direct(exr_decode_pipeline_t *decode)
{
  exr_result_t rv;
  int height, start_y;
  uint64_t dataoffset, toread;
  uint8_t *cdata;
  EXR_PROMOTE_READ_CONST_CONTEXT_OR_ERROR_NO_PART(decode->context, decode->part_index);
  ...
  for (int y = 0; y < height; ++y) {
    for (int c = 0; c < decode->channel_count; ++c) {
      ...
      toread = (uint64_t)decc->width * (uint64_t)decc->bytes_per_element;

      if (decc->height == 0)
        continue;

      if (decc->y_samples > 1) {
        if (((start_y + y) % decc->y_samples) != 0)
          continue;
        cdata += ((uint64_t)(y / decc->y_samples) * (uint64_t)decc->user_line_stride);
      }
      else {
      ...
      rv = pctxt->do_read(pctxt, cdata, toread, &dataoffset, NULL, EXR_MUST_READ_ALL);
...
static exr_result_t default_read_chunk(exr_decode_pipeline_t *decode)
{
  exr_result_t rv;
  EXR_PROMOTE_READ_CONST_CONTEXT_AND_PART_OR_ERROR(decode->context, decode->part_index);
      ...
      part->storage_mode == EXR_STORAGE_DEEP_TILED)
  {
    ...
    if ((decode->decode_flags & EXR_DECODE_SAMPLE_DATA_ONLY)) {
      rv = exr_read_deep_chunk(decode->context,
                               decode->part_index,
                               &(decode->chunk),
                               NULL,
                               decode->packed_sample_count_table);
    ...
    if (rv != EXR_ERR_SUCCESS)
      return rv;
    rv = exr_read_chunk(
        decode->context, decode->part_index, &(decode->chunk), decode->packed_buffer);
  }

  return rv;
}

static exr_result_t decompress_data(const struct _internal_exr_context *pctxt,
  ...
  switch (ctype) {
    case EXR_COMPRESSION_NONE:
      return pctxt->report_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "no compression set but still trying to decompress");

    case EXR_COMPRESSION_RLE:
      rv = internal_exr_undo_rle(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_ZIP:
    case EXR_COMPRESSION_ZIPS:
      rv = internal_exr_undo_zip(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_PIZ:
      rv = internal_exr_undo_piz(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_PXR24:
      rv = internal_exr_undo_pxr24(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_B44:
      rv = internal_exr_undo_b44(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_B44A:
      rv = internal_exr_undo_b44a(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_DWAA:
      rv = internal_exr_undo_dwaa(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_DWAB:
      rv = internal_exr_undo_dwab(decode, packbufptr, packsz, unpackbufptr, unpacksz);
      break;
    case EXR_COMPRESSION_LAST_TYPE:
    default:
      return pctxt->print_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "Compression technique 0x%02X invalid", ctype);
  }

  return rv;
}

static exr_result_t default_decompress_chunk(exr_decode_pipeline_t *decode)
{
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_SCANLINE ||
      part->storage_mode == EXR_STORAGE_DEEP_TILED)
  {
    uint64_t sampsize = (((uint64_t)decode->chunk.width) * ((uint64_t)decode->chunk.height));
    sampsize *= sizeof(int32_t);

    rv = decompress_data(pctxt,
                         part->comp_type,
                         decode,
                         decode->packed_sample_count_table,
                         decode->chunk.sample_count_table_size,
...
static exr_result_t unpack_sample_table(const struct _internal_exr_context *pctxt,
                                        exr_decode_pipeline_t *decode)
{
  exr_result_t rv = EXR_ERR_SUCCESS;
  ...
  if (totsamp < 0 || (((uint64_t)totsamp) * combSampSize) > decode->chunk.unpacked_size) {
    rv = pctxt->report_error(pctxt, EXR_ERR_INVALID_SAMPLE_DATA, "Corrupt sample count table");
  }
  return rv;
}
...
exr_result_t exr_decoding_initialize(exr_const_context_t ctxt,
                                     int part_index,
                                     ...
                                     exr_decode_pipeline_t *decode)
{
  ...
  *decode = nil;
  ...
  rv = internal_coding_fill_channel_info(&(decode->channels),
...
exr_result_t exr_decoding_choose_default_routines(exr_const_context_t ctxt,
                                                  int part_index,
                                                  exr_decode_pipeline_t *decode)
{
  int32_t isdeep = 0, chanstofill = 0, chanstounpack = 0, sametype = -2, sameouttype = -2,
          samebpc = 0, sameoutbpc = 0, hassampling = 0, hastypechange = 0, simpinterleave = 0,
          simpinterleaverev = 0, simplineoff = 0, sameoutinc = 0;
  ...
  for (int c = 0; c < decode->channel_count; ++c) {
    ...
    if (decc->height == 0 || !decc->decode_to_ptr)
      continue;
     ...
     */
    if (decc->user_bytes_per_element != 2 && decc->user_bytes_per_element != 4)
      return pctxt->print_error(
          pctxt,
          EXR_ERR_INVALID_ARGUMENT,
    ...
    if (decc->x_samples != 1 || decc->y_samples != 1)
      hassampling = 1;

    ++chanstofill;
    if (decc->user_pixel_stride != decc->bytes_per_element)
      ++chanstounpack;
    if (decc->user_data_type != decc->data_type)
      ++hastypechange;
    ...
    if (simpinterleave == 0) {
      ...
      if (simpinterleave > 0 &&
          decc->decode_to_ptr != (interleaveptr + c * decc->user_bytes_per_element))
      {
        simpinterleave = -1;
      }
   ...
   * to all the channels */
  if (!isdeep && part->comp_type == EXR_COMPRESSION_NONE && chanstounpack == 0 &&
      hastypechange == 0 && chanstofill > 0 && chanstofill == decode->channel_count)
  {
  ...
  decode->unpack_and_convert_fn = internal_exr_match_decode(decode,
                                                            isdeep,
                                                            chanstofill,
                                                            chanstounpack,
                                                            sametype,
  ...
  if (!decode->unpack_and_convert_fn)
    return pctxt->report_error(
        pctxt, EXR_ERR_ARGUMENT_OUT_OF_RANGE, "Unable to choose valid unpack routine");
...
exr_result_t exr_decoding_update(exr_const_context_t ctxt,
                                 int part_index,
                                 const exr_chunk_info_t *cinfo,
                                 exr_decode_pipeline_t *decode)
{
...
exr_result_t exr_decoding_run(exr_const_context_t ctxt,
                              int part_index,
                              exr_decode_pipeline_t *decode)
{
  exr_result_t rv;
  ...
  if (!decode->read_fn)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Decode pipeline has no read_fn declared");
  rv = decode->read_fn(decode);
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->report_error(pctxt, rv, "Unable to read pixel data block from context");

  if (rv == EXR_ERR_SUCCESS)
    rv = update_pack_unpack_ptrs(decode);
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->report_error(
        pctxt, rv, "Decode pipeline unable to update pack / unpack pointers");

  if (rv == EXR_ERR_SUCCESS && decode->decompress_fn)
    rv = decode->decompress_fn(decode);
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->report_error(pctxt, rv, "Decode pipeline unable to decompress data");
  ...
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->report_error(pctxt, rv, "Decode pipeline unable to unpack deep sample table");

  if (rv == EXR_ERR_SUCCESS && decode->realloc_nonimage_data_fn)
    rv = decode->realloc_nonimage_data_fn(decode);
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->report_error(
        pctxt, rv, "Decode pipeline unable to realloc deep sample table info");

  if (rv == EXR_ERR_SUCCESS && decode->unpack_and_convert_fn)
    rv = decode->unpack_and_convert_fn(decode);
  if (rv != EXR_ERR_SUCCESS)
    return pctxt->report_error(pctxt, rv, "Decode pipeline unable to unpack and convert data");
