...
exr_result_t exr_attr_string_vector_init(exr_context_t ctxt,
                                         ...
                                         int32_t nent)
{
  ...
  if (!sv)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid reference to string vector object to assign to");
...
exr_result_t exr_attr_string_vector_copy(exr_context_t ctxt,
                                         exr_attr_string_vector_t *sv,
                                         const exr_attr_string_vector_t *src)
{
  ...
  for (int i = 0; rv == EXR_ERR_SUCCESS && i < src->n_strings; ++i) {
    rv = exr_attr_string_set_with_length(ctxt,
...
exr_result_t exr_attr_string_vector_init_entry(exr_context_t ctxt,
                                               exr_attr_string_vector_t *sv,
                                               int32_t idx,
                                               int32_t len)
{
  ...
  if (sv) {
    ...
    return exr_attr_string_init(ctxt, EXR_CONST_CAST(exr_attr_string_t *, sv->strings + idx), len);
  }
...
exr_result_t exr_attr_string_vector_set_entry_with_length(
    exr_context_t ctxt, exr_attr_string_vector_t *sv, int32_t idx, const char *s, int32_t len)
{
  INTERN_EXR_PROMOTE_CONTEXT_OR_ERROR(ctxt);
...
exr_result_t exr_attr_string_vector_set_entry(exr_context_t ctxt,
                                              exr_attr_string_vector_t *sv,
                                              int32_t idx,
                                              const char *s)
{
...
exr_result_t exr_attr_string_vector_add_entry_with_length(exr_context_t ctxt,
                                                          exr_attr_string_vector_t *sv,
                                                          const char *s,
                                                          int32_t len)
{
  ...
  rv = exr_attr_string_create_with_length(ctxt, nlist + sv->n_strings, s, len);
  if (rv == EXR_ERR_SUCCESS)
    sv->n_strings = nent;
  return rv;
}
...
exr_result_t exr_attr_string_vector_add_entry(exr_context_t ctxt,
                                              exr_attr_string_vector_t *sv,
                                              const char *s)
{
  int32_t len = 0;
