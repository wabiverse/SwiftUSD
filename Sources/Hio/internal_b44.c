...
static inline void convertFromLinear(uint16_t s[16])
{
  for (int i = 0; i < 16; ++i)
    s[i] = exrcore_expTable[s[i]];
}

static inline void convertToLinear(uint16_t s[16])
{
  for (int i = 0; i < 16; ++i)
...
static inline int shiftAndRound(int x, int shift)
{
  int a, b;
  //
  // Compute
 ...
 *  0xffff		NAN			0x8000
 */
static int pack(const uint16_t s[16], uint8_t b[14], int flatfields, int exactmax)
{
  int d[16];
  int r[15];
  int rMin;
...
static inline void unpack14(const uint8_t b[14], uint16_t s[16])
{
  uint16_t shift, bias;
  ...
  s[4] = (uint16_t)((uint32_t)s[0] +
                    (uint32_t)((((uint32_t)(b[2] << 4) | (uint32_t)(b[3] >> 4)) & 0x3fu)
                               << shift) -
                    bias);
  s[8] = (uint16_t)((uint32_t)s[4] +
                    (uint32_t)((((uint32_t)(b[3] << 2) | (uint32_t)(b[4] >> 6)) & 0x3fu)
                               << shift) -
                    bias);
  s[12] = (uint16_t)((uint32_t)s[8] + (uint32_t)((uint32_t)(b[4] & 0x3fu) << shift) - bias);

  s[1] = (uint16_t)((uint32_t)s[0] + (uint32_t)((uint32_t)(b[5] >> 2) << shift) - bias);
  s[5] = (uint16_t)((uint32_t)s[4] +
                    (uint32_t)((((uint32_t)(b[5] << 4) | (uint32_t)(b[6] >> 4)) & 0x3fu)
                               << shift) -
                    bias);
  s[9] = (uint16_t)((uint32_t)s[8] +
                    (uint32_t)((((uint32_t)(b[6] << 2) | (uint32_t)(b[7] >> 6)) & 0x3fu)
                               << shift) -
                    bias);
  s[13] = (uint16_t)((uint32_t)s[12] + (uint32_t)((uint32_t)(b[7] & 0x3fu) << shift) - bias);

  s[2] = (uint16_t)((uint32_t)s[1] + (uint32_t)((uint32_t)(b[8] >> 2) << shift) - bias);
  s[6] = (uint16_t)((uint32_t)s[5] +
                    (uint32_t)((((uint32_t)(b[8] << 4) | (uint32_t)(b[9] >> 4)) & 0x3fu)
                               << shift) -
                    bias);
  s[10] = (uint16_t)((uint32_t)s[9] +
                     (uint32_t)((((uint32_t)(b[9] << 2) | (uint32_t)(b[10] >> 6)) & 0x3fu)
                                << shift) -
                     bias);
  s[14] = (uint16_t)((uint32_t)s[13] + (uint32_t)((uint32_t)(b[10] & 0x3fu) << shift) - bias);

  s[3] = (uint16_t)((uint32_t)s[2] + (uint32_t)((uint32_t)(b[11] >> 2) << shift) - bias);
  s[7] = (uint16_t)((uint32_t)s[6] +
                    (uint32_t)((((uint32_t)(b[11] << 4) | (uint32_t)(b[12] >> 4)) & 0x3fu)
                               << shift) -
                    bias);
  s[11] = (uint16_t)((uint32_t)s[10] +
                     (uint32_t)((((uint32_t)(b[12] << 2) | (uint32_t)(b[13] >> 6)) & 0x3fu)
                                << shift) -
                     bias);
  s[15] = (uint16_t)((uint32_t)s[14] + (uint32_t)((uint32_t)(b[13] & 0x3fu) << shift) - bias);
...
static inline void unpack3(const uint8_t b[3], uint16_t s[16])
{
  s[0] = ((uint16_t)(b[0] << 8)) | ((uint16_t)b[1]);
...
static exr_result_t compress_b44_impl(exr_encode_pipeline_t *encode, int flat_field)
{
...
static exr_result_t uncompress_b44_impl(exr_decode_pipeline_t *decode,
                                        ...
                                        uint64_t uncomp_buf_size)
{
  ...
  for (int c = 0; c < decode->channel_count; ++c) {
    ...
    nx = curc->width;
    ny = curc->height;
    nBytes = (uint64_t)(ny) * (uint64_t)(nx) * (uint64_t)(curc->bytes_per_element);
    ...
    for (int y = 0; y < ny; y += 4) {
      ...
      for (int x = 0; x < nx; x += 4) {
        ...
        n = (x + 3 < nx) ? 4 * sizeof(uint16_t) : (uint64_t)(nx - x) * sizeof(uint16_t);
        if (y + 3 < ny) {
          memcpy(row0, &s[0], n);
          memcpy(row1, &s[4], n);
          memcpy(row2, &s[8], n);
...
static uint64_t compute_scratch_buffer_size(exr_decode_pipeline_t *decode,
                                            uint64_t uncompressed_size)
{
  ...
  for (int c = 0; c < decode->channel_count; ++c) {
    ...
    comp += (uint64_t)(ny) * (uint64_t)(nx) * (uint64_t)(curc->bytes_per_element);
  }
  if (comp > ret)
    ret = comp;
  return ret;
...
exr_result_t internal_exr_undo_b44(exr_decode_pipeline_t *decode,
                                   const void *compressed_data,
                                   uint64_t comp_buf_size,
                                   void *uncompressed_data,
                                   uint64_t uncompressed_size)
{
  ...
  return uncompress_b44_impl(
      decode, compressed_data, comp_buf_size, uncompressed_data, uncompressed_size);
}

exr_result_t internal_exr_undo_b44a(exr_decode_pipeline_t *decode,
                                    const void *compressed_data,
                                    uint64_t comp_buf_size,
                                    void *uncompressed_data,
                                    uint64_t uncompressed_size)
