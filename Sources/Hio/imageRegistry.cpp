...
HioImageRegistry::_ConstructImage(std::string const &filename)
{
  ...
  {
    // Unknown prim type.
    TF_DEBUG(HIO_DEBUG_TEXTURE_IMAGE_PLUGINS).Msg("[PluginLoad] Unknown image type '%s' for file '%s'\n", fileExtension.GetText(), filename.c_str());
    return nullptr;
  }
  ...
  TF_DEBUG(HIO_DEBUG_TEXTURE_IMAGE_PLUGINS).Msg("[PluginLoad] Loaded plugin '%s' for image type '%s' for "
                                                "file '%s'\n",
                                                pluginType.GetTypeName().c_str(), fileExtension.GetText(), filename.c_str());
