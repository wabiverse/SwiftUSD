...
static exr_result_t LossyDctEncoder_base_construct(LossyDctEncoder *e,
...
static exr_result_t LossyDctEncoder_construct(LossyDctEncoder *e,
                                              float quantBaseError,
...
static exr_result_t LossyDctEncoderCsc_construct(LossyDctEncoder *e,
                                                 float quantBaseError,
...
static exr_result_t LossyDctEncoder_execute(void *(*alloc_fn)(size_t),
                                            void (*free_fn)(void *),
                                            LossyDctEncoder *e);

static void LossyDctEncoder_rleAc(LossyDctEncoder *e, uint16_t *block, uint16_t **acPtr);

/**************************************/

exr_result_t LossyDctEncoder_base_construct(LossyDctEncoder *e,
                                            float quantBaseError,
                                            uint8_t *packedAc,
                                            uint8_t *packedDc,
                                            const uint16_t *toNonlinear,
                                            ...
                                            int height)
{
  ...
  //
  int jpegQuantTableY[] = {16, 11, 10, 16, 24,  40,  51,  61,  12, 12, 14, 19, 26,  58,  60,  55,
  ...
  int jpegQuantTableCbCr[] = {17, 18, 24, 47, 99, 99, 99, 99, 18, 21, 26, 66, 99, 99, 99, 99,
  ...
  if (e->_quantBaseError < 0)
    e->_quantBaseError = 0;

  for (int idx = 0; idx < 64; ++idx) {
    e->_quantTableY[idx] = (e->_quantBaseError * (float)(jpegQuantTableY[idx]) /
                            (float)(jpegQuantTableYMin));

    e->_quantTableCbCr[idx] = (e->_quantBaseError * (float)(jpegQuantTableCbCr[idx]) /
...
exr_result_t LossyDctEncoder_construct(LossyDctEncoder *e,
                                       float quantBaseError,
                                       DctCoderChannelData *rowPtrs,
                                       uint8_t *packedAc,
                                       uint8_t *packedDc,
...
//
exr_result_t LossyDctEncoderCsc_construct(LossyDctEncoder *e,
                                          float quantBaseError,
                                          DctCoderChannelData *rowPtrsR,
                                          DctCoderChannelData *rowPtrsG,
                                          DctCoderChannelData *rowPtrsB,
...
//
static void toZigZag(uint16_t *dst, uint16_t *src)
{
  static const int remap[] = {0,  1,  8,  16, 9,  2,  3,  10, 17, 24, 32, 25, 18, 11, 4,  5,
...
//
static inline int countSetBits(uint16_t src)
{
...
//
static inline uint16_t quantize(float dctval, float errorTolerance)
{
  uint16_t tmp;
  ...
  for (int targetNumSetBits = numSetBits - 1; targetNumSetBits >= 0; --targetNumSetBits) {
    tmp = *closest;

    if (fabsf(half_to_float(tmp) - srcFloat) < errorTolerance)
      return tmp;
                                     ...
                                     LossyDctEncoder *e)
{
  ...
  if (tmpHalfBufferElements) {
    tmpHalfBuffer = (uint16_t *)alloc_fn((size_t)tmpHalfBufferElements * sizeof(uint16_t));
    if (!tmpHalfBuffer)
  ...
  for (int chan = 1; chan < numComp; ++chan)
    chanData[chan]->_dc_comp = chanData[chan - 1]->_dc_comp + numBlocksX * numBlocksY;
  ...
  for (int blocky = 0; blocky < numBlocksY; ++blocky) {
    for (int blockx = 0; blockx < numBlocksX; ++blockx) {
      ...
      if (numComp == 3) {
        csc709Forward64(chanData[0]->_dctData, chanData[1]->_dctData, chanData[2]->_dctData);
      }
      ...
      for (int chan = 0; chan < numComp; ++chan) {
        ...
        for (int i = 0; i < 64; ++i) {
          halfCoef[i] = quantize(chanData[chan]->_dctData[i], quantTable[i]);
        }
...
void LossyDctEncoder_rleAc(LossyDctEncoder *e, uint16_t *block, uint16_t **acPtr)
{
  ...
  while (dctComp < 64) {
    ...
    while ((dctComp + runLen < 64) && (block[dctComp + runLen] == rleSymbol)) {
      runLen++;
    }
