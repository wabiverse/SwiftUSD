...
static exr_result_t attr_init(struct _internal_exr_context *ctxt, exr_attribute_t *nattr)
{
  switch (nattr->type) {
    case EXR_ATTR_BOX2I: {
...
static exr_result_t attr_destroy(struct _internal_exr_context *ctxt, exr_attribute_t *attr)
{
  ...
  switch (attr->type) {
    case EXR_ATTR_CHLIST:
      ...
      break;
    case EXR_ATTR_FLOAT_VECTOR:
      rv = exr_attr_float_vector_destroy((exr_context_t)ctxt, attr->floatvector);
      break;
    case EXR_ATTR_PREVIEW:
    ...
    case EXR_ATTR_STRING:
      ...
      break;
    case EXR_ATTR_STRING_VECTOR:
      rv = exr_attr_string_vector_destroy((exr_context_t)ctxt, attr->stringvector);
      break;
    case EXR_ATTR_OPAQUE:
      rv = exr_attr_opaquedata_destroy((exr_context_t)ctxt, attr->opaque);
      break;
    case EXR_ATTR_BOX2I:
    case EXR_ATTR_BOX2F:
    case EXR_ATTR_CHROMATICITIES:
...
exr_result_t exr_attr_list_compute_size(exr_context_t ctxt,
                                        ...
                                        uint64_t *out)
{
  ...
  if (!list)
    return pctxt->report_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "Missing list to compute size");

  if (!out)
    return pctxt->report_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "Expected output pointer");
    ...
    switch (cur->type) {
      ...
      case EXR_ATTR_OPAQUE:
        if (cur->opaque->packed_data)
          ...
          int32_t sz = 0;
          rv = exr_attr_opaquedata_pack(ctxt, cur->opaque, &sz, NULL);
...
exr_result_t exr_attr_list_find_by_name(exr_const_context_t ctxt,
                                        exr_attribute_list_t *list,
                                        const char *name,
                                        exr_attribute_t **out)
{
  ...
  if (!out)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid output pointer passed to find_by_name");

  if (!name || name[0] == '\0')
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid name passed to find_by_name");

  if (!list)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid list pointer passed to find_by_name");
...
static exr_result_t add_to_list(struct _internal_exr_context *ctxt,
                                exr_attribute_list_t *list,
                                ...
                                const char *name)
{
  ...
  if (nattrsz > list->num_alloced) {
    ...
    if ((size_t)nattrsz > nsize)
      nsize = (size_t)(nattrsz) + 1;
    attrs = (exr_attribute_t **)ctxt->alloc_fn(sizeof(exr_attribute_t *) * nsize * 2);
    if (!attrs) {
      ctxt->free_fn(nattr);
...
static exr_result_t validate_attr_arguments(struct _internal_exr_context *ctxt,
                                            exr_attribute_list_t *list,
                                            const char *name,
                                            int32_t data_len,
                                            uint8_t **data_ptr,
                                            exr_attribute_t **attr)
{
  ...
  if (!list) {
    return ctxt->report_error(
        ctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid list pointer to attr_list_add");
  }
  ...
  *attr = NULL;
  ...
  if (!name || name[0] == '\0') {
    return ctxt->report_error(ctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid name to add_by_type");
  }

  /* is it already in the list? */
  rv = exr_attr_list_find_by_name((exr_const_context_t)ctxt, list, name, &nattr);
...
static void check_attr_handler(struct _internal_exr_context *pctxt, exr_attribute_t *attr)
{
  if (attr->type == EXR_ATTR_OPAQUE) {
    ...
    exr_result_t rv = exr_attr_list_find_by_name(
        (exr_const_context_t)pctxt, &(pctxt->custom_handlers), attr->type_name, &handler);
    if (rv == EXR_ERR_SUCCESS && handler) {
      ...
      attr->opaque->destroy_unpacked_func_ptr = handler->opaque->destroy_unpacked_func_ptr;
    }
  }
}
...
static exr_result_t create_attr_block(struct _internal_exr_context *pctxt,
                                      exr_attribute_t **attr,
                                      size_t dblocksize,
                                      int32_t data_len,
                                      uint8_t **data_ptr,
...
exr_result_t exr_attr_list_add_by_type(exr_context_t ctxt,
                                       exr_attribute_list_t *list,
                                       const char *name,
                                       const char *type,
                                       int32_t data_len,
                                       ...
                                       exr_attribute_t **attr)
{
  ...
  if (!type || type[0] == '\0') {
    return pctxt->report_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid type to add_by_type");
  }

  rval = validate_attr_arguments(pctxt, list, name, data_len, data_ptr, attr);
  ...
  if (known) {
    rval = create_attr_block(
        pctxt, &nattr, known->exp_size, data_len, data_ptr, name, nlen, NULL, 0);
    ...
    rval = create_attr_block(
        pctxt, &nattr, sizeof(exr_attr_opaquedata_t), data_len, data_ptr, name, nlen, type, tlen);
...
exr_result_t exr_attr_list_add(exr_context_t ctxt,
                               exr_attribute_list_t *list,
                               const char *name,
                               exr_attribute_type_t type,
                               int32_t data_len,
...
exr_result_t exr_attr_list_add_static_name(exr_context_t ctxt,
                                           exr_attribute_list_t *list,
                                           const char *name,
                                           exr_attribute_type_t type,
                                           int32_t data_len,
                                           ...
                                           exr_attribute_t **attr)
{
  ...
  rval = create_attr_block(pctxt, &nattr, known->exp_size, data_len, data_ptr, NULL, 0, NULL, 0);
...
exr_result_t exr_attr_list_remove(exr_context_t ctxt,
                                  exr_attribute_list_t *list,
                                  exr_attribute_t *attr)
{
  ...
  if (!attr) {
    return pctxt->report_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "NULL attribute passed to remove");
  }

  if (!list) {
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid list pointer to remove attribute");
  }
  ...
  if (attridx == -1) {
    return pctxt->report_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "Attribute not in list");
  }
