...
static exr_result_t LossyDctDecoder_base_construct(LossyDctDecoder *d,
                                                   uint8_t *packedAc,
                                                   uint8_t *packedAcEnd,
                                                   uint8_t *packedDc,
...
static exr_result_t LossyDctDecoder_construct(LossyDctDecoder *d,
...
static exr_result_t LossyDctDecoderCsc_construct(LossyDctDecoder *d,
...
static exr_result_t LossyDctDecoder_execute(void *(*alloc_fn)(size_t),
                                            void (*free_fn)(void *),
                                            LossyDctDecoder *d);
...
//
static int LossyDctDecoder_unRleAc(LossyDctDecoder *d,
                                   int *lastNonZero,
                                   uint16_t **currAcComp,
...
//
exr_result_t LossyDctDecoder_construct(LossyDctDecoder *d,
                                       DctCoderChannelData *rowPtrs,
                                       uint8_t *packedAc,
                                       uint8_t *packedAcEnd,
                                       uint8_t *packedDc,
                                       ...
                                       int height)
{
  ...
  rv = LossyDctDecoder_base_construct(d, packedAc, packedAcEnd, packedDc, toLinear, width, height);
...
// in NULL here.
//
exr_result_t LossyDctDecoderCsc_construct(LossyDctDecoder *d,
                                          DctCoderChannelData *rowPtrsR,
                                          DctCoderChannelData *rowPtrsG,
                                          DctCoderChannelData *rowPtrsB,
                                          uint8_t *packedAc,
...
exr_result_t LossyDctDecoder_base_construct(LossyDctDecoder *d,
                                            uint8_t *packedAc,
                                            uint8_t *packedAcEnd,
                                            uint8_t *packedDc,
                                            const uint16_t *toLinear,
...
exr_result_t LossyDctDecoder_execute(void *(*alloc_fn)(size_t),
                                     void (*free_fn)(void *),
                                     LossyDctDecoder *d)
{
  exr_result_t rv;
  ...
  rowBlockHandle = alloc_fn((size_t)numComp * (size_t)numBlocksX * 64 * sizeof(uint16_t) +
                            _SSE_ALIGNMENT);
  if (!rowBlockHandle)
  ...
  for (int blocky = 0; blocky < numBlocksY; ++blocky) {
    ...
    for (int blockx = 0; blockx < numBlocksX; ++blockx) {
      ...
      for (int comp = 0; comp < numComp; ++comp) {
...
#ifdef IMF_HAVE_SSE2
        {
          ...
          dst[0] = _mm_insert_epi16(_mm_setzero_si128(), *currDcComp[comp]++, 0);
        }
#else /* IMF_HAVE_SSE2 */
        ...
        rv = LossyDctDecoder_unRleAc(d, &lastNonZero, &currAcComp, acCompEnd, halfZigData);
        if (rv != EXR_ERR_SUCCESS) {
      ...
      if (numComp == 3) {
        if (!blockIsConstant) {
          csc709Inverse64(chanData[0]->_dctData, chanData[1]->_dctData, chanData[2]->_dctData);
        }
        else {
          csc709Inverse(chanData[0]->_dctData, chanData[1]->_dctData, chanData[2]->_dctData);
        }
      }
      ...
      for (int comp = 0; comp < numComp; ++comp) {
        if (!blockIsConstant) {
          (*convertFloatToHalf64)(&rowBlock[comp][blockx * 64], chanData[comp]->_dctData);
        }
        else {
#ifdef IMF_HAVE_SSE2
          ...
          dst[0] = _mm_set1_epi16((short)float_to_half(chanData[comp]->_dctData[0]));
    ...
    for (int comp = 0; comp < numComp; ++comp) {
      ...
      for (int y = 8 * blocky; y < 8 * blocky + maxY; ++y) {
        if ((uintptr_t)(chanData[comp]->_rows[y]) & _SSE_ALIGNMENT_MASK)
      ...
      if (fastPath) {
        ...
        for (int y = 8 * blocky; y < 8 * blocky + maxY; ++y) {
          ...
          for (int blockx = 0; blockx < numFullBlocksX; ++blockx) {
            uint16_t *src = &rowBlock[comp][blockx * 64 + ((y & 0x7) * 8)];
      ...
      if (numFullBlocksX != numBlocksX) {
        for (int y = 8 * blocky; y < 8 * blocky + maxY; ++y) {
          uint16_t *src = (uint16_t *)&rowBlock[comp][numFullBlocksX * 64 + ((y & 0x7) * 8)];
...
// prior to calling
//
exr_result_t LossyDctDecoder_unRleAc(LossyDctDecoder *d,
                                     int *lastNonZero,
                                     uint16_t **currAcComp,
