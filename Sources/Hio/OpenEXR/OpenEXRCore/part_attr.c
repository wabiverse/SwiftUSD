...
exr_result_t exr_get_attribute_by_index(exr_const_context_t ctxt,
                                        int part_index,
...
exr_result_t exr_get_attribute_by_name(exr_const_context_t ctxt,
                                       int part_index,
                                       const char *name,
                                       const exr_attribute_t **outattr)
{
  ...
  rv = exr_attr_list_find_by_name(EXR_CONST_CAST(exr_context_t, ctxt),
...
exr_result_t exr_get_attribute_list(exr_const_context_t ctxt,
                                    int part_index,
                                    exr_attr_list_access_mode_t mode,
                                    int32_t *count,
                                    const exr_attribute_t **outlist)
{
  ...
  if (outlist && *count >= part->attributes.num_attributes)
    memcpy(EXR_CONST_CAST(exr_attribute_t **, outlist),
           srclist,
           sizeof(exr_attribute_t *) * (size_t)part->attributes.num_attributes);
...
exr_result_t exr_attr_declare_by_type(exr_context_t ctxt,
                                      int part_index,
                                      const char *name,
                                      const char *type,
                                      exr_attribute_t **outattr)
{
  ...
  if (pctxt->mode != EXR_CONTEXT_WRITE)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_NOT_OPEN_WRITE));

  rv = exr_attr_list_add_by_type(ctxt, &(part->attributes), name, type, 0, NULL, outattr);
  return EXR_UNLOCK_AND_RETURN_PCTXT(rv);
}

/**************************************/

exr_result_t exr_attr_declare(exr_context_t ctxt,
                              int part_index,
                              const char *name,
                              ...
                              exr_attribute_t **outattr)
{
  ...
  rv = exr_attr_list_add(ctxt, &(part->attributes), name, type, 0, NULL, outattr);
  return EXR_UNLOCK_AND_RETURN_PCTXT(rv);
}

/**************************************/

exr_result_t exr_initialize_required_attr(exr_context_t ctxt,
                                          int part_index,
...
{
  exr_attr_box2i_t dispWindow = {.min = {.x = 0, .y = 0},
                                 .max = {.x = (width - 1), .y = (height - 1)}};
  exr_attr_v2f_t swc = {.x = 0.f, .y = 0.f};
  return exr_initialize_required_attr(ctxt,
                                      part_index,
                                      &dispWindow,
                                      &dispWindow,
                                      1.f,
...
static exr_result_t copy_attr(exr_context_t ctxt,
                              ...
                              int *update_tiles)
{
  ...
  switch (aname[0]) {
    case 'c':
      if (0 == strcmp(aname, EXR_REQ_CHANNELS_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_CHANNELS_STR, type, 0, NULL, &(part->channels));
        attr = part->channels;
      }
      else if (0 == strcmp(aname, EXR_REQ_COMP_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_COMP_STR, type, 0, NULL, &(part->compression));
        attr = part->compression;
        if (rv == EXR_ERR_SUCCESS)
          part->comp_type = (exr_compression_t)srca->uc;
      }
      else if (0 == strcmp(aname, EXR_REQ_CHUNK_COUNT_STR)) {
        rv = exr_attr_list_add_static_name(ctxt,
                                           &(part->attributes),
    ...
    case 'd':
      if (0 == strcmp(aname, EXR_REQ_DATA_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_DATA_STR, type, 0, NULL, &(part->dataWindow));
        attr = part->dataWindow;
        if (rv == EXR_ERR_SUCCESS)
          part->data_window = *(srca->box2i);
        *update_tiles = 1;
      }
      else if (0 == strcmp(aname, EXR_REQ_DISP_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_DISP_STR, type, 0, NULL, &(part->displayWindow));
    ...
    case 'l':
      if (0 == strcmp(aname, EXR_REQ_LO_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_LO_STR, type, 0, NULL, &(part->lineOrder));
        attr = part->lineOrder;
        if (rv == EXR_ERR_SUCCESS)
    ...
    case 'n':
      if (0 == strcmp(aname, EXR_REQ_NAME_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_NAME_STR, type, 0, NULL, &(part->name));
        attr = part->name;
      }
      break;
    case 'p':
      if (0 == strcmp(aname, EXR_REQ_PAR_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_PAR_STR, type, 0, NULL, &(part->pixelAspectRatio));
    ...
    case 't':
      if (0 == strcmp(aname, EXR_REQ_TILES_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_TILES_STR, type, 0, NULL, &(part->tiles));
        attr = part->tiles;
        *update_tiles = 1;
      }
      else if (0 == strcmp(aname, EXR_REQ_TYPE_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_TYPE_STR, type, 0, NULL, &(part->type));
        attr = part->type;
      }
      break;
    case 'v':
      if (0 == strcmp(aname, EXR_REQ_VERSION_STR)) {
        rv = exr_attr_list_add_static_name(
            ctxt, &(part->attributes), EXR_REQ_VERSION_STR, type, 0, NULL, &(part->version));
        attr = part->version;
      }
      break;
    default:
  ...
  if (rv == EXR_ERR_UNKNOWN && !attr) {
    rv = exr_attr_list_add(ctxt, &(part->attributes), aname, type, 0, NULL, &(attr));
  }
  ...
  switch (type) {
    case EXR_ATTR_BOX2I:
      *(attr->box2i) = *(srca->box2i);
      break;
    case EXR_ATTR_BOX2F:
      *(attr->box2f) = *(srca->box2f);
      break;
    ...
    case EXR_ATTR_FLOAT_VECTOR:
      rv = exr_attr_float_vector_create(
          ctxt, attr->floatvector, srca->floatvector->arr, srca->floatvector->length);
      break;
    case EXR_ATTR_INT:
      attr->i = srca->i;
      break;
    case EXR_ATTR_KEYCODE:
      *(attr->keycode) = *(srca->keycode);
      break;
    ...
    case EXR_ATTR_PREVIEW:
      rv = exr_attr_preview_create(
          ctxt, attr->preview, srca->preview->width, srca->preview->height, srca->preview->rgba);
      break;
    case EXR_ATTR_RATIONAL:
      *(attr->rational) = *(srca->rational);
      break;
    ...
    case EXR_ATTR_STRING_VECTOR:
      rv = exr_attr_string_vector_copy(ctxt, attr->stringvector, srca->stringvector);
      break;
    case EXR_ATTR_TILEDESC:
      *(attr->tiledesc) = *(srca->tiledesc);
      break;
    case EXR_ATTR_TIMECODE:
      *(attr->timecode) = *(srca->timecode);
      break;
    case EXR_ATTR_V2I:
      *(attr->v2i) = *(srca->v2i);
      break;
...
exr_result_t exr_copy_unset_attributes(exr_context_t ctxt,
                                       int part_index,
                                       ...
                                       int src_part_index)
{
  ...
  if (src_part_index < 0 || src_part_index >= srcctxt->num_parts) {
    if (srcctxt != pctxt)
      EXR_UNLOCK(srcctxt);
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(pctxt,
  ...
  rv = EXR_ERR_SUCCESS;
  for (int a = 0; rv == EXR_ERR_SUCCESS && a < srcpart->attributes.num_attributes; ++a) {
    const exr_attribute_t *srca = srcpart->attributes.entries[a];
    exr_attribute_t *attr = NULL;

    rv = exr_attr_list_find_by_name(
        ctxt, (exr_attribute_list_t *)&(part->attributes), srca->name, &attr);
...
#define REQ_ATTR_GET_IMPL(name, entry, t) \
  ...
  if (!out) \
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT( \
        pctxt->print_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "NULL output for '%s'", #name)); \
  if (part->name) { \
    if (part->name->type != t) \
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT( \
          pctxt->print_error(pctxt, \
...
#define REQ_ATTR_FIND_CREATE(name, t) \
  ...
  if (!part->name) { \
    rv = exr_attr_list_add(ctxt, &(part->attributes), #name, t, 0, NULL, &(part->name)); \
  } \
...
exr_result_t exr_get_channels(exr_const_context_t ctxt,
                              int part_index,
...
exr_result_t exr_add_channel(exr_context_t ctxt,
                             int part_index,
                             const char *name,
                             ...
                             int32_t ysamp)
{
  ...
  if (rv == EXR_ERR_SUCCESS) {
    rv = exr_attr_chlist_add(ctxt, attr->chlist, name, ptype, islinear, xsamp, ysamp);
  }
  return EXR_UNLOCK_AND_RETURN_PCTXT(rv);
}
...
exr_result_t exr_set_channels(exr_context_t ctxt,
                              int part_index,
                              const exr_attr_chlist_t *channels)
{
  if (!channels)
    return EXR_CTXT(ctxt)->report_error(
        EXR_CTXT(ctxt), EXR_ERR_INVALID_ARGUMENT, "No channels provided for channel list");
...
exr_result_t exr_get_compression(exr_const_context_t ctxt, int part_index, exr_compression_t *out)
{
...
exr_result_t exr_set_data_window(exr_context_t ctxt, int part_index, const exr_attr_box2i_t *dw)
{
  if (!dw)
    return EXR_CTXT(ctxt)->report_error(
        EXR_CTXT(ctxt), EXR_ERR_INVALID_ARGUMENT, "Missing value for data window assignment");
...
exr_result_t exr_get_display_window(exr_const_context_t ctxt,
                                    int part_index,
...
exr_result_t exr_set_display_window(exr_context_t ctxt, int part_index, const exr_attr_box2i_t *dw)
{
  if (!dw)
    return EXR_CTXT(ctxt)->report_error(
        EXR_CTXT(ctxt), EXR_ERR_INVALID_ARGUMENT, "Missing value for data window assignment");
...
exr_result_t exr_get_pixel_aspect_ratio(exr_const_context_t ctxt, int part_index, float *out)
{
...
exr_result_t exr_get_screen_window_center(exr_const_context_t ctxt,
                                          int part_index,
                                          exr_attr_v2f_t *out)
{
...
exr_result_t exr_set_screen_window_center(exr_context_t ctxt,
                                          int part_index,
                                          const exr_attr_v2f_t *swc)
{
  ...
  if (!swc)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Missing value for data window assignment"));
...
exr_result_t exr_get_tile_descriptor(exr_const_context_t ctxt,
                                     int part_index,
                                     uint32_t *xsize,
                                     uint32_t *ysize,
                                     ...
                                     exr_tile_round_mode_t *round)
{
  ...
  if (part->tiles) {
    ...
    if (ysize)
      *ysize = out->y_size;
    if (level)
      *level = EXR_GET_TILE_LEVEL_MODE(*out);
    if (round)
      *round = EXR_GET_TILE_ROUND_MODE(*out);
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(EXR_ERR_SUCCESS);
  }
  return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(EXR_ERR_NO_ATTR_BY_NAME);
}
...
exr_result_t exr_set_tile_descriptor(exr_context_t ctxt,
                                     int part_index,
                                     uint32_t x_size,
                                     uint32_t y_size,
                                     ...
                                     exr_tile_round_mode_t round_mode)
{
  ...
  if (pctxt->mode == EXR_CONTEXT_READ)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_NOT_OPEN_WRITE));
  if (pctxt->mode == EXR_CONTEXT_WRITING_DATA)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_ALREADY_WROTE_ATTRS));
  if (part->storage_mode == EXR_STORAGE_SCANLINE ||
      part->storage_mode == EXR_STORAGE_DEEP_SCANLINE)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_TILE_SCAN_MIXEDAPI, "Attempt to set tile descriptor on scanline part"));

  if (!part->tiles) {
    rv = exr_attr_list_add(
        ctxt, &(part->attributes), "tiles", EXR_ATTR_TILEDESC, 0, NULL, &(part->tiles));
  }
...
exr_result_t exr_get_name(exr_const_context_t ctxt, int part_index, const char **out)
{
  ...
  if (!out)
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(
        pctxt->print_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "NULL output for 'name'"));
...
exr_result_t exr_set_name(exr_context_t ctxt, int part_index, const char *val)
{
  ...
  if (!val || val[0] == '\0')
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid string passed trying to set 'name'"));
  ...
  if (bytes >= (size_t)INT32_MAX)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(pctxt,
                                                          ...
                                                          "String too large to store (%" PRIu64
                                                          " bytes) into 'name'",
                                                          (uint64_t)bytes));

  if (rv == EXR_ERR_SUCCESS) {
    if (attr->string->length == (int32_t)bytes && attr->string->alloc_size > 0) {
      /* we own the string... */
      ...
      rv = exr_attr_string_set_with_length(ctxt, attr->string, val, (int32_t)bytes);
    }
  }
...
#define ATTR_FIND_ATTR(t, entry) \
  ...
  if (!name || name[0] == '\0') \
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->report_error( \
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid name for " #entry " attribute query")); \
  rv = exr_attr_list_find_by_name( \
      ctxt, EXR_CONST_CAST(exr_attribute_list_t *, &(part->attributes)), name, &attr); \
  if (rv != EXR_ERR_SUCCESS) \
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(rv); \
  if (attr->type != t) \
  return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT( \
      pctxt->print_error(pctxt, \
                         ...
                         "'%s' requested type '" #entry "', but stored attributes is type '%s'", \
                         name, \
                         attr->type_name))
...
#define ATTR_FIND_CREATE(t, entry) \
  ...
  if (rv == EXR_ERR_NO_ATTR_BY_NAME) { \
    if (pctxt->mode != EXR_CONTEXT_WRITE) \
      return EXR_UNLOCK_AND_RETURN_PCTXT(rv); \
\
    rv = exr_attr_list_add(ctxt, &(part->attributes), name, t, 0, NULL, &(attr)); \
  } \
...
exr_result_t exr_attr_get_box2i(exr_const_context_t ctxt,
                                int part_index,
                                const char *name,
                                exr_attr_box2i_t *out)
{
...
exr_result_t exr_attr_set_box2i(exr_context_t ctxt,
                                int part_index,
                                const char *name,
...
exr_result_t exr_attr_get_box2f(exr_const_context_t ctxt,
                                int part_index,
                                const char *name,
...
exr_result_t exr_attr_set_box2f(exr_context_t ctxt,
                                int part_index,
                                const char *name,
...
exr_result_t exr_attr_get_channels(exr_const_context_t ctxt,
                                   int part_index,
                                   const char *name,
                                   const exr_attr_chlist_t **out)
{
...
exr_result_t exr_attr_set_channels(exr_context_t ctxt,
                                   int part_index,
                                   const char *name,
                                   const exr_attr_chlist_t *channels)
{
  ...
  if (!channels)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "No input values for setting '%s', type 'chlist'", name));

  rv = exr_attr_list_find_by_name(ctxt, (exr_attribute_list_t *)&(part->attributes), name, &attr);

  if (rv == EXR_ERR_NO_ATTR_BY_NAME) {
    rv = exr_attr_list_add(ctxt, &(part->attributes), name, EXR_ATTR_CHLIST, 0, NULL, &(attr));
  }
  ...
  if (rv == EXR_ERR_SUCCESS) {
    ...
    if (!channels)
      return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "No channels provided for channel list"));
    ...
    for (int c = 0; c < numchans; ++c) {
      ...
      rv = exr_attr_chlist_add_with_length(ctxt,
                                           &clist,
                                           cur->name.str,
                                           cur->name.length,
                                           cur->pixel_type,
...
exr_result_t exr_attr_get_chromaticities(exr_const_context_t ctxt,
                                         int part_index,
                                         const char *name,
...
exr_result_t exr_attr_set_chromaticities(exr_context_t ctxt,
                                         int part_index,
                                         const char *name,
...
exr_result_t exr_attr_get_compression(exr_const_context_t ctxt,
                                      int part_index,
                                      const char *name,
...
exr_result_t exr_attr_set_compression(exr_context_t ctxt,
                                      int part_index,
                                      const char *name,
...
exr_result_t exr_attr_get_double(exr_const_context_t ctxt,
                                 int part_index,
                                 const char *name,
                                 double *out)
{
...
exr_result_t exr_attr_get_envmap(exr_const_context_t ctxt,
                                 int part_index,
                                 const char *name,
                                 exr_envmap_t *out)
{
...
exr_result_t exr_attr_set_envmap(exr_context_t ctxt,
                                 int part_index,
                                 const char *name,
                                 exr_envmap_t eval)
{
  ...
  if (eval >= EXR_ENVMAP_LAST_TYPE)
    return EXR_CTXT(ctxt)->print_error(EXR_CTXT(ctxt),
                                       EXR_ERR_ARGUMENT_OUT_OF_RANGE,
...
exr_result_t exr_attr_get_float(exr_const_context_t ctxt,
                                int part_index,
                                const char *name,
                                float *out)
{
...
exr_result_t exr_attr_get_float_vector(
    exr_const_context_t ctxt, int part_index, const char *name, int32_t *sz, const float **out)
{
  ATTR_FIND_ATTR(EXR_ATTR_FLOAT_VECTOR, floatvector);
  if (sz)
    *sz = attr->floatvector->length;
  if (out)
    *out = attr->floatvector->arr;
  return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(rv);
}

exr_result_t exr_attr_set_float_vector(
    exr_context_t ctxt, int part_index, const char *name, int32_t sz, const float *val)
{
  exr_attribute_t *attr = NULL;
  exr_result_t rv = EXR_ERR_SUCCESS;
  ...
  if (sz < 0 || bytes > (size_t)INT32_MAX)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid size (%d) for float vector '%s'", sz, name));
  ...
  if (rv == EXR_ERR_NO_ATTR_BY_NAME) {
    ...
    rv = exr_attr_list_add(
        ctxt, &(part->attributes), name, EXR_ATTR_FLOAT_VECTOR, 0, NULL, &(attr));
    if (rv == EXR_ERR_SUCCESS)
      rv = exr_attr_float_vector_create(ctxt, attr->floatvector, val, sz);
  }
  else if (rv == EXR_ERR_SUCCESS) {
    if (attr->type != EXR_ATTR_FLOAT_VECTOR)
      return EXR_UNLOCK_AND_RETURN_PCTXT(
          pctxt->print_error(pctxt,
                             ...
                             name,
                             attr->type_name));
    if (attr->floatvector->length == sz && attr->floatvector->alloc_size > 0) {
...
exr_result_t exr_attr_get_int(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              int32_t *out)
{
...
exr_result_t exr_attr_get_keycode(exr_const_context_t ctxt,
                                  int part_index,
                                  const char *name,
...
exr_result_t exr_attr_set_keycode(exr_context_t ctxt,
                                  int part_index,
                                  const char *name,
...
exr_result_t exr_attr_get_lineorder(exr_const_context_t ctxt,
                                    int part_index,
                                    const char *name,
                                    exr_lineorder_t *out)
{
...
exr_result_t exr_attr_set_lineorder(exr_context_t ctxt,
                                    int part_index,
                                    const char *name,
                                    exr_lineorder_t lval)
{
...
exr_result_t exr_attr_get_m33f(exr_const_context_t ctxt,
                               int part_index,
                               const char *name,
                               exr_attr_m33f_t *out)
{
...
exr_result_t exr_attr_set_m33f(exr_context_t ctxt,
                               int part_index,
                               const char *name,
...
exr_result_t exr_attr_get_m33d(exr_const_context_t ctxt,
                               int part_index,
                               const char *name,
                               exr_attr_m33d_t *out)
{
...
exr_result_t exr_attr_set_m33d(exr_context_t ctxt,
                               int part_index,
                               const char *name,
...
exr_result_t exr_attr_get_m44f(exr_const_context_t ctxt,
                               int part_index,
                               const char *name,
                               exr_attr_m44f_t *out)
{
...
exr_result_t exr_attr_set_m44f(exr_context_t ctxt,
                               int part_index,
                               const char *name,
...
exr_result_t exr_attr_get_m44d(exr_const_context_t ctxt,
                               int part_index,
                               const char *name,
                               exr_attr_m44d_t *out)
{
...
exr_result_t exr_attr_set_m44d(exr_context_t ctxt,
                               int part_index,
                               const char *name,
...
exr_result_t exr_attr_get_preview(exr_const_context_t ctxt,
                                  int part_index,
                                  const char *name,
...
exr_result_t exr_attr_set_preview(exr_context_t ctxt,
                                  int part_index,
                                  const char *name,
                                  const exr_attr_preview_t *val)
{
  ...
  if (!val)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "No input value for setting '%s', type 'preview'", name));

  if (rv == EXR_ERR_NO_ATTR_BY_NAME) {
    if (pctxt->mode != EXR_CONTEXT_WRITE)
      return EXR_UNLOCK_AND_RETURN_PCTXT(rv);

    rv = exr_attr_list_add(ctxt, &(part->attributes), name, EXR_ATTR_PREVIEW, 0, NULL, &(attr));
    if (rv == EXR_ERR_SUCCESS)
      rv = exr_attr_preview_create(ctxt, attr->preview, val->width, val->height, val->rgba);
  }
  else if (rv == EXR_ERR_SUCCESS) {
    ...
    if (attr->preview->width == val->width && attr->preview->height == val->height &&
        attr->preview->alloc_size > 0)
    {
      size_t copybytes = val->width * val->height * 4;
      memcpy(EXR_CONST_CAST(void *, attr->preview->rgba), val->rgba, copybytes);
    }
    else if (pctxt->mode != EXR_CONTEXT_WRITE) {
      return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
          pctxt,
...
exr_result_t exr_attr_get_rational(exr_const_context_t ctxt,
                                   int part_index,
                                   const char *name,
...
exr_result_t exr_attr_set_rational(exr_context_t ctxt,
                                   int part_index,
                                   const char *name,
...
exr_result_t exr_attr_get_string(
    exr_const_context_t ctxt, int part_index, const char *name, int32_t *length, const char **out)
{
...
exr_result_t exr_attr_set_string(exr_context_t ctxt,
                                 int part_index,
                                 const char *name,
                                 const char *val)
{
  ...
  if (rv == EXR_ERR_NO_ATTR_BY_NAME) {
    ...
    rv = exr_attr_list_add(ctxt, &(part->attributes), name, EXR_ATTR_STRING, 0, NULL, &(attr));
    if (rv == EXR_ERR_SUCCESS)
      rv = exr_attr_string_create_with_length(ctxt, attr->string, val, (int32_t)bytes);
  }
  else if (rv == EXR_ERR_SUCCESS) {
...
exr_result_t exr_attr_get_string_vector(
    exr_const_context_t ctxt, int part_index, const char *name, int32_t *size, const char **out)
{
  ATTR_FIND_ATTR(EXR_ATTR_STRING_VECTOR, stringvector);
  if (!size)
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "size parameter required to query stringvector"));
  if (out) {
...
exr_result_t exr_attr_set_string_vector(
    exr_context_t ctxt, int part_index, const char *name, int32_t size, const char **val)
{
  exr_attribute_t *attr = NULL;
  exr_result_t rv = EXR_ERR_SUCCESS;
  ...
  if (size < 0)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid size (%d) for string vector '%s'", size, name));
  ...
  if (rv == EXR_ERR_NO_ATTR_BY_NAME) {
    ...
    rv = exr_attr_list_add(
        ctxt, &(part->attributes), name, EXR_ATTR_STRING_VECTOR, 0, NULL, &(attr));
    if (rv == EXR_ERR_SUCCESS)
      rv = exr_attr_string_vector_init(ctxt, attr->stringvector, size);
    for (int32_t i = 0; rv == EXR_ERR_SUCCESS && i < size; ++i)
      rv = exr_attr_string_vector_set_entry(ctxt, attr->stringvector, i, val[i]);
  }
  else if (rv == EXR_ERR_SUCCESS) {
    if (attr->type != EXR_ATTR_STRING_VECTOR)
      return EXR_UNLOCK_AND_RETURN_PCTXT(
          pctxt->print_error(pctxt,
                             ...
                             name,
                             attr->type_name));
    if (attr->stringvector->n_strings == size && attr->stringvector->alloc_size > 0) {
      if (pctxt->mode != EXR_CONTEXT_WRITE) {
        for (int32_t i = 0; rv == EXR_ERR_SUCCESS && i < size; ++i) {
          size_t curlen;
          if (!val[i])
          ...
          curlen = strlen(val[i]);
          if (curlen != (size_t)attr->stringvector->strings[i].length)
            return EXR_UNLOCK_AND_RETURN_PCTXT(
                pctxt->print_error(pctxt,
                                   EXR_ERR_INVALID_ARGUMENT,
                                   "'%s' string %d in string vector is different size (old %d new "
                                   "%d), unable to update",
                                   name,
                                   i,
...
exr_result_t exr_attr_get_tiledesc(exr_const_context_t ctxt,
                                   int part_index,
                                   const char *name,
...
exr_result_t exr_attr_set_tiledesc(exr_context_t ctxt,
                                   int part_index,
                                   const char *name,
                                   const exr_attr_tiledesc_t *val)
{
  if (name && 0 == strcmp(name, EXR_REQ_TILES_STR)) {
    ...
    return exr_set_tile_descriptor(ctxt,
                                   part_index,
                                   val->x_size,
                                   val->y_size,
...
exr_result_t exr_attr_get_timecode(exr_const_context_t ctxt,
                                   int part_index,
                                   const char *name,
...
exr_result_t exr_attr_set_timecode(exr_context_t ctxt,
                                   int part_index,
                                   const char *name,
...
exr_result_t exr_attr_get_v2i(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              exr_attr_v2i_t *out)
{
...
exr_result_t exr_attr_set_v2i(exr_context_t ctxt,
                              int part_index,
                              const char *name,
...
exr_result_t exr_attr_get_v2f(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              exr_attr_v2f_t *out)
{
...
exr_result_t exr_attr_set_v2f(exr_context_t ctxt,
                              int part_index,
                              const char *name,
...
exr_result_t exr_attr_get_v2d(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              exr_attr_v2d_t *out)
{
...
exr_result_t exr_attr_set_v2d(exr_context_t ctxt,
                              int part_index,
                              const char *name,
...
exr_result_t exr_attr_get_v3i(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              exr_attr_v3i_t *out)
{
...
exr_result_t exr_attr_set_v3i(exr_context_t ctxt,
                              int part_index,
                              const char *name,
...
exr_result_t exr_attr_get_v3f(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              exr_attr_v3f_t *out)
{
...
exr_result_t exr_attr_set_v3f(exr_context_t ctxt,
                              int part_index,
                              const char *name,
...
exr_result_t exr_attr_get_v3d(exr_const_context_t ctxt,
                              int part_index,
                              const char *name,
                              exr_attr_v3d_t *out)
{
...
exr_result_t exr_attr_set_v3d(exr_context_t ctxt,
                              int part_index,
                              const char *name,
...
exr_result_t exr_attr_get_user(exr_const_context_t ctxt,
                               int part_index,
                               const char *name,
                               const char **type,
                               int32_t *size,
                               const void **out)
{
  ...
  if (rv == EXR_ERR_SUCCESS) {
    ...
    if (attr->opaque->pack_func_ptr) {
      if (size)
        *size = attr->opaque->unpacked_size;
      if (out)
        *out = attr->opaque->unpacked_data;
    }
    else {
      if (size)
        *size = attr->opaque->packed_alloc_size;
      if (out)
        *out = attr->opaque->packed_data;
    }
  }
...
exr_result_t exr_attr_set_user(exr_context_t ctxt,
                               int part_index,
                               const char *name,
                               const char *type,
                               int32_t size,
                               const void *out)
{
  ...
  if (rv == EXR_ERR_NO_ATTR_BY_NAME) {
    if (pctxt->mode != EXR_CONTEXT_WRITE)
      return EXR_UNLOCK_AND_RETURN_PCTXT(rv);
    rv = exr_attr_list_add_by_type(ctxt, &(part->attributes), name, type, 0, NULL, &(attr));
  }
  else if (rv == EXR_ERR_SUCCESS) {
