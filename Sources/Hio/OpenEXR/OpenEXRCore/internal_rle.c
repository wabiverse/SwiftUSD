...
uint64_t internal_rle_compress(void *out, uint64_t outbytes, const void *src, uint64_t srcbytes)
{
  int8_t *cbuf = out;
...
static void reorder_and_predict(void *scratch, const void *packed, uint64_t packedbytes)
{
  int8_t *t1 = scratch;
...
exr_result_t internal_exr_apply_rle(exr_encode_pipeline_t *encode)
{
  ...
  outb = internal_rle_compress(
      encode->compressed_buffer, encode->compressed_alloc_size, encode->scratch_buffer_1, srcb);
...
uint64_t internal_rle_decompress(uint8_t *out, uint64_t outsz, const uint8_t *src, uint64_t packsz)
{
  ...
  while (unpackbytes < packsz) {
    if (*in < 0) {
      ...
      ++unpackbytes;
      if (unpackbytes + count > packsz)
        return EXR_ERR_CORRUPT_CHUNK;
      if (outbytes + count > outsz)
        return EXR_ERR_CORRUPT_CHUNK;
...
static void unpredict_and_reorder(void *out, void *scratch, uint64_t packedbytes)
{
  int8_t *t1 = scratch;
  int8_t *t2 = t1 + (packedbytes + 1) / 2;
  int8_t *s = out;
...
exr_result_t internal_exr_undo_rle(
    exr_decode_pipeline_t *decode, const void *src, uint64_t packsz, void *out, uint64_t outsz)
{
  exr_result_t rv;
  uint64_t unpackb;
  ...
  unpackb = internal_rle_decompress(decode->scratch_buffer_1, outsz, src, packsz);
  if (unpackb != outsz)
    return EXR_ERR_CORRUPT_CHUNK;
