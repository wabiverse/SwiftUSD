...
static inline uint32_t float_to_float24(float f)
{
  union {
    float f;
    uint32_t i;
...
static exr_result_t apply_pxr24_impl(exr_encode_pipeline_t *encode)
{
  ...
  for (int y = 0; y < encode->chunk.height; ++y) {
    ...
    for (int c = 0; c < encode->channel_count; ++c) {
      ...
      if (curc->height == 0 || (curc->y_samples > 1 && (cury % curc->y_samples) != 0))
        continue;
...
static exr_result_t undo_pxr24_impl(exr_decode_pipeline_t *decode,
                                    ...
                                    uint64_t scratch_size)
{
  ...
  rstat = exr_uncompress_buffer(
      decode->context, compressed_data, comp_buf_size, scratch_data, uncompressed_size, &outSize);
  ...
  for (int y = 0; y < decode->chunk.height; ++y) {
    ...
    for (int c = 0; c < decode->channel_count; ++c) {
      ...
      int w = curc->width;
      uint64_t nBytes = (uint64_t)(w) * (uint64_t)(curc->bytes_per_element);
      ...
      switch (curc->data_type) {
        case EXR_PIXEL_UINT: {
          ...
          for (int x = 0; x < w; ++x) {
            uint32_t diff = (((uint32_t)(*(ptr[0]++)) << 24) | ((uint32_t)(*(ptr[1]++)) << 16) |
        ...
        case EXR_PIXEL_HALF: {
          ...
          for (int x = 0; x < w; ++x) {
            uint32_t diff = (((uint32_t)(*(ptr[0]++)) << 8) | ((uint32_t)(*(ptr[1]++))));
            pixel += diff;
...
exr_result_t internal_exr_undo_pxr24(exr_decode_pipeline_t *decode,
                                     const void *compressed_data,
                                     uint64_t comp_buf_size,
                                     void *uncompressed_data,
                                     uint64_t uncompressed_size)
