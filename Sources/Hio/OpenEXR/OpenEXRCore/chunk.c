...
/* msvc w/ c11 support is only very new, until we know what the preprocessor checks are, provide
 * defaults */
#  include <windows.h>

#  define atomic_load(object) InterlockedOr64((int64_t volatile *)object, 0)

static inline int atomic_compare_exchange_strong(uint64_t volatile *object,
                                                 uint64_t *expected,
                                                 uint64_t desired)
{
  uint64_t prev = (uint64_t)InterlockedCompareExchange64(object, desired, *expected);
  if (prev == *expected)
    return 1;
  *expected = prev;
  return 0;
...
static exr_result_t extract_chunk_table(const struct _internal_exr_context *ctxt,
...
static exr_result_t validate_and_compute_tile_chunk_off(const struct _internal_exr_context *ctxt,
                                                        const struct _internal_exr_part *part,
                                                        int tilex,
                                                        int tiley,
                                                        int levelx,
                                                        ...
                                                        int32_t *chunkoffout)
{
  ...
  if (!part->tiles || part->num_tile_levels_x <= 0 || part->num_tile_levels_y <= 0 ||
      !part->tile_level_tile_count_x || !part->tile_level_tile_count_y)
  {
    return ctxt->print_error(
        ctxt, EXR_ERR_MISSING_REQ_ATTR, "Tile descriptor data missing or corrupt");
  }
  ...
  switch (EXR_GET_TILE_LEVEL_MODE((*tiledesc))) {
    ...
    case EXR_TILE_MIPMAP_LEVELS:
      if (levelx != levely) {
        return ctxt->print_error(ctxt,
                                 EXR_ERR_INVALID_ARGUMENT,
                                 "Request for tile (%d, %d) level (%d, %d), but single level and "
                                 "mipmap tiles must have same level x and y",
                                 tilex,
                                 tiley,
                                 levelx,
                                 levely);
    ...
    case EXR_TILE_RIPMAP_LEVELS:
      ...
      for (int ly = 0; ly < levely; ++ly) {
        for (int lx = 0; lx < levelx; ++lx) {
          chunkoff += ((int64_t)part->tile_level_tile_count_x[lx] *
                       (int64_t)part->tile_level_tile_count_y[ly]);
        }
      }
      for (int lx = 0; lx < levelx; ++lx) {
        chunkoff += ((int64_t)part->tile_level_tile_count_x[lx] * (int64_t)numy);
      }
      chunkoff += tiley * numx + tilex;
      break;
    case EXR_TILE_LAST_TYPE:
    default:
      return ctxt->print_error(ctxt, EXR_ERR_UNKNOWN, "Invalid tile description");
  }
...
static exr_result_t extract_chunk_leader(const struct _internal_exr_context *ctxt,
                                         const struct _internal_exr_part *part,
                                         int partnum,
                                         uint64_t offset,
                                         ...
                                         struct priv_chunk_leader *leaderdata)
{
  ...
  rv = ctxt->do_read(
      ctxt, data, (size_t)ntoread * sizeof(int32_t), &nextoffset, NULL, EXR_MUST_READ_ALL);
  ...
  {
    rv = ctxt->do_read(
        ctxt, leaderdata->deep_data, 3 * sizeof(int64_t), &nextoffset, NULL, EXR_MUST_READ_ALL);
    if (rv != EXR_ERR_SUCCESS)
      return rv;
...
static exr_result_t extract_chunk_size(const struct _internal_exr_context *ctxt,
                                       const struct _internal_exr_part *part,
                                       int partnum,
                                       uint64_t offset,
                                       uint64_t *next_offset)
{
  ...
  return extract_chunk_leader(ctxt, part, partnum, offset, next_offset, &leader);
}

/**************************************/

static exr_result_t read_and_validate_chunk_leader(const struct _internal_exr_context *ctxt,
                                                   const struct _internal_exr_part *part,
                                                   int partnum,
                                                   uint64_t offset,
                                                   int *indexio,
                                                   uint64_t *next_offset)
{
  ...
  rv = extract_chunk_leader(ctxt, part, partnum, offset, next_offset, &leader);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
      ...
      part->storage_mode == EXR_STORAGE_DEEP_SCANLINE)
  {
    ...
    *indexio = (int)chunk;
    ...
    rv = validate_and_compute_tile_chunk_off(
        ctxt, part, leader.tile_x, leader.tile_y, leader.level_x, leader.level_y, &cidx);
...
static exr_result_t reconstruct_chunk_table(const struct _internal_exr_context *ctxt,
                                            const struct _internal_exr_part *part,
                                            uint64_t *chunktable)
{
  exr_result_t rv = EXR_ERR_SUCCESS;
  ...
  if (ctxt->file_size > 0)
    max_offset = (uint64_t)ctxt->file_size;
  ...
  if (partnum > 0) {
    ...
    rv = extract_chunk_size(ctxt, curpart, partnum - 1, chunk_start, &offset_start);
    if (rv != EXR_ERR_SUCCESS)
      return rv;
  }
                                        ...
                                        uint64_t *chunkminoffset)
{
  ...
  *chunkminoffset = chunkoff + chunkbytes;
  ...
  ctable = (uint64_t *)atomic_load(EXR_CONST_CAST(atomic_uintptr_t *, &(part->chunk_table)));
  if (ctable == NULL) {
    int64_t nread = 0;
    ...
    if (part->chunk_count <= 0)
      return ctxt->report_error(ctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid file with no chunks");

    if (ctxt->file_size > 0 && chunkbytes + chunkoff > (uint64_t)ctxt->file_size)
      return ctxt->print_error(ctxt,
                               EXR_ERR_INVALID_ARGUMENT,
    ...
    rv = ctxt->do_read(ctxt, ctable, chunkbytes, &chunkoff, &nread, EXR_MUST_READ_ALL);
    if (rv != EXR_ERR_SUCCESS) {
      ctxt->free_fn(ctable);
      return rv;
    }
    ...
    if (!ctxt->disable_chunk_reconstruct) {
      ...
      if (ctxt->file_size > 0)
        maxoff = (uint64_t)ctxt->file_size;
      for (int ci = 0; ci < part->chunk_count; ++ci) {
        uint64_t cchunk = one_to_native64(ctable[ci]);
        if (cchunk < chunkoff || cchunk >= maxoff)
          complete = 0;
        ctable[ci] = cchunk;
      }
    ...
    if (!atomic_compare_exchange_strong(
            EXR_CONST_CAST(atomic_uintptr_t *, &(part->chunk_table)), &eptr, nptr))
    {
      ctxt->free_fn(ctable);
...
static exr_result_t alloc_chunk_table(const struct _internal_exr_context *ctxt,
                                      const struct _internal_exr_part *part,
                                      uint64_t **chunktable)
{
  uint64_t *ctable = NULL;
...
static uint64_t compute_chunk_unpack_size(
    int y, int width, int height, int lpc, const struct _internal_exr_part *part)
{
  ...
  if (part->chan_has_line_sampling || height != lpc) {
    ...
    for (int c = 0; c < chanlist->num_channels; ++c) {
      ...
      if (curc->x_sampling > 1)
        chansz /= ((uint64_t)curc->x_sampling);
      chansz *= (uint64_t)compute_sampled_lines(height, curc->y_sampling, y);
      unpacksize += chansz;
    }
  }
  else
                                          ...
                                          exr_chunk_info_t *cinfo)
{
  ...
  if (cidx < 0 || cidx >= part->chunk_count) {
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
  ...
  rv = pctxt->do_read(
      pctxt, data, (size_t)(rdcnt) * sizeof(int32_t), &dataoff, NULL, EXR_MUST_READ_ALL);
  ...
  if (miny != data[rdcnt]) {
    return pctxt->print_error(pctxt,
                              EXR_ERR_BAD_CHUNK_LEADER,
                              "Preparing to read scanline %d (chunk %d), found corrupt leader: "
                              "scanline says %d, expected %d",
                              y,
                              cidx,
                              data[rdcnt],
                              miny);
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_SCANLINE) {
    rv = pctxt->do_read(pctxt, ddata, 3 * sizeof(int64_t), &dataoff, NULL, EXR_MUST_READ_ALL);
    if (rv != EXR_ERR_SUCCESS) {
      return rv;
    }
    ...
    if (ddata[0] < 0) {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Preparing to read scanline %d (chunk %d), found corrupt leader: "
                                "invalid sample table size %" PRId64,
                                y,
                                cidx,
                                ddata[0]);
    }
    if (ddata[1] < 0 || ddata[1] > (int64_t)INT_MAX) {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Preparing to read scanline %d (chunk %d), found corrupt leader: "
                                "invalid packed data size %" PRId64,
                                y,
                                cidx,
                                ddata[1]);
    }
    if (ddata[2] < 0 || ddata[2] > (int64_t)INT_MAX) {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Preparing to scanline %d (chunk %d), found corrupt leader: "
                                "unsupported unpacked data size %" PRId64,
                                y,
                                cidx,
                                ddata[2]);
    }
    ...
    if (fsize > 0 &&
        ((cinfo->sample_count_data_offset + cinfo->sample_count_table_size) > ((uint64_t)fsize) ||
         (cinfo->data_offset + cinfo->packed_size) > ((uint64_t)fsize)))
    {
      return pctxt->print_error(
          pctxt,
          EXR_ERR_BAD_CHUNK_LEADER,
          "Preparing to scanline %d (chunk %d), found corrupt leader: sample table and data "
          "result in access past end of the file: sample table size %" PRId64
    ...
    uint64_t unpacksize = compute_chunk_unpack_size(y, cinfo->width, cinfo->height, lpc, part);
    ...
    if (fsize > 0 && (cinfo->data_offset + cinfo->packed_size) > ((uint64_t)fsize)) {
      return pctxt->print_error(
          pctxt,
          EXR_ERR_BAD_CHUNK_LEADER,
  ...
  if (cinfo->packed_size == 0 && cinfo->unpacked_size > 0)
    return pctxt->report_error(pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid packed size of 0");
  return EXR_ERR_SUCCESS;
}
                                      ...
                                      exr_chunk_info_t *cinfo)
{
  ...
  cidx = 0;
  rv = validate_and_compute_tile_chunk_off(pctxt, part, tilex, tiley, levelx, levely, &cidx);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  if (dataoff < chunkmin || (fsize > 0 && dataoff > (uint64_t)fsize)) {
    return pctxt->print_error(pctxt,
                              EXR_ERR_BAD_CHUNK_LEADER,
                              "Corrupt chunk offset table: tile (%d, %d), level (%d, %d), chunk "
                              "index %d recorded at file offset %" PRIu64,
                              tilex,
                              tiley,
                              levelx,
                              levely,
  ...
  rv = pctxt->do_read(
      pctxt, data, (uint64_t)(ntoread) * sizeof(int32_t), &dataoff, &nread, EXR_MUST_READ_ALL);
  if (rv != EXR_ERR_SUCCESS) {
    return pctxt->print_error(
        pctxt,
        rv,
  ...
  if (tdata[2] != levelx) {
    return pctxt->print_error(pctxt,
                              EXR_ERR_BAD_CHUNK_LEADER,
                              "Corrupt tile (%d, %d), level (%d, %d) (chunk %d): bad tile mip/rip "
                              "level X (%d, expect %d)",
                              tilex,
                              tiley,
                              levelx,
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_TILED) {
    ...
    if (ddata[0] < 0 || (ddata[0] == 0 && (ddata[1] != 0 || ddata[2] != 0))) {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Corrupt deep tile (%d, %d), level (%d, %d) (chunk %d): invalid "
                                "sample table size %" PRId64,
                                tilex,
                                tiley,
                                levelx,
                                levely,
    ...
    if (ddata[1] < 0 || ddata[1] > (int64_t)INT32_MAX || (ddata[1] == 0 && ddata[2] != 0)) {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Corrupt deep tile (%d, %d), level (%d, %d) (chunk %d): invalid "
                                "packed data size %" PRId64,
                                tilex,
                                tiley,
                                levelx,
                                levely,
    ...
    if (ddata[2] < 0 || ddata[2] > (int64_t)INT32_MAX || (ddata[2] == 0 && ddata[1] != 0)) {
      return pctxt->print_error(
          pctxt,
          EXR_ERR_BAD_CHUNK_LEADER,
         ...
         (cinfo->data_offset + cinfo->packed_size) > ((uint64_t)fsize)))
    {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Corrupt deep tile (%d, %d), level (%d, %d) (chunk %d): access "
                                "past end of the file: sample table size %" PRId64
                                " + data size %" PRId64 " larger than file %" PRId64,
                                tilex,
                                tiley,
                                levelx,
    ...
    if (tdata[4] < 0 || ((uint64_t)tdata[4]) > unpacksize || (tdata[4] == 0 && unpacksize != 0)) {
      return pctxt->print_error(pctxt,
                                EXR_ERR_BAD_CHUNK_LEADER,
                                "Corrupt tile (%d, %d), level (%d, %d) (chunk %d): invalid packed "
                                "size %d vs unpacked size %" PRIu64,
                                tilex,
                                tiley,
                                levelx,
                                levely,
      ...
      if (finpos > (uint64_t)fsize) {
        return pctxt->print_error(pctxt,
                                  EXR_ERR_BAD_CHUNK_LEADER,
                                  "Corrupt tile (%d, %d), level (%d, %d) (chunk %d): access past "
                                  "end of file: packed size (%d) at offset %" PRIu64
                            ...
                            void *packed_data)
{
  ...
  if (cinfo->type != (uint8_t)part->storage_mode)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "mismatched storage type for chunk block info");
  if (cinfo->compression != (uint8_t)part->comp_type)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "mismatched compression type for chunk block info");
  ...
  if (part->comp_type == EXR_COMPRESSION_NONE)
    rmode = EXR_ALLOW_SHORT_READ;

  toread = cinfo->packed_size;
  if (toread > 0) {
    nread = 0;
    rv = pctxt->do_read(pctxt, packed_data, toread, &dataoffset, &nread, rmode);

    if (rmode == EXR_ALLOW_SHORT_READ && nread < (int64_t)toread)
      memset(((uint8_t *)packed_data) + nread, 0, toread - (uint64_t)(nread));
  }
  else
    rv = EXR_ERR_SUCCESS;
                                 ...
                                 void *sample_data)
{
  ...
  if (pctxt->file_size > 0 && cinfo->sample_count_data_offset > (uint64_t)pctxt->file_size)
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
  ...
  if (pctxt->file_size > 0 && cinfo->data_offset > (uint64_t)pctxt->file_size)
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
                              "chunk block info data offset (%" PRIu64
                              ") past end of file (%" PRId64 ")",
  ...
  if (sample_data && cinfo->sample_count_table_size > 0) {
    ...
    nread = 0;
    rv = pctxt->do_read(pctxt, sample_data, toread, &dataoffset, &nread, rmode);
  }
...
static exr_result_t write_scan_chunk(struct _internal_exr_context *pctxt,
                                     int part_index,
                                     ...
                                     uint64_t sample_data_size)
{
  ...
  if (part->storage_mode != EXR_STORAGE_DEEP_SCANLINE && packed_size > (uint64_t)INT32_MAX)
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_SCANLINE && (!sample_data || sample_data_size == 0))
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
  ...
  if (part->lineorder != EXR_LINEORDER_RANDOM_Y && pctxt->last_output_chunk != (cidx - 1)) {
  ...
  rv = pctxt->do_write(
      pctxt, data, (uint64_t)(wrcnt) * sizeof(int32_t), &(pctxt->output_file_offset));
  if (rv == EXR_ERR_SUCCESS && part->storage_mode == EXR_STORAGE_DEEP_SCANLINE) {
    int64_t ddata[3];
    ddata[0] = (int64_t)sample_data_size;
    ddata[1] = (int64_t)packed_size;
    ddata[2] = (int64_t)unpacked_size;
    rv = pctxt->do_write(pctxt, ddata, 3 * sizeof(uint64_t), &(pctxt->output_file_offset));

    if (rv == EXR_ERR_SUCCESS)
      rv = pctxt->do_write(pctxt, sample_data, sample_data_size, &(pctxt->output_file_offset));
  }
  if (rv == EXR_ERR_SUCCESS && packed_size > 0)
    rv = pctxt->do_write(pctxt, packed_data, packed_size, &(pctxt->output_file_offset));
  ...
  if (rv == EXR_ERR_SUCCESS) {
    ...
    if (pctxt->output_chunk_count == part->chunk_count) {
      ...
      rv = pctxt->do_write(
          pctxt, ctable, sizeof(uint64_t) * (uint64_t)(part->chunk_count), &chunkoff);
                                           ...
                                           exr_chunk_info_t *cinfo)
{
  ...
  if (!cinfo)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_INVALID_ARGUMENT));

  if (part->storage_mode == EXR_STORAGE_TILED || part->storage_mode == EXR_STORAGE_DEEP_TILED) {
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_SCAN_TILE_MIXEDAPI));
  }

  if (pctxt->mode != EXR_CONTEXT_WRITING_DATA) {
    if (pctxt->mode == EXR_CONTEXT_WRITE)
      return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_HEADER_NOT_WRITTEN));
  ...
  *cinfo = nil;
  ...
  cinfo->data_offset = 0;
  cinfo->packed_size = 0;
  cinfo->unpacked_size = compute_chunk_unpack_size(y, cinfo->width, cinfo->height, lpc, part);
                                       ...
                                       exr_chunk_info_t *cinfo)
{
  ...
  if (part->storage_mode == EXR_STORAGE_SCANLINE ||
      part->storage_mode == EXR_STORAGE_DEEP_SCANLINE)
  {
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_TILE_SCAN_MIXEDAPI));
  }
  ...
  if (((int64_t)(tilex) * (int64_t)(tilew) + (int64_t)(tilew) +
       (int64_t)(part->data_window.min.x) - 1) > (int64_t)(part->data_window.max.x))
  {
    int64_t sz = (int64_t)(part->data_window.max.x) - (int64_t)(part->data_window.min.x) + 1;
    tilew = (int)(sz - ((int64_t)(tilex) * (int64_t)(tilew)));
  }

  if (((int64_t)(tiley) * (int64_t)(tileh) + (int64_t)(tileh) +
       (int64_t)(part->data_window.min.y) - 1) > (int64_t)(part->data_window.max.y))
  {
    int64_t sz = (int64_t)(part->data_window.max.y) - (int64_t)(part->data_window.min.y) + 1;
    ...
    exr_context_t ctxt, int part_index, int y, const void *packed_data, uint64_t packed_size)
{
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_SCANLINE)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_USE_SCAN_DEEP_WRITE));

  rv = write_scan_chunk(pctxt, part_index, part, y, packed_data, packed_size, 0, NULL, 0);
  return EXR_UNLOCK_AND_RETURN_PCTXT(rv);
}
...
/* pull most of the logic to here to avoid having to unlock at every
 * error exit point and re-use mostly shared logic */
static exr_result_t write_tile_chunk(struct _internal_exr_context *pctxt,
                                     int part_index,
                                     struct _internal_exr_part *part,
                                     int tilex,
                                     int tiley,
                                     ...
                                     uint64_t sample_data_size)
{
  ...
  if (part->storage_mode != EXR_STORAGE_DEEP_TILED && packed_size > (uint64_t)INT32_MAX)
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
                              "Packed data size %" PRIu64 " too large (max %" PRIu64 ")",
                              (uint64_t)packed_size,
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_TILED && (!sample_data || sample_data_size == 0))
    return pctxt->print_error(pctxt,
                              EXR_ERR_INVALID_ARGUMENT,
                              "Invalid sample count data argument size %" PRIu64 " pointer %p",
                              (uint64_t)sample_data_size,
                                  ...
                                  uint64_t packed_size)
{
  ...
  if (part->storage_mode == EXR_STORAGE_DEEP_TILED)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_USE_TILE_DEEP_WRITE));

  rv = write_tile_chunk(
      pctxt, part_index, part, tilex, tiley, levelx, levely, packed_data, packed_size, 0, NULL, 0);
  return EXR_UNLOCK_AND_RETURN_PCTXT(rv);
}
...
exr_result_t internal_validate_next_chunk(exr_encode_pipeline_t *encode,
                                          ...
                                          const struct _internal_exr_part *part)
{
  ...
  if (part->storage_mode == EXR_STORAGE_TILED || part->storage_mode == EXR_STORAGE_DEEP_TILED) {
    rv = validate_and_compute_tile_chunk_off(pctxt,
                                             part,
  ...
  if (rv == EXR_ERR_SUCCESS) {
    ...
    else if (part->lineorder != EXR_LINEORDER_RANDOM_Y && pctxt->last_output_chunk != (cidx - 1)) {
      rv = pctxt->print_error(pctxt,
                              EXR_ERR_INCORRECT_CHUNK,
