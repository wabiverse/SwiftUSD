...
static void DctCoderChannelData_construct(DctCoderChannelData *d, exr_pixel_type_t t)
{
  memset(d, 0, sizeof(DctCoderChannelData));
  d->_type = t;
}

static void DctCoderChannelData_destroy(void (*free_fn)(void *), DctCoderChannelData *d)
{
  if (d->_rows)
    free_fn(d->_rows);
}

static exr_result_t DctCoderChannelData_push_row(void *(*alloc_fn)(size_t),
                                                 void (*free_fn)(void *),
...
static inline CscPrefixMapItem *CscPrefixMap_find(CscPrefixMapItem *mapl,
                                                  int maxSize,
                                                  const char *cname,
                                                  size_t prefixlen)
{
  ...
  for (; idx < maxSize; ++idx) {
    ...
    if (prefixlen == mapl[idx].prefix_len && !strncmp(cname, mapl[idx].name, prefixlen)) {
      break;
    }
  }
