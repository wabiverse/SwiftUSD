...
/** @brief Struct to hold color chromaticities to interpret the tristimulus color values in the
 * image data. */
typedef struct {
  float red_x;
  float red_y;
...
typedef struct {
  ...
  exr_result_t (*unpack_func_ptr)(
      exr_context_t ctxt, const void *data, int32_t attrsize, int32_t *outsize, void **outbuffer);
...
typedef enum {
  ...
  EXR_ATTR_CHROMATICITIES, /**< Values to specify color space of colors in file @see
                              exr_attr_chromaticities_t. */
  ...
  EXR_ATTR_STRING_VECTOR,  /**< 0 or more text strings (int + string). number is based on attribute
                              size. */
