...
exr_result_t exr_attr_chlist_init(exr_context_t ctxt, exr_attr_chlist_t *clist, int nchans)
{
  ...
  if (!clist)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid channel list pointer to chlist_add_with_length");

  if (nchans < 0)
    return pctxt->print_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Negative number of channels requested (%d)", nchans);

  *clist = nil;

  if (nchans > 0) {
    nlist = (exr_attr_chlist_entry_t *)pctxt->alloc_fn(sizeof(*nlist) * (size_t)nchans);
    if (nlist == NULL)
...
exr_result_t exr_attr_chlist_add(exr_context_t ctxt,
                                 ...
                                 int32_t ysamp)
{
  ...
  return exr_attr_chlist_add_with_length(ctxt, clist, name, len, ptype, islinear, xsamp, ysamp);
}

/**************************************/

exr_result_t exr_attr_chlist_add_with_length(exr_context_t ctxt,
                                             exr_attr_chlist_t *clist,
                                             const char *name,
                                             int32_t namelen,
                                             exr_pixel_type_t ptype,
                                             ...
                                             int32_t ysamp)
{
  ...
  if (newcount > clist->num_alloced) {
    ...
    if (newcount > nsz)
      nsz = newcount + 1;
    nlist = (exr_attr_chlist_entry_t *)pctxt->alloc_fn(sizeof(*nlist) * (size_t)nsz);
    if (nlist == NULL) {
...
exr_result_t exr_attr_chlist_duplicate(exr_context_t ctxt,
...
exr_result_t exr_attr_chlist_destroy(exr_context_t ctxt, exr_attr_chlist_t *clist)
{
  ...
  if (clist) {
    ...
    exr_attr_chlist_entry_t *entries = EXR_CONST_CAST(exr_attr_chlist_entry_t *, clist->entries);

    for (int i = 0; i < nc; ++i)
      exr_attr_string_destroy(ctxt, &(entries[i].name));
    if (entries)
      pctxt->free_fn(entries);
    *clist = nil;
  }
  return EXR_ERR_SUCCESS;
}
