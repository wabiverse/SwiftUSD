...
static void bitmapFromData(const uint16_t *data,
                           uint64_t nData,
                           uint8_t *bitmap,
                           uint16_t *minNonZero,
                           uint16_t *maxNonZero)
...
static inline uint16_t forwardLutFromBitmap(const uint8_t *bitmap, uint16_t *lut)
{
  uint16_t k = 0;
...
static inline uint16_t reverseLutFromBitmap(const uint8_t *bitmap, uint16_t *lut)
{
  uint32_t n, k = 0;
...
static inline void applyLut(const uint16_t *lut, uint16_t *data, uint64_t nData)
{
  for (uint64_t i = 0; i < nData; ++i)
    data[i] = lut[data[i]];
}
...
static inline void wenc14(uint16_t a, uint16_t b, uint16_t *l, uint16_t *h)
{
...
static inline void wenc16(uint16_t a, uint16_t b, uint16_t *l, uint16_t *h)
{
...
static void wav_2D_encode(uint16_t *in, int nx, int ox, int ny, int oy, uint16_t mx)
{
  int w14 = (mx < (1 << 14)) ? 1 : 0;
  int n = (nx > ny) ? ny : nx;
  int p = 1;   // == 1 <<  level
...
static void wav_2D_decode(uint16_t *in,  // io: values are transformed in place
                          int nx,        // i : x size
                          int ox,        // i : x offset
                          int ny,        // i : y size
                          int oy,        // i : y offset
...
exr_result_t internal_exr_apply_piz(exr_encode_pipeline_t *encode)
{
  ...
  rv = internal_encode_alloc_buffer(encode,
                                    ...
                                    BITMAP_SIZE * sizeof(uint8_t) +
                                        USHORT_RANGE * sizeof(uint16_t) + hufSpareBytes);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  bitmapFromData(encode->scratch_buffer_1, ndata, bitmap, &minNonZero, &maxNonZero);
  ...
  rv = internal_huf_compress(&nBytes,
                             out,
...
exr_result_t internal_exr_undo_piz(
    exr_decode_pipeline_t *decode, const void *src, uint64_t packsz, void *outptr, uint64_t outsz)
{
  uint8_t *out = outptr;
  uint64_t nOut = 0;
  uint8_t *scratch, *tmp;
  ...
  // Huffman decoding
  //
  if (nBytes + sizeof(uint32_t) > packsz)
    return EXR_ERR_CORRUPT_CHUNK;

  hufbytes = unaligned_load32(packed + nBytes);
  nBytes += sizeof(uint32_t);

  if (nBytes + hufbytes > packsz)
    return EXR_ERR_CORRUPT_CHUNK;

  wavbuf = decode->scratch_buffer_1;
  rv = internal_huf_decompress(
      decode, packed + nBytes, hufbytes, wavbuf, outsz / 2, hufspare, hufSpareBytes);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  for (int y = 0; y < decode->chunk.height; ++y) {
    ...
    for (int c = 0; c < decode->channel_count; ++c) {
      ...
      nx = curc->width;
      ny = curc->height;
      nBytes = ((uint64_t)curc->width) * ((uint64_t)curc->bytes_per_element);
