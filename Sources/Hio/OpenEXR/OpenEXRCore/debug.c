...
static void print_attr(const exr_attribute_t *a, int verbose)
{
  if (!a)
    return;
  ...
  switch (a->type) {
    ...
    case EXR_ATTR_TILEDESC: {
      ...
      uint8_t lvlMode = (uint8_t)EXR_GET_TILE_LEVEL_MODE(*(a->tiledesc));
      uint8_t rndMode = (uint8_t)EXR_GET_TILE_ROUND_MODE(*(a->tiledesc));
    ...
    case EXR_ATTR_V3I:
      ...
      break;
    case EXR_ATTR_V3F:
      printf("[ %g, %g, %g ]", (double)a->v3f->x, (double)a->v3f->y, (double)a->v3f->z);
      break;
    case EXR_ATTR_V3D:
...
exr_result_t exr_print_context_info(exr_const_context_t ctxt, int verbose)
{
  ...
  if (verbose) {
    printf("File '%s': ver %d flags%s%s%s%s\n",
           ...
           pctxt->max_name_length == EXR_LONGNAME_MAXLEN ? " longnames" : " shortnames",
