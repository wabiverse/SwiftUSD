...
static exr_result_t validate_req_attr(struct _internal_exr_context *f,
                                      struct _internal_exr_part *curpart,
                                      int adddefault)
{
  exr_result_t rv = EXR_ERR_SUCCESS;
  if (!curpart->channels)
    return f->print_error(f, EXR_ERR_MISSING_REQ_ATTR, "'channels' attribute not found");
  if (!curpart->compression) {
    if (adddefault) {
      rv = exr_attr_list_add_static_name((exr_context_t)f,
                                         &(curpart->attributes),
                                         "compression",
      ...
      return f->print_error(f, EXR_ERR_MISSING_REQ_ATTR, "'compression' attribute not found");
    }
  }
  ...
  if (!curpart->dataWindow) {
    if (adddefault) {
      ...
      *(curpart->dataWindow->box2i) = defdw;
      ...
      return f->print_error(f, EXR_ERR_MISSING_REQ_ATTR, "'dataWindow' attribute not found");
    }
  }
  ...
  if (!curpart->displayWindow) {
    if (adddefault) {
      ...
      *(curpart->displayWindow->box2i) = defdw;
      ...
      return f->print_error(f, EXR_ERR_MISSING_REQ_ATTR, "'displayWindow' attribute not found");
    }
  }
  ...
  if (!curpart->lineOrder) {
    if (adddefault) {
      ...
      return f->print_error(f, EXR_ERR_MISSING_REQ_ATTR, "'lineOrder' attribute not found");
    }
  }
  ...
  if (!curpart->pixelAspectRatio) {
    if (adddefault) {
      ...
      return f->print_error(f, EXR_ERR_MISSING_REQ_ATTR, "'pixelAspectRatio' attribute not found");
    }
  }
    ...
    if (adddefault) {
      ...
      *(curpart->screenWindowCenter->v2f) = defswc;
      ...
      return f->print_error(
          f, EXR_ERR_MISSING_REQ_ATTR, "'screenWindowCenter' attribute not found");
    }
  }
  ...
  if (!curpart->screenWindowWidth) {
    if (adddefault) {
      ...
      return f->print_error(
          f, EXR_ERR_MISSING_REQ_ATTR, "'screenWindowWidth' attribute not found");
    }
  }
  ...
  if (f->is_multipart || f->has_nonimage_data) {
    if (f->is_multipart && !curpart->name)
      return f->print_error(
          f, EXR_ERR_MISSING_REQ_ATTR, "'name' attribute for multipart file not found");
    if (!curpart->type)
      return f->print_error(
          f, EXR_ERR_MISSING_REQ_ATTR, "'type' attribute for v2+ file not found");
    if (f->has_nonimage_data && !curpart->version)
      return f->print_error(
          f, EXR_ERR_MISSING_REQ_ATTR, "'version' attribute for deep file not found");
    if (!curpart->chunkCount)
      return f->print_error(f,
...
static exr_result_t validate_image_dimensions(struct _internal_exr_context *f,
                                              struct _internal_exr_part *curpart)
{
  ...
  if (dspw.min.x > dspw.max.x || dspw.min.y > dspw.max.y || dspw.min.x <= -kLargeVal ||
      dspw.min.y <= -kLargeVal || dspw.max.x >= kLargeVal || dspw.max.y >= kLargeVal)
    return f->print_error(f,
  ...
  if (dw.min.x > dw.max.x || dw.min.y > dw.max.y || dw.min.x <= -kLargeVal ||
      dw.min.y <= -kLargeVal || dw.max.x >= kLargeVal || dw.max.y >= kLargeVal)
    return f->print_error(f,
                          EXR_ERR_INVALID_ATTR,
  ...
  if (maxw > 0 && maxw < w)
    return f->print_error(
        f, EXR_ERR_INVALID_ATTR, "Invalid width (%" PRId64 ") too large (max %d)", w, maxw);

  if (maxh > 0 && maxh < h)
    return f->print_error(
        f, EXR_ERR_INVALID_ATTR, "Invalid height (%" PRId64 ") too large (max %d)", h, maxh);
  ...
  if (maxw > 0 && maxh > 0) {
    ...
    if (ccount > maxNum)
      return f->print_error(f,
                            EXR_ERR_INVALID_ATTR,
                            "Invalid chunkCount (%" PRId64 ") exceeds maximum area of %" PRId64 "",
                            ccount,
                            maxNum);
  }
  ...
  if (!isnormal(par) || par < 1e-6f || par > 1e+6f)
    return f->print_error(f, EXR_ERR_INVALID_ATTR, "Invalid pixel aspect ratio %g", (double)par);

  if (sww < 0.f)
    return f->print_error(f, EXR_ERR_INVALID_ATTR, "Invalid screen window width %g", (double)sww);
...
static exr_result_t validate_channels(struct _internal_exr_context *f,
                                      struct _internal_exr_part *curpart,
                                      const exr_attr_chlist_t *channels)
{
  ...
  if (!channels)
    return f->report_error(
        f, EXR_ERR_INVALID_ARGUMENT, "Missing required channels attribute to validate against");
  if (!curpart->dataWindow)
    return f->report_error(
        f,
  ...
  for (int c = 0; c < channels->num_channels; ++c) {
    ...
    if (dw.min.x % xsamp)
      return f->print_error(f,
                            EXR_ERR_INVALID_ATTR,
                            "channel '%s': minimum x coordinate (%d) of the data window is not a "
                            "multiple of the x subsampling factor (%d)",
                            channels->entries[c].name.str,
                            dw.min.x,
                            xsamp);
    if (dw.min.y % ysamp)
      return f->print_error(f,
                            EXR_ERR_INVALID_ATTR,
                            "channel '%s': minimum y coordinate (%d) of the data window is not a "
                            "multiple of the y subsampling factor (%d)",
                            channels->entries[c].name.str,
                            dw.min.y,
                            ysamp);
    if (w % xsamp)
...
static exr_result_t validate_part_type(struct _internal_exr_context *f,
                                       struct _internal_exr_part *curpart)
{
  ...
  if (curpart->type) {
    ...
    if (f->is_singlepart_tiled && curpart->storage_mode != EXR_STORAGE_TILED) {
      ...
      f->print_error(f,
                     EXR_ERR_INVALID_ATTR,
                     "attribute 'type': Mismatch between file flags and type string '%s', "
                     "believing file flags",
                     curpart->type->string->str);

      if (f->mode == EXR_CONTEXT_WRITE)
...
static exr_result_t validate_tile_data(struct _internal_exr_context *f,
                                       struct _internal_exr_part *curpart)
{
      ...
      curpart->storage_mode == EXR_STORAGE_DEEP_TILED)
  {
    ...
    if (!curpart->tiles)
      return f->print_error(
          f, EXR_ERR_MISSING_REQ_ATTR, "'tiles' attribute for tiled file not found");

    desc = curpart->tiles->tiledesc;
    levmode = EXR_GET_TILE_LEVEL_MODE(*desc);
    rndmode = EXR_GET_TILE_ROUND_MODE(*desc);

    if (desc->x_size == 0 || desc->y_size == 0 || desc->x_size > (uint32_t)(INT_MAX / 4) ||
    ...
    if ((int)levmode < EXR_TILE_ONE_LEVEL || (int)levmode >= EXR_TILE_LAST_TYPE)
      return f->print_error(f,
                            EXR_ERR_INVALID_ATTR,
                            "Invalid level mode (%d) in tile description header",
                            (int)levmode);

    if ((int)rndmode < EXR_TILE_ROUND_DOWN || (int)rndmode >= EXR_TILE_ROUND_LAST_TYPE)
      return f->print_error(f,
                            EXR_ERR_INVALID_ATTR,
...
static exr_result_t validate_deep_data(struct _internal_exr_context *f,
                                       struct _internal_exr_part *curpart)
{
      ...
      curpart->storage_mode == EXR_STORAGE_DEEP_TILED)
  {
    ...
    // none, rle, zips
    if (curpart->comp_type != EXR_COMPRESSION_NONE && curpart->comp_type != EXR_COMPRESSION_RLE &&
        curpart->comp_type != EXR_COMPRESSION_ZIPS)
      return f->report_error(f, EXR_ERR_INVALID_ATTR, "Invalid compression for deep data");
...
exr_result_t internal_exr_validate_read_part(struct _internal_exr_context *f,
                                             struct _internal_exr_part *curpart)
{
  exr_result_t rv;
...
exr_result_t internal_exr_validate_write_part(struct _internal_exr_context *f,
                                              struct _internal_exr_part *curpart)
{
  exr_result_t rv;
