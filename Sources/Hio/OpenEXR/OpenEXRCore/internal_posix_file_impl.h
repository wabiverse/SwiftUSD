  ...
  if (!failed && pf->tmp_filename.str) {
    ...
    if (mvret < 0)
      return pf->print_error(
          pf, EXR_ERR_FILE_ACCESS, "Unable to rename temporary file: %s", strerror(rv));
  }
                                 ...
                                 exr_stream_error_func_ptr_t error_cb)
{
  ...
  if (sizeof(size_t) == 4) {
    if (sz >= (uint64_t)UINT32_MAX) {
      if (error_cb)
        error_cb(ctxt, EXR_ERR_INVALID_ARGUMENT, "read request size too large for architecture");
      return retsz;
    }
  }
...
#  endif
      if (error_cb) {
        if (spos == (uint64_t)-1)
          ...
          error_cb(ctxt, EXR_ERR_READ_IO, "Unable to seek to requested position");
      }
      return retsz;
    }
  }
...
#endif
  if (retsz < 0 && error_cb)
    error_cb(ctxt, EXR_ERR_READ_IO, "Unable to read %" PRIu64 " bytes: %s", sz, strerror(errno));
  return retsz;
}
...
#  endif
      if (error_cb) {
        if (spos == (uint64_t)-1)
          ...
          error_cb(ctxt, EXR_ERR_WRITE_IO, "Unable to seek to requested position");
      }
      return retsz;
    }
  }
...
#  ifdef ILMTHREAD_THREADING_ENABLED
  ...
  if (fd != 0)
    return file->print_error(
        file, EXR_ERR_OUT_OF_MEMORY, "Unable to initialize file mutex: %s", strerror(fd));
#  endif
#endif
  ...
  if (fd < 0)
    return file->print_error(
        file, EXR_ERR_FILE_ACCESS, "Unable to open file for read: %s", strerror(errno));
...
#  endif
  ...
  if (fd < 0)
    return file->print_error(
        file, EXR_ERR_FILE_ACCESS, "Unable to open file for write: %s", strerror(errno));
  fh->fd = fd;
...
static exr_result_t make_temp_filename(struct _internal_exr_context *ret)
{
  ...
  if (tmpname) {
    ...
    if (lastslash) {
      ...
      strncpy(tmpname + nPrev + tlen, srcfile + nPrev, (uint64_t)(ret->filename.length) - nPrev);
      tmpname[newlen] = '\0';
    }
    else {
