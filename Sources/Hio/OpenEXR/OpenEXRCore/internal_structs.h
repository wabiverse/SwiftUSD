...
enum _INTERNAL_EXR_READ_MODE { EXR_MUST_READ_ALL = 0, EXR_ALLOW_SHORT_READ = 1 };
...
struct _internal_exr_context {
  ...
  exr_result_t (*do_read)(const struct _internal_exr_context *file,
                          void *,
                          uint64_t,
                          uint64_t *,
                          int64_t *,
                          enum _INTERNAL_EXR_READ_MODE);
  exr_result_t (*do_write)(struct _internal_exr_context *file, const void *, uint64_t, uint64_t *);

  exr_result_t (*standard_error)(const struct _internal_exr_context *ctxt, exr_result_t code);
  exr_result_t (*report_error)(const struct _internal_exr_context *ctxt,
                               exr_result_t code,
                               const char *msg);
  exr_result_t (*print_error)(const struct _internal_exr_context *ctxt,
                              exr_result_t code,
                              const char *msg,
...
static inline void internal_exr_lock(const struct _internal_exr_context *c)
{
#ifdef ILMTHREAD_THREADING_ENABLED
  struct _internal_exr_context *nonc = EXR_CONST_CAST(struct _internal_exr_context *, c);
#  ifdef _WIN32
...
static inline void internal_exr_unlock(const struct _internal_exr_context *c)
{
#ifdef ILMTHREAD_THREADING_ENABLED
  struct _internal_exr_context *nonc = EXR_CONST_CAST(struct _internal_exr_context *, c);
#  ifdef _WIN32
...
#define EXR_UNLOCK(c) internal_exr_unlock((const struct _internal_exr_context *)c)

#define EXR_LOCK_WRITE(c) \
  if (c->mode == EXR_CONTEXT_WRITE) \
  internal_exr_lock(c)
...
#define EXR_RETURN_WRITE(c) ((c->mode == EXR_CONTEXT_WRITE) ? internal_exr_unlock(c) : ((void)0))

#define EXR_UNLOCK_AND_RETURN_PCTXT(v) ((void)(EXR_UNLOCK(pctxt)), v)
#define EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(v) ((void)(EXR_RETURN_WRITE(pctxt)), v)
...
#define EXR_PROMOTE_LOCKED_CONTEXT_AND_PART_OR_ERROR(c, pi) \
  ...
  if (pi < 0 || pi >= pctxt->num_parts) \
    return ((void)(EXR_UNLOCK(pctxt)), \
            pctxt->print_error( \
                pctxt, EXR_ERR_ARGUMENT_OUT_OF_RANGE, "Part index (%d) out of range", pi)); \
  part = pctxt->parts[pi]
...
#define EXR_PROMOTE_CONST_CONTEXT_AND_PART_OR_ERROR(c, pi) \
  ...
  EXR_LOCK_WRITE(pctxt); \
  if (pi < 0 || pi >= pctxt->num_parts) \
    return ((void)(EXR_RETURN_WRITE(pctxt)), \
            pctxt->print_error( \
                pctxt, EXR_ERR_ARGUMENT_OUT_OF_RANGE, "Part index (%d) out of range", pi)); \
  part = pctxt->parts[pi]
...
EXR_INTERNAL exr_result_t internal_exr_add_part(struct _internal_exr_context *,
                                                struct _internal_exr_part **,
                                                int *new_index);
EXR_INTERNAL void internal_exr_revert_add_part(struct _internal_exr_context *,
                                               struct _internal_exr_part **,
                                               int *new_index);

EXR_INTERNAL exr_result_t internal_exr_context_restore_handlers(struct _internal_exr_context *ctxt,
                                                                exr_result_t rv);
EXR_INTERNAL exr_result_t internal_exr_alloc_context(struct _internal_exr_context **out,
