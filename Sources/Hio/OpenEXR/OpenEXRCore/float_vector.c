...
exr_result_t exr_attr_float_vector_init(exr_context_t ctxt,
                                        ...
                                        int32_t nent)
{
  ...
  if (!fv)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid reference to float vector object to initialize");
...
exr_result_t exr_attr_float_vector_init_static(exr_context_t ctxt,
                                               exr_attr_float_vector_t *fv,
                                               const float *arr,
                                               int32_t nent)
{
  ...
  if (!arr)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid reference to float array object to initialize");
...
exr_result_t exr_attr_float_vector_create(exr_context_t ctxt,
                                          exr_attr_float_vector_t *fv,
                                          const float *arr,
                                          int32_t nent)
{
  ...
  if (!fv || !arr)
    return pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid (NULL) arguments to float vector create");
