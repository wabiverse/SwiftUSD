...
static exr_result_t default_pack_deep(exr_encode_pipeline_t *encode)
{
  (void)encode;
  return EXR_ERR_INVALID_ARGUMENT;
}

static exr_result_t default_pack(exr_encode_pipeline_t *encode)
{
  ...
  for (int y = 0; y < encode->chunk.height; ++y) {
    ...
    for (int c = 0; c < encode->channel_count; ++c) {
      ...
      if (encc->y_samples > 1) {
        if ((cury % encc->y_samples) != 0)
          continue;
        if (cdata)
          cdata += ((uint64_t)(y / encc->y_samples) * (uint64_t)encc->user_line_stride);
      }
      else {
        ...
        case EXR_PIXEL_HALF:
          switch (encc->user_data_type) {
            ...
            case EXR_PIXEL_FLOAT: {
              uint16_t *dst = (uint16_t *)dstbuffer;
              for (int x = 0; x < w; ++x) {
                uint16_t cval = float_to_half(*((const float *)cdata));
            ...
            case EXR_PIXEL_UINT: {
              uint16_t *dst = (uint16_t *)dstbuffer;
              for (int x = 0; x < w; ++x) {
                uint16_t cval = uint_to_half(*((const uint32_t *)cdata));
                unaligned_store16(dst, cval);
                ++dst;
                cdata += pixincrement;
              }
        ...
        case EXR_PIXEL_FLOAT:
          switch (encc->user_data_type) {
            case EXR_PIXEL_HALF: {
              ...
              for (int x = 0; x < w; ++x) {
                uint32_t fint = half_to_float_int(*((const uint16_t *)cdata));
            ...
            case EXR_PIXEL_UINT: {
              uint32_t *dst = (uint32_t *)dstbuffer;
              for (int x = 0; x < w; ++x) {
                uint32_t fint = uint_to_float_int(*((const uint32_t *)cdata));
                unaligned_store32(dst, fint);
                ++dst;
                cdata += pixincrement;
              }
...
internal_exr_pack_fn internal_exr_match_encode(exr_encode_pipeline_t *encode, int isdeep)
{
  (void)encode;
  if (isdeep)
    return &default_pack_deep;
