...
static inline int hufLength(uint64_t code)
{
  return (int)(code & 63);
}

static inline uint64_t hufCode(uint64_t code)
{
  return code >> 6;
}

static inline exr_result_t outputBits(
    int nBits, uint64_t bits, uint64_t *c, int *lc, uint8_t **outptr, uint8_t *outend)
{
  uint8_t *out = *outptr;
  *c <<= nBits;
  *lc += nBits;
...
static inline uint64_t getBits(uint32_t nBits, uint64_t *c, uint32_t *lc, const uint8_t **inptr)
{
...
static void hufCanonicalCodeTable(uint64_t *hcode)
{
  uint64_t n[59];
  uint64_t c;
...
static inline int FHeapCompare(uint64_t *a, uint64_t *b)
{
  return ((*a > *b) || ((*a == *b) && (a > b)));
}

static inline void intern_push_heap(uint64_t **first,
                                    size_t holeIndex,
                                    size_t topIndex,
                                    uint64_t *value)
{
...
static inline void adjust_heap(uint64_t **first, size_t holeIndex, size_t len, uint64_t *value)
{
...
static inline void push_heap(uint64_t **first, uint64_t **last)
{
  uint64_t *value = *(last - 1);
  intern_push_heap(first, (size_t)(last - first) - 1, 0, value);
}

static inline void intern_pop_heap(uint64_t **first, uint64_t **last, uint64_t **result)
{
...
static inline void pop_heap(uint64_t **first, uint64_t **last)
{
  if (last - first > 1) {
    --last;
...
static void make_heap(uint64_t **first, uint64_t len)
{
  size_t parent;
...
static void hufBuildEncTable(
    uint64_t *frq, uint32_t *im, uint32_t *iM, uint32_t *hlink, uint64_t **fHeap, uint64_t *scode)
{
  //
...
static exr_result_t hufPackEncTable(const uint64_t *hcode,  // i : encoding table [HUF_ENCSIZE]
    ...
    if (l == 0) {
      ...
      if (zerun >= 2) {
        if (zerun >= SHORTEST_LONG_RUN) {
          ...
          rv = outputBits(6, SHORT_ZEROCODE_RUN + zerun - 2, &c, &lc, &out, pend);
          if (rv != EXR_ERR_SUCCESS)
            return rv;
        }
        continue;
...
static exr_result_t hufUnpackEncTable(const uint8_t **pcode,  // io: ptr to packed table (updated)
...
static exr_result_t hufBuildDecTable(const struct _internal_exr_context *pctxt,
  ...
  for (; im <= iM; im++) {
    ...
    if (l > HUF_DECBITS) {
      ...
      if (pl->p) {
        ...
        pl->p = (uint32_t *)alloc_fn(sizeof(uint32_t));
      }
...
static void hufFreeDecTable(const struct _internal_exr_context *pctxt, HufDec *hdecod)
{
  void (*free_fn)(void *) = pctxt ? pctxt->free_fn : internal_exr_free;
...
static inline exr_result_t outputCode(
    uint64_t code, uint64_t *c, int *lc, uint8_t **out, uint8_t *outend)
{
  return outputBits(hufLength(code), hufCode(code), c, lc, out, outend);
}

static inline exr_result_t sendCode(uint64_t sCode,
                                    int runCount,
                                    uint64_t runCode,
                                    uint64_t *c,
                                    int *lc,
...
static inline exr_result_t hufEncode(const uint64_t *hcode,
                                     const uint16_t *in,
                                     const uint64_t ni,
                                     uint32_t rlc,
                                     uint8_t *out,
...
static exr_result_t hufDecode(const uint64_t *hcode,  // i : encoding table
  ...
  while (in < ie) {
    ...
    while (lc >= HUF_DECBITS) {
      ...
      if (pl->len) {
        ...
        if (!pl->p)
          return EXR_ERR_CORRUPT_CHUNK;  // wrong code
        ...
        for (j = 0; j < pl->lit; j++) {
          ...
          if (lc >= l) {
            if (hufCode(hcode[decbuf[j]]) == ((c >> (lc - l)) & (((uint64_t)(1) << l) - 1))) {
              //
              // Found : get long code
              //

              lc -= l;
              getCode(decbuf[j], rlc, c, lc, in, ie, out, outb, oe);
              break;
            }
          }
        }
...
static inline void countFrequencies(uint64_t *freq, const uint16_t *data, uint64_t n)
{
...
static inline void writeUInt(uint8_t *b, uint32_t i)
{
  b[0] = (uint8_t)(i);
  b[1] = (uint8_t)(i >> 8);
  b[2] = (uint8_t)(i >> 16);
...
static inline uint32_t readUInt(const uint8_t *b)
{
...
typedef struct FastHufDecoder {
  ...
  //
  int _lookupSymbol[1 << TABLE_LOOKUP_BITS]; /* value = (codeLen << 24) | symbol */

  uint64_t _tableMin;
} FastHufDecoder;

static exr_result_t FastHufDecoder_buildTables(const struct _internal_exr_context *pctxt,
                                               FastHufDecoder *fhd,
                                               uint64_t *base,
                                               uint64_t *offset)
{
  ...
  for (uint64_t i = 0; i < 1 << TABLE_LOOKUP_BITS; ++i) {
    ...
    for (int codeLen = fhd->_minCodeLength; codeLen <= fhd->_maxCodeLength; ++codeLen) {
      if (fhd->_ljBase[codeLen] <= value) {
        uint64_t id = fhd->_ljOffset[codeLen] + (value >> (64 - codeLen));
        if (id < (uint64_t)(fhd->_numSymbols)) {
          fhd->_lookupSymbol[i] = (fhd->_idToSymbol[id] | (codeLen << 24));
        }
        else {
          if (pctxt)
            pctxt->print_error(pctxt, EXR_ERR_CORRUPT_CHUNK, "Huffman decode error (Overrun)");
          return EXR_ERR_CORRUPT_CHUNK;
        }
        break;
      }
  ...
  if (minIdx < 0) {
    ...
    fhd->_tableMin = fhd->_ljBase[minIdx];
  }
  return EXR_ERR_SUCCESS;
}

static inline void FastHufDecoder_refill(
    uint64_t *buffer,
...
static inline uint64_t fasthuf_read_bits(int numBits,
                                         uint64_t *buffer,
                                         int *bufferNumBits,
...
static exr_result_t fasthuf_initialize(const struct _internal_exr_context *pctxt,
                                       FastHufDecoder *fhd,
                                       const uint8_t **table,
                                       uint64_t numBytes,
                                       uint32_t minSymbol,
                                       ...
                                       int rleSymbol)
{
  ...
  for (uint64_t symbol = (uint64_t)minSymbol; symbol <= (uint64_t)maxSymbol; symbol++) {
    if (currByte >= topByte) {
      if (pctxt)
        pctxt->print_error(
            pctxt, EXR_ERR_CORRUPT_CHUNK, "Error decoding Huffman table (Truncated table data).");
      return EXR_ERR_CORRUPT_CHUNK;
    }
    ...
    if (codeLen < (uint64_t)SHORT_ZEROCODE_RUN) {
      ...
      symbol += fasthuf_read_bits(8, &currBits, &currBitCount, &currByte) + SHORTEST_LONG_RUN - 1;
    }
    else
  ...
  if ((size_t)fhd->_numSymbols > sizeof(fhd->_idToSymbol) / sizeof(int)) {
    if (pctxt)
      pctxt->print_error(
          pctxt, EXR_ERR_CORRUPT_CHUNK, "Error decoding Huffman table (Too many symbols).");
    return EXR_ERR_CORRUPT_CHUNK;
  }
...

  {
    ...
    for (int l = fhd->_minCodeLength; l <= fhd->_maxCodeLength; ++l) {
      countTmp[l] = (double)codeCount[l] * (double)(2ll << (fhd->_maxCodeLength - l));
    }
  ...
  for (uint64_t symbol = (uint64_t)minSymbol; symbol <= (uint64_t)maxSymbol; symbol++) {
    ...
    if (codeLen < (uint64_t)SHORT_ZEROCODE_RUN) {
      ...
      if (mapping[codeLen] >= (uint64_t)fhd->_numSymbols) {
        if (pctxt)
          pctxt->print_error(
              pctxt, EXR_ERR_CORRUPT_CHUNK, "Huffman decode error (Invalid symbol in header)");
        return EXR_ERR_CORRUPT_CHUNK;
      }
...
static inline int fasthuf_decode_enabled(void)
{
...
static exr_result_t fasthuf_decode(const struct _internal_exr_context *pctxt,
                                   FastHufDecoder *fhd,
                                   const uint8_t *src,
                                   uint64_t numSrcBits,
                                   uint16_t *dst,
                                   uint64_t numDstElems)
{
  ...
  while (dstIdx < numDstElems) {
    ...
    if (fhd->_tableMin <= buffer) {
      int tableIdx = fhd->_lookupSymbol[buffer >> (64 - TABLE_LOOKUP_BITS)];
      ...
      while (fhd->_ljBase[codeLen] > buffer /* && codeLen <= _maxCodeLength */)
        codeLen++;

      if (codeLen > fhd->_maxCodeLength) {
        if (pctxt)
          pctxt->print_error(
              pctxt, EXR_ERR_CORRUPT_CHUNK, "Huffman decode error (Decoded an invalid symbol)");
        return EXR_ERR_CORRUPT_CHUNK;
      }
    ...
    if (symbol == fhd->_rleSymbol) {
      if (bufferNumBits < 8) {
        FastHufDecoder_refill(
            &buffer, 64 - bufferNumBits, &bufferBack, &bufferBackNumBits, &currByte, &numSrcBits);
      ...
      if (rleCount <= 0) {
        if (pctxt)
          pctxt->print_error(
              pctxt, EXR_ERR_CORRUPT_CHUNK, "Huffman decode error (Invalid RLE length)");
        return EXR_ERR_CORRUPT_CHUNK;
      }
  ...
  if (numSrcBits != 0) {
    if (pctxt)
      pctxt->print_error(pctxt,
                         EXR_ERR_CORRUPT_CHUNK,
                         "Huffman decode error (%d bits of compressed data remains after filling "
...
uint64_t internal_exr_huf_decompress_spare_bytes(void)
{
  ...
  //    ret += HUF_ENCSIZE * sizeof (uint64_t*); // fheap
  //    ret += HUF_ENCSIZE * sizeof (uint64_t);  // scode
  if (sizeof(FastHufDecoder) > ret)
    ret = sizeof(FastHufDecoder);
  return ret;
}

exr_result_t internal_huf_compress(uint64_t *encbytes,
                                   void *out,
                                   uint64_t outsz,
                                   const uint16_t *raw,
                                   uint64_t nRaw,
                                   ...
                                   uint64_t sparebytes)
{
  ...
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  tableLength = (uint32_t)(((uintptr_t)tableEnd) - ((uintptr_t)tableStart));
  dataStart = tableEnd;
  ...
  *encbytes = (((uintptr_t)dataStart) + ((uintptr_t)dataLength) - ((uintptr_t)compressed));
  return EXR_ERR_SUCCESS;
}

exr_result_t internal_huf_decompress(exr_decode_pipeline_t *decode,
                                     ...
                                     uint64_t sparebytes)
{
  ...
  if (hufInfoBlockSize + nBytes > nCompressed)
    return EXR_ERR_OUT_OF_MEMORY;
