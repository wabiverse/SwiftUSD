...
static exr_result_t dispatch_read(const struct _internal_exr_context *ctxt,
                                  void *buf,
                                  uint64_t sz,
                                  uint64_t *offsetp,
                                  int64_t *nread,
                                  enum _INTERNAL_EXR_READ_MODE rmode)
{
  ...
  if (!offsetp)
    return ctxt->report_error(
        ctxt, EXR_ERR_INVALID_ARGUMENT, "read requested with no output offset pointer");

  if (ctxt->read_fn)
    rval = ctxt->read_fn((exr_const_context_t)ctxt,
                         ctxt->user_data,
                         buf,
                         sz,
                         *offsetp,
...
static exr_result_t dispatch_write(struct _internal_exr_context *ctxt,
                                   const void *buf,
                                   uint64_t sz,
                                   uint64_t *offsetp)
{
  ...
  if (!offsetp)
    return ctxt->report_error(
        ctxt, EXR_ERR_INVALID_ARGUMENT, "write requested with no output offset pointer");

  if (ctxt->write_fn)
    rval = ctxt->write_fn((exr_const_context_t)ctxt,
                          ctxt->user_data,
                          buf,
                          sz,
                          *offsetp,
...
static exr_result_t process_query_size(struct _internal_exr_context *ctxt,
                                       exr_context_initializer_t *inits)
{
  if (inits->size_fn) {
    ctxt->file_size = (inits->size_fn)((exr_const_context_t)ctxt, ctxt->user_data);
  }
  else {
    ctxt->file_size = -1;
  }
...
static inline exr_context_initializer_t fill_context_data(
...
exr_result_t exr_test_file_header(const char *filename, const exr_context_initializer_t *ctxtdata)
{
  ...
  if (filename && filename[0] != '\0') {
    rv = internal_exr_alloc_context(
        &ret, &inits, EXR_CONTEXT_READ, sizeof(struct _internal_exr_filehandle));
    if (rv == EXR_ERR_SUCCESS) {
      ret->do_read = &dispatch_read;

      rv = exr_attr_string_create((exr_context_t)ret, &(ret->filename), filename);
      if (rv == EXR_ERR_SUCCESS) {
        if (!inits.read_fn) {
        ...
        if (rv == EXR_ERR_SUCCESS)
          rv = internal_exr_check_magic(ret);
      }
    ...
    inits.error_handler_fn(
        NULL, EXR_ERR_INVALID_ARGUMENT, "Invalid filename passed to test file header function");
...
exr_result_t exr_start_read(exr_context_t *ctxt,
                            const char *filename,
                            const exr_context_initializer_t *ctxtdata)
{
  exr_result_t rv = EXR_ERR_UNKNOWN;
  ...
  if (!ctxt) {
    if (!(inits.flags & EXR_CONTEXT_FLAG_SILENT_HEADER_PARSE))
      inits.error_handler_fn(
          NULL, EXR_ERR_INVALID_ARGUMENT, "Invalid context handle passed to start_read function");
  ...
  if (filename && filename[0] != '\0') {
    ...
    if (rv == EXR_ERR_SUCCESS) {
      ...
      if (rv == EXR_ERR_SUCCESS) {
        ...
        if (rv == EXR_ERR_SUCCESS)
          rv = process_query_size(ret, &inits);
        if (rv == EXR_ERR_SUCCESS)
          rv = internal_exr_parse_header(ret);
      }
    ...
    if (!(inits.flags & EXR_CONTEXT_FLAG_SILENT_HEADER_PARSE))
      inits.error_handler_fn(
          NULL, EXR_ERR_INVALID_ARGUMENT, "Invalid filename passed to start_read function");
    rv = EXR_ERR_INVALID_ARGUMENT;
  }
...
exr_result_t exr_start_write(exr_context_t *ctxt,
                             const char *filename,
                             ...
                             const exr_context_initializer_t *ctxtdata)
{
  ...
  if (filename && filename[0] != '\0') {
    rv = internal_exr_alloc_context(
        &ret, &inits, EXR_CONTEXT_WRITE, sizeof(struct _internal_exr_filehandle));
    if (rv == EXR_ERR_SUCCESS) {
    ...
    inits.error_handler_fn(
        NULL, EXR_ERR_INVALID_ARGUMENT, "Invalid filename passed to start_write function");
    rv = EXR_ERR_INVALID_ARGUMENT;
  }
...
exr_result_t exr_start_inplace_header_update(exr_context_t *ctxt,
                                             const char *filename,
                                             const exr_context_initializer_t *ctxtdata)
{
...
exr_result_t exr_get_file_name(exr_const_context_t ctxt, const char **name)
{
  ...
  return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_INVALID_ARGUMENT));
}
...
exr_result_t exr_register_attr_type_handler(
    exr_context_t ctxt,
    const char *type,
    ...
    void (*destroy_unpacked_func_ptr)(exr_context_t ctxt, void *data, int32_t datasize))
{
  ...
  if (!type || type[0] == '\0')
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid type to register_attr_handler"));
  ...
  rv = exr_attr_list_find_by_name(ctxt, &(pctxt->custom_handlers), type, &ent);
  if (rv == EXR_ERR_SUCCESS)
    return EXR_UNLOCK_AND_RETURN_PCTXT(
        pctxt->print_error(pctxt,
                           EXR_ERR_INVALID_ARGUMENT,
  ...
  ent = NULL;
  rv = exr_attr_list_add_by_type(ctxt, &(pctxt->custom_handlers), type, type, 0, NULL, &ent);
  if (rv != EXR_ERR_SUCCESS)
    return EXR_UNLOCK_AND_RETURN_PCTXT(
        pctxt->print_error(pctxt, rv, "Unable to register custom handler for type '%s'", type));
  ...
  for (int p = 0; p < pctxt->num_parts; ++p) {
    ...
    if (curattrs) {
      ...
      for (int a = 0; a < nattr; ++a) {
        ...
        if (ent->type_name_length == tlen && 0 == strcmp(ent->type_name, type)) {
          ent->opaque->unpack_func_ptr = unpack_func_ptr;
          ent->opaque->pack_func_ptr = pack_func_ptr;
          ent->opaque->destroy_unpacked_func_ptr = destroy_unpacked_func_ptr;
        }
  ...
  if (oldval > newval) {
    for (int p = 0; p < pctxt->num_parts; ++p) {
      ...
      for (int a = 0; a < curp->attributes.num_attributes; ++a) {
        ...
        if (curattr->name_length > newval || curattr->type_name_length > newval) {
          return EXR_UNLOCK_AND_RETURN_PCTXT(
              pctxt->print_error(pctxt,
                                 EXR_ERR_NAME_TOO_LONG,
                                 "Part %d, attribute '%s' (type '%s') has a name too long for new "
                                 "longname setting (%d)",
                                 curp->part_index,
                                 curattr->name,
                                 curattr->type_name,
...
exr_result_t exr_write_header(exr_context_t ctxt)
{
  ...
  if (pctxt->num_parts == 0)
    return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->report_error(
        pctxt, EXR_ERR_FILE_BAD_HEADER, "No parts defined in file prior to writing data"));
  ...
  for (int p = 0; rv == EXR_ERR_SUCCESS && p < pctxt->num_parts; ++p) {
    ...
    if (!curp->channels)
      return EXR_UNLOCK_AND_RETURN_PCTXT(pctxt->print_error(
          pctxt, EXR_ERR_MISSING_REQ_ATTR, "Part %d is missing channel list", p));
  ...
  if (rv == EXR_ERR_SUCCESS)
    rv = internal_exr_write_header(pctxt);
