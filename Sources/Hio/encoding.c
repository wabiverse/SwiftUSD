...
static exr_result_t default_compress_chunk(exr_encode_pipeline_t *encode)
{
  exr_result_t rv;
  EXR_PROMOTE_CONST_CONTEXT_AND_PART_OR_ERROR_NO_LOCK(encode->context, encode->part_index);
  ...
  switch (part->comp_type) {
    case EXR_COMPRESSION_NONE:
      return pctxt->report_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "no compression set but still trying to compress");

    case EXR_COMPRESSION_RLE:
      rv = internal_exr_apply_rle(encode);
      break;
    ...
    case EXR_COMPRESSION_PIZ:
      rv = internal_exr_apply_piz(encode);
      break;
    case EXR_COMPRESSION_PXR24:
      rv = internal_exr_apply_pxr24(encode);
      break;
    case EXR_COMPRESSION_B44:
      rv = internal_exr_apply_b44(encode);
      break;
    case EXR_COMPRESSION_B44A:
      rv = internal_exr_apply_b44a(encode);
      break;
    case EXR_COMPRESSION_DWAA:
      rv = internal_exr_apply_dwaa(encode);
      break;
    case EXR_COMPRESSION_DWAB:
      rv = internal_exr_apply_dwab(encode);
      break;
...
static exr_result_t default_yield(exr_encode_pipeline_t *encode)
{
  exr_result_t rv;
  EXR_PROMOTE_CONST_CONTEXT_AND_PART_OR_ERROR(encode->context, encode->part_index);
...
static exr_result_t default_write_chunk(exr_encode_pipeline_t *encode)
{
  exr_result_t rv;
  ...
  switch (encode->chunk.type) {
    case EXR_STORAGE_SCANLINE:
      rv = exr_write_scanline_chunk(EXR_CONST_CAST(exr_context_t, encode->context),
                                    encode->part_index,
    ...
    case EXR_STORAGE_TILED:
      rv = exr_write_tile_chunk(EXR_CONST_CAST(exr_context_t, encode->context),
                                encode->part_index,
    ...
    case EXR_STORAGE_DEEP_SCANLINE:
      if (!encode->packed_sample_count_table || encode->packed_sample_count_bytes == 0)
        return EXR_ERR_INVALID_ARGUMENT;
      rv = exr_write_deep_scanline_chunk(EXR_CONST_CAST(exr_context_t, encode->context),
                                         encode->part_index,
                                         encode->chunk.start_y,
                                         encode->compressed_buffer,
                                         encode->compressed_bytes,
    ...
    case EXR_STORAGE_DEEP_TILED:
      if (!encode->packed_sample_count_table || encode->packed_sample_count_bytes == 0)
        return EXR_ERR_INVALID_ARGUMENT;
      rv = exr_write_deep_tile_chunk(EXR_CONST_CAST(exr_context_t, encode->context),
                                     encode->part_index,
                                     encode->chunk.start_x,
                                     encode->chunk.start_y,
                                     encode->chunk.level_x,
...
exr_result_t exr_encoding_initialize(exr_const_context_t ctxt,
                                     int part_index,
                                     ...
                                     exr_encode_pipeline_t *encode)
{
  ...
  if (pctxt->mode != EXR_CONTEXT_WRITING_DATA) {
    ...
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->standard_error(pctxt, EXR_ERR_NOT_OPEN_WRITE));
  }

  *encode = nil;

  rv = internal_coding_fill_channel_info(&(encode->channels),
...
exr_result_t exr_encoding_choose_default_routines(exr_const_context_t ctxt,
                                                  int part_index,
                                                  exr_encode_pipeline_t *encode)
{
  int32_t isdeep = 0;
...
exr_result_t exr_encoding_update(exr_const_context_t ctxt,
                                 int part_index,
                                 const exr_chunk_info_t *cinfo,
                                 exr_encode_pipeline_t *encode)
{
...
exr_result_t exr_encoding_run(exr_const_context_t ctxt,
                              int part_index,
                              exr_encode_pipeline_t *encode)
{
      ...
      part->storage_mode == EXR_STORAGE_DEEP_TILED)
  {
    if (encode->sample_count_table == NULL ||
        encode->sample_count_alloc_size !=
            (((size_t)encode->chunk.width) * ((size_t)encode->chunk.height) * sizeof(int32_t)))
    {
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->report_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "Invalid / missing sample count table for deep data"));
    }
  }
  ...
  for (int c = 0; c < encode->channel_count; ++c) {
    ...
    if (encc->width == 0)
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->print_error(
          pctxt, EXR_ERR_INVALID_ARGUMENT, "Unexpected 0-width chunk to encode"));
     ...
     */
    if (encc->user_bytes_per_element != 2 && encc->user_bytes_per_element != 4)
      return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(pctxt->print_error(
          pctxt,
          EXR_ERR_INVALID_ARGUMENT,
    ...
    packed_bytes += ((uint64_t)(encc->height) * (uint64_t)(encc->width) *
  ...
  if (encode->convert_and_pack_fn) {
    ...
    return EXR_UNLOCK_WRITE_AND_RETURN_PCTXT(
        pctxt->report_error(pctxt,
                            EXR_ERR_INVALID_ARGUMENT,
                            "Encode pipeline has no packing function declared and packed buffer "
                            "is null or appears to need packing"));
  }
  ...
  {
    priv_from_native32(encode->sample_count_table, encode->chunk.width * encode->chunk.height);
  }
  ...
  if (rv == EXR_ERR_SUCCESS) {
    if (encode->compress_fn && encode->packed_bytes > 0) {
      ...
      internal_encode_free_buffer(encode,
                                  EXR_TRANSCODE_BUFFER_COMPRESSED,
                                  &(encode->compressed_buffer),
                                  &(encode->compressed_alloc_size));
      ...
      encode->packed_sample_count_bytes = (((size_t)encode->chunk.width) *
  ...
  if ((part->storage_mode == EXR_STORAGE_DEEP_SCANLINE ||
      ...
      encode->sample_count_table != NULL)
  {
    priv_to_native32(encode->sample_count_table, encode->chunk.width * encode->chunk.height);
  }
