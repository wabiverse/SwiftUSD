...
exr_result_t nanoexr_get_attribute_by_index(exr_const_context_t ctxt,
                                            int part_index,
                                            int i,
...
bool nanoexr_Gaussian_resample(const nanoexr_ImageData_t *src, nanoexr_ImageData_t *dst)
{
...
static void err_cb(exr_const_context_t f, int code, const char *msg)
{
...
int nanoexr_read_header(nanoexr_Reader_t *reader,
                        exr_read_func_ptr_t readFn,
                        nanoexr_attrRead attrRead,
...
exr_result_t nanoexr_write_exr(const char *filename,
                               nanoexr_attrsAdd attrsAdd,
                               void *attrsAdd_userData,
                               int width,
                               int height,
                               bool flipped,
                               exr_pixel_type_t pixel_type,
                               uint8_t *red,
                               int32_t redPixelStride,
                               int32_t redLineStride,
                               uint8_t *green,
                               int32_t greenPixelStride,
                               int32_t greenLineStride,
                               uint8_t *blue,
                               int32_t bluePixelStride,
                               int32_t blueLineStride,
                               uint8_t *alpha,
                               int32_t alphaPixelStride,
                               int32_t alphaLineStride)
{
  ...
  exr_result_t result = exr_start_write(&exr, filename, EXR_WRITE_FILE_DIRECTLY, &init);
  ...
  result = exr_initialize_required_attr(exr,
                                        partidx,
                                        &dataw,
                                        &dispw,
  ...
  if (alpha) {
    result = exr_add_channel(
        exr, partidx, "A", pixel_type, EXR_PERCEPTUALLY_LOGARITHMIC, 1, 1);  // x & y sampling rate
    if (result != EXR_ERR_SUCCESS) {
      return result;
    }
  }
  ...
  if (blue) {
    result = exr_add_channel(
        exr, partidx, "B", pixel_type, EXR_PERCEPTUALLY_LOGARITHMIC, 1, 1);  // x & y sampling rate
    if (result != EXR_ERR_SUCCESS) {
      return result;
    }
  }
  ...
  if (green) {
    result = exr_add_channel(
        exr, partidx, "G", pixel_type, EXR_PERCEPTUALLY_LOGARITHMIC, 1, 1);  // x & y sampling rate
    if (result != EXR_ERR_SUCCESS) {
      return result;
    }
  }
  ...
  exr_attr_chromaticities_t chroma = {0.6400f,
                                      0.3300f,  // red
                                      0.3000f,
                                      0.6000f,  // green
                                      0.1500f,
...
static bool strIsComponent(const char *layerName, const char *str, const char *component)
{
  if (!str || !component)
    return false;
...
static void nanoexr_cleanup(exr_context_t exr, exr_decode_pipeline_t *decoder)
{
  if (exr && decoder) {
...
static void tiled_exr_err_cb(exr_const_context_t f, int code, const char *msg)
{
  fprintf(stderr, "err_cb ERROR %d: %s\n", code, msg);
}

static exr_result_t _nanoexr_rgba_decoding_initialize(exr_context_t exr,
    ...
    for (int tileY = 0; tileY < yTiles; ++tileY) {
      for (int tileX = 0; tileX < xTiles; ++tileX) {
        ...
        if (decoder.channels == NULL) {
          rv = _nanoexr_rgba_decoding_initialize(
                                       ...
                                       int *rgbaIndex)
{
  ...
  do {
    ...
    for (int chunky = img->dataWindowMinY; chunky < img->dataWindowMaxY;
         chunky += scanLinesPerChunk)
    {
      exr_chunk_info_t cinfo;
      ...
      if (decoder.channels == NULL) {
        ...
        for (int c = 0; c < decoder.channel_count; ++c) {
          ...
          decoder.channels[c].user_line_stride = decoder.channels[c].user_pixel_stride *
                                                 img->width;
          decoder.channels[c].user_bytes_per_element = bytesPerChannel;
        }
...
void fill_channel_u16(nanoexr_ImageData_t *img, int channel, uint16_t value)
{
  for (int y = 0; y < img->height; ++y) {
    for (int x = 0; x < img->width; ++x) {
      uint8_t *curpixel = img->data + y * img->width * img->channelCount * 2 +
...
void fill_channel_u32(nanoexr_ImageData_t *img, int channel, uint32_t value)
{
  for (int y = 0; y < img->height; ++y) {
    for (int x = 0; x < img->width; ++x) {
      uint8_t *curpixel = img->data + y * img->width * img->channelCount * 4 +
...
void copy_channel_u16(nanoexr_ImageData_t *img,
                      uint8_t *dst_data,
                      uint8_t *src_data,
                      int dst_channel,
                      int src_channel)
...
void copy_channel_u32(nanoexr_ImageData_t *img,
                      uint8_t *dst_data,
                      uint8_t *src_data,
                      int dst_channel,
                      int src_channel)
{
  for (int y = 0; y < img->height; ++y) {
    for (int x = 0; x < img->width; ++x) {
      uint8_t *curpixel = src_data + y * img->width * img->channelCount * 4 +
                          x * img->channelCount * 4 + src_channel * 4;
      uint8_t *topixel = dst_data + y * img->width * img->channelCount * 4 +
...
void copy_channel_float(nanoexr_ImageData_t *img,
                        uint8_t *dst_data,
                        uint8_t *src_data,
                        int dst_channel,
                        int src_channel)
