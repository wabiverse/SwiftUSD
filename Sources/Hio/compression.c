                                 ...
                                 size_t *actual_out)
{
  ...
  struct libdeflate_options opt = {.sizeof_options = sizeof(struct libdeflate_options),
                                   .malloc_func = pctxt ? pctxt->alloc_fn : internal_exr_alloc,
                                   .free_func = pctxt ? pctxt->free_fn : internal_exr_free};

#else
  libdeflate_set_memory_allocator(pctxt ? pctxt->alloc_fn : internal_exr_alloc,
  ...
  if (comp) {
    size_t outsz;
    outsz = libdeflate_zlib_compress(comp, in, in_bytes, out, out_bytes_avail);
                                   ...
                                   size_t *actual_out)
{
  ...
  if (decomp) {
    res = libdeflate_zlib_decompress_ex(
        decomp, in, in_bytes, out, out_bytes_avail, &actual_in_bytes, actual_out);
