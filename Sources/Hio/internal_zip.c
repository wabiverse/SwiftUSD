...
static void reconstruct(uint8_t *buf, uint64_t outSize)
{
...
#else
static void reconstruct(uint8_t *buf, uint64_t sz)
{
  uint8_t *t = buf + 1;
  uint8_t *stop = buf + sz;
  while (t < stop) {
...
#ifdef IMF_HAVE_SSE2
static void interleave(uint8_t *out, const uint8_t *source, uint64_t outSize)
{
...
void internal_zip_deconstruct_bytes(uint8_t *scratch, const uint8_t *source, uint64_t count)
{
  int p;
  uint8_t *t1 = scratch;
...
static exr_result_t undo_zip_impl(exr_decode_pipeline_t *decode,
                                  ...
                                  uint64_t scratch_size)
{
  ...
  if (scratch_size < uncompressed_size)
    return EXR_ERR_INVALID_ARGUMENT;

  res = exr_uncompress_buffer(decode->context,
                              compressed_data,
                              comp_buf_size,
                              scratch_data,
                              scratch_size,
...
exr_result_t internal_exr_undo_zip(exr_decode_pipeline_t *decode,
                                   const void *compressed_data,
                                   uint64_t comp_buf_size,
                                   void *uncompressed_data,
                                   uint64_t uncompressed_size)
...
static exr_result_t apply_zip_impl(exr_encode_pipeline_t *encode)
{
  int level;
  size_t compbufsz;
  exr_result_t rv;

  rv = exr_get_zip_compression_level(encode->context, encode->part_index, &level);
  if (rv != EXR_ERR_SUCCESS)
    return rv;
  ...
  rv = exr_compress_buffer(encode->context,
                           level,
