...
TF_DEFINE_ENV_SETTING(USDIMAGINGGL_ENGINE_ENABLE_SCENE_INDEX,
                      false,
      ...
      _allowAsynchronousSceneProcessing(allowAsynchronousSceneProcessing)
{
  ...
  if (!SetRendererPlugin(!rendererPluginId.IsEmpty() ? rendererPluginId :
...
void UsdImagingGLEngine::_UpdateDomeLightCameraVisibility()
{
  ...
  // being on.
  const bool domeLightCamVisSetting = _renderDelegate->GetRenderSetting<bool>(
  ...
  if (_domeLightCameraVisibility != domeLightCamVisSetting) {
    ...
    SdfPathVector domeLights = _renderIndex->GetSprimSubtree(HdPrimTypeTokens->domeLight,
                                                             SdfPath::AbsoluteRootPath());
    for (SdfPathVector::iterator domeLightIt = domeLights.begin(); domeLightIt != domeLights.end();
         ++domeLightIt)
    {
...
void UsdImagingGLEngine::Render(const UsdPrim &root, const UsdImagingGLRenderParams &params)
{
  ...
  const SdfPathVector paths = {
      root.GetPath().ReplacePrefix(SdfPath::AbsoluteRootPath(), _sceneDelegateId)};
                                          ...
                                          IntersectionResultVector *outResults)
{
  ...
  for (HdxPickHit &hit : allHits) {
    ...
    if (_sceneDelegate) {
      ...
      res.hitInstancerPath = hit.instancerId.ReplacePrefix(_sceneDelegateId,
                                                           SdfPath::AbsoluteRootPath());
...
bool UsdImagingGLEngine::GetGPUEnabled() const
{
  return _gpuEnabled;
}
...
HdSceneIndexBaseRefPtr UsdImagingGLEngine::_AppendOverridesSceneIndices(
    HdSceneIndexBaseRefPtr const &inputScene)
{
  ...
  sceneIndex = _materialPruningSceneIndex = HdsiPrimTypePruningSceneIndex::New(
  ...
  sceneIndex = _lightPruningSceneIndex = HdsiPrimTypePruningSceneIndex::New(sceneIndex,
                                                                            lightPruningInputArgs);
...
void UsdImagingGLEngine::_SetRenderDelegate(HdPluginRenderDelegateUniqueHandle &&renderDelegate)
{
  ...
  // the unique id.
  const std::string renderInstanceId = TfStringPrintf("UsdImagingGLEngine_%s_%p",
  ...
  if (_GetUseSceneIndices()) {
    ...
    info.overridesSceneIndexCallback = std::bind(
        &UsdImagingGLEngine::_AppendOverridesSceneIndices, this, std::placeholders::_1);
    ...
    _sceneIndex = _displayStyleSceneIndex = HdsiLegacyDisplayStyleOverrideSceneIndex::New(
        _sceneIndex);
  ...
  _taskController = std::make_unique<HdxTaskController>(
      _renderIndex.get(), _ComputeControllerPath(_renderDelegate), _gpuEnabled);
