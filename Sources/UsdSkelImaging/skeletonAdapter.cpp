...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    // computation inputs
    (blendShapeOffsets)(blendShapeOffsetRanges)(numBlendShapeOffsetRanges)(blendShapeWeights)(skinningMethod)(geomBindXform)(hasConstantInfluences)(influences)(numInfluencesPerComponent)(primWorldToLocal)(restPoints)

        (skelLocalToWorld)(skinningXforms)(skinningScaleXforms)(skinningDualQuats)
    ...
    // gpu compute kernels
    (skinPointsLBSKernel)(skinPointsDQSKernel)(skinPointsSimpleKernel)

    // skel primvar names
    ((skelJointIndices, "skel:jointIndices"))((skelJointWeights, "skel:jointWeights"))(
        (skelSkinningMethod, "skel:skinningMethod"))((skelGeomBindXform, "skel:geomBindTransform"))
                                                ...
                                                const UsdImagingInstancerContext *instancerContext)
{
  ...
  if (bindingIt != _skelBindingMap.end()) {
    ...
    for (UsdSkelSkinningQuery const &query : binding.GetSkinningTargets()) {
      ...
      SdfPath skinnedPrimPath = ResolveCachePath(skinnedPrim.GetPath(), instancerContext);
                                                      ...
                                                      HdExtComputationContext *context)
{
  ...
  if (numInfluencesPerComponent.UncheckedGet<int>() > 0) {
    ...
    const _SkinnedPrimData *skinnedPrimData = _GetSkinnedPrimData(cachePath.GetParentPath());
    if (skinnedPrimData) {
      skinningMethodToken = skinnedPrimData->skinningQuery.GetSkinningMethod();
    }
    TF_DEBUG(USDIMAGING_COMPUTATIONS).Msg("Skinning Method: %s\n", skinningMethodToken.GetText());

    if (!hasConstantInfluences.UncheckedGet<bool>()) {

      UsdSkelSkinPoints(skinningMethodToken,
      ...
      if (UsdSkelSkinTransform(skinningMethodToken,
                               geomBindXform.UncheckedGet<GfMatrix4f>(),
                               skinningXforms.UncheckedGet<VtMatrix4fArray>(),
                               influences.UncheckedGet<VtVec2fArray>(),
                               &skinnedTransform))
    ...
    const UsdImagingInstancerContext *instancerContext) const
{
  ...
  if (_IsSkinningComputationPath(cachePath)) {
    ...
    SdfPath skinnedPrimPath = ResolveCachePath(prim.GetPath(), instancerContext);
    ...
    const UsdImagingInstancerContext *instancerContext) const
{
  if (_IsSkinnedPrimPath(cachePath)) {
    ...
    const SdfPath skinnedPrimPath = ResolveCachePath(prim.GetPath(), instancerContext);
...
bool _ExtractSkinningScaleXforms(const VtMatrix4fArray &skinningXforms,
                                 VtMatrix3fArray *skinningScaleXforms)
{
  ...
  for (size_t i = 0; i < skinningXforms.size(); ++i) {
    ...
    if (!matrix.Factor(&scaleOrientMat, &scale, &factoredRotMat, &translation, &perspMat)) {
      ...
      const GfMatrix4f tmpNonScaleXform = factoredRotMat *
                                          GfMatrix4f(1.0).SetTranslate(translation);
...
bool _ExtractSkinningDualQuats(const VtMatrix4fArray &skinningXforms,
    ...
    const UsdImagingInstancerContext *instancerContext) const
{
  ...
  SdfPath skinnedPrimCachePath = ResolveCachePath(prim.GetPath(), instancerContext);
  ...
  if (name == _tokens->skinningXforms || name == _tokens->skinningScaleXforms ||
      name == _tokens->skinningDualQuats || name == _tokens->skelLocalToWorld ||
      name == _tokens->blendShapeWeights)
  {
        ...
        name == _tokens->skinningDualQuats)
    {
      ...
      if (name == _tokens->skinningScaleXforms) {
        ...
        VtMatrix3fArray skinningScaleXforms;
        if (!TF_VERIFY(_ExtractSkinningScaleXforms(skinningXforms, &skinningScaleXforms)))
          return VtValue();
      ...
      if (name == _tokens->skinningDualQuats) {
        ...
        VtVec4fArray skinningDualQuats;
        if (!TF_VERIFY(_ExtractSkinningDualQuats(skinningXforms, &skinningDualQuats)))
          return VtValue();
    ...
    VtValue *sampleValues)
{
      ...
      name == _tokens->blendShapeWeights)
  {
    ...
    if (name == _tokens->skelLocalToWorld) {
      ...
      SdfPath skelCachePath = ResolveCachePath(skelPrim.GetPath(), instancerContext);
    ...
    VtValue *sampleValues)
{
  ...
  if (name == _tokens->skinningMethod) {
    ...
    const TfToken skinningMethod = skinnedPrimData->skinningQuery.GetSkinningMethod();
    ...
    const UsdImagingInstancerContext *instancerContext) const
{
  ...
  SdfPath skinnedPrimCachePath = ResolveCachePath(skinnedPrim.GetPath(), instancerContext);
...
VtVec3fArray UsdSkelImagingSkeletonAdapter::_GetSkinnedPrimPoints(
    const UsdPrim &skinnedPrim, const SdfPath &skinnedPrimCachePath, UsdTimeCode time) const
{
...
/* static */
std::string UsdSkelImagingSkeletonAdapter::_LoadSkinningComputeKernel(const TfToken &kernelKey)
{
  TRACE_FUNCTION();
...
/* static */
const std::string &UsdSkelImagingSkeletonAdapter::_GetLBSSkinningComputeKernel()
{
...
/* static */
const std::string &UsdSkelImagingSkeletonAdapter::_GetDQSSkinningComputeKernel()
{
  ...
  if (requestedBits & HdChangeTracker::DirtyPrimvar) {
    ...
    for (auto it = primvars.begin(); it != primvars.end();) {
      if (it->name == _tokens->skelJointIndices || it->name == _tokens->skelJointWeights ||
          it->name == _tokens->skelSkinningMethod || it->name == _tokens->skelGeomBindXform)
      {
