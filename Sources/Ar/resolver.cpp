...
std::pair<bool, std::string> _ValidateResourceIdentifierScheme(
    const std::string_view &caseFoldedScheme)
{
  ...
  const auto it = std::find_if(
      std::next(caseFoldedScheme.begin()), caseFoldedScheme.end(), [](const char c) {
  ...
  if (it != caseFoldedScheme.end()) {
    ...
    return std::make_pair(false,
                          TfStringPrintf("'%s' not allowed in scheme. "
  ...
  void _InitializeURIResolvers(const std::vector<_ResolverInfo> &availableResolvers)
  {
    ...
    for (const _ResolverInfo &resolverInfo : availableResolvers) {
      ...
      for (std::string uriScheme : resolverInfo.uriSchemes) {
        ...
        else if (const auto validation = _ValidateResourceIdentifierScheme(uriScheme);
                 !validation.first)
        {
          TF_WARN(
