...
fn main() {
  ...
  let bridge_files = vec!["Sources/Rust/main.rs"];
...
fn compile_swift() {
  let swift_package_dir = manifest_dir().join(".");
  ...
  cmd.current_dir(swift_package_dir)
    ...
    .args(&["-Xswiftc", "-o"])
    .args(&["-Xswiftc", swift_library_static_lib_dir().join("librust-usd.a").to_str().unwrap()])
    .args(&[
      "-Xswiftc",
      "-import-objc-header",
      "-Xswiftc",
...
fn swift_library_static_lib_dir() -> PathBuf {
  ...
  manifest_dir().join(format!(".build/{}", debug_or_release))
}
