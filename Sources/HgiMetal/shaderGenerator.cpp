...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (bufferBindings)(samplerBindings)(textureBindings));
...
class ShaderStageData final {
 public:
  ...
  HgiMetalShaderSectionPtrVector AccumulateBufferBindings(
      const HgiShaderFunctionBufferDescVector &buffers, HgiMetalShaderGenerator *generator);
  HgiMetalShaderSectionPtrVector AccumulateTextureBindings(
      const HgiShaderFunctionTextureDescVector &textures, HgiMetalShaderGenerator *generator);
                        ...
                        const std::string templateWrapper = std::string())
{
  ...
  HgiMetalStructTypeDeclarationShaderSection *const section =
      generator->CreateShaderSection<HgiMetalStructTypeDeclarationShaderSection>(
          typeName, members, templateWrapper);
      ...
      _inputs(_AccumulateParamsAndBlockParams(
          ...
          AccumulateParams(descriptor.stageInputs,
                           ...
                           descriptor.shaderStage == HgiShaderStageVertex ||
                               descriptor.shaderStage == HgiShaderStagePostTessellationControl ||
                               descriptor.shaderStage == HgiShaderStagePostTessellationVertex))),
    ...
    HgiShaderStage stage)
{
  ...
  for (const HgiShaderFunctionParamBlockDesc &p : params) {
    ...
    HgiMetalStructTypeDeclarationShaderSection *const blockStruct =
        generator->CreateShaderSection<HgiMetalStructTypeDeclarationShaderSection>(
            p.blockName + "_" + p.instanceName, blockMembers);

    HgiMetalInterstageBlockShaderSection *const blockSection =
        generator->CreateShaderSection<HgiMetalInterstageBlockShaderSection>(
            p.blockName, p.instanceName, blockStruct);
  ...
  for (int i = 0; i <= maxBindIndex; i++) {
    ...
    if (p) {
      section = generator->CreateShaderSection<HgiMetalBufferShaderSection>(
          p->nameInShader, _tokens->bufferBindings, p->type, p->binding, p->writable, attributes);
    }
    else {
      // Unused padding entry
      section = generator->CreateShaderSection<HgiMetalBufferShaderSection>(
          "_unused" + std::to_string(i), attributes);
    }
    stageShaderSections.push_back(section);
  }
  return stageShaderSections;
  ...
  for (size_t i = 0; i < textures.size(); i++) {
    ...
    HgiMetalSamplerShaderSection *const samplerSection =
        generator->CreateShaderSection<HgiMetalSamplerShaderSection>(
            texName, _tokens->samplerBindings, textures[i].arraySize, samplerAttributes);
...
{
  if (computeDesc.localSize[0] > 0 && computeDesc.localSize[1] > 0 && computeDesc.localSize[2] > 0)
  {
  ...
  switch (descriptor.shaderStage) {
    case HgiShaderStageVertex: {
      return std::make_unique<HgiMetalShaderStageEntryPoint>(
          stageData, this, "vsInput", "vsInput", "vertex", "vsInput", functionAttributesSS.str());
    }
    case HgiShaderStageFragment: {
      _BuildFragmentAttribute(functionAttributesSS, descriptor.fragmentDescriptor);

      return std::make_unique<HgiMetalShaderStageEntryPoint>(
          stageData, this, "fs", "Frag", "fragment", "vsOutput", functionAttributesSS.str());
    }
    ...
    case HgiShaderStagePostTessellationVertex: {
      ...
      return std::make_unique<HgiMetalShaderStageEntryPoint>(
          stageData, this, "tv", "TessVert", "vertex", "tvInput", functionAttributesSS.str());
    }
    case HgiShaderStagePostTessellationControl: {
      _BuildTessAttribute(functionAttributesSS, descriptor.tessellationDescriptor);
      return std::make_unique<HgiMetalShaderStageEntryPoint>(
          stageData, this, "ptc", "TessControl", "vertex", "tcInput", functionAttributesSS.str());
    }
    default: {
