...
void HgiMetalResourceBindings::BindResources(
    HgiMetal *hgi,
    ...
    MTL::Buffer *argBuffer)
{
  ...
  for (HgiTextureBindDesc const &texDesc : _descriptor.textures)
  {
    ...
    if ((texDesc.stageUsage & HgiShaderStageVertex) || texDesc.stageUsage & HgiShaderStagePostTessellationVertex)
    {
      size_t offsetSampler = HgiMetalArgumentOffsetSamplerVS + (texDesc.bindingIndex * sizeof(void *));
      argEncoderSampler->setArgumentBuffer(argBuffer, offsetSampler);
      argEncoderSampler->setSamplerState(metalSampler, 0);

      size_t offsetTexture = HgiMetalArgumentOffsetTextureVS + (texDesc.bindingIndex * sizeof(void *));
    ...
    {
      size_t offsetSampler = HgiMetalArgumentOffsetSamplerFS + (texDesc.bindingIndex * sizeof(void *));
      argEncoderSampler->setArgumentBuffer(argBuffer, offsetSampler);
      argEncoderSampler->setSamplerState(metalSampler, 0);

      size_t offsetTexture = HgiMetalArgumentOffsetTextureFS + (texDesc.bindingIndex * sizeof(void *));
      argEncoderTexture->setArgumentBuffer(argBuffer, offsetTexture);
      argEncoderTexture->setTexture(metalTexture, 0);
    }
    if (metalTexture)
  ...
  for (HgiBufferBindDesc const &bufDesc : _descriptor.buffers)
  {
        ...
        (bufDesc.stageUsage & HgiShaderStagePostTessellationVertex))
    {
      ...
      NS::UInteger argBufferOffset = HgiMetalArgumentOffsetBufferVS + bufDesc.bindingIndex * sizeof(void *);
    ...
    {
      NS::UInteger argBufferOffset = HgiMetalArgumentOffsetBufferFS + bufDesc.bindingIndex * sizeof(void *);
      argEncoderBuffer->setArgumentBuffer(argBuffer, argBufferOffset);
      argEncoderBuffer->setBuffer(bufferId, offset, 0);
    }
    MTL::ResourceUsage usageFlags = MTL::ResourceUsageRead;
    ...
    MTL::Buffer *argBuffer)
{
  ...
  for (HgiTextureBindDesc const &texDesc : _descriptor.textures)
  {
    ...
    if (texDesc.stageUsage & HgiShaderStageCompute)
    {
      ...
      size_t offsetTexture = HgiMetalArgumentOffsetTextureCS + (texDesc.bindingIndex * sizeof(void *));
      argEncoderTexture->setArgumentBuffer(argBuffer, offsetTexture);
  ...
  for (HgiBufferBindDesc const &bufDesc : _descriptor.buffers)
  {
    ...
    MTL::Buffer *bufferId = metalbuffer->GetBufferId();
    NS::UInteger offset = bufDesc.offsets.front();
    size_t argBufferOffset = HgiMetalArgumentOffsetBufferCS + bufDesc.bindingIndex * sizeof(void *);
    argEncoderBuffer->setArgumentBuffer(argBuffer, argBufferOffset);
    argEncoderBuffer->setBuffer(bufferId, offset, 0);
    MTL::ResourceUsage usage = MTL::ResourceUsageRead;
    if (bufDesc.writable)
