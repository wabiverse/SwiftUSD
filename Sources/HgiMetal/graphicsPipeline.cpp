...
static void _SetTessellationStateFromShaderFunctions(
    MTL::RenderPipelineDescriptor *stateDesc, HgiShaderFunctionHandleVector const &shaderFuncs)
{
...
void HgiMetalGraphicsPipeline::_CreateRenderPipelineState(HgiMetal *hgi)
{
  ...
  bool const icbSupport = hgi->GetCapabilities()->IsSet(
...
void HgiMetalGraphicsPipeline::_CreateDepthStencilState(HgiMetal *hgi)
{
...
void HgiMetalGraphicsPipeline::BindPipeline(MTL::RenderCommandEncoder *renderEncoder)
{
  ...
  if (_descriptor.primitiveType == HgiPrimitiveTypePatchList) {
    if (_constantTessFactors == nullptr) {
      ...
      if (_descriptor.tessellationState.patchType == HgiTessellationState::PatchType::Triangle) {
        ...
        _constantTessFactors = renderEncoder->device()->newBuffer(
            &triangleFactors, sizeof(triangleFactors), MTL::ResourceStorageModeShared);
      }
        ...
        _constantTessFactors = renderEncoder->device()->newBuffer(
            &quadFactors, sizeof(quadFactors), MTL::ResourceStorageModeShared);
      }
    }
  ...
  if (_descriptor.depthState.depthBiasEnabled) {
    renderEncoder->setDepthBias(
        dsState.depthBiasConstantFactor, dsState.depthBiasSlopeFactor, 0.0f);
  }

  if (_descriptor.depthState.stencilTestEnabled) {
    renderEncoder->setStencilReferenceValues(dsState.stencilFront.referenceValue,
