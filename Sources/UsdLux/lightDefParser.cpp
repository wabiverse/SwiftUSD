...
/*static*/
const UsdLux_LightDefParserPlugin::ShaderIdToAPITypeNameMap &UsdLux_LightDefParserPlugin::
...
NdrNodeUniquePtr UsdLux_LightDefParserPlugin::Parse(const NdrNodeDiscoveryResult &discoveryResult)
{
  ...
  const TfToken &primTypeName = (shaderIdToAPITypeNameMap.find(discoveryResult.identifier) ==
  ...
  const TfTokenVector schemas(
      {_tokens->LightAPI, primTypeName, _tokens->ShadowAPI, _tokens->ShapingAPI});
