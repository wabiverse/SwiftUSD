...
UsdRelationship UsdShadeMaterialBindingAPI::GetCollectionBindingRel(
    const TfToken &bindingName, const TfToken &materialPurpose) const
{
...
/* static */
bool UsdShadeMaterialBindingAPI::CollectionBinding::IsCollectionBindingRel(
...
bool UsdShadeMaterialBindingAPI::RemovePrimFromBindingCollection(
    const UsdPrim &prim, const TfToken &bindingName, const TfToken &materialPurpose) const
{
                                                           ...
                                                           bool supportLegacyBindings)
{
  ...
  {
    TF_WARN(
        "Found material bindings on prim at path (%s) but "
...
/* static */
const SdfPath UsdShadeMaterialBindingAPI::GetResolvedTargetPathFromBindingRel(
    const UsdRelationship &bindingRel)
{
  if (!bindingRel) {
    return SdfPath();
  ...
  return UsdShadeMaterialBindingAPI::CollectionBinding::IsCollectionBindingRel(bindingRel) ?
             targetPaths[1] :
             targetPaths[0];
}
  ...
  for (auto const &purpose : materialPurposes) {
    ...
    for (UsdPrim p = GetPrim(); !p.IsPseudoRoot(); p = p.GetParent()) {
      ...
      if (directBindingPtr && directBindingPtr->GetMaterialPurpose() == purpose) {
        ...
        if (!hasValidTargetPath || (GetMaterialBindingStrength(directBindingRel) ==
    ...
    if (hasValidTargetPath) {
      ...
      if (boundMaterial.GetPrim()) {
        return (boundMaterial.GetPrim().IsA<UsdShadeMaterial>()) ? boundMaterial :
                                                                   UsdShadeMaterial();
      }
