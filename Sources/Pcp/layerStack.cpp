...
class Pcp_ComputeRelocationsForLayerStackWorkspace {
 ...
 public:
  ...
  // map.
  using PathToProcessedRelocateMap =
      std::unordered_map<SdfPath, ProcessedRelocatesMap::value_type *, TfHash>;
 ...
 private:
  ...
  void _CollectRelocates(const SdfLayerRefPtr &layer,
    ...
    for (const auto &[sourcePath, targetPath] : relocates) {
      ...
      if (_IsValidRelocatesEntryForComposing(source, target, &errorMessage)) {
        ...
        processedRelocates.try_emplace(
            std::move(source), std::move(target), SdfSite(layer, primPath));
      }
      else {
  ...
  void _ConformLegacyRelocates()
  {
    ...
    for (auto &[oldSource, newSource] : relocateSourcesToMove) {
          ...
          !success && it->second.targetPath != oldIt->second.targetPath)
      {
        ...
        TF_WARN(
            "Couldn't conform relocate from %s to %s to use the "
  ...
  void _ValidateAndRemoveConflictingRelocates()
  {
    ...
    for (auto &relocatesEntry : processedRelocates) {
      ...
      if (!targetPath.IsEmpty()) {
        ...
        if (auto [it, success] = targetPathToProcessedRelocateMap.emplace(targetPath,
                                                                          &relocatesEntry);
            !success)
        {
      ...
      if (!targetPath.IsEmpty()) {
        ...
        if (auto it = processedRelocates.find(targetPath); it != processedRelocates.end()) {
          _LogInvalidConflictingRelocate(
              relocatesEntry, *it, ConflictReason::TargetIsConflictSource);
          _LogInvalidConflictingRelocate(
              *it, relocatesEntry, ConflictReason::SourceIsConflictTarget);
        }
             ...
             pathToCheck = pathToCheck.GetParentPath())
        {
          if (auto it = processedRelocates.find(pathToCheck); it != processedRelocates.end()) {
            _LogInvalidConflictingRelocate(
                relocatesEntry, *it, ConflictReason::TargetIsConflictSourceDescendant);
          }
        }
      }
           ...
           pathToCheck = pathToCheck.GetParentPath())
      {
        if (auto it = processedRelocates.find(pathToCheck); it != processedRelocates.end()) {
          _LogInvalidConflictingRelocate(
              relocatesEntry, *it, ConflictReason::SourceIsConflictSourceDescendant);
        }
      }
    }
    ...
    // platforms.
    std::sort(_invalidConflictingRelocates.begin(),
  ...
  void _ComputeSourceOriginForTargetIfNeeded(
  ...
  void _LogInvalidConflictingRelocate(
  ...
  bool _LogInvalidSameTargetRelocates(const ProcessedRelocatesMap::value_type &entry)
  {
    ...
    const auto [it, createNewError] = _invalidSameTargetRelocates.try_emplace(
    ...
    err->sources.push_back(
        {entry.first, entry.second.owningSite.layer, entry.second.owningSite.path});
...
PcpLayerStack::PcpLayerStack(const PcpLayerStackIdentifier &identifier,
    ...
    : _identifier(identifier),
      _expressionVariables([&identifier, &registry]() {
        ...
        const PcpLayerStackPtr overrideLayerStack = overrideLayerStackId != selfId ?
                                                        registry.Find(overrideLayerStackId) :
                                                        TfNullPtr;
        ...
        if (overrideLayerStack) {
          composedExpressionVars = PcpExpressionVariables::Compute(
              selfId, rootLayerStackId, &overrideLayerStack->GetExpressionVariables());
...
void PcpLayerStack::Apply(const PcpLayerStackChanges &changes, PcpLifeboat *lifeboat)
{
      ...
      changes._didChangeExpressionVariablesSource)
  {
    ...
    const auto updateExpressionVariables = [&](const VtDictionary &newExprVars,
      ...
      const PcpLayerStackIdentifier &newSourceId = newSource.ResolveLayerStackIdentifier(
    ...
    if (changes.didChangeSignificantly) {
      const PcpExpressionVariables newExprVars = PcpExpressionVariables::Compute(
      ...
      updateExpressionVariables(
          changes.didChangeExpressionVariables ? changes.newExpressionVariables :
                                                 _expressionVariables->GetVariables(),
          changes._didChangeExpressionVariablesSource ? changes._newExpressionVariablesSource :
  ...
  if (changes.didChangeLayers || changes.didChangeLayerOffsets) {
    ...
    if (changes.didChangeSignificantly) {
      ...
      errors.insert(
          errors.end(), changes.newRelocatesErrors.begin(), changes.newRelocatesErrors.end());
    }
                             ...
                             const Pcp_MutedLayers &mutedLayers)
{
  ...
  if (_identifier.sessionLayer) {
                                 ...
                                 &canonicalMutedPath))
    {
      ...
      _sessionLayerTree = _BuildLayerStack(_identifier.sessionLayer,
                                           sessionLayerOffset,
                                           sessionTcps,
    ...
    PcpErrorVector *errors)
{
  ...
  for (size_t i = 0; i != numSublayers; ++i) {
    if (Pcp_IsVariableExpression(sublayers[i])) {
      sublayers[i] = Pcp_EvaluateVariableExpression(sublayers[i],
                                                    *_expressionVariables,
                                                    "sublayer",
                                                    layer,
                                                    SdfPath::AbsoluteRootPath(),
                                                    &_expressionVariableDependencies,
                                                    errors);
