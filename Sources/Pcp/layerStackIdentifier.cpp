...
PcpLayerStackIdentifier::PcpLayerStackIdentifier(
    const SdfLayerHandle &rootLayer_, const SdfLayerHandle &sessionLayer_,
    const ArResolverContext &pathResolverContext_,
    const PcpExpressionVariablesSource &expressionVariablesOverrideSource_)
    : rootLayer(rootLayer_), sessionLayer(sessionLayer_),
...
PcpLayerStackIdentifier::operator=(const PcpLayerStackIdentifier &rhs) {
  if (this != &rhs) {
    ...
    const_cast<PcpExpressionVariablesSource &>(
...
bool PcpLayerStackIdentifier::operator==(const This &rhs) const {
  return std::tie(_hash, rootLayer, sessionLayer, pathResolverContext,
                  expressionVariablesOverrideSource) ==
         std::tie(rhs._hash, rhs.rootLayer, rhs.sessionLayer,
                  rhs.pathResolverContext,
                  rhs.expressionVariablesOverrideSource);
}

bool PcpLayerStackIdentifier::operator<(const This &rhs) const {
  return std::tie(sessionLayer, rootLayer, pathResolverContext,
                  expressionVariablesOverrideSource) <
...
static std::string Pcp_FormatIdentifier(std::ostream &os,
...
static void _PrintIdentifier(std::ostream &s,
