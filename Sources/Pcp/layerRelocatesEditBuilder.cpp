...
PcpLayerRelocatesEditBuilder::PcpLayerRelocatesEditBuilder(
    const PcpLayerStackPtr &layerStack, const SdfLayerHandle &addNewRelocatesLayer)
{
  if (!layerStack) {
  ...
  for (const auto &layer : layerStack->GetLayers()) {
    ...
    const bool layerHasRelocates = layer->HasField(
        SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates, &layerRelocates);
...
void PcpLayerRelocatesEditBuilder::_RemoveRelocatesWithErrors(const PcpErrorVector &errors)
{
  ...
  if (!relocatesToDeletePerLayer.empty()) {
    for (auto &[layer, relocates] : _layerRelocatesEdits) {
      ...
      relocates.erase(
          std::remove_if(relocates.begin(),
                         relocates.end(),
                         [&](const auto &relocate) { return relocatesToDelete->count(relocate); }),
          relocates.end());
    }
  }
  ...
  if (!relocateSourcePathsToDelete.empty()) {
    for (auto &[layer, relocates] : _layerRelocatesEdits) {
      ...
      const auto eraseIt = std::remove_if(
          relocates.begin(), relocates.end(), [&](const auto &relocate) {
...
static void _PrintToWhyNot(std::string *whyNot,
...
void PcpLayerRelocatesEditBuilder::_UpdateExistingRelocates(const SdfPath &source,
                                                            const SdfPath &target)
{
  ...
  for (auto &[layer, relocates] : _layerRelocatesEdits) {
    ...
    for (auto &[existingSource, existingTarget] : relocates) {
      ...
      if (existingSource.HasPrefix(source)) {
        existingSource = existingSource.ReplacePrefix(source, target);
        _layersWithRelocatesChanges.insert(layer);
      }
      // If the existing target source would be ancestrally relocated by
      // the new relocate, apply the relocate to it.
      if (existingTarget.HasPrefix(source)) {
        existingTarget = existingTarget.ReplacePrefix(source, target);
        _layersWithRelocatesChanges.insert(layer);
      }
    }
    ...
    relocates.erase(
        std::remove_if(relocates.begin(),
                       relocates.end(),
                       [](const auto &relocate) { return relocate.first == relocate.second; }),
        relocates.end());
  }
}

bool PcpLayerRelocatesEditBuilder::Relocate(const SdfPath &newSource,
                                            const SdfPath &newTarget,
                                            std::string *whyNot)
{
