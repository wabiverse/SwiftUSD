...
_GetLayerStackSitesForEdit(PcpNamespaceEdits *result, const SdfPath &oldPath,
...
static bool
_RelocatesMapContainsPrimOrDescendant(const SdfRelocatesMapProxy &reloMap,
...
static void _AddRelocateEditsForLayerStack(PcpNamespaceEdits *result,
  ...
  TF_FOR_ALL(pathIt, relocatePrimPaths) {
    TF_FOR_ALL(layerIt, layerStack->GetLayers()) {
      ...
      if (_RelocatesMapContainsPrimOrDescendant(prim->GetRelocates(),
                                                oldRelocatePath)) {

        PcpNamespaceEdits::LayerStackSites &layerStackSites =
            _GetLayerStackSitesForEdit(result, oldRelocatePath,
                                       newRelocatePath);
...
static SdfPath _TranslatePathAndTargetPaths(const PcpNodeRef &node,
...
static void _TranslatePathsAndEditRelocates(PcpNamespaceEdits *result,
  ...
  if (i != relocates.end()) {
    ...
    if (newParentPath.IsEmpty()) {
      ...
      _AddRelocateEditsForLayerStack(result, layerStack, cacheIndex,
...
static bool _AddLayerStackSite(PcpNamespaceEdits *result,
                               const PcpNodeRef &node, size_t cacheIndex,
                               SdfPath *oldNodePath, SdfPath *newNodePath) {
  bool final = false;
  ...
  _TranslatePathsAndEditRelocates(result, node, cacheIndex, oldNodePath,
                                  newNodePath);
...
PcpComputeNamespaceEdits(const PcpCache *primaryCache,
  ...
  if (primSites.empty()) {
    ...
    // relocatesLayer.
    for (size_t cacheIndex = 0, n = caches.size(); cacheIndex != n;
         ++cacheIndex) {
      ...
      PcpDependencyVector deps = cache->FindSiteDependencies(
          relocatesLayer, primPath, PcpDependencyTypeAnyNonVirtual,
          /* recurseOnSite */ true,
          /* recurseOnIndex */ true,
          /* filter */ true);
      ...
      for (const PcpDependency &dep : deps) {
        Pcp_ForEachDependentNode(dep.sitePath, relocatesLayer, dep.indexPath,
                                 *cache, visitNodeFn);
      }
    }
  } else {
    ...
    for (size_t cacheIndex = 0, n = caches.size(); cacheIndex != n;
      ...
      for (const SdfSite &primSite : primSites) {
        PcpDependencyVector deps = cache->FindSiteDependencies(
            primSite.layer, primPath, PcpDependencyTypeAnyNonVirtual,
            /* recurseOnSite */ false,
        ...
        auto visitNodeFn = [&](const SdfPath &depIndexPath,
          ...
          TF_DEBUG(PCP_NAMESPACE_EDIT)
              .Msg(" found dep node: <%s> -> <%s> %s\n", depIndexPath.GetText(),
                   ...
                   PcpDependencyFlagsToString(PcpClassifyNodeDependency(node))
                       .c_str());
          _CacheNodeHelper::InsertCacheNodePair(cacheIndex, node, &nodes);
        };
        for (const PcpDependency &dep : deps) {
          Pcp_ForEachDependentNode(dep.sitePath, primSite.layer, dep.indexPath,
                                   *cache, visitNodeFn);
        }
      }
      ...
      if (cache == primaryCache && curPath.IsPrimPath()) {
        ...
        for (const PcpDependency &dep : primaryCache->FindSiteDependencies(
          ...
          if (TF_VERIFY(index, "Reported descendent dependency "
            ...
            for (const PcpNodeRef &node :
              ...
              if (node.GetLayerStack() == primaryLayerStack &&
                  !node.GetPath().IsRootPrimPath() && !node.IsDueToAncestor()) {
  ...
  for (const auto &cacheAndNode : nodes) {
    ...
    TF_DEBUG(PCP_NAMESPACE_EDIT)
        .Msg("\n processing node:\n"
             ...
             "  oldNodePath:     <%s>\n"
             "  newNodePath:     <%s>\n",
             TfStringify(caches[cacheIndex]->GetLayerStack()->GetIdentifier())
                 .c_str(),
             ...
             curPath.GetText(), newPath.GetText(), oldNodePath.GetText(),
             newNodePath.GetText());
    ...
    if (sites.insert(node.GetSite()).second) {
      ...
      _AddRelocateEditsForLayerStack(&result, node.GetLayerStack(), cacheIndex,
    ...
    while (node.GetParentNode()) {
      TF_DEBUG(PCP_NAMESPACE_EDIT)
          .Msg("  - traverse to parent of <%s>.  <%s> -> <%s>\n",
               node.GetPath().GetText(), oldNodePath.GetText(),
               newNodePath.GetText());
      ...
      if (_AddLayerStackSite(newSite ? &result : nullptr, node, cacheIndex,
                             &oldNodePath, &newNodePath)) {
  ...
  if (newPath.IsEmpty() && curPath.IsPrimPath()) {
    ...
    for (size_t cacheIndex = 0, n = caches.size(); cacheIndex != n;
      ...
      for (const PcpLayerStackPtr &layerStack : layerStacks) {
        PcpDependencyVector deps = cache->FindSiteDependencies(
            layerStack, primPath, PcpDependencyTypeAnyNonVirtual,
            /* recurseOnSite */ true,
            /* recurseOnIndex */ true,
            /* filter */ true);
        auto visitNodeFn = [&](const SdfPath &depIndexPath,
        ...
        for (const PcpDependency &dep : deps) {
          ...
          if (PcpComposeSiteHasPrimSpecs(layerStack, dep.sitePath)) {
            Pcp_ForEachDependentNode(dep.sitePath, layerStack, dep.indexPath,
                                     *cache, visitNodeFn);
          }
        }
      }
      ...
      TF_FOR_ALL(i, descendantPathsAndNodes) {
        ...
        for (; node; node = node.GetParentNode()) {
          SdfPath path =
              descendantPath.ReplacePrefix(descendantPrimPath, node.GetPath());
  ...
  if (TfDebug::IsEnabled(PCP_NAMESPACE_EDIT)) {
    TF_DEBUG(PCP_NAMESPACE_EDIT)
        .Msg("PcpComputeNamespaceEdits():\n"
             ...
             "  newPath: <%s>\n",
             TfStringify(primaryLayerStack->GetIdentifier()).c_str(),
             curPath.GetText(), newPath.GetText());
