...
#define PCP_DEFINE_RANGE(Range, Iterator, ReverseIterator)                     \
  ...
  template <> struct Tf_IteratorInterface<Range, true> {                       \
    ...
    static IteratorType Begin(Range &c) { return IteratorType(c.second); }     \
    static IteratorType End(Range &c) { return IteratorType(c.first); }        \
  };                                                                           \
                                                                               \
  template <> struct Tf_IteratorInterface<const Range, true> {                 \
    ...
    static IteratorType End(Range const &c) { return IteratorType(c.first); }  \
  };                                                                           \
                                                                               \
  template <> struct Tf_ShouldIterateOverCopy<Range> : std::true_type {};      \
                                                                               \
  template <> struct Tf_ShouldIterateOverCopy<const Range> : std::true_type {}
