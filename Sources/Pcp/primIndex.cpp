                                                   ...
                                                   const SdfLayerOffset &offset = SdfLayerOffset())
{
  ...
  if (PcpMapExpression reloMapExpr = targetNode.GetLayerStack()->GetExpressionForRelocatesAtPath(
          targetPath);
      !reloMapExpr.IsNull())
  {
...
inline static size_t _ScanAncestralArcs(PcpNodeRef const &node)
{
  ...
  if (const size_t restrictedDepth = node.GetSpecContributionRestrictedDepth();
      restrictedDepth != 0)
  {
...
struct Task {
  ...
  // TfHash support.
  template<class HashState> friend void TfHashAppend(HashState &h, Task const &task)
  {
  ...
  {
    return type == rhs.type && node == rhs.node && vsetPath == rhs.vsetPath &&
  ...
  friend std::ostream &operator<<(std::ostream &os, Task const &task)
  {
    ...
    if (!task.vsetName.empty()) {
      os << TfStringPrintf(", vsetPath=%s, vsetName=%s, vsetNum=%d",
                           task.vsetPath.GetText(),
                           task.vsetName.c_str(),
                           task.vsetNum);
    }
    return os << ")";
...
struct Pcp_PrimIndexer {
  ...
  using _VariantTraversalCaches =
      std::unordered_map<std::pair<PcpNodeRef, SdfPath>, _VariantTraversalCache, TfHash>;
  ...
  _VariantTraversalCache &GetVariantTraversalCache(PcpNodeRef const &node,
  ...
  static inline SdfPath _MapPathToNodeParentPayloadInclusionPath(
      PcpMapExpression const &mapToParentExpr, PcpArcType arcType, SdfPath const &path)
  {
    ...
    const bool isInternalReferenceOrPayload = mapToParent.HasRootIdentity() &&
    ...
    if (isInternalReferenceOrPayload) {
      ...
      PcpMapFunction newMapFunction = PcpMapFunction::Create(sourceToTargetMap,
                                                             mapToParent.GetTimeOffset());
  ...
  // prim index root.
  static inline SdfPath _MapPathToNodeRootPayloadInclusionPath(PcpNodeRef const &node,
                                                               SdfPath const &path)
  {
  ...
  SdfPath MapNodePathToPayloadInclusionPath(PcpNodeRef const &node, SdfPath const &path) const
  {
         ...
         it.NextFrame())
    {
      ...
      p = _MapPathToNodeParentPayloadInclusionPath(
          it.previousFrame->arcToParent->mapToParent, it.previousFrame->arcToParent->type, p);
                          ...
                          _ArcOptions options = _ArcOptions())
{
  ...
  return _AddArc(
      indexer, arcType, parent, origin, site, mapExpr, arcSiblingNum, namespaceDepth, options);
}
                                                            ...
                                                            const std::string &fileFormatTarget)
{
  ...
  if (!assetPath.empty()) {
    SdfFileFormatConstPtr fileFormat = SdfFileFormat::FindByExtension(
        SdfFileFormat::GetFileExtension(assetPath), fileFormatTarget);
    if (fileFormat) {
...
{
  const PcpDynamicFileFormatInterface *dynamicFileFormat = _GetDynamicFileFormat(
      payload, indexer.inputs.fileFormatTarget);
  ...
  for (size_t i = 0; i < arcs.size(); ++i) {
    ...
    if (isInternal) {
      ...
      _ComposeFieldsForFileFormatArguments(
          node, *indexer, refOrPayload, nodePathAtIntroduction, info.arcNum, &args);
                              ...
                              const SdfPath &nodePathAtIntroduction)
{
  ...
  PcpComposeSitePayloads(node.GetLayerStack(),
                         nodePathAtIntroduction,
                         &payloadArcs,
                         &payloadInfo,
                         &exprVarDependencies,
  ...
  while (payloadIt != payloadArcs.end()) {
    const bool isDynamicPayload = _GetDynamicFileFormat(*payloadIt,
...
static bool _PrimSpecExistsUnderNodeAtIntroduction(const PcpNodeRef &node,
                                                   Pcp_PrimIndexer *indexer)
{
  ...
  return node.GetDepthBelowIntroduction() == 0 ?
             ...
             _PrimSpecExistsUnderNode(node, [](const PcpNodeRef &node) {
               return PcpComposeSiteHasPrimSpecs(node.GetLayerStack(),
                                                 node.GetPathAtIntroduction());
             });
}
...
static void _EvalUnresolvedPrimPathError(const PcpNodeRef &node, Pcp_PrimIndexer *indexer)
{
  ...
  if (!_PrimSpecExistsUnderNodeAtIntroduction(node, indexer)) {
    ...
    if (const SdfPath parentPath = parentNode.GetPath(); parentPath.ContainsPrimVariantSelection())
    {
      ...
      parentNodePath = parentNodePath.ReplacePrefix(parentPath.StripAllVariantSelections(),
                                                    parentPath);
    }
...
static void _EvalNodeRelocations(const PcpNodeRef &node, Pcp_PrimIndexer *indexer)
{
  ...
  PcpNodeRef newNode = _AddArc(indexer,
                               PcpArcTypeRelocate,
                               /* parent = */ node,
                               /* origin = */ node,
                                    ...
                                    Pcp_PrimIndexer *indexer)
{
  ...
  //
  opts.directNodeShouldContributeSpecs = (inheritPath != parent.GetPath()) &&
  ...
  opts.includeAncestralOpinions = opts.directNodeShouldContributeSpecs &&
                                  !inheritPath.IsRootPrimPath();
                                  ...
                                  Pcp_PrimIndexer *indexer)
{
  ...
  if (destNode.GetArcType() == PcpArcTypeRelocate) {
    ...
    _EvalImpliedClassTree(
        destNode.GetParentNode(), srcNode, newTransferFunc, srcNodeIsStartOfTree, indexer);
  ...
  for (const PcpNodeRef &srcChild : Pcp_GetChildren(srcNode)) {
    ...
    if (destChild && _HasClassBasedChild(srcChild)) {
      ...
      _EvalImpliedClassTree(destChild,
                            srcChild,
                            childTransferFunc,
...
static void _EvalImpliedClasses(PcpNodeRef node, Pcp_PrimIndexer *indexer)
{
  ...
  _EvalImpliedClassTree(node.GetParentNode(),
                        node,
                        transferFunc,
                                            ...
                                            Pcp_PrimIndexer *indexer)
{
  ...
  for (PcpNodeRef childNode : Pcp_GetChildren(srcNode)) {
    if (!PcpIsSpecializeArc(childNode.GetArcType())) {
      _PropagateSpecializesTreeToRoot(newNode.first,
                                      childNode,
                                      newNode.first,
...
static void _FindSpecializesToPropagateToRoot(PcpNodeRef node, Pcp_PrimIndexer *indexer)
{
  ...
  if (PcpIsSpecializeArc(node.GetArcType())) {
    ...
    _PropagateSpecializesTreeToRoot(
        node.GetRootNode(), node, node, node.GetMapToRoot(), node, indexer);
                                   ...
                                   Pcp_PrimIndexer *indexer)
{
  ...
  for (PcpNodeRef childNode : Pcp_GetChildren(srcNode)) {
    _PropagateArcsToOrigin(
        newNode.first, childNode, childNode.GetMapToParent(), srcTreeRoot, indexer);
  }
}
...
static void _FindArcsToPropagateToOrigin(const PcpNodeRef &node, Pcp_PrimIndexer *indexer)
{
  ...
  for (PcpNodeRef childNode : Pcp_GetChildren(node)) {
    ...
    _PropagateArcsToOrigin(
        node.GetOriginNode(), childNode, childNode.GetMapToParent(), node, indexer);
  }
}
...
static bool _NodeCanContributeAncestralOpinions(const PcpNodeRef &node,
                                            ...
                                            Pcp_PrimIndexer *indexer)
{
  ...
  const bool foundSelection = PcpComposeSiteVariantSelection(
      node.GetLayerStack(), pathInNode, vset, vsel, &exprVarDependencies, &errors);
                                     ...
                                     PcpNodeRef *nodeWithVsel)
{
  ...
  const auto [pathInStartNode,
              startNode] = Pcp_TranslatePathFromNodeToRootOrClosestNode(node, pathInNode);
                                   ...
                                   bool isAncestral)
{
  ...
  const Task::Type variantTaskType = (isAncestral ? Task::Type::EvalNodeAncestralVariantAuthored :
...
static void _EvalNodeAncestralDynamicPayloads(const PcpNodeRef &node, Pcp_PrimIndexer *indexer)
{
  ...
  for (SdfPath path = node.GetPath().GetParentPath(); !path.IsAbsoluteRootPath();
                                     ...
                                     bool isAncestral)
{
  ...
  _ComposeVariantSelection(
      node, vsetPath.StripAllVariantSelections(), indexer, vset, &vsel, &nodeWithVsel);
  ...
  if (_ShouldUseVariantFallback(indexer, vset, vsel, vselFallback, nodeWithVsel)) {
    ...
    indexer->AddTask(Task((isAncestral ? Task::Type::EvalNodeAncestralVariantFallback :
  ...
  if (vsel.empty()) {
    ...
    indexer->AddTask(Task((isAncestral ? Task::Type::EvalNodeAncestralVariantNoneFound :
...
template<> auto _GetSourceArcs<PcpArcTypeReference>(const PcpNodeRef &node, PcpArcInfoVector *info)
{
...
template<> auto _GetSourceArcs<PcpArcTypePayload>(const PcpNodeRef &node, PcpArcInfoVector *info)
{
...
static bool _CullMatchingChildrenInSubtree(
    PcpNodeRef node, const std::unordered_set<PcpLayerStackSite, TfHash> &culledSites)
{
...
static void _CullSubtreesWithNoOpinionsHelper(
    PcpNodeRef node,
    const PcpLayerStackSite &rootSite,
  ...
  while (tasksAreLeft) {
    ...
    switch (task.type) {
      ...
      case Task::Type::EvalNodeDynamicPayloads:
        _EvalNodePayloads(
            task.node, &indexer, Task::Type::EvalNodeDynamicPayloads, task.node.GetPath());
        break;
      ...
      case Task::Type::EvalNodeAncestralVariantAuthored:
        _EvalNodeAuthoredVariant(task.node,
                                 &indexer,
                                 task.vsetPath,
                                 task.vsetName,
                                 task.vsetNum,
      ...
      case Task::Type::EvalNodeAncestralVariantFallback:
        _EvalNodeFallbackVariant(task.node,
                                 &indexer,
                                 task.vsetPath,
                                 task.vsetName,
                                 task.vsetNum,
                                         ...
                                         PcpTokenSet *prohibitedNameSet) const
{
  ...
  if (IsInstanceable()) {
    _ComposePrimChildNamesForInstance(*this, nameOrder, &nameSet, prohibitedNameSet);
  }
  else {
    _ComposePrimChildNames(*this, GetRootNode(), nameOrder, &nameSet, prohibitedNameSet);
  }
