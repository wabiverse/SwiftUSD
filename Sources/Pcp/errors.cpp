...
std::string PcpErrorInconsistentPropertyType::ToString() const {
  return TfStringPrintf(
      ...
      rootSite.path.GetString().c_str(), definingLayerIdentifier.c_str(),
      definingSpecPath.GetString().c_str(),
      (definingSpecType == SdfSpecTypeAttribute ? "an attribute"
                                                : "a relationship"),
      conflictingLayerIdentifier.c_str(),
      conflictingSpecPath.GetString().c_str(),
      (conflictingSpecType == SdfSpecTypeAttribute ? "an attribute"
                                                   : "a relationship"));
}
...
std::string PcpErrorInconsistentAttributeType::ToString() const {
  return TfStringPrintf(
      ...
      "The conflicting spec will be ignored.",
      rootSite.path.GetString().c_str(), definingLayerIdentifier.c_str(),
      definingSpecPath.GetString().c_str(), definingValueType.GetText(),
      conflictingLayerIdentifier.c_str(),
      conflictingSpecPath.GetString().c_str(), conflictingValueType.GetText());
}
...
std::string PcpErrorInvalidInstanceTargetPath::ToString() const {
  ...
  return TfStringPrintf(
      ...
      (ownerSpecType == SdfSpecTypeAttribute ? "attribute connection"
                                             : "relationship target"),
      targetPath.GetText(), owningPath.GetText(),
...
std::string PcpErrorInvalidReferenceOffset::ToString() const {
  return TfStringPrintf("Invalid %s offset %s for @%s@<%s> introduced by %s. "
                        ...
                        TfEnum::GetDisplayName(arcType).c_str(),
                        TfStringify(offset).c_str(), assetPath.c_str(),
                        targetPath.GetText(),
...
std::string PcpErrorInvalidSublayerOwnership::ToString() const {
  ...
  return TfStringPrintf("The following sublayers for layer @%s@ have the "
                        "same owner '%s': %s",
                        layer->GetIdentifier().c_str(), owner.c_str(),
...
std::string PcpErrorPropertyPermissionDenied::ToString() const {
  return TfStringPrintf("The layer at @%s@ has an illegal opinion about "
                        ...
                        layerPath.c_str(),
                        propType == SdfSpecTypeAttribute ? "an attribute"
                                                         : "a relationship",
                        propPath.GetText());
}
...
std::string PcpErrorVariableExpressionError::ToString() const {
  ...
  auto makeSourceStr = [this]() {
    ...
    result += TfStringPrintf("in @%s@",
                             sourceLayer ? sourceLayer->GetIdentifier().c_str()
                                         : "<expired>");
    return result;
  };

  return TfStringPrintf(R"(Error evaluating expression %s for %s %s: %s)",
                        expression.substr(0, 32).c_str(), context.c_str(),
                        makeSourceStr().c_str(), expressionError.c_str());
}
