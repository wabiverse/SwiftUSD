...
void Pcp_Dependencies::Add(
  ...
  if (!fileFormatDependencyData.IsEmpty()) {
    ...
    auto addNamesToDepMapFn = [](_FileFormatArgumentFieldDepMap &depMap,
                                 const TfToken::Set &names) {
...
void Pcp_Dependencies::Remove(const PcpPrimIndex &primIndex,
  ...
  if (it != _fileFormatArgumentDependencyMap.end()) {
    if (TF_VERIFY(!it->second.IsEmpty())) {
      ...
      auto removeNamesFromDepMapFn = [](_FileFormatArgumentFieldDepMap &depMap,
                                        const TfToken::Set &names) {
