...
PcpMapExpression PcpMapExpression::Constant(const Value &value) {
  return PcpMapExpression(
      _Node::New(_OpConstant, _NodeRefPtr(), _NodeRefPtr(), value));
}
...
struct PcpMapExpression::_Node::_NodeMap {
  ...
  typedef tbb::concurrent_hash_map<Key, PcpMapExpression::_Node *,
                                   _KeyHashEq<Key>>
      MapType;
...
PcpMapExpression::_Node::New(_Op op_, const _NodeRefPtr &arg1_,
...
inline size_t PcpMapExpression::_Node::Key::GetHash() const {
  return TfHash::Combine(op, get_pointer(arg1), get_pointer(arg2),
                         valueForConstant);
}
