...
namespace
{
      ...
      int usdEncodingVersion)
  {
    ...
    default:
      TF_DEBUG(NDR_PARSING).Msg("Invalid/unsupported usdEncodingVersion %d. "
      ...
      const TfToken &name)
  {
    ...
    else if (sdrType == SdrPropertyTypes->Float)
    {
      ...
      else
      {
        ...
        if (arrayVal.size() != arraySize)
        {
          ...
          TF_DEBUG(SDR_TYPE_CONFORMANCE).Msg("Default value for fixed size float array type does not "
    ...
    if (!isSdrValueConformed)
    {
      TF_DEBUG(SDR_TYPE_CONFORMANCE).Msg("Expected type for defaultValue for property: %s is %s, "
                                         "but %s was provided.",
