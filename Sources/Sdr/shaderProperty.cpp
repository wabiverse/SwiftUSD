                                           ...
                                           const NdrTokenMap &metadata)
{
  ...
  if (type == SdrPropertyTypes->Vstruct) {
    return std::make_pair(isArray ? SdfValueTypeNames->FloatArray : SdfValueTypeNames->Float,
                          type);
  }
                                           ...
                                           const NdrTokenMap &metadata)
{
  ...
  {
    return std::make_pair(isArray ? SdfValueTypeNames->TokenArray : SdfValueTypeNames->Token,
                          type);
  }
...
bool SdrShaderProperty::CanConnectTo(const NdrProperty &other) const
{
  ...
  const NdrSdfTypeIndicator &sdfInputTypeInd = _GetTypeAsSdfType(
      inputType, inputArraySize, inputMetadata, _usdEncodingVersion);
  const NdrSdfTypeIndicator &sdfOutputTypeInd = _GetTypeAsSdfType(
      outputType, outputArraySize, outputMetadata, _usdEncodingVersion);
  ...
  bool inputIsFloat4 = (inputType == SdrPropertyTypes->Color4) ||
                       (sdfInputType == SdfValueTypeNames->Float4);

  bool outputIsFloat4 = (outputType == SdrPropertyTypes->Color4) ||
...
{
  _sdfTypeDefaultValue = _ConformSdfTypeDefaultValue(
      _defaultValue, _type, _arraySize, _metadata, _usdEncodingVersion);
