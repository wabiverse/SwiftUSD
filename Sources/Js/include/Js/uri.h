...
template<typename ValueType, typename Allocator = CrtAllocator> class GenericUri {
 public:
 ...
 private:
  ...
  std::size_t Allocate(std::size_t len)
  {
    ...
    // terminated. Order: scheme, auth, path, query, frag, base, uri Note need to set, increment,
    // assign in 3 stages to avoid compiler warning bug.
