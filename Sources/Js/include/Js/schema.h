 ...
 public:
    ...
    if (spec_.oapi != kVersion20 && spec_.oapi != kVersion30)
      if (const ValueType *v = GetMember(value, GetPatternPropertiesString())) {
        ...
        for (ConstMemberIterator itr = v->MemberBegin(); itr != v->MemberEnd(); ++itr) {
          ...
          patternProperties_[patternPropertyCount_].pattern = CreatePattern(
              itr->name, schemaDocument, r);
          schemaDocument->CreateSchema(
              &patternProperties_[patternPropertyCount_].schema, r, itr->value, document, id_);
          patternPropertyCount_++;
        }
      }
    ...
    if (const ValueType *v = GetMember(value, GetItemsString())) {
      ...
      if (v->IsObject())  // List validation
        ...
        for (ConstValueIterator itr = v->Begin(); itr != v->End(); ++itr, index++)
          schemaDocument->CreateSchema(
              &itemsTuple_[itemsTupleCount_++], q.Append(index, allocator_), *itr, document, id_);
      }
    }
  ...
  bool Key(Context &context, const Ch *str, SizeType len, bool) const
  {
    ...
    {  // patternProperties are not additional properties
      // Must set valueSchema for when kValidateContinueOnErrorFlag is set, else reports spurious
      // type error
      context.valueSchema = typeless_;
...
template<typename SchemaDocumentType> class IGenericRemoteSchemaDocumentProvider {
 public:
  ...
  virtual const SchemaDocumentType *GetRemoteDocument(
      const GenericUri<ValueType, AllocatorType> uri, Specification &spec)
  {
...
template<typename ValueT, typename Allocator = CrtAllocator> class GenericSchemaDocument {
 public:
  ...
  /*!
     ...
     Can be null. \param pointer An optional JSON pointer to the start of the schema document
      \param spec Optional schema draft or OpenAPI version. Used if no specification in document.
     Defaults to draft-04.
  */
...
#undef RAPIDJSON_STRING_
  ...
  static SchemaDraft GetSchemaDraft(const ValueType &document)
  {
    ...
    typename ValueType::ConstMemberIterator itr = document.FindMember(
        SchemaType::GetSchemaString());
                       ...
                       const UriType &id)
  {
    ...
    if (itr->value.IsString()) {
      ...
      if (len == 0)
        ...
        // Search from the root. Returns the subschema in the document and its absolute JSON
        // pointer.
        ...
        if (!base) {
          ...
          if (!remoteProvider_)
                    ...
                    ref, spec_))
            {
              ...
              if (len <= 1 || s[1] == '/') {
                ...
                if (!pointer.IsValid())
                  ...
                  if (const SchemaType *sc = remoteDocument->GetSchema(pointer)) {
                    ...
                    SchemaErrorValue(
                        kSchemaErrorRefUnknown, source, ref.GetString(), ref.GetStringLength());
                }
              }
              else
              ...
              SchemaErrorValue(
                  kSchemaErrorRefNoRemoteSchema, source, ref.GetString(), ref.GetStringLength());
          }
        }
          ...
          if (len <= 1 || s[1] == '/') {
            ...
            if (!relPointer.IsValid())
              ...
              if (const ValueType *pv = relPointer.Get(*base)) {
                ...
                if (IsCyclicRef(pointer))
                  SchemaErrorValue(
                      kSchemaErrorRefCyclical, source, ref.GetString(), ref.GetStringLength());
                else {
            ...
            // Search from the base we just established. Returns the subschema in the document and
            // its absolute JSON pointer.
            else if (const ValueType *pv = FindId(
                         *base,
                         ref,
                         pointer,
...
class GenericSchemaValidator
  ...
  */
  ...
  {
    AddNumberError(
        kValidateErrorMaxProperties, ValueType(actualCount).Move(), SValue(expectedCount).Move());
  }
  void TooFewProperties(SizeType actualCount, SizeType expectedCount)
  {
    AddNumberError(
        kValidateErrorMinProperties, ValueType(actualCount).Move(), SValue(expectedCount).Move());
  }
