...
struct _InputHandler : public rj::BaseReaderHandler<rj::UTF8<>, _InputHandler> {
  ...
  bool EndObject(rj::SizeType memberCount) {
    ...
    for (size_t i = 0; i < memberCount; ++i) {
      object.insert(std::make_pair(std::move(keys[kstart + i]),
...
static rj::Value _ToImplObjectValue(const JsObject &object,
                                    Allocator &allocator) {
  rj::Value value(rj::kObjectType);

  for (const auto &p : object) {
    value.AddMember(rj::Value(p.first.c_str(), allocator).Move(),
                    _JsValueToImplValue(p.second, allocator), allocator);
  }
...
static rj::Value _ToImplArrayValue(const JsArray &array, Allocator &allocator) {
  ...
  for (const auto &e : array) {
    value.PushBack(rj::Value(_JsValueToImplValue(e, allocator)).Move(),
                   allocator);
  }
...
template <typename Allocator>
static rj::Value _JsValueToImplValue(const JsValue &value,
                                     Allocator &allocator) {
  ...
  case JsValue::IntType:
    return value.IsUInt64() ? rj::Value(value.GetUInt64())
                            : rj::Value(value.GetInt64());
...
JsValue JsParseStream(std::istream &istr, JsParseError *error) {
  ...
  return JsParseString(std::string((std::istreambuf_iterator<char>(istr)),
...
JsValue JsParseString(const std::string &data, JsParseError *error) {
  ...
  TF_VERIFY(handler.values.size() == 1, "Unexpected value count: %zu",
            handler.values.size());
...
class Js_PolymorphicWriter : public Js_PolymorphicWriterInterface,
                             public TWriter {
public:
...
bool JsWriter::WriteValue(std::nullptr_t) { return _impl->GetWriter()->Null(); }

bool JsWriter::WriteValue(bool b) { return _impl->GetWriter()->Bool(b); }

bool JsWriter::WriteValue(int i) { return _impl->GetWriter()->Int(i); }

bool JsWriter::WriteValue(unsigned u) { return _impl->GetWriter()->Uint(u); }
...
bool JsWriter::WriteValue(double d) { return _impl->GetWriter()->Double(d); }
...
bool JsWriter::BeginObject() { return _impl->GetWriter()->StartObject(); }
...
bool JsWriter::EndObject() { return _impl->GetWriter()->EndObject(); }

bool JsWriter::BeginArray() { return _impl->GetWriter()->StartArray(); }

bool JsWriter::EndArray() { return _impl->GetWriter()->EndArray(); }
