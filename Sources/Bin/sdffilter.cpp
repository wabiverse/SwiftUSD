...
class SdfFilterPseudoFileFormat : public SdfTextFileFormat {
 ...
 public:
  SdfFilterPseudoFileFormat(string description = "<< human readable >>")
      : SdfTextFileFormat(
            TfToken("pseudosdf"), TfToken(description), SdfTextFileFormatTokens->Target)
  {
  }
...
double StringToDouble(const std::string &s)
{
  ...
  if (pos != s.size()) {
    throw std::invalid_argument(TfStringPrintf("invalid string '%s'", s.c_str()));
  }
  return d;
}
  ...
  for (auto const &spec : timeSpecs) {
    try {
      if (TfStringContains(spec, "..")) {
        ...
        timeRanges->emplace_back(StringToDouble(elts[0]), StringToDouble(elts[1]));
      }
      else {
      ...
      throw std::invalid_argument(TfStringPrintf("time out of range '%s'", spec.c_str()));
    }
  }
...
int main(int argc, char const *argv[])
{
  ...
  app.add_option("inputFiles", inputFiles, "The input files to process")
      ->required(true)
      ->option_text("...");
  ...
  app.add_option("--timeSamplesSizeLimit",
  ...
  const std::map<std::string, SortKey> sortKeyMap{{"path", SortByPath}, {"field", SortByField}};

  app.add_option("--sortBy",
                 sortKey,
