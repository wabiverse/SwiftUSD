...
{
  app->add_option("inputPath", args.inputPath, "The input file to process")->required();
...
template<typename PrimType> std::string GetPrimLabel(const PrimType &prim)
{
...
void PrintPrim(const Args &args, const PrimType &prim, const std::string &prefix, bool isLast)
{
  ...
  const std::string label = args.simple ? GetName(prim) : GetPrimLabel(prim);
  ...
  if (args.metadata) {
    ...
    for (const auto &key : metadata) {
      if (key.IsEmpty() || key == SdfFieldKeys->TypeName || key == SdfFieldKeys->Specifier ||
          key == SdfFieldKeys->Kind || key == SdfFieldKeys->Active)
      {
        continue;
      }
...
template<typename StageOrLayer> void PrintTree(const Args &args, const StageOrLayer &stageOrLayer)
{
  std::cout << "/\n";
...
int USDTree(const Args &args)
{
  ...
  const ArResolverContext context = resolver.CreateDefaultContextForAsset(args.inputPath);
