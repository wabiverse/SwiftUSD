...
{
  app->add_option("inputFiles", args.inputFiles, "A list of input files")
      ->required(true)
      ->option_text("...");
  ...
  app->add_flag("--skipSourceFileComment",
                args.skipSourceFileComment,
...
static void Quarantine(const std::string &filepath)
{
  ...
  if (std::rename(filepath.c_str(), newName.c_str()) != 0) {
    std::cerr << "Failed to rename possibly corrupt output file from " << filepath << " to "
...
static int UsdCat(const Args &args)
{
  ...
  if (args.layerMetadata && (args.flatten || args.flattenLayerStack)) {
    ...
    std::cerr << "error: --layerMetadata cannot be used together "
              << "with " << (args.flatten ? "--flatten" : "--flattenLayerStack") << "\n";
    return 1;
  }
  ...
  for (auto &input : args.inputFiles) {
    ...
    if (args.flatten) {
      ...
      auto srcLayer = SdfLayer::OpenAsAnonymous(input, /* metadataOnly = */ true);
    ...
    if (!args.output.empty()) {
      ...
      if (!errMark.IsClean()) {
        ...
        std::cerr << "Error exporting " << std::quoted(input) << " to " << std::quoted(args.output)
                  << " - ";
        for (const auto &err : errMark) {
          std::cerr << err.GetCommentary() << "\n";
        }
      ...
      if (errMark.IsClean()) {
        ...
        std::cerr << "Error writing " << std::quoted(input) << " to stdout - ";
        for (const auto &err : errMark) {
          std::cerr << err.GetCommentary() << "\n";
        }
      }
