...
HdRenderPassStateSharedPtr HdxOitResolveTask::_GetContextRenderPassState(
...
void HdxOitResolveTask::_UpdateCameraFraming(HdTaskContext *const ctx)
{
  const HdRenderPassStateSharedPtr ctxRenderPassState = _GetContextRenderPassState(ctx);
  if (!ctxRenderPassState) {
    TF_CODING_ERROR(
        "Unable to set camera framing data due to missing "
        "render pass state on task context");
    return;
  }

  _renderPassState->SetCamera(ctxRenderPassState->GetCamera());
  _renderPassState->SetOverrideWindowPolicy(ctxRenderPassState->GetOverrideWindowPolicy());
  ...
  if (framing.IsValid()) {
    ...
    _renderPassState->SetViewport(ctxRenderPassState->GetViewport());
  }
}
                                           ...
                                           GfVec2i const &screenSize)
{
  ...
  if (createOitBuffers) {
    ...
    // Counter Buffer
    //
    HdBufferSpecVector counterSpecs{{HdxTokens->hdxOitCounterBuffer, HdTupleType{HdTypeInt32, 1}}};
    _counterBar = hdStResourceRegistry->AllocateSingleBufferArrayRange(
        /*role*/ HdxTokens->oitCounter, counterSpecs, HdBufferArrayUsageHintBitsStorage);
    //
    // Index Buffer
    //
    HdBufferSpecVector indexSpecs{{HdxTokens->hdxOitIndexBuffer, HdTupleType{HdTypeInt32, 1}}};
    _indexBar = hdStResourceRegistry->AllocateSingleBufferArrayRange(
        /*role*/ HdxTokens->oitIndices, indexSpecs, HdBufferArrayUsageHintBitsStorage);

    //
    // Data Buffer
    //
    HdBufferSpecVector dataSpecs{{HdxTokens->hdxOitDataBuffer, HdTupleType{HdTypeFloatVec4, 1}}};
    _dataBar = hdStResourceRegistry->AllocateSingleBufferArrayRange(
        /*role*/ HdxTokens->oitData, dataSpecs, HdBufferArrayUsageHintBitsStorage);

    //
    // Depth Buffer
    //
    HdBufferSpecVector depthSpecs{{HdxTokens->hdxOitDepthBuffer, HdTupleType{HdTypeFloat, 1}}};
    _depthBar = hdStResourceRegistry->AllocateSingleBufferArrayRange(
        /*role*/ HdxTokens->oitDepth, depthSpecs, HdBufferArrayUsageHintBitsStorage);

    //
    // Uniforms
    //
    HdBufferSpecVector uniformSpecs{{HdxTokens->oitScreenSize, HdTupleType{HdTypeInt32Vec2, 1}}};
    _uniformBar = hdStResourceRegistry->AllocateUniformBufferArrayRange(
        /*role*/ HdxTokens->oitUniforms, uniformSpecs, HdBufferArrayUsageHintBitsUniform);
  }
