...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         ...
                         ((skydomeFrag, "SkydomeFragment"))(skydomeTexture));

HdxSkydomeTask::HdxSkydomeTask(HdSceneDelegate *delegate, SdfPath const &id)
    : HdxTask(id), _setupTask(), _settingsVersion(0), _skydomeVisibility(true)
{
}
...
void HdxSkydomeTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  ...
  const unsigned int currentSettingsVersion = renderDelegate->GetRenderSettingsVersion();
  if (_settingsVersion != currentSettingsVersion) {
    _settingsVersion = currentSettingsVersion;
    _skydomeVisibility = renderDelegate->GetRenderSetting<bool>(
...
void HdxSkydomeTask::Execute(HdTaskContext *ctx)
{
  ...
  if (!_skydomeVisibility || !haveColorAOV || !haveDomeLight || !_GetSkydomeTexture(ctx)) {
