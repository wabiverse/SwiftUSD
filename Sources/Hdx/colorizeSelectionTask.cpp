...
void HdxColorizeSelectionTask::Execute(HdTaskContext *ctx)
{
  ...
  if (!_pipelineCreated) {
    ...
    _compositor->SetAttachmentLoadStoreOp(HgiAttachmentLoadOpLoad, HgiAttachmentStoreOpStore);
  }
...
void HdxColorizeSelectionTask::_ColorizeSelection()
{
  ...
  const int32_t *iiddata = _instanceId ? reinterpret_cast<int32_t *>(_instanceId->Map()) : nullptr;
  const int32_t *eiddata = _elementId ? reinterpret_cast<int32_t *>(_elementId->Map()) : nullptr;
...
{
  out << "ColorizeSelectionTask Params: (...) " << pv.enableSelectionHighlight << " "
      << pv.enableLocateHighlight << " " << pv.selectionColor << " " << pv.locateColor << " "
      << pv.primIdBufferPath << " " << pv.instanceIdBufferPath << " " << pv.elementIdBufferPath;
  return out;
}
