...
\
      /* Resolve mode */ \
      (resolveNearestToCamera)(resolveNearestToCenter)(resolveUnique)(resolveAll)(resolveDeep)
...
struct HdxPrimOriginInfo {
  ...
  /// picked prim.
  HDX_API
  static HdxPrimOriginInfo FromPickHit(HdRenderIndex *renderIndex, const HdxPickHit &hit);
  ...
  ///
  HDX_API
  SdfPath GetFullPath(const TfToken &nameInPrimOrigin = HdPrimOriginSchemaTokens->scenePath) const;
  ...
  HdInstancerContext ComputeInstancerContext(
      const TfToken &nameInPrimOrigin = HdPrimOriginSchemaTokens->scenePath) const;
