...
class HdxEffectsShader {
 ...
 protected:
  ...
  HDX_API
  HdxEffectsShader(Hgi *hgi, const std::string &debugName);

  HDX_API
  void _SetColorAttachments(const HgiAttachmentDescVector &colorAttachmentDescs);

  HDX_API
  void _SetDepthAttachment(const HgiAttachmentDesc &depthAttachmentDesc);

  HDX_API
  void _SetPrimitiveType(HgiPrimitiveType primitiveType);

  HDX_API
  void _SetShaderProgram(const HgiShaderProgramHandle &shaderProgram);

  HDX_API
  void _SetVertexBufferDescs(const HgiVertexBufferDescVector &vertexBufferDescs);

  HDX_API
  void _SetDepthStencilState(const HgiDepthStencilState &depthStencilState);

  HDX_API
  void _SetMultiSampleState(const HgiMultiSampleState &multiSampleState);

  HDX_API
  void _SetRasterizationState(const HgiRasterizationState &rasterizationState);

  HDX_API
  void _SetShaderConstants(uint32_t byteSize, const void *data, HgiShaderStage stageUsage);

  HDX_API
  void _SetTextureBindings(const HgiTextureBindDescVector &textures);

  HDX_API
  void _SetBufferBindings(const HgiBufferBindDescVector &buffers);

  // Creates a graphics commands object, records draw commands to it via the
  // overridden _RecordDrawCmds, and then submits them.
  HDX_API
  void _CreateAndSubmitGraphicsCmds(const HgiTextureHandleVector &colorTextures,
  ...
  HDX_API
  void _DrawNonIndexed(const HgiBufferHandle &vertexBuffer,
  ...
  HDX_API
  void _DrawIndexed(const HgiBufferHandle &vertexBuffer,
 ...
 private:
  void _CreatePipeline(const HgiTextureHandleVector &colorTextures,
                       const HgiTextureHandleVector &colorResolveTextures,
                       const HgiTextureHandle &depthTexture,
                       const HgiTextureHandle &depthResolveTexture);
