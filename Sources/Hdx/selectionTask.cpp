  ...
  if (sel && (sel->GetVersion() != _lastVersion)) {
    ...
    if (!_selOffsetBar) {
      ...
      _selOffsetBar = hdStResourceRegistry->AllocateSingleBufferArrayRange(
          /*role*/ HdxTokens->selection, offsetSpecs, HdBufferArrayUsageHintBitsStorage);
    }

    const VtVec4fArray ptColors = sel->GetSelectedPointColors(renderIndex);
    const size_t numPtColors = ptColors.size();
    if (!_selUniformBar || numPtColors > _pointColorsBufferSize) {
      ...
      _pointColorsBufferSize = std::max<size_t>(numPtColors, s_minNumPointColors);
      ...
      uniformSpecs.emplace_back(HdxTokens->selectionPointColors,
                                HdTupleType{HdTypeFloatVec4, _pointColorsBufferSize});
      _selUniformBar = hdStResourceRegistry->AllocateUniformBufferArrayRange(
          /*role*/ HdxTokens->selection, uniformSpecs, HdBufferArrayUsageHintBitsUniform);
    }
    ...
    hdStResourceRegistry->AddSources(
        ...
        {std::make_shared<HdVtBufferSource>(HdxTokens->selColor, VtValue(_params.selectionColor)),
         ...
         std::make_shared<HdVtBufferSource>(
             HdxTokens->selectionPointColors, VtValue(ptColors), ptColors.size())});
    ...
    _hasSelection = sel->GetSelectionOffsetBuffer(
        renderIndex, _params.enableSelectionHighlight, _params.enableLocateHighlight, &offsets);
...
bool operator==(const HdxSelectionTaskParams &lhs, const HdxSelectionTaskParams &rhs)
{
  return lhs.enableSelectionHighlight == rhs.enableSelectionHighlight &&
         ...
         lhs.selectionColor == rhs.selectionColor && lhs.locateColor == rhs.locateColor;
}
