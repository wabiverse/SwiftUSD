...
HdxColorCorrectionTask::HdxColorCorrectionTask(HdSceneDelegate *delegate, SdfPath const &id)
    : HdxTask(id), _workDispatcher(std::make_unique<WorkDispatcher>())
{
...
void HdxColorCorrectionTask::_CreateOpenColorIOResourcesImpl(
    Hgi *hgi,
    ...
    HdxColorCorrectionTask::_OCIOResources *result)
{
  ...
  const char *display = params.displayOCIO.empty() ? config->getDefaultDisplay() :
                                                     params.displayOCIO.c_str();

  const char *view = params.viewOCIO.empty() ? config->getDefaultView(display) :
  ...
  const int num3Dentries = 3 * params.lut3dSizeOCIO * params.lut3dSizeOCIO * params.lut3dSizeOCIO;
...
std::string HdxColorCorrectionTask::_CreateOpenColorIOShaderCode(std::string &ocioGpuShaderText,
...
static void _RGBtoRGBA(float const *lutValues,
                       int &valueCount,
...
struct HdxColorCorrectionTask_UboBuilder {
           ...
           uint32_t count = 1)
  {
    ...
    ubos.emplace_back(
        _UniformBufferDesc{typeName, name, std::vector<uint8_t>(), (uint32_t)dataSize, count});
    ...
    HdxColorCorrectionTask::_OCIOResources *result)
{
  ...
  shaderDesc->setLanguage(hgi->GetAPIName() == HgiTokens->Metal ? OCIO::GPU_LANGUAGE_MSL_2_0 :
  ...
  for (size_t i = 0; i < shaderDesc->getNum3DTextures(); ++i) {
    ...
    result->luts.emplace_back(_TextureSamplerDesc{texDesc, sampDesc, float4AdaptedLutValues});
  }
...
#    endif  // OCIO_VERSION_HEX >= 0x02030000
    ...
    int channelPerPix = channel == OCIO::GpuShaderCreator::TextureType::TEXTURE_RED_CHANNEL ? 1 :
                                                                                              4;
    int valueCount = channelPerPix * width * height;
    HgiFormat fmt = channelPerPix == 1 ? HgiFormatFloat32 : HgiFormatFloat32Vec3;
    std::vector<float> float4AdaptedLutValues;
    ...
    texDesc.type =
#    if OCIO_VERSION_HEX >= 0x02030000
        dimensions == OCIO::GpuShaderCreator::TextureDimensions::TEXTURE_1D ? HgiTextureType1D :
                                                                              HgiTextureType2D;
#    else
...
#    endif  // OCIO_VERSION_HEX >= 0x02030000
    ...
    // Sampler description
    HgiSamplerDesc sampDesc;
    sampDesc.magFilter = interpolation == OCIO::Interpolation::INTERP_NEAREST ?
                             HgiSamplerFilterNearest :
                             HgiSamplerFilterLinear;
    sampDesc.minFilter = interpolation == OCIO::Interpolation::INTERP_NEAREST ?
                             HgiSamplerFilterNearest :
                             HgiSamplerFilterLinear;
    sampDesc.addressModeU = HgiSamplerAddressModeClampToEdge;
    sampDesc.addressModeV = HgiSamplerAddressModeClampToEdge;
  ...
  for (uint32_t i = 0, n = shaderDesc->getNumUniforms(); i < n; ++i) {
    ...
    switch (data.m_type) {
      ...
      case OCIO::UNIFORM_VECTOR_INT: {
        ...
        uboBuilder.Add<int>("int", name, data.m_vectorInt.m_getVector(), bufferLength);
      } break;
      case OCIO::UNIFORM_VECTOR_FLOAT: {
        int bufferLength = data.m_vectorFloat.m_getSize();
        uboBuilder.Add<float>("float", name, data.m_vectorFloat.m_getVector(), bufferLength);
      } break;
  ...
  for (_UniformBufferDesc const &uboDesc : result->ubos) {
    if (uboDesc.count == 1) {
      result->constantValues.insert(
          result->constantValues.end(), uboDesc.data.begin(), uboDesc.data.end());
                                                                 ...
                                                                 HgiShaderFunctionDesc &fragDesc)
{
  ...
  for (TextureSamplerInfo const &texInfo : _textureLUTs) {
    HgiShaderFunctionAddTexture(&fragDesc, texInfo.texName, bindingIdx, texInfo.dim);
    ++bindingIdx;
    if (_GetHgi()->GetAPIName() == HgiTokens->Metal) {
      ...
      // one Hgi provides.
      if (TF_VERIFY(!texInfo.texName.empty() && !texInfo.samplerName.empty())) {
        ...
        if (offset != std::string::npos) {
          ...
          ocioGpuShaderText.erase(prevNewLineOffset, nextNewLineOffset - prevNewLineOffset);

          // Now find and replace second occurrence.
          offset = ocioGpuShaderText.find(texInfo.samplerName);
          while (offset != std::string::npos) {
            ocioGpuShaderText.replace(
                offset, samplerNameLength, texInfo.texName.c_str(), texNameLength);
  ...
  for (BufferInfo const &buffInfo : _bufferConstants) {
    if (buffInfo.count == 1) {
      ...
      HgiShaderFunctionAddConstantParam(&fragDesc, buffInfo.name + "_count", "int");

      HgiShaderFunctionAddBuffer(
          &fragDesc, buffInfo.name, buffInfo.typeName, bindingIdx++, HgiBindingTypeUniformArray);
      ...
      if (_GetHgi()->GetAPIName() == HgiTokens->Metal) {
        ...
        if (offset != std::string::npos) {
          ...
          ocioGpuShaderText.replace(
              offset, buffInfo.name.length(), dummyUniformName.c_str(), dummyUniformName.length());
        }
      }
    }
  }
...
void HdxColorCorrectionTask::_CreateOpenColorIOResources(
    Hgi *hgi, HdxColorCorrectionTaskParams const &params, _OCIOResources *result)
{
  ...
  try {
    ...
    TF_WARN("_CreateOpenColorIOResourcesImpl threw a C++ exception: %s", e.what());
  }
  catch (...) {
...
void HdxColorCorrectionTask::_CreateOpenColorIOLUTBindings(HgiResourceBindingsDesc &resourceDesc)
{
...
void HdxColorCorrectionTask::_SetConstants(HgiGraphicsCmds *gfxCmds)
{
  if (_ocioResources.constantValues.size() < sizeof(_screenSize)) {
    _ocioResources.constantValues.resize(sizeof(_screenSize));
  }
  memcpy(_ocioResources.constantValues.data(), _screenSize, sizeof(_screenSize));
...
static unsigned char _TextureDimensionCount(HgiTextureDesc &texDesc)
{
  switch (texDesc.type) {
...
bool HdxColorCorrectionTask::_CreateShaderResources()
{
...
#if OCIO_VERSION_HEX < 0x02000000
  if (useOCIO) {
    HgiShaderFunctionAddTexture(&fragDesc, "Lut3DIn", /*bindIndex = */ 1, /*dimensions = */ 3);
  }
#endif
  ...
  if (useOCIO) {
    ...
    for (_TextureSamplerDesc lut : _ocioResources.luts) {
      ...
      _textureLUTs.emplace_back(TextureSamplerInfo{_TextureDimensionCount(lut.textureDesc),
    ...
    for (_UniformBufferDesc const &ubo : _ocioResources.ubos) {
      ...
      _bufferConstants.emplace_back(BufferInfo{
          ubo.typeName,
          ubo.name,
          ubo.count,
          ubo.count > 1 ? _GetHgi()->CreateBuffer(bufferDesc) : HgiHandle<HgiBuffer>()});
    }

    fsCode += _CreateOpenColorIOShaderCode(_ocioResources.gpuShaderText, fragDesc);
...
bool HdxColorCorrectionTask::_CreateBufferResources()
{
  ...
  constexpr float vertData[][6] = {{-1, 3, 0, 1, 0, 2}, {-1, -1, 0, 1, 0, 0}, {3, -1, 0, 1, 2, 0}};
...
bool HdxColorCorrectionTask::_CreateAovSampler()
{
  if (_aovSampler) {
    return true;
  }
                                   ...
                                   HdDirtyBits *dirtyBits)
{
  ...
  if ((*dirtyBits) & HdChangeTracker::DirtyParams) {
    if (_GetTaskParams(delegate, &_params)) {
      ...
      _workDispatcher->Cancel();
      _workDispatcher->Wait();
      _workDispatcher->Run(&_CreateOpenColorIOResources, _GetHgi(), _params, &_ocioResources);
    }
  }
