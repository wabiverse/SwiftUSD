...
bool _IsDependentOn(HdStDrawTarget const *drawTarget, HdStDrawTarget const *otherDrawTarget)
{
...
static void _SortDrawTargets(HdStDrawTargetPtrVector const &drawTargets,
  ...
  for (_DrawTargetEntry const &entry : drawTargetEntries) {
    if (HdStDrawTarget *const drawTarget = entry.drawTarget) {
      if (drawTarget->IsEnabled()) {
        result.push_back({std::make_unique<HdSt_RenderPass>(renderIndex, HdRprimCollection()),
...
HdxDrawTargetTask::_CameraInfo HdxDrawTargetTask::_ComputeCameraInfo(
    const HdRenderIndex &renderIndex, const HdStDrawTarget *const drawTarget)
{
  ...
  const HdStDrawTargetRenderPassState *const state = drawTarget->GetDrawTargetRenderPassState();
  ...
  const GfMatrix4d projectionMatrix = CameraUtilConformedWindow(camera->ComputeProjectionMatrix(),
                                                                camera->GetWindowPolicy(),
                                                                aspect) *
                                      yflip;

  return {
      camera->GetTransform().GetInverse(), projectionMatrix, viewport, camera->GetClipPlanes()};
}
...
{
  const HdStDrawTargetRenderPassState *const state = info->target->GetDrawTargetRenderPassState();
...
void HdxDrawTargetTask::Execute(HdTaskContext *ctx)
{
  ...
  for (const _RenderPassInfo &renderPassInfo : _renderPassesInfo) {
    HdStRenderPassStateSharedPtr const renderPassState = renderPassInfo.renderPassState;
