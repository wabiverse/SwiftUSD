...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         ...
                         (PickBuffer)(PickBufferBinding)(Picking)
...
void HdxPickTask::_InitIfNeeded()
{
  ...
  if (!_pickBuffer) {
    HdStResourceRegistrySharedPtr const &hdStResourceRegistry =
        std::dynamic_pointer_cast<HdStResourceRegistry>(_index->GetResourceRegistry());

    if (hdStResourceRegistry) {

      HdBufferSpecVector bufferSpecs{{_tokens->PickBuffer, HdTupleType{HdTypeInt32, 1}}};

      _pickBuffer = hdStResourceRegistry->AllocateSingleBufferArrayRange(
          _tokens->Picking, bufferSpecs, HdBufferArrayUsageHintBitsStorage);
    }
  }
...
void HdxPickTask::_CreateAovBindings()
{
  ...
  // don't clear them at all, so that previously-drawn items are retained.
  {
    _widgetDepthStencilBuffer = std::make_unique<HdStRenderBuffer>(
        hdStResourceRegistry.get(), _GetAovPath(_tokens->widgetDepthStencil));
...
HdRenderBuffer const *HdxPickTask::_FindAovBuffer(TfToken const &aovName) const
{
  HdRenderPassAovBindingVector::const_iterator bindingIt = std::find_if(
      ...
      [&aovName](HdRenderPassAovBinding const &binding) { return binding.aovName == aovName; });
...
void HdxPickTask::Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  ...
  HdRenderPassStateSharedPtr states[] = {
      _pickableRenderPassState, _occluderRenderPassState, _widgetRenderPassState};
  ...
  for (auto &state : states) {
    ...
    bool enableDepthWrite = (state == _occluderRenderPassState) ||
...
void HdxPickTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  ...
  HdStRenderPassState *extendedState = dynamic_cast<HdStRenderPassState *>(
      _pickableRenderPassState.get());

  HdStRenderPassShaderSharedPtr renderPassShader = extendedState ?
  ...
  if (renderPassShader) {
    if (_pickBuffer) {
      ...
      renderPassShader->RemoveBufferBinding(_tokens->PickBufferBinding);
    }
  }
}

void HdxPickTask::_ClearPickBuffer()
{
  if (!_pickBuffer) {
    return;
  }
  ...
  if (_contextParams.resolveMode == HdxPickTokens->resolveDeep) {
    const int numSubBuffers = _contextParams.maxNumDeepEntries / PICK_BUFFER_SUBBUFFER_CAPACITY;
    const int entryStorageOffset = PICK_BUFFER_HEADER_SIZE + numSubBuffers;
    const int entryStorageSize = numSubBuffers * PICK_BUFFER_SUBBUFFER_CAPACITY *
                                 PICK_BUFFER_ENTRY_SIZE;
    ...
    pickBufferInit.push_back(_contextParams.pickTarget == HdxPickTokens->pickFaces ? 1 : 0);
    pickBufferInit.push_back(_contextParams.pickTarget == HdxPickTokens->pickEdges ? 1 : 0);
    pickBufferInit.push_back(_contextParams.pickTarget == HdxPickTokens->pickPoints ? 1 : 0);
  ...
  HdBufferSourceSharedPtr bufferSource = std::make_shared<HdVtBufferSource>(
      _tokens->PickBuffer, VtValue(pickBufferInit));
...
void HdxPickTask::_ResolveDeep()
{
  ...
  for (int subBuffer = 0; subBuffer < numSubBuffers; ++subBuffer) {
    ...
    const int subBufferOffset = entryStorageOffset + subBuffer * PICK_BUFFER_SUBBUFFER_CAPACITY *
                                                         PICK_BUFFER_ENTRY_SIZE;
    ...
    for (int j = 0; j < numEntries; ++j) {
      ...
      hit.elementIndex = _contextParams.pickTarget == HdxPickTokens->pickFaces ? partIndex : -1;
      hit.edgeIndex = _contextParams.pickTarget == HdxPickTokens->pickEdges ? partIndex : -1;
      hit.pointIndex = _contextParams.pickTarget == HdxPickTokens->pickPoints ? partIndex : -1;
...
bool HdxPickResult::_ResolveHit(
    const int index, const int x, const int y, const float z, HdxPickHit *const hit) const
{
  ...
  _index->GetSceneDelegateAndInstancerIds(hit->objectId, &(hit->delegateId), &(hit->instancerId));
  ...
  const GfVec3d ndcHit(((double)x / _bufferSize[0]) * 2.0 - 1.0,
...
static SdfPath _ComputeInstancedByPath(HdContainerDataSourceHandle const &primSource)
{
...
//
static std::tuple<SdfPath, HdContainerDataSourceHandle, VtArray<int>, VtArray<SdfPath>>
_ComputeInstancerAndInstanceIndicesAndLocations(HdSceneIndexBaseRefPtr const &sceneIndex,
                                                ...
                                                HdContainerDataSourceHandle const &primSource)
{
  ...
  HdInstancerTopologySchema schema = HdInstancerTopologySchema::GetFromParent(instancerSource);
  if (!schema) {
    return {SdfPath(), nullptr, {}, {}};
  }

  HdPathArrayDataSourceHandle const instanceLocationsDs = schema.GetInstanceLocations();

  return {instancerPath,
          instancerSource,
          schema.ComputeInstanceIndicesForProto(primPath),
          instanceLocationsDs ? instanceLocationsDs->GetTypedValue(0.0f) : VtArray<SdfPath>()};
}

HdxPrimOriginInfo HdxPrimOriginInfo::FromPickHit(HdRenderIndex *const renderIndex,
                                                 const HdxPickHit &hit)
{
  HdxPrimOriginInfo result;

  HdSceneIndexBaseRefPtr const sceneIndex = renderIndex->GetTerminalSceneIndex();
  // Fallback value.
  ...
  HdContainerDataSourceHandle primSource = sceneIndex->GetPrim(path).dataSource;

  // First, ask the prim itself for the prim origin data source.
  // This will only be valid when scene indices are enabled.
  result.primOrigin = HdPrimOriginSchema::GetFromParent(primSource).GetContainer();
  ...
  while (true) {
    ...
    std::tie(path, primSource, instanceIndices, instanceLocations) =
        _ComputeInstancerAndInstanceIndicesAndLocations(sceneIndex, path, primSource);
    ...
    ctx.instancerPrimOrigin = HdPrimOriginSchema::GetFromParent(primSource).GetContainer();
    ...
    if (ctx.instanceId >= 0 && ctx.instanceId < static_cast<int>(instanceLocations.size())) {
      ctx.instanceSceneIndexPath = instanceLocations[ctx.instanceId];

      HdPrimOriginSchema schema = HdPrimOriginSchema::GetFromParent(
  ...
  std::reverse(result.instancerContexts.begin(), result.instancerContexts.end());
...
static bool _AppendPrimOriginToPath(HdContainerDataSourceHandle const &primOriginDs,
...
SdfPath HdxPrimOriginInfo::GetFullPath(const TfToken &nameInPrimOrigin) const
{
  SdfPath path;
  ...
  for (const HdxInstancerContext &ctx : instancerContexts) {
    _AppendPrimOriginToPath(ctx.instancePrimOrigin, nameInPrimOrigin, &path);
  }
  _AppendPrimOriginToPath(primOrigin, nameInPrimOrigin, &path);
  return path;
}

HdInstancerContext HdxPrimOriginInfo::ComputeInstancerContext(
    const TfToken &nameInPrimOrigin) const
{
  ...
  for (const HdxInstancerContext &ctx : instancerContexts) {
    ...
    if (_AppendPrimOriginToPath(ctx.instancerPrimOrigin, nameInPrimOrigin, &instancer)) {
    ...
    _AppendPrimOriginToPath(ctx.instancePrimOrigin, nameInPrimOrigin, &prefix);
  }
...
{
  size_t hash = 0;
  hash = TfHash::Combine(hash, _GetPrimId(index), _GetInstanceId(index));
...
bool HdxPickResult::_IsValidHit(int index) const
{
  ...
  if (_pickTarget == HdxPickTokens->pickEdges) {
    ...
    if (_GetInstanceId(index) != -1) {
      SdfPath const primId = _index->GetRprimPathFromPrimId(_GetPrimId(index));
      if (!primId.IsEmpty()) {
        SdfPath delegateId;
        SdfPath instancerId;
        _index->GetSceneDelegateAndInstancerIds(primId, &delegateId, &instancerId);
