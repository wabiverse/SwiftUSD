...
std::vector<GfMatrix4d> HdxSimpleLightTask::_ComputeShadowMatrices(
    const HdCamera *const camera, HdxShadowMatrixComputationSharedPtr const &computation) const
{
                              ...
                              HdDirtyBits *dirtyBits)
{
      ...
      (tracker.GetSprimIndexVersion() != _sprimIndexVersion))
  {
    ...
    _lightIds.clear();
    _numLightIds = _AppendLightsOfType(
  ...
  if (renderDelegate->GetRenderSettingsVersion() != _settingsVersion) {
    _maxLights = size_t(renderIndex.GetRenderDelegate()->GetRenderSetting<int>(
  ...
  if (verifyNumLights && (_numLightIds > _maxLights)) {
    TF_WARN(
        "Hydra Storm supports up to %zu lights, truncating the "
        "%zu found lights to this max.",
        _maxLights,
        _numLightIds);
  }
  ...
  if (lightingContext->GetUseShadows()) {
    ...
    if (shadowIndex > -1) {
      for (size_t lightId = 0; lightId < _glfSimpleLights.size(); ++lightId) {
        if (!_glfSimpleLights[lightId].HasShadow()) {
          continue;
        }
        // Complete the shadow setup for this light
        const int shadowStart = _glfSimpleLights[lightId].GetShadowIndexStart();
        const int shadowEnd = _glfSimpleLights[lightId].GetShadowIndexEnd();
        ...
        for (int shadowId = shadowStart; shadowId <= shadowEnd; ++shadowId) {
          shadows->SetViewMatrix(shadowId, _glfSimpleLights[lightId].GetTransform().GetInverse());
...
void HdxSimpleLightTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  ...
  if (!_lightingBar) {
    ...
    _lightingBar = hdStResourceRegistry->AllocateUniformBufferArrayRange(
        HdxSimpleLightTaskTokens->lighting, bufferSpecs, HdBufferArrayUsageHintBitsUniform);

    _lightingShader->AddBufferBinding(HdStBindingRequest(HdStBinding::UBO,
  ...
  if (_rebuildLightAndShadowBufferSources) {
    ...
    HdBufferSourceSharedPtrVector sources = {
        ...
        std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->hasShadow,
                                           VtValue(hasShadow)),
