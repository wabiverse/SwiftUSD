...
void HdxAovInputTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  ...
  if (_aovBuffer) {
    _UpdateIntermediateTexture(
        _aovTextureIntermediate, _aovBuffer, HgiTextureUsageBitsColorTarget);
  }

  // Do the same for the intermediate depth texture.
  if (_depthBuffer) {
    _UpdateIntermediateTexture(
        _depthTextureIntermediate, _depthBuffer, HgiTextureUsageBitsDepthTarget);
  }
}
...
void HdxAovInputTask::Execute(HdTaskContext *ctx)
{
  ...
  if (_depthBuffer) {
    ...
    (*ctx)[HdxAovTokens->depthIntermediate] = VtValue(_depthTextureIntermediate);
  }
...
void HdxAovInputTask::_UpdateIntermediateTexture(HgiTextureHandle &texture,
                                                 HdRenderBuffer *buffer,
                                                 HgiTextureUsageBits usage)
{
  const GfVec3i dim(buffer->GetWidth(), buffer->GetHeight(), buffer->GetDepth());
  ...
  HgiFormat hgiFormat = HdxHgiConversions::GetHgiFormat(hdFormat);
