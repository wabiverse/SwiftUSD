...
bool UsdUtils_AssetLocalizationPackage::Build(const SdfAssetPath &assetPath,
                                              const std::string &firstLayerName)
{
  ...
  _firstLayerName = firstLayerName.empty() ? TfGetBaseName(_rootLayer->GetRealPath()) :
                                             firstLayerName;
  ...
  for (const auto &layerDep : _layersToCopy) {
    ...
    if (!packagedFiles.insert(layerDep.second).second) {
      TF_WARN(
          ...
          layerDep.second.c_str(),
          layerDep.first.c_str());
      continue;
    }
    ...
    if (!layerToAdd) {
      TF_WARN(
          "Unable to open layer at path \"%s\" while writing package."
          " Skipping export of dependency @%s@.",
          layerDep.first.c_str(),
          layerDep.second.c_str());
      continue;
...
UsdUtilsDependencyInfo UsdUtils_AssetLocalizationPackage::_ProcessDependency(
    ...
    const SdfLayerRefPtr &layer, const UsdUtilsDependencyInfo &depInfo)
{
  ...
  const std::string remappedPath = _ProcessAssetPath(
      layer, depInfo.GetAssetPath(), &isRelativeOutput);
  ...
  if (isRelativeOutput) {
    ...
    if (containingLayerInfo != _layersToCopy.end()) {
      packagePath = TfNormPath(
          TfStringCatPaths(TfGetPathName(containingLayerInfo->second), depInfo.GetAssetPath()));
    }
  }
                                                                 ...
                                                                 bool *isRelativePathOut)
{
  ...
  if (!isContextDependentPath && !_PathIsURIResolvable(refPath)) {
    ...
    if (isRelativePath) {
      ...
      const bool refPathIsOutsideAssetLocation = !resolvedRefPath.empty() &&
                                                 !TfStringStartsWith(
  ...
  if (result == layerPath) {
    ...
    // renamed root layer.
    return resultPointsToRoot && !_firstLayerName.empty() ? _firstLayerName :
...
bool UsdUtils_AssetLocalizationPackage::_AddLayerToPackage(SdfLayerRefPtr sourceLayer,
...
bool UsdUtils_AssetLocalizationPackage::_AddAssetToPackage(const std::string &srcPath,
                                                           const std::string &destPath)
{
  TF_DEBUG(USDUTILS_CREATE_PACKAGE)
