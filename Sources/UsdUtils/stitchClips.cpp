...
namespace
{
  ...
  _NormalizeClipAssetPaths(const SdfLayerHandle &resultLayer,
                           ...
                           const TfToken &clipSet)
  {
    const auto currentAssetPaths = _GetUnboxedValue<_SdfAssetArray>(resultLayer, clipPath,
  ...
  {
    return layer->HasStartTimeCode() ? layer->GetStartTimeCode() : (_HasStartFrame(layer) ? _GetStartFrame(layer) : 0.0);
  }
                                 ...
                                 const SdfPath &clipPath)
{
  ...
  if (!clipLayersAreValid || !_UsdUtilsStitchClipsManifestImpl(
                                 manifestLayer, topologyLayer, clipLayers, clipPath))
  {
    return false;
  }
                         ...
                         const TfToken &clipSet)
{
  ...
  if (!clipLayersAreValid || !_UsdUtilsStitchClipsImpl(resultLayer, topologyLayer, manifestLayer,
                                                       clipLayers, clipPath,
