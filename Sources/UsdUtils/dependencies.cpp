...
namespace
{
  ...
  public:
    ...
    using RemapAssetPathFunc = std::function<std::string(const std::string &assetPath,
    ...
    using ProcessAssetPathFunc = std::function<void(const std::string &assetPath,
                                                    const SdfLayerRefPtr &layer,
    ...
    _FileAnalyzer(const std::string &referencePath,
                  ...
                  const ProcessAssetPathFunc &processPathFunc = {}) : _filePath(resolvedPath),
    ...
    _FileAnalyzer(const SdfLayerHandle &layer,
                  ...
                  bool enableMetadataFiltering = false,
                  const RemapAssetPathFunc &remapPathFunc = {},
                  const ProcessAssetPathFunc &processPathFunc = {}) : _layer(layer),
                                                                      _refTypesToInclude(refTypesToInclude),
                                                                      _metadataFilteringEnabled(enableMetadataFiltering),
                                                                      _remapPathFunc(remapPathFunc),
                                                                      _processPathFunc(processPathFunc)
    ...
    if (_remapPathFunc)
    {
      _layer->GetSubLayerPaths().ModifyItemEdits(
          [this](const std::string &path)
          {
            ...
            return remappedPath.empty() ? boost::optional<std::string>() : boost::optional<std::string>(std::move(remappedPath));
          });
    }
    else
    {
  ...
  public:
                        ...
                        &dependenciesToSkip = std::vector<std::string>())
    {
      ...
      // of asset paths.
      const auto remapAssetPathFunc = destDir.empty() ? _FileAnalyzer::RemapAssetPathFunc() : [&layerDependenciesMap, &dirRemapper, &destDir, &rootFilePath, &origRootFilePath, &firstLayerName](const std::string &ap, const SdfLayerRefPtr &layer)
      {
                      ...
                      const std::vector<std::string> &dependenciesToSkip = std::vector<std::string>())
{
  TF_DEBUG(USDUTILS_CREATE_USDZ_PACKAGE).Msg("Creating USDZ package at '%s' "
                                             "containing asset @%s@.\n",
                                             usdzFilePath.c_str(), assetPath.GetAssetPath().c_str());
  ...
  for (auto &layerAndDestPath : layerExportMap)
  {
    ...
    TF_DEBUG(USDUTILS_CREATE_USDZ_PACKAGE).Msg(".. adding layer @%s@ to package at path '%s'.\n", layer->GetIdentifier().c_str(), destPath.c_str());
  ...
  for (auto &fileSrcAndDestPath : fileCopyMap)
  {
    ...
    TF_DEBUG(USDUTILS_CREATE_USDZ_PACKAGE).Msg(".. adding file '%s' to package at path '%s'.\n", srcPath.c_str(), destPath.c_str());
    ...
    const std::string &firstLayerName)
{
  ...
  TF_DEBUG(USDUTILS_CREATE_USDZ_PACKAGE).Msg("Flattening asset @%s@ located at '%s' to temporary layer at "
                                             "path '%s'.\n",
