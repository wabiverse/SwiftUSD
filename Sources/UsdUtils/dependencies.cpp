                                       ...
                                       std::vector<std::string> *payloads)
{
  UsdUtils_ExtractExternalReferences(
      filePath, UsdUtils_LocalizationContext::ReferenceType::All, subLayers, references, payloads);
}
                                    ...
                                    const std::function<UsdUtilsProcessingFunc> &processingFunc)
{
  ...
  UsdUtils_ReadOnlyLocalizationDelegate delegate(
      std::bind(&UsdUtils_ComputeAllDependenciesClient::Process,
                &client,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3));
  ...
  if (outLayers) {
    ...
    std::sort(outLayers->begin() + 1,
              outLayers->end(),
              [](const SdfLayerRefPtr &a, const SdfLayerRefPtr &b) {
...
{
  auto processingFunc = [&modifyFn](const SdfLayerRefPtr &,
