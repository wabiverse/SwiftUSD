...
class _FileAnalyzer {
 public:
  ...
  using ProcessAssetPathFunc = std::function<void(
      const std::string &assetPath, const SdfLayerRefPtr &layer, const _DepType &depType)>;
        ...
        _processPathFunc(processPathFunc)
  {
    ...
    if (!_layer) {
      TF_WARN("Unable to open layer at path @%s@.", referencePath.c_str());
      return;
    }
 ...
 private:
  ...
  using ShouldProcessAssetValueFunc = std::function<bool(const std::string &key)>;
...
std::string _FileAnalyzer::_RemapDependency(const std::string &rawRefPath)
{
  if (_remapPathFunc) {
...
{
  return _UpdateAssetValue(std::string(), val, [](const std::string &) { return true; });
}

VtValue _FileAnalyzer::_UpdateAssetValue(const std::string &key,
                                         const VtValue &val,
                                         const ShouldProcessAssetValueFunc shouldProcessFunc)
{
  if (val.IsHolding<SdfAssetPath>()) {
    ...
    if (!rawAssetPath.empty()) {
      ...
      return remappedPath.empty() ? VtValue() : VtValue(SdfAssetPath(remappedPath));
    }
  }
    ...
    for (const SdfAssetPath &assetPath : originalArray) {
      ...
      if (!rawAssetPath.empty()) {
        const std::string remappedPath = shouldProcessFunc(key) ?
                                             _ProcessDependency(rawAssetPath,
                                                                _DepType::Reference) :
                                             _RemapDependency(rawAssetPath);
    ...
    for (const auto &p : originalDict) {
      ...
      VtValue updatedVal = _UpdateAssetValue(dictKey, p.second, shouldProcessFunc);
...
{
  if (_remapPathFunc) {
    _layer->GetSubLayerPaths().ModifyItemEdits([this](const std::string &path) {
      std::string remappedPath = _ProcessDependency(path, _DepType::Sublayer);
      return remappedPath.empty() ? boost::optional<std::string>() :
...
{
  if (_remapPathFunc) {
    primSpec->GetPayloadList().ModifyItemEdits([this](const SdfPayload &payload) {
      return _RemapRefOrPayload<SdfPayload, _DepType::Payload>(payload);
    });
  }
  else {
...
static bool _IgnoreAssetInfoIdentifier(const std::string &key)
{
...
{
  if (_remapPathFunc) {
    primSpec->GetReferenceList().ModifyItemEdits([this](const SdfReference &ref) {
      return _RemapRefOrPayload<SdfReference, _DepType::Reference>(ref);
    });
  }
  else {
    for (SdfReference const &ref : primSpec->GetReferenceList().GetAddedOrExplicitItems()) {
...
class _AssetLocalizer {
 public:
                  ...
                  const std::vector<std::string> &dependenciesToSkip = std::vector<std::string>())
  {
    ...
    const bool isAnonymousLayer = SdfLayer::IsAnonymousLayerIdentifier(assetPathStr);
    ...
    while (!filesToLocalize.empty()) {
      ...
      for (std::string ref : layerDepIt->second) {
        ...
        if (UsdShadeUdimUtils::IsUdimIdentifier(ref)) {
          resolvedRefFilePath = UsdShadeUdimUtils::ResolveUdimPath(ref, fileAnalyzer.GetLayer());
        }
        else {
 ...
 private:
  ...
  void _ResolveUdimPaths(const std::string &srcFilePath, const std::string &destFilePath)
  {
    ...
    for (const auto &resolvedPath : resolvedPaths) {
      const std::string destUdimPath = UsdShadeUdimUtils::ReplaceUdimPattern(destFilePath,
                                                                             resolvedPath.second);
                                             ...
                                             bool *isRelativePathOut)
{
  ...
  if (!isContextDependentPath) {
    ...
    if (isRelativePath) {
      ...
      const bool refPathIsOutsideAssetLocation = !TfStringStartsWith(
          TfNormPath(TfGetPathName(resolvedRefPath)), TfNormPath(TfGetPathName(rootFilePath)));
                                       ...
                                       std::vector<std::string> *payloads)
{
  ...
  const auto processPathFunc = [&subLayers, &references, &payloads](const std::string &assetPath,
                                                                    const SdfLayerRefPtr &layer,
                                                                    const _DepType &depType) {
  ...
  if (SdfLayer::IsAnonymousLayerIdentifier(filePath)) {
    _FileAnalyzer(SdfLayer::Find(filePath),
                  refTypesToInclude,
