...
UsdUtilsDependencyInfo UsdUtils_ProcessedPathCache::GetProcessedInfo(
...
std::vector<std::string> UsdUtils_WritableLocalizationDelegate::ProcessSublayers(
    const SdfLayerRefPtr &layer)
{
  ...
  for (const std::string &sublayerPath : sublayerPaths) {
    ...
    dependencies.insert(
        dependencies.end(), info.GetDependencies().begin(), info.GetDependencies().end());
  }
...
std::vector<std::string> UsdUtils_WritableLocalizationDelegate::ProcessPayloads(
    const SdfLayerRefPtr &layer, const SdfPrimSpecHandle &primSpec)
{
  return _ProcessReferencesOrPayloads<SdfPayloadListOp, UsdUtils_DependencyType::Payload>(
      layer, primSpec, SdfFieldKeys->Payload);
}

std::vector<std::string> UsdUtils_WritableLocalizationDelegate::ProcessReferences(
    const SdfLayerRefPtr &layer, const SdfPrimSpecHandle &primSpec)
{
    ...
    const SdfLayerRefPtr &layer, const SdfPrimSpecHandle &primSpec, const TfToken &listOpToken)
{
  ...
  const bool modified = processedListOps.ModifyOperations(
      [this, &layer, &dependencies](const typename ListOpType::ItemType &item) {
        return _ProcessRefOrPayload<typename ListOpType::ItemType, DEP_TYPE>(
...
void UsdUtils_WritableLocalizationDelegate::BeginProcessValue(const SdfLayerRefPtr &layer,
                                                              const VtValue &val)
{
    ...
    const std::vector<std::string> &dependencies)
{
  ...
  if (relativeKeyPath.empty()) {
    ...
    _currentValueDictionary.SetValueAtPath(relativeKeyPath,
                                           VtValue(SdfAssetPath(info.GetAssetPath())));
  }
...
void UsdUtils_WritableLocalizationDelegate::EndProcessingValuePathArray(
    const SdfLayerRefPtr &layer, const std::string &keyPath)
{
  const std::string relativeKeyPath = _GetRelativeKeyPath(keyPath);
...
VtValue UsdUtils_WritableLocalizationDelegate::_GetUpdatedValue(const VtValue &val)
{
  ...
  if (val.IsHolding<SdfAssetPath>()) {
    ...
    updatedValue = _currentValuePath.GetAssetPath().empty() &&
    ...
    updatedValue = _currentValuePathArray.empty() && !originalArray.empty() ?
                       VtValue() :
    ...
    std::vector<std::string> dependencies)
{
  ...
  const std::string keyPath = clipSetName + ":" +
...
SdfLayerRefPtr UsdUtils_WritableLocalizationDelegate::_GetOrCreateWritableLayer(
    const SdfLayerRefPtr &layer)
{
  if (!layer) {
    return nullptr;
  ...
  SdfLayerRefPtr copiedLayer = SdfLayer::CreateAnonymous(
      layer->GetDisplayName(), layer->GetFileFormat(), layer->GetFileFormatArguments());
...
SdfLayerConstHandle UsdUtils_WritableLocalizationDelegate::GetLayerUsedForWriting(
    const SdfLayerRefPtr &layer)
{
...
std::vector<std::string> UsdUtils_ReadOnlyLocalizationDelegate::ProcessSublayers(
    const SdfLayerRefPtr &layer)
{
  std::vector<std::string> dependencies;
...
std::vector<std::string> UsdUtils_ReadOnlyLocalizationDelegate::ProcessPayloads(
    const SdfLayerRefPtr &layer, const SdfPrimSpecHandle &primSpec)
{
...
std::vector<std::string> UsdUtils_ReadOnlyLocalizationDelegate::ProcessReferences(
    const SdfLayerRefPtr &layer, const SdfPrimSpecHandle &primSpec)
{
    ...
    const SdfLayerRefPtr &layer, const std::vector<RefOrPayloadType> &appliedItems)
{
  ...
  for (const auto &refOrPayload : appliedItems) {
    ...
    UsdUtilsDependencyInfo info = _pathCache.GetProcessedInfo(
        layer, {refOrPayload.GetAssetPath(), {}}, dependencyType);
    ...
    const std::vector<std::string> &dependencies)
{
  ...
  return _AllDependenciesForInfo(_pathCache.GetProcessedInfo(
      layer, {authoredPath, dependencies}, UsdUtils_DependencyType::Reference));
}
...
{
  return _AllDependenciesForInfo(_pathCache.GetProcessedInfo(
      layer, {templateAssetPath, dependencies}, UsdUtils_DependencyType::ClipTemplateAssetPath));
}
