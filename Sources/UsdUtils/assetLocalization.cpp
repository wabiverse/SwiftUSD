...
void UsdUtils_LocalizationContext::_EnqueueDependencies(
    const SdfLayerRefPtr layer, const std::vector<std::string> &dependencies)
{
...
void UsdUtils_LocalizationContext::_EnqueueDependency(const SdfLayerRefPtr layer,
                                                      const std::string &assetPath)
{
  ...
  if (resolvedPath.empty()) {
    TF_WARN(
        "Failed to resolve reference @%s@ with computed asset path "
        "@%s@ found in layer @%s@.",
        assetPath.c_str(),
        anchoredPath.c_str(),
...
static std::string _GetTemplateAssetPathForClipSet(const SdfPrimSpecHandle &primSpec,
...
void UsdUtils_LocalizationContext::_ProcessMetadata(const SdfLayerRefPtr &layer,
                                                    const SdfPrimSpecHandle &primSpec)
{
  ...
  for (const auto &clipSet : clipSets) {
    ...
    const std::vector<std::string> dependencies = _delegate->ProcessClipTemplateAssetPath(
        layer, primSpec, clipSet, templatePath, clipFiles);
...
std::vector<std::string> UsdUtils_LocalizationContext::_GetTemplatedClips(
    const SdfLayerRefPtr &layer, const std::string &templateAssetPath)
{
...
void UsdUtils_LocalizationContext::_ProcessPayloads(const SdfLayerRefPtr &layer,
                                                    const SdfPrimSpecHandle &primSpec)
{
...
void UsdUtils_LocalizationContext::_ProcessReferences(const SdfLayerRefPtr &layer,
                                                      const SdfPrimSpecHandle &primSpec)
{
...
void UsdUtils_LocalizationContext::_ProcessProperties(const SdfLayerRefPtr &layer,
                                                      const SdfPrimSpecHandle &primSpec)
{
                                                      ...
                                                      bool processingMetadata)
{
  ...
  if (val.IsHolding<SdfAssetPath>()) {
    ...
    const std::vector<std::string> processedDeps = _delegate->ProcessValuePath(
    ...
    for (const SdfAssetPath &assetPath : originalArray) {
      ...
      const std::vector<std::string> processedDeps = _delegate->ProcessValuePathArrayElement(
          layer, keyPath, rawAssetPath, dependencies);
...
std::vector<std::string> UsdUtils_LocalizationContext::_GetDependencies(
    const SdfLayerRefPtr &layer, const std::string &assetPath)
{
...
bool UsdUtils_LocalizationContext::_ShouldFilterAssetPath(const std::string &key,
                                                          bool processingMetadata)
{
...
{
  return val.IsHolding<SdfAssetPath>() || val.IsHolding<VtArray<SdfAssetPath>>() ||
    ...
    std::vector<std::string> *outPayloads)
{
  ...
  UsdUtils_ReadOnlyLocalizationDelegate delegate(
      std::bind(&UsdUtils_ExtractExternalReferencesClient::Process,
                &client,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3));
