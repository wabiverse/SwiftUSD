...
class memory_input_base<tracking_mode::eager, Eol, Source> {
 public:
  ...
  memory_input_base(const iterator_t &in_begin,
                    const char *in_end,
                    T &&in_source) noexcept(std::is_nothrow_constructible_v<Source, T &&>)
...
class memory_input : public internal::memory_input_base<P, Eol, Source> {
 public:
  ...
  template<typename T>
  memory_input(const std::string_view in_string,
               T &&in_source) noexcept(std::is_nothrow_constructible_v<Source, T &&>)
      : memory_input(in_string.data(), in_string.size(), std::forward<T>(in_source))
  {
  }
  ...
  memory_input(const char *in_begin,
               ...
               const std::size_t in_column) noexcept(std::is_nothrow_constructible_v<Source, T &&>)
  ...
  template<rewind_mode M> void restart(const internal::marker<iterator_t, M> &m) noexcept
  {
    iterator() = m.iterator();
  }
  ...
  template<rewind_mode M> [[nodiscard]] internal::marker<iterator_t, M> mark() noexcept
  {
