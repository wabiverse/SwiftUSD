...
namespace internal {
template<typename Peek, typename... Ps> struct predicates_and_test {
  using peek_t = Peek;
...
template<typename Peek, typename P> struct predicate_not_test {
  using peek_t = Peek;
  using data_t = typename Peek::data_t;
...
template<typename Peek, typename... Ps> struct predicates_or_test {
  using peek_t = Peek;
  using data_t = typename Peek::data_t;
...
struct predicates : private Test<Peek, Ps...> {
  using peek_t = Peek;
  using data_t = typename Peek::data_t;
  ...
  template<int Eol> static constexpr bool can_match_eol = test(Eol);
...
template<template<typename, typename...> class Test, typename Peek> struct predicates<Test, Peek> {
...
struct analyze_traits<Name, internal::predicates<Test, Peek, Ps...>> : analyze_any_traits<> {};
