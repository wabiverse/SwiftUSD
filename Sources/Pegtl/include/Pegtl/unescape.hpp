...
[[nodiscard]] inline bool utf8_append_utf32(std::string &string, const unsigned utf32)
{
  ...
  if (utf32 <= 0x7ff) {
    char tmp[] = {char(((utf32 & 0x7c0) >> 6) | 0xc0), char(((utf32 & 0x03f)) | 0x80)};
...
template<typename I> [[nodiscard]] I unhex_char(const char c)
{
  switch (c) {
    case '0':
    case '1':
...
template<typename I> [[nodiscard]] I unhex_string(const char *begin, const char *end)
{
  I r = 0;
  while (begin != end) {
    r <<= 4;
...
struct append_all {
  template<typename ActionInput> static void apply(const ActionInput &in, std::string &s)
  {
...
template<typename T, char... Rs> struct unescape_c {
  ...
  [[nodiscard]] static char apply_two(const ActionInput &in,
...
#else
  template<typename ActionInput> static bool apply(const ActionInput &in, std::string &s)
  {
    assert(!in.empty());  // First character MUST be present, usually 'u' or 'U'.
