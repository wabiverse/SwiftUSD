...
namespace internal {
[[nodiscard]] inline auto get_position(const position &p) noexcept(
    std::is_nothrow_copy_constructible_v<position>)
{
  return p;
}

template<typename ParseInput>
[[nodiscard]] position get_position(const ParseInput &in) noexcept(
    noexcept(position(in.position())))
{
  return in.position();
}
