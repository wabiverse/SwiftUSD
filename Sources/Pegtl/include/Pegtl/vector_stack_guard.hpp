...
template<typename... Cs> class [[nodiscard]] vector_stack_guard {
 public:
  template<typename... Ts> vector_stack_guard(std::vector<Cs...> &vector, Ts &&...ts) : m_s(vector)
  {
...
vector_stack_guard(std::vector<Cs...> &,
                   const typename std::vector<Cs...>::value_type &) -> vector_stack_guard<Cs...>;
