...
template<char Open, char Marker> struct raw_string_open {
  ...
  [[nodiscard]] static bool match(ParseInput &in,
                                  std::size_t &marker_size) noexcept(noexcept(in.size(0)))
  {
...
template<char Marker, char Close> struct at_raw_string_close {
  ...
  template<apply_mode A,
           ...
           typename ParseInput>
  [[nodiscard]] static bool match(ParseInput &in,
                                  const std::size_t &marker_size) noexcept(noexcept(in.size(0)))
  {
...
struct raw_string_until : raw_string_until<Cond, seq<Rules...>> {};
...
template<typename Cond> struct raw_string_until<Cond> {
                                  ...
                                  States &&...)
  {
    ...
    while (
        !Control<Cond>::template match<A, rewind_mode::required, Action, Control>(in, marker_size))
    {
      if (in.empty()) {
        return false;
      }
...
template<typename Cond, typename Rule> struct raw_string_until<Cond, Rule> {
  using rule_t = raw_string_until;
...
template<char Open, char Marker, char Close, typename... Contents> struct raw_string {
  ...
  [[nodiscard]] static bool match(ParseInput &in, States &&...st)
  {
    ...
    if (Control<internal::raw_string_open<Open, Marker>>::template match<A, M, Action, Control>(
            in, marker_size))
    {
...
struct analyze_traits<Name, raw_string<Open, Marker, Close>> : analyze_any_traits<> {};
