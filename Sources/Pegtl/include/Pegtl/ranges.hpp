...
template<typename Peek, typename Peek::data_t... Cs> struct ranges {
  ...
  [[nodiscard]] static constexpr bool test(std::index_sequence<Is...> /*unused*/,
                                           const data_t c) noexcept
  {
...
struct ranges<Peek, C> : one<result_on_found::success, Peek, C> {};

template<typename Peek> struct ranges<Peek> : failure {};
