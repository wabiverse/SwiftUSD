...
struct analyze_entry {
  explicit analyze_entry(const analyze_type in_type) noexcept : type(in_type) {}
...
class analyze_cycles_impl {
 public:
  ...
  [[nodiscard]] std::size_t problems()
  {
    ...
    // The number of problems returned is not very informative as some problems will be found
    // multiple times.
    return m_problems;
  }

  template<typename Rule> [[nodiscard]] bool consumes() const
  {
    // The name "consumes" is a shortcut for "the analyze cycles algorithm could prove that this
    // rule always consumes when it succeeds".
    return m_results.at(demangle<Rule>());
  }

 protected:
  explicit analyze_cycles_impl(const int verbose) noexcept : m_verbose(verbose), m_problems(0) {}

  [[nodiscard]] const std::pair<const std::string_view, analyze_entry> &find(
  ...
  [[nodiscard]] bool work(const std::pair<const std::string_view, analyze_entry> &entry,
                          const bool accum)
  {
    ...
    if (!accum) {
      ...
      if ((m_verbose >= 0) && (m_trace.front() == entry.first)) {
        ...
        std::cerr << "WARNING: Possible cycle without progress at rule " << entry.first
                  << std::endl;
        if (m_verbose > 0) {
          for (const auto &r : m_trace) {
...
void analyze_insert_impl(type_list<Subs...> /*unused*/,
                         std::vector<std::string_view> &subs,
                         std::map<std::string_view, analyze_entry> &entry)
{
  (subs.emplace_back(analyze_insert<Subs>(entry)), ...);
}

template<typename Grammar> struct analyze_cycles : analyze_cycles_impl {
  explicit analyze_cycles(const int verbose) : analyze_cycles_impl(verbose)
  {
...
template<typename Grammar> [[nodiscard]] std::size_t analyze(const int verbose = 1)
{
