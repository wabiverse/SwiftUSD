...
struct mmap_holder {
  ...
  explicit mmap_holder(const internal::filesystem::path &path) : data(path) {}
...
struct mmap_input : private internal::mmap_holder, public memory_input<P, Eol> {
  mmap_input(const internal::filesystem::path &path, const std::string &source)
      : internal::mmap_holder(path), memory_input<P, Eol>(data.begin(), data.end(), source)
  {
  }
