...
namespace
{
  ...
  inline static std::vector<std::shared_ptr<T>>
  ...
  template <typename T>
  inline static std::vector<mx::ElementPtr>
      ...
      const D &usdDownstream)
  {
    ...
    {
      TF_DEBUG(USDMTLX_READER).Msg("    + Connected <%s> -> <%s>\n", sourcePath.GetText(), usdDownstream.GetAttr().GetPath().GetText());
    }
  }
  ...
  private:
    ...
    std::map<std::string,
             std::map<std::string, UsdShadeConnectableAPI>>
        _shaders;
  ...
  _Context::_Context(const UsdStagePtr &stage, const SdfPath &internalPath)
      : _stage(stage), _collectionsPath(internalPath.AppendChild(TfToken("Collections"))), _looksPath(internalPath.AppendChild(TfToken("Looks"))), _materialsPath(internalPath.AppendChild(TfToken("Materials"))), _nodeGraphsPath(internalPath.AppendChild(TfToken("NodeGraphs"))), _shadersPath(internalPath.AppendChild(TfToken("Shaders")))
  {
    // Do nothing
  }
      ...
      const mx::ConstDocumentPtr &mtlxDocument)
  {
    ...
    if (!nodeGraph)
    {
      ...
      if (mtlxNodeGraph)
      {
        TF_DEBUG(USDMTLX_READER).Msg("Add node graph: %s at path %s\n", mtlxNodeGraph->getName().c_str(), _nodeGraphsPath.GetString().c_str());
  ...
  _Context::AddShaderNode(const mx::ConstNodePtr &mtlxShaderNode)
  {
    ...
    for (mx::InputPtr mtlxInput : mtlxShaderNode->getInputs())
    {
      ...
      if (auto outputName = _Attr(mtlxInput, names.output))
      {
        ...
        mx::NodeGraphPtr mtlxNodeGraph = mtlxInput->getDocument()->getNodeGraph(_Attr(mtlxInput, names.nodegraph).str());
