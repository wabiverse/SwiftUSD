...
// static
PxOsdMeshTopology GeomUtilCylinderMeshGenerator::GenerateTopology(const size_t numRadial,
                                                                  const bool closedSweep)
{
  if (numRadial < minNumRadial) {
    ...
    const _PointWriter<PointType> &ptWriter)
{
  ...
  for (size_t ringIdx = 0; ringIdx < 2; ++ringIdx) {
    for (size_t radIdx = 0; radIdx < numRadialPoints; ++radIdx) {
      ptWriter.Write(
          PointType(bottomRadius * ringXY[radIdx][0], bottomRadius * ringXY[radIdx][1], zMin));
    }
  }
