...
HdEmbreeMesh::HdEmbreeMesh(SdfPath const &id)
    : HdMesh(id), _rtcMeshId(RTC_INVALID_GEOMETRY_ID), _rtcMeshScene(nullptr), _adjacencyValid(false), _normalsValid(false), _refined(false), _smoothNormals(false), _doubleSided(false), _cullStyle(HdCullStyleDontCare)
{
}
...
void HdEmbreeMesh::_InitRepr(TfToken const &reprToken,
  ...
  if (numEdgeCreases > 0)
  {
    ...
    for (size_t i = 0; i < creaseLengths.size(); ++i)
    {
      ...
      for (int j = 0; j < numEdges; ++j)
      {
        ...
        embreeCreaseWeights[embreeEdgeIndex] = weightPerCrease ? creaseWeights[i] : creaseWeights[embreeEdgeIndex];
...
void HdEmbreeMesh::_UpdatePrimvarSources(HdSceneDelegate *sceneDelegate,
                                         HdDirtyBits dirtyBits)
{
  HD_TRACE_FUNCTION();
...
void HdEmbreeMesh::_CreatePrimvarSampler(TfToken const &name, VtValue const &data,
