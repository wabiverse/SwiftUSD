...
// static
size_t GeomUtilCylinderMeshGenerator::ComputeNumPoints(const size_t numRadial,
...
// static
PxOsdMeshTopology
GeomUtilCylinderMeshGenerator::GenerateTopology(const size_t numRadial,
                                                const bool closedSweep) {
  if (numRadial < minNumRadial) {
  ...
  return _GenerateCappedQuadTopology(
      ...
      /* topCapStyle =    */ CapStyleSeparateEdge, closedSweep);
}

// static
template <typename PointType>
void GeomUtilCylinderMeshGenerator::_GeneratePointsImpl(
    const size_t numRadial, const typename PointType::ScalarType bottomRadius,
  ...
  for (size_t ringIdx = 0; ringIdx < 2; ++ringIdx) {
    for (size_t radIdx = 0; radIdx < numRadialPoints; ++radIdx) {
      ptWriter.Write(PointType(bottomRadius * ringXY[radIdx][0],
                               bottomRadius * ringXY[radIdx][1], zMin));
    }
  }
  ...
  for (size_t ringIdx = 0; ringIdx < 2; ++ringIdx) {
    for (size_t radIdx = 0; radIdx < numRadialPoints; ++radIdx) {
      ptWriter.Write(PointType(topRadius * ringXY[radIdx][0],
                               topRadius * ringXY[radIdx][1], zMax));
    }
  }
