...
HdSt_MaterialNetworkShader::HdSt_MaterialNetworkShader()
    : HdStShaderCode(), _fragmentSource(), _geometrySource(), _params(), _paramSpec(), _paramArray(), _primvarNames(_CollectPrimvarNames(_params)), _isEnabledPrimvarFiltering(_IsEnabledMaterialPrimvarFiltering()), _computedHash(0), _isValidComputedHash(false), _computedTextureSourceHash(0), _isValidComputedTextureSourceHash(false), _materialTag()
{
}

HdSt_MaterialNetworkShader::~HdSt_MaterialNetworkShader() = default;

void HdSt_MaterialNetworkShader::_SetSource(
...
/*virtual*/
void HdSt_MaterialNetworkShader::AddBindings(
...
HdSt_MaterialNetworkShader::ComputeTextureSourceHash() const
{
  ...
  const bool useBindlessHandles = _namedTextureHandles.empty() ? false : _namedTextureHandles[0].handle->UseBindlessHandles();
...
/*static*/
bool HdSt_MaterialNetworkShader::CanAggregate(HdStShaderCodeSharedPtr const &shaderA,
    ...
    HdBufferSourceSharedPtrVector *const sources)
{
  const TfToken sourceName(
      param.name.GetString() + HdSt_ResourceBindingSuffixTokens->fallback.GetString());
