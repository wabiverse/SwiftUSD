...
HdSt_PipelineDrawBatch::HdSt_PipelineDrawBatch(
    ...
    : HdSt_DrawBatch(drawItemInstance), _drawCommandBufferDirty(false), _bufferArraysHash(0), _barElementOffsetsHash(0), _numVisibleItems(0), _numTotalVertices(0), _numTotalElements(0)
      /* The following two values are set before draw by
       * SetEnableTinyPrimCulling(). */
      ,
      _useTinyPrimCulling(false), _dirtyCullingProgram(false)
      /* The following four values are initialized in _Init(). */
      ,
      _useDrawIndexed(true), _useInstancing(false), _useGpuCulling(false), _useInstanceCulling(false), _allowGpuFrustumCulling(allowGpuFrustumCulling), _allowIndirectCommandEncoding(allowIndirectCommandEncoding), _instanceCountOffset(0), _cullInstanceCountOffset(0), _drawCoordOffset(0), _patchBaseVertexByteOffset(0)
{
...
void HdSt_PipelineDrawBatch::_CompileBatch(
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  TRACE_FUNCTION();
...
HdSt_PipelineDrawBatch::Validate(bool deepValidation)
{
  ...
  TF_DEBUG(HDST_DRAW_BATCH).Msg("Validating pipeline draw batch %p (deep validation = %d)...\n", (void *)(this), deepValidation);
  ...
  if (deepValidation)
  {
    ...
    for (size_t item = 0; item < numDrawItemInstances; ++item)
    {
      ...
      {
        TF_DEBUG(HDST_DRAW_BATCH).Msg("   Deep validation: Found draw item that fails aggregation"
    ...
    {
      TF_DEBUG(HDST_DRAW_BATCH).Msg("   Deep validation: Element offsets hash mismatch."
...
void HdSt_PipelineDrawBatch::PrepareDraw(
...
void HdSt_PipelineDrawBatch::EncodeDraw(
    HdStRenderPassStateSharedPtr const &renderPassState,
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  if (_HasNothingToDraw())
...
namespace
{
  ...
  struct _BindingState : public _DrawItemState
  {
    _BindingState(
        ...
        : _DrawItemState(drawItem), dispatchBuffer(dispatchBuffer), binder(binder), glslProgram(glslProgram), shaders(shaders), geometricShader(geometricShader)
    {
    }
...
void HdSt_PipelineDrawBatch::ExecuteDraw(
    HgiGraphicsCmds *gfxCmds,
    HdStRenderPassStateSharedPtr const &renderPassState,
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
...
void HdSt_PipelineDrawBatch::_ExecuteDrawIndirect(
    HgiGraphicsCmds *gfxCmds,
    HdStBufferArrayRangeSharedPtr const &indexBar)
{
  TRACE_FUNCTION();

  HdStBufferResourceSharedPtr paramBuffer = _dispatchBuffer->GetBufferArrayRange()->GetResource(HdTokens->drawDispatch);
...
void HdSt_PipelineDrawBatch::_ExecuteDrawImmediate(
    HgiGraphicsCmds *gfxCmds,
    HdStBufferArrayRangeSharedPtr const &indexBar)
{
  TRACE_FUNCTION();
...
void HdSt_PipelineDrawBatch::_ExecutePTCS(
...
void HdSt_PipelineDrawBatch::DrawItemInstanceChanged(
    HdStDrawItemInstance const *instance)
{
  ...
  std::vector<uint32_t>::iterator instanceCountIt =
      _drawCommandBuffer.begin() + batchIndex * commandNumUints + _instanceCountOffset;
  std::vector<uint32_t>::iterator cullInstanceCountIt =
      _drawCommandBuffer.begin() + batchIndex * commandNumUints + _cullInstanceCountOffset;
...
void HdSt_PipelineDrawBatch::_CullingProgram::Initialize(
    bool useDrawIndexed,
