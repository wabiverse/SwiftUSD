  ...
  void Process(SdfLayerHandle layer, ReportParams const &p)
  {
             ...
             p.outputType == OutputLayer)
    {
      ...
      else
      {
        ...
        outputLayer = !p.outputFile.empty() ? SdfLayer::CreateNew(p.outputFile, formatArgs) : SdfLayer::CreateAnonymous(p.outputFormat.empty() ? string() : TfStringPrintf(".%s", p.outputFormat.c_str()));
      }
...
int main(int argc, char const *argv[])
{
  ...
  {
    arraySizeLimit =
        outputType == OutputPseudoLayer ? 8 : outputType == OutputLayer ? -1
                                                                        : 0;
  }
