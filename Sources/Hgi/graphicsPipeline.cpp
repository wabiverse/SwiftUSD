...
HgiVertexAttributeDesc::HgiVertexAttributeDesc()
    : format(HgiFormatFloat32Vec4), offset(0), shaderBindLocation(0)
{
}
...
HgiVertexBufferDesc::HgiVertexBufferDesc()
    : bindingIndex(0), vertexStepFunction(HgiVertexBufferStepFunctionPerVertex), vertexStride(0)
{
}
...
HgiMultiSampleState::HgiMultiSampleState()
    : multiSampleEnable(true), alphaToCoverageEnable(false), alphaToOneEnable(false), sampleCount(HgiSampleCount1)
{
}
...
HgiRasterizationState::HgiRasterizationState()
    : polygonMode(HgiPolygonModeFill), lineWidth(1.0f), cullMode(HgiCullModeBack), winding(HgiWindingCounterClockwise), rasterizerEnabled(true), depthClampEnabled(false), depthRange(0.f, 1.f), conservativeRaster(false), numClipDistances(0)
{
}
...
HgiDepthStencilState::HgiDepthStencilState()
    : depthTestEnabled(true), depthWriteEnabled(true), depthCompareFn(HgiCompareFunctionLess), depthBiasEnabled(false), depthBiasConstantFactor(0.0f), depthBiasSlopeFactor(0.0f), stencilTestEnabled(false), stencilFront(), stencilBack()
{
}
...
HgiStencilState::HgiStencilState()
    : compareFn(HgiCompareFunctionAlways), referenceValue(0), stencilFailOp(HgiStencilOpKeep), depthFailOp(HgiStencilOpKeep), depthStencilPassOp(HgiStencilOpKeep), readMask(0xffffffff), writeMask(0xffffffff)
{
}
...
HgiGraphicsShaderConstantsDesc::HgiGraphicsShaderConstantsDesc()
    : byteSize(0), stageUsage(HgiShaderStageFragment)
{
}
...
HgiTessellationLevel::HgiTessellationLevel()
    : innerTessLevel{0, 0}, outerTessLevel{0, 0, 0, 0}
{
}

HgiTessellationState::HgiTessellationState()
    : patchType(Triangle), primitiveIndexSize(0), tessellationLevel()
{
}
