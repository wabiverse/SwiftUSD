...
TF_REGISTRY_FUNCTION(TfType) { TfType::Define<VtDictionary>(); }
...
VtDictionary::iterator VtDictionary::end() { return iterator(); }
...
bool VtDictionary::empty() const { return _dictMap ? _dictMap->empty() : true; }

void VtDictionary::swap(VtDictionary &dict) { _dictMap.swap(dict._dictMap); }
...
VtDictionary::insert(const value_type &obj) {
  ...
  return std::pair<iterator, bool>(iterator(_dictMap.get(), inserted.first),
                                   inserted.second);
}

VtValue const *VtDictionary::GetValueAtPath(string const &keyPath,
...
VtDictionary::GetValueAtPath(vector<string> const &keyElems) const {
  ...
  vector<string>::const_iterator start = keyElems.begin(),
                                 last = keyElems.end() - 1;
...
void VtDictionary::_SetValueAtPathImpl(
    vector<string>::const_iterator curKeyElem,
    vector<string>::const_iterator keyElemEnd, VtValue const &value) {
...
void VtDictionary::SetValueAtPath(string const &keyPath, VtValue const &value,
...
void VtDictionary::SetValueAtPath(vector<string> const &keyPath,
                                  VtValue const &value) {
  if (keyPath.empty())
    return;
...
void VtDictionary::_EraseValueAtPathImpl(
    vector<string>::const_iterator curKeyElem,
...
void VtDictionary::EraseValueAtPath(string const &keyPath,
                                    char const *delimiters) {
  vector<string> keyElems = TfStringSplit(keyPath, delimiters);
  if (keyElems.empty())
    return;
...
VtDictionary const &VtGetEmptyDictionary() { return *_emptyDictionary; }
...
void VtDictionaryOver(VtDictionary *strong, const VtDictionary &weak,
                      bool coerceToWeakerOpinionType) {
  if (!strong) {
...
void VtDictionaryOver(const VtDictionary &strong, VtDictionary *weak,
                      bool coerceToWeakerOpinionType) {
  if (!weak) {
    TF_CODING_ERROR("VtDictionaryOver: NULL dictionary pointer");
    return;
  ...
  if (coerceToWeakerOpinionType) {
    ...
    TF_FOR_ALL(it, strong) { (*weak)[it->first] = it->second; }
  }
}
...
void VtDictionaryOverRecursive(VtDictionary *strong, const VtDictionary &weak,
                               bool coerceToWeakerOpinionType) {
  if (!strong) {
...
void VtDictionaryOverRecursive(const VtDictionary &strong, VtDictionary *weak,
                               bool coerceToWeakerOpinionType) {
  if (!weak) {
    TF_CODING_ERROR("VtDictionaryOverRecursive: NULL dictionary pointer.");
    return;
