...
class VtValue {
  ...
  using _IsTriviallyCopyable =
      std::integral_constant<bool, boost::has_trivial_constructor<T>::value &&
  ...
  private:
    ...
    using _ProxyHoldsTypeFunc = bool (*)(_Storage const &,
                                         std::type_info const &);
  ...
  protected:
    constexpr _TypeInfo(
        const std::type_info &ti, const std::type_info &elementTi,
        int knownTypeIndex, bool isArray, bool isHashable, bool isProxy,
        _CopyInitFunc copyInit, _DestroyFunc destroy, _MoveFunc move,
        _CanHashFunc canHash, _HashFunc hash, _EqualFunc equal,
        _EqualPtrFunc equalPtr, _MakeMutableFunc makeMutable,
        _GetPyObjFunc getPyObj, _StreamOutFunc streamOut,
        _GetTypeidFunc getTypeid, _IsArrayValuedFunc isArrayValued,
        _GetElementTypeidFunc getElementTypeid, _GetShapeDataFunc getShapeData,
        _GetNumElementsFunc getNumElements, _ProxyHoldsTypeFunc proxyHoldsType,
        ...
        : typeInfo(ti), elementTypeInfo(elementTi),
          knownTypeIndex(knownTypeIndex), isProxy(isProxy), isArray(isArray),
          isHashable(isHashable)
          // Function table
          ,
          _copyInit(copyInit), _destroy(destroy), _move(move),
          _canHash(canHash), _hash(hash), _equal(equal), _equalPtr(equalPtr),
          _makeMutable(makeMutable), _getPyObj(getPyObj), _streamOut(streamOut),
          _getTypeid(getTypeid), _isArrayValued(isArrayValued),
          _getElementTypeid(getElementTypeid), _getShapeData(getShapeData),
          _getNumElements(getNumElements), _proxyHoldsType(proxyHoldsType),
          _getProxiedType(getProxiedType), _getProxiedTypeid(getProxiedTypeid),
  ...
  struct _ArrayHelper<Array,
                      typename std::enable_if<VtIsArray<Array>::value>::type> {
  ...
  template <class T>
  static inline auto _TypedProxyEqualityImpl(T const &a, T const &b, int)
      -> decltype(a == b) {
    return a == b;
  }
  // Function used in case T does not have equality comparison.
  template <class NoEqual>
  static inline bool _TypedProxyEqualityImpl(NoEqual const &a, NoEqual const &b,
                                             long) {
    return VtGetProxiedObject(a) == VtGetProxiedObject(b);
  }

  template <class T>
  static inline auto _ErasedProxyEqualityImpl(T const &a, T const &b, int)
      -> decltype(a == b) {
    return a == b;
  }
  // Function used in case T does not have equality comparison.
  template <class NoEqual>
  static inline bool _ErasedProxyEqualityImpl(NoEqual const &a,
                                              NoEqual const &b, long) {
  ...
  struct _TypeInfoImpl : public _TypeInfo {
    ...
    constexpr _TypeInfoImpl()
        : _TypeInfo(typeid(T), _ArrayHelper<T>::GetElementTypeid(),
                    Vt_KnownValueTypeDetail::GetIndex<T>(), VtIsArray<T>::value,
                    VtIsHashable<T>(), IsProxy, &This::_CopyInit,
                    &This::_Destroy, &This::_Move, &This::_CanHash,
                    &This::_Hash, &This::_Equal, &This::_EqualPtr,
                    &This::_MakeMutable, &This::_GetPyObj, &This::_StreamOut,

                    &This::_GetTypeid,

                    // Array support.
                    &This::_IsArrayValued, &This::_GetElementTypeid,
                    &This::_GetShapeData, &This::_GetNumElements,

                    // Proxy support.
                    &This::_ProxyHoldsType, &This::_GetProxiedType,
                    &This::_GetProxiedTypeid, &This::_GetProxiedObjPtr,
  ...
  private:
    ...
    static std::ostream &_StreamOut(_Storage const &storage,
                                    std::ostream &out) {
    ...
    static bool _ProxyHoldsType(_Storage const &storage,
  ...
  template <class T>
  struct _RemoteTypeInfo
      : _TypeInfoImpl<T,                                 // type
                      boost::intrusive_ptr<_Counted<T>>, // container
                      _RemoteTypeInfo<T>                 // CRTP
                      > {
    constexpr _RemoteTypeInfo()
        : _TypeInfoImpl<T, boost::intrusive_ptr<_Counted<T>>,
                        _RemoteTypeInfo<T>>() {}
  ...
  template <class T> struct _TypeInfoFor {
    ...
    typedef std::conditional_t<_UsesLocalStore<T>::value, _LocalTypeInfo<T>,
                               _RemoteTypeInfo<T>>
        Type;
  };
  ...
  template <class T>
  std::enable_if_t<std::is_same<T, typename Vt_ValueGetStored<T>::Type>::value>
  _Init(T const &obj) {
    _info = GetTypeInfo<T>();
...
public:
  ...
  /// a TfPyObjWrapper.
  template <class T> explicit VtValue(T const &obj) { _Init(obj); }
...
#ifndef doxygen
  template <class T>
  inline std::enable_if_t<_TypeInfoFor<T>::Type::IsLocal &&
...
#else
  template <class T>
  std::enable_if_t<!_TypeInfoFor<T>::Type::IsLocal ||
                       !_TypeInfoFor<T>::Type::HasTrivialCopy,
                   VtValue &>
  ...
  template <class T> T const &UncheckedGet() const & { return _Get<T>(); }

  /// \overload In case *this is an rvalue, move the held value out and return
  /// by value.
  template <class T> T UncheckedGet() && { return UncheckedRemove<T>(); }
  ...
  ///
  /// \sa \ref VtValue_Casting
  VT_API static VtValue CastToTypeid(VtValue const &val,
...
private:
  ...
  // not already \p to.
  VT_API static VtValue _PerformCast(std::type_info const &to,
                                     VtValue const &val);

  // Return true if \p from == \p to or if there is a registered cast to
  // convert VtValues holding \p from to \p to.
  VT_API static bool _CanCast(std::type_info const &from,
                              std::type_info const &to);
...
#define _VT_DECLARE_ZERO_VALUE_FACTORY(r, unused, elem)                        \
  template <>                                                                  \
  VT_API Vt_DefaultValueHolder                                                 \
  Vt_DefaultValueFactory<VT_TYPE(elem)>::Invoke();

BOOST_PP_SEQ_FOR_EACH(_VT_DECLARE_ZERO_VALUE_FACTORY, unused,
                      VT_VEC_VALUE_TYPES VT_MATRIX_VALUE_TYPES
...
template <> inline VtValue VtValue::Get<VtValue>() && {
...
template <> inline VtValue VtValue::UncheckedGet<VtValue>() && {
  return std::move(*this);
}

template <> inline bool VtValue::IsHolding<VtValue>() const { return true; }

// Specialize VtValue::IsHolding<void>() to always return false.
template <> inline bool VtValue::IsHolding<void>() const { return false; }
