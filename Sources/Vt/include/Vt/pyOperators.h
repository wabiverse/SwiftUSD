...
namespace {
template <class T> struct _ArrayPyOpHelp {
...
template <> struct _ArrayPyOpHelp<bool> {
...
#define VTOPERATOR_WRAP_PYTYPE_BOOL(func, pytype, op)                          \
  VTOPERATOR_WRAP_PYTYPE_BOOL_BASE(func, VtArray<T> const &vec,                \
                                   pytype const &obj,                          \
                                   (vec[i] op(T) extract<T>(obj[i])))          \
  VTOPERATOR_WRAP_PYTYPE_BOOL_BASE(func, pytype const &obj,                    \
                                   VtArray<T> const &vec,                      \
                                   ((T)extract<T>(obj[i]) op vec[i]))

#define VTOPERATOR_WRAP_BOOL(func, op)                                         \
  VTOPERATOR_WRAP_PYTYPE_BOOL(func, boost::python::list, op)                   \
  VTOPERATOR_WRAP_PYTYPE_BOOL(func, boost::python::tuple, op)

// to be used to actually declare the wrapping with def() on the class
#define VTOPERATOR_WRAPDECLARE_BOOL(func)                                      \
  def(#func, (VtArray<bool>(*)(VtArray<Type> const &,                          \
                               VtArray<Type> const &))Vt##func<Type>);         \
  def(#func,                                                                   \
      (VtArray<bool>(*)(Type const &, VtArray<Type> const &))Vt##func<Type>);  \
  def(#func,                                                                   \
      (VtArray<bool>(*)(VtArray<Type> const &, Type const &))Vt##func<Type>);  \
  def(#func,                                                                   \
      (VtArray<bool>(*)(VtArray<Type> const &, boost::python::tuple const &))Vt##func<Type>); \
  def(#func,                                                                   \
      (VtArray<bool>(*)(boost::python::tuple const &, VtArray<Type> const &))Vt##func<Type>); \
  def(#func,                                                                   \
      (VtArray<bool>(*)(VtArray<Type> const &, list const &))Vt##func<Type>);  \
  def(#func,                                                                   \
      (VtArray<bool>(*)(list const &, VtArray<Type> const &))Vt##func<Type>);
