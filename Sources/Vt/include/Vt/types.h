...
using Vt_ValueTypeList =
    TfMetaApply<TfMetaTail,
                TfMetaList<void TF_PP_SEQ_FOR_EACH(_VT_MAP_TYPE_LIST, ~, VT_VALUE_TYPES)>>;

namespace Vt_KnownValueTypeDetail {

// Implement compile-time value type indexes.
// Base case -- unknown types get index -1.
template<typename T> constexpr int GetIndexImpl(TfMetaList<>)
{
  return -1;
}

template<typename T, typename Typelist> constexpr int GetIndexImpl(Typelist)
{
  if (std::is_same_v<T, TfMetaApply<TfMetaHead, Typelist>>) {
    return 0;
  }
  else if (const int indexOfTail = GetIndexImpl<T>(TfMetaApply<TfMetaTail, Typelist>{});
           indexOfTail >= 0)
  {
...
template<typename T> constexpr int GetIndex()
{
