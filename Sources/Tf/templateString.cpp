...
static const char *const _IdentChars = "abcdefghijklmnopqrstuvwxyz"
...
bool TfTemplateString::_FindNextPlaceHolder(size_t *pos,
  ...
  *pos = _data->template_.find(_Sigil, *pos);
  ...
  if (_data->template_[nextpos] == _Sigil) {
    ...
    *pos = *pos + 2;
    ...
    size_t len =
        (endpos == string::npos ? _data->template_.length() : endpos) - *pos;
...
string TfTemplateString::_Evaluate(const Mapping &mapping,
                                   vector<string> *errors) const {
  string result;
  size_t pos = 0;
  ...
  TF_FOR_ALL(it, _data->placeholders) {
    ...
    result.insert(result.end(), _data->template_.begin() + pos,
