...
void TfNotice::_DelivererBase::_BeginDelivery(
    const TfNotice &notice, const TfWeakBase *sender,
    const type_info &senderType, const TfWeakBase *listener,
...
void TfNotice::_DelivererBase::_EndDelivery(
    const vector<TfNotice::WeakProbePtr> &probes) {
...
size_t TfNotice::_Send(const TfWeakBase *s, const void *senderUniqueId,
  ...
  return Tf_NoticeRegistry::_GetInstance()._Send(*this, noticeType, s,
                                                 senderUniqueId, senderType);
}

size_t TfNotice::_SendWithType(const TfType &noticeType, const TfWeakBase *s,
...
size_t TfNotice::Send() const { return _Send(0, 0, typeid(void)); }
...
void TfNotice::_VerifyFailedCast(const type_info &toType,
  ...
  Tf_NoticeRegistry::_GetInstance()._VerifyFailedCast(toType, notice,
                                                      castNotice);
}
