...
template<typename WeakPtr = void> struct RefPtrFactory {
  template<typename FactoryResultPtr> struct apply {
    using WeakPtrType = std::conditional_t<std::is_same<WeakPtr, void>::value,
...
template<typename SIG, size_t EXTRA_ARITY = 0> struct CtorBase {
  typedef SIG Sig;
...
template<typename SIG, size_t EXTRA_ARITY> SIG *CtorBase<SIG, EXTRA_ARITY>::_func = nullptr;
...
template<typename R, typename... Args> struct InitCtor<R(Args...)> : CtorBase<R(Args...)> {
  ...
  template<typename CLS> static void __init__(object &self, Args... args)
  {
    TfErrorMark m;
...
template<typename R, typename... Args> struct NewCtor<R(Args...)> : CtorBase<R(Args...)> {
  ...
  template<class CLS> static bp::object __new__(object &cls, Args... args)
  {
    typedef typename CLS::metadata::held_type HeldType;
    TfErrorMark m;
    R r((Base::_func(args...)));
...
struct InitCtorWithVarArgs<R(Args...)> :
  ...
  static_assert((Arity::value >= 2) &&
                    (std::is_same_v<const bp::tuple &,
                                    typename Base::Traits::template NthArg<(Arity::value - 2)>>) &&
                    (std::is_same_v<const bp::dict &,
                                 ...
                                 std::index_sequence<I...>)
  {
    ...
    if (positionalArgs < expectedNamedArgs) {
      std::array<std::string, Arity::value - 2> positionalArgTypes = {
          {(bp::type_id<typename Base::Traits::template NthArg<I>>().name())...}};
      std::string joinedTypes = TfStringJoin(
          std::begin(positionalArgTypes), std::end(positionalArgTypes), ", ");
    ...
    Install<CLS>(
        ...
        args[0],
        Base::_func(
            bp::extract<std::remove_reference_t<typename Base::Traits::template NthArg<I>>>(
                args[I + 1])...,
            bp::tuple(args.slice(expectedNamedArgs + 1, bp::len(args))),
            kwargs),
        m);
  ...
  {
    return __init__impl<CLS>(args, kwargs, std::make_index_sequence<Arity::value - 2>());
  }
};
...
struct InitCtorWithBackReference<R(SelfRef, Args...)> : CtorBase<R(SelfRef, Args...)> {
  ...
  template<typename CLS> static void __init__(SelfRef self, Args... args)
  {
    TfErrorMark m;
...
struct NewCtorWithClassReference<R(ClsRef, Args...)> : CtorBase<R(ClsRef, Args...)> {
  ...
  template<class CLS> static bp::object __new__(ClsRef cls, Args... args)
  {
    typedef typename CLS::metadata::held_type HeldType;
    TfErrorMark m;
