...
template<typename TypeVector> struct Tf_BaseTypeInfos;

template<> struct Tf_BaseTypeInfos<TfType::Bases<>> {
  static const size_t NumBases = 0;
  std::type_info const **baseTypeInfos = nullptr;
};

template<typename... Bases> struct Tf_BaseTypeInfos<TfType::Bases<Bases...>> {
  static const size_t NumBases = sizeof...(Bases);
  std::type_info const *baseTypeInfos[NumBases] = {&typeid(Bases)...};
};

template<class Derived, typename TypeVector> struct Tf_TypeCastFunctions;

template<class Derived> struct Tf_TypeCastFunctions<Derived, TfType::Bases<>> {
...
template<typename T, typename BaseTypes> TfType const &TfType::Define()
{
  ...
  return _DefineImpl(typeid(T),
                     btis.baseTypeInfos,
                     tcfs.castFunctions,
                     btis.NumBases,
                     TfSizeofType<T>::value,
                     std::is_pod_v<T>,
                     std::is_enum_v<T>);
}
