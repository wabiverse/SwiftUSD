...
template<class Ret, class ArgTypeList> struct Tf_FuncSig {
  ...
  template<size_t N> using NthArg = std::tuple_element_t<N, ArgsTuple>;
};

// Produce a new Tf_FuncSig from FuncSig by removing the initial "this"
// argument.
template<class FuncSig>
using Tf_RemoveThisArg =
    Tf_FuncSig<typename FuncSig::ReturnType, TfMetaApply<TfMetaTail, typename FuncSig::ArgTypes>>;

// For callable function objects, get signature from operator(), but remove the
// 'this' arg from that.
template<class Fn> struct Tf_GetFuncSig {
  using Type = Tf_RemoveThisArg<
      typename Tf_GetFuncSig<decltype(&std::remove_reference<Fn>::type::operator())>::Type>;
};

// Member function pointers, with optional 'const' and ref-qualifiers.
template<class Ret, class Cls, class... Args> struct Tf_GetFuncSig<Ret (Cls::*)(Args...)> {
...
template<class Ret, class Cls, class... Args> struct Tf_GetFuncSig<Ret (Cls::*)(Args...) const> {
...
template<class Ret, class... Args> struct Tf_GetFuncSig<Ret (*)(Args...)> {
  using Type = Tf_FuncSig<Ret, TfMetaList<Args...>>;
};

// Function traits.
template<class Fn> using TfFunctionTraits = typename Tf_GetFuncSig<Fn>::Type;
