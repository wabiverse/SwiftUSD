...
template<typename ValueType> class TfDelegatedCountPtr {
 public:
  ...
  static_assert(
      std::is_same_v<void, decltype(TfDelegatedCountIncrement(std::declval<RawPtrType>()))>);
  static_assert(
      std::is_same_v<void, decltype(TfDelegatedCountDecrement(std::declval<RawPtrType>()))>);
 ...
 public:
  ...
  TfDelegatedCountPtr(TfDelegatedCountIncrementTagType,
                      RawPtrType rawPointer) noexcept(IncrementIsNoExcept())
      : _pointer{rawPointer}
  {
    _IncrementIfValid();
  }
  ...
  explicit TfDelegatedCountPtr(const TfDelegatedCountPtr<OtherType> &ptr,
                               std::enable_if_t<_IsPtrConvertible<OtherType>::value, int> =
                                   0) noexcept(IncrementIsNoExcept())
      : _pointer(ptr.get())
  {
    _IncrementIfValid();
  }
  ...
  TfDelegatedCountPtr(TfDelegatedCountPtr &&ptr) noexcept : _pointer(ptr.get())
  {
  ...
  /// `nullptr`.
  TfDelegatedCountPtr &operator=(const TfDelegatedCountPtr &ptr) noexcept(
...
{
  return TfDelegatedCountPtr<ValueType>(TfDelegatedCountIncrementTag,
