...
public:
  ...
  TfHashMap() : _Base() {}
  explicit TfHashMap(size_type n, const hasher &hf = hasher(),
  ...
  TfHashMap(InputIterator first, InputIterator last, size_type n = 0,
  ...
  template <class Key2, class Mapped2, class HashFn2, class EqualKey2,
            class Alloc2>
  friend bool
...
public:
  ...
  explicit TfHashMultiMap(size_type n, const hasher &hf = hasher(),
                          const key_equal &eql = key_equal(),
                          const allocator_type &alloc = allocator_type())
      : _Base(n, hf, eql, alloc) {}
  explicit TfHashMultiMap(const allocator_type &alloc)
      : _Base(0, hasher(), key_equal(), alloc) {}
  template <class InputIterator>
  TfHashMultiMap(InputIterator first, InputIterator last, size_type n = 0,
...
operator==(const TfHashMultiMap<Key, Mapped, HashFn, EqualKey, Alloc> &lhs,
  ...
  typedef typename TfHashMultiMap<Key, Mapped, HashFn, EqualKey, Alloc>::_Base
      _Base;
  return static_cast<const _Base &>(lhs) == static_cast<const _Base &>(rhs);
}
