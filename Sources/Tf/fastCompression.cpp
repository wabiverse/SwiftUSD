...
size_t TfFastCompression::CompressToBuffer(char const *input, char *compressed, size_t inputSize)
{
  ...
  if (inputSize <= LZ4_MAX_INPUT_SIZE) {
    ...
    compressed += static_cast<size_t>(
        1 + LZ4_compress_default(input,
                                 compressed + 1,
    ...
    *compressed++ = nWholeChunks + (partChunkSz ? 1 : 0);
    auto writeChunk = [](char const *&input, char *&output, size_t size) {
      ...
      int32_t n = LZ4_compress_default(
          input, output, static_cast<int>(size), LZ4_compressBound(static_cast<int>(size)));
      memcpy(o, &n, sizeof(n));
      output += n;
      input += size;
    };
                                               ...
                                               size_t maxOutputSize)
{
  ...
  if (nChunks == 0) {
    ...
    int nDecompressed = LZ4_decompress_safe(
        compressed, output, static_cast<int>(compressedSize - 1), static_cast<int>(maxOutputSize));
    if (nDecompressed < 0) {
      TF_RUNTIME_ERROR(
