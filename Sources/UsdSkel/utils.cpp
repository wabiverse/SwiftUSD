                               ...
                               bool *hasJointScale = nullptr)
{
  ...
  for (size_t ji = 0; ji < jointXforms.size(); ++ji) {
    ...
    jointDualQuats[ji] = GfDualQuatd(GfQuatd(rotationQ.GetReal(), rotationQ.GetImaginary()),
                                     translation);
                    ...
                    const bool inSerial)
{
  ...
  _ParallelForN(points.size(), /* inSerial = */ inSerial, [&](size_t start, size_t end) {
    for (size_t pi = start; pi < end; ++pi) {
      ...
      const int pivotIdx = _GetPivotJointIndex(
          pi, jointDualQuats.size(), influenceFn, numInfluencesPerPoint);
      if (pivotIdx >= 0)
...
bool UsdSkelSkinPoints(const TfToken &skinningMethod,
                       ...
                       const bool inSerial)
{
  return _NonInterleavedSkinPoints(skinningMethod,
                                   geomBindTransform,
                                   jointXforms,
                                   jointIndices,
                                   jointWeights,
...
bool UsdSkelSkinPoints(const TfToken &skinningMethod,
                       ...
                       const bool inSerial)
{
  return _InterleavedSkinPoints(skinningMethod,
                                geomBindTransform,
                                jointXforms,
                                influences,
                                numInfluencesPerPoint,
                     ...
                     const bool inSerial)
{
  ...
  bool hasJointScale(false);
  _ConvertToQuaternions(
      jointXforms, TfSpan<GfQuatd>(jointQuats), TfSpan<Matrix3>(jointScales), &hasJointScale);
  ...
  _ParallelForN(normals.size(), inSerial, [&](size_t start, size_t end) {
    ...
    for (size_t ni = start; ni < end; ++ni) {
      ...
      GfQuatd pivotQuat(0.0);
      const int pivotIdx = _GetPivotJointIndex(
          pi, jointQuats.size(), influenceFn, numInfluencesPerPoint);
      if (pivotIdx >= 0)
                             ...
                             const bool inSerial)
{
  ...
  if (skinningMethod == UsdSkelTokens->classicLinear) {
    ...
    return _SkinNormalsDQS(geomBindTransform,
                           jointXforms,
                           influenceFn,
                           _IdentityPointIndexFn(),
                           numInfluencesPerPoint,
...
bool UsdSkelSkinNormals(const TfToken &skinningMethod,
                        ...
                        const bool inSerial)
{
  return _NonInterleavedSkinNormals(skinningMethod,
                                    geomBindTransform,
                                    jointXforms,
                                    jointIndices,
                                    jointWeights,
...
bool UsdSkelSkinNormals(const TfToken &skinningMethod,
                        ...
                        const bool inSerial)
{
  return _InterleavedSkinNormals(skinningMethod,
                                 geomBindTransform,
                                 jointXforms,
                                 influences,
                                 numInfluencesPerPoint,
...
bool UsdSkelSkinFaceVaryingNormals(const TfToken &skinningMethod,
                                   ...
                                   const bool inSerial)
{
  return _SkinFaceVaryingNormals(skinningMethod,
                                 geomBindTransform,
                                 jointXforms,
                                 jointIndices,
                                 jointWeights,
...
bool UsdSkelSkinFaceVaryingNormalsLBS(const GfMatrix3d &geomBindTransform,
                                      ...
                                      const bool inSerial)
{
  return _SkinFaceVaryingNormals(UsdSkelTokens->classicLinear,
                                 geomBindTransform,
                                 jointXforms,
                                 jointIndices,
                                 jointWeights,
                              ...
                              Matrix4 *xform)
{
  ...
  GfQuatd pivotQuat(0.0);
  const int pivotIdx = _GetPivotJointIndex(
      0, jointDualQuats.size(), influencesFn, influencesFn.size());
  if (pivotIdx >= 0)
    pivotQuat = jointDualQuats[pivotIdx].GetReal();
...
bool UsdSkelSkinTransform(const TfToken &skinningMethod,
                          ...
                          GfMatrix4d *xform)
{
  return UsdSkel_NonInterleavedSkinTransform(skinningMethod,
                                             geomBindTransform,
                                             TfSpan<const GfMatrix4d>(jointXforms, numJoints),
                                             TfSpan<const int>(jointIndices, numInfluences),
                                             TfSpan<const float>(jointWeights, numInfluences),
                             ...
                             GfMatrix4d *xform)
{
  return UsdSkelSkinTransform(
      UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, influences, xform);
}
