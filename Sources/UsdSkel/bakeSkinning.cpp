...
std::string _DeformationFlagsToString(int flags, const char *indent)
{
  return TfStringPrintf(
      ...
      indent,
      bool(flags & UsdSkelBakeSkinningParms::DeformPointsWithSkinning),
      indent,
      bool(flags & UsdSkelBakeSkinningParms::DeformNormalsWithSkinning),
      indent,
      bool(flags & UsdSkelBakeSkinningParms::DeformXformWithSkinning),
      indent,
...
bool _SkinningAdapter::_ComputeFaceVertexIndices(const UsdTimeCode time)
{
  return _faceVertexIndicesTask.Run(
      time, GetPrim(), "compute face vertex indices", [&](UsdTimeCode time) {
...
{
  if (!_ComputeSkinningMethod(time) || !_ComputeGeomBindXform(time) ||
      !_ComputeJointInfluences(time))
  {
    return;
  }
...
void _SkinningAdapter::_DeformPointsWithSkinning(const GfMatrix4d &skelToGprimXf)
{
  ...
  _points.hasSampleAtCurrentTime = UsdSkelSkinPoints(_skinningMethod,
                                                     _geomBindXform,
                                                     xformsForPrim,
                                                     _jointIndices,
                                                     _jointWeights,
...
void _SkinningAdapter::_DeformNormalsWithSkinning(const GfMatrix4d &skelToGprimXf)
{
  ...
  if (_faceVertexIndicesTask) {
    ...
    _normals.hasSampleAtCurrentTime = UsdSkelSkinNormals(
        _skinningMethod,
        _geomBindInvTransposeXform,
        xformsForPrim,
        _jointIndices,
...
void _SkinningAdapter::_DeformXformWithSkinning(const GfMatrix4d &skelLocalToWorldXform)
{
  ...
  _xform.hasSampleAtCurrentTime = UsdSkelSkinTransform(
      _skinningMethod, _geomBindXform, xformsForPrim, _jointIndices, _jointWeights, &_xform.value);
