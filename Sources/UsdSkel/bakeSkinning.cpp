...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    ((xformOpTransform, "xformOp:transform"))(Xform));
...
namespace
{
  ...
  _DeformationFlagsToString(int flags, const char *indent)
  {
    return TfStringPrintf(
        ...
        indent, bool(flags & UsdSkelBakeSkinningParms::DeformPointsWithBlendShapes),
        indent, bool(flags & UsdSkelBakeSkinningParms::DeformNormalsWithBlendShapes));
  }
  ...
  struct _Task
  {
             ...
             const Fn &fn)
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]     Try to run task '%s' for <%s>.\n", name, prim.GetPath().GetText());
      ...
      if (_mightBeTimeVarying || _isFirstSample || time.IsDefault())
      {
        ...
        TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]       Ran task '%s' for <%s>. "
                                           "Result: %d\n",
      ...
      {
        TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]       Skipping task '%s' "
      ...
      : _skelQuery(skelQuery)
  {
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Creating adapter for skel at <%s>\n", skelQuery.GetPrim().GetPath().GetText());
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]\n  Initial state for skel <%s>:\n"
                                       "    _skinningXformsTask: %s\n"
                                       "    _skinningInvTransposeXformsTask: %s\n"
                                       "    _blendShapeWeightsTask: %s\n"
                                       "    _skelLocalToWorldXformTask: %s\n",
                                       GetPrim().GetPath().GetText(), _skinningXformsTask.GetDescription().c_str(), _skinningInvTransposeXformsTask.GetDescription().c_str(), _blendShapeWeightsTask.GetDescription().c_str(), _skelLocalToWorldXformTask.GetDescription().c_str());
  }
                                ...
                                UsdGeomXformCache *xfCache)
  {
    ...
    if (ShouldProcessAtTime(timeIndex))
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Updating transform for "
                                         "<%s> @ time %s\n",
  ...
  _SkelAdapter::UpdateAnimation(const UsdTimeCode time, const size_t timeIndex)
  {
    ...
    if (ShouldProcessAtTime(timeIndex))
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Updating animation for "
                                         "<%s> @ time %s\n",
        ...
        _skelAdapter(skelAdapter), _layerIndex(layerIndex)
  {
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Creating adapter for skinning prim at <%s>\n", skinningQuery.GetPrim().GetPath().GetText());
    ...
    if (!_flags)
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   No computations active "
                                         "for <%s>: skipping\n",
                                         skinningQuery.GetPrim().GetPath().GetText());
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]\n  Computation state for skinnable prim <%s>:\n"
                                       ...
                                       "  Deformation flags:\n%s",
                                       _skinningQuery.GetPrim().GetPath().GetText(), _restPointsTask.GetDescription().c_str(), _restNormalsTask.GetDescription().c_str(), _faceVertexIndicesTask.GetDescription().c_str(), _skinningMethodTask.GetDescription().c_str(), _geomBindXformTask.GetDescription().c_str(), _geomBindInvTransposeXformTask.GetDescription().c_str(), _jointInfluencesTask.GetDescription().c_str(), _localToWorldXformTask.GetDescription().c_str(), _parentToWorldXformTask.GetDescription().c_str(), _DeformationFlagsToString(_flags, "    ").c_str());
  }
  ...
  _SkinningAdapter::Write(const UsdTimeCode time, const size_t timeIndex)
  {
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Write data for <%s> @ time %s\n", GetPrim().GetPath().GetText(), TfStringify(time).c_str());
                                    ...
                                    UsdGeomXformCache *xfCache)
  {
    ...
    if (ShouldProcessAtTime(timeIndex))
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Updating transforms for "
                                         "<%s> @ time %s\n",
                                         GetPrim().GetPath().GetText(), TfStringify(xfCache->GetTime()).c_str());
  ...
  _SkinningAdapter::Update(const UsdTimeCode time, const size_t timeIndex)
  {
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Updating skinning for "
                                       "<%s> @ time %s\n",
                                       GetPrim().GetPath().GetText(), TfStringify(time).c_str());
      ...
      const std::vector<UsdTimeCode> &times)
  {
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Updating extents for %zu prims\n", adaptersToUpdate.size());
      ...
      const std::vector<UsdTimeCode> &times)
  {
    ...
    TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Updating extent hints for %zu prims\n", modelToAdaptersMap.size());
                         ...
                         const GfInterval &interval)
{
  ...
  TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Baking skinning over range %s "
                                     "for %zu bindings in %zu layers.\n"
                                     "Deformation flags:\n%s",
                                     TfStringify(interval).c_str(), parms.bindings.size(), parms.layers.size(), _DeformationFlagsToString(parms.deformationFlags, "    ").c_str());
  ...
  TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Baking skinning for %zu prims, "
  ...
  std::unique_ptr<SdfChangeBlock> changeBlock(new SdfChangeBlock);
  {
    ...
    for (size_t ti = 0; ti < times.size(); ++ti)
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning]   Baking skinning at sample "
                                         "%zu (time=%s)\n",
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Reading skel transforms at time %s\n", TfStringify(time).c_str());
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Reading skinned prim transforms "
                                         "at time %s\n",
                                         TfStringify(time).c_str());
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Reading skel animation at time %s\n", TfStringify(time).c_str());
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Updating skinning adapters at time %s\n", TfStringify(time).c_str());
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Writing data to layers at time %s\n", TfStringify(time).c_str());
      ...
      {
        TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Memory limit exceeded "
...
bool UsdSkelBakeSkinning(const UsdPrimRange &range, const GfInterval &interval)
{
  ...
  for (auto it = range.begin(); it != range.end(); ++it)
  {
    if (it->IsA<UsdSkelRoot>())
    {
      ...
      TF_DEBUG(USDSKEL_BAKESKINNING).Msg("[UsdSkelBakeSkinning] Populating cache for <%s>\n", it->GetPath().GetText());
