...
//
#ifndef USDVOL_GENERATED_VOLUMEFIELDASSET_H
#define USDVOL_GENERATED_VOLUMEFIELDASSET_H

/// \file usdVol/volumeFieldAsset.h
...
/// as the value.
///
class UsdVolVolumeFieldAsset : public UsdVolFieldBase
{
public:
    ...
    /// Construct a UsdVolVolumeFieldAsset on UsdPrim \p prim .
    /// Equivalent to UsdVolVolumeFieldAsset::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdVolVolumeFieldAsset(const UsdPrim& prim=UsdPrim())
        : UsdVolFieldBase(prim)
    {
    }

    /// Construct a UsdVolVolumeFieldAsset on the prim held by \p schemaObj .
    /// Should be preferred over UsdVolVolumeFieldAsset(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdVolVolumeFieldAsset(const UsdSchemaBase& schemaObj)
    ...
    /// Destructor.
    USDVOL_API
    virtual ~UsdVolVolumeFieldAsset();
    ...
    /// Return a UsdVolVolumeFieldAsset holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdVolVolumeFieldAsset(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDVOL_API
    static UsdVolVolumeFieldAsset
