...
class Hdui_ValueItemModel : public QAbstractItemModel {
 public:
  Hdui_ValueItemModel(VtValue value, QObject *parent = nullptr)
      : QAbstractItemModel(parent), _value(value)
  {
  }
...
template<typename T> class Hdui_TypedArrayValueItemModel : public Hdui_ValueItemModel {
 public:
  ...
  QVariant data(const QModelIndex &index, int role = Qt::DisplayRole) const override
  {
    ...
    if (role == Qt::ForegroundRole && index.column() == 1) {
      return QVariant(QPalette().brush(QPalette::Disabled, QPalette::WindowText));
    }
...
Hdui_ValueItemModel *Hdui_GetModelFromValue(VtValue value, QObject *parent = nullptr)
{
...
HduiDataSourceValueTreeView::HduiDataSourceValueTreeView(QWidget *parent) : QTreeView(parent)
{
  setUniformRowHeights(true);
  setItemsExpandable(false);
}

void HduiDataSourceValueTreeView::SetDataSource(const HdSampledDataSourceHandle &dataSource)
{
...
void HduiDataSourceValueTreeView::Refresh()
{
