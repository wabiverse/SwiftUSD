...
class Hdui_SceneIndexPrimTreeWidgetItem : public QTreeWidgetItem {
 public:
  Hdui_SceneIndexPrimTreeWidgetItem(QTreeWidgetItem *parentItem,
                                    const SdfPath &primPath,
                                    bool queryOnExpansion = false)
      : QTreeWidgetItem(parentItem), _primPath(primPath), _queryOnExpansion(queryOnExpansion)
  {
    if (queryOnExpansion) {
  ...
  void WasExpanded(HduiSceneIndexTreeWidget *treeWidget)
  {
    ...
    for (const SdfPath &childPath : treeWidget->_inputSceneIndex->GetChildPrimPaths(_primPath)) {
      ...
      Hdui_SceneIndexPrimTreeWidgetItem *childItem = new Hdui_SceneIndexPrimTreeWidgetItem(
          this, childPath, true);
...
HduiSceneIndexTreeWidget::HduiSceneIndexTreeWidget(QWidget *parent) : QTreeWidget(parent)
{
  ...
  connect(this, &QTreeWidget::itemSelectionChanged, [this]() {
            ...
            dynamic_cast<Hdui_SceneIndexPrimTreeWidgetItem *>(items[0]))
    {
      ...
      Q_EMIT PrimSelected(primItem->GetPrimPath(),
                          this->_inputSceneIndex->GetPrim(primItem->GetPrimPath()).dataSource);
    }
  });
...
void HduiSceneIndexTreeWidget::PrimsAdded(const HdSceneIndexBase &sender,
                                          const AddedPrimEntries &entries)
{
  for (const AddedPrimEntry &entry : entries) {
    if (Hdui_SceneIndexPrimTreeWidgetItem *item = _GetPrimItem(entry.primPath)) {
...
void HduiSceneIndexTreeWidget::PrimsRemoved(const HdSceneIndexBase &sender,
                                            const RemovedPrimEntries &entries)
{
  ...
  for (const RemovedPrimEntry &entry : entries) {
    if (Hdui_SceneIndexPrimTreeWidgetItem *item = _GetPrimItem(entry.primPath, false)) {
...
void HduiSceneIndexTreeWidget::PrimsDirtied(const HdSceneIndexBase &sender,
                                            const DirtiedPrimEntries &entries)
{
  ...
  if (Hdui_SceneIndexPrimTreeWidgetItem *item = dynamic_cast<Hdui_SceneIndexPrimTreeWidgetItem *>(
          items[0]))
  {
...
void HduiSceneIndexTreeWidget::PrimsRenamed(const HdSceneIndexBase &sender,
                                            const RenamedPrimEntries &entries)
{
  ConvertPrimsRenamedToRemovedAndAdded(sender, entries, this);
}

void HduiSceneIndexTreeWidget::SetSceneIndex(HdSceneIndexBaseRefPtr inputSceneIndex)
{
  if (_inputSceneIndex) {
...
void HduiSceneIndexTreeWidget::Requery(bool lazy)
{
  //_primItems.clear();
  // clear();
...
Hdui_SceneIndexPrimTreeWidgetItem *HduiSceneIndexTreeWidget::_GetPrimItem(const SdfPath &primPath,
                                                                          bool createIfNecessary)
{
  ...
  Hdui_SceneIndexPrimTreeWidgetItem *item = new Hdui_SceneIndexPrimTreeWidgetItem(parentItem,
                                                                                  primPath);
...
void HduiSceneIndexTreeWidget::_RemoveSubtree(const SdfPath &primPath)
{
...
void HduiSceneIndexTreeWidget::_AddPrimItem(const SdfPath &primPath,
                                            Hdui_SceneIndexPrimTreeWidgetItem *item)
{
  _primItems[primPath] = item;
}

void HduiSceneIndexTreeWidget::contextMenuEvent(QContextMenuEvent *event)
{
