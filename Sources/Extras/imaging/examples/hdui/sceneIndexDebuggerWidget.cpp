...
HduiSceneIndexDebuggerWidget::HduiSceneIndexDebuggerWidget(QWidget *parent) : QWidget(parent)
{
  ...
  QObject::connect(_siTreeWidget,
                   &HduiSceneIndexTreeWidget::PrimSelected,
                   [this](const SdfPath &primPath, HdContainerDataSourceHandle dataSource) {
  ...
  QObject::connect(_dsTreeWidget,
                   ...
                   [this](HdDataSourceBaseHandle dataSource) {
                     this->_valueTreeView->SetDataSource(HdSampledDataSource::Cast(dataSource));
                   });

  QObject::connect(_siTreeWidget,
                   &HduiSceneIndexTreeWidget::PrimDirtied,
                   [this](const SdfPath &primPath, const HdDataSourceLocatorSet &locators) {
  ...
  QObject::connect(_siChooser,
                   ...
                   [this](const std::string &name, HdSceneIndexBaseRefPtr sceneIndex) {
                     this->SetSceneIndex(name, sceneIndex, true);
                   });

  QObject::connect(_goToInputButtonMenu,
                   &QMenu::aboutToShow,
                   this,
                   &HduiSceneIndexDebuggerWidget::_FillGoToInputMenu);

  QObject::connect(loggerButton, &QPushButton::clicked, [this]() {
    HduiSceneIndexObserverLoggingWidget *loggingWidget = new HduiSceneIndexObserverLoggingWidget();

    loggingWidget->SetLabel(_nameLabel->text().toStdString());
    loggingWidget->show();
    if (this->_currentSceneIndex) {
      loggingWidget->GetTreeView()->SetSceneIndex(this->_currentSceneIndex);
    }
  });
}
...
class _InputSelectionItem : public QTreeWidgetItem {
 public:
  _InputSelectionItem(QTreeWidgetItem *parent) : QTreeWidgetItem(parent) {}
...
void HduiSceneIndexDebuggerWidget::_FillGoToInputMenu()
{
  ...
  QObject::connect(menuTreeWidget,
  ...
  QObject::connect(
      ...
      [this, menu, menuTreeWidget](QTreeWidgetItem *item, int column) {
        if (_InputSelectionItem *selectionItem = dynamic_cast<_InputSelectionItem *>(item)) {
  ...
  _AddSceneIndexToTreeMenu(menuTreeWidget->invisibleRootItem(), _currentSceneIndex, false);
...
void HduiSceneIndexDebuggerWidget::_AddSceneIndexToTreeMenu(QTreeWidgetItem *parentItem,
                                                            HdSceneIndexBaseRefPtr sceneIndex,
                                                            bool includeSelf)
{
  if (!sceneIndex) {
