...
HduiRegisteredSceneIndexChooser::HduiRegisteredSceneIndexChooser(QWidget *parent)
    : QPushButton("Choose Scene Index", parent), _menu(new QMenu)
{
  setMenu(_menu);
  ...
  QObject::connect(_menu, &QMenu::aboutToShow, [this]() {
    this->_menu->clear();
    bool noneFound = true;
    for (const std::string &name : HdSceneIndexNameRegistry::GetInstance().GetRegisteredNames()) {
  ...
  QObject::connect(_menu, &QMenu::triggered, [this](QAction *action) {
    ...
    if (HdSceneIndexBaseRefPtr sceneIndex =
            HdSceneIndexNameRegistry::GetInstance().GetNamedSceneIndex(name))
    {
