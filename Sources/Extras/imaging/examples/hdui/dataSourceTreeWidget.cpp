...
class Hdui_DataSourceTreeWidgetItem : public QTreeWidgetItem {
 public:
  Hdui_DataSourceTreeWidgetItem(const HdDataSourceLocator &locator,
        ...
        _childrenBuilt(false)
  {
    ...
    if (HdContainerDataSource::Cast(dataSource) || HdVectorDataSource::Cast(dataSource)) {
  ...
  {
    if (_childrenBuilt) {
      if (HdContainerDataSourceHandle containerDataSource = HdContainerDataSource::Cast(
              dataSource))
      {
        ...
        for (int i = 0, e = childCount(); i < e; ++i) {
          if (Hdui_DataSourceTreeWidgetItem *childItem =
                  dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(child(i)))
          {
            const TfToken childName = childItem->GetLocator().GetLastElement();

            HdDataSourceBaseHandle childDs = containerDataSource->Get(childName);
        ...
        // add any new items
        for (const TfToken &childName : containerDataSource->GetNames()) {
          if (usedNames.find(childName) == usedNames.end()) {

            if (HdDataSourceBaseHandle childDs = containerDataSource->Get(childName)) {
              new Hdui_DataSourceTreeWidgetItem(_locator.Append(childName), this, childDs);
            }
          }
        }
      ...
      else if (HdVectorDataSourceHandle vectorDataSource = HdVectorDataSource::Cast(dataSource)) {

        HdVectorDataSourceHandle existingVectorDataSource = HdVectorDataSource::Cast(dataSource);
        ...
        if (!existingVectorDataSource ||
            childCount() != static_cast<int>(vectorDataSource->GetNumElements()))
        {
          _dataSource = dataSource;
          _RebuildChildren();
          return;
        ...
        for (size_t i = 0, e = vectorDataSource->GetNumElements(); i != e; ++i) {
          if (Hdui_DataSourceTreeWidgetItem *childItem =
                  dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(child(i)))
          {
 ...
 private:
  ...
  {
    _childrenBuilt = true;
    if (HdContainerDataSourceHandle container = HdContainerDataSource::Cast(_dataSource)) {
      TfDenseHashSet<TfToken, TfHash> usedNames;
      ...
      for (const TfToken &childName : container->GetNames()) {
        ...
        usedNames.insert(childName);
        if (HdDataSourceBaseHandle childDataSource = container->Get(childName)) {
          new Hdui_DataSourceTreeWidgetItem(_locator.Append(childName), this, childDataSource);
        }
      }
    }
    else if (HdVectorDataSourceHandle vectorDs = HdVectorDataSource::Cast(_dataSource)) {
      char buffer[16];
      for (size_t i = 0, e = vectorDs->GetNumElements(); i != e; ++i) {
        ...
        new Hdui_DataSourceTreeWidgetItem(
            _locator.Append(TfToken(buffer)), this, vectorDs->GetElement(i));
      }
    }
  }
};
...
HduiDataSourceTreeWidget::HduiDataSourceTreeWidget(QWidget *parent) : QTreeWidget(parent)
{
  setHeaderLabels({"Name"});
  setAllColumnsShowFocus(true);

  connect(this, &QTreeWidget::itemExpanded, [](QTreeWidgetItem *item) {
    if (Hdui_DataSourceTreeWidgetItem *dsItem = dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(
            item))
    {
      dsItem->WasExpanded();
    }
...
void HduiDataSourceTreeWidget::SetPrimDataSource(const SdfPath &primPath,
                                                 HdContainerDataSourceHandle const &dataSource)
{
  clear();
  if (dataSource) {
    Hdui_DataSourceTreeWidgetItem *item = new Hdui_DataSourceTreeWidgetItem(
        HdDataSourceLocator(), invisibleRootItem(), dataSource);

    item->setText(0, primPath.GetName().c_str());
  }
}

void HduiDataSourceTreeWidget::PrimDirtied(const SdfPath &primPath,
            ...
            item))
    {
      ...
      if (!loc.IsEmpty()) {
        if (locators.Contains(loc)) {
          ...
          dsItem->SetDirty(HdContainerDataSource::Get(primDataSource, loc));
        }
...
static void _DumpDataSource(HduiDataSourceTreeWidget *treeWidget, std::ostream &out)
{
  for (int i = 0; i < treeWidget->topLevelItemCount(); ++i) {
    if (auto item = dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(treeWidget->topLevelItem(i))) {
      HdDebugPrintDataSource(out, item->GetDataSource());
    }
  }
}

void HduiDataSourceTreeWidget::contextMenuEvent(QContextMenuEvent *event)
{
  const bool enable = topLevelItemCount() > 0;
  QMenu menu;
  QAction *dumpToStdout = menu.addAction("Dump to stdout",
                                         [this]() { _DumpDataSource(this, std::cout); });
