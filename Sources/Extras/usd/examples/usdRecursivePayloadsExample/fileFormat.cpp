...
TF_DEFINE_PUBLIC_TOKENS(UsdRecursivePayloadsExampleFileFormatTokens,
                        USD_RECURSIVE_PAYLOADS_EXAMPLE_FILE_FORMAT_TOKENS);

TF_DEFINE_PRIVATE_TOKENS(_propertyNameTokens,
                         ((xformOpOrder, "xformOpOrder"))((xformOpTranslate,
                                                           "xformOp:translate")));

TF_DEFINE_PRIVATE_TOKENS(_primNameTokens, ((Root, "Root"))((Geom, "geom"))((Xform, "Xform")));
...
struct _Params {
  ...
  template<typename ValueExtractor> void ExtractValues(const ValueExtractor &extractor)
  {
    ...
    if (depth > 1) {
      extractor.Extract(UsdRecursivePayloadsExampleFileFormatTokens->Radius, &radius);
      extractor.Extract(UsdRecursivePayloadsExampleFileFormatTokens->Height, &height);
    }
  }
  ...
  SdfFileFormat::FileFormatArguments ToArgs() const
  {
    ...
    args[UsdRecursivePayloadsExampleFileFormatTokens->Depth] = TfStringify(depth);
    args[UsdRecursivePayloadsExampleFileFormatTokens->Num] = TfStringify(num);
    ...
    if (depth > 1) {
      args[UsdRecursivePayloadsExampleFileFormatTokens->Radius] = TfStringify(radius);
      args[UsdRecursivePayloadsExampleFileFormatTokens->Height] = TfStringify(height);
    }
    return args;
  }
};
...
class FromArgsValueExtractor {
 public:
  FromArgsValueExtractor(const SdfFileFormat::FileFormatArguments &args) : _args(args) {}

  template<class T> bool Extract(const TfToken &argName, T *outValue) const
  {
    ...
    if (!success) {
      TF_CODING_ERROR("Could not convert arg string '%s' to value of type %s",
                      argName.GetText(),
...
UsdRecursivePayloadsExampleFileFormat::UsdRecursivePayloadsExampleFileFormat()
    : SdfFileFormat(UsdRecursivePayloadsExampleFileFormatTokens->Id,
...
bool UsdRecursivePayloadsExampleFileFormat::CanRead(const string &filePath) const
{
  return true;
}
...
static SdfPrimSpecHandle _CreateRecursiveChildSpec(const std::string &payloadId,
                                                   ...
                                                   const SdfPrimSpecHandle &rootSpec)
{
  ...
  std::string specName = TfStringPrintf("%s_%s_%d_%d",
                                        _primNameTokens->Xform.GetText(),
                                        payloadId.c_str(),
                                        childParams.depth,
                                        childIndex);
  ...
  xformSpec->SetField(UsdRecursivePayloadsExampleFileFormatTokens->Num, childParams.num);
  xformSpec->SetField(UsdRecursivePayloadsExampleFileFormatTokens->Height, childParams.height);
  xformSpec->SetField(UsdRecursivePayloadsExampleFileFormatTokens->Depth, childParams.depth);
  xformSpec->SetField(UsdRecursivePayloadsExampleFileFormatTokens->Radius, childParams.radius);
  ...
  if (payloadId.empty()) {
    ...
    xformSpec->GetPayloadList().Add(SdfPayload(SdfLayer::CreateIdentifier(layerPath, newArgs)));
  }
...
// for a dynamic layer.
static SdfLayerRefPtr _GenerateDynamicLayer(const SdfFileFormat::FileFormatArguments &args,
                                            const string &layerPath)
{
  ...
  TfMapLookup(args, UsdRecursivePayloadsExampleFileFormatTokens->PayloadId, &payloadId);
  ...
  SdfPrimSpecHandle rootSpec = SdfPrimSpec::New(
      SdfLayerHandle(genLayer), _primNameTokens->Root, SdfSpecifierDef, _primNameTokens->Xform);
  ...
  SdfPrimSpecHandle geomSpec = SdfPrimSpec::New(rootSpec, _primNameTokens->Geom, SdfSpecifierDef);
  ...
  if (params.depth > 1) {
    ...
    for (int i = 0; i < params.num; ++i) {
      ...
      SdfAttributeSpecHandle attrSpec = SdfAttributeSpec::New(
          xformSpec, _propertyNameTokens->xformOpTranslate, SdfGetValueTypeNameForValue(posVal));
      attrSpec->SetDefaultValue(posVal);

      // The order attribute spec is necessary and is always the same
      // for all these prims.
      static const VtValue orderVal(VtTokenArray({_propertyNameTokens->xformOpTranslate}));
      SdfAttributeSpecHandle orderAttrSpec = SdfAttributeSpec::New(
          xformSpec, _propertyNameTokens->xformOpOrder, SdfGetValueTypeNameForValue(orderVal));
...
bool UsdRecursivePayloadsExampleFileFormat::Read(SdfLayer *layer,
...
bool UsdRecursivePayloadsExampleFileFormat::WriteToString(const SdfLayer &layer,
                                                          std::string *str,
...
static bool _ExtractPayloadId(const std::string &assetPath, std::string *payloadId)
{
  ...
  const auto &it = args.find(UsdRecursivePayloadsExampleFileFormatTokens->PayloadId.GetString());
  if (it != args.end()) {
    *payloadId = it->second;
    return true;
  }
                            ...
                            VtDictionary *dict)
{
  ...
  VtValue value;
  if (!context.ComposeValue(UsdRecursivePayloadsExampleFileFormatTokens->ArgDict, &value)) {
    return;
  }
...
void UsdRecursivePayloadsExampleFileFormat::ComposeFieldsForFileFormatArguments(
    ...
    VtValue *dependencyContextData) const
{
  ...
  if (_ExtractPayloadId(assetPath, &payloadId)) {
    ...
    customDependencyData[UsdRecursivePayloadsExampleFileFormatTokens->PayloadId] = payloadId;
  }
...
bool UsdRecursivePayloadsExampleFileFormat::CanFieldChangeAffectFileFormatArguments(
    const TfToken &field,
    ...
    const VtValue &dependencyContextData) const
{
  ...
  if (field == UsdRecursivePayloadsExampleFileFormatTokens->ArgDict) {
    ...
    auto it = depDataDict.find(UsdRecursivePayloadsExampleFileFormatTokens->PayloadId);
    ...
    if (oldValue.IsHolding<int>() && newValue.IsHolding<int>() &&
        oldValue.UncheckedGet<int>() < 1 && newValue.UncheckedGet<int>() < 1)
    {
      return false;
    }
  }
