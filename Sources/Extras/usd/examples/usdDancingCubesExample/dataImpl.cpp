...
TF_DEFINE_PRIVATE_TOKENS(_propertyNameTokens,
                         ((xformOpOrder, "xformOpOrder"))((xformOpTranslate, "xformOp:translate"))(
                             (xformOpRotateXYZ, "xformOp:rotateXYZ"))((displayColor,
...
TF_MAKE_STATIC_DATA((_LeafPrimPropertyMap), _LeafPrimProperties)
{

  // Define the default value types for our animated properties.
  (*_LeafPrimProperties)[_propertyNameTokens->xformOpTranslate].defaultValue = VtValue(GfVec3d(0));
  (*_LeafPrimProperties)[_propertyNameTokens->xformOpRotateXYZ].defaultValue = VtValue(GfVec3f(0));
  (*_LeafPrimProperties)[_propertyNameTokens->displayColor].defaultValue = VtValue(
      VtVec3fArray({GfVec3f(1)}));

  // xformOpOrder is a non-animated property and is specifically translate,
  // rotate for all our geom prims.
  (*_LeafPrimProperties)[_propertyNameTokens->xformOpOrder].defaultValue = VtValue(
      VtTokenArray{_propertyNameTokens->xformOpTranslate, _propertyNameTokens->xformOpRotateXYZ});
  ...
  for (auto &it : *_LeafPrimProperties) {
    it.second.typeName = SdfSchema::GetInstance().FindType(it.second.defaultValue).GetAsToken();
  }
}
...
SdfSpecType UsdDancingCubesExample_DataImpl::GetSpecType(const SdfPath &path) const
{
...
bool UsdDancingCubesExample_DataImpl::Has(const SdfPath &path,
                                          const TfToken &field,
                                          VtValue *value) const
{
...
void UsdDancingCubesExample_DataImpl::VisitSpecs(const SdfAbstractData &data,
...
const std::vector<TfToken> &UsdDancingCubesExample_DataImpl::List(const SdfPath &path) const
{
  ...
  if (path.IsPropertyPath()) {
    ...
    const _LeafPrimPropertyInfo *propInfo = TfMapLookupPtr(*_LeafPrimProperties,
                                                           path.GetNameToken());
    if (propInfo && _leafPrimDataMap.count(path.GetAbsoluteRootOrPrimPath())) {
      ...
      if (propInfo->isAnimated) {
        static std::vector<TfToken> animPropFields(
            {SdfFieldKeys->TypeName, SdfFieldKeys->Default, SdfFieldKeys->TimeSamples});
        return animPropFields;
      }
      else {
        static std::vector<TfToken> nonAnimPropFields(
            {SdfFieldKeys->TypeName, SdfFieldKeys->Default});
    ...
    // Pseudoroot fields.
    static std::vector<TfToken> pseudoRootFields({SdfChildrenKeys->PrimChildren,
    ...
    if (_leafPrimDataMap.count(path)) {
      static std::vector<TfToken> leafPrimFields(
          {SdfFieldKeys->Specifier, SdfFieldKeys->TypeName, SdfChildrenKeys->PropertyChildren});
      return leafPrimFields;
    }
    else {
      static std::vector<TfToken> nonLeafPrimFields(
          {SdfFieldKeys->Specifier, SdfChildrenKeys->PrimChildren});
...
const std::set<double> &UsdDancingCubesExample_DataImpl::ListAllTimeSamples() const
{
  // The set of all time sample times is cached.
  return _animTimeSampleTimes;
}

const std::set<double> &UsdDancingCubesExample_DataImpl::ListTimeSamplesForPath(
...
bool UsdDancingCubesExample_DataImpl::GetBracketingTimeSamples(double time,
                                                               double *tLower,
                                                               double *tUpper) const
{
...
size_t UsdDancingCubesExample_DataImpl::GetNumTimeSamplesForPath(const SdfPath &path) const
{
...
bool UsdDancingCubesExample_DataImpl::GetBracketingTimeSamplesForPath(const SdfPath &path,
                                                                      double time,
                                                                      double *tLower,
                                                                      double *tUpper) const
{
...
bool UsdDancingCubesExample_DataImpl::QueryTimeSample(const SdfPath &path,
                                                      double time,
                                                      VtValue *value) const
{
  // Only leaf prim properties have time samples
  const _LeafPrimData *val = TfMapLookupPtr(_leafPrimDataMap, path.GetAbsoluteRootOrPrimPath());
  if (!val) {
    return false;
  }
  ...
  if (path.GetNameToken() == _propertyNameTokens->xformOpTranslate) {
    ...
    RETURN_TRUE_WITH_OPTIONAL_VALUE(val->pos + GfVec3d(_GetTranslateOffset(offsetTime)));
  }
  if (path.GetNameToken() == _propertyNameTokens->xformOpRotateXYZ) {
    // Animated rotation.
    RETURN_TRUE_WITH_OPTIONAL_VALUE(GfVec3f(_GetRotateAmount(offsetTime)));
  }
  if (path.GetNameToken() == _propertyNameTokens->displayColor) {
    // Animated color value.
    RETURN_TRUE_WITH_OPTIONAL_VALUE(VtVec3fArray({_GetColor(offsetTime)}));
  }
  return false;
}

void UsdDancingCubesExample_DataImpl::_InitFromParams()
{
  ...
  // in the cube layout
  const double frameOffsetAmount = _params.framesPerCycle / (3.0 * _params.perSide);
...
bool UsdDancingCubesExample_DataImpl::_IsAnimatedProperty(const SdfPath &path) const
{
...
bool UsdDancingCubesExample_DataImpl::_HasPropertyDefaultValue(const SdfPath &path,
                                                               VtValue *value) const
{
  // Check that it is a property id.
  if (!path.IsPropertyPath()) {
...
bool UsdDancingCubesExample_DataImpl::_HasPropertyTypeNameValue(const SdfPath &path,
                                                                VtValue *value) const
{
  // Check that it is a property id.
  if (!path.IsPropertyPath()) {
...
double UsdDancingCubesExample_DataImpl::_GetTranslateOffset(double time) const
{
...
double UsdDancingCubesExample_DataImpl::_GetRotateAmount(double time) const
{
  // Rotation value wasn't cached as it's just a linear function over time.
  return 360.0 * time / _animCycleSampleData.size();
}

GfVec3f UsdDancingCubesExample_DataImpl::_GetColor(double time) const
{
  ...
  prevFrameIdx = prevFrameIdx % _animCycleSampleData.size();
  return GfLerp(
      alpha, _animCycleSampleData[prevFrameIdx].color, _animCycleSampleData[nextFrameIdx].color);
}
