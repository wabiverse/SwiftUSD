...
//
#include "UsdVolImaging/dataSourceFieldAsset.h"
...
static TfTokenVector _ConcatenateAttributeNames(const TfTokenVector &left,
...
template<typename FieldSubclass> static const TfTokenVector &_GetStaticUsdAttributeNames()
{
...
static const TfTokenVector &_GetUsdAttributeNames(UsdPrim usdPrim)
{
...
TfTokenVector UsdImagingDataSourceFieldAsset::GetNames()
{
  return _GetUsdAttributeNames(_usdPrim);
}

HdDataSourceBaseHandle UsdImagingDataSourceFieldAsset::Get(const TfToken &name)
{
...
TfTokenVector UsdImagingDataSourceFieldAssetPrim::GetNames()
{
...
HdDataSourceBaseHandle UsdImagingDataSourceFieldAssetPrim::Get(const TfToken &name)
{
  if (name == HdVolumeFieldSchemaTokens->volumeField) {
    return UsdImagingDataSourceFieldAsset::New(
        _GetSceneIndexPath(), _GetUsdPrim(), _GetStageGlobals());
  }

  return UsdImagingDataSourcePrim::Get(name);
}

HdDataSourceLocatorSet UsdImagingDataSourceFieldAssetPrim::Invalidate(
    UsdPrim const &prim,
    ...
    const UsdImagingPropertyInvalidationType invalidationType)
{
  ...
  if (prim.IsA<UsdVolOpenVDBAsset>()) {
    fieldNames = _GetStaticUsdAttributeNames<UsdVolOpenVDBAsset>();
  }
  else if (prim.IsA<UsdVolField3DAsset>()) {
    fieldNames = _GetStaticUsdAttributeNames<UsdVolField3DAsset>();
  }
  else {
    TF_CODING_ERROR("Unsupported field type.");
    return locators;
  ...
  for (const TfToken &propertyName : properties) {
    if (std::find(fieldNames.begin(), fieldNames.end(), propertyName) != fieldNames.end()) {
