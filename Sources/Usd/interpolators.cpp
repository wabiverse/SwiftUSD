...
template <class Src>
bool Usd_UntypedInterpolator::_Interpolate(const Src &src, const SdfPath &path,
                                           double time, double lower,
                                           double upper) {
  if (_attr.GetStage()->GetInterpolationType() == UsdInterpolationTypeHeld) {
    return Usd_HeldInterpolator<VtValue>(_result).Interpolate(src, path, time,
                                                              lower, upper);
  }
...
#define _MAKE_CLAUSE(r, unused, type)                                          \
  {                                                                            \
    ...
    if (attrValueType == valueType) {                                          \
      type result;                                                             \
      if (Usd_LinearInterpolator<type>(&result).Interpolate(src, path, time,   \
                                                            lower, upper)) {   \
        *_result = result;                                                     \
        return true;                                                           \
      }                                                                        \
