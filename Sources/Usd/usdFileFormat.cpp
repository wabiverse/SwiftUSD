...
_GetFileFormatForArguments(const SdfFileFormat::FileFormatArguments &args) {
  ...
  if (it != args.end()) {
    ...
    TF_CODING_ERROR(
        ...
        "Defaulting to '%s'.",
        UsdUsdFileFormatTokens->FormatArg.GetText(), format.c_str(),
...
UsdUsdFileFormat::UsdUsdFileFormat()
    : SdfFileFormat(UsdUsdFileFormatTokens->Id, UsdUsdFileFormatTokens->Version,
...
bool UsdUsdFileFormat::Read(SdfLayer *layer, const string &resolvedPath,
...
bool UsdUsdFileFormat::_ReadHelper(SdfLayer *layer, const string &resolvedPath,
                                   bool metadataOnly) const {
  ...
  // Try binary usdc format first, since that's most common, then usda text.
  {
    ...
    if (usdaFileFormat->_ReadFromAsset(layer, resolvedPath, asset,
                                       metadataOnly)) {
      return true;
    }
    m.Clear();
  ...
  if (usdcFileFormat->_CanReadFromAsset(resolvedPath, asset)) {
    return usdcFileFormat->_ReadFromAsset(layer, resolvedPath, asset,
                                          metadataOnly, Detached);
  }

  if (usdaFileFormat->_CanReadFromAsset(resolvedPath, asset)) {
    return usdaFileFormat->_ReadFromAsset(layer, resolvedPath, asset,
                                          metadataOnly);
  }
...
UsdUsdFileFormat::_GetUnderlyingFileFormatForLayer(const SdfLayer &layer) {
...
bool UsdUsdFileFormat::ReadFromString(SdfLayer *layer,
                                      const std::string &str) const {
  return _GetUnderlyingFileFormatForLayer(*layer)->ReadFromString(layer, str);
}

bool UsdUsdFileFormat::WriteToString(const SdfLayer &layer, std::string *str,
                                     const std::string &comment) const {
  return _GetUnderlyingFileFormatForLayer(layer)->WriteToString(layer, str,
                                                                comment);
}
