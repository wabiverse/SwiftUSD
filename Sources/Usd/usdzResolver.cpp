...
std::string Usd_UsdzResolver::Resolve(const std::string &packagePath,
  ...
  UsdZipFile zipFile;
  std::tie(asset, zipFile) =
      Usd_UsdzResolverCache::GetInstance().FindOrOpenZipFile(packagePath);

  if (!zipFile) {
    return std::string();
  }
  return zipFile.Find(packagedPath) != zipFile.end() ? packagedPath
                                                     : std::string();
}
...
public:
  explicit _Asset(std::shared_ptr<ArAsset> &&sourceAsset, UsdZipFile &&zipFile,
                  const char *dataInZipFile, size_t offsetInZipFile,
                  size_t sizeInZipFile)
      : _sourceAsset(std::move(sourceAsset)), _zipFile(std::move(zipFile)),
        _dataInZipFile(dataInZipFile), _offsetInZipFile(offsetInZipFile),
...
Usd_UsdzResolver::OpenAsset(const std::string &packagePath,
                            const std::string &packagedPath) {
  std::shared_ptr<ArAsset> asset;
  UsdZipFile zipFile;
  std::tie(asset, zipFile) =
  ...
  return std::shared_ptr<ArAsset>(new _Asset(std::move(asset),
                                             std::move(zipFile), iter.GetFile(),
