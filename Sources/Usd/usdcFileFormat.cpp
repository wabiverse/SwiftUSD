...
UsdUsdcFileFormat::UsdUsdcFileFormat()
    : SdfFileFormat(
          UsdUsdcFileFormatTokens->Id, Usd_CrateData::GetSoftwareVersionToken(),
          UsdUsdFileFormatTokens->Target, UsdUsdcFileFormatTokens->Id) {}
...
bool UsdUsdcFileFormat::_CanReadFromAsset(
    const string &filePath, const std::shared_ptr<ArAsset> &asset) const {
  return Usd_CrateData::CanRead(filePath, asset);
}

bool UsdUsdcFileFormat::Read(SdfLayer *layer, const string &resolvedPath,
...
bool UsdUsdcFileFormat::_ReadFromAsset(SdfLayer *layer,
                                       ...
                                       bool metadataOnly, bool detached) const {
  TRACE_FUNCTION();
...
bool UsdUsdcFileFormat::WriteToFile(const SdfLayer &layer,
  ...
  if (auto dataDest = TfDynamic_cast<Usd_CrateDataRefPtr>(
          InitData(FileFormatArguments()))) {
...
bool UsdUsdcFileFormat::ReadFromString(SdfLayer *layer,
                                       const std::string &str) const {
  return SdfFileFormat::FindById(UsdUsdaFileFormatTokens->Id)
      ->ReadFromString(layer, str);
}

bool UsdUsdcFileFormat::WriteToString(const SdfLayer &layer, std::string *str,
