...
{
  const std::string &errorStrTemplate = "Expected array of strings for " + token.GetString() +
                                        " metadata";
  const std::string &errorStr = validatorName.IsEmpty() ? errorStrTemplate :
                                                          errorStrTemplate + " for validator " +
  ...
  for (const PlugPluginPtr &plugin : plugins) {
    ...
    for (const auto &validatorEntry : validators) {
      ...
      metadata.name = TfToken(
          TfStringJoin(std::vector<std::string>{pluginName.GetString(), validatorName.GetString()},
                       _tokens->PluginValidatorNameDelimiter.GetText()));
      validatorNames.push_back(metadata.name);

      metadata.doc = doc->GetString();

      metadata.schemaTypes = _ParseStringArrayValue(
          validatorDict, _tokens->SchemaTypes, metadata.name);

      metadata.keywords = keywords;
      const TfTokenVector &localKeywords = _ParseStringArrayValue(
          validatorDict, _tokens->Keywords, metadata.name);
      metadata.keywords.reserve(metadata.keywords.size() + localKeywords.size());
      metadata.keywords.insert(
          metadata.keywords.end(), localKeywords.begin(), localKeywords.end());
...
void UsdValidationRegistry::_RegisterPluginValidator(const TfToken &validatorName,
...
std::vector<const UsdValidator *> UsdValidationRegistry::GetOrLoadValidatorsByName(
    const TfTokenVector &validatorNames)
{
...
void UsdValidationRegistry::RegisterPluginValidatorSuite(
    const TfToken &suiteName, const std::vector<const UsdValidator *> &containedValidators)
{
  UsdValidatorMetadata metadata;
  ...
  _RegisterValidatorSuite(metadata,
                          containedValidators,
                          /* addMetadata */ false);
}

void UsdValidationRegistry::RegisterValidatorSuite(
...
void UsdValidationRegistry::_RegisterValidatorSuite(
    const UsdValidatorMetadata &metadata,
    const std::vector<const UsdValidator *> &containedValidators,
    bool addMetadata)
{
  if (!_CheckMetadata(metadata,
                      /* checkForPrimTask */ false,
                      /* expectSuite */ true))
  {
    return;
  }
...

  {
    ...
    std::unique_ptr<UsdValidatorSuite> validatorSuite = std::make_unique<UsdValidatorSuite>(
...
const UsdValidatorSuite *UsdValidationRegistry::GetOrLoadValidatorSuiteByName(
...
bool UsdValidationRegistry::GetValidatorMetadata(const TfToken &name,
...
UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForPlugin(
    const TfToken &pluginName) const
{
  return GetValidatorMetadataForPlugins({pluginName});
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForKeyword(
    const TfToken &keyword) const
{
  return GetValidatorMetadataForKeywords({keyword});
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForSchemaType(
...
UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForKeywords(
...
bool UsdValidationRegistry::_AddValidatorMetadata(const UsdValidatorMetadata &metadata)
{
  ...
  if (didAddValidatorMetadata) {
    ...
    _UpdateValidatorNamesMappings(
        _schemaTypeToValidatorNames, metadata.name, metadata.schemaTypes);
