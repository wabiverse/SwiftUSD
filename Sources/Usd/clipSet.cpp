    ...
    clipLayer->Traverse(clipPrimPath, [&manifestLayer, &clipLayer](const SdfPath &path) {
          ...
          clipLayer->GetNumTimeSamplesForPath(path) != 0)
      {
        SdfJustCreatePrimAttributeInLayer(
            manifestLayer, path, layerPtr->GetSchema().FindType(typeName), variability);
      }
    });
  }
...
std::vector<double> Usd_ClipSet::GetTimeSamplesInInterval(const SdfPath &path,
                                                          const GfInterval &interval) const
{
  ...
  for (const Usd_ClipRefPtr &clip : valueClips) {
    ...
    if (!interval.Intersects(GfInterval(clip->startTime,
                                        clip->endTime,
                                        /* minClosed = */ true,
                                        /* maxClosed = */ false)))
    {
  ...
  if (timeSamples.empty() && std::none_of(valueClips.begin(),
                                          valueClips.end(),
