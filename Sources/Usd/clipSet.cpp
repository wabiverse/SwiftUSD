...
SdfLayerRefPtr Usd_GenerateClipManifest(const SdfLayerHandleVector &clipLayers,
  ...
  if (std::any_of(clipLayers.begin(), clipLayers.end(),
  ...
  for (const SdfLayerHandle &clipLayer : clipLayers) {
    clipLayer->Traverse(clipPrimPath, [&manifestLayer,
                                       &clipLayer](const SdfPath &path) {
      ...
      if (!manifestLayer->HasSpec(path) &&
        ...
        SdfJustCreatePrimAttributeInLayer(
            manifestLayer, path, layerPtr->GetSchema().FindType(typeName),
            variability);
      }
    });
  }
...
static bool _ValidateClipFields(const VtArray<SdfAssetPath> &clipAssetPaths,
  ...
  if (!(path.IsAbsolutePath() && path.IsPrimPath())) {
    *errMsg = TfStringPrintf(
        ...
        clipPrimPath.c_str(), UsdClipsAPIInfoKeys->primPath.GetText());
    return false;
  }
  ...
  for (const auto &startFrameAndClipIndex : clipActive) {
    ...
    if (!status.second) {
      *errMsg = TfStringPrintf(
          ...
          (int)startFrameAndClipIndex[1], startFrameAndClipIndex[0],
          UsdClipsAPIInfoKeys->active.GetText(), status.first->second);
      return false;
    }
  }
  ...
  if (clipTimes) {
    ...
    for (const auto &stageTimeAndClipTime : *clipTimes) {
      int &numSeen =
          stageTimesMap.emplace(stageTimeAndClipTime[0], 0).first->second;
      numSeen += 1;
...
Usd_ClipSetRefPtr Usd_ClipSet::New(const std::string &name,
  ...
  //      message..
  if (!_ValidateClipFields(*clipDef.clipAssetPaths, *clipDef.clipPrimPath,
...
Usd_ClipSet::Usd_ClipSet(const std::string &name_,
                         const Usd_ClipSetDefinition &clipDef)
    : name(name_), sourceLayerStack(clipDef.sourceLayerStack),
      sourcePrimPath(clipDef.sourcePrimPath),
      sourceLayer(clipDef.sourceLayerStack
                      ->GetLayers()[clipDef.indexOfLayerWhereAssetPathsFound]),
...
bool Usd_ClipSet::_ClipContributesValue(const Usd_ClipRefPtr &clip,
...
bool Usd_ClipSet::GetBracketingTimeSamplesForPath(const SdfPath &path,
  ...
  if (_ClipContributesValue(activeClip, path)) {
    if (!TF_VERIFY(activeClip->GetBracketingTimeSamplesForPath(path, time,
                                                               lower, upper))) {
      return false;
    }
  ...
  for (size_t i = clipIndex; !foundLower && i-- != 0;) {
    ...
    if (!TF_VERIFY(clip->GetBracketingTimeSamplesForPath(path, time, &tmpLower,
                                                         &tmpUpper))) {
      return false;
    }
...
std::vector<double>
Usd_ClipSet::GetTimeSamplesInInterval(const SdfPath &path,
                                      const GfInterval &interval) const {
  std::vector<double> timeSamples;

  for (const Usd_ClipRefPtr &clip : valueClips) {
    if (interval.IsMaxOpen() ? clip->startTime >= interval.GetMax()
    ...
    Usd_CopyTimeSamplesInInterval(clip->ListTimeSamplesForPath(path), interval,
                                  &timeSamples);
  }
  ...
  if (timeSamples.empty() &&
      std::none_of(valueClips.begin(), valueClips.end(),
...
size_t Usd_ClipSet::_FindClipIndexForTime(double time) const {
  ...
  return TF_VERIFY(clipIndex < valueClips.size() &&
