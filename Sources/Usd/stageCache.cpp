...
class StageContainer {
 public:
  ...
  const StagesById &ById() const
  {
    return _byId;
  }
  /// Return an unordered_map of the ids indexed by stage
  const IdsByStage &ByStage() const
  {
    return _byStage;
  }
  ...
  /// root layer.
  const StagesByRootLayer &ByRootLayer() const
  {
    return _byRootLayer;
  }
  ...
  {
    return _EraseAllIf(rootLayer, [](const auto &stage) { return true; }, erased);
  }
                  ...
                  vector<Entry> *erased)
  {
    return _EraseAllIf(
        rootLayer,
        [&sessionLayer](const auto &stage) { return stage->GetSessionLayer() == sessionLayer; },
        erased);
  }
