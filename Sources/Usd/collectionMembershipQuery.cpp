                          ...
                          SdfPathSet *includedPaths)
{
  ...
  if (query.HasExpression() && (expansionRule == UsdTokens->expandPrims ||
                                expansionRule == UsdTokens->expandPrimsAndProperties))
  {
...
UsdObjectCollectionExpressionEvaluator ::UsdObjectCollectionExpressionEvaluator(
    UsdStageWeakPtr const &stage, SdfPathExpression const &expr)
    : _stage(stage),
      _evaluator(SdfMakePathExpressionEval(expr, UsdGetCollectionPredicateLibrary()))
{
}

SdfPredicateFunctionResult UsdObjectCollectionExpressionEvaluator ::Match(
...
Usd_CollectionMembershipQueryBase::Usd_CollectionMembershipQueryBase(
    const PathExpansionRuleMap &pathExpansionRuleMap, const SdfPathSet &includedCollections)
...
bool Usd_CollectionMembershipQueryBase::_IsPathIncludedByRuleMap(
    const SdfPath &path, const TfToken &parentExpansionRule, TfToken *expansionRule) const
{
  // Coding Error if one passes in a relative path to _IsPathIncludedByRuleMap
  if (!path.IsAbsolutePath()) {
    TF_CODING_ERROR("Relative paths are not allowed");
...
size_t Usd_CollectionMembershipQueryBase::_Hash::operator()(
    ...
    Usd_CollectionMembershipQueryBase::PathExpansionRuleMap const &ruleMap)
{
  ...
  struct StackEntry {
    ...
    StackEntry(SdfPath &&self_, Rule rule_) : self(std::move(self_)), rule(rule_)
    {
      ...
      if (!self.IsAbsoluteRootPath()) {
        expr = Expr::MakeOp(
            rule == Exclude ? Expr::Difference : Expr::Union, Expr::WeakerRef(), std::move(expr));
      }
    }
    SdfPath self;
    Expr expr;
  ...
  return Expr::MakeOp(Expr::ImpliedUnion,
                      ...
                      Expr::MakeOp(Expr::Intersection,
                                   Expr::MakeComplement(std::move(explicitExcludes)),
                                   std::move(result)));
}
