...
bool UsdObject::GetMetadataByDictKey(const TfToken &key, const TfToken &keyPath,
                                     VtValue *value) const {
  return _GetMetadataImpl(key, value, keyPath);
}

bool UsdObject::_GetMetadataImpl(const TfToken &key, VtValue *value,
                                 const TfToken &keyPath) const {
  return _GetStage()->_GetMetadata(*this, key, keyPath, /*useFallbacks=*/true,
                                   value);
}
...
bool UsdObject::SetMetadataByDictKey(const TfToken &key, const TfToken &keyPath,
                                     const VtValue &value) const {
  return _SetMetadataImpl(key, value, keyPath);
}

bool UsdObject::_SetMetadataImpl(const TfToken &key, const VtValue &value,
                                 const TfToken &keyPath) const {
...
bool UsdObject::ClearMetadataByDictKey(const TfToken &key,
                                       const TfToken &keyPath) const {
...
bool UsdObject::HasMetadataDictKey(const TfToken &key,
                                   const TfToken &keyPath) const {
  return _GetStage()->_HasMetadata(*this, key, keyPath,
                                   /*useFallbacks*/ true);
}

bool UsdObject::HasAuthoredMetadataDictKey(const TfToken &key,
                                           const TfToken &keyPath) const {
  return _GetStage()->_HasMetadata(*this, key, keyPath,
                                   /*useFallbacks*/ false);
}
...
void UsdObject::SetCustomDataByKey(const TfToken &keyPath,
                                   const VtValue &value) const {
...
void UsdObject::SetAssetInfoByKey(const TfToken &keyPath,
                                  const VtValue &value) const {
...
std::string UsdObject::_GetObjectDescription(const std::string &preface) const {
  if (_type == UsdTypePrim) {
    ...
    return TfStringPrintf("%sattribute '%s' on ", preface.c_str(),
                          _PropName().GetText()) +
           _Prim().GetDescription(_ProxyPrimPath());
  } else if (_type == UsdTypeRelationship) {
    return TfStringPrintf("%srelationship '%s' on ", preface.c_str(),
                          _PropName().GetText()) +
           _Prim().GetDescription(_ProxyPrimPath());
  } else if (_type == UsdTypeProperty) {
    return TfStringPrintf("%sproperty '%s' on ", preface.c_str(),
                          _PropName().GetText()) +
           _Prim().GetDescription(_ProxyPrimPath());
...
std::string UsdDescribe(const UsdObject &obj) { return obj.GetDescription(); }
