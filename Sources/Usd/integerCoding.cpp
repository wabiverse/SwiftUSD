...
template <class Int>
inline typename std::enable_if<std::is_integral<Int>::value &&
...
inline
    typename std::enable_if<std::is_integral<Int>::value &&
                                std::is_signed<Int>::value && sizeof(Int) == 4,
                            int32_t>::type
    _Signed(Int x) {
  return x;
}
...
inline
    typename std::enable_if<std::is_integral<Int>::value &&
                                std::is_signed<Int>::value && sizeof(Int) == 8,
                            int64_t>::type
    _Signed(Int x) {
  return x;
}
...
template <class Int> struct _SmallTypes {
  typedef typename std::conditional<sizeof(Int) == 4, int8_t, int16_t>::type
      SmallInt;
  typedef typename std::conditional<sizeof(Int) == 4, int16_t, int32_t>::type
      MediumInt;
};
...
void _EncodeNHelper(
    ...
    typename std::make_signed<
        typename std::iterator_traits<Iterator>::value_type>::type &prevVal,
    char *&codesOut, char *&vintsOut) {
...
template <int N, class Iterator>
void _DecodeNHelper(
    char const *&codesIn, char const *&vintsIn,
    typename std::iterator_traits<Iterator>::value_type commonValue,
    typename std::make_signed<
        typename std::iterator_traits<Iterator>::value_type>::type &prevVal,
    Iterator &output) {
...
size_t _CompressIntegers(Int const *begin, size_t numInts, char *output) {
  // Working space.
  std::unique_ptr<char[]> encodeBuffer(
      new char[_GetEncodedBufferSize<Int>(numInts)]);

  // Encode first.
  size_t encodedSize = _EncodeIntegers(begin, numInts, encodeBuffer.get());

  // Then compress.
  return TfFastCompression::CompressToBuffer(encodeBuffer.get(), output,
                                             encodedSize);
}
...
size_t Usd_IntegerCompression::DecompressFromBuffer(char const *compressed,
                                                    ...
                                                    size_t numInts,
                                                    char *workingSpace) {
  return _DecompressIntegers(compressed, compressedSize, ints, numInts,
                             workingSpace);
}
