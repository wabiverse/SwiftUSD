...
std::string Usd_DescribePrimData(const Usd_PrimData *p, SdfPath const &proxyPrimPath)
{
  ...
  const Usd_PrimDataConstPtr prototypeForInstance = isInstance && p->_stage ? p->GetPrototype() :
                                                                              nullptr;

  std::string desc = TfStringPrintf(
      "%s%s%sprim %s<%s> ",
      isDead ? "expired " : (p->_flags[Usd_PrimActiveFlag] ? "" : "inactive "),
  ...
  if (!isDead) {
    ...
    if (isInstanceProxy || isPrototype || isInPrototype) {
      desc += TfStringPrintf("using prim index <%s> ",
