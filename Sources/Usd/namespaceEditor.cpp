 ...
 private:
  ...
  void _Run(const UsdStageRefPtr &stage)
  {
    WorkWithScopedParallelism([this, &stage]() {
      ...
      WorkParallelForEach(
          range.begin(), range.end(), [this](UsdPrim const &prim) { _VisitPrim(prim); });
      _dispatcher.Wait();
    });
  }
  ...
  void _VisitPrim(UsdPrim const &prim)
  {
    ...
    for (Usd_Resolver res(&(prim.GetPrimIndex())); res.IsValid(); res.NextLayer()) {
      ...
      if (!layer->HasField(
              primSpecPath, SdfChildrenKeys->PropertyChildren, &primSpecPropertyNames))
      {
        continue;
      }
      ...
      for (const TfToken &propName : primSpecPropertyNames) {
        ...
        _WorkQueueEntry &workEntry = workEntriesPerProperty
                                         .emplace(prim.GetPrimPath().AppendProperty(propName),
                                                  _WorkQueueEntry())
                                         .first->second;
...
bool UsdNamespaceEditor::ReparentPrim(const UsdPrim &prim,
...
{
  return _AddPropertyMove(property.GetPath(),
                          newParent.GetPrimPath().AppendProperty(property.GetName()));
}
...
static bool _IsValidPropertyToEdit(const UsdPrim &prim,
...
UsdNamespaceEditor::_ProcessedEdit UsdNamespaceEditor::_EditProcessor::ProcessEdit(
    const UsdStageRefPtr &stage, const _EditDescription &editDesc, const EditOptions &editOptions)
{
  _ProcessedEdit processedEdit;
      ...
      _processedEdit(processedEdit)
{
  ...
  _processedEdit->edits.Add(editDesc.oldPath, editDesc.newPath, SdfNamespaceEdit::Same);
...
bool UsdNamespaceEditor::_EditProcessor::_ProcessNewPath()
{
  ...
  if (_editDesc.editType == _EditType::Reparent) {
    ...
    // edit.
    _processedEdit->createParentSpecIfNeededPath = _editDesc.newPath.GetParentPath();
    _processedEdit->removeInertAncestorOvers = true;

    // Validate that the stage does have a prim at the new parent path to
    // reparent to.
    std::string whyNot;
    if (!_IsValidNewParentPath(
            _stage, _editDesc.oldPath, _processedEdit->createParentSpecIfNeededPath, &whyNot))
    {
...
void UsdNamespaceEditor::_EditProcessor::_ProcessPrimEditRequiresRelocates(
    const PcpPrimIndex &primIndex)
{
  ...
  PcpLayerRelocatesEditBuilder builder(_nodeForEditTarget.GetLayerStack(), _editTarget.GetLayer());
  std::string error;
...
void UsdNamespaceEditor::_EditProcessor::_ProcessPropEditRequiresRelocates(
    const PcpPrimIndex &primIndex)
{
  ...
  for (const PcpNodeRef &subtreeNode : subtreeRange) {
    ...
    const bool hasPropertySpecs = std::any_of(
...
void UsdNamespaceEditor::_EditProcessor::_GatherLayersToEdit()
{
  ...
  for (const SdfLayerRefPtr &layer : layers) {
    if (layer->HasSpec(_editDesc.oldPath)) {
      _processedEdit->layersToEdit.push_back(layer);
    }
  }

  // Validate whether the necessary spec edits can actually be performed on
  // each layer that needs to be edited.
  for (const auto &layer : _processedEdit->layersToEdit) {
    ...
    if (!_editDesc.newPath.IsEmpty() && layer->HasSpec(_editDesc.newPath)) {
      _processedEdit->errors.push_back(
          TfStringPrintf("The spec @%s@<%s> "
...
void UsdNamespaceEditor::_EditProcessor::_GatherTargetListOpEdits()
{
  ...
  // namespace path.
  _TargetingPropertyDependencies deps = _TargetingPropertyDependencyCollector::GetDependencies(
      _stage);
  ...
  const auto range = deps.targetedPathToTargetingPropertiesPathTable.FindSubtreeRange(
      _editDesc.oldPath);
  ...
  for (const SdfPath &propertyPath : propPathsWithAffectedTargets) {
    ...
    const _PropertySpecWithAuthoredTargetsVector *propertySpecs = TfMapLookupPtr(
        deps.composedPropertyToSpecsWithAuthoredTargetsMap, propertyPath);
    ...
    for (auto rIt = propertySpecs->rbegin(); rIt != propertySpecs->rend(); ++rIt) {
      ...
      rIt->GetTargetListOp().ApplyOperations(
          &targetsRequireRelocates, [&](SdfListOpType opType, const SdfPath &inPath) {
            const SdfPath translatedPath = rIt->originatingNode.GetMapToRoot().MapSourceToTarget(
                inPath);
...
bool UsdNamespaceEditor::_ProcessedEdit::Apply()
{
  ...
  auto applyEditsToLayersFn = [&]() {
    for (const auto &layer : layersToEdit) {
      ...
      if (!createParentSpecIfNeededPath.IsEmpty() &&
          !SdfJustCreatePrimInLayer(layer, createParentSpecIfNeededPath))
      {
        TF_CODING_ERROR(
  ...
  if (!targetPathListOpErrors.empty()) {
    TF_WARN(
        "Failed to update the following targets and/or connections for "
