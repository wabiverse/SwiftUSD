...
/* virtual */
const TfType &UsdModelAPI::_GetTfType() const { return _GetStaticTfType(); }
...
bool UsdModelAPI::IsKind(const TfToken &baseKind,
...
bool UsdModelAPI::IsModel() const { return GetPrim().IsModel(); }

bool UsdModelAPI::IsGroup() const { return GetPrim().IsGroup(); }
...
bool UsdModelAPI::GetPayloadAssetDependencies(
    VtArray<SdfAssetPath> *assetDeps) const {
  return _GetAssetInfoByKey(UsdModelAPIAssetInfoKeys->payloadAssetDependencies,
                            assetDeps);
}

void UsdModelAPI::SetPayloadAssetDependencies(
    const VtArray<SdfAssetPath> &assetDeps) const {
  GetPrim().SetAssetInfoByKey(
      UsdModelAPIAssetInfoKeys->payloadAssetDependencies, VtValue(assetDeps));
}
