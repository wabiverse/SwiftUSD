...
PcpNodeRef UsdPrimCompositionQueryArc::GetTargetNode() const { return _node; }
...
using _PcpComposeFunc = void (*)(PcpLayerStackRefPtr const &, SdfPath const &,
...
static bool _GetIntroducingComposeInfo(const UsdPrimCompositionQueryArc &arc,
  ...
  composeFunc(arc.GetIntroducingNode().GetLayerStack(),
              arc.GetIntroducingPrimPath(), &result, &info);
...
UsdResolveTarget UsdPrimCompositionQueryArc::MakeResolveTargetUpTo(
...
UsdResolveTarget UsdPrimCompositionQueryArc::MakeResolveTargetStrongerThan(
    const SdfLayerHandle &subLayer) const {
...
bool UsdPrimCompositionQueryArc::GetIntroducingListEditor(
...
bool UsdPrimCompositionQueryArc::GetIntroducingListEditor(
  ...
  PcpSourceArcInfo info;
  if (!_GetIntroducingComposeInfo<SdfPayload>(*this, &PcpComposeSitePayloads,
                                              &info, payload)) {
    return false;
  }
...
bool UsdPrimCompositionQueryArc::GetIntroducingListEditor(
  ...
  if (GetArcType() == PcpArcTypeInherit) {
    ...
    if (!_GetIntroducingComposeInfo<SdfPath>(*this, &PcpComposeSiteInherits,
                                             &info, path)) {
      return false;
    }
    ...
    *editor = _GetIntroducingPrimSpec(*this, info)->GetInheritPathList();
    ...
    if (!_GetIntroducingComposeInfo<SdfPath>(*this, &PcpComposeSiteSpecializes,
                                             &info, path)) {
      return false;
    }
...
bool UsdPrimCompositionQueryArc::HasSpecs() const { return _node.HasSpecs(); }
...
/*static*/
UsdPrimCompositionQuery
UsdPrimCompositionQuery::GetDirectReferences(const UsdPrim &prim) {
  Filter filter;
...
/*static*/
UsdPrimCompositionQuery
UsdPrimCompositionQuery::GetDirectInherits(const UsdPrim &prim) {
  Filter filter;
  filter.dependencyTypeFilter = DependencyTypeFilter::Direct;
...
/*static*/
UsdPrimCompositionQuery
UsdPrimCompositionQuery::GetDirectRootLayerArcs(const UsdPrim &prim) {
  Filter filter;
  filter.dependencyTypeFilter = DependencyTypeFilter::Direct;
...
UsdPrimCompositionQuery::GetCompositionArcs() {
  ...
  if (_filter.arcTypeFilter != ArcTypeFilter::All) {
    filterTests.push_back(
        std::bind(_TestArcType, std::placeholders::_1, _filter));
  }
  if (_filter.dependencyTypeFilter != DependencyTypeFilter::All) {
    filterTests.push_back(
        std::bind(_TestDependencyType, std::placeholders::_1, _filter));
  }
  if (_filter.arcIntroducedFilter != ArcIntroducedFilter::All) {
    filterTests.push_back(
        std::bind(_TestArcIntroduced, std::placeholders::_1, _filter));
  }
  if (_filter.hasSpecsFilter != HasSpecsFilter::All) {
    filterTests.push_back(
        std::bind(_TestHasSpecs, std::placeholders::_1, _filter));
  }
