...
/// (!UsdPrimIsDefined || UsdPrimIsAbstract) && (UsdPrimIsModel ||
/// !UsdPrimIsActive) \endcode
///
///
...
private:
  ...
  bool _Eval(const PrimPtr &prim, bool isInstanceProxy) const {
    ...
    const Usd_PrimFlagBits primFlags =
        Usd_PrimFlagBits(prim->_GetFlags())
  ...
  friend bool Usd_EvalPredicate(const Usd_PrimFlagsPredicate &pred,
                                const PrimPtr &prim, bool isInstanceProxy) {
  ...
  // Equality comparison.
  friend bool operator==(const Usd_PrimFlagsPredicate &lhs,
                         const Usd_PrimFlagsPredicate &rhs) {
    return lhs._mask == rhs._mask && lhs._values == rhs._values &&
...
inline Usd_PrimFlagsConjunction operator&&(Usd_PrimFlags lhs,
                                           Usd_PrimFlags rhs) {
...
inline Usd_PrimFlagsDisjunction operator||(Usd_PrimFlags lhs,
                                           Usd_PrimFlags rhs) {
  return Usd_Term(lhs) || Usd_Term(rhs);
}
...
/// // If prim is an instance, return the children that pass this predicate
/// // as instance proxy prims.
/// prim.GetFilteredChildren(UsdTraverseInstanceProxies(UsdPrimIsModel ||
/// UsdPrimIsGroup)); \endcode
///
