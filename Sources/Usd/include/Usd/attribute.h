...
/// the ability to selectively block an attribute's value for only some
/// intervals in its authored range of samples.  One can block an attribute's
/// value at time *t* by calling `attr.Set(SdfValueBlock, t)` When an attribute
/// is thusly "partially blocked", UsdAttribute::Get() will succeed only for
/// those time intervals whose left/earlier bracketing timeSample is **not**
/// SdfValueBlock.
///
...
///
/// \section Usd_AssetPathValuedAttributes Attributes of type SdfAssetPath and
/// UsdAttribute::Get()
///
