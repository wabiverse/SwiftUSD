...
struct TimeSamples {
  ...
  bool operator==(TimeSamples const &other) const {
    return valueRep == other.valueRep && times == other.times &&
           values == other.values && valuesFileOffset == other.valuesFileOffset;
  }
  ...
  friend size_t hash_value(TimeSamples const &ts) {
    return TfHash::Combine(ts.valueRep, ts.times, ts.values,
                           ts.valuesFileOffset);
  }
...
private:
  ...
  struct _FileRange {
    _FileRange() = default;
    _FileRange(FILE *file, int64_t startOffset, int64_t length,
               bool hasOwnership)
        : file(file), startOffset(startOffset),
          length(file && length == -1 ? ArchGetFileLength(file) - startOffset
                                      : length),
          hasOwnership(hasOwnership) {}
    ~_FileRange();
    _FileRange(_FileRange &&other)
        : file(other.file), startOffset(other.startOffset),
          length(other.length), hasOwnership(other.hasOwnership) {
      other.file = nullptr;
    }
    ...
    class _Impl {
      ...
      struct ZeroCopySource : public Vt_ArrayForeignDataSource {
        ...
        friend size_t hash_value(ZeroCopySource const &z) {
          return TfHash::Combine(reinterpret_cast<uintptr_t>(z._addr),
                                 z._numBytes);
        }
      ...
      explicit _Impl(ArchConstFileMapping &&mapping, int64_t offset,
                     int64_t length)
          : _refCount(0), _mapping(std::move(mapping)),
            _start(_mapping.get() + offset),
            _length(length == -1 ? ArchGetFileMappingLength(_mapping)
                                 : length) {}

      // Add an an externally referenced page range.
      Vt_ArrayForeignDataSource *_AddRangeReference(void const *addr,
                                                    size_t numBytes);
  ...
  public:
    ...
    // Add an an externally referenced page range.
    Vt_ArrayForeignDataSource *AddRangeReference(void const *addr,
                                                 size_t numBytes) {
  ...
  struct Spec {
    ...
    bool operator==(const Spec &other) const {
      return pathIndex == other.pathIndex &&
             fieldSetIndex == other.fieldSetIndex && specType == other.specType;
    }
...
private:
  ...
  static _FileMapping _MmapAsset(char const *fileName,
                                 ArAssetSharedPtr const &asset);
  ...
  template <class Fn>
  void _WriteSection(_Writer &w, _SectionName name, _TableOfContents &toc,
                     Fn writeFn) const;
  ...
  template <class Iter>
  Iter _BuildCompressedPathDataRecursive(size_t &curIndex, Iter cur, Iter end,
  ...
  struct _DeferredSpec {
    ...
    _DeferredSpec(PathIndex p, SdfSpecType t, vector<FieldIndex> &&of,
                  vector<FieldValuePair> &&dof,
                  vector<pair<TfToken, TimeSamples>> &&ts)
        : path(p), specType(t), ordinaryFields(std::move(of)),
  ...
  mutable unordered_map<ValueRep, TimeSamples::SharedTimes, _Hasher>
      _sharedTimes;
  ...
  boost::container::flat_map<std::type_index,
                             std::function<ValueRep(VtValue const &)>>
      _packValueFunctions;
