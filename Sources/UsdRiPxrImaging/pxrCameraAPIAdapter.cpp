...
std::pair<TfToken, TfToken> _SplitNamespace(const TfToken &name)
{
  ...
  return {TfToken(str.substr(0, i)), TfToken(str.substr(i + 1))};
}

HdSampledDataSourceHandle _DataSourceAuthoredAttributeNew(
...
_GetNamespacedAttributeMappingsForAppliedSchema(const TfToken &appliedSchemaName)
{
  std::vector<UsdImagingDataSourceMapped::AttributeMapping> result;

  const UsdPrimDefinition *const primDef =
      UsdSchemaRegistry::GetInstance().FindAppliedAPIPrimDefinition(appliedSchemaName);
  if (!primDef) {
    TF_CODING_ERROR("Could not find definition for applied schema '%s'.",
                    appliedSchemaName.GetText());
    return result;
  }

  for (const TfToken &usdName : primDef->GetPropertyNames()) {
    const std::pair<TfToken, TfToken> namespaceAndName = _SplitNamespace(usdName);
    ...
    result.push_back({usdName,
                      HdDataSourceLocator(namespaceAndName.first, namespaceAndName.second),
...
const UsdImagingDataSourceMapped::AttributeMappings &_GetMappings()
{
  static const UsdImagingDataSourceMapped::AttributeMappings result(
      _GetNamespacedAttributeMappingsForAppliedSchema(TfToken(_appliedSchemaName)),
...
HdContainerDataSourceHandle UsdRiPxrImagingCameraAPIAdapter::GetImagingSubprimData(
    UsdPrim const &prim,
  ...
  return HdRetainedContainerDataSource::New(
      ...
      HdCameraSchema::Builder()
          .SetNamespacedProperties(
              UsdImagingDataSourceMapped::New(prim, prim.GetPath(), _GetMappings(), stageGlobals))
          .Build());
}

HdDataSourceLocatorSet UsdRiPxrImagingCameraAPIAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
    TfToken const &appliedInstanceName,
