...
static bool _HasInputPrefix(const TfToken &attrName, std::string *inputName = nullptr)
{
  const std::pair<std::string, bool> strippedInputName = SdfPath::StripPrefixNamespace(attrName,
                                                                                       "inputs");
...
static HdContainerDataSourceHandle _ComputeResourceDS(UsdPrim const &prim,
                                                      TfToken const &shaderId,
                                                      TfToken const &primType)
{
...
UsdRiPxrImaging_DataSourceRenderTerminalPrim<TerminalSchema>::
    UsdRiPxrImaging_DataSourceRenderTerminalPrim(
        ...
        const TfToken &shaderId,
        const UsdImagingDataSourceStageGlobals &stageGlobals)
    : UsdImagingDataSourcePrim(sceneIndexPath, usdPrim, stageGlobals), _shaderId(shaderId)
{
}

template<typename TerminalSchema>
TfTokenVector UsdRiPxrImaging_DataSourceRenderTerminalPrim<TerminalSchema>::GetNames()
{
...
template<typename TerminalSchema>
HdDataSourceBaseHandle UsdRiPxrImaging_DataSourceRenderTerminalPrim<TerminalSchema>::Get(
    const TfToken &name)
{
  if (name == TerminalSchema::GetSchemaToken()) {
    return HdRetainedContainerDataSource::New(TfToken("resource"),
...
template<typename TerminalSchema>
HdDataSourceLocatorSet UsdRiPxrImaging_DataSourceRenderTerminalPrim<TerminalSchema>::Invalidate(
    UsdPrim const &prim,
    const TfToken &subprim,
    const TfTokenVector &properties,
