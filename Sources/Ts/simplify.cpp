...
static void _SetLeftTangentLength(TsKeyFrame *key, double length)
{
  if (!key)
    return;
...
static void _SetRightTangentLength(TsKeyFrame *key, double length)
{
  if (!key)
    return;
  if (!key->SupportsTangents())
...
static double _ComputeError(const TsSpline &spline,
...
static double _ComputeErrorForLength(bool affectRight,
                                     double l,
                                     const TsKeyFrame &k,
                                     TsSpline *spline,
                                     const GfInterval &spanInterval,
                          ...
                          const GfInterval &valsInterval)
{
  ...
  std::vector<TsKeyFrame> keyFrames = spline->GetKeyFramesInMultiInterval(
      GfMultiInterval(spanInterval));
  ...
  // stretch
  double initialErr = _ComputeError(*spline,
                                    spanInterval,
                                    vals,
                                    valsInterval,
                                    /* useMax = */ false);
  ...
  for (; iter < 100; iter++) {
    ...
    while (1) {
      ...
      double g = (lo + hi) * .5;
      bool isK0 = (iter & 1) == 0;
      double err0 = _ComputeErrorForLength(
          isK0, (g - ldel), isK0 ? k0 : k1, spline, spanInterval, vals, valsInterval);
      double err1 = _ComputeErrorForLength(
          isK0, (g + ldel), isK0 ? k0 : k1, spline, spanInterval, vals, valsInterval);
      ...
      if (hi - lo < .00005) {
        ...
        thisErr = _ComputeErrorForLength(
            isK0, g, isK0 ? k0 : k1, spline, spanInterval, vals, valsInterval);
        break;
      }
    }
...
static bool _IsSplineKinkyInInterval(const TsSpline &spline, const GfInterval &spanInterval)
{
...
double _ComputeErrorIfKeyRemoved(TsSpline *spline,
                                 TsTime t,
                                 const GfInterval &spanInterval,
                                 const std::vector<double> &vals,
                                 const GfInterval &valsInterval)
...
static void _SetKnotInfoErrorIfKeyRemoved(std::vector<_EditSimplifyKnotInfo> &ki,
                                          size_t i,
                                          TsSpline *spline,
                                          const std::vector<double> &vals,
                                          const GfInterval &valsInterval)
{
  ...
  if (ki[i].removable) {
    ...
    ki[i].errIfRemoved = _ComputeErrorIfKeyRemoved(
        spline, ki[i].t, GfInterval(ki[i - 1].t, ki[i + 1].t), vals, valsInterval);
  }
  else {
...
static bool _IsKnotOnPlateau(const TsSpline &spline, const TsKeyFrame &key)
{
  const TsKeyFrameMap &keyMap = spline.GetKeyFrames();

  TsKeyFrameMap::const_iterator kIter = keyMap.lower_bound(key.GetTime());
...
static bool _IsKnotAnExtreme(const TsSpline &spline, const TsKeyFrame &k, double tolerance)
{

  std::pair<TsExtrapolationType, TsExtrapolationType> extrap = spline.GetExtrapolation();
  ...
  const TsKeyFrameMap &keyMap = spline.GetKeyFrames();
  // This points at k
  TsKeyFrameMap::const_iterator kIter = keyMap.lower_bound(k.GetTime());
  if (!TF_VERIFY(kIter != keyMap.end()))
    return false;
                      ...
                      double extremeMaxErrFract)
{
  ...
  // Extra one before
  if (!valsInterval.IsMinClosed() && splineCopy.count(valsInterval.GetMin())) {
    ...
    std::optional<TsKeyFrame> before = splineCopy.GetClosestKeyFrameBefore(
        intervals.GetBounds().GetMin());
  ...
  // Extra one after
  if (!valsInterval.IsMaxClosed() && splineCopy.count(valsInterval.GetMax())) {
    ...
    std::optional<TsKeyFrame> after = splineCopy.GetClosestKeyFrameAfter(
        intervals.GetBounds().GetMax());
  ...
  // Get all the keys
  std::vector<TsKeyFrame> keyFrames = splineCopy.GetKeyFramesInMultiInterval(
      GfMultiInterval(valsInterval));
  ...
  for (double t = valsInterval.GetMin(); t <= valsInterval.GetMax(); t += 1.0) {
  ...
  std::pair<TsExtrapolationType, TsExtrapolationType> extrap = splineCopy.GetExtrapolation();
  ...
  for (size_t i = 0; i < keyFrames.size(); ++i) {
    ...
    bool rightAdjacent = i < keyFrames.size() - 1 && (keyFrames[i + 1].GetTime() - t) == 1;
    // Is there a knot 1 frame adjacent to the left?
    bool leftAdjacent = i > 0 && (t - keyFrames[i - 1].GetTime()) == 1;
...
#if SIMPLIFY_DEBUG
        printf(
            "TsSimplifySpline keyFrame: %ld at time %g"
    ...
    else if (t == fullRange.GetMin() && rightAdjacent && extrap.first == TsExtrapolationHeld) {
    ...
    else if (t == fullRange.GetMax() && leftAdjacent && extrap.second == TsExtrapolationHeld) {
...
#if SIMPLIFY_DEBUG
    printf(
        "TsSimplifySpline keyFrame: %ld at time %g result slope"
        " %g/%g length %g/%g\n",
        i,
        t,
        k.GetLeftTangentSlope().Get<double>(),
        k.GetRightTangentSlope().Get<double>(),
        k.GetLeftTangentLength(),
        k.GetRightTangentLength());
#endif
  }
  ...
  for (size_t i = 0; i < keyFrames.size(); ++i) {
    ...
    k.removable = intervals.Contains(k.t) &&
                  ...
                  // removable
                  i != 0 && !_IsKnotAnExtreme(splineCopy, keyFrames[i], extremeTolerance);

#if SIMPLIFY_DEBUG
    printf("TsSimplifySpline keyFrame: %ld at time %g %s\n",
           i,
           k.t,
           intervals.Contains(k.t) ? "CONTAINED" : "NOT CONTAINED");
    printf("TsSimplifySpline keyFrame: %ld at time %g %s\n",
           i,
           k.t,
           _IsKnotAnExtreme(splineCopy, keyFrames[i], extremeTolerance) ? "EXTREME" : "normal");
    printf("TsSimplifySpline keyFrame: %ld at time %g %s\n",
           i,
           k.t,
  ...
  for (size_t i = 0; i < ki.size(); ++i) {
    _SetKnotInfoErrorIfKeyRemoved(ki, i, &splineCopy, vals, valsInterval);
  }
  ...
  while (1) {
    ...
    if (ki[bestIndex].errIfRemoved <= tolerance) {
      ...
      _SimplifySpan(
          &splineCopy, GfInterval(ki[bestIndex - 1].t, ki[bestIndex + 1].t), vals, valsInterval);
      ...
      // span.
      _SetKnotInfoErrorIfKeyRemoved(ki, bestIndex - 1, &splineCopy, vals, valsInterval);
      _SetKnotInfoErrorIfKeyRemoved(ki, bestIndex, &splineCopy, vals, valsInterval);
...
void TsSimplifySplinesInParallel(const std::vector<TsSpline *> &splines,
                                 ...
                                 double extremeMaxErrFract)
{
  ...
  if (useFullRangeForEach) {
    WorkParallelForEach(splines.begin(), splines.end(), [&](TsSpline *spline) {
      TsSimplifySpline(
          spline, GfMultiInterval(spline->GetFrameRange()), maxErrorFraction, extremeMaxErrFract);
    });
    return;
  }
  ...
  if (splines.size() != intervals.size()) {
    TF_CODING_ERROR("splines size %zd != intervals size %zd", splines.size(), intervals.size());
    return;
  }
  // If just one, don't bother to construct the arg for WorkParallelForEach,
  // just call TsSimplifySpline()
  if (numSplines == 1) {
    TsSimplifySpline(splines[0], intervals[0], maxErrorFraction, extremeMaxErrFract);
    return;
  }
  ...
  WorkParallelForEach(args.begin(), args.end(), [&](SplineAndIntervals &splineAndIntervals) {
    TsSimplifySpline(
        splineAndIntervals.first, splineAndIntervals.second, maxErrorFraction, extremeMaxErrFract);
  });
}
