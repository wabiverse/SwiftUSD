                                                     ...
                                                     TsSide side)
{
  ...
  // value discontinuity).
  if ((!kf.HasTangents()) && kf.GetIsDualValued()) {
    return TsExtrapolationHeld;
  }
...
TsExtrapolationType Ts_GetEffectiveExtrapolationType(const TsKeyFrame &kf,
                                                     ...
                                                     TsSide side)
{
  return Ts_GetEffectiveExtrapolationType(kf, spline.GetExtrapolation(), spline.size() == 1, side);
}

////////////////////////////////////////////////////////////////////////

static VtValue _GetExtrapolationSlope(TsSpline::const_iterator i, const TsSpline &val, TsSide side)
{
  ...
  switch (Ts_GetEffectiveExtrapolationType(kf, val, side)) {
    ...
    case TsExtrapolationLinear:
      if (kf.HasTangents()) {
        slope = (side == TsLeft) ? kf.GetLeftTangentSlope() : kf.GetRightTangentSlope();
      }
      else {
...
static VtValue _Extrapolate(TsTime time,
...
static VtValue _GetSlopeToAdjacentKnot(TsSpline::const_iterator i, TsSide side)
{
  // Set i and j to the left and right key frames of the segment
  ...
  if (atKnot) {
    ...
    if (evalType == Ts_EvalValue) {
      ...
      if (!knotIt->IsExtrapolatable()) {
        ...
        else if (knotIt->GetKnotType() == TsKnotHeld && prevIt->GetKnotType() == TsKnotBezier) {
          // Left derivative of held knot after Bezier = zero.
          return knotIt->GetZero();
        }
        else if (knotIt->GetKnotType() == TsKnotHeld && prevIt->GetKnotType() == TsKnotLinear) {
    ...
    return (evalType == Ts_EvalValue) ? _Extrapolate(time, nextIt, val, TsLeft) :
                                        _GetExtrapolationSlope(nextIt, val, TsLeft);
  }
  else if (afterEnd) {
    // After last knot.  Extrapolate to the right.
    return (evalType == Ts_EvalValue) ? _Extrapolate(time, prevIt, val, TsRight) :
                                        _GetExtrapolationSlope(prevIt, val, TsRight);
  }
  else {
    // Between knots.  Interpolate.
    if (evalType == Ts_EvalDerivative && prevIt->IsExtrapolatable() &&
        !prevIt->SupportsTangents() && prevIt->GetKnotType() == TsKnotLinear)
    {
      ...
      return (evalType == Ts_EvalValue) ?
                 Ts_UntypedEvalCache::EvalUncached(*prevIt, *nextIt, time) :
                 Ts_UntypedEvalCache::EvalDerivativeUncached(*prevIt, *nextIt, time);
    }
  }
...
bool Ts_IsSegmentValueMonotonic(const TsKeyFrame &kf1, const TsKeyFrame &kf2)
{
  ...
  if (kf1.GetKnotType() == TsKnotBezier && kf2.GetKnotType() == TsKnotBezier &&
      kf1VtVal.IsHolding<double>() && kf2LeftVtVal.IsHolding<double>() &&
      ...
      kf2LeftTangentSlopeVtVal.IsHolding<double>())
  {
    ...
    double x1 = kf1VtVal.Get<double>() +
                (kf1.GetRightTangentSlope().Get<double>() * kf1.GetRightTangentLength());
    double x2 = kf2LeftVtVal.Get<double>() -
                (kf2.GetLeftTangentSlope().Get<double>() * kf2.GetLeftTangentLength());
    ...
    if (Ts_SolveQuadratic(polyDeriv, &root0, &root1)) {
      ...
      //  (monotonic = false)
               ...
               ((LT(0, root0) && LT(root0, 1)) || (LT(0, root1) && LT(root1, 1))))
      {
        monotonic = false;
      }
    }
...
static std::pair<TsSpline::const_iterator, TsSpline::const_iterator> _GetBounds(
    const TsSpline &val, TsTime startTime, TsTime endTime)
{
...
template<typename T>
static std::pair<T, T> _GetBezierRange(const Ts_Bezier<T> *bezier,
                                       double startTime,
                                       double endTime)
{
  ...
  if (startTime > bezier->timePoints[0] || endTime < bezier->timePoints[3]) {
    if (startTime > bezier->timePoints[0]) {
      uMin = GfClamp(Ts_SolveCubic(bezier->timeCoeff, startTime), 0.0, 1.0);
    }
    if (endTime < bezier->timePoints[3]) {
      uMax = GfClamp(Ts_SolveCubic(bezier->timeCoeff, endTime), 0.0, 1.0);
    }
    if (uMin > uMax) {
      uMin = uMax;
    }
...
// side will be ignored
template<typename T>
static std::pair<T, T> _GetSegmentRange(const Ts_EvalCache<T> *cache,
                                        double startTime,
                                        double endTime)
{
...
template<typename T>
static std::pair<VtValue, VtValue> _GetCurveRange(const TsSpline &val,
                                                  double startTime,
                                                  double endTime)
{
  T min = std::numeric_limits<T>::infinity();
  ...
  TsSpline::const_iterator i2 = i;
  i2++;
  while (i != j) {
    if (i2 != val.end()) {
      Ts_EvalCache<T> cache(*i, *i2);

      std::pair<T, T> range = _GetSegmentRange<T>(&cache, startTime, endTime);
...
template<typename T>
static double _BezierHeight(const TsTime timeBezier[4],
                            const T valueBezier[4],
                            double timeScale,
                            double valueScale)
{
...
template<typename T>
static void _SubdivideBezier(const T inBezier[4], T outBezier[4], double u, bool leftSide)
{
  if (leftSide) {
    // Left Bezier
...
template<typename T>
static void _SampleBezier(const TsTime timeBezier[4],
                          const T valueBezier[4],
                          double startTime,
                          double endTime,
                          double timeScale,
                          ...
                          TsSamples &samples)
{
  ...
  if (timeBezier[0] >= endTime || timeBezier[3] <= startTime) {
    return;
  }
  ...
  if (e <= toleranceFactor * tolerance) {
    ...
    samples.push_back(TsValueSample(
        timeBezier[0], VtValue(valueBezier[0]), timeBezier[3], VtValue(valueBezier[3])));
  }
  ...
  // Blur sample if we're below the tolerance in time
    ...
    std::pair<T, T> range = _GetBezierRange<T>(&tmpBezier, startTime, endTime);
    samples.push_back(TsValueSample(GfMax(timeBezier[0], startTime),
                                    VtValue(range.first),
  ...
  // Subdivide
    ...
    // Recurse
    _SampleBezier(
        leftTime, leftValue, startTime, endTime, timeScale, valueScale, tolerance, samples);
    _SampleBezier(
        rightTime, rightValue, startTime, endTime, timeScale, valueScale, tolerance, samples);
  }
}

template<typename T>
static void _SampleBezierClip(const TsTime timePoly[4],
                              const TsTime timeBezier[4],
                              const T valueBezier[4],
                              double startTime,
                              double endTime,
                              double timeScale,
                              ...
                              TsSamples &samples)
{
  ...
  if (Ts_SolveQuadratic(timeDeriv, &root0, &root1)) {
    if (root0 >= 0.0 - rootTolerance && root1 <= 1.0 + rootTolerance) {
      ...
      if (t0 >= t1) {
        ...
        root0 = Ts_SolveCubicInInterval(timePoly, timeDeriv, t, GfInterval(0, root0));
        root1 = Ts_SolveCubicInInterval(timePoly, timeDeriv, t, GfInterval(root1, 1));
        ...
        if (numSamples1 > 0) {
          ...
          samples.push_back(TsValueSample(0, VtValue(), 0, VtValue()));
          numSamples1++;
        }
        ...
        if (numSamples1 != numSamples2) {
          ...
          TsTime d = GfMin(0.001, 0.001 * (s.rightTime - s.leftTime));
          s.rightTime -= d;
  ...
  _SampleBezier(
      timeBezier, valueBezier, startTime, endTime, timeScale, valueScale, tolerance, samples);
}

// Sample segment with results in samples.
template<typename T>
static void _SampleSegment(const Ts_EvalCache<T, TsTraits<T>::interpolatable> *cache,
                           double startTime,
                           double endTime,
                           double timeScale,
                           double valueScale,
...
static void _AddExtrapolateSample(const TsSpline &val,
                                  double t,
                                  double dtExtrapolate,
                                  TsSamples &samples)
{
  ...
  if (dtExtrapolate < 0.0) {
    ...
    samples.push_back(TsValueSample(t + dtExtrapolate, yLeft, t, yRight));
  }
  else {
    yLeft = val.Eval(t, TsRight);
    yRight = val.Eval(t + dtExtrapolate, TsLeft);
    samples.push_back(TsValueSample(t, yLeft, t + dtExtrapolate, yRight));
  }
}
...
static void _EvalLinear(const TsSpline &val, TsTime startTime, TsTime endTime, TsSamples &samples)
{
  ...
  if (startTime < first.GetTime()) {
    ...
    _AddExtrapolateSample(
        val, first.GetTime(), (startTime - first.GetTime() - extrapolateDistance), samples);
  ...
  std::pair<TsSpline::const_iterator, TsSpline::const_iterator> bounds = _GetBounds(
  ...
  while (i != j) {
    ...
    samples.push_back(TsValueSample(t0, val.Eval(t0, TsRight), t1, val.Eval(t1, TsLeft)));
  }
  ...
  if (endTime > last.GetTime()) {
    ...
    _AddExtrapolateSample(
        val, last.GetTime(), (endTime - last.GetTime() + extrapolateDistance), samples);
  }
}

template<typename T>
static void _EvalCurve(const TsSpline &val,
                       TsTime startTime,
                       TsTime endTime,
                       double timeScale,
                       double valueScale,
                       ...
                       TsSamples &samples)
{
  ...
  while (i != j) {
    if (i2 != val.end()) {
      ...
      _SampleSegment<T>(&cache, startTime, endTime, timeScale, valueScale, tolerance, samples);
    }
                    ...
                    double tolerance)
{
  ...
  const std::type_info &t = val.GetTypeid();
  if (TfSafeTypeCompare(t, typeid(double))) {
    _EvalCurve<double>(val, startTime, endTime, timeScale, valueScale, tolerance, samples);
  }
  else if (TfSafeTypeCompare(t, typeid(float))) {
    _EvalCurve<float>(val, startTime, endTime, timeScale, valueScale, tolerance, samples);
  }
  else {
...
template<typename T>
static void _Breakdown(TsKeyFrameMap *k,
                       ...
                       const TsKeyFrameMap::iterator &k3)
{
  ...
  if (k2->SupportsTangents()) {
    k2->SetLeftTangentSlope(VtValue((leftValue[3] - leftValue[2]) / (leftTime[3] - leftTime[2])));
    k2->SetRightTangentSlope(
        VtValue((rightValue[1] - rightValue[0]) / (rightTime[1] - rightTime[0])));
  }
...
void Ts_Breakdown(TsKeyFrameMap *k)
{
  ...
  TsKeyFrameMap::iterator k2 = k1;
  k2++;
  TsKeyFrameMap::iterator k3 = k2;
  k3++;

  if (k1->GetTime() >= k2->GetTime() || k2->GetTime() >= k3->GetTime()) {
