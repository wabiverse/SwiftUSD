...
class Ts_SplineChangedIntervalHelper {
 ...
 public:
  Ts_SplineChangedIntervalHelper(const TsSpline *s1, const TsSpline *s2);
  GfInterval ComputeChangedInterval();

 private:
  // Helper functions for tightening the changed interval from the left
  static KeyFrameReverseIterator _GetFirstKeyFrame(const TsSpline &spline,
                                                   KeyFrameReverseIterator kf);
  KeyFrameIterator _GetNextNonFlatKnot(const TsSpline &spline,
  ...
  static KeyFrameIterator _GetLastKeyFrame(const TsSpline &spline, KeyFrameIterator kf);
  KeyFrameReverseIterator _GetPreviousNonFlatKnot(const TsSpline &spline,
...
Ts_SplineChangedIntervalHelper::Ts_SplineChangedIntervalHelper(const TsSpline *s1,
                                                               const TsSpline *s2)
    : _s1(s1), _s2(s2)
{
}

GfInterval Ts_SplineChangedIntervalHelper::ComputeChangedInterval()
{
  TRACE_FUNCTION();
...
Ts_SplineChangedIntervalHelper::KeyFrameIterator Ts_SplineChangedIntervalHelper::_GetLastKeyFrame(
    const TsSpline &spline, KeyFrameIterator kf)
{
  TF_VERIFY(kf + 1 == spline.end());

  if (Ts_GetEffectiveExtrapolationType(*kf, spline, TsRight) == TsExtrapolationHeld) {
    return spline.end();
  }
  else {
    return kf;
...
Ts_SplineChangedIntervalHelper::KeyFrameIterator Ts_SplineChangedIntervalHelper::
    _GetNextNonFlatKnot(const TsSpline &spline, const KeyFrameIterator &startKeyFrame)
{
  TRACE_FUNCTION();
  ...
  while (kf != spline.end()) {
    ...
    if (kf != startKeyFrame) {
      ...
      // non-flat one.
      if (!prevHeldValue.IsEmpty() && kf->GetValue() != prevHeldValue) {
        return kf;
      }
    }
...
// any more.
bool Ts_SplineChangedIntervalHelper::_TightenToNextKeyFrame(bool extrapolateHeldLeft)
{
  TRACE_FUNCTION();
  ...
  if (!extrapolateHeldLeft || !_s1Iter->GetIsDualValued() ||
      _s1Iter->GetValue() == _s1Iter->GetLeftValue())
  {
    s1NextNonFlat = _GetNextNonFlatKnot(*_s1, _s1Iter);
  }
  if (!extrapolateHeldLeft || !_s2Iter->GetIsDualValued() ||
  ...
  // interval tightening.
  if (extrapolateHeldLeft || (s1NextNonFlat != _s1Iter && s2NextNonFlat != _s2Iter &&
                              _s1Iter->GetValue() == _s2Iter->GetValue()))
  {
    ...
    TsTime s1NextKfTime = (s1NextNonFlat == _s1->end()) ? infinity : s1NextNonFlat->GetTime();
    TsTime s2NextKfTime = (s2NextNonFlat == _s2->end()) ? infinity : s2NextNonFlat->GetTime();
...
void Ts_SplineChangedIntervalHelper::_TightenFromLeft()
{
  TRACE_FUNCTION();
  ...
  const TsExtrapolationType aExtrapLeft = Ts_GetEffectiveExtrapolationType(*_s1Iter, *_s1, TsLeft);
  const TsExtrapolationType bExtrapLeft = Ts_GetEffectiveExtrapolationType(*_s2Iter, *_s2, TsLeft);

  // We can't tighten if the extrapolations or the extrapolated values are
  // different.
  if (aExtrapLeft != bExtrapLeft || _s1Iter->GetLeftValue() != _s2Iter->GetLeftValue()) {
    return;
  }
  ...
  // time.
           ...
           _s1Iter->GetLeftTangentSlope() == _s2Iter->GetLeftTangentSlope())
  {
    bool closed = _s1Iter->GetValue() != _s2Iter->GetValue();
    _changedInterval.SetMin(_s1Iter->GetTime(), closed);
...
// Return the iterator representing the last keyframe: if extrapolating, return
// end(); otherwise, return the last keyframe
Ts_SplineChangedIntervalHelper::KeyFrameReverseIterator Ts_SplineChangedIntervalHelper::
    _GetFirstKeyFrame(const TsSpline &spline, KeyFrameReverseIterator kf)
{
  TF_VERIFY(kf + 1 == spline.rend());

  if (Ts_GetEffectiveExtrapolationType(*kf, spline, TsLeft) == TsExtrapolationHeld) {
    return spline.rend();
  }
  else {
    return kf;
...
// startKeyFrame.
Ts_SplineChangedIntervalHelper::KeyFrameReverseIterator Ts_SplineChangedIntervalHelper::
    _GetPreviousNonFlatKnot(const TsSpline &spline, const KeyFrameReverseIterator &startKeyFrame)
{
  TRACE_FUNCTION();
  ...
  while (kf != spline.rend()) {
    ...
    if (prevKeyFrameIt->GetKnotType() == TsKnotHeld) {
      if (kf == startKeyFrame || kf->GetLeftValue() == prevKeyFrameIt->GetValue()) {
        ++kf;
        continue;
      }
    }
...
// any more.
bool Ts_SplineChangedIntervalHelper::_TightenToPreviousKeyFrame(bool extrapolateHeldRight)
{
  TRACE_FUNCTION();
  const TsTime infinity = std::numeric_limits<TsTime>::infinity();
  ...
  if (extrapolateHeldRight) {
    if (_s1ReverseIter->GetValue() != _s2ReverseIter->GetValue()) {
      return false;
    }
  }

  // First find the previous non flat knots from the current key frames.
  KeyFrameReverseIterator s1PrevNonFlat = _GetPreviousNonFlatKnot(*_s1, _s1ReverseIter);
  KeyFrameReverseIterator s2PrevNonFlat = _GetPreviousNonFlatKnot(*_s2, _s2ReverseIter);
  ...
  const VtValue s1PrevValue = s1PrevNonFlat == _s1->rend() ? _s1->begin()->GetLeftValue() :
                                                             s1PrevNonFlat->GetValue();
  const VtValue s2PrevValue = s2PrevNonFlat == _s2->rend() ? _s2->begin()->GetLeftValue() :
                                                             s2PrevNonFlat->GetValue();
    ...
    if (s1PrevNonFlat != _s1ReverseIter) {
      if (s1PrevValue != _s1ReverseIter->GetValue() ||
          (_s1ReverseIter->GetIsDualValued() &&
           _s1ReverseIter->GetValue() != _s1ReverseIter->GetLeftValue()))
      {
    ...
    if (s2PrevNonFlat != _s2ReverseIter) {
      if (s2PrevValue != _s2ReverseIter->GetValue() ||
          (_s2ReverseIter->GetIsDualValued() &&
           _s2ReverseIter->GetValue() != _s2ReverseIter->GetLeftValue()))
      {
  ...
  // the same value on both splines, then we can do the flat segment
  // interval tightening.
  if (extrapolateHeldRight || (s1PrevNonFlat != _s1ReverseIter &&
                               s2PrevNonFlat != _s2ReverseIter && s1PrevValue == s2PrevValue))
  {

    // If the splines are flat to the end, then there is no
    // evaluative difference between the two.  Return an empty
    // interval.
    const TsTime s1PrevKfTime = (s1PrevNonFlat == _s1->rend()) ? -infinity :
                                                                 s1PrevNonFlat->GetTime();
    const TsTime s2PrevKfTime = (s2PrevNonFlat == _s2->rend()) ? -infinity :
                                                                 s2PrevNonFlat->GetTime();
    ...
    else  // (s2PrevKfTime == s1PrevKfTime)
    {
      ...
      // different.
      closed = s1PrevNonFlat->GetLeftValue() != s2PrevNonFlat->GetLeftValue();
  ...
  // segment equivalence check.
    ...
    if (_s1ReverseIter->IsEquivalentAtSide(*_s2ReverseIter, TsLeft)) {
      ...
      ++_s1ReverseIter;
      ++_s2ReverseIter;
      if (_s1ReverseIter != _s1->rend() && _s2ReverseIter != _s2->rend() &&
          _s1ReverseIter->IsEquivalentAtSide(*_s2ReverseIter, TsRight))
      {
        ...
        // different.
        const bool closed = (_s1ReverseIter->GetLeftValue() != _s2ReverseIter->GetLeftValue());
...
void Ts_SplineChangedIntervalHelper::_TightenFromRight()
{
  TRACE_FUNCTION();
  ...
  const TsExtrapolationType aExtrapRight = Ts_GetEffectiveExtrapolationType(
      *_s1ReverseIter, *_s1, TsRight);
  const TsExtrapolationType bExtrapRight = Ts_GetEffectiveExtrapolationType(
      *_s2ReverseIter, *_s2, TsRight);

  // We can't tighten if the extrapolations or the extrapolated values are
  // different.
  if (aExtrapRight != bExtrapRight || _s1ReverseIter->GetValue() != _s2ReverseIter->GetValue()) {
    return;
  }
  ...
  // slopes match, do we not have a change after the last keyframes
           ...
           _s1ReverseIter->GetRightTangentSlope() == _s2ReverseIter->GetRightTangentSlope())
  {
