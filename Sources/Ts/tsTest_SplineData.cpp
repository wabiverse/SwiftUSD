...
TsTest_SplineData::Knot::Knot(const Knot &other) = default;

TsTest_SplineData::Knot &TsTest_SplineData::Knot::operator=(const Knot &other) = default;

bool TsTest_SplineData::Knot::operator==(const Knot &other) const
{
  return time == other.time && nextSegInterpMethod == other.nextSegInterpMethod &&
         value == other.value && isDualValued == other.isDualValued &&
         preValue == other.preValue && preSlope == other.preSlope &&
...
TsTest_SplineData::InnerLoopParams::InnerLoopParams(const InnerLoopParams &other) = default;

TsTest_SplineData::InnerLoopParams &TsTest_SplineData::InnerLoopParams::operator=(
    const InnerLoopParams &other) = default;

bool TsTest_SplineData::InnerLoopParams::operator==(const InnerLoopParams &other) const
{
  return enabled == other.enabled && protoStart == other.protoStart &&
         protoEnd == other.protoEnd && preLoopStart == other.preLoopStart &&
         postLoopEnd == other.postLoopEnd && closedEnd == other.closedEnd &&
...
bool TsTest_SplineData::InnerLoopParams::IsValid() const
{
  ...
  if (protoEnd <= protoStart)
    return false;
  if (preLoopStart > protoStart)
    return false;
  if (postLoopEnd < protoEnd)
    return false;
...
TsTest_SplineData::Extrapolation::Extrapolation(const TsTest_SplineData::ExtrapMethod methodIn)
    : method(methodIn)
{
}

TsTest_SplineData::Extrapolation::Extrapolation(const Extrapolation &other) = default;

TsTest_SplineData::Extrapolation &TsTest_SplineData::Extrapolation::operator=(
...
TsTest_SplineData::TsTest_SplineData(const TsTest_SplineData &other) = default;

TsTest_SplineData &TsTest_SplineData::operator=(const TsTest_SplineData &other) = default;

bool TsTest_SplineData::operator==(const TsTest_SplineData &other) const
{
  return _isHermite == other._isHermite && _knots == other._knots &&
         _preExtrap == other._preExtrap && _postExtrap == other._postExtrap &&
...
std::string TsTest_SplineData::GetDebugDescription() const
{
  std::ostringstream ss;
  ...
  if (_innerLoopParams.enabled) {
    ss << "Loop:" << std::endl
       << "  start " << _innerLoopParams.protoStart << ", end " << _innerLoopParams.protoEnd
       << ", preStart " << _innerLoopParams.preLoopStart << ", postEnd "
       << _innerLoopParams.postLoopEnd << ", closed " << _innerLoopParams.closedEnd << ", offset "
       << _innerLoopParams.valueOffset << std::endl;
  }

  ss << "Knots:" << std::endl;
  for (const Knot &knot : _knots) {
    ss << "  " << knot.time << ": " << knot.value << ", "
