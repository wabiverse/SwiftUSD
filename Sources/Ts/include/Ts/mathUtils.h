...
//   3*c[3]*u^2 + 2*c[2] + c[1]
template<typename T> void Ts_CubicDerivative(const T poly[4], double deriv[3])
{
  deriv[2] = 3. * poly[3];
  deriv[1] = 2. * poly[2];
  deriv[0] = poly[1];
...
template<typename T> T Ts_EvalQuadratic(const T c[3], double u)
{
  return u * (u * c[2] + c[1]) + c[0];
}

// Evaluate the cubic polynomial in c[] at u.
template<typename T> T Ts_EvalCubic(const T c[4], double u)
{
  return u * (u * (u * c[3] + c[2]) + c[1]) + c[0];
}

template<typename T> T Ts_EvalCubicDerivative(const T c[4], double u)
{
