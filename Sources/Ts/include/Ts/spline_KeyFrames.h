...
class TsSpline_KeyFrames {
 ...
 private:
  typedef std::pair<TsKeyFrameMap::iterator, TsKeyFrameMap::iterator> _KeyFrameRange;
  ...
  void _UnrollKeyFrames(TsKeyFrameMap *keyFrames, const TsLoopParams &params);
  ...
  TsExtrapolationType _GetEffectiveExtrapolationType(const TsKeyFrame &keyFrame,
                                                     const TsSide &side) const;

  /// The Spline calls these when the loop params have changed.
  void _LoopParamsChanged(bool loopingChanged, bool valueOffsetChanged, bool domainChanged);
