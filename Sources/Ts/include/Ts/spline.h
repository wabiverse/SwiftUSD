...
class TsSpline final {
 public:
  ...
  bool ClearRedundantKeyFrames(const VtValue &defaultValue = VtValue(),
                               const GfMultiInterval &intervals = GfMultiInterval(
  ...
  TS_API
  std::vector<TsKeyFrame> GetKeyFramesInMultiInterval(const GfMultiInterval &) const;
  ...
  TS_API
  void SetKeyFrame(TsKeyFrame kf, GfInterval *intervalAffected = nullptr);

  /// Checks if the given keyframe is a valid candidate to set,
  /// optionally returning the reason if it cannot.
  TS_API
  bool CanSetKeyFrame(const TsKeyFrame &kf, std::string *reason = nullptr) const;
  ...
  TS_API
  std::optional<TsKeyFrame> Breakdown(double x,
                                      TsKnotType type,
                                      bool flatTangents,
  ...
  TS_API
  void Breakdown(const std::set<double> &times,
                 TsKnotType type,
                 bool flatTangents,
                 double tangentLength,
                 const VtValue &value = VtValue(),
  ...
  /// existing key frames at the given times.
  TS_API
  void Breakdown(const std::vector<double> &times,
                 TsKnotType type,
                 bool flatTangents,
                 double tangentLength,
  ...
  /// time range affected.
  TS_API
  void RemoveKeyFrame(TsTime time, GfInterval *intervalAffected = nullptr);
  ...
  TS_API
  std::optional<TsKeyFrame> GetClosestKeyFrame(TsTime targetTime) const;

  /// \brief Finds the closest keyframe before the given time.
  /// Returns an empty value if no such keyframe exists.
  TS_API
  std::optional<TsKeyFrame> GetClosestKeyFrameBefore(TsTime targetTime) const;

  /// \brief Finds the closest keyframe after the given time.
  /// Returns an empty value if no such keyframe exists.
  TS_API
  std::optional<TsKeyFrame> GetClosestKeyFrameAfter(TsTime targetTime) const;
  ...
  TS_API
  bool IsKeyFrameRedundant(TsTime keyFrameTime, const VtValue &defaultValue = VtValue()) const;
  ...
  /// frames is flat.
  TS_API
  bool IsSegmentFlat(const TsKeyFrame &kf1, const TsKeyFrame &kf2) const;
  ...
  ///
  TS_API
  bool IsSegmentValueMonotonic(const TsKeyFrame &kf1, const TsKeyFrame &kf2) const;
  ...
  TS_API
  bool IsSegmentValueMonotonic(TsTime startTime, TsTime endTime) const;
  ...
  /// value differences.
  TS_API
  bool IsVaryingSignificantly() const;

  /// Sets the spline's extrapolation type on each side.
  TS_API
  void SetExtrapolation(TsExtrapolationType left, TsExtrapolationType right);

  /// Returns the spline's extrapolation type on each side (\c first
  /// is the left side).
  TS_API
  std::pair<TsExtrapolationType, TsExtrapolationType> GetExtrapolation() const;
  ...
  TS_API
  VtValue Eval(TsTime time, TsSide side = TsRight) const;
  ...
  TS_API
  VtValue EvalDerivative(TsTime time, TsSide side = TsRight) const;
  ...
  TS_API
  TsSamples Sample(TsTime startTime,
                   TsTime endTime,
                   double timeScale,
                   double valueScale,
  ...
  TS_API
  std::pair<VtValue, VtValue> GetRange(TsTime startTime, TsTime endTime) const;
 ...
 private:
  ...
  void _GetBreakdown(TsKeyFrameMap *newKeyframes,
                     double x,
                     TsKnotType type,
                     bool flatTangents,
                     double tangentLength,
  ...
  void _BreakdownSamples(const _Samples &samples,
                         TsKnotType type,
                         bool flatTangents,
                         double tangentLength,
                         GfInterval *intervalAffected,
