...
template<typename T> class Ts_TypedData : public Ts_Data {
 public:
  typedef T ValueType;
  typedef Ts_TypedData<T> This;

  Ts_TypedData(const T &);
  Ts_TypedData(const TsTime &t,
               bool isDual,
               const T &leftValue,
               const T &rightValue,
  ...
  // given keyframe.
  std::shared_ptr<Ts_EvalCache<T, TsTraits<T>::interpolatable>> CreateTypedEvalCache(
 ...
 private:
  ...
  template<class V> struct _Values {

    explicit _Values(V const &lhv = TsTraits<T>::zero,
  ...
  class _ValuesHolder {
   private:
    ...
    struct _LocalStorage {
      _LocalStorage(_Values<T> &&values) : _data(std::move(values)) {}

      const _Values<T> &Get() const
      {
        return _data;
      }
      _Values<T> &GetMutable()
      {
        return _data;
      }
    ...
    struct _HeapStorage {
      _HeapStorage(_Values<T> &&values) : _data(new _Values<T>(std::move(values))) {}

      // Copy constructor: deep-copies data.
      _HeapStorage(const _HeapStorage &other) : _data(new _Values<T>(other.Get())) {}
    ...
    using _Storage = typename std::conditional<_isSmall, _LocalStorage, _HeapStorage>::type;

   public:
    // Construct from _Values rvalue.
    explicit _ValuesHolder(_Values<T> &&values) : _storage(std::move(values)) {}

    // Copy constructor.
    _ValuesHolder(const _ValuesHolder &other) : _storage(other._storage) {}
  ...
  static_assert(sizeof(_ValuesHolder) == sizeof(_Values<double>),
...
class Ts_PolymorphicDataHolder {
 public:
  ...
  template<typename T> void New(const T &val)
  {
  ...
  {
    new (&_storage)
        Ts_TypedData<T>(t, isDual, leftValue, rightValue, leftTangentSlope, rightTangentSlope);
  }

  // Copy constructor.
  template<typename T> void New(const Ts_TypedData<T> &other)
  {
 ...
 private:
  ...
  using _Storage =
      typename std::aligned_storage<sizeof(Ts_TypedData<double>), sizeof(void *)>::type;
...
Ts_TypedData<T>::Ts_TypedData(const TsTime &t,
                              ...
                              const T &leftTangentSlope,
                              const T &rightTangentSlope)
    : _values(_Values<T>(leftValue, rightValue, leftTangentSlope, rightTangentSlope)),
      _leftTangentLength(0.0),
      _rightTangentLength(0.0),
      _knotType(TsKnotHeld),
      _isDual(isDual),
...
template<typename T> void Ts_TypedData<T>::CloneInto(Ts_PolymorphicDataHolder *holder) const
{
  holder->New(*this);
}
...
std::shared_ptr<Ts_UntypedEvalCache> Ts_TypedData<T>::CreateEvalCache(Ts_Data const *kf2) const
{
  ...
  Ts_TypedData<T> const *typedKf2 = static_cast<Ts_TypedData<T> const *>(kf2);

  // Construct and return a new EvalCache of the appropriate type.
  return std::make_shared<Ts_EvalCache<T, TsTraits<T>::interpolatable>>(this, typedKf2);
}

template<typename T>
std::shared_ptr<Ts_EvalCache<T, TsTraits<T>::interpolatable>> Ts_TypedData<
    T>::CreateTypedEvalCache(Ts_Data const *kf2) const
{
  Ts_TypedData<T> const *typedKf2 = static_cast<Ts_TypedData<T> const *>(kf2);
...
template<typename T> VtValue Ts_TypedData<T>::EvalUncached(Ts_Data const *kf2, TsTime time) const
{
  // Cast kf2 to the correct typed data.  This is a private class, and we
  // assume kf2 is from the same spline, so it will have the same value type.
  Ts_TypedData<T> const *typedKf2 = static_cast<Ts_TypedData<T> const *>(kf2);

  return Ts_EvalCache<T, TsTraits<T>::interpolatable>(this, typedKf2).Eval(time);
}

template<typename T>
VtValue Ts_TypedData<T>::EvalDerivativeUncached(Ts_Data const *kf2, TsTime time) const
{
  // Cast kf2 to the correct typed data.  This is a private class, and we
  // assume kf2 is from the same spline, so it will have the same value type.
  Ts_TypedData<T> const *typedKf2 = static_cast<Ts_TypedData<T> const *>(kf2);

  return Ts_EvalCache<T, TsTraits<T>::interpolatable>(this, typedKf2).EvalDerivative(time);
}

template<typename T> bool Ts_TypedData<T>::operator==(const Ts_Data &rhs) const
{
  if (!TsTraits<T>::supportsTangents) {
    return GetKnotType() == rhs.GetKnotType() && GetTime() == rhs.GetTime() &&
           GetValue() == rhs.GetValue() && GetIsDualValued() == rhs.GetIsDualValued() &&
           (!GetIsDualValued() || (GetLeftValue() == rhs.GetLeftValue()));
  }

  return GetTime() == rhs.GetTime() && GetValue() == rhs.GetValue() &&
         GetKnotType() == rhs.GetKnotType() && GetIsDualValued() == rhs.GetIsDualValued() &&
         (!GetIsDualValued() || (GetLeftValue() == rhs.GetLeftValue())) &&
...
template<typename T> TsKnotType Ts_TypedData<T>::GetKnotType() const
{
  return _knotType;
}

template<typename T> void Ts_TypedData<T>::SetKnotType(TsKnotType knotType)
{
  std::string reason;
...
template<typename T>
bool Ts_TypedData<T>::CanSetKnotType(TsKnotType knotType, std::string *reason) const
{
...
template<typename T> VtValue Ts_TypedData<T>::GetValue() const
{
  return VtValue(_GetRightValue());
}

template<typename T> VtValue Ts_TypedData<T>::GetValueDerivative() const
{
  if (TsTraits<T>::supportsTangents) {
...
template<typename T> void Ts_TypedData<T>::SetValue(VtValue val)
{
  ...
  if (!v.IsEmpty()) {
    ...
    TF_CODING_ERROR(
        "cannot convert type '%s' to '%s' to assign "
        "to keyframe",
...
template<typename T> bool Ts_TypedData<T>::GetIsDualValued() const
{
  return _isDual;
}

template<typename T> void Ts_TypedData<T>::SetIsDualValued(bool isDual)
{
...
template<typename T> VtValue Ts_TypedData<T>::GetLeftValue() const
{
  return VtValue(_isDual ? _GetLeftValue() : _GetRightValue());
}

template<typename T> VtValue Ts_TypedData<T>::GetLeftValueDerivative() const
{
  if (TsTraits<T>::supportsTangents) {
...
template<typename T> void Ts_TypedData<T>::SetLeftValue(VtValue val)
{
  ...
  if (!v.IsEmpty()) {
    ...
    TF_CODING_ERROR(
        "cannot convert type '%s' to '%s' to assign to "
        "keyframe",
        val.GetTypeName().c_str(),
        ArchGetDemangled(typeid(ValueType)).c_str());
  }
...
template<typename T> VtValue Ts_TypedData<T>::GetZero() const
{
  return VtValue(TsTraits<T>::zero);
}

template<typename T> bool Ts_TypedData<T>::ValueCanBeInterpolated() const
{
  return TsTraits<T>::interpolatable;
}

template<typename T> bool Ts_TypedData<T>::ValueCanBeExtrapolated() const
{
  return TsTraits<T>::extrapolatable;
}

template<typename T> bool Ts_TypedData<T>::HasTangents() const
{
  return TsTraits<T>::supportsTangents && _knotType == TsKnotBezier;
}

template<typename T> bool Ts_TypedData<T>::ValueTypeSupportsTangents() const
{
...
template<typename T> VtValue Ts_TypedData<T>::GetLeftTangentSlope() const
{
  if (!TsTraits<T>::supportsTangents) {
...
template<typename T> VtValue Ts_TypedData<T>::GetRightTangentSlope() const
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> TsTime Ts_TypedData<T>::GetLeftTangentLength() const
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> TsTime Ts_TypedData<T>::GetRightTangentLength() const
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> void Ts_TypedData<T>::SetLeftTangentSlope(VtValue val)
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> void Ts_TypedData<T>::SetRightTangentSlope(VtValue val)
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> void Ts_TypedData<T>::SetLeftTangentLength(TsTime newLen)
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
  ...
  if (newLen < 0.0) {
    if (-newLen < TS_LENGTH_EPSILON) {
      ...
      TF_CODING_ERROR("Cannot set tangent length to negative value; ignoring");
      return;
    }
  }
...
template<typename T> void Ts_TypedData<T>::SetRightTangentLength(TsTime newLen)
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> bool Ts_TypedData<T>::GetTangentSymmetryBroken() const
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> void Ts_TypedData<T>::SetTangentSymmetryBroken(bool broken)
{
  if (!TsTraits<T>::supportsTangents) {
    TF_CODING_ERROR("keyframes of type '%s' do not have tangents",
                    ArchGetDemangled(typeid(ValueType)).c_str());
...
template<typename T> void Ts_TypedData<T>::ResetTangentSymmetryBroken()
{
...
template<> TS_API void Ts_TypedData<float>::ResetTangentSymmetryBroken();

template<> TS_API void Ts_TypedData<double>::ResetTangentSymmetryBroken();

template<> TS_API bool Ts_TypedData<float>::ValueCanBeInterpolated() const;

template<> TS_API bool Ts_TypedData<double>::ValueCanBeInterpolated() const;
