...
TsSpline::TsSpline() : _data(new TsSpline_KeyFrames()) {}
...
void TsSpline::_Detach()
{
  TfAutoMallocTag2 tag("Ts", "TsSpline::_Detach");

  if (!_data.unique()) {
    std::shared_ptr<TsSpline_KeyFrames> newData(new TsSpline_KeyFrames(*_data));
    _data.swap(newData);
  }
}

bool TsSpline ::ClearRedundantKeyFrames(const VtValue &defaultValue /* = VtValue() */,
                                        const GfMultiInterval &intervals /* = infinity */)
{
  ...
  bool needToTestIntervals = intervals !=
                             GfMultiInterval(GfInterval(-std::numeric_limits<double>::infinity(),
...
const TsKeyFrameMap &TsSpline::GetKeyFrames() const
{
  return _data->GetKeyFrames();
}

const TsKeyFrameMap &TsSpline::GetRawKeyFrames() const
{
  return _data->GetNormalKeyFrames();
}

std::vector<TsKeyFrame> TsSpline::GetKeyFramesInMultiInterval(
...
GfInterval TsSpline::GetFrameRange() const
{
  if (IsEmpty())
    return GfInterval();
  ...
  return GfInterval(keyframes.begin()->GetTime(), keyframes.rbegin()->GetTime());
}

void TsSpline::SwapKeyFrames(std::vector<TsKeyFrame> *swapInto)
{
  _Detach();
  _data->SwapKeyFrames(swapInto);
}

bool TsSpline::CanSetKeyFrame(const TsKeyFrame &kf, std::string *reason) const
{
  if (IsEmpty())
    return true;
...
bool TsSpline::KeyFrameIsInLoopedRange(const TsKeyFrame &kf)
{
  TsLoopParams loopParams = GetLoopParams();
  if (loopParams.GetLooping()) {
    GfInterval loopedInterval = loopParams.GetLoopedInterval();
    GfInterval masterInterval = loopParams.GetMasterInterval();
...
void TsSpline::SetKeyFrame(TsKeyFrame kf, GfInterval *intervalAffected)
{
...
void TsSpline::RemoveKeyFrame(TsTime time, GfInterval *intervalAffected)
{
  _Detach();
  _data->RemoveKeyFrame(time, intervalAffected);
}

std::optional<TsKeyFrame> TsSpline::Breakdown(double x,
                                              TsKnotType type,
                                              bool flatTangents,
...
void TsSpline::_GetBreakdown(TsKeyFrameMap *newKeyframes,
                             double x,
                             TsKnotType type,
                             bool flatTangents,
                             double tangentLength,
                             const VtValue &value) const
{
  ...
  if (keyframes.empty()) {
    ...
    (*newKeyframes)[x] = TsKeyFrame(
        x, kfValue, type, VtValue(), VtValue(), tangentLength, tangentLength);
    return;
  }
  ...
  if (type != TsKnotBezier || !keyframes.begin()->SupportsTangents()) {
  ...
  // shape
  (*newKeyframes)[x] = TsKeyFrame(x, kfValue, type, slope, slope, tangentLength, tangentLength);
...
void TsSpline::Breakdown(const std::set<double> &times,
                         TsKnotType type,
                         bool flatTangents,
                         double tangentLength,
                         const VtValue &value,
                         ...
                         TsKeyFrameMap *keyFramesAtTimes)
{
  ...
  _BreakdownMultipleValues(
      timesVec, type, flatTangents, tangentLength, values, intervalAffected, keyFramesAtTimes);
}

void TsSpline::Breakdown(const std::vector<double> &times,
                         TsKnotType type,
                         bool flatTangents,
                         double tangentLength,
...
{
  _BreakdownMultipleValues(
      times, type, flatTangents, tangentLength, values, intervalAffected, keyFramesAtTimes);
}
...
void TsSpline::Breakdown(const std::vector<double> &times,
                         ...
                         TsKeyFrameMap *keyFramesAtTimes)
{
  _BreakdownMultipleKnotTypes(
      times, types, flatTangents, tangentLength, values, intervalAffected, keyFramesAtTimes);
}

void TsSpline::_BreakdownMultipleValues(const std::vector<double> &times,
                                        TsKnotType type,
                                        bool flatTangents,
                                        double tangentLength,
                                        const std::vector<VtValue> &values,
...
void TsSpline::_BreakdownMultipleKnotTypes(const std::vector<double> &times,
                                           const std::vector<TsKnotType> &types,
                                           bool flatTangents,
                                           double tangentLength,
                                           const std::vector<VtValue> &values,
                                           ...
                                           TsKeyFrameMap *keyFramesAtTimes)
{
  ...
  if (samplesPerKnotType.count(TsKnotBezier) > 0) {
    _BreakdownSamples(samplesPerKnotType[TsKnotBezier],
                      TsKnotBezier,
                      flatTangents,
                      tangentLength,
                      intervalAffected,
  ...
  for (const auto &mapIt : samplesPerKnotType) {
    ...
    if (type != TsKnotBezier) {
      _BreakdownSamples(
          mapIt.second, type, flatTangents, tangentLength, intervalAffected, keyFramesAtTimes);
    }
  }
}

void TsSpline::_BreakdownSamples(const _Samples &samples,
                                 TsKnotType type,
                                 bool flatTangents,
                                 double tangentLength,
                                 GfInterval *intervalAffected,
                                 TsKeyFrameMap *keyFramesAtTimes)
{
  ...
  for (const auto &sample : samples) {
    GfInterval interval;
    std::optional<TsKeyFrame> kf = Breakdown(sample.first,
                                             type,
                                             flatTangents,
                                             tangentLength,
                                             sample.second,
...
void TsSpline::Clear()
{
  ...
  if (!_data.unique()) {
    ...
    std::shared_ptr<TsSpline_KeyFrames> newData(new TsSpline_KeyFrames(*_data, &empty));
    _data.swap(newData);
  }
  else {
...
std::optional<TsKeyFrame> TsSpline::GetClosestKeyFrame(TsTime targetTime) const
{
...
std::optional<TsKeyFrame> TsSpline::GetClosestKeyFrameBefore(TsTime targetTime) const
{
  const TsKeyFrame *k = Ts_GetClosestKeyFrameBefore(GetKeyFrames(), targetTime);
  if (k) {
    return *k;
  }
  return std::optional<TsKeyFrame>();
...
std::optional<TsKeyFrame> TsSpline::GetClosestKeyFrameAfter(TsTime targetTime) const
{
  const TsKeyFrame *k = Ts_GetClosestKeyFrameAfter(GetKeyFrames(), targetTime);
  if (k) {
    return *k;
  }
  return std::optional<TsKeyFrame>();
...
bool TsSpline::IsKeyFrameRedundant(const TsKeyFrame &keyFrame,
                                   const VtValue &defaultValue /* = VtValue() */) const
{
  return Ts_IsKeyFrameRedundant(GetKeyFrames(), keyFrame, GetLoopParams(), defaultValue);
}

bool TsSpline::IsKeyFrameRedundant(TsTime keyFrameTime,
                                   const VtValue &defaultValue /* = VtValue() */) const
{
  TsKeyFrameMap const &keyFrames = GetKeyFrames();
...
bool TsSpline::HasRedundantKeyFrames(const VtValue &defaultValue /* = VtValue() */) const
{
...
bool TsSpline::IsSegmentFlat(const TsKeyFrame &kf1, const TsKeyFrame &kf2) const
{
  return Ts_IsSegmentFlat(kf1, kf2);
}

bool TsSpline::IsSegmentFlat(TsTime startTime, TsTime endTime) const
{
  TsKeyFrameMap const &keyFrames = GetKeyFrames();
...
bool TsSpline::IsSegmentValueMonotonic(const TsKeyFrame &kf1, const TsKeyFrame &kf2) const
{
  return Ts_IsSegmentValueMonotonic(kf1, kf2);
}

bool TsSpline::IsSegmentValueMonotonic(TsTime startTime, TsTime endTime) const
{
  TsKeyFrameMap const &keyFrames = GetKeyFrames();
...
bool TsSpline::IsVarying() const
{
  return _IsVarying(/* tolerance */ 0.0);
}

bool TsSpline::IsVaryingSignificantly() const
{
  return _IsVarying(/* tolerance */ 1e-6);
}

bool TsSpline::_IsVarying(double tolerance) const
{
  const TsKeyFrameMap &keyFrames = GetKeyFrames();
  ...
  std::pair<TsExtrapolationType, TsExtrapolationType> extrap = GetExtrapolation();
  ...
  for (cur = keyFrames.begin(); cur != keyFrames.end(); ++cur) {
    ...
    if (curKeyFrame.HasTangents()) {
      ...
      bool checkLeft = isFirst ? (extrap.first != TsExtrapolationHeld) :
      ...
      bool checkRight = isLast ? (extrap.second != TsExtrapolationHeld) :
...
void TsSpline::SetExtrapolation(TsExtrapolationType left, TsExtrapolationType right)
{
  _Detach();
  _data->SetExtrapolation(TsExtrapolationPair(left, right));
}

std::pair<TsExtrapolationType, TsExtrapolationType> TsSpline::GetExtrapolation() const
{
  return _data->GetExtrapolation();
}

const std::type_info &TsSpline::GetTypeid() const
{
  TsKeyFrameMap const &keyframes = GetKeyFrames();
  if (keyframes.empty())
    return typeid(void);
...
std::string TsSpline::GetTypeName() const
{
  return ArchGetDemangled(GetTypeid());
}

VtValue TsSpline::Eval(TsTime time, TsSide side) const
{
...
static std::optional<TsKeyFrame> _FindHoldKey(const TsSpline &spline, TsTime time, TsSide side)
{
  if (spline.IsEmpty()) {
    return {};
  }
...
VtValue TsSpline::EvalHeld(TsTime time, TsSide side) const
{
  if (IsEmpty()) {
    return VtValue();
  }
...
VtValue TsSpline::EvalDerivative(TsTime time, TsSide side) const
{
...
bool TsSpline::DoSidesDiffer(const TsTime time) const
{
  ...
  if (kfIt->GetIsDualValued() && kfIt->GetLeftValue() != kfIt->GetValue()) {
    return true;
  }
  ...
  if (kfIt != keyFrames.begin()) {
    ...
    if (prevKfIt->GetKnotType() == TsKnotHeld && prevKfIt->GetValue() != kfIt->GetValue()) {
      return true;
    }
  }
...
TsSamples TsSpline::Sample(
    TsTime startTime, TsTime endTime, double timeScale, double valueScale, double tolerance) const
{
  return Ts_Sample(*this, startTime, endTime, timeScale, valueScale, tolerance);
}

std::pair<VtValue, VtValue> TsSpline::GetRange(TsTime startTime, TsTime endTime) const
{
  return Ts_GetRange(*this, startTime, endTime);
}

bool TsSpline::IsLinear() const
{
  ...
  TF_FOR_ALL(it, *this)
  {
    if (it->GetKnotType() != TsKnotLinear || !it->GetValue().IsHolding<double>() ||
        it->GetIsDualValued())
    {
      return false;
    }
...
void TsSpline::BakeSplineLoops()
{
  _Detach();
  _data->BakeSplineLoops();
}

void TsSpline::SetLoopParams(const TsLoopParams &params)
{
  _Detach();
  _data->SetLoopParams(params);
}

TsLoopParams TsSpline::GetLoopParams() const
{
  return _data->GetLoopParams();
}

bool TsSpline::IsTimeLooped(TsTime time) const
{
  const TsLoopParams &params = GetLoopParams();

  return params.GetLooping() && params.GetLoopedInterval().Contains(time) &&
         !params.GetMasterInterval().Contains(time);
}

TsSpline::const_iterator TsSpline::find(const TsTime &time) const
{
  return const_iterator(GetKeyFrames().find(time));
}

TsSpline::const_iterator TsSpline::lower_bound(const TsTime &time) const
{
  return const_iterator(GetKeyFrames().lower_bound(time));
}

TsSpline::const_iterator TsSpline::upper_bound(const TsTime &time) const
{
