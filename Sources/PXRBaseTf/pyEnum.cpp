...
string Tf_PyEnumRepr(object const &self) {
  ...
  return TfStringGetSuffix(moduleName) + "." +
         (baseName.empty() ? string() : baseName + ".") + name;
}
...
static bool _IsPythonKeyword(const std::string &name) {
  static const char *_PythonKeywords[] = {
      ...
      "else",  "except", "exec",  "finally",  "for",    "from",     "global",
  ...
  return std::binary_search(std::begin(_PythonKeywords),
                            std::end(_PythonKeywords), name);
}
...
void Tf_PyEnumAddAttribute(boost::python::scope &s, const std::string &name,
  ...
  if (PyObject_HasAttrString(s.ptr(), name.c_str())) {
    TF_CODING_ERROR("Ignoring enum value '%s'; an attribute with that "
