...
static std::vector<std::string> _GetCallStacks() {
  ...
  TF_FOR_ALL(stack, stacks) {
    TF_FOR_ALL(func, *stack) {
      ...
      if (name.empty()) {
        ArchGetAddressInfo(reinterpret_cast<void *>(*func), NULL, NULL, &name,
                           NULL);
        if (name.empty()) {
          name = "<unknown>";
        }
...
static void _Report(TfMallocTag::CallTree const &self,
...
static std::string _LogReport(TfMallocTag::CallTree const &self,
                              std::string const &rootName) {
  string tmpFile;
  ArchMakeTmpFile(std::string("callSiteReport") +
                      (rootName.empty() ? "" : "_") + rootName,
                  &tmpFile);
