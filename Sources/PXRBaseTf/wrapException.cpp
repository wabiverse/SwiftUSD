...
static void Translate(TfBaseException const &exc) {
  ...
  if (!throwStack.empty()) {
    ...
    if (throwStackMsg.size() > MaxFramesInMsg) {
      ...
      throwStackMsg.push_back(TfStringPrintf("... %zu more frame%s", additional,
                                             additional == 1 ? "" : "s"));
    }
  ...
  if (TfCallContext const &cc = exc.GetThrowContext()) {
    contextMsg = TfStringPrintf("%s at %s:%zu", cc.GetFunction(), cc.GetFile(),
                                cc.GetLine());
  }

  // Raise the Python exception.
  PyErr_Format(
      tfExceptionClass, "%s - %s%s%s%s", exc.what(),
  ...
  if (TF_VERIFY(cppExc)) {
    ...
    std::unique_ptr<std::exception_ptr> cppExecPtrPtr(
        new std::exception_ptr(cppExc));
