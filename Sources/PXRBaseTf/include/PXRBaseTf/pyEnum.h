...
private:
  ...
  private:
    ...
    template <typename U> static U _GetEnumValue(PyObject *src, U *) {
      return U(Tf_PyEnumRegistry::GetInstance()
                   ._objectsToEnums[src]
                   .GetValueAsInt());
    }
  ...
  template <class T> struct _EnumToPython {
    static PyObject *convert(T t) {
      return Tf_PyEnumRegistry ::GetInstance()._ConvertEnumToPython(TfEnum(t));
    }
  };
...
struct Tf_PyEnumWrapper : public Tf_PyEnum,
                          boost::totally_ordered<Tf_PyEnumWrapper> {
  ...
  friend TfEnum operator|(Tf_PyEnumWrapper const &lhs,
    ...
    if (lhs.value.IsA(rhs.value.GetType())) {
      return TfEnum(lhs.value.GetType(),
                    lhs.value.GetValueAsInt() | rhs.value.GetValueAsInt());
    }
...
template <typename T> struct Tf_TypedPyEnumWrapper : Tf_PyEnumWrapper {
  ...
  static boost::python::object GetValueFromName(const std::string &name) {
    ...
    return found ? boost::python::object(value) : boost::python::object();
  }
};
...
TF_API
void Tf_PyEnumAddAttribute(boost::python::scope &s, const std::string &name,
...
private:
  typedef boost::python::class_<Tf_TypedPyEnumWrapper<T>,
                                boost::python::bases<Tf_PyEnumWrapper>>
      _EnumPyClassType;
