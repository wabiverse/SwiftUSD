...
///
template <class Key, class Data, class HashFn,
          class EqualKey = std::equal_to<Key>, unsigned Threshold = 128>
...
private:
  ...
  class _IteratorBase : public boost::iterator_facade<
...
public:
  ...
  /// An iterator type for this map.  Note that it provides access to the
  /// This::value_type only.
  typedef _IteratorBase<const value_type, typename _Vector::const_iterator>
      const_iterator;
...
public:
  /// Ctor.
  ///
  explicit TfDenseHashMap(const HashFn &hashFn = HashFn(),
  ...
  TfDenseHashMap(const TfDenseHashMap &rhs)
      : HashFn(rhs), EqualKey(rhs), _vector(rhs._vector),
...
private:
  ...
  _Vector &_vec() { return _vector; }

  // Helper to access the hash functor.
  HashFn &_hash() { return *this; }

  // Helper to access the equality functor.
  EqualKey &_equ() { return *this; }

  // Helper to access the storage vector.
  const _Vector &_vec() const { return _vector; }

  // Helper to access the hash functor.
  const HashFn &_hash() const { return *this; }

  // Helper to access the equality functor.
  const EqualKey &_equ() const { return *this; }
