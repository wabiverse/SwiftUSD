...
template <typename WeakPtr = void> struct RefPtrFactory {
  template <typename FactoryResultPtr> struct apply {
    using WeakPtrType =
        std::conditional_t<std::is_same<WeakPtr, void>::value,
...
#define BOOST_PP_FILENAME_1 "PXRBaseTf/makePyConstructor.h"
...
TfMakePyConstructor(T *func, const std::string &doc = std::string()) {
  ...
  return Tf_MakePyConstructor::InitVisitor<Tf_MakePyConstructor::InitCtor<T>>(
      doc);
}
...
TfMakePyConstructorWithBackReference(T *func,
...
TfMakePyConstructorWithVarArgs(T *func,
                               const std::string &doc = std::string()) {
  // Instantiate to set static constructor pointer, then return the visitor.
...
template <typename T>
Tf_MakePyConstructor::NewVisitor<typename Tf_MakePyConstructor::NewCtor<T>>
TfMakePyNew(T *func, const std::string &doc = std::string()) {
  // Instantiate to set static constructor pointer, then return the visitor.
  Tf_MakePyConstructor::NewCtor<T> Ctor(func);
  return Tf_MakePyConstructor::NewVisitor<Tf_MakePyConstructor::NewCtor<T>>(
      doc);
}
...
template <typename T> struct Tf_PySequenceToListConverterRefPtrFactory {
  ...
  PyObject *operator()(T seq) const {
    ...
    typedef typename Tf_MakePyConstructor::RefPtrFactory<>::apply<
        typename SeqType::value_type>::type RefPtrFactory;

    boost::python::list l;
    for (typename SeqType::const_iterator i = seq.begin(); i != seq.end();
         ++i) {
...
#define VAR_SIGNATURE                                                          \
  R(BOOST_PP_ENUM_PARAMS(N, A) BOOST_PP_COMMA_IF(N) const bp::tuple &,         \
    const bp::dict &)
...
struct InitCtorWithVarArgs<VAR_SIGNATURE> : CtorBase<VAR_SIGNATURE> {
  ...
  static bp::object __init__(const bp::tuple &args, const bp::dict &kwargs) {
    ...
    Install<CLS>(
        ...
        Base::_func(BOOST_PP_ENUM(N, EXTRACT_REQ_ARG_A, args) BOOST_PP_COMMA_IF(
                        N) bp::tuple(args.slice(N + 1, numArgs)),
                    kwargs),
        m);
