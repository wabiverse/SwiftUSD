...
UsdAttribute UsdPhysicsCollisionGroup::CreateMergeGroupNameAttr(VtValue const &defaultValue,
...
bool UsdPhysicsCollisionGroup::CollisionGroupTable::IsCollisionEnabled(
    ...
    const UsdStage &stage)
{
  ...
  for (const UsdPhysicsCollisionGroup &groupA : allSceneGroups) {
    ...
    if (!invertedAttr.IsAuthored() || !invertedFilter) {
      ...
      std::set<unsigned int> requestedGroups(filteredGroupIndices.begin(),
                                             filteredGroupIndices.end());
  ...
  for (unsigned int iA = 0; iA < allSceneGroups.size(); iA++) {
    for (unsigned int iB = iA; iB < allSceneGroups.size(); iB++) {
      ...
      const unsigned int mergedTableIndex = _GetCollisionTableIndex(
          groupAId, groupBId, nextPrimId);
