...
GlfSimpleShadowArray::GlfSimpleShadowArray() : _framebuffer(0),
...
void GlfSimpleShadowArray::SetShadowMapResolutions(
...
void GlfSimpleShadowArray::_AllocTextures()
{
  ...
  for (GfVec2i const &size : _resolutions)
  {
    ...
    TF_DEBUG(GLF_DEBUG_SHADOW_TEXTURES).Msg("Created shadow map texture of size %dx%d "
                                            "(id %#x)\n",
                                            size[0], size[1], id);
  }
