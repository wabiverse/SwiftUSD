...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (index));

static mx::FileSearchPath _ComputeSearchPaths()
{
...
const mx::FileSearchPath &HdMtlxSearchPaths()
{
  static const mx::FileSearchPath searchPaths = _ComputeSearchPaths();
  return searchPaths;
}

static mx::DocumentPtr _ComputeStdLibraries()
{
...
const mx::DocumentPtr &HdMtlxStdLibraries()
{
...
static std::string _GetMxNodeString(mx::NodeDefPtr const &mxNodeDef)
{
  ...
  return mxNodeDef->hasNamespace() ? mxNodeDef->getNamespace() + ":" + mxNodeDef->getNodeString() :
...
static TfToken _GetMxNodeType(mx::DocumentPtr const &mxDoc, TfToken const &hdNodeType)
{
...
static bool _ContainsTexcoordNode(mx::NodeDefPtr const &mxNodeDef)
{
...
static std::string _GetInputType(mx::NodeDefPtr const &mxNodeDef, std::string const &mxInputName)
{
...
static mx::NodePtr _AddMaterialXNode(HdMaterialNetworkInterface *netInterface,
                                     ...
                                     HdMtlxTexturePrimvarData *mxHdData)
{
  ...
  // to the mxNode
  TfTokenVector hdNodeParamNames = netInterface->GetAuthoredNodeParameterNames(hdNodeName);
  for (TfToken const &paramName : hdNodeParamNames) {
    ...
    const HdMaterialNetworkInterface::NodeParamData paramData = netInterface->GetNodeParameterData(
        hdNodeName, paramName);
    ...
    const std::pair<std::string, bool> result = SdfPath::StripPrefixNamespace(
  ...
  if (mxNodeCategory == "texcoord" || _ContainsTexcoordNode(mxNodeDef)) {
    if (mxHdData) {
      ...
      if (std::find(hdNodeParamNames.begin(), hdNodeParamNames.end(), _tokens->index) ==
          hdNodeParamNames.end())
      {
        netInterface->SetNodeParameterValue(hdNodeName, _tokens->index, VtValue(0));
      }
...
static void _AddInput(HdMaterialNetworkInterface *netInterface,
                      ...
                      mx::InputPtr *mxInput)
{
  ...
  if (mxNextNode->isMultiOutputType()) {
    ...
    if (mxNextNodeDef) {
      mx::OutputPtr mxConnOutput = mxNextNodeDef->getOutput(conn.upstreamOutputName.GetString());
...
static void _AddNodeGraphOutput(HdMaterialNetworkInterface *netInterface,
                                HdMaterialNetworkInterface::InputConnection const &conn,
...
static void _GatherUpstreamNodes(HdMaterialNetworkInterface *netInterface,
                                 ...
                                 HdMtlxTexturePrimvarData *mxHdData)
{
  ...
  if (netInterface->GetNodeType(hdNodeName).IsEmpty()) {
    TF_WARN("Could not find the connected Node '%s'", hdConnection.upstreamNodeName.GetText());
    return;
  }
  ...
  mx::NodePtr mxCurrNode = _AddMaterialXNode(
      netInterface, hdNodeName, mxDoc, *mxNodeGraph, addedNodeNames, connectionName, mxHdData);

  if (!mxCurrNode) {
    return;
  }

  TfTokenVector hdConnectionNames = netInterface->GetNodeInputConnectionNames(hdNodeName);

  // Continue traversing the upsteam connections to create the mxNodeGraph
  for (TfToken connName : hdConnectionNames) {
    const auto inputConnections = netInterface->GetNodeInputConnection(hdNodeName, connName);
    for (const auto &currConnection : inputConnections) {
      // Gather the nodes uptream from the mxCurrNode
      _GatherUpstreamNodes(netInterface,
                           currConnection,
                           mxDoc,
                           mxNodeGraph,
                           addedNodeNames,
      ...
      if (!mxInput) {
        _AddInput(netInterface, currConnection, connName, mxDoc, mxCurrNode, mxNextNode, &mxInput);
      }
...
mx::DocumentPtr HdMtlxCreateMtlxDocumentFromHdNetwork(HdMaterialNetwork2 const &hdNetwork,
                                                      ...
                                                      HdMtlxTexturePrimvarData *mxHdData)
{
  ...
  HdMaterialNetwork2Interface netInterface(materialPath,
...
static void _AddParameterInputsToTerminalNode(HdMaterialNetworkInterface *netInterface,
                                              TfToken const &terminalNodeName,
                                              TfToken const &mxType,
                                              mx::NodePtr const &mxShaderNode)
{
  TfTokenVector paramNames = netInterface->GetAuthoredNodeParameterNames(terminalNodeName);
    ...
    HdMtlxTexturePrimvarData *mxHdData)
{
  ...
  for (TfToken const &cName : terminalNodeConnectionNames) {
    ...
    const auto inputConnections = netInterface->GetNodeInputConnection(terminalNodeName, cName);
    for (const auto &currConnection : inputConnections) {
      ...
      mx::OutputPtr mxOutput;
      _AddNodeGraphOutput(netInterface,
                          currConnection,
                          fullOutputName,
                          mxDoc,
                          mxNodeGraph,
      ...
      mx::InputPtr mxInput;
      _AddInput(
          netInterface, currConnection, cName, mxDoc, mxShaderNode, mxUpstreamNode, &mxInput);
      mxInput->setConnectedOutput(mxOutput);
    }
  }
}

MaterialX::DocumentPtr HdMtlxCreateMtlxDocumentFromHdMaterialNetworkInterface(
    HdMaterialNetworkInterface *netInterface,
    TfToken const &terminalNodeName,
    TfTokenVector const &terminalNodeConnectionNames,
    ...
    HdMtlxTexturePrimvarData *mxHdData)
{
  ...
  TfToken mxType = _GetMxNodeType(mxDoc, netInterface->GetNodeType(terminalNodeName));
  mx::NodePtr mxShaderNode = mxDoc->addNode(mxType.GetString(), "Surface", "surfaceshader");
  mx::NodePtr mxMaterial = mxDoc->addMaterialNode(mxDoc->createValidChildName(materialName),
                                                  mxShaderNode);
  ...
  _AddParameterInputsToTerminalNode(netInterface, terminalNodeName, mxType, mxShaderNode);
