...
static NcColorSpace _colorSpaces[] = {{_acescg,
                                       {0.713, 0.293},
                                       {0.165, 0.830},
                                       {0.128, 0.044},
                                       _WpACES,
...
static const char *_colorSpaceNames[] = {_acescg,
                                         _adobergb,
                                         _g18_ap1,
                                         _g18_rec709,
                                         _g22_ap1,
...
static void _NcInitColorSpace(NcColorSpace *cs)
{
  ...
  if (gamma == 1.f) {
    ...
    if (a <= 0.f) {
      ...
      cs->K0 = a / (gamma - 1.f);
      cs->phi = (a / expf(logf(gamma * a / (gamma + gamma * a - 1.f - a)) * gamma)) /
                (gamma - 1.f);
    }
  }
...
{
  NcM33f t = NcM33fMultiply(NcM3ffInvert(NcGetRGBToXYZMatrix(src)), NcGetXYZToRGBMatrix(dst));
  return t;
}
...
NcRGB NcTransformColor(const NcColorSpace *dst, const NcColorSpace *src, NcRGB rgb)
{
  ...
  NcM33f tx = NcM33fMultiply(NcGetRGBToXYZMatrix(src), NcGetXYZToRGBMatrix(dst));
...
void NcTransformColors(const NcColorSpace *dst, const NcColorSpace *src, NcRGB *rgb, size_t count)
{
  ...
  for (size_t i = start; i < count; i++) {
    NcRGB in = rgb[i];
    NcRGB out = {tx.m[0] * in.r + tx.m[1] * in.g + tx.m[2] * in.b,
...
void NcTransformColorsWithAlpha(const NcColorSpace *dst,
...
NcXYZ NcRGBToXYZ(const NcColorSpace *ct, NcRGB rgb)
{
  ...
  return (NcXYZ){m.m[0] * rgb.r + m.m[1] * rgb.g + m.m[2] * rgb.b,
...
NcRGB NcXYZToRGB(const NcColorSpace *ct, NcXYZ xyz)
{
  ...
  NcRGB rgb = {m.m[0] * xyz.x + m.m[1] * xyz.y + m.m[2] * xyz.z,
...
NcRGB NcYxyToRGB(const NcColorSpace *cs, NcYxy c)
{
  ...
  NcRGB ret = (NcRGB){
      sign_of(rgb.r) * rgb.r / maxc, sign_of(rgb.g) * rgb.g / maxc, sign_of(rgb.b) * rgb.b / maxc};
  return ret;
}
