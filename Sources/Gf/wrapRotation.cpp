...
static tuple _DecomposeRotation3(const GfMatrix4d &rot, const GfVec3d &TwAxis,
                                 const GfVec3d &FBAxis, const GfVec3d &LRAxis,
                                 double handedness, double thetaTwHint,
                                 double thetaFBHint, double thetaLRHint,
                                 bool useHint) {
  double angle[3] = {thetaTwHint, thetaFBHint, thetaLRHint};

  GfRotation::DecomposeRotation(
      rot, TwAxis, FBAxis, LRAxis, handedness, &(angle[0]), &(angle[1]),
      &(angle[2]), nullptr /* thetaSwHint */, useHint, nullptr /* swShift */);

  return make_tuple(angle[0], angle[1], angle[2]);
}

static tuple _DecomposeRotation(const GfMatrix4d &rot, const GfVec3d &TwAxis,
                                const GfVec3d &FBAxis, const GfVec3d &LRAxis,
                                double handedness, const object &thetaTwHint,
                                const object &thetaFBHint,
                                const object &thetaLRHint,
                                const object &thetaSwHint, bool useHint,
                                const object &swShiftIn) {
  double angle[4] = {
      thetaTwHint.ptr() != Py_None ? boost::python::extract<double>(thetaTwHint)
                                   : 0.0,
      thetaFBHint.ptr() != Py_None ? boost::python::extract<double>(thetaFBHint)
                                   : 0.0,
      thetaLRHint.ptr() != Py_None ? boost::python::extract<double>(thetaLRHint)
                                   : 0.0,
      thetaSwHint.ptr() != Py_None ? boost::python::extract<double>(thetaSwHint)
                                   : 0.0};
  double swShift = swShiftIn.ptr() != Py_None
                       ? boost::python::extract<double>(swShiftIn)
                       : 0.0;
  ...
  GfRotation::DecomposeRotation(
      ...
      thetaSwHint.ptr() != Py_None ? &(angle[3]) : nullptr, useHint,
...
static tuple
_MatchClosestEulerRotation(const double targetTw, const double targetFB,
                           const double targetLR, const double targetSw,
                           const object &thetaTw, const object &thetaFB,
                           const object &thetaLR, const object &thetaSw) {
  double angle[4] = {
      thetaTw.ptr() != Py_None ? boost::python::extract<double>(thetaTw) : 0.0,
      thetaFB.ptr() != Py_None ? boost::python::extract<double>(thetaFB) : 0.0,
      thetaLR.ptr() != Py_None ? boost::python::extract<double>(thetaLR) : 0.0,
      thetaSw.ptr() != Py_None ? boost::python::extract<double>(thetaSw) : 0.0};
...
static size_t __hash__(GfRotation const &self) { return TfHash()(self); }
...
void wrapRotation() {
  ...
  class_<This>("Rotation", "3-space rotation", init<>())
      ...
      .def("DecomposeRotation3", _DecomposeRotation3,
           (arg("rot"), arg("twAxis"), arg("fbAxis"), arg("lrAxis"),
            arg("handedness"), arg("thetaTwHint") = 0.0,
            arg("thetaFBHint") = 0.0, arg("thetaLRHint") = 0.0,
            arg("useHint") = false))
      .staticmethod("DecomposeRotation3")

      .def("DecomposeRotation", _DecomposeRotation,
           (arg("rot"), arg("twAxis"), arg("fbAxis"), arg("lrAxis"),
            arg("handedness"), arg("thetaTwHint"), arg("thetaFBHint"),
            arg("thetaLRHint"), arg("thetaSwHint") = object(),
            arg("useHint") = false, arg("swShift") = object()))
