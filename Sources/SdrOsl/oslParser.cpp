...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    ((arraySize, "arraySize"))((pageStr, "page"))((oslPageDelimiter, "."))((vstructMember, "vstructmember"))(sdrDefinitionName)
    ...
    const OSL::OSLQuery &query, const NdrNodeDiscoveryResult &discoveryResult) const
{
  ...
  for (size_t i = 0; i < nParams; ++i)
  {
    ...
    for (auto metaIt = metadata.cbegin(); metaIt != metadata.cend();)
    {
      ...
      {
        TF_DEBUG(NDR_PARSING).Msg("Ignoring bad 'arraySize' attribute on property [%s] "
                                  "on OSL shader [%s]",
  ...
  for (const OslParameter &metaParam : param->metadata)
  {
    ...
    if (entryName == _tokens->vstructMember)
    {
      ...
      if (!vstruct.empty())
      {
        ...
        {
          TF_DEBUG(NDR_PARSING).Msg("Bad virtual structure member in %s.%s:%s", discoveryResult.name.c_str(), param->name.string().c_str(), vstruct.c_str());
        }
      }
    }
