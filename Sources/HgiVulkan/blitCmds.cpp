...
static VkImageAspectFlags _GetImageAspectMaskForCopy(HgiTextureUsage textureUsage)
{
  ...
  VkImageAspectFlags aspectFlags = HgiVulkanConversions::GetImageAspectFlag(textureUsage);
...
void HgiVulkanBlitCmds::CopyTextureToBuffer(HgiTextureToBufferOp const &copyOp)
{
  ...
  HgiVulkanBuffer *dstBuffer = static_cast<HgiVulkanBuffer *>(copyOp.gpuDestinationBuffer.Get());
  if (!TF_VERIFY(dstBuffer && dstBuffer->GetVulkanBuffer(), "Invalid buffer")) {
    return;
  }
...
void HgiVulkanBlitCmds::CopyBufferToTexture(HgiBufferToTextureOp const &copyOp)
{
  ...
  HgiBufferHandle const &srcBufHandle = copyOp.gpuSourceBuffer;
  HgiVulkanBuffer *srcBuffer = static_cast<HgiVulkanBuffer *>(srcBufHandle.Get());
  if (!TF_VERIFY(srcBuffer && srcBuffer->GetVulkanBuffer(), "Invalid buffer")) {
    return;
  }

  HgiVulkanTexture *dstTexture = static_cast<HgiVulkanTexture *>(
      copyOp.gpuDestinationTexture.Get());
  if (!TF_VERIFY(dstTexture && dstTexture->GetImage(), "Invalid texture handle")) {
    return;
  }
  HgiTextureDesc const &texDesc = dstTexture->GetDescriptor();
  ...
  vkCmdCopyBufferToImage(_commandBuffer->GetVulkanCommandBuffer(),
                         srcBuffer->GetVulkanBuffer(),
                         dstTexture->GetImage(),
...
void HgiVulkanBlitCmds::GenerateMipMaps(HgiTextureHandle const &texture)
{
  ...
  VkFormat format = HgiVulkanConversions::GetFormat(desc.format, isDepthBuffer);
...
void HgiVulkanBlitCmds::FillBuffer(HgiBufferHandle const &buffer, uint8_t value)
{
  ...
  const uint32_t value32Bit = static_cast<uint32_t>(value) | static_cast<uint32_t>(value) << 8 |
                              static_cast<uint32_t>(value) << 16 |
                              static_cast<uint32_t>(value) << 24;

  vkCmdFillBuffer(_commandBuffer->GetVulkanCommandBuffer(),
                  buf->GetVulkanBuffer(),
                  0,
                  VK_WHOLE_SIZE,
                  value32Bit);
...
void HgiVulkanBlitCmds::InsertMemoryBarrier(HgiMemoryBarrier barrier)
{
  _CreateCommandBuffer();
