...
CameraUtilFraming::CameraUtilFraming(const GfRange2f &displayWindow,
    ...
    : displayWindow(displayWindow), dataWindow(dataWindow), pixelAspectRatio(pixelAspectRatio)
{
}
...
GfMatrix4d CameraUtilFraming::ApplyToProjectionMatrix(
    const GfMatrix4d &projectionMatrix, const CameraUtilConformWindowPolicy windowPolicy) const
{
...
//
static CameraUtilConformWindowPolicy _InvertPolicy(
...
GfRange2f CameraUtilFraming::ComputeFilmbackWindow(
    const float cameraAspectRatio, const CameraUtilConformWindowPolicy windowPolicy) const
{
  return GfRange2f(CameraUtilConformedWindow(
      displayWindow, _InvertPolicy(windowPolicy), _SafeDiv(cameraAspectRatio, pixelAspectRatio)));
}
