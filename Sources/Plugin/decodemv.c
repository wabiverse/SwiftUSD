...
static void read_cdef(AV1_COMMON *cm, aom_reader *r, MACROBLOCKD *const xd)
{
  ...
  if (mi_row_in_sb == 0 && mi_col_in_sb == 0) {
    xd->cdef_transmitted[0] = xd->cdef_transmitted[1] = xd->cdef_transmitted[2] =
  ...
  if (!xd->cdef_transmitted[index] && !skip_txfm) {
    ...
    const int grid_idx = get_mi_grid_idx(
        mi_params, xd->mi_row & first_block_mask, xd->mi_col & first_block_mask);
    MB_MODE_INFO *const mbmi = mi_params->mi_grid_base[grid_idx];
    mbmi->cdef_strength = aom_read_literal(r, cm->cdef_info.cdef_bits, ACCT_STR);
    xd->cdef_transmitted[index] = true;
  }
}

static int read_delta_qindex(AV1_COMMON *cm,
                             ...
                             MB_MODE_INFO *const mbmi)
{
  ...
  if ((bsize != cm->seq_params.sb_size || mbmi->skip_txfm == 0) && read_delta_q_flag) {
...
static int read_delta_lflevel(const AV1_COMMON *const cm,
                              aom_reader *r,
                              ...
                              int mi_row)
{
  ...
  if ((bsize != cm->seq_params.sb_size || mbmi->skip_txfm == 0) && read_delta_lf_flag) {
...
static uint8_t read_cfl_alphas(FRAME_CONTEXT *const ec_ctx, aom_reader *r, int8_t *signs_out)
{
  const int8_t joint_sign = aom_read_symbol(r, ec_ctx->cfl_sign_cdf, CFL_JOINT_SIGNS, "cfl:signs");
  uint8_t idx = 0;
...
static INTERINTRA_MODE read_interintra_mode(MACROBLOCKD *xd, aom_reader *r, int size_group)
{
  const INTERINTRA_MODE ii_mode = (INTERINTRA_MODE)aom_read_symbol(
      r, xd->tile_ctx->interintra_mode_cdf[size_group], INTERINTRA_MODES, ACCT_STR);
  return ii_mode;
}

static PREDICTION_MODE read_inter_mode(FRAME_CONTEXT *ec_ctx, aom_reader *r, int16_t ctx)
{
  ...
  is_zeromv = aom_read_symbol(r, ec_ctx->zeromv_cdf[mode_ctx], 2, ACCT_STR) == 0;
  if (is_zeromv)
    return GLOBALMV;
...
static MOTION_MODE read_motion_mode(AV1_COMMON *cm,
                                    MACROBLOCKD *xd,
                                    MB_MODE_INFO *mbmi,
                                    aom_reader *r)
{
  if (cm->features.switchable_motion_mode == 0)
  ...
  if (last_motion_mode_allowed == OBMC_CAUSAL) {
    motion_mode = aom_read_symbol(r, xd->tile_ctx->obmc_cdf[mbmi->bsize], 2, ACCT_STR);
    return (MOTION_MODE)(SIMPLE_TRANSLATION + motion_mode);
  }
  else {
    motion_mode = aom_read_symbol(
        r, xd->tile_ctx->motion_mode_cdf[mbmi->bsize], MOTION_MODES, ACCT_STR);
    return (MOTION_MODE)(SIMPLE_TRANSLATION + motion_mode);
  }
}

static PREDICTION_MODE read_inter_compound_mode(MACROBLOCKD *xd, aom_reader *r, int16_t ctx)
{
  const int mode = aom_read_symbol(
      r, xd->tile_ctx->inter_compound_mode_cdf[ctx], INTER_COMPOUND_MODES, ACCT_STR);
...
static int read_segment_id(AV1_COMMON *const cm,
                           ...
                           int skip)
{
  ...
  const int segment_id = av1_neg_deinterleave(coded_id, pred, seg->last_active_segid + 1);
...
static int dec_get_segment_id(
    const AV1_COMMON *cm, const uint8_t *segment_ids, int mi_offset, int x_mis, int y_mis)
{
  int segment_id = INT_MAX;

  for (int y = 0; y < y_mis; y++)
    for (int x = 0; x < x_mis; x++)
      segment_id = AOMMIN(segment_id, segment_ids[mi_offset + y * cm->mi_params.mi_cols + x]);

  assert(segment_id >= 0 && segment_id < MAX_SEGMENTS);
  return segment_id;
}

static void set_segment_id(AV1_COMMON *cm, int mi_offset, int x_mis, int y_mis, int segment_id)
{
  assert(segment_id >= 0 && segment_id < MAX_SEGMENTS);

  for (int y = 0; y < y_mis; y++)
    for (int x = 0; x < x_mis; x++)
      cm->cur_frame->seg_map[mi_offset + y * cm->mi_params.mi_cols + x] = segment_id;
}

static int read_intra_segment_id(
    AV1_COMMON *const cm, const MACROBLOCKD *const xd, int bsize, aom_reader *r, int skip)
{
  struct segmentation *const seg = &cm->seg;
  if (!seg->enabled)
  ...
  for (int y = 0; y < y_mis; y++)
    for (int x = 0; x < x_mis; x++)
      current_segment_ids[mi_offset + y * mi_params->mi_cols + x] =
          last_segment_ids ? last_segment_ids[mi_offset + y * mi_params->mi_cols + x] : 0;
}

static int get_predicted_segment_id(AV1_COMMON *const cm, int mi_offset, int x_mis, int y_mis)
{
...
static int read_inter_segment_id(AV1_COMMON *const cm,
                                 ...
                                 aom_reader *r)
{
  ...
  if (!seg->update_map) {
    copy_segment_id(
        mi_params, cm->last_frame_seg_map, cm->cur_frame->seg_map, mi_offset, x_mis, y_mis);
...
static int read_skip_mode(AV1_COMMON *cm, const MACROBLOCKD *xd, int segment_id, aom_reader *r)
{
  ...
  if (!is_comp_ref_allowed(xd->mi[0]->bsize))
    return 0;
  ...
  FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
  const int skip_mode = aom_read_symbol(r, ec_ctx->skip_mode_cdfs[ctx], 2, ACCT_STR);
  return skip_mode;
}

static int read_skip_txfm(AV1_COMMON *cm, const MACROBLOCKD *xd, int segment_id, aom_reader *r)
{
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP)) {
    return 1;
  }
    ...
    FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
    const int skip_txfm = aom_read_symbol(r, ec_ctx->skip_txfm_cdfs[ctx], 2, ACCT_STR);
    return skip_txfm;
  }
}
...
static void merge_colors(uint16_t *colors,
...
static void read_palette_colors_y(MACROBLOCKD *const xd,
                                  int bit_depth,
  ...
  if (idx < n) {
    ...
    if (idx < n) {
      ...
      for (; idx < n; ++idx) {
        ...
        pmi->palette_colors[idx] = clamp(
            pmi->palette_colors[idx - 1] + delta, 0, (1 << bit_depth) - 1);
...
static void read_palette_colors_uv(MACROBLOCKD *const xd,
                                   int bit_depth,
                                   PALETTE_MODE_INFO *const pmi,
                                   aom_reader *r)
{
  ...
  if (idx < n) {
    ...
    merge_colors(pmi->palette_colors + PALETTE_MAX_SIZE, cached_colors, n, n_cached_colors);
  }
  else {
    memcpy(pmi->palette_colors + PALETTE_MAX_SIZE, cached_colors, n * sizeof(cached_colors[0]));
  }
  ...
  if (aom_read_bit(r, ACCT_STR)) {  // Delta encoding.
    ...
    pmi->palette_colors[2 * PALETTE_MAX_SIZE] = aom_read_literal(r, bit_depth, ACCT_STR);
    for (int i = 1; i < n; ++i) {
      int delta = aom_read_literal(r, bits, ACCT_STR);
      if (delta && aom_read_bit(r, ACCT_STR))
        delta = -delta;
    ...
    for (int i = 0; i < n; ++i) {
      pmi->palette_colors[2 * PALETTE_MAX_SIZE + i] = aom_read_literal(r, bit_depth, ACCT_STR);
    }
  }
}

static void read_palette_mode_info(AV1_COMMON *const cm, MACROBLOCKD *const xd, aom_reader *r)
{
  ...
  if (mbmi->mode == DC_PRED) {
    ...
    const int modev = aom_read_symbol(
        r, xd->tile_ctx->palette_y_mode_cdf[bsize_ctx][palette_mode_ctx], 2, ACCT_STR);
    if (modev) {
      pmi->palette_size[0] = aom_read_symbol(r,
  ...
  if (num_planes > 1 && mbmi->uv_mode == UV_DC_PRED && xd->is_chroma_ref) {
    ...
    if (modev) {
      pmi->palette_size[1] = aom_read_symbol(r,
...
{
  MB_MODE_INFO *const mbmi = xd->mi[0];
  FILTER_INTRA_MODE_INFO *filter_intra_mode_info = &mbmi->filter_intra_mode_info;
                      ...
                      aom_reader *r)
{
  ...
  *tx_type = DCT_DCT;
  ...
  if (mbmi->skip_txfm || segfeature_active(&cm->seg, mbmi->segment_id, SEG_LVL_SKIP))
    return;
  ...
  if (get_ext_tx_types(tx_size, inter_block, cm->features.reduced_tx_set_used) > 1) {
    ...
    FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
    if (inter_block) {
      *tx_type = av1_ext_tx_inv[tx_set_type]
                               [aom_read_symbol(r,
      ...
      *tx_type = av1_ext_tx_inv[tx_set_type][aom_read_symbol(
          r,
          ec_ctx->intra_ext_tx_cdf[eset][square_tx_size][intra_mode],
          av1_num_ext_tx_set[tx_set_type],
          ACCT_STR)];
    }
  }
...
static INLINE void read_mv(
    aom_reader *r, MV *mv, const MV *ref, nmv_context *ctx, MvSubpelPrecision precision);

static INLINE int is_mv_valid(const MV *mv);

static INLINE int assign_dv(AV1_COMMON *cm,
                            MACROBLOCKD *xd,
                            int_mv *mv,
                            const int_mv *ref_mv,
                            int mi_row,
                            ...
                            aom_reader *r)
{
  ...
  int valid = is_mv_valid(&mv->as_mv) &&
              av1_is_dv_valid(
                  mv->as_mv, cm, xd, mi_row, mi_col, bsize, cm->seq_params.mib_size_log2);
  return valid;
}

static void read_intrabc_info(AV1_COMMON *const cm, DecoderCodingBlock *dcb, aom_reader *r)
{
  MACROBLOCKD *const xd = &dcb->xd;
  MB_MODE_INFO *const mbmi = xd->mi[0];
  FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
  ...
  if (mbmi->use_intrabc) {
    ...
    valid_dv = valid_dv &&
               assign_dv(cm, xd, &mbmi->mv[0], &dv_ref, xd->mi_row, xd->mi_col, bsize, r);
    if (!valid_dv) {
      // Intra bc motion vectors are not valid - signal corrupt frame
      aom_internal_error(xd->error_info, AOM_CODEC_CORRUPT_FRAME, "Invalid intrabc dv");
    }
  }
}
...
static void read_delta_q_params(AV1_COMMON *const cm, MACROBLOCKD *const xd, aom_reader *r)
{
  DeltaQInfo *const delta_q_info = &cm->delta_q_info;

  if (delta_q_info->delta_q_present_flag) {
    MB_MODE_INFO *const mbmi = xd->mi[0];
    xd->current_base_qindex += read_delta_qindex(cm, xd, r, mbmi) * delta_q_info->delta_q_res;
    ...
    if (delta_q_info->delta_lf_present_flag) {
      ...
      if (delta_q_info->delta_lf_multi) {
        const int frame_lf_count = av1_num_planes(cm) > 1 ? FRAME_LF_COUNT : FRAME_LF_COUNT - 2;
        for (int lf_id = 0; lf_id < frame_lf_count; ++lf_id) {
          const int tmp_lvl = xd->delta_lf[lf_id] +
                              read_delta_lflevel(
                                  cm, r, ec_ctx->delta_lf_multi_cdf[lf_id], mbmi, mi_col, mi_row) *
                                  delta_q_info->delta_lf_res;
          mbmi->delta_lf[lf_id] = xd->delta_lf[lf_id] = clamp(
              tmp_lvl, -MAX_LOOP_FILTER, MAX_LOOP_FILTER);
        }
      }
      else {
        const int tmp_lvl = xd->delta_lf_from_base +
                            read_delta_lflevel(cm, r, ec_ctx->delta_lf_cdf, mbmi, mi_col, mi_row) *
                                delta_q_info->delta_lf_res;
        mbmi->delta_lf_from_base = xd->delta_lf_from_base = clamp(
            tmp_lvl, -MAX_LOOP_FILTER, MAX_LOOP_FILTER);
      }
    }
  }
                                       ...
                                       aom_reader *r)
{
  ...
  xd->left_txfm_context = xd->left_txfm_context_buffer + (mi_row & MAX_MIB_MASK);
  ...
  mbmi->angle_delta[PLANE_TYPE_Y] = (use_angle_delta && av1_is_directional_mode(mbmi->mode)) ?
                                        read_angle_delta(
                                            r, ec_ctx->angle_delta_cdf[mbmi->mode - V_PRED]) :
                                        0;

  if (!cm->seq_params.monochrome && xd->is_chroma_ref) {
    mbmi->uv_mode = read_intra_mode_uv(ec_ctx, r, is_cfl_allowed(xd), mbmi->mode);
...
static int read_mv_component(aom_reader *r, nmv_component *mvcomp, int use_subpel, int usehp)
{
  int mag, d, fr, hp;
  const int sign = aom_read_symbol(r, mvcomp->sign_cdf, 2, ACCT_STR);
  const int mv_class = aom_read_symbol(r, mvcomp->classes_cdf, MV_CLASSES, ACCT_STR);
    ...
    aom_reader *r, MV *mv, const MV *ref, nmv_context *ctx, MvSubpelPrecision precision)
{
  ...
  const MV_JOINT_TYPE joint_type = (MV_JOINT_TYPE)aom_read_symbol(
      r, ctx->joints_cdf, MV_JOINTS, ACCT_STR);
...
#define READ_REF_BIT(pname) aom_read_symbol(r, av1_get_pred_cdf_##pname(xd), 2, ACCT_STR)

static COMP_REFERENCE_TYPE read_comp_reference_type(const MACROBLOCKD *xd, aom_reader *r)
{
...
static void set_ref_frames_for_skip_mode(AV1_COMMON *const cm, MV_REFERENCE_FRAME ref_frame[2])
{
...
static void read_ref_frames(AV1_COMMON *const cm,
                            MACROBLOCKD *const xd,
                            aom_reader *r,
                            int segment_id,
                            MV_REFERENCE_FRAME ref_frame[2])
{
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ref_frame[0] = (MV_REFERENCE_FRAME)get_segdata(&cm->seg, segment_id, SEG_LVL_REF_FRAME);
...
static INLINE int assign_mv(AV1_COMMON *cm,
                            MACROBLOCKD *xd,
...
static int read_is_inter_block(AV1_COMMON *const cm,
                               MACROBLOCKD *const xd,
                               int segment_id,
                               aom_reader *r)
{
  ...
  FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
  const int is_inter = aom_read_symbol(r, ec_ctx->intra_inter_cdf[ctx], 2, ACCT_STR);
  return is_inter;
}

#if DEC_MISMATCH_DEBUG
static void dec_dump_logs(
    AV1_COMMON *cm, MB_MODE_INFO *const mbmi, int mi_row, int mi_col, int16_t mode_ctx)
{
  int_mv mv[2] = {{0}};
                                       ...
                                       aom_reader *r)
{
  ...
  if (mbmi->skip_mode) {
        ...
        segfeature_active(&cm->seg, mbmi->segment_id, SEG_LVL_GLOBALMV))
    {
      ...
      const int mode_ctx = av1_mode_context_analyzer(inter_mode_ctx, mbmi->ref_frame);
      if (is_compound)
        mbmi->mode = read_inter_compound_mode(xd, r, mode_ctx);
      else
        mbmi->mode = read_inter_mode(ec_ctx, r, mode_ctx);
      if (mbmi->mode == NEWMV || mbmi->mode == NEW_NEWMV || have_nearmv_in_inter_mode(mbmi->mode))
  ...
  if (!is_compound && mbmi->mode != GLOBALMV) {
    av1_find_best_ref_mvs(allow_hp,
  ...
  if (is_compound && mbmi->mode != GLOBAL_GLOBALMV) {
    ...
    lower_mv_precision(&nearestmv[0].as_mv, allow_hp, features->cur_frame_force_integer_mv);
    ...
    nearmv[0] = xd->ref_mv_stack[mbmi->ref_frame[0]][1 + mbmi->ref_mv_idx].this_mv;
  }
  ...
  const int mv_corrupted_flag = !assign_mv(cm,
                                           xd,
                                           mbmi->mode,
                                           mbmi->ref_frame,
                                           mbmi->mv,
  ...
  if (cm->seq_params.enable_interintra_compound && !mbmi->skip_mode && is_interintra_allowed(mbmi))
  {
    const int bsize_group = size_group_lookup[bsize];
    const int interintra = aom_read_symbol(r, ec_ctx->interintra_cdf[bsize_group], 2, ACCT_STR);
    assert(mbmi->ref_frame[1] == NONE_FRAME);
    if (interintra) {
      const INTERINTRA_MODE interintra_mode = read_interintra_mode(xd, r, bsize_group);
  ...
  mbmi->motion_mode = SIMPLE_TRANSLATION;
  if (is_motion_variation_allowed_bsize(mbmi->bsize) && !mbmi->skip_mode && !has_second_ref(mbmi))
  {
  ...
  if (has_second_ref(mbmi) && !mbmi->skip_mode) {
    ...
    if (mbmi->comp_group_idx == 0) {
      if (cm->seq_params.order_hint_info.enable_dist_wtd_comp) {
        ...
        mbmi->interinter_comp.type = mbmi->compound_idx ? COMPOUND_AVERAGE : COMPOUND_DISTWTD;
      }
      else {
      ...
      if (is_interinter_compound_used(COMPOUND_WEDGE, bsize)) {
        mbmi->interinter_comp.type =
            COMPOUND_WEDGE +
            aom_read_symbol(r, ec_ctx->compound_type_cdf[bsize], MASKED_COMPOUND_TYPES, ACCT_STR);
      }
      else {
      ...
      if (mbmi->interinter_comp.type == COMPOUND_WEDGE) {
        ...
        mbmi->interinter_comp.mask_type = aom_read_literal(r, MAX_DIFFWTD_MASK_BITS, ACCT_STR);
      }
    }
  }

  read_mb_interp_filter(xd, features->interp_filter, cm->seq_params.enable_dual_filter, mbmi, r);
  ...
  if (mbmi->motion_mode == WARPED_CAUSAL) {
    ...
    if (av1_find_projection(mbmi->num_proj_ref,
                            pts,
                            pts_inref,
                            bsize,
                            mbmi->mv[0].as_mv.row,
                                       ...
                                       aom_reader *r)
{
  ...
  xd->above_txfm_context = cm->above_contexts.txfm[xd->tile.tile_row] + xd->mi_col;
  xd->left_txfm_context = xd->left_txfm_context_buffer + (xd->mi_row & MAX_MIB_MASK);
...
static void intra_copy_frame_mvs(
    AV1_COMMON *const cm, int mi_row, int mi_col, int x_mis, int y_mis)
{
  const int frame_mvs_stride = ROUND_POWER_OF_TWO(cm->mi_params.mi_cols, 1);
  MV_REF *frame_mvs = cm->cur_frame->mvs + (mi_row >> 1) * frame_mvs_stride + (mi_col >> 1);
...
void av1_read_mode_info(
    AV1Decoder *const pbi, DecoderCodingBlock *dcb, aom_reader *r, int x_mis, int y_mis)
{
  AV1_COMMON *const cm = &pbi->common;
  MACROBLOCKD *const xd = &dcb->xd;
