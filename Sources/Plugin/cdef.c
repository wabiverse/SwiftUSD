...
#include "Plugin/hioAvif/aom/av1/common/cdef_block.h"
#include "Plugin/hioAvif/aom/av1/common/reconinter.h"

static int is_8x8_block_skip(MB_MODE_INFO **grid, int mi_row, int mi_col, int mi_stride)
{
                             ...
                             BLOCK_SIZE bs)
{
  ...
  for (int r = 0; r < maxr; r += r_step) {
    for (int c = 0; c < maxc; c += c_step) {
      if (!is_8x8_block_skip(grid, mi_row + r, mi_col + c, mi_params->mi_stride)) {
...
static void copy_sb8_16(AV1_COMMON *cm,
                        uint16_t *dst,
                        int dstride,
                        const uint8_t *src,
                        int src_voffset,
                        ...
                        int hsize)
{
  if (cm->seq_params.use_highbitdepth) {
    const uint16_t *base = &CONVERT_TO_SHORTPTR(src)[src_voffset * sstride + src_hoffset];
...
static INLINE void fill_rect(uint16_t *dst, int dstride, int v, int h, uint16_t x)
{
  for (int i = 0; i < v; i++) {
    for (int j = 0; j < h; j++) {
      dst[i * dstride + j] = x;
...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm, MACROBLOCKD *xd)
{
  ...
  for (int fbr = 0; fbr < nvfb; fbr++) {
    for (int pli = 0; pli < num_planes; pli++) {
      const int block_height = (MI_SIZE_64X64 << mi_high_l2[pli]) + 2 * CDEF_VBORDER;
    ...
    for (int fbc = 0; fbc < nhfb; fbc++) {
      ...
      if (mi_params->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride +
          ...
          mi_params->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride + MI_SIZE_64X64 * fbc]
                  ->cdef_strength == -1)
      {
        cdef_left = 0;
        continue;
      ...
      const int mbmi_cdef_strength =
          mi_params->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride + MI_SIZE_64X64 * fbc]
              ->cdef_strength;
      level = cdef_info->cdef_strengths[mbmi_cdef_strength] / CDEF_SEC_STRENGTHS;
      sec_strength = cdef_info->cdef_strengths[mbmi_cdef_strength] % CDEF_SEC_STRENGTHS;
      sec_strength += sec_strength == 3;
      uv_level = cdef_info->cdef_uv_strengths[mbmi_cdef_strength] / CDEF_SEC_STRENGTHS;
      uv_sec_strength = cdef_info->cdef_uv_strengths[mbmi_cdef_strength] % CDEF_SEC_STRENGTHS;
      uv_sec_strength += uv_sec_strength == 3;
      if ((level == 0 && sec_strength == 0 && uv_level == 0 && uv_sec_strength == 0) ||
          (cdef_count = av1_cdef_compute_sb_list(
               mi_params, fbr * MI_SIZE_64X64, fbc * MI_SIZE_64X64, dlist, BLOCK_64X64)) == 0)
      {
        cdef_left = 0;
        continue;
      }
      ...
      for (int pli = 0; pli < num_planes; pli++) {
        ...
        if (cm->seq_params.use_highbitdepth) {
          ...
          av1_cdef_filter_fb(&xd->plane[pli].dst.buf[xd->plane[pli].dst.stride *
                                                         (MI_SIZE_64X64 * fbr << mi_high_l2[pli]) +
                                                     (fbc * MI_SIZE_64X64 << mi_wide_l2[pli])],
                             NULL,
                             xd->plane[pli].dst.stride,
