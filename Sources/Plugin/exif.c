...
avifResult avifGetExifOrientationOffset(const uint8_t *exif, size_t exifSize, size_t *offset)
{
  ...
  for (uint16_t field = 0; field < fieldCount; ++field) {  // for each field interoperability array
    ...
    // Orientation attribute according to JEITA CP-3451C section 4.6.4 (TIFF Rev. 6.0 Attribute
    // Information):
...
{
  const avifTransformFlags otherFlags = image->transformFlags &
                                        ~(AVIF_TRANSFORM_IROT | AVIF_TRANSFORM_IMIR);
  size_t offset;
  const avifResult result = avifGetExifOrientationOffset(
      image->exif.data, image->exif.size, &offset);
  if (result != AVIF_RESULT_OK) {
    return result;
  }
  if (offset < image->exif.size) {
    ...
    switch (orientation) {
      case 1:  // The 0th row is at the visual top of the image, and the 0th column is the visual
               // left-hand side.
        image->transformFlags = otherFlags;
        image->irot.angle = 0;  // ignored
        image->imir.axis = 0;   // ignored
        return AVIF_RESULT_OK;
      case 2:  // The 0th row is at the visual top of the image, and the 0th column is the visual
               // right-hand side.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IMIR;
        image->irot.angle = 0;  // ignored
        image->imir.axis = 1;
        return AVIF_RESULT_OK;
      case 3:  // The 0th row is at the visual bottom of the image, and the 0th column is the
               // visual right-hand side.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IROT;
        image->irot.angle = 2;
        image->imir.axis = 0;  // ignored
        return AVIF_RESULT_OK;
      case 4:  // The 0th row is at the visual bottom of the image, and the 0th column is the
               // visual left-hand side.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IMIR;
        image->irot.angle = 0;  // ignored
        image->imir.axis = 0;
        return AVIF_RESULT_OK;
      case 5:  // The 0th row is the visual left-hand side of the image, and the 0th column is the
               // visual top.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IROT | AVIF_TRANSFORM_IMIR;
        image->irot.angle = 1;  // applied before imir according to MIAF spec ISO/IEC 28002-12:2021
                                // - section 7.3.6.7
        image->imir.axis = 0;
        return AVIF_RESULT_OK;
      case 6:  // The 0th row is the visual right-hand side of the image, and the 0th column is the
               // visual top.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IROT;
        image->irot.angle = 3;
        image->imir.axis = 0;  // ignored
        return AVIF_RESULT_OK;
      case 7:  // The 0th row is the visual right-hand side of the image, and the 0th column is the
               // visual bottom.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IROT | AVIF_TRANSFORM_IMIR;
        image->irot.angle = 3;  // applied before imir according to MIAF spec ISO/IEC 28002-12:2021
                                // - section 7.3.6.7
        image->imir.axis = 0;
        return AVIF_RESULT_OK;
      case 8:  // The 0th row is the visual left-hand side of the image, and the 0th column is the
               // visual bottom.
        image->transformFlags = otherFlags | AVIF_TRANSFORM_IROT;
        image->irot.angle = 1;
        image->imir.axis = 0;  // ignored
  ...
  // The orientation tag is not mandatory (only recommended) according to JEITA CP-3451C
  // section 4.6.8.A. The default value is 1 if the orientation tag is missing, meaning:
  //   The 0th row is at the visual top of the image, and the 0th column is the visual left-hand
  //   side.
  image->transformFlags = otherFlags;
  image->irot.angle = 0;  // ignored
  image->imir.axis = 0;   // ignored
