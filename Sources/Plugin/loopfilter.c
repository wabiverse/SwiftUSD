...
filter_mask2(uint8_t limit, uint8_t blimit, uint8_t p1, uint8_t p0, uint8_t q0, uint8_t q1)
{
  int8_t mask = 0;
...
static INLINE int8_t filter_mask(uint8_t limit,
                                 uint8_t blimit,
                                 uint8_t p3,
                                 uint8_t p2,
                                 uint8_t p1,
...
static INLINE int8_t filter_mask3_chroma(uint8_t limit,
                                         uint8_t blimit,
                                         uint8_t p2,
                                         uint8_t p1,
                                         uint8_t p0,
...
static INLINE int8_t flat_mask3_chroma(
    uint8_t thresh, uint8_t p2, uint8_t p1, uint8_t p0, uint8_t q0, uint8_t q1, uint8_t q2)
{
  int8_t mask = 0;
...
static INLINE void filter4(
    int8_t mask, uint8_t thresh, uint8_t *op1, uint8_t *op0, uint8_t *oq0, uint8_t *oq1)
{
...
static INLINE void filter6(int8_t mask,
                           uint8_t thresh,
                           int8_t flat,
                           uint8_t *op2,
                           uint8_t *op1,
...
static INLINE void filter8(int8_t mask,
                           uint8_t thresh,
                           int8_t flat,
                           uint8_t *op3,
                           uint8_t *op2,
...
void aom_lpf_horizontal_6_c(
    uint8_t *s, int p, const uint8_t *blimit, const uint8_t *limit, const uint8_t *thresh)
{
  int i;
  int count = 4;
...
static INLINE void filter14(int8_t mask,
                            uint8_t thresh,
                            int8_t flat,
                            int8_t flat2,
                            uint8_t *op6,
                            ...
                            uint8_t *oq6)
{
  if (flat2 && flat && mask) {
    ...
    *op2 = ROUND_POWER_OF_TWO(p6 * 3 + p5 + p4 + p3 * 2 + p2 * 2 + p1 * 2 + p0 + q0 + q1 + q2 + q3,
                              4);
    ...
    *oq2 = ROUND_POWER_OF_TWO(p3 + p2 + p1 + p0 + q0 + q1 * 2 + q2 * 2 + q3 * 2 + q4 + q5 + q6 * 3,
                              4);
...
static void mb_lpf_horizontal_edge_w(uint8_t *s,
                                     int p,
                                     const uint8_t *blimit,
                                     const uint8_t *limit,
                                     const uint8_t *thresh,
                                     int count)
{
  ...
  for (i = 0; i < step * count; ++i) {
    const uint8_t p6 = s[-7 * p], p5 = s[-6 * p], p4 = s[-5 * p], p3 = s[-4 * p], p2 = s[-3 * p],
                  p1 = s[-2 * p], p0 = s[-p];
...
static void mb_lpf_vertical_edge_w(uint8_t *s,
                                   int p,
                                   const uint8_t *blimit,
                                   const uint8_t *limit,
                                   const uint8_t *thresh,
...
#if CONFIG_AV1_HIGHBITDEPTH
// Should we apply any filter at all: 11111111 yes, 00000000 no ?
static INLINE int8_t highbd_filter_mask2(
    uint8_t limit, uint8_t blimit, uint16_t p1, uint16_t p0, uint16_t q0, uint16_t q1, int bd)
{
  int8_t mask = 0;
...
// Should we apply any filter at all: 11111111 yes, 00000000 no ?
static INLINE int8_t highbd_filter_mask(uint8_t limit,
                                        uint8_t blimit,
                                        uint16_t p3,
                                        uint16_t p2,
                                        uint16_t p1,
...
static INLINE int8_t highbd_filter_mask3_chroma(uint8_t limit,
                                                uint8_t blimit,
                                                uint16_t p2,
                                                uint16_t p1,
                                                uint16_t p0,
...
static INLINE int8_t highbd_flat_mask3_chroma(uint8_t thresh,
                                              uint16_t p2,
                                              uint16_t p1,
                                              uint16_t p0,
                                              uint16_t q0,
...
highbd_hev_mask(uint8_t thresh, uint16_t p1, uint16_t p0, uint16_t q0, uint16_t q1, int bd)
{
  int16_t hev = 0;
  int16_t thresh16 = (uint16_t)thresh << (bd - 8);
...
static INLINE void highbd_filter4(int8_t mask,
                                  uint8_t thresh,
                                  uint16_t *op1,
                                  uint16_t *op0,
                                  uint16_t *oq0,
                                      ...
                                      int bd)
{
  ...
  aom_highbd_lpf_vertical_4_c(s + 4 * pitch, pitch, blimit1, limit1, thresh1, bd);
}

static INLINE void highbd_filter6(int8_t mask,
                                  uint8_t thresh,
                                  int8_t flat,
                                  uint16_t *op2,
                                  uint16_t *op1,
...
static INLINE void highbd_filter8(int8_t mask,
                                  uint8_t thresh,
                                  int8_t flat,
                                  uint16_t *op3,
                                  uint16_t *op2,
...
void aom_highbd_lpf_horizontal_8_c(
    uint16_t *s, int p, const uint8_t *blimit, const uint8_t *limit, const uint8_t *thresh, int bd)
{
  int i;
  int count = 4;
...
void aom_highbd_lpf_horizontal_6_c(
    uint16_t *s, int p, const uint8_t *blimit, const uint8_t *limit, const uint8_t *thresh, int bd)
{
  int i;
  int count = 4;
                                      ...
                                      int bd)
{
  ...
  aom_highbd_lpf_vertical_6_c(s + 4 * pitch, pitch, blimit1, limit1, thresh1, bd);
}
                                      ...
                                      int bd)
{
  ...
  aom_highbd_lpf_vertical_8_c(s + 4 * pitch, pitch, blimit1, limit1, thresh1, bd);
}

static INLINE void highbd_filter14(int8_t mask,
                                   uint8_t thresh,
                                   int8_t flat,
                                   int8_t flat2,
                                   uint16_t *op6,
...
static void highbd_mb_lpf_horizontal_edge_w(uint16_t *s,
                                            int p,
                                            const uint8_t *blimit,
                                            const uint8_t *limit,
                                            const uint8_t *thresh,
...
static void highbd_mb_lpf_vertical_edge_w(uint16_t *s,
                                          int p,
                                          const uint8_t *blimit,
                                          const uint8_t *limit,
                                          const uint8_t *thresh,
...
void aom_highbd_lpf_vertical_14_c(
    uint16_t *s, int p, const uint8_t *blimit, const uint8_t *limit, const uint8_t *thresh, int bd)
{
                                       ...
                                       int bd)
{
  ...
  highbd_mb_lpf_vertical_edge_w(s + 4 * pitch, pitch, blimit1, limit1, thresh1, 4, bd);
}
