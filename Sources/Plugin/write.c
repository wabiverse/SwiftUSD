...
static avifResult writeConfigBox(avifRWStream *s,
                                 const avifCodecConfigurationBox *cfg,
...
static void splitTilesLog2(
    uint32_t dim1, uint32_t dim2, int tilesLog2, int *tileDim1Log2, int *tileDim2Log2)
{
  assert(dim1 >= dim2);
...
void avifSetTileConfiguration(
    int threads, uint32_t width, uint32_t height, int *tileRowsLog2, int *tileColsLog2)
{
  *tileRowsLog2 = 0;
  *tileColsLog2 = 0;
  if (threads > 1) {
...
{
  avifCodecEncodeOutput *encodeOutput = (avifCodecEncodeOutput *)avifAlloc(
      sizeof(avifCodecEncodeOutput));
...
avifResult avifCodecEncodeOutputAddSample(avifCodecEncodeOutput *encodeOutput,
                                          const uint8_t *data,
                                          size_t len,
                                          avifBool sync)
{
...
typedef struct avifEncoderItem {
  ...
  uint32_t cellIndex;  // Which row-major cell index corresponds to this item. only present on
                       // image items
  ...
  // the reconstructed image of a grid item will be trimmed to these dimensions (only present on
  // grid items)
  uint32_t gridWidth;
  uint32_t gridHeight;

  uint32_t extraLayerCount;  // if non-zero (legal range [1-(AVIF_MAX_AV1_LAYER_COUNT-1)]), this is
                             // a layered AV1 image
...
typedef struct avifEncoderData {
  ...
  avifEncoderItemIdArray alternativeItemIDs;  // list of item ids for an 'altr' box (group of
                                              // alternatives to each other)
  avifBool singleImage;  // if true, the AVIF_ADD_IMAGE_FLAG_SINGLE flag was set on the first call
...
static avifEncoderItem *avifEncoderDataCreateItem(avifEncoderData *data,
                                                  const char *type,
                                                  const char *infeName,
                                                  size_t infeNameSize,
                                                  uint32_t cellIndex)
...
typedef struct avifItemPropertyDedup {
  ...
  avifRWStream s;  // Temporary stream for each new property, checked against already-written boxes
                   // for deduplications
...
// Resets the dedup's temporary write stream in preparation for a single item property's worth of
// writing
                                              ...
                                              avifBool essential)
{
  ...
  if (propertyIndex == 0) {
    // Write a new property, and remember its location in the output stream for future
    // deduplication
    ...
    property->index =
        ++dedup->nextIndex;  // preincrement so the first new index is 1 (as ipma is 1-indexed)
...
avifResult avifEncoderSetCodecSpecificOption(avifEncoder *encoder,
                                             const char *key,
                                             const char *value)
{
...
static avifBool avifEncoderDetectChanges(const avifEncoder *encoder,
                                         avifEncoderChanges *encoderChanges)
{
  ...
  *encoderChanges = 0;
  ...
  if ((lastEncoder->codecChoice != encoder->codecChoice) ||
      (lastEncoder->maxThreads != encoder->maxThreads) || (lastEncoder->speed != encoder->speed) ||
      (lastEncoder->keyframeInterval != encoder->keyframeInterval) ||
      (lastEncoder->timescale != encoder->timescale) ||
      (lastEncoder->repetitionCount != encoder->repetitionCount) ||
      (lastEncoder->extraLayerCount != encoder->extraLayerCount))
                                               ...
                                               avifItemPropertyDedup *dedup)
{
  ...
  AVIF_CHECKRES(avifRWStreamWriteU16(
      dedupStream, imageMetadata->colorPrimaries));  // unsigned int(16) colour_primaries;
  AVIF_CHECKRES(avifRWStreamWriteU16(
      dedupStream,
      imageMetadata->transferCharacteristics));  // unsigned int(16) transfer_characteristics;
  AVIF_CHECKRES(avifRWStreamWriteU16(
      dedupStream, imageMetadata->matrixCoefficients));  // unsigned int(16) matrix_coefficients;
  AVIF_CHECKRES(avifRWStreamWriteBits(dedupStream,
                                      (imageMetadata->yuvRange == AVIF_RANGE_FULL) ? 1 : 0,
                                      /*bitCount=*/1));  // unsigned int(1) full_range_flag;
  AVIF_CHECKRES(
...
//
// Track properties, however, are implicitly associated by the track in which they are contained,
// so there is no need to build a property association box (ipma), and no way to deduplicate/reuse
// a property. In this case, the ipma and dedup properties should/will be set to NULL, and this
                                                  ...
                                                  avifItemPropertyDedup *dedup)
{
  ...
  // HEIF 6.5.5.1, from Amendment 3 allows multiple colr boxes: "at most one for a given value of
  // colour type" Therefore, *always* writing an nclx box, even if an a prof box was already
  // written above.
  AVIF_CHECKRES(
      avifEncoderWriteNclxProperty(dedupStream, outputStream, imageMetadata, ipma, dedup));

  return avifEncoderWriteExtendedColorProperties(
      dedupStream, outputStream, imageMetadata, ipma, dedup);
}
                                                ...
                                                avifItemPropertyDedup *dedup)
{
  ...
  if (imageMetadata->clli.maxCLL || imageMetadata->clli.maxPALL) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteU16(
        dedupStream, imageMetadata->clli.maxCLL));  // unsigned int(16) max_content_light_level;
    AVIF_CHECKRES(avifRWStreamWriteU16(
        dedupStream,
        imageMetadata->clli.maxPALL));  // unsigned int(16) max_pic_average_light_level;
                                                          ...
                                                          avifItemPropertyDedup *dedup)
{
  ...
  if (imageMetadata->transformFlags & AVIF_TRANSFORM_PASP) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->pasp.hSpacing));  // unsigned int(32) hSpacing;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->pasp.vSpacing));  // unsigned int(32) vSpacing;
  ...
  if (imageMetadata->transformFlags & AVIF_TRANSFORM_CLAP) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.widthN));  // unsigned int(32) cleanApertureWidthN;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.widthD));  // unsigned int(32) cleanApertureWidthD;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.heightN));  // unsigned int(32) cleanApertureHeightN;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.heightD));  // unsigned int(32) cleanApertureHeightD;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.horizOffN));  // unsigned int(32) horizOffN;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.horizOffD));  // unsigned int(32) horizOffD;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.vertOffN));  // unsigned int(32) vertOffN;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        dedupStream, imageMetadata->clap.vertOffD));  // unsigned int(32) vertOffD;
  ...
  if (imageMetadata->transformFlags & AVIF_TRANSFORM_IROT) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteBits(
        dedupStream, imageMetadata->irot.angle & 0x3, /*bitCount=*/2));  // unsigned int (2) angle;
  ...
  if (imageMetadata->transformFlags & AVIF_TRANSFORM_IMIR) {
    ...
    AVIF_CHECKRES(
        avifRWStreamWriteBits(dedupStream, 0, /*bitCount=*/7));  // unsigned int(7) reserved = 0;
    AVIF_CHECKRES(avifRWStreamWriteBits(
        dedupStream, imageMetadata->imir.axis ? 1 : 0, /*bitCount=*/1));  // unsigned int(1) axis;
...
static avifResult avifEncoderWriteTrackMetaBox(avifEncoder *encoder, avifRWStream *s)
{
  ...
  for (uint32_t trakItemIndex = 0; trakItemIndex < encoder->data->items.count; ++trakItemIndex) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteU32(
        s, (uint32_t)item->metadataPayload.size));  // unsigned int(length_size*8) extent_length;
  }
  ...
  for (uint32_t trakItemIndex = 0; trakItemIndex < encoder->data->items.count; ++trakItemIndex) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteChars(
        s, item->infeName, item->infeNameSize));  // string item_name; (writing null terminator)
    if (item->infeContentType && item->infeContentTypeSize)
    {  // string content_type; (writing null terminator)
...
static avifResult avifWriteGridPayload(avifRWData *data,
                                       uint32_t gridCols,
                                       uint32_t gridRows,
                                       uint32_t gridWidth,
                                       uint32_t gridHeight)
                                                ...
                                                const avifGainMapMetadata *metadata)
{
  ...
  avifBool useCommonDenominator = metadata->baseHdrHeadroomD == denom &&
                                  metadata->alternateHdrHeadroomD == denom;
  for (int c = 0; c < channelCount; ++c) {
    useCommonDenominator = useCommonDenominator && metadata->gainMapMinD[c] == denom &&
...
static avifResult avifEncoderDataCreateExifItem(avifEncoderData *data, const avifRWData *exif)
{
  ...
  const avifResult result = avifGetExifTiffHeaderOffset(
      exif->data, exif->size, &exifTiffHeaderOffset);
  if (result != AVIF_RESULT_OK) {
...
// Same as avifImageCopy() but pads the dstImage with border pixel values to reach dstWidth and
// dstHeight. Returns NULL if a memory allocation failed.
static avifImage *avifImageCopyAndPad(const avifImage *srcImage,
                                      uint32_t dstWidth,
                                      uint32_t dstHeight)
{
  ...
  for (int plane = AVIF_CHAN_Y; plane <= AVIF_CHAN_A; ++plane) {
    ...
    const uint32_t srcPlaneHeight = avifImagePlaneHeight(
        srcImage, plane);  // 0 for A if no alpha and 0 for UV if 4:0:0.
    ...
    const uint32_t dstPlaneHeight = avifImagePlaneHeight(
        dstImage, plane);  // 0 for A if no alpha and 0 for UV if 4:0:0.
...
// Note: The topLevelItemID output argument has the type uint16_t* instead of avifEncoderItem**
// because
                                           ...
                                           uint16_t *topLevelItemID)
{
  ...
  if (cellCount > 1) {
    avifEncoderItem *gridItem = avifEncoderDataCreateItem(
        encoder->data, "grid", infeName, infeNameSize, 0);
    AVIF_CHECKRES(avifWriteGridPayload(
        &gridItem->metadataPayload, gridCols, gridRows, gridWidth, gridHeight));
  ...
  for (uint32_t cellIndex = 0; cellIndex < cellCount; ++cellIndex) {
    avifEncoderItem *item = avifEncoderDataCreateItem(
        encoder->data, encoder->data->imageItemType, infeName, infeNameSize, cellIndex);
...
// This function is called after every color frame is encoded. It returns AVIF_TRUE if a keyframe
// needs to be forced for the next alpha frame to be encoded, AVIF_FALSE otherwise.
                                                           ...
                                                           avifAddImageFlags addImageFlags)
{
  ...
  if (data->frames.count == 0) {
    // data->frames.count is the number of frames that have been encoded so far by previous calls
    // to avifEncoderAddImage. If this is the first frame, there is no need to force keyframe.
    return AVIF_FALSE;
  }
  ...
  if (isLaggedOutput) {
    // If the encoder is operating with lag, then there is no way to determine if the last encoded
    // frame was a keyframe until the encoder outputs it (after the lag). So do not force keyframe
    // for alpha channel in this case.
    return AVIF_FALSE;
  }
...
{
  return (itemCategory == AVIF_ITEM_ALPHA) ? AVIF_RESULT_ENCODE_ALPHA_FAILED :
...
static uint32_t avifGridWidth(uint32_t gridCols,
                              const avifImage *firstCell,
                              const avifImage *bottomRightCell)
{
  return (gridCols - 1) * firstCell->width + bottomRightCell->width;
}

static uint32_t avifGridHeight(uint32_t gridRows,
                               const avifImage *firstCell,
                               const avifImage *bottomRightCell)
{
                                   ...
                                   avifDiagnostics *diag)
{
  ...
  for (uint32_t cellIndex = 0; cellIndex < cellCount; ++cellIndex) {
...
#endif
    const uint32_t expectedCellWidth = ((cellIndex + 1) % gridCols) ? tileWidth :
                                                                      bottomRightCell->width;
    const uint32_t expectedCellHeight = (cellIndex < (cellCount - gridCols)) ?
                                            tileHeight :
    ...
    //   All input images of a grid image item shall use the same coding format, chroma sampling
    //   format, and the same decoder configuration (see 7.3.6.2).
    if ((cellImage->depth != firstCell->depth) || (cellImage->yuvFormat != firstCell->yuvFormat) ||
        ...
        (!!cellImage->alphaPlane != !!firstCell->alphaPlane) ||
        (cellImage->alphaPremultiplied != firstCell->alphaPremultiplied))
    {
      avifDiagnosticsPrintf(
          diag,
          "all grid cells should have the same value for: depth, yuvFormat, yuvRange, "
          "colorPrimaries, "
  ...
  if ((bottomRightCell->width > tileWidth) || (bottomRightCell->height > tileHeight)) {
    avifDiagnosticsPrintf(diag,
                          "the last %s cell can be smaller but not larger than the other cells "
                          "which are %dx%d, found %dx%d",
                          validateGainMap ? "gain map" : "image",
                          tileWidth,
                          tileHeight,
                          bottomRightCell->width,
  ...
  if ((cellCount > 1) &&
      !avifAreGridDimensionsValid(
          firstCell->yuvFormat, gridWidth, gridHeight, tileWidth, tileHeight, diag))
  {
    return AVIF_RESULT_INVALID_IMAGE_GRID;
  }
                                              ...
                                              avifAddImageFlags addImageFlags)
{
  ...
  if (encoder->extraLayerCount >= AVIF_MAX_AV1_LAYER_COUNT) {
    avifDiagnosticsPrintf(&encoder->diag,
                          "extraLayerCount [%u] must be less than %d",
  ...
  if (!firstCell->width || !firstCell->height || !bottomRightCell->width ||
      !bottomRightCell->height)
  {
    return AVIF_RESULT_NO_CONTENT;
  }

  AVIF_CHECKRES(avifValidateGrid(
      gridCols, gridRows, cellImages, /*validateGainMap=*/AVIF_FALSE, &encoder->diag));
  ...
  for (uint32_t cellIndex = 0; cellIndex < cellCount; ++cellIndex) {
    ...
    if (cellHasGainMap != hasGainMap) {
      avifDiagnosticsPrintf(
          &encoder->diag,
          "cells should either all have a gain map image, or none of them should, found a mix");
      return AVIF_RESULT_INVALID_IMAGE_GRID;
    }
    if (hasGainMap) {
  ...
  if (hasGainMap) {
    ...
    AVIF_CHECKRES(avifValidateGrid(
        gridCols, gridRows, cellImages, /*validateGainMap=*/AVIF_TRUE, &encoder->diag));
  }
#endif  // AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP
  ...
  encoder->data->quantizer = avifQualityToQuantizer(
      encoder->quality, encoder->minQuantizer, encoder->maxQuantizer);
  encoder->data->quantizerAlpha = avifQualityToQuantizer(
      encoder->qualityAlpha, encoder->minQuantizerAlpha, encoder->maxQuantizerAlpha);
#if defined(AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP)
  encoder->data->quantizerGainMap = avifQualityToQuantizer(
      encoder->qualityGainMap, AVIF_QUANTIZER_BEST_QUALITY, AVIF_QUANTIZER_WORST_QUALITY);
#endif
  ...
  if (encoder->data->items.count == 0) {
    ...
    const uint32_t gridWidth = avifGridWidth(gridCols, firstCell, bottomRightCell);
    const uint32_t gridHeight = avifGridHeight(gridRows, firstCell, bottomRightCell);
    AVIF_CHECKRES(avifEncoderAddImageItems(
        encoder, gridCols, gridRows, gridWidth, gridHeight, AVIF_ITEM_COLOR, &colorItemID));
    ...
    if (encoder->data->alphaPresent) {
      ...
      AVIF_CHECKRES(avifEncoderAddImageItems(
          encoder, gridCols, gridRows, gridWidth, gridHeight, AVIF_ITEM_ALPHA, &alphaItemID));
    ...
    if (firstCell->gainMap.image) {
      avifEncoderItem *toneMappedItem = avifEncoderDataCreateItem(
          encoder->data,
          "tmap",
          infeNameGainMap,
          /*infeNameSize=*/strlen(infeNameGainMap) + 1,
          /*cellIndex=*/0);
      if (!avifWriteToneMappedImagePayload(&toneMappedItem->metadataPayload,
                                           &firstCell->gainMap.metadata))
      {
        avifDiagnosticsPrintf(
            &encoder->diag,
            "failed to write gain map metadata, some values may be negative or too large");
        return AVIF_RESULT_ENCODE_GAIN_MAP_FAILED;
      }
      // Even though the 'tmap' item is related to the gain map, it shares most of its properties
      // (width/height, depth etc.) with the color item.
      ...
      *alternativeItemID = colorItemID;
      ...
      const uint32_t gainMapGridWidth = avifGridWidth(
          gridCols,
          cellImages[0]->gainMap.image,
          cellImages[gridCols * gridRows - 1]->gainMap.image);
      const uint32_t gainMapGridHeight = avifGridHeight(
          gridRows,
          cellImages[0]->gainMap.image,
          cellImages[gridCols * gridRows - 1]->gainMap.image);
      ...
      // dimg item references in item id order, so as long as colorItemID < gainMapItemID, the
    ...
    if (hasGainMap) {
      avifDiagnosticsPrintf(&encoder->diag,
                            "gain maps are not supported for image sequences or layered images");
      return AVIF_RESULT_NOT_IMPLEMENTED;
    }
#endif

    const avifImage *imageMetadata = encoder->data->imageMetadata;
    // Image metadata that are copied to the configuration property and nclx boxes are not allowed
    // to change. If the first image in the sequence had an alpha plane (even if fully opaque), all
    // subsequent images must have alpha as well.
    if ((imageMetadata->depth != firstCell->depth) ||
        (imageMetadata->yuvFormat != firstCell->yuvFormat) ||
  ...
  for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
    ...
    if (item->codec) {
      ...
      if ((cellImage->width != firstCellImage->width) ||
          (cellImage->height != firstCellImage->height))
      {
        paddedCellImage = avifImageCopyAndPad(
            cellImage, firstCellImage->width, firstCellImage->height);
        if (!paddedCellImage) {
          return AVIF_RESULT_OUT_OF_MEMORY;
        }
        cellImage = paddedCellImage;
      }
      const int quantizer = (item->itemCategory == AVIF_ITEM_ALPHA) ?
      ...
      // If alpha channel is present, set disableLaggedOutput to AVIF_TRUE. If the encoder supports
      // it, this enables avifEncoderDataShouldForceKeyframeForAlpha to force a keyframe in the
      // alpha channel whenever a keyframe has been encoded in the color channel for animated
      // images.
      avifResult encodeResult = item->codec->encodeImage(
          item->codec,
          encoder,
          cellImage,
      ...
      if (itemIndex == 0 &&
          avifEncoderDataShouldForceKeyframeForAlpha(encoder->data, item, addImageFlags))
      {
...
avifResult avifEncoderAddImage(avifEncoder *encoder,
...
static size_t avifEncoderFindExistingChunk(avifRWStream *s,
                                           size_t mdatStartOffset,
                                           const uint8_t *data,
                                           size_t size)
{
  ...
  for (uint32_t itemPasses = 0; itemPasses < 3; ++itemPasses) {
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
      ...
      if ((encoder->extraLayerCount > 0) && (item->encodeOutput->samples.count > 0)) {
        ...
        avifEncoderItemReference *ref = (item->itemCategory == AVIF_ITEM_ALPHA) ?
                                            avifArrayPush(layeredAlphaItems) :
      ...
      if (item->encodeOutput->samples.count > 0) {
        ...
        chunkOffset = avifEncoderFindExistingChunk(
            s, mdatStartOffset, sample->data.data, sample->data.size);
      }
      else {
        chunkOffset = avifEncoderFindExistingChunk(
            s, mdatStartOffset, item->metadataPayload.data, item->metadataPayload.size);
      }
      ...
      if (!chunkOffset) {
        ...
        if (item->encodeOutput->samples.count > 0) {
          for (uint32_t sampleIndex = 0; sampleIndex < item->encodeOutput->samples.count;
               ++sampleIndex)
          {
    ...
    do {
      ...
      for (uint32_t itemIndex = 0; itemIndex < layeredItemCount; ++itemIndex) {
        for (int samplePass = 0; samplePass < 2; ++samplePass) {
          ...
          avifEncoderItemReferenceArray *currentItems = (samplePass == 0) ? layeredAlphaItems :
                                                                            layeredColorItems;
          ...
          size_t chunkOffset = avifEncoderFindExistingChunk(
              s, mdatStartOffset, data->data, data->size);
          if (!chunkOffset) {
            // We've never seen this chunk before; write it out
            chunkOffset = avifRWStreamOffset(s);
...
static avifResult avifWriteAltrGroup(avifRWStream *s, const avifEncoderItemIdArray *itemIDs)
{
  ...
  AVIF_CHECKRES(avifRWStreamWriteU32(
      s, (uint32_t)itemIDs->count));  // unsigned int(32) num_entities_in_group;
  for (uint32_t i = 0; i < itemIDs->count; ++i) {
    AVIF_CHECKRES(
        avifRWStreamWriteU32(s, (uint32_t)itemIDs->item_id[i]));  // unsigned int(32) entity_id;
  }
...
{
  // The ExtendedMetaBox has no size and box type because these are already set by the
  // CondensedImageBox.
  ...
  AVIF_CHECKRES(avifRWStreamWriteBox(stream, "iprp", AVIF_BOX_SIZE_TBD, &iprp));
  {
    ...
    AVIF_CHECKRES(avifRWStreamWriteBox(stream, "ipco", AVIF_BOX_SIZE_TBD, &ipco));
    {
      ...
      AVIF_CHECKRES(avifEncoderWriteHDRProperties(
          stream, stream, imageMetadata, /*ipma=*/NULL, /*dedup=*/NULL));
      AVIF_CHECKRES(avifEncoderWriteExtendedColorProperties(
          stream, stream, imageMetadata, /*ipma=*/NULL, /*dedup=*/NULL));
    }
    ...
    AVIF_CHECKRES(avifRWStreamWriteFullBox(stream, "ipma", AVIF_BOX_SIZE_TBD, 0, 0, &ipma));
    {
      ...
      const uint8_t numNonEssentialProperties =
          ((imageMetadata->clli.maxCLL || imageMetadata->clli.maxPALL) ? 1 : 0) +
          ((imageMetadata->transformFlags & AVIF_TRANSFORM_PASP) ? 1 : 0);
      const uint8_t numEssentialProperties =
          ((imageMetadata->transformFlags & AVIF_TRANSFORM_CLAP) ? 1 : 0) +
      ...
      AVIF_CHECKRES(avifRWStreamWriteU32(stream, 1));  // unsigned int(32) entry_count;
      {
        ...
        for (uint8_t i = 0; i < ipmaCount; ++i) {
          AVIF_CHECKRES(avifRWStreamWriteBits(stream,
                                              (i >= numNonEssentialProperties) ? 1 : 0,
                                              /*bitCount=*/1));  // bit(1) essential;
          ...
          AVIF_CHECKRES(avifRWStreamWriteBits(
              stream, 9 + i, /*bitCount=*/7));  // unsigned int(7) property_index;
        }
      }
    }
...
// Returns true if the image can be encoded with a CondensedImageBox instead of a full regular
// MetaBox.
static avifBool avifEncoderIsCondensedImageBoxCompatible(const avifEncoder *encoder)
{
  ...
  for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
    ...
    if (!memcmp(item->type, "Exif", 4) && !memcmp(item->infeName, "Exif", item->infeNameSize)) {
      ...
      continue;  // Exif metadata can be stored in the CondensedImageBox if exif_tiff_header_offset
                 // is 0.
    }
...
static avifResult avifEncoderWriteCondensedImageBox(avifEncoder *encoder,
                                                    avifRWStream *s,
                                                    avifRWData *extendedMeta);

static avifResult avifEncoderWriteFileTypeBoxAndCondensedImageBox(avifEncoder *encoder,
                                                                  avifRWData *output)
{
  avifRWStream s;
                                                    ...
                                                    avifRWData *extendedMeta)
{
  ...
  const avifRWData *alphaData = alphaItem ? &alphaItem->encodeOutput->samples.sample[0].data :
                                            NULL;
  ...
  if (image->icc.size) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteBits(
        s, image->matrixCoefficients, 8));  // unsigned int(8) matrix_coefficients;
    AVIF_CHECKRES(
         ...
         (image->matrixCoefficients == AVIF_MATRIX_COEFFICIENTS_UNSPECIFIED)))
    {
      ...
      const avifConiColorType colorType = ((image->colorPrimaries >> 5 == 0) &&
                                           (image->transferCharacteristics >> 5 == 0) &&
      ...
      AVIF_CHECKRES(avifRWStreamWriteBits(
          s, image->colorPrimaries, numBitsPerComponent));  // unsigned int(5) color_primaries;
      AVIF_CHECKRES(avifRWStreamWriteBits(
          s,
          image->transferCharacteristics,
          numBitsPerComponent));  // unsigned int(5) transfer_characteristics;
      AVIF_CHECKRES(
          avifRWStreamWriteBits(s,
                                image->matrixCoefficients,
                                numBitsPerComponent));  // unsigned int(5) matrix_coefficients;
    }
  }
  ...
  AVIF_CHECKRES(avifRWStreamWriteVarInt(
      s, (uint32_t)colorData->size - 1));  // varint(32) main_item_data_size;

  AVIF_CHECKRES(avifRWStreamWriteBits(s, alphaItem ? 1 : 0, 1));  // unsigned int(1) has_alpha;
  if (alphaItem) {
    AVIF_CHECKRES(avifRWStreamWriteBits(
        s, image->alphaPremultiplied, 1));         // unsigned int(1) alpha_is_premultiplied;
    AVIF_CHECKRES(avifRWStreamWriteVarInt(s, 4));  // varint(32) alpha_item_codec_config_size;
    AVIF_CHECKRES(avifRWStreamWriteVarInt(
        s, (uint32_t)alphaData->size));  // varint(32) alpha_item_data_size;
  }
  ...
  AVIF_CHECKRES(avifRWStreamWriteBits(
      s, extendedMeta->size ? 1 : 0, 1));  // unsigned int(1) has_extended_meta;
  if (extendedMeta->size) {
    AVIF_CHECKRES(avifRWStreamWriteVarInt(
        s, (uint32_t)extendedMeta->size - 1));  // varint(32) extended_meta_size;
  }
                                              ...
                                              const avifImage *const imageMetadata)
{
  for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
    ...
    avifItemPropertyDedupStart(dedup);
    uint8_t channelCount = (item->itemCategory == AVIF_ITEM_ALPHA ||
                            (itemMetadata->yuvFormat == AVIF_PIXEL_FORMAT_YUV400)) ?
                               1 :
                               3;
    avifBoxMarker pixi;
    ...
    for (uint8_t chan = 0; chan < channelCount; ++chan) {
      AVIF_CHECKRES(avifRWStreamWriteU8(
          &dedup->s, (uint8_t)itemMetadata->depth));  // unsigned int (8) bits_per_channel;
    }
    ...
    if (item->itemCategory == AVIF_ITEM_ALPHA) {
      ...
      if (isToneMappedImage) {
        ...
        // Technically, the gain map is not an HDR image, but in the API, this is the most
        // convenient place to put this data.

        AVIF_CHECKRES(avifEncoderWriteHDRProperties(
            &dedup->s, s, imageMetadata->gainMap.image, &item->ipma, dedup));
#endif
      }
      else {
        AVIF_CHECKRES(
    ...
    if (item->extraLayerCount > 0) {
      ...
      AVIF_CHECKRES(avifRWStreamWriteBits(
          &dedup->s, largeSize ? 1 : 0, /*bitCount=*/1));  // unsigned int(1) large_size;
...
avifResult avifEncoderFinish(avifEncoder *encoder, avifRWData *output)
{
  ...
  for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
    ...
    if (item->codec) {
      ...
      if ((item->extraLayerCount > 0) &&
          (item->encodeOutput->samples.count != item->extraLayerCount + 1))
      {
        // Check whether user has sent enough frames to encoder.
        avifDiagnosticsPrintf(&encoder->diag,
                              "Expected %u frames given to avifEncoderAddImage() to encode this "
                              "layered image according to extraLayerCount, but got %u frames.",
  ...
  for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
    ...
    if (item->encodeOutput->samples.count > 0) {
      ...
      AVIF_CHECKERR(avifSequenceHeaderParse(
                        &sequenceHeader, (const avifROData *)&firstSample->data, codecType),
  ...
  if ((encoder->headerFormat == AVIF_HEADER_REDUCED) &&
      avifEncoderIsCondensedImageBoxCompatible(encoder))
  {
  ...
  //   If the primary item or all the items referenced by the primary item are AV1 image items made
  //   only of Intra Frames, the brand "avio" should be used in the compatible_brands field of the
  //   FileTypeBox.
  // See https://aomediacodec.github.io/av1-avif/v1.1.0.html#image-and-image-collection-brand.
  // This rule corresponds to using the "avio" brand in all cases except for layered images,
  // because:
  ...
  if (isSequence) {
    ...
    //   compatible_brands field of the FileTypeBox, the item constraints for this brand shall be
    //   met and at least one of the AV1 image sequences shall be made only of AV1 Samples marked
    //   as sync.
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
      ...
      if (onlySyncSamples) {
        useAvioBrand =
            AVIF_TRUE;  // at least one of the AV1 image sequences is made only of sync samples
        break;
      }
    }
  }
    ...
    // and the specification says the brand "should" be used, not "shall". Leverage that
    // opportunity to save four bytes for still images.
  ...
  AVIF_CHECKRES(avifRWStreamWriteBits(&s, 0, /*bitCount=*/4));  // unsigned int(4) reserved;
  AVIF_CHECKRES(avifRWStreamWriteU16(
      &s, (uint16_t)encoder->data->items.count));  // unsigned int(16) item_count;
  ...
  for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
    ...
    if (item->extraLayerCount > 0) {
      ...
      for (uint32_t i = 0; i < layerCount; ++i) {
        AVIF_CHECKRES(avifEncoderItemAddMdatFixup(item, &s));
        AVIF_CHECKRES(avifRWStreamWriteU32(
            &s, 0 /* set later */));  // unsigned int(offset_size*8) extent_offset;
        AVIF_CHECKRES(
            avifRWStreamWriteU32(&s,
                                 (uint32_t)item->encodeOutput->samples.sample[i]
                                     .data.size));  // unsigned int(length_size*8) extent_length;
      }
      continue;
    }
    ...
    AVIF_CHECKRES(avifRWStreamWriteU32(
        &s, 0 /* set later */));  // unsigned int(offset_size*8) extent_offset;
    AVIF_CHECKRES(avifRWStreamWriteU32(
        &s, (uint32_t)contentSize));  // unsigned int(length_size*8) extent_length;
  }
  ...
  AVIF_CHECKRES(avifRWStreamWriteFullBox(&s, "iinf", AVIF_BOX_SIZE_TBD, 0, 0, &iinf));
  AVIF_CHECKRES(avifRWStreamWriteU16(
      &s, (uint16_t)encoder->data->items.count));  //  unsigned int(16) entry_count;
  ...
  AVIF_CHECKRES(avifRWStreamWriteFullBox(&s, "ipma", AVIF_BOX_SIZE_TBD, 0, 0, &ipma));
  {
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
      ...
      for (int i = 0; i < item->ipma.count; ++i) {
        AVIF_CHECKRES(avifRWStreamWriteBits(
            &s, item->ipma.essential[i] ? 1 : 0, /*bitCount=*/1));  // bit(1) essential;
        AVIF_CHECKRES(avifRWStreamWriteBits(
            &s, item->ipma.associations[i], /*bitCount=*/7));  // unsigned int(7) property_index;
      }
    }
  }
  ...
  if (isSequence) {
    ...
    AVIF_CHECKRES(avifRWStreamWriteU32(
        &s, 0x00010000));  // template int(32) rate = 0x00010000; // typically 1.0
    AVIF_CHECKRES(avifRWStreamWriteU16(
        &s, 0x0100));  // template int(16) volume = 0x0100; // typically, full volume
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
      ...
      AVIF_CHECKRES(avifRWStreamWriteZeros(
          &s, sizeof(uint32_t) * 2));              // const unsigned int(32)[2] reserved = 0;
      ...
      AVIF_CHECKRES(avifRWStreamWriteU16(
          &s, 0));  // template int(16) volume = {if track_is_audio 0x0100 else 0};
      AVIF_CHECKRES(avifRWStreamWriteU16(&s, 0));  // const unsigned int(16) reserved = 0;
      AVIF_CHECKRES(avifRWStreamWrite(
          &s,
          unityMatrix,
          sizeof(unityMatrix)));  // template int(32)[9] matrix= // {
      ...
      AVIF_CHECKRES(avifRWStreamWriteU64(
          &s, framesDurationInTimescales));        // unsigned int(64) segment_duration;
      ...
      AVIF_CHECKRES(avifRWStreamWriteU16(
          &s, 21956));  // bit(1) pad = 0; unsigned int(5)[3] language; ("und")
      ...
      AVIF_CHECKRES(avifRWStreamWriteU32(&s, 0));  // unsigned int(32) pre_defined = 0;
      AVIF_CHECKRES(
          avifRWStreamWriteChars(&s,
                                 (item->itemCategory == AVIF_ITEM_ALPHA) ? "auxv" : "pict",
      ...
      AVIF_CHECKRES(avifRWStreamWriteU16(
          &s, 0));  // template unsigned int(16) graphicsmode = 0; (copy over the existing image)
      AVIF_CHECKRES(
      ...
      AVIF_CHECKRES(avifRWStreamWriteU32(&s, 1));  // unsigned int(32) entry_count;
      AVIF_CHECKRES(avifRWStreamWriteFullBox(
          &s, "url ", /*contentSize=*/0, 0, 1, /*marker=*/NULL));  // flags:1 means data is in this
                                                                   // file
      avifRWStreamFinishBox(&s, dref);
      avifRWStreamFinishBox(&s, dinf);

      // The boxes within the "stbl" box are ordered using the following recommendation in ISO/IEC
      // 14496-12, Section 6.2.3: 4) It is recommended that the boxes within the Sample Table Box
      // be in the following order: Sample Description (stsd), Time to Sample (stts), Sample to
      // Chunk (stsc), Sample Size (stsz), Chunk Offset (stco).
      //
      ...
      AVIF_CHECKRES(
          avifRWStreamWriteBox(&s, encoder->data->imageItemType, AVIF_BOX_SIZE_TBD, &imageItem));
      ...
      AVIF_CHECKRES(avifRWStreamWriteU16(&s, 0));    // const unsigned int(16) reserved = 0;
      AVIF_CHECKRES(avifRWStreamWriteZeros(
          &s, sizeof(uint32_t) * 3));  // unsigned int(32)[3] pre_defined = 0;
      AVIF_CHECKRES(
      ...
      for (uint32_t sampleCount = 0, frameIndex = 0; frameIndex < encoder->data->frames.count;
           ++frameIndex)
      {
        ...
        AVIF_CHECKRES(avifRWStreamWriteU32(
            &s, (uint32_t)frame->durationInTimescales));  // unsigned int(32) sample_delta;
        sampleCount = 0;
        ++sttsEntryCount;
      }
      size_t prevOffset = avifRWStreamOffset(&s);
      ...
      AVIF_CHECKRES(avifRWStreamWriteU32(
          &s, item->encodeOutput->samples.count));  // unsigned int(32) samples_per_chunk;
      ...
      AVIF_CHECKRES(avifRWStreamWriteU32(
          &s, item->encodeOutput->samples.count));  // unsigned int(32) sample_count;
      for (uint32_t sampleIndex = 0; sampleIndex < item->encodeOutput->samples.count;
           ++sampleIndex)
      {
        avifEncodeSample *sample = &item->encodeOutput->samples.sample[sampleIndex];
        AVIF_CHECKRES(avifRWStreamWriteU32(
            &s, (uint32_t)sample->data.size));  // unsigned int(32) entry_size;
      }
...
static avifResult writeCodecConfig(avifRWStream *s, const avifCodecConfigurationBox *cfg)
{
  ...
  AVIF_CHECKRES(avifRWStreamWriteBits(
      s, cfg->seqLevelIdx0, /*bitCount=*/5));  // unsigned int (5) seq_level_idx_0;
  ...
  AVIF_CHECKRES(avifRWStreamWriteBits(
      s, cfg->chromaSubsamplingY, /*bitCount=*/1));  // unsigned int (1) chroma_subsampling_y;
  AVIF_CHECKRES(avifRWStreamWriteBits(
      s, cfg->chromaSamplePosition, /*bitCount=*/2));  // unsigned int (2) chroma_sample_position;

  AVIF_CHECKRES(avifRWStreamWriteBits(s, 0, /*bitCount=*/3));  // unsigned int (3) reserved = 0;
  AVIF_CHECKRES(avifRWStreamWriteBits(
      s, 0, /*bitCount=*/1));  // unsigned int (1) initial_presentation_delay_present;
