...
#include "Plugin/hioAvif/aom/config/aom_version.h"
...
#include "Plugin/hioAvif/aom/aomdx.h"
#include "Plugin/hioAvif/aom/internal/aom_codec_internal.h"
#include "Plugin/hioAvif/aom/internal/aom_image_internal.h"
...
#include "Plugin/hioAvif/aom/av1/common/obu_util.h"

#include "Plugin/hioAvif/aom/av1/decoder/decodeframe.h"
#include "Plugin/hioAvif/aom/av1/decoder/decoder.h"
#include "Plugin/hioAvif/aom/av1/decoder/obu.h"

#include "Plugin/hioAvif/aom/av1/av1_iface_common.h"
...
static aom_codec_err_t decoder_init(aom_codec_ctx_t *ctx)
{
  ...
  if (!ctx->priv) {
    aom_codec_alg_priv_t *const priv = (aom_codec_alg_priv_t *)aom_calloc(1, sizeof(*priv));
    if (priv == NULL)
...
{
  const uint32_t num_units_in_display_tick = aom_rb_read_unsigned_literal(rb, 32);
...
static aom_codec_err_t parse_decoder_model_info(struct aom_read_bit_buffer *rb,
                                                int *buffer_delay_length_minus_1)
{
  *buffer_delay_length_minus_1 = aom_rb_read_literal(rb, 5);
  const uint32_t num_units_in_decoding_tick = aom_rb_read_unsigned_literal(rb, 32);
  const uint8_t buffer_removal_time_length_minus_1 = aom_rb_read_literal(rb, 5);
  const uint8_t frame_presentation_time_length_minus_1 = aom_rb_read_literal(rb, 5);
...
static aom_codec_err_t parse_op_parameters_info(struct aom_read_bit_buffer *rb,
                                                int buffer_delay_length_minus_1)
{
                                              ...
                                              aom_codec_stream_info_t *si)
{
  ...
  if (is_reduced_header) {
    ...
    if (timing_info_present_flag) {
      if ((status = parse_timing_info(rb)) != AOM_CODEC_OK)
        return status;
      decoder_model_info_present_flag = aom_rb_read_bit(rb);
      if (decoder_model_info_present_flag) {
        if ((status = parse_decoder_model_info(rb, &buffer_delay_length_minus_1)) != AOM_CODEC_OK)
          return status;
      }
    }
    const uint8_t initial_display_delay_present_flag = aom_rb_read_bit(rb);
    const uint8_t operating_points_cnt_minus_1 = aom_rb_read_literal(rb,
    ...
    for (int i = 0; i < operating_points_cnt_minus_1 + 1; i++) {
      ...
      if (i == 0)
        operating_point_idc0 = operating_point_idc;
      int seq_level_idx = aom_rb_read_literal(rb, LEVEL_BITS);  // level
      if (seq_level_idx > 7)
        aom_rb_read_bit(rb);  // tier
      if (decoder_model_info_present_flag) {
        const uint8_t decoder_model_present_for_this_op = aom_rb_read_bit(rb);
        if (decoder_model_present_for_this_op) {
          if ((status = parse_op_parameters_info(rb, buffer_delay_length_minus_1)) != AOM_CODEC_OK)
            return status;
        }
      }
      if (initial_display_delay_present_flag) {
        const uint8_t initial_display_delay_present_for_this_op = aom_rb_read_bit(rb);
  ...
  if (aom_get_num_layers_from_operating_point_idc(operating_point_idc0,
                                                ...
                                                int *is_intra_only)
{
  ...
  if (data + data_sz <= data || data_sz < 1)
  ...
  while (1) {
    ...
    if (obu_header.type == OBU_SEQUENCE_HEADER) {
      ...
      if (parse_operating_points(&rb, reduced_still_picture_hdr, si) != AOM_CODEC_OK) {
        return AOM_CODEC_ERROR;
      }
    ...
    else if (obu_header.type == OBU_FRAME_HEADER || obu_header.type == OBU_FRAME) {
    ...
    if (data_sz == 0)
      break;  // exit if we're out of OBUs
    status = aom_read_obu_header_and_size(
        data, data_sz, si->is_annexb, &obu_header, &payload_size, &bytes_read);
    if (status != AOM_CODEC_OK)
      return status;
  }
  if (got_sequence_header && found_keyframe)
    si->is_kf = 1;
...
static aom_codec_err_t decoder_get_si(aom_codec_alg_priv_t *ctx, aom_codec_stream_info_t *si)
{
...
static void set_error_detail(aom_codec_alg_priv_t *ctx, const char *const error)
{
  ctx->base.err_detail = error;
}

static aom_codec_err_t update_error_state(aom_codec_alg_priv_t *ctx,
...
static int frame_worker_hook(void *arg1, void *arg2)
{
  ...
  int result = av1_receive_compressed_data(
...
static INLINE void check_resync(aom_codec_alg_priv_t *const ctx, const AV1Decoder *const pbi)
{
  // Clear resync flag if worker got a key frame or intra only frame.
  if (ctx->need_resync == 1 && pbi->need_resync == 0 && frame_is_intra_only(&pbi->common))
    ctx->need_resync = 0;
}
                                  ...
                                  void *user_priv)
{
  ...
  if (!ctx->si.h) {
    ...
    const aom_codec_err_t res = decoder_peek_si_internal(*data, data_sz, &ctx->si, &is_intra_only);
    if (res != AOM_CODEC_OK)
      return res;
                                       ...
                                       void *user_priv)
{
  ...
  FrameWorkerData *const frame_worker_data = (FrameWorkerData *)ctx->frame_worker->data1;
  AV1Decoder *const pbi = frame_worker_data->pbi;
  AV1_COMMON *const cm = &pbi->common;
  ...
  for (int i = 0; i < REF_FRAMES; ++i)
    if (cm->ref_frame_map[i] == cm->cur_frame)
      data2->idx = i;
  data2->buf = data;
                                      ...
                                      void *user_priv)
{
  ...
  if (ctx->is_annexb) {
    ...
    if (aom_uleb_decode(data_start, data_sz, &temporal_unit_size, &length_of_size) != 0) {
      return AOM_CODEC_CORRUPT_FRAME;
    }
  ...
  while (data_start < data_end) {
    ...
    if (ctx->is_annexb) {
      ...
      size_t length_of_size;
      if (aom_uleb_decode(
              data_start, (size_t)(data_end - data_start), &frame_size, &length_of_size) != 0)
      {
        return AOM_CODEC_CORRUPT_FRAME;
      }
      data_start += length_of_size;
...
static void *AllocWithGetFrameBufferCb(void *priv, size_t size)
{
  ...
  if (param->pool->get_fb_cb(param->pool->cb_priv, size, param->fb) < 0)
    return NULL;
  if (param->fb->data == NULL || param->fb->size < size)
    return NULL;
                                        ...
                                        aom_film_grain_t *grain_params)
{
  ...
  param.pool = pool;
  param.fb = fb;
  if (!aom_img_alloc_with_cb(
          grain_img, img->fmt, w_even, h_even, 16, AllocWithGetFrameBufferCb, &param))
  {
    return NULL;
  }
...
static aom_image_t *decoder_get_frame(aom_codec_alg_priv_t *ctx, aom_codec_iter_t *iter)
{
  ...
  if (ctx->frame_worker != NULL) {
    ...
    if (winterface->sync(worker)) {
      ...
      aom_film_grain_t *grain_params;
      if (av1_get_raw_frame(frame_worker_data->pbi, *index, &sd, &grain_params) == 0) {
        ...
        if (pbi->ext_tile_debug && tiles->single_tile_decoding && pbi->dec_tile_row >= 0) {
          ...
          if (num_planes > 1) {
            for (plane = 1; plane < MAX_MB_PLANE; ++plane) {
              ctx->img.planes[plane] += mi_row * (MI_SIZE >> ssy) * ctx->img.stride[plane];
            }
          }
          ctx->img.d_h = AOMMIN(tile_height, cm->mi_params.mi_rows - mi_row) * MI_SIZE;
        }

        if (pbi->ext_tile_debug && tiles->single_tile_decoding && pbi->dec_tile_col >= 0) {
          int tile_width, tile_height;
          av1_get_uniform_tile_size(cm, &tile_width, &tile_height);
        ...
        if (pbi->skip_film_grain)
          grain_params->apply_grain = 0;
        aom_image_t *res = add_grain_if_needed(ctx, img, &ctx->image_with_grain, grain_params);
        if (!res) {
          aom_internal_error(
              &pbi->common.error, AOM_CODEC_CORRUPT_FRAME, "Grain systhesis failed\n");
        }
        *index += 1;  // Advance the iterator to point to the next image
        return res;
      }
...
static aom_codec_err_t decoder_set_fb_fn(aom_codec_alg_priv_t *ctx,
...
static aom_codec_err_t ctrl_set_reference(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (data) {
    ...
    return av1_set_reference_dec(
        &frame_worker_data->pbi->common, frame->idx, frame->use_external_ref, &sd);
  }
  else {
    return AOM_CODEC_INVALID_PARAM;
  }
...
static aom_codec_err_t ctrl_copy_reference(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_reference(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_new_frame_image(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_copy_new_frame_image(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (img) {
    ...
    if (av1_get_frame_to_show(frame_worker_data->pbi, &new_frame) == 0) {
      ...
      return av1_copy_new_frame_dec(&frame_worker_data->pbi->common, &new_frame, &sd);
    }
    else {
      return AOM_CODEC_ERROR;
    }
...
static aom_codec_err_t ctrl_get_last_ref_updates(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (update_info) {
    if (ctx->frame_worker) {
      AVxWorker *const worker = ctx->frame_worker;
      FrameWorkerData *const frame_worker_data = (FrameWorkerData *)worker->data1;
      *update_info = frame_worker_data->pbi->common.current_frame.refresh_frame_flags;
      return AOM_CODEC_OK;
    }
    else {
      return AOM_CODEC_ERROR;
...
static aom_codec_err_t ctrl_get_last_quantizer(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (ctx->frame_worker == NULL)
    return AOM_CODEC_ERROR;
  *arg = ((FrameWorkerData *)ctx->frame_worker->data1)->pbi->common.quant_params.base_qindex;
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_get_fwd_kf_value(aom_codec_alg_priv_t *ctx, va_list args)
{
  int *const arg = va_arg(args, int *);
  if (arg == NULL)
    return AOM_CODEC_INVALID_PARAM;
...
static aom_codec_err_t ctrl_get_altref_present(aom_codec_alg_priv_t *ctx, va_list args)
{
  int *const arg = va_arg(args, int *);
  if (arg == NULL)
    return AOM_CODEC_INVALID_PARAM;
  if (ctx->frame_worker == NULL)
    return AOM_CODEC_ERROR;
  *arg = ((FrameWorkerData *)ctx->frame_worker->data1)->pbi->is_arf_frame_present;
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_get_frame_flags(aom_codec_alg_priv_t *ctx, va_list args)
{
  int *const arg = va_arg(args, int *);
  if (arg == NULL)
    return AOM_CODEC_INVALID_PARAM;
...
static aom_codec_err_t ctrl_get_tile_info(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (tile_info) {
    if (ctx->frame_worker) {
      ...
      for (int tile_col = 1; tile_col <= tile_cols; tile_col++) {
        tile_info->tile_widths[tile_col - 1] = tiles->col_start_sb[tile_col] -
                                               tiles->col_start_sb[tile_col - 1];
      }

      for (int tile_row = 1; tile_row <= tile_rows; tile_row++) {
        tile_info->tile_heights[tile_row - 1] = tiles->row_start_sb[tile_row] -
                                                tiles->row_start_sb[tile_row - 1];
      }
...
static aom_codec_err_t ctrl_get_screen_content_tools_info(aom_codec_alg_priv_t *ctx, va_list args)
{
  aom_screen_content_tools_info *const sc_info = va_arg(args, aom_screen_content_tools_info *);
  if (sc_info) {
    if (ctx->frame_worker) {
      AVxWorker *const worker = ctx->frame_worker;
      FrameWorkerData *const frame_worker_data = (FrameWorkerData *)worker->data1;
      const AV1Decoder *pbi = frame_worker_data->pbi;
      sc_info->allow_screen_content_tools = pbi->common.features.allow_screen_content_tools;
...
static aom_codec_err_t ctrl_get_still_picture(aom_codec_alg_priv_t *ctx, va_list args)
{
  aom_still_picture_info *const still_picture_info = va_arg(args, aom_still_picture_info *);
  if (still_picture_info) {
    if (ctx->frame_worker) {
      AVxWorker *const worker = ctx->frame_worker;
      FrameWorkerData *const frame_worker_data = (FrameWorkerData *)worker->data1;
      const AV1Decoder *pbi = frame_worker_data->pbi;
      still_picture_info->is_still_picture = (int)pbi->common.seq_params.still_picture;
...
static aom_codec_err_t ctrl_get_sb_size(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_show_existing_frame_flag(aom_codec_alg_priv_t *ctx, va_list args)
{
  int *const arg = va_arg(args, int *);
  if (arg == NULL)
    return AOM_CODEC_INVALID_PARAM;
  if (ctx->frame_worker == NULL)
    return AOM_CODEC_ERROR;
  *arg = ((FrameWorkerData *)ctx->frame_worker->data1)->pbi->common.show_existing_frame;
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_get_s_frame_info(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (s_frame_info) {
    if (ctx->frame_worker) {
      ...
      s_frame_info->is_s_frame_at_altref = pbi->sframe_info.is_s_frame_at_altref;
      return AOM_CODEC_OK;
    }
    else {
      return AOM_CODEC_ERROR;
...
static aom_codec_err_t ctrl_get_frame_corrupted(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_frame_size(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_frame_header_info(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_tile_data(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (tile_data) {
    if (ctx->frame_worker) {
      ...
      tile_data->coded_tile_data = pbi->tile_buffers[pbi->dec_tile_row][pbi->dec_tile_col].data;
      return AOM_CODEC_OK;
    }
    else {
      return AOM_CODEC_ERROR;
...
static aom_codec_err_t ctrl_set_ext_ref_ptr(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_render_size(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_bit_depth(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_img_fmt_t get_img_format(int subsampling_x, int subsampling_y, int use_highbitdepth)
{
  aom_img_fmt_t fmt = 0;
...
static aom_codec_err_t ctrl_get_img_format(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_tile_size(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_tile_count(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_set_invert_tile_order(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->invert_tile_order = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_byte_alignment(aom_codec_alg_priv_t *ctx, va_list args)
{
  ...
  if (byte_alignment != legacy_byte_alignment &&
      (byte_alignment < min_byte_alignment || byte_alignment > max_byte_alignment ||
...
static aom_codec_err_t ctrl_set_skip_loop_filter(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_set_skip_film_grain(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_get_accounting(aom_codec_alg_priv_t *ctx, va_list args)
{
...
static aom_codec_err_t ctrl_set_decode_tile_row(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->decode_tile_row = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_decode_tile_col(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->decode_tile_col = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_tile_mode(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->tile_mode = va_arg(args, unsigned int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_is_annexb(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->is_annexb = va_arg(args, unsigned int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_operating_point(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->operating_point = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_output_all_layers(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->output_all_layers = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_inspection_callback(aom_codec_alg_priv_t *ctx, va_list args)
{
#if !CONFIG_INSPECTION
  (void)ctx;
  (void)args;
...
static aom_codec_err_t ctrl_ext_tile_debug(aom_codec_alg_priv_t *ctx, va_list args)
{
  ctx->ext_tile_debug = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_row_mt(aom_codec_alg_priv_t *ctx, va_list args)
{
...
aom_codec_iface_t aom_codec_av1_dx_algo = {
    ...
    AOM_CODEC_CAP_DECODER | AOM_CODEC_CAP_EXTERNAL_FRAME_BUFFER,  // aom_codec_caps_t
