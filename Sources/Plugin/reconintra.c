...
static const uint8_t *get_has_tr_table(PARTITION_TYPE partition, BLOCK_SIZE bsize)
{
...
static int has_top_right(const AV1_COMMON *cm,
                         BLOCK_SIZE bsize,
                         int mi_row,
                         int mi_col,
                         int top_available,
                         ...
                         int ss_y)
{
  ...
  if (row_off > 0) {  // Just need to check if enough pixels on the right.
    ...
    const int this_blk_index = ((blk_row_in_sb + 0) << (MAX_MIB_SIZE_LOG2 - bw_in_mi_log2)) +
                               blk_col_in_sb + 0;
...
static const uint8_t *get_has_bl_table(PARTITION_TYPE partition, BLOCK_SIZE bsize)
{
  const uint8_t *ret = NULL;
  // If this is a mixed vertical partition, look up bsize in orders_vert.
  if (partition == PARTITION_VERT_A || partition == PARTITION_VERT_B) {
...
static int has_bottom_left(const AV1_COMMON *cm,
                           BLOCK_SIZE bsize,
                           int mi_row,
                           int mi_col,
  ...
  if (col_off > 0) {
    ...
    if (blk_col_in_sb == 0) {
      const int blk_start_row_off = blk_row_in_sb
                                        << (bh_in_mi_log2 + MI_SIZE_LOG2 - MI_SIZE_LOG2) >>
                                    ss_y;
...
typedef void (*intra_pred_fn)(uint8_t *dst,
                              ptrdiff_t stride,
                              const uint8_t *above,
                              const uint8_t *left);
...
typedef void (*intra_high_pred_fn)(
...
static void dr_predictor(uint8_t *dst,
                         ptrdiff_t stride,
                         TX_SIZE tx_size,
                         const uint8_t *above,
                         const uint8_t *left,
                         ...
                         int angle)
{
  ...
  if (angle > 0 && angle < 90) {
    av1_dr_prediction_z1(dst, stride, bw, bh, above, left, upsample_above, dx, dy);
  }
  else if (angle > 90 && angle < 180) {
    av1_dr_prediction_z2(dst, stride, bw, bh, above, left, upsample_above, upsample_left, dx, dy);
  }
...
static void highbd_dr_predictor(uint16_t *dst,
                                ptrdiff_t stride,
                                TX_SIZE tx_size,
                                const uint16_t *above,
                                const uint16_t *left,
                                ...
                                int bd)
{
  ...
  if (angle > 0 && angle < 90) {
    av1_highbd_dr_prediction_z1(dst, stride, bw, bh, above, left, upsample_above, dx, dy, bd);
  }
  else if (angle > 90 && angle < 180) {
    ...
    av1_highbd_dr_prediction_z3(dst, stride, bw, bh, above, left, upsample_left, dx, dy, bd);
  }
  else if (angle == 90) {
...
DECLARE_ALIGNED(16, const int8_t, av1_filter_intra_taps[FILTER_INTRA_MODES][8][8]) = {
    {
        {-6, 10, 0, 0, 0, 12, 0, 0},
        {-5, 2, 10, 0, 0, 9, 0, 0},
        {-3, 1, 1, 10, 0, 7, 0, 0},
    ...
    for (c = 1; c < bw + 1; c += 4) {
      ...
      for (int k = 0; k < 8; ++k) {
        ...
        buffer[r + r_offset][c + c_offset] = clip_pixel(
            ROUND_POWER_OF_TWO(pr, FILTER_INTRA_SCALE_BITS));
      }
    }
...
#if CONFIG_AV1_HIGHBITDEPTH
static void highbd_filter_intra_predictor(uint16_t *dst,
                                          ptrdiff_t stride,
                                          TX_SIZE tx_size,
                                          const uint16_t *above,
                                          const uint16_t *left,
...
static void filter_intra_edge_corner(uint8_t *p_above, uint8_t *p_left)
{
  ...
  int s = (p_left[0] * kernel[0]) + (p_above[-1] * kernel[1]) + (p_above[0] * kernel[2]);
  s = (s + 8) >> 4;
  p_above[-1] = s;
  p_left[-1] = s;
}
                                        ...
                                        int plane)
{
  ...
  if (need_above) {
    ...
    if (n_top_px > 0) {
      ...
      if (i < num_top_pixels_needed)
        aom_memset16(&above_row[i], above_row[i - 1], num_top_pixels_needed - i);
    }
    else if (n_left_px > 0) {
  ...
  if (use_filter_intra) {
    highbd_filter_intra_predictor(
        dst, dst_stride, tx_size, above_row, left_col, filter_intra_mode, xd->bd);
    return;
  }
  ...
  if (is_dr_mode) {
    ...
    if (!disable_edge_filter) {
      ...
      upsample_above = av1_use_intra_edge_upsample(txwpx, txhpx, p_angle - 90, filt_type);
      if (need_above && upsample_above) {
        const int n_px = txwpx + (need_right ? txhpx : 0);
        av1_upsample_intra_edge_high(above_row, n_px, xd->bd);
      }
      upsample_left = av1_use_intra_edge_upsample(txhpx, txwpx, p_angle - 180, filt_type);
...
static void build_intra_predictors(const MACROBLOCKD *xd,
                                   const uint8_t *ref,
                                   int ref_stride,
                                   uint8_t *dst,
                                   int dst_stride,
...
static INLINE BLOCK_SIZE scale_chroma_bsize(BLOCK_SIZE bsize, int subsampling_x, int subsampling_y)
{
...
void av1_predict_intra_block(const AV1_COMMON *cm,
                             ...
                             int plane)
{
  ...
  if (use_palette) {
    ...
    const uint16_t *const palette = mbmi->palette_mode_info.palette_colors +
  ...
  const int bottom_available = (yd > 0) &&
                               (mi_row + ((row_off + txh) << ss_y) < xd->tile.mi_row_end);
  ...
  const int have_top_right = has_top_right(cm,
                                           bsize,
                                           mi_row,
                                           mi_col,
                                           have_top,
                                           ...
                                           ss_x,
                                           ss_y);
  const int have_bottom_left = has_bottom_left(cm,
                                               bsize,
                                               mi_row,
                                               mi_col,
                                               bottom_available,
...
void av1_predict_intra_block_facade(
    const AV1_COMMON *cm, MACROBLOCKD *xd, int plane, int blk_col, int blk_row, TX_SIZE tx_size)
{
  const MB_MODE_INFO *const mbmi = xd->mi[0];
  ...
  const FILTER_INTRA_MODE filter_intra_mode = (plane == AOM_PLANE_Y &&
  ...
  if (plane != AOM_PLANE_Y && mbmi->uv_mode == UV_CFL_PRED) {
    ...
    const BLOCK_SIZE plane_bsize = get_plane_block_size(
        mbmi->bsize, pd->subsampling_x, pd->subsampling_y);
    (void)plane_bsize;
