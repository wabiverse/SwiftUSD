...
extern "C" avifResult avifIsCompliant(const uint8_t *data, size_t size)
{
  ...
  parseFunc(&topReader);
  // gpac/ComplianceWarden will print the formatted result page to stdout, warnings and errors
  // inclusive.
  AVIF_CHECKERR(!checkComplianceStd(topReader.myBox, topReader.specs[0]),
