...
#include "Plugin/hioAvif/aom/av1/common/av1_loopfilter.h"
...
static void loop_filter_alloc(
    AV1LfSync *lf_sync, AV1_COMMON *cm, int rows, int width, int num_workers)
{
  lf_sync->rows = rows;
#if CONFIG_MULTITHREAD
  {
    ...
    for (j = 0; j < MAX_MB_PLANE; j++) {
      CHECK_MEM_ERROR(cm, lf_sync->mutex_[j], aom_malloc(sizeof(*(lf_sync->mutex_[j])) * rows));
      if (lf_sync->mutex_[j]) {
        for (i = 0; i < rows; ++i) {
      ...
      CHECK_MEM_ERROR(cm, lf_sync->cond_[j], aom_malloc(sizeof(*(lf_sync->cond_[j])) * rows));
      if (lf_sync->cond_[j]) {
        for (i = 0; i < rows; ++i) {
    ...
    CHECK_MEM_ERROR(cm, lf_sync->job_mutex, aom_malloc(sizeof(*(lf_sync->job_mutex))));
    if (lf_sync->job_mutex) {
      pthread_mutex_init(lf_sync->job_mutex, NULL);
    }
  }
#endif  // CONFIG_MULTITHREAD
  CHECK_MEM_ERROR(cm, lf_sync->lfdata, aom_malloc(num_workers * sizeof(*(lf_sync->lfdata))));
  lf_sync->num_workers = num_workers;

  for (int j = 0; j < MAX_MB_PLANE; j++) {
    CHECK_MEM_ERROR(
        cm, lf_sync->cur_sb_col[j], aom_malloc(sizeof(*(lf_sync->cur_sb_col[j])) * rows));
  }
  CHECK_MEM_ERROR(
      cm, lf_sync->job_queue, aom_malloc(sizeof(*(lf_sync->job_queue)) * rows * MAX_MB_PLANE * 2));
  // Set up nsync.
...
static INLINE void sync_read(AV1LfSync *const lf_sync, int r, int c, int plane)
{
#if CONFIG_MULTITHREAD
...
static INLINE void sync_write(AV1LfSync *const lf_sync, int r, int c, const int sb_cols, int plane)
{
#if CONFIG_MULTITHREAD
  const int nsync = lf_sync->sync_range;
  int cur;
...
static void enqueue_lf_jobs(AV1LfSync *lf_sync,
                            AV1_COMMON *cm,
                            int start,
                            int stop,
#if CONFIG_LPF_MASK
...
{
  const int sb_cols = ALIGN_POWER_OF_TWO(cm->mi_params.mi_cols, MAX_MIB_SIZE_LOG2) >>
                      MAX_MIB_SIZE_LOG2;
      ...
      if (dir == 0) {
        for (mi_col = 0; mi_col < cm->mi_params.mi_cols; mi_col += MAX_MIB_SIZE) {
          ...
          av1_setup_dst_planes(
              planes, cm->seq_params.sb_size, frame_buffer, mi_row, mi_col, plane, plane + 1);
...
static int loop_filter_row_worker(void *arg1, void *arg2)
{
  ...
  thread_loop_filter_rows(
      lf_data->frame_buffer, lf_data->cm, lf_data->planes, lf_data->xd, lf_sync);
  return 1;
}
...
static INLINE void thread_loop_filter_bitmask_rows(const YV12_BUFFER_CONFIG *const frame_buffer,
                                                   ...
                                                   AV1LfSync *const lf_sync)
{
  const int sb_cols = ALIGN_POWER_OF_TWO(cm->mi_params.mi_cols, MIN_MIB_SIZE_LOG2) >>
                      MIN_MIB_SIZE_LOG2;
  int mi_row, mi_col, plane, dir;
  int r, c;
  (void)xd;
  ...
  while (1) {
    ...
    if (cur_job_info != NULL) {
      ...
      if (dir == 0) {
        for (mi_col = 0; mi_col < cm->mi_params.mi_cols; mi_col += MI_SIZE_64X64) {
          c = mi_col >> MIN_MIB_SIZE_LOG2;

          av1_setup_dst_planes(
              planes, BLOCK_64X64, frame_buffer, mi_row, mi_col, plane, plane + 1);

          av1_filter_block_plane_bitmask_vert(cm, &planes[plane], plane, mi_row, mi_col);
          sync_write(lf_sync, r, c, sb_cols, plane);
        }
      }
      else if (dir == 1) {
        for (mi_col = 0; mi_col < cm->mi_params.mi_cols; mi_col += MI_SIZE_64X64) {
          ...
          av1_setup_dst_planes(
              planes, BLOCK_64X64, frame_buffer, mi_row, mi_col, plane, plane + 1);
          av1_filter_block_plane_bitmask_horz(cm, &planes[plane], plane, mi_row, mi_col);
        }
      }
    }
    else {
...
static int loop_filter_bitmask_row_worker(void *arg1, void *arg2)
{
  AV1LfSync *const lf_sync = (AV1LfSync *)arg1;
  LFWorkerData *const lf_data = (LFWorkerData *)arg2;
  thread_loop_filter_bitmask_rows(
      lf_data->frame_buffer, lf_data->cm, lf_data->planes, lf_data->xd, lf_sync);
  return 1;
}
                                ...
                                AV1LfSync *lf_sync)
{
  ...
  int sb_rows;
  if (is_decoding) {
    sb_rows = ALIGN_POWER_OF_TWO(cm->mi_params.mi_rows, MIN_MIB_SIZE_LOG2) >> MIN_MIB_SIZE_LOG2;
  }
  else {
    sb_rows = ALIGN_POWER_OF_TWO(cm->mi_params.mi_rows, MAX_MIB_SIZE_LOG2) >> MAX_MIB_SIZE_LOG2;
  }
#else
  // Number of superblock rows and cols
  const int sb_rows = ALIGN_POWER_OF_TWO(cm->mi_params.mi_rows, MAX_MIB_SIZE_LOG2) >>
                      MAX_MIB_SIZE_LOG2;
#endif
  const int num_workers = nworkers;
  int i;

  if (!lf_sync->sync_range || sb_rows != lf_sync->rows || num_workers > lf_sync->num_workers) {
  ...
  for (i = 0; i < MAX_MB_PLANE; i++) {
    memset(lf_sync->cur_sb_col[i], -1, sizeof(*(lf_sync->cur_sb_col[i])) * sb_rows);
  }

  enqueue_lf_jobs(lf_sync,
                  cm,
                  start,
                  stop,
#if CONFIG_LPF_MASK
                              ...
                              AV1LfSync *lf_sync)
{
  ...
  if (is_decoding) {
    ...
    for (int plane = plane_start; plane < plane_end; plane++) {
      ...
      struct macroblockd_plane *pd = xd->plane;
      av1_setup_dst_planes(pd, cm->seq_params.sb_size, frame, 0, 0, plane, plane + 1);

      av1_build_bitmask_vert_info(cm, &pd[plane], plane);
      av1_build_bitmask_horz_info(cm, &pd[plane], plane);
    }
    loop_filter_rows_mt(frame,
                        cm,
                        xd,
                        start_mi_row,
                        end_mi_row,
...
static INLINE void lr_sync_write(void *const lr_sync, int r, int c, const int sb_cols, int plane)
{
#  if CONFIG_MULTITHREAD
  AV1LrSync *const loop_res_sync = (AV1LrSync *)lr_sync;
  const int nsync = loop_res_sync->sync_range;
...
static void loop_restoration_alloc(AV1LrSync *lr_sync,
                                   AV1_COMMON *cm,
                                   int num_workers,
                                   int num_rows_lr,
                                   int num_planes,
...
#  if CONFIG_MULTITHREAD
  {
    ...
    for (j = 0; j < num_planes; j++) {
      CHECK_MEM_ERROR(
          cm, lr_sync->mutex_[j], aom_malloc(sizeof(*(lr_sync->mutex_[j])) * num_rows_lr));
      if (lr_sync->mutex_[j]) {
      ...
      CHECK_MEM_ERROR(
          cm, lr_sync->cond_[j], aom_malloc(sizeof(*(lr_sync->cond_[j])) * num_rows_lr));
      if (lr_sync->cond_[j]) {
        for (i = 0; i < num_rows_lr; ++i) {
    ...
    CHECK_MEM_ERROR(cm, lr_sync->job_mutex, aom_malloc(sizeof(*(lr_sync->job_mutex))));
...
#  endif  // CONFIG_MULTITHREAD
  CHECK_MEM_ERROR(
      cm, lr_sync->lrworkerdata, aom_malloc(num_workers * sizeof(*(lr_sync->lrworkerdata))));

  for (int worker_idx = 0; worker_idx < num_workers; ++worker_idx) {
    if (worker_idx < num_workers - 1) {
      CHECK_MEM_ERROR(cm,
                      lr_sync->lrworkerdata[worker_idx].rst_tmpbuf,
                      (int32_t *)aom_memalign(16, RESTORATION_TMPBUF_SIZE));
      CHECK_MEM_ERROR(
          cm, lr_sync->lrworkerdata[worker_idx].rlbs, aom_malloc(sizeof(RestorationLineBuffers)));
    }
    else {
  ...
  for (int j = 0; j < num_planes; j++) {
    CHECK_MEM_ERROR(
        cm, lr_sync->cur_sb_col[j], aom_malloc(sizeof(*(lr_sync->cur_sb_col[j])) * num_rows_lr));
  }
  CHECK_MEM_ERROR(cm,
                  lr_sync->job_queue,
...
#  endif  // CONFIG_MULTITHREAD
    ...
    if (lr_sync->lrworkerdata) {
      for (int worker_idx = 0; worker_idx < num_workers - 1; worker_idx++) {
        LRWorkerData *const workerdata_data = lr_sync->lrworkerdata + worker_idx;
...
static void enqueue_lr_jobs(AV1LrSync *lr_sync, AV1LrStruct *lr_ctxt, AV1_COMMON *cm)
{
  ...
  for (int plane = 0; plane < num_planes; plane++) {
    if (cm->rst_info[plane].frame_restoration_type == RESTORE_NONE)
      continue;
    num_even_lr_jobs = num_even_lr_jobs + ((ctxt[plane].rsi->vert_units_per_tile + 1) >> 1);
  }
  lr_job_counter[0] = 0;
  ...
  for (int plane = 0; plane < num_planes; plane++) {
    ...
    while (y0 < tile_h) {
      ...
      if ((i & 1) == 0) {
        lr_job_queue[lr_job_counter[i & 1]].v_copy_start = limits.v_start + RESTORATION_BORDER;
        lr_job_queue[lr_job_counter[i & 1]].v_copy_end = limits.v_end - RESTORATION_BORDER;
        if (i == 0) {
        ...
        lr_job_queue[lr_job_counter[i & 1]].v_copy_start = AOMMAX(
            limits.v_start - RESTORATION_BORDER, tile_rect.top);
        lr_job_queue[lr_job_counter[i & 1]].v_copy_end = AOMMIN(limits.v_end + RESTORATION_BORDER,
                                                                tile_rect.bottom);
      }
      lr_job_counter[i & 1]++;
...
static int loop_restoration_row_worker(void *arg1, void *arg2)
{
  ...
  static const copy_fun copy_funs[3] = {
      aom_yv12_partial_coloc_copy_y, aom_yv12_partial_coloc_copy_u, aom_yv12_partial_coloc_copy_v};
  ...
  while (1) {
    ...
    if (cur_job_info != NULL) {
      ...
      on_sync_read = cur_job_info->sync_mode == 1 ? lr_sync_read : av1_lr_sync_read_dummy;
      on_sync_write = cur_job_info->sync_mode == 0 ? lr_sync_write : av1_lr_sync_write_dummy;

      av1_foreach_rest_unit_in_row(&limits,
                                   &(ctxt[plane].tile_rect),
                                   lr_ctxt->on_rest_unit,
                                   lr_unit_row,
                                   ctxt[plane].rsi->restoration_unit_size,
                                   unit_idx0,
                                   ctxt[plane].rsi->horz_units_per_tile,
                                   ctxt[plane].rsi->vert_units_per_tile,
                                   plane,
                                   &ctxt[plane],
                                   lrworkerdata->rst_tmpbuf,
                                   lrworkerdata->rlbs,
                                   on_sync_read,
                                   on_sync_write,
                                   lr_sync);

      copy_funs[plane](lr_ctxt->dst,
                       lr_ctxt->frame,
                       ctxt[plane].tile_rect.left,
                       ctxt[plane].tile_rect.right,
...
static void foreach_rest_unit_in_planes_mt(
    AV1LrStruct *lr_ctxt, AVxWorker *workers, int nworkers, AV1LrSync *lr_sync, AV1_COMMON *cm)
{
  FilterFrameCtxt *ctxt = lr_ctxt->ctxt;
  ...
  for (int plane = 0; plane < num_planes; plane++) {
    ...
    num_rows_lr = AOMMAX(num_rows_lr, av1_lr_count_units_in_tile(unit_size, max_tile_h));
  }
  ...
  if (!lr_sync->sync_range || num_rows_lr != lr_sync->rows || num_workers > lr_sync->num_workers ||
      num_planes != lr_sync->num_planes)
  {
    av1_loop_restoration_dealloc(lr_sync, num_workers);
    loop_restoration_alloc(lr_sync, cm, num_workers, num_rows_lr, num_planes, cm->width);
  }
                                          ...
                                          void *lr_ctxt)
{
  ...
  av1_loop_restoration_filter_frame_init(loop_rest_ctxt, frame, cm, optimized_lr, num_planes);

  foreach_rest_unit_in_planes_mt(loop_rest_ctxt, workers, num_workers, lr_sync, cm);
}
#endif
