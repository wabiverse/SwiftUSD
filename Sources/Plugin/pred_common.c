...
int av1_get_pred_context_switchable_interp(const MACROBLOCKD *xd, int dir)
{
  ...
  if (xd->up_available)
    above_type = get_ref_filter_type(xd->mi[-xd->mi_stride], xd, dir, ref_frame);
...
int av1_get_palette_cache(const MACROBLOCKD *const xd, int plane, uint16_t *cache)
{
  ...
  const MB_MODE_INFO *const above_mi = (row % (1 << MIN_SB_SIZE_LOG2)) ? xd->above_mbmi : NULL;
...
int av1_get_reference_mode_context(const MACROBLOCKD *xd)
{
  ...
  if (has_above && has_left) {  // both edges available
    if (!has_second_ref(above_mbmi) && !has_second_ref(left_mbmi))
      ...
      ctx = 2 + (IS_BACKWARD_REF_FRAME(above_mbmi->ref_frame[0]) || !is_inter_block(above_mbmi));
    else if (!has_second_ref(left_mbmi))
      // one of two edges uses comp pred (2/3)
      ctx = 2 + (IS_BACKWARD_REF_FRAME(left_mbmi->ref_frame[0]) || !is_inter_block(left_mbmi));
  ...
  if (above_in_image && left_in_image) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (a_sg && l_sg) {  // single/single
        ...
        const int uni_rfc = a_sg ? has_uni_comp_refs(left_mbmi) : has_uni_comp_refs(above_mbmi);
...
static int get_pred_context_ll2_or_l3gld(const MACROBLOCKD *xd)
{
  ...
  const int last3_gld_count = ref_counts[LAST3_FRAME] + ref_counts[GOLDEN_FRAME];
...
static int get_pred_context_last_or_last2(const MACROBLOCKD *xd)
{
  ...
  const int pred_context = (last_count == last2_count) ? 1 : ((last_count < last2_count) ? 0 : 2);
...
static int get_pred_context_brfarf2_or_arf(const MACROBLOCKD *xd)
{
  ...
  const int brfarf2_count = ref_counts[BWDREF_FRAME] + ref_counts[ALTREF2_FRAME];
  const int arf_count = ref_counts[ALTREF_FRAME];

  const int pred_context = (brfarf2_count == arf_count) ? 1 :
...
static int get_pred_context_brf_or_arf2(const MACROBLOCKD *xd)
{
  ...
  const int pred_context = (brf_count == arf2_count) ? 1 : ((brf_count < arf2_count) ? 0 : 2);
...
int av1_get_pred_context_single_ref_p1(const MACROBLOCKD *xd)
{
  ...
  const int pred_context = (fwd_count == bwd_count) ? 1 : ((fwd_count < bwd_count) ? 0 : 2);
