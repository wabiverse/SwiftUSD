...
// |a| and |b| hold int32_t values. The int64_t type is used so that we can negate INT32_MIN
// without overflowing int32_t.
...
avifBool avifFractionCD(avifFraction *a, avifFraction *b)
{
  ...
  if (a->d != b->d) {
    ...
    if (overflowsInt32(anNew) || overflowsInt32(adNew) || overflowsInt32(bnNew) ||
        overflowsInt32(bdNew))
    {
      return AVIF_FALSE;
    }
    a->n = (int32_t)anNew;
...
static avifBool avifDoubleToUnsignedFractionImpl(double v,
                                                 uint32_t maxNumerator,
                                                 uint32_t *numerator,
                                                 uint32_t *denominator)
{
  ...
  // Maximum denominator: makes sure that the numerator is <= maxNumerator and the denominator is
  // <= UINT32_MAX.
