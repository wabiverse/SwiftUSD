...
VtMatrix4dArray HdEmbreeInstancer::ComputeInstanceTransforms(SdfPath const &prototypeId)
{
  ...
  if (_primvarMap.count(HdInstancerTokens->instanceTranslations) > 0) {
    HdEmbreeBufferSampler sampler(*_primvarMap[HdInstancerTokens->instanceTranslations]);
    for (size_t i = 0; i < instanceIndices.size(); ++i) {
      GfVec3f translate;
  ...
  if (_primvarMap.count(HdInstancerTokens->instanceTransforms) > 0) {
    HdEmbreeBufferSampler sampler(*_primvarMap[HdInstancerTokens->instanceTransforms]);
    for (size_t i = 0; i < instanceIndices.size(); ++i) {
