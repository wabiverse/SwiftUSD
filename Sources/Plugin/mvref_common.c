...
static AOM_INLINE void add_ref_mv_candidate(const MB_MODE_INFO *const candidate,
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    for (ref = 0; ref < 2; ++ref) {
      if (candidate->ref_frame[ref] == rf[0]) {
        const int is_gm_block = is_global_mv_block(candidate, gm_params[rf[0]].wmtype);
        const int_mv this_refmv = is_gm_block ? gm_mv_candidates[0] : get_block_mv(candidate, ref);
...
static AOM_INLINE void scan_row_mbmi(const AV1_COMMON *cm,
                                     ...
                                     int *processed_rows)
{
  ...
  if (abs(row_offset) > 1) {
    col_offset = 1;
    if ((mi_col & 0x01) && xd->width < width_8x8)
      --col_offset;
  }
  ...
  for (int i = 0; i < end_mi;) {
    ...
    if (xd->width >= width_8x8 && xd->width <= n4_w) {
      uint16_t inc = AOMMIN(-max_row_offset + row_offset + 1, mi_size_high[candidate_bsize]);
...
static AOM_INLINE void scan_col_mbmi(const AV1_COMMON *cm,
                                     const MACROBLOCKD *xd,
                                     int mi_row,
                                     const MV_REFERENCE_FRAME rf[2],
                                     int col_offset,
                                     ...
                                     int *processed_cols)
{
  ...
  if (abs(col_offset) > 1) {
    row_offset = 1;
    if ((mi_row & 0x01) && xd->height < n8_h_8)
      --row_offset;
  }
  const int use_step_16 = (xd->height >= 16);

  for (i = 0; i < end_mi;) {
    const MB_MODE_INFO *const candidate = xd->mi[(row_offset + i) * xd->mi_stride + col_offset];
    const int candidate_bsize = candidate->bsize;
    ...
    if (xd->height >= n8_h_8 && xd->height <= n4_h) {
      int inc = AOMMIN(-max_col_offset + col_offset + 1, mi_size_wide[candidate_bsize]);
      // Obtain range used in weight calculation.
      weight = AOMMAX(weight, inc);
...
static AOM_INLINE void scan_blk_mbmi(const AV1_COMMON *cm,
                                     const MACROBLOCKD *xd,
                                     const int mi_row,
                                     const int mi_col,
                                     const MV_REFERENCE_FRAME rf[2],
                                     ...
                                     uint8_t *refmv_count)
{
  ...
  if (is_inside(tile, mi_col, mi_row, &mi_pos)) {
    const MB_MODE_INFO *const candidate = xd->mi[mi_pos.row * xd->mi_stride + mi_pos.col];
...
static int has_top_right(
    const AV1_COMMON *cm, const MACROBLOCKD *xd, int mi_row, int mi_col, int bs)
{
  ...
  if (xd->width > xd->height) {
    if (!xd->is_first_horizontal_rect)
      has_tr = 0;
  }
...
static int check_sb_border(const int mi_row,
                           const int mi_col,
                          ...
                          int16_t *mode_context)
{
  ...
  const TPL_MV_REF *prev_frame_mvs = cm->tpl_mvs +
                                     ((mi_row + mi_pos.row) >> 1) *
  ...
  const int cur_offset_0 = get_relative_dist(
      &cm->seq_params.order_hint_info, cur_frame_index, frame0_index);
  int idx;
  ...
  lower_mv_precision(&this_refmv.as_mv, allow_high_precision_mv, force_integer_mv);
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    get_mv_projection(&comp_refmv.as_mv,
                      ...
                      cur_offset_1,
                      prev_frame_mvs->ref_frame_offset);
    lower_mv_precision(&comp_refmv.as_mv, allow_high_precision_mv, force_integer_mv);
...
static AOM_INLINE void process_compound_ref_mv_candidate(const MB_MODE_INFO *const candidate,
...
static AOM_INLINE void setup_ref_mv_list(const AV1_COMMON *cm,
                                         const MACROBLOCKD *xd,
                                         MV_REFERENCE_FRAME ref_frame,
                                         uint8_t *const refmv_count,
                                         CANDIDATE_MV ref_mv_stack[MAX_REF_MV_STACK_SIZE],
                                         ...
                                         int16_t *mode_context)
{
  ...
  *refmv_count = 0;
  ...
  if (cm->features.allow_ref_frame_mvs) {
    ...
    const int step_h = (xd->height >= mi_size_high[BLOCK_64X64]) ? mi_size_high[BLOCK_16X16] :
                                                                   mi_size_high[BLOCK_8X8];
    const int step_w = (xd->width >= mi_size_wide[BLOCK_64X64]) ? mi_size_wide[BLOCK_16X16] :
  ...
  if (rf[1] > NONE_FRAME) {
    ...
    if (*refmv_count < MAX_MV_REF_CANDIDATES) {
      ...
      for (int idx = 0; idx < 2; ++idx) {
        int comp_idx = 0;
        for (int list_idx = 0; list_idx < ref_id_count[idx] && comp_idx < MAX_MV_REF_CANDIDATES;
             ++list_idx, ++comp_idx)
          comp_list[comp_idx][idx] = ref_id[idx][list_idx];
        for (int list_idx = 0; list_idx < ref_diff_count[idx] && comp_idx < MAX_MV_REF_CANDIDATES;
             ++list_idx, ++comp_idx)
    ...
    for (int idx = 0; idx < *refmv_count; ++idx) {
      clamp_mv_ref(&ref_mv_stack[idx].this_mv.as_mv,
                   xd->width << MI_SIZE_LOG2,
                   xd->height << MI_SIZE_LOG2,
                   xd);
         ...
         abs(max_row_offset) >= 1 && idx < mi_size && *refmv_count < MAX_MV_REF_CANDIDATES;)
    {
      ...
      process_single_ref_mv_candidate(
          candidate, cm, ref_frame, refmv_count, ref_mv_stack, ref_mv_weight);
      idx += mi_size_wide[candidate->bsize];
    }
    ...
    if (mv_ref_list != NULL) {
      ...
      for (int idx = 0; idx < AOMMIN(MAX_MV_REF_CANDIDATES, *refmv_count); ++idx) {
...
void av1_find_mv_refs(const AV1_COMMON *cm,
                      const MACROBLOCKD *xd,
                      MB_MODE_INFO *mi,
                      MV_REFERENCE_FRAME ref_frame,
                      ...
                      int16_t *mode_context)
{
  ...
  setup_ref_mv_list(cm,
                    ...
                    ref_mv_weight[ref_frame],
                    mv_ref_list ? mv_ref_list[ref_frame] : NULL,
                    gm_mv,
                    mi_row,
                    mi_col,
                    mode_context);
...
void av1_setup_frame_buf_refs(AV1_COMMON *cm)
{
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    ...
    if (buf != NULL) {
      ...
      cm->cur_frame->ref_display_order_hint[ref_frame - LAST_FRAME] = buf->display_order_hint;
    }
  }
}
...
void av1_setup_frame_sign_bias(AV1_COMMON *cm)
{
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    ...
    if (cm->seq_params.order_hint_info.enable_order_hint && buf != NULL) {
      ...
      cm->ref_frame_sign_bias[ref_frame] = (get_relative_dist(&cm->seq_params.order_hint_info,
                                                              ref_order_hint,
                                                              (int)cm->current_frame.order_hint) <=
                                            0) ?
                                               0 :
                                               1;
    }
...
static int get_block_position(
    AV1_COMMON *cm, int *mi_r, int *mi_c, int blk_row, int blk_col, MV mv, int sign_bias)
{
...
// frames.
static int motion_field_projection(AV1_COMMON *cm, MV_REFERENCE_FRAME start_frame, int dir)
{
  TPL_MV_REF *tpl_mvs_base = cm->tpl_mvs;
  int ref_offset[REF_FRAMES] = {0};

  const RefCntBuffer *const start_frame_buf = get_ref_frame_buf(cm, start_frame);
  ...
  for (MV_REFERENCE_FRAME rf = LAST_FRAME; rf <= INTER_REFS_PER_FRAME; ++rf) {
    ref_offset[rf] = get_relative_dist(
        &cm->seq_params.order_hint_info, start_frame_order_hint, ref_order_hints[rf - LAST_FRAME]);
  }
...
void av1_setup_motion_field(AV1_COMMON *cm)
{
  ...
  TPL_MV_REF *tpl_mvs_base = cm->tpl_mvs;
  int size = ((cm->mi_params.mi_rows + MAX_MIB_SIZE) >> 1) * (cm->mi_params.mi_stride >> 1);
  ...
  if (ref_buf[LAST_FRAME - LAST_FRAME] != NULL) {
    ...
    if (!is_lst_overlay)
      motion_field_projection(cm, LAST_FRAME, 2);
    --ref_stamp;
  }
  ...
  if (ref_stamp >= 0)
    motion_field_projection(cm, LAST2_FRAME, 2);
}
...
uint8_t av1_findSamples(const AV1_COMMON *cm, MACROBLOCKD *xd, int *pts, int *pts_inref)
{
  ...
  if (up_available) {
    ...
    if (xd->width <= superblock_width) {
      ...
      if (col_offset < 0)
        do_tl = 0;
      if (col_offset + superblock_width > xd->width)
        do_tr = 0;
      ...
      // Handle "current block width > above block width" case.
      for (int i = 0; i < AOMMIN(xd->width, cm->mi_params.mi_cols - mi_col); i += superblock_width)
      {
  ...
  // Top-right block
  if (do_tr && has_top_right(cm, xd, mi_row, mi_col, AOMMAX(xd->width, xd->height))) {
    const POSITION trb_pos = {-1, xd->width};
    const TileInfo *const tile = &xd->tile;
    if (is_inside(tile, mi_col, mi_row, &trb_pos)) {
      const int mi_row_offset = -1;
      const int mi_col_offset = xd->width;
      const MB_MODE_INFO *mbmi = xd->mi[mi_col_offset + mi_row_offset * mi_stride];
...
void av1_setup_skip_mode_allowed(AV1_COMMON *cm)
{
  ...
  for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
    ...
    else if (get_relative_dist(order_hint_info, ref_order_hint, cur_order_hint) > 0) {
      // Backward reference
...
static AOM_INLINE void set_ref_frame_info(int *remapped_ref_idx,
                                          int frame_idx,
                        ...
                        int gld_map_idx)
{
  ...
  const int cur_frame_sort_idx = 1 << cm->seq_params.order_hint_info.order_hint_bits_minus_1;
  ...
  if (lst_frame_sort_idx == -1 || lst_frame_sort_idx >= cur_frame_sort_idx) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Inter frame requests a look-ahead frame as LAST");
  }
  if (gld_frame_sort_idx == -1 || gld_frame_sort_idx >= cur_frame_sort_idx) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Inter frame requests a look-ahead frame as GOLDEN");
  }

  // Sort ref frames based on their frame_offset values.
  qsort(ref_frame_info, REF_FRAMES, sizeof(REF_FRAME_INFO), compare_ref_frame_info);
  ...
  if (bwd_start_idx <= bwd_end_idx) {
    set_ref_frame_info(remapped_ref_idx, ALTREF_FRAME - LAST_FRAME, &ref_frame_info[bwd_end_idx]);
  ...
  if (bwd_start_idx <= bwd_end_idx) {
    set_ref_frame_info(
        remapped_ref_idx, BWDREF_FRAME - LAST_FRAME, &ref_frame_info[bwd_start_idx]);
  ...
  if (bwd_start_idx <= bwd_end_idx) {
    set_ref_frame_info(
        remapped_ref_idx, ALTREF2_FRAME - LAST_FRAME, &ref_frame_info[bwd_start_idx]);
  ...
  for (int i = fwd_start_idx; i <= fwd_end_idx; ++i) {
    ...
    if (ref_frame_info[i].map_idx == lst_map_idx) {
      set_ref_frame_info(remapped_ref_idx, LAST_FRAME - LAST_FRAME, &ref_frame_info[i]);
      ref_flag_list[LAST_FRAME - LAST_FRAME] = 1;
    }

    // == GOLDEN_FRAME ==
    if (ref_frame_info[i].map_idx == gld_map_idx) {
      set_ref_frame_info(remapped_ref_idx, GOLDEN_FRAME - LAST_FRAME, &ref_frame_info[i]);
  ...
  for (ref_idx = 0; ref_idx < (INTER_REFS_PER_FRAME - 2); ref_idx++) {
    ...
    while (fwd_start_idx <= fwd_end_idx && (ref_frame_info[fwd_end_idx].map_idx == lst_map_idx ||
    ...
    set_ref_frame_info(remapped_ref_idx, ref_frame - LAST_FRAME, &ref_frame_info[fwd_end_idx]);
  ...
  for (; ref_idx < (INTER_REFS_PER_FRAME - 2); ref_idx++) {
    ...
    if (ref_flag_list[ref_frame - LAST_FRAME] == 1)
      continue;
    set_ref_frame_info(remapped_ref_idx, ref_frame - LAST_FRAME, &ref_frame_info[fwd_start_idx]);
    ref_flag_list[ref_frame - LAST_FRAME] = 1;
  }
