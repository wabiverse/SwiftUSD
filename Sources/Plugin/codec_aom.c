...
#  include "Plugin/hioAvif/aom/aomcx.h"
#endif
...
// Speeds 7-9 were added to all intra mode in
                                     ...
                                     avifImage *image)
{
  if (!codec->internal->decoderInitialized) {
    ...
    if (aom_codec_control(
            &codec->internal->decoder, AV1D_SET_OPERATING_POINT, codec->operatingPoint))
    {
      return AVIF_FALSE;
    }
...
#  endif
    ...
    if (image->width && image->height) {
      if ((image->width != codec->internal->image->d_w) ||
    ...
    image->yuvRange = (codec->internal->image->range == AOM_CR_STUDIO_RANGE) ? AVIF_RANGE_LIMITED :
                                                                               AVIF_RANGE_FULL;
...
static avifBool aomOptionParseEnum(const char *str,
...
static avifBool avifKeyEqualsName(const char *key, const char *name, avifBool alpha)
{
  ...
  return !strcmp(key, name) ||
         (!strncmp(key, prefix, prefixLen) && !strcmp(key + prefixLen, name)) ||
         (!strncmp(key, shortPrefix, shortPrefixLen) && !strcmp(key + shortPrefixLen, name));
}

static avifBool avifProcessAOMOptionsPreInit(avifCodec *codec,
                                             avifBool alpha,
                                             struct aom_codec_enc_cfg *cfg)
{
...
static avifBool avifProcessAOMOptionsPostInit(avifCodec *codec, avifBool alpha)
{
  for (uint32_t i = 0; i < codec->csOptions->count; ++i) {
    ...
    if (!strncmp(entry->key, otherPrefix, otherPrefixLen) ||
        !strncmp(entry->key, otherShortPrefix, otherShortPrefixLen))
    {
      continue;
    }
      ...
      if (avifKeyEqualsName(entry->key, aomOptionDefs[j].name, alpha)) {
        ...
        switch (aomOptionDefs[j].type) {
          ...
          case AVIF_AOM_OPTION_STR:
            success = aom_codec_control(&codec->internal->encoder,
                                        aomOptionDefs[j].controlId,
                                        entry->value) == AOM_CODEC_OK;
            break;
...
static avifBool avifFindAOMScalingMode(const avifFraction *avifMode, AOM_SCALING_MODE *aomMode)
{
  ...
  for (int i = 0; i < scalingModeMapSize; ++i) {
    if (scalingModeMap[i].avifMode.n == simplifiedFraction.n &&
        scalingModeMap[i].avifMode.d == simplifiedFraction.d)
    {
...
static avifBool doesLevelMatch(
    int width, int height, int levelWidth, int levelHeight, int levelDimMult)
{
  const int64_t levelLumaPels = (int64_t)levelWidth * levelHeight;
  const int64_t lumaPels = (int64_t)width * height;
  return lumaPels <= levelLumaPels && width <= levelWidth * levelDimMult &&
         height <= levelHeight * levelDimMult;
}
...
#  endif
    ...
    if (err != AOM_CODEC_OK) {
      avifDiagnosticsPrintf(
          codec->diag, "aom_codec_enc_config_default() failed: %s", aom_codec_err_to_string(err));
      return AVIF_RESULT_UNKNOWN_ERROR;
    }
...
#  endif
    ...
    if (aomVersion == aomVersion_3_6_0) {
           ...
           doesLevelMatch(image->width, image->height, 32768, 17408, 2)))
      {
        avifDiagnosticsPrintf(
            codec->diag,
            "Detected libaom v3.6.0 bug with large images. Upgrade to libaom v3.6.1 or later.");
        return AVIF_RESULT_UNKNOWN_ERROR;
      }
    }
    ...
    if (aom_codec_enc_init(&codec->internal->encoder, encoderInterface, cfg, encoderFlags) !=
        AOM_CODEC_OK)
    {
      avifDiagnosticsPrintf(codec->diag,
    ...
    if (aomCpuUsed != -1) {
      if (aom_codec_control(&codec->internal->encoder, AOME_SET_CPUUSED, aomCpuUsed) !=
          AOM_CODEC_OK)
      {
        return AVIF_RESULT_UNKNOWN_ERROR;
      }
...
#  endif
    ...
    if (!codec->internal->tuningSet) {
      if (aom_codec_control(&codec->internal->encoder, AOME_SET_TUNING, AOM_TUNE_SSIM) !=
          AOM_CODEC_OK)
      {
        return AVIF_RESULT_UNKNOWN_ERROR;
      }
    ...
    if (alpha) {
      if (encoderChanges &
          (AVIF_ENCODER_CHANGE_MIN_QUANTIZER_ALPHA | AVIF_ENCODER_CHANGE_MAX_QUANTIZER_ALPHA))
      {
    ...
    const int quantizerChangedBit = alpha ? AVIF_ENCODER_CHANGE_QUANTIZER_ALPHA :
                                            AVIF_ENCODER_CHANGE_QUANTIZER;
  ...
  if (encoder->extraLayerCount > 0) {
    aom_codec_control(
        &codec->internal->encoder, AOME_SET_SPATIAL_LAYER_ID, codec->internal->currentLayer);
  }
  ...
  if ((aomScalingMode.h_scaling_mode != AOME_NORMAL) ||
      (aomScalingMode.v_scaling_mode != AOME_NORMAL))
  {
  ...
  if (aomImageAllocated) {
    ...
    aomImage.bps = bps;
    // See avifImageCalcAOMFmt(). libaom doesn't have AOM_IMG_FMT_I400, so we use AOM_IMG_FMT_I420
    // as a substitute for monochrome.
    aomImage.x_chroma_shift = (alpha || codec->internal->formatInfo.monochrome) ?
                                  1 :
                                  codec->internal->formatInfo.chromaShiftX;
    aomImage.y_chroma_shift = (alpha || codec->internal->formatInfo.monochrome) ?
                                  1 :
                                  codec->internal->formatInfo.chromaShiftY;
  }
  ...
  if (monochromeRequested) {
    if (codec->internal->monochromeEnabled) {
      ...
      if (!aomImageAllocated) {
        ...
        AVIF_CHECKERR(
            monoUVPlane != NULL,
            AVIF_RESULT_OUT_OF_MEMORY);  // No need for aom_img_free() because !aomImageAllocated
  ...
  if (codec->internal->currentLayer > 0) {
    encodeFlags |= AOM_EFLAG_NO_REF_GF | AOM_EFLAG_NO_REF_ARF | AOM_EFLAG_NO_REF_BWD |
                   AOM_EFLAG_NO_REF_ARF2 | AOM_EFLAG_NO_UPD_GF | AOM_EFLAG_NO_UPD_ARF;
  }
  aom_codec_err_t encodeErr = aom_codec_encode(
      &codec->internal->encoder, &aomImage, 0, 1, encodeFlags);
  ...
  for (;;) {
    ...
    if (pkt->kind == AOM_CODEC_CX_FRAME_PKT) {
      AVIF_CHECKRES(avifCodecEncodeOutputAddSample(output,
                                                   pkt->data.frame.buf,
                                                   pkt->data.frame.sz,
  ...
  if ((addImageFlags & AVIF_ADD_IMAGE_FLAG_SINGLE) ||
      ((encoder->extraLayerCount > 0) &&
       (encoder->extraLayerCount == codec->internal->currentLayer)))
  {
    // Flush and clean up encoder resources early to save on overhead when encoding alpha or grid
    // images, as encoding is finished now. For layered image, encoding finishes when the last
    // layer is encoded.
...
static avifBool aomCodecEncodeFinish(avifCodec *codec, avifCodecEncodeOutput *output)
{
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (pkt->kind == AOM_CODEC_CX_FRAME_PKT) {
        gotPacket = AVIF_TRUE;
        const avifResult result = avifCodecEncodeOutputAddSample(
            output,
            pkt->data.frame.buf,
            pkt->data.frame.sz,
            (pkt->data.frame.flags & AOM_FRAME_IS_KEY));
        if (result != AVIF_RESULT_OK) {
          avifDiagnosticsPrintf(codec->diag,
                                "avifCodecEncodeOutputAddSample() failed: %s",
