 ...
 *
 ****************************************************************************/
static void scale1d_2t1_i(const unsigned char *source,
                          int source_step,
 ...
 *
 ****************************************************************************/
static void scale1d_2t1_ps(const unsigned char *source,
                           int source_step,
                           unsigned int source_scale,
                           unsigned int source_length,
                           unsigned char *dest,
 ...
 *
 ****************************************************************************/
static void scale1d_c(const unsigned char *source,
                      int source_step,
                      unsigned int source_scale,
                      unsigned int source_length,
                      unsigned char *dest,
                      ...
                      unsigned int dest_length)
{
  ...
  while (dest < dest_end) {
    *dest = (unsigned char)((left_modifier * left_pixel + right_modifier * right_pixel +
                             round_value) /
                            dest_scale);
    ...
    unsigned int interlaced)
{
  ...
  typedef void (*Scale1D)(const unsigned char *source,
                          int source_step,
                          unsigned int source_scale,
                          unsigned int source_length,
                          unsigned char *dest,
  ...
  void (*horiz_line_scale)(
      const unsigned char *, unsigned int, unsigned char *, unsigned int) = NULL;
  void (*vert_band_scale)(unsigned char *, int, unsigned char *, int, unsigned int) = NULL;
  ...
  if (ratio_scalable) {
    ...
    for (k = 0; k < (dest_height + dest_band_height - 1) / dest_band_height; ++k) {
      ...
      for (i = 0; i < source_band_height; ++i) {
        ...
        horiz_line_scale(line_src, source_width, temp_area + (i + 1) * dest_pitch, dest_width);
      }
  ...
  for (k = 0; k < bands; ++k) {
    ...
    for (i = 1; i < source_band_height + 1; ++i) {
      if (k * source_band_height + i < source_height) {
        Scale1Dh(source + i * source_pitch,
                 1,
                 hscale,
                 source_width + 1,
                 temp_area + i * dest_pitch,
                 1,
                 hratio,
                 dest_width);
      }
 ...
 *
 ****************************************************************************/
void aom_scale_frame(YV12_BUFFER_CONFIG *src,
                     ...
                     const int num_planes)
{
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    Scale2D((unsigned char *)src->buffers[plane],
            src->strides[is_uv],
            src->widths[is_uv],
            src->heights[is_uv],
            (unsigned char *)dst->buffers[plane],
            dst->strides[is_uv],
            plane_dw,
            plane_dh,
            temp_area,
