...
void av1_get_tile_limits(AV1_COMMON *const cm)
{
  ...
  const int mi_cols = ALIGN_POWER_OF_TWO(cm->mi_params.mi_cols, seq_params->mib_size_log2);
  const int mi_rows = ALIGN_POWER_OF_TWO(cm->mi_params.mi_rows, seq_params->mib_size_log2);
                             ...
                             CommonTileParams *const tiles)
{
  ...
  if (tiles->uniform_spacing) {
    ...
    if (tiles->cols > 1) {
      tiles->min_inner_width = narrowest_inner_tile_sb << seq_params->mib_size_log2;
    }
  }
}
...
void av1_tile_set_row(TileInfo *tile, const AV1_COMMON *cm, int row)
{
  ...
  int mi_row_start = cm->tiles.row_start_sb[row] << cm->seq_params.mib_size_log2;
  int mi_row_end = cm->tiles.row_start_sb[row + 1] << cm->seq_params.mib_size_log2;
  tile->tile_row = row;
...
void av1_tile_set_col(TileInfo *tile, const AV1_COMMON *cm, int col)
{
  ...
  int mi_col_start = cm->tiles.col_start_sb[col] << cm->seq_params.mib_size_log2;
  int mi_col_end = cm->tiles.col_start_sb[col + 1] << cm->seq_params.mib_size_log2;
  tile->tile_col = col;
...
{
  int mi_rows_aligned_to_sb = ALIGN_POWER_OF_TWO(tile.mi_row_end - tile.mi_row_start,
...
{
  int mi_cols_aligned_to_sb = ALIGN_POWER_OF_TWO(tile.mi_col_end - tile.mi_col_start,
                                                 cm->seq_params.mib_size_log2);
...
AV1PixelRect av1_get_tile_rect(const TileInfo *tile_info, const AV1_COMMON *cm, int is_uv)
{
  AV1PixelRect r;
  ...
  if (av1_superres_scaled(cm)) {
    av1_calculate_unscaled_superres_size(&r.left, &r.top, cm->superres_scale_denominator);
    av1_calculate_unscaled_superres_size(&r.right, &r.bottom, cm->superres_scale_denominator);
  }
  ...
  if (tiles->uniform_spacing) {
    ...
    *h = tiles->height;
    ...
    for (int i = 0; i < tiles->cols; ++i) {
      const int tile_width_sb = tiles->col_start_sb[i + 1] - tiles->col_start_sb[i];
    ...
    for (int i = 0; i < tiles->rows; ++i) {
      const int tile_height_sb = tiles->row_start_sb[i + 1] - tiles->row_start_sb[i];
...
int av1_is_min_tile_width_satisfied(const AV1_COMMON *cm)
{
  ...
  return ((cm->tiles.min_inner_width << MI_SIZE_LOG2) >= (64 << av1_superres_scaled(cm)));
}
