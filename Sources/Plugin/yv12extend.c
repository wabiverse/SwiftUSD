...
static void extend_plane(uint8_t *const src,
                         int src_stride,
                         int width,
                         int height,
                         int extend_top,
...
static void extend_plane_high(uint8_t *const src8,
                              int src_stride,
                              int width,
                              int height,
                              int extend_top,
...
void aom_yv12_extend_frame_borders_c(YV12_BUFFER_CONFIG *ybf, const int num_planes)
{
  ...
  if (ybf->flags & YV12_FLAG_HIGHBITDEPTH) {
    for (int plane = 0; plane < num_planes; ++plane) {
      ...
      extend_plane_high(ybf->buffers[plane],
                        ybf->strides[is_uv],
                        ybf->crop_widths[is_uv],
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    const int is_uv = plane > 0;
    const int plane_border = ybf->border >> is_uv;
    extend_plane(ybf->buffers[plane],
                 ybf->strides[is_uv],
                 ybf->crop_widths[is_uv],
                 ybf->crop_heights[is_uv],
                 plane_border,
...
static void extend_frame(YV12_BUFFER_CONFIG *const ybf, int ext_size, const int num_planes)
{
...
void aom_extend_frame_inner_borders_c(YV12_BUFFER_CONFIG *ybf, const int num_planes)
{
  const int inner_bw = (ybf->border > AOMINNERBORDERINPIXELS) ? AOMINNERBORDERINPIXELS :
                                                                ybf->border;
...
#if CONFIG_AV1_HIGHBITDEPTH
  if (ybf->flags & YV12_FLAG_HIGHBITDEPTH) {
    extend_plane_high(ybf->y_buffer,
                      ybf->y_stride,
                      ybf->y_crop_width,
                      ybf->y_crop_height,
                      ext_size,
...
#if CONFIG_AV1_HIGHBITDEPTH
  assert((src_bc->flags & YV12_FLAG_HIGHBITDEPTH) == (dst_bc->flags & YV12_FLAG_HIGHBITDEPTH));
...
void aom_yv12_copy_y_c(const YV12_BUFFER_CONFIG *src_ybc, YV12_BUFFER_CONFIG *dst_ybc)
{
  int row;
...
void aom_yv12_copy_u_c(const YV12_BUFFER_CONFIG *src_bc, YV12_BUFFER_CONFIG *dst_bc)
{
  int row;
...
#if CONFIG_AV1_HIGHBITDEPTH
  if (src_ybc->flags & YV12_FLAG_HIGHBITDEPTH) {
    const uint16_t *src16 = CONVERT_TO_SHORTPTR(src + vstart1 * src_ybc->y_stride + hstart1);
    uint16_t *dst16 = CONVERT_TO_SHORTPTR(dst + vstart2 * dst_ybc->y_stride + hstart2);
...
void aom_yv12_partial_coloc_copy_y_c(const YV12_BUFFER_CONFIG *src_ybc,
                                     ...
                                     int vend)
{
  aom_yv12_partial_copy_y_c(src_ybc, hstart, hend, vstart, vend, dst_ybc, hstart, vstart);
}
...
#if CONFIG_AV1_HIGHBITDEPTH
  if (src_bc->flags & YV12_FLAG_HIGHBITDEPTH) {
    const uint16_t *src16 = CONVERT_TO_SHORTPTR(src + vstart1 * src_bc->uv_stride + hstart1);
    uint16_t *dst16 = CONVERT_TO_SHORTPTR(dst + vstart2 * dst_bc->uv_stride + hstart2);
    for (row = vstart1; row < vend1; ++row) {
      memcpy(dst16, src16, (hend1 - hstart1) * sizeof(uint16_t));
      src16 += src_bc->uv_stride;
      dst16 += dst_bc->uv_stride;
...
void aom_yv12_partial_coloc_copy_u_c(const YV12_BUFFER_CONFIG *src_bc,
                                     ...
                                     int vend)
{
  aom_yv12_partial_copy_u_c(src_bc, hstart, hend, vstart, vend, dst_bc, hstart, vstart);
}
...
void aom_yv12_partial_coloc_copy_v_c(const YV12_BUFFER_CONFIG *src_bc,
                                     ...
                                     int vend)
{
  aom_yv12_partial_copy_v_c(src_bc, hstart, hend, vstart, vend, dst_bc, hstart, vstart);
}

int aom_yv12_realloc_with_new_border_c(YV12_BUFFER_CONFIG *ybf,
                                       int new_border,
                                       int byte_alignment,
                                       int num_planes)
{
  if (ybf) {
    if (new_border == ybf->border)
      return 0;
    YV12_BUFFER_CONFIG new_buf;
    memset(&new_buf, 0, sizeof(new_buf));
    const int error = aom_alloc_frame_buffer(&new_buf,
                                             ybf->y_crop_width,
                                             ybf->y_crop_height,
                                             ybf->subsampling_x,
                                             ybf->subsampling_y,
