...
#include "Plugin/hioAvif/aom/av1/common/timing.h"
...
aom_codec_err_t aom_get_num_layers_from_operating_point_idc(int operating_point_idc,
  ...
  if (operating_point_idc == 0) {
    ...
    *number_temporal_layers = 0;
    for (int j = 0; j < MAX_NUM_SPATIAL_LAYERS; j++) {
      *number_spatial_layers += (operating_point_idc >> (j + MAX_NUM_TEMPORAL_LAYERS)) & 0x1;
    }
...
static int is_obu_in_current_operating_point(AV1Decoder *pbi, const ObuHeader *obu_header)
{
...
static int byte_alignment(AV1_COMMON *const cm, struct aom_read_bit_buffer *const rb)
{
...
static int read_bitstream_level(AV1_LEVEL *seq_level_idx, struct aom_read_bit_buffer *rb)
{
...
{
  return !memcmp(seq_params_old, seq_params_new, offsetof(SequenceHeader, op_params));
}
...
static uint32_t read_sequence_header_obu(AV1Decoder *pbi, struct aom_read_bit_buffer *rb)
{
  ...
  if (seq_params->reduced_still_picture_hdr) {
    ...
    seq_params->operating_points_cnt_minus_1 = aom_rb_read_literal(rb, OP_POINTS_CNT_MINUS_1_BITS);
    for (int i = 0; i < seq_params->operating_points_cnt_minus_1 + 1; i++) {
      seq_params->operating_point_idc[i] = aom_rb_read_literal(rb, OP_POINTS_IDC_BITS);
      ...
      if (seq_params->seq_level_idx[i] >= SEQ_LEVEL_4_0)
        ...
        seq_params->tier[i] = 0;
      if (seq_params->decoder_model_info_present_flag) {
        seq_params->op_params[i].decoder_model_param_present_flag = aom_rb_read_bit(rb);
        if (seq_params->op_params[i].decoder_model_param_present_flag)
          av1_read_op_parameters_info(
              &seq_params->op_params[i],
              seq_params->decoder_model_info.encoder_decoder_buffer_delay_length,
              rb);
      }
      else {
      ...
      if (seq_params->timing_info_present && seq_params->timing_info.equal_picture_interval &&
          !seq_params->op_params[i].decoder_model_param_present_flag)
      {
      ...
      if (seq_params->display_model_info_present_flag) {
        seq_params->op_params[i].display_model_param_present_flag = aom_rb_read_bit(rb);
        if (seq_params->op_params[i].display_model_param_present_flag) {
          seq_params->op_params[i].initial_display_delay = aom_rb_read_literal(rb, 4) + 1;
  ...
  if (operating_point < 0 || operating_point > seq_params->operating_points_cnt_minus_1)
    operating_point = 0;
  pbi->current_operating_point = seq_params->operating_point_idc[operating_point];
  if (aom_get_num_layers_from_operating_point_idc(pbi->current_operating_point,
...
{
  const uint32_t hdr_size = av1_decode_frame_headers_and_setup(pbi, rb, trailing_bits_present);
                                      ...
                                      int tile_start_implicit)
{
  ...
  if (tiles->large_scale || num_tiles == 1 || !tile_start_and_end_present_flag) {
    *start_tile = 0;
                                        ...
                                        int tile_start_implicit)
{
  ...
  header_size = read_tile_group_header(pbi, rb, &start_tile, &end_tile, tile_start_implicit);
  if (header_size == -1 || byte_alignment(cm, rb))
    return 0;
  data += header_size;
  av1_decode_tg_tiles_and_wrapup(
      pbi, data, data_end, p_data_end, start_tile, end_tile, is_first_tg);
...
static void alloc_tile_list_buffer(AV1Decoder *pbi)
{
  ...
  const int output_frame_width = (pbi->output_frame_width_in_tiles_minus_1 + 1) *
                                 tile_width_in_pixels;
  const int output_frame_height = (pbi->output_frame_height_in_tiles_minus_1 + 1) *
  ...
  assert((pbi->tile_count_minus_1 + 1) <= (pbi->output_frame_width_in_tiles_minus_1 + 1) *
  ...
  if (aom_alloc_frame_buffer(
          ...
          (cm->seq_params.use_highbitdepth && (cm->seq_params.bit_depth > AOM_BITS_8)),
          0,
...
static void yv12_tile_copy(const YV12_BUFFER_CONFIG *src,
                           int hstart1,
                           int hend1,
                           int vstart1,
                           int vend1,
                           ...
                           int plane)
{
  ...
  const uint16_t *src16 = CONVERT_TO_SHORTPTR(src->buffers[plane] + vstart1 * src_stride +
                                              hstart1);
  uint8_t *dst8 = dst->buffers[plane] + vstart2 * dst_stride + hstart2;

  for (row = vstart1; row < vend1; ++row) {
    for (col = 0; col < (hend1 - hstart1); ++col)
      *dst8++ = (uint8_t)(*src16++);
...
static void copy_decoded_tile_to_tile_list_buffer(AV1Decoder *pbi, int tile_idx)
{
  ...
  for (plane = 0; plane < num_planes; ++plane) {
    ...
    if (cm->seq_params.use_highbitdepth && cm->seq_params.bit_depth == AOM_BITS_8) {
      yv12_tile_copy(cur_frame,
                     hstart1,
                     hend1,
                     vstart1,
                     vend1,
      ...
      switch (plane) {
        case 0:
          aom_yv12_partial_copy_y(
              cur_frame, hstart1, hend1, vstart1, vend1, &pbi->tile_list_outbuf, hstart2, vstart2);
          break;
        case 1:
                                              ...
                                              int *frame_decoding_finished)
{
  ...
  for (i = 0; i <= pbi->tile_count_minus_1; i++) {
    ...
    av1_set_reference_dec(cm, cm->remapped_ref_idx[0], 1, &pbi->ext_refs.refs[ref_idx]);

    pbi->dec_tile_row = aom_rb_read_literal(rb, 8);
    pbi->dec_tile_col = aom_rb_read_literal(rb, 8);
    if (pbi->dec_tile_row < 0 || pbi->dec_tile_col < 0 || pbi->dec_tile_row >= cm->tiles.rows ||
    ...
    av1_decode_tg_tiles_and_wrapup(
        pbi, data, data + pbi->coded_tile_data_size, p_data_end, start_tile, end_tile, 0);
                                ...
                                aom_metadata_insert_flags_t insert_flag)
{
  ...
  if (!pbi->metadata) {
    ...
    if (!pbi->metadata) {
      aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR, "Failed to allocate metadata array");
    }
  }
  aom_metadata_t *metadata = aom_img_metadata_alloc(metadata_type, data, sz, insert_flag);
  if (!metadata) {
    aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR, "Error allocating metadata");
  }
  aom_metadata_t **metadata_array = (aom_metadata_t **)realloc(
      pbi->metadata->metadata_array, (pbi->metadata->sz + 1) * sizeof(metadata));
  if (!metadata_array) {
    aom_img_metadata_free(metadata);
    aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR, "Error growing metadata array");
  }
...
static void read_metadata_itut_t35(AV1Decoder *const pbi, const uint8_t *data, size_t sz)
{
  AV1_COMMON *const cm = &pbi->common;
  if (sz == 0) {
    aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "itu_t_t35_country_code is missing");
  }
  int country_code_size = 1;
  if (*data == 0xFF) {
    if (sz == 1) {
      aom_internal_error(
          &cm->error, AOM_CODEC_CORRUPT_FRAME, "itu_t_t35_country_code_extension_byte is missing");
    }
    ++country_code_size;
  }
  ...
  if (end_index < country_code_size) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "No trailing bits found in ITU-T T.35 metadata OBU");
  }
  ...
  alloc_read_metadata(pbi, OBU_METADATA_TYPE_ITUT_T35, data, end_index, AOM_MIF_ANY_FRAME);
}

// On success, returns the number of bytes read from 'data'. On failure, calls
// aom_internal_error() and does not return.
static size_t read_metadata_hdr_cll(AV1Decoder *const pbi, const uint8_t *data, size_t sz)
{
  const size_t kHdrCllPayloadSize = 4;
  AV1_COMMON *const cm = &pbi->common;
  if (sz < kHdrCllPayloadSize) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Incorrect HDR CLL metadata payload size");
  }
  alloc_read_metadata(pbi, OBU_METADATA_TYPE_HDR_CLL, data, kHdrCllPayloadSize, AOM_MIF_ANY_FRAME);
  return kHdrCllPayloadSize;
}

// On success, returns the number of bytes read from 'data'. On failure, calls
// aom_internal_error() and does not return.
static size_t read_metadata_hdr_mdcv(AV1Decoder *const pbi, const uint8_t *data, size_t sz)
{
  const size_t kMdcvPayloadSize = 24;
  AV1_COMMON *const cm = &pbi->common;
  if (sz < kMdcvPayloadSize) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Incorrect HDR MDCV metadata payload size");
  }
  alloc_read_metadata(pbi, OBU_METADATA_TYPE_HDR_MDCV, data, kMdcvPayloadSize, AOM_MIF_ANY_FRAME);
...
static void read_metadata_timecode(struct aom_read_bit_buffer *rb)
{
  ...
  const int full_timestamp_flag = aom_rb_read_bit(rb);  // full_timestamp_flag f(1)
...
// returns 0.
static size_t read_padding(AV1_COMMON *const cm, const uint8_t *data, size_t sz)
{
...
// returns -1.
int aom_decode_frame_from_obus(struct AV1Decoder *pbi,
                               const uint8_t *data,
                               const uint8_t *data_end,
                               const uint8_t **p_data_end)
{
  ...
  while (!frame_decoding_finished && cm->error.error_code == AOM_CODEC_OK) {
    ...
    aom_codec_err_t status = aom_read_obu_header_and_size(
        data, bytes_available, pbi->is_annexb, &obu_header, &payload_size, &bytes_read);
    ...
    switch (obu_header.type) {
      ...
      case OBU_FRAME:
        ...
        if (!pbi->seen_frame_header || (cm->tiles.large_scale && !pbi->camera_frame_header_ready))
        {
      ...
      case OBU_TILE_GROUP:
        ...
        decoded_payload_size += read_one_tile_group_obu(pbi,
                                                        &rb,
                                                        is_first_tg_obu_received,
                                                        data + obu_payload_offset,
                                                        data + payload_size,
                                                        p_data_end,
      ...
      case OBU_TILE_LIST:
        ...
        decoded_payload_size = read_and_decode_one_tile_list(
            pbi, &rb, data, data + payload_size, p_data_end, &frame_decoding_finished);
        if (cm->error.error_code != AOM_CODEC_OK)
          return -1;
        break;
      case OBU_PADDING:
      ...
      default:
        ...
        if (payload_size > 0 && get_last_nonzero_byte(data, payload_size) == 0) {
          cm->error.error_code = AOM_CODEC_CORRUPT_FRAME;
          return -1;
        }
        decoded_payload_size = payload_size;
