...
static aom_image_t *img_alloc_helper(aom_image_t *img,
                                     aom_img_fmt_t fmt,
                                     unsigned int d_w,
                                     unsigned int d_h,
                                     ...
                                     void *cb_priv)
{
  ...
  if (!img_data) {
    const uint64_t alloc_size = (fmt & AOM_IMG_FMT_PLANAR) ?
                                    (uint64_t)(h + 2 * border) * stride_in_bytes * bps /
                                        bit_depth :
...
aom_image_t *aom_img_alloc(
    aom_image_t *img, aom_img_fmt_t fmt, unsigned int d_w, unsigned int d_h, unsigned int align)
{
  return img_alloc_helper(img, fmt, d_w, d_h, align, align, 1, 0, NULL, NULL, NULL);
}

aom_image_t *aom_img_alloc_with_cb(aom_image_t *img,
                                   aom_img_fmt_t fmt,
                                   unsigned int d_w,
                                   unsigned int d_h,
                                   unsigned int align,
                                   ...
                                   void *cb_priv)
{
  return img_alloc_helper(img, fmt, d_w, d_h, align, align, 1, 0, NULL, alloc_cb, cb_priv);
}
                          ...
                          unsigned char *img_data)
{
   ...
   * not NULL. */
  return img_alloc_helper(img, fmt, d_w, d_h, 1, stride_align, 1, 0, img_data, NULL, NULL);
}
...
aom_image_t *aom_img_alloc_with_border(aom_image_t *img,
                                       ...
                                       unsigned int border)
{
  return img_alloc_helper(img, fmt, d_w, d_h, align, align, size_align, border, NULL, NULL, NULL);
}
                     ...
                     unsigned int border)
{
  ...
  if (x + w <= img->w && y + h <= img->h) {
    ...
    if (!(img->fmt & AOM_IMG_FMT_PLANAR)) {
      img->planes[AOM_PLANE_PACKED] = img->img_data + x * img->bps / 8 +
                                      y * img->stride[AOM_PLANE_PACKED];
    }
    else {
      const int bytes_per_sample = (img->fmt & AOM_IMG_FMT_HIGHBITDEPTH) ? 2 : 1;
      data = img->img_data;

      img->planes[AOM_PLANE_Y] = data + x * bytes_per_sample + y * img->stride[AOM_PLANE_Y];
      ...
      if (!(img->fmt & AOM_IMG_FMT_UV_FLIP)) {
        img->planes[AOM_PLANE_U] = data + uv_x * bytes_per_sample +
                                   uv_y * img->stride[AOM_PLANE_U];
        data += ((img->h >> img->y_chroma_shift) + 2 * uv_border_h) * img->stride[AOM_PLANE_U];
        img->planes[AOM_PLANE_V] = data + uv_x * bytes_per_sample +
...
{
  aom_metadata_array_t *arr = (aom_metadata_array_t *)calloc(1, sizeof(aom_metadata_array_t));
  if (!arr)
    return NULL;
  if (sz > 0) {
    arr->metadata_array = (aom_metadata_t **)calloc(sz, sizeof(aom_metadata_t *));
                         ...
                         aom_metadata_insert_flags_t insert_flag)
{
  ...
  aom_metadata_t *metadata = aom_img_metadata_alloc(type, data, sz, insert_flag);
  if (!metadata)
    return -1;
  aom_metadata_t **metadata_array = (aom_metadata_t **)realloc(
      img->metadata->metadata_array, (img->metadata->sz + 1) * sizeof(metadata));
  if (!metadata_array) {
