...
static const int avifColorPrimariesTableSize = sizeof(avifColorPrimariesTables) /
                                               sizeof(avifColorPrimariesTables[0]);
...
void avifColorPrimariesGetValues(avifColorPrimaries acp, float outPrimaries[8])
{
  for (int i = 0; i < avifColorPrimariesTableSize; ++i) {
    if (avifColorPrimariesTables[i].colorPrimariesEnum == acp) {
      memcpy(outPrimaries,
             avifColorPrimariesTables[i].primaries,
             sizeof(avifColorPrimariesTables[i].primaries));
      return;
    }
  }
  ...
  memcpy(outPrimaries,
         avifColorPrimariesTables[0].primaries,
         sizeof(avifColorPrimariesTables[0].primaries));
}
...
static const struct avifMatrixCoefficientsTable matrixCoefficientsTables[] = {
    ...
    //{ AVIF_MATRIX_COEFFICIENTS_BT2020_CL, "BT.2020 (constant luminance)", 0.2627f, 0.0593f }, //
    // FIXME: It is not an linear transformation.
    //{ AVIF_MATRIX_COEFFICIENTS_SMPTE2085, "ST 2085", 0.0f, 0.0f }, // FIXME: ST2085 can't
    // represent using Kr and Kb.
    //{ AVIF_MATRIX_COEFFICIENTS_CHROMA_DERIVED_CL, "Chromaticity-derived constant luminance
    // system", 0.0f, 0.0f } // FIXME: It is not an linear transformation.
    //{ AVIF_MATRIX_COEFFICIENTS_ICTCP, "BT.2100-0 ICtCp", 0.0f, 0.0f }, // FIXME: This can't
    // represent using Kr and Kb.
};

static const int avifMatrixCoefficientsTableSize = sizeof(matrixCoefficientsTables) /
                                                   sizeof(matrixCoefficientsTables[0]);
...
//
// Transfer characteristics are defined in ITU-T H.273
// https://www.itu.int/rec/T-REC-H.273-201612-S/en with formulas for linear to gamma conversion in
// Table 3. This is based on tongyuantongyu's implementation in
// https://github.com/AOMediaCodec/libavif/pull/444 with some fixes/changes in the first commit:
// - Fixed 5 transfer curves where toLinear and toGamma functions were swapped (470M, 470BG,
// Log100,
//   Log100Sqrt10 and SMPTE428)
// - 'avifToLinearLog100' and 'avifToLinearLog100Sqrt10' were modified to return the middle of the
//   range of linear values that are gamma-encoded to 0.0 in order to reduce the max round trip
//   error, based on vrabaud's change in
...
//   [0.0, 1000/203] respectively, where a value of 1.0 means SDR white brightness (203 nits), and
//   any value above 1.0 is brigther.
...
static const int avifTransferCharacteristicsTableSize = sizeof(transferCharacteristicsTables) /
                                                        sizeof(transferCharacteristicsTables[0]);
...
void avifColorPrimariesComputeYCoeffs(avifColorPrimaries colorPrimaries, float coeffs[3])
{
  ...
  float const kr =
      (rY * (wX * (gY * bZ - bY * gZ) + wY * (bX * gZ - gX * bZ) + wZ * (gX * bY - bX * gY))) /
      (wY * (rX * (gY * bZ - bY * gZ) + gX * (bY * rZ - rY * bZ) + bX * (rY * gZ - gY * rZ)));
  // (Eq. 32)
  float const kb =
      (bY * (wX * (rY * gZ - gY * rZ) + wY * (gX * rZ - rX * gZ) + wZ * (rX * gY - gX * rY))) /
      (wY * (rX * (gY * bZ - bY * gZ) + gX * (bY * rZ - rY * bZ) + bX * (rY * gZ - gY * rZ)));
  // (Eq. 33)
  coeffs[0] = kr;
  coeffs[2] = kb;
