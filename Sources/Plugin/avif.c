...
#define AVIF_VERSION_STRING \
  (STR(AVIF_VERSION_MAJOR) "." STR(AVIF_VERSION_MINOR) "." STR(AVIF_VERSION_PATCH))
...
avifImage *avifImageCreate(uint32_t width,
                           uint32_t height,
                           uint32_t depth,
...
avifResult avifImageSetViewRect(avifImage *dstImage,
                                const avifImage *srcImage,
                                const avifCropRect *rect)
{
  avifPixelFormatInfo formatInfo;
  avifGetPixelFormatInfo(srcImage->yuvFormat, &formatInfo);
  if ((rect->width > srcImage->width) || (rect->height > srcImage->height) ||
      (rect->x > (srcImage->width - rect->width)) || (rect->y > (srcImage->height - rect->height)))
  {
    return AVIF_RESULT_INVALID_ARGUMENT;
  }
  if (!formatInfo.monochrome &&
      ((rect->x & formatInfo.chromaShiftX) || (rect->y & formatInfo.chromaShiftY)))
  {
    return AVIF_RESULT_INVALID_ARGUMENT;
  }
  avifImageFreePlanes(dstImage,
                      AVIF_PLANES_ALL);  // dstImage->imageOwnsYUVPlanes and
                                         // dstImage->imageOwnsAlphaPlane set to AVIF_FALSE.
  avifImageCopyNoAlloc(dstImage, srcImage);
  ...
  if (srcImage->yuvPlanes[AVIF_CHAN_Y]) {
    for (int yuvPlane = AVIF_CHAN_Y; yuvPlane <= AVIF_CHAN_V; ++yuvPlane) {
      if (srcImage->yuvRowBytes[yuvPlane]) {
        const size_t planeX = (yuvPlane == AVIF_CHAN_Y) ? rect->x :
                                                          (rect->x >> formatInfo.chromaShiftX);
        const size_t planeY = (yuvPlane == AVIF_CHAN_Y) ? rect->y :
                                                          (rect->y >> formatInfo.chromaShiftY);
        dstImage->yuvPlanes[yuvPlane] = srcImage->yuvPlanes[yuvPlane] +
                                        planeY * srcImage->yuvRowBytes[yuvPlane] +
                                        planeX * pixelBytes;
  ...
  if (srcImage->alphaPlane) {
    dstImage->alphaPlane = srcImage->alphaPlane + (size_t)rect->y * srcImage->alphaRowBytes +
...
avifResult avifImageAllocatePlanes(avifImage *image, avifPlanesFlags planes)
{
  ...
  if ((planes & AVIF_PLANES_YUV) && (image->yuvFormat != AVIF_PIXEL_FORMAT_NONE)) {
    ...
    if (!info.monochrome) {
      ...
      const uint32_t shiftedW = (uint32_t)(((uint64_t)image->width + info.chromaShiftX) >>
                                           info.chromaShiftX);
      const uint32_t shiftedH = (uint32_t)(((uint64_t)image->height + info.chromaShiftY) >>
                                           info.chromaShiftY);
...
avifBool avifDimensionsTooLarge(uint32_t width,
                                uint32_t height,
                                ...
                                uint32_t imageDimensionLimit)
{
  ...
  if ((imageDimensionLimit != 0) &&
      ((width > imageDimensionLimit) || (height > imageDimensionLimit)))
  {
    return AVIF_TRUE;
  }
  return AVIF_FALSE;
...
void avifRGBImageSetDefaults(avifRGBImage *rgb, const avifImage *image)
{
  ...
  rgb->alphaPremultiplied =
      AVIF_FALSE;  // Most expect RGBA output to *not* be premultiplied. Those that do can opt-in
                   // by setting this to match image->alphaPremultiplied or forcing this to true
...

{
  ...
  if ((cropRect->x > (UINT32_MAX - cropRect->width)) ||
      ((cropRect->x + cropRect->width) > imageW) ||
      (cropRect->y > (UINT32_MAX - cropRect->height)) ||
  ...
  if ((yuvFormat == AVIF_PIXEL_FORMAT_YUV420) || (yuvFormat == AVIF_PIXEL_FORMAT_YUV422)) {
    if (((cropRect->x % 2) != 0) || ((cropRect->width % 2) != 0)) {
      avifDiagnosticsPrintf(diag,
                            "[Strict] crop rect X offset and width must both be even due to this "
                            "image's YUV subsampling");
      return AVIF_FALSE;
    }
  }
  if (yuvFormat == AVIF_PIXEL_FORMAT_YUV420) {
    if (((cropRect->y % 2) != 0) || ((cropRect->height % 2) != 0)) {
      avifDiagnosticsPrintf(diag,
                            "[Strict] crop rect Y offset and height must both be even due to this "
                            "image's YUV subsampling");
      return AVIF_FALSE;
    }
  }
                                             ...
                                             avifDiagnostics *diag)
{
  ...
  if ((imageW > INT32_MAX) || (imageH > INT32_MAX)) {
    avifDiagnosticsPrintf(
        diag, "[Strict] image width %u or height %u is greater than INT32_MAX", imageW, imageH);
    return AVIF_FALSE;
  }
  ...
  if ((cropX.n % cropX.d) != 0) {
    avifDiagnosticsPrintf(
        diag, "[Strict] calculated crop X offset %d/%d is not an integer", cropX.n, cropX.d);
    return AVIF_FALSE;
  }
  ...
  if ((cropY.n % cropY.d) != 0) {
    avifDiagnosticsPrintf(
        diag, "[Strict] calculated crop Y offset %d/%d is not an integer", cropY.n, cropY.d);
    return AVIF_FALSE;
  }
...
avifBool avifAreGridDimensionsValid(avifPixelFormat yuvFormat,
                                    uint32_t imageW,
                                    uint32_t imageH,
                                    uint32_t tileW,
                                    uint32_t tileH,
                                    avifDiagnostics *diag)
{
  ...
  // ISO/IEC 23000-22:2019, Section 7.3.11.4.2:
  //   - when the images are in the 4:2:2 chroma sampling format the horizontal tile offsets and
  //   widths,
  //     and the output width, shall be even numbers;
  //   - when the images are in the 4:2:0 chroma sampling format both the horizontal and vertical
  //   tile
  ...
  //   The chroma plane of the whole image is 325 pixels wide. The chroma plane of each cell is 33
  //   pixels wide. 33*10 - 325 gives 5 extra pixels with no specified destination in the
  //   reconstructed image.

  // Tile offsets are not enforced since they depend on tile size (ISO/IEC 23008-12:2017,
  // Section 6.6.2.3.1):
  //   The reconstructed image is formed by tiling the input images into a grid [...] without gap
  //   or overlap
  if ((((yuvFormat == AVIF_PIXEL_FORMAT_YUV420) || (yuvFormat == AVIF_PIXEL_FORMAT_YUV422)) &&
    ...
    if (!strcmp(entry->key, key)) {
      if (value) {
        ...
        if (csOptions->count > 0) {
          memmove(&csOptions->entries[i],
                  &csOptions->entries[i + 1],
                  (csOptions->count - i) * (size_t)csOptions->elementSize);
        }
      }
      return AVIF_RESULT_OK;
    }
...
static struct AvailableCodec *findAvailableCodec(avifCodecChoice choice,
