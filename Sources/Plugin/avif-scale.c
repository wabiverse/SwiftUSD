...
#endif
#include "Plugin/hioAvif/AVIF/src/src-libyuv/libyuv.h"
#if defined(__clang__)
#  pragma clang diagnostic pop
#endif

// This should be configurable and/or smarter. kFilterBox has the highest quality but is the
// slowest.
                                   ...
                                   avifDiagnostics *diag)
{
  ...
  if ((dstWidth == 0) || (dstHeight == 0)) {
    avifDiagnosticsPrintf(diag,
                          "avifImageScaleWithLimit requested invalid dst dimensions [%ux%u]",
                          dstWidth,
                          dstHeight);
  ...
  if (avifDimensionsTooLarge(dstWidth, dstHeight, imageSizeLimit, imageDimensionLimit)) {
    avifDiagnosticsPrintf(
        diag,
        "avifImageScaleWithLimit requested dst dimensions that are too large [%ux%u]",
        dstWidth,
        dstHeight);
  ...
  if (srcYUVPlanes[0] || srcAlphaPlane) {
    ...
    if (srcWidth > 16384) {
      avifDiagnosticsPrintf(
          diag,
          "avifImageScaleWithLimit requested invalid width scale for libyuv [%u -> %u]",
          srcWidth,
          dstWidth);
    ...
    if (srcHeight > 16384) {
      avifDiagnosticsPrintf(
          diag,
          "avifImageScaleWithLimit requested invalid height scale for libyuv [%u -> %u]",
          srcHeight,
          dstHeight);
      result = AVIF_RESULT_NOT_IMPLEMENTED;
      goto cleanup;
  ...
  if (srcYUVPlanes[0]) {
    ...
    if (allocationResult != AVIF_RESULT_OK) {
      avifDiagnosticsPrintf(
          diag, "Allocation of YUV planes failed: %s", avifResultToString(allocationResult));
...
#if LIBYUV_VERSION >= 1880
        const int failure = ScalePlane(srcPlane,
                                       srcStride,
                                       srcW,
                                       srcH,
                                       dstPlane,
  ...
  if (srcAlphaPlane) {
    ...
    if (allocationResult != AVIF_RESULT_OK) {
      avifDiagnosticsPrintf(
          diag, "Allocation of alpha plane failed: %s", avifResultToString(allocationResult));
...
avifResult avifImageScale(avifImage *image,
                          uint32_t dstWidth,
                          uint32_t dstHeight,
                          avifDiagnostics *diag)
{
  avifDiagnosticsClearError(diag);
  return avifImageScaleWithLimit(image,
                                 dstWidth,
                                 dstHeight,
