...
avifBool avifROStreamSkip(avifROStream *stream, size_t byteCount)
{
  ...
  if (!avifROStreamHasBytesLeft(stream, byteCount)) {
    avifDiagnosticsPrintf(stream->diag,
                          "%s: Failed to skip %zu bytes, truncated data?",
                          stream->diagContext,
                          byteCount);
    return AVIF_FALSE;
  }
...
avifBool avifROStreamRead(avifROStream *stream, uint8_t *data, size_t size)
{
  ...
  if (!avifROStreamHasBytesLeft(stream, size)) {
    avifDiagnosticsPrintf(
        stream->diag, "%s: Failed to read %zu bytes, truncated data?", stream->diagContext, size);
    return AVIF_FALSE;
  }
...
avifBool avifROStreamReadUX8(avifROStream *stream, uint64_t *v, uint64_t factor)
{
  ...
  if (factor == 0) {
    ...
    *v = tmp;
    ...
    // Unsupported factor
    avifDiagnosticsPrintf(stream->diag,
                          "%s: Failed to read UX8 value; Unsupported UX8 factor [%" PRIu64 "]",
                          stream->diagContext,
                          factor);
    return AVIF_FALSE;
  }
  ...
  *v = 0;
  while (bitCount) {
    if (stream->numUsedBitsInPartialByte == 0) {
      AVIF_CHECK(
          avifROStreamSkip(stream, sizeof(uint8_t)));  // Book a new partial byte in the stream.
    }
    ...
    const uint32_t bits = (*packedBits >> (8 - stream->numUsedBitsInPartialByte)) &
                          ((1 << numBits) - 1);
...
avifBool avifROStreamReadString(avifROStream *stream, char *output, size_t outputSize)
{
  ...
  if (!foundNullTerminator) {
    avifDiagnosticsPrintf(stream->diag,
                          "%s: Failed to find a NULL terminator when reading a string",
                          stream->diagContext);
    return AVIF_FALSE;
  }
...
avifBool avifROStreamReadBoxHeaderPartial(avifROStream *stream, avifBoxHeader *header)
{
  ...
  if ((size < bytesRead) || ((size - bytesRead) > SIZE_MAX)) {
    avifDiagnosticsPrintf(
        stream->diag, "%s: Header size overflow check failure", stream->diagContext);
    return AVIF_FALSE;
  }
...
avifBool avifROStreamReadBoxHeader(avifROStream *stream, avifBoxHeader *header)
{
  ...
  if (header->size > avifROStreamRemainingBytes(stream)) {
    avifDiagnosticsPrintf(
        stream->diag, "%s: Child box too large, possibly truncated data", stream->diagContext);
    return AVIF_FALSE;
  }
  return AVIF_TRUE;
}
...
avifBool avifROStreamReadAndEnforceVersion(avifROStream *stream, uint8_t enforcedVersion)
{
  ...
  if (version != enforcedVersion) {
    avifDiagnosticsPrintf(stream->diag,
                          "%s: Expecting box version %u, got version %u",
                          stream->diagContext,
                          enforcedVersion,
                          version);
    return AVIF_FALSE;
