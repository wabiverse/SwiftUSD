...
#include "Plugin/hioAvif/aom/av1/common/convolve.h"
#include "Plugin/hioAvif/aom/av1/common/filter.h"
...
void av1_convolve_2d_sobel_y_c(const uint8_t *src,
                               int src_stride,
                               double *dst,
                               int dst_stride,
                               int w,
    ...
    for (int x = 0; x < w; ++x) {
      ...
      if (conv_params->do_average) {
        ...
        tmp -= (1 << (offset_bits - conv_params->round_1)) +
               (1 << (offset_bits - conv_params->round_1 - 1));
        dst[y * dst_stride + x] = clip_pixel(ROUND_POWER_OF_TWO(tmp, round_bits));
      }
      else {
...
static void convolve_2d_scale_wrapper(const uint8_t *src,
                                      int src_stride,
                                      uint8_t *dst,
                                      int dst_stride,
                                      int w,
...
static void convolve_2d_facade_compound(const uint8_t *src,
                                        int src_stride,
                                        uint8_t *dst,
                                        int dst_stride,
                                        int w,
                                        ...
                                        ConvolveParams *conv_params)
{
  ...
  if (!need_x && !need_y) {
    av1_dist_wtd_convolve_2d_copy(src, src_stride, dst, dst_stride, w, h, conv_params);
  }
  else if (need_x && !need_y) {
    av1_dist_wtd_convolve_x(
        src, src_stride, dst, dst_stride, w, h, filter_params_x, subpel_x_qn, conv_params);
  }
  else if (!need_x && need_y) {
    av1_dist_wtd_convolve_y(
        src, src_stride, dst, dst_stride, w, h, filter_params_y, subpel_y_qn, conv_params);
  }
  else {
    assert(need_y && need_x);
    av1_dist_wtd_convolve_2d(src,
                             src_stride,
                             dst,
                             dst_stride,
                             w,
...
static void convolve_2d_facade_single(const uint8_t *src,
                                      int src_stride,
                                      uint8_t *dst,
                                      int dst_stride,
                                      int w,
                                      ...
                                      ConvolveParams *conv_params)
{
  ...
  if (!need_x && !need_y) {
    ...
    av1_convolve_y_sr(src, src_stride, dst, dst_stride, w, h, filter_params_y, subpel_y_qn);
  }
  else {
    assert(need_x && need_y);
    av1_convolve_2d_sr(src,
                       src_stride,
                       dst,
                       dst_stride,
                       w,
...
void av1_convolve_2d_facade(const uint8_t *src,
                            int src_stride,
                            uint8_t *dst,
                            int dst_stride,
                            int w,
                            ...
                            ConvolveParams *conv_params)
{
  ...
  if (scaled) {
    ...
    convolve_2d_facade_compound(src,
                                src_stride,
                                dst,
                                dst_stride,
                                w,
                                 ...
                                 int bd)
{
  ...
  for (int y = 0; y < im_h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      assert(0 <= sum && sum < (1 << (bd + FILTER_BITS + 1)));
      im_block[y * im_stride + x] = ROUND_POWER_OF_TWO(sum, conv_params->round_0);
    }
  }
  ...
  for (y = 0; y < h; ++y) {
    for (x = 0; x < w; ++x) {
      ...
      if (conv_params->do_average) {
        ...
        tmp -= (1 << (offset_bits - conv_params->round_1)) +
               (1 << (offset_bits - conv_params->round_1 - 1));
        dst[y * dst_stride + x] = clip_pixel_highbd(ROUND_POWER_OF_TWO(tmp, round_bits), bd);
      }
      else {
        dst16[y * dst16_stride + x] = res;
      }
...
static void highbd_convolve_2d_facade_compound(const uint16_t *src,
                                               int src_stride,
                                               uint16_t *dst,
                                               int dst_stride,
                                               const int w,
                                               ...
                                               int bd)
{
  ...
  if (!need_x && !need_y) {
    ...
    av1_highbd_dist_wtd_convolve_y(
        src, src_stride, dst, dst_stride, w, h, filter_params_y, subpel_y_qn, conv_params, bd);
  }
  else {
    assert(need_x && need_y);
    av1_highbd_dist_wtd_convolve_2d(src,
                                    src_stride,
                                    dst,
                                    dst_stride,
                                    w,
...
static void highbd_convolve_2d_facade_single(const uint16_t *src,
                                             int src_stride,
                                             uint16_t *dst,
                                             int dst_stride,
                                             const int w,
                                             ...
                                             int bd)
{
  ...
  if (!need_x && !need_y) {
    ...
    av1_highbd_convolve_y_sr(
        src, src_stride, dst, dst_stride, w, h, filter_params_y, subpel_y_qn, bd);
  }
  else {
    assert(need_x && need_y);
    av1_highbd_convolve_2d_sr(src,
                              src_stride,
                              dst,
                              dst_stride,
                              w,
...
void av1_highbd_convolve_2d_facade(const uint8_t *src8,
                                   int src_stride,
                                   uint8_t *dst8,
                                   int dst_stride,
                                   int w,
                                   ...
                                   int bd)
{
  ...
  const InterpFilterParams *filter_params_x = need_filter_params_x ? interp_filters[0] : NULL;
  const InterpFilterParams *filter_params_y = need_filter_params_y ? interp_filters[1] : NULL;
  ...
  if (scaled) {
    ...
    av1_highbd_convolve_2d_scale(src,
                                 src_stride,
                                 dst,
                                 dst_stride,
                                 w,
    ...
    highbd_convolve_2d_facade_compound(src,
                                       src_stride,
                                       dst,
                                       dst_stride,
                                       w,
    ...
    highbd_convolve_2d_facade_single(src,
                                     src_stride,
                                     dst,
                                     dst_stride,
                                     w,
...
#if CONFIG_AV1_HIGHBITDEPTH
static INLINE int highbd_horz_scalar_product(const uint16_t *a, const int16_t *b)
{
  int sum = 0;
  for (int k = 0; k < SUBPEL_TAPS; ++k)
    sum += a[k] * b[k];
...
static void convolve_add_src_horiz_hip(const uint8_t *src,
...
static void convolve_add_src_vert_hip(const uint16_t *src,
                                      ptrdiff_t src_stride,
                                      uint8_t *dst,
                                      ptrdiff_t dst_stride,
                                      ...
                                      int round1_bits)
{
  ...
  for (int x = 0; x < w; ++x) {
    ...
    for (int y = 0; y < h; ++y) {
      ...
      const int rounding = ((int)src_y[(SUBPEL_TAPS / 2 - 1) * src_stride] << FILTER_BITS) -
                           (1 << (bd + round1_bits - 1));
      const int sum = highbd_vert_scalar_product(src_y, src_stride, y_filter) + rounding;
                                   ...
                                   const ConvolveParams *conv_params)
{
  ...
  const int intermediate_height = (((h - 1) * y_step_q4 + y0_q4) >> SUBPEL_BITS) + SUBPEL_TAPS - 1;
...
#if CONFIG_AV1_HIGHBITDEPTH
static void highbd_convolve_add_src_horiz_hip(const uint8_t *src8,
                                              ptrdiff_t src_stride,
                                              uint16_t *dst,
                                              ptrdiff_t dst_stride,
                                              const InterpKernel *x_filters,
...
static void highbd_convolve_add_src_vert_hip(const uint16_t *src,
                                             ptrdiff_t src_stride,
                                             uint8_t *dst8,
                                             ptrdiff_t dst_stride,
                                             const InterpKernel *y_filters,
