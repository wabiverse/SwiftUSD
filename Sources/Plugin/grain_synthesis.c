...
static void init_arrays(const aom_film_grain_t *params,
                        int luma_stride,
                        int chroma_stride,
                        int ***pred_pos_luma_p,
                        int ***pred_pos_chroma_p,
                        ...
                        int chroma_subsamp_x)
{
  ...
  *y_col_buf = (int *)aom_malloc(sizeof(**y_col_buf) * (luma_subblock_size_y + 2) * 2);
  *cb_col_buf = (int *)aom_malloc(sizeof(**cb_col_buf) *
                                  (chroma_subblock_size_y + (2 >> chroma_subsamp_y)) *
                                  (2 >> chroma_subsamp_x));
  *cr_col_buf = (int *)aom_malloc(sizeof(**cr_col_buf) *
                                  (chroma_subblock_size_y + (2 >> chroma_subsamp_y)) *
                                  (2 >> chroma_subsamp_x));

  *luma_grain_block = (int *)aom_malloc(sizeof(**luma_grain_block) * luma_grain_samples);
  *cb_grain_block = (int *)aom_malloc(sizeof(**cb_grain_block) * chroma_grain_samples);
  *cr_grain_block = (int *)aom_malloc(sizeof(**cr_grain_block) * chroma_grain_samples);
}

static void dealloc_arrays(const aom_film_grain_t *params,
                           int ***pred_pos_luma,
                           int ***pred_pos_chroma,
                           int **luma_grain_block,
                           int **cb_grain_block,
...
static int generate_luma_grain_block(const aom_film_grain_t *params,
                                     int **pred_pos_luma,
                                     int *luma_grain_block,
...
{
  if (params->num_y_points == 0) {
    memset(luma_grain_block, 0, sizeof(*luma_grain_block) * luma_block_size_y * luma_grain_stride);
    return 0;
  }
  ...
  for (int i = 0; i < luma_block_size_y; i++)
    for (int j = 0; j < luma_block_size_x; j++)
      luma_grain_block[i * luma_grain_stride + j] =
          (gaussian_sequence[get_random_number(gauss_bits)] + ((1 << gauss_sec_shift) >> 1)) >>
          gauss_sec_shift;
    ...
    for (int j = left_pad; j < luma_block_size_x - right_pad; j++) {
      ...
      for (int pos = 0; pos < num_pos_luma; pos++) {
        wsum = wsum + params->ar_coeffs_y[pos] *
                          luma_grain_block[(i + pred_pos_luma[pos][0]) * luma_grain_stride + j +
                                           pred_pos_luma[pos][1]];
      }
...
// Return 0 for success, -1 for failure
static int generate_chroma_grain_blocks(const aom_film_grain_t *params,
                                        //                                  int** pred_pos_luma,
                                        int **pred_pos_chroma,
                                        int *luma_grain_block,
                                        int *cb_grain_block,
...
static void init_scaling_function(const int scaling_points[][2], int num_points, int scaling_lut[])
{
  if (num_points == 0)
    return;
  ...
  for (int point = 0; point < num_points - 1; point++) {
    ...
    for (int x = 0; x < delta_x; x++) {
      scaling_lut[scaling_points[point][0] + x] = scaling_points[point][1] +
                               ...
                               int mc_identity)
{
  ...
  int apply_cb = (params->num_cb_points > 0 || params->chroma_scaling_from_luma) ? 1 : 0;
  int apply_cr = (params->num_cr_points > 0 || params->chroma_scaling_from_luma) ? 1 : 0;
    ...
    for (int j = 0; j < (half_luma_width << (1 - chroma_subsamp_x)); j++) {
      ...
      if (apply_cb) {
        cb[i * chroma_stride + j] = clamp(
            cb[i * chroma_stride + j] + ((scale_LUT(scaling_lut_cb,
      ...
      if (apply_cr) {
        cr[i * chroma_stride + j] = clamp(
            cr[i * chroma_stride + j] + ((scale_LUT(scaling_lut_cr,
    ...
    for (int i = 0; i < (half_luma_height << 1); i++) {
      for (int j = 0; j < (half_luma_width << 1); j++) {
        luma[i * luma_stride + j] = clamp(
            luma[i * luma_stride + j] + ((scale_LUT(scaling_lut_y, luma[i * luma_stride + j], 8) *
...
static void add_noise_to_block_hbd(const aom_film_grain_t *params,
                                   uint16_t *luma,
                                   uint16_t *cb,
                                   uint16_t *cr,
                                   int luma_stride,
                                   ...
                                   int mc_identity)
{
  ...
  int apply_y = params->num_y_points > 0 ? 1 : 0;
  int apply_cb = (params->num_cb_points > 0 || params->chroma_scaling_from_luma) > 0 ? 1 : 0;
  int apply_cr = (params->num_cr_points > 0 || params->chroma_scaling_from_luma) > 0 ? 1 : 0;
...
static void copy_rect(uint8_t *src,
                      int src_stride,
                      uint8_t *dst,
                      int dst_stride,
                      int width,
...
static void copy_area(int *src, int src_stride, int *dst, int dst_stride, int width, int height)
{
  while (height) {
...
static void extend_even(
    uint8_t *dst, int dst_stride, int width, int height, int use_high_bit_depth)
{
  ...
  if (use_high_bit_depth) {
    ...
    width = (width + 1) & (~1);
    if (height & 1) {
      memcpy(&dst16[height * dst16_stride],
    ...
    width = (width + 1) & (~1);
    if (height & 1) {
      memcpy(&dst[height * dst_stride], &dst[(height - 1) * dst_stride], sizeof(*dst) * width);
    }
  }
}

static void ver_boundary_overlap(int *left_block,
                                 int left_stride,
                                 int *right_block,
                                 int right_stride,
                                 int *dst_block,
                                 ...
                                 int height)
{
  if (width == 1) {
    ...
    while (height) {
      dst_block[0] = clamp(
          (27 * left_block[0] + 17 * right_block[0] + 16) >> 5, grain_min, grain_max);
      dst_block[1] = clamp(
          (17 * left_block[1] + 27 * right_block[1] + 16) >> 5, grain_min, grain_max);
      left_block += left_stride;
      right_block += right_stride;
      dst_block += dst_stride;
      --height;
...
static void hor_boundary_overlap(int *top_block,
                                 int top_stride,
                                 int *bottom_block,
                                 int bottom_stride,
                                 int *dst_block,
                                 ...
                                 int height)
{
  if (height == 1) {
    ...
    while (width) {
      dst_block[0] = clamp(
          (27 * top_block[0] + 17 * bottom_block[0] + 16) >> 5, grain_min, grain_max);
      dst_block[dst_stride] = clamp(
          (17 * top_block[top_stride] + 27 * bottom_block[bottom_stride] + 16) >> 5,
          grain_min,
          grain_max);
      ++top_block;
      ++bottom_block;
...
int av1_add_film_grain(const aom_film_grain_t *params, const aom_image_t *src, aom_image_t *dst)
{
  uint8_t *luma, *cb, *cr;
  ...
  copy_rect(src->planes[AOM_PLANE_Y],
            src->stride[AOM_PLANE_Y],
            dst->planes[AOM_PLANE_Y],
  ...
  if (!src->monochrome) {
    copy_rect(src->planes[AOM_PLANE_U],
              src->stride[AOM_PLANE_U],
              dst->planes[AOM_PLANE_U],
              dst->stride[AOM_PLANE_U],
              width >> chroma_subsamp_x,
              height >> chroma_subsamp_y,
              use_high_bit_depth);

    copy_rect(src->planes[AOM_PLANE_V],
              src->stride[AOM_PLANE_V],
              dst->planes[AOM_PLANE_V],
              dst->stride[AOM_PLANE_V],
              width >> chroma_subsamp_x,
              height >> chroma_subsamp_y,
              use_high_bit_depth);
  }
  ...
  return av1_add_film_grain_run(params,
                                luma,
                                cb,
                                cr,
                                height,
...
int av1_add_film_grain_run(const aom_film_grain_t *params,
                           uint8_t *luma,
                           uint8_t *cb,
                           uint8_t *cr,
                           int height,
                           ...
                           int mc_identity)
{
  ...
  int luma_block_size_y = top_pad + 2 * ar_padding + luma_subblock_size_y * 2 + bottom_pad;
  int luma_block_size_x = left_pad + 2 * ar_padding + luma_subblock_size_x * 2 + 2 * ar_padding +
                          right_pad;
  ...
  if (generate_luma_grain_block(params,
                                pred_pos_luma,
                                luma_grain_block,
                                luma_block_size_y,
                                luma_block_size_x,
  ...
  if (generate_chroma_grain_blocks(params,
                                   //                               pred_pos_luma,
                                   pred_pos_chroma,
                                   luma_grain_block,
                                   cb_grain_block,
  ...
  for (int y = 0; y < height / 2; y += (luma_subblock_size_y >> 1)) {
    ...
    for (int x = 0; x < width / 2; x += (luma_subblock_size_x >> 1)) {
      ...
      if (overlap && x) {
        ...
        ver_boundary_overlap(cb_col_buf,
                             2 >> chroma_subsamp_x,
        ...
        ver_boundary_overlap(cr_col_buf,
                             2 >> chroma_subsamp_x,
      ...
      if (use_high_bit_depth) {
        add_noise_to_block_hbd(
            ...
            luma_stride,
            chroma_stride,
            luma_grain_block + (luma_offset_y + (i << 1)) * luma_grain_stride + luma_offset_x +
                (j << 1),
            cb_grain_block +
