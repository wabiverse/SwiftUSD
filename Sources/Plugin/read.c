...
#include "Plugin/hioAvif/AVIF/src/avif/internal.h"
...
// Temporary storage for ipco/stsd contents until they can be associated and memcpy'd to an
// avifDecoderItem
...
static const avifProperty *avifPropertyArrayFind(const avifPropertyArray *properties,
                                                 const char *type)
{
...
typedef struct avifDecoderItem {
  ...
  uint8_t type[4];
  size_t size;
  avifBool idatStored;  // If true, offset is relative to the associated meta box's idat box (iloc
  ...
  avifRWData mergedExtents;  // if set, is a single contiguous block of this item's extents (unused
                             // when extents.count == 1)
  ...
  uint32_t dimgIdx;   // if dimgForId is non-zero, this is the zero-based index of this item in the
                      // list of Item #{dimgForID}'s dimg
  uint32_t premByID;  // if non-zero, this item is premultiplied by Item #{premByID}
  avifBool hasUnsupportedEssentialProperty;  // If true, this item cites a property flagged as
                                             // 'essential' that libavif doesn't support (yet).
                                             // Ignore the item, if so.
  avifBool ipmaSeen;     // if true, this item already received a property association
  avifBool progressive;  // if true, this item has progressive layers (a1lx), but does not select a
                         // specific layer (the layer_id value in lsel is set to 0xFFFF)
#if defined(AVIF_ENABLE_EXPERIMENTAL_AVIR)
  avifPixelFormat coniPixelFormat;  // Set from the CondensedImageBox, if present
                                    // (AVIF_PIXEL_FORMAT_NONE otherwise)
#endif
} avifDecoderItem;
...
typedef struct avifSampleTable {
  ...
  uint32_t allSamplesSize;  // If this is non-zero, sampleSizes will be empty and all samples will
                            // be this size
} avifSampleTable;
...
{
  for (uint32_t i = 0; i < sampleTable->sampleDescriptions.count; ++i) {
    const avifCodecType codecType = avifGetCodecType(
        sampleTable->sampleDescriptions.description[i].format);
...
{
  avifCodecDecodeInput *decodeInput = (avifCodecDecodeInput *)avifAlloc(
      sizeof(avifCodecDecodeInput));
  if (decodeInput == NULL) {
    return NULL;
  }
...
static uint32_t avifGetSampleCountOfChunk(const avifSampleTableSampleToChunkArray *sampleToChunks,
                                          uint32_t chunkIndex)
{
  uint32_t sampleCount = 0;
  for (int sampleToChunkIndex = sampleToChunks->count - 1; sampleToChunkIndex >= 0;
       --sampleToChunkIndex)
  {
    const avifSampleTableSampleToChunk *sampleToChunk =
        &sampleToChunks->sampleToChunk[sampleToChunkIndex];
                                                          ...
                                                          avifDiagnostics *diag)
{
    ...
    for (uint32_t sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex) {
      ...
      if (sampleSize == 0) {
        ...
        avifSampleTableSampleSize *sampleSizePtr =
            &sampleTable->sampleSizes.sampleSize[sampleSizeIndex];
  ...
  for (uint32_t syncSampleIndex = 0; syncSampleIndex < sampleTable->syncSamples.count;
       ++syncSampleIndex)
  {
    uint32_t frameIndex = sampleTable->syncSamples.syncSample[syncSampleIndex].sampleNumber -
                          1;  // sampleNumber is 1-based
                                                          ...
                                                          avifDiagnostics *diag)
{
  ...
  // equivalent to layer_id == 0xFFFF during the transitional period. Remove !lselProp when the
  // test images have been updated to the v1.1.0 spec.
  ...
  if (lselProp && (lselProp->u.lsel.layerID != 0xFFFF)) {
    ...
    size_t sampleSize = 0;
    if (layerCount > 0) {
      // Optimization: If we're selecting a layer that doesn't require the entire image's payload
      // (hinted via the a1lx box)

      if (lselProp->u.lsel.layerID >= layerCount) {
        avifDiagnosticsPrintf(diag,
                              "lsel property requests layer index [%u] which isn't present in "
                              "a1lx property ([%u] layers)",
                              ...
                              avifDiagnostics *diagnostics)
{
  ...
  if (*uniqueBoxFlags & flag) {
    ...
    avifDiagnosticsPrintf(diagnostics,
                          "Box[%s] contains a duplicate unique box of type '%s'",
                          parentBoxType,
                          boxType);
    return AVIF_FALSE;
  }
...
typedef struct avifTile {
  ...
  avifCodecType codecType;
  // This may point to a codec that it owns or point to a shared codec that it does not own. In the
  // shared case, this will point to one of the avifCodec instances in avifDecoderData.
...
// This holds one "meta" box (from the BMFF and HEIF standards) worth of relevant-to-AVIF
// information.
...
// * If a meta box is parsed from inside of a track ("trak") box, any metadata (EXIF/XMP) items
// inside
...
typedef struct avifMeta {
  ...
  // (single image color planes, alpha planes, EXIF, XMP, etc). Each item has a unique integer ID
  // >1, and is defined by a series of child boxes in a meta box:
  //  * iloc - location:     byte offset to item data, item size in bytes
  //  * iinf - information:  type of item (color planes, alpha plane, EXIF, XMP)
  //  * ipco - properties:   dimensions, aspect ratio, image transformations, references to other
  //  items
  //  * ipma - associations: Attaches an item in the properties list to a given item
  //
  // Items are lazily created in this array when any of the above boxes refer to one by a new
  // (unseen) ID, and are then further modified/updated as new information for an item's ID is
  // parsed.
...
typedef struct avifDecoderData {
  ...
  // When decoding AVIF images with grid, use a single decoder instance for all the tiles instead
  // of creating a decoder instance for each tile. If that is the case, |codec| will be used by all
  // the tiles.
  //
  // There are some edge cases where we will still need multiple decoder instances:
  // * For animated AVIF with alpha, we will need two instances (one for the color planes and one
  // for the alpha plane since they are both
  //   encoded as separate video sequences). In this case, |codec| will be used for the color
  //   planes and |codecAlpha| will be used for the alpha plane.
  // * For grid images with multiple layers. In this case, each tile will need its own decoder
  // instance since there would be
  //   multiple layers in each tile. In this case, |codec| and |codecAlpha| are not used and each
  //   tile will have its own decoder instance.
  // * For grid images where the operating points of all the tiles are not the same. In this case,
  ...
  const avifSampleTable *sourceSampleTable;  // NULL unless (source == AVIF_DECODER_SOURCE_TRACKS),
                                             // owned by an avifTrack
  avifBool cicpSet;  // True if avifDecoder's image has had its CICP set correctly yet.
                     ...
                     //   The colour information property takes precedence over any colour
                     //   information in the image bitstream, i.e. if the property is present,
                     //   colour information in the bitstream shall be ignored.
} avifDecoderData;
...
static void avifDecoderDataResetCodec(avifDecoderData *data)
{
  for (unsigned int i = 0; i < data->tiles.count; ++i) {
    ...
    if (tile->image) {
      avifImageFreePlanes(tile->image,
                          AVIF_PLANES_ALL);  // forget any pointers into codec image buffers
    }
    if (tile->codec) {
...
static avifResult avifDecoderItemMaxExtent(const avifDecoderItem *item,
                                           ...
                                           avifExtent *outExtent)
{
  ...
  size_t remainingBytes =
      sample->size;  // This may be smaller than item->size if the item is progressive
                                                    ...
                                                    const avifStrictFlags strictFlags)
{
  ...
  if (!configProp) {
    ...
    avifDiagnosticsPrintf(diag,
                          "Item ID %u of type '%.4s' is missing mandatory %s property",
                          item->id,
  ...
  if (!memcmp(item->type, "grid", 4)) {
    for (uint32_t i = 0; i < item->meta->items.count; ++i) {
      ...
      const avifProperty *tileConfigProp = avifPropertyArrayFind(&tile->properties,
                                                                 configPropName);
      if (!tileConfigProp) {
        avifDiagnosticsPrintf(diag,
      ...
      {
        avifDiagnosticsPrintf(diag,
                              "The fields of the %s property of tile item ID %u of type '%.4s' "
                              "differs from other tiles",
                              configPropName,
                              tile->id,
                              (const char *)tile->type);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
  ...
  if (pixiProp) {
    ...
    for (uint8_t i = 0; i < pixiProp->u.pixi.planeCount; ++i) {
      if (pixiProp->u.pixi.planeDepths[i] != configDepth) {
        ...
        avifDiagnosticsPrintf(diag,
                              "Item ID %u depth specified by pixi property [%u] does not match %s "
                              "property depth [%u]",
                              item->id,
  ...
  if (strictFlags & AVIF_STRICT_CLAP_VALID) {
    ...
    if (clapProp) {
      ...
      if (!ispeProp) {
        avifDiagnosticsPrintf(diag,
                              "[Strict] Item ID %u is missing an ispe property, so its clap "
      ...
      avifBool validClap = avifCropRectConvertCleanApertureBox(
          &cropRect, &clapProp->u.clap, imageW, imageH, configFormat, diag);
      if (!validClap) {
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
    }
                                      ...
                                      avifDiagnostics *diag)
{
  ...
  if (item->idatStored) {
    ...
    if (item->meta->idat.size > 0) {
      ...
      // no associated idat box was found in the meta box, bail out
      avifDiagnosticsPrintf(
          diag, "Item ID %u is stored in an idat, but no associated idat box was found", item->id);
      return AVIF_RESULT_NO_CONTENT;
    }
  }
  ...
  if ((io->sizeHint > 0) && (item->size > io->sizeHint)) {
    ...
    avifDiagnosticsPrintf(
        diag, "Item ID %u reported size failed size hint sanity check. Truncated data?", item->id);
    return AVIF_RESULT_TRUNCATED_DATA;
  }
  ...
  const size_t readOutputSize = (partialByteCount && (partialByteCount < maxOutputSize)) ?
                                    partialByteCount :
                                    maxOutputSize;
  ...
  for (uint32_t extentIter = 0; extentIter < item->extents.count; ++extentIter) {
    ...
    if (idatBuffer) {
      if (extent->offset > idatBuffer->size) {
        avifDiagnosticsPrintf(
            diag, "Item ID %u has impossible extent offset in idat buffer", item->id);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
      ...
      if (extent->size > idatBuffer->size - extentOffset) {
        avifDiagnosticsPrintf(
            diag, "Item ID %u has impossible extent size in idat buffer", item->id);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
      ...
      if ((io->sizeHint > 0) && (extent->offset > io->sizeHint)) {
        avifDiagnosticsPrintf(
            diag,
            "Item ID %u extent offset failed size hint sanity check. Truncated data?",
            item->id);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
  ...
  if (remainingBytes != 0) {
    ...
    avifDiagnosticsPrintf(
        diag, "Item ID %u has %zu unexpected trailing bytes", item->id, remainingBytes);
    return AVIF_RESULT_TRUNCATED_DATA;
  }
                                                    ...
                                                    avifItemCategory itemCategory)
{
  ...
  for (uint32_t i = 0; i < gridItem->meta->items.count; ++i) {
    ...
    // According to HEIF (ISO 14496-12), Section 6.6.2.3.1, the SingleItemTypeReferenceBox of type
    // 'dimg' identifies the input images of the derived image item of type 'grid'. Since the
    // reference_count shall be equal to rows*columns, unknown tile item types cannot be skipped
    // but must be considered as errors.
    const avifCodecType tileCodecType = avifGetCodecType(item->type);
    if (tileCodecType == AVIF_CODEC_TYPE_UNKNOWN) {
      avifDiagnosticsPrintf(&decoder->diag,
                            "Tile item ID %u has an unknown item type '%.4s'",
                            item->id,
                            (const char *)item->type);
    ...
    const avifTile *tile = avifDecoderDataCreateTile(decoder->data,
                                                     tileCodecType,
                                                     item->width,
                                                     item->height,
                                                     avifDecoderItemOperatingPoint(item));
    ...
    if (firstTileItem == NULL) {
      ...
      // Adopt the configuration property of the first image item tile, so that it can be queried
      // from the top-level color/alpha item during avifDecoderReset().
      ...
      if (!srcProp) {
        avifDiagnosticsPrintf(
            &decoder->diag, "Grid image's first tile is missing an %s property", configPropName);
        return AVIF_RESULT_INVALID_IMAGE_GRID;
      }
      ...
      *dstProp = *srcProp;
      ...
      if (itemCategory == AVIF_ITEM_COLOR && item->progressive) {
        gridItem->progressive =
            AVIF_TRUE;  // Propagate the progressive status to the top-level grid item.
      }
    }
...
// Allocates the dstImage based on the grid image requirements. Also verifies some spec compliance
// rules for grids.
static avifResult avifDecoderDataAllocateGridImagePlanes(avifDecoderData *data,
                                                         ...
                                                         avifImage *dstImage)
{
  ...
  if (((tile->image->width * grid->columns) < grid->outputWidth) ||
      ((tile->image->height * grid->rows) < grid->outputHeight))
  {
    avifDiagnosticsPrintf(data->diag,
                          "Grid image tiles do not completely cover the image (HEIF (ISO/IEC "
                          "23008-12:2017), Section 6.6.2.3.1)");
    return AVIF_RESULT_INVALID_IMAGE_GRID;
  }
  // Tiles in the rightmost column and bottommost row must overlap the reconstructed image grid
  // canvas. See MIAF (ISO/IEC 23000-22:2019), Section 7.3.11.4.2, Figure 2.
  if (((tile->image->width * (grid->columns - 1)) >= grid->outputWidth) ||
      ((tile->image->height * (grid->rows - 1)) >= grid->outputHeight))
  {
    avifDiagnosticsPrintf(data->diag,
                          "Grid image tiles in the rightmost column and bottommost row do not "
                          "overlap the reconstructed image grid canvas. See MIAF (ISO/IEC "
                          "23000-22:2019), Section 7.3.11.4.2, Figure 2");
    return AVIF_RESULT_INVALID_IMAGE_GRID;
  }
  ...
  if ((dstImage->width != grid->outputWidth) || (dstImage->height != grid->outputHeight) ||
      ...
      (!alpha && (dstImage->yuvFormat != tile->image->yuvFormat)))
  {
    if (alpha) {
...
// After verifying that the relevant properties of the tile match those of the first tile, copies
// over the pixels from the tile into dstImage.
                                               ...
                                               unsigned int tileIndex)
{
  ...
  if (tile != firstTile) {
    // Check for tile consistency. All tiles in a grid image should match the first tile in the
    // properties checked below.
    if ((tile->image->width != firstTile->image->width) ||
        (tile->image->height != firstTile->image->height) ||
        (tile->image->depth != firstTile->image->depth) ||
        (tile->image->yuvFormat != firstTile->image->yuvFormat) ||
        (tile->image->yuvRange != firstTile->image->yuvRange) ||
  ...
  avifCropRect dstViewRect = {firstTile->image->width * colIndex,
                              firstTile->image->height * rowIndex,
  ...
  avifImageCopySamples(&dstView,
                       &srcView,
                       (tile->input->itemCategory == AVIF_ITEM_ALPHA) ? AVIF_PLANES_A :
                                                                        AVIF_PLANES_YUV);

  return AVIF_TRUE;
}

// If colorId == 0 (a sentinel value as item IDs must be nonzero), accept any found EXIF/XMP
// metadata. Passing in 0 is used when finding metadata in a meta box embedded in a trak box, as
// any items inside of a meta box that is inside of a trak box are implicitly associated to the
// track.
static avifResult avifDecoderFindMetadata(avifDecoder *decoder,
                                          avifMeta *meta,
                                          avifImage *image,
                                          uint32_t colorId)
{
  ...
  for (uint32_t itemIndex = 0; itemIndex < meta->items.count; ++itemIndex) {
    ...
    if (!decoder->ignoreExif && !memcmp(item->type, "Exif", 4)) {
      ...
      avifResult readResult = avifDecoderItemRead(
          item, decoder->io, &exifContents, 0, 0, &decoder->diag);
      if (readResult != AVIF_RESULT_OK) {
        return readResult;
      }
      ...
      BEGIN_STREAM(
          exifBoxStream, exifContents.data, exifContents.size, &decoder->diag, "Exif header");
#if defined(AVIF_ENABLE_EXPERIMENTAL_AVIR)
      ...
      AVIF_CHECKRES(avifRWDataSet(&image->exif,
             ...
             !memcmp(item->contentType.contentType, xmpContentType, xmpContentTypeSize))
    {
      ...
      avifResult readResult = avifDecoderItemRead(
          item, decoder->io, &xmpContents, 0, 0, &decoder->diag);
      if (readResult != AVIF_RESULT_OK) {
        return readResult;
      }
...
static avifResult avifParseItemLocationBox(avifMeta *meta,
                                           const uint8_t *raw,
                                           size_t rawLen,
                                           avifDiagnostics *diag)
{
  ...
  uint8_t version;
  AVIF_CHECKERR(avifROStreamReadVersionAndFlags(&s, &version, NULL),
  ...
  AVIF_CHECKERR(avifROStreamReadBits8(&s, &offsetSize, /*bitCount=*/4),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(4) offset_size;
  AVIF_CHECKERR(avifROStreamReadBits8(&s, &lengthSize, /*bitCount=*/4),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(4) length_size;
  AVIF_CHECKERR(avifROStreamReadBits8(&s, &baseOffsetSize, /*bitCount=*/4),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(4) base_offset_size;
  if (((version == 1) || (version == 2)) && (baseOffsetSize != 0)) {
    avifDiagnosticsPrintf(
        diag, "Box[iloc] has an unsupported base_offset_size [%u]", baseOffsetSize);
    return AVIF_RESULT_BMFF_PARSE_FAILED;
  }
  uint32_t reserved;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &reserved, /*bitCount=*/4),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(4) reserved;

  uint16_t tmp16;
  uint32_t itemCount;
  if (version < 2) {
    AVIF_CHECKERR(avifROStreamReadU16(&s, &tmp16),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) item_count;
    itemCount = tmp16;
  }
  else {
    AVIF_CHECKERR(avifROStreamReadU32(&s, &itemCount),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) item_count;
  }
  for (uint32_t i = 0; i < itemCount; ++i) {
    uint32_t itemID;
    if (version < 2) {
      AVIF_CHECKERR(avifROStreamReadU16(&s, &tmp16),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) item_ID;
      itemID = tmp16;
    }
    else {
      AVIF_CHECKERR(avifROStreamReadU32(&s, &itemID),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) item_ID;
    }
    ...
    if (meta->fromConi && itemID < 5) {
      ...
      avifDiagnosticsPrintf(
          diag, "%s: Box[iloc] has a forbidden item ID [%u]", s.diagContext, itemID);
      return AVIF_RESULT_BMFF_PARSE_FAILED;
    }
#endif  // AVIF_ENABLE_EXPERIMENTAL_AVIR
    ...
    if ((version == 1) || (version == 2)) {
      AVIF_CHECKERR(avifROStreamReadBits(&s, &reserved, /*bitCount=*/12),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(12) reserved = 0;
      if (reserved) {
      ...
      if ((constructionMethod != 0 /* file */) && (constructionMethod != 1 /* idat */)) {
        ...
        avifDiagnosticsPrintf(
            diag, "Box[iloc] has an unsupported construction method [%u]", constructionMethod);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
      if (constructionMethod == 1) {
        ...
        if (meta->fromConi) {
          ...
          avifDiagnosticsPrintf(diag,
                                "%s: Box[iloc] has a forbidden construction method [%u]",
                                s.diagContext,
                                constructionMethod);
          return AVIF_RESULT_BMFF_PARSE_FAILED;
        }
    ...
    AVIF_CHECKERR(avifROStreamReadUX8(&s, &baseOffset, baseOffsetSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  //
    ...
    for (int extentIter = 0; extentIter < extentCount; ++extentIter) {
      ...
      AVIF_CHECKERR(avifROStreamReadUX8(&s, &extentOffset, offsetSize),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      uint64_t extentLength;  // unsigned int(offset_size*8) extent_length;
      AVIF_CHECKERR(avifROStreamReadUX8(&s, &extentLength, lengthSize),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      ...
      if (extentOffset > UINT64_MAX - baseOffset) {
        avifDiagnosticsPrintf(
            diag,
            "Item ID [%u] contains an extent offset which overflows: [base: %" PRIu64
            " offset:%" PRIu64 "]",
            itemID,
            baseOffset,
            extentOffset);
      ...
      if (extentLength > SIZE_MAX) {
        avifDiagnosticsPrintf(diag,
                              "Item ID [%u] contains an extent length which overflows: [%" PRIu64
                              "]",
                              itemID,
                              extentLength);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
                                      ...
                                      avifDiagnostics *diag)
{
  ...
  if (fieldLength == 16) {
    ...
    AVIF_CHECK(
        avifROStreamReadU16(&s, &outputWidth16));  // unsigned int(FieldLength) output_width;
    AVIF_CHECK(
        avifROStreamReadU16(&s, &outputHeight16));  // unsigned int(FieldLength) output_height;
    ...
    AVIF_CHECK(
        avifROStreamReadU32(&s, &grid->outputWidth));  // unsigned int(FieldLength) output_width;
    AVIF_CHECK(
        avifROStreamReadU32(&s, &grid->outputHeight));  // unsigned int(FieldLength) output_height;
  }
  if ((grid->outputWidth == 0) || (grid->outputHeight == 0)) {
    avifDiagnosticsPrintf(diag,
                          "Grid box contains illegal dimensions: [%u x %u]",
                          grid->outputWidth,
                          grid->outputHeight);
    return AVIF_FALSE;
  }
  if (avifDimensionsTooLarge(
          grid->outputWidth, grid->outputHeight, imageSizeLimit, imageDimensionLimit))
  {
    avifDiagnosticsPrintf(diag,
                          "Grid box dimensions are too large: [%u x %u]",
                          grid->outputWidth,
                          grid->outputHeight);
    return AVIF_FALSE;
  }
...
static avifBool avifParseImageSpatialExtentsProperty(avifProperty *prop,
                                                     const uint8_t *raw,
                                                     size_t rawLen,
                                                     avifDiagnostics *diag)
{
...
static avifBool avifParseAuxiliaryTypeProperty(avifProperty *prop,
                                               const uint8_t *raw,
                                               size_t rawLen,
                                               avifDiagnostics *diag)
{
...
static avifBool avifParseColourInformationBox(avifProperty *prop,
                                              uint64_t rawOffset,
                                              const uint8_t *raw,
                                              size_t rawLen,
                                              avifDiagnostics *diag)
{
  ...
  if (!memcmp(colorType, "rICC", 4) || !memcmp(colorType, "prof", 4)) {
    ...
    // Remember the offset of the ICC payload relative to the beginning of the stream. A direct
    // pointer cannot be stored because decoder->io->persistent could have been AVIF_FALSE when
    // obtaining raw through decoder->io->read(). The bytes could be copied now instead of
    // remembering the offset, but it is as invasive as passing rawOffset everywhere.
    colr->iccOffset = rawOffset + avifROStreamOffset(&s);
    colr->iccSize = avifROStreamRemainingBytes(&s);
  }
  else if (!memcmp(colorType, "nclx", 4)) {
    AVIF_CHECK(
        avifROStreamReadU16(&s, &colr->colorPrimaries));  // unsigned int(16) colour_primaries;
    AVIF_CHECK(avifROStreamReadU16(
        &s, &colr->transferCharacteristics));  // unsigned int(16) transfer_characteristics;
    AVIF_CHECK(avifROStreamReadU16(
        &s, &colr->matrixCoefficients));  // unsigned int(16) matrix_coefficients;
    uint8_t full_range_flag;
    AVIF_CHECK(avifROStreamReadBits8(
        &s, &full_range_flag, /*bitCount=*/1));  // unsigned int(1) full_range_flag;
    colr->range = full_range_flag ? AVIF_RANGE_FULL : AVIF_RANGE_LIMITED;
    uint8_t reserved;
    AVIF_CHECK(
        avifROStreamReadBits8(&s, &reserved, /*bitCount=*/7));  // unsigned int(7) reserved = 0;
    if (reserved) {
...
static avifBool avifParseContentLightLevelInformationBox(avifProperty *prop,
                                                         const uint8_t *raw,
                                                         size_t rawLen,
                                                         avifDiagnostics *diag)
{
  ...
  AVIF_CHECK(
      avifROStreamReadU16(&s, &clli->maxPALL));  // unsigned int(16) max_pic_average_light_level
  return AVIF_TRUE;
}

// Implementation of section 2.3.3 of AV1 Codec ISO Media File Format Binding specification v1.2.0.
// See https://aomediacodec.github.io/av1-isobmff/v1.2.0.html#av1codecconfigurationbox-syntax.
static avifBool avifParseCodecConfiguration(avifROStream *s,
                                            ...
                                            avifDiagnostics *diag)
{
  ...
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->seqProfile, /*bitCount=*/3));  // unsigned int (3) seq_profile;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->seqLevelIdx0, /*bitCount=*/5));  // unsigned int (5) seq_level_idx_0;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->seqTier0, /*bitCount=*/1));  // unsigned int (1) seq_tier_0;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->highBitdepth, /*bitCount=*/1));  // unsigned int (1) high_bitdepth;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->twelveBit, /*bitCount=*/1));  // unsigned int (1) twelve_bit;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->monochrome, /*bitCount=*/1));  // unsigned int (1) monochrome;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->chromaSubsamplingX, /*bitCount=*/1));  // unsigned int (1) chroma_subsampling_x;
  AVIF_CHECK(avifROStreamReadBits8(
      s, &config->chromaSubsamplingY, /*bitCount=*/1));  // unsigned int (1) chroma_subsampling_y;
  AVIF_CHECK(avifROStreamReadBits8(s,
                                   &config->chromaSamplePosition,
                                   /*bitCount=*/2));  // unsigned int (2) chroma_sample_position;
...
{
  char diagContext[10];
  snprintf(diagContext,
           sizeof(diagContext),
           "Box[%.4s]",
           configPropName);  // "Box[av1C]" or "Box[av2C]"
  BEGIN_STREAM(s, raw, rawLen, diag, diagContext);
  return avifParseCodecConfiguration(&s, &prop->u.av1C, configPropName, diag);
}

static avifBool avifParsePixelAspectRatioBoxProperty(avifProperty *prop,
                                                     const uint8_t *raw,
                                                     size_t rawLen,
                                                     avifDiagnostics *diag)
{
...
static avifBool avifParseCleanApertureBoxProperty(avifProperty *prop,
                                                  const uint8_t *raw,
                                                  size_t rawLen,
                                                  avifDiagnostics *diag)
{
...
static avifBool avifParseImageRotationProperty(avifProperty *prop,
                                               const uint8_t *raw,
                                               size_t rawLen,
                                               avifDiagnostics *diag)
{
  ...
  uint8_t reserved;
  AVIF_CHECK(
      avifROStreamReadBits8(&s, &reserved, /*bitCount=*/6));  // unsigned int (6) reserved = 0;
  if (reserved) {
...
static avifBool avifParseImageMirrorProperty(avifProperty *prop,
                                             const uint8_t *raw,
                                             size_t rawLen,
                                             avifDiagnostics *diag)
{
...
static avifBool avifParsePixelInformationProperty(avifProperty *prop,
                                                  const uint8_t *raw,
                                                  size_t rawLen,
                                                  avifDiagnostics *diag)
{
  ...
  if (pixi->planeCount > MAX_PIXI_PLANE_DEPTHS) {
    avifDiagnosticsPrintf(
        diag, "Box[pixi] contains unsupported plane count [%u]", pixi->planeCount);
    return AVIF_FALSE;
  }
  for (uint8_t i = 0; i < pixi->planeCount; ++i) {
    AVIF_CHECK(
        avifROStreamRead(&s, &pixi->planeDepths[i], 1));  // unsigned int (8) bits_per_channel;
  }
  return AVIF_TRUE;
}

static avifBool avifParseOperatingPointSelectorProperty(avifProperty *prop,
                                                        const uint8_t *raw,
                                                        size_t rawLen,
                                                        avifDiagnostics *diag)
{
  ...
  if (a1op->opIndex > 31) {  // 31 is AV1's max operating point value
    avifDiagnosticsPrintf(
        diag, "Box[a1op] contains an unsupported operating point [%u]", a1op->opIndex);
    return AVIF_FALSE;
  }
  return AVIF_TRUE;
}

static avifBool avifParseLayerSelectorProperty(avifProperty *prop,
                                               const uint8_t *raw,
                                               size_t rawLen,
                                               avifDiagnostics *diag)
{
...
static avifBool avifParseAV1LayeredImageIndexingProperty(avifProperty *prop,
                                                         const uint8_t *raw,
                                                         size_t rawLen,
                                                         avifDiagnostics *diag)
{
                                                    ...
                                                    avifDiagnostics *diag)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "ispe", 4)) {
      AVIF_CHECKERR(
          avifParseImageSpatialExtentsProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "auxC", 4)) {
      AVIF_CHECKERR(
          avifParseAuxiliaryTypeProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "colr", 4)) {
      AVIF_CHECKERR(avifParseColourInformationBox(prop,
      ...
      AVIF_CHECKERR(avifParseCodecConfigurationBoxProperty(
                        prop, avifROStreamCurrent(&s), header.size, "av1C", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
#if defined(AVIF_CODEC_AVM)
    }
    else if (!memcmp(header.type, "av2C", 4)) {
      AVIF_CHECKERR(avifParseCodecConfigurationBoxProperty(
                        prop, avifROStreamCurrent(&s), header.size, "av2C", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
#endif
    }
    else if (!memcmp(header.type, "pasp", 4)) {
      AVIF_CHECKERR(
          avifParsePixelAspectRatioBoxProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "clap", 4)) {
      AVIF_CHECKERR(
          avifParseCleanApertureBoxProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "irot", 4)) {
      AVIF_CHECKERR(
          avifParseImageRotationProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "imir", 4)) {
      AVIF_CHECKERR(avifParseImageMirrorProperty(prop, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "pixi", 4)) {
      AVIF_CHECKERR(
          avifParsePixelInformationProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "a1op", 4)) {
      AVIF_CHECKERR(avifParseOperatingPointSelectorProperty(
                        prop, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "lsel", 4)) {
      AVIF_CHECKERR(
          avifParseLayerSelectorProperty(prop, avifROStreamCurrent(&s), header.size, diag),
          AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "a1lx", 4)) {
      AVIF_CHECKERR(avifParseAV1LayeredImageIndexingProperty(
                        prop, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "clli", 4)) {
      AVIF_CHECKERR(avifParseContentLightLevelInformationBox(
                        prop, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
...
static avifResult avifParseItemPropertyAssociation(avifMeta *meta,
                                                   const uint8_t *raw,
                                                   size_t rawLen,
                                                   avifDiagnostics *diag,
                                                   uint32_t *outVersionAndFlags)
{
#if defined(AVIF_ENABLE_EXPERIMENTAL_AVIR)
  if (meta->fromConi) {
  ...
  uint8_t version;
  uint32_t flags;
  AVIF_CHECKERR(avifROStreamReadVersionAndFlags(&s, &version, &flags),
                AVIF_RESULT_BMFF_PARSE_FAILED);
  ...
  *outVersionAndFlags = ((uint32_t)version << 24) | flags;
  ...
  for (uint32_t entryIndex = 0; entryIndex < entryCount; ++entryIndex) {
    ...
    for (uint8_t associationIndex = 0; associationIndex < associationCount; ++associationIndex) {
      uint8_t essential;
      AVIF_CHECKERR(avifROStreamReadBits8(&s, &essential, /*bitCount=*/1),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // bit(1) essential;
      uint32_t propertyIndex;
      AVIF_CHECKERR(
          avifROStreamReadBits(&s, &propertyIndex, /*bitCount=*/propertyIndexIsU15 ? 15 : 7),
      ...
      if (propertyIndex >= meta->properties.count) {
        avifDiagnosticsPrintf(diag,
                              "Box[ipma] for item ID [%u] contains an illegal property index [%u] "
                              "(out of [%u] properties)",
                              itemID,
                              propertyIndex,
      ...
      static const char *supportedTypes[] = {"ispe",
                                             "auxC",
                                             "colr",
                                             "av1C",
#if defined(AVIF_CODEC_AVM)
        ...
        if (essential) {
          ...
          for (size_t i = 0; i < nonessentialTypesCount; ++i) {
            if (!memcmp(srcProp->type, nonessentialTypes[i], 4)) {
              avifDiagnosticsPrintf(diag,
                                    "Item ID [%u] has a %s property association which must not be "
                                    "marked essential, but is",
                                    itemID,
                                    nonessentialTypes[i]);
              return AVIF_RESULT_BMFF_PARSE_FAILED;
            }
          ...
          static const char *const essentialTypes[] = {
              ...
              // HEIF: Section 6.5.11.1: "essential shall be equal to 1 for an 'lsel' item
              // property."
              "lsel"
          ...
          for (size_t i = 0; i < essentialTypesCount; ++i) {
            if (!memcmp(srcProp->type, essentialTypes[i], 4)) {
              avifDiagnosticsPrintf(diag,
                                    "Item ID [%u] has a %s property association which must be "
                                    "marked essential, but is not",
                                    itemID,
                                    essentialTypes[i]);
              return AVIF_RESULT_BMFF_PARSE_FAILED;
            }
...
static avifBool avifParsePrimaryItemBox(avifMeta *meta,
                                        const uint8_t *raw,
                                        size_t rawLen,
                                        avifDiagnostics *diag)
{
...
static avifBool avifParseItemDataBox(avifMeta *meta,
                                     const uint8_t *raw,
                                     size_t rawLen,
                                     avifDiagnostics *diag)
{
...
static avifResult avifParseItemPropertiesBox(
    avifMeta *meta, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(ipmaHeader.type, "ipma", 4)) {
      ...
      AVIF_CHECKRES(avifParseItemPropertyAssociation(
          meta, avifROStreamCurrent(&s), ipmaHeader.size, diag, &versionAndFlags));
      for (uint32_t i = 0; i < versionAndFlagsSeenCount; ++i) {
        if (versionAndFlagsSeen[i] == versionAndFlags) {
          ...
          // flags.
          avifDiagnosticsPrintf(diag,
                                "Multiple Box[ipma] with a given pair of values of version and "
                                "flags. See HEIF (ISO 23008-12:2017) 9.3.1");
          return AVIF_RESULT_BMFF_PARSE_FAILED;
        }
      }
...
static avifResult avifParseItemInfoEntry(avifMeta *meta,
                                         const uint8_t *raw,
                                         size_t rawLen,
                                         avifDiagnostics *diag)
{
  ...
  if (version != 2 && version != 3) {
    avifDiagnosticsPrintf(
        s.diag, "%s: Expecting box version 2 or 3, got version %u", s.diagContext, version);
    return AVIF_RESULT_BMFF_PARSE_FAILED;
  }
  ...
  if (version == 2) {
    uint16_t tmp;
    AVIF_CHECKERR(avifROStreamReadU16(&s, &tmp),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) item_ID;
    itemID = tmp;
  }
  else {
  ...
  if (!memcmp(itemType, "mime", 4)) {
    AVIF_CHECKERR(avifROStreamReadString(&s, NULL, 0),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // string item_name; (skipped)
    AVIF_CHECKERR(avifROStreamReadString(&s, contentType.contentType, CONTENTTYPE_SIZE),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // string content_type;
  }
  else {
  ...
  if (meta->fromConi && itemID < 5) {
    ...
    avifDiagnosticsPrintf(diag,
                          "%s: Box[infe] of type %.4s has a forbidden item ID [%u]",
                          s.diagContext,
                          itemType,
                          itemID);
    return AVIF_RESULT_BMFF_PARSE_FAILED;
...
static avifResult avifParseItemInfoBox(avifMeta *meta,
                                       const uint8_t *raw,
                                       size_t rawLen,
                                       avifDiagnostics *diag)
{
  ...
  if (version == 0) {
    uint16_t tmp;
    AVIF_CHECKERR(avifROStreamReadU16(&s, &tmp),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) entry_count;
    entryCount = tmp;
  }
  else if (version == 1) {
    AVIF_CHECKERR(avifROStreamReadU32(&s, &entryCount),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) entry_count;
  }
  else {
...
static avifResult avifParseItemReferenceBox(avifMeta *meta,
                                            const uint8_t *raw,
                                            size_t rawLen,
                                            avifDiagnostics *diag)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (version == 0) {
      uint16_t tmp;
      AVIF_CHECKERR(avifROStreamReadU16(&s, &tmp),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) from_item_ID;
      fromID = tmp;
      AVIF_CHECKRES(avifCheckItemID(
          "iref", fromID, diag));  // ISO 14496-12 section 8.11.12.1: "index values start at 1"
    }
    else if (version == 1) {
      AVIF_CHECKERR(avifROStreamReadU32(&s, &fromID),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) from_item_ID;
    ...
    AVIF_CHECKERR(avifROStreamReadU16(&s, &referenceCount),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) reference_count;
    ...
    for (uint16_t refIndex = 0; refIndex < referenceCount; ++refIndex) {
      ...
      if (version == 0) {
        uint16_t tmp;
        AVIF_CHECKERR(avifROStreamReadU16(&s, &tmp),
                      AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(16) to_item_ID;
        toID = tmp;
        AVIF_CHECKRES(avifCheckItemID("iref", toID, diag));
      }
      else if (version == 1) {
        AVIF_CHECKERR(avifROStreamReadU32(&s, &toID),
                      AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) to_item_ID;
        AVIF_CHECKRES(avifCheckItemID("iref", toID, diag));
      }
      else {
        // unsupported iref version, skip it
...
static avifResult avifParseMetaBox(
    avifMeta *meta, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (firstBox) {
      if (!memcmp(header.type, "hdlr", 4)) {
        AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 0, "meta", "hdlr", diag),
                      AVIF_RESULT_BMFF_PARSE_FAILED);
        AVIF_CHECKERR(avifParseHandlerBox(avifROStreamCurrent(&s), header.size, diag),
                      AVIF_RESULT_BMFF_PARSE_FAILED);
        firstBox = AVIF_FALSE;
      }
      else {
      ...
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 1, "meta", "iloc", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemLocationBox(meta, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "pitm", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 2, "meta", "pitm", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKERR(avifParsePrimaryItemBox(meta, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "idat", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 3, "meta", "idat", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKERR(avifParseItemDataBox(meta, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "iprp", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 4, "meta", "iprp", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemPropertiesBox(
          meta, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "iinf", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 5, "meta", "iinf", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemInfoBox(meta, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "iref", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 6, "meta", "iref", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
                                        ...
                                        avifDiagnostics *diag)
{
  ...
  if (version == 1) {
    ...
    track->trackDuration = (trackDuration == AVIF_INDEFINITE_DURATION32) ?
  ...
  if ((track->width == 0) || (track->height == 0)) {
    avifDiagnosticsPrintf(
        diag, "Track ID [%u] has an invalid size [%ux%u]", track->id, track->width, track->height);
    return AVIF_FALSE;
  }
...
static avifBool avifParseMediaHeaderBox(avifTrack *track,
                                        const uint8_t *raw,
                                        size_t rawLen,
                                        avifDiagnostics *diag)
{
...
static avifResult avifParseChunkOffsetBox(avifSampleTable *sampleTable,
                                          ...
                                          avifDiagnostics *diag)
{
  ...
  for (uint32_t i = 0; i < entryCount; ++i) {
    uint64_t offset;
    if (largeOffsets) {
      AVIF_CHECKERR(avifROStreamReadU64(&s, &offset),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) chunk_offset;
    }
    else {
      uint32_t offset32;
      AVIF_CHECKERR(avifROStreamReadU32(&s, &offset32),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) chunk_offset;
...
static avifResult avifParseSampleToChunkBox(avifSampleTable *sampleTable,
                                            const uint8_t *raw,
                                            size_t rawLen,
                                            avifDiagnostics *diag)
{
  ...
  for (uint32_t i = 0; i < entryCount; ++i) {
    avifSampleTableSampleToChunk *sampleToChunk = (avifSampleTableSampleToChunk *)avifArrayPush(
        &sampleTable->sampleToChunks);
    AVIF_CHECKERR(sampleToChunk != NULL, AVIF_RESULT_OUT_OF_MEMORY);
    AVIF_CHECKERR(avifROStreamReadU32(&s, &sampleToChunk->firstChunk),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) first_chunk;
    AVIF_CHECKERR(avifROStreamReadU32(&s, &sampleToChunk->samplesPerChunk),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) samples_per_chunk;
    ...
    if (i == 0) {
      if (sampleToChunk->firstChunk != 1) {
        avifDiagnosticsPrintf(
            diag, "Box[stsc] does not begin with chunk 1 [%u]", sampleToChunk->firstChunk);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
    }
    else {
...
static avifResult avifParseSampleSizeBox(avifSampleTable *sampleTable,
                                         const uint8_t *raw,
                                         size_t rawLen,
                                         avifDiagnostics *diag)
{
  ...
  AVIF_CHECKERR(avifROStreamReadU32(&s, &allSamplesSize),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) sample_size;
  AVIF_CHECKERR(avifROStreamReadU32(&s, &sampleCount),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) sample_count;
  ...
  if (allSamplesSize > 0) {
    ...
    for (uint32_t i = 0; i < sampleCount; ++i) {
      avifSampleTableSampleSize *sampleSize = (avifSampleTableSampleSize *)avifArrayPush(
          &sampleTable->sampleSizes);
      AVIF_CHECKERR(sampleSize != NULL, AVIF_RESULT_OUT_OF_MEMORY);
      AVIF_CHECKERR(avifROStreamReadU32(&s, &sampleSize->size),
                    AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) entry_size;
    }
  }
  return AVIF_RESULT_OK;
}

static avifResult avifParseSyncSampleBox(avifSampleTable *sampleTable,
                                         const uint8_t *raw,
                                         size_t rawLen,
                                         avifDiagnostics *diag)
{
  ...
  for (uint32_t i = 0; i < entryCount; ++i) {
    ...
    AVIF_CHECKERR(avifROStreamReadU32(&s, &sampleNumber),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) sample_number;
...
static avifResult avifParseTimeToSampleBox(avifSampleTable *sampleTable,
                                           const uint8_t *raw,
                                           size_t rawLen,
                                           avifDiagnostics *diag)
{
  ...
  for (uint32_t i = 0; i < entryCount; ++i) {
    avifSampleTableTimeToSample *timeToSample = (avifSampleTableTimeToSample *)avifArrayPush(
        &sampleTable->timeToSamples);
    AVIF_CHECKERR(timeToSample != NULL, AVIF_RESULT_OUT_OF_MEMORY);
    AVIF_CHECKERR(avifROStreamReadU32(&s, &timeToSample->sampleCount),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) sample_count;
    AVIF_CHECKERR(avifROStreamReadU32(&s, &timeToSample->sampleDelta),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(32) sample_delta;
  }
  return AVIF_RESULT_OK;
}
                                                ...
                                                avifDiagnostics *diag)
{
  ...
  for (uint32_t i = 0; i < entryCount; ++i) {
    ...
    AVIF_CHECKERR(avifROStreamReadBoxHeader(&s, &sampleEntryHeader),
                  AVIF_RESULT_BMFF_PARSE_FAILED);

    avifSampleDescription *description = (avifSampleDescription *)avifArrayPush(
    ...
    {
      AVIF_CHECKRES(avifParseItemPropertyContainerBox(
...
static avifResult avifParseSampleTableBox(
    avifTrack *track, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  if (track->sampleTable) {
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "stco", 4)) {
      AVIF_CHECKRES(avifParseChunkOffsetBox(
          track->sampleTable, AVIF_FALSE, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "co64", 4)) {
      AVIF_CHECKRES(avifParseChunkOffsetBox(
          track->sampleTable, AVIF_TRUE, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "stsc", 4)) {
      AVIF_CHECKRES(avifParseSampleToChunkBox(
          track->sampleTable, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "stsz", 4)) {
      AVIF_CHECKRES(
          avifParseSampleSizeBox(track->sampleTable, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "stss", 4)) {
      AVIF_CHECKRES(
          avifParseSyncSampleBox(track->sampleTable, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "stts", 4)) {
      AVIF_CHECKRES(avifParseTimeToSampleBox(
          track->sampleTable, avifROStreamCurrent(&s), header.size, diag));
    }
...
static avifResult avifParseMediaInformationBox(
    avifTrack *track, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "stbl", 4)) {
      AVIF_CHECKRES(avifParseSampleTableBox(
          track, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), header.size, diag));
    }
...
static avifResult avifParseMediaBox(
    avifTrack *track, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "mdhd", 4)) {
      AVIF_CHECKERR(avifParseMediaHeaderBox(track, avifROStreamCurrent(&s), header.size, diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
    else if (!memcmp(header.type, "minf", 4)) {
      AVIF_CHECKRES(avifParseMediaInformationBox(
          track, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), header.size, diag));
    }
...
static avifBool avifTrackReferenceBox(avifTrack *track,
                                      const uint8_t *raw,
                                      size_t rawLen,
                                      avifDiagnostics *diag)
{
...
static avifBool avifParseEditListBox(avifTrack *track,
                                     const uint8_t *raw,
                                     size_t rawLen,
                                     avifDiagnostics *diag)
{
...
static avifBool avifParseEditBox(avifTrack *track,
                                 const uint8_t *raw,
                                 size_t rawLen,
                                 avifDiagnostics *diag)
{
                                    ...
                                    uint32_t imageDimensionLimit)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "tkhd", 4)) {
      ...
      AVIF_CHECKRES(avifParseMetaBox(track->meta,
                                     rawOffset + avifROStreamOffset(&s),
                                     avifROStreamCurrent(&s),
                                     header.size,
                                     data->diag));
      ...
      AVIF_CHECKRES(avifParseMediaBox(track,
                                      rawOffset + avifROStreamOffset(&s),
                                      avifROStreamCurrent(&s),
                                      header.size,
                                      data->diag));
      ...
      AVIF_CHECKERR(avifTrackReferenceBox(track, avifROStreamCurrent(&s), header.size, data->diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
    }
      ...
      AVIF_CHECKERR(avifParseEditBox(track, avifROStreamCurrent(&s), header.size, data->diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
  ...
  if (!edtsBoxSeen) {
    ...
    if (track->trackDuration == AVIF_INDEFINITE_DURATION64) {
      ...
      //
      // Since libavif uses repetitionCount (which is 0-based), we subtract the value by 1 to
      // derive the number of repetitions.
      assert(track->segmentDuration != 0);
      // We specifically check for trackDuration == 0 here and not when it is actually read in
      // order to accept files which inadvertently has a trackDuration of 0 without any edit lists.
      if (track->trackDuration == 0) {
        avifDiagnosticsPrintf(data->diag, "Invalid track duration 0.");
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
      const uint64_t repetitionCount = (track->trackDuration / track->segmentDuration) +
                                       (track->trackDuration % track->segmentDuration != 0) - 1;
                                    ...
                                    uint32_t imageDimensionLimit)
{
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "trak", 4)) {
      AVIF_CHECKRES(avifParseTrackBox(data,
                                      rawOffset + avifROStreamOffset(&s),
                                      avifROStreamCurrent(&s),
                                      header.size,
                                      imageSizeLimit,
...
#if defined(AVIF_ENABLE_EXPERIMENTAL_AVIR)
static avifResult avifParseExtendedMeta(
    avifMeta *meta, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  BEGIN_STREAM(s, raw, rawLen, diag, "extendedMeta");
  // The extendedMeta field has no size and box type because these are already set by the
  // CondensedImageBox.
  ...
  while (avifROStreamHasBytesLeft(&s, 1)) {
    ...
    if (!memcmp(header.type, "hdlr", 4) || !memcmp(header.type, "dinf", 4) ||
        !memcmp(header.type, "idat", 4) || !memcmp(header.type, "pitm", 4))
    {
      avifDiagnosticsPrintf(diag,
                            "Box[coni] shall have no Box[%.4s] in its extendedMeta field",
                            (const char *)header.type);
      return AVIF_RESULT_BMFF_PARSE_FAILED;
    }
    else if (!memcmp(header.type, "iinf", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 5, "coni", "iinf", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemInfoBox(meta, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "iloc", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 1, "coni", "iloc", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemLocationBox(meta, avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "iprp", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 4, "coni", "iprp", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemPropertiesBox(
          meta, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), header.size, diag));
    }
    else if (!memcmp(header.type, "iref", 4)) {
      AVIF_CHECKERR(uniqueBoxSeen(&uniqueBoxFlags, 6, "coni", "iref", diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseItemReferenceBox(meta, avifROStreamCurrent(&s), header.size, diag));
    }
...
static avifProperty *avifDecoderItemAddProperty(avifDecoderItem *item,
                                                const avifProperty *metaProperty)
{
...
static avifResult avifParseCondensedImageBox(
    avifMeta *meta, uint64_t rawOffset, const uint8_t *raw, size_t rawLen, avifDiagnostics *diag)
{
  // Experimental box.
  ...
  // Read the bit fields.
  // TODO(yguyon): Implement or return an appropriate error in cases where
  // AVIF_RESULT_NOT_IMPLEMENTED is returned.

  uint32_t version;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &version, 2),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(2) version;
  AVIF_CHECKERR(version == 0, AVIF_RESULT_NOT_IMPLEMENTED);

  uint32_t width, height;
  AVIF_CHECKERR(avifROStreamReadVarInt(&s, &width),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) width_minus_one;
  AVIF_CHECKERR(width != UINT32_MAX, AVIF_RESULT_BMFF_PARSE_FAILED);
  ++width;
  AVIF_CHECKERR(avifROStreamReadVarInt(&s, &height),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) height_minus_one;
  AVIF_CHECKERR(height != UINT32_MAX, AVIF_RESULT_BMFF_PARSE_FAILED);
  ++height;

  uint32_t isFloat;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &isFloat, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) is_float;
  AVIF_CHECKERR(!isFloat, AVIF_RESULT_NOT_IMPLEMENTED);
  uint32_t bitDepth;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &bitDepth, 4),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) bit_depth_minus_one;
  ++bitDepth;
  AVIF_CHECKERR((bitDepth == 8) || (bitDepth == 10) || (bitDepth == 12),
                AVIF_RESULT_UNSUPPORTED_DEPTH);
  uint32_t isMonochrome;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &isMonochrome, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) is_monochrome;
  uint32_t isSubsampled = AVIF_FALSE;
  if (!isMonochrome) {
    AVIF_CHECKERR(avifROStreamReadBits(&s, &isSubsampled, 1),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) is_subsampled;
  }
  uint32_t fullRange;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &fullRange, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) full_range;
  uint32_t colorType;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &colorType, 2),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(2) color_type;
  ...
  if (colorType == AVIF_CONI_COLOR_TYPE_ICC) {
    ...
    AVIF_CHECKERR(avifROStreamReadBits(&s, &matrixCoefficients, 8),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(8) matrix_coefficients;
    AVIF_CHECKERR(avifROStreamReadVarInt(&s, &iccDataSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) icc_data_size_minus_one;
  ...
  AVIF_CHECKERR(avifROStreamReadBits(&s, &hasExplicitCodecTypes, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) has_explicit_codec_types;
  ...
  AVIF_CHECKERR(avifROStreamReadVarInt(&s, &colorItemCodecConfigSize),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) main_item_codec_config_size;
  AVIF_CHECKERR(colorItemCodecConfigSize == 4, AVIF_RESULT_NOT_IMPLEMENTED);
  AVIF_CHECKERR(avifROStreamReadVarInt(&s, &colorItemDataSize),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) main_item_data_size_minus_one;
  ...
  AVIF_CHECKERR(avifROStreamReadBits(&s, &hasAlpha, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) has_alpha;
  if (hasAlpha) {
    AVIF_CHECKERR(avifROStreamReadBits(&s, &alphaIsPremultiplied, 1),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) alpha_is_premultiplied;
    uint32_t alphaItemCodecConfigSize;
    AVIF_CHECKERR(avifROStreamReadVarInt(&s, &alphaItemCodecConfigSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) alpha_item_codec_config_size;
    AVIF_CHECKERR(alphaItemCodecConfigSize == 4, AVIF_RESULT_NOT_IMPLEMENTED);
    AVIF_CHECKERR(avifROStreamReadVarInt(&s, &alphaItemDataSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) alpha_item_data_size;
  }

  uint32_t hasExtendedMeta, hasExif, hasXMP;
  uint32_t extendedMetaSize = 0, exifSize = 0, xmpSize = 0;
  AVIF_CHECKERR(avifROStreamReadBits(&s, &hasExtendedMeta, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) has_extended_meta;
  if (hasExtendedMeta) {
    AVIF_CHECKERR(avifROStreamReadVarInt(&s, &extendedMetaSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) extended_meta_size_minus_one;
    AVIF_CHECKERR(extendedMetaSize != UINT32_MAX, AVIF_RESULT_BMFF_PARSE_FAILED);
    ++extendedMetaSize;
  }
  AVIF_CHECKERR(avifROStreamReadBits(&s, &hasExif, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) has_exif;
  if (hasExif) {
    AVIF_CHECKERR(avifROStreamReadVarInt(&s, &exifSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) exif_data_size_minus_one;
    AVIF_CHECKERR(exifSize != UINT32_MAX, AVIF_RESULT_BMFF_PARSE_FAILED);
    ++exifSize;
  }
  AVIF_CHECKERR(avifROStreamReadBits(&s, &hasXMP, 1),
                AVIF_RESULT_BMFF_PARSE_FAILED);  // unsigned int(1) has_xmp;
  if (hasXMP) {
    AVIF_CHECKERR(avifROStreamReadVarInt(&s, &xmpSize),
                  AVIF_RESULT_BMFF_PARSE_FAILED);  // varint(32) xmp_data_size_minus_one;
  ...
  if (s.numUsedBitsInPartialByte) {
    uint32_t padding;
    AVIF_CHECKERR(avifROStreamReadBits(&s, &padding, 8 - s.numUsedBitsInPartialByte),
                  AVIF_RESULT_BMFF_PARSE_FAILED);
  ...
  // Make sure all extended_meta, metadata and coded chunks fit into the 'coni' box whose size is
  // rawLen. There should be no missing nor unused byte.
  AVIF_CHECKERR(avifROStreamRemainingBytes(&s) == (uint64_t)extendedMetaSize + iccDataSize +
                                                      alphaItemDataSize + colorItemDataSize +
                                                      exifSize + xmpSize,
                AVIF_RESULT_BMFF_PARSE_FAILED);
  ...
  AVIF_CHECKERR(avifDecoderItemAddProperty(colorItem, colorCodecConfigProp),
  ...
  colrPropNCLX->u.colr.transferCharacteristics = (avifTransferCharacteristics)
  ...
  if (iccDataSize) {
    ...
    AVIF_CHECKERR(avifMetaCreateProperty(meta, "skip"),
                  AVIF_RESULT_OUT_OF_MEMORY);  // Placeholder.
  }
  ...
  if (hasAlpha) {
    ...
    AVIF_CHECKERR(avifDecoderItemAddProperty(alphaItem, alphaCodecConfigProp),
                  AVIF_RESULT_OUT_OF_MEMORY);
  ...
  if (hasExtendedMeta) {
    ...
    AVIF_CHECKRES(avifParseExtendedMeta(meta,
                                        rawOffset + avifROStreamOffset(&s),
                                        avifROStreamCurrent(&s),
                                        extendedMetaSize,
                                        diag));
...
static avifBool avifParseFileTypeBox(avifFileType *ftyp,
                                     const uint8_t *raw,
                                     size_t rawLen,
                                     avifDiagnostics *diag)
{
  ...
  if ((compatibleBrandsBytes % 4) != 0) {
    avifDiagnosticsPrintf(
        diag,
        "Box[ftyp] contains a compatible brands section that isn't divisible by 4 [%zu]",
        compatibleBrandsBytes);
    return AVIF_FALSE;
  }
...
{
  // Note: this top-level function is the only avifParse*() function that returns avifResult
  // instead of avifBool. Be sure to use AVIF_CHECKERR() in this function with an explicit error
  // result instead of simply using AVIF_CHECK().
  ...
  for (;;) {
    ...
    BEGIN_STREAM(headerStream,
                 headerContents.data,
                 headerContents.size,
                 &decoder->diag,
                 "File-level box header");
    avifBoxHeader header;
    AVIF_CHECKERR(avifROStreamReadBoxHeaderPartial(&headerStream, &header),
                  AVIF_RESULT_BMFF_PARSE_FAILED);
...
#if defined(AVIF_ENABLE_EXPERIMENTAL_AVIR)
    if (!memcmp(header.type, "ftyp", 4) || !memcmp(header.type, "meta", 4) ||
        !memcmp(header.type, "moov", 4) || !memcmp(header.type, "coni", 4))
    {
#else
    if (!memcmp(header.type, "ftyp", 4) || !memcmp(header.type, "meta", 4) ||
        !memcmp(header.type, "moov", 4))
    {
#endif
    ...
    if (!memcmp(header.type, "ftyp", 4)) {
      ...
      avifFileType ftyp;
      AVIF_CHECKERR(avifParseFileTypeBox(&ftyp, boxContents.data, boxContents.size, data->diag),
                    AVIF_RESULT_BMFF_PARSE_FAILED);
      if (!avifFileTypeIsCompatible(&ftyp)) {
        return AVIF_RESULT_INVALID_FTYP;
      }
      ftypSeen = AVIF_TRUE;
      memcpy(data->majorBrand,
             ftyp.majorBrand,
             4);  // Remember the major brand for future AVIF_DECODER_SOURCE_AUTO decisions
      ...
      AVIF_CHECKRES(
          avifParseMetaBox(data->meta, boxOffset, boxContents.data, boxContents.size, data->diag));
      metaSeen = AVIF_TRUE;
    }
    else if (!memcmp(header.type, "moov", 4)) {
      AVIF_CHECKERR(!moovSeen, AVIF_RESULT_BMFF_PARSE_FAILED);
      AVIF_CHECKRES(avifParseMovieBox(data,
                                      boxOffset,
                                      boxContents.data,
                                      boxContents.size,
                                      decoder->imageSizeLimit,
      ...
      AVIF_CHECKRES(avifParseCondensedImageBox(
          data->meta, boxOffset, boxContents.data, boxContents.size, data->diag));
      coniSeen = AVIF_TRUE;
#endif  // AVIF_ENABLE_EXPERIMENTAL_AVIR
    }
    ...
    if (ftypSeen && !needsConi && coniSeen) {
      // The 'coni' box should be ignored if there is no 'avir' brand, but libavif allows reading
      // them in any order.
    ...
    // * If AVIF_ENABLE_EXPERIMENTAL_AVIR is defined and the brand 'avir' is present, require a
    // coni box
#if defined(AVIF_ENABLE_EXPERIMENTAL_AVIR)
...
static avifBool avifFileTypeHasBrand(avifFileType *ftyp, const char *brand)
{
  ...
  for (int compatibleBrandIndex = 0; compatibleBrandIndex < ftyp->compatibleBrandsCount;
       ++compatibleBrandIndex)
  {
                                        ...
                                        avifExtent *outExtent)
{
  ...
  for (uint32_t currentFrameIndex = startFrameIndex; currentFrameIndex <= endFrameIndex;
       ++currentFrameIndex)
  {
...
static avifResult avifDecoderPrepareSample(avifDecoder *decoder,
                                           ...
                                           size_t partialByteCount)
{
  if (!sample->data.size || sample->partialData) {
    ...
    if (sample->itemID) {
      ...
      avifResult readResult = avifDecoderItemRead(
          item, decoder->io, &itemContents, offset, bytesToRead, &decoder->diag);
      if (readResult != AVIF_RESULT_OK) {
        return readResult;
      }
      ...
      avifResult readResult = decoder->io->read(
          decoder->io, 0, sample->offset, bytesToRead, &sampleContents);
      if (readResult != AVIF_RESULT_OK) {
        return readResult;
      }
  ...
  for (uint32_t itemIndex = 0; itemIndex < data->meta->items.count; ++itemIndex) {
    ...
    if (ispeProp) {
      ...
      if ((item->width == 0) || (item->height == 0)) {
        avifDiagnosticsPrintf(data->diag,
                              "Item ID [%u] has an invalid size [%ux%u]",
                              item->id,
                              item->width,
                              item->height);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      ...
      if (avifDimensionsTooLarge(
              item->width, item->height, decoder->imageSizeLimit, decoder->imageDimensionLimit))
      {
        avifDiagnosticsPrintf(data->diag,
                              "Item ID [%u] dimensions are too large [%ux%u]",
                              item->id,
                              item->width,
                              item->height);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      ...
      if (auxCProp && isAlphaURN(auxCProp->u.auxC.auxType)) {
        ...
        avifDiagnosticsPrintf(
            data->diag, "Item ID [%u] is missing a mandatory ispe property", item->id);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
      }
    }
  }
...
static avifResult avifCodecCreateInternal(avifCodecChoice choice,
                                          const avifTile *tile,
                                          avifDiagnostics *diag,
                                          avifCodec **codec)
{
...
static avifBool avifTilesCanBeDecodedWithSameCodecInstance(avifDecoderData *data)
{
  ...
  if (numStolenImageBuffers > 0 && numImageBuffers > 1) {
    // Single tile image with single tile alpha plane or gain map. In this case each tile needs its
    // own decoder since the planes will be "stolen". Stealing either the color or the alpha plane
    // (or gain map) will invalidate the other ones when decode is called the second (or third)
    // time.
    return AVIF_FALSE;
  }
  ...
  for (unsigned int i = 1; i < data->tiles.count; ++i) {
    ...
    if (tile->operatingPoint != firstTileOperatingPoint ||
        tile->input->allLayers != firstTileAllLayers)
    {
      return AVIF_FALSE;
    }
...
static avifResult avifDecoderCreateCodecs(avifDecoder *decoder)
{
  ...
  if (data->source == AVIF_DECODER_SOURCE_TRACKS) {
    // In this case, we will use at most two codec instances (one for the color planes and one for
    // the alpha plane). Gain maps are not supported.
    AVIF_CHECKRES(avifCodecCreateInternal(
        decoder->codecChoice, &decoder->data->tiles.tile[0], &decoder->diag, &data->codec));
    data->tiles.tile[0].codec = data->codec;
    if (data->tiles.count > 1) {
      AVIF_CHECKRES(avifCodecCreateInternal(
          decoder->codecChoice, &decoder->data->tiles.tile[1], &decoder->diag, &data->codecAlpha));
      data->tiles.tile[1].codec = data->codecAlpha;
    }
  }
  else {
    // In this case, we will use one codec instance when there is only one tile or when all of the
    // following conditions are met:
    //   - The image must have exactly one layer (i.e.) decoder->imageCount == 1.
    //   - All the tiles must have the same operating point (because the codecs take operating
    //   point once at initialization
    //     and do not allow it to be changed later).
    //   - All the tiles must have the same value for allLayers (because the codecs take allLayers
    //   once at initialization
    //     and do not allow it to be changed later).
    //   - If the image has a single tile, it must not have a single tile alpha plane (in this case
    //   we will steal the planes
    //     from the decoder, so we cannot use the same decoder for both the color and the alpha
    //     planes).
    ...
    avifBool canUseSingleCodecInstance = (data->tiles.count == 1) ||
                                         (decoder->imageCount == 1 &&
                                          avifTilesCanBeDecodedWithSameCodecInstance(data));
    if (canUseSingleCodecInstance) {
      ...
      for (unsigned int i = 0; i < decoder->data->tiles.count; ++i) {
        ...
        AVIF_CHECKRES(
            avifCodecCreateInternal(decoder->codecChoice, tile, &decoder->diag, &tile->codec));
      }
    }
  }
  return AVIF_RESULT_OK;
...
// Returns AVIF_TRUE if the item should be skipped. Items should be skipped for one of the
// following reasons:
//  * Size is 0.
...
static avifBool avifDecoderItemShouldBeSkipped(const avifDecoderItem *item)
{
  return !item->size || item->hasUnsupportedEssentialProperty ||
         (avifGetCodecType(item->type) == AVIF_CODEC_TYPE_UNKNOWN &&
          memcmp(item->type, "grid", 4)) ||
...
// Finds the alpha item whose parent item is colorItem and sets it in the alphaItem output
// parameter. Returns AVIF_RESULT_OK on success. Note that *alphaItem can be NULL even if the
// return value is AVIF_RESULT_OK. If the colorItem is a grid and the alpha item is represented as
// a set of auxl items to each color tile, then a fake item will be created and *isAlphaItemInInput
// will be set to AVIF_FALSE. In this case, the alpha item merely exists to hold the locations of
// the alpha tile items. The data of this item need not be read and the pixi property cannot be
// validated. Otherwise, *isAlphaItemInInput will be set to AVIF_TRUE when *alphaItem is not NULL.
                                        ...
                                        avifBool *isAlphaItemInInput)
{
  ...
  // If color item is a grid, check if there is an alpha channel which is represented as an auxl
  // item to each color tile item.
  ...
  if (alphaItemCount != colorItemCount) {
    // Not all the color items had an alpha auxiliary attached to it. Report this case as an image
  ...
  const avifResult result = avifMetaFindOrCreateItem(
      meta, maxItemID + 1, alphaItem);  // Create new empty item.
  ...
  for (uint32_t itemIndex = 0; itemIndex < data->meta->items.count; ++itemIndex) {
    ...
    if (!memcmp(item->type, "tmap", 4)) {
      ...
      if (numDimgItemIDs != 2) {
        avifDiagnosticsPrintf(
            data->diag,
            "Expected box[tmap] to have 2 items associated with 'dimg': found [%d] instead",
            numDimgItemIDs);
                                             ...
                                             avifCodecType *gainMapCodecType)
{
  ...
  *gainMapCodecType = AVIF_CODEC_TYPE_UNKNOWN;
  ...
  AVIF_CHECKRES(avifDecoderDataFindToneMappedImageItem(
      data, colorItem, &toneMappedImageItemTmp, &gainMapItemID));
  ...
  if (avifDecoderItemShouldBeSkipped(gainMapItemTmp)) {
    avifDiagnosticsPrintf(
        data->diag, "Box[tmap] gain map item %d is not a supported image type", gainMapItemID);
    return AVIF_RESULT_INVALID_TONE_MAPPED_IMAGE;
  }
  ...
  if (decoder->enableDecodingGainMap) {
    ...
    for (uint32_t propertyIndex = 0; propertyIndex < gainMapItemTmp->properties.count;
         ++propertyIndex)
    {
      ...
      if (!memcmp(prop->type, "colr", 4) && prop->u.colr.hasNCLX) {
        ...
        decoder->image->gainMap.image->transferCharacteristics =
            prop->u.colr.transferCharacteristics;
    ...
    const avifProperty *clliProp = avifPropertyArrayFind(&toneMappedImageItemTmp->properties,
                                                         "clli");
    if (clliProp) {
...
static avifResult avifDecoderGenerateImageTiles(avifDecoder *decoder,
                                                avifTileInfo *info,
                                                avifDecoderItem *item,
                                                avifItemCategory itemCategory)
{
  ...
  if ((info->grid.rows > 0) && (info->grid.columns > 0)) {
    ...
    assert(codecType != AVIF_CODEC_TYPE_UNKNOWN);
    avifTile *tile = avifDecoderDataCreateTile(
        decoder->data, codecType, item->width, item->height, avifDecoderItemOperatingPoint(item));
...
// Populates depth, yuvFormat and yuvChromaSamplePosition fields on 'image' based on data from the
// codec config property (e.g. "av1C").
static avifResult avifReadCodecConfigProperty(avifImage *image,
                                              const avifPropertyArray *properties,
                                              avifCodecType codecType)
{
  const avifProperty *configProp = avifPropertyArrayFind(
      properties, avifGetConfigurationPropertyName(codecType));
  if (configProp) {
    ...
    image->yuvChromaSamplePosition = (avifChromaSamplePosition)
...
avifResult avifDecoderReset(avifDecoder *decoder)
{
  ...
  if (data->source == AVIF_DECODER_SOURCE_TRACKS) {
    ...
    if (colorTrack->meta) {
      ...
      avifResult findResult = avifDecoderFindMetadata(
          decoder, colorTrack->meta, decoder->image, 0);
    ...
    avifTile *colorTile = avifDecoderDataCreateTile(
        data, colorCodecType, colorTrack->width, colorTrack->height, operatingPoint);
    ...
    if (alphaTrack) {
      avifTile *alphaTile = avifDecoderDataCreateTile(
          data, alphaCodecType, alphaTrack->width, alphaTrack->height, operatingPoint);
    ...
    if (colorTrack->mediaTimescale) {
      decoder->duration = (double)decoder->durationInTimescales /
                          (double)colorTrack->mediaTimescale;
    }
    else {
      decoder->duration = 0;
    }
    // If the alphaTrack->repetitionCount and colorTrack->repetitionCount are different, we will
    // simply use the colorTrack's repetitionCount.
    decoder->repetitionCount = colorTrack->repetitionCount;

    memset(&decoder->imageTiming,
           0,
           sizeof(decoder->imageTiming));  // to be set in avifDecoderNextImage()
    ...
    decoder->image->alphaPremultiplied = decoder->alphaPresent &&
                                         (colorTrack->premByID == alphaTrack->id);
  }
  else {
    // Create from items
...
#if defined(AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP)
    avifDecoderItem *toneMappedImageItem;
    const avifResult findGainMapResult = avifDecoderFindGainMapItem(
        decoder,
        mainItems[AVIF_ITEM_COLOR],
        &toneMappedImageItem,
    ...
    if (toneMappedImageItem != NULL && decoder->enableParsingGainMapMetadata) {
      ...
      avifROData tmapData;
      AVIF_CHECKRES(
          avifDecoderItemRead(toneMappedImageItem, decoder->io, &tmapData, 0, 0, data->diag));

      AVIF_CHECKERR(
          avifParseToneMappedImageBox(
              &decoder->image->gainMap.metadata, tmapData.data, tmapData.size, data->diag),
          AVIF_RESULT_INVALID_TONE_MAPPED_IMAGE);
    }
#endif  // AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP

    // Find Exif and/or XMP metadata, if any
    AVIF_CHECKRES(avifDecoderFindMetadata(
        decoder, data->meta, decoder->image, mainItems[AVIF_ITEM_COLOR]->id));
    ...
    for (int c = AVIF_ITEM_COLOR; c < AVIF_ITEM_CATEGORY_COUNT; ++c) {
      ...
      if (c == AVIF_ITEM_ALPHA && !mainItems[c]->width && !mainItems[c]->height) {
        // NON-STANDARD: Alpha subimage does not have an ispe property; adopt width/height from
        // color item
      ...
      if (c == AVIF_ITEM_ALPHA && !isAlphaItemInInput) {
        // In this case, the made up grid item will not have an associated pixi property. So
        // validate everything else but the pixi property.
        strictFlags &= ~AVIF_STRICT_PIXI_REQUIRED;
      }
      AVIF_CHECKRES(
          avifDecoderItemValidateProperties(mainItems[c],
                                            avifGetConfigurationPropertyName(codecType[c]),
                                            &decoder->diag,
                                            strictFlags));
    }
    ...
    decoder->image->alphaPremultiplied = decoder->alphaPresent &&
                                         (mainItems[AVIF_ITEM_COLOR]->premByID ==
                                          mainItems[AVIF_ITEM_ALPHA]->id);
    ...
    if (mainItems[AVIF_ITEM_GAIN_MAP]) {
      ...
      // codec config property from the first tile of a grid to the grid item (when grids are
      // used).
  ...
  // Find and adopt all colr boxes "at most one for a given value of colour type" (HEIF 6.5.5.1,
  // from Amendment 3) Accept one of each type, and bail out if more than one of a given type is
  // provided.
  ...
  for (uint32_t propertyIndex = 0; propertyIndex < colorProperties->count; ++propertyIndex) {
    ...
    if (!memcmp(prop->type, "colr", 4)) {
      if (prop->u.colr.hasICC) {
        ...
        avifROData icc;
        const avifResult readResult = decoder->io->read(
            decoder->io, 0, prop->u.colr.iccOffset, prop->u.colr.iccSize, &icc);
        if (readResult != AVIF_RESULT_OK) {
          return readResult;
        }
  ...
  if (!data->cicpSet && (data->tiles.count > 0)) {
    ...
    if (firstTile->input->samples.count > 0) {
      ...
      // of the first sample. Read in successively larger chunks until we successfully parse the
      // sequence.
...
#endif
  return (itemCategory == AVIF_ITEM_ALPHA) ? AVIF_RESULT_DECODE_ALPHA_FAILED :
                                         ...
                                         avifTileInfo *info)
{
  ...
  for (unsigned int tileIndex = oldDecodedTileCount; tileIndex < info->tileCount; ++tileIndex) {
    ...
    if (!tile->codec->getNextImage(tile->codec,
                                   decoder,
                                   sample,
                                   tile->input->itemCategory == AVIF_ITEM_ALPHA,
                                   &isLimitedRangeAlpha,
                                   tile->image))
    {
    ...
    // See
    // https://aomediacodec.github.io/av1-isobmff/v1.2.0.html#av1codecconfigurationbox-semantics.
        ...
        default:
              ...
              (decoder->image->depth != src->depth))
          {
            if (tile->input->itemCategory == AVIF_ITEM_ALPHA) {
              avifDiagnosticsPrintf(&decoder->diag,
                                    "The color image item does not match the alpha image item in "
                                    "width, height, or bit depth");
...
avifResult avifDecoderNextImage(avifDecoder *decoder)
{
  ...
  assert(decoder->data->tiles.count ==
         (decoder->data->tileInfos[AVIF_ITEM_CATEGORY_COUNT - 1].firstTileIndex +
  ...
  for (int c = 0; c < AVIF_ITEM_CATEGORY_COUNT; ++c) {
    prepareTileResult[c] = avifDecoderPrepareTiles(
        decoder, nextImageIndex, &decoder->data->tileInfos[c]);
  ...
  if (!avifDecoderDataFrameFullyDecoded(decoder->data)) {
    ...
    for (int c = 0; c < AVIF_ITEM_CATEGORY_COUNT; ++c) {
      assert((prepareTileResult[c] == AVIF_RESULT_OK) ||
             (prepareTileResult[c] == AVIF_RESULT_WAITING_ON_IO));
  ...
  // avifDecoderNthImage(decoder, decoder->imageIndex + 1) is equivalent to
  // avifDecoderNextImage(decoder) if the previous call to avifDecoderNextImage() returned
  ...
  if (decoder->data->sourceSampleTable) {
    ...
    avifResult timingResult = avifDecoderNthImageTiming(
        decoder, decoder->imageIndex, &decoder->imageTiming);
                                     ...
                                     avifImageTiming *outTiming)
{
  ...
  for (int imageIndex = 0; imageIndex < (int)frameIndex; ++imageIndex) {
    outTiming->ptsInTimescales += avifSampleTableGetImageDelta(decoder->data->sourceSampleTable,
                                                               imageIndex);
  }
  outTiming->durationInTimescales = avifSampleTableGetImageDelta(decoder->data->sourceSampleTable,
                                                                 frameIndex);
...
avifBool avifDecoderIsKeyframe(const avifDecoder *decoder, uint32_t frameIndex)
{
  ...
  for (unsigned int i = 0; i < decoder->data->tiles.count; ++i) {
    ...
    if ((frameIndex >= tile->input->samples.count) ||
        !tile->input->samples.sample[frameIndex].sync)
    {
      return AVIF_FALSE;
    }
  }
  ...
  for (int c = 0; c < AVIF_ITEM_CATEGORY_COUNT; ++c) {
    ...
    if (c == AVIF_ITEM_GAIN_MAP) {
      ...
      if (decoder->gainMapPresent && decoder->enableDecodingGainMap && gainMap != NULL &&
          gainMap->height != 0)
      {
        uint32_t gainMapRowCount = avifGetDecodedRowCount(
            decoder, &decoder->data->tileInfos[AVIF_ITEM_GAIN_MAP], gainMap);
        if (gainMap->height != decoder->image->height) {
          const uint32_t scaledGainMapRowCount = (uint32_t)floorf(
              (float)gainMapRowCount / gainMap->height * decoder->image->height);
          // Make sure it matches the formula described in the comment of
          // avifDecoderDecodedRowCount() in avif.h.
...
#endif
    const uint32_t rowCount = avifGetDecodedRowCount(
        decoder, &decoder->data->tileInfos[c], decoder->image);
...
avifResult avifDecoderReadMemory(avifDecoder *decoder,
                                 avifImage *image,
                                 const uint8_t *data,
                                 size_t size)
{
