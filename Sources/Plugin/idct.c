...
// idct
void av1_highbd_iwht4x4_add(const tran_low_t *input, uint8_t *dest, int stride, int eob, int bd)
{
  if (eob > 1)
                                   ...
                                   const TxfmParam *txfm_param)
{
  ...
  av1_inv_txfm2d_add_4x8_c(
      src, CONVERT_TO_SHORTPTR(dest), stride, txfm_param->tx_type, txfm_param->bd);
}
...
void av1_highbd_inv_txfm_add_16x16_c(const tran_low_t *input,
                                     uint8_t *dest,
                                     int stride,
                                     const TxfmParam *txfm_param)
{
  ...
  av1_inv_txfm2d_add_16x16_c(src, CONVERT_TO_SHORTPTR(dest), stride, tx_type, bd);
}
                                     ...
                                     const TxfmParam *txfm_param)
{
  ...
  av1_inv_txfm2d_add_32x32_c(src, CONVERT_TO_SHORTPTR(dest), stride, tx_type, bd);
}
                                     ...
                                     const TxfmParam *txfm_param)
{
  ...
  const int32_t *src = cast_to_int32(input);
  assert(tx_type == DCT_DCT);
  av1_inv_txfm2d_add_64x64_c(src, CONVERT_TO_SHORTPTR(dest), stride, tx_type, bd);
}
