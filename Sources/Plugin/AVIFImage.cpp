...
template<typename T> class ImageProcessor {
 public:
  ...
  static bool ResizeImage(const float *src, float *dst, int width, int height, int channelCount)
  {
    nanoexr_ImageData_t srcImg = {(uint8_t *)src,
...
bool Hio_AVIFImage::ReadCropped(int const cropTop,
                                int const cropBottom,
                                int const cropLeft,
                                int const cropRight,
                                StorageSpec const &storage)
{
  ...
  rgb.isFloat = true;
  rgb.pixels = (uint8_t *)calloc(channelCount * bytesPerChannel * rgb.width * rgb.height, 1);
  ...
  if (flip) {
    ImageProcessor<GfHalf>::FlipImage((GfHalf *)img.data, img.width, img.height, img.channelCount);
  }

  // resize
  bool resizing = (img.width != storage.width) || (img.width != storage.height);
  ...
  if (resizing) {
    ...
    ImageProcessor<uint16_t>::HalfToFloat(
        inputHalf, f32.data(), img.width, img.height, channelCount);
  ...
  if (_imposeSRGBTransfer) {
    ...
    if (inputFloat == nullptr) {
      ...
      outF32.resize(img.width * img.height * channelCount);
      ImageProcessor<uint16_t>::HalfToFloat(
          inputHalf, outF32.data(), img.width, img.height, channelCount);
      inputFloat = outF32.data();
      inputHalf = nullptr;
    }
  ...
  if (outputIsHalf) {
    if (inputHalf != nullptr) {
      ...
      memcpy(storage.data, inputHalf, rgb.width * rgb.height * channelCount * sizeof(GfHalf));
    }
    else {
      // convert float to half
      ImageProcessor<float>::FloatToHalf(
          inputFloat, (GfHalf *)storage.data, rgb.width, rgb.height, channelCount);
    }
  }
  else {
    // output float case
    if (inputHalf != nullptr) {
      ...
      ImageProcessor<GfHalf>::HalfToFloat(
          inputHalf, (float *)storage.data, rgb.width, rgb.height, channelCount);
    }
    else {
      // copy rgb.pixels to storage.data
      memcpy(storage.data, inputFloat, rgb.width * rgb.height * channelCount * sizeof(float));
    }
  }
                                    ...
                                    bool /*suppressErrors*/)
{
  ...
  _imposeSRGBTransfer = (sourceColorSpace == HioImage::SourceColorSpace::Auto ||
                         sourceColorSpace == HioImage::SourceColorSpace::SRGB) &&
                        (_avifImage->transferCharacteristics ==
