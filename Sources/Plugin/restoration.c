...
#include "Plugin/hioAvif/aom/av1/common/restoration.h"
...
void av1_alloc_restoration_struct(AV1_COMMON *cm, RestorationInfo *rsi, int is_uv)
{
  ...
  CHECK_MEM_ERROR(cm,
                  rsi->unit_info,
                  (RestorationUnitInfo *)aom_memalign(16, sizeof(*rsi->unit_info) * nunits));
}
...
static void extend_frame_lowbd(
    uint8_t *data, int width, int height, int stride, int border_horz, int border_vert)
{
  uint8_t *data_p;
  int i;
  for (i = 0; i < height; ++i) {
...
static void extend_frame_highbd(
    uint16_t *data, int width, int height, int stride, int border_horz, int border_vert)
{
  uint16_t *data_p;
  int i, j;
  for (i = 0; i < height; ++i) {
...
static void copy_tile_highbd(
    int width, int height, const uint16_t *src, int src_stride, uint16_t *dst, int dst_stride)
{
...
void av1_extend_frame(
    uint8_t *data, int width, int height, int stride, int border_horz, int border_vert, int highbd)
{
#if CONFIG_AV1_HIGHBITDEPTH
  if (highbd) {
    extend_frame_highbd(
...
static void copy_tile_lowbd(
    int width, int height, const uint8_t *src, int src_stride, uint8_t *dst, int dst_stride)
{
  for (int i = 0; i < height; ++i)
    memcpy(dst + i * dst_stride, src + i * src_stride, width);
}

static void copy_tile(int width,
                      int height,
                      const uint8_t *src,
                      int src_stride,
                      uint8_t *dst,
                      ...
                      int highbd)
{
  ...
  if (highbd) {
    copy_tile_highbd(
        width, height, CONVERT_TO_SHORTPTR(src), src_stride, CONVERT_TO_SHORTPTR(dst), dst_stride);
    return;
  }
#endif
  (void)highbd;
                                     ...
                                     int *copy_below)
{
  ...
  *copy_below = 1;
  ...
  const int this_stripe_height = full_stripe_height - (first_stripe_in_tile ? runit_offset : 0);
  const int last_stripe_in_tile = (limits->v_start + this_stripe_height >= tile_rect->bottom);
                                             ...
                                             int opt)
{
  ...
  const int line_width = (limits->h_end - limits->h_start) + 2 * RESTORATION_EXTRA_HORZ;
  ...
  if (!opt) {
    if (copy_above) {
      ...
      for (int i = -RESTORATION_BORDER; i < 0; ++i) {
        ...
        memcpy(
            rlbs->tmp_save_above[i + RESTORATION_BORDER], REAL_PTR(use_highbd, dst8), line_size);
  ...
  if (!opt) {
    if (copy_above) {
      ...
      for (int i = -RESTORATION_BORDER; i < 0; ++i) {
        uint8_t *dst8 = data8_tl + i * data_stride;
        memcpy(
            REAL_PTR(use_highbd, dst8), rlbs->tmp_save_above[i + RESTORATION_BORDER], line_size);
      }
    }
                                 ...
                                 int bit_depth)
{
  ...
  for (int j = 0; j < stripe_width; j += procunit_width) {
    ...
    uint8_t *dst_p = dst + j;
    av1_wiener_convolve_add_src(src_p,
                                src_stride,
                                dst_p,
                                dst_stride,
...
*/
static void boxsum1(
    int32_t *src, int width, int height, int src_stride, int sqr, int32_t *dst, int dst_stride)
{
  int i, j, a, b, c;
...
static void boxsum(int32_t *src,
                   int width,
                   int height,
                   int src_stride,
                   int r,
...
static void calculate_intermediate_result(int32_t *dgd,
                                          int width,
                                          int height,
                                          int dgd_stride,
                                          int bit_depth,
...
static void selfguided_restoration_fast_internal(int32_t *dgd,
                                                 int width,
                                                 int height,
                                                 int dgd_stride,
                                                 int32_t *dst,
                                                 ...
                                                 int radius_idx)
{
  ...
  calculate_intermediate_result(
      dgd, width, height, dgd_stride, bit_depth, sgr_params_idx, radius_idx, 1, A, B);
  A += SGRPROJ_BORDER_VERT * buf_stride + SGRPROJ_BORDER_HORZ;
  B += SGRPROJ_BORDER_VERT * buf_stride + SGRPROJ_BORDER_HORZ;
...
static void selfguided_restoration_internal(int32_t *dgd,
                                            int width,
                                            int height,
                                            int dgd_stride,
                                            int32_t *dst,
                                            ...
                                            int radius_idx)
{
  ...
  int i, j;
  calculate_intermediate_result(
      dgd, width, height, dgd_stride, bit_depth, sgr_params_idx, radius_idx, 0, A, B);
  A += SGRPROJ_BORDER_VERT * buf_stride + SGRPROJ_BORDER_HORZ;
  B += SGRPROJ_BORDER_VERT * buf_stride + SGRPROJ_BORDER_HORZ;
...
void av1_apply_selfguided_restoration_c(const uint8_t *dat8,
                                        int width,
                                        int height,
                                        int stride,
                                        int eps,
                                  ...
                                  int bit_depth)
{
  ...
  for (int j = 0; j < stripe_width; j += procunit_width) {
    ...
    av1_apply_selfguided_restoration(src + j,
                                     w,
                                     stripe_height,
                                     src_stride,
                                     rui->sgrproj_info.ep,
                                        ...
                                        int bit_depth)
{
  ...
  for (int j = 0; j < stripe_width; j += procunit_width) {
    ...
    av1_highbd_wiener_convolve_add_src(src8_p,
                                       src_stride,
                                       dst8_p,
                                       dst_stride,
                                       rui->wiener_info.hfilter,
                                         ...
                                         int bit_depth)
{
  for (int j = 0; j < stripe_width; j += procunit_width) {
    int w = AOMMIN(procunit_width, stripe_width - j);
    av1_apply_selfguided_restoration(src8 + j,
                                     w,
                                     stripe_height,
                                     src_stride,
                                     rui->sgrproj_info.ep,
...
static const stripe_filter_fun stripe_filters[NUM_STRIPE_FILTERS] = {wiener_filter_stripe,
                                      ...
                                      int optimized_lr)
{
  ...
  while (i < unit_h) {
    ...
    const int tile_stripe = (remaining_stripes.v_start - tile_rect->top + runit_offset) /
                            full_stripe_height;
    ...
    const int nominal_stripe_height = full_stripe_height - ((tile_stripe == 0) ? runit_offset : 0);
                                 ...
                                 RestorationLineBuffers *rlbs)
{
  ...
  av1_loop_restoration_filter_unit(limits,
                                            ...
                                            int num_planes)
{
  ...
  if (aom_realloc_frame_buffer(lr_ctxt->dst,
                               frame_width,
                               frame_height,
                               ...
                               NULL) < 0)
    aom_internal_error(
        &cm->error, AOM_CODEC_MEM_ERROR, "Failed to allocate restoration dst buffer");
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    av1_extend_frame(frame->buffers[plane],
                     plane_width,
                     plane_height,
                     frame->strides[is_uv],
                     RESTORATION_BORDER,
...
void av1_loop_restoration_copy_planes(AV1LrStruct *loop_rest_ctxt, AV1_COMMON *cm, int num_planes)
{
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    copy_funs[plane](loop_rest_ctxt->dst,
                     loop_rest_ctxt->frame,
                     tile_rect.left,
                     tile_rect.right,
                     tile_rect.top,
                     tile_rect.bottom);
...
static void foreach_rest_unit_in_planes(AV1LrStruct *lr_ctxt, AV1_COMMON *cm, int num_planes)
{
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    av1_foreach_rest_unit_in_plane(cm,
                                   plane,
...
void av1_lr_sync_write_dummy(void *const lr_sync, int r, int c, const int sb_cols, int plane)
{
  (void)lr_sync;
  (void)r;
  (void)c;
...
void av1_foreach_rest_unit_in_plane(const struct AV1Common *cm,
                                    int plane,
                                    rest_unit_visitor_t on_rest_unit,
                                    void *priv,
...
int av1_loop_restoration_corners_in_sb(const struct AV1Common *cm,
                                       int plane,
                                       int mi_row,
                                       int mi_col,
                                       BLOCK_SIZE bsize,
                                       ...
                                       int *rrow1)
{
  ...
  const int mi_to_num_x = av1_superres_scaled(cm) ? mi_size_x * cm->superres_scale_denominator :
                                                    mi_size_x;
...
static void extend_lines(
    uint8_t *buf, int width, int height, int stride, int extend, int use_highbitdepth)
{
  for (int i = 0; i < height; ++i) {
    if (use_highbitdepth) {
...
static void save_deblock_boundary_lines(const YV12_BUFFER_CONFIG *frame,
                                        const AV1_COMMON *cm,
                                        int plane,
                                        int row,
                                        int stripe,
                                        ...
                                        RestorationStripeBoundaries *boundaries)
{
  ...
  const int lines_to_save = AOMMIN(RESTORATION_CTX_VERT, frame->crop_heights[is_uv] - row);
  ...
  if (av1_superres_scaled(cm)) {
    ...
    if (use_highbd)
      av1_upscale_normative_rows(cm,
                                 CONVERT_TO_BYTEPTR(src_rows),
                                 frame->strides[is_uv],
                                 CONVERT_TO_BYTEPTR(bdry_rows),
...
static void save_tile_row_boundary_lines(
    const YV12_BUFFER_CONFIG *frame, int use_highbd, int plane, AV1_COMMON *cm, int after_cdef)
{
  const int is_uv = plane > 0;
  const int ss_y = is_uv && cm->seq_params.subsampling_y;
    ...
    if (!after_cdef) {
      ...
      if (use_deblock_above) {
        save_deblock_boundary_lines(
            frame, cm, plane, y0 - RESTORATION_CTX_VERT, frame_stripe, use_highbd, 1, boundaries);
      }
      if (use_deblock_below) {
        save_deblock_boundary_lines(frame, cm, plane, y1, frame_stripe, use_highbd, 0, boundaries);
      }
    }
    else {
      ...
      if (!use_deblock_above) {
        save_cdef_boundary_lines(frame, cm, plane, y0, frame_stripe, use_highbd, 1, boundaries);
      }
      if (!use_deblock_below) {
        save_cdef_boundary_lines(
            frame, cm, plane, y1 - 1, frame_stripe, use_highbd, 0, boundaries);
      }
    }
  }
}
