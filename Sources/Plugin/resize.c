...
static void interpolate_core(const uint8_t *const input,
                             int in_length,
                             uint8_t *output,
                             int out_length,
...
static void interpolate_double_prec(const double *const input,
                                    int in_length,
                                    double *output,
                                    int out_length)
{
  ...
  interpolate_core_double_prec(
      input, in_length, output, out_length, &interp_filters[0][0], SUBPEL_TAPS);
}
...
static int32_t get_upscale_convolve_x0(int in_length, int out_length, int32_t x_step_qn)
{
  ...
  const int32_t x0 = (-((out_length - in_length) << (RS_SCALE_SUBPEL_BITS - 1)) + out_length / 2) /
                         out_length +
  ...
  if (l1 > l2) {
    ...
    for (i = 0; i < length; i += 2) {
      ...
      for (j = 1; j < filter_len_half; ++j) {
        sum += (input[(i - j < 0 ? 0 : i - j)] + input[(i + j >= length ? length - 1 : i + j)]) *
               filter[j];
      }
      sum >>= FILTER_BITS;
      *optr++ = clip_pixel(sum);
...
static void resize_multistep(
    const uint8_t *const input, int length, uint8_t *output, int olength, uint8_t *otmp)
{
  if (length == olength) {
...
static void upscale_multistep_double_prec(const double *const input,
                                          int length,
                                          double *output,
                                          int olength)
{
...
static void fill_col_to_arr_double_prec(double *img, int stride, int len, double *arr)
{
  int i;
  double *iptr = img;
  double *aptr = arr;
...
static void fill_arr_to_col_double_prec(double *img, int stride, int len, double *arr)
{
  int i;
  double *iptr = img;
  double *aptr = arr;
...
void av1_resize_plane(const uint8_t *const input,
                      int height,
                      int width,
                      int in_stride,
                      uint8_t *output,
                      ...
                      int out_stride)
{
  ...
  uint8_t *tmpbuf = (uint8_t *)aom_malloc(sizeof(uint8_t) * AOMMAX(width, height));
...
void av1_upscale_plane_double_prec(const double *const input,
                                   int height,
                                   int width,
                                   int in_stride,
                                   double *output,
                                   ...
                                   int out_stride)
{
  ...
  assert(height2 > 0);
  for (i = 0; i < height; ++i)
    upscale_multistep_double_prec(input + in_stride * i, width, intbuf + width2 * i, width2);
  for (i = 0; i < width2; ++i) {
...
static void upscale_normative_rect(const uint8_t *const input,
                                   int height,
                                   int width,
                                   int in_stride,
                                   uint8_t *output,
                                   ...
                                   int pad_right)
{
  ...
  if (pad_right) {
    tmp_right = (uint8_t *)aom_malloc(sizeof(*tmp_right) * border_cols * height);
    for (int i = 0; i < height; i++) {
...
static void highbd_interpolate_core(const uint16_t *const input,
                                    int in_length,
                                    uint16_t *output,
                                    int out_length,
                                    int bd,
...
static void highbd_interpolate(
    const uint16_t *const input, int in_length, uint16_t *output, int out_length, int bd)
{
  const InterpKernel *interp_filters = choose_interp_filter(in_length, out_length);
...
static void highbd_down2_symeven(const uint16_t *const input, int length, uint16_t *output, int bd)
{
  // Actual filter len = 2 * filter_len_half.
...
static void highbd_resize_multistep(
    const uint16_t *const input, int length, uint16_t *output, int olength, uint16_t *otmp, int bd)
{
  if (length == olength) {
    memcpy(output, input, sizeof(output[0]) * length);
    return;
...
void av1_highbd_resize_plane(const uint8_t *const input,
                             int height,
                             int width,
                             int in_stride,
                             uint8_t *output,
                             ...
                             int bd)
{
  ...
  uint16_t *tmpbuf = (uint16_t *)aom_malloc(sizeof(uint16_t) * AOMMAX(width, height));
  uint16_t *arrbuf = (uint16_t *)aom_malloc(sizeof(uint16_t) * height);
  uint16_t *arrbuf2 = (uint16_t *)aom_malloc(sizeof(uint16_t) * height2);
  if (intbuf == NULL || tmpbuf == NULL || arrbuf == NULL || arrbuf2 == NULL)
    goto Error;
  for (i = 0; i < height; ++i) {
    highbd_resize_multistep(CONVERT_TO_SHORTPTR(input + in_stride * i),
                            width,
                            intbuf + width2 * i,
                            width2,
                            tmpbuf,
  ...
  for (i = 0; i < width2; ++i) {
    ...
    highbd_fill_arr_to_col(CONVERT_TO_SHORTPTR(output + i), out_stride, height2, arrbuf2);
  }
                                          ...
                                          int bd)
{
  ...
  av1_highbd_convolve_horiz_rs(CONVERT_TO_SHORTPTR(input - 1),
                               in_stride,
                               CONVERT_TO_SHORTPTR(output),
                               out_stride,
                               width2,
                               height2,
                               &av1_resize_filter_normative[0][0],
...
void av1_resize_frame420(const uint8_t *const y,
                         int y_stride,
                         ...
                         int owidth)
{
  ...
  av1_resize_plane(u, height / 2, width / 2, uv_stride, ou, oheight / 2, owidth / 2, ouv_stride);
  av1_resize_plane(v, height / 2, width / 2, uv_stride, ov, oheight / 2, owidth / 2, ouv_stride);
}

void av1_resize_frame422(const uint8_t *const y,
                         int y_stride,
                         const uint8_t *const u,
                         const uint8_t *const v,
                         int uv_stride,
...
{
  av1_resize_plane(y, height, width, y_stride, oy, oheight, owidth, oy_stride);
  av1_resize_plane(u, height, width / 2, uv_stride, ou, oheight, owidth / 2, ouv_stride);
  av1_resize_plane(v, height, width / 2, uv_stride, ov, oheight, owidth / 2, ouv_stride);
}

void av1_resize_frame444(const uint8_t *const y,
                         int y_stride,
                         const uint8_t *const u,
                         const uint8_t *const v,
                         int uv_stride,
...
{
  av1_resize_plane(y, height, width, y_stride, oy, oheight, owidth, oy_stride);
  av1_resize_plane(u, height, width, uv_stride, ou, oheight, owidth, ouv_stride);
  av1_resize_plane(v, height, width, uv_stride, ov, oheight, owidth, ouv_stride);
}

#if CONFIG_AV1_HIGHBITDEPTH
void av1_highbd_resize_frame420(const uint8_t *const y,
                                int y_stride,
                                const uint8_t *const u,
                                const uint8_t *const v,
                                int uv_stride,
                                ...
                                int bd)
{
  av1_highbd_resize_plane(y, height, width, y_stride, oy, oheight, owidth, oy_stride, bd);
  av1_highbd_resize_plane(
      u, height / 2, width / 2, uv_stride, ou, oheight / 2, owidth / 2, ouv_stride, bd);
  av1_highbd_resize_plane(
      v, height / 2, width / 2, uv_stride, ov, oheight / 2, owidth / 2, ouv_stride, bd);
}

void av1_highbd_resize_frame422(const uint8_t *const y,
                                int y_stride,
                                const uint8_t *const u,
                                const uint8_t *const v,
                                int uv_stride,
                                ...
                                int bd)
{
  av1_highbd_resize_plane(y, height, width, y_stride, oy, oheight, owidth, oy_stride, bd);
  av1_highbd_resize_plane(
      u, height, width / 2, uv_stride, ou, oheight, owidth / 2, ouv_stride, bd);
  av1_highbd_resize_plane(
      v, height, width / 2, uv_stride, ov, oheight, owidth / 2, ouv_stride, bd);
}

void av1_highbd_resize_frame444(const uint8_t *const y,
                                int y_stride,
                                const uint8_t *const u,
                                const uint8_t *const v,
                                int uv_stride,
...
{
  av1_highbd_resize_plane(y, height, width, y_stride, oy, oheight, owidth, oy_stride, bd);
  av1_highbd_resize_plane(u, height, width, uv_stride, ou, oheight, owidth, ouv_stride, bd);
  av1_highbd_resize_plane(v, height, width, uv_stride, ov, oheight, owidth, ouv_stride, bd);
}
#endif  // CONFIG_AV1_HIGHBITDEPTH
                                   ...
                                   const int num_planes)
{
  ...
  assert(filter == BILINEAR || filter == EIGHTTAP_SMOOTH || filter == EIGHTTAP_REGULAR);
  const InterpKernel *const kernel = filter == BILINEAR ? av1_bilinear_filters :
  ...
  for (int i = 0; i < AOMMIN(num_planes, MAX_MB_PLANE); ++i) {
    ...
    for (int y = 0; y < dst_h; y += 16) {
      ...
      for (int x = 0; x < dst_w; x += 16) {
        ...
        const uint8_t *src_ptr = srcs[i] + (y / factor) * src_h / dst_h * src_stride +
                                 (x / factor) * src_w / dst_w;
        uint8_t *dst_ptr = dsts[i] + (y / factor) * dst_stride + (x / factor);

        aom_scaled_2d(src_ptr,
                      src_stride,
                      dst_ptr,
                      dst_stride,
                      kernel,
                                              ...
                                              const int num_planes)
{
  ...
  for (int i = 0; i < AOMMIN(num_planes, MAX_MB_PLANE); ++i) {
    ...
    if (src->flags & YV12_FLAG_HIGHBITDEPTH)
      av1_highbd_resize_plane(src->buffers[i],
                              src->crop_heights[is_uv],
                              src->crop_widths[is_uv],
                              src->strides[is_uv],
                              dst->buffers[i],
                              dst->crop_heights[is_uv],
                              dst->crop_widths[is_uv],
                              dst->strides[is_uv],
                              bd);
    else
      av1_resize_plane(src->buffers[i],
                       src->crop_heights[is_uv],
                       src->crop_widths[is_uv],
                       src->strides[is_uv],
                       dst->buffers[i],
...
void av1_upscale_normative_rows(const AV1_COMMON *cm,
                                const uint8_t *src,
                                int src_stride,
                                uint8_t *dst,
                                int dst_stride,
                                ...
                                int rows)
{
  ...
  const int upscaled_plane_width = ROUND_POWER_OF_TWO(cm->superres_upscaled_width, ss_x);
                                          ...
                                          const bool for_psnr)
{
  ...
  const bool scaling_required = for_psnr ?
                                    (cm->superres_upscaled_width != unscaled->y_crop_width ||
...
static void copy_buffer_config(const YV12_BUFFER_CONFIG *const src, YV12_BUFFER_CONFIG *const dst)
{
...
void av1_superres_upscale(AV1_COMMON *cm, BufferPool *const pool)
{
  ...
  if (aom_alloc_frame_buffer(&copy_buffer,
                             aligned_width,
                             cm->height,
                             ...
                             byte_alignment))
    aom_internal_error(
        &cm->error, AOM_CODEC_MEM_ERROR, "Failed to allocate copy buffer for superres upscaling");
  ...
  if (pool != NULL) {
    ...
    if (aom_realloc_frame_buffer(frame_to_show,
    ...
    if (aom_alloc_frame_buffer(frame_to_show,
                               cm->superres_upscaled_width,
                               cm->superres_upscaled_height,
                               seq_params->subsampling_x,
                               seq_params->subsampling_y,
