...
#include "Plugin/hioAvif/aom/config/aom_scale_rtcd.h"
...
#include "Plugin/hioAvif/aom/aom_dsp/binary_codes_reader.h"
#include "Plugin/hioAvif/aom/aom_dsp/bitreader.h"
#include "Plugin/hioAvif/aom/aom_dsp/bitreader_buffer.h"
#include "Plugin/hioAvif/aom/aom_mem/aom_mem.h"
#include "Plugin/hioAvif/aom/aom_ports/aom_timer.h"
#include "Plugin/hioAvif/aom/aom_ports/mem.h"
#include "Plugin/hioAvif/aom/aom_ports/mem_ops.h"
#include "Plugin/hioAvif/aom/aom_scale/aom_scale.h"
#include "Plugin/hioAvif/aom/aom_util/aom_thread.h"

#if CONFIG_BITSTREAM_DEBUG || CONFIG_MISMATCH_DEBUG
#  include "Plugin/hioAvif/aom/aom_util/debug_util.h"
#endif  // CONFIG_BITSTREAM_DEBUG || CONFIG_MISMATCH_DEBUG

#include "Plugin/hioAvif/aom/av1/common/alloccommon.h"
#include "Plugin/hioAvif/aom/av1/common/cdef.h"
#include "Plugin/hioAvif/aom/av1/common/cfl.h"
...
#include "Plugin/hioAvif/aom/av1/common/entropymv.h"
#include "Plugin/hioAvif/aom/av1/common/frame_buffers.h"
#include "Plugin/hioAvif/aom/av1/common/idct.h"
#include "Plugin/hioAvif/aom/av1/common/mvref_common.h"
#include "Plugin/hioAvif/aom/av1/common/obmc.h"
#include "Plugin/hioAvif/aom/av1/common/pred_common.h"
#include "Plugin/hioAvif/aom/av1/common/quant_common.h"
#include "Plugin/hioAvif/aom/av1/common/reconinter.h"
#include "Plugin/hioAvif/aom/av1/common/reconintra.h"
#include "Plugin/hioAvif/aom/av1/common/resize.h"
#include "Plugin/hioAvif/aom/av1/common/seg_common.h"
#include "Plugin/hioAvif/aom/av1/common/thread_common.h"
#include "Plugin/hioAvif/aom/av1/common/tile_common.h"
#include "Plugin/hioAvif/aom/av1/common/warped_motion.h"
#include "Plugin/hioAvif/aom/av1/decoder/decodeframe.h"
#include "Plugin/hioAvif/aom/av1/decoder/decodemv.h"
#include "Plugin/hioAvif/aom/av1/decoder/decoder.h"
#include "Plugin/hioAvif/aom/av1/decoder/decodetxb.h"
#include "Plugin/hioAvif/aom/av1/decoder/detokenize.h"
...
#define MC_TEMP_BUF_PELS \
  (((MAX_SB_SIZE) * 2 + (AOM_INTERP_EXTEND) * 2) * ((MAX_SB_SIZE) * 2 + (AOM_INTERP_EXTEND) * 2))
...
static AOM_INLINE void set_planes_to_neutral_grey(const SequenceHeader *const seq_params,
  ...
  if (seq_params->use_highbitdepth) {
    ...
    for (int plane = only_chroma; plane < MAX_MB_PLANE; plane++) {
      ...
      if (buf->crop_heights[is_uv] > 0) {
        ...
        for (int row_idx = 1; row_idx < buf->crop_heights[is_uv]; row_idx++) {
          memcpy(
              &base[row_idx * buf->strides[is_uv]], base, sizeof(*base) * buf->crop_widths[is_uv]);
        }
      }
    }
  }
    ...
    for (int plane = only_chroma; plane < MAX_MB_PLANE; plane++) {
      ...
      for (int row_idx = 0; row_idx < buf->crop_heights[is_uv]; row_idx++) {
        memset(&buf->buffers[plane][row_idx * buf->uv_stride], 1 << 7, buf->crop_widths[is_uv]);
      }
    }
  }
}
...
static AOM_INLINE void loop_restoration_read_sb_coeffs(
    const AV1_COMMON *const cm, MACROBLOCKD *xd, aom_reader *const r, int plane, int runit_idx);
#endif
...
static TX_MODE read_tx_mode(struct aom_read_bit_buffer *rb, int coded_lossless)
{
  if (coded_lossless)
    return ONLY_4X4;
                                               ...
                                               int reduced_tx_set)
{
  ...
  av1_inverse_transform_block(
      &dcb->xd, dqcoeff, plane, tx_type, tx_size, dst, stride, eob, reduced_tx_set);
  memset(dqcoeff, 0, (scan_line + 1) * sizeof(dqcoeff[0]));
}

static AOM_INLINE void read_coeffs_tx_intra_block(const AV1_COMMON *const cm,
...
static AOM_INLINE void cfl_store_inter_block_void(AV1_COMMON *const cm, MACROBLOCKD *const xd)
{
  (void)cm;
  (void)xd;
}

static AOM_INLINE void predict_and_reconstruct_intra_block(const AV1_COMMON *const cm,
                                                           DecoderCodingBlock *dcb,
                                                           aom_reader *const r,
                                                           const int plane,
                                                           const int row,
  ...
  if (!mbmi->skip_txfm) {
    ...
    if (eob_data->eob) {
      ...
      const TX_TYPE tx_type = av1_get_tx_type(
          xd, plane_type, row, col, tx_size, reduced_tx_set_used);
      struct macroblockd_plane *const pd = &xd->plane[plane];
      uint8_t *dst = &pd->dst.buf[(row * pd->dst.stride + col) << MI_SIZE_LOG2];
      inverse_transform_block(
          dcb, plane, tx_type, tx_size, dst, pd->dst.stride, reduced_tx_set_used);
    }
  }
...
static AOM_INLINE void inverse_transform_inter_block(const AV1_COMMON *const cm,
                                                     DecoderCodingBlock *dcb,
                                                     aom_reader *const r,
                                                     const int plane,
                                                     const int blk_row,
                                                     ...
                                                     const TX_SIZE tx_size)
{
  ...
  // tx_type was read out in av1_read_coeffs_txb.
  const TX_TYPE tx_type = av1_get_tx_type(
      xd, plane_type, blk_row, blk_col, tx_size, reduced_tx_set_used);
  ...
  mi_to_pixel_loc(
      &pixel_c, &pixel_r, mi_col, mi_row, blk_col, blk_row, pd->subsampling_x, pd->subsampling_y);
  mismatch_check_block_tx(dst,
                          pd->dst.stride,
...
static AOM_INLINE void set_cb_buffer_offsets(DecoderCodingBlock *dcb, TX_SIZE tx_size, int plane)
{
  dcb->cb_offset[plane] += tx_size_wide[tx_size] * tx_size_high[tx_size];
  dcb->txb_offset[plane] = dcb->cb_offset[plane] / (TX_SIZE_W_MIN * TX_SIZE_H_MIN);
}
                                             ...
                                             int *eob_total)
{
  ...
  const TX_SIZE plane_tx_size =
      plane ? av1_get_max_uv_txsize(mbmi->bsize, pd->subsampling_x, pd->subsampling_y) :
              mbmi->inter_tx_size[av1_get_txb_size_index(plane_bsize, blk_row, blk_col)];
  ...
  if (tx_size == plane_tx_size || plane) {
    td->read_coeffs_tx_inter_block_visit(cm, dcb, r, plane, blk_row, blk_col, tx_size);
    ...
    for (int row = 0; row < tx_size_high_unit[tx_size]; row += bsh) {
      for (int col = 0; col < tx_size_wide_unit[tx_size]; col += bsw) {
        ...
        decode_reconstruct_tx(
            cm, td, r, mbmi, plane, plane_bsize, offsetr, offsetc, block, sub_txs, eob_total);
        block += sub_step;
      }
    }
  }
                                   ...
                                   int y_mis)
{
  ...
  set_mi_row_col(xd, tile, mi_row, bh, mi_col, bw, mi_params->mi_rows, mi_params->mi_cols);

  av1_setup_dst_planes(xd->plane, bsize, &cm->cur_frame->buf, mi_row, mi_col, 0, num_planes);
}
                                         ...
                                         BLOCK_SIZE bsize)
{
  ...
  if (bsize >= BLOCK_8X8 && (seq_params->subsampling_x || seq_params->subsampling_y)) {
    const BLOCK_SIZE uv_subsize =
        ss_size_lookup[bsize][seq_params->subsampling_x][seq_params->subsampling_y];
    if (uv_subsize == BLOCK_INVALID)
      aom_internal_error(xd->error_info, AOM_CODEC_CORRUPT_FRAME, "Invalid block size.");
  }
}
                                              ...
                                              int h)
{
  ...
  do {
    ...
    if (left)
      aom_memset16(dst, ref_row[0], left);

    if (copy)
      memcpy(dst + left, ref_row + x + left, copy * sizeof(uint16_t));

    if (right)
      aom_memset16(dst + left + copy, ref_row[w - 1], right);
...
static AOM_INLINE void build_mc_border(const uint8_t *src,
                                       int src_stride,
                                       uint8_t *dst,
                                       int dst_stride,
                                       int x,
...
static INLINE int update_extend_mc_border_params(const struct scale_factors *const sf,
                                                 ...
                                                 int *y_pad)
{
  ...
  if ((!is_intrabc) && (!do_warp) &&
      (is_scaled || scaled_mv.col || scaled_mv.row || (frame_width & 0x7) || (frame_height & 0x7)))
  {
...
{
  int x_pad = 0, y_pad = 0;
  if (update_extend_mc_border_params(sf,
                                     pre_buf,
                                     scaled_mv,
                                     &block,
                                     subpel_x_mv,
                                     ...
                                     &y_pad))
  {
    ...
    const uint8_t *const buf_ptr = pre_buf->buf0 + block.y0 * pre_buf->stride + block.x0;
    ...
    // Extend the border.
    if (highbd) {
      highbd_build_mc_border(buf_ptr,
                             buf_stride,
                             mc_buf,
                             b_w,
                             block.x0,
                                   ...
                                   int *subpel_y_mv)
{
  ...
  if (is_scaled) {
    ...
    temp_mv = clamp_mv_to_umv_border_sb(
        xd, src_mv, bw, bh, inter_pred_params->subsampling_x, inter_pred_params->subsampling_y);
...
static void dec_calc_subpel_params_and_extend(const MV *const src_mv,
                                              InterPredParams *const inter_pred_params,
                                              MACROBLOCKD *const xd,
                                              int mi_x,
                                              int mi_y,
...
static AOM_INLINE void dec_build_inter_predictor(
    const AV1_COMMON *cm, DecoderCodingBlock *dcb, int mi_row, int mi_col, BLOCK_SIZE bsize)
{
  MACROBLOCKD *const xd = &dcb->xd;
  const int num_planes = av1_num_planes(cm);
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    dec_build_inter_predictors(
        cm, dcb, plane, xd->mi[0], 0, xd->plane[plane].width, xd->plane[plane].height, mi_x, mi_y);
    if (is_interintra_pred(xd->mi[0])) {
      BUFFER_SET ctx = {
          {xd->plane[0].dst.buf, xd->plane[1].dst.buf, xd->plane[2].dst.buf},
          {xd->plane[0].dst.stride, xd->plane[1].dst.stride, xd->plane[2].dst.stride}};
      av1_build_interintra_predictor(
          cm, xd, xd->plane[plane].dst.buf, xd->plane[plane].dst.stride, &ctx, plane, bsize);
    }
  }
}

static INLINE void dec_build_prediction_by_above_pred(MACROBLOCKD *const xd,
                                                      int rel_mi_row,
                                                      int rel_mi_col,
                                                      uint8_t op_mi_size,
                                                      int dir,
                                                      ...
                                                      const int num_planes)
{
  ...
  av1_setup_build_prediction_by_above_pred(
      xd, rel_mi_col, op_mi_size, &backup_mbmi, ctxt, num_planes);
  ...
  for (int j = 0; j < num_planes; ++j) {
    ...
    int bh = clamp(block_size_high[bsize] >> (pd->subsampling_y + 1),
                   4,
                   block_size_high[BLOCK_64X64] >> (pd->subsampling_y + 1));

    if (av1_skip_u4x4_pred_in_obmc(bsize, pd, 0))
      continue;
    dec_build_inter_predictors(
        ctxt->cm, (DecoderCodingBlock *)ctxt->dcb, j, &backup_mbmi, 1, bw, bh, mi_x, mi_y);
  }
}

static AOM_INLINE void dec_build_prediction_by_above_preds(const AV1_COMMON *cm,
                                                           DecoderCodingBlock *dcb,
                                                           ...
                                                           int tmp_stride[MAX_MB_PLANE])
{
  ...
  const BLOCK_SIZE bsize = xd->mi[0]->bsize;
  foreach_overlappable_nb_above(cm,
                                xd,
...
static INLINE void dec_build_prediction_by_left_pred(MACROBLOCKD *const xd,
                                                     int rel_mi_row,
                                                     int rel_mi_col,
                                                     uint8_t op_mi_size,
                                                     int dir,
                                                     ...
                                                     const int num_planes)
{
  ...
  av1_setup_build_prediction_by_left_pred(
      xd, rel_mi_row, op_mi_size, &backup_mbmi, ctxt, num_planes);
  ...
  for (int j = 0; j < num_planes; ++j) {
    const struct macroblockd_plane *pd = &xd->plane[j];
    int bw = clamp(block_size_wide[bsize] >> (pd->subsampling_x + 1),
                   4,
                   block_size_wide[BLOCK_64X64] >> (pd->subsampling_x + 1));
    int bh = (op_mi_size << MI_SIZE_LOG2) >> pd->subsampling_y;

    if (av1_skip_u4x4_pred_in_obmc(bsize, pd, 1))
      continue;
    dec_build_inter_predictors(
        ctxt->cm, (DecoderCodingBlock *)ctxt->dcb, j, &backup_mbmi, 1, bw, bh, mi_x, mi_y);
  }
...
static AOM_INLINE void dec_build_prediction_by_left_preds(const AV1_COMMON *cm,
                                                          DecoderCodingBlock *dcb,
                                                          uint8_t *tmp_buf[MAX_MB_PLANE],
                                                          int tmp_width[MAX_MB_PLANE],
                                                          int tmp_height[MAX_MB_PLANE],
                                                          int tmp_stride[MAX_MB_PLANE])
{
  ...
  const BLOCK_SIZE bsize = xd->mi[0]->bsize;
  foreach_overlappable_nb_left(cm,
                               xd,
                                                          ...
                                                          DecoderCodingBlock *dcb)
{
  ...
  dec_build_prediction_by_above_preds(cm, dcb, dst_buf1, dst_width1, dst_height1, dst_stride1);
  dec_build_prediction_by_left_preds(cm, dcb, dst_buf2, dst_width2, dst_height2, dst_stride2);
  const int mi_row = xd->mi_row;
  const int mi_col = xd->mi_col;
  av1_setup_dst_planes(
      xd->plane, xd->mi[0]->bsize, &cm->cur_frame->buf, mi_row, mi_col, 0, num_planes);
  av1_build_obmc_inter_prediction(cm, xd, dst_buf1, dst_stride1, dst_buf2, dst_stride2);
}

static AOM_INLINE void cfl_store_inter_block(AV1_COMMON *const cm, MACROBLOCKD *const xd)
{
  MB_MODE_INFO *mbmi = xd->mi[0];
                                           ...
                                           BLOCK_SIZE bsize)
{
  ...
  for (int ref = 0; ref < 1 + has_second_ref(mbmi); ++ref) {
    ...
    if (frame < LAST_FRAME) {
      ...
      const struct scale_factors *ref_scale_factors = get_ref_scale_factors_const(cm, frame);

      xd->block_ref_scale_factors[ref] = ref_scale_factors;
      av1_setup_pre_planes(xd, ref, &ref_buf->buf, mi_row, mi_col, ref_scale_factors, num_planes);
    }
  }
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    int pixel_c, pixel_r;
    mi_to_pixel_loc(
        &pixel_c, &pixel_r, mi_col, mi_row, 0, 0, pd->subsampling_x, pd->subsampling_y);
    if (!is_chroma_reference(mi_row, mi_col, bsize, pd->subsampling_x, pd->subsampling_y))
      continue;
    mismatch_check_block_pre(pd->dst.buf,
                             pd->dst.stride,
                             cm->current_frame.order_hint,
                             plane,
                             pixel_c,
...
static AOM_INLINE void set_color_index_map_offset(MACROBLOCKD *const xd, int plane, aom_reader *r)
{
  (void)r;
  Av1ColorMapParam params;
  const MB_MODE_INFO *const mbmi = xd->mi[0];
  av1_get_block_dimensions(
      mbmi->bsize, plane, xd, &params.plane_width, &params.plane_height, NULL, NULL);
                                                ...
                                                BLOCK_SIZE bsize)
{
  ...
  if (!is_inter_block(mbmi)) {
    int row, col;
    assert(bsize ==
           get_plane_block_size(bsize, xd->plane[0].subsampling_x, xd->plane[0].subsampling_y));
    ...
    for (row = 0; row < max_blocks_high; row += mu_blocks_high) {
      for (col = 0; col < max_blocks_wide; col += mu_blocks_wide) {
        for (int plane = 0; plane < num_planes; ++plane) {
          ...
          const int unit_height = ROUND_POWER_OF_TWO(AOMMIN(mu_blocks_high + row, max_blocks_high),
                                                     pd->subsampling_y);
          const int unit_width = ROUND_POWER_OF_TWO(AOMMIN(mu_blocks_wide + col, max_blocks_wide),
                                                    pd->subsampling_x);
          ...
          for (int blk_row = row >> pd->subsampling_y; blk_row < unit_height; blk_row += stepr) {
            for (int blk_col = col >> pd->subsampling_x; blk_col < unit_width; blk_col += stepc) {
              td->read_coeffs_tx_intra_block_visit(cm, dcb, r, plane, blk_row, blk_col, tx_size);
      ...
      for (row = 0; row < max_blocks_high; row += mu_blocks_high) {
        for (col = 0; col < max_blocks_wide; col += mu_blocks_wide) {
          for (int plane = 0; plane < num_planes; ++plane) {
            ...
            for (blk_row = row >> ss_y; blk_row < unit_height; blk_row += bh_var_tx) {
              for (blk_col = col >> ss_x; blk_col < unit_width; blk_col += bw_var_tx) {
                decode_reconstruct_tx(cm,
                                      td,
                                      r,
                                      mbmi,
                                      plane,
...
static AOM_INLINE void set_inter_tx_size(MB_MODE_INFO *mbmi,
                                         int stride_log2,
                                         int tx_w_log2,
                                         int tx_h_log2,
                                         int min_txs,
                                         ...
                                         int blk_col)
{
  for (int idy = 0; idy < tx_size_high_unit[split_size]; idy += tx_size_high_unit[min_txs]) {
    for (int idx = 0; idx < tx_size_wide_unit[split_size]; idx += tx_size_wide_unit[min_txs]) {
...
static AOM_INLINE void read_tx_size_vartx(MACROBLOCKD *xd,
                                          MB_MODE_INFO *mbmi,
                                          TX_SIZE tx_size,
                                          int depth,
#if CONFIG_LPF_MASK
                                          ...
                                          aom_reader *r)
{
  ...
  if (depth == MAX_VARTX_DEPTH) {
    set_inter_tx_size(
        mbmi, stride_log2, tx_w_log2, tx_h_log2, txs, tx_size, tx_size, blk_row, blk_col);
  ...
  if (is_split) {
    ...
    if (sub_txs == TX_4X4) {
      set_inter_tx_size(
          mbmi, stride_log2, tx_w_log2, tx_h_log2, txs, tx_size, sub_txs, blk_row, blk_col);
      mbmi->tx_size = sub_txs;
      txfm_partition_update(
          xd->above_txfm_context + blk_col, xd->left_txfm_context + blk_row, sub_txs, tx_size);
#if CONFIG_LPF_MASK
      if (store_bitmask) {
        av1_store_bitmask_vartx(
            cm, mi_row + blk_row, mi_col + blk_col, txsize_to_bsize[tx_size], TX_4X4, mbmi);
      }
#endif
      return;
    }
    ...
    if (depth + 1 == MAX_VARTX_DEPTH && store_bitmask) {
      av1_store_bitmask_vartx(
          cm, mi_row + blk_row, mi_col + blk_col, txsize_to_bsize[tx_size], sub_txs, mbmi);
      store_bitmask = 0;
    }
#endif
    ...
    if (store_bitmask) {
      av1_store_bitmask_vartx(
          cm, mi_row + blk_row, mi_col + blk_col, txsize_to_bsize[tx_size], tx_size, mbmi);
    }
#endif
  }
}

static TX_SIZE read_selected_tx_size(const MACROBLOCKD *const xd, aom_reader *r)
{
  ...
  FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
  const int depth = aom_read_symbol(
      r, ec_ctx->tx_size_cdf[tx_size_cat][ctx], max_depths + 1, ACCT_STR);
  assert(depth >= 0 && depth <= max_depths);
  const TX_SIZE tx_size = depth_to_tx_size(depth, bsize);
  return tx_size;
}

static TX_SIZE read_tx_size(const MACROBLOCKD *const xd,
                            TX_MODE tx_mode,
                            int is_inter,
                            int allow_select_inter,
                            aom_reader *r)
{
  const BLOCK_SIZE bsize = xd->mi[0]->bsize;
                                          ...
                                          BLOCK_SIZE bsize)
{
  ...
  if (cm->features.tx_mode == TX_MODE_SELECT && block_signals_txsize(bsize) && !mbmi->skip_txfm &&
      inter_block_tx && !xd->lossless[mbmi->segment_id])
  {
    ...
    mbmi->tx_size = read_tx_size(xd, cm->features.tx_mode, inter_block_tx, !mbmi->skip_txfm, r);
    if (inter_block_tx)
      memset(mbmi->inter_tx_size, mbmi->tx_size, sizeof(mbmi->inter_tx_size));
    set_txfm_ctxs(
        mbmi->tx_size, xd->width, xd->height, mbmi->skip_txfm && is_inter_block(mbmi), xd);
#if CONFIG_LPF_MASK
    ...
    if (w <= mi_size_wide[BLOCK_64X64] && h <= mi_size_high[BLOCK_64X64]) {
      ...
      for (int row = 0; row < h; row += mi_size_high[BLOCK_64X64]) {
        for (int col = 0; col < w; col += mi_size_wide[BLOCK_64X64]) {
          av1_store_bitmask_univariant_tx(cm, mi_row + row, mi_col + col, BLOCK_64X64, mbmi);
        }
      }
    }
#endif
  ...
  if (w <= mi_size_wide[BLOCK_64X64] && h <= mi_size_high[BLOCK_64X64]) {
    ...
    for (int row = 0; row < h; row += mi_size_high[BLOCK_64X64]) {
      for (int col = 0; col < w; col += mi_size_wide[BLOCK_64X64]) {
        av1_store_bitmask_other_info(
            cm, mi_row + row, mi_col + col, BLOCK_64X64, mbmi, row == 0, col == 0);
      }
    }
  }
#endif
  ...
  if (cm->delta_q_info.delta_q_present_flag) {
    for (int i = 0; i < MAX_SEGMENTS; i++) {
      const int current_qindex = av1_get_qindex(&cm->seg, i, xd->current_base_qindex);
...
static AOM_INLINE void set_offsets_for_pred_and_recon(
    AV1Decoder *const pbi, ThreadData *const td, int mi_row, int mi_col, BLOCK_SIZE bsize)
{
  AV1_COMMON *const cm = &pbi->common;
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  DecoderCodingBlock *const dcb = &td->dcb;
  ...
  xd->tx_type_map = &mi_params->tx_type_map[mi_row * mi_params->mi_stride + mi_col];
...
static AOM_INLINE void decode_block(AV1Decoder *const pbi,
                                    ThreadData *const td,
                                    int mi_row,
                                    int mi_col,
                                    aom_reader *r,
...
static PARTITION_TYPE read_partition(MACROBLOCKD *xd,
                                     int mi_row,
                                     int mi_col,
                                     aom_reader *r,
                                     int has_rows,
                                        ...
                                        int parse_decode_flag)
{
  ...
  if (parse_decode_flag & 1) {
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
      ...
      if (av1_loop_restoration_corners_in_sb(
              cm, plane, mi_row, mi_col, bsize, &rcol0, &rcol1, &rrow0, &rrow1))
      {
  ...
  if (subsize == BLOCK_INVALID) {
    aom_internal_error(xd->error_info,
  ...
  if (get_plane_block_size(subsize, pd_u->subsampling_x, pd_u->subsampling_y) == BLOCK_INVALID) {
    aom_internal_error(xd->error_info,
                       AOM_CODEC_CORRUPT_FRAME,
...
#define DEC_BLOCK(db_r, db_c, db_subsize) \
  block_visit[parse_decode_flag]( \
      pbi, td, DEC_BLOCK_STX_ARG(db_r), (db_c), reader, DEC_BLOCK_EPT_ARG(db_subsize))
#define DEC_PARTITION(db_r, db_c, db_subsize) \
  decode_partition( \
      pbi, td, DEC_BLOCK_STX_ARG(db_r), (db_c), reader, (db_subsize), parse_decode_flag)
...
static AOM_INLINE void setup_bool_decoder(const uint8_t *data,
                                          ...
                                          uint8_t allow_update_cdf)
{
  ...
  if (!read_is_valid(data, read_size, data_end))
    aom_internal_error(
        error_info, AOM_CODEC_CORRUPT_FRAME, "Truncated packet or corrupt tile length");
...
static AOM_INLINE void setup_segmentation(AV1_COMMON *const cm, struct aom_read_bit_buffer *rb)
{
  ...
  if (!seg->enabled) {
    if (cm->cur_frame->seg_map) {
      memset(cm->cur_frame->seg_map, 0, (cm->cur_frame->mi_rows * cm->cur_frame->mi_cols));
    }
  ...
  if (cm->seg.enabled && cm->prev_frame && (cm->mi_params.mi_rows == cm->prev_frame->mi_rows) &&
...
static AOM_INLINE void decode_restoration_mode(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  ...
  for (int p = 0; p < num_planes; ++p) {
    ...
    if (aom_rb_read_bit(rb)) {
      rsi->frame_restoration_type = aom_rb_read_bit(rb) ? RESTORE_SGRPROJ : RESTORE_WIENER;
    }
    else {
      rsi->frame_restoration_type = aom_rb_read_bit(rb) ? RESTORE_SWITCHABLE : RESTORE_NONE;
    }
  ...
  if (!all_none) {
    assert(cm->seq_params.sb_size == BLOCK_64X64 || cm->seq_params.sb_size == BLOCK_128X128);
                                          ...
                                          aom_reader *rb)
{
  ...
  if (wiener_win == WIENER_WIN)
    wiener_info->vfilter[0] = wiener_info->vfilter[WIENER_WIN - 1] =
        aom_read_primitive_refsubexpfin(rb,
  ...
  wiener_info->vfilter[WIENER_HALFWIN] = -2 * (wiener_info->vfilter[0] + wiener_info->vfilter[1] +
  ...
  wiener_info->hfilter[2] = wiener_info->hfilter[WIENER_WIN - 3] =
      ...
      WIENER_FILT_TAP2_MINV;
  // The central element has an implicit +WIENER_FILT_STEP
  wiener_info->hfilter[WIENER_HALFWIN] = -2 * (wiener_info->hfilter[0] + wiener_info->hfilter[1] +
                                           ...
                                           aom_reader *rb)
{
  ...
  if (params->r[0] == 0) {
    ...
    sgrproj_info->xqd[1] = aom_read_primitive_refsubexpfin(rb,
    ...
    sgrproj_info->xqd[0] = aom_read_primitive_refsubexpfin(rb,
    ...
    const AV1_COMMON *const cm, MACROBLOCKD *xd, aom_reader *const r, int plane, int runit_idx)
{
  ...
  if (rsi->frame_restoration_type == RESTORE_SWITCHABLE) {
    rui->restoration_type = aom_read_symbol(
        r, xd->tile_ctx->switchable_restore_cdf, RESTORE_SWITCHABLE_TYPES, ACCT_STR);
    switch (rui->restoration_type) {
      ...
      case RESTORE_SGRPROJ:
        ...
        break;
      default:
        assert(rui->restoration_type == RESTORE_NONE);
        break;
    }
  }
...
static AOM_INLINE void setup_loopfilter(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  const int num_planes = av1_num_planes(cm);
...
static AOM_INLINE void setup_cdef(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  const int num_planes = av1_num_planes(cm);
  ...
  for (int i = 0; i < cdef_info->nb_cdef_strengths; i++) {
    ...
    cdef_info->cdef_uv_strengths[i] = num_planes > 1 ?
...
static AOM_INLINE void setup_segmentation_dequant(AV1_COMMON *const cm, MACROBLOCKD *const xd)
{
  ...
  for (int i = 0; i < max_segments; ++i) {
    ...
    quant_params->y_dequant_QTX[i][0] = av1_dc_quant_QTX(
        qindex, quant_params->y_dc_delta_q, bit_depth);
    quant_params->y_dequant_QTX[i][1] = av1_ac_quant_QTX(qindex, 0, bit_depth);
    quant_params->u_dequant_QTX[i][0] = av1_dc_quant_QTX(
        qindex, quant_params->u_dc_delta_q, bit_depth);
    quant_params->u_dequant_QTX[i][1] = av1_ac_quant_QTX(
        qindex, quant_params->u_ac_delta_q, bit_depth);
    quant_params->v_dequant_QTX[i][0] = av1_dc_quant_QTX(
        qindex, quant_params->v_dc_delta_q, bit_depth);
    quant_params->v_dequant_QTX[i][1] = av1_ac_quant_QTX(
        qindex, quant_params->v_ac_delta_q, bit_depth);
    const int use_qmatrix = av1_use_qmatrix(quant_params, xd, i);
    // NB: depends on base index so there is only 1 set per frame
    // No quant weighting when lossless or signalled not using QM
    const int qmlevel_y = use_qmatrix ? quant_params->qmatrix_level_y : NUM_QM_LEVELS - 1;
    for (int j = 0; j < TX_SIZES_ALL; ++j) {
      quant_params->y_iqmatrix[i][j] = av1_iqmatrix(quant_params, qmlevel_y, AOM_PLANE_Y, j);
    }
    const int qmlevel_u = use_qmatrix ? quant_params->qmatrix_level_u : NUM_QM_LEVELS - 1;
    for (int j = 0; j < TX_SIZES_ALL; ++j) {
      quant_params->u_iqmatrix[i][j] = av1_iqmatrix(quant_params, qmlevel_u, AOM_PLANE_U, j);
    }
    const int qmlevel_v = use_qmatrix ? quant_params->qmatrix_level_v : NUM_QM_LEVELS - 1;
    for (int j = 0; j < TX_SIZES_ALL; ++j) {
      quant_params->v_iqmatrix[i][j] = av1_iqmatrix(quant_params, qmlevel_v, AOM_PLANE_V, j);
    }
  }
}
...
static AOM_INLINE void setup_render_size(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
                                      ...
                                      int *height)
{
  ...
  if (aom_rb_read_bit(rb)) {
    cm->superres_scale_denominator = (uint8_t)aom_rb_read_literal(rb, SUPERRES_SCALE_BITS);
    cm->superres_scale_denominator += SUPERRES_SCALE_DENOMINATOR_MIN;
    // Don't edit cm->width or cm->height directly, or the buffers won't get
    // resized correctly
    av1_calculate_scaled_superres_size(width, height, cm->superres_scale_denominator);
  }
  else {
...
static AOM_INLINE void resize_context_buffers(AV1_COMMON *cm, int width, int height)
{
#if CONFIG_SIZE_LIMIT
  if (width > DECODE_WIDTH_LIMIT || height > DECODE_HEIGHT_LIMIT)
    aom_internal_error(&cm->error,
                       AOM_CODEC_CORRUPT_FRAME,
  ...
  if (cm->width != width || cm->height != height) {
    ...
    if (new_mi_cols > cm->mi_params.mi_cols || new_mi_rows > cm->mi_params.mi_rows) {
...
static AOM_INLINE void setup_buffer_pool(AV1_COMMON *cm)
{
  ...
  if (aom_realloc_frame_buffer(&cm->cur_frame->buf,
                               cm->width,
                               cm->height,
  ...
  cm->cur_frame->buf.transfer_characteristics = seq_params->transfer_characteristics;
  cm->cur_frame->buf.matrix_coefficients = seq_params->matrix_coefficients;
  cm->cur_frame->buf.monochrome = seq_params->monochrome;
  cm->cur_frame->buf.chroma_sample_position = seq_params->chroma_sample_position;
                                        ...
                                        struct aom_read_bit_buffer *rb)
{
  ...
  if (frame_size_override_flag) {
    ...
    if (width > seq_params->max_frame_width || height > seq_params->max_frame_height) {
      aom_internal_error(&cm->error,
                         AOM_CODEC_CORRUPT_FRAME,
...
static AOM_INLINE void setup_sb_size(SequenceHeader *seq_params, struct aom_read_bit_buffer *rb)
{
...
static INLINE int valid_ref_frame_img_fmt(aom_bit_depth_t ref_bit_depth,
                                          ...
                                          int this_yss)
{
  return ref_bit_depth == this_bit_depth && ref_xss == this_xss && ref_yss == this_yss;
}

static AOM_INLINE void setup_frame_size_with_refs(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  int width, height;
  int found = 0;
  ...
  for (int i = LAST_FRAME; i <= ALTREF_FRAME; ++i) {
    if (aom_rb_read_bit(rb)) {
      ...
      if (ref_buf == NULL) {
        aom_internal_error(
            &cm->error, AOM_CODEC_CORRUPT_FRAME, "Invalid condition: invalid reference buffer");
      }
      else {
  ...
  if (width <= 0 || height <= 0)
    aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "Invalid frame size");

  // Check to make sure at least one of frames that this frame references
  // has valid dimensions.
  for (int i = LAST_FRAME; i <= ALTREF_FRAME; ++i) {
    const RefCntBuffer *const ref_frame = get_ref_frame_buf(cm, i);
    has_valid_ref_frame |= valid_ref_frame_size(
        ref_frame->buf.y_crop_width, ref_frame->buf.y_crop_height, width, height);
  }
  if (!has_valid_ref_frame)
    aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "Referenced frame has invalid size");
  for (int i = LAST_FRAME; i <= ALTREF_FRAME; ++i) {
    const RefCntBuffer *const ref_frame = get_ref_frame_buf(cm, i);
    if (!valid_ref_frame_img_fmt(ref_frame->buf.bit_depth,
                                 ...
                                 seq_params->subsampling_y))
      aom_internal_error(
          &cm->error, AOM_CODEC_CORRUPT_FRAME, "Referenced frame has incompatible color format");
  }
  setup_buffer_pool(cm);
}
...
static AOM_INLINE void read_tile_info_max_tile(AV1_COMMON *const cm,
                                               struct aom_read_bit_buffer *const rb)
{
  const SequenceHeader *const seq_params = &cm->seq_params;
  CommonTileParams *const tiles = &cm->tiles;
  int width_mi = ALIGN_POWER_OF_TWO(cm->mi_params.mi_cols, seq_params->mib_size_log2);
  int height_mi = ALIGN_POWER_OF_TWO(cm->mi_params.mi_rows, seq_params->mib_size_log2);
  ...
  if (tiles->uniform_spacing) {
    ...
    for (i = 0, start_sb = 0; width_sb > 0 && i < MAX_TILE_COLS; i++) {
      const int size_sb = 1 + rb_read_uniform(rb, AOMMIN(width_sb, tiles->max_width_sb));
  ...
  av1_calculate_tile_cols(seq_params, cm->mi_params.mi_rows, cm->mi_params.mi_cols, tiles);
  ...
  if (tiles->uniform_spacing) {
    ...
    for (i = 0, start_sb = 0; height_sb > 0 && i < MAX_TILE_ROWS; i++) {
      const int size_sb = 1 + rb_read_uniform(rb, AOMMIN(height_sb, tiles->max_height_sb));
...
void av1_set_single_tile_decoding_mode(AV1_COMMON *const cm)
{
  ...
  if (cm->tiles.large_scale) {
    ...
    const int no_cdef = cdef_info->cdef_bits == 0 && cdef_info->cdef_strengths[0] == 0 &&
                        cdef_info->cdef_uv_strengths[0] == 0;
    const int no_restoration = rst_info[0].frame_restoration_type == RESTORE_NONE &&
...
static AOM_INLINE void read_tile_info(AV1Decoder *const pbi, struct aom_read_bit_buffer *const rb)
{
  AV1_COMMON *const cm = &pbi->common;
  ...
  if (cm->tiles.rows * cm->tiles.cols > 1) {
    ...
    pbi->context_update_tile_id = aom_rb_read_literal(rb,
                                                      cm->tiles.log2_rows + cm->tiles.log2_cols);
    if (pbi->context_update_tile_id >= cm->tiles.rows * cm->tiles.cols) {
      aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "Invalid context_update_tile_id");
    }
    // tile size magnitude
...
#if EXT_TILE_DEBUG
static AOM_INLINE void read_ext_tile_info(AV1Decoder *const pbi,
                                          struct aom_read_bit_buffer *const rb)
{
  AV1_COMMON *const cm = &pbi->common;
  ...
  if (mod > 0)
    aom_rb_read_literal(rb, CHAR_BIT - mod);
...
static AOM_INLINE void get_ls_tile_buffer(const uint8_t *const data_end,
                                          struct aom_internal_error_info *error_info,
                                          const uint8_t **data,
                                          ...
                                          int tile_copy_mode)
{
  ...
  *data += tile_size_bytes;
  ...
  if (size > (size_t)(data_end - *data))
    aom_internal_error(
        error_info, AOM_CODEC_CORRUPT_FRAME, "Truncated packet or corrupt tile size");
...
static const uint8_t *get_ls_tile_buffers(AV1Decoder *pbi,
                                          const uint8_t *data,
                                          const uint8_t *data_end,
                                          TileBufferDec (*const tile_buffers)[MAX_TILE_COLS])
{
  ...
  if (!have_tiles) {
    ...
    const int tile_copy_mode = ((AOMMAX(tile_width, tile_height) << MI_SIZE_LOG2) <= 256) ? 1 : 0;
...
static const uint8_t *get_ls_single_tile_buffer(AV1Decoder *pbi,
                                                const uint8_t *data,
                                                TileBufferDec (*const tile_buffers)[MAX_TILE_COLS])
{
  assert(pbi->dec_tile_row >= 0 && pbi->dec_tile_col >= 0);
  tile_buffers[pbi->dec_tile_row][pbi->dec_tile_col].data = data;
  tile_buffers[pbi->dec_tile_row][pbi->dec_tile_col].size = (size_t)pbi->coded_tile_data_size;
  return data + pbi->coded_tile_data_size;
}

// Reads the next tile returning its size and adjusting '*data' accordingly
// based on 'is_last'.
static AOM_INLINE void get_tile_buffer(const uint8_t *const data_end,
...
static AOM_INLINE void get_tile_buffers(AV1Decoder *pbi,
                                        const uint8_t *data,
                                        const uint8_t *data_end,
                                        TileBufferDec (*const tile_buffers)[MAX_TILE_COLS],
                                        int start_tile,
                                        int end_tile)
{
  ...
  for (int r = 0; r < tile_rows; ++r) {
    for (int c = 0; c < tile_cols; ++c, ++tc) {
      ...
      if (tc < start_tile || tc > end_tile)
        continue;

      if (data + hdr_offset >= data_end)
        aom_internal_error(
            &cm->error, AOM_CODEC_CORRUPT_FRAME, "Data ended before all tiles were read.");
      data += hdr_offset;
      get_tile_buffer(data_end, pbi->tile_size_bytes, is_last, &pbi->common.error, &data, buf);
    }
  }
}

static AOM_INLINE void set_cb_buffer(AV1Decoder *pbi,
                                     DecoderCodingBlock *dcb,
...
static AOM_INLINE void decoder_alloc_tile_data(AV1Decoder *pbi, const int n_tiles)
{
  AV1_COMMON *const cm = &pbi->common;
  aom_free(pbi->tile_data);
  CHECK_MEM_ERROR(cm, pbi->tile_data, aom_memalign(32, n_tiles * sizeof(*pbi->tile_data)));
...
static AOM_INLINE void dec_row_mt_alloc(AV1DecRowMTSync *dec_row_mt_sync, AV1_COMMON *cm, int rows)
{
...
#if CONFIG_MULTITHREAD
  {
    ...
    CHECK_MEM_ERROR(
        cm, dec_row_mt_sync->mutex_, aom_malloc(sizeof(*(dec_row_mt_sync->mutex_)) * rows));
    ...
    CHECK_MEM_ERROR(
        cm, dec_row_mt_sync->cond_, aom_malloc(sizeof(*(dec_row_mt_sync->cond_)) * rows));
  ...
  CHECK_MEM_ERROR(
      cm, dec_row_mt_sync->cur_sb_col, aom_malloc(sizeof(*(dec_row_mt_sync->cur_sb_col)) * rows));
...
static INLINE void sync_read(AV1DecRowMTSync *const dec_row_mt_sync, int r, int c)
{
#if CONFIG_MULTITHREAD
...
static INLINE void sync_write(AV1DecRowMTSync *const dec_row_mt_sync,
                              int r,
                              int c,
                              const int sb_cols)
{
...
static AOM_INLINE void decode_tile_sb_row(AV1Decoder *pbi,
                                          ThreadData *const td,
                                          TileInfo tile_info,
                                          const int mi_row)
{
  AV1_COMMON *const cm = &pbi->common;
  const int num_planes = av1_num_planes(cm);
  TileDataDec *const tile_data = pbi->tile_data + tile_info.tile_row * cm->tiles.cols +
                                 tile_info.tile_col;
  const int sb_cols_in_tile = av1_get_sb_cols_in_tile(cm, tile_info);
  const int sb_row_in_tile = (mi_row - tile_info.mi_row_start) >> cm->seq_params.mib_size_log2;
  int sb_col_in_tile = 0;

  for (int mi_col = tile_info.mi_col_start; mi_col < tile_info.mi_col_end;
       mi_col += cm->seq_params.mib_size, sb_col_in_tile++)
  {
    set_cb_buffer(pbi, &td->dcb, pbi->cb_buffer_base, num_planes, mi_row, mi_col);

    sync_read(&tile_data->dec_row_mt_sync, sb_row_in_tile, sb_col_in_tile);

    // Decoding of the super-block
    decode_partition(pbi, td, mi_row, mi_col, td->bit_reader, cm->seq_params.sb_size, 0x2);

    sync_write(&tile_data->dec_row_mt_sync, sb_row_in_tile, sb_col_in_tile, sb_cols_in_tile);
  }
}

static int check_trailing_bits_after_symbol_coder(aom_reader *r)
{
  if (aom_reader_has_overflowed(r))
    return -1;
...
static AOM_INLINE void set_decode_func_pointers(ThreadData *td, int parse_decode_flag)
{
...
static AOM_INLINE void decode_tile(AV1Decoder *pbi,
                                   ThreadData *const td,
                                   int tile_row,
                                   int tile_col)
{
  ...
  av1_zero_above_context(cm, xd, tile_info.mi_col_start, tile_info.mi_col_end, tile_row);
  ...
  int corrupted = (check_trailing_bits_after_symbol_coder(td->bit_reader)) ? 1 : 0;
  aom_merge_corrupted_flag(&dcb->corrupted, corrupted);
}

static const uint8_t *decode_tiles(
    AV1Decoder *pbi, const uint8_t *data, const uint8_t *data_end, int start_tile, int end_tile)
{
  AV1_COMMON *const cm = &pbi->common;
  ...
  for (tile_row = tile_rows_start; tile_row < tile_rows_end; ++tile_row) {
    ...
    for (tile_col = tile_cols_start; tile_col < tile_cols_end; ++tile_col) {
      ...
      if (row * tiles->cols + col < start_tile || row * tiles->cols + col > end_tile)
        continue;
      ...
      setup_bool_decoder(tile_bs_buf->data,
                         data_end,
                         tile_bs_buf->size,
                         &cm->error,
                         td->bit_reader,
      ...
      if (pbi->acct_enabled) {
        ...
        td->bit_reader->accounting->last_tell_frac = aom_reader_tell_frac(td->bit_reader);
      }
      else {
      ...
      av1_init_above_context(&cm->above_contexts, av1_num_planes(cm), row, &td->dcb.xd);
      ...
      if (pbi->dcb.corrupted)
        aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "Failed to decode tile data");
    }
  }
...
static AOM_INLINE void tile_worker_hook_init(AV1Decoder *const pbi,
                                             ...
                                             uint8_t allow_update_cdf)
{
  ...
  setup_bool_decoder(tile_buffer->data,
                     thread_data->data_end,
                     tile_buffer->size,
                     &thread_data->error_info,
                     td->bit_reader,
                     allow_update_cdf);
#if CONFIG_ACCOUNTING
  if (pbi->acct_enabled) {
...
#if CONFIG_ACCOUNTING
  if (pbi->acct_enabled) {
    tile_data->bit_reader.accounting->last_tell_frac = aom_reader_tell_frac(
...
static int tile_worker_hook(void *arg1, void *arg2)
{
  ...
  while (!td->dcb.corrupted) {
    ...
    if (cur_job_info != NULL) {
      ...
      tile_worker_hook_init(pbi, thread_data, tile_buffer, tile_data, allow_update_cdf);
      // decode tile
      int tile_row = tile_data->tile_info.tile_row;
      int tile_col = tile_data->tile_info.tile_col;
...
static INLINE int get_max_row_mt_workers_per_tile(AV1_COMMON *cm, TileInfo tile)
{
  ...
  int max_workers = sb_rows == 1 ? AOM_MIN_THREADS_PER_TILE : AOM_MAX_THREADS_PER_TILE;
  return max_workers;
}
                             ...
                             int *end_of_frame)
{
  ...
  *end_of_frame = (frame_row_mt_info->mi_rows_decode_started ==
  ...
  assert(frame_row_mt_info->mi_rows_parse_done >= frame_row_mt_info->mi_rows_decode_started);
  if (frame_row_mt_info->mi_rows_parse_done == frame_row_mt_info->mi_rows_decode_started)
    return 0;

  // Choose the tile to decode.
  for (tile_row_idx = tile_rows_start; tile_row_idx < tile_rows_end; ++tile_row_idx) {
    for (tile_col_idx = tile_cols_start; tile_col_idx < tile_cols_end; ++tile_col_idx) {
      ...
      num_mis_to_decode = (dec_row_mt_sync->mi_rows - dec_row_mt_sync->mi_rows_decode_started) *
                          dec_row_mt_sync->mi_cols;
      ...
      if (num_mis_waiting_for_decode > 0) {
        ...
        if (num_threads_working == min_threads_working && num_mis_to_decode > max_mis_to_decode &&
            num_threads_working < get_max_row_mt_workers_per_tile(cm, tile_data->tile_info))
        {
  ...
  next_job_info->mi_row = dec_row_mt_sync->mi_rows_decode_started + tile_info.mi_row_start;
  ...
  assert(frame_row_mt_info->mi_rows_parse_done >= frame_row_mt_info->mi_rows_decode_started);
#if CONFIG_MULTITHREAD
  if (frame_row_mt_info->mi_rows_decode_started == frame_row_mt_info->mi_rows_to_decode) {
...
static AOM_INLINE void parse_tile_row_mt(AV1Decoder *pbi,
                                         ThreadData *const td,
                                         TileDataDec *const tile_data)
{
  AV1_COMMON *const cm = &pbi->common;
       ...
       mi_row += cm->seq_params.mib_size)
  {
         ...
         mi_col += cm->seq_params.mib_size)
    {
      ...
      decode_partition(pbi, td, mi_row, mi_col, td->bit_reader, cm->seq_params.sb_size, 0x1);
...
static int row_mt_worker_hook(void *arg1, void *arg2)
{
  ...
  while (1) {
    ...
    TileDataDec *tile_data = pbi->tile_data + tile_row * cm->tiles.cols + tile_col;
...
static AOM_INLINE void enqueue_tile_jobs(AV1Decoder *pbi,
                                         AV1_COMMON *cm,
                                         int tile_rows_start,
                                         int tile_rows_end,
                                         int tile_cols_start,
                                         ...
                                         int end_tile)
{
  ...
  for (int row = tile_rows_start; row < tile_rows_end; row++) {
    for (int col = tile_cols_start; col < tile_cols_end; col++) {
      if (row * cm->tiles.cols + col < start_tile || row * cm->tiles.cols + col > end_tile)
        continue;
                                      ...
                                      int tile_cols)
{
  ...
  {
    CHECK_MEM_ERROR(
        cm, tile_mt_info->job_mutex, aom_malloc(sizeof(*tile_mt_info->job_mutex) * num_tiles));
...
#endif
  CHECK_MEM_ERROR(
      cm, tile_mt_info->job_queue, aom_malloc(sizeof(*tile_mt_info->job_queue) * num_tiles));
}
                                           ...
                                           int use_highbd)
{
  for (int ref = 0; ref < 2; ref++) {
    ...
    if (use_highbd) {
      ...
      CHECK_MEM_ERROR(cm, thread_data->mc_buf[ref], (uint8_t *)aom_memalign(16, buf_size));
  ...
  CHECK_MEM_ERROR(
      cm,
      thread_data->tmp_conv_dst,
      aom_memalign(32, MAX_SB_SIZE * MAX_SB_SIZE * sizeof(*thread_data->tmp_conv_dst)));
  for (int i = 0; i < 2; ++i) {
    CHECK_MEM_ERROR(
        cm,
        thread_data->tmp_obmc_bufs[i],
        aom_memalign(16,
                     2 * MAX_MB_PLANE * MAX_SB_SQUARE * sizeof(*thread_data->tmp_obmc_bufs[i])));
  }
}
                                         ...
                                         int num_workers)
{
  ...
  for (int worker_idx = 0; worker_idx < num_workers; ++worker_idx) {
    ...
    for (int j = 0; j < 2; ++j) {
      thread_data->td->dcb.xd.tmp_obmc_bufs[j] = thread_data->td->tmp_obmc_bufs[j];
    }
...
static AOM_INLINE void decode_mt_init(AV1Decoder *pbi)
{
  ...
  if (pbi->num_workers == 0) {
    ...
    CHECK_MEM_ERROR(cm, pbi->tile_workers, aom_malloc(num_threads * sizeof(*pbi->tile_workers)));
    CHECK_MEM_ERROR(cm, pbi->thread_data, aom_malloc(num_threads * sizeof(*pbi->thread_data)));
    ...
    for (worker_idx = 0; worker_idx < num_threads; ++worker_idx) {
      ...
      if (worker_idx != 0 && !winterface->reset(worker)) {
        aom_internal_error(&cm->error, AOM_CODEC_ERROR, "Tile decoder thread creation failed");
      }

      if (worker_idx != 0) {
        // Allocate thread data.
        CHECK_MEM_ERROR(cm, thread_data->td, aom_memalign(32, sizeof(*thread_data->td)));
...
static AOM_INLINE void tile_mt_queue(AV1Decoder *pbi,
                                     int tile_cols,
                                     int tile_rows,
                                     int tile_rows_start,
                                     int tile_rows_end,
                                     ...
                                     int end_tile)
{
  ...
  enqueue_tile_jobs(pbi,
                    ...
                    start_tile,
                    end_tile);
  qsort(pbi->tile_mt_info.job_queue,
        pbi->tile_mt_info.jobs_enqueued,
        sizeof(pbi->tile_mt_info.job_queue[0]),
        compare_tile_buffers);
}

static const uint8_t *decode_tiles_mt(
    AV1Decoder *pbi, const uint8_t *data, const uint8_t *data_end, int start_tile, int end_tile)
{
  AV1_COMMON *const cm = &pbi->common;
  CommonTileParams *const tiles = &cm->tiles;
...
static AOM_INLINE void dec_alloc_cb_buf(AV1Decoder *pbi)
{
  ...
  if (pbi->cb_buffer_alloc_size < size) {
    ...
    CHECK_MEM_ERROR(
        cm, pbi->cb_buffer_base, aom_memalign(32, sizeof(*pbi->cb_buffer_base) * size));
    memset(pbi->cb_buffer_base, 0, sizeof(*pbi->cb_buffer_base) * size);
    pbi->cb_buffer_alloc_size = size;
  }
}

static AOM_INLINE void row_mt_frame_init(AV1Decoder *pbi,
                                         int tile_rows_start,
                                         int tile_rows_end,
                                         int tile_cols_start,
                                         int tile_cols_end,
                                         ...
                                         int max_sb_rows)
{
  ...
  for (int tile_row = tile_rows_start; tile_row < tile_rows_end; ++tile_row) {
    for (int tile_col = tile_cols_start; tile_col < tile_cols_end; ++tile_col) {
      ...
      TileDataDec *const tile_data = pbi->tile_data + tile_row * cm->tiles.cols + tile_col;
      TileInfo tile_info = tile_data->tile_info;
      ...
      tile_data->dec_row_mt_sync.mi_rows = ALIGN_POWER_OF_TWO(
          tile_info.mi_row_end - tile_info.mi_row_start, cm->seq_params.mib_size_log2);
      tile_data->dec_row_mt_sync.mi_cols = ALIGN_POWER_OF_TWO(
          tile_info.mi_col_end - tile_info.mi_col_start, cm->seq_params.mib_size_log2);

      frame_row_mt_info->mi_rows_to_decode += tile_data->dec_row_mt_sync.mi_rows;

      // Initialize cur_sb_col to -1 for all SB rows.
      memset(tile_data->dec_row_mt_sync.cur_sb_col,
             -1,
  ...
  if (pbi->row_mt_mutex_ == NULL) {
    CHECK_MEM_ERROR(cm, pbi->row_mt_mutex_, aom_malloc(sizeof(*(pbi->row_mt_mutex_))));
  ...
  if (pbi->row_mt_cond_ == NULL) {
    CHECK_MEM_ERROR(cm, pbi->row_mt_cond_, aom_malloc(sizeof(*(pbi->row_mt_cond_))));
    if (pbi->row_mt_cond_) {
...
static const uint8_t *decode_tiles_row_mt(
    AV1Decoder *pbi, const uint8_t *data, const uint8_t *data_end, int start_tile, int end_tile)
{
  AV1_COMMON *const cm = &pbi->common;
  CommonTileParams *const tiles = &cm->tiles;
  ...
  for (int row = 0; row < tile_rows; row++) {
    for (int col = 0; col < tile_cols; col++) {
      ...
      max_sb_rows = AOMMAX(max_sb_rows, av1_get_sb_rows_in_tile(cm, tile_data->tile_info));
...
// aom_internal_error().
static AOM_INLINE void read_bitdepth(struct aom_read_bit_buffer *rb,
                                     ...
                                     struct aom_internal_error_info *error_info)
{
  ...
  if (seq_params->profile == PROFILE_2 && high_bitdepth) {
    ...
    aom_internal_error(
        error_info, AOM_CODEC_UNSUP_BITSTREAM, "Unsupported profile/bit-depth combination");
  }
#if !CONFIG_AV1_HIGHBITDEPTH
  if (seq_params->bit_depth > AOM_BITS_8) {
    aom_internal_error(error_info,
...
void av1_read_film_grain_params(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  ...
  if (!pars->update_parameters) {
    ...
    if (!found) {
      aom_internal_error(&cm->error,
                         ...
                         cm->remapped_ref_idx[0],
                         cm->remapped_ref_idx[1],
                         cm->remapped_ref_idx[2],
                         cm->remapped_ref_idx[3],
                         cm->remapped_ref_idx[4],
                         cm->remapped_ref_idx[5],
                         cm->remapped_ref_idx[6]);
    }
    RefCntBuffer *const buf = cm->ref_frame_map[film_grain_params_ref_idx];
    if (buf == NULL) {
      aom_internal_error(
          &cm->error, AOM_CODEC_UNSUP_BITSTREAM, "Invalid Film grain reference idx");
    }
    if (!buf->film_grain_params_present) {
      aom_internal_error(
          &cm->error, AOM_CODEC_UNSUP_BITSTREAM, "Film grain reference parameters not available");
    }
       ...
       (pars->num_y_points == 0)))
  {
    ...
    for (int i = 0; i < pars->num_cb_points; i++) {
      ...
      if (i && pars->scaling_points_cb[i - 1][0] >= pars->scaling_points_cb[i][0])
        aom_internal_error(&cm->error,
                           AOM_CODEC_UNSUP_BITSTREAM,
                           "First coordinate of the scaling function points "
                           "shall be increasing.");
    ...
    for (int i = 0; i < pars->num_cr_points; i++) {
      ...
      if (i && pars->scaling_points_cr[i - 1][0] >= pars->scaling_points_cr[i][0])
        aom_internal_error(&cm->error,
                           AOM_CODEC_UNSUP_BITSTREAM,
                           "First coordinate of the scaling function points "
                           "shall be increasing.");
...
static AOM_INLINE void read_film_grain(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  if (cm->seq_params.film_grain_params_present && (cm->show_frame || cm->showable_frame)) {
  ...
  memcpy(&cm->cur_frame->film_grain_params, &cm->film_grain_params, sizeof(aom_film_grain_t));
}
                           ...
                           struct aom_internal_error_info *error_info)
{
  ...
  seq_params->use_highbitdepth = seq_params->bit_depth > AOM_BITS_8 || !allow_lowbitdepth;
  // monochrome bit (not needed for PROFILE_1)
  const int is_monochrome = seq_params->profile != PROFILE_1 ? aom_rb_read_bit(rb) : 0;
      ...
      seq_params->matrix_coefficients == AOM_CICP_MC_IDENTITY)
  {
    ...
    if (!(seq_params->profile == PROFILE_1 ||
          (seq_params->profile == PROFILE_2 && seq_params->bit_depth == AOM_BITS_12)))
    {
      aom_internal_error(error_info,
                         AOM_CODEC_UNSUP_BITSTREAM,
                                 ...
                                 struct aom_read_bit_buffer *rb)
{
  ...
  if (timing_info->num_units_in_display_tick == 0 || timing_info->time_scale == 0) {
    aom_internal_error(error,
                       AOM_CODEC_UNSUP_BITSTREAM,
                       "num_units_in_display_tick and time_scale must be greater than 0.");
  }
  timing_info->equal_picture_interval = aom_rb_read_bit(rb);  // Equal picture interval bit
...
void av1_read_decoder_model_info(aom_dec_model_info_t *decoder_model_info,
                                 struct aom_read_bit_buffer *rb)
{
  decoder_model_info->encoder_decoder_buffer_delay_length = aom_rb_read_literal(rb, 5) + 1;
  decoder_model_info->num_units_in_decoding_tick = aom_rb_read_unsigned_literal(
      rb,
      32);  // Number of units in a decoding tick
  decoder_model_info->buffer_removal_time_length = aom_rb_read_literal(rb, 5) + 1;
  decoder_model_info->frame_presentation_time_length = aom_rb_read_literal(rb, 5) + 1;
}
...
void av1_read_op_parameters_info(aom_dec_model_op_parameters_t *op_params,
                                 ...
                                 struct aom_read_bit_buffer *rb)
{
  op_params->decoder_buffer_delay = aom_rb_read_unsigned_literal(rb, buffer_delay_length);
  op_params->encoder_buffer_delay = aom_rb_read_unsigned_literal(rb, buffer_delay_length);
...
void av1_read_sequence_header(AV1_COMMON *cm,
                              struct aom_read_bit_buffer *rb,
                              SequenceHeader *seq_params)
{
  ...
  if (seq_params->frame_id_numbers_present_flag) {
    ...
    seq_params->frame_id_length = aom_rb_read_literal(rb, 3) + seq_params->delta_frame_id_length +
                                  1;
  ...
  if (seq_params->reduced_still_picture_hdr) {
    ...
    seq_params->order_hint_info.order_hint_bits_minus_1 =
        seq_params->order_hint_info.enable_order_hint ? aom_rb_read_literal(rb, 3) : -1;
  }
                                     ...
                                     int allow_hp)
{
  ...
  *params = default_warp_params;
  ...
  if (type >= ROTZOOM) {
    ...
    params->wmmat[3] =
        aom_rb_read_signed_primitive_refsubexpfin(
            rb, GM_ALPHA_MAX + 1, SUBEXPFIN_K, (ref_params->wmmat[3] >> GM_ALPHA_PREC_DIFF)) *
        GM_ALPHA_DECODE_FACTOR;
  }
  ...
  if (type >= AFFINE) {
    params->wmmat[4] =
        aom_rb_read_signed_primitive_refsubexpfin(
            rb, GM_ALPHA_MAX + 1, SUBEXPFIN_K, (ref_params->wmmat[4] >> GM_ALPHA_PREC_DIFF)) *
        GM_ALPHA_DECODE_FACTOR;
  ...
  if (type >= TRANSLATION) {
    const int trans_bits = (type == TRANSLATION) ? GM_ABS_TRANS_ONLY_BITS - !allow_hp :
                                                   GM_ABS_TRANS_BITS;
    const int trans_dec_factor = (type == TRANSLATION) ?
                                     GM_TRANS_ONLY_DECODE_FACTOR * (1 << !allow_hp) :
                                     GM_TRANS_DECODE_FACTOR;
    const int trans_prec_diff = (type == TRANSLATION) ? GM_TRANS_ONLY_PREC_DIFF + !allow_hp :
                                                        GM_TRANS_PREC_DIFF;
    params->wmmat[0] =
        aom_rb_read_signed_primitive_refsubexpfin(
            rb, (1 << trans_bits) + 1, SUBEXPFIN_K, (ref_params->wmmat[0] >> trans_prec_diff)) *
        trans_dec_factor;
    params->wmmat[1] =
        aom_rb_read_signed_primitive_refsubexpfin(
            rb, (1 << trans_bits) + 1, SUBEXPFIN_K, (ref_params->wmmat[1] >> trans_prec_diff)) *
        trans_dec_factor;
  }
...
static AOM_INLINE void read_global_motion(AV1_COMMON *cm, struct aom_read_bit_buffer *rb)
{
  for (int frame = LAST_FRAME; frame <= ALTREF_FRAME; ++frame) {
    const WarpedMotionParams *ref_params = cm->prev_frame ? &cm->prev_frame->global_motion[frame] :
                                                            &default_warp_params;
    int good_params = read_global_motion_params(
        &cm->global_motion[frame], ref_params, rb, cm->features.allow_high_precision_mv);
    if (!good_params) {
  ...
  memcpy(cm->cur_frame->global_motion, cm->global_motion, REF_FRAMES * sizeof(WarpedMotionParams));
}
...
static AOM_INLINE void show_existing_frame_reset(AV1Decoder *const pbi, int existing_frame_idx)
{
  AV1_COMMON *const cm = &pbi->common;
...
// return.
static int read_uncompressed_header(AV1Decoder *pbi, struct aom_read_bit_buffer *rb)
{
  AV1_COMMON *const cm = &pbi->common;
  const SequenceHeader *const seq_params = &cm->seq_params;
  ...
  if (!pbi->sequence_header_ready) {
    aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "No sequence header");
  }
  ...
  if (seq_params->reduced_still_picture_hdr) {
    ...
    if (cm->show_existing_frame) {
      ...
      if (frame_to_show == NULL) {
        aom_internal_error(
            &cm->error, AOM_CODEC_UNSUP_BITSTREAM, "Buffer does not contain a decoded frame");
      }
      ...
      if (seq_params->frame_id_numbers_present_flag) {
        ...
        if (display_frame_id != cm->ref_frame_id[existing_frame_idx] ||
          ...
          aom_internal_error(
              &cm->error, AOM_CODEC_CORRUPT_FRAME, "Reference buffer frame ID mismatch");
      }
      lock_buffer_pool(pool);
      ...
      if (!frame_to_show->showable_frame) {
        aom_internal_error(
            &cm->error, AOM_CODEC_UNSUP_BITSTREAM, "Buffer does not contain a showable frame");
      }
      // Section 6.8.2: It is a requirement of bitstream conformance that when
      ...
      if (pbi->reset_decoder_state)
        frame_to_show->showable_frame = 0;
    ...
    if (seq_params->still_picture && (current_frame->frame_type != KEY_FRAME || !cm->show_frame)) {
      aom_internal_error(
          &cm->error, AOM_CODEC_CORRUPT_FRAME, "Still pictures must be coded as shown keyframes");
    }
    ...
    features->error_resilient_mode = frame_is_sframe(cm) ||
                                             (current_frame->frame_type == KEY_FRAME &&
                                              cm->show_frame) ?
                                         1 :
                                         aom_rb_read_bit(rb);
  ...
  if (seq_params->force_screen_content_tools == 2) {
    ...
    features->allow_screen_content_tools = seq_params->force_screen_content_tools;
  }
  ...
  if (!seq_params->reduced_still_picture_hdr) {
    if (seq_params->frame_id_numbers_present_flag) {
      ...
      if (have_prev_frame_id) {
        ...
        if (prev_frame_id == cm->current_frame_id || diff_frame_id >= (1 << (frame_id_length - 1)))
        {
          aom_internal_error(
              &cm->error, AOM_CODEC_CORRUPT_FRAME, "Invalid value of current_frame_id");
        }
      }
    ...
    if (cm->buffer_removal_time_present) {
      for (int op_num = 0; op_num < seq_params->operating_points_cnt_minus_1 + 1; op_num++) {
        if (seq_params->op_params[op_num].decoder_model_param_present_flag) {
          if ((((seq_params->operating_point_idc[op_num] >> cm->temporal_layer_id) & 0x1) &&
               ((seq_params->operating_point_idc[op_num] >> (cm->spatial_layer_id + 8)) & 0x1)) ||
  ...
  if (current_frame->frame_type == KEY_FRAME) {
    ...
    if (current_frame->frame_type == INTRA_ONLY_FRAME) {
      ...
      current_frame->refresh_frame_flags = frame_is_sframe(cm) ?
                                               0xFF :
  ...
  if (!frame_is_intra_only(cm) || current_frame->refresh_frame_flags != 0xFF) {
    ...
    if (features->error_resilient_mode && seq_params->order_hint_info.enable_order_hint) {
      for (int ref_idx = 0; ref_idx < REF_FRAMES; ref_idx++) {
        ...
        if (buf == NULL || order_hint != buf->order_hint) {
          ...
          lock_buffer_pool(pool);
          if (aom_realloc_frame_buffer(&buf->buf,
                                       ...
                                       features->byte_alignment,
                                       &buf->raw_frame_buffer,
                                       pool->get_fb_cb,
                                       pool->cb_priv))
          {
            decrease_ref_count(buf, pool);
  ...
  if (current_frame->frame_type == KEY_FRAME) {
    ...
    if (current_frame->frame_type == INTRA_ONLY_FRAME) {
      cm->cur_frame->film_grain_params_present = seq_params->film_grain_params_present;
      setup_frame_size(cm, frame_size_override_flag, rb);
      if (features->allow_screen_content_tools && !av1_superres_scaled(cm))
        features->allow_intrabc = aom_rb_read_bit(rb);
    }
      ...
      if (frame_refs_short_signaling) {
        ...
        if (lst_buf == NULL)
          aom_internal_error(
              &cm->error, AOM_CODEC_CORRUPT_FRAME, "Inter frame requests nonexistent reference");
        if (gld_buf == NULL)
          aom_internal_error(
              &cm->error, AOM_CODEC_CORRUPT_FRAME, "Inter frame requests nonexistent reference");
      ...
      for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
        ...
        if (pbi->valid_for_referencing[ref] == 0)
          aom_internal_error(
              &cm->error, AOM_CODEC_CORRUPT_FRAME, "Reference frame not valid for referencing");
        ...
        if (seq_params->frame_id_numbers_present_flag) {
          ...
          int ref_frame_id = ((cm->current_frame_id - (delta_frame_id_minus_1 + 1) +
                               (1 << frame_id_length)) %
                              (1 << frame_id_length));
    ...
    if (features->primary_ref_frame != PRIMARY_REF_NONE && get_primary_ref_frame_buf(cm) == NULL) {
      aom_internal_error(&cm->error,
                         AOM_CODEC_CORRUPT_FRAME,
    ...
    if (!(current_frame->frame_type == INTRA_ONLY_FRAME) && pbi->need_resync != 1) {
      ...
      for (int i = LAST_FRAME; i <= ALTREF_FRAME; ++i) {
        const RefCntBuffer *const ref_buf = get_ref_frame_buf(cm, i);
        struct scale_factors *const ref_scale_factors = get_ref_scale_factors(cm, i);
        ...
        if ((!av1_is_valid_scale(ref_scale_factors)))
          aom_internal_error(
              &cm->error, AOM_CODEC_UNSUP_BITSTREAM, "Reference frame has invalid dimensions");
      }
    }
  }
  ...
  if (might_bwd_adapt) {
    features->refresh_frame_context = aom_rb_read_bit(rb) ? REFRESH_FRAME_CONTEXT_DISABLED :
  ...
  if (!av1_is_min_tile_width_satisfied(cm)) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Minimum tile width requirement not satisfied");
  }

  CommonQuantParams *const quant_params = &cm->quant_params;
  setup_quantization(quant_params, av1_num_planes(cm), cm->seq_params.separate_uv_delta_q, rb);
      ...
      above_contexts->num_tile_rows < cm->tiles.rows)
  {
    ...
    if (av1_alloc_above_context_buffers(
            above_contexts, cm->tiles.rows, cm->mi_params.mi_cols, av1_num_planes(cm)))
    {
      aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR, "Failed to allocate context buffers");
    }
  }
  ...
  cm->delta_q_info.delta_q_present_flag = quant_params->base_qindex > 0 ? aom_rb_read_bit(rb) : 0;
  if (cm->delta_q_info.delta_q_present_flag) {
  ...
  current_frame->skip_mode_info.skip_mode_flag = current_frame->skip_mode_info.skip_mode_allowed ?
                                                     aom_rb_read_bit(rb) :
                                                     0;
  ...
  if (features->allow_ref_frame_mvs && !frame_might_allow_ref_frame_mvs(cm)) {
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Frame wrongly requests reference frame MVs");
  }
...
struct aom_read_bit_buffer *av1_init_read_bit_buffer(AV1Decoder *pbi,
                                                     struct aom_read_bit_buffer *rb,
                                                     const uint8_t *data,
                                                     const uint8_t *data_end)
{
...
void av1_read_frame_size(struct aom_read_bit_buffer *rb,
                         int num_bits_width,
                         int num_bits_height,
                         int *width,
                         int *height)
                                            ...
                                            int trailing_bits_present)
{
  ...
  aom_bitstream_queue_set_frame_read(cm->current_frame.order_hint * 2 + cm->show_frame);
#endif
#if CONFIG_MISMATCH_DEBUG
  ...
  if (!cm->tiles.single_tile_decoding && (pbi->dec_tile_row >= 0 || pbi->dec_tile_col >= 0)) {
    pbi->dec_tile_row = -1;
    pbi->dec_tile_col = -1;
  }

  const uint32_t uncomp_hdr_size = (uint32_t)aom_rb_bytes_read(
      rb);  // Size of the uncompressed header
  YV12_BUFFER_CONFIG *new_fb = &cm->cur_frame->buf;
  xd->cur_buf = new_fb;
  if (av1_allow_intrabc(cm)) {
    av1_setup_scale_factors_for_frame(&cm->sf_identity,
                                      xd->cur_buf->y_crop_width,
    ...
    if (pbi->reset_decoder_state) {
      ...
      *cm->fc = *cm->default_frame_context;
      if (!cm->fc->initialized)
        aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME, "Uninitialized entropy context.");
    }
  ...
  av1_setup_block_planes(
      xd, cm->seq_params.subsampling_x, cm->seq_params.subsampling_y, num_planes);
  if (cm->features.primary_ref_frame == PRIMARY_REF_NONE) {
    // use the default frame context values
    *cm->fc = *cm->default_frame_context;
  }
                                    ...
                                    int initialize_flag)
{
  ...
  if (pbi->max_threads > 1 && !(tiles->large_scale && !pbi->ext_tile_debug) && pbi->row_mt)
    *p_data_end = decode_tiles_row_mt(pbi, data, data_end, start_tile, end_tile);
  ...
  if (!cm->features.allow_intrabc && !tiles->single_tile_decoding) {
    if (cm->lf.filter_level[0] || cm->lf.filter_level[1]) {
      if (pbi->num_workers > 1) {
        ...
        av1_loop_filter_frame(&cm->cur_frame->buf,
                              cm,
                              &pbi->dcb.xd,
#if CONFIG_LPF_MASK
                              1,
    ...
    const int do_cdef = !pbi->skip_loop_filter && !cm->features.coded_lossless &&
...
#if !CONFIG_REALTIME_ONLY
    const int do_loop_restoration = cm->rst_info[0].frame_restoration_type != RESTORE_NONE ||
                                    cm->rst_info[1].frame_restoration_type != RESTORE_NONE ||
                                    cm->rst_info[2].frame_restoration_type != RESTORE_NONE;
    if (!optimized_loop_restoration) {
      if (do_loop_restoration)
        av1_loop_restoration_save_boundary_lines(&pbi->common.cur_frame->buf, cm, 0);
      ...
      if (do_loop_restoration) {
        av1_loop_restoration_save_boundary_lines(&pbi->common.cur_frame->buf, cm, 1);
        if (pbi->num_workers > 1) {
          av1_loop_restoration_filter_frame_mt((YV12_BUFFER_CONFIG *)xd->cur_buf,
                                               cm,
          ...
          av1_loop_restoration_filter_frame(
              (YV12_BUFFER_CONFIG *)xd->cur_buf, cm, optimized_loop_restoration, &pbi->lr_ctxt);
        }
      }
    }
    else {
  ...
  if (!pbi->dcb.corrupted) {
    ...
    aom_internal_error(
        &cm->error, AOM_CODEC_CORRUPT_FRAME, "Decode failed. Frame data is corrupted.");
  }
