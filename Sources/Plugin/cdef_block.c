...
void av1_cdef_filter_fb(uint8_t *dst8,
                        uint16_t *dst16,
                        int dstride,
                        uint16_t *in,
                        int xdec,
                        ...
                        int coeff_shift)
{
  ...
  const int bsize = ydec ? (xdec ? BLOCK_4X4 : BLOCK_8X4) : (xdec ? BLOCK_4X8 : BLOCK_8X8);
  ...
  for (bi = 0; bi < cdef_count; bi++) {
    ...
    bx = dlist[bi].bx;
    if (dst8) {
      cdef_filter_block(&dst8[(by << bh_log2) * dstride + (bx << bw_log2)],
                        NULL,
                        dstride,
