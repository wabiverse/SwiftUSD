...
#include "Plugin/hioAvif/aom/aom_dsp/recenter.h"
...
{
  int leading_zeros = 0;
  while (leading_zeros < 32 && !aom_rb_read_bit(rb))
    ++leading_zeros;
  // Maximum 32 bits.
  if (leading_zeros == 32)
    return UINT32_MAX;
...
static uint16_t aom_rb_read_primitive_quniform(struct aom_read_bit_buffer *rb, uint16_t n)
{
  if (n <= 1)
    return 0;
...
static uint16_t aom_rb_read_primitive_refsubexpfin(struct aom_read_bit_buffer *rb,
                                                   uint16_t n,
                                                   uint16_t k,
                                                   uint16_t ref)
{
  return inv_recenter_finite_nonneg(n, ref, aom_rb_read_primitive_subexpfin(rb, n, k));
}

int16_t aom_rb_read_signed_primitive_refsubexpfin(struct aom_read_bit_buffer *rb,
                                                  uint16_t n,
                                                  uint16_t k,
                                                  int16_t ref)
{
