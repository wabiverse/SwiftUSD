      ...
      for (int k = -4; k < AOMMIN(4, p_row + p_height - i - 4); ++k) {
        ...
        for (int l = -4; l < AOMMIN(4, p_col + p_width - j - 4); ++l) {
          ...
          if (conv_params->is_compound) {
            CONV_BUF_TYPE *p = &conv_params->dst[(i - p_row + k + 4) * conv_params->dst_stride +
                                                 (j - p_col + l + 4)];
...
void highbd_warp_plane(WarpedMotionParams *wm,
...
int64_t av1_calc_highbd_frame_error(const uint16_t *const ref,
                                    int stride,
...
static int64_t highbd_segmented_frame_error(const uint16_t *const ref,
                                            int stride,
                                            const uint16_t *const dst,
                                            int p_width,
                                            int p_height,
  ...
  for (int i = 0; i < p_height; i += WARP_ERROR_BLOCK) {
    for (int j = 0; j < p_width; j += WARP_ERROR_BLOCK) {
      ...
      sum_error += av1_calc_highbd_frame_error(
          ref + j + i * stride, stride, dst + j + i * p_stride, patch_w, patch_h, p_stride, bd);
    }
  }
  return sum_error;
}
...
void warp_plane(WarpedMotionParams *wm,
...
static int64_t segmented_frame_error(const uint8_t *const ref,
                                     int stride,
                                     const uint8_t *const dst,
                                     int p_width,
                                     int p_height,
                                     ...
                                     int segment_map_stride)
{
  ...
  for (int i = 0; i < p_height; i += WARP_ERROR_BLOCK) {
    for (int j = 0; j < p_width; j += WARP_ERROR_BLOCK) {
      ...
      patch_w = AOMMIN(error_bsize_w, p_width - j);
      patch_h = AOMMIN(error_bsize_h, p_height - i);
      sum_error += av1_calc_frame_error(
          ref + j + i * stride, stride, dst + j + i * p_stride, patch_w, patch_h, p_stride);
    }
  }
  return sum_error;
}

int64_t av1_frame_error(int use_hbd,
                        int bd,
                        const uint8_t *ref,
                        int stride,
                        uint8_t *dst,
...
#if CONFIG_AV1_HIGHBITDEPTH
  if (use_hbd) {
    return av1_calc_highbd_frame_error(CONVERT_TO_SHORTPTR(ref),
                                       stride,
...
int64_t av1_segmented_frame_error(int use_hbd,
                                  int bd,
                                  const uint8_t *ref,
                                  int stride,
                                  uint8_t *dst,
                                  ...
                                  int segment_map_stride)
{
#if CONFIG_AV1_HIGHBITDEPTH
  if (use_hbd) {
    return highbd_segmented_frame_error(CONVERT_TO_SHORTPTR(ref),
                                        stride,
                                        CONVERT_TO_SHORTPTR(dst),
                                        p_width,
                                        p_height,
  ...
  (void)use_hbd;
  (void)bd;
  return segmented_frame_error(
      ref, stride, dst, p_width, p_height, p_stride, segment_map, segment_map_stride);
}
...
static int find_affine_int(int np,
                           const int *pts1,
                           const int *pts2,
                           BLOCK_SIZE bsize,
                           int mvy,
...
int av1_find_projection(int np,
                        const int *pts1,
                        const int *pts2,
                        BLOCK_SIZE bsize,
                        int mvy,
