...
#define swap_endian_32(val, raw) \
  do { \
    val = ((raw >> 24) & 0x000000ff) | ((raw >> 8) & 0x0000ff00) | ((raw << 8) & 0x00ff0000) | \
          ((raw << 24) & 0xff000000); \
  } while (0)
...
#define mem_get_ne_aligned_generic(end, sz) \
  static AOM_INLINE unsigned MEM_VALUE_T mem_get_##end##sz##_aligned(const void *vmem) \
  { \
...
#define mem_get_sne_aligned_generic(end, sz) \
  static AOM_INLINE signed MEM_VALUE_T mem_get_s##end##sz##_aligned(const void *vmem) \
  { \
...
#define mem_put_ne_aligned_generic(end, sz) \
  static AOM_INLINE void mem_put_##end##sz##_aligned(void *vmem, MEM_VALUE_T val) \
  { \
