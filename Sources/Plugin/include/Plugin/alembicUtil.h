...
template<> struct PODTraitsFromType<PXR_NS::GfHalf> : public Float16PODTraits {};
}  // namespace ALEMBIC_VERSION_NS
}  // namespace Util
...
template<typename T>
using UsdAbc_SetParameter = std::conditional<std::is_arithmetic_v<T>,
                                             std::add_const_t<T>,
...
struct UsdAbc_AlembicType {
  ...
  friend bool operator!=(UsdAbc_AlembicType const &lhs, UsdAbc_AlembicType const &rhs)
  {
    return !(lhs == rhs);
  }
...
class UsdAbc_AlembicDataAny {
 ...
 private:
  ...
  // Visitor for assignment.
  template<class T> struct _SetTyped {
...
class _SampleForAlembic {
 public:
  ...
  _SampleForAlembic(const Error &error)
      : _numSamples(0), _value(_HolderValue(new _ErrorHolder(error.message)))
  {
    // Do nothing
  }
  ...
  _SampleForAlembic(const T &value)
      : _numSamples(1), _value(_HolderValue(new _RawHolder<T>(value)))
  {
    // Do nothing
  }
  ...
  _SampleForAlembic(const std::vector<T> &value)
      : _numSamples(value.size()), _value(_MakeRawArrayHolder(value))
  {
    // Do nothing
  }
