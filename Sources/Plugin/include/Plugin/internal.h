...
#include "Plugin/hioAvif/AVIF/src/avif/avif.h"  // IWYU pragma: export
...
// Used for debugging. Define AVIF_BREAK_ON_ERROR to catch the earliest failure during encoding or
// decoding.
...
// Returns a function to map from gamma-encoded values in the [0.0, 1.0] range to linear extended
// SDR values. Extended SDR values are in [0.0, 1.0] for SDR transfer chracteristics (all transfer
// characteristics except PQ and HLG) and can go beyond 1.0 for HDR transfer characteristics:
...
avifTransferFunction avifTransferCharacteristicsGetGammaToLinearFunction(
    avifTransferCharacteristics atc);
// Same as above in the opposite direction. toGamma(toLinear(v)) ~= v.
avifTransferFunction avifTransferCharacteristicsGetLinearToGammaFunction(
    avifTransferCharacteristics atc);
...
AVIF_NODISCARD avifBool avifArrayCreate(void *arrayStruct,
                                        uint32_t elementSize,
...
AVIF_NODISCARD avifBool avifDoubleToSignedFraction(double v,
                                                   int32_t *numerator,
                                                   uint32_t *denominator);
// Creates a uint32 fraction that is approximately equal to 'v'.
// Returns AVIF_FALSE if 'v' is < 0 or > UINT32_MAX or NaN.
AVIF_NODISCARD avifBool avifDoubleToUnsignedFraction(double v,
                                                     uint32_t *numerator,
                                                     uint32_t *denominator);
...
// If the AVIF_PLANES_YUV bit is set in planes, then srcImage and dstImage must have the same
// yuvFormat and yuvRange. Ignores the gainMap field (which exists only if
...
typedef enum avifReformatMode {
  ...
  AVIF_REFORMAT_MODE_IDENTITY,              // Pack GBR directly into YUV planes
...
typedef struct avifYUVColorSpaceInfo {
  ...
  float biasUV;   // The value of 0.5 for the appropriate bit depth (128 for 8 bit, 512 for 10 bit,
                  // 2048 for 12 bit).
...
// Retrieves the pixel value at position (x, y) expressed as floats in [0, 1]. If the image's
// format doesn't have alpha, rgbaPixel[3] is set to 1.0f.
void avifGetRGBAPixel(const avifRGBImage *src,
                      uint32_t x,
                      uint32_t y,
                      const avifRGBColorSpaceInfo *info,
                      float rgbaPixel[4]);
// Sets the pixel value at position (i, j) from RGBA values expressed as floats in [0, 1]. If the
// image's format doesn't support alpha, rgbaPixel[3] is ignored.
...
// * AVIF_RESULT_NOT_IMPLEMENTED - The fast path for this combination is not implemented with
// libyuv, use built-in RGB conversion
...
// * reformatAlpha - if set to AVIF_TRUE, the function will attempt to copy the alpha channel to
// the output RGB image using libyuv.
// * alphaReformattedWithLibYUV - Output parameter. If reformatAlpha is set to true and libyuv was
// able to copy over the alpha channel, then this will be set to AVIF_TRUE. Otherwise, this will be
// set to AVIF_FALSE. The value in this parameter is valid only if the return value of the function
// is AVIF_RESULT_OK or AVIF_RESULT_NOT_IMPLEMENTED. Returns:
// * AVIF_RESULT_OK              - Converted successfully with libyuv
// * AVIF_RESULT_NOT_IMPLEMENTED - The fast path for this combination is not implemented with
// libyuv, use built-in YUV conversion
// * [any other error]           - Return error to caller
avifResult avifImageYUVToRGBLibYUV(const avifImage *image,
                                   avifRGBImage *rgb,
                                   avifBool reformatAlpha,
                                   avifBool *alphaReformattedWithLibYUV);

// Returns:
// * AVIF_RESULT_OK              - Converted successfully with libsharpyuv
// * AVIF_RESULT_NOT_IMPLEMENTED - libsharpyuv is not compiled in, or doesn't support this type of
// input
// * [any other error]           - Return error to caller
avifResult avifImageRGBToYUVLibSharpYUV(avifImage *image,
                                        const avifRGBImage *rgb,
                                        const avifReformatState *state);

// Returns:
// * AVIF_RESULT_OK               - Converted successfully with libyuv.
// * AVIF_RESULT_NOT_IMPLEMENTED  - The fast path for this conversion is not implemented with
// libyuv, use built-in conversion.
...
// * AVIF_RESULT_NOT_IMPLEMENTED - The fast path for this combination is not implemented with
// libyuv, use built-in (Un)Premultiply
// * [any other error]           - Return error to caller
...
AVIF_NODISCARD avifBool avifDimensionsTooLarge(uint32_t width,
                                               uint32_t height,
...
// Attempts to parse the image->exif payload for Exif orientation and sets image->transformFlags,
// image->irot and image->imir on success. Returns AVIF_RESULT_INVALID_EXIF_PAYLOAD on failure.
avifResult avifImageExtractExifOrientationToIrotImir(avifImage *image);

// ---------------------------------------------------------------------------
// avifCodecDecodeInput

// Legal spatial_id values are [0,1,2,3], so this serves as a sentinel value for "do not filter by
// spatial_id"
...
typedef struct avifDecodeSample {
  ...
  uint32_t itemID;  // if non-zero, data comes from a mergedExtents buffer in an avifDecoderItem,
                    // not a file offset
  uint64_t offset;  // additional offset into data. Can be used to offset into an itemID's payload
                    // as well.
  size_t size;      //
  uint8_t
      spatialID;  // If set to a value other than AVIF_SPATIAL_ID_UNSET, output frames from this
                  // sample should be skipped until the output frame's spatial_id matches this ID.
...
typedef struct avifCodecDecodeInput {
  ...
  avifBool
      allLayers;  // if true, the underlying codec must decode all layers, not just the best layer
...
avifResult avifCodecSpecificOptionsSet(avifCodecSpecificOptions *csOptions,
                                       const char *key,
                                       const char *value);  // if(value==NULL), key is deleted
...
// encoded and EncodeFinish is called, the number of samples emitted must match the number of
// submitted frames. avifCodecEncodeImageFunc may return AVIF_RESULT_UNKNOWN_ERROR to automatically
// emit the appropriate AVIF_RESULT_ENCODE_COLOR_FAILED or AVIF_RESULT_ENCODE_ALPHA_FAILED
// depending on the alpha argument. avifCodecEncodeImageFunc should use tileRowsLog2 and
// tileColsLog2 instead of encoder->tileRowsLog2, encoder->tileColsLog2, and encoder->autoTiling.
// The caller of avifCodecEncodeImageFunc is responsible for automatic tiling if
// encoder->autoTiling is set to AVIF_TRUE. The actual tiling values are passed to
// avifCodecEncodeImageFunc as parameters. Similarly, avifCodecEncodeImageFunc should use the
// quantizer parameter instead of encoder->quality and encoder->qualityAlpha. If
// disableLaggedOutput is AVIF_TRUE, then the encoder will emit the output frame without any lag
// (if supported). Note that disableLaggedOutput is only used by the first call to this function
// (which initializes the encoder) and is ignored by the subsequent calls.
//
...
typedef avifBool (*avifCodecEncodeFinishFunc)(struct avifCodec *codec,
                                              avifCodecEncodeOutput *output);
typedef void (*avifCodecDestroyInternalFunc)(struct avifCodec *codec);

typedef struct avifCodec {
  avifCodecSpecificOptions *csOptions;  // Contains codec-specific key/value pairs for advanced
                                        // tuning. If a codec uses a value, it must mark it as
                                        // used. This array is NOT owned by avifCodec.
...
avifResult avifCodecCreate(avifCodecChoice choice,
...
AVIF_NODISCARD avifCodec *avifCodecCreateGav1(
    void);                               // requires AVIF_CODEC_LIBGAV1 (codec_libgav1.c)
...
void avifROStreamStart(avifROStream *stream,
                       avifROData *raw,
                       avifDiagnostics *diag,
                       const char *diagContext);
...
AVIF_NODISCARD avifBool avifROStreamReadU16Endianness(avifROStream *stream,
                                                      uint16_t *v,
                                                      avifBool littleEndian);
AVIF_NODISCARD avifBool avifROStreamReadU32(avifROStream *stream, uint32_t *v);
AVIF_NODISCARD avifBool avifROStreamReadU32Endianness(avifROStream *stream,
                                                      uint32_t *v,
                                                      avifBool littleEndian);
AVIF_NODISCARD avifBool
avifROStreamReadUX8(avifROStream *stream,
                    uint64_t *v,
                    uint64_t factor);  // Reads a factor*8 sized uint, saves in v
AVIF_NODISCARD avifBool avifROStreamReadU64(avifROStream *stream, uint64_t *v);
AVIF_NODISCARD avifBool avifROStreamReadString(avifROStream *stream,
                                               char *output,
                                               size_t outputSize);
AVIF_NODISCARD avifBool avifROStreamReadBoxHeader(
    avifROStream *stream,
    avifBoxHeader
        *header);  // This fails if the size reported by the header cannot fit in the stream
AVIF_NODISCARD avifBool avifROStreamReadBoxHeaderPartial(
    avifROStream *stream,
    avifBoxHeader *header);  // This doesn't require that the full box can fit in the stream
AVIF_NODISCARD avifBool
avifROStreamReadVersionAndFlags(avifROStream *stream,
                                uint8_t *version,
                                uint32_t *flags);  // version and flags ptrs are both optional
AVIF_NODISCARD avifBool avifROStreamReadAndEnforceVersion(
    avifROStream *stream, uint8_t enforcedVersion);  // currently discards flags
...
// (with a call to avifRWStreamFinishBox)
...
AVIF_NODISCARD avifBool avifSequenceHeaderParse(avifSequenceHeader *header,
                                                const avifROData *sample,
                                                avifCodecType codecType);

#if defined(AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP)
// Performs tone mapping on a base image using the provided gain map.
// The HDR headroom is log2 of the ratio of HDR to SDR white brightness of the display to tone map
// for. 'toneMappedImage' should have the 'format', 'depth', and 'isFloat' fields set to the
// desired values. If non NULL, 'clli' will be filled with the light level information of the tone
// mapped image. NOTE: only used in tests for now, might be added to the public API at some point.
struct avifRGBImage;
