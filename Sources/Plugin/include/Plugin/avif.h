...
#define AVIF_VERSION \
  ((AVIF_VERSION_MAJOR * 1000000) + (AVIF_VERSION_MINOR * 10000) + (AVIF_VERSION_PATCH * 100) + \
   AVIF_VERSION_DEVEL)
...
// a 12 hour AVIF image sequence, running at 60 fps (a basic sanity check as this is quite
// ridiculous)
...
// This value is used if an animated AVIF file does not have repetitions specified using an
// EditList box. Applications can choose to handle this case however they want.
...
typedef enum avifChannelIndex {
  ...
  // This may not be used in yuvPlanes and yuvRowBytes, but is available for use with
  // avifImagePlane().
  AVIF_CHAN_A = 3
} avifChannelIndex;
...
AVIF_API unsigned int avifLibYUVVersion(
    void);  // returns 0 if libavif wasn't compiled with libyuv support
...
typedef enum AVIF_NODISCARD avifResult {
  ...
  AVIF_RESULT_WAITING_ON_IO = 23,  // similar to EAGAIN/EWOULDBLOCK, this means the avifIO doesn't
                                   // have necessary data available yet
  ...
  AVIF_RESULT_CANNOT_CHANGE_SETTING =
      27,                               // a setting that can't change is changed during encoding
...
typedef enum avifHeaderFormat {
  // AVIF file with an "avif" brand, a MetaBox and all its required boxes for maximum
  // compatibility.
  AVIF_HEADER_FULL,
...
// Validates the first bytes of the Exif payload and finds the TIFF header offset (up to
// UINT32_MAX).
AVIF_API avifResult avifGetExifTiffHeaderOffset(const uint8_t *exif,
                                                size_t exifSize,
                                                size_t *offset);
...
AVIF_API avifResult avifGetExifOrientationOffset(const uint8_t *exif,
                                                 size_t exifSize,
                                                 size_t *offset);
...
// When monochrome is AVIF_TRUE, chromaShiftX and chromaShiftY are set to 1 according to the AV1
// specification but they should be ignored.
//
// Note: This function implements the second table on page 119 of the AV1 specification
// version 1.0.0 with Errata 1. For monochrome 4:0:0, subsampling_x and subsampling are specified
// as 1 to allow an AV1 implementation that only supports profile 0 to hardcode subsampling_x and
// subsampling_y to 1.
...
AVIF_API avifColorPrimaries avifColorPrimariesFind(const float inPrimaries[8],
                                                   const char **outName);
...
AVIF_API avifResult avifTransferCharacteristicsGetGamma(avifTransferCharacteristics atc,
                                                        float *gamma);
...
typedef struct avifCleanApertureBox {
  ...
  // a fractional number which defines the exact clean aperture width, in counted pixels, of the
  // video image
  uint32_t widthN;
  uint32_t widthD;

  // a fractional number which defines the exact clean aperture height, in counted pixels, of the
  // video image
  uint32_t heightN;
  uint32_t heightD;

  // a fractional number which defines the horizontal offset of clean aperture centre minus
  // (width-1)/2. Typically 0.
  uint32_t horizOffN;
  uint32_t horizOffD;

  // a fractional number which defines the vertical offset of clean aperture centre minus
  // (height-1)/2. Typically 0.
  uint32_t vertOffN;
  uint32_t vertOffD;
...
// These will return AVIF_FALSE if the resultant values violate any standards, and if so, the
// output values are not guaranteed to be complete or correct and should not be used.
AVIF_NODISCARD AVIF_API avifBool
avifCropRectConvertCleanApertureBox(avifCropRect *cropRect,
...
typedef struct avifContentLightLevelInformationBox {
  ...
  // location of a luma sample in a corresponding 4:4:4 representation, denoted as E_R, E_G, and
  // E_B, the maximum component intensity is defined as E_Max = Max(E_R, Max(E_G, E_B)). The light
  // level corresponding to the stimulus is then defined as the CIE 1931 luminance corresponding to
  // equal amplitudes of E_Max for all three colour primary intensities for red, green, and blue
  // (with appropriate scaling to reflect the nominal luminance level associated with peak white,
  // e.g. ordinarily scaling to associate peak white with 10 000 candelas per square metre when
  // transfer_characteristics is equal to 16).
...
AVIF_NODISCARD AVIF_API avifBool avifGainMapMetadataDoubleToFractions(
    avifGainMapMetadata *dst, const avifGainMapMetadataDouble *src);
// Converts a avifGainMapMetadata to avifGainMapMetadataDouble by converting fractions to double
// values. Returns AVIF_FALSE if some denominators are zero.
AVIF_NODISCARD AVIF_API avifBool avifGainMapMetadataFractionsToDouble(
    avifGainMapMetadataDouble *dst, const avifGainMapMetadata *src);
...
typedef struct avifImage {
  ...
  uint32_t height;
  uint32_t
      depth;  // all planes must share this depth; if depth>8, all planes are uint16_t internally
  ...
  // transformFlags. On decode, only honor the values in boxes with the associated transform flag
  // set.
  ...
  // Metadata - set with avifImageSetMetadata*() before write, check .size>0 for existence after
  // read
  avifRWData exif;
  avifRWData xmp;
...
// avifImageCreate() and avifImageCreateEmpty() return NULL if arguments are invalid or if a memory
// allocation failed.
AVIF_NODISCARD AVIF_API avifImage *avifImageCreate(uint32_t width,
                                                   uint32_t height,
                                                   uint32_t depth,
                                                   avifPixelFormat yuvFormat);
AVIF_NODISCARD AVIF_API avifImage *avifImageCreateEmpty(
    void);  // helper for making an image to decode into
// Performs a deep copy of an image, including all metadata and planes, and the gain map
// metadata/planes if present and if AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP is defined.
AVIF_API avifResult avifImageCopy(avifImage *dstImage,
...
AVIF_API avifResult avifImageSetViewRect(avifImage *dstImage,
                                         const avifImage *srcImage,
...
// Warning: If the Exif payload is set and invalid, avifEncoderWrite() may return
// AVIF_RESULT_INVALID_EXIF_PAYLOAD.
AVIF_API avifResult avifImageSetMetadataExif(avifImage *image,
                                             const uint8_t *exif,
                                             size_t exifSize);
// Sets XMP metadata.
...
AVIF_API avifResult avifImageAllocatePlanes(
    avifImage *image, avifPlanesFlags planes);  // Ignores any pre-existing planes
AVIF_API void avifImageFreePlanes(avifImage *image,
                                  avifPlanesFlags planes);  // Ignores already-freed planes
AVIF_API void avifImageStealPlanes(avifImage *dstImage,
                                   avifImage *srcImage,
                                   avifPlanesFlags planes);
...
// speedy encode or decode, despite the complexities of occasionally needing two AV1 codec
// instances (due to alpha payloads being separate from color payloads). If your system has a hard
// ceiling on the number of threads that can ever be in flight at a given time, please account for
// this accordingly.

// ---------------------------------------------------------------------------
// Scaling

// Scales the YUV/A planes in-place. dstWidth and dstHeight must both be <=
// AVIF_DEFAULT_IMAGE_DIMENSION_LIMIT and dstWidth*dstHeight should be <=
// AVIF_DEFAULT_IMAGE_SIZE_LIMIT.
AVIF_API avifResult avifImageScale(avifImage *image,
                                   uint32_t dstWidth,
                                   uint32_t dstHeight,
                                   avifDiagnostics *diag);
...
// avifImageRGBToYUV() and avifImageYUVToRGB() will perform depth rescaling and limited<->full
...
typedef enum avifChromaUpsampling {
  AVIF_CHROMA_UPSAMPLING_AUTOMATIC =
      0,  // Chooses best trade off of speed/quality (uses BILINEAR libyuv if available,
  ...
  AVIF_CHROMA_UPSAMPLING_BEST_QUALITY =
      2,  // Chooses the best quality upsampling, given settings (same as BILINEAR)
...
typedef struct avifRGBImage {
  ...
  avifChromaUpsampling
      chromaUpsampling;  // How to upsample from 4:2:0 or 4:2:2 UV when converting to RGB (ignored
                         // for 4:4:4 and 4:0:0). Ignored when converting to YUV. Defaults to
                         // AVIF_CHROMA_UPSAMPLING_AUTOMATIC.
  avifChromaDownsampling
      chromaDownsampling;  // How to downsample to 4:2:0 or 4:2:2 UV when converting from RGB
                           // (ignored for 4:4:4 and 4:0:0). Ignored when converting to RGB.
                           // Defaults to AVIF_CHROMA_DOWNSAMPLING_AUTOMATIC.
  avifBool avoidLibYUV;    // If AVIF_FALSE and libyuv conversion between RGB and YUV (including
                           // upsampling or downsampling if any) is available for the
                           // avifImage/avifRGBImage combination, then libyuv is used. Default is
                           // AVIF_FALSE.
  avifBool ignoreAlpha;    // Used for XRGB formats, treats formats containing alpha (such as ARGB)
                           // as if they were RGB, treating the alpha bits as if they were all 1.
  avifBool
      alphaPremultiplied;  // indicates if RGB value is pre-multiplied by alpha. Default: false
  avifBool isFloat;  // indicates if RGBA values are in half float (f16) format. Valid only when
                     // depth == 16. Default: false
  int maxThreads;    // Number of threads to be used for the YUV to RGB conversion. Note that this
                   // value is ignored for RGB to YUV conversion. Setting this to zero has the same
                   // effect as setting it to one. Negative values are invalid. Default: 1.
...
// This function should return a block of memory that *must* remain valid until another read call
// to this avifIO struct is made (reusing a read buffer is acceptable/expected).
//
...
typedef avifResult (*avifIOReadFunc)(
    struct avifIO *io, uint32_t readFlags, uint64_t offset, size_t size, avifROData *out);

typedef avifResult (*avifIOWriteFunc)(
    struct avifIO *io, uint32_t writeFlags, uint64_t offset, const uint8_t *data, size_t size);
...
typedef struct avifDecoder {
  ...
  // Defaults to AVIF_CODEC_CHOICE_AUTO: Preference determined by order in availableCodecs table
  // (avif.c)
  ...
  // Counts and timing for the current image in an image sequence. Uninteresting for single image
  // files.
  ...
  uint64_t
      durationInTimescales;  // duration of a single playback of the image sequence in "timescales"
  int repetitionCount;  // number of times the sequence has to be repeated. This can also be one of
                        // AVIF_REPETITION_COUNT_INFINITE or AVIF_REPETITION_COUNT_UNKNOWN.
                        // Essentially, if repetitionCount is a non-negative integer `n`, then the
                        // image sequence should be played back `n + 1` times.
  ...
  // avifDecoderNextImage() or avifDecoderNthImage(), as decoder->image->alphaPlane won't exist
  // yet.
  ...
  // This is true when avifDecoderParse() detects an image sequence track in the image. If this is
  // true, the image can be decoded either as an animated image sequence or as a still image (the
  // primary image item) by setting avifDecoderSetSource to the appropriate source.
...
// Simple interfaces to decode a single image, independent of the decoder afterwards (decoder may
// be destroyed).
AVIF_API avifResult avifDecoderRead(avifDecoder *decoder,
                                    avifImage *image);  // call avifDecoderSetIO*() first
AVIF_API avifResult avifDecoderReadMemory(avifDecoder *decoder,
                                          avifImage *image,
                                          const uint8_t *data,
                                          size_t size);
AVIF_API avifResult avifDecoderReadFile(avifDecoder *decoder,
                                        avifImage *image,
                                        const char *filename);

// Multi-function alternative to avifDecoderRead() for image sequences and gaining direct access
// to the decoder's YUV buffers (for performance's sake). Data passed into avifDecoderParse() is
// NOT copied, so it must continue to exist until the decoder is destroyed.
//
// Usage / function call order is:
// * avifDecoderCreate()
// * avifDecoderSetSource() - optional, the default (AVIF_DECODER_SOURCE_AUTO) is usually
// sufficient
// * avifDecoderSetIO*()
// * avifDecoderParse()
...
// "nearest" keyframe means the keyframe prior to this frame index (returns frameIndex if it is a
// keyframe) These functions may be used after a successful call (AVIF_RESULT_OK) to
// avifDecoderParse().
AVIF_NODISCARD AVIF_API avifBool avifDecoderIsKeyframe(const avifDecoder *decoder,
                                                       uint32_t frameIndex);
AVIF_API uint32_t avifDecoderNearestKeyframe(const avifDecoder *decoder, uint32_t frameIndex);

// Timing helper - This does not change the current image or invoke the codec (safe to call
// repeatedly) This function may be used after a successful call (AVIF_RESULT_OK) to
// avifDecoderParse().
AVIF_API avifResult avifDecoderNthImageTiming(const avifDecoder *decoder,
                                              uint32_t frameIndex,
...
// from the luma plane of decoder->image, and alpha if any. The corresponding rows from the chroma
// planes, if any, can also be accessed (half rounded up if subsampled, same number of rows
// otherwise). If a gain map is present and AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP is on and
// enableDecodingGainMap is also on, the gain map's planes can also be accessed in the same way. If
// the gain map's height is different from the main image, then the number of available gain map
// rows is at least: roundf((float)decoded_row_count / decoder->image->height *
// decoder->image->gainMap.image->height) When gain map scaling is needed, callers might choose to
// use a few less rows depending on how many rows are needed by the scaling algorithm, to avoid the
// last row(s) changing when more data becomes available. decoder->allowIncremental must be set to
// true before calling avifDecoderNextImage() or avifDecoderNthImage(). Returns
// decoder->image->height when the last call to avifDecoderNextImage() or avifDecoderNthImage()
// returned AVIF_RESULT_OK. Returns 0 in all other cases. WARNING: Experimental feature.
...
// depends on to decode (such as samples for reference frames), from the nearest keyframe up to
// this Nth frame.
//
// If avifDecoderNthImageMaxExtent() returns AVIF_RESULT_OK and the extent's size is 0 bytes, this
// signals that libavif doesn't expect to call avifIO's Read for this frame's decode. This happens
// if data for this frame was read as a part of avifDecoderParse() (typically in an idat box inside
// of a meta box).
//
// This function may be used after a successful call (AVIF_RESULT_OK) to avifDecoderParse().
AVIF_API avifResult avifDecoderNthImageMaxExtent(const avifDecoder *decoder,
                                                 uint32_t frameIndex,
...
//   Tiling values range [0-6], where the value indicates a request for 2^n tiles in that
//   dimension. If autoTiling is set to AVIF_TRUE, libavif ignores tileRowsLog2 and tileColsLog2
//   and automatically chooses suitable tiling values.
// * Speed range: [AVIF_SPEED_SLOWEST - AVIF_SPEED_FASTEST]. Slower should make for a better
// quality
//   image in less bytes. AVIF_SPEED_DEFAULT means "Leave the AV1 codec to its default speed
//   settings"./ If avifEncoder uses rav1e, the speed value is directly passed through (0-10). If
//   libaom is used, a combination of settings are tweaked to simulate this speed range.
// * Extra layer count: [0 - (AVIF_MAX_AV1_LAYER_COUNT-1)]. Non-zero value indicates a layered
//   (progressive) image.
// * Some encoder settings can be changed after encoding starts. Changes will take effect in the
// next
...
typedef struct avifEncoder {
  ...
  int maxThreads;
  int speed;
  int keyframeInterval;  // Any set of |keyframeInterval| consecutive frames will have at least one
                         // keyframe. When it is 0, there is no such restriction.
  uint64_t timescale;    // timescale of the media (Hz)
  int repetitionCount;   // Number of times the image sequence should be repeated. This can also be
                         // set to AVIF_REPETITION_COUNT_INFINITE for infinite repetitions.  Only
                         // applicable for image sequences. Essentially, if repetitionCount is a
                         // non-negative integer `n`, then the image sequence should be played back
                         // `n + 1` times. Defaults to AVIF_REPETITION_COUNT_INFINITE.
...
AVIF_API avifResult avifEncoderWrite(avifEncoder *encoder,
...
AVIF_API avifResult avifEncoderAddImage(avifEncoder *encoder,
                                        const avifImage *image,
...
// See the codec documentation to know if a setting is persistent or applied only to the next
// frame. key must be non-NULL, but passing a NULL value will delete the pending key, if it exists.
...
AVIF_API avifResult avifEncoderSetCodecSpecificOption(avifEncoder *encoder,
                                                      const char *key,
                                                      const char *value);

#if defined(AVIF_ENABLE_EXPERIMENTAL_GAIN_MAP)
// Returns the size in bytes of the AV1 image item containing gain map samples, or 0 if no gain map
// was encoded.
