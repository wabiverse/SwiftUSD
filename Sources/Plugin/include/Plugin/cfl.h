...
static INLINE CFL_ALLOWED_TYPE is_cfl_allowed(const MACROBLOCKD *xd)
{
  ...
  return (CFL_ALLOWED_TYPE)(block_size_wide[bsize] <= 32 && block_size_high[bsize] <= 32);
}

// Do we need to save the luma pixels from the current block,
// for a possible future CfL prediction?
static INLINE CFL_ALLOWED_TYPE store_cfl_required(const AV1_COMMON *cm, const MACROBLOCKD *xd)
{
  const MB_MODE_INFO *mbmi = xd->mi[0];
  ...
  return (CFL_ALLOWED_TYPE)(!is_inter_block(mbmi) && mbmi->uv_mode == UV_CFL_PRED);
}
...
void cfl_load_dc_pred(MACROBLOCKD *const xd,
                      uint8_t *dst,
                      int dst_stride,
                      TX_SIZE tx_size,
                      CFL_PRED_TYPE pred_plane);
...
#define CFL_SUBSAMPLE(arch, sub, bd, width, height) \
  ...
  { \
    cfl_luma_subsampling_##sub##_##bd##_##arch(cfl_type, input_stride, output_q3, width, height); \
  }
...
#define CFL_SUBSAMPLE_FUNCTIONS(arch, sub, bd) \
  ...
  cfl_subsample_##bd##_fn cfl_get_luma_subsampling_##sub##_##bd##_##arch(TX_SIZE tx_size) \
  { \
...
#define CFL_SUB_AVG_X(arch, width, height, round_offset, num_pel_log2) \
  void cfl_subtract_average_##width##x##height##_##arch(const uint16_t *src, int16_t *dst) \
  { \
    subtract_average_##arch(src, dst, width, height, round_offset, num_pel_log2); \
  }
...
#define CFL_SUB_AVG_FN(arch) \
  ...
  cfl_subtract_average_fn cfl_get_subtract_average_fn_##arch(TX_SIZE tx_size) \
  { \
...
#define CFL_PREDICT_lbd(arch, width, height) \
  ...
  { \
    cfl_predict_lbd_##arch(pred_buf_q3, dst, dst_stride, alpha_q3, width, height); \
  }

#if CONFIG_AV1_HIGHBITDEPTH
#  define CFL_PREDICT_hbd(arch, width, height) \
    void cfl_predict_hbd_##width##x##height##_##arch( \
        const int16_t *pred_buf_q3, uint16_t *dst, int dst_stride, int alpha_q3, int bd) \
    { \
      cfl_predict_hbd_##arch(pred_buf_q3, dst, dst_stride, alpha_q3, bd, width, height); \
    }
#endif

// This wrapper exists because clang format does not like calling macros with
// lowercase letters.
#define CFL_PREDICT_X(arch, width, height, bd) CFL_PREDICT_##bd(arch, width, height)
