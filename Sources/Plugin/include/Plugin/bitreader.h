...
#define aom_read(r, prob, ACCT_STR_NAME) aom_read_(r, prob ACCT_STR_ARG(ACCT_STR_NAME))
#define aom_read_bit(r, ACCT_STR_NAME) aom_read_bit_(r ACCT_STR_ARG(ACCT_STR_NAME))
...
static INLINE void aom_process_accounting(const aom_reader *r ACCT_STR_PARAM)
{
  if (r->accounting != NULL) {
    ...
    aom_accounting_record(r->accounting, ACCT_STR_NAME, tell_frac - r->accounting->last_tell_frac);
...
static INLINE int aom_read_(aom_reader *r, int prob ACCT_STR_PARAM)
{
...
#if CONFIG_BITSTREAM_DEBUG
  {
    ...
    if ((ref_nsymbs != 2) || (ref_cdf[0] != (aom_cdf_prob)p) || (ref_cdf[1] != 32767)) {
      fprintf(stderr,
              ...
              32767,
              ref_cdf[0]);
      for (i = 1; i < ref_nsymbs; ++i)
        fprintf(stderr, ", %d", ref_cdf[i]);
...
#if CONFIG_ACCOUNTING
  if (ACCT_STR_NAME)
    aom_process_accounting(r, ACCT_STR_NAME);
...
static INLINE int aom_read_literal_(aom_reader *r, int bits ACCT_STR_PARAM)
{
  ...
  for (bit = bits - 1; bit >= 0; bit--)
    literal |= aom_read_bit(r, NULL) << bit;
#if CONFIG_ACCOUNTING
  if (ACCT_STR_NAME)
    aom_process_accounting(r, ACCT_STR_NAME);
#endif
...
static INLINE int aom_read_cdf_(aom_reader *r, const aom_cdf_prob *cdf, int nsymbs ACCT_STR_PARAM)
{
  int symb;
  assert(cdf != NULL);
...
static INLINE int aom_read_symbol_(aom_reader *r, aom_cdf_prob *cdf, int nsymbs ACCT_STR_PARAM)
{
  int ret;
