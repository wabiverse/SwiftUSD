...
avifBool avifGetYUVColorSpaceInfo(const avifImage *image, avifYUVColorSpaceInfo *info)
{
  ...
  // These matrix coefficients values are currently unsupported. Revise this list as more support
  // is added.
  //
  // YCgCo performs limited-full range adjustment on R,G,B but the current implementation performs
  // range adjustment on Y,U,V. So YCgCo with limited range is unsupported.
  if ((image->matrixCoefficients == 3 /* CICP reserved */) ||
      ((image->matrixCoefficients == AVIF_MATRIX_COEFFICIENTS_YCGCO || useYCgCo) &&
      ...
      (image->matrixCoefficients >= AVIF_MATRIX_COEFFICIENTS_LAST))
  {
    return AVIF_FALSE;
  }
  ...
  info->rangeY = (float)((info->range == AVIF_RANGE_LIMITED) ? (219 << (info->depth - 8)) :
                                                               info->maxChannel);
  info->rangeUV = (float)((info->range == AVIF_RANGE_LIMITED) ? (224 << (info->depth - 8)) :
                                                                info->maxChannel);

  return AVIF_TRUE;
}

static avifBool avifPrepareReformatState(const avifImage *image,
...
static int avifYUVColorSpaceInfoUVToUNorm(avifYUVColorSpaceInfo *info, float v)
{
  ...
  // range adjustment on Y,U,V. So YCgCo with limited range is unsupported.
#if defined(AVIF_ENABLE_EXPERIMENTAL_YCGCO_R)
  assert((info->mode != AVIF_REFORMAT_MODE_YCGCO && info->mode != AVIF_REFORMAT_MODE_YCGCO_RE &&
          info->mode != AVIF_REFORMAT_MODE_YCGCO_RO) ||
...
avifResult avifImageRGBToYUV(avifImage *image, const avifRGBImage *rgb)
{
  ...
  avifResult allocationResult = avifImageAllocatePlanes(
      image, hasAlpha ? AVIF_PLANES_ALL : AVIF_PLANES_YUV);
  ...
  if ((rgb->chromaDownsampling == AVIF_CHROMA_DOWNSAMPLING_SHARP_YUV) &&
      (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV420))
  {
    ...
    if (libSharpYUVResult != AVIF_RESULT_OK) {
      // Return the error if sharpyuv was requested but failed for any reason, including
      // libsharpyuv not being available.
    ...
    for (uint32_t outerJ = 0; outerJ < image->height; outerJ += 2) {
      for (uint32_t outerI = 0; outerI < image->width; outerI += 2) {
        ...
        for (int bJ = 0; bJ < blockH; ++bJ) {
          for (int bI = 0; bI < blockW; ++bI) {
            ...
            if (state.rgb.channelBytes > 1) {
              rgbPixel[0] =
                  *((uint16_t *)(&rgb->pixels[state.rgb.offsetBytesR + (i * state.rgb.pixelBytes) +
                                              (j * rgb->rowBytes)])) /
                  rgbMaxChannelF;
              rgbPixel[1] =
                  *((uint16_t *)(&rgb->pixels[state.rgb.offsetBytesG + (i * state.rgb.pixelBytes) +
                                              (j * rgb->rowBytes)])) /
                  rgbMaxChannelF;
              rgbPixel[2] =
                  *((uint16_t *)(&rgb->pixels[state.rgb.offsetBytesB + (i * state.rgb.pixelBytes) +
                                              (j * rgb->rowBytes)])) /
                  rgbMaxChannelF;
            }
            else {
              rgbPixel[0] = rgb->pixels[state.rgb.offsetBytesR + (i * state.rgb.pixelBytes) +
                                        (j * rgb->rowBytes)] /
                            rgbMaxChannelF;
              rgbPixel[1] = rgb->pixels[state.rgb.offsetBytesG + (i * state.rgb.pixelBytes) +
                                        (j * rgb->rowBytes)] /
                            rgbMaxChannelF;
              rgbPixel[2] = rgb->pixels[state.rgb.offsetBytesB + (i * state.rgb.pixelBytes) +
                                        (j * rgb->rowBytes)] /
                            rgbMaxChannelF;
            }
            ...
            if (alphaMode != AVIF_ALPHA_MULTIPLY_MODE_NO_OP) {
              ...
              if (state.rgb.channelBytes > 1) {
                a = *((uint16_t
                           *)(&rgb->pixels[state.rgb.offsetBytesA + (i * state.rgb.pixelBytes) +
                                           (j * rgb->rowBytes)])) /
                    rgbMaxChannelF;
              }
              else {
                a = rgb->pixels[state.rgb.offsetBytesA + (i * state.rgb.pixelBytes) +
                                (j * rgb->rowBytes)] /
                    rgbMaxChannelF;
              }
            ...
            else if (state.yuv.mode == AVIF_REFORMAT_MODE_YCGCO_RE ||
                     state.yuv.mode == AVIF_REFORMAT_MODE_YCGCO_RO)
            {
              // Formulas from JVET-U0093.
              const int R = (int)avifRoundf(
                  AVIF_CLAMP(rgbPixel[0] * rgbMaxChannelF, 0.0f, rgbMaxChannelF));
              const int G = (int)avifRoundf(
                  AVIF_CLAMP(rgbPixel[1] * rgbMaxChannelF, 0.0f, rgbMaxChannelF));
              const int B = (int)avifRoundf(
                  AVIF_CLAMP(rgbPixel[2] * rgbMaxChannelF, 0.0f, rgbMaxChannelF));
              const int Co = R - B;
              const int t = B + (Co >> 1);
              const int Cg = G - t;
            ...
            if (state.yuv.channelBytes > 1) {
              uint16_t *pY =
                  (uint16_t *)&yuvPlanes[AVIF_CHAN_Y][(i * 2) + (j * yuvRowBytes[AVIF_CHAN_Y])];
              *pY = (uint16_t)avifYUVColorSpaceInfoYToUNorm(&state.yuv, yuvBlock[bI][bJ].y);
              if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) {
                // YUV444, full chroma
                uint16_t *pU =
                    (uint16_t *)&yuvPlanes[AVIF_CHAN_U][(i * 2) + (j * yuvRowBytes[AVIF_CHAN_U])];
                *pU = (uint16_t)avifYUVColorSpaceInfoUVToUNorm(&state.yuv, yuvBlock[bI][bJ].u);
                uint16_t *pV =
                    (uint16_t *)&yuvPlanes[AVIF_CHAN_V][(i * 2) + (j * yuvRowBytes[AVIF_CHAN_V])];
                *pV = (uint16_t)avifYUVColorSpaceInfoUVToUNorm(&state.yuv, yuvBlock[bI][bJ].v);
              }
            }
            else {
              yuvPlanes[AVIF_CHAN_Y][i + (j * yuvRowBytes[AVIF_CHAN_Y])] = (uint8_t)
                  avifYUVColorSpaceInfoYToUNorm(&state.yuv, yuvBlock[bI][bJ].y);
              if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) {
                // YUV444, full chroma
                yuvPlanes[AVIF_CHAN_U][i + (j * yuvRowBytes[AVIF_CHAN_U])] = (uint8_t)
                    avifYUVColorSpaceInfoUVToUNorm(&state.yuv, yuvBlock[bI][bJ].u);
                yuvPlanes[AVIF_CHAN_V][i + (j * yuvRowBytes[AVIF_CHAN_V])] = (uint8_t)
                    avifYUVColorSpaceInfoUVToUNorm(&state.yuv, yuvBlock[bI][bJ].v);
              }
            }
          }
        }
        ...
        if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV400) {
          ...
          if (state.yuv.channelBytes > 1) {
            uint16_t *pU =
                (uint16_t *)&yuvPlanes[AVIF_CHAN_U][(uvI * 2) + (uvJ * yuvRowBytes[AVIF_CHAN_U])];
            *pU = (uint16_t)avifYUVColorSpaceInfoUVToUNorm(&state.yuv, avgU);
            uint16_t *pV =
                (uint16_t *)&yuvPlanes[AVIF_CHAN_V][(uvI * 2) + (uvJ * yuvRowBytes[AVIF_CHAN_V])];
            *pV = (uint16_t)avifYUVColorSpaceInfoUVToUNorm(&state.yuv, avgV);
          }
          else {
            yuvPlanes[AVIF_CHAN_U][uvI + (uvJ * yuvRowBytes[AVIF_CHAN_U])] = (uint8_t)
                avifYUVColorSpaceInfoUVToUNorm(&state.yuv, avgU);
            yuvPlanes[AVIF_CHAN_V][uvI + (uvJ * yuvRowBytes[AVIF_CHAN_V])] = (uint8_t)
                avifYUVColorSpaceInfoUVToUNorm(&state.yuv, avgV);
          }
        }
          ...
          for (int bJ = 0; bJ < blockH; ++bJ) {
            ...
            int uvJ = outerJ + bJ;
            if (state.yuv.channelBytes > 1) {
              uint16_t *pU = (uint16_t *)&yuvPlanes[AVIF_CHAN_U]
                                                   [(uvI * 2) + (uvJ * yuvRowBytes[AVIF_CHAN_U])];
              *pU = (uint16_t)avifYUVColorSpaceInfoUVToUNorm(&state.yuv, avgU);
              uint16_t *pV = (uint16_t *)&yuvPlanes[AVIF_CHAN_V]
                                                   [(uvI * 2) + (uvJ * yuvRowBytes[AVIF_CHAN_V])];
              *pV = (uint16_t)avifYUVColorSpaceInfoUVToUNorm(&state.yuv, avgV);
            }
            else {
              yuvPlanes[AVIF_CHAN_U][uvI + (uvJ * yuvRowBytes[AVIF_CHAN_U])] = (uint8_t)
...
static avifBool avifCreateYUVToRGBLookUpTables(float **unormFloatTableY,
                                               float **unormFloatTableUV,
                                               uint32_t depth,
...
static void avifStoreRGB8Pixel(avifRGBFormat format,
                               uint8_t R,
                               uint8_t G,
                               uint8_t B,
                               uint8_t *ptrR,
                                              ...
                                              avifAlphaMultiplyMode alphaMultiplyMode)
{
  ...
  AVIF_CHECKERR(
      avifCreateYUVToRGBLookUpTables(&unormFloatTableY, &unormFloatTableUV, image->depth, state),
    ...
    for (uint32_t i = 0; i < image->width; ++i) {
      ...
      if (hasColor) {
        ...
        if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) {
          ...
          // How many bytes to add to a uint8_t pointer index to get to the adjacent (lesser)
          // sample in a given direction
          ...
          if (image->depth == 8) {
            ...
            unormU[0][0] = *(
                (const uint16_t *)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes)]);
            unormV[0][0] = *(
                (const uint16_t *)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes)]);
            unormU[1][0] = *(
                (const uint16_t *)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes) + uAdjCol]);
            unormV[1][0] = *(
                (const uint16_t *)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes) + vAdjCol]);
            unormU[0][1] = *(
                (const uint16_t *)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes) + uAdjRow]);
            unormV[0][1] = *(
                (const uint16_t *)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes) + vAdjRow]);
            unormU[1][1] = *(
                (const uint16_t
                     *)&uPlane[(uvJ * uRowBytes) + (uvI * yuvChannelBytes) + uAdjCol + uAdjRow]);
            unormV[1][1] = *(
                (const uint16_t
                     *)&vPlane[(uvJ * vRowBytes) + (uvI * yuvChannelBytes) + vAdjCol + vAdjRow]);
              ...
              (rgb->chromaUpsampling == AVIF_CHROMA_UPSAMPLING_NEAREST))
          {
            ...
            Cb = (unormFloatTableUV[unormU[0][0]] * (9.0f / 16.0f)) +
                 (unormFloatTableUV[unormU[1][0]] * (3.0f / 16.0f)) +
                 (unormFloatTableUV[unormU[0][1]] * (3.0f / 16.0f)) +
                 (unormFloatTableUV[unormU[1][1]] * (1.0f / 16.0f));
            Cr = (unormFloatTableUV[unormV[0][0]] * (9.0f / 16.0f)) +
                 (unormFloatTableUV[unormV[1][0]] * (3.0f / 16.0f)) +
                 (unormFloatTableUV[unormV[0][1]] * (3.0f / 16.0f)) +
                 (unormFloatTableUV[unormV[1][1]] * (1.0f / 16.0f));
          }
        }
      }
...
static avifResult avifImageYUV16ToRGB16Color(const avifImage *image,
                                             avifRGBImage *rgb,
                                             avifReformatState *state)
{
  const float kr = state->yuv.kr;
  ...
  for (uint32_t j = 0; j < image->height; ++j) {
    ...
    const uint16_t *const ptrY =
        (uint16_t *)&image->yuvPlanes[AVIF_CHAN_Y][(j * image->yuvRowBytes[AVIF_CHAN_Y])];
    const uint16_t *const ptrU =
        (uint16_t *)&image->yuvPlanes[AVIF_CHAN_U][(uvJ * image->yuvRowBytes[AVIF_CHAN_U])];
    const uint16_t *const ptrV =
        (uint16_t *)&image->yuvPlanes[AVIF_CHAN_V][(uvJ * image->yuvRowBytes[AVIF_CHAN_V])];
    uint8_t *ptrR = &rgb->pixels[state->rgb.offsetBytesR + (j * rgb->rowBytes)];
    uint8_t *ptrG = &rgb->pixels[state->rgb.offsetBytesG + (j * rgb->rowBytes)];
    uint8_t *ptrB = &rgb->pixels[state->rgb.offsetBytesB + (j * rgb->rowBytes)];
...
static avifResult avifImageYUV16ToRGB16Mono(const avifImage *image,
                                            avifRGBImage *rgb,
                                            avifReformatState *state)
{
  const float kr = state->yuv.kr;
  ...
  const uint32_t rgbPixelBytes = state->rgb.pixelBytes;
  float *unormFloatTableY = NULL;
  AVIF_CHECKERR(avifCreateYUVToRGBLookUpTables(&unormFloatTableY, NULL, image->depth, state),
                AVIF_RESULT_OUT_OF_MEMORY);
...
static avifResult avifImageYUV16ToRGB8Color(const avifImage *image,
                                            avifRGBImage *rgb,
                                            avifReformatState *state)
{
  const float kr = state->yuv.kr;
...
static avifResult avifImageYUV16ToRGB8Mono(const avifImage *image,
                                           avifRGBImage *rgb,
                                           avifReformatState *state)
{
  const float kr = state->yuv.kr;
...
static avifResult avifImageYUV8ToRGB16Color(const avifImage *image,
                                            avifRGBImage *rgb,
                                            avifReformatState *state)
{
  const float kr = state->yuv.kr;
  ...
  for (uint32_t j = 0; j < image->height; ++j) {
    const uint32_t uvJ = j >> state->yuv.formatInfo.chromaShiftY;
    const uint8_t *const ptrY =
        &image->yuvPlanes[AVIF_CHAN_Y][(j * image->yuvRowBytes[AVIF_CHAN_Y])];
    const uint8_t *const ptrU =
        &image->yuvPlanes[AVIF_CHAN_U][(uvJ * image->yuvRowBytes[AVIF_CHAN_U])];
    const uint8_t *const ptrV =
        &image->yuvPlanes[AVIF_CHAN_V][(uvJ * image->yuvRowBytes[AVIF_CHAN_V])];
    uint8_t *ptrR = &rgb->pixels[state->rgb.offsetBytesR + (j * rgb->rowBytes)];
    uint8_t *ptrG = &rgb->pixels[state->rgb.offsetBytesG + (j * rgb->rowBytes)];
    uint8_t *ptrB = &rgb->pixels[state->rgb.offsetBytesB + (j * rgb->rowBytes)];
...
static avifResult avifImageYUV8ToRGB16Mono(const avifImage *image,
                                           avifRGBImage *rgb,
                                           avifReformatState *state)
{
  const float kr = state->yuv.kr;
...
static avifResult avifImageIdentity8ToRGB8ColorFullRange(const avifImage *image,
                                                         avifRGBImage *rgb,
                                                         avifReformatState *state)
{
  const uint32_t rgbPixelBytes = state->rgb.pixelBytes;
  for (uint32_t j = 0; j < image->height; ++j) {
    const uint8_t *const ptrY =
        &image->yuvPlanes[AVIF_CHAN_Y][(j * image->yuvRowBytes[AVIF_CHAN_Y])];
    const uint8_t *const ptrU =
        &image->yuvPlanes[AVIF_CHAN_U][(j * image->yuvRowBytes[AVIF_CHAN_U])];
    const uint8_t *const ptrV =
        &image->yuvPlanes[AVIF_CHAN_V][(j * image->yuvRowBytes[AVIF_CHAN_V])];
    uint8_t *ptrR = &rgb->pixels[state->rgb.offsetBytesR + (j * rgb->rowBytes)];
    uint8_t *ptrG = &rgb->pixels[state->rgb.offsetBytesG + (j * rgb->rowBytes)];
    uint8_t *ptrB = &rgb->pixels[state->rgb.offsetBytesB + (j * rgb->rowBytes)];
...
static avifResult avifImageYUV8ToRGB8Color(const avifImage *image,
                                           avifRGBImage *rgb,
                                           avifReformatState *state)
{
  const float kr = state->yuv.kr;
...
static avifResult avifImageYUV8ToRGB8Mono(const avifImage *image,
                                          avifRGBImage *rgb,
                                          avifReformatState *state)
{
  const float kr = state->yuv.kr;
...
static avifResult avifImageYUVToRGBImpl(const avifImage *image,
                                        avifRGBImage *rgb,
                                        avifReformatState *state,
                                        avifAlphaMultiplyMode alphaMultiplyMode)
{
  ...
  if (!rgb->avoidLibYUV && ((alphaMultiplyMode == AVIF_ALPHA_MULTIPLY_MODE_NO_OP) ||
                            avifRGBFormatHasAlpha(rgb->format)))
  {
    avifResult libyuvResult = avifImageYUVToRGBLibYUV(
        image, rgb, reformatAlpha, &alphaReformattedWithLibYUV);
    if (libyuvResult == AVIF_RESULT_OK) {
  ...
  if (!convertedWithLibYUV) {
    ...
    const avifBool hasColor = (image->yuvRowBytes[AVIF_CHAN_U] &&
         ...
         avifRGBFormatHasAlpha(rgb->format)))
    {
      ...
      //   unless the YUV data isn't subsampled or they explicitly requested
      //   AVIF_CHROMA_UPSAMPLING_NEAREST.
      // * None of these fast paths currently handle alpha (un)multiply, so avoid all of them
      //   if we can't do alpha (un)multiply as a separated post step (destination format doesn't
      //   have alpha).
...
#endif
{
  ...
  data->result = avifImageYUVToRGBImpl(
      &data->image, &data->rgb, data->state, data->alphaMultiplyMode);
#if defined(_WIN32)
  return 0;
#else
  return NULL;
...
{
#if defined(_WIN32)
  return WaitForSingleObject(tdata->thread, INFINITE) == WAIT_OBJECT_0 &&
         CloseHandle(tdata->thread) != 0;
#else
...
{
  // It is okay for rgb->maxThreads to be equal to zero in order to allow clients to zero
  // initialize the avifRGBImage struct with memset.
  ...
  if (image->alphaPlane) {
    if (!avifRGBFormatHasAlpha(rgb->format) || rgb->ignoreAlpha) {
      // if we are converting some image with alpha into a format without alpha, we should do
      // 'premultiply alpha' before discarding alpha plane. This has the same effect of rendering
      // this image on a black background, which makes sense.
  ...
  // When yuv format is 420 and chromaUpsampling could be BILINEAR, there is a dependency across
  // the horizontal borders of each job. So we disallow multithreading in that case.
  ...
  for (i = 0; i < jobs; ++i, startRow += rowsPerJob) {
    ...
    const avifCropRect rect = {.x = 0,
                               .y = startRow,
                               .width = image->width,
                               .height = (i == jobs - 1) ? rowsForLastJob : rowsPerJob};
...
// Limited -> Full
// Plan: subtract limited offset, then multiply by ratio of FULLSIZE/LIMITEDSIZE (rounding), then
// clamp. RATIO = (FULLY - 0) / (MAXLIMITEDY - MINLIMITEDY)
// -----------------------------------------
...
#define LIMITED_TO_FULL(MINLIMITEDY, MAXLIMITEDY, FULLY) \
  v = (((v - MINLIMITEDY) * FULLY) + ((MAXLIMITEDY - MINLIMITEDY) / 2)) / \
      (MAXLIMITEDY - MINLIMITEDY); \
...
void avifGetRGBAPixel(const avifRGBImage *src,
                      uint32_t x,
                      uint32_t y,
                      const avifRGBColorSpaceInfo *info,
                      float rgbaPixel[4])
{
  assert(src != NULL);
  ...
  if (info->channelBytes > 1) {
    ...
    uint16_t a = avifRGBFormatHasAlpha(src->format) ?
                     *((uint16_t *)(&srcPixel[info->offsetBytesA])) :
                     (uint16_t)info->maxChannel;
    if (src->isFloat) {
    ...
    if (src->format == AVIF_RGB_FORMAT_RGB_565) {
      ...
      rgbaPixel[3] = avifRGBFormatHasAlpha(src->format) ?
                         (srcPixel[info->offsetBytesA] / info->maxChannelF) :
                         1.0f;
    }
  }
}

void avifSetRGBAPixel(const avifRGBImage *dst,
                      uint32_t x,
                      uint32_t y,
                      const avifRGBColorSpaceInfo *info,
