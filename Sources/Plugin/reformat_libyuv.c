...
#    pragma clang diagnostic ignored \
        "-Wstrict-prototypes"  // "this function declaration is not a prototype"
...
// libyuv is a C++ library and defines custom types (struct, enum, etc) in the libyuv namespace
// when the libyuv header files are included by C++ code. When accessed from a C library like
// libavif, via a function pointer, this leads to signature mismatches in the CFI sanitizers since
// libyuv itself, compiled as C++ code, has the types within the namespace and the C code has the
// types without the namespace. In order to avoid this CFI error, we tag some functions as an
// exception when being compiled with CFI enabled.  For more details on clang's CFI see:
// https://clang.llvm.org/docs/ControlFlowIntegrity.html. For a simpler example of this bug, please
// see: https://github.com/vigneshvg/cpp_c_potential_cfi_bug
#  if defined(__clang__)
...
static int avifReorderARGBThenConvertToYUV(
    int (*ReorderARGB)(const uint8_t *, int, uint8_t *, int, int, int),
    int (*ConvertToYUV)(
        const uint8_t *, int, uint8_t *, int, uint8_t *, int, uint8_t *, int, int, int),
    ...
    int height)
{
  ...
  const int soft_allocation_limit =
      16384;  // Arbitrarily chosen trade-off between CPU and memory footprints.
  ...
  if ((height == 1) || ((int64_t)src_stride_argb * height <= soft_allocation_limit)) {
    ...
    // The last row of an odd number of RGB rows to be converted to vertically subsampled YUV is
    // treated differently by libyuv, so make sure all steps but the last one process a multiple of
    // min_num_rows rows. Try to process the highest multiple of min_num_rows rows possible in a
    // single step without allocating more than soft_allocation_limit, unless min_num_rows rows
    // need more than that.
    num_allocated_rows = AVIF_MAX(1, soft_allocation_limit / (src_stride_argb * min_num_rows)) *
                         min_num_rows;
  }
...
avifResult avifImageRGBToYUVLibYUV8bpc(avifImage *image, const avifRGBImage *rgb)
{
  ...
  // libavif uses byte-order when describing pixel formats, such that the R in RGBA is the lowest
  // address, similar to PNG. libyuv orders in word-order, so libavif's RGBA would be referred to
  // in libyuv as ABGR.
  ...
  if ((image->matrixCoefficients == AVIF_MATRIX_COEFFICIENTS_BT470BG) ||
  ...
  // TODO: Use SplitRGBPlane() for AVIF_MATRIX_COEFFICIENTS_IDENTITY if faster than the built-in
  // implementation
  return AVIF_RESULT_NOT_IMPLEMENTED;
}
...
// libavif uses byte-order when describing pixel formats, such that the R in RGBA is the lowest
// address, similar to PNG. libyuv orders in word-order, so libavif's RGBA would be referred to in
// libyuv as ABGR.  In addition, swapping U and V in any of the calls, along with using the Yvu
// matrix instead of Yuv matrix, swaps B and R in these orderings as well.
//
...
typedef int (*YUV400ToRGBMatrix)(
    const uint8_t *, int, uint8_t *, int, const struct YuvConstants *, int, int);
...
typedef int (*YUVAToRGBMatrixFilter)(const uint8_t *,
                                     ...
                                     int,
                                     enum FilterMode);
typedef int (*YUVToRGBMatrix)(const uint8_t *,
                              int,
                              const uint8_t *,
                              int,
                              const uint8_t *,
...
{
  return chromaUpsampling != AVIF_CHROMA_UPSAMPLING_BILINEAR &&
         chromaUpsampling != AVIF_CHROMA_UPSAMPLING_BEST_QUALITY;
}

// Returns AVIF_TRUE if the given yuvFormat and yuvDepth can be converted to 8-bit RGB using
// libyuv, AVIF_FALSE otherwise. When AVIF_TRUE is returned, exactly one function pointers will be
// populated with the appropriate conversion function. If alphaPreferred is set to AVIF_TRUE, then
// a function that can also copy the alpha channel will be preferred if available.
                                            ...
                                            LibyuvConversionFunction *lcf)
{
  ...
  static const YUVToRGBMatrixFilter
      lutYuvToRgbMatrixFilter[AVIF_RGB_FORMAT_COUNT][AVIF_PIXEL_FORMAT_COUNT] = {
          // { NONE, YUV444, YUV422, YUV420, YUV400 }                           // AVIF_RGB_FORMAT_
  ...
  static const YUVAToRGBMatrixFilter
      lutYuvaToRgbMatrixFilter[AVIF_RGB_FORMAT_COUNT][AVIF_PIXEL_FORMAT_COUNT] = {
          // { NONE, YUV444, YUV422, YUV420, YUV400 }                           // AVIF_RGB_FORMAT_
  ...
  static const YUVToRGBMatrixFilterHighBitDepth
      lutYuvToRgbMatrixFilterHighBitDepth[2][AVIF_RGB_FORMAT_COUNT][AVIF_PIXEL_FORMAT_COUNT] = {
          // 10bpc
          {
              // { NONE, YUV444, YUV422, YUV420, YUV400 }                           //
              // AVIF_RGB_FORMAT_
              {NULL, NULL, NULL, NULL, NULL},                                      // RGB
  ...
  static const YUVAToRGBMatrixFilterHighBitDepth
      lutYuvaToRgbMatrixFilterHighBitDepth[2][AVIF_RGB_FORMAT_COUNT][AVIF_PIXEL_FORMAT_COUNT] = {
          // 10bpc
          {
              // { NONE, YUV444, YUV422, YUV420, YUV400 }                           //
              // AVIF_RGB_FORMAT_
  ...
  // Lookup table for YUV To RGB Matrix (4:4:4 or nearest-neighbor filter).  First dimension is for
  // the YUV bit depth.
  static const YUVToRGBMatrixHighBitDepth
      lutYuvToRgbMatrixHighBitDepth[2][AVIF_RGB_FORMAT_COUNT][AVIF_PIXEL_FORMAT_COUNT] = {
          // 10bpc
          {
              // { NONE, YUV444, YUV422, YUV420, YUV400 }                           //
              // AVIF_RGB_FORMAT_
  ...
  // Lookup table for YUVA To RGB Matrix (4:4:4 or nearest-neighbor filter).  First dimension is
  // for the YUV bit depth.
  static const YUVAToRGBMatrixHighBitDepth
      lutYuvaToRgbMatrixHighBitDepth[2][AVIF_RGB_FORMAT_COUNT][AVIF_PIXEL_FORMAT_COUNT] = {
          // 10bpc
          {
              // { NONE, YUV444, YUV422, YUV420, YUV400 }                           //
              // AVIF_RGB_FORMAT_
  ...
  if (yuvDepth > 8) {
    ...
    if (yuvFormat != AVIF_PIXEL_FORMAT_YUV444) {
      if (alphaPreferred) {
        lcf->yuvaToRgbMatrixFilterHighBitDepth =
            lutYuvaToRgbMatrixFilterHighBitDepth[depthIndex][rgb->format][yuvFormat];
        if (lcf->yuvaToRgbMatrixFilterHighBitDepth != NULL) {
          return AVIF_TRUE;
        }
      }
      lcf->yuvToRgbMatrixFilterHighBitDepth =
          lutYuvToRgbMatrixFilterHighBitDepth[depthIndex][rgb->format][yuvFormat];
      if (lcf->yuvToRgbMatrixFilterHighBitDepth != NULL) {
        return AVIF_TRUE;
      }
    }
    if (yuvFormat == AVIF_PIXEL_FORMAT_YUV444 ||
        nearestNeighborFilterAllowed(rgb->chromaUpsampling))
    {
      if (alphaPreferred) {
        lcf->yuvaToRgbMatrixHighBitDepth =
            lutYuvaToRgbMatrixHighBitDepth[depthIndex][rgb->format][yuvFormat];
        if (lcf->yuvaToRgbMatrixHighBitDepth != NULL) {
          return AVIF_TRUE;
        }
      }
      lcf->yuvToRgbMatrixHighBitDepth =
          lutYuvToRgbMatrixHighBitDepth[depthIndex][rgb->format][yuvFormat];
      if (lcf->yuvToRgbMatrixHighBitDepth != NULL) {
        return AVIF_TRUE;
      }
    }
    // Fallthrough is intentional. No high bitdepth libyuv function was found. Check if there is an
    // 8-bit libyuv function which can used with a downshift.
  }
...
static void getLibYUVConstants(const avifImage *image,
                               ...
                               const struct YuvConstants **matrixYVU)
{
  ...
  const avifMatrixCoefficients matrixCoefficients = yuv400WithIdentityMatrix ?
...
static avifResult avifImageDownshiftTo8bpc(const avifImage *image,
                                           avifImage *image8,
                                           avifBool downshiftAlpha)
{
  avifImageSetDefaults(image8);
  avifImageCopyNoAlloc(image8, image);
  image8->depth = 8;
  // downshiftAlpha will be true only if the image has an alpha plane. So it is safe to pass
  // AVIF_PLANES_ALL here in that case.
  assert(!downshiftAlpha || image->alphaPlane);
  AVIF_CHECKRES(
      avifImageAllocatePlanes(image8, downshiftAlpha ? AVIF_PLANES_ALL : AVIF_PLANES_YUV));
...
IGNORE_CFI_ICALL avifResult avifImageYUVToRGBLibYUV(const avifImage *image,
                                                    avifRGBImage *rgb,
                                                    avifBool reformatAlpha,
                                                    avifBool *alphaReformattedWithLibYUV)
{
  *alphaReformattedWithLibYUV = AVIF_FALSE;
  ...
  if (!image->alphaPlane || !image->alphaRowBytes) {
    // If the image does not have an alpha plane, then libyuv always prefills the output RGB image
    // with opaque alpha values.
  ...
  const enum FilterMode filter = ((rgb->chromaUpsampling == AVIF_CHROMA_UPSAMPLING_FASTEST) ||
                                  (rgb->chromaUpsampling == AVIF_CHROMA_UPSAMPLING_NEAREST)) ?
                                     ...
                                     kFilterBilinear;
  if (lcf.yuvToRgbMatrixFilterHighBitDepth != NULL) {
    libyuvResult = lcf.yuvToRgbMatrixFilterHighBitDepth(
        (const uint16_t *)image->yuvPlanes[AVIF_CHAN_Y],
    ...
    libyuvResult = lcf.yuvaToRgbMatrixFilterHighBitDepth(
        (const uint16_t *)image->yuvPlanes[AVIF_CHAN_Y],
        image->yuvRowBytes[AVIF_CHAN_Y] / 2,
        (const uint16_t *)image->yuvPlanes[uPlaneIndex],
        image->yuvRowBytes[uPlaneIndex] / 2,
    ...
    *alphaReformattedWithLibYUV = AVIF_TRUE;
    ...
    if (inputIsHighBitDepth) {
      const avifBool downshiftAlpha = (lcf.yuvaToRgbMatrixFilter != NULL ||
                                       lcf.yuvaToRgbMatrix != NULL);
...
avifResult avifRGBImagePremultiplyAlphaLibYUV(avifRGBImage *rgb)
{
  ...
  if (rgb->format == AVIF_RGB_FORMAT_RGBA || rgb->format == AVIF_RGB_FORMAT_BGRA) {
    if (ARGBAttenuate(
            rgb->pixels, rgb->rowBytes, rgb->pixels, rgb->rowBytes, rgb->width, rgb->height) != 0)
    {
      return AVIF_RESULT_REFORMAT_FAILED;
    }
    return AVIF_RESULT_OK;
