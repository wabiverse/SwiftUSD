                                ...
                                StorageSpec const &storage)
{
  ...
  size_t strideLength = imageInput->spec().width * imageInput->spec().pixel_bytes();
  size_t readStride = (storage.flipped) ? (-strideLength) : (strideLength);
  ...
  if (imageInput->spec().format == TypeDesc::DOUBLE) {
    res = imageInput->read_image(TypeDesc::FLOAT, start, AutoStride, readStride, AutoStride);
  }
  else {
    res = imageInput->read_image(
        imageInput->spec().format, start, AutoStride, readStride, AutoStride);
  }

  if (!res) {
    TF_RUNTIME_ERROR("Failed to read image pixel from \"%s\", error = %s",
                     _filename.c_str(),
                     imageInput->geterror().c_str());
    return false;
  }
