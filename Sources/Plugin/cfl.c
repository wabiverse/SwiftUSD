...
void cfl_store_dc_pred(MACROBLOCKD *const xd,
                       const uint8_t *input,
...
static void cfl_load_dc_pred_lbd(
    const int16_t *dc_pred_cache, uint8_t *dst, int dst_stride, int width, int height)
{
...
static void cfl_load_dc_pred_hbd(
    const int16_t *dc_pred_cache, uint16_t *dst, int dst_stride, int width, int height)
{
...
void cfl_load_dc_pred(
    MACROBLOCKD *const xd, uint8_t *dst, int dst_stride, TX_SIZE tx_size, CFL_PRED_TYPE pred_plane)
{
  ...
  if (is_cur_buf_hbd(xd)) {
    ...
    cfl_load_dc_pred_hbd(xd->cfl.dc_pred_cache[pred_plane], dst_16, dst_stride, width, height);
    return;
  }
  cfl_load_dc_pred_lbd(xd->cfl.dc_pred_cache[pred_plane], dst, dst_stride, width, height);
}
...
static INLINE void cfl_pad(CFL_CTX *cfl, int width, int height)
{
  ...
  if (diff_height > 0) {
    uint16_t *recon_buf_q3 = cfl->recon_buf_q3 + ((height - diff_height) * CFL_BUF_LINE);
...
static void subtract_average_c(
    const uint16_t *src, int16_t *dst, int width, int height, int round_offset, int num_pel_log2)
{
...
static INLINE int cfl_idx_to_alpha(uint8_t alpha_idx, int8_t joint_sign, CFL_PRED_TYPE pred_type)
{
  const int alpha_sign = (pred_type == CFL_PRED_U) ? CFL_SIGN_U(joint_sign) :
                                                     CFL_SIGN_V(joint_sign);
  if (alpha_sign == CFL_SIGN_ZERO)
    return 0;
  const int abs_alpha_q3 = (pred_type == CFL_PRED_U) ? CFL_IDX_U(alpha_idx) : CFL_IDX_V(alpha_idx);
  return (alpha_sign == CFL_SIGN_POS) ? abs_alpha_q3 + 1 : -abs_alpha_q3 - 1;
}

static INLINE void cfl_predict_lbd_c(
    const int16_t *ac_buf_q3, uint8_t *dst, int dst_stride, int alpha_q3, int width, int height)
{
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
...
void cfl_predict_hbd_c(const int16_t *ac_buf_q3,
                       uint16_t *dst,
                       int dst_stride,
                       int alpha_q3,
                       int bit_depth,
                       ...
                       int height)
{
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      dst[i] = clip_pixel_highbd(get_scaled_luma_q0(alpha_q3, ac_buf_q3[i]) + dst[i], bit_depth);
    }
    dst += dst_stride;
    ac_buf_q3 += CFL_BUF_LINE;
  }
...
void cfl_predict_block(
    MACROBLOCKD *const xd, uint8_t *dst, int dst_stride, TX_SIZE tx_size, int plane)
{
  CFL_CTX *const cfl = &xd->cfl;
  ...
  const int alpha_q3 = cfl_idx_to_alpha(mbmi->cfl_alpha_idx, mbmi->cfl_alpha_signs, plane - 1);
  assert((tx_size_high[tx_size] - 1) * CFL_BUF_LINE + tx_size_wide[tx_size] <= CFL_BUF_SQUARE);
#if CONFIG_AV1_HIGHBITDEPTH
  if (is_cur_buf_hbd(xd)) {
    uint16_t *dst_16 = CONVERT_TO_SHORTPTR(dst);
    cfl_get_predict_hbd_fn(tx_size)(cfl->ac_buf_q3, dst_16, dst_stride, alpha_q3, xd->bd);
    return;
  }
#endif
...
static void cfl_luma_subsampling_420_lbd_c(
    const uint8_t *input, int input_stride, uint16_t *output_q3, int width, int height)
{
...
static void cfl_luma_subsampling_422_lbd_c(
    const uint8_t *input, int input_stride, uint16_t *output_q3, int width, int height)
{
...
static void cfl_luma_subsampling_444_lbd_c(
    const uint8_t *input, int input_stride, uint16_t *output_q3, int width, int height)
{
  assert((height - 1) * CFL_BUF_LINE + width <= CFL_BUF_SQUARE);
  for (int j = 0; j < height; j++) {
...
#if CONFIG_AV1_HIGHBITDEPTH
static void cfl_luma_subsampling_420_hbd_c(
    const uint16_t *input, int input_stride, uint16_t *output_q3, int width, int height)
{
  for (int j = 0; j < height; j += 2) {
    for (int i = 0; i < width; i += 2) {
      const int bot = i + input_stride;
...
static void cfl_luma_subsampling_444_hbd_c(
    const uint16_t *input, int input_stride, uint16_t *output_q3, int width, int height)
{
  assert((height - 1) * CFL_BUF_LINE + width <= CFL_BUF_SQUARE);
  for (int j = 0; j < height; j++) {
...
#if CONFIG_AV1_HIGHBITDEPTH
static INLINE cfl_subsample_hbd_fn cfl_subsampling_hbd(TX_SIZE tx_size, int sub_x, int sub_y)
{
  if (sub_x == 1) {
    if (sub_y == 1) {
...
static INLINE cfl_subsample_lbd_fn cfl_subsampling_lbd(TX_SIZE tx_size, int sub_x, int sub_y)
{
  if (sub_x == 1) {
    if (sub_y == 1) {
...
static void cfl_store(CFL_CTX *cfl,
                      const uint8_t *input,
                      int input_stride,
                      int row,
                      int col,
                      ...
                      int use_hbd)
{
  ...
  uint16_t *recon_buf_q3 = cfl->recon_buf_q3 + (store_row * CFL_BUF_LINE + store_col);
#if CONFIG_AV1_HIGHBITDEPTH
  if (use_hbd) {
    cfl_subsampling_hbd(tx_size, sub_x, sub_y)(
        CONVERT_TO_SHORTPTR(input), input_stride, recon_buf_q3);
  }
  else {
...
static INLINE void sub8x8_adjust_offset(
    const CFL_CTX *cfl, int mi_row, int mi_col, int *row_out, int *col_out)
{
...
void cfl_store_tx(MACROBLOCKD *const xd, int row, int col, TX_SIZE tx_size, BLOCK_SIZE bsize)
{
  CFL_CTX *const cfl = &xd->cfl;
...
static INLINE int max_intra_block_width(const MACROBLOCKD *xd,
                                        ...
                                        TX_SIZE tx_size)
{
  const int max_blocks_wide = max_block_wide(xd, plane_bsize, plane) << MI_SIZE_LOG2;
...
static INLINE int max_intra_block_height(const MACROBLOCKD *xd,
                                         ...
                                         TX_SIZE tx_size)
{
  const int max_blocks_high = max_block_high(xd, plane_bsize, plane) << MI_SIZE_LOG2;
...
void cfl_store_block(MACROBLOCKD *const xd, BLOCK_SIZE bsize, TX_SIZE tx_size)
{
  ...
  cfl_store(cfl, pd->dst.buf, pd->dst.stride, row, col, tx_size, is_cur_buf_hbd(xd));
}
