...
static int realloc_frame_buffer_aligned(YV12_BUFFER_CONFIG *ybf,
                                        int width,
                                        int height,
                                        int ss_x,
                                        int ss_y,
...
#  endif
    ...
    ybf->y_buffer = (uint8_t *)aom_align_addr(buf + (border * y_stride) + border, aom_byte_align);
    ybf->u_buffer = (uint8_t *)aom_align_addr(
        buf + yplane_size + (uv_border_h * uv_stride) + uv_border_w, aom_byte_align);
    ybf->v_buffer = (uint8_t *)aom_align_addr(buf + yplane_size + uvplane_size +
...
static int calc_stride_and_planesize(const int ss_x,
                                     const int ss_y,
                                     const int aligned_width,
                                     const int aligned_height,
                                     const int border,
                                     ...
                                     const int uv_height)
{
  ...
  *yplane_size = (aligned_height + 2 * border) * (uint64_t)(*y_stride) + byte_alignment;

  *uv_stride = *y_stride >> ss_x;
  *uvplane_size = (uv_height + 2 * (border >> ss_y)) * (uint64_t)(*uv_stride) + byte_alignment;
  return 0;
}
                             ...
                             void *cb_priv)
{
  ...
  if (ybf) {
    ...
    int error = calc_stride_and_planesize(ss_x,
                                          ss_y,
                                          aligned_width,
                                          aligned_height,
                                          border,
    ...
    if (error)
      return error;
    return realloc_frame_buffer_aligned(ybf,
                                        width,
                                        height,
                                        ss_x,
                                        ss_y,
                           ...
                           int byte_alignment)
{
  if (ybf) {
    ...
    return aom_realloc_frame_buffer(ybf,
                                    width,
                                    height,
                                    ss_x,
                                    ss_y,
...
int aom_copy_metadata_to_frame_buffer(YV12_BUFFER_CONFIG *ybf, const aom_metadata_array_t *arr)
{
  ...
  for (size_t i = 0; i < ybf->metadata->sz; i++) {
    ybf->metadata->metadata_array[i] = aom_img_metadata_alloc(arr->metadata_array[i]->type,
                                                              arr->metadata_array[i]->payload,
                                                              arr->metadata_array[i]->sz,
                                                              arr->metadata_array[i]->insert_flag);
