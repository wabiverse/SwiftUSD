...
aom_codec_err_t aom_codec_control(aom_codec_ctx_t *ctx, int ctrl_id, ...)
{
  ...
  for (aom_codec_ctrl_fn_map_t *entry = ctx->iface->ctrl_maps; !at_ctrl_map_end(entry); ++entry) {
...
aom_codec_err_t aom_codec_set_option(aom_codec_ctx_t *ctx, const char *name, const char *value)
{
  ...
  ctx->err = ctx->iface->set_option((aom_codec_alg_priv_t *)ctx->priv, name, value);
  return ctx->err;
}
