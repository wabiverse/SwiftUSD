...
UsdAppUtilsFrameRecorder::UsdAppUtilsFrameRecorder(const TfToken &rendererPluginId,
                                                   bool gpuEnabled)
...
void UsdAppUtilsFrameRecorder::SetColorCorrectionMode(const TfToken &colorCorrectionMode)
{
  if (_imagingEngine.GetGPUEnabled()) {
    ...
    if (colorCorrectionMode != HdxColorCorrectionTokens->disabled) {
      TF_WARN(
          "Color correction presently unsupported when the GPU is "
          "disabled.");
    }
    _colorCorrectionMode = HdxColorCorrectionTokens->disabled;
  }
...
class TextureBufferWriter {
 public:
  TextureBufferWriter(UsdImagingGLEngine *engine) : _engine(engine), _colorRenderBuffer(nullptr)
  {
...
static bool _RenderProductsGenerated(const UsdStagePtr &stage,
                                     const SdfPath &renderSettingsPrimPath)
{
  ...
  for (const auto &productPath : renderProductTargets) {
    ...
    if (ArchOpenFile(productName.GetText(), "r")) {
      ...
      TF_WARN(
          "Missing generated Product '%s' from RenderProduct prim "
