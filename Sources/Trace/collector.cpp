...
TraceCollector::TraceCollector()
    : _label("TraceRegistry global collector"), _measuredScopeOverhead(0)
#ifdef PXR_PYTHON_SUPPORT_ENABLED
      ,
...
TraceCollector::~TraceCollector() { SetEnabled(false); }

void TraceCollector::SetEnabled(bool isEnabled) {
  _isEnabled.store((int)isEnabled, std::memory_order_release);
}

void TraceCollector::Scope(const TraceKey &key, TimeStamp start,
...
TraceCollector::TimeStamp TraceCollector::_BeginEvent(const Key &key,
                                                      TraceCategoryId cat) {
...
TraceCollector::TimeStamp TraceCollector::_EndEvent(const Key &key,
                                                    TraceCategoryId cat) {
...
TraceCollector::TimeStamp TraceCollector::_MarkerEvent(const Key &key,
                                                       TraceCategoryId cat) {
...
void TraceCollector::_EndEventAtTime(const Key &key, double ms,
                                     TraceCategoryId cat) {
...
void TraceCollector::_BeginEventAtTime(const Key &key, double ms,
                                       TraceCategoryId cat) {
...
void TraceCollector::_MarkerEventAtTime(const Key &key, double ms,
                                        TraceCategoryId cat) {
  TfAutoMallocTag2 tag("Trace",
...
_MakePythonScopeKey(const TfPyTraceInfo &info) {
  string keyString = TfStringPrintf("%s() (py) in %s:%d (%s)", info.funcName,
...
TraceCollector::_PerThreadData::BeginEvent(const Key &key,
                                           TraceCategoryId cat) {
...
TraceCollector::TimeStamp
TraceCollector::_PerThreadData::MarkerEvent(const Key &key,
                                            TraceCategoryId cat) {
...
void TraceCollector::_PerThreadData::BeginEventAtTime(const Key &key, double ms,
                                                      TraceCategoryId cat) {
  AtomicRef lock(_writing);
  TfAutoMallocTag2 tag("Trace",
...
void TraceCollector::_PerThreadData::EndEventAtTime(const Key &key, double ms,
                                                    TraceCategoryId cat) {
  AtomicRef lock(_writing);
  TfAutoMallocTag2 tag("Trace",
...
void TraceCollector::_PerThreadData::_EndScope(const TraceKey &key,
                                               TraceCategoryId cat) {
  _events.load(std::memory_order_acquire)
      ->EmplaceBack(TraceEvent::End, key, cat);
}

void TraceCollector::_PerThreadData::CounterDelta(const Key &key, double value,
                                                  TraceCategoryId cat) {
  AtomicRef lock(_writing);
  EventList *events = _events.load(std::memory_order_acquire);
  events->EmplaceBack(TraceEvent::CounterDelta, events->CacheKey(key), value,
                      cat);
}

void TraceCollector::_PerThreadData::CounterValue(const Key &key, double value,
                                                  TraceCategoryId cat) {
  AtomicRef lock(_writing);
  EventList *events = _events.load(std::memory_order_acquire);
  events->EmplaceBack(TraceEvent::CounterValue, events->CacheKey(key), value,
                      cat);
}
