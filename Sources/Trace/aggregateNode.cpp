...
TraceAggregateNodeRefPtr TraceAggregateNode::Append(Id id, const TfToken &key,
                                                    TimeStamp ts, int c,
                                                    int xc) {
...
void TraceAggregateNode::AdjustForOverheadAndNoise(
  ...
  for (TraceAggregateNodeRefPtr const &child : _children) {
    child->AdjustForOverheadAndNoise(scopeOverhead, timerQuantum,
                                     &localNumDescendantNodes);
  }
...
void TraceAggregateNode::_MergeRecursive(const TraceAggregateNodeRefPtr &node) {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (!n) {
      ...
      bool nonMarkerIntoNonMarker =
          (!child->IsRecursionMarker() && !n->IsRecursionMarker());
      //
