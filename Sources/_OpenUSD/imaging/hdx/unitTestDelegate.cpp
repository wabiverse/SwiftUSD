...
Hdx_UnitTestDelegate::Hdx_UnitTestDelegate(HdRenderIndex *index,
    ...
    : HdSceneDelegate(index, delegateId), _refineLevel(0)
{
  // add camera
...
void Hdx_UnitTestDelegate::UpdateCamera(SdfPath const &id,
                                        TfToken const &key,
                                        VtValue value)
{
  _ValueCache &cache = _valueCacheMap[id];
...
void Hdx_UnitTestDelegate::SetLight(SdfPath const &id, TfToken const &key,
                                    VtValue value)
{
  _ValueCache &cache = _valueCacheMap[id];
  cache[key] = value;
...
void Hdx_UnitTestDelegate::UpdateTransform(SdfPath const &id,
...
void Hdx_UnitTestDelegate::AddRenderBuffer(SdfPath const &id,
...
void Hdx_UnitTestDelegate::SetDrawTarget(SdfPath const &id, TfToken const &key,
                                         VtValue value)
{
  _ValueCache &cache = _valueCacheMap[id];
  cache[key] = value;
...
void Hdx_UnitTestDelegate::AddTet(SdfPath const &id, GfMatrix4d const &transform,
...
bool Hdx_UnitTestDelegate::WriteRenderBufferToFile(SdfPath const &id,
