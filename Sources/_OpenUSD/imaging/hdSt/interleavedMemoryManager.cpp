...
{
  const int uniformBufferOffsetAlignment = _resourceRegistry->GetHgi()->GetCapabilities()->GetUniformBufferOffsetAlignment();
  const int maxUniformBlockSize = _resourceRegistry->GetHgi()->GetCapabilities()->GetMaxUniformBlockSize();
...
HdStInterleavedSSBOMemoryManager::CreateBufferArray(
    ...
    HdBufferArrayUsageHint usageHint)
{
  const int maxShaderStorageBlockSize = _resourceRegistry->GetHgi()->GetCapabilities()->GetMaxShaderStorageBlockSize();
...

{
  ...
  */
   ...
   */
  const bool useCppShaderPadding = _resourceRegistry->GetHgi()->GetCapabilities()->IsSet(HgiDeviceCapabilitiesBitsCppShaderPadding);
    ...
    int stride)
{
  ...
  HdStBufferResourceSharedPtr bufferRes = std::make_shared<HdStBufferResource>(GetRole(), tupleType, offset, stride);
...
void HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::SetBufferArray(HdBufferArray *bufferArray)
{
