...
HdSt_OsdStencilTableBufferSource::HdSt_OsdStencilTableBufferSource(
    ...
    HdSt_MeshTopology::Interpolation interpolation,
    int fvarChannel)
    : _subdivision(subdivision), _osdTopology(osdTopology), _name(name), _gpuStencilTable(gpuStencilTable), _interpolation(interpolation), _fvarChannel(fvarChannel), _resultData(nullptr), _resultNumElements(0), _resultTupleType(HdTupleType{HdTypeInt32, 0})
{
}
    ...
    int fvarChannel) const
{
  ...
  return (interpolation == HdSt_MeshTopology::INTERPOLATE_VERTEX) ? _vertexStencils.get() : (interpolation == HdSt_MeshTopology::INTERPOLATE_VARYING) ? _varyingStencils.get()
...
HdSt_OsdRefineComputationCPU::HdSt_OsdRefineComputationCPU(
    ...
    HdSt_MeshTopology::Interpolation interpolation,
    int fvarChannel)
    : _topology(topology), _source(source), _osdTopology(osdTopology), _interpolation(interpolation), _fvarChannel(fvarChannel)
{
}
...
HdSt_OsdRefineComputationGPU::HdSt_OsdRefineComputationGPU(
    ...
    HdSt_GpuStencilTableSharedPtr const &gpuStencilTable,
    HdSt_MeshTopology::Interpolation interpolation)
    : _topology(topology), _primvarName(primvarName), _gpuStencilTable(gpuStencilTable), _interpolation(interpolation)
{
}
...
void HdSt_Subdivision::RefineGPU(
...
bool HdSt_OsdFvarIndexComputation::Resolve()
{
      ...
      _topology->RefinesToBoxSplineTrianglePatches())
  {
    ...
    // Bundle groups of 12 or 16 patch control vertices
    int const arraySize = patchTable ? patchTable->GetFVarPatchDescriptor(_channel).GetNumControlVertices()
                                     : 0;
...
HdSt_OsdBaseFaceToRefinedFacesMapComputation::
    HdSt_OsdBaseFaceToRefinedFacesMapComputation(
        HdSt_Subdivision const *subdivision,
        HdBufferSourceSharedPtr const &osdTopology) : _subdivision(subdivision), _osdTopology(osdTopology)
{
}
      ...
      HdResourceRegistry *resourceRegistry)
  {
    ...
    std::string const defines =
        TfStringPrintf("#define EVAL_STENCILS_NUM_ELEMENTS %zd\n",
                       elementStride);
    HdStGLSLProgramSharedPtr computeProgram = HdStGLSLProgram::GetComputeProgram(shaderToken,
                                                                                 defines,
                                                                                 hdStResourceRegistry,
