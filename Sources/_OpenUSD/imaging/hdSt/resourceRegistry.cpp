...
HdStResourceRegistry::HdStResourceRegistry(Hgi *const hgi)
    : _hgi(hgi), _numBufferSourcesToResolve(0)
      ...
      _textureHandleRegistry(std::make_unique<HdSt_TextureHandleRegistry>(this)), _stagingBuffer(std::make_unique<HdStStagingBuffer>(this))
{
}
...
HdStResourceRegistry::GetGlobalComputeCmds(HgiComputeDispatch dispatchMethod)
{
  ...
  bool const concurrentDispatchSupported =
      _hgi->GetCapabilities()->IsSet(HgiDeviceCapabilitiesBitsConcurrentDispatch);
...
void HdStResourceRegistry::SetMemoryRequestForTextureType(
    const HdTextureType textureType,
    const size_t memoryRequest)
{
