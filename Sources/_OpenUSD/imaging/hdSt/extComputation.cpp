...
HdStExtComputation::HdStExtComputation(SdfPath const &id)
    : HdExtComputation(id), _inputRange()
{
}
                              ...
                              HdDirtyBits *dirtyBits)
{
  ...
  TF_DEBUG(HD_EXT_COMPUTATION_UPDATED).Msg("HdStExtComputation::Sync for %s (dirty bits = 0x%x)\n", GetId().GetText(), *dirtyBits);
  ...
  if (!inputs.empty())
  {
    if (_IsEnabledSharedExtComputationData() && IsInputAggregation())
    {
      ...
      if (barInstance.IsFirstInstance())
      {
        ...
        TF_DEBUG(HD_SHARED_EXT_COMPUTATION_DATA).Msg("Allocated shared ExtComputation buffer range: %s: %p\n", GetId().GetText(), (void *)_inputRange.get());
      }
      else
      {
        ...
        TF_DEBUG(HD_SHARED_EXT_COMPUTATION_DATA).Msg("Reused shared ExtComputation buffer range: %s: %p\n", GetId().GetText(), (void *)_inputRange.get());
      }
    }
    else
    {
      ...
      if (!_inputRange || !_inputRange->IsValid())
      {
        ...
        TF_DEBUG(HD_SHARED_EXT_COMPUTATION_DATA).Msg("Allocated unshared ExtComputation buffer range: %s: %p\n", GetId().GetText(), (void *)_inputRange.get());
      }
      else
      {
        ...
        if (useExistingRange)
        {
          ...
          TF_DEBUG(HD_SHARED_EXT_COMPUTATION_DATA).Msg("Reused unshared ExtComputation buffer range: "
                                                       "%s: %p\n",
                                                       GetId().GetText(), (void *)_inputRange.get());
        }
        else
        {
          _inputRange = _AllocateComputationDataRange(
              std::move(inputs), resourceRegistry);
          TF_DEBUG(HD_SHARED_EXT_COMPUTATION_DATA).Msg("Couldn't reuse existing unshared range. Allocated a "
                                                       "new one.%s: %p\n",
                                                       GetId().GetText(), (void *)_inputRange.get());
        }
      }
