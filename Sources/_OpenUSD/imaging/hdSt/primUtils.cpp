    ...
    SdfPath const &rprimId)
{
  ...
  for (auto const &spec : curBarSpecs)
  {
    ...
    bool isInNewList =
        std::find(newPrimvarNames.begin(), newPrimvarNames.end(), spec.name) != newPrimvarNames.end();
    ...
    bool isInGeneratedList =
        std::find(internallyGeneratedPrimvarNames.begin(),
                  internallyGeneratedPrimvarNames.end(), spec.name) != internallyGeneratedPrimvarNames.end();

    if (!isInGeneratedList)
    {
      TF_DEBUG(HD_RPRIM_UPDATED).Msg("%s: Found primvar %s that has been removed\n", rprimId.GetText(), spec.name.GetText());
    ...
    SdfPath const &rprimId)
{
  ...
  for (const auto &curSpec : curBarSpecs)
  {
    ...
    if (newSpec != updatedSpecs.end() &&
        curSpec != *newSpec)
    {
      TF_DEBUG(HD_RPRIM_UPDATED).Msg("%s: Found primvar %s that has been replaced\n", rprimId.GetText(), curSpec.name.GetText());
    ...
    HdChangeTracker *changeTracker)
{
  ...
  if (curRange == newRange)
  {
    ...
    TF_DEBUG(HD_RPRIM_UPDATED).Msg("%s: BAR at draw coord %d is still (%p)\n", id.GetText(), drawCoordIndex, curRange.get());
  ...
  if (curRangeValid)
  {
    ...
    TF_DEBUG(HD_RPRIM_UPDATED).Msg("%s: Marking garbage collection needed to possibly reclaim BAR %p"
