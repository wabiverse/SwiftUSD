...
void NdrRegistry::_FindAndInstantiateDiscoveryPlugins()
{
  ...
  for (const TfType &discoveryPluginType : discoveryPluginTypes)
  {
    ...
    {
      TF_DEBUG(NDR_DISCOVERY).Msg("[PXR_NDR_DISABLE_PLUGINS] Disabled NdrDiscoveryPlugin '%s'\n", pluginName.c_str());
      continue;
    }
...
void NdrRegistry::_InstantiateParserPlugins(
    const std::set<TfType> &parserPluginTypes)
{
  ...
  for (const TfType &parserPluginType : orderedPluginTypes)
  {
    ...
    if (disabledPlugins.find(pluginName) != disabledPlugins.end())
    {
      TF_DEBUG(NDR_DISCOVERY).Msg("[PXR_NDR_DISABLE_PLUGINS] Disabled NdrParserPlugin '%s'\n", pluginName.c_str());
      continue;
    }
    TF_DEBUG(NDR_DISCOVERY).Msg("Found NdrParserPlugin '%s' for discovery types:\n", parserPluginType.GetTypeName().c_str());
