...
NdrNodeConstPtr NdrRegistry::_GetNodeInNameRangeWithSourceType(
    _DiscoveryResultPtrsByNameRange range, const TfToken &sourceType, NdrVersionFilter filter)
{
  for (auto it = range.first; it != range.second; ++it) {
    const NdrNodeDiscoveryResult &dr = *(it->second);
    if (dr.sourceType != sourceType) {
...
void NdrRegistry::_InstantiateParserPlugins(const std::set<TfType> &parserPluginTypes)
{
  ...
  std::sort(orderedPluginTypes.begin(),
            orderedPluginTypes.end(),
            [](const TfType &a, const TfType &b) { return a.GetTypeName() < b.GetTypeName(); });
