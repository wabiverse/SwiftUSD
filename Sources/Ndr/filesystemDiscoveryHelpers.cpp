    ...
    const NdrParseIdentifierFn &parseIdentifierFn)
{
  for (const std::string &fileName : dirFileNames)
  {
    ...
    if (extIter != allowedExtensions.end())
    {
      ...
      {
        TF_DEBUG(NDR_DISCOVERY).Msg("Found a duplicate node with identifier [%s] "
      ...
      TfToken family, name;
      NdrVersion version;
      const bool parsed = parseIdentifierFn ? parseIdentifierFn(identifier, &family, &name, &version) : NdrFsHelpersSplitShaderIdentifier(identifier, &family, &name, &version);
      if (!parsed)
      {
