...
/*static*/
HdGpGenerativeProcedural *HdGpGenerativeProceduralResolvingSceneIndex::_ConstructProcedural(
    const TfToken &typeName, const SdfPath &proceduralPrimPath)
{
  return HdGpGenerativeProceduralPluginRegistry::GetInstance().ConstructProcedural(
      typeName, proceduralPrimPath);
}
...
HdGpGenerativeProceduralResolvingSceneIndex::HdGpGenerativeProceduralResolvingSceneIndex(
    const HdSceneIndexBaseRefPtr &inputScene, const TfToken &targetPrimTypeName)
    : HdSingleInputFilteringSceneIndexBase(inputScene),
...
/* virtual */
HdSceneIndexPrim HdGpGenerativeProceduralResolvingSceneIndex::GetPrim(
    const SdfPath &primPath) const
{
  ...
  if (it != _generatedPrims.end()) {
    if (_ProcEntry *procEntry = it->second.responsibleProc.load()) {
      ...
      if (std::shared_ptr<HdGpGenerativeProcedural> proc = procEntry->proc) {
        return proc->GetChildPrim(_GetInputSceneIndex(), primPath);
      }
    }
  }
...
/*static*/
void HdGpGenerativeProceduralResolvingSceneIndex::_CombinePathArrays(const _DensePathSet &s,
                                                                     SdfPathVector *v)
{
  if (v->empty()) {
...
/* virtual */
SdfPathVector HdGpGenerativeProceduralResolvingSceneIndex::GetChildPrimPaths(
    const SdfPath &primPath) const
{
  ...
  SdfPathVector inputResult = _GetInputSceneIndex()->GetChildPrimPaths(primPath);
  ...
  if (prim.primType == _targetPrimTypeName) {
    ...
    if (_ProcEntry *procEntry = _UpdateProcedural(primPath, false, &notices)) {
...
/* virtual */
void HdGpGenerativeProceduralResolvingSceneIndex::_PrimsAdded(
  ...
  {  // _dependencies and _procedural lock aquire
    ...
    for (auto it = entries.begin(), e = entries.end(); it != e; ++it) {
      ...
      if (entry.primType == _targetPrimTypeName) {
        ...
        notices.added.emplace_back(entry.primPath,
      ...
      _DependencyMap::const_iterator dIt = _dependencies.find(entryPrimParentPath);
      if (dIt != _dependencies.end()) {
        for (const SdfPath &dependentPath : dIt->second) {
          ...
          if (proceduralsToCook.find(dependentPath) != proceduralsToCook.end()) {
            continue;
          }

          _ProcEntryMap::const_iterator procIt = _procedurals.find(dependentPath);
          if (procIt == _procedurals.end()) {
            continue;
          }

          const _ProcEntry &procEntry = procIt->second;
          const auto dslIt = procEntry.dependencies.find(entryPrimParentPath);

          if (dslIt == procEntry.dependencies.end()) {
            continue;
          }

          if (dslIt->second.Intersects(HdGpGenerativeProcedural::GetChildNamesDependencyKey())) {
    ...
    if (proceduralsToCook.size() >= parallelThreshold) {
...

      {
        ...
        WorkWithScopedParallelism([&]() {
          WorkParallelForEach(cookEntries.begin(), cookEntries.end(), [this](const _CookEntry &e) {
            this->_UpdateProcedural(
                e.first,
                true,
                const_cast<HdGpGenerativeProceduralResolvingSceneIndex::_Notices *>(&e.second));
          });
        });
      }
      ...
      for (const _CookEntry &e : cookEntries) {
        notices.added.insert(notices.added.end(), e.second.added.begin(), e.second.added.end());
        notices.removed.insert(
            notices.removed.end(), e.second.removed.begin(), e.second.removed.end());
        notices.dirtied.insert(
            notices.dirtied.end(), e.second.dirtied.begin(), e.second.dirtied.end());
      }
    }
    else {
      for (const SdfPath &p : proceduralsToCook) {
...
/* virtual */
void HdGpGenerativeProceduralResolvingSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::RemovedPrimEntries &entries)
{
  using _PathSetMap = TfDenseHashMap<SdfPath, TfDenseHashSet<SdfPath, TfHash>, TfHash>;
  ...
  for (const HdSceneIndexObserver::RemovedPrimEntry &entry : entries) {
    _PathSetMap::const_iterator it = dependencyAncestors.find(entry.primPath);
    if (it != dependencyAncestors.end()) {
      for (const SdfPath &dependencyPath : it->second) {
        _DependencyMap::const_iterator dIt = _dependencies.find(dependencyPath);
        if (dIt != _dependencies.end()) {
          ...
          for (const SdfPath &dependentPath : dIt->second) {
            ...
            // removed.
            if (removedProcedurals.find(dependentPath) == removedProcedurals.end()) {
      ...
      _DependencyMap::const_iterator dIt = _dependencies.find(entry.primPath.GetParentPath());
      if (dIt != _dependencies.end()) {
        for (const SdfPath &dependentPath : dIt->second) {
          ...
          const _ProcEntry &procEntry = procIt->second;
          const auto dslIt = procEntry.dependencies.find(entry.primPath.GetParentPath());
          if (dslIt == procEntry.dependencies.end()) {
            continue;
          }
...
/* virtual */
void HdGpGenerativeProceduralResolvingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
...

  {
    ...
    for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
      if (_procedurals.find(entry.primPath) != _procedurals.end()) {
        invalidatedProceduralDependencies[entry.primPath][entry.primPath].insert(
            entry.dirtyLocators);
      }

      _DependencyMap::const_iterator it = _dependencies.find(entry.primPath);
      ...
      for (const SdfPath &dependentPath : it->second) {
        ...
        if (dslIt != procEntry.dependencies.end()) {
          if (entry.dirtyLocators.Intersects(dslIt->second)) {
            invalidatedProceduralDependencies[dependentPath][entry.primPath].insert(
                entry.dirtyLocators);
          }
        }
      }
  ...
  if (!invalidatedProceduralDependencies.empty()) {
    ...
    if (invalidatedProceduralDependencies.size() >= parallelThreshold) {
        ...
        WorkWithScopedParallelism([&]() {
          WorkParallelForEach(cookEntries.begin(), cookEntries.end(), [this](const _CookEntry &e) {
            this->_UpdateProcedural(
                e.path,
                true,
                const_cast<HdGpGenerativeProceduralResolvingSceneIndex::_Notices *>(&e.notices),
                e.deps);
          });
        });
      }
      ...
      // combine all of the resulting notices following parallel cook
      for (const _CookEntry &e : cookEntries) {
        notices.added.insert(notices.added.end(), e.notices.added.begin(), e.notices.added.end());
        notices.removed.insert(
            notices.removed.end(), e.notices.removed.begin(), e.notices.removed.end());
        notices.dirtied.insert(
            notices.dirtied.end(), e.notices.dirtied.begin(), e.notices.dirtied.end());
      }
    }
    else {
      for (const auto &pathDepsPair : invalidatedProceduralDependencies) {
        _UpdateProcedural(pathDepsPair.first, true, &notices, &pathDepsPair.second);
      }
    }
...
{
  HdSceneIndexPrim procPrim = _GetInputSceneIndex()->GetPrim(proceduralPrimPath);
  ...
  HdPrimvarsSchema primvars = HdPrimvarsSchema::GetFromParent(procPrim.dataSource);

  HdSampledDataSourceHandle procTypeDs =
  ...
  if (!procEntry.proc || procType != procEntry.typeName) {
    proc.reset(_ConstructProcedural(procType, proceduralPrimPath));

    if (proc) {
      bool result = proc->AsyncBegin(_attemptAsync);
      if (_attemptAsync && result) {
        _activeSyncProcedurals[proceduralPrimPath] = TfCreateWeakPtr(&procEntry);
      }
    }
  }
  else {
    ...
    if (procEntry.proc && _attemptAsync &&
        _activeSyncProcedurals.find(proceduralPrimPath) == _activeSyncProcedurals.end())
    {
  ...
  if (procEntry.state.compare_exchange_strong(current, _ProcEntry::StateDependenciesCooking)) {
    ...
    if (!newDependencies.empty() || !dependencesToRemove.empty()) {
      ...
      for (const auto &pathLocatorsPair : newDependencies) {
        const SdfPath &dependencyPath = pathLocatorsPair.first;
        if (procEntry.dependencies.find(dependencyPath) == procEntry.dependencies.end()) {
          _dependencies[dependencyPath].insert(proceduralPrimPath);
        }
      }
      for (const SdfPath &dependencyPath : dependencesToRemove) {
        _DependencyMap::iterator it = _dependencies.find(dependencyPath);
        if (it != _dependencies.end()) {
    ...
    const HdGpGenerativeProcedural::DependencyMap *dirtiedDependencies) const
{
  ...
  HdGpGenerativeProcedural::ChildPrimTypeMap newChildTypes = procEntry.proc->Update(
      _GetInputSceneIndex(),
  ...
  if (procEntry.state.compare_exchange_strong(current, _ProcEntry::StateCooking)) {

    std::unique_lock<std::mutex> cookLock(procEntry.cookMutex);

    _UpdateProceduralResult(&procEntry, proceduralPrimPath, newChildTypes, outputNotices);
...
void HdGpGenerativeProceduralResolvingSceneIndex::_RemoveProcedural(
    const SdfPath &proceduralPrimPath, _Notices *outputNotices) const
{
  ...
  if (!procEntry.dependencies.empty()) {
    ...
    for (const auto &pathLocatorsPair : procEntry.dependencies) {
      _DependencyMap::iterator dIt = _dependencies.find(pathLocatorsPair.first);
...
void HdGpGenerativeProceduralResolvingSceneIndex::_GarbageCollect()
{
...
void HdGpGenerativeProceduralResolvingSceneIndex::_SystemMessage(
    const TfToken &messageType, const HdDataSourceBaseHandle &args)
{
  if (!_attemptAsync) {
  ...
  for (auto &pathEntryPair : _activeSyncProcedurals) {
    ...
    HdGpGenerativeProcedural::AsyncState result = procEntryPtr->proc->AsyncUpdate(
        procEntryPtr->childTypes, &primTypes, &notices.dirtied);

    if (result == HdGpGenerativeProcedural::FinishedWithNewChanges ||
        result == HdGpGenerativeProcedural::ContinuingWithNewChanges)
    {
      _UpdateProceduralResult(get_pointer(procEntryPtr), proceduralPrimPath, primTypes, &notices);
      primTypes.clear();
    }
...
void HdGpGenerativeProceduralResolvingSceneIndex::_UpdateProceduralResult(
    _ProcEntry *procEntryPtr,
    const SdfPath &proceduralPrimPath,
    ...
    _Notices *outputNotices) const
{
  ...
  if (procEntry.childTypes.empty()) {
    for (const auto &pathTypePair : newChildTypes) {
      ...
      outputNotices->added.emplace_back(childPrimPath, pathTypePair.second);

      if (childPrimPath.HasPrefix(proceduralPrimPath)) {
        for (const SdfPath &p : childPrimPath.GetAncestorsRange()) {
          if (p == proceduralPrimPath) {
            break;
          }
          procEntry.childHierarchy[p.GetParentPath()].insert(p);
    ...
    for (const auto &pathTypePair : newChildTypes) {
      ...
      if (it != procEntry.childTypes.end() && pathTypePair.second == it->second) {
    ...
    for (const auto &pathTypePair : procEntry.childTypes) {
      if (newChildTypes.find(pathTypePair.first) == newChildTypes.end()) {
        if (newChildHierarchy.find(pathTypePair.first) == newChildHierarchy.end()) {
          outputNotices->removed.emplace_back(pathTypePair.first);
        ...
        newChildHierarchy != procEntry.childHierarchy)
    {
      for (const auto &pathPathSetPair : newChildHierarchy) {
        ...
        if (procEntry.childHierarchy.find(parentPath) == procEntry.childHierarchy.end()) {
          // if it's also not directly in our current
          // childTypes, add it as a type-less prim
          if (newChildTypes.find(parentPath) == newChildTypes.end()) {
            addAsIntermediate = true;
          }
          else if (procEntry.childTypes.find(parentPath) != procEntry.childTypes.end()) {
          ...
          // an intermediate
          if (procEntry.childTypes.find(parentPath) != procEntry.childTypes.end() &&
              newChildTypes.find(parentPath) == newChildTypes.end())
          {
            addAsIntermediate = true;
          }
        }
        ...
        if (addAsIntermediate) {
          ...
          outputNotices->added.emplace_back(parentPath, TfToken());
        }
      }
      ...
      for (const auto &pathPathSetPair : procEntry.childHierarchy) {
        ...
        if (newChildHierarchy.find(parentPath) == newChildHierarchy.end()) {
  ...
  for (const SdfPath &generatedPrimPath : generatedPrims) {
    ...
    _generatedPrims[generatedPrimPath].responsibleProc.store(&procEntry);
  }
