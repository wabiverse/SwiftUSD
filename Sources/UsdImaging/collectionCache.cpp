...
void UsdImaging_CollectionCache::_MarkCollectionContentDirty(
    UsdStageWeakPtr const &stage, UsdCollectionAPI::MembershipQuery const &query)
{
  SdfPathSet linkedPaths = UsdComputeIncludedPathsFromCollection(query, stage);
  std::merge(_dirtyPaths.begin(),
             _dirtyPaths.end(),
             linkedPaths.begin(),
             linkedPaths.end(),
...
bool UsdImaging_CollectionCache::UpdateCollection(UsdCollectionAPI const &c)
{
