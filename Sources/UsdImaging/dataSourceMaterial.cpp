...
SdfPath _RelativePath(const SdfPath &prefix, const SdfPath &path)
{
  return prefix.IsEmpty() ? path : path.ReplacePrefix(prefix, SdfPath::ReflexiveRelativePath());
}

// Extract the renderContext from an output name, ex:
// "outputs:surface" -> ""
// "outputs:ri:surface" -> "ri"
TfToken _GetRenderContextForShaderOutput(UsdShadeOutput const &output)
{
...
bool _Contains(const TfTokenVector &v, const TfToken &t)
{
    ...
    for (const UsdShadeInput &input : consumers) {
      consumerContainers.push_back(
          HdMaterialInterfaceMappingSchema::Builder()
              .SetNodePath(HdRetainedTypedSampledDataSource<TfToken>::New(
                  _RelativePath(_material.GetPrim().GetPath(), input.GetPrim().GetPath())
                      .GetToken()))
              .SetInputName(HdRetainedTypedSampledDataSource<TfToken>::New(input.GetBaseName()))
              .Build());
    }

    return HdRetainedSmallVectorDataSource::New(consumerContainers.size(),
                                                consumerContainers.data());
  }

 private:
  _UsdImagingDataSourceInterfaceMappings(const UsdShadeMaterial &material) : _material(material)
  {
...
class _UsdImagingDataSourceShadingNodeParameters : public HdContainerDataSource {
 public:
  ...
  {
    TfTokenVector result;
    const std::vector<UsdShadeInput> shadeNodeInputs = _shaderNode.GetInputs();
    result.reserve(shadeNodeInputs.size());
    for (UsdShadeInput const &input : shadeNodeInputs) {
      for (UsdAttribute const &attr : input.GetValueProducingAttributes()) {
        UsdShadeAttributeType attrType = UsdShadeUtils::GetType(attr.GetName());
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    for (UsdAttribute const &attr : input.GetValueProducingAttributes()) {
      UsdShadeAttributeType attrType = UsdShadeUtils::GetType(attr.GetName());
      if (attrType == UsdShadeAttributeType::Input) {
        const HdDataSourceLocator paramValueLocator(name,
                                                    HdMaterialNodeParameterSchemaTokens->value);
        return HdMaterialNodeParameterSchema::Builder()
            .SetValue(UsdImagingDataSourceAttributeNew(
                attr, _stageGlobals, _sceneIndexPath, _locatorPrefix.Append(paramValueLocator)))
            .SetColorSpace(UsdImagingDataSourceAttributeColorSpace::New(attr))
            .Build();
      }
    }
        ...
        _shaderNode.GetPrim().IsA<UsdLuxLightFilter>())
    {
      ...
      return HdMaterialNodeParameterSchema::Builder()
          .SetValue(UsdImagingDataSourceAttributeNew(
              input, _stageGlobals, _sceneIndexPath, _locatorPrefix.Append(paramValueLocator)))
          .Build();
    }
 ...
 private:
  _UsdImagingDataSourceShadingNodeParameters(UsdShadeShader shaderNode,
...
class _UsdImagingDataSourceShadingNodeInputs : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    UsdShadeAttributeVector attrs = input.GetValueProducingAttributes(/*shaderOutputsOnly =*/true);
    ...
    for (const UsdAttribute &attr : attrs) {
      const TfToken outputPath(
          _RelativePath(_materialPrefix, attr.GetPrim().GetPath()).GetToken());
    ...
    return HdRetainedSmallVectorDataSource::New(elements.size(), elements.data());
  }

 private:
  _UsdImagingDataSourceShadingNodeInputs(UsdShadeShader shaderNode,
                                         const UsdImagingDataSourceStageGlobals &stageGlobals,
                                         const SdfPath &materialPrefix)
      : _shaderNode(shaderNode), _stageGlobals(stageGlobals), _materialPrefix(materialPrefix)
  {
  }
...
template<typename T>
class _UsdImagingDataSourceRenderContextIdentifiers : public HdContainerDataSource {
 public:
  ...
  TfTokenVector GetNames() override
  {
    ...
    for (const TfToken &propNameToken : _t.GetPrim().GetPropertyNames()) {
      ...
      static const std::string suffix = ":" + _GetTypedShaderId().GetString();
      if (TfStringEndsWith(propName, suffix)) {
        result.push_back(TfToken(propName.substr(0, propName.size() - suffix.size())));
      }
    }
 ...
 private:
  ...
  _UsdImagingDataSourceRenderContextIdentifiers(const T &t) : _t(t) {}

  T _t;
};

template<>
TfToken _UsdImagingDataSourceRenderContextIdentifiers<UsdLuxLightAPI>::_GetTypedShaderId()
{
  return UsdLuxTokens->lightShaderId;
}

template<>
TfToken _UsdImagingDataSourceRenderContextIdentifiers<UsdLuxLightFilter>::_GetTypedShaderId()
{
...
class _UsdImagingNodeTypeInfoSource : public HdContainerDataSource {
 public:
  ...
  TfTokenVector GetNames() override
  {
    ...
    // Missing metadata:
    // subIdentifier
    for (const TfToken &propNameToken : _shaderNode.GetPrim().GetPropertyNames()) {
      const std::string &propName = propNameToken.GetString();
      if (propNameToken != UsdShadeTokens->infoId && TfStringStartsWith(propName, _GetPrefix())) {
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == UsdShadeTokens->sdrMetadata) {
      ...
      return HdRetainedTypedSampledDataSource<VtDictionary>::New(metadata);
    }
 ...
 private:
  _UsdImagingNodeTypeInfoSource(const UsdShadeShader &shaderNode,
                                const UsdImagingDataSourceStageGlobals &stageGlobals)
...
class _UsdImagingDataSourceShadingNode : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdMaterialNodeSchemaTokens->nodeIdentifier) {
      ...
      else if (UsdLuxLightFilter lightFilter = UsdLuxLightFilter(_shaderNode.GetPrim())) {
        nodeId = lightFilter.GetShaderId({_renderContext});
      }
      else if (UsdLuxLightAPI light = UsdLuxLightAPI(_shaderNode.GetPrim())) {
        nodeId = light.GetShaderId({_renderContext});
      }
      else if (UsdShadeNodeGraph nodegraph = UsdShadeNodeGraph(_shaderNode.GetPrim())) {
        nodeId = TfToken();
      }
    ...
    if (name == HdMaterialNodeSchemaTokens->renderContextNodeIdentifiers) {
      if (UsdLuxLightAPI light = UsdLuxLightAPI(_shaderNode.GetPrim())) {
        return _UsdImagingDataSourceRenderContextIdentifiers<UsdLuxLightAPI>::New(light);
      }
      if (UsdLuxLightFilter lightFilter = UsdLuxLightFilter(_shaderNode.GetPrim())) {
        return _UsdImagingDataSourceRenderContextIdentifiers<UsdLuxLightFilter>::New(lightFilter);
      }
      return nullptr;
    }
    ...
    if (name == HdMaterialNodeSchemaTokens->nodeTypeInfo) {
      if (_shaderNode.GetImplementationSource() != UsdShadeTokens->id) {
        return _UsdImagingNodeTypeInfoSource::New(_shaderNode, _stageGlobals);
      }
      return nullptr;
    }
    ...
    if (name == HdMaterialNodeSchemaTokens->parameters) {
      return _UsdImagingDataSourceShadingNodeParameters::New(
          ...
          _locatorPrefix.IsEmpty() ?
              _locatorPrefix :
              _locatorPrefix.Append(_shaderNode.GetPrim().GetPath().GetToken())
 ...
 private:
  _UsdImagingDataSourceShadingNode(UsdShadeShader shaderNode,
                                   const UsdImagingDataSourceStageGlobals &stageGlobals,
...
UsdImagingDataSourceMaterial::UsdImagingDataSourceMaterial(
    ...
    : _usdPrim(usdPrim), _stageGlobals(stageGlobals), _fixedTerminalName(fixedTerminalName)
{
}
...
TfTokenVector UsdImagingDataSourceMaterial::GetNames()
{
  ...
  for (const UsdShadeOutput &output : UsdShadeNodeGraph(_usdPrim).GetOutputs()) {
...
using _TokenDataSourceMap = TfDenseHashMap<TfToken, HdDataSourceBaseHandle, TfHash>;
                       ...
                       const SdfPath &materialPrefix = SdfPath())
{
  ...
  HdDataSourceBaseHandle nodeValue = _UsdImagingDataSourceShadingNode::New(
      shadeNode, stageGlobals, renderContext, sceneIndexPath, locatorPrefix, materialPrefix);
  ...
  for (UsdShadeInput const &input : shadeNode.GetInputs()) {
    for (const UsdAttribute &attr : input.GetValueProducingAttributes(/*shaderOutputsOnly =*/true))
    {
...
static HdDataSourceBaseHandle _BuildNetwork(UsdShadeConnectableAPI const &terminalNode,
                                            ...
                                            const HdDataSourceLocator &locatorPrefix)
{
  ...
  _WalkGraph(terminalNode,
             ...
             renderContext,
             sceneIndexPath,
             locatorPrefix.IsEmpty() ? locatorPrefix :
  ...
  HdContainerDataSourceHandle terminalsDs = HdRetainedContainerDataSource::New(
      terminalName,
      HdMaterialConnectionSchema::Builder()
          .SetUpstreamNodePath(HdRetainedTypedSampledDataSource<TfToken>::New(
              terminalNode.GetPrim().GetPath().GetToken()))
          .SetUpstreamNodeOutputName(HdRetainedTypedSampledDataSource<TfToken>::New(terminalName))
          .Build());

  HdContainerDataSourceHandle nodesDs = HdRetainedContainerDataSource::New(
  ...
  return HdMaterialNetworkSchema::Builder()
      ...
      .SetInterfaceMappings(HdLazyContainerDataSource::New(
          [material]() { return _UsdImagingDataSourceInterfaceMappings ::New(material); }))
      .Build();
}

static UsdShadeConnectableAPI _ComputeOutputSource(const UsdShadeMaterial &usdMat,
                                                   ...
                                                   UsdShadeConnectionSourceInfo sourceInfo)
{
  ...
  if (outputName == UsdShadeTokens->volume) {
    return UsdShadeConnectableAPI(
        usdMat.ComputeVolumeSource(renderContext, &sourceInfo.sourceName, &sourceInfo.sourceType));
  }
  return UsdShadeConnectableAPI(sourceInfo.source.GetPrim());
}

static HdDataSourceBaseHandle _BuildMaterial(UsdShadeNodeGraph const &usdMat,
                                             UsdImagingDataSourceStageGlobals const &stageGlobals,
                                             TfToken const &renderContext,
                                             const SdfPath &sceneIndexPath,
                                             const HdDataSourceLocator &locatorPrefix)
{
  ...
  for (UsdShadeOutput &output : usdMat.GetOutputs()) {
    ...
    if (!renderContext.IsEmpty()) {
      ...
      outputName = TfToken(outputName.GetString().substr(renderContext.size() + 1));
    }

    for (const UsdShadeConnectionSourceInfo &sourceInfo : output.GetConnectedSources()) {
      ...
      SdfPath upstreamPath = _RelativePath(materialPrefix, upstreamShader.GetPath());

      terminalsValues.push_back(
          HdMaterialConnectionSchema::Builder()
              .SetUpstreamNodePath(
                  HdRetainedTypedSampledDataSource<TfToken>::New(upstreamPath.GetToken()))
              .SetUpstreamNodeOutputName(
                  HdRetainedTypedSampledDataSource<TfToken>::New(sourceInfo.sourceName))
              .Build());
    }
  }
  ...
  HdContainerDataSourceHandle terminalsDs = HdRetainedContainerDataSource::New(
      terminalsNames.size(), terminalsNames.data(), terminalsValues.data());
  ...
  return HdMaterialNetworkSchema::Builder()
      ...
      .SetInterfaceMappings(
          _UsdImagingDataSourceInterfaceMappings::New(UsdShadeMaterial(usdMat.GetPrim())))
      .Build();
}

HdDataSourceBaseHandle UsdImagingDataSourceMaterial::Get(const TfToken &name)
{
  TRACE_FUNCTION();
  ...
  if (_fixedTerminalName.IsEmpty()) {
    networkDs = _BuildMaterial(UsdShadeNodeGraph(_usdPrim),
                               _stageGlobals,
                               name,
                               _usdPrim.GetPath(),
...
TfTokenVector UsdImagingDataSourceMaterialPrim::GetNames()
{
...
HdDataSourceBaseHandle UsdImagingDataSourceMaterialPrim::Get(const TfToken &name)
{
  if (name == HdMaterialSchema::GetSchemaToken()) {
    return UsdImagingDataSourceMaterial::New(_GetUsdPrim(), _GetStageGlobals());
  }
  return UsdImagingDataSourcePrim::Get(name);
}

HdDataSourceLocatorSet UsdImagingDataSourceMaterialPrim::Invalidate(
    UsdPrim const &prim,
