...
UsdImagingDataSourceVisibility::UsdImagingDataSourceVisibility(
    ...
    : _visibilityQuery(visibilityQuery), _stageGlobals(stageGlobals)
{
  if (_visibilityQuery.ValueMightBeTimeVarying()) {
    _stageGlobals.FlagAsTimeVarying(sceneIndexPath, HdVisibilitySchema::GetDefaultLocator());
  }
}
...
UsdImagingDataSourcePurpose::UsdImagingDataSourcePurpose(
    const UsdAttributeQuery &purposeQuery, const UsdImagingDataSourceStageGlobals &stageGlobals)
    : _purposeQuery(purposeQuery), _stageGlobals(stageGlobals)
{
}
...
UsdImagingDataSourceExtentCoordinate::UsdImagingDataSourceExtentCoordinate(
    const HdVec3fArrayDataSourceHandle &extentDs, const SdfPath &attrPath, unsigned int index)
    : _extentDs(extentDs), _attrPath(attrPath), _index(index)
{
}
...
{
  return _extentDs->GetContributingSampleTimesForInterval(startTime, endTime, outSampleTimes);
}
    ...
    const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  if (extentQuery.ValueMightBeTimeVarying()) {
    stageGlobals.FlagAsTimeVarying(sceneIndexPath, HdExtentSchema::GetDefaultLocator());
  }
...
HdDataSourceBaseHandle UsdImagingDataSourceExtent::Get(const TfToken &name)
{
  ...
  if (name == HdExtentSchemaTokens->min) {
    return UsdImagingDataSourceExtentCoordinate::New(_extentDs, _attrPath, 0);
  }
  else if (name == HdExtentSchemaTokens->max) {
    return UsdImagingDataSourceExtentCoordinate::New(_extentDs, _attrPath, 1);
  }
...
static TfTokenVector _UsdToHdPurposes(const TfTokenVector &v)
{
...
static const TfTokenVector &_OrderedPurposes()
{
  static const TfTokenVector result = _UsdToHdPurposes(
...
TfTokenVector UsdImagingDataSourceExtentsHint::GetNames()
{
  if (!_extentDs) {
    return {};
  }
...
HdDataSourceBaseHandle UsdImagingDataSourceExtentsHint::Get(const TfToken &name)
{
  if (!_extentDs) {
    return nullptr;
  }
  ...
  for (size_t i = 0; i < n; i++) {
    if (name == orderedPurposes[i]) {
      return HdExtentSchema::Builder()
          .SetMin(UsdImagingDataSourceExtentCoordinate::New(_extentDs, _attrPath, 2 * i))
          .SetMax(UsdImagingDataSourceExtentCoordinate::New(_extentDs, _attrPath, 2 * i + 1))
          .Build();
    }
  }
  return nullptr;
...
UsdImagingDataSourceXformResetXformStack::UsdImagingDataSourceXformResetXformStack(
    ...
    const UsdImagingDataSourceStageGlobals &stageGlobals)
    : _xformQuery(xformQuery), _stageGlobals(stageGlobals)
{
}
    ...
    : _xformQuery(xformQuery), _stageGlobals(stageGlobals)
{
  if (_xformQuery.TransformMightBeTimeVarying()) {
    _stageGlobals.FlagAsTimeVarying(sceneIndexPath, HdXformSchema::GetDefaultLocator());
  }
}
...
UsdImagingDataSourcePrimOrigin::UsdImagingDataSourcePrimOrigin(const UsdPrim &usdPrim)
    : _usdPrim(usdPrim)
{
}

TfTokenVector UsdImagingDataSourcePrimOrigin::GetNames()
{
  return {HdPrimOriginSchemaTokens->scenePath};
}
...
//
static SdfPath _ComputePrototypeRelativePath(const UsdPrim &prim)
{
...
HdDataSourceBaseHandle UsdImagingDataSourcePrimOrigin::Get(const TfToken &name)
{
  if (name == HdPrimOriginSchemaTokens->scenePath) {
    ...
    return DataSource::New(OriginPath(_ComputePrototypeRelativePath(_usdPrim)));
  }
...
UsdImagingDataSourcePrim_ModelAPI::UsdImagingDataSourcePrim_ModelAPI(const UsdModelAPI &model)
    : _model(model)
{
}

TfTokenVector UsdImagingDataSourcePrim_ModelAPI::GetNames()
{
  return {
...
HdDataSourceBaseHandle UsdImagingDataSourcePrim_ModelAPI::Get(const TfToken &name)
{
  TRACE_FUNCTION();
  if (name == UsdImagingModelSchemaTokens->modelPath) {
    return HdRetainedTypedSampledDataSource<SdfPath>::New(_model.GetPrim().GetPath());
  }
    ...
    if (_model.GetAssetIdentifier(&assetIdentifier)) {
      return HdRetainedTypedSampledDataSource<SdfAssetPath>::New(assetIdentifier);
    }
    return nullptr;
  }
    ...
    if (_model.GetAssetName(&assetName)) {
      return HdRetainedTypedSampledDataSource<std::string>::New(assetName);
    }
    return nullptr;
  }
    ...
    if (_model.GetAssetVersion(&assetVersion)) {
      return HdRetainedTypedSampledDataSource<std::string>::New(assetVersion);
    }
    return nullptr;
  }
  return nullptr;
...
UsdImagingDataSourcePrim::UsdImagingDataSourcePrim(
    ...
    UsdPrim usdPrim,
    const UsdImagingDataSourceStageGlobals &stageGlobals)
    : _sceneIndexPath(sceneIndexPath), _usdPrim(usdPrim), _stageGlobals(stageGlobals)
{
}
...
TfTokenVector UsdImagingDataSourcePrim::GetNames()
{
  ...
  if (UsdAttributeQuery(UsdGeomModelAPI(_GetUsdPrim()).GetExtentsHintAttr()).HasAuthoredValue()) {
...
HdDataSourceBaseHandle UsdImagingDataSourcePrim::Get(const TfToken &name)
{
  ...
  if (name == HdXformSchema::GetSchemaToken()) {
    ...
    return UsdImagingDataSourcePrimvars::New(_GetSceneIndexPath(),
                                             _GetUsdPrim(),
    ...
    UsdAttributeQuery extentsHintQuery(UsdGeomModelAPI(_GetUsdPrim()).GetExtentsHintAttr());
    if (extentsHintQuery.HasAuthoredValue()) {
      return UsdImagingDataSourceExtentsHint::New(
          extentsHintQuery, _sceneIndexPath, _GetStageGlobals());
    }
    else {
      return nullptr;
    }
    ...
    return UsdImagingDataSourceUsdPrimInfo::New(_GetUsdPrim());
  }
  else if (name == HdPrimOriginSchema::GetSchemaToken()) {
    return UsdImagingDataSourcePrimOrigin::New(_GetUsdPrim());
  }
  return nullptr;
}

/*static*/ HdDataSourceLocatorSet UsdImagingDataSourcePrim::Invalidate(
    UsdPrim const &prim,
    ...
    if (UsdGeomPrimvarsAPI::CanContainPropertyName(propertyName)) {
      if (invalidationType == UsdImagingPropertyInvalidationType::Resync) {
        ...
        locators.insert(HdPrimvarsSchema::GetDefaultLocator().Append(
            TfToken(propertyName.data() + prefixLength)));
      }
    }
  }
