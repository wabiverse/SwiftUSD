    ...
    const UsdPrim &usdPrim)
{
  if (usdPrim.IsA<UsdGeomPointBased>()) {
    static const UsdImagingDataSourceCustomPrimvars::Mappings forPointBased = {
        {HdPrimvarsSchemaTokens->points, UsdGeomTokens->points},
        {HdTokens->velocities, UsdGeomTokens->velocities},
        {HdTokens->accelerations, UsdGeomTokens->accelerations},
        {HdTokens->nonlinearSampleCount, UsdGeomTokens->motionNonlinearSampleCount},
        {HdTokens->blurScale, UsdGeomTokens->motionBlurScale},
        {HdPrimvarsSchemaTokens->normals, UsdGeomTokens->normals},
    };

    if (usdPrim.IsA<UsdGeomCurves>()) {
      static const UsdImagingDataSourceCustomPrimvars::Mappings forCurves = _Merge(
          forPointBased, {{HdPrimvarsSchemaTokens->widths, UsdGeomTokens->widths}});
      return forCurves;
    }
...
HdDataSourceBaseHandle UsdImagingDataSourceGprim::Get(const TfToken &name)
{
  ...
  if (name == HdPrimvarsSchema::GetSchemaToken()) {
    const UsdImagingDataSourceCustomPrimvars::Mappings &mappings = _GetCustomPrimvarMappings(
        _GetUsdPrim());
    if (mappings.empty()) {
      return result;
    }
    return HdOverlayContainerDataSource::New(
        ...
        UsdImagingDataSourceCustomPrimvars::New(
            _GetSceneIndexPath(), _GetUsdPrim(), mappings, _GetStageGlobals()));
  }
...
/*static*/
HdDataSourceLocatorSet UsdImagingDataSourceGprim::Invalidate(
    UsdPrim const &prim,
    ...
    UsdImagingPropertyInvalidationType invalidationType)
{
  ...
  if (subprim.IsEmpty()) {
    const UsdImagingDataSourceCustomPrimvars::Mappings &mappings = _GetCustomPrimvarMappings(prim);

    if (!mappings.empty()) {
      result.insert(UsdImagingDataSourceCustomPrimvars::Invalidate(properties, mappings));
    }
  }
