...
TfTokenVector UsdImagingCameraAdapter::GetImagingSubprims(UsdPrim const &prim)
{
  return {TfToken()};
}

TfToken UsdImagingCameraAdapter::GetImagingSubprimType(UsdPrim const &prim, TfToken const &subprim)
{
  if (subprim.IsEmpty()) {
...
HdDataSourceLocatorSet UsdImagingCameraAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
...
{
  if (subprim.IsEmpty()) {
    return UsdImagingDataSourceCameraPrim::Invalidate(prim, subprim, properties, invalidationType);
  }
    ...
    UsdImagingInstancerContext const *instancerContext) const
{
  ...
  for (UsdAttribute const &attr : attrs) {
    ...
    if (attr.GetBaseName() == UsdGeomTokens->clippingPlanes) {
      continue;
    }
    ...
    if (_IsVarying(
            prim, attr.GetName(), HdCamera::DirtyParams, attr.GetName(), timeVaryingBits, false))
    {
      return;
    }
  }
