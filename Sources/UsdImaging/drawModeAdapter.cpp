...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (cardSurface)(cardTexture)(cardUvCoords)

        (cardsUv)

            (subsetXPos)(subsetYPos)(subsetZPos)(subsetXNeg)(subsetYNeg)(subsetZNeg)

                (subsetMaterialXPos)(subsetMaterialYPos)(subsetMaterialZPos)(subsetMaterialXNeg)(subsetMaterialYNeg)(subsetMaterialZNeg)

                    (worldtoscreen)(worldToNDC)

                        (displayRoughness)(diffuseColor)(opacity)(opacityThreshold)

                            (file)(st)(rgb)(a)(fallback)(wrapS)(wrapT)(clamp)
...
static AxesMask _GetOppositeFace(AxesMask face)
{
  switch (face) {
    case xPos:
      return xNeg;
...
static TfToken _GetSubsetTokenForFace(AxesMask axis)
{
  switch (axis) {
    case xPos:
      return _tokens->subsetXPos;
    case yPos:
      return _tokens->subsetYPos;
    case zPos:
      return _tokens->subsetZPos;
    case xNeg:
      return _tokens->subsetXNeg;
    case yNeg:
      return _tokens->subsetYNeg;
    case zNeg:
      return _tokens->subsetZNeg;
    default:
      return TfToken();  // multiple axes not supported
  }
}

static TfToken _GetSubsetMaterialTokenForFace(AxesMask axis)
{
  switch (axis) {
    case xPos:
      return _tokens->subsetMaterialXPos;
    case yPos:
      return _tokens->subsetMaterialYPos;
    case zPos:
      return _tokens->subsetMaterialZPos;
    case xNeg:
      return _tokens->subsetMaterialXNeg;
    case yNeg:
      return _tokens->subsetMaterialYNeg;
    case zNeg:
      return _tokens->subsetMaterialZNeg;
    default:
      return TfToken();  // multiple axes not supported
  }
}

static uint8_t _GetAxesMask(UsdPrim const &prim, UsdTimeCode time)
{
...
UsdImagingDrawModeAdapter::UsdImagingDrawModeAdapter() : BaseAdapter(), _schemaColor(0)
{
...
bool UsdImagingDrawModeAdapter::ShouldCullChildren() const
{
  return true;
}

bool UsdImagingDrawModeAdapter::CanPopulateUsdInstance() const
{
  return true;
}

bool UsdImagingDrawModeAdapter::IsSupported(UsdImagingIndexProxy const *index) const
{
  return true;
}

SdfPath UsdImagingDrawModeAdapter::Populate(UsdPrim const &prim,
                                            UsdImagingIndexProxy *index,
                                            UsdImagingInstancerContext const *instancerContext)
{
  const SdfPath cachePath = ResolveCachePath(prim.GetPath(), instancerContext);
  const SdfPath proxyPrimPath = ResolveProxyPrimPath(cachePath, instancerContext);
  ...
  if (drawMode == UsdGeomTokens->origin || drawMode == UsdGeomTokens->bounds) {
    ...
    index->InsertRprim(HdPrimTypeTokens->basisCurves, cachePath, proxyPrim, rprimAdapter);
    ...
    index->InsertRprim(HdPrimTypeTokens->mesh, cachePath, proxyPrim, rprimAdapter);
    HD_PERF_COUNTER_INCR(UsdImagingTokens->usdPopulatedPrimCount);
  }
  else {
    TF_CODING_ERROR(
  ...
  if (drawMode == UsdGeomTokens->cards) {
    ...
    for (const AxesMask face : faces) {
      if (mask & face) {
        SdfPath materialPath = prim.GetPath().AppendChild(_GetSubsetMaterialTokenForFace(face));
...
bool UsdImagingDrawModeAdapter::_IsMaterialPath(SdfPath const &path) const
{
  TfToken nameToken = path.GetNameToken();
  return nameToken == _tokens->subsetMaterialXPos || nameToken == _tokens->subsetMaterialYPos ||
         nameToken == _tokens->subsetMaterialZPos || nameToken == _tokens->subsetMaterialXNeg ||
         nameToken == _tokens->subsetMaterialYNeg || nameToken == _tokens->subsetMaterialZNeg;
}

void UsdImagingDrawModeAdapter::ProcessPrimResync(SdfPath const &cachePath,
                                                  UsdImagingIndexProxy *index)
...
void UsdImagingDrawModeAdapter::ProcessPrimRemoval(SdfPath const &cachePath,
                                                   UsdImagingIndexProxy *index)
{
  if (_IsMaterialPath(cachePath)) {
...
void UsdImagingDrawModeAdapter::_RemovePrim(SdfPath const &cachePath, UsdImagingIndexProxy *index)
{
  TF_CODING_ERROR("_RemovePrim called on draw mode adapter!");
}

void UsdImagingDrawModeAdapter::MarkDirty(UsdPrim const &prim,
                                          ...
                                          UsdImagingIndexProxy *index)
{
  if (_IsMaterialPath(cachePath)) {
    ...
    HdDirtyBits bitsMask = HdChangeTracker::DirtyTopology | HdChangeTracker::DirtyPoints |
...
void UsdImagingDrawModeAdapter::MarkTransformDirty(UsdPrim const &prim,
                                                   SdfPath const &cachePath,
                                                   UsdImagingIndexProxy *index)
{
  if (!_IsMaterialPath(cachePath)) {
...
void UsdImagingDrawModeAdapter::MarkVisibilityDirty(UsdPrim const &prim,
                                                    SdfPath const &cachePath,
                                                    UsdImagingIndexProxy *index)
{
  if (!_IsMaterialPath(cachePath)) {
...
void UsdImagingDrawModeAdapter::MarkMaterialDirty(UsdPrim const &prim,
                                                  SdfPath const &cachePath,
                                                  UsdImagingIndexProxy *index)
{
  if (_IsMaterialPath(cachePath)) {
...
bool UsdImagingDrawModeAdapter::_HasVaryingExtent(UsdPrim const &prim) const
{
  UsdAttribute attr;
...
void UsdImagingDrawModeAdapter::_ComputeGeometryData(UsdPrim const &prim,
                                                     SdfPath const &cachePath,
                                                     UsdTimeCode time,
    ...
    *extent = _ComputeExtent(prim, _HasVaryingExtent(prim) ? time : UsdTimeCode::EarliestTime());
    ...
    if (cardGeometry == UsdGeomTokens->fromTexture) {
      ...
      *extent = _ComputeExtent(prim, _HasVaryingExtent(prim) ? time : UsdTimeCode::EarliestTime());
      ...
      if (cardGeometry == UsdGeomTokens->cross || cardGeometry == UsdGeomTokens->box) {
        _GenerateCardsGeometry(
            topology, points, *extent, axes_mask, cardGeometry, generateSubsets, prim);
      }
      else {
        TF_CODING_ERROR(
...
/*virtual*/
VtValue UsdImagingDrawModeAdapter::GetTopology(UsdPrim const &prim,
                                               SdfPath const &cachePath,
                                               UsdTimeCode time) const
{
  ...
  VtValue uv;
  GfRange3d extent;
  _ComputeGeometryData(prim, cachePath, time, drawMode, &topology, &points, &extent, &uv);
  return topology;
}

/*virtual*/
GfRange3d UsdImagingDrawModeAdapter::GetExtent(UsdPrim const &prim,
                                               SdfPath const &cachePath,
                                               UsdTimeCode time) const
{
  TRACE_FUNCTION();
...
/*virtual*/
bool UsdImagingDrawModeAdapter::GetDoubleSided(UsdPrim const &prim,
                                               SdfPath const &cachePath,
                                               UsdTimeCode time) const
{
  return false;
...
/*virtual*/
VtValue UsdImagingDrawModeAdapter::Get(UsdPrim const &prim,
                                       SdfPath const &cachePath,
                                       TfToken const &key,
                                       UsdTimeCode time,
...
/*virtual*/
SdfPath UsdImagingDrawModeAdapter::GetMaterialId(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
                                                 UsdTimeCode time) const
{
...
/*virtual*/
VtValue UsdImagingDrawModeAdapter::GetMaterialResource(UsdPrim const &prim,
                                                       SdfPath const &cachePath,
                                                       UsdTimeCode time) const
{
  if (!_IsMaterialPath(cachePath)) {
...
void UsdImagingDrawModeAdapter::_CheckForTextureVariability(UsdPrim const &prim,
...
void UsdImagingDrawModeAdapter::TrackVariability(
    UsdPrim const &prim,
    SdfPath const &cachePath,
...
void UsdImagingDrawModeAdapter::UpdateForTime(
    UsdPrim const &prim,
    SdfPath const &cachePath,
    UsdTimeCode time,
...
HdDirtyBits UsdImagingDrawModeAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                             SdfPath const &cachePath,
...
void UsdImagingDrawModeAdapter::_GenerateOriginGeometry(VtValue *topo,
                                                        VtValue *points,
...
void UsdImagingDrawModeAdapter::_GenerateBoundsGeometry(VtValue *topo,
                                                        VtValue *points,
                                                        GfRange3d const &extents) const
{
...
void UsdImagingDrawModeAdapter::_GenerateCardsGeometry(VtValue *topo,
                                                       VtValue *points,
                                                       GfRange3d const &extents,
                                                       uint8_t axes_mask,
                                                       TfToken cardGeometry,
                                                       bool generateSubsets,
                                                       UsdPrim const &prim) const
{
  ...
  auto generateSubset = [&](AxesMask face) {
    ...
    TfToken material = _GetSubsetMaterialTokenForFace(axes_mask & face ? face :
                                                                         _GetOppositeFace(face));
...
void UsdImagingDrawModeAdapter::_SanityCheckFaceSizes(SdfPath const &cachePath,
                                                      GfRange3d const &extents,
...
void UsdImagingDrawModeAdapter::_GenerateCardsFromTextureGeometry(
    VtValue *topo, VtValue *points, VtValue *uv, GfRange3d *extents, UsdPrim const &prim) const
{
  UsdGeomModelAPI model(prim);
  SdfPath primPath = prim.GetPath();
  if (!model) {
    TF_CODING_ERROR(
        "Prim <%s> has model:cardGeometry = fromTexture,"
        " but GeomModelAPI is not applied!",
        primPath.GetText());
    return;
  }
  ...
  for (size_t i = 0; i < faces.size(); ++i) {
    ...
    TfToken material = _GetSubsetMaterialTokenForFace(AxesMask(faces[i].second));
    if (!subset.IsEmpty() && !material.IsEmpty()) {
      geomSubsets.emplace_back(HdGeomSubset{HdGeomSubset::TypeFaceSet,
                                            SdfPath(subset),
                                            // materialBinding path must be absolute!
...
bool UsdImagingDrawModeAdapter::_GetMatrixFromImageMetadata(UsdAttribute const &attr,
                                                            GfMatrix4d *mat) const
{
  ...
  if (!img->GetMetadata(_tokens->worldtoscreen, &worldtoscreen)) {
    if (img->GetMetadata(_tokens->worldToNDC, &worldtoscreen)) {
      TF_WARN(
          "The texture asset '%s' referenced at <%s> may have been "
      ...
      TF_WARN(
          "The texture asset '%s' referenced at <%s> lacks a "
...
void UsdImagingDrawModeAdapter::_GenerateTextureCoordinates(VtValue *uv, uint8_t axes_mask) const
{
...
GfRange3d UsdImagingDrawModeAdapter::_ComputeExtent(UsdPrim const &prim,
...
/*virtual*/
HdCullStyle UsdImagingDrawModeAdapter::GetCullStyle(UsdPrim const &prim,
                                                    SdfPath const &cachePath,
                                                    UsdTimeCode time) const
{
...
/*virtual*/
GfMatrix4d UsdImagingDrawModeAdapter::GetTransform(UsdPrim const &prim,
                                                   SdfPath const &cachePath,
                                                   UsdTimeCode time,
