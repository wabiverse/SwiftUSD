...
UsdImagingDataSourceTetMeshTopology::UsdImagingDataSourceTetMeshTopology(
    ...
    : _sceneIndexPath(sceneIndexPath), _usdTetMesh(usdTetMesh), _stageGlobals(stageGlobals)
{
}

TfTokenVector UsdImagingDataSourceTetMeshTopology::GetNames()
{
  return {
...
HdDataSourceBaseHandle UsdImagingDataSourceTetMeshTopology::Get(const TfToken &name)
{
  if (name == HdTetMeshTopologySchemaTokens->tetVertexIndices) {
    ...
    return UsdImagingDataSourceAttribute<TfToken>::New(_usdTetMesh.GetOrientationAttr(),
                                                       _stageGlobals);
  }
  return nullptr;
}
...
TfTokenVector UsdImagingDataSourceTetMesh::GetNames()
{
  return {
...
HdDataSourceBaseHandle UsdImagingDataSourceTetMesh::Get(const TfToken &name)
{
  if (name == HdTetMeshSchemaTokens->topology) {
    return UsdImagingDataSourceTetMeshTopology::New(_sceneIndexPath, _usdTetMesh, _stageGlobals);
  }
  if (name == HdTetMeshSchemaTokens->doubleSided) {
    return UsdImagingDataSourceAttribute<bool>::New(_usdTetMesh.GetDoubleSidedAttr(),
                                                    _stageGlobals);
  }
  return nullptr;
}
...
TfTokenVector UsdImagingDataSourceTetMeshPrim::GetNames()
{
...
HdDataSourceBaseHandle UsdImagingDataSourceTetMeshPrim::Get(const TfToken &name)
{
  if (name == HdTetMeshSchemaTokens->tetMesh) {
    return UsdImagingDataSourceTetMesh::New(
        _GetSceneIndexPath(), UsdGeomTetMesh(_GetUsdPrim()), _GetStageGlobals());
  }
  else {
...
/*static*/
HdDataSourceLocatorSet UsdImagingDataSourceTetMeshPrim::Invalidate(
    UsdPrim const &prim,
