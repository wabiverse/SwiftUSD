...
TF_DEFINE_PUBLIC_TOKENS(UsdImaging_NiPrototypeSceneIndexTokens,
                        USDIMAGING_NI_PROTOTYPE_SCENE_INDEX_TOKENS);

namespace {

bool _IsUsdInstance(HdContainerDataSourceHandle const &primSource)
{
...
HdDataSourceBaseHandle _ResetXformToIdentityDataSource()
{
...
const HdContainerDataSourceHandle &_UnderlaySource()
{
...
HdContainerDataSourceHandle _PrototypeRootOverlaySource(const HdContainerDataSourceHandle &ds)
{
  static HdContainerDataSourceHandle const overlayDs = HdRetainedContainerDataSource::New(
      HdInstancedBySchema::GetSchemaToken(),
      UsdImaging_NiPrototypeSceneIndex::GetInstancedByDataSource(),
      // The prototypes should always be defined at the origin.
      HdXformSchema::GetSchemaToken(),
      _ResetXformToIdentityDataSource());
  return HdOverlayContainerDataSource::OverlayedContainerDataSources(overlayDs, ds);
}

}  // namespace

UsdImaging_NiPrototypeSceneIndexRefPtr UsdImaging_NiPrototypeSceneIndex::New(
    HdSceneIndexBaseRefPtr const &inputSceneIndex,
    const bool forNativePrototype,
    HdContainerDataSourceHandle const &prototypeRootOverlayDs)
{
  return TfCreateRefPtr(new UsdImaging_NiPrototypeSceneIndex(
      inputSceneIndex, forNativePrototype, prototypeRootOverlayDs));
}

UsdImaging_NiPrototypeSceneIndex::UsdImaging_NiPrototypeSceneIndex(
    HdSceneIndexBaseRefPtr const &inputSceneIndex,
    const bool forNativePrototype,
    HdContainerDataSourceHandle const &prototypeRootOverlayDs)
    : HdSingleInputFilteringSceneIndexBase(inputSceneIndex),
      _forNativePrototype(forNativePrototype),
      _prototypeRootOverlaySource(_PrototypeRootOverlaySource(prototypeRootOverlayDs))
{
}

/* static */
const SdfPath &UsdImaging_NiPrototypeSceneIndex::GetInstancerPath()
{
  static const SdfPath path = SdfPath::AbsoluteRootPath().AppendChild(
      UsdImaging_NiPrototypeSceneIndexTokens->instancer);
  return path;
}

/* static */
const SdfPath &UsdImaging_NiPrototypeSceneIndex::GetPrototypePath()
{
  static const SdfPath path = GetInstancerPath().AppendChild(
      UsdImaging_NiPrototypeSceneIndexTokens->prototype);
  return path;
}

/* static */
const HdDataSourceBaseHandle &UsdImaging_NiPrototypeSceneIndex::GetInstancedByDataSource()
{
  ...
  static const HdDataSourceBaseHandle &ds =
      HdInstancedBySchema::Builder()
          .SetPaths(DataSource::New({UsdImaging_NiPrototypeSceneIndex::GetInstancerPath()}))
          .SetPrototypeRoots(
              DataSource::New({UsdImaging_NiPrototypeSceneIndex::GetPrototypePath()}))
          .Build();

  return ds;
}

HdSceneIndexPrim UsdImaging_NiPrototypeSceneIndex::GetPrim(const SdfPath &primPath) const
{
  ...
  if (primPath.GetPathElementCount() == n) {
    ...
    prim.dataSource = HdOverlayContainerDataSource::New(_prototypeRootOverlaySource,
                                                        prim.dataSource);
  }
  else {
    // primPath is an ancestor of /UsdNiInstancer/UsdNiPrototype

    prim.dataSource = HdOverlayContainerDataSource::New(prim.dataSource, _UnderlaySource());
  }

  return prim;
}

SdfPathVector UsdImaging_NiPrototypeSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void UsdImaging_NiPrototypeSceneIndex::_PrimsAdded(
    const HdSceneIndexBase &, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  _SendPrimsAdded(entries);
}

void UsdImaging_NiPrototypeSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  _SendPrimsDirtied(entries);
}

void UsdImaging_NiPrototypeSceneIndex::_PrimsRemoved(
