...
class _PrimAdapterAPISchemaAdapter : public UsdImagingAPISchemaAdapter {
 public:
  _PrimAdapterAPISchemaAdapter(const UsdImagingPrimAdapterSharedPtr &primAdapter)
      : _primAdapter(primAdapter)
  {
  }

  TfTokenVector GetImagingSubprims(UsdPrim const &prim,
      ...
      const UsdImagingPropertyInvalidationType invalidationType) override
  {
    ...
    return _primAdapter->InvalidateImagingSubprim(prim, subprim, properties, invalidationType);
  }
...
UsdImaging_AdapterManager::UsdImaging_AdapterManager()
    : _keylessAPISchemaAdapters(
          UsdImagingAdapterRegistry::GetInstance().ConstructKeylessAPISchemaAdapters())
{
}
...
const UsdImaging_AdapterManager::AdaptersEntry &UsdImaging_AdapterManager::LookupAdapters(
    const UsdPrim &prim)
{
  if (!prim) {
...
const UsdImaging_AdapterManager::AdaptersEntry &UsdImaging_AdapterManager::_LookupAdapters(
    const UsdPrimTypeInfo &typeInfo)
{
  ...
  const auto itAndBool = _typeInfoToAdaptersEntry.insert({key, _ComputeAdapters(typeInfo)});

  return itAndBool.first->second;
}

UsdImaging_AdapterManager::AdaptersEntry UsdImaging_AdapterManager::_ComputeAdapters(
    const UsdPrimTypeInfo &typeInfo)
{
  AdaptersEntry result;

  // contains both auto-applied and manually applied schemas
  const TfTokenVector appliedSchemas = typeInfo.GetPrimDefinition().GetAppliedAPISchemas();

  result.allAdapters.reserve(_keylessAPISchemaAdapters.size() + 1 + appliedSchemas.size());

  // first add keyless adapters as they have a stronger opinion than any
  // keyed adapter
  result.allAdapters.insert(result.allAdapters.end(),
                            _keylessAPISchemaAdapters.begin(),
                            _keylessAPISchemaAdapters.end());

  // Then any prim-type schema - using the _BasePrimAdapterAPISchemaAdapter
  // If no prim adapter was registered.
  const _WrappedPrimAdapterEntry &entry = _LookupWrappedPrimAdapter(typeInfo.GetSchemaTypeName());
  ...
  for (const TfToken &schemaToken : appliedSchemas) {
    const std::pair<TfToken, TfToken> tokenPair = UsdSchemaRegistry::GetTypeNameAndInstance(
        schemaToken);
    if (UsdImagingAPISchemaAdapterSharedPtr const a = _LookupAPISchemaAdapter(tokenPair.first)) {
...
const UsdImaging_AdapterManager::_WrappedPrimAdapterEntry &UsdImaging_AdapterManager::
    _LookupWrappedPrimAdapter(const TfToken &primType)
{
  ...
  const auto itAndBool = _primTypeToWrappedPrimAdapterEntry.insert(
      {primType, _ComputeWrappedPrimAdapter(primType)});

  return itAndBool.first->second;
}

UsdImaging_AdapterManager::_WrappedPrimAdapterEntry UsdImaging_AdapterManager::
    _ComputeWrappedPrimAdapter(const TfToken &schemaName)
{
  ...
  if (entry.primAdapter) {
    entry.apiSchemaAdapter = std::make_shared<_PrimAdapterAPISchemaAdapter>(entry.primAdapter);
  }
  else {
...
UsdImagingAPISchemaAdapterSharedPtr UsdImaging_AdapterManager::_LookupAPISchemaAdapter(
    const TfToken &schemaName)
{
  ...
  // Construct and store in cache if not in cache yet.
  UsdImagingAdapterRegistry &reg = UsdImagingAdapterRegistry::GetInstance();
  UsdImagingAPISchemaAdapterSharedPtr const adapter = reg.ConstructAPISchemaAdapter(schemaName);
