...
namespace {
using _PrimSource_0 = UsdImagingDataSourceImplicitsPrim<UsdGeomCylinder, HdCylinderSchema>;
using _PrimSource_1 = UsdImagingDataSourceImplicitsPrim<UsdGeomCylinder_1, HdCylinderSchema>;
}  // namespace
...
TfTokenVector UsdImagingCylinderAdapter::GetImagingSubprims(UsdPrim const &prim)
{
  return {TfToken()};
}

TfToken UsdImagingCylinderAdapter::GetImagingSubprimType(UsdPrim const &prim,
...
HdContainerDataSourceHandle UsdImagingCylinderAdapter::GetImagingSubprimData(
    UsdPrim const &prim,
    TfToken const &subprim,
    const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  if (subprim.IsEmpty()) {
    if (prim.IsA<UsdGeomCylinder>()) {
      return _PrimSource_0::New(prim.GetPath(), prim, stageGlobals);
    }
    else {  // IsA<UsdGeomCylinder_1>()
      return _PrimSource_1::New(prim.GetPath(), prim, stageGlobals);
    }
  }
  return nullptr;
}

HdDataSourceLocatorSet UsdImagingCylinderAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
    ...
    const UsdImagingPropertyInvalidationType invalidationType)
{
  if (subprim.IsEmpty()) {
    if (prim.IsA<UsdGeomCylinder>()) {
      return _PrimSource_0::Invalidate(prim, subprim, properties, invalidationType);
    }
    else {  // IsA<UsdGeomCylinder_1>()
      return _PrimSource_1::Invalidate(prim, subprim, properties, invalidationType);
    }
  }
                                                             ...
                                                             TfToken const &propertyName)
{
  if (propertyName == UsdGeomTokens->height || propertyName == UsdGeomTokens->radius ||
      propertyName == UsdGeomTokens->radiusBottom || propertyName == UsdGeomTokens->radiusTop ||
                         ...
                         double &radiusTopOut)
{
  if (!cylinder.GetRadiusBottomAttr().Get(&radiusBottomOut, time)) {
    TF_WARN(
        "Could not evaluate double-valued bottom radius attribute on "
        "prim %s",
        cylinder.GetPath().GetText());
  }
  if (!cylinder.GetRadiusTopAttr().Get(&radiusTopOut, time)) {
    TF_WARN(
        "Could not evaluate double-valued top radius attribute on "
        "prim %s",
        cylinder.GetPath().GetText());
  }
}
...
VtValue UsdImagingCylinderAdapter::GetPoints(UsdPrim const &prim, UsdTimeCode time) const
{
  ...
  GeomUtilCylinderMeshGenerator::GeneratePoints(
      points.begin(), numRadial, radiusBottom, radiusTop, height, sweepDegrees, &basis);
