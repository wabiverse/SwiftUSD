...
TfTokenVector UsdImagingPointInstancerAdapter::GetImagingSubprims(UsdPrim const &prim)
{
  return {TfToken()};
}

TfToken UsdImagingPointInstancerAdapter::GetImagingSubprimType(UsdPrim const &prim,
...
HdContainerDataSourceHandle UsdImagingPointInstancerAdapter::GetImagingSubprimData(
    UsdPrim const &prim,
    TfToken const &subprim,
    const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  if (subprim.IsEmpty()) {
    return UsdImagingDataSourcePointInstancerPrim::New(prim.GetPath(), prim, stageGlobals);
  }
  return nullptr;
}

HdDataSourceLocatorSet UsdImagingPointInstancerAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
    ...
    UsdImagingInstancerContext const *instancerContext)
{
      ...
      isdigit(instancerContext->childName.GetString()[0]))
  {
    ...
    instancerCachePath = instancerCachePath.AppendVariantSelection("instance",
...
SdfPath UsdImagingPointInstancerAdapter::ResolveCachePath(
    const SdfPath &usdPath, const UsdImagingInstancerContext * /* unused */) const
{
  ...
  while (!treeStack.empty()) {
    ...
    if (adapter) {
      ...
      if (adapter->IsInstancerAdapter()) {
        ...
        ctx = {instancerContext->instancerCachePath,
               instancerContext->childName,
        ...
        TfToken protoName(TfStringPrintf(
            "proto%d_%s_id%d", protoIndex, populatePrim.GetPath().GetName().c_str(), protoID++));

        if (populatePrim.IsPrototype() && TF_VERIFY(instancerChain.size() > 1)) {
      ...
      const SdfPath newProtoPath = adapter->Populate(populatePrim, index, &ctx);
      TF_VERIFY(!newProtoPath.IsEmpty());
      if (!TF_VERIFY(protoPath == newProtoPath,
                     "protoPath changed "
                     "from <%s> to <%s>",
                     protoPath.GetText(),
    ...
    UsdImagingInstancerContext const *instancerContext) const
{
  ...
  if (IsChildPath(cachePath)) {
    ...
    _ProtoPrim &proto = const_cast<_ProtoPrim &>(_GetProtoPrim(instancerPath, cachePath));
    ...
    if (requestedBits & HdChangeTracker::DirtyPrimvar) {
      ...
      if (instancer.UsesOrientationsf(&orientationsAttr)) {
        ...
        if (orientationsAttr.Get(&orientationsf, time)) {
          _MergePrimvar(&vPrimvars, HdInstancerTokens->instanceRotations, HdInterpolationInstance);
        }
      }
      else {
      ...
      if (instancer.GetScalesAttr().Get(&scales, time)) {
        _MergePrimvar(&vPrimvars, HdInstancerTokens->instanceScales, HdInterpolationInstance);
      }

      VtVec3fArray velocities;
      if (instancer.GetVelocitiesAttr().Get(&velocities, time)) {
        _MergePrimvar(&vPrimvars, HdTokens->velocities, HdInterpolationInstance);
      }

      VtVec3fArray accelerations;
      if (instancer.GetAccelerationsAttr().Get(&accelerations, time)) {
        _MergePrimvar(&vPrimvars, HdTokens->accelerations, HdInterpolationInstance);
      }

      VtVec3fArray angularVelocities;
      if (instancer.GetAngularVelocitiesAttr().Get(&angularVelocities, time)) {
        _MergePrimvar(&vPrimvars, HdTokens->angularVelocities, HdInterpolationInstance);
      }
      ...
      for (auto const &pv : primvars) {
        ...
        if (usdInterp == UsdGeomTokens->constant || usdInterp == UsdGeomTokens->uniform) {
                                                                   ...
                                                                   TfToken const &propertyName)
{
  ...
  if (propertyName == UsdGeomTokens->positions || propertyName == UsdGeomTokens->orientations ||
      propertyName == UsdGeomTokens->orientationsf || propertyName == UsdGeomTokens->scales ||
      propertyName == UsdGeomTokens->velocities || propertyName == UsdGeomTokens->accelerations ||
...
void UsdImagingPointInstancerAdapter::MarkLightParamsDirty(const UsdPrim &prim,
                                                           ...
                                                           UsdImagingIndexProxy *index)
{
  if (IsChildPath(cachePath)) {
    ...
    static const HdDirtyBits lightParamsDirty = HdChangeTracker::DirtyParams;
    index->MarkInstancerDirty(cachePath, lightParamsDirty);
  }
}

void UsdImagingPointInstancerAdapter::MarkCollectionsDirty(const UsdPrim &prim,
                                                           const SdfPath &cachePath,
                                                           UsdImagingIndexProxy *index)
{
  if (IsChildPath(cachePath)) {
    ...
    static const HdDirtyBits collectionsDirty = HdChangeTracker::DirtyCategories;
...
UsdImagingPointInstancerAdapter::_ProtoPrim const &UsdImagingPointInstancerAdapter::_GetProtoPrim(
    SdfPath const &instrPath, SdfPath const &cachePath) const
{
...
SdfPath UsdImagingPointInstancerAdapter::GetScenePrimPath(
    SdfPath const &cachePath, int instanceIndex, HdInstancerContext *instancerContext) const
{
  HD_TRACE_FUNCTION();
...
SdfPathVector UsdImagingPointInstancerAdapter::GetInstancerPrototypes(
    UsdPrim const &usdPrim, SdfPath const &cachePath) const
{
  HD_TRACE_FUNCTION();
...
bool UsdImagingPointInstancerAdapter::IsChildPath(const SdfPath &path) const
{
  TRACE_FUNCTION();
...
/*virtual*/
VtValue UsdImagingPointInstancerAdapter::GetLightParamValue(const UsdPrim &prim,
                                                            const SdfPath &cachePath,
                                                            ...
                                                            UsdTimeCode time) const
{
  if (IsChildPath(cachePath)) {
    ...
    const _ProtoPrim proto = _GetProtoPrim(instancerPath, cachePath);
    UsdPrim protoPrim = _GetProtoUsdPrim(proto);
    return proto.adapter->GetLightParamValue(protoPrim, cachePath, paramName, time);
  }
  return BaseAdapter::GetLightParamValue(prim, cachePath, paramName, time);
}

/*virtual*/
VtValue UsdImagingPointInstancerAdapter::GetMaterialResource(const UsdPrim &prim,
                                                             const SdfPath &cachePath,
                                                             UsdTimeCode time) const
{
  if (IsChildPath(cachePath)) {
                                             ...
                                             VtIntArray *outIndices) const
{
  ...
  if (IsChildPath(cachePath)) {
    ...
    else if (UsdGeomPrimvar pv = UsdGeomPrimvarsAPI(usdPrim).GetPrimvar(key)) {
      VtValue value;
      if (outIndices) {
  ...
  if (IsChildPath(cachePath)) {
    ...
    if (usdPrim.GetPath() == cachePath.GetAbsoluteRootOrPrimPath() && hydraInstanceIndex != -1) {
      ...
      for (auto const &pair : instrData->protoPrimMap) {
        ...
        UsdPrim selectionPrim = _GetPrim(pair.first.GetParentPath());
    ...
    for (auto const &pair : instrData->protoPrimMap) {
      ...
      if (selectionCount == selectionPathVec.size()) {
        ...
        selectionPrim = _GetPrim(pair.first.GetParentPath());
      }
...
GfMatrix4d UsdImagingPointInstancerAdapter::GetRelativeInstancerTransform(
    SdfPath const &parentInstancerCachePath, SdfPath const &cachePath, UsdTimeCode time) const
{
  const UsdPrim &target = _GetPrim(cachePath.GetPrimPath());
  const GfMatrix4d targetXf = BaseAdapter::GetTransform(target, cachePath, time);
  if (parentInstancerCachePath.IsEmpty()) {
  ...
  const GfMatrix4d protoRootParentXf = BaseAdapter::GetTransform(
      protoRootParent, protoRootParentPath, time);
