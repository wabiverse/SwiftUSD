...
TF_DEFINE_PUBLIC_TOKENS(UsdImagingStageSceneIndexTokens, USDIMAGING_STAGE_SCENE_INDEX_TOKENS);
...
bool _Contains(const TfTokenVector &vec, const TfToken &t)
{
  return std::find(vec.begin(), vec.end(), t) != vec.end();
}

bool _Contains(const TfDenseHashSet<TfToken, TfHash> &s, const TfToken &t)
{
  return s.find(t) != s.end();
}

TfTokenVector _GetImagingSubprims(UsdPrim const &prim, const AdapterEntries &entries)
{
  switch (entries.size()) {
    case 0: {
    ...
    default: {
      ...
      for (const AdapterEntry &entry : entries) {
        ...
        for (const TfToken &subprim :
             entry.adapter->GetImagingSubprims(prim, entry.appliedInstanceName))
        {
          if (subprim.IsEmpty()) {
            continue;
          }
...
TfToken _GetImagingSubprimType(const AdapterEntries &entries,
                               UsdPrim const &prim,
                               const TfToken &subprim)
{
  // strongest non-empty opinion wins
  for (const AdapterEntry &entry : entries) {
    const TfToken result = entry.adapter->GetImagingSubprimType(
    ...
    const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  ...
  for (const AdapterEntry &entry : entries) {
    if (HdContainerDataSourceHandle ds = entry.adapter->GetImagingSubprimData(
  ...
  return HdOverlayContainerDataSource::New(containers.size(), containers.data());
}
    ...
    const UsdImagingPropertyInvalidationType invalidationType)
{
  ...
  if (entries.size() == 1) {
    return entries[0].adapter->InvalidateImagingSubprim(
        prim, subprim, entries[0].appliedInstanceName, properties, invalidationType);
  }

  HdDataSourceLocatorSet result;

  for (const AdapterEntry &entry : entries) {
    result.insert(entry.adapter->InvalidateImagingSubprim(
        prim, subprim, entry.appliedInstanceName, properties, invalidationType));
  }

  return result;
}

bool _GetIncludeUnloadedPrims(HdContainerDataSourceHandle const &inputArgs)
{
  if (!inputArgs) {
    return false;
  }
  HdBoolDataSourceHandle const ds = HdBoolDataSource::Cast(
      inputArgs->Get(UsdImagingStageSceneIndexTokens->includeUnloadedPrims));
  if (!ds) {
    return false;
  }
...
UsdImagingStageSceneIndex::UsdImagingStageSceneIndex(HdContainerDataSourceHandle const &inputArgs)
...
HdSceneIndexPrim UsdImagingStageSceneIndex::GetPrim(const SdfPath &path) const
{
  ...
  const AdapterEntries &entries = _adapterManager->LookupAdapters(prim).allAdapters;

  return {_GetImagingSubprimType(entries, prim, subprim),
...
SdfPathVector UsdImagingStageSceneIndex::GetChildPrimPaths(const SdfPath &path) const
{
  ...
  const UsdImaging_AdapterManager::AdaptersEntry &entry = _adapterManager->LookupAdapters(prim);

  if (!(entry.primAdapter && entry.primAdapter->GetPopulationMode() ==
                                 UsdImagingPrimAdapter::RepresentsSelfAndDescendents))
  {
    UsdPrimSiblingRange range = prim.GetFilteredChildren(_GetPrimPredicate());
  ...
  for (const TfToken &subprim : _GetImagingSubprims(prim, entry.allAdapters)) {
    if (!subprim.IsEmpty()) {
...
void UsdImagingStageSceneIndex::SetTime(UsdTimeCode time, const bool forceDirtyingTimeDeps)
{
  TRACE_FUNCTION();
...
void UsdImagingStageSceneIndex::SetStage(UsdStageRefPtr stage)
{
  ...
  if (_stage) {
    _objectsChangedNoticeKey = TfNotice::Register(
        TfCreateWeakPtr(this), &UsdImagingStageSceneIndex::_OnUsdObjectsChanged, _stage);
  }
...
void UsdImagingStageSceneIndex::_PopulateSubtree(UsdPrim subtreeRoot)
{
  ...
  for (auto it = range.begin(); it != range.end(); ++it) {
    ...
    const SdfPath primPath = prim.GetPath();
    const TfTokenVector subprims = _GetImagingSubprims(prim, entry.allAdapters);

    for (TfToken const &subprim : subprims) {
      const SdfPath subpath = subprim.IsEmpty() ? primPath : primPath.AppendProperty(subprim);

      addedPrims.emplace_back(subpath, _GetImagingSubprimType(entry.allAdapters, prim, subprim));
    }
...
Usd_PrimFlagsPredicate UsdImagingStageSceneIndex::_GetPrimPredicate() const
{
  ...
  static const Usd_PrimFlagsConjunction commonFlags = UsdPrimIsActive && !UsdPrimIsAbstract;
...
void UsdImagingStageSceneIndex::_OnUsdObjectsChanged(UsdNotice::ObjectsChanged const &notice,
                                                     UsdStageWeakPtr const &sender)
{
  ...
  TF_DEBUG(USDIMAGING_CHANGES)
      .Msg(
          "[Objects Changed] Notice received "
  ...
  const UsdNotice::ObjectsChanged::PathRange pathsToResync = notice.GetResyncedPaths();
  for (auto it = pathsToResync.begin(); it != pathsToResync.end(); ++it) {
    if (it->IsAbsoluteRootOrPrimPath()) {
      ...
      _usdPropertiesToResync[it->GetPrimPath()].push_back(it->GetNameToken());
      TF_DEBUG(USDIMAGING_CHANGES).Msg(" - Property resync queued: %s\n", it->GetText());
    }
  ...
  const UsdNotice::ObjectsChanged::PathRange pathsToUpdate = notice.GetChangedInfoOnlyPaths();
    ...
    if (it->IsAbsoluteRootOrPrimPath()) {
      ...
      for (const TfToken &field : changedFields) {
        ...
        if (fieldDef && fieldDef->IsPlugin()) {
          _usdPrimsToResync.push_back(*it);
          TF_DEBUG(USDIMAGING_CHANGES)
              .Msg(" - Resync due to prim update queued: %s\n", it->GetText());
          break;
        }
      }
    }
    else if (it->IsPropertyPath()) {
      _usdPropertiesToUpdate[it->GetPrimPath()].push_back(it->GetNameToken());
...
UsdImagingStageSceneIndex::_PrimAdapterPair UsdImagingStageSceneIndex::_FindResponsibleAncestor(
    const UsdPrim &prim) const
{
  ...
  while (parentPrim) {
    ...
    if (primAdapter &&
        primAdapter->GetPopulationMode() == UsdImagingPrimAdapter::RepresentsSelfAndDescendents)
    {
...
static void _DeletePrefix(const SdfPath &prefix, std::map<SdfPath, TfTokenVector> *const m)
{
...
void UsdImagingStageSceneIndex::_ApplyPendingResyncs()
{
  ...
  for (size_t i = 0; i < _usdPrimsToResync.size(); ++i) {
    ...
    if (primAdapter &&
        primAdapter->GetPopulationMode() == UsdImagingPrimAdapter::RepresentedByAncestor)
    {
      _PrimAdapterPair ancestor = _FindResponsibleAncestor(prim);
      if (ancestor.second) {
        TF_DEBUG(USDIMAGING_CHANGES)
            .Msg("Invalidating <%s> due to resync of descendant <%s>\n",
...
void UsdImagingStageSceneIndex::ApplyPendingUpdates()
{
  if (!_stage) {
    return;
  }

  if (_usdPropertiesToUpdate.empty() && _usdPropertiesToResync.empty() &&
...
void UsdImagingStageSceneIndex::_ComputeDirtiedEntries(
    ...
    HdSceneIndexObserver::DirtiedPrimEntries *const dirtiedPrims) const
{
  for (auto const &pair : pathToUsdProperties) {
    ...
    if (entry.primAdapter &&
        entry.primAdapter->GetPopulationMode() == UsdImagingPrimAdapter::RepresentedByAncestor)
    {
      ...
      if (ancestor.second) {
        ...
        for (const TfToken &subprim : parentAdapter->GetImagingSubprims(parentPrim)) {

          HdDataSourceLocatorSet dirtyLocators =
              parentAdapter->InvalidateImagingSubprimFromDescendent(
                  parentPrim, prim, subprim, properties, invalidationType);
    ...
    for (TfToken const &subprim : subprims) {
      const HdDataSourceLocatorSet dirtyLocators = _InvalidateImagingSubprim(
          entry.allAdapters, prim, subprim, properties, invalidationType);
      ...
      if (!dirtyLocators.IsEmpty()) {
        ...
        if (dirtyLocators.Contains(repopulateLocator)) {
          ...
          SdfPath const subpath = subprim.IsEmpty() ? primPath : primPath.AppendProperty(subprim);
...
void UsdImagingStageSceneIndex::_StageGlobals::FlagAsTimeVarying(
    const SdfPath &hydraPath, const HdDataSourceLocator &locator) const
{
...
void UsdImagingStageSceneIndex::_StageGlobals::SetTime(
    UsdTimeCode time, HdSceneIndexObserver::DirtiedPrimEntries *dirtied)
{
  TRACE_FUNCTION();
...
void UsdImagingStageSceneIndex::_StageGlobals::RemoveAssetPathDependentsUnder(const SdfPath &path)
{
  TRACE_FUNCTION();
...
void UsdImagingStageSceneIndex::_StageGlobals::InvalidateAssetPathDependentsUnder(
    const SdfPath &path,
    ...
    std::map<SdfPath, TfTokenVector> *propertiesToInvalidate) const
{
  ...
  for (auto it = beginAndEndPair.first; it != beginAndEndPair.second; ++it) {
    ...
    if (depPath.IsAbsoluteRootOrPrimPath()) {
      ...
      TF_DEBUG(USDIMAGING_CHANGES)
          .Msg(" - Resync due to asset path resync queued: %s\n", depPath.GetText());
    }
    else if (depPath.IsPrimPropertyPath()) {
      (*propertiesToInvalidate)[depPath.GetPrimPath()].push_back(depPath.GetNameToken());

      TF_DEBUG(USDIMAGING_CHANGES)
          .Msg(" - Property update due to asset path resync queued: %s\n", depPath.GetText());
    }
  }
}
