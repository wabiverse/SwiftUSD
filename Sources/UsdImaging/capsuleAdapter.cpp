...
TfTokenVector UsdImagingCapsuleAdapter::GetImagingSubprims(UsdPrim const &prim)
{
  return {TfToken()};
}

TfToken UsdImagingCapsuleAdapter::GetImagingSubprimType(UsdPrim const &prim,
...
HdContainerDataSourceHandle UsdImagingCapsuleAdapter::GetImagingSubprimData(
    UsdPrim const &prim,
    TfToken const &subprim,
...
HdDataSourceLocatorSet UsdImagingCapsuleAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
...
static void extractRadii(UsdGeomCapsule const &capsule,
                         UsdTimeCode time,
...
static void extractRadii(UsdGeomCapsule_1 const &capsule,
                         UsdTimeCode time,
                         double &radiusBottomOut,
                         double &radiusTopOut)
{
...
static void extractCapsuleParameters(UsdPrim const &prim,
                                     UsdTimeCode time,
                                     double &heightOut,
                                     double &radiusBottomOut,
                                     double &radiusTopOut,
...
VtValue UsdImagingCapsuleAdapter::GetPoints(UsdPrim const &prim, UsdTimeCode time) const
{
  ...
  const size_t numPoints = GeomUtilCapsuleMeshGenerator::ComputeNumPoints(numRadial, numCapAxial);

  VtVec3fArray points(numPoints);
  const double sweep = 360;
  GeomUtilCapsuleMeshGenerator::GeneratePoints(
      points.begin(), numRadial, numCapAxial, radiusBottom, radiusTop, height, sweep, &basis);
