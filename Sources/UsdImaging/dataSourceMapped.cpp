...
using _DataSourceInfo =
    std::variant<UsdImagingDataSourceMapped::AttributeMapping, _ContainerMappingsSharedPtr>;
...
UsdImagingDataSourceMapped::UsdImagingDataSourceMapped(
    ...
    : UsdImagingDataSourceMapped(
          usdPrim, sceneIndexPath, mappings._containerMappings, stageGlobals)
{
}
...
TfTokenVector UsdImagingDataSourceMapped::GetNames()
{
  return _containerMappings->hdNames;
}

HdDataSourceBaseHandle UsdImagingDataSourceMapped::Get(const TfToken &name)
{
  // Look for name in hdNames.

  const auto itName = std::lower_bound(
      _containerMappings->hdNames.begin(), _containerMappings->hdNames.end(), name);

  if (!(itName != _containerMappings->hdNames.end() && *itName == name)) {
    return nullptr;
  }

  const auto itInfo = _containerMappings->dataSourceInfos.begin() +
  ...
  if (auto const mapping = std::get_if<AttributeMapping>(&info)) {
    ...
    return mapping->factory(attr, _stageGlobals, _sceneIndexPath, mapping->hdLocator);
  }
  else {
    // We are in the nested case.
    return UsdImagingDataSourceMapped::New(
        _usdPrim, _sceneIndexPath, std::get<_ContainerMappingsSharedPtr>(info), _stageGlobals);
  }
}

HdDataSourceLocatorSet UsdImagingDataSourceMapped::Invalidate(const TfTokenVector &usdNames,
                                                              const AttributeMappings &mappings)
{
...
_ContainerMappingsSharedPtr _FindOrCreateChild(
    const TfToken &name, _ContainerMappingsSharedPtr const &containerMappings)
{
  const auto itName = std::lower_bound(
      containerMappings->hdNames.begin(), containerMappings->hdNames.end(), name);
  const auto itInfo = containerMappings->dataSourceInfos.begin() +
  ...
  if (itName != containerMappings->hdNames.end() && *itName == name) {
    ...
    if (!child) {
      const auto &mapping = std::get<UsdImagingDataSourceMapped::AttributeMapping>(info);
      TF_CODING_ERROR(
...
void _Add(const HdDataSourceLocator &locator,
          ...
          _ContainerMappingsSharedPtr containerMappings)
{
  ...
  if (n == 0) {
    TF_CODING_ERROR(
        "Expected non-trivial data source locator for "
        "attribute %s.",
        mapping.usdName.GetText());
    return;
  }
...
UsdImagingDataSourceMapped::AttributeMappings::AttributeMappings(
    const std::vector<AttributeMapping> &mappings, const HdDataSourceLocator &dataSourcePrefix)
{
  ...
  for (const AttributeMapping &mapping : mappings) {
    ...
    const AttributeMapping absoluteMapping{
        mapping.usdName, dataSourcePrefix.Append(mapping.hdLocator), mapping.factory};
