...
TF_DEFINE_PRIVATE_TOKENS(_tokens, ((propagatedPrototypesScope, "UsdNiPropagatedPrototypes")));

namespace UsdImaging_NiInstanceAggregationSceneIndex_Impl {

// Gets primvars from prim at given path in scene index.
HdPrimvarsSchema _GetPrimvarsSchema(HdSceneIndexBaseRefPtr const &sceneIndex,
                                    const SdfPath &primPath)
{
  if (!sceneIndex) {
    return HdPrimvarsSchema(nullptr);
  }
  return HdPrimvarsSchema::GetFromParent(sceneIndex->GetPrim(primPath).dataSource);
}

// Gets primvar from prim at given path with given name in scene index.
HdPrimvarSchema _GetPrimvarSchema(HdSceneIndexBaseRefPtr const &sceneIndex,
                                  const SdfPath &primPath,
                                  const TfToken &primvarName)
{
  return _GetPrimvarsSchema(sceneIndex, primPath).GetPrimvar(primvarName);
}
...
bool _IsConstantPrimvar(HdSceneIndexBaseRefPtr const &sceneIndex,
                        const SdfPath &primPath,
                        const TfToken &primvarName)
{
...
TfTokenVector _GetConstantPrimvarNames(HdSceneIndexBaseRefPtr const &sceneIndex,
                                       const SdfPath &primPath)
{
  ...
  for (const TfToken &name : primvarsSchema.GetPrimvarNames()) {
    ...
    if (HdTokenDataSourceHandle const interpolationSrc = primvarSchema.GetInterpolation()) {
      const TfToken interpolation = interpolationSrc->GetTypedValue(0.0f);
...
VtValue _GetPrimvarValue(HdSceneIndexBaseRefPtr const &sceneIndex,
                         const SdfPath &primPath,
                         const TfToken &primvarName)
{
  HdSampledDataSourceHandle const ds =
      _GetPrimvarSchema(sceneIndex, primPath, primvarName).GetPrimvarValue();
  if (!ds) {
    return {};
  }
...
template<typename T>
T _GetTypedPrimvarValue(HdSceneIndexBaseRefPtr const &sceneIndex,
                        const SdfPath &primPath,
                        const TfToken &primvarName)
{
  const VtValue value = _GetPrimvarValue(sceneIndex, primPath, primvarName);
...
GfMatrix4d _GetPrimTransform(HdSceneIndexBaseRefPtr const &sceneIndex, const SdfPath &primPath)
{
  ...
  HdMatrixDataSourceHandle const ds = HdXformSchema::GetFromParent(primSource).GetMatrix();
  if (!ds) {
    return id;
  }
...
//
template<typename T> class _PrimvarValueDataSource : public HdTypedSampledDataSource<VtArray<T>> {
 public:
  ...
  VtArray<T> GetTypedValue(const HdSampledDataSource::Time shutterOffset) override
  {
    VtArray<T> result(_instances->size());

    int i = 0;
    for (const SdfPath &instance : *_instances) {
      result[i] = _GetTypedPrimvarValue<T>(_inputSceneIndex, instance, _primvarName);
      i++;
    }
    return result;
  }
 ...
 private:
  _PrimvarValueDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                          std::shared_ptr<SdfPathSet> const &instances,
                          const TfToken &primvarName)
      : _inputSceneIndex(inputSceneIndex), _instances(instances), _primvarName(primvarName)
  {
  }
...
class _PrimvarValueDataSourceFactory {
 public:
  template<class T> HdDataSourceBaseHandle operator()(const T &v) const
  {
    return _PrimvarValueDataSource<T>::New(_inputSceneIndex, _instances, _primvarName);
  }

  template<class T> HdDataSourceBaseHandle operator()(const VtArray<T> &array) const
  {
    return _PrimvarValueDataSource<T>::New(_inputSceneIndex, _instances, _primvarName);
  }
  ...
  _PrimvarValueDataSourceFactory(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                                 std::shared_ptr<SdfPathSet> const &instances,
                                 const TfToken &primvarName)
      : _inputSceneIndex(inputSceneIndex), _instances(instances), _primvarName(primvarName)
  {
...
HdDataSourceBaseHandle _MakePrimvarValueDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                                                   std::shared_ptr<SdfPathSet> const &instances,
                                                   const TfToken &primvarName)
{
  if (instances->empty()) {
  ...
  const VtValue value = _GetPrimvarValue(inputSceneIndex, primPath, primvarName);

  return VtVisitValue(value,
                      _PrimvarValueDataSourceFactory(inputSceneIndex, instances, primvarName));
}
 ...
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdPrimvarSchemaTokens->primvarValue) {
      return _MakePrimvarValueDataSource(_inputSceneIndex, _instances, _primvarName);
    }
    ...
    if (name == HdPrimvarSchemaTokens->role) {
      ...
      return _GetPrimvarSchema(_inputSceneIndex, primPath, name).GetRole();
    }
    return nullptr;
  }

 private:
  _PrimvarDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                     std::shared_ptr<SdfPathSet> const &instances,
                     const TfToken &primvarName)
      : _inputSceneIndex(inputSceneIndex), _instances(instances), _primvarName(primvarName)
  {
...
class _InstanceTransformPrimvarValueDataSource : public HdMatrixArrayDataSource {
 ...
 private:
  _InstanceTransformPrimvarValueDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                                           std::shared_ptr<SdfPathSet> const &instances)
      : _inputSceneIndex(inputSceneIndex), _instances(instances)
  {
  }
...
class _InstanceTransformPrimvarDataSource : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdPrimvarSchemaTokens->primvarValue) {
      return _InstanceTransformPrimvarValueDataSource::New(_inputSceneIndex, _instances);
    }
 ...
 private:
  _InstanceTransformPrimvarDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                                      std::shared_ptr<SdfPathSet> const &instances)
      : _inputSceneIndex(inputSceneIndex), _instances(instances)
  {
  }
...
class _PrimvarsDataSource : public HdContainerDataSource {
 public:
  ...
  TfTokenVector GetNames() override
  {
    TfTokenVector result;
    if (!_instances->empty()) {
      result = _GetConstantPrimvarNames(_inputSceneIndex, *(_instances->begin()));
    }
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdInstancerTokens->instanceTransforms) {
      return _InstanceTransformPrimvarDataSource::New(_inputSceneIndex, _instances);
    }
    if (!_instances->empty()) {
      if (_IsConstantPrimvar(_inputSceneIndex, *(_instances->begin()), name)) {
        return _PrimvarDataSource::New(_inputSceneIndex, _instances, name);
      }
    }
    return nullptr;
  }
 ...
 private:
  _PrimvarsDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                      std::shared_ptr<SdfPathSet> const &instances)
      : _inputSceneIndex(inputSceneIndex), _instances(instances)
  {
  }
...
// [0, 1, ..., n-1]
VtArray<int> _Range(const int n)
{
...
class _InstanceIndicesDataSource : public HdVectorDataSource {
 ...
 private:
  _InstanceIndicesDataSource(std::shared_ptr<SdfPathSet> const &instances) : _instances(instances)
  {
  }
...
class _InstanceLocationsDataSource : public HdPathArrayDataSource {
 ...
 private:
  _InstanceLocationsDataSource(std::shared_ptr<SdfPathSet> const &instances)
...
bool _GetVisibility(HdSceneIndexBaseRefPtr const &sceneIndex, const SdfPath &primPath)
{
  if (!sceneIndex) {
    return true;
  }

  HdContainerDataSourceHandle const primDs = sceneIndex->GetPrim(primPath).dataSource;
  HdBoolDataSourceHandle const ds = HdVisibilitySchema::GetFromParent(primDs).GetVisibility();
  if (!ds) {
    return true;
  }
  return ds->GetTypedValue(0.0f);
...
VtBoolArray _ComputeMask(HdSceneIndexBaseRefPtr const &sceneIndex,
                         std::shared_ptr<SdfPathSet> const &instances)
{
...
class _InstancerTopologyDataSource : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdInstancerTopologySchemaTokens->prototypes) {
      return HdRetainedTypedSampledDataSource<VtArray<SdfPath>>::New({_prototypePath});
    }
 ...
 private:
  _InstancerTopologyDataSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                               const SdfPath &prototypePath,
                               std::shared_ptr<SdfPathSet> const &instances)
      : _inputSceneIndex(inputSceneIndex), _prototypePath(prototypePath), _instances(instances)
  {
  }
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdInstancedBySchema::GetSchemaToken()) {
      ...
      if (HdInstancedBySchema schema = HdInstancedBySchema::GetFromParent(
              _inputSceneIndex->GetPrim(_enclosingPrototypeRoot).dataSource))
      {
      ...
      if (_forNativePrototype) {
        ...
        return UsdImaging_NiPrototypeSceneIndex::GetInstancedByDataSource();
      }
      return nullptr;
    }
    if (name == HdInstancerTopologySchema::GetSchemaToken()) {
      return _InstancerTopologyDataSource::New(_inputSceneIndex, _prototypePath, _instances);
    }
    if (name == HdPrimvarsSchema::GetSchemaToken()) {
      return _PrimvarsDataSource::New(_inputSceneIndex, _instances);
    }
    return nullptr;
  }

 private:
  _InstancerPrimSource(HdSceneIndexBaseRefPtr const &inputSceneIndex,
...
std::string _ComputeConstantPrimvarsRoleHash(HdPrimvarsSchema primvarsSchema)
{
  ...
  for (const TfToken &name : primvarsSchema.GetPrimvarNames()) {
    HdPrimvarSchema primvarSchema = primvarsSchema.GetPrimvar(name);
    if (HdTokenDataSourceHandle const interpolationSrc = primvarSchema.GetInterpolation()) {
      if (interpolationSrc->GetTypedValue(0.0f) == HdPrimvarSchemaTokens->constant) {
        TfToken role;
        if (HdTokenDataSourceHandle const roleSrc = primvarSchema.GetRole()) {
...
//
TfToken _ComputeBindingHash(HdContainerDataSourceHandle const &primSource,
                            const TfTokenVector &instanceDataSourceNames)
{
  std::string result = _ComputeConstantPrimvarsRoleHash(
  ...
  for (const TfToken &name : instanceDataSourceNames) {
    if (HdDataSourceBaseHandle const ds = primSource->Get(name)) {
      result += TfStringPrintf("_%s%zx", name.GetText(), HdDataSourceHash(ds, 0.0f, 0.0f));
    }
  }
...
SdfPath _GetUsdPrototypePath(HdContainerDataSourceHandle const &primSource)
{
...
TfToken _GetUsdPrototypeName(HdContainerDataSourceHandle const &primSource)
{
...
SdfPath _GetPrototypeRoot(HdContainerDataSourceHandle const &primSource)
{
  HdInstancedBySchema schema = HdInstancedBySchema::GetFromParent(primSource);
...
HdContainerDataSourceHandle _MakeBindingCopy(HdContainerDataSourceHandle const &primSource,
                                             const TfTokenVector &instanceDataSourceNames)
{
  TfTokenVector names;
  ...
  return HdRetainedContainerDataSource::New(names.size(), names.data(), dataSources.data());
}
...
struct _InstanceInfo {
  ...
  {
    return enclosingPrototypeRoot.AppendChild(_tokens->propagatedPrototypesScope)
  ...
  {
    return GetBindingPrimPath().AppendChild(prototypeName);
  }

  // A path like
  // /MyPiPrototype/UsdNiPropagtedPrototypes/Binding312...436/__Prototype_1/UsdNiInstancer that is
  // where the instancer actually is.
  SdfPath GetInstancerPath() const
  {
    return GetPropagatedPrototypeBase().AppendChild(
        UsdImaging_NiPrototypeSceneIndexTokens->instancer);
  }

  // A path like
  // /MyPiPrototype/UsdNiPropagtedPrototypes/Binding312...436/__Prototype_1/UsdNiInstancer/UsdNiPrototype
  // the path where the propagating scene index needs to insert a copy
  // of the USD prototype.
  SdfPath GetPrototypePath() const
  {
    return GetInstancerPath().AppendChild(UsdImaging_NiPrototypeSceneIndexTokens->prototype);
  }
};
...
class _InstanceObserver : public HdSceneIndexObserver {
 ...
 private:
  ...
  void _AddInstance(const SdfPath &primPath, const _InstanceInfo &info);
  void _RemovePrim(const SdfPath &primPath);
  _PathToInstanceInfo::iterator _RemoveInstance(const SdfPath &primPath,
                                                const _PathToInstanceInfo::iterator &it);
  void _ResyncPrim(const SdfPath &primPath);
  void _DirtyInstancerForInstance(const SdfPath &instance, const HdDataSourceLocatorSet &locators);
  ...
  _RemovalLevel _RemoveInstanceFromInfoToInstance(const SdfPath &primPath,
  ...
  void _DirtyInstancesAndResetPointer(_PathToIntSharedPtr *const instanceToIndex);

  // Get prim data source for the named USD instance.
  HdContainerDataSourceHandle _GetDataSourceForInstance(const SdfPath &primPath);

  // Get data source for instance data source locator for
  // an instance.
  HdContainerDataSourceHandle _GetInstanceSchemaDataSource(const SdfPath &primPath);
  ...
  int _GetInstanceIndex(const _InstanceInfo &info, const SdfPath &instancePath);

  // Given instance info identifying an instancer, get
  // the instance to instance id map. That is, compute
  // it if necessary.
  _PathToIntSharedPtr _GetInstanceToIndex(const _InstanceInfo &info);

  // Given instance info identifying an instancer, compute
  // the instance to instance id map.
  _PathToIntSharedPtr _ComputeInstanceToIndex(const _InstanceInfo &info);
...
static HdDataSourceLocatorSet _ComputeResyncLocators(const TfTokenVector &instanceDataSourceNames)
{
  HdDataSourceLocatorSet result;
  // The enclosing scope of the native instance might have changed.
  result.insert(
      HdInstancedBySchema::GetDefaultLocator().Append(HdInstancedBySchemaTokens->prototypeRoots));
...
_InstanceObserver::_InstanceObserver(HdSceneIndexBaseRefPtr const &inputScene,
                                     const bool forNativePrototype,
                                     const TfTokenVector &instanceDataSourceNames)
...
void _InstanceObserver::PrimsAdded(const HdSceneIndexBase &sender, const AddedPrimEntries &entries)
{
...
void _GetPrimvarValueLocatorsAndNeedsResyncFlag(const HdDataSourceLocatorSet &locators,
...
void _InstanceObserver::PrimsDirtied(const HdSceneIndexBase &sender,
                                     const DirtiedPrimEntries &entries)
{
  ...
  for (const DirtiedPrimEntry &entry : entries) {
    ...
    {
      static const HdDataSourceLocatorSet xformLocators{HdXformSchema::GetDefaultLocator()};
...

    {
      ...
      _GetPrimvarValueLocatorsAndNeedsResyncFlag(locators, &primvarValueLocators, &needsResync);
      if (needsResync) {
...
void _InstanceObserver::PrimsRemoved(const HdSceneIndexBase &sender,
                                     const RemovedPrimEntries &entries)
{
  ...
  for (const RemovedPrimEntry &entry : entries) {
    ...
    while (it != _instanceToInfo.end() && it->first.HasPrefix(path)) {
      it = _RemoveInstance(path, it);
    }
  }
}

void _InstanceObserver::PrimsRenamed(const HdSceneIndexBase &sender,
                                     const RenamedPrimEntries &entries)
{
  ConvertPrimsRenamedToRemovedAndAdded(sender, entries, this);
}

void _InstanceObserver::_Populate()
{
  for (const SdfPath &primPath : HdSceneIndexPrimView(_inputScene, SdfPath::AbsoluteRootPath())) {
    _AddPrim(primPath);
  }
}

_InstanceInfo _InstanceObserver::_GetInfo(const HdContainerDataSourceHandle &primSource)
{
  ...
  if (result.enclosingPrototypeRoot.IsEmpty()) {
    if (_forNativePrototype) {
      result.enclosingPrototypeRoot = UsdImaging_NiPrototypeSceneIndex::GetPrototypePath();
    }
    else {
      result.enclosingPrototypeRoot = SdfPath::AbsoluteRootPath();
    }
  }
  result.bindingHash = _ComputeBindingHash(primSource, _instanceDataSourceNames);

  return result;
}

_InstanceInfo _InstanceObserver::_GetInfo(const SdfPath &primPath)
{
  return _GetInfo(_inputScene->GetPrim(primPath).dataSource);
}

void _InstanceObserver::_AddInstance(const SdfPath &primPath, const _InstanceInfo &info)
{
  _Map1 &bindingHashToPrototypeNameToInstances = _infoToInstance[info.enclosingPrototypeRoot];

  _Map0 &prototypeNameToInstances = bindingHashToPrototypeNameToInstances[info.bindingHash];

  if (prototypeNameToInstances.empty()) {
    _retainedSceneIndex->AddPrims(
        {{info.GetBindingPrimPath(),
          TfToken(),
          _MakeBindingCopy(_inputScene->GetPrim(primPath).dataSource, _instanceDataSourceNames)}});
  }

  const SdfPath instancerPath = info.GetInstancerPath();

  std::shared_ptr<SdfPathSet> &instances = prototypeNameToInstances[info.prototypeName];
  if (instances) {
    ...
    _retainedSceneIndex->DirtyPrims({{instancerPath, locators}});
  }
  else {
    ...
    _retainedSceneIndex->AddPrims(
        {// Add propagated prototype base prim
         {info.GetPropagatedPrototypeBase(), TfToken(), HdRetainedContainerDataSource::New()},
         // instancer which is child of base prim.
         {instancerPath,
          HdPrimTypeTokens->instancer,
          _InstancerPrimSource::New(_inputScene,
  ...
  _retainedSceneIndex->AddPrims({{primPath, TfToken(), _GetDataSourceForInstance(primPath)}});
...
void _InstanceObserver::_AddPrim(const SdfPath &primPath)
{
...
void _InstanceObserver::_RemovePrim(const SdfPath &primPath)
{
...
void _InstanceObserver::_ResyncPrim(const SdfPath &primPath)
{
  _RemovePrim(primPath);
  _AddPrim(primPath);
}

_InstanceObserver::_PathToInstanceInfo::iterator _InstanceObserver::_RemoveInstance(
    const SdfPath &primPath, const _PathToInstanceInfo::iterator &it)
{
  const _InstanceInfo &info = it->second;

  const SdfPath instancerPath = info.GetInstancerPath();

  const _RemovalLevel level = _RemoveInstanceFromInfoToInstance(primPath, info);

  if (level > _RemovalLevel::None) {
    // Remove instance data source we added in _AddInstance.
    _retainedSceneIndex->RemovePrims({{primPath}});
  }
  ...
  if (level >= _RemovalLevel::Instancer) {
    ...
    // Remove instancer.
    _retainedSceneIndex->RemovePrims({{instancerPath}});
  ...
  if (level >= _RemovalLevel::BindingScope) {
    ...
    _retainedSceneIndex->RemovePrims({{info.GetBindingPrimPath()}});
  }

  return _instanceToInfo.erase(it);
}

_InstanceObserver::_RemovalLevel _InstanceObserver::_RemoveInstanceFromInfoToInstance(
    const SdfPath &primPath, const _InstanceInfo &info)
{
...
void _InstanceObserver::_DirtyInstancesAndResetPointer(_PathToIntSharedPtr *const instanceToIndex)
{
  if (!*instanceToIndex) {
    return;
  }
  ...
  *instanceToIndex = nullptr;
  ...
  for (const auto &instanceAndIndex : *original) {
    static const HdDataSourceLocatorSet locators{HdInstanceSchema::GetDefaultLocator()};
    _retainedSceneIndex->DirtyPrims({{instanceAndIndex.first, locators}});
  }
}

void _InstanceObserver::_DirtyInstancerForInstance(const SdfPath &instance,
...
HdContainerDataSourceHandle _InstanceObserver::_GetDataSourceForInstance(const SdfPath &primPath)
{
  ...
  return HdRetainedContainerDataSource::New(
      HdInstanceSchema::GetSchemaToken(), HdLazyContainerDataSource::New([self, primPath]() {
        if (self) {
...
HdContainerDataSourceHandle _InstanceObserver::_GetInstanceSchemaDataSource(
    const SdfPath &primPath)
{
  auto it = _instanceToInfo.find(primPath);
  if (it == _instanceToInfo.end()) {
  ...
  static HdIntDataSourceHandle const prototypeIndexDs = HdRetainedTypedSampledDataSource<int>::New(
      0);

  return HdInstanceSchema::Builder()
      .SetInstancer(HdRetainedTypedSampledDataSource<SdfPath>::New(info.GetInstancerPath()))
      .SetPrototypeIndex(prototypeIndexDs)
      .SetInstanceIndex(
          HdRetainedTypedSampledDataSource<int>::New(_GetInstanceIndex(info, primPath)))
      .Build();
}

int _InstanceObserver::_GetInstanceIndex(const _InstanceInfo &info, const SdfPath &instancePath)
{
  TRACE_FUNCTION();
...
_InstanceObserver::_PathToIntSharedPtr _InstanceObserver::_GetInstanceToIndex(
    const _InstanceInfo &info)
{
  TRACE_FUNCTION();
...
_InstanceObserver::_PathToIntSharedPtr _InstanceObserver::_ComputeInstanceToIndex(
    const _InstanceInfo &info)
{
  TRACE_FUNCTION();
...
UsdImaging_NiInstanceAggregationSceneIndex::UsdImaging_NiInstanceAggregationSceneIndex(
    HdSceneIndexBaseRefPtr const &inputScene,
    const bool forNativePrototype,
    const TfTokenVector &instanceDataSourceNames)
    : _instanceObserver(std::make_unique<_InstanceObserver>(
...
std::vector<HdSceneIndexBaseRefPtr> UsdImaging_NiInstanceAggregationSceneIndex::GetInputScenes()
    const
{
  return {_instanceObserver->GetRetainedSceneIndex()};
}

HdSceneIndexPrim UsdImaging_NiInstanceAggregationSceneIndex::GetPrim(const SdfPath &primPath) const
{
  return _instanceObserver->GetRetainedSceneIndex()->GetPrim(primPath);
}

SdfPathVector UsdImaging_NiInstanceAggregationSceneIndex::GetChildPrimPaths(
    const SdfPath &primPath) const
{
  return _instanceObserver->GetRetainedSceneIndex()->GetChildPrimPaths(primPath);
}

/* static */
TfToken UsdImaging_NiInstanceAggregationSceneIndex::GetPrototypeNameFromInstancerPath(
    const SdfPath &primPath)
{
  ...
  if (primPath.GetNameToken() != UsdImaging_NiPrototypeSceneIndexTokens->instancer) {
    return TfToken();
  }
...
/* static */
SdfPath UsdImaging_NiInstanceAggregationSceneIndex::GetBindingScopeFromInstancerPath(
    const SdfPath &primPath)
{
  return primPath.GetParentPath().GetParentPath();
}

UsdImaging_NiInstanceAggregationSceneIndex::_RetainedSceneIndexObserver::
    _RetainedSceneIndexObserver(UsdImaging_NiInstanceAggregationSceneIndex *const owner)
    : _owner(owner)
{
}

void UsdImaging_NiInstanceAggregationSceneIndex::_RetainedSceneIndexObserver::PrimsAdded(
    const HdSceneIndexBase &sender, const AddedPrimEntries &entries)
{
  _owner->_SendPrimsAdded(entries);
}

void UsdImaging_NiInstanceAggregationSceneIndex::_RetainedSceneIndexObserver::PrimsDirtied(
    const HdSceneIndexBase &sender, const DirtiedPrimEntries &entries)
{
  _owner->_SendPrimsDirtied(entries);
}

void UsdImaging_NiInstanceAggregationSceneIndex::_RetainedSceneIndexObserver::PrimsRemoved(
    const HdSceneIndexBase &sender, const RemovedPrimEntries &entries)
{
  _owner->_SendPrimsRemoved(entries);
}

void UsdImaging_NiInstanceAggregationSceneIndex::_RetainedSceneIndexObserver::PrimsRenamed(
