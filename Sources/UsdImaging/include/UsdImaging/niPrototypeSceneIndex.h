...
#define USDIMAGING_NI_PROTOTYPE_SCENE_INDEX_TOKENS \
  ((instancer, "UsdNiInstancer"))((prototype, "UsdNiPrototype"))

TF_DECLARE_PUBLIC_TOKENS(UsdImaging_NiPrototypeSceneIndexTokens,
...
///
class UsdImaging_NiPrototypeSceneIndex : public HdSingleInputFilteringSceneIndexBase {
 public:
  ...
  //
  static UsdImaging_NiPrototypeSceneIndexRefPtr New(
  ...
  static const SdfPath &GetInstancerPath();

  /// Get path of the copy of the USD prototype that is a child of the
  /// instancer.
  static const SdfPath &GetPrototypePath();

  /// Get's data source for instancedBy schema for prims within this
  /// prototype.
  static const HdDataSourceBaseHandle &GetInstancedByDataSource();
 ...
 private:
  UsdImaging_NiPrototypeSceneIndex(HdSceneIndexBaseRefPtr const &inputSceneIndex,
                                   bool forNativePrototype,
