...
template<typename Strategy, typename ImplData = bool> class UsdImaging_ResolvedAttributeCache {
 ...
 public:
  ...
  UsdImaging_ResolvedAttributeCache()
      : _time(UsdTimeCode::Default()), _rootPath(SdfPath::AbsoluteRootPath()), _cacheVersion(1)
  {
  }
 ...
 private:
  ...
  struct _Entry {
    ...
    _Entry(const query_type &query_, const value_type &value_, unsigned version_)
        : query(query_), value(value_), version(version_)
    {
    }

    _Entry(const _Entry &other) : query(other.query), value(other.value)
    {
      version.store(other.version.load());
    }

    _Entry(_Entry &&other) : query(std::move(other.query)), value(std::move(other.value))
    {
      version.store(other.version.load());
    }
...
void UsdImaging_ResolvedAttributeCache<Strategy, ImplData>::_SetCacheEntryForPrim(
    const UsdPrim &prim, value_type const &value, _Entry *entry) const
{
  // Note: _cacheVersion is not allowed to change during cache access.
  unsigned v = entry->version;
  if (v < _cacheVersion && entry->version.compare_exchange_strong(v, _cacheVersion.load())) {
    entry->value = value;
...
struct UsdImaging_MaterialStrategy {
  ...
  static query_type MakeQuery(UsdPrim const &prim, ImplData *implData)
  {
    ...
    UsdShadeMaterial materialPrim = UsdShadeMaterialBindingAPI(prim).ComputeBoundMaterial(
    ...
    const SdfPath targetPath = UsdShadeMaterialBindingAPI::GetResolvedTargetPathFromBindingRel(
        bindingRel);
    return targetPath;
  }
  ...
  static value_type ComputeMaterialPath(UsdPrim const &prim, ImplData *implData)
  {
    ...
    UsdRelationship bindingRel;
    UsdShadeMaterialBindingAPI(prim).ComputeBoundMaterial(&implData->GetBindingsCache(),
                                                          &implData->GetCollectionQueryCache(),
                                                          implData->GetMaterialPurpose(),
                                                          &bindingRel);
...
struct UsdImaging_CoordSysBindingStrategy {
                            ...
                            query_type const *query)
  {
    ...
    UsdShadeCoordSysAPI coordSysAPI = UsdShadeCoordSysAPI(prim, TfToken("noop"));
