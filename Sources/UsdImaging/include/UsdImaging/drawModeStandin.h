...
class UsdImaging_DrawModeStandin {
 public:
  ...
  void ComputePrimAddedEntries(HdSceneIndexObserver::AddedPrimEntries *entries) const;

  /// Compute removed entries for the stand-in geometry
  void ComputePrimRemovedEntries(HdSceneIndexObserver::RemovedPrimEntries *entries) const;
  ...
  virtual void ProcessDirtyLocators(const HdDataSourceLocatorSet &dirtyLocator,
 ...
 protected:
  ...
  /// accepts paths relative to the typeless container.
  virtual HdContainerDataSourceHandle _GetDescendantPrimSource(const SdfPath &path) const = 0;

  UsdImaging_DrawModeStandin(const SdfPath &path, const HdContainerDataSourceHandle &primSource)
      : _path(path), _primSource(primSource)
  {
  }
...
using UsdImaging_DrawModeStandinSharedPtr = std::shared_ptr<UsdImaging_DrawModeStandin>;
