...
class UsdImagingAdapterRegistry : public TfSingleton<UsdImagingAdapterRegistry> {
  ...
  std::shared_ptr<T> _ConstructAdapter(TfToken const &adapterKey, const _TypeMap &tm);

  template<typename T, typename factoryT>
  std::shared_ptr<T> _ConstructAdapter(TfToken const &adapterKey, const TfType &adapterType);
 ...
 public:
  ...
  /// Returns NULL if no adapter was registered for this key.
  USDIMAGING_API
  UsdImagingAPISchemaAdapterSharedPtr ConstructAPISchemaAdapter(TfToken const &adapterKey);
