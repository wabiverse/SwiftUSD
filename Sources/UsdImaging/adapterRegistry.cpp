...
UsdImagingAdapterRegistry::UsdImagingAdapterRegistry()
{
  ...
  TF_FOR_ALL(typeIt, types)
  {
    ...
    if (_typeMap.count(primTypeName) != 0) {
      TF_CODING_ERROR(
          "[PluginDiscover] A prim adapter for primType '%s' "
    ...
    JsObject::const_iterator includeFamilyIt = metadata.find("includeSchemaFamily");
    if (includeFamilyIt != metadata.end()) {
      if (!includeFamilyIt->second.Is<bool>()) {
        TF_RUNTIME_ERROR(
            "[PluginDiscover] includeSchemaFamily "
            "metadata was corrupted for plugin '%s'; not holding "
            "bool\n",
            typeIt->GetTypeName().c_str());
        continue;
        ...
        includeSchemaFamilies.push_back(std::make_pair(primTypeName, includeDerived));
      }
    }
  }
  ...
  for (auto const &pair : includeSchemaFamilies) {
         ...
         UsdSchemaRegistry::FindSchemaInfosInFamily(familyName))
    {
      if (_typeMap.emplace(schemaInfo->identifier, adapterType).second) {
        const TfToken typeName = UsdSchemaRegistry::GetSchemaTypeName(schemaInfo->type);
        TF_DEBUG(USDIMAGING_PLUGINS)
            .Msg(
  ...
  TF_FOR_ALL(typeIt, types)
  {
    ...
    if (it == metadata.end()) {
      TF_RUNTIME_ERROR(
          "[PluginDiscover] apiSchemaName metadata was not "
          "present for plugin '%s'\n",
          typeIt->GetTypeName().c_str());
      continue;
    }
    if (!it->second.Is<std::string>()) {
      TF_RUNTIME_ERROR(
          "[PluginDiscover] apiSchemaName metadata was "
          "corrupted for plugin '%s'\n",
          typeIt->GetTypeName().c_str());
      continue;
    }
...
std::shared_ptr<T> UsdImagingAdapterRegistry::_ConstructAdapter(TfToken const &adapterKey,
                                                                const _TypeMap &tm)
{
                                                                ...
                                                                const TfType &adapterType)
{
  ...
  factoryT *factory = adapterType.GetFactory<factoryT>();
  if (!factory) {
    TF_CODING_ERROR(
    ...
    TfToken const &adapterKey)
{
  ...
  return _ConstructAdapter<UsdImagingPrimAdapter, UsdImagingPrimAdapterFactoryBase>(adapterKey,
                                                                                    _typeMap);
}

bool UsdImagingAdapterRegistry::HasAPISchemaAdapter(TfToken const &adapterKey)
{
  return _apiSchemaTypeMap.find(adapterKey) != _apiSchemaTypeMap.end();
}

UsdImagingAPISchemaAdapterSharedPtr UsdImagingAdapterRegistry::ConstructAPISchemaAdapter(
    TfToken const &adapterKey)
{
  return _ConstructAdapter<UsdImagingAPISchemaAdapter, UsdImagingAPISchemaAdapterFactoryBase>(
      adapterKey, _apiSchemaTypeMap);
}

const TfTokenVector &UsdImagingAdapterRegistry::GetAPISchemaAdapterKeys()
{
  return _apiSchemaAdapterKeys;
}

UsdImagingAdapterRegistry::ApiSchemaAdapters UsdImagingAdapterRegistry::
    ConstructKeylessAPISchemaAdapters()
{
  ...
  for (const TfType &adapterType : _keylessApiSchemaAdapterTypes) {
    if (UsdImagingAPISchemaAdapterSharedPtr instance =
            _ConstructAdapter<UsdImagingAPISchemaAdapter, UsdImagingAPISchemaAdapterFactoryBase>(
                TfToken(), adapterType))
    {
