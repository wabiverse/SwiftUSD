...
TfTokenVector UsdImagingRenderSettingsAdapter::GetImagingSubprims(UsdPrim const &prim)
{
  return {TfToken()};
}

TfToken UsdImagingRenderSettingsAdapter::GetImagingSubprimType(UsdPrim const &prim,
...
HdContainerDataSourceHandle UsdImagingRenderSettingsAdapter::GetImagingSubprimData(
    UsdPrim const &prim,
    TfToken const &subprim,
    const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  if (subprim.IsEmpty()) {
    return UsdImagingDataSourceRenderSettingsPrim::New(prim.GetPath(), prim, stageGlobals);
  }

  return nullptr;
}

HdDataSourceLocatorSet UsdImagingRenderSettingsAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
...
bool UsdImagingRenderSettingsAdapter::IsSupported(UsdImagingIndexProxy const *index) const
{
  return index->IsBprimTypeSupported(HdPrimTypeTokens->renderSettings);
}

SdfPath UsdImagingRenderSettingsAdapter::Populate(
    UsdPrim const &prim,
    ...
    UsdImagingInstancerContext const *instancerContext)
{
  ...
  //     and var to aid with change processing.
  {
    ...
    for (SdfPath const &target : targets) {
      if (UsdRenderProduct product = UsdRenderProduct(prim.GetStage()->GetPrimAtPath(target))) {
  ...
  {
    const TfToken outputTokens[] = {_tokens->outputsRiIntegrator,
    ...
    for (const auto &token : outputTokens) {
      ...
      for (auto const &connPath : connections) {
        const UsdPrim &connPrim = prim.GetStage()->GetPrimAtPath(connPath.GetPrimPath());
        if (connPrim) {
...
void UsdImagingRenderSettingsAdapter::_RemovePrim(SdfPath const &cachePath,
                                                  UsdImagingIndexProxy *index)
{
  index->RemoveBprim(HdPrimTypeTokens->renderSettings, cachePath);
}

void UsdImagingRenderSettingsAdapter::TrackVariability(
    UsdPrim const &prim,
...
void UsdImagingRenderSettingsAdapter::UpdateForTime(
    UsdPrim const &prim,
    SdfPath const &cachePath,
    UsdTimeCode time,
...
HdDirtyBits UsdImagingRenderSettingsAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                                   SdfPath const &cachePath,
                                                                   TfToken const &propertyName)
{
  ...
  return HdRenderSettings::DirtyNamespacedSettings | HdRenderSettings::DirtyRenderProducts;
}

void UsdImagingRenderSettingsAdapter::MarkDirty(UsdPrim const &prim,
                                                SdfPath const &cachePath,
                                                HdDirtyBits dirty,
                                                UsdImagingIndexProxy *index)
{
...
static HdRenderSettings::RenderProducts _ToHdRenderProducts(UsdRenderSpec const &renderSpec)
{
  ...
  for (auto const &product : renderSpec.products) {
    ...
    for (size_t const &varId : product.renderVarIndices) {
      UsdRenderSpec::RenderVar const &rv = renderSpec.renderVars[varId];
    ...
    hdProduct.aspectRatioConformPolicy = product.aspectRatioConformPolicy;
...
VtValue UsdImagingRenderSettingsAdapter::Get(UsdPrim const &prim,
                                             SdfPath const &cachePath,
                                             TfToken const &key,
                                             UsdTimeCode time,
                                             VtIntArray *outIndices) const
{
  ...
  if (key == HdRenderSettingsPrimTokens->namespacedSettings) {
    return VtValue(UsdRenderComputeNamespacedSettings(prim, _GetRenderSettingsNamespaces()));
  }

  if (key == HdRenderSettingsPrimTokens->renderProducts) {
    const UsdRenderSpec renderSpec = UsdRenderComputeSpec(UsdRenderSettings(prim),
                                                          _GetRenderSettingsNamespaces());
  ...
  TF_CODING_ERROR("Property %s not supported for RenderSettings by UsdImaging, path: %s",
                  key.GetText(),
                  cachePath.GetText());
  return VtValue();
}
