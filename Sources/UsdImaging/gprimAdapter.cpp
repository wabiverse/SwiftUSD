...
HdDataSourceLocatorSet UsdImagingGprimAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    ...
    UsdImagingInstancerContext const *instancerContext) const
{
  ...
  if (requestedBits & HdChangeTracker::DirtyPoints) {
    ...
    if (blurScaleCache->GetValue(prim).has_value) {
      _MergePrimvar(
          &vPrimvars, HdTokens->blurScale, HdInterpolationConstant, HdPrimvarRoleTokens->none);
    }
  }
...
void UsdImagingGprimAdapter::MarkCollectionsDirty(UsdPrim const &prim,
                                                  SdfPath const &cachePath,
                                                  UsdImagingIndexProxy *index)
{
                                    ...
                                    VtIntArray *outIndices) const
{
  ...
  if (key == HdTokens->displayColor) {
    ...
    const int value = cache->GetTime() == time ?
                          cache->GetValue(prim) :
                          UsdImaging_NonlinearSampleCountStrategy::ComputeNonlinearSampleCount(
                              prim, time);
...
/* static */
GfMatrix4d UsdImagingGprimAdapter::GetImplicitBasis(TfToken const &axis)
{
  GfVec4d u, v, spine;
...
TfTokenVector UsdImagingGprimAdapter::_CollectMaterialPrimvars(
    SdfPathVector const &materialUsdPaths, UsdTimeCode time) const
{
  TfTokenVector primvars;
