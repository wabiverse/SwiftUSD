...
TfToken::HashSet _GetPurposes(HdContainerDataSourceHandle const &inputArgs)
{
  static const TfToken defaultTokens[] = {HdTokens->geometry};
  static const TfToken::HashSet defaultSet{std::begin(defaultTokens), std::end(defaultTokens)};

  if (!inputArgs) {
    return defaultSet;
  }

  HdTypedVectorSchema<HdTokenDataSource> vecSchema(HdVectorDataSource::Cast(
      inputArgs->Get(UsdImagingExtentResolvingSceneIndexTokens->purposes)));
  if (!vecSchema) {
    return defaultSet;
  }
...
struct _Info {
  _Info(HdContainerDataSourceHandle const &inputArgs) : purposes(_GetPurposes(inputArgs)) {}
...
using _DirtyEntryPredicate = bool (*)(const HdSceneIndexObserver::DirtiedPrimEntry &);
using _DirtyEntryTransform =
    HdSceneIndexObserver::DirtiedPrimEntry (*)(const HdSceneIndexObserver::DirtiedPrimEntry &);

template<_DirtyEntryPredicate Predicate, _DirtyEntryTransform Transform>
class _TransformedEntries {
 public:
  _TransformedEntries(const HdSceneIndexObserver::DirtiedPrimEntries &entries) : _entries(entries)
  {
    size_t i = 0;
    ...
    _newEntries.insert(_newEntries.end(), _entries.begin(), _entries.begin() + i);
  ...
  const HdSceneIndexObserver::DirtiedPrimEntries &GetEntries() const
  {
...
bool _ContainsExtentsHintWithoutExtent(const HdSceneIndexObserver::DirtiedPrimEntry &entry)
{
  return entry.dirtyLocators.Intersects(UsdImagingExtentsHintSchema::GetDefaultLocator()) &&
         !entry.dirtyLocators.Contains(HdExtentSchema::GetDefaultLocator());
}

HdSceneIndexObserver::DirtiedPrimEntry _ExtentAdded(
...
class _PrimSource : public HdContainerDataSource {
 ...
 private:
  _PrimSource(HdContainerDataSourceHandle const &primSource, _InfoSharedPtr const &info)
      : _primSource(primSource), _info(info)
  {
  }

  UsdImagingExtentsHintSchema _GetExtentsHints() const
  {
    return UsdImagingExtentsHintSchema::GetFromParent(_primSource);
  }
  ...
  HdDataSourceBaseHandle _GetExtentFromExtentsHint() const
  {
    ...
    if (_info->purposes.size() == 1) {
      return extentsHintSchema.GetExtent(*_info->purposes.begin()).GetContainer();
    }
    ...
    for (const TfToken &purpose : _info->purposes) {
      ...
      if (minDs && maxDs) {
        bbox.UnionWith(GfRange3d(minDs->GetTypedValue(0.0f), maxDs->GetTypedValue(0.0f)));
      }
    }

    return HdExtentSchema::Builder()
        .SetMin(HdRetainedTypedSampledDataSource<GfVec3d>::New(bbox.GetMin()))
        .SetMax(HdRetainedTypedSampledDataSource<GfVec3d>::New(bbox.GetMax()))
        .Build();
  }
...
UsdImagingExtentResolvingSceneIndexRefPtr UsdImagingExtentResolvingSceneIndex::New(
    HdSceneIndexBaseRefPtr const &inputSceneIndex, HdContainerDataSourceHandle const &inputArgs)
{
  return TfCreateRefPtr(new UsdImagingExtentResolvingSceneIndex(inputSceneIndex, inputArgs));
}

UsdImagingExtentResolvingSceneIndex::UsdImagingExtentResolvingSceneIndex(
    const HdSceneIndexBaseRefPtr &inputSceneIndex, HdContainerDataSourceHandle const &inputArgs)
    : HdSingleInputFilteringSceneIndexBase(inputSceneIndex),
      _info(std::make_shared<_Info>(inputArgs))
{
}

UsdImagingExtentResolvingSceneIndex::~UsdImagingExtentResolvingSceneIndex() = default;

HdSceneIndexPrim UsdImagingExtentResolvingSceneIndex::GetPrim(const SdfPath &primPath) const
{
...
SdfPathVector UsdImagingExtentResolvingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void UsdImagingExtentResolvingSceneIndex::_PrimsAdded(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  _SendPrimsAdded(entries);
}

void UsdImagingExtentResolvingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  _TransformedEntries<
      ...
      /* Transform = */ _ExtentAdded>
      newEntries(entries);

  _SendPrimsDirtied(newEntries.GetEntries());
}

void UsdImagingExtentResolvingSceneIndex::_PrimsRemoved(
