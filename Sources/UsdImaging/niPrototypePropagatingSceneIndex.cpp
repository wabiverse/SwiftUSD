...
TF_DEFINE_ENV_SETTING(USDIMAGING_SHOW_NATIVE_PROTOTYPE_SCENE_INDICES,
                      false,
...
class UsdImagingNiPrototypePropagatingSceneIndex::_SceneIndexCache {
 public:
  ...
  SceneIndices GetSceneIndicesForPrototype(
      ...
      HdContainerDataSourceHandle const &prototypeRootOverlayDs)
  {
    ...
    _SceneIndices2 &sceneIndices2 = _prototypeToBindingHashToSceneIndices[prototypeName];
    _SceneIndices1 &sceneIndices1 = sceneIndices2.hashToSceneIndices[prototypeRootOverlayDsHash];

    // Check whether weak ptr references valid scene index.
    HdSceneIndexBaseRefPtr isolatingSceneIndex = sceneIndices2.isolatingSceneIndex;
    if (!isolatingSceneIndex) {
      // Allocate new scene index if not.
      isolatingSceneIndex = _ComputeIsolatingSceneIndex(prototypeName);
      // Store weak ptr so that it can be re-used in the future.
      sceneIndices2.isolatingSceneIndex = isolatingSceneIndex;
    }
    ...
    if (!result.prototypeSceneIndex) {
      result.prototypeSceneIndex = _ComputePrototypeSceneIndex(
          isolatingSceneIndex, forPrototype, prototypeRootOverlayDs);
      sceneIndices1.prototypeSceneIndex = result.prototypeSceneIndex;
    }

    result.instanceAggregationSceneIndex = sceneIndices1.instanceAggregationSceneIndex;
    if (!result.instanceAggregationSceneIndex) {
      result.instanceAggregationSceneIndex = _ComputeInstanceAggregationSceneIndex(
          result.prototypeSceneIndex, forPrototype);
      sceneIndices1.instanceAggregationSceneIndex = result.instanceAggregationSceneIndex;
    }

    return result;
  }

  void GarbageCollect(const TfToken &prototypeName, const size_t prototypeRootOverlayDsHash)
  {
 ...
 private:
  ...
  HdSceneIndexBaseRefPtr _ComputeIsolatingSceneIndex(const TfToken &prototypeName) const
  {
    if (prototypeName.IsEmpty()) {
      return UsdImaging_NiPrototypePruningSceneIndex::New(_inputSceneIndex);
    }
    else {
  ...
  HdSceneIndexBaseRefPtr _ComputePrototypeSceneIndex(
      ...
      HdContainerDataSourceHandle const &prototypeRootOverlayDs)
  {
    ...
    sceneIndex = UsdImaging_NiPrototypeSceneIndex::New(
        sceneIndex, forPrototype, prototypeRootOverlayDs);
    ...
    // correctly.
    sceneIndex = HdFlatteningSceneIndex::New(sceneIndex, UsdImagingFlattenedDataSourceProviders());
    if (_sceneIndexAppendCallback) {
      // Typically adds the UsdImagingDrawModeSceneIndex.
      sceneIndex = _sceneIndexAppendCallback(sceneIndex);
    }

    return sceneIndex;
  }

  HdSceneIndexBaseRefPtr _ComputeInstanceAggregationSceneIndex(
      HdSceneIndexBaseRefPtr const &prototypeSceneIndex, const bool forPrototype)
  {
    return UsdImaging_NiInstanceAggregationSceneIndex::New(
        prototypeSceneIndex, forPrototype, _instanceDataSourceNames);
  }

  static void _GarbageCollect(std::map<size_t, _SceneIndices1> *hashToSceneIndices,
                              const size_t prototypeRootOverlayDsHash)
  {
...
// destruction.
class UsdImagingNiPrototypePropagatingSceneIndex::_MergingSceneIndexEntry {
 public:
  _MergingSceneIndexEntry(const SdfPath &prefix,
                          HdSceneIndexBaseRefPtr const &sceneIndex,
                          HdMergingSceneIndexRefPtr const &mergingSceneIndex)
      : _rerootingSceneIndex(UsdImagingRerootingSceneIndex::New(
            sceneIndex,
...
UsdImagingNiPrototypePropagatingSceneIndexRefPtr UsdImagingNiPrototypePropagatingSceneIndex::New(
    ...
    const SceneIndexAppendCallback &sceneIndexAppendCallback)
{
  return _New(/* prototypeName = */ TfToken(),
              ...
              std::make_shared<_SceneIndexCache>(
                  inputSceneIndex, instanceDataSourceNames, sceneIndexAppendCallback));
}

UsdImagingNiPrototypePropagatingSceneIndexRefPtr UsdImagingNiPrototypePropagatingSceneIndex::_New(
    const TfToken &prototypeName,
    HdContainerDataSourceHandle const &prototypeRootOverlayDs,
    _SceneIndexCacheSharedPtr const &cache)
{
  return TfCreateRefPtr(new UsdImagingNiPrototypePropagatingSceneIndex(
      prototypeName, prototypeRootOverlayDs, cache));
}

UsdImagingNiPrototypePropagatingSceneIndex::UsdImagingNiPrototypePropagatingSceneIndex(
    const TfToken &prototypeName,
    HdContainerDataSourceHandle const &prototypeRootOverlayDs,
    _SceneIndexCacheSharedPtr const &cache)
    : _prototypeName(prototypeName),
      _prototypeRootOverlayDsHash(HdDataSourceHash(prototypeRootOverlayDs, 0.0f, 0.0f)),
      _cache(cache),
      ...
      _mergingSceneIndexObserver(this)
{
  ...
  const _SceneIndexCache::SceneIndices sceneIndices = _cache->GetSceneIndicesForPrototype(
  ...
  _mergingSceneIndex->AddInputScene(sceneIndices.prototypeSceneIndex, SdfPath::AbsoluteRootPath());
  _mergingSceneIndex->AddInputScene(sceneIndices.instanceAggregationSceneIndex,
...
void UsdImagingNiPrototypePropagatingSceneIndex::_Populate(
    HdSceneIndexBaseRefPtr const &instanceAggregationSceneIndex)
{
  TRACE_FUNCTION();

  for (const SdfPath &primPath :
       HdSceneIndexPrimView(instanceAggregationSceneIndex, SdfPath::AbsoluteRootPath()))
  {
    _AddPrim(primPath);
  }
}

static HdContainerDataSourceHandle _GetBindingScopeDataSource(
    HdSceneIndexBaseRefPtr const &sceneIndex, const SdfPath &primPath)
{
  const SdfPath bindingScope =
      UsdImaging_NiInstanceAggregationSceneIndex::GetBindingScopeFromInstancerPath(primPath);

  return sceneIndex->GetPrim(bindingScope).dataSource;
}

void UsdImagingNiPrototypePropagatingSceneIndex::_AddPrim(const SdfPath &primPath)
{
  const TfToken prototypeName =
      UsdImaging_NiInstanceAggregationSceneIndex::GetPrototypeNameFromInstancerPath(primPath);
  if (prototypeName.IsEmpty()) {
    return;
  }

  _MergingSceneIndexEntryUniquePtr &entry = _instancersToMergingSceneIndexEntry[primPath];
  ...
  entry = std::make_unique<_MergingSceneIndexEntry>(
      ...
      UsdImagingNiPrototypePropagatingSceneIndex::_New(
          ...
          _GetBindingScopeDataSource(_instanceAggregationSceneIndex, primPath),
          _cache),
      _mergingSceneIndex);
}

template<typename Container> static void _ErasePrefix(Container *const c, const SdfPath &prefix)
{
...
void UsdImagingNiPrototypePropagatingSceneIndex::_RemovePrim(const SdfPath &primPath)
{
  TRACE_FUNCTION();
...
std::vector<HdSceneIndexBaseRefPtr> UsdImagingNiPrototypePropagatingSceneIndex::GetInputScenes()
    const
{
...
std::vector<HdSceneIndexBaseRefPtr> UsdImagingNiPrototypePropagatingSceneIndex::
    GetEncapsulatedScenes() const
{
  return {_mergingSceneIndex};
}

HdSceneIndexPrim UsdImagingNiPrototypePropagatingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  TRACE_FUNCTION();
...
SdfPathVector UsdImagingNiPrototypePropagatingSceneIndex::GetChildPrimPaths(
...
UsdImagingNiPrototypePropagatingSceneIndex::_InstanceAggregationSceneIndexObserver::
    _InstanceAggregationSceneIndexObserver(UsdImagingNiPrototypePropagatingSceneIndex *const owner)
    : _owner(owner)
{
}

void UsdImagingNiPrototypePropagatingSceneIndex::_InstanceAggregationSceneIndexObserver::
    PrimsAdded(const HdSceneIndexBase &sender, const AddedPrimEntries &entries)
{
  TRACE_FUNCTION();
...
void UsdImagingNiPrototypePropagatingSceneIndex::_InstanceAggregationSceneIndexObserver::
    PrimsDirtied(const HdSceneIndexBase &sender, const DirtiedPrimEntries &entries)
{
  // No need to handle this.
}

void UsdImagingNiPrototypePropagatingSceneIndex::_InstanceAggregationSceneIndexObserver::
    PrimsRemoved(const HdSceneIndexBase &sender, const RemovedPrimEntries &entries)
{
  TRACE_FUNCTION();
...
void UsdImagingNiPrototypePropagatingSceneIndex::_InstanceAggregationSceneIndexObserver::
    PrimsRenamed(const HdSceneIndexBase &sender, const RenamedPrimEntries &entries)
{
  ConvertPrimsRenamedToRemovedAndAdded(sender, entries, this);
}

UsdImagingNiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::_MergingSceneIndexObserver(
    UsdImagingNiPrototypePropagatingSceneIndex *const owner)
    : _owner(owner)
{
}

void UsdImagingNiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsAdded(
    const HdSceneIndexBase &sender, const AddedPrimEntries &entries)
{
  _owner->_SendPrimsAdded(entries);
}

void UsdImagingNiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsDirtied(
    const HdSceneIndexBase &sender, const DirtiedPrimEntries &entries)
{
  _owner->_SendPrimsDirtied(entries);
}

void UsdImagingNiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsRemoved(
    const HdSceneIndexBase &sender, const RemovedPrimEntries &entries)
{
  _owner->_SendPrimsRemoved(entries);
}

void UsdImagingNiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsRenamed(
