...
TfTokenVector UsdImagingConeAdapter::GetImagingSubprims(UsdPrim const &prim)
{
  return {TfToken()};
}

TfToken UsdImagingConeAdapter::GetImagingSubprimType(UsdPrim const &prim, TfToken const &subprim)
{
  if (subprim.IsEmpty()) {
...
HdContainerDataSourceHandle UsdImagingConeAdapter::GetImagingSubprimData(
    UsdPrim const &prim,
...
HdDataSourceLocatorSet UsdImagingConeAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
...
VtValue UsdImagingConeAdapter::GetPoints(UsdPrim const &prim, UsdTimeCode time) const
{
  ...
  GeomUtilConeMeshGenerator::GeneratePoints(points.begin(), numRadial, radius, height, &basis);
