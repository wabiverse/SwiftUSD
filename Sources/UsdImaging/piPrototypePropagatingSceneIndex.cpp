...
TF_DEFINE_ENV_SETTING(USDIMAGING_SHOW_POINT_PROTOTYPE_SCENE_INDICES,
                      false,
...
class _PropagatedPrototypesSource : public HdContainerDataSource {
 public:
  ...
  void AddPropagatedPrototype(const TfToken &instancerHash, const SdfPath &propagatedPrototype)
  {
    _instancerHashToPropagatedPrototype[instancerHash] = propagatedPrototype;
  }
...
class _UsdPrimInfoSceneIndex : public HdRetainedSceneIndex {
 public:
  ...
  void AddPropagatedPrototype(const SdfPath &prototype,
                              const TfToken &instancerHash,
                              const SdfPath &propagatedPrototype);

  // Removes entry for prim at prototype and locator
  // __usdPrimInfo:piPropagatedPrototypes:INSTANCER_HASH.
  void RemovePropagatedPrototype(const SdfPath &prototype, const TfToken &instancerHash);

 private:
  _PropagatedPrototypesSourceHandle _GetDataSource(const SdfPath &prototype);
  _PropagatedPrototypesSourceHandle _CreateDataSource(const SdfPath &prototype);
  _PropagatedPrototypesSourceHandle _GetOrCreateDataSource(const SdfPath &prototype);
};

_PropagatedPrototypesSourceHandle _UsdPrimInfoSceneIndex::_GetDataSource(const SdfPath &prototype)
{
  HdContainerDataSourceHandle const primSource = GetPrim(prototype).dataSource;
  UsdImagingUsdPrimInfoSchema schema = UsdImagingUsdPrimInfoSchema::GetFromParent(primSource);
  return _PropagatedPrototypesSource::Cast(schema.GetPiPropagatedPrototypes());
}

_PropagatedPrototypesSourceHandle _UsdPrimInfoSceneIndex::_CreateDataSource(
    const SdfPath &prototype)
{
  _PropagatedPrototypesSourceHandle const ds = _PropagatedPrototypesSource::New();
  AddPrims({{prototype,
             TfToken(),
             HdRetainedContainerDataSource::New(
                 UsdImagingUsdPrimInfoSchema::GetSchemaToken(),
                 UsdImagingUsdPrimInfoSchema::Builder().SetPiPropagatedPrototypes(ds).Build())}});
  return ds;
}

_PropagatedPrototypesSourceHandle _UsdPrimInfoSceneIndex::_GetOrCreateDataSource(
    const SdfPath &prototype)
{
  if (_PropagatedPrototypesSourceHandle const ds = _GetDataSource(prototype)) {
    return ds;
  }
  else {
...
void _UsdPrimInfoSceneIndex::AddPropagatedPrototype(const SdfPath &prototype,
                                                    const TfToken &instancerHash,
                                                    const SdfPath &propagatedPrototype)
{
  _PropagatedPrototypesSourceHandle const ds = _GetOrCreateDataSource(prototype);
  ds->AddPropagatedPrototype(instancerHash, propagatedPrototype);
  static const HdDataSourceLocator locator =
      UsdImagingUsdPrimInfoSchema::GetDefaultLocator().Append(
          UsdImagingUsdPrimInfoSchemaTokens->piPropagatedPrototypes);
  DirtyPrims({{prototype, locator}});
}

void _UsdPrimInfoSceneIndex::RemovePropagatedPrototype(const SdfPath &prototype,
                                                       const TfToken &instancerHash)
{
  _PropagatedPrototypesSourceHandle const ds = _GetDataSource(prototype);
  if (!ds) {
    return;
  }
...
struct _Context {
  _Context(HdSceneIndexBaseRefPtr const &inputSceneIndex)
  ...
  {
    mergingSceneIndex->AddInputScene(instancerSceneIndex, SdfPath::AbsoluteRootPath());
    mergingSceneIndex->AddInputScene(usdPrimInfoSceneIndex, SdfPath::AbsoluteRootPath());
  }
...
class _InstancerObserver final : public HdSceneIndexObserver {
 public:
  _InstancerObserver(_ContextSharedPtr const &context);
 ...
 private:
  ...
  // prototype.
  void _UpdateInstancerPrototypes(_Map0 *prototypeToInstancerObserver,
                                  const SdfPath &instancer,
                                  const VtArray<SdfPath> &prototypes);

  // Convenience methods for _UpdateInstancerPrototypes.
  void _UpdateInstancer(_Map0 *prototypeToInstancerObserver, const SdfPath &path);
  void _UpdateInstancer(const SdfPath &path, const HdSceneIndexPrim &prim);
  void _UpdateInstancer(const SdfPath &path);
...
HdSceneIndexBaseRefPtr _RerootingSceneIndex(HdSceneIndexBaseRefPtr const &sceneIndex,
                                            ...
                                            const SdfPath &dstPrefix)
{
  if (srcPrefix.IsAbsoluteRootPath() && dstPrefix.IsAbsoluteRootPath()) {
    ...
    return UsdImagingRerootingSceneIndex::New(sceneIndex, srcPrefix, dstPrefix);
  }
}

_InstancerObserver::_InstancerObserver(_ContextSharedPtr const &context)
    : _InstancerObserver(context,
                         /* instancer = */ SdfPath(),
...
_InstancerObserver::_InstancerObserver(_ContextSharedPtr const &context,
    ...
    : _context(context),
      ...
      _prototypeSceneIndex(UsdImaging_PiPrototypeSceneIndex::New(
          // Isolate the prototype
          _RerootingSceneIndex(context->inputSceneIndex, prototype, prototype),
          instancer,
          prototype)),
      _rerootingSceneIndex(
          _RerootingSceneIndex(_prototypeSceneIndex, prototype, propagatedPrototype))
{
  _context->mergingSceneIndex->AddInputScene(_rerootingSceneIndex, propagatedPrototype);
...
SdfPath _InstancerObserver::_RerootedPath(const SdfPath &instancer) const
{
  return instancer.ReplacePrefix(_prototype, _propagatedPrototype);
}

TfToken _InstancerObserver::_InstancerHash(const SdfPath &instancer) const
{
...
HdContainerDataSourceHandle _InstancerTopology(const VtArray<SdfPath> &prototypes)
{
  return HdRetainedContainerDataSource::New(
      HdInstancerTopologySchema::GetSchemaToken(),
      HdInstancerTopologySchema::Builder()
          .SetPrototypes(HdRetainedTypedSampledDataSource<VtArray<SdfPath>>::New(prototypes))
          .Build());
}

void _InstancerObserver::_UpdateInstancerPrototypes(_Map0 *const prototypeToInstancerObserver,
                                                    const SdfPath &instancer,
                                                    const VtArray<SdfPath> &prototypes)
{
...

  {
         ...
         it != prototypeToInstancerObserver->end();)
    {
      ...
      if (prototypeSet.find(prototype) == prototypeSet.end()) {
        _context->usdPrimInfoSceneIndex->RemovePropagatedPrototype(prototype, instancerHash);
  ...
  for (const SdfPath &prototype : prototypes) {
    const SdfPath propagatedPrototype = prototype.AppendChild(instancerHash);
    propagatedPrototypes.push_back(propagatedPrototype);
    _InstancerObserverUniquePtr &observer = (*prototypeToInstancerObserver)[prototype];
    if (!observer) {
  ...
  _context->instancerSceneIndex->AddPrims({{rerootedInstancer,
                                            HdPrimTypeTokens->instancer,
                                            _InstancerTopology(propagatedPrototypes)}});
}

VtArray<SdfPath> _GetPrototypes(const HdSceneIndexPrim &instancer)
{
  HdInstancerTopologySchema topologySchema = HdInstancerTopologySchema::GetFromParent(
      instancer.dataSource);
...
void _InstancerObserver::_UpdateInstancer(_Map0 *const prototypeToInstancerObserver,
                                          const SdfPath &path)
{
  _UpdateInstancerPrototypes(
      prototypeToInstancerObserver, path, _GetPrototypes(_prototypeSceneIndex->GetPrim(path)));
}

void _InstancerObserver::_UpdateInstancer(const SdfPath &path, const HdSceneIndexPrim &prim)
{
  _UpdateInstancerPrototypes(&_subinstancerObservers[path], path, _GetPrototypes(prim));
}

void _InstancerObserver::_UpdateInstancer(const SdfPath &path)
{
  _UpdateInstancer(&_subinstancerObservers[path], path);
}

void _InstancerObserver::_Populate()
{
...
void _InstancerObserver::PrimsAdded(const HdSceneIndexBase &sender,
  ...
  for (const AddedPrimEntry &entry : entries) {
    ...
    if (entry.primType == HdPrimTypeTokens->instancer) {
      ...
      if (_subinstancerObservers.erase(path) > 0) {
        _context->instancerSceneIndex->RemovePrims({{_RerootedPath(path)}});
      }
    }
  }
}

void _InstancerObserver::PrimsDirtied(const HdSceneIndexBase &sender,
                                      const DirtiedPrimEntries &entries)
{
  TRACE_FUNCTION();

  static const HdDataSourceLocator locator = HdInstancerTopologySchema::GetDefaultLocator().Append(
...
void _InstancerObserver::PrimsRemoved(const HdSceneIndexBase &sender,
                                      const RemovedPrimEntries &entries)
{
  ...
  for (const RemovedPrimEntry &entry : entries) {
    ...
    while (it != _subinstancerObservers.end() && it->first.HasPrefix(path)) {
...
void _InstancerObserver::PrimsRenamed(const HdSceneIndexBase &sender,
...
UsdImagingPiPrototypePropagatingSceneIndexRefPtr UsdImagingPiPrototypePropagatingSceneIndex::New(
    HdSceneIndexBaseRefPtr const &inputSceneIndex)
{
  return TfCreateRefPtr(new UsdImagingPiPrototypePropagatingSceneIndex(inputSceneIndex));
}

UsdImagingPiPrototypePropagatingSceneIndex::UsdImagingPiPrototypePropagatingSceneIndex(
    HdSceneIndexBaseRefPtr const &inputSceneIndex)
...
std::vector<HdSceneIndexBaseRefPtr> UsdImagingPiPrototypePropagatingSceneIndex::GetInputScenes()
    const
{
...
std::vector<HdSceneIndexBaseRefPtr> UsdImagingPiPrototypePropagatingSceneIndex::
    GetEncapsulatedScenes() const
{
  return {_context->mergingSceneIndex};
}

HdSceneIndexPrim UsdImagingPiPrototypePropagatingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  TRACE_FUNCTION();
...
SdfPathVector UsdImagingPiPrototypePropagatingSceneIndex::GetChildPrimPaths(
...
UsdImagingPiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::_MergingSceneIndexObserver(
...
void UsdImagingPiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsAdded(
    const HdSceneIndexBase &sender, const AddedPrimEntries &entries)
{
  _owner->_SendPrimsAdded(entries);
}

void UsdImagingPiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsDirtied(
    const HdSceneIndexBase &sender, const DirtiedPrimEntries &entries)
{
  _owner->_SendPrimsDirtied(entries);
}

void UsdImagingPiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsRemoved(
    const HdSceneIndexBase &sender, const RemovedPrimEntries &entries)
{
  _owner->_SendPrimsRemoved(entries);
}

void UsdImagingPiPrototypePropagatingSceneIndex::_MergingSceneIndexObserver::PrimsRenamed(
