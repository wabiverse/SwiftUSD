...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (lightFilterType)(LightAPI));
  ...
  if (_timeVaryingPrimCacheValid) {
    for (SdfPath const &path : _timeVaryingPrimCache) {
      ...
      it->second.adapter->MarkDirty(
          it->second.usdPrim, path, it->second.timeVaryingBits, &indexProxy);
    }
  }
  else {
                                             ...
                                             SdfPathVector *affectedCachePaths)
{
  ...
  TF_DEBUG(USDIMAGING_CHANGES)
      .Msg("[_GatherDependencies] No entry in flattened cache for %s\n ", subtree.GetText());
...
void UsdImagingDelegate::ApplyPendingUpdates()
{
  ...
  if (!_usdPathsToResync.empty()) {
    ...
    for (const SdfPath &usdPath : usdPathsToResync) {
      TF_DEBUG(USDIMAGING_CHANGES)
          .Msg("[usdPathsToResync] Updating cache map for %s\n", usdPath.GetText());
      auto pair = resyncDependenciesCache.insert(std::make_pair(usdPath, SdfPathVector()));
      if (!pair.second) {
        // No insertion happened, path has been inserted
        continue;
      }
      auto &affectedCachePaths = pair.first->second;
      resyncPathsCacheDispatcher.Run([this, &usdPath, &affectedCachePaths]() {
  ...
  if (!_usdPathsToUpdate.empty()) {
    ...
    for (const auto &pathIt : usdPathsToUpdate) {
      ...
      TF_DEBUG(USDIMAGING_CHANGES)
          .Msg("[usdPathsToUpdate] Updating cache map for %s\n", usdPath.GetText());
      auto pair = updateDependenciesCache.insert(std::make_pair(usdPath, SdfPathVector()));
      if (!pair.second) {
        // No insertion happened, path has been inserted
        continue;
      }
      auto &affectedCachePaths = pair.first->second;
      updatePathsCacheDispatcher.Run([this, &usdPath, &affectedCachePaths]() {
        _GatherDependencies(usdPath, &affectedCachePaths);
      });
    }
    ...
    TF_FOR_ALL(pathIt, usdPathsToUpdate)
    {
      ...
      if (usdPath.IsPropertyPath() || usdPath.IsAbsoluteRootOrPrimPath()) {
        ...
        const bool resyncNeeded = _RefreshUsdObject(usdPath,
  ...
  if (!pathsDirtiedByCollections.empty()) {
    ...
    for (const SdfPath &dirtyPath : pathsDirtiedByCollections) {
      TF_DEBUG(USDIMAGING_CHANGES)
          .Msg("[Update]: invalidate collection member prim %s\n", dirtyPath.GetText());
...
// notice
bool _HasConnectionChanged(const SdfPath &path, const PathRange &pathRange)
{
                                              ...
                                              UsdStageWeakPtr const &sender)
{
  ...
  for (const auto &path : pathsToResync) {
    if (path.IsPrimPropertyPath() && _HasConnectionChanged(path, pathsToResync)) {
  ...
  const PathRange assetPathsToResync = notice.GetResolvedAssetPathsResyncedPaths();
...
static bool _IsMaterialAdapter(const UsdImagingPrimAdapterSharedPtr &adapter)
{
...
bool UsdImagingDelegate::_RefreshUsdObject(SdfPath const &usdPath,
                                           ...
                                           SdfPathSet *allTrackedVariabilityPaths)
{
  ...
  if (usdPath.IsAbsoluteRootOrPrimPath()) {
    ...
    else if (attrName == UsdGeomTokens->visibility || attrName == UsdGeomTokens->purpose ||
             ...
             UsdGeomXformable::IsTransformationAffectedByAttrNamed(attrName))
    {
    ...
    else if (usdPrim && (usdPrim.IsA<UsdShadeShader>() || usdPrim.IsA<UsdShadeNodeGraph>())) {
      ...
      while (usdPrim && !usdPrim.IsA<UsdShadeMaterial>() && !usdPrim.IsPrototype()) {
    ...
    for (auto &pair : _hdPrimInfoMap) {
      ...
      if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
        primInfo.adapter->MarkDirty(primInfo.usdPrim,
                                    cachePath,
                                    HdLight::DirtyParams | HdLight::DirtyResource,
                                    &indexProxy);
      }
    }
  }
  ...
  if (TF_VERIFY(primInfo)) {
    ...
    if (topology.IsHolding<HdMeshTopology>()) {
      ...
      for (const HdGeomSubset &subset : meshTopology.GetGeomSubsets()) {
        geomSubsets.push_back(HdGeomSubset{subset.type,
...
/*virtual*/
HdModelDrawMode UsdImagingDelegate::GetModelDrawMode(SdfPath const &id)
{
...
void UsdImagingDelegate::SetRootInstancerId(SdfPath const &instancerId)
{
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
      primInfo.adapter->MarkDirty(
          primInfo.usdPrim, cachePath, HdChangeTracker::DirtyInstancer, &indexProxy);
    }
  }
}
...
VtArray<TfToken> UsdImagingDelegate::GetCategories(SdfPath const &id)
{
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (primInfo && primInfo->adapter && primInfo->adapter->IsInstancerAdapter() &&
...
SdfPath UsdImagingDelegate::GetMaterialId(SdfPath const &rprimId)
{
  ...
  if (TF_VERIFY(primInfo, "No primInfo for <%s> <%s>", rprimId.GetText(), cachePath.GetText())) {
...
VtValue UsdImagingDelegate::GetLightParamValue(SdfPath const &id, TfToken const &paramName)
{
  ...
  return primInfo->adapter->GetLightParamValue(primInfo->usdPrim, cachePath, paramName, _time);
}
...
HdExtComputationPrimvarDescriptorVector UsdImagingDelegate::GetExtComputationPrimvarDescriptors(
    SdfPath const &computationId, HdInterpolation interpolation)
{
  HD_TRACE_FUNCTION();
  SdfPath cachePath = ConvertIndexPathToCachePath(computationId);
