...
const HdSceneIndexPrim &UsdImaging_DrawModeStandin::GetPrim() const
{
  static HdSceneIndexPrim empty{TfToken(), nullptr};
  return empty;
}

HdSceneIndexPrim UsdImaging_DrawModeStandin::GetDescendantPrim(const SdfPath &path) const
{
  if (path.IsAbsolutePath() && path.HasPrefix(_path)) {
    const SdfPath relPath = path.MakeRelativePath(_path);
    return {_GetDescendantPrimType(relPath), _GetDescendantPrimSource(relPath)};
  }
  if (!path.IsAbsolutePath()) {
    return {_GetDescendantPrimType(path), _GetDescendantPrimSource(path)};
  }
  return {TfToken(), nullptr};
}

SdfPathVector UsdImaging_DrawModeStandin::GetDescendantPrimPaths() const
{
...
void UsdImaging_DrawModeStandin::ComputePrimAddedEntries(
    HdSceneIndexObserver::AddedPrimEntries *entries) const
{
  entries->push_back({_path, TfToken()});
  const SdfPathVector &relPaths = _GetDescendantPaths();
  for (const SdfPath &relPath : relPaths) {
    entries->push_back({_path.AppendPath(relPath), _GetDescendantPrimType(relPath)});
  }
}

void UsdImaging_DrawModeStandin::ComputePrimRemovedEntries(
...
TF_DEFINE_PRIVATE_TOKENS(_UsdUVTextureTokens,

                         (fallback)(file)(st)(wrapS)(wrapT)

                             (rgb)(a)(clamp));

TF_DEFINE_PRIVATE_TOKENS(_UsdPrimvarReaderTokens,

                         (fallback)(varname)(result));

TF_DEFINE_PRIVATE_TOKENS(_UsdPreviewSurfaceTokens,

                         (diffuseColor)(opacity)(opacityThreshold));
...
class _DisplayColorVec3fDataSource final : public HdVec3fDataSource {
 public:
  ...
  {
    if (HdVec3fDataSourceHandle src = _schema.GetDrawModeColor()) {
      return src->GetContributingSampleTimesForInterval(startTime, endTime, outSampleTimes);
    }
 ...
 private:
  _DisplayColorVec3fDataSource(const UsdImagingGeomModelSchema schema) : _schema(schema) {}
...
class _Vec4fFromVec3fDataSource final : public HdVec4fDataSource {
 public:
  ...
  bool GetContributingSampleTimesForInterval(Time startTime,
                                             ...
                                             std::vector<Time> *outSampleTimes)
  {
    return _vec3fSource->GetContributingSampleTimesForInterval(startTime, endTime, outSampleTimes);
  }

 private:
  _Vec4fFromVec3fDataSource(const HdVec3fDataSourceHandle source, const float alpha)
      : _vec3fSource(source), _alpha(alpha)
  {
  }
...
class _PrimvarsDataSource : public HdContainerDataSource {
 public:
  TfTokenVector GetNames() override
  {
    return {HdPrimvarsSchemaTokens->widths, HdTokens->displayColor, HdTokens->displayOpacity};
  }

  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdPrimvarsSchemaTokens->widths) {
      static HdDataSourceBaseHandle const src = _PrimvarDataSource::New(
          HdRetainedTypedSampledDataSource<VtFloatArray>::New(VtFloatArray{1.0f}),
    ...
    if (name == HdTokens->displayColor) {
      ...
      return _PrimvarDataSource::New(
          _DisplayColorVec3fDataSource::New(UsdImagingGeomModelSchema::GetFromParent(_primSource)),
          HdPrimvarSchemaTokens->constant,
 ...
 protected:
  _PrimvarsDataSource(const HdContainerDataSourceHandle &primSource) : _primSource(primSource) {}
...
class _PrimDataSource : public HdContainerDataSource {
    ...
    if (name == HdLegacyDisplayStyleSchemaTokens->displayStyle) {
      static const HdDataSourceBaseHandle src =
          HdLegacyDisplayStyleSchema::Builder()
              .SetCullStyle(
                  HdRetainedTypedSampledDataSource<TfToken>::New(HdCullStyleTokens->back))
              .Build();
      return src;
    }
    return nullptr;
 ...
 protected:
  _PrimDataSource(const HdContainerDataSourceHandle &primSource) : _primSource(primSource) {}
...
TF_DEFINE_PRIVATE_TOKENS(_primNameTokens,
...
class _BoundsPointsPrimvarValueDataSource final : public HdVec3fArrayDataSource {
 ...
 private:
  _BoundsPointsPrimvarValueDataSource(const HdContainerDataSourceHandle &primSource)
...
class _BoundsPrimvarsDataSource final : public _PrimvarsDataSource {
 public:
  ...
  TfTokenVector GetNames() override
  {
    static const TfTokenVector result = _Concat(_PrimvarsDataSource::GetNames(),
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdPrimvarsSchemaTokens->points) {
      return _PrimvarDataSource::New(_BoundsPointsPrimvarValueDataSource::New(_primSource),
 ...
 private:
  _BoundsPrimvarsDataSource(const HdContainerDataSourceHandle &primSource)
...
class _BoundsPrimDataSource : public _PrimDataSource {
 ...
 private:
  _BoundsPrimDataSource(const HdContainerDataSourceHandle &primSource)
...
class _BoundsStandin : public UsdImaging_DrawModeStandin {
 public:
  _BoundsStandin(const SdfPath &path, const HdContainerDataSourceHandle &primSource)
      : UsdImaging_DrawModeStandin(path, primSource)
  {
  }

  const SdfPathVector _GetDescendantPaths() const override
  {
    static const SdfPathVector paths{SdfPath(_primNameTokens->boundsCurves)};
    return paths;
  }

  TfToken _GetDescendantPrimType(const SdfPath & /*path*/) const override
  {
    return HdPrimTypeTokens->basisCurves;
  }

  HdContainerDataSourceHandle _GetDescendantPrimSource(const SdfPath & /*path*/) const override
  {
    return _BoundsPrimDataSource::New(_primSource);
  }

  void ProcessDirtyLocators(const HdDataSourceLocatorSet &dirtyLocators,
                            ...
                            bool *needsRefresh) override
  {
    ...
    const bool dirtyExtent = dirtyLocators.Intersects(HdExtentSchema::GetDefaultLocator());
    ...
    const bool dirtyColor = dirtyLocators.Intersects(colorLocator);
    ...
    if (dirtyExtent || dirtyColor) {
      ...
      if (dirtyExtent) {
        ...
        static const HdDataSourceLocator pointsValue = HdPrimvarsSchema::GetPointsLocator().Append(
...
class _OriginPrimvarsDataSource : public _PrimvarsDataSource {
 public:
  ...
  {
    if (name == HdPrimvarsSchemaTokens->points) {
      static const HdDataSourceBaseHandle src = _PrimvarDataSource::New(
 ...
 private:
  _OriginPrimvarsDataSource(const HdContainerDataSourceHandle &primSource)
      : _PrimvarsDataSource(primSource)
  {
  }
};

HdContainerDataSourceHandle _ComputeOriginTopology()
{
...
class _OriginPrimDataSource : public _PrimDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdBasisCurvesSchemaTokens->basisCurves) {
      static const HdDataSourceBaseHandle basisCurvesSrc =
          HdBasisCurvesSchema::Builder().SetTopology(_ComputeOriginTopology()).Build();
      return basisCurvesSrc;
    }
    if (name == HdPrimvarsSchemaTokens->primvars) {
 ...
 private:
  _OriginPrimDataSource(const HdContainerDataSourceHandle &primSource)
      : _PrimDataSource(primSource)
  {
  }
};
...
class _OriginStandin : public UsdImaging_DrawModeStandin {
 public:
  _OriginStandin(const SdfPath &path, const HdContainerDataSourceHandle &primSource)
      : UsdImaging_DrawModeStandin(path, primSource)
  {
  }
  ...
  const SdfPathVector _GetDescendantPaths() const override
  {
    static const SdfPathVector paths{SdfPath(_primNameTokens->originCurves)};
    return paths;
  }
...
TF_DEFINE_PRIVATE_TOKENS(_primvarNameTokens,

                         (cardsUv)(displayRoughness));

TF_DEFINE_PRIVATE_TOKENS(_materialNodeNameTokens, (cardSurface)(cardTexture)(cardUvCoords));

TF_DEFINE_PRIVATE_TOKENS(_imageMetadataTokens,

                         (worldtoscreen)(worldToNDC));

// Helper to produce, e.g., FooXPosBar.
std::array<TfToken, 6> _AddAxesToNames(const std::string &prefix, const std::string &postfix)
{
  return {TfToken(prefix + "XPos" + postfix),
...
using _MaterialsDict =
    std::unordered_map<TfToken, HdContainerDataSourceHandle, TfToken::HashFunctor>;
...
class _CardsDataCache {
 public:
  _CardsDataCache(const SdfPath &primPath, const HdContainerDataSourceHandle &primSource)
      : _primPath(primPath), _primSource(primSource)
  {
  }

  /// Card geometry, that is, box, cross, or fromTexture.
  TfToken GetCardGeometry()
  {
    return _GetCardsData()->cardGeometry;
  }
  ...
  VtVec3fArray GetPoints()
  {
    return _GetCardsData()->points;
  }
  ...
  HdContainerDataSourceHandle GetExtent()
  {
    return _GetCardsData()->extent;
  }

  /// The value for the cardsUV primvar.
  HdDataSourceBaseHandle GetUVs()
  {
    return _GetCardsData()->uvs;
  }

  /// An individual face geometry subset.
  HdContainerDataSourceHandle GetGeomSubset(const TfToken &name)
  {
  ...
  /// The topology.
  HdContainerDataSourceHandle GetMeshTopology()
  {
    return _GetCardsData()->meshTopology;
  }

  /// The materials.
  const _MaterialsDict &GetMaterials()
  {
    return _GetCardsData()->materials;
  }
 ...
 private:
  ...
  struct _CardsData {
    ...
    HdContainerDataSourceHandle GetSubset(const TfToken &name)
    {
   ...
   private:
    static VtVec3fArray _ComputePoints(const _SchemaValues &values);
    static VtVec2fArray _ComputeUVs(const _SchemaValues &values);
    static SubsetMap _ComputeGeomSubsets(const _SchemaValues &values, const SdfPath &primPath);
    static const _MaterialsDict _ComputeMaterials(const _SchemaValues &values);
    static HdContainerDataSourceHandle _ComputeExtent(const TfToken &cardGeometry,
  ...
  std::shared_ptr<_CardsData> _GetCardsData()
  {
    ...
    auto data = std::make_shared<_CardsData>(
        _SchemaValues(UsdImagingGeomModelSchema::GetFromParent(_primSource)), _primPath);
...
bool GetWorldToScreenFromImageMetadata(HdAssetPathDataSourceHandle const &src,
                                       GfMatrix4d *const mat)
{
  ...
  if (!img->GetMetadata(_imageMetadataTokens->worldtoscreen, &worldtoscreen)) {
    if (img->GetMetadata(_imageMetadataTokens->worldToNDC, &worldtoscreen)) {
      TF_WARN(
          "The texture asset '%s' may have been authored by an "
      ...
      TF_WARN(
          "The texture asset '%s' lacks a worldtoscreen matrix in "
  ...
  if (cardGeometry == UsdGeomTokens->fromTexture) {
    for (size_t i = 0; i < 3; i++) {
      for (size_t j = 0; j < 2; j++) {
        ...
        if (GetWorldToScreenFromImageMetadata(texturePaths[k], &worldToScreen[k])) {
          hasTexture[k] = true;
          hasFace[k] = true;
        }
      }
...
HdContainerDataSourceHandle _DisjointQuadTopology(const size_t n)
{
  return HdMeshTopologySchema::Builder()
      .SetFaceVertexCounts(HdRetainedTypedSampledDataSource<VtIntArray>::New(VtIntArray(n, 4)))
      .SetFaceVertexIndices(HdRetainedTypedSampledDataSource<VtIntArray>::New(_Range(4 * n)))
      .SetOrientation(
          HdRetainedTypedSampledDataSource<TfToken>::New(HdMeshTopologySchemaTokens->rightHanded))
      .Build();
}

/// Compute cached data from schema values.
_CardsDataCache::_CardsData::_CardsData(const _SchemaValues &values, const SdfPath &primPath)
...
GfVec3f _Transform(const GfVec3f &v, size_t i)
{
  switch (i) {
...
VtVec3fArray _CardsDataCache::_CardsData::_ComputePoints(const _SchemaValues &values)
{
  VtVec3fArray points;
  ...
  if (values.cardGeometry == UsdGeomTokens->fromTexture) {
    ...
    const GfVec3f pts[4] = {{x, 1, 1}, {x, 0, 1}, {x, 0, 0}, {x, 1, 0}};
    ...
    for (size_t i = 0; i < 3; i++) {
      if (values.hasFace[i]) {
        ...
        for (size_t k = 0; k < 4; k++) {
          ...
          // required axis.
          points.push_back(_Transform(pts[k], i));
        }
      }
      if (values.hasFace[i + 3]) {
        ...
        for (size_t k = 0; k < 4; k++) {
          ...
          points.push_back(one - _Transform(pts[3 - k], i));
        }
      }
    }
  }
...
_CardsDataCache::_CardsData::SubsetMap _CardsDataCache::_CardsData::_ComputeGeomSubsets(
    const _SchemaValues &values, const SdfPath &primPath)
{
  static const std::array<TfToken, 6> subsetNameTokens = _AddAxesToNames("subset", "");
  static const std::array<TfToken, 6> materialNameTokens = _AddAxesToNames("subsetMaterial", "");
  ...
  if (values.hasTexture.count()) {
    ...
    for (size_t i = 0; i < 6; i++) {
      ...
      static HdTokenDataSourceHandle const typeSource = HdGeomSubsetSchema::BuildTypeDataSource(
          HdGeomSubsetSchemaTokens->typeFaceSet);
      static const HdBoolDataSourceHandle trueSource = HdRetainedTypedSampledDataSource<bool>::New(
          true);
      ...
      static const TfToken materialBindingNames[] = {HdMaterialBindingsSchemaTokens->allPurpose};
          ...
          {subsetNameTokens[vi],
           HdRetainedContainerDataSource::New(
               ...
               HdGeomSubsetSchema::Builder()
                   .SetType(typeSource)
                   .SetIndices(HdRetainedTypedSampledDataSource<VtIntArray>::New({subsetIndex}))
                   .Build(),

               HdVisibilitySchema::GetSchemaToken(),
               HdVisibilitySchema::Builder().SetVisibility(trueSource).Build(),

               HdMaterialBindingsSchema::GetSchemaToken(),
               HdMaterialBindingsSchema::BuildRetained(TfArraySize(materialBindingNames),
...
HdContainerDataSourceHandle _CardsDataCache::_CardsData::_ComputeExtent(
    const TfToken &cardGeometry, const VtVec3fArray &points)
{
  if (cardGeometry != UsdGeomTokens->fromTexture) {
...
GfVec2f _GetUV(const float u, const float v, const bool flipU, const bool flipV)
{
  return GfVec2f(flipU ? 1.0f - u : u, flipV ? 1.0f - v : v);
}

void _FillUVs(const bool flipU, const bool flipV, VtVec2fArray *uvs)
{
...
VtVec2fArray _CardsDataCache::_CardsData::_ComputeUVs(const _SchemaValues &values)
{
  VtVec2fArray uvs;
...
HdDataSourceBaseHandle _ComputeConnection(const TfToken &nodeName, const TfToken &outputName)
{
  HdDataSourceBaseHandle srcs[] = {
      HdMaterialConnectionSchema::Builder()
          .SetUpstreamNodePath(HdRetainedTypedSampledDataSource<TfToken>::New(nodeName))
          .SetUpstreamNodeOutputName(HdRetainedTypedSampledDataSource<TfToken>::New(outputName))
          .Build()};
  return HdRetainedSmallVectorDataSource::New(TfArraySize(srcs), srcs);
}
...
HdDataSourceBaseHandle _CardsTextureNode(const HdAssetPathDataSourceHandle &file,
                                         const HdDataSourceBaseHandle &fallback)
{
  ...
  const HdDataSourceBaseHandle inputConnections[] = {
      _ComputeConnection(_materialNodeNameTokens->cardUvCoords, _UsdPrimvarReaderTokens->result)};

  static const TfToken paramsNames[] = {_UsdUVTextureTokens->wrapS,
  ...
  const HdDataSourceBaseHandle paramsValues[] = {
      HdMaterialNodeParameterSchema::Builder()
          .SetValue(HdRetainedTypedSampledDataSource<TfToken>::New(_UsdUVTextureTokens->clamp))
          .Build(),
      HdMaterialNodeParameterSchema::Builder()
          .SetValue(HdRetainedTypedSampledDataSource<TfToken>::New(_UsdUVTextureTokens->clamp))
          .Build(),
      HdMaterialNodeParameterSchema::Builder()
          ...
          .Build(),
      HdMaterialNodeParameterSchema::Builder().SetValue(file).Build(),
      HdMaterialNodeParameterSchema::Builder()
          .SetValue(HdRetainedTypedSampledDataSource<TfToken>::New(_primvarNameTokens->cardsUv))
          .Build()};

  return HdMaterialNodeSchema::Builder()
      .SetNodeIdentifier(
          HdRetainedTypedSampledDataSource<TfToken>::New(UsdImagingTokens->UsdUVTexture))
      .SetParameters(
          HdRetainedContainerDataSource::New(TfArraySize(paramsNames), paramsNames, paramsValues))
      .SetInputConnections(HdRetainedContainerDataSource::New(
          TfArraySize(inputConnectionNames), inputConnectionNames, inputConnections))
      .Build();
}

HdDataSourceBaseHandle _CardsSurfaceNode(const bool hasTexture,
                                         const HdDataSourceBaseHandle &fallback)
{
  static const HdTokenDataSourceHandle nodeIdentifier =
      HdRetainedTypedSampledDataSource<TfToken>::New(UsdImagingTokens->UsdPreviewSurface);
  ...
  static const HdDataSourceBaseHandle pointOne =
      HdMaterialNodeParameterSchema::Builder()
          ...
          .Build();
  static const HdDataSourceBaseHandle fallbackParam = HdMaterialNodeParameterSchema::Builder()
                                                          .SetValue(
                                                              HdSampledDataSource::Cast(fallback))
                                                          .Build();
  ...
  if (hasTexture) {
    ...
    inputConnections.push_back(
        _ComputeConnection(_materialNodeNameTokens->cardTexture, _UsdUVTextureTokens->rgb));
    inputConnectionNames.push_back(_UsdPreviewSurfaceTokens->opacity);
    inputConnections.push_back(
        _ComputeConnection(_materialNodeNameTokens->cardTexture, _UsdUVTextureTokens->a));
  ...
  return HdMaterialNodeSchema::Builder()
      ...
      .SetParameters(HdRetainedContainerDataSource::New(
          parameterNames.size(), parameterNames.data(), parameters.data()))
      .SetInputConnections(HdRetainedContainerDataSource::New(
          inputConnectionNames.size(), inputConnectionNames.data(), inputConnections.data()))
      .Build();
}

HdDataSourceBaseHandle _CardsUVNode()
{
...
const _MaterialsDict _CardsDataCache::_CardsData::_ComputeMaterials(const _SchemaValues &values)
{
  static const std::array<TfToken, 6> materialNameTokens = _AddAxesToNames("subsetMaterial", "");

  const HdDataSourceBaseHandle vec4Fallback = _Vec4fFromVec3fDataSource::New(values.drawModeColor,
                                                                             1.0f);
  ...
  if (values.hasTexture.count()) {
    for (auto i = 0; i < 6; ++i) {
      ...
      if (values.hasTexture[i]) {
        ...
        nodes.push_back(_CardsSurfaceNode(values.hasTexture[i], values.drawModeColor));
        // Card Texture
        nodeNames.push_back(_materialNodeNameTokens->cardTexture);
        nodes.push_back(_CardsTextureNode(values.texturePaths[i], vec4Fallback));
        // Card UvCords
        ...
        const HdContainerDataSourceHandle terminals = HdRetainedContainerDataSource::New(
            HdMaterialTerminalTokens->surface,
            HdMaterialConnectionSchema::Builder()
                .SetUpstreamNodePath(HdRetainedTypedSampledDataSource<TfToken>::New(
                    _materialNodeNameTokens->cardSurface))
                .SetUpstreamNodeOutputName(HdRetainedTypedSampledDataSource<TfToken>::New(
                    HdMaterialTerminalTokens->surface))
                .Build());
        ...
        networks.push_back(HdMaterialNetworkSchema::Builder()
                               ...
                               .Build());
        materials.emplace(materialNameTokens[i],
                          HdRetainedContainerDataSource::New(
                              HdMaterialSchemaTokens->material,
                              HdMaterialSchema::BuildRetained(
                                  networkNames.size(), networkNames.data(), networks.data())));
      }
    }
  }
  return materials;
...
class _CardsPointsPrimvarValueDataSource : public HdVec3fArrayDataSource {
 ...
 private:
  _CardsPointsPrimvarValueDataSource(const HdContainerDataSourceHandle &primSource,
                                     const _CardsDataCacheSharedPtr &dataCache)
      : _primSource(primSource), _dataCache(dataCache)
  {
  }
...
class _CardsPrimvarsDataSource : public _PrimvarsDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    if (name == HdPrimvarsSchemaTokens->points) {
      return _PrimvarDataSource::New(
          _CardsPointsPrimvarValueDataSource::New(_primSource, _dataCache),
          HdPrimvarSchemaTokens->vertex,
          HdPrimvarSchemaTokens->point);
    }
    if (name == _primvarNameTokens->cardsUv) {
      return _PrimvarDataSource::New(
          _dataCache->GetUVs(), HdPrimvarSchemaTokens->vertex, TfToken());
    }
 ...
 private:
  _CardsPrimvarsDataSource(const HdContainerDataSourceHandle &primSource,
                           const _CardsDataCacheSharedPtr &dataCache)
      : _PrimvarsDataSource(primSource), _dataCache(dataCache)
  {
  }
...
class _CardsPrimDataSource : public _PrimDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (name == HdExtentSchemaTokens->extent) {
      if (HdContainerDataSourceHandle const src = _dataCache->GetExtent()) {
        return src;
      }
 ...
 private:
  _CardsPrimDataSource(const SdfPath &path,
                       const HdContainerDataSourceHandle &primSource,
                       const _CardsDataCacheSharedPtr &dataCache)
      : _PrimDataSource(primSource), _path(path), _dataCache(dataCache)
  {
  }
...
HdDataSourceLocatorSet _ComputeMaterialColorInputLocators()
{
  static const auto nodes = HdDataSourceLocator(HdMaterialSchemaTokens->universalRenderContext)
                                .Append(HdMaterialNetworkSchemaTokens->nodes);
  return {nodes.Append(_materialNodeNameTokens->cardTexture)
              .Append(HdMaterialNodeSchemaTokens->parameters)
              .Append(_UsdUVTextureTokens->fallback),
          nodes.Append(_materialNodeNameTokens->cardSurface)
              .Append(HdMaterialNodeSchemaTokens->parameters)
...
class _CardsStandin : public UsdImaging_DrawModeStandin {
 public:
  _CardsStandin(const SdfPath &path, const HdContainerDataSourceHandle &primSource)
      : UsdImaging_DrawModeStandin(path, primSource),
                            ...
                            bool *needsRefresh) override
  {
    ...
    if (dirtyLocators.Intersects(colorLocator)) {
      ...
      primDirtyLocators.insert(displayColorValue);
      entries->push_back({_path.AppendChild(_primNameTokens->cardsMesh), primDirtyLocators});
      static const HdDataSourceLocatorSet materialColorInputs =
          _ComputeMaterialColorInputLocators();
      for (const auto &kv : _dataCache->GetMaterials()) {
        entries->push_back({_path.AppendChild(kv.first), materialColorInputs});
      }
      return;
    }

    entries->push_back({_path.AppendChild(_primNameTokens->cardsMesh), dirtyLocators});
  }
...
UsdImaging_DrawModeStandinSharedPtr UsdImaging_GetDrawModeStandin(
    const TfToken &drawMode, const SdfPath &path, const HdContainerDataSourceHandle &primSource)
{
  if (drawMode.IsEmpty()) {
    return nullptr;
  }
