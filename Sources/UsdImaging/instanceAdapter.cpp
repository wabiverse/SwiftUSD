                                             ...
                                             SdfPath const &parentProxyPath)
{
  ...
  if (instancerData.instancePaths.empty()) {
    ...
    for (auto iter = range.begin(); iter != range.end(); ++iter) {
      ...
      const SdfPath newProtoPath = _InsertProtoPrim(&iter, primAdapter, ctx, index);
      if (!TF_VERIFY(protoPath == newProtoPath,
                     "protoPath changed from "
                     "<%s> to <%s>",
                     protoPath.GetText(),
  ...
  while (!depInstancePaths.empty()) {
    ...
    if (index->IsPopulated(depInstancerPath)) {
      ...
      index->MarkInstancerDirty(
          depInstancerPath, HdChangeTracker::DirtyPrimvar | HdChangeTracker::DirtyInstanceIndex);
    ...
    UsdImagingInstancerContext const *instancerContext) const
{
  if (_IsChildPrim(prim, cachePath)) {
    ...
    _ProtoPrim const &proto = _GetProtoPrim(instancerPath, cachePath, &instancerContext);
...
size_t UsdImagingInstanceAdapter::_CountAllInstancesToDrawImpl(
    UsdPrim const &instancer, _InstancerDrawCounts *drawCounts) const
{
  // NOTE: This logic is almost exactly similar to the logic in
...
bool UsdImagingInstanceAdapter::_GatherInstanceTransformsTimeSamples(
    UsdPrim const &instancer, GfInterval interval, std::vector<double> *outTimes) const
{
  HD_TRACE_FUNCTION();
                                                         ...
                                                         UsdTimeCode time) const
{
  ...
  if (dv.IsHolding<GfHalf>()) {
    ...
    return _ComputeInheritedPrimvar<SdfAssetPath>(instancer, primvarName, result, time);
  }
  else {
    TF_WARN(
...
struct UsdImagingInstanceAdapter::_GetInstanceCategoriesFn {
  _GetInstanceCategoriesFn(const UsdImagingInstanceAdapter *adapter,
                           const UsdImaging_CollectionCache *cc,
                           std::vector<VtTokenArray> *result)
      : _adapter(adapter), _cc(cc), _result(result)
  {
  }
                                                            ...
                                                            UsdTimeCode time) const
{
  ...
  if (_GetProtoPrimForChild(instancerPrim, instancerPath, &proto, &instancerContext)) {
    return proto->adapter->GetInstancerTransform(_GetPrim(proto->path), instancerPath, time);
  }
...
bool UsdImagingInstanceAdapter::IsChildPath(const SdfPath &path) const
{
  TRACE_FUNCTION();
...
/*virtual*/
VtValue UsdImagingInstanceAdapter::GetLightParamValue(const UsdPrim &prim,
...
/*virtual*/
VtValue UsdImagingInstanceAdapter::GetMaterialResource(const UsdPrim &prim,
                                                       const SdfPath &cachePath,
                                                       UsdTimeCode time) const
{
  UsdImagingInstancerContext instancerContext;
...
UsdImagingInstanceAdapter::_ProtoPrim const &UsdImagingInstanceAdapter::_GetProtoPrim(
    SdfPath const &instancerPath, SdfPath const &cachePath, UsdImagingInstancerContext *ctx) const
{
...
bool UsdImagingInstanceAdapter::_ComputeInstanceMapVariability(
    UsdPrim const &instancerPrim, _InstancerData const &instrData) const
{
...
struct UsdImagingInstanceAdapter::_ComputeInstanceMapFn {
  _ComputeInstanceMapFn(const UsdImagingInstanceAdapter *adapter_,
                        ...
                        VtIntArray *indices_)
      : adapter(adapter_), time(time_), visibility(visibility_), indices(indices_)
  {
  }
...
struct UsdImagingInstanceAdapter::_GetScenePrimPathsFn {
  ...
  bool operator()(const std::vector<UsdPrim> &instanceContext, size_t instanceIdx)
  {
    ...
    if (instanceIndices[instanceIdxShifted] != (std::numeric_limits<int>::max)()) {
      ...
      result[instanceIndices[instanceIdxShifted]] = adapter->_GetPrimPathFromInstancerChain(
          instanceChain);
    }
    return true;
  }
  ...
  if (_IsChildPrim(usdPrim, cachePath)) {
    ...
    if (!instanceIndices.empty()) {
      ...
      for (size_t i = 0; i < instanceIndices.size(); i++) {
        const size_t instanceIndex = static_cast<size_t>(instanceIndices[i]);
      ...
      if (validIndices > 0) {
        ...
        _GetScenePrimPathsFn primPathsFn(this, requestedIndicesMap, minIdx, result, proto.path);
    ...
    if (!instanceIndices.empty()) {
      ...
      _GetScenePrimPathsFn primPathsFn(
          this, requestedIndicesMap, minIdx, result, SdfPath::EmptyPath());
  ...
  bool operator()(const std::vector<UsdPrim> &instanceContext, size_t instanceIdx)
  {
    ...
    if (selectionCount == selectionPathVec.size()) {
      for (auto const &pair : instrData->primMap) {
        UsdPrim prefixPrim = adapter->_GetPrim(pair.first.GetParentPath());
      ...
      for (auto const &pair : instrData->primMap) {
        if (pair.second.path.HasPrefix(selectionPathVec[selectionCount])) {
          ...
          selectionPrim = adapter->_GetPrim(pair.first.GetParentPath());
        }
    ...
    UsdPrim const &usdPrim, SdfPath const &id, UsdTimeCode time) const
{
  if (IsChildPath(id)) {
    ...
    _ProtoPrim const &proto = _GetProtoPrim(instancerPath, id, &instancerContext);
...
GfMatrix4d UsdImagingInstanceAdapter::GetRelativeInstancerTransform(
    SdfPath const &parentInstancerPath, SdfPath const &instancerPath, UsdTimeCode time) const
{
