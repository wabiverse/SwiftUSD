...
UsdImagingDataSourceSubdivisionTags::UsdImagingDataSourceSubdivisionTags(
    UsdGeomMesh usdMesh, const UsdImagingDataSourceStageGlobals &stageGlobals)
    : _usdMesh(usdMesh), _stageGlobals(stageGlobals)
{
}
...
UsdImagingDataSourceMeshTopology::UsdImagingDataSourceMeshTopology(
    ...
    : _sceneIndexPath(sceneIndexPath), _usdMesh(usdMesh), _stageGlobals(stageGlobals)
{
}
...
HdDataSourceBaseHandle UsdImagingDataSourceMeshTopology::Get(const TfToken &name)
{
  if (name == HdMeshTopologySchemaTokens->faceVertexCounts) {
    ...
    return UsdImagingDataSourceAttribute<VtIntArray>::New(
        _usdMesh.GetFaceVertexCountsAttr(), _stageGlobals, _sceneIndexPath, locator);
  }
  else if (name == HdMeshTopologySchemaTokens->faceVertexIndices) {
    static const HdDataSourceLocator locator = HdMeshTopologySchema::GetDefaultLocator().Append(
        name);
    return UsdImagingDataSourceAttribute<VtIntArray>::New(
        _usdMesh.GetFaceVertexIndicesAttr(), _stageGlobals, _sceneIndexPath, locator);
  }
  else if (name == HdMeshTopologySchemaTokens->holeIndices) {
    static const HdDataSourceLocator locator = HdMeshTopologySchema::GetDefaultLocator().Append(
        name);
    return UsdImagingDataSourceAttribute<VtIntArray>::New(
        _usdMesh.GetHoleIndicesAttr(), _stageGlobals, _sceneIndexPath, locator);
  }
...
HdDataSourceBaseHandle UsdImagingDataSourceMeshPrim::Get(const TfToken &name)
{
  if (name == HdMeshSchemaTokens->mesh) {
    return UsdImagingDataSourceMesh::New(
        _GetSceneIndexPath(), UsdGeomMesh(_GetUsdPrim()), _GetStageGlobals());
  }
  else {
...
/*static*/
HdDataSourceLocatorSet UsdImagingDataSourceMeshPrim::Invalidate(
    UsdPrim const &prim,
