...
class _MaterialBindingsDataSource : public HdContainerDataSource {
 public:
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (HdDataSourceBaseHandle const bindingDs = _primBindings->Get(purpose)) {
      return bindingDs;
    }
    return parentSchema.GetContainer();
  }
...
HdContainerDataSourceHandle UsdImagingFlattenedDirectMaterialBindingsDataSourceProvider::
    GetFlattenedDataSource(const Context &ctx) const
{
  return _MaterialBindingsDataSource::UseOrCreateNew(ctx.GetInputDataSource(),
                                                     ctx.GetFlattenedDataSourceFromParentPrim());
}

void UsdImagingFlattenedDirectMaterialBindingsDataSourceProvider::
    ComputeDirtyLocatorsForDescendants(HdDataSourceLocatorSet *const locators) const
{
