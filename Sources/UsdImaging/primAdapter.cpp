...
TfTokenVector UsdImagingPrimAdapter::GetImagingSubprims(UsdPrim const &prim)
{
...
TfToken UsdImagingPrimAdapter::GetImagingSubprimType(UsdPrim const &prim, TfToken const &subprim)
{
  return TfToken();
}
...
HdDataSourceLocatorSet UsdImagingPrimAdapter::InvalidateImagingSubprim(
    UsdPrim const &prim,
    TfToken const &subprim,
...
UsdImagingPrimAdapter::PopulationMode UsdImagingPrimAdapter::GetPopulationMode()
{
  return RepresentsSelf;
}

HdDataSourceLocatorSet UsdImagingPrimAdapter::InvalidateImagingSubprimFromDescendent(
    UsdPrim const &prim,
    ...
    const UsdImagingPropertyInvalidationType invalidationType)
{
  return InvalidateImagingSubprim(descendentPrim, subprim, properties, invalidationType);
}
...
/*virtual*/
void UsdImagingPrimAdapter::MarkCollectionsDirty(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
                                                 UsdImagingIndexProxy *index)
{
}
...
static void _GetTimeSamplesForInterval(const UsdAttribute &attr,
  ...
  if (pv && pv.HasValue()) {
    if (pv.ValueMightBeTimeVarying()) {
      ...
      if (sampleIndices && pv.IsIndexed()) {
        ...
        pv.GetIndicesAttr().GetTimeSamplesInInterval(interval, &indicesTimeSamples);
        // Combine them, merge and uniquify
        const size_t mid = timeSamples.size();
        timeSamples.insert(
            timeSamples.end(), indicesTimeSamples.begin(), indicesTimeSamples.end());
        // since both sets are already sorted, inplace_merge beats sort
        std::inplace_merge(timeSamples.begin(), timeSamples.begin() + mid, timeSamples.end());
...
static VtValue _GetUsdPrimAttribute(const UsdPrim &prim, const TfToken &attrName, UsdTimeCode time)
{
  VtValue value;
...
/*static*/
UsdAttribute UsdImagingPrimAdapter::LookupLightParamAttribute(UsdPrim const &prim,
...
VtValue UsdImagingPrimAdapter::GetLightParamValue(const UsdPrim &prim,
                                                  ...
                                                  UsdTimeCode time) const
{
  ...
  if (!light) {
    ...
    if (paramName == HdTokens->lightFilterLink) {
      ...
      return VtValue(collectionCache.GetIdForCollection(lightFilterLink));
    }
    // fallback to usd attributes
...
const UsdImagingPrimAdapterSharedPtr &UsdImagingPrimAdapter::_GetPrimAdapter(
    UsdPrim const &prim, bool ignoreInstancing) const
{
...
TfTokenVector UsdImagingPrimAdapter::_GetRenderSettingsNamespaces() const
{
  return _delegate->GetRenderIndex().GetRenderDelegate()->GetRenderSettingsNamespaces();
}
                                                    ...
                                                    HdInterpolation *interpOverride) const
{
  ...
  if (primvar.Get(&v, time)) {
    HdInterpolation interp = interpOverride ?
                                 *interpOverride :
...
// if necessary
/*static*/
void _ProcessPrimvarChange(bool primvarOnPrim,
...
UsdStageRefPtr UsdImagingPrimAdapter::_GetStage() const
{
...
UsdGeomPrimvar UsdImagingPrimAdapter::_GetInheritedPrimvar(UsdPrim const &prim,
...
SdfPath UsdImagingPrimAdapter::ResolveCachePath(
                                               ...
                                               bool ignoreRootTransform) const
{
  ...
  if (isCoordSys.second && !prim.GetPath().HasPrefix(xformRoot)) {
    TF_WARN(
        "Prim associated with '%s' has path <%s> which is not under "
...
namespace {
template<typename T> T _GetAttrValue(UsdAttribute const &attr, T defaultVal)
{
  if (attr) {
    VtValue val;
    attr.Get(&val);
...
HdModelDrawMode UsdImagingPrimAdapter::GetFullModelDrawMode(UsdPrim const &prim)
{
  ...
  modelDrawMode.drawModeColor = _GetAttrValue<GfVec3f>(geomModelAPI.GetModelDrawModeColorAttr(),
                                                       GfVec3f(0.18));

  modelDrawMode.cardGeometry = _GetAttrValue<TfToken>(geomModelAPI.GetModelCardGeometryAttr(),
                                                      modelDrawMode.cardGeometry);
