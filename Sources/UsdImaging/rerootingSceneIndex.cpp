...
class _RerootingSceneIndexPathDataSource : public HdPathDataSource {
 public:
  ...
  _RerootingSceneIndexPathDataSource(const SdfPath &srcPrefix,
                                     const SdfPath &dstPrefix,
                                     HdPathDataSourceHandle const &inputDataSource)
      : _srcPrefix(srcPrefix), _dstPrefix(dstPrefix), _inputDataSource(inputDataSource)
  {
  }
...
class _RerootingSceneIndexPathArrayDataSource : public HdPathArrayDataSource {
 public:
  ...
  _RerootingSceneIndexPathArrayDataSource(const SdfPath &srcPrefix,
                                          const SdfPath &dstPrefix,
...
class _RerootingSceneIndexContainerDataSource : public HdContainerDataSource {
 public:
  ...
  _RerootingSceneIndexContainerDataSource(const SdfPath &srcPrefix,
                                          const SdfPath &dstPrefix,
  ...
  HdDataSourceBaseHandle Get(const TfToken &name) override
  {
    ...
    if (auto childPathDataSource = HdTypedSampledDataSource<SdfPath>::Cast(childSource)) {
      return _RerootingSceneIndexPathDataSource::New(_srcPrefix, _dstPrefix, childPathDataSource);
    }

    if (auto childPathArrayDataSource = HdTypedSampledDataSource<VtArray<SdfPath>>::Cast(
            childSource))
    {
...
UsdImagingRerootingSceneIndex::UsdImagingRerootingSceneIndex(
    HdSceneIndexBaseRefPtr const &inputScene, const SdfPath &srcPrefix, const SdfPath &dstPrefix)
...
HdSceneIndexPrim UsdImagingRerootingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  if (!primPath.HasPrefix(_dstPrefix)) {
    return {TfToken(), nullptr};
  }

  const SdfPath inputScenePath = _srcEqualsDst ? primPath : _DstPathToSrcPath(primPath);
  ...
  if (prim.dataSource) {
    ...
    if (primPath == _dstPrefix) {
      prim.dataSource = HdOverlayContainerDataSource::New(
          HdSystemSchema::ComposeAsPrimDataSource(_GetInputSceneIndex(), inputScenePath, nullptr),
          prim.dataSource);
    }
  }
...
SdfPathVector UsdImagingRerootingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  ...
  if (primPath.HasPrefix(_dstPrefix)) {
    ...
    SdfPathVector result = _GetInputSceneIndex()->GetChildPrimPaths(_DstPathToSrcPath(primPath));
...
void UsdImagingRerootingSceneIndex::_PrimsAdded(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  if (_srcEqualsDst) {
    ...
    for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
      prefixedEntries.emplace_back(_SrcPathToDstPath(entry.primPath), entry.primType);
    }
  }
  else {
    for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
...
void UsdImagingRerootingSceneIndex::_PrimsRemoved(
...
void UsdImagingRerootingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender, const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  if (_srcEqualsDst) {
    ...
    for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
      prefixedEntries.emplace_back(_SrcPathToDstPath(entry.primPath), entry.dirtyLocators);
    }
  }
  else {
    for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
...
inline SdfPath UsdImagingRerootingSceneIndex::_SrcPathToDstPath(const SdfPath &primPath) const
{
  return primPath.ReplacePrefix(_srcPrefix, _dstPrefix);
}

inline SdfPath UsdImagingRerootingSceneIndex::_DstPathToSrcPath(const SdfPath &primPath) const
{
