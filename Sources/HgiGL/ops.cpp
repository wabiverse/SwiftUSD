...
HgiGLOpsFn HgiGLOps::CopyTextureGpuToCpu(HgiTextureGpuToCpuOp const &copyOp)
{
  return [copyOp] {
    ...
    if (texDesc.usage & HgiTextureUsageBitsDepthTarget) {
      ...
      HgiGLConversions::GetFormat(texDesc.format, texDesc.usage, &glFormat, &glPixelType);
    }
...
HgiGLOpsFn HgiGLOps::CopyTextureToBuffer(HgiTextureToBufferOp const &copyOp)
{
  return [copyOp] {
    ...
    if (HgiIsCompressed(texDesc.format)) {
      ...
      GLenum format = 0;
      GLenum type = 0;
      HgiGLConversions::GetFormat(texDesc.format, texDesc.usage, &format, &type);
      glGetTextureImage(
...
HgiGLOpsFn HgiGLOps::BindVertexBuffers(HgiVertexBufferBindingVector const &bindings)
{
  return [bindings] {
    ...
    for (HgiVertexBufferBinding const &binding : bindings) {
      ...
      glBindVertexBuffer(binding.index, buf->GetBufferId(), binding.byteOffset, desc.vertexStride);
    }
  ...
  return [buffer, value] {
    ...
    if (glBuffer && glBuffer->GetBufferId()) {
      glClearNamedBufferData(
          glBuffer->GetBufferId(), GL_R8UI, GL_RED_INTEGER, GL_UNSIGNED_BYTE, &value);
      HGIGL_POST_PENDING_GL_ERRORS();
    }
  };
}
...
HgiGLOpsFn HgiGLOps::InsertMemoryBarrier(HgiMemoryBarrier barrier)
{
  return [barrier] {
