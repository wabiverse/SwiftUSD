...
static bool _GetSrcTextureDimensionsAndName(HdStSimpleLightingShaderSharedPtr const &shader,
                                            GfVec3i *srcDim,
                                            ...
                                            HgiSamplerHandle *srcSamplerName)
{
  ...
  const HdStUvSamplerObject *const srcSamplerObject = dynamic_cast<HdStUvSamplerObject *>(
...
void HdSt_DomeLightComputationGPU::Execute(HdBufferArrayRangeSharedPtr const &range,
                                           HdResourceRegistry *const resourceRegistry)
{
  ...
  HdStGLSLProgramSharedPtr const computeProgram = HdStGLSLProgram::GetComputeProgram(
      ...
      [&](HgiShaderFunctionDesc &computeDesc) {
        ...
        computeDesc.computeDescriptor.localSize = GfVec3i(localSize, localSize, 1);

        HgiShaderFunctionAddTexture(&computeDesc,
                                    "inTexture",
                                    /*bindIndex = */ 0);
        HgiShaderFunctionAddWritableTexture(&computeDesc,
                                            "outTexture",
                                            /*bindIndex = */ 1,
                                            /*dimensions = */ 2,
                                            HgiFormatFloat16Vec4);
        if (hasUniforms) {
          HgiShaderFunctionAddConstantParam(&computeDesc, "inRoughness", HdStTokens->_float);
        }
  ...
  HdStTextureHandleSharedPtr const &dstTextureHandle = shader->GetTextureHandle(_shaderToken);
  if (!TF_VERIFY(dstTextureHandle)) {
    return;
  }

  HdStDynamicUvTextureObject *const dstUvTextureObject =
      dynamic_cast<HdStDynamicUvTextureObject *>(dstTextureHandle->GetTextureObject().get());
