...
TF_DEFINE_ENV_SETTING(HDST_ENABLE_DRAW_ITEMS_CACHE,
                      false,
                      "Enable usage of the draw items cache in Storm.");

static bool _IsDrawItemsCacheEnabled()
{
...
unsigned int _GetGeomSubsetDrawItemsVersion(HdRenderIndex *renderIndex)
{
  HdStRenderParam *stRenderParam = static_cast<HdStRenderParam *>(
      renderIndex->GetRenderDelegate()->GetRenderParam());
...
HdSt_RenderPass::HdSt_RenderPass(HdRenderIndex *index, HdRprimCollection const &collection)
...
bool HdSt_RenderPass::HasDrawItems(TfTokenVector const &renderTags) const
{
...
void HdSt_RenderPass::_Execute(HdRenderPassStateSharedPtr const &renderPassState,
                               TfTokenVector const &renderTags)
{
  ...
  HgiGraphicsCmdsUniquePtr prepareGfxCmds = _hgi->CreateGraphicsCmds(HgiGraphicsCmdsDesc());
  if (!TF_VERIFY(prepareGfxCmds)) {
    return;
  }

  HdRprimCollection const &collection = GetRprimCollection();
  std::string prepareName = "HdSt_RenderPass: Prepare " + collection.GetMaterialTag().GetString();

  prepareGfxCmds->PushDebugGroup(prepareName.c_str());

  _cmdBuffer.PrepareDraw(prepareGfxCmds.get(), stRenderPassState, GetRenderIndex());
  ...
  std::string passName = "HdSt_RenderPass: " + collection.GetMaterialTag().GetString();
...
void HdSt_RenderPass::_MarkCollectionDirty()
{
...
static HdStDrawItemsCachePtr _GetDrawItemsCache(HdRenderIndex *renderIndex)
{
  HdStRenderDelegate *renderDelegate = static_cast<HdStRenderDelegate *>(
...
void HdSt_RenderPass::_UpdateDrawItems(TfTokenVector const &renderTags)
{
  HD_TRACE_FUNCTION();
  ...
  if (_IsDrawItemsCacheEnabled()) {
    ...
    HdDrawItemConstPtrVectorSharedPtr cachedEntry = cache->GetDrawItems(
  ...
  const unsigned int geomSubsetDrawItemsVersion = _GetGeomSubsetDrawItemsVersion(GetRenderIndex());
  ...
  const bool geomSubsetDrawItemsChanged = _geomSubsetDrawItemsVersion !=
                                          geomSubsetDrawItemsVersion;
  ...
  if (collectionChanged || rprimRenderTagChanged || materialTagsChanged ||
      geomSubsetDrawItemsChanged || taskRenderTagsChanged)
  {
    ...
    if (TfDebug::IsEnabled(HDST_DRAW_ITEM_GATHER)) {
      ...
      if (geomSubsetDrawItemsChanged) {
        TfDebug::Helper::Msg("GeomSubsetDrawItemsChanged (version = %d -> %d)\n",
                             _geomSubsetDrawItemsVersion,
                             geomSubsetDrawItemsVersion);
      }
...
void HdSt_RenderPass::_UpdateCommandBuffer(TfTokenVector const &renderTags)
{
  HD_TRACE_FUNCTION();
