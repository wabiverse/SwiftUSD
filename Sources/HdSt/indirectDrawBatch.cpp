...
HdSt_IndirectDrawBatch::HdSt_IndirectDrawBatch(HdStDrawItemInstance *drawItemInstance,
...
/*virtual*/
void HdSt_IndirectDrawBatch::_Init(HdStDrawItemInstance *drawItemInstance)
{
...
void HdSt_IndirectDrawBatch::SetEnableTinyPrimCulling(bool tinyPrimCulling)
{
...
/* static */
bool HdSt_IndirectDrawBatch::IsEnabledGPUFrustumCulling()
{
  ...
  return isEnabledGPUFrustumCulling && !TfDebug::IsEnabled(HDST_DISABLE_FRUSTUM_CULLING);
}

/* static */
bool HdSt_IndirectDrawBatch::IsEnabledGPUCountVisibleInstances()
{
...
/* static */
bool HdSt_IndirectDrawBatch::IsEnabledGPUInstanceFrustumCulling()
{
...
void _AddInstanceCullResourceViews(HdStDispatchBufferSharedPtr const &cullInput,
                                   _DrawCommandTraits const &traits)
{
  // cull indirect command
  cullInput->AddBufferResourceView(
      HdTokens->drawDispatch, {HdTypeInt32, 1}, traits.cullCount_offset);
  ...
  if (traits.instancerNumLevels > 0) {
    cullInput->AddBufferResourceView(HdTokens->drawingCoordI,
                                     {HdTypeInt32, traits.instancerNumLevels},
                                     traits.drawingCoordI_offset);
  }
  // cull draw index
  cullInput->AddBufferResourceView(
      _tokens->drawCommandIndex, {HdTypeInt32, 1}, traits.baseInstance_offset);
}

void _AddNonInstanceCullResourceViews(HdStDispatchBufferSharedPtr const &cullInput,
                                      _DrawCommandTraits const &traits)
{
  // cull indirect command
  cullInput->AddBufferResourceView(HdTokens->drawDispatch, {HdTypeInt32, 1}, traits.count_offset);
  // cull drawing coord 0
  cullInput->AddBufferResourceView(
      HdTokens->drawingCoord0, {HdTypeInt32Vec4, 1}, traits.drawingCoord0_offset);
  // cull draw index
  ...
  cullInput->AddBufferResourceView(
      _tokens->instanceCountInput, {HdTypeInt32, 1}, traits.instanceCount_offset);
}
...
TextureResourceID _GetTextureResourceID(HdStDrawItem const *drawItem)
{
...
void HdSt_IndirectDrawBatch::_CompileBatch(HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  TRACE_FUNCTION();
  ...
  _DrawCommandTraits const traits = _GetDrawCommandTraits(
      instancerNumLevels, _useDrawIndexed, _useInstanceCulling);
  ...
  TextureResourceID const firstTextureResourceID = _GetTextureResourceID(
  ...
  for (size_t item = 0; item < numDrawItemInstances; ++item) {
    ...
    if (_allowTextureResourceRebinding && !_needsTextureResourceRebinding) {
      TextureResourceID const textureResourceID = _GetTextureResourceID(drawItem);
...
HdSt_DrawBatch::ValidationResult HdSt_IndirectDrawBatch::Validate(bool deepValidation)
{
...
void HdSt_IndirectDrawBatch::_ValidateCompatibility(
    ...
    std::vector<HdStBufferArrayRangeSharedPtr> const &instanceBars) const
{
  ...
  for (HdStDrawItemInstance const *itemInstance : _drawItemInstances) {
    ...
    std::vector<HdStBufferArrayRangeSharedPtr> itmInstanceBars(instancerNumLevels);
    if (instanceIndexBar) {
...
bool HdSt_IndirectDrawBatch::_HasNothingToDraw() const
{
  return (_useDrawIndexed && _numTotalElements == 0) ||
         (!_useDrawIndexed && _numTotalVertices == 0);
}

void HdSt_IndirectDrawBatch::PrepareDraw(HgiGraphicsCmds *,
...
void HdSt_IndirectDrawBatch::EncodeDraw(HdStRenderPassStateSharedPtr const &renderPassState,
                                        HdStResourceRegistrySharedPtr const &resourceRegistry,
                                        bool /*firstDrawBatch*/)
{
  // No implementation.
...
struct _BindingState : public _DrawItemState {
  ...
  void BindResourcesForDrawing(HdStRenderPassStateSharedPtr const &renderPassState,
                               HgiCapabilities const &hgiCapabilities) const;
  void UnbindResourcesForDrawing(HdStRenderPassStateSharedPtr const &renderPassState,
                                 HgiCapabilities const &hgiCapabilities) const;
...
void _BindingState::BindResourcesForViewTransformation() const
{
...
void _BindingState::UnbindResourcesForViewTransformation() const
{
  glUseProgram(0);
...
void _BindingState::BindResourcesForDrawing(HdStRenderPassStateSharedPtr const &renderPassState,
                                            HgiCapabilities const &hgiCapabilities) const
{
  ...
  for (HdStShaderCodeSharedPtr const &shader : shaders) {
    ...
    shader->BindResources(glslProgram->GetProgram()->GetRawResource(), binder);
  }
...
void _BindingState::UnbindResourcesForDrawing(HdStRenderPassStateSharedPtr const &renderPassState,
                                              HgiCapabilities const &hgiCapabilities) const
{
...
void _BindTextureResources(HdStDrawItem const *drawItem,
                           HdSt_ResourceBinder const &binder,
                           HdStGLSLProgramSharedPtr const &glslProgram,
  ...
  if (*currentTextureResourceID != textureResourceID) {
    *currentTextureResourceID = textureResourceID;
    if (HdSt_MaterialNetworkShaderSharedPtr const &materialNetworkShader =
            drawItem->GetMaterialNetworkShader())
    {
      materialNetworkShader->BindResources(glslProgram->GetProgram()->GetRawResource(), binder);
    }
  }
}
...
void HdSt_IndirectDrawBatch::ExecuteDraw(HgiGraphicsCmds *gfxCmds,
                                         HdStRenderPassStateSharedPtr const &renderPassState,
                                         HdStResourceRegistrySharedPtr const &resourceRegistry,
                                         bool /*firstDrawBatch*/)
{
...
void HdSt_IndirectDrawBatch::_ExecuteDraw(HdStRenderPassStateSharedPtr const &renderPassState,
                                          HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  TRACE_FUNCTION();
  GLF_GROUP_FUNCTION();
  ...
  bool const drawIndirect = !_needsTextureResourceRebinding &&
  ...
  if (geometricShader->IsPrimTypePatches()) {
    glPatchParameteri(GL_PATCH_VERTICES, geometricShader->GetPrimitiveIndexSize());
  }

  if (drawIndirect) {
    _ExecuteDrawIndirect(geometricShader, _dispatchBuffer, state.indexBar);
  }
  else {
    _ExecuteDrawImmediate(geometricShader, _dispatchBuffer, state.indexBar, program);
  }
...
void HdSt_IndirectDrawBatch::_ExecuteDrawIndirect(
    HdSt_GeometricShaderSharedPtr const &geometricShader,
    HdStDispatchBufferSharedPtr const &dispatchBuffer,
    HdStBufferArrayRangeSharedPtr const &indexBar)
{
  ...
  GLenum const primitiveMode = HdStGLConversions::GetPrimitiveMode(geometricShader.get());
  uint32_t const stride = dispatchBuffer->GetCommandNumUints() * sizeof(uint32_t);
  ...
  if (!_useDrawIndexed) {
    ...
    glMultiDrawArraysIndirect(primitiveMode, 0, drawCount, stride);
  }
  else {
    TF_DEBUG(HDST_DRAW).Msg(
    ...
    glMultiDrawElementsIndirect(primitiveMode, GL_UNSIGNED_INT, 0, drawCount, stride);
  }
}

void HdSt_IndirectDrawBatch::_ExecuteDrawImmediate(
    HdSt_GeometricShaderSharedPtr const &geometricShader,
    HdStDispatchBufferSharedPtr const &dispatchBuffer,
    HdStBufferArrayRangeSharedPtr const &indexBar,
    _DrawingProgram const &program)
{
  ...
  bool const rebindTextureResources = _needsTextureResourceRebinding &&
                                      programUsesTextureResources;

  TextureResourceID currentTextureResourceID = _GetTextureResourceID(
      _drawItemInstances[0]->GetDrawItem());
  ...
  if (!_useDrawIndexed) {
    ...
    for (uint32_t i = 0; i < drawCount; ++i) {
      ...
      if (rebindTextureResources) {
        _BindTextureResources(_drawItemInstances[i]->GetDrawItem(),
                              program.GetBinder(),
                              program.GetGLSLProgram(),
      ...
      glDrawArraysInstancedBaseInstance(
          primitiveMode, cmd->baseVertex, cmd->count, cmd->instanceCount, cmd->baseInstance);
    }
  }
  else {
    TF_DEBUG(HDST_DRAW).Msg(
    ...
    for (uint32_t i = 0; i < drawCount; ++i) {
      ...
      uint32_t const indexBufferByteOffset = static_cast<uint32_t>(cmd->baseIndex *
                                                                   sizeof(uint32_t));
...
void HdSt_IndirectDrawBatch::_ExecuteFrustumCull(
    ...
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  ...
  if (!TF_VERIFY(cullingProgram.IsValid()))
    return;

  HdStBufferResourceSharedPtr cullCommandBuffer = _dispatchBufferCullInput->GetResource(
  ...
  HgiGraphicsPipelineSharedPtr const &pso = _GetCullPipeline(
      resourceRegistry,
      state.glslProgram,
      _useInstanceCulling ? sizeof(UniformsInstanced) : sizeof(Uniforms));
  ...
  state.binder.BindBuffer(_tokens->dispatchBuffer, _dispatchBuffer->GetEntireResource());
  ...
  if (_useInstanceCulling) {
    ...
    cullParamsInstanced.drawCommandNumUints = _dispatchBuffer->GetCommandNumUints();
    ...
    cullGfxCmds->SetConstantValues(
        psoHandle, HgiShaderStageVertex, bindLoc, sizeof(UniformsInstanced), &cullParamsInstanced);
  ...
  state.binder.UnbindBuffer(_tokens->dispatchBuffer, _dispatchBuffer->GetEntireResource());
...
void HdSt_IndirectDrawBatch::DrawItemInstanceChanged(HdStDrawItemInstance const *instance)
{
...
void HdSt_IndirectDrawBatch::_BeginGPUCountVisibleInstances(
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  if (!_resultBuffer) {
...
void HdSt_IndirectDrawBatch::_EndGPUCountVisibleInstances(
...
HdSt_IndirectDrawBatch::_CullingProgram &HdSt_IndirectDrawBatch::_GetCullingProgram(
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  if (!_cullingProgram.GetGLSLProgram() || _dirtyCullingProgram) {
    // create a culling shader key
    HdSt_CullingShaderKey shaderKey(
...
void HdSt_IndirectDrawBatch::_CullingProgram::Initialize(bool useDrawIndexed,
...
/* virtual */
void HdSt_IndirectDrawBatch::_CullingProgram::_GetCustomBindings(
    HdStBindingRequestVector *customBindings, bool *enableInstanceDraw) const
{
  ...
  if (_useInstanceCulling) {
    customBindings->push_back(
        HdStBindingRequest(HdStBinding::DRAW_INDEX_INSTANCE, _tokens->drawCommandIndex));
  }
  else {
    // non-instance culling
    customBindings->push_back(
        HdStBindingRequest(HdStBinding::DRAW_INDEX, _tokens->drawCommandIndex));
    customBindings->push_back(
        HdStBindingRequest(HdStBinding::DRAW_INDEX, _tokens->instanceCountInput));
  }
