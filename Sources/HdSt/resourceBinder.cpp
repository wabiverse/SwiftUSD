...
TF_DEFINE_PUBLIC_TOKENS(HdSt_ResourceBindingSuffixTokens, HDST_RESOURCE_BINDING_SUFFIX_TOKENS);
...
struct BindingLocator {
  BindingLocator() : uniformLocation(0), bufferLocation(0), attribLocation(0), textureUnit(0) {}
...
{
  return TfToken(token.GetString() + HdSt_ResourceBindingSuffixTokens->layout.GetString());
}

static TfTokenVector _GetInstancerFilterNames(HdStDrawItem const *drawItem)
{
  TfTokenVector filterNames = HdInstancer::GetBuiltinPrimvarNames();
  ;

  HdSt_MaterialNetworkShaderSharedPtr materialNetworkShader = drawItem->GetMaterialNetworkShader();
...
void HdSt_ResourceBinder::ResolveBindings(
    ...
    HgiCapabilities const *capabilities)
{
  ...
  const bool bindlessTexturesEnabled = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsBindlessTextures);
  const bool isMetal = capabilities->IsSet(HgiDeviceCapabilitiesBitsMetalTessellation);
  ...
  HdStBinding::Type drawingCoordBindingType = instanceDraw ? HdStBinding::DRAW_INDEX_INSTANCE :
  ...
  HdStBinding constantPrimvarBinding = locator.GetBinding(structBufferBindingType,
                                                          _tokens->constantPrimvars);

  if (HdBufferArrayRangeSharedPtr constantBar_ = drawItem->GetConstantPrimvarRange()) {

    HdStBufferArrayRangeSharedPtr constantBar = std::static_pointer_cast<HdStBufferArrayRange>(
        constantBar_);
  ...
  for (int i = 0; i < instancerNumLevels; ++i) {
    if (HdBufferArrayRangeSharedPtr instanceBar_ = drawItem->GetInstancePrimvarRange(i)) {
      ...
      HdStBufferArrayRangeSharedPtr instanceBar = std::static_pointer_cast<HdStBufferArrayRange>(
          instanceBar_);
      ...
      TF_FOR_ALL(it, instanceBar->GetResources())
      {
        ...
        HdStBinding instancePrimvarBinding = locator.GetBinding(arrayBufferBindingType, name);
  ...
  if (HdBufferArrayRangeSharedPtr vertexBar_ = drawItem->GetVertexPrimvarRange()) {
    ...
    HdStBufferArrayRangeSharedPtr vertexBar = std::static_pointer_cast<HdStBufferArrayRange>(
        vertexBar_);
    ...
    TF_FOR_ALL(it, vertexBar->GetResources())
    {
      TfToken const &name = it->first;
      TfToken glName = HdStGLConversions::GetGLSLIdentifier(name);
      HdStBinding vertexPrimvarBinding = locator.GetBinding(HdStBinding::VERTEX_ATTR, name);
  ...
  if (HdBufferArrayRangeSharedPtr varyingBar_ = drawItem->GetVaryingPrimvarRange()) {
    ...
    HdStBufferArrayRangeSharedPtr varyingBar = std::static_pointer_cast<HdStBufferArrayRange>(
        varyingBar_);

    for (const auto &resource : varyingBar->GetResources()) {
      TfToken const &name = resource.first;
      TfToken glName = HdStGLConversions::GetGLSLIdentifier(name);
      HdStBinding varyingPrimvarBinding = locator.GetBinding(arrayBufferBindingType, name);
  ...
  if (HdBufferArrayRangeSharedPtr topologyBar_ = drawItem->GetTopologyRange()) {
    ...
    HdStBufferArrayRangeSharedPtr topologyBar = std::static_pointer_cast<HdStBufferArrayRange>(
        topologyBar_);
    ...
    TF_FOR_ALL(it, topologyBar->GetResources())
    {
      ...
      if (name == HdTokens->indices) {
        if (isMetal && drawItem->GetVaryingPrimvarRange()) {
          ...
          HdStBinding const binding = locator.GetBinding(HdStBinding::SSBO, name);
          _bindingMap[name] = binding;

          HdType const componentType = HdGetComponentType(resource->GetTupleType().type);
          TfToken const glType = HdStGLConversions::GetGLSLTypename(componentType);
        ...
        HdStBinding binding = locator.GetBinding(arrayBufferBindingType, name);
        _bindingMap[name] = binding;
        ...
        else if (_TokenContainsString(name, HdStTokens->fvarIndices.GetString())) {
          metaDataOut->fvarIndicesBindings.push_back(bindingDecl);
        }
        else if (_TokenContainsString(name, HdStTokens->fvarPatchParam.GetString())) {
  ...
  if (HdBufferArrayRangeSharedPtr topVisBar_ = drawItem->GetTopologyVisibilityRange()) {
    ...
    // topology visibility
    HdStBinding topologyVisibilityBinding = locator.GetBinding(
    ...
    HdStBufferArrayRangeSharedPtr topVisBar = std::static_pointer_cast<HdStBufferArrayRange>(
        topVisBar_);
  ...
  if (HdBufferArrayRangeSharedPtr elementBar_ = drawItem->GetElementPrimvarRange()) {
    ...
    HdStBufferArrayRangeSharedPtr elementBar = std::static_pointer_cast<HdStBufferArrayRange>(
        elementBar_);
    ...
    TF_FOR_ALL(it, elementBar->GetResources())
    {
      TfToken const &name = it->first;
      TfToken glName = HdStGLConversions::GetGLSLIdentifier(name);
      HdStBinding elementPrimvarBinding = locator.GetBinding(arrayBufferBindingType, name);
  ...
  if (HdBufferArrayRangeSharedPtr fvarBar_ = drawItem->GetFaceVaryingPrimvarRange()) {
    ...
    HdStBufferArrayRangeSharedPtr fvarBar = std::static_pointer_cast<HdStBufferArrayRange>(
        fvarBar_);
    ...
    TF_FOR_ALL(it, fvarBar->GetResources())
    {
      ...
      HdStBinding fvarPrimvarBinding = locator.GetBinding(arrayBufferBindingType, name);
  ...
  HdStBinding drawingCoord0Binding = locator.GetBinding(drawingCoordBindingType,
  ...
  HdStBinding drawingCoord1Binding = locator.GetBinding(drawingCoordBindingType,
  ...
  HdStBinding drawingCoord2Binding = locator.GetBinding(drawingCoordBindingType,
  ...
  if (instancerNumLevels > 0) {
    HdStBinding drawingCoordIBinding = HdStBinding(HdStBinding::DRAW_INDEX_INSTANCE_ARRAY,
  ...
  if (HdBufferArrayRangeSharedPtr instanceIndexBar_ = drawItem->GetInstanceIndexRange()) {
    ...
    HdStBufferResourceSharedPtr instanceIndices = instanceIndexBar->GetResource(
        HdInstancerTokens->instanceIndices);
    HdStBufferResourceSharedPtr culledInstanceIndices = instanceIndexBar->GetResource(
        HdInstancerTokens->culledInstanceIndices);

    if (instanceIndices) {
      HdStBinding instanceIndexArrayBinding = locator.GetBinding(
          arrayBufferBindingType, HdInstancerTokens->instanceIndices);
    ...
    if (culledInstanceIndices) {
      HdStBinding culledInstanceIndexArrayBinding = locator.GetBinding(
  ...
  HdStBinding dispatchBinding(HdStBinding::DISPATCH, /*location=(not used)*/ 0);
  ...
  TF_FOR_ALL(shader, shaders)
  {
    ...
    HdStBufferArrayRangeSharedPtr shaderBar = std::static_pointer_cast<HdStBufferArrayRange>(
        shaderBar_);
    if (shaderBar) {
      HdStBinding shaderParamBinding = locator.GetBinding(structBufferBindingType,
    ...
    for (HdSt_MaterialParam const &param : params) {
      const bool isMaterialShader = ((*shader) == drawItem->GetMaterialNetworkShader());
      ...
      TfToken glType = HdStGLConversions::GetGLSLTypename(
          _AdjustHdType(valueType.type, glSwizzle));
      TfToken const &name = param.name;
      TfToken glName = HdStGLConversions::GetGLSLIdentifier(name);

      if (param.IsFallback()) {
        metaDataOut->shaderParameterBinding[HdStBinding(HdStBinding::FALLBACK,
        ...
        if (param.textureType == HdStTextureType::Ptex) {
          ...
          metaDataOut->shaderParameterBinding[layoutBinding] = MetaData::ShaderParameterAccessor(
              /*name=*/glLayoutName,
              /*type=*/HdStGLConversions::GetGLSLTypename(HdType::HdTypeInt32));
          ...
          if (param.IsArrayOfTextures()) {
            ...
            for (size_t i = 0; i < numTextures; i++) {
              HdStBinding textureBinding = bindless ?
                                               HdStBinding(
                                                   ...
                                                   bindlessTextureLocation++) :
                                               HdStBinding(HdStBinding::ARRAY_OF_TEXTURE_2D,
                                                           locator.uniformLocation++,
                                                           locator.textureUnit++);
              if (i == 0) {
                firstBinding = textureBinding;
              }
              TfToken const indexedName(name.GetString() + std::to_string(i));
              // used for non-bindless
              _bindingMap[indexedName] = textureBinding;
            }

            // Only fill metadata for the first binding.
            metaDataOut->shaderParameterBinding[firstBinding] = MetaData::ShaderParameterAccessor(
                /*name=*/glName,
                /*type=*/glType,
                /*swizzle=*/glSwizzle,
                /*inPrimvars=*/param.samplerCoords,
            ...
            // 2d texture
            HdStBinding textureBinding = bindless ? HdStBinding(HdStBinding::BINDLESS_TEXTURE_2D,
                                                                bindlessTextureLocation++) :
          ...
          // 3d texture
          HdStBinding textureBinding = bindless ? HdStBinding(HdStBinding::BINDLESS_TEXTURE_FIELD,
                                                              bindlessTextureLocation++) :
        ...
        HdStBinding binding = HdStBinding(HdStBinding::TRANSFORM_2D, shaderTransform2dLocation++);
        metaDataOut->shaderParameterBinding[binding] = MetaData::ShaderParameterAccessor(
            /*name=*/glName,
            /*type=*/glType,
            /*swizzle=*/glSwizzle,
  ...
  TF_FOR_ALL(it, customBindings)
  {
    if (it->IsInterleavedBufferArray()) {
      ...
      HdStBinding binding = locator.GetBinding(it->GetBindingType(), it->GetName());
      ...
      HdStBufferArrayRangeSharedPtr bar = std::static_pointer_cast<HdStBufferArrayRange>(bar_);

      for (auto const &nameRes : bar->GetResources()) {
        HdTupleType valueType = nameRes.second->GetTupleType();
        TfToken glType = HdStGLConversions::GetGLSLTypename(valueType.type);
        sblock.entries.emplace_back(
            nameRes.first, glType, nameRes.second->GetOffset(), valueType.count, concatenateNames);
      }
      ...
      if (it->IsBufferArray()) {
        ...
        for (auto const &nameRes : bar->GetResources()) {
          HdStBinding binding = locator.GetBinding(it->GetBindingType(), nameRes.first);
          BindingDeclaration b(
              nameRes.first,
              HdStGLConversions::GetGLSLTypename(nameRes.second->GetTupleType().type),
              binding,
              it->isWritable());
        ...
        BindingDeclaration b(it->GetName(),
                             HdStGLConversions::GetGLSLTypename(it->GetDataType()),
                             binding,
                             it->isWritable());
...
void HdSt_ResourceBinder::ResolveComputeBindings(HdBufferSpecVector const &readWriteBufferSpecs,
                                                 ...
                                                 HgiCapabilities const *capabilities)
{
  ...
  // GL context caps
  HdStBinding::Type bindingType = capabilities->IsSet(HgiDeviceCapabilitiesBitsBindlessBuffers) ?
                                      HdStBinding::BINDLESS_SSBO_RANGE :
                                      HdStBinding::SSBO;
  ...
  for (HdBufferSpec const &spec : readWriteBufferSpecs) {
    ...
    metaDataOut->computeReadWriteData[binding] = MetaData::Primvar(
        spec.name, HdStGLConversions::GetGLSLTypename(spec.tupleType.type));
  }
...
void HdSt_ResourceBinder::GetBufferBindingDesc(HgiResourceBindingsDesc *bindingsDesc,
                                               TfToken const &name,
...
{
  if (!resource || !resource->GetHandle())
    return;

  HdStBinding binding = GetBinding(name, level);

  HgiShaderStage stageUsage = HgiShaderStageVertex | HgiShaderStagePostTessellationControl |
  ...
  switch (binding.GetType()) {
    ...
    case HdStBinding::UBO:
      ...
      desc.sizes = {static_cast<uint32_t>(numElements * resource->GetStride())};
      desc.bindingIndex = static_cast<uint32_t>(binding.GetLocation());
...
void HdSt_ResourceBinder::GetBufferArrayBindingDesc(HgiResourceBindingsDesc *bindingsDesc,
                                                    HdStBufferArrayRangeSharedPtr const &bar) const
{
  ...
  for (auto const &it : bar->GetResources()) {
    GetBufferBindingDesc(bindingsDesc, it.first, it.second, it.second->GetOffset());
  }
}

void HdSt_ResourceBinder::GetInterleavedBufferArrayBindingDesc(
    ...
    TfToken const &name) const
{
  ...
  GetBufferBindingDesc(bindingsDesc, name, bar->GetResource(), bar->GetResource()->GetOffset());
}

void HdSt_ResourceBinder::GetInstanceBufferArrayBindingDesc(
    HgiResourceBindingsDesc *bindingsDesc,
    HdStBufferArrayRangeSharedPtr const &bar,
    int level) const
{
  ...
  for (auto const &it : bar->GetResources()) {
    if (HasBinding(it.first, level)) {
      GetBufferBindingDesc(bindingsDesc, it.first, it.second, it.second->GetOffset(), level);
    }
  }
}

void HdSt_ResourceBinder::GetBindingRequestBindingDesc(HgiResourceBindingsDesc *bindingsDesc,
                                                       HdStBindingRequest const &req) const
{
  if (req.IsTypeless()) {
    ...
    GetBufferBindingDesc(bindingsDesc, req.GetName(), resource, req.GetByteOffset());
  }
  else if (req.IsInterleavedBufferArray()) {
    // note: interleaved buffer needs only 1 binding
    HdStBufferArrayRangeSharedPtr bar = std::static_pointer_cast<HdStBufferArrayRange>(
        req.GetBar());

    GetBufferBindingDesc(bindingsDesc, req.GetName(), bar->GetResource(), req.GetByteOffset());
  }
...
void HdSt_ResourceBinder::GetTextureBindingDesc(HgiResourceBindingsDesc *bindingsDesc,
                                                TfToken const &name,
                                                ...
                                                HgiTextureHandle const &texelTexture) const
{
  ...
  texelDesc.stageUsage = HgiShaderStageGeometry | HgiShaderStageFragment |
                         HgiShaderStagePostTessellationVertex;
...
void HdSt_ResourceBinder::GetTextureWithLayoutBindingDesc(
    HgiResourceBindingsDesc *bindingsDesc,
    TfToken const &name,
    HgiSamplerHandle const &texelSampler,
...
{
  if (!texelSampler.Get() || !texelTexture.Get() || !layoutSampler.Get() || !layoutTexture.Get()) {
    return;
  }
...
void HdSt_ResourceBinder::BindBuffer(TfToken const &name,
                                     ...
                                     int numElements) const
{
  ...
  switch (type) {
    ...
    case HdStBinding::BINDLESS_UNIFORM:
      ...
      {
        HgiGLBuffer *bufferGL = static_cast<HgiGLBuffer *>(buffer->GetHandle().Get());
...
void HdSt_ResourceBinder::UnbindBuffer(TfToken const &name,
                                       HdStBufferResourceSharedPtr const &buffer,
                                       int level) const
{
  HD_TRACE_FUNCTION();
...
void HdSt_ResourceBinder::BindConstantBuffer(
...
void HdSt_ResourceBinder::UnbindConstantBuffer(
    HdStBufferArrayRangeSharedPtr const &constantBar) const
{
  if (!constantBar)
    return;
...
void HdSt_ResourceBinder::BindInterleavedBuffer(
    HdStBufferArrayRangeSharedPtr const &interleavedBar, TfToken const &name) const
{
  if (!interleavedBar)
    return;
...
void HdSt_ResourceBinder::UnbindInterleavedBuffer(
    HdStBufferArrayRangeSharedPtr const &interleavedBar, TfToken const &name) const
{
  if (!interleavedBar)
    return;
...
void HdSt_ResourceBinder::BindInstanceBufferArray(HdStBufferArrayRangeSharedPtr const &bar,
                                                  int level) const
{
  if (!bar)
    return;
...
void HdSt_ResourceBinder::UnbindInstanceBufferArray(HdStBufferArrayRangeSharedPtr const &bar,
                                                    int level) const
{
  if (!bar)
    return;
...
void HdSt_ResourceBinder::BindBufferArray(HdStBufferArrayRangeSharedPtr const &bar) const
{
  if (!bar)
    return;
...
void HdSt_ResourceBinder::Bind(HdStBindingRequest const &req) const
{
  if (req.IsTypeless()) {
    return;
  }
...
void HdSt_ResourceBinder::Unbind(HdStBindingRequest const &req) const
{
  if (req.IsTypeless()) {
    return;
  }
...
void HdSt_ResourceBinder::UnbindBufferArray(HdStBufferArrayRangeSharedPtr const &bar) const
{
  if (!bar)
    return;
...
void HdSt_ResourceBinder::BindUniformi(TfToken const &name, int count, const int *value) const
{
  HdStBinding uniformLocation = GetBinding(name);
  if (uniformLocation.GetLocation() == HdStBinding::NOT_EXIST)
    return;
...
void HdSt_ResourceBinder::BindUniformArrayi(TfToken const &name, int count, const int *value) const
{
  HdStBinding uniformLocation = GetBinding(name);
  if (uniformLocation.GetLocation() == HdStBinding::NOT_EXIST)
    return;
...
void HdSt_ResourceBinder::BindUniformui(TfToken const &name,
                                        int count,
...
void HdSt_ResourceBinder::BindUniformf(TfToken const &name, int count, const float *value) const
{
  HdStBinding uniformLocation = GetBinding(name);
  if (uniformLocation.GetLocation() == HdStBinding::NOT_EXIST)
    return;
  ...
  if (!TF_VERIFY(uniformLocation.IsValid()))
    return;
  if (!TF_VERIFY(uniformLocation.GetType() == HdStBinding::UNIFORM))
    return;
...
HdSt_ResourceBinder::MetaData::ID HdSt_ResourceBinder::MetaData::ComputeHash() const
{
  ID hash = 0;
  ...
  TF_FOR_ALL(blockIt, customInterleavedBindings)
  {
    ...
    TF_FOR_ALL(it, blockIt->second.entries)
    {
      ...
      hash = TfHash::Combine(
          hash, entry.name.Hash(), entry.dataType, entry.offset, entry.arraySize);
    }
  }
  ...
  TF_FOR_ALL(it, instanceData)
  {
    ...
    hash = TfHash::Combine(hash, primvar.name.Hash(), primvar.dataType, primvar.level);
  }
  hash = TfHash::Combine(hash, 0);  // separator
  TF_FOR_ALL(it, vertexData)
  {
    ...
    hash = TfHash::Combine(hash, primvar.name.Hash(), primvar.dataType);
  }
  hash = TfHash::Combine(hash, 0);  // separator
  ...
  TF_FOR_ALL(it, fvarData)
  {
    ...
    hash = TfHash::Combine(hash, primvar.name.Hash(), primvar.dataType, primvar.channel);
  }
  hash = TfHash::Combine(hash, 0);  // separator
  ...
  TF_FOR_ALL(it, shaderParameterBinding)
  {
    ...
    hash = TfHash::Combine(hash, entry.name.Hash(), entry.dataType, entry.swizzle);
  }
...
/* static */
uint64_t HdSt_ResourceBinder::GetSamplerBindlessHandle(HgiSamplerHandle const &samplerHandle,
                                                       HgiTextureHandle const &textureHandle)
{
  HgiGLSampler *const glSampler = const_cast<HgiGLSampler *>(
      dynamic_cast<const HgiGLSampler *>(samplerHandle.Get()));

  HgiGLTexture *const glTexture = const_cast<HgiGLTexture *>(
...
/* static */
uint64_t HdSt_ResourceBinder::GetTextureBindlessHandle(HgiTextureHandle const &textureHandle)
{
  HgiGLTexture *const glTexture = const_cast<HgiGLTexture *>(
      dynamic_cast<const HgiGLTexture *>(textureHandle.Get()));
...
static bool _IsBindless(HdStBinding const &binding)
{
  switch (binding.GetType()) {
...
static void _BindGLTextureAndSampler(int const textureUnit,
...
void HdSt_ResourceBinder::BindTexture(const TfToken &name,
                                      ...
                                      const bool bind) const
{
  ...
  _BindGLTextureAndSampler(binding.GetTextureUnit(),
                           (bind && textureHandle) ? textureHandle->GetRawResource() : 0,
                           (bind && samplerHandle) ? samplerHandle->GetRawResource() : 0);
}

void HdSt_ResourceBinder::BindTextureWithLayout(TfToken const &name,
                                                HgiSamplerHandle const &texelSampler,
                                                HgiTextureHandle const &texelTexture,
                                                HgiSamplerHandle const &layoutSampler,
                                                ...
                                                const bool bind) const
{
  ...
  _BindGLTextureAndSampler(texelBinding.GetTextureUnit(),
                           (bind && texelTexture) ? texelTexture->GetRawResource() : 0,
                           (bind && texelSampler) ? texelSampler->GetRawResource() : 0);

  HdStBinding const layoutBinding = GetBinding(_ConcatLayout(name));

  _BindGLTextureAndSampler(layoutBinding.GetTextureUnit(),
