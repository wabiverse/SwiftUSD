...
HdBufferSpecVector HdStInterleavedMemoryManager::GetBufferSpecs(
...
size_t HdStInterleavedMemoryManager::GetResourceAllocation(
    HdBufferArraySharedPtr const &bufferArray, VtDictionary &result) const
{
...
// ---------------------------------------------------------------------------
HdBufferArraySharedPtr HdStInterleavedUBOMemoryManager::CreateBufferArray(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
  const int uniformBufferOffsetAlignment =
      _resourceRegistry->GetHgi()->GetCapabilities()->GetUniformBufferOffsetAlignment();
  const int maxUniformBlockSize =
      _resourceRegistry->GetHgi()->GetCapabilities()->GetMaxUniformBlockSize();
...
HdStAggregationStrategy::AggregationId HdStInterleavedUBOMemoryManager::ComputeAggregationId(
    HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint) const
{
  ...
  return static_cast<AggregationId>(TfHash::Combine(salt, bufferSpecs, usageHint));
}

// ---------------------------------------------------------------------------
//  HdStInterleavedSSBOMemoryManager
// ---------------------------------------------------------------------------
HdBufferArraySharedPtr HdStInterleavedSSBOMemoryManager::CreateBufferArray(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
  const int maxShaderStorageBlockSize =
      _resourceRegistry->GetHgi()->GetCapabilities()->GetMaxShaderStorageBlockSize();
...
HdStAggregationStrategy::AggregationId HdStInterleavedSSBOMemoryManager::ComputeAggregationId(
    HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint) const
{
  static size_t salt = ArchHash(__FUNCTION__, sizeof(__FUNCTION__));
...
static inline size_t _ComputePadding(int alignment, size_t currentOffset)
{
  return ((alignment - (currentOffset & (alignment - 1))) & (alignment - 1));
}

static inline size_t _ComputeAlignment(HdTupleType tupleType)
{
      ...
      _bufferUsage(0)
{
  ...
  */
   ...
   */
  const bool useCppShaderPadding = _resourceRegistry->GetHgi()->GetCapabilities()->IsSet(
  ...
  if (_stride > _maxSize) {
    TF_WARN(
        "Computed stride = %zu of interleaved buffer is larger than max"
        " size %zu, cannot create buffer.",
        _stride,
        _maxSize);
    _SetMaxNumRanges(0);
    return;
  ...
  if (_stride == 0) {
    TF_WARN(
        "Computed stride = %zu of interleaved buffer is 0, cannot "
        " create buffer.",
        _stride);
    _SetMaxNumRanges(0);
    return;
  ...
  TF_DEBUG_MSG(HD_BUFFER_ARRAY_INFO, "Create interleaved buffer array: stride = %zu\n", _stride);
...
HdStBufferResourceSharedPtr HdStInterleavedMemoryManager::_StripedInterleavedBuffer::_AddResource(
    TfToken const &name, HdTupleType tupleType, int offset, int stride)
{
  HD_TRACE_FUNCTION();
...
bool HdStInterleavedMemoryManager::_StripedInterleavedBuffer::GarbageCollect()
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdStInterleavedMemoryManager::_StripedInterleavedBuffer::Reallocate(
    ...
    HdBufferArraySharedPtr const &curRangeOwner)
{
  ...
  for (size_t idx = 0; idx < ranges.size(); ++idx) {
    _StripedInterleavedBufferRangeSharedPtr range =
        std::static_pointer_cast<_StripedInterleavedBufferRange>(ranges[idx]);
    if (!range) {
      TF_CODING_ERROR("_StripedInterleavedBufferRange expired unexpectedly.");
      continue;
    }
...
void HdStInterleavedMemoryManager::_StripedInterleavedBuffer::_DeallocateResources()
{
...
void HdStInterleavedMemoryManager::_StripedInterleavedBuffer::DebugDump(std::ostream &out) const
{
...
HdStBufferResourceSharedPtr HdStInterleavedMemoryManager::_StripedInterleavedBuffer::GetResource()
    const
{
  HD_TRACE_FUNCTION();
...
HdBufferSpecVector HdStInterleavedMemoryManager::_StripedInterleavedBuffer::GetBufferSpecs() const
{
...
bool HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::IsAssigned() const
{
  return (_stripedBuffer != nullptr);
}

bool HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::IsImmutable() const
{
  return (_stripedBuffer != nullptr) && _stripedBuffer->IsImmutable();
}

bool HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::RequiresStaging() const
{
  return true;
}

bool HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::Resize(int numElements)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::CopyData(
    HdBufferSourceSharedPtr const &bufferSource)
{
  ...
  HdStBufferResourceSharedPtr VBO = _stripedBuffer->GetResource(bufferSource->GetName());
  ...
  size_t const sourceDataSize = HdDataSizeOfTupleType(bufferSource->GetTupleType());
  ...
  if (sourceDataSize <= vboDataSize) {
    ...
    TF_WARN(
        "Source data size (%zu bytes) is larger than buffer resource "
  ...
  HdStStagingBuffer *stagingBuffer = GetResourceRegistry()->GetStagingBuffer();
  ...
  HD_PERF_COUNTER_ADD(HdStPerfTokens->copyBufferCpuToGpu, (double)_numElements);
}

VtValue HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::ReadData(
...
size_t HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::GetMaxNumElements() const
{
  return _stripedBuffer->GetMaxNumElements();
}

HdBufferArrayUsageHint HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::GetUsageHint()
    const
{
  if (!TF_VERIFY(_stripedBuffer)) {
...
HdStBufferResourceSharedPtr HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::
    GetResource() const
{
  if (!TF_VERIFY(_stripedBuffer))
    return HdStBufferResourceSharedPtr();
...
HdStBufferResourceNamedList const &HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::
    GetResources() const
{
  if (!TF_VERIFY(_stripedBuffer)) {
...
void HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::SetBufferArray(
    HdBufferArray *bufferArray)
{
  _stripedBuffer = static_cast<_StripedInterleavedBuffer *>(bufferArray);
}

void HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::DebugDump(
    std::ostream &out) const
{
  out << "[StripedIBR] index = " << _index << "\n";
}

const void *HdStInterleavedMemoryManager::_StripedInterleavedBufferRange::_GetAggregation() const
{
