...
static void _DumpShaderSource(HgiShaderFunctionDesc const &desc)
{
                                 ...
                                 size_t debugID)
{
  ...
  if (TfDebug::IsEnabled(HDST_DUMP_SHADER_SOURCEFILE) ||
      (TfDebug::IsEnabled(HDST_DUMP_FAILING_SHADER_SOURCEFILE) && !shaderFn->IsValid()))
  {
...
static std::string _GetScopeDescriptionLabel(HgiShaderProgramDesc const &desc)
{
...
HdStGLSLProgram::HdStGLSLProgram(TfToken const &role, HdStResourceRegistry *const registry)
    : _registry(registry), _role(role)
{
...
/* static */
static HdStGLSLProgram::ID _ComputeHash(TfToken const &sourceFile,
...
bool HdStGLSLProgram::CompileShader(HgiShaderStage stage, std::string const &shaderSource)
{
  HD_TRACE_FUNCTION();
  ...
  TF_DESCRIBE_SCOPE("Compiling GLSL shader" + _GetScopeDescriptionLabel(_programDesc));
...
bool HdStGLSLProgram::CompileShader(HgiShaderFunctionDesc const &desc)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
bool HdStGLSLProgram::Link()
{
  ...
  TF_DESCRIBE_SCOPE("Linking GLSL shader" + _GetScopeDescriptionLabel(_programDesc));
...
bool HdStGLSLProgram::Validate() const
{
...
HdStGLSLProgramSharedPtr HdStGLSLProgram::GetComputeProgram(TfToken const &shaderToken,
                                                            HdStResourceRegistry *resourceRegistry)
{
  return GetComputeProgram(HdStPackageComputeShader(), shaderToken, resourceRegistry);
}

HdStGLSLProgramSharedPtr HdStGLSLProgram::GetComputeProgram(TfToken const &shaderFileName,
                                                            ...
                                                            HdStResourceRegistry *resourceRegistry)
{
  ...
  if (programInstance.IsFirstInstance()) {
    ...
    TF_DEBUG(HDST_LOG_COMPUTE_SHADER_PROGRAM_MISSES)
        .Msg("(MISS) First compute program instance for %s (hash = %zu)\n",
             shaderFileName.GetText(),
             hash);

    // if not exists, create new one
    HdStGLSLProgramSharedPtr newProgram = std::make_shared<HdStGLSLProgram>(
    ...
    if (!newProgram->CompileShader(HgiShaderStageCompute, glslfx.GetSource(shaderToken))) {
    ...
    TF_DEBUG(HDST_LOG_COMPUTE_SHADER_PROGRAM_HITS)
        .Msg("(HIT) Found compute program instance for %s (hash = %zu)\n",
             shaderFileName.GetText(),
             hash);
  }
...
HdStGLSLProgramSharedPtr HdStGLSLProgram::GetComputeProgram(
    TfToken const &shaderToken,
    HdStResourceRegistry *resourceRegistry,
    PopulateDescriptorCallback populateDescriptor)
{
  return GetComputeProgram(shaderToken, std::string(), resourceRegistry, populateDescriptor);
}
    ...
    PopulateDescriptorCallback populateDescriptor)
{
  return GetComputeProgram(
      HdStPackageComputeShader(), shaderToken, defines, resourceRegistry, populateDescriptor);
}
