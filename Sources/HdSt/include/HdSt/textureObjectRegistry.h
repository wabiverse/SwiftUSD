...
class HdSt_TextureObjectRegistry final {
 public:
  ...
  ///
  HDST_API
  HdStTextureObjectSharedPtr AllocateTextureObject(const HdStTextureIdentifier &textureId,
  ...
  ///
  HDST_API
  void MarkTextureObjectDirty(HdStTextureObjectPtr const &textureObject);
 ...
 private:
  HdStTextureObjectSharedPtr _MakeTextureObject(const HdStTextureIdentifier &textureId,
                                                HdStTextureType textureType);

  std::atomic<int64_t> _totalTextureMemory;

  // Registry for texture and sampler objects.
  HdInstanceRegistry<HdStTextureObjectSharedPtr> _textureObjectRegistry;
