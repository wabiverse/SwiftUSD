...
using HdSt_FallbackLightingShaderSharedPtr = std::shared_ptr<class HdSt_FallbackLightingShader>;
...
class HdStRenderPassState : public HdRenderPassState {
 public:
  ...
  HDST_API
  void ApplyStateFromGeometricShader(HdSt_ResourceBinder const &binder,
  ...
  //
  HDST_API
  HgiGraphicsCmdsDesc MakeGraphicsCmdsDesc(HdRenderIndex const *renderIndex) const;

  // Helper to initialize graphics pipeline descriptor state including
  // any additional state from the geometric shader.
  HDST_API
  void InitGraphicsPipelineDesc(HgiGraphicsPipelineDesc *pipeDesc,
                                HdSt_GeometricShaderSharedPtr const &geometricShader,
                                bool firstDrawBatch) const;

  /// Generates the hash for the settings used to init the graphics pipeline.
  HDST_API
  uint64_t GetGraphicsPipelineHash(HdSt_GeometricShaderSharedPtr const &geometricShader,
                                   bool firstDrawBatch) const;
  ...
  HDST_API
  AxisAlignedAffineTransform ComputeImageToHorizontallyNormalizedFilmback() const;
 ...
 private:
  ...
  void _InitPrimitiveState(HgiGraphicsPipelineDesc *pipeDesc,
                           HdSt_GeometricShaderSharedPtr const &geometricShader) const;
  void _InitAttachmentState(HgiGraphicsPipelineDesc *pipeDesc, bool firstDrawBatch) const;
  ...
  void _InitRasterizationState(HgiRasterizationState *rasterizationState,
                               HdSt_GeometricShaderSharedPtr const &geometricShader) const;
