...
class HdStTextureUtils {
 public:
  ...
  template<typename T> class AlignedBuffer {
   public:
    AlignedBuffer() : AlignedBuffer(nullptr) {}
   ...
   private:
    ...
    explicit AlignedBuffer(T *alignedPtr) : _alignedPtr(alignedPtr, ArchAlignedFree) {}
  ...
  HDST_API
  static AlignedBuffer<uint8_t> HgiTextureReadback(Hgi *const hgi,
                                                   HgiTextureHandle const &texture,
                                                   size_t *bufferSize);

  /// Returns a buffer with data of type T read back from \p texture.
  template<typename T>
  static AlignedBuffer<T> HgiTextureReadback(Hgi *const hgi,
                                             HgiTextureHandle const &texture,
                                             size_t *bufferSize);
};

template<typename T>
HdStTextureUtils::AlignedBuffer<T> HdStTextureUtils::HgiTextureReadback(
    Hgi *const hgi, HgiTextureHandle const &texture, size_t *bufferSize)
{
  HdStTextureUtils::AlignedBuffer<uint8_t> buffer = HdStTextureUtils::HgiTextureReadback(
