...
class HdSt_BasisCurvesIndexBuilderComputation : public HdComputedBufferSource {
 public:
  HdSt_BasisCurvesIndexBuilderComputation(HdBasisCurvesTopology *topology, bool forceLines);
...
template<typename T>
VtArray<T> HdSt_ExpandVarying(const SdfPath &id,
                              const TfToken &name,
                              size_t numVerts,
                              ...
                              const T fallbackValue)
{
  ...
  if (wrap == HdTokens->periodic) {
    ...
    TF_WARN(
        "HdStBasisCurves(%s) - Periodic expansion hasn't been"
        " implemented; expanding primvar %s as if non-periodic.",
        id.GetText(),
        name.GetText());
  }

  if (basis == HdTokens->catmullRom || basis == HdTokens->centripetalCatmullRom ||
      basis == HdTokens->bspline)
  {
    ...
    TF_WARN(
        "HdStBasisCurves(%s) - Varying interpolation of primvar %s has"
...
template<typename T>
class HdSt_BasisCurvesPrimvarInterpolaterComputation : public HdComputedBufferSource {
 public:
  HdSt_BasisCurvesPrimvarInterpolaterComputation(HdSt_BasisCurvesTopologySharedPtr topology,
  ...
  virtual bool Resolve() override
  {
    ...
    const size_t numVertsExpected = _topology->CalculateNeededNumberOfControlPoints();
    ...
    if (authoredSize == 0 && _name == HdTokens->points) {
      ...
      if (authoredSize == numVertsExpected) {
        ...
        std::fill(primvars.begin(), primvars.end(), _authoredPrimvar[0]);
      }
      else if (_topology->HasIndices() && authoredSize > numVertsExpected) {
        ...
        s << "HdStBasisCurves(" << _id.GetText() << ")"
          << "- Primvar " << _name.GetText() << " has incorrect size for vertex interpolation "
      ...
      const size_t numVaryingExpected = _topology->CalculateNeededNumberOfVaryingControlPoints();
      ...
      if (authoredSize == numVaryingExpected) {
        if (_topology->GetCurveType() == HdTokens->linear) {
          ...
          primvars = HdSt_ExpandVarying<T>(_id,
                                           _name,
                                           numVertsExpected,
        ...
        std::stringstream s;
        s << "HdStBasisCurves(" << _id.GetText() << ")"
          << "- Primvar " << _name.GetText() << " has incorrect size for varying interpolation "
