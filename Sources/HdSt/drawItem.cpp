...
HdStDrawItem::HdStDrawItem(HdRprimSharedData const *sharedData)
    : HdDrawItem(sharedData), _materialIsFinal(false)
{
...
static HdBufferArrayRangeSharedPtr _GetShaderBar(HdSt_MaterialNetworkShaderSharedPtr const &shader)
{
  return shader ? shader->GetShaderData() : nullptr;
}

size_t HdStDrawItem::GetBufferArraysHash() const
{
  size_t hash = TfHash::Combine(_GetVersion(GetConstantPrimvarRange()),
  ...
  for (int i = 0; i < instancerNumLevels; ++i) {
    hash = TfHash::Combine(hash, _GetVersion(GetInstancePrimvarRange(i)));
  }

  return hash;
}

size_t HdStDrawItem::GetElementOffsetsHash() const
{
  size_t hash = TfHash::Combine(_GetElementOffset(GetConstantPrimvarRange()),
...
bool HdStDrawItem::IntersectsViewVolume(GfMatrix4d const &viewProjMatrix) const
{
