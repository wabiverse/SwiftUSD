...
HdStDrawTarget::HdStDrawTarget(SdfPath const &id)
    : HdSprim(id), _enabled(true), _resolution(512, 512)
{
}
...
/*virtual*/
void HdStDrawTarget::Sync(HdSceneDelegate *sceneDelegate,
                          ...
                          HdDirtyBits *dirtyBits)
{
  ...
  if (bits & DirtyDTEnable) {
    const VtValue vtValue = sceneDelegate->Get(id, HdStDrawTargetTokens->enable);
  ...
  if (bits & DirtyDTCamera) {
    const VtValue vtValue = sceneDelegate->Get(id, HdStDrawTargetTokens->camera);
    _drawTargetRenderPassState.SetCamera(vtValue.Get<SdfPath>());
  }

  if (bits & DirtyDTResolution) {
    const VtValue vtValue = sceneDelegate->Get(id, HdStDrawTargetTokens->resolution);
  ...
  if (bits & DirtyDTAovBindings) {
    const VtValue aovBindingsValue = sceneDelegate->Get(id, HdStDrawTargetTokens->aovBindings);
  ...
  if (bits & DirtyDTDepthPriority) {
    const VtValue depthPriorityValue = sceneDelegate->Get(id, HdStDrawTargetTokens->depthPriority);
  ...
  if (bits & DirtyDTCollection) {
    const VtValue vtValue = sceneDelegate->Get(id, HdStDrawTargetTokens->collection);
...
// virtual
HdDirtyBits HdStDrawTarget::GetInitialDirtyBitsMask() const
{
  return AllDirty;
}

/*static*/
void HdStDrawTarget::GetDrawTargets(HdRenderIndex *const renderIndex,
