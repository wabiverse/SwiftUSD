...
HdSt_BasisCurvesIndexBuilderComputation::HdSt_BasisCurvesIndexBuilderComputation(
    ...
    : _topology(topology), _forceLines(forceLines)
{
}

void HdSt_BasisCurvesIndexBuilderComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // index buffer
...
HdSt_BasisCurvesIndexBuilderComputation::_BuildCubicIndexArray()
{
  ...
  */
  ...
  //
  const bool pinned = (_topology->GetCurveWrap() == HdTokens->pinned) && (vStep == 1);

  auto addPinnedSegment = [&](int startIndex, int curveIndex, int cvCount, bool start) {
    ...
    if (start) {
      ...
      indices.push_back(GfVec4i(lastSeg[1], lastSeg[2], lastSeg[3], lastSeg[3]));
      primIndices.push_back(curveIndex);

      if (basis == HdTokens->bspline) {
        indices.push_back(GfVec4i(lastSeg[2], lastSeg[3], lastSeg[3], lastSeg[3]));
        primIndices.push_back(curveIndex);
      }
    }
  };
...
bool HdSt_BasisCurvesIndexBuilderComputation::Resolve()
{
  if (!_TryLock())
    return false;
...
bool HdSt_BasisCurvesIndexBuilderComputation::_CheckValid() const
{
  return true;
}

bool HdSt_BasisCurvesIndexBuilderComputation::HasChainedBuffer() const
{
  return true;
}

HdBufferSourceSharedPtrVector HdSt_BasisCurvesIndexBuilderComputation::GetChainedBuffers() const
{
