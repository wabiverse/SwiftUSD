...
static size_t _GetMaterialTagsVersion(HdRenderIndex *renderIndex)
{
...
static size_t _GetGeomSubsetDrawItemsVersion(HdRenderIndex *renderIndex)
{
  HdStRenderParam *stRenderParam = static_cast<HdStRenderParam *>(
      renderIndex->GetRenderDelegate()->GetRenderParam());
...
HdDrawItemConstPtrVectorSharedPtr HdSt_DrawItemsCache::GetDrawItems(
    ...
    HdDrawItemConstPtrVectorSharedPtr const &curDrawItems)
{
  ...
  std::pair<_Cache::iterator, bool> ret = _cache.insert(
  ...
  const bool staleEntry = !cacheMiss && _IsCacheEntryStale(val, collection.GetName(), renderIndex);
  ...
  if (TfDebug::IsEnabled(HDST_DRAWITEMS_CACHE)) {
    ...
    if (cacheMiss) {
      ss << "[MISS] Didn't find cache entry for collection " << collection << ", render tags "
         << renderTags << ". Fetching draw items...\n";
    }
    else if (staleEntry) {
      ss << "[MISS] Found stale cache entry for collection " << collection << ", render tags "
         << renderTags << ". Fetching updated draw items...\n";
    }
    else {
      ss << "[HIT] Found up-to-date cache entry for collection " << collection << ", render tags "
         << renderTags << "\n";
    }
...
void HdSt_DrawItemsCache::GarbageCollect()
{
  TRACE_FUNCTION();
  ...
  for (typename _Cache::iterator it = _cache.begin(); it != _cache.end();) {
...
bool HdSt_DrawItemsCache::_IsCacheEntryStale(HdSt_DrawItemsCache::_CacheValue const &val,
                                             TfToken const &collectionName,
                                             HdRenderIndex *renderIndex) const
{
  HdChangeTracker const &tracker = renderIndex->GetChangeTracker();
  const bool collectionVersionChanged = val.collectionVersion !=
                                        tracker.GetCollectionVersion(collectionName);
  const bool renderTagsVersionChanged = val.renderTagsVersion != tracker.GetRenderTagVersion();
  const bool materialTagsVersionChanged = val.materialTagsVersion !=
                                          _GetMaterialTagsVersion(renderIndex);
  const bool geomSubsetDrawItemsChanged = val.geomSubsetDrawItemsVersion !=
                                          _GetGeomSubsetDrawItemsVersion(renderIndex);

  return collectionVersionChanged || renderTagsVersionChanged || materialTagsVersionChanged ||
         geomSubsetDrawItemsChanged;
}

void HdSt_DrawItemsCache::_UpdateCacheEntry(HdRprimCollection const &collection,
                                            TfTokenVector const &renderTags,
                                            HdRenderIndex *renderIndex,
                                            HdSt_DrawItemsCache::_CacheValue *val)
{
  ...
  val->collectionVersion = tracker.GetCollectionVersion(collection.GetName());
  val->renderTagsVersion = tracker.GetRenderTagVersion();
  val->materialTagsVersion = _GetMaterialTagsVersion(renderIndex);
  val->geomSubsetDrawItemsVersion = _GetGeomSubsetDrawItemsVersion(renderIndex);

  val->drawItems = std::make_shared<HdDrawItemConstPtrVector>();

  const HdStRenderParam *const renderParam = static_cast<HdStRenderParam *>(
      renderIndex->GetRenderDelegate()->GetRenderParam());
  if (renderParam->HasMaterialTag(collection.GetMaterialTag()) &&
      (renderTags.empty() || renderParam->HasAnyRenderTag(renderTags)))
  {
    HdDrawItemConstPtrVector drawItems = renderIndex->GetDrawItems(collection, renderTags);
