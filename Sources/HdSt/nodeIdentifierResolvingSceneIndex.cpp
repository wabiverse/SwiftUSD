...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         (implementationSource)(sourceCode)(sourceAsset)((
                             sourceAssetSubIdentifier, "sourceAsset:subIdentifier"))(sdrMetadata)
...
template<typename T>
T _GetNodeTypeInfo(const HdMaterialNetworkInterface *const interface,
                   const TfToken &nodeName,
                   const TfToken &key)
{
  return interface->GetNodeTypeInfoValue(nodeName, key).GetWithDefault<T>();
}

template<typename T>
T _GetNodeTypeInfoForSourceType(const HdMaterialNetworkInterface *const interface,
                                const TfToken &nodeName,
                                const TfToken &key)
{
...
SdrShaderNodeConstPtr _GetSdrShaderNodeFromSourceAsset(
    const HdMaterialNetworkInterface *const interface, const TfToken &nodeName)
{
...
SdrShaderNodeConstPtr _GetSdrShaderNodeFromSourceCode(
    const HdMaterialNetworkInterface *const interface, const TfToken &nodeName)
{
  const std::string sourceCode = _GetNodeTypeInfoForSourceType<std::string>(
...
SdrShaderNodeConstPtr _GetSdrShaderNode(const HdMaterialNetworkInterface *const interface,
                                        const TfToken &nodeName)
{
  const TfToken implementationSource = _GetNodeTypeInfo<TfToken>(
...
void _SetNodeTypeFromSourceAssetInfo(const TfToken &nodeName,
                                     HdMaterialNetworkInterface *const interface)
{
  ...
  if (SdrShaderNodeConstPtr const sdrNode = _GetSdrShaderNode(interface, nodeName)) {
    interface->SetNodeType(nodeName, sdrNode->GetIdentifier());
  }
}

void _SetNodeTypesFromSourceAssetInfo(HdMaterialNetworkInterface *const interface)
{
...
// static
HdSt_NodeIdentifierResolvingSceneIndexRefPtr HdSt_NodeIdentifierResolvingSceneIndex::New(
    HdSceneIndexBaseRefPtr const &inputSceneIndex)
{
  return TfCreateRefPtr(new HdSt_NodeIdentifierResolvingSceneIndex(inputSceneIndex));
}
...
HdSt_NodeIdentifierResolvingSceneIndex::~HdSt_NodeIdentifierResolvingSceneIndex() = default;

HdSt_NodeIdentifierResolvingSceneIndex::FilteringFnc HdSt_NodeIdentifierResolvingSceneIndex::
    _GetFilteringFunction() const
{
