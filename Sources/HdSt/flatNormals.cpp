...
HdSt_FlatNormalsComputationCPU::HdSt_FlatNormalsComputationCPU(
    ...
    bool packed)
    : _topology(topology), _points(points), _dstName(dstName), _packed(packed)
{
}

void HdSt_FlatNormalsComputationCPU::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
TfToken const &HdSt_FlatNormalsComputationCPU::GetName() const
{
  return _dstName;
}

bool HdSt_FlatNormalsComputationCPU::Resolve()
{
...
bool HdSt_FlatNormalsComputationCPU::_CheckValid() const
{
...
int HdSt_FlatNormalsComputationGPU::GetNumOutputElements() const
{
  return _numFaces;
}

void HdSt_FlatNormalsComputationGPU::Execute(HdBufferArrayRangeSharedPtr const &range_,
                                             HdResourceRegistry *resourceRegistry)
{
  ...
  HdStBufferArrayRangeSharedPtr range = std::static_pointer_cast<HdStBufferArrayRange>(range_);
  HdStBufferArrayRangeSharedPtr vertexRange = std::static_pointer_cast<HdStBufferArrayRange>(
      _vertexRange);
  HdStBufferArrayRangeSharedPtr topologyRange = std::static_pointer_cast<HdStBufferArrayRange>(
      _topologyRange);
  ...
  HdStBufferResourceSharedPtr indices = topologyRange->GetResource(HdTokens->indices);
  ...
  HdStGLSLProgramSharedPtr computeProgram = HdStGLSLProgram::GetComputeProgram(
      shaderToken, hdStResourceRegistry, [&](HgiShaderFunctionDesc &computeDesc) {
        ...
        HgiShaderFunctionAddBuffer(
            &computeDesc, "points", srcType, BufferBinding_Points, HgiBindingTypePointer);
        HgiShaderFunctionAddWritableBuffer(
            &computeDesc, "normals", dstType, BufferBinding_Normals);
  ...
  uint64_t rbHash = (uint64_t)TfHash::Combine(points->GetHandle().GetId(),
...
void HdSt_FlatNormalsComputationGPU::GetBufferSpecs(HdBufferSpecVector *specs) const
{
