...
TF_DEFINE_ENV_SETTING(HDST_ENABLE_BROKEN_SHADER_VISUAL_FEEDBACK,
                      false,
...
bool _ProvideVisualFeedbackForBrokenShaders()
{
  static const bool enabled = TfGetEnvSetting(HDST_ENABLE_BROKEN_SHADER_VISUAL_FEEDBACK);
  return enabled;
}

const std::string &_GetPrimPathSubstringForDebugLogging()
{
  ...
  //
  static const std::string substring = TfGetenv("HDST_DEBUG_SHADER_PROGRAM_FOR_PRIM");
  return substring;
}

bool _LogShaderCacheLookupForDrawBatch(
    std::vector<HdStDrawItemInstance const *> const &_drawItemInstances)
{
  ...
  for (auto const &drawItemInstance : _drawItemInstances) {
    ...
    if (TF_VERIFY(drawItem)) {
      if (drawItem->GetRprimID().GetString().find(substring) != std::string::npos) {
...
HdSt_DrawBatch::HdSt_DrawBatch(HdStDrawItemInstance *drawItemInstance,
    ...
    : _allowTextureResourceRebinding(allowTextureResourceRebinding), _shaderHash(0)
{
}

/*virtual*/
void HdSt_DrawBatch::_Init(HdStDrawItemInstance *drawItemInstance)
{
...
void HdSt_DrawBatch::DrawItemInstanceChanged(HdStDrawItemInstance const * /*instance*/) {}

void HdSt_DrawBatch::SetEnableTinyPrimCulling(bool tinyPrimCulling) {}
...
bool HdSt_DrawBatch::Append(HdStDrawItemInstance *drawItemInstance)
{
...
/*static*/
bool HdSt_DrawBatch::_CanAggregateMaterials(HdStDrawItem const *drawItem0,
                                            HdStDrawItem const *drawItem1)
{
  if (drawItem0->GetMaterialIsFinal() != drawItem1->GetMaterialIsFinal()) {
    return false;
  }

  HdStShaderCodeSharedPtr const &shaderA = drawItem0->GetMaterialNetworkShader();
  HdStShaderCodeSharedPtr const &shaderB = drawItem1->GetMaterialNetworkShader();
...
bool HdSt_DrawBatch::_CanAggregateTextures(HdStDrawItem const *drawItem0,
                                           HdStDrawItem const *drawItem1)
{
...
bool HdSt_DrawBatch::_IsAggregated(HdStDrawItem const *drawItem0, HdStDrawItem const *drawItem1)
{
...
bool HdSt_DrawBatch::Rebuild()
{
  ...
  HdStDrawItemInstance *batchItem = const_cast<HdStDrawItemInstance *>(instances.front());
  ...
  for (size_t i = 1; i < instances.size(); ++i) {
    HdStDrawItemInstance *item = const_cast<HdStDrawItemInstance *>(instances[i]);
...
static HdSt_MaterialNetworkShaderSharedPtr _GetFallbackMaterialNetworkShader()
{
...
static HdSt_MaterialNetworkShaderSharedPtr _GetInvalidMaterialNetworkShader()
{
  static std::once_flag once;
...
HdSt_DrawBatch::_DrawingProgram &HdSt_DrawBatch::_GetDrawingProgram(
    ...
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  ...
  if (!state->GetUseSceneMaterials() && !firstDrawItem->GetMaterialIsFinal()) {
    materialNetworkShader = _GetFallbackMaterialNetworkShader();
  }

  size_t materialNetworkShaderHash = materialNetworkShader ? materialNetworkShader->ComputeHash() :
                                                             0;
  ...
  if (!_program.GetGLSLProgram() || shaderChanged) {
    ...
    if (!_program.CompileShader(firstDrawItem, resourceRegistry, logCacheLookup)) {
      ...
      const HdSt_MaterialNetworkShaderSharedPtr shader = _ProvideVisualFeedbackForBrokenShaders() ?
                                                             _GetInvalidMaterialNetworkShader() :
                                                             _GetFallbackMaterialNetworkShader();

      _program.SetMaterialNetworkShader(shader);

      bool res = _program.CompileShader(firstDrawItem, resourceRegistry, logCacheLookup);

      // We expect the invalid/fallback shader to always compile.
      TF_VERIFY(res,
                "Failed to compile with the invalid/fallback "
...
bool HdSt_DrawBatch::_DrawingProgram::IsValid() const
{
  return _glslProgram && _glslProgram->Validate();
}

bool HdSt_DrawBatch::_DrawingProgram::CompileShader(
    ...
    bool logCacheLookup /* = false */)
{
  ...
  HdSt_CodeGen codeGen(_geometricShader, shaders, drawItem->GetMaterialTag(), std::move(metaData));

  HdStGLSLProgram::ID hash = codeGen.ComputeHash();

  {
    // ask registry to see if there's already compiled program
    HdInstance<HdStGLSLProgramSharedPtr> programInstance = resourceRegistry->RegisterGLSLProgram(
        hash);

    if (programInstance.IsFirstInstance()) {

      if (TfDebug::IsEnabled(HDST_LOG_DRAWING_SHADER_PROGRAM_MISSES) && logCacheLookup) {
      ...
      HdStGLSLProgramSharedPtr glslProgram = codeGen.Compile(resourceRegistry.get());
      ...
      if (TfDebug::IsEnabled(HDST_LOG_DRAWING_SHADER_PROGRAM_HITS) && logCacheLookup) {
...
/* virtual */
void HdSt_DrawBatch::_DrawingProgram::_GetCustomBindings(HdStBindingRequestVector *customBindings,
...
/* virtual */
bool HdSt_DrawBatch::_DrawingProgram::_Link(HdStGLSLProgramSharedPtr const &glslProgram)
{
