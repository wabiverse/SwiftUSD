                                              ...
                                              HdSt_MaterialParamVector *params)
{
  ...
  rotParam.name = TfToken(paramName.GetString() + "_" + HdStTokens->rotation.GetString());
  rotParam.fallbackValue = _GetParamFallbackValue(network, node, HdStTokens->rotation);
  params->push_back(std::move(rotParam));

  HdSt_MaterialParam scaleParam;
  scaleParam.paramType = HdSt_MaterialParam::ParamTypeFallback;
  scaleParam.name = TfToken(paramName.GetString() + "_" + HdStTokens->scale.GetString());
  scaleParam.fallbackValue = _GetParamFallbackValue(network, node, HdStTokens->scale);
  ...
  transParam.name = TfToken(paramName.GetString() + "_" + HdStTokens->translation.GetString());
  transParam.fallbackValue = _GetParamFallbackValue(network, node, HdStTokens->translation);
...
//
static std::unique_ptr<HdStSubtextureIdentifier> _GetSubtextureIdentifier(
    ...
    const TfToken &sourceColorSpace)
{
  ...
  if (textureType == HdStTextureType::Udim) {
    return std::make_unique<HdStUdimSubtextureIdentifier>(premultiplyAlpha, sourceColorSpace);
  }
    ...
    TfToken const &materialTag)
{
  ...
  if (paramName == _tokens->diffuseColor && materialTag != HdStMaterialTagTokens->masked) {
  ...
  if (sourceColorSpace.IsEmpty()) {
    const std::string sourceColorSpaceStr = _ResolveParameter(
        node, sdrNode, _tokens->sourceColorSpace, HdStTokens->colorSpaceAuto.GetString());
      ...
      if (upstreamSdr) {
        ...
        if (sdrRole == SdrNodeRole->Primvar) {
          ...
          if (!transform2dParams.empty()) {
            HdSt_MaterialParam const &transform2dParam = transform2dParams.front();
  ...
  texScaleParam.name = TfToken(paramName.GetString() + "_" + HdStTokens->storm.GetString() + "_" +
                               HdStTokens->scale.GetString());
  texScaleParam.fallbackValue = VtValue(
      _ResolveParameter(node, sdrNode, HdStTokens->scale, GfVec4f(1.0f)));
  ...
  texBiasParam.name = TfToken(paramName.GetString() + "_" + HdStTokens->storm.GetString() + "_" +
                              HdStTokens->bias.GetString());
  texBiasParam.fallbackValue = VtValue(
      _ResolveParameter(node, sdrNode, HdStTokens->bias, GfVec4f(0.0f)));
                                  ...
                                  TfToken const &materialTag)
{
  ...
  for (HdSt_MaterialParam &p : *params) {
    if (p.paramType != HdSt_MaterialParam::ParamTypeAdditionalPrimvar && p.fallbackValue.IsEmpty())
    {
...
HdStMaterialNetwork::HdStMaterialNetwork()
    : _materialTag(HdStMaterialTagTokens->defaultMaterialTag), _surfaceGfxHash(0)
{
}

HdStMaterialNetwork::~HdStMaterialNetwork() = default;

void HdStMaterialNetwork::ProcessMaterialNetwork(SdfPath const &materialId,
                                                 ...
                                                 HdStResourceRegistry *resourceRegistry)
{
  ...
  // 'volume' network.
  bool isVolume = false;
  HdMaterialNetwork2 surfaceNetwork = HdConvertToHdMaterialNetwork2(hdNetworkMap, &isVolume);
...
#ifdef PXR_MATERIALX_SUPPORT_ENABLED
    if (!isVolume) {
      _materialXGfx = HdSt_ApplyMaterialXFilter(&surfaceNetwork,
                                                materialId,
                                                *surfTerminal,
                                                surfTerminalPath,
                                                &_materialParams,
...
TfToken const &HdStMaterialNetwork::GetMaterialTag() const
{
  return _materialTag;
}

std::string const &HdStMaterialNetwork::GetFragmentCode() const
{
  return _fragmentSource;
}

std::string const &HdStMaterialNetwork::GetVolumeCode() const
{
  return _volumeSource;
}

std::string const &HdStMaterialNetwork::GetGeometryCode() const
{
  return _geometrySource;
}

std::string const &HdStMaterialNetwork::GetDisplacementCode() const
{
  return _displacementSource;
}

VtDictionary const &HdStMaterialNetwork::GetMetadata() const
{
  return _materialMetadata;
}

HdSt_MaterialParamVector const &HdStMaterialNetwork::GetMaterialParams() const
{
  return _materialParams;
}

HdStMaterialNetwork::TextureDescriptorVector const &HdStMaterialNetwork::GetTextureDescriptors()
    const
{
