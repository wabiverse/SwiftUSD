...
void HdStMesh::UpdateRenderTag(HdSceneDelegate *delegate, HdRenderParam *renderParam)
{
  HdStUpdateRenderTag(delegate, renderParam, this);
}

void HdStMesh::Sync(HdSceneDelegate *delegate,
                    ...
                    TfToken const &reprToken)
{
  ...
  if (*dirtyBits & (HdChangeTracker::DirtyDisplayStyle | HdChangeTracker::NewRepr)) {
    updateMaterialTags = true;
  }
  ...
  if (updateMaterialTags || (GetMaterialId().IsEmpty() && displayOpacity != _displayOpacity)) {
    _UpdateMaterialTagsForAllReprs(delegate, renderParam);
  }

  if (updateMaterialNetworkShader || updateGeometricShader) {
    _UpdateShadersForAllReprs(
        delegate, renderParam, updateMaterialNetworkShader, updateGeometricShader);
  }
...
void HdStMesh::Finalize(HdRenderParam *renderParam)
{
  HdStMarkGarbageCollectionNeeded(renderParam);

  HdStRenderParam *const stRenderParam = static_cast<HdStRenderParam *>(renderParam);
  ...
  for (auto const &reprPair : _reprs) {
    ...
    _MeshReprConfig::DescArray const &descs = _GetReprDesc(reprToken);
    ...
    for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
      ...
      {
        HdStDrawItem *drawItem = static_cast<HdStDrawItem *>(repr->GetDrawItem(drawItemIndex++));
      ...
      for (size_t i = 0; i < numGeomSubsets; ++i) {
        HdStDrawItem *drawItem = static_cast<HdStDrawItem *>(
            repr->GetDrawItemForGeomSubset(geomSubsetDescIndex, numGeomSubsets, i));
...
HdMeshTopologySharedPtr HdStMesh::GetTopology() const
{
  return _topology;
}
...
/* static */
bool HdStMesh::IsEnabledPackedNormals()
{
                                            ...
                                            HdSt_MeshTopologySharedPtr topology)
{
  ...
  HdPrimvarDescriptorVector primvars = HdStGetPrimvarDescriptors(
      this,
      drawItem,
      sceneDelegate,
...
void HdStMesh::_UpdateDrawItemsForGeomSubsets(HdSceneDelegate *sceneDelegate,
                                              HdRenderParam *renderParam,
                                              HdStDrawItem *drawItem,
                                              ...
                                              size_t oldNumGeomSubsets)
{
  ...
  if (numGeomSubsets != oldNumGeomSubsets) {
    ...
    if (drawItem->HasInstancer()) {
      const size_t numInstanceLevels = drawItem->GetInstancePrimvarNumLevels();
      if (numGeomSubsets < oldNumGeomSubsets) {
        // less geom susbets than before
        // move instance primvar levels toward start
        for (size_t i = 0; i < numInstanceLevels; ++i) {
          HdBufferArrayRangeSharedPtr instancePvRange = drawItem->GetInstancePrimvarRange(i);
          HdStUpdateDrawItemBAR(
              instancePvRange, newInstancePvIndex + i, &_sharedData, renderParam, &changeTracker);
        }
      }
      else {
    ...
    for (auto const &reprPair : _reprs) {
      ...
      if (oldNumGeomSubsets != 0) {
        ...
        for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
          ...
          if (desc.geomStyle == HdMeshGeomStyleInvalid || desc.geomStyle == HdMeshGeomStylePoints)
          {
            continue;
          }
          ...
          for (size_t i = 0; i < oldNumGeomSubsets; ++i) {
            HdStDrawItem *subsetDrawItem = static_cast<HdStDrawItem *>(
                currRepr->GetDrawItemForGeomSubset(geomSubsetDescIndex, oldNumGeomSubsets, i));
      ...
      for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
        ...
        mainDrawItem->GetDrawingCoord()->SetInstancePrimvarBaseIndex(newInstancePvIndex);
        ...
        for (size_t i = 0; i < numGeomSubsets; ++i) {
          ...
          std::unique_ptr<HdStDrawItem> subsetDrawItem = std::make_unique<HdStDrawItem>(
              &_sharedData);
          subsetDrawItem->SetMaterialNetworkShader(
              HdStGetMaterialNetworkShader(this, sceneDelegate, geomSubset.materialId));

          // Each of the geom subset draw items need to have a unique
          // topology drawing coord
          HdDrawingCoord *drawingCoord = subsetDrawItem->GetDrawingCoord();
          switch (desc.geomStyle) {
            case HdMeshGeomStyleHull:
            case HdMeshGeomStyleHullEdgeOnly:
            case HdMeshGeomStyleHullEdgeOnSurf: {
              drawingCoord->SetTopologyIndex(HdStMesh::FreeSlot + 2 * i + 1);
              break;
            }
            default: {
              drawingCoord->SetTopologyIndex(HdStMesh::FreeSlot + 2 * i);
              break;
            }
          }
          drawingCoord->SetInstancePrimvarBaseIndex(newInstancePvIndex);
                                 ...
                                 int geomSubsetDescIndex)
{
      ...
      HdChangeTracker::IsAnyPrimvarDirty(*dirtyBits, id))
  {
    ...
    HdSt_MeshTopology::RefineMode refineMode = HdSt_MeshTopology::RefineModeUniform;
    _limitNormals = false;
    ...
    if (dirtyTopology) {
      HdStProcessTopologyVisibility(meshTopology.GetInvisibleFaces(),
    ...
    bool const hasBuiltinBarycentrics = resourceRegistry->GetHgi()->GetCapabilities()->IsSet(
    ...
    HdSt_MeshTopologySharedPtr topology = HdSt_MeshTopology::New(
        meshTopology,
        refineLevel,
        refineMode,
        (hasBuiltinBarycentrics || hasMetalTessellation) ? HdSt_MeshTopology::QuadsTriangulated :
                                                           HdSt_MeshTopology::QuadsUntriangulated);

    // Gather and sanitize geom subsets
    const HdGeomSubsets &oldGeomSubsets = _topology ? _topology->GetGeomSubsets() :
                                                      HdGeomSubsets();
    ...
    if (geomSubsets != oldGeomSubsets) {
      _UpdateDrawItemsForGeomSubsets(sceneDelegate,
                                     renderParam,
                                     drawItem,
                                     reprToken,
                                     repr,
        ...
        HdChangeTracker::IsAnyPrimvarDirty(*dirtyBits, id))
    {
      _GatherFaceVaryingTopologies(
          sceneDelegate, repr, desc, drawItem, geomSubsetDescIndex, dirtyBits, id, topology);
    ...
    for (const auto &it : _fvarTopologyTracker->GetTopologyToPrimvarVector()) {
      ...
      for (const TfToken &it2 : it.second) {
        _topologyId = ArchHash64(it2.GetText(), it2.size(), _topologyId);
      }
    }
...

  {
    ...
    if (geomSubsets.empty() || desc.geomStyle == HdMeshGeomStylePoints) {
      ...
      if (rangeInstance.IsFirstInstance()) {
        ...
        HdBufferArrayUsageHint usageHint = HdBufferArrayUsageHintBitsIndex |
      ...
      HdStUpdateDrawItemBAR(newRange,
                            drawItem->GetDrawingCoord()->GetTopologyIndex(),
                            &_sharedData,
                            renderParam,
                            &changeTracker);
      ...
      if (refineLevelForDesc > 0) {
        ...
        {
          for (size_t i = 0; i < _fvarTopologyTracker->GetNumTopologies(); ++i) {
            fvarIndicesSource = _topology->GetOsdFvarIndexBuilderComputation(i);
      ...
      HdBufferSourceSharedPtr geomSubsetFaceIndicesHelperSource =
          _topology->GetGeomSubsetFaceIndexHelperComputation(refined, quadrangulated);
      resourceRegistry->AddSource(geomSubsetFaceIndicesHelperSource);

      if (refined) {
        _topology->GetOsdBaseFaceToRefinedFacesMapComputation(resourceRegistry.get());
      }

      // For original draw item
      const std::vector<int> *nonSubsetFaces = _topology->GetNonSubsetFaces();
      _CreateTopologyRangeForGeomSubset(resourceRegistry,
                                        ...
                                        indicesSource,
                                        fvarIndicesSource,
                                        geomSubsetFaceIndicesHelperSource,
    ...
    bool refined)
{
  HdTopology::ID subsetTopologyId = ArchHash64(
      (const char *)faceIndices.cdata(), sizeof(int) * faceIndices.size(), _topologyId);
  ...
  if (rangeInstance.IsFirstInstance()) {
    ...
    HdBufferSourceSharedPtr geomSubsetFaceIndicesSource =
        _topology->GetGeomSubsetFaceIndexBuilderComputation(geomSubsetFaceIndicesHelperSource,
                                                            faceIndices);

    if (refined) {
      resourceRegistry->AddSource(geomSubsetFaceIndicesSource);

      HdBufferSourceSharedPtr subsetSource = _topology->GetRefinedIndexSubsetComputation(
          indicesSource, geomSubsetFaceIndicesSource);
      sources.push_back(subsetSource);

      if (fvarIndicesSource) {
        HdBufferSourceSharedPtr fvarSubsetSource = _topology->GetRefinedIndexSubsetComputation(
            fvarIndicesSource, geomSubsetFaceIndicesSource);
        sources.push_back(fvarSubsetSource);
      }
    }
    else {
      HdBufferSourceSharedPtr subsetSource = _topology->GetIndexSubsetComputation(
          indicesSource, geomSubsetFaceIndicesSource);
      sources.push_back(subsetSource);
...
void HdStMesh::_PopulateAdjacency(HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  ...
  HdInstance<HdSt_VertexAdjacencyBuilderSharedPtr> vertexAdjacencyBuilderInstance =
  ...
  if (vertexAdjacencyBuilderInstance.IsFirstInstance()) {
    ...
    HdBufferSourceSharedPtr vertexAdjacencyComputation =
        vertexAdjacencyBuilder->GetSharedVertexAdjacencyBuilderComputation(_topology.get());
    ...
    HdBufferSourceSharedPtr vertexAdjacencyBufferSource =
        std::make_shared<HdSt_VertexAdjacencyBufferSource>(
            vertexAdjacencyBuilder->GetVertexAdjacency(), vertexAdjacencyComputation);
    ...
    HdBufferArrayRangeSharedPtr vertexAdjacencyRange =
        resourceRegistry->AllocateNonUniformBufferArrayRange(
            HdTokens->topology, bufferSpecs, HdBufferArrayUsageHintBitsStorage);

    vertexAdjacencyBuilder->SetVertexAdjacencyRange(vertexAdjacencyRange);
    resourceRegistry->AddSource(vertexAdjacencyRange, vertexAdjacencyBufferSource);
                                  ...
                                  HdStComputationComputeQueuePairVector *computations)
{
  ...
  HdStComputationSharedPtr computation = topology->GetQuadrangulateComputationGPU(
                           ...
                           int channel = 0)
{
  ...
  // GPU subdivision
  HdStComputationSharedPtr computation = topology->GetOsdRefineComputationGPU(
      source->GetName(),
...
{
  if (doRefine) {
    _RefinePrimvar(source, topology, resourceRegistry, computations, interpolation);
  }
    ...
    int channel)
{
  ...
  if (doRefine) {
    _RefinePrimvar(source,
                   ...
                   resourceRegistry,
                   computations,
                   HdSt_MeshTopology::INTERPOLATE_FACEVARYING,
                   channel);
  }
  else if (doQuadrangulate) {
...
static bool _GetDoubleSupport(const HdStResourceRegistrySharedPtr &resourceRegistry)
{
  const HgiCapabilities *capabilities = resourceRegistry->GetHgi()->GetCapabilities();
                                       ...
                                       bool requireSmoothNormals)
{
  ...
  HdPrimvarDescriptorVector varyingPvs = HdStGetPrimvarDescriptors(
      this,
      drawItem,
      sceneDelegate,
  ...
  HdSt_GetExtComputationPrimvarsComputations(id,
                                             sceneDelegate,
                                             compPrimvars,
                                             *dirtyBits,
                                             &sources,
  ...
  for (HdPrimvarDescriptor const &primvar : primvars) {
    ...
    if (!value.IsEmpty()) {
      ...
      if (source->GetNumElements() == 0 && source->GetName() != HdTokens->points) {
  ...
  if (requireSmoothNormals && (*dirtyBits & DirtySmoothNormals)) {
    ...
    *dirtyBits &= ~DirtySmoothNormals;
    ...
    generatedNormalsName = usePackedSmoothNormals ? HdStTokens->packedSmoothNormals :
    ...
    if (_pointsDataType != HdTypeInvalid) {
      ...
      HdStComputationSharedPtr smoothNormalsComputation =
          std::make_shared<HdSt_SmoothNormalsComputationGPU>(_vertexAdjacencyBuilder.get(),
                                                             HdTokens->points,
      ...
      if (doRefine) {
        HdStComputationSharedPtr computation = _topology->GetOsdRefineComputationGPU(
            HdStTokens->smoothNormals,
            _pointsDataType,
            resourceRegistry.get(),
            HdSt_MeshTopology::INTERPOLATE_VERTEX);
        ...
        HdStComputationSharedPtr computation = _topology->GetQuadrangulateComputationGPU(
            HdStTokens->smoothNormals, _pointsDataType, GetId());
  ...
  if (hasDirtyPrimvarDesc) {
    ...
    if (!generatedNormalsName.IsEmpty()) {
      ...
      internallyGeneratedPrimvars = {HdStTokens->packedSmoothNormals, HdStTokens->smoothNormals};
    }
  ...
  if (HdStIsEnabledSharedVertexPrimvar()) {
    ...
    if (!HdStIsValidBAR(bar)) {
      ...
      _vertexPrimvarId = HdStComputeSharedPrimvarId(_topologyId, allSources, computations);
                                            ...
                                            HdDirtyBits *dirtyBits)
{
  ...
  HdBufferArrayRangeSharedPtr range = resourceRegistry->UpdateNonUniformBufferArrayRange(
      HdTokens->primvar, bar, bufferSpecs, removedSpecs, HdBufferArrayUsageHintBitsStorage);
                                        ...
                                        bool requireFlatNormals)
{
  ...
  if (requireFlatNormals && (*dirtyBits & DirtyFlatNormals)) {
    *dirtyBits &= ~DirtyFlatNormals;
    ...
    generatedNormalsName = usePackedNormals ? HdStTokens->packedFlatNormals :
                                              HdStTokens->flatNormals;
    ...
    if (_pointsDataType != HdTypeInvalid) {
      ...
      HdStComputationSharedPtr flatNormalsComputation =
          std::make_shared<HdSt_FlatNormalsComputationGPU>(drawItem->GetTopologyRange(),
  ...
  if (hasDirtyPrimvarDesc) {
    ...
    if (!generatedNormalsName.IsEmpty()) {
      ...
      internallyGeneratedPrimvars = {HdStTokens->packedFlatNormals, HdStTokens->flatNormals};
    }
...
bool HdStMesh::_MaterialHasPtex(const HdRenderIndex &renderIndex, const SdfPath &materialId) const
{
                               ...
                               const HdSt_MeshTopologySharedPtr &topology) const
{
  ...
  materialHasPtex = materialHasPtex || _MaterialHasPtex(renderIndex, GetMaterialId());

  const HdGeomSubsets &geomSubsets = topology->GetGeomSubsets();
  for (const HdGeomSubset &geomSubset : geomSubsets) {
    materialHasPtex = materialHasPtex || _MaterialHasPtex(renderIndex, geomSubset.materialId);
  }
...
bool HdStMesh::_MaterialHasLimitSurface(const HdRenderIndex &renderIndex,
...
bool HdStMesh::_UseLimitRefinement(const HdRenderIndex &renderIndex,
...
static bool _CanUseTriangulatedFlatNormals(HdSt_MeshTopologySharedPtr const &topology)
{
  // For triangle subdivison or subdivision scheme "none" we
  // can use triangulated flat normals.
  return topology->RefinesToTriangles() || topology->GetScheme() == PxOsdOpenSubdivTokens->none;
}

HdBufferArrayRangeSharedPtr HdStMesh::_GetSharedPrimvarRange(
    uint64_t primvarId,
    ...
    HdStResourceRegistrySharedPtr const &resourceRegistry) const
{
  ...
  if (barInstance.IsFirstInstance()) {
    ...
    range = resourceRegistry->UpdateNonUniformImmutableBufferArrayRange(
        HdTokens->primvar, curRange, updatedOrAddedSpecs, removedSpecs, usageHint);
                               ...
                               int geomSubsetDescIndex)
{
      ...
      HdChangeTracker::IsAnyPrimvarDirty(*dirtyBits, id))
  {
    ...
    if (desc.geomStyle != HdMeshGeomStylePoints) {
      const HdGeomSubsets &geomSubsets = _topology ? _topology->GetGeomSubsets() : HdGeomSubsets();
      const size_t numGeomSubsets = geomSubsets.size();
      for (size_t i = 0; i < numGeomSubsets; ++i) {
        HdStDrawItem *subsetDrawItem = static_cast<HdStDrawItem *>(
            repr->GetDrawItemForGeomSubset(geomSubsetDescIndex, numGeomSubsets, i));
        ...
        subsetDrawItem->SetMaterialNetworkShader(
            HdStGetMaterialNetworkShader(this, sceneDelegate, geomSubsets[i].materialId));
      }
    }
  }
  ...
  if (HdStShouldPopulateConstantPrimvars(dirtyBits, id)) {
    HdPrimvarDescriptorVector constantPrimvars = HdStGetPrimvarDescriptors(
        this,
        drawItem,
        sceneDelegate,
  ...
  if (HdChangeTracker::IsAnyPrimvarDirty(*dirtyBits, id)) {
    _PopulateFaceVaryingPrimvars(
        sceneDelegate, renderParam, repr, desc, drawItem, geomSubsetDescIndex, dirtyBits);
  }
                                              ...
                                              const SdfPath &materialId)
{
  ...
  // entrypoint exists.
  bool hasCustomDisplacement = hasCustomDisplacementTerminal && desc.useCustomDisplacement &&
  ...
  HdStResourceRegistrySharedPtr resourceRegistry = std::static_pointer_cast<HdStResourceRegistry>(
...
// virtual
HdDirtyBits HdStMesh::_PropagateDirtyBits(HdDirtyBits bits) const
{
...
void HdStMesh::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits)
{
  ...
  if (isNew) {
    ...
    *dirtyBits |= HdChangeTracker::NewRepr;
    ...
    size_t numGeomSubsets = _topology ? _topology->GetGeomSubsets().size() : 0;
    ...
    for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
      ...
      {
        HdRepr::DrawItemUniquePtr drawItem = std::make_unique<HdStDrawItem>(&_sharedData);
        ...
        drawingCoord->SetInstancePrimvarBaseIndex(HdStMesh::FreeSlot + 2 * numGeomSubsets);
      }
                           ...
                           HdDirtyBits *dirtyBits)
{
  ...
  for (size_t descIdx = 0; descIdx < reprDescs.size(); ++descIdx) {
    ...
    HdStDrawItem *drawItem = static_cast<HdStDrawItem *>(curRepr->GetDrawItem(drawItemIndex++));
    ...
    for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
      ...
      // Update original draw item
      {
        ...
        if (updateMaterialNetworkShader) {
          materialNetworkShader = HdStGetMaterialNetworkShader(this, sceneDelegate);
          drawItem->SetMaterialNetworkShader(materialNetworkShader);
        }
        if (updateGeometricShader) {
          _UpdateDrawItemGeometricShader(
              sceneDelegate, renderParam, drawItem, descs[descIdx], GetMaterialId());
        }
      }
      ...
      for (size_t i = 0; i < numGeomSubsets; ++i) {
        ...
        if (updateMaterialNetworkShader) {
          materialNetworkShader = HdStGetMaterialNetworkShader(this, sceneDelegate, materialId);
          drawItem->SetMaterialNetworkShader(materialNetworkShader);
        }
        if (updateGeometricShader) {
          _UpdateDrawItemGeometricShader(
              sceneDelegate, renderParam, drawItem, descs[descIdx], materialId);
        }
      }
      geomSubsetDescIndex++;
    }
...
void HdStMesh::_UpdateMaterialTagsForAllReprs(HdSceneDelegate *sceneDelegate,
                                              HdRenderParam *renderParam)
{
  TF_DEBUG(HD_RPRIM_UPDATED)
      .Msg("(%s) - Updating material tags for draw items of all reprs.\n", GetId().GetText());
  ...
  for (auto const &reprPair : _reprs) {
    ...
    for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
      ...
      {
        HdStDrawItem *drawItem = static_cast<HdStDrawItem *>(repr->GetDrawItem(drawItemIndex++));
        HdStSetMaterialTag(sceneDelegate,
                           renderParam,
                           drawItem,
                           this->GetMaterialId(),
                           _displayOpacity,
...
HdDirtyBits HdStMesh::GetInitialDirtyBitsMask() const
{
