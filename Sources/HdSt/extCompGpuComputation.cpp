...
void HdStExtCompGpuComputation::Execute(HdBufferArrayRangeSharedPtr const &outputRange,
                                        HdResourceRegistry *resourceRegistry)
{
  ...
  HdStBufferArrayRangeSharedPtr outputBar = std::static_pointer_cast<HdStBufferArrayRange>(
      outputRange);
  TF_VERIFY(outputBar);
  ...
  for (HdExtComputationPrimvarDescriptor const &compPrimvar : _compPrimvars) {
    ...
    HdStBufferResourceSharedPtr const &buffer = outputBar->GetResource(compPrimvar.name);
  ...
  for (HdBufferArrayRangeSharedPtr const &input : _resource->GetInputs()) {
    HdStBufferArrayRangeSharedPtr const &inputBar = std::static_pointer_cast<HdStBufferArrayRange>(
        input);

    for (HdStBufferResourceNamedPair const &it : inputBar->GetResources()) {
  ...
  if (resourceBindingsInstance.IsFirstInstance()) {
    ...
    for (HdExtComputationPrimvarDescriptor const &compPvar : _compPrimvars) {
      ...
      HdStBufferResourceSharedPtr const &buffer = outputBar->GetResource(compPvar.name);
...
void HdStExtCompGpuComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // nothing
}

int HdStExtCompGpuComputation::GetDispatchCount() const
{
  return _dispatchCount;
}

int HdStExtCompGpuComputation::GetNumOutputElements() const
{
  return _elementCount;
}

HdStExtCompGpuComputationResourceSharedPtr const &HdStExtCompGpuComputation::GetResource() const
{
  return _resource;
}

/* static */
HdStExtCompGpuComputationSharedPtr HdStExtCompGpuComputation::CreateGpuComputation(
    ...
    HdExtComputationPrimvarDescriptorVector const &compPrimvars)
{
  ...
  HdSt_ExtCompComputeShaderSharedPtr shader = std::make_shared<HdSt_ExtCompComputeShader>(
      sourceComp);
  ...
  HdStExtComputation const *deviceSourceComp = static_cast<HdStExtComputation const *>(sourceComp);
  ...
  for (HdExtComputationInputDescriptor const &desc : sourceComp->GetComputationInputs()) {
    HdStExtComputation const *deviceInputComp = static_cast<HdStExtComputation const *>(
  ...
  return std::make_shared<HdStExtCompGpuComputation>(sourceComp->GetId(),
                                                     resource,
                                                     compPrimvars,
    ...
    if (!sourceComp->GetGpuKernelSource().empty()) {
      ...
      for (HdExtComputationPrimvarDescriptor const &compPrimvar : compPrimvars) {
        ...
        if (HdChangeTracker::IsPrimvarDirty(dirtyBits, id, compPrimvar.name)) {
          ...
          if (!cpuComputation) {
            // Create the computation for the first dirty primvar
            cpuComputation = HdStExtCompCpuComputation::CreateComputation(
