...
{
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DRAW, "Reports diagnostics for drawing");
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DRAW_BATCH, "Reports diagnostics for draw batches");
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_FORCE_DRAW_BATCH_REBUILD, "Forces rebuild of draw batches.");
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DRAW_ITEM_GATHER,
                              "Reports when draw items are fetched for a render pass.");
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DRAWITEMS_CACHE, "Reports lookups from the draw items cache.");

  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DISABLE_FRUSTUM_CULLING, "Disable view frustum culling");
  ...
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DUMP_GLSLFX_CONFIG, "Print composed GLSLFX configuration");
  ...
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DUMP_FAILING_SHADER_SOURCEFILE,
                              ...
                              "fail compilation");
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_DUMP_SHADER_SOURCE, "Print generated shader source code");
  ...
  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_MATERIAL_ADDED, "Report when a material is added");

  TF_DEBUG_ENVIRONMENT_SYMBOL(HDST_MATERIAL_REMOVED, "Report when a material is removed");
}
