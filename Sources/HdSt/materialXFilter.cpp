...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (st)(texcoord)(geomprop)(index)((defaultInput, "default"))(filename)

    // Opacity Parameters - USD Preview Surface
    (UsdPreviewSurface)(opacity)(opacityThreshold)
    // Opacity Parameters - Standard Surface
    (standard_surface)(transmission)
    // Opacity Parameters - Open PBR
    (open_pbr_surface)(transmission_weight)(geometry_opacity)
    // Opacity Parameters - GlTF
    (gltf_pbr)(alpha_mode)(alpha_cutoff)(alpha)
    // Opacity Parameters - Other Surface shaders
    (convert)(ND_convert_color4_surfaceshader)(ND_convert_vector4_surfaceshader)

    // Fallback Dome Light Tokens
    (domeLightFallback)(ND_image_color3)(file)
...
TF_DEFINE_PRIVATE_TOKENS(_topologicalTokens,
...
TF_DEFINE_PRIVATE_TOKENS(_mxTextureParamTokens,  // mxTextureTokens <-> HdStTextureTokens
                         (filtertype)(uaddressmode)(vaddressmode));
...
static mx::ShaderPtr _GenMaterialXShader(mx::GenContext &mxContext, mx::ElementPtr const &mxElem)
{
...
static mx::GenContext _CreateHdStMaterialXContext(HdSt_MxShaderGenInfo const &mxHdInfo,
                                                  TfToken const &apiName)
{
  ...
  if (apiName == HgiTokens->OpenGL) {
    ...
    TF_CODING_ERROR("MaterialX Shader Generator doesn't support %s API.", apiName.GetText());
...
mx::ShaderPtr HdSt_GenMaterialXShader(mx::DocumentPtr const &mxDoc,
                                      ...
                                      TfToken const &apiName)
{
  ...
  mxContext.getOptions().hwTransparency = mxHdInfo.materialTag !=
...
static void _GetMxInputAsHdTextureParam(std::string const &mxInputName,
                                        ...
                                        std::map<TfToken, VtValue> *hdTextureParams)
{
  ...
  if (mxInputName == _mxTextureParamTokens->filtertype) {
    (*hdTextureParams)[HdStTextureTokens->minFilter] = _GetHdFilterValue(mxInputValue);
    (*hdTextureParams)[HdStTextureTokens->magFilter] = VtValue(HdStTextureTokens->linear);
  }

  // Properties specific to <image> nodes:
  else if (mxInputName == _mxTextureParamTokens->uaddressmode) {
    (*hdTextureParams)[HdStTextureTokens->wrapS] = _GetHdSamplerValue(mxInputValue);
  }
  else if (mxInputName == _mxTextureParamTokens->vaddressmode) {
    (*hdTextureParams)[HdStTextureTokens->wrapT] = _GetHdSamplerValue(mxInputValue);
  }
}

static void _AddDefaultMtlxTextureValues(mx::NodeDefPtr const &nodeDef,
                                         std::map<TfToken, VtValue> *hdTextureParams)
{
  // Add the stdlib texture node default values
  {
    ...
    (*hdTextureParams)[HdStTextureTokens->wrapS] = VtValue(HdStTextureTokens->repeat);
    (*hdTextureParams)[HdStTextureTokens->wrapT] = VtValue(HdStTextureTokens->repeat);
  ...
  // Add custom texture node default values
  {
    ...
    for (TfToken const &inputName : _mxTextureParamTokens->allTokens) {
      ...
      if (mxInput->hasValueString()) {
        _GetMxInputAsHdTextureParam(inputName, mxInput->getValueString(), hdTextureParams);
      }
    }
  }
}

static void _AddFallbackDomeLightTextureNode(HdMaterialNetwork2 *hdNetwork,
                                             SdfPath const &hdTerminalNodePath)
{
  ...
  hdDomeTextureNode.parameters[_tokens->file] = VtValue(
      SdfAssetPath(HdStPackageFallbackDomeLightTexture(), HdStPackageFallbackDomeLightTexture()));
  const SdfPath domeTexturePath = hdTerminalNodePath.ReplaceName(_tokens->domeLightFallback);
  ...
  hdNetwork->nodes[hdTerminalNodePath].inputConnections[domeTextureConn.upstreamOutputName] = {
      domeTextureConn};
}
...
static void _UpdateMxHdTextureMap(std::set<SdfPath> const &hdTextureNodes,
                                  ...
                                  mx::StringMap *mxHdTextureMap)
{
  ...
  for (SdfPath const &texturePath : hdTextureNodes) {
    ...
    if (mtlxTextureInfo != hdMtlxTextureInfo.end()) {
      for (std::string const &fileInputName : mtlxTextureInfo->second) {
        ...
        const std::string newConnName = texturePath.GetName() + "_" + fileInputName;
  ...
  const mx::NodeDefPtr mxMaterialNodeDef = HdMtlxStdLibraries()->getNodeDef(
      hdTerminalNode.nodeTypeId.GetString());
...
static void _UpdatePrimvarNodes(mx::DocumentPtr const &mxDoc,
                                ...
                                mx::StringMap *mxHdPrimvarDefaultValueMap)
{
  for (auto const &primvarPath : hdPrimvarNodes) {
    ...
    if (primvarNameIt != hdPrimvarNode.parameters.end()) {
      std::string const &primvarName = HdMtlxConvertToString(primvarNameIt->second);
      ...
      const auto defaultPrimvarValueIt = hdPrimvarNode.parameters.find(_tokens->defaultInput);
      if (hdPrimvarNode.parameters.end() != defaultPrimvarValueIt) {
        defaultPrimvarValue = HdMtlxConvertToString(defaultPrimvarValueIt->second);
      }
    ...
    if (primvarNameIt != hdPrimvarNode.parameters.end()) {
      ...
      const SdrShaderNodeConstPtr sdrTexCoordNode = sdrRegistry.GetShaderNodeByIdentifierAndType(
...
template<typename T>
static bool _ParamDiffersFrom(HdMaterialNode2 const &terminal,
...
static std::string const &_GetUsdPreviewSurfaceMaterialTag(HdMaterialNode2 const &terminal)
{
...
static std::string const &_GetStandardSurfaceMaterialTag(HdMaterialNode2 const &terminal)
{
...
static std::string const &_GetOpenPBRSurfaceMaterialTag(HdMaterialNode2 const &terminal)
{
...
static std::string const &_GetGlTFSurfaceMaterialTag(HdMaterialNode2 const &terminal)
{
...
static const mx::TypeDesc *_GetMxTypeDescription(std::string const &typeName)
{
...
static mx::NodePtr _AddStrippedSurfaceNode(mx::DocumentPtr mxDocument,
                                           std::string const &nodeName,
                                           HdMaterialNode2 const &hdNode,
                                           HdMaterialNetwork2 const &hdNetwork)
{
  // Add the hdNode to the mxDocument
  mx::NodeDefPtr mxNodeDef = HdMtlxStdLibraries()->getNodeDef(hdNode.nodeTypeId.GetString());
  ...
  for (auto const &connIt : hdNode.inputConnections) {
    const mx::InputPtr mxInputDef = mxNodeDef->getActiveInput(connIt.first.GetString());
    if (!mxInputDef) {
      continue;
    }
  ...
  for (auto const &paramIt : hdNode.parameters) {
    const mx::InputPtr mxInputDef = mxNodeDef->getActiveInput(paramIt.first.GetString());
    if (!mxInputDef) {
      continue;
    }
    auto const *mxTypeDesc = _GetMxTypeDescription(mxInputDef->getType());
...
static bool _IsTransparentShader(HdMaterialNetwork2 const &hdNetwork,
...
static std::string const &_GetMaterialTag(HdMaterialNetwork2 const &hdNetwork,
                                          HdMaterialNode2 const &terminal)
{
  SdrRegistry &sdrRegistry = SdrRegistry::GetInstance();
  ...
  mx::NodeDefPtr mxNodeDef = stdLibraries->getNodeDef(mtlxSdrNode->GetIdentifier().GetString());
      ...
      activeOutputs.back()->getType() != mx::SURFACE_SHADER_TYPE_STRING)
  {
    ...
    auto const *typeDesc = _GetMxTypeDescription(activeOutputs.back()->getType());
...
static bool _NodeUsesTexcoordPrimvar(const SdrShaderNodeConstPtr mtlxSdrNode)
{
...
static void _ConnectPrimvarNodesToTerminalNode(SdfPath const &terminalNodePath,
                                               HdMaterialNetwork2 *hdNetwork)
{
  ...
  for (auto &hdNodePair : hdNetwork->nodes) {
    ...
    const std::string newConnName = hdNodePair.first.GetName() + "_primvarconn";
    ...
    hdNetwork->nodes[terminalNodePath].inputConnections[primvarConn.upstreamOutputName] = {
        primvarConn};
  }
}
...
static TfToken _GetDefaultTexcoordName()
{
  SdrRegistry &sdrRegistry = SdrRegistry::GetInstance();
  const SdrShaderNodeConstPtr mtlxSdrNode = sdrRegistry.GetShaderNodeByIdentifierAndType(
  ...
  return primvarIt != metadata.end() ? TfToken(primvarIt->second) : _tokens->st;
}

// Add the Hydra texture node parameters to the texture node and connect the
// texture node to the terminal node
static void _UpdateTextureNode(TfToken mtlxParamName,
                               HdMaterialNetwork2 *hdNetwork,
                               ...
                               SdfPath const &textureNodePath)
{
  ...
  _AddDefaultMtlxTextureValues(
      HdMtlxStdLibraries()->getNodeDef(hdTextureNode.nodeTypeId.GetString()), &hdParameters);
  ...
  hdNetwork->nodes[terminalNodePath].inputConnections[mtlxParamName] = {textureConn};
}

static void _ReplaceFilenameInput(HdMaterialNetwork2 *hdNetwork,
                                  SdfPath const &hdTerminalNodePath,
                                  std::string const &mxFilenameInputName)
{
  const auto &hdTerminalNode = hdNetwork->nodes.at(hdTerminalNodePath);
  const mx::NodeDefPtr mxNodeDef = HdMtlxStdLibraries()->getNodeDef(
      hdTerminalNode.nodeTypeId.GetString());
  if (!mxNodeDef) {
    return;
  }
  ...
  for (mx::NodePtr const &node : impl->asA<mx::NodeGraph>()->getNodes()) {
    for (mx::InputPtr const &input : node->getInputs()) {
      ...
      while (dotNode && dotNode->getCategory() == "dot") {
        ...
        if (dotInput && dotInput->getInterfaceName() == mxFilenameInputName) {
          mxTextureNode = node;
          break;
        }
  ...
  for (TfToken const &mxInputName : _mxTextureParamTokens->allTokens) {
    ...
    if (mxInput && mxInput->hasValueString()) {
      _GetMxInputAsHdTextureParam(mxInputName, mxInput->getValueString(), &terminalTextureParams);
    }
  }
  ...
  for (auto const &param : hdTerminalNode.parameters) {
    ...
    std::string const &mxInputName = param.first.GetString();
    std::string const mxInputValue = HdMtlxConvertToString(param.second);
    _GetMxInputAsHdTextureParam(mxInputName, mxInputValue, &terminalTextureParams);
  }
  ...
  const SdfPath terminalTexturePath = hdTerminalNodePath.AppendChild(filenameToken);
  ...
  hdNetwork->nodes[hdTerminalNodePath].inputConnections[terminalTextureConn.upstreamOutputName] = {
      terminalTextureConn};
}

// Gather the Material Params from the glslfx ShaderPtr
void _AddMaterialXParams(mx::ShaderPtr const &glslfxShader,
                         HdMaterialNetwork2 *hdNetwork,
                         SdfPath const &terminalNodePath,
                         ...
                         HdSt_MaterialParamVector *materialParams)
{
  ...
  for (auto const &node : hdNetwork->nodes) {
    ...
    for (auto const &param : node.second.parameters) {
      if (param.second.IsHolding<std::string>() || param.second.IsHolding<TfToken>()) {
        continue;
      }
      mxParamNameToValue.emplace(annonNodeNamePrefix + param.first.GetString(), param.second);
    }
  }
  ...
  for (auto const &pathPair : hdToAnnonNodePathMap) {
    if (pathPair.first != terminalNodePath) {
      annonToHdNodePathMap.emplace(pathPair.second.GetName(), pathPair.first);
    }
  }
  ...
  for (size_t i = 0; i < paramsBlock.size(); ++i) {
    ...
    const auto paramValueIt = mxParamNameToValue.find(variable->getVariable());
    ...
    // variable to get the value.
      ...
      std::istringstream valueStream(varValue ? varValue->getValueString() : std::string());
      if (varType->getBaseType() == mx::TypeDesc::BASETYPE_BOOLEAN) {
        ...
        if (varType->getSize() == 1) {
          ...
          GfVec4f val;
          valueStream >> val[0] >> separator >> val[1] >> separator >> val[2] >> separator >>
              val[3];
          param.fallbackValue = VtValue(val);
        }
      }
    ...
    if (varType->getSemantic() == mx::TypeDesc::SEMANTIC_FILENAME) {
      ...
      if (hdNodePathIt != annonToHdNodePathMap.end()) {
        _UpdateTextureNode(param.name, hdNetwork, terminalNodePath, hdNodePathIt->second);
      }
      else {
        // Storm does not expect textures/filename to be direct inputs
        // on materials, replace this filename input with a connection
        // to an image node
        _ReplaceFilenameInput(hdNetwork, terminalNodePath, variable->getVariable());
      }
    }
  }
}

static mx::ShaderPtr _GenerateMaterialXShader(HdMaterialNetwork2 const &hdNetwork,
                                              ...
                                              bool const bindlessTexturesEnabled)
{
  ...
  const mx::DocumentPtr mtlxDoc = HdMtlxCreateMtlxDocumentFromHdNetwork(
      hdNetwork, terminalNode, terminalNodePath, materialPath, stdLibraries, &hdMtlxData);

  // Add domelight and other textures to mxHdInfo so the proper entry points
  // get generated in MaterialXShaderGen
  _UpdateMxHdTextureMap(hdMtlxData.hdTextureNodes,
                        hdMtlxData.mxHdTextureMap,
                        terminalNode,
                        terminalNodePath,
                        &mxHdInfo.textureMap);

  _UpdatePrimvarNodes(mtlxDoc,
                      hdNetwork,
                      hdMtlxData.hdPrimvarNodes,
                      &mxHdInfo.primvarMap,
  ...
  return HdSt_GenMaterialXShader(mtlxDoc, stdLibraries, searchPaths, mxHdInfo, apiName);
}
...
static bool _IsTopologicalShader(TfToken const &nodeId)
{
  static const TfToken::HashSet topologicalTokenSet(_topologicalTokens->allTokens.begin(),
...
size_t _BuildEquivalentMaterialNetwork(HdMaterialNetwork2 const &hdNetwork,
                                       ...
                                       HdAnnonNodePathMap *annonNodePathMap)
{
  ...
  for (const auto &terminal : hdNetwork.terminals) {
    topoNetwork->terminals.emplace(
        terminal.first,
        HdMaterialConnection2{(*annonNodePathMap)[terminal.second.upstreamNode],
  ...
  for (const auto &nodePair : hdNetwork.nodes) {
    ...
    if (_IsTopologicalShader(inNode.nodeTypeId)) {
      ...
      for (const auto &param : inNode.parameters) {
        const auto colorManagedInput = SdfPath::StripPrefixNamespace(param.first.GetString(),
    ...
    for (const auto &connPair : inNode.inputConnections) {
      ...
      for (const auto &inConn : connPair.second) {
        outConn.emplace_back(HdMaterialConnection2{(*annonNodePathMap)[inConn.upstreamNode],
                                                   inConn.upstreamOutputName});
      }
      outNode.inputConnections.emplace(connPair.first, std::move(outConn));
    }
    topoNetwork->nodes.emplace((*annonNodePathMap)[nodePair.first], std::move(outNode));
  }
...
mx::ShaderPtr HdSt_ApplyMaterialXFilter(HdMaterialNetwork2 *hdNetwork,
                                        SdfPath const &materialPath,
                                        HdMaterialNode2 const &terminalNode,
                                        SdfPath const &terminalNodePath,
                                        HdSt_MaterialParamVector *materialParams,
                                        HdStResourceRegistry *resourceRegistry)
{
  ...
  auto topoHash = _BuildEquivalentMaterialNetwork(*hdNetwork, &annonNetwork, &annonNodePathMap);
  ...
  const bool bindlessTexturesEnabled = resourceRegistry->GetHgi()->GetCapabilities()->IsSet(
  ...
  HdInstance<mx::ShaderPtr> glslfxInstance = resourceRegistry->RegisterMaterialXShader(
      shaderHash.GetCode());

  if (glslfxInstance.IsFirstInstance()) {
    try {
      glslfxShader = _GenerateMaterialXShader(annonNetwork,
                                              materialPath,
                                              terminalNode,
                                              anonTerminalNodePath,
                                              materialTag,
                                              apiName,
      ...
      TF_CODING_ERROR(
          "Unable to create the Glslfx Shader.\n"
          "MxException: %s",
          exception.what());
    }
  ...
  _AddMaterialXParams(glslfxShader, hdNetwork, terminalNodePath, annonNodePathMap, materialParams);

  // Create a new terminal node with the glslfxShader
  if (glslfxShader) {
    const std::string glslfxSourceCode = glslfxShader->getSourceCode(mx::Stage::PIXEL);
