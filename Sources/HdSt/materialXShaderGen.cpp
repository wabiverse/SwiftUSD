...
void HdStMaterialXShaderGen<Base>::_EmitGlslfxHeader(mx::ShaderStage &mxStage) const
{
    ...
    for (mx::StringMap::const_reference primvarPair : _mxHdPrimvarMap) {
      ...
      if (mxType == nullptr) {
        TF_WARN("MaterialX geomprop '%s' has unknown type '%s'",
                primvarPair.first.c_str(),
                primvarPair.second.c_str());
      }
      const std::string type = mxType ? Base::_syntax->getTypeName(mxType) : "vec2";
...
template<typename Base>
void HdStMaterialXShaderGen<Base>::_EmitMxSurfaceShader(const mx::ShaderGraph &mxGraph,
                                                        ...
                                                        mx::ShaderStage &mxStage) const
{
      ...
      !mxGraph.hasClassification(mx::ShaderNode::Classification::SHADER))
  {
                                  ...
                                  mx::ShaderNode::Classification::SURFACE))
    {
      ...
      Base::emitFunctionCalls(
          mxGraph, mxContext, mxStage, mx::ShaderNode::Classification::TEXTURE);

#if MATERIALX_MAJOR_VERSION == 1 && MATERIALX_MINOR_VERSION == 38 && MATERIALX_BUILD_VERSION <= 4
      // Emit function calls for all surface shader nodes.
      // These will internally emit their closure function calls.
      Base::emitFunctionCalls(mxGraph,
                              mxContext,
                              mxStage,
    ...
    if (outputConnection) {
      ...
      if (mxGraph.hasClassification(mx::ShaderNode::Classification::SURFACE)) {
        if (mxContext.getOptions().hwTransparency) {
          ...
          emitLine("if (outAlpha < " + mx::HW::T_ALPHA_THRESHOLD + ")", mxStage, false);
          Base::emitScopeBegin(mxStage);
      ...
      const std::string outputValue = outputSocket->getValue() ?
                                          Base::_syntax->getValue(outputSocket->getType(),
      ...
      if (!outputSocket->getType()->isFloat4()) {
        ...
        emitLine(Base::_syntax->getTypeName(outputSocket->getType()) + " " + finalOutput + " = " +
                     outputValue,
                 mxStage);
        Base::toVec4(outputSocket->getType(), finalOutput);
...
static std::string _GetTexcoordName(mx::VariableBlock const &vertexDataBlock,
                                    std::string const &defaultTexcoordName)
{
  ...
  for (size_t i = 0; i < vertexDataBlock.size(); ++i) {
    ...
    if (mxVariableName.compare(0, mx::HW::T_TEXCOORD.size(), mx::HW::T_TEXCOORD) == 0) {
      return defaultTexcoordName;
    }
    // Use the geomprop name if this is a vec2 geomprop value node
    if (mxVariableName.compare(0, mx::HW::T_IN_GEOMPROP.size(), mx::HW::T_IN_GEOMPROP) == 0 &&
...
template<typename Base>
void HdStMaterialXShaderGen<Base>::_EmitMxInitFunction(mx::VariableBlock const &vertexData,
                                                       mx::ShaderStage &mxStage) const
{
  ...
  const std::string texcoordName = _GetTexcoordName(vertexData, _defaultTexcoordName);
  Base::emitString(
      TfStringPrintf(MxHdWorldSpaceVectors.c_str(), texcoordName.c_str(), texcoordName.c_str()),
      mxStage);
  ...
  const mx::VariableBlock &paramsBlock = mxStage.getUniformBlock(mx::HW::PUBLIC_UNIFORMS);
  for (size_t i = 0; i < paramsBlock.size(); ++i) {
    ...
    if (!_IsHardcodedPublicUniform(*variableType)) {
      emitLine(variable->getVariable() + " = HdGet_" + variable->getVariable() + "()", mxStage);
    }
  }
  Base::emitLineBreak(mxStage);
...
template<typename Base>
void HdStMaterialXShaderGen<Base>::_EmitMxVertexDataDeclarations(
    ...
    mx::ShaderStage &mxStage) const
{
  ...
  for (size_t i = 0; i < block.size(); ++i) {
    auto const &lineSeparator = (i == block.size() - 1) ? mx::EMPTY_STRING : separator;
...
template<typename Base>
std::string HdStMaterialXShaderGen<Base>::_EmitMxVertexDataLine(const mx::ShaderPort *variable,
                                                                std::string const &separator) const
{
      ...
      mxVariableName.compare(mx::HW::T_TANGENT_WORLD) == 0)
  {
    ...
    const std::string geompropName = mxVariableName.substr(mx::HW::T_IN_GEOMPROP.size() + 1);

    // Get the Default Value for the gromprop
    std::string defaultValueString = Base::_syntax->getDefaultValue(variable->getType());
    ...
    if (defaultValueIt != _mxHdPrimvarDefaultValueMap.end()) {
      if (!defaultValueIt->second.empty()) {
        defaultValueString = Base::_syntax->getTypeName(variable->getType()) + "(" +
...
template<typename Base>
void HdStMaterialXShaderGen<Base>::emitVariableDeclarations(mx::VariableBlock const &block,
                                                            ...
                                                            bool assignValue) const
{
  ...
  for (size_t i = 0; i < block.size(); ++i) {
    ...
    {
      Base::emitVariableDeclaration(
          variable, mx::EMPTY_STRING, context, stage, false /* assignValue */);
    }
    // Otherwise assign the value from MaterialX
    else {
      Base::emitVariableDeclaration(variable, qualifier, context, stage, assignValue);
    }
...
template<typename Base>
void HdStMaterialXShaderGen<Base>::emitLine(const std::string &str,
                                            mx::ShaderStage &stage,
                                            bool semicolon) const
{
  ...
  if (_emittingSurfaceNode && str == "vec3 L = lightShader.direction") {
    emitLine("occlusion = u_lightData[activeLightIndex].shadowOcclusion", stage);
  }
}

template<typename Base>
void HdStMaterialXShaderGen<Base>::_EmitConstantsUniformsAndTypeDefs(
    mx::GenContext &mxContext, mx::ShaderStage &mxStage, const std::string &constQualifier) const
{
...
template<typename Base>
void HdStMaterialXShaderGen<Base>::_EmitDataStructsAndFunctionDefinitions(
    ...
    MaterialX::StringMap *tokenSubstitutions) const
{
  ...
  if (mxContext.getOptions().fileTextureVerticalFlip) {
    (*tokenSubstitutions)[mx::ShaderGenerator::T_FILE_TRANSFORM_UV] = "mx_transform_uv_vflip.glsl";
  }
  else {
    (*tokenSubstitutions)[mx::ShaderGenerator::T_FILE_TRANSFORM_UV] = "mx_transform_uv.glsl";
  }
...
class HdStMaterialXSurfaceNodeGenGlsl : public mx::SurfaceNodeGlsl {
 public:
  ...
  void emitFunctionCall(const mx::ShaderNode &node,
                        mx::GenContext &context,
                        mx::ShaderStage &stage) const override
  {
    HdStMaterialXShaderGenGlsl &shadergen = static_cast<HdStMaterialXShaderGenGlsl &>(
...
mx::ShaderPtr HdStMaterialXShaderGenGlsl::generate(const std::string &shaderName,
...
void HdStMaterialXShaderGenGlsl::_EmitGlslfxShader(const mx::ShaderGraph &mxGraph,
                                                   mx::GenContext &mxContext,
                                                   mx::ShaderStage &mxStage) const
{
  // Add a per-light shadowOcclusion value to the lightData uniform block
  addStageUniform(mx::HW::LIGHT_DATA, mx::Type::FLOAT, "shadowOcclusion", mxStage);
...
// Similar to GlslShaderGenerator::emitPixelStage() with alterations and
// additions to match Pxr's codeGen
void HdStMaterialXShaderGenGlsl::_EmitMxFunctions(const mx::ShaderGraph &mxGraph,
                                                  mx::GenContext &mxContext,
                                                  mx::ShaderStage &mxStage) const
{
  mx::ShaderGenerator::emitLibraryInclude(
      "stdlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_math.glsl", mxContext, mxStage);

  // Add type definitions
  emitTypeDefinitions(mxContext, mxStage);

  _EmitConstantsUniformsAndTypeDefs(mxContext, mxStage, _syntax->getConstantQualifier());
  ...
  if (!_bindlessTexturesEnabled) {
    ...
    emitLine("#ifdef HD_HAS_domeLightIrradiance", mxStage, false);
    emitLine(
        "#define u_envRadiance "
        "HdGetSampler_domeLightPrefilter() ",
        mxStage,
        false);
    emitLine(
        "#define u_envIrradiance "
        "HdGetSampler_domeLightIrradiance() ",
        mxStage,
        false);
    emitLine("#else", mxStage, false);
    emitLine(
        "#define u_envRadiance "
        "HdGetSampler_domeLightFallback()",
        mxStage,
        false);
    emitLine(
        "#define u_envIrradiance "
  ...
  _EmitDataStructsAndFunctionDefinitions(mxGraph, mxContext, mxStage, &_tokenSubstitutions);
}
...
class HdStMaterialXSurfaceNodeGenVkGlsl : public mx::SurfaceNodeGlsl {
 public:
  ...
  void emitFunctionCall(const mx::ShaderNode &node,
                        ...
                        mx::ShaderStage &stage) const override
  {
    HdStMaterialXShaderGenVkGlsl &shadergen = static_cast<HdStMaterialXShaderGenVkGlsl &>(
        context.getShaderGenerator());
...
// Generates a glslfx shader and stores that in the pixel shader stage where it
// can be retrieved with getSourceCode()
mx::ShaderPtr HdStMaterialXShaderGenVkGlsl::generate(const std::string &shaderName,
                                                     mx::ElementPtr mxElement,
                                                     mx::GenContext &mxContext) const
{
  mx::ShaderPtr shader = createShader(shaderName, mxElement, mxContext);
...
void HdStMaterialXShaderGenVkGlsl::_EmitGlslfxShader(const mx::ShaderGraph &mxGraph,
                                                     mx::GenContext &mxContext,
                                                     mx::ShaderStage &mxStage) const
{
  // Add a per-light shadowOcclusion value to the lightData uniform block
...
// Similar to GlslShaderGenerator::emitPixelStage() with alterations and
// additions to match Pxr's codeGen
void HdStMaterialXShaderGenVkGlsl::_EmitMxFunctions(const mx::ShaderGraph &mxGraph,
                                                    mx::GenContext &mxContext,
                                                    mx::ShaderStage &mxStage) const
{
  emitLibraryInclude(
      "stdlib/" + mx::VkShaderGenerator::TARGET + "/lib/mx_math.glsl", mxContext, mxStage);
...
class HdStMaterialXSurfaceNodeGenMsl : public mx::SurfaceNodeMsl {
 public:
  ...
  void emitFunctionCall(const mx::ShaderNode &node,
                        ...
                        mx::ShaderStage &stage) const override
  {
    HdStMaterialXShaderGenMsl &shadergen = static_cast<HdStMaterialXShaderGenMsl &>(
        context.getShaderGenerator());
...
HdStMaterialXShaderGenMsl::HdStMaterialXShaderGenMsl(HdSt_MxShaderGenInfo const &mxHdInfo)
...
// Generates a glslfx shader and stores that in the pixel shader stage where it
// can be retrieved with getSourceCode()
mx::ShaderPtr HdStMaterialXShaderGenMsl::generate(const std::string &shaderName,
                                                  mx::ElementPtr mxElement,
                                                  mx::GenContext &mxContext) const
{
  mx::ShaderPtr shader = createShader(shaderName, mxElement, mxContext);
...
void HdStMaterialXShaderGenMsl::_EmitGlslfxMetalShader(const mx::ShaderGraph &mxGraph,
                                                       mx::GenContext &mxContext,
                                                       mx::ShaderStage &mxStage) const
{
  ...
  if (!resourceBindingCtx) {
    mxContext.pushUserData(mx::HW::USER_DATA_BINDING_CONTEXT,
                           mx::MslResourceBindingContext::create());
    resourceBindingCtx = mxContext.getUserData<mx::HwResourceBindingContext>(
...
void HdStMaterialXShaderGenMsl::_EmitGlslfxMetalHeader(mx::GenContext &mxContext,
                                                       mx::ShaderStage &mxStage) const
{
  _EmitGlslfxHeader(mxStage);
  emitLineBreak(mxStage);
...
// additions to match Pxr's codeGen
void HdStMaterialXShaderGenMsl::_EmitMxFunctions(const mx::ShaderGraph &mxGraph,
                                                 mx::GenContext &mxContext,
                                                 mx::ShaderStage &mxStage) const
{
  mx::ShaderGenerator::emitLibraryInclude(
      "pbrlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_microfacet.glsl", mxContext, mxStage);
  mx::ShaderGenerator::emitLibraryInclude(
      "stdlib/" + mx::MslShaderGenerator::TARGET + "/lib/mx_math.metal", mxContext, mxStage);
  _EmitConstantsUniformsAndTypeDefs(mxContext, mxStage, _syntax->getConstantQualifier());
  ...
  if (!_bindlessTexturesEnabled) {
    ...
    emitLine(
        ...
        "samplerBind_domeLightPrefilter} ",
        mxStage,
        false);
    emitLine(
        "#define u_envIrradiance "
        "MetalTexture{HdGetSampler_domeLightIrradiance(), "
        "samplerBind_domeLightIrradiance} ",
        mxStage,
        false);
    emitLine("#else", mxStage, false);
    emitLine(
