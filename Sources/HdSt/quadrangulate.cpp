...
HdSt_QuadInfoBuilderComputation::HdSt_QuadInfoBuilderComputation(HdSt_MeshTopology *topology,
                                                                 SdfPath const &id)
...
void HdSt_QuadIndexBuilderComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
bool HdSt_QuadIndexBuilderComputation::Resolve()
{
  ...
  HdMeshUtil meshUtil(_topology, _id);
  if (_topology->TriangulateQuads()) {
    meshUtil.ComputeTriQuadIndices(&quadsFaceVertexIndices, &primitiveParam, &quadsEdgeIndices);
  }
  else {
  ...
  if (_topology->TriangulateQuads()) {
    _SetResult(
        std::make_shared<HdVtBufferSource>(HdTokens->indices, VtValue(quadsFaceVertexIndices), 6));
  }
  else {
    _SetResult(
        std::make_shared<HdVtBufferSource>(HdTokens->indices, VtValue(quadsFaceVertexIndices), 4));
  }
...
HdBufferSourceSharedPtrVector HdSt_QuadIndexBuilderComputation::GetChainedBuffers() const
{
...
bool HdSt_QuadrangulateTableComputation::Resolve()
{
...
void HdSt_QuadrangulateTableComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // quadinfo computation produces an index buffer for quads.
  specs->emplace_back(HdTokens->quadInfo, HdTupleType{HdTypeInt32, 1});
}

bool HdSt_QuadrangulateTableComputation::_CheckValid() const
{
  return true;
}
...
HdSt_QuadrangulateComputation::HdSt_QuadrangulateComputation(
    ...
    SdfPath const &id)
    : _id(id), _topology(topology), _source(source), _quadInfoBuilder(quadInfoBuilder)
{
}
...
void HdSt_QuadrangulateComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
HdBufferSourceSharedPtr HdSt_QuadrangulateComputation::GetPreChainedBuffer() const
{
  return _source;
}
...
void HdSt_QuadrangulateFaceVaryingComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // produces same spec buffer as source
  _source->GetBufferSpecs(specs);
}
...
HdSt_QuadrangulateComputationGPU::HdSt_QuadrangulateComputationGPU(HdSt_MeshTopology *topology,
...
void HdSt_QuadrangulateComputationGPU::Execute(HdBufferArrayRangeSharedPtr const &range,
                                               HdResourceRegistry *resourceRegistry)
{
  ...
  HdStGLSLProgramSharedPtr computeProgram = HdStGLSLProgram::GetComputeProgram(
      shaderToken, hdStResourceRegistry, [&](HgiShaderFunctionDesc &computeDesc) {
        ...
        if (shaderToken == HdStGLSLProgramTokens->quadrangulateFloat) {
          ...
          HgiShaderFunctionAddWritableBuffer(
              &computeDesc, "primvar", HdStTokens->_double, BufferBinding_Primvar);
        }
        ...
        static const std::string params[] = {"vertexOffset",  // offset in aggregated buffer
                                             "quadInfoStride",
                                             "quadInfoOffset",
                                             "maxNumVert",
  ...
  HdStBufferArrayRangeSharedPtr range_ = std::static_pointer_cast<HdStBufferArrayRange>(range);
  ...
  HdStBufferResourceSharedPtr quadrangulateTable = quadrangulateTableRange_->GetResource();
  ...
  uint64_t rbHash = (uint64_t)TfHash::Combine(primvar->GetHandle().GetId(),
...
void HdSt_QuadrangulateComputationGPU::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // nothing
  //
...
int HdSt_QuadrangulateComputationGPU::GetNumOutputElements() const
{
  HdQuadInfo const *quadInfo = _topology->GetQuadInfo();
