...
HdStAssetUvTextureCpuData::HdStAssetUvTextureCpuData(
    ...
    : _generateMipmaps(false), _wrapInfo{HdWrapNoOpinion, HdWrapNoOpinion}
{
  TRACE_FUNCTION();

  // Open all mips for the image.
  const std::vector<HioImageSharedPtr> mips = HdStTextureUtils::GetAllMipImages(filePath,
                                                                                sourceColorSpace);
  if (mips.empty()) {
    return;
  }
  ...
  //
  _textureDesc.dimensions = HdStTextureUtils::ComputeDimensionsFromTargetMemory(
      mips,
      _textureDesc.format,
      /* tileCount = */ 1,
      targetMemory,
...

  {
    ...
    for (size_t i = 0; i < numUsableMips; ++i) {
      if (!HdStTextureUtils::ReadAndConvertImage(mips[firstMip + i],
...
void HdStAssetUvTextureCpuData::_SetWrapInfo(HioImageSharedPtr const &image)
{
  _wrapInfo.first = _GetWrapMode(image, HioAddressDimensionU);
  _wrapInfo.second = _GetWrapMode(image, HioAddressDimensionV);
}

const HgiTextureDesc &HdStAssetUvTextureCpuData::GetTextureDesc() const
{
  return _textureDesc;
}

bool HdStAssetUvTextureCpuData::GetGenerateMipmaps() const
{
  return _generateMipmaps;
}

bool HdStAssetUvTextureCpuData::IsValid() const
{
