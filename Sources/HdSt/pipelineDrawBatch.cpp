...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         ...
                         (constantPrimvars)
                             ...
                             (dispatchBuffer)(drawCullInput)

                                 (drawIndirect)(drawIndirectCull)(drawIndirectResult)

                                     (ulocCullParams));

TF_DEFINE_ENV_SETTING(HDST_ENABLE_PIPELINE_DRAW_BATCH_GPU_FRUSTUM_CULLING,
                      true,
                      "Enable pipeline draw batching GPU frustum culling");

HdSt_PipelineDrawBatch::HdSt_PipelineDrawBatch(HdStDrawItemInstance *drawItemInstance,
...
/*virtual*/
void HdSt_PipelineDrawBatch::_Init(HdStDrawItemInstance *drawItemInstance)
{
  ...
  _useInstanceCulling = _useInstancing && _useGpuCulling && IsEnabledGPUInstanceFrustumCulling();

  if (_useGpuCulling) {
    _cullingProgram.Initialize(_useDrawIndexed, _useInstanceCulling, _bufferArraysHash);
  }

  TF_DEBUG(HDST_DRAW_BATCH).Msg("   Resetting dispatch buffer.\n");
  _dispatchBuffer.reset();
}

void HdSt_PipelineDrawBatch::SetEnableTinyPrimCulling(bool tinyPrimCulling)
{
...
/* static */
bool HdSt_PipelineDrawBatch::IsEnabled(Hgi const *hgi)
{
...
bool HdSt_PipelineDrawBatch::IsEnabledGPUFrustumCulling()
{
  ...
  static bool isEnabled = TfGetEnvSetting(HDST_ENABLE_PIPELINE_DRAW_BATCH_GPU_FRUSTUM_CULLING);
  return isEnabled && HdSt_IndirectDrawBatch::IsEnabledGPUFrustumCulling();
}

/* static */
bool HdSt_PipelineDrawBatch::IsEnabledGPUCountVisibleInstances()
{
  return HdSt_IndirectDrawBatch::IsEnabledGPUCountVisibleInstances();
}

/* static */
bool HdSt_PipelineDrawBatch::IsEnabledGPUInstanceFrustumCulling()
{
                           ...
                           size_t const uint32Alignment)
{
  ...
  traits->numUInt32 = sizeof(CmdType) / sizeof(uint32_t) + instancerNumLevels;

  if (uint32Alignment > 0) {
    size_t const alignMask = uint32Alignment - 1;
    size_t const alignedNumUInt32 = (traits->numUInt32 + alignMask) & ~alignMask;
...
template<typename CmdType> void _SetInstanceCullTraits(_DrawCommandTraits *traits)
{
  traits->cullCount_offset = offsetof(CmdType, cullCount);
  traits->cullInstanceCount_offset = offsetof(CmdType, cullInstanceCount);
}

template<typename CmdType> void _SetDrawingCoordTraits(_DrawCommandTraits *traits)
{
  // drawingCoord bundles are located by the offsets to their first elements
  traits->drawingCoord0_offset = offsetof(CmdType, drawingCoord) +
                                 offsetof(_DrawingCoord, modelDC);
  traits->drawingCoord1_offset = offsetof(CmdType, drawingCoord) + offsetof(_DrawingCoord, fvarDC);
  traits->drawingCoord2_offset = offsetof(CmdType, drawingCoord) +
                                 offsetof(_DrawingCoord, topVisDC);
  ...
  traits->patchBaseVertex_offset = offsetof(CmdType, drawingCoord) +
                                   offsetof(_DrawingCoord, vertexDC);
}

_DrawCommandTraits _GetDrawCommandTraits(int const instancerNumLevels,
                                         ...
                                         size_t const uint32Alignment)
{
  ...
  if (!useDrawIndexed) {
    if (useInstanceCulling) {
      ...
      _SetDrawCommandTraits<CmdType>(&traits, instancerNumLevels, uint32Alignment);
...
void _AddDrawResourceViews(HdStDispatchBufferSharedPtr const &dispatchBuffer,
                           _DrawCommandTraits const &traits)
{
  // draw indirect command
  dispatchBuffer->AddBufferResourceView(
      HdTokens->drawDispatch, {HdTypeInt32, 1}, traits.count_offset);
  // drawing coord 0
  dispatchBuffer->AddBufferResourceView(
      HdTokens->drawingCoord0, {HdTypeInt32Vec4, 1}, traits.drawingCoord0_offset);
  // drawing coord 1
  dispatchBuffer->AddBufferResourceView(
      HdTokens->drawingCoord1, {HdTypeInt32Vec4, 1}, traits.drawingCoord1_offset);
  // drawing coord 2
  dispatchBuffer->AddBufferResourceView(
      HdTokens->drawingCoord2, {HdTypeInt32Vec2, 1}, traits.drawingCoord2_offset);
  // instance drawing coords
  if (traits.instancerNumLevels > 0) {
    dispatchBuffer->AddBufferResourceView(HdTokens->drawingCoordI,
                                          {HdTypeInt32, traits.instancerNumLevels},
                                          traits.drawingCoordI_offset);
  }
}

HdBufferArrayRangeSharedPtr _GetShaderBar(HdSt_MaterialNetworkShaderSharedPtr const &shader)
{
...
struct _DrawItemState {
  explicit _DrawItemState(HdStDrawItem const *drawItem)
      : constantBar(
            std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetConstantPrimvarRange())),
        indexBar(std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetTopologyRange())),
        topVisBar(std::static_pointer_cast<HdStBufferArrayRange>(
            drawItem->GetTopologyVisibilityRange())),
        elementBar(
            std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetElementPrimvarRange())),
        fvarBar(std::static_pointer_cast<HdStBufferArrayRange>(
            drawItem->GetFaceVaryingPrimvarRange())),
        varyingBar(
            std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetVaryingPrimvarRange())),
        vertexBar(
            std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetVertexPrimvarRange())),
        shaderBar(std::static_pointer_cast<HdStBufferArrayRange>(
            _GetShaderBar(drawItem->GetMaterialNetworkShader()))),
        instanceIndexBar(
            std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetInstanceIndexRange()))
  {
    instancePrimvarBars.resize(drawItem->GetInstancePrimvarNumLevels());
    for (size_t i = 0; i < instancePrimvarBars.size(); ++i) {
      instancePrimvarBars[i] = std::static_pointer_cast<HdStBufferArrayRange>(
...
uint32_t _GetElementOffset(HdBufferArrayRangeSharedPtr const &range)
{
  return range ? range->GetElementOffset() : 0;
}

uint32_t _GetElementCount(HdBufferArrayRangeSharedPtr const &range)
{
  return range ? range->GetNumElements() : 0;
}

uint32_t _GetInstanceCount(HdStDrawItemInstance const *drawItemInstance,
                           ...
                           int const instanceIndexWidth)
{
  ...
  uint32_t const numInstances = instanceIndexBar ? instanceIndexBar->GetNumElements() : 1;
  uint32_t const instanceCount = drawItemInstance->IsVisible() ?
                                     (numInstances / instanceIndexWidth) :
                                     0;
  return instanceCount;
}

HdStBufferResourceSharedPtr _AllocateTessFactorsBuffer(
    HdStDrawItem const *drawItem, HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  if (!drawItem) {
    return nullptr;
  }
  ...
  HdStBufferArrayRangeSharedPtr indexBar(
      std::static_pointer_cast<HdStBufferArrayRange>(drawItem->GetTopologyRange()));
  if (!indexBar) {
    return nullptr;
  }

  HdStBufferResourceSharedPtr indexBuffer = indexBar->GetResource(HdTokens->indices);
  if (!indexBuffer) {
    return nullptr;
  }
  ...
  size_t const byteSizeOfTuple = HdDataSizeOfTupleType(indexBuffer->GetTupleType());
  size_t const byteSizeOfResource = indexBufferHandle->GetByteSizeOfResource();
...
void HdSt_PipelineDrawBatch::_CompileBatch(HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  TRACE_FUNCTION();
  ...
  bool const useMetalTessellation =
      _drawItemInstances[0]->GetDrawItem()->GetGeometricShader()->GetUseMetalTessellation();
  ...
  _DrawCommandTraits const traits = _GetDrawCommandTraits(
      instancerNumLevels, _useDrawIndexed, _useInstanceCulling, uint32Alignment);
  ...
  for (size_t item = 0; item < numDrawItemInstances; ++item) {
    ...
    uint32_t const numElements = vertexCount != 0 ? _GetElementCount(dc.indexBar) : 0;

    uint32_t const baseIndex = primitiveDC * numIndicesPerPrimitive;
    uint32_t const indexCount = numElements * numIndicesPerPrimitive;

    uint32_t const instanceCount = _GetInstanceCount(
        drawItemInstance, dc.instanceIndexBar, traits.instanceIndexWidth);
  ...
  _dispatchBuffer = resourceRegistry->RegisterDispatchBuffer(
      _tokens->drawIndirect, numDrawItemInstances, traits.numUInt32);

  // allocate tessFactors buffer for Metal tessellation
  if (useMetalTessellation &&
      _drawItemInstances[0]->GetDrawItem()->GetGeometricShader()->IsPrimTypePatches())
  {

    _tessFactorsBuffer = _AllocateTessFactorsBuffer(_drawItemInstances[0]->GetDrawItem(),
                                                    resourceRegistry);
  }
  ...
  if (_useGpuCulling) {
    ...
    _dispatchBufferCullInput = resourceRegistry->RegisterDispatchBuffer(
        _tokens->drawIndirectCull, numDrawItemInstances, traits.numUInt32);
...
HdSt_DrawBatch::ValidationResult HdSt_PipelineDrawBatch::Validate(bool deepValidation)
{
  if (!TF_VERIFY(!_drawItemInstances.empty())) {
    return ValidationResult::RebuildAllBatches;
  }

  TF_DEBUG(HDST_DRAW_BATCH)
      .Msg("Validating pipeline draw batch %p (deep validation = %d)...\n",
           (void *)(this),
           deepValidation);
  ...
  if (_bufferArraysHash != bufferArraysHash) {
    ...
    TF_DEBUG(HDST_DRAW_BATCH).Msg("   Buffer arrays hash changed. Need to rebuild batch.\n");
  ...
  TF_DEBUG(HDST_DRAW_BATCH).Msg("   Validation passed. No need to rebuild batch.\n");
...
void HdSt_PipelineDrawBatch::PrepareDraw(HgiGraphicsCmds *gfxCmds,
                                         ...
                                         HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  ...
  if (_useGpuCulling) {
    ...
    _ExecuteFrustumCull(updateBufferData, renderPassState, resourceRegistry);
  }
}

void HdSt_PipelineDrawBatch::EncodeDraw(HdStRenderPassStateSharedPtr const &renderPassState,
                                        HdStResourceRegistrySharedPtr const &resourceRegistry,
                                        bool firstDrawBatch)
{
  if (_HasNothingToDraw())
  ...
  bool const drawICB = _allowIndirectCommandEncoding &&
  ...
  if (drawICB) {
    _PrepareIndirectCommandBuffer(renderPassState, resourceRegistry, firstDrawBatch);
  }
}
...
struct _BindingState : public _DrawItemState {
  _BindingState(HdStDrawItem const *drawItem,
  ...
  void GetBindingsForViewTransformation(HgiResourceBindingsDesc *bindingsDesc) const;

  // Core resources plus additional resources needed for drawing.
  void GetBindingsForDrawing(HgiResourceBindingsDesc *bindingsDesc,
...
void _BindingState::GetBindingsForViewTransformation(HgiResourceBindingsDesc *bindingsDesc) const
{
  ...
  if (instanceIndexBar) {
    for (size_t level = 0; level < instancePrimvarBars.size(); ++level) {
      binder.GetInstanceBufferArrayBindingDesc(bindingsDesc, instancePrimvarBars[level], level);
    }
    binder.GetBufferArrayBindingDesc(bindingsDesc, instanceIndexBar);
  }
}

void _BindingState::GetBindingsForDrawing(HgiResourceBindingsDesc *bindingsDesc,
                                          HdStBufferResourceSharedPtr const &tessFactorsBuffer,
                                          bool bindTessFactors) const
{
  ...
  if (tessFactorsBuffer) {
    binder.GetBufferBindingDesc(
        bindingsDesc, HdTokens->tessFactors, tessFactorsBuffer, tessFactorsBuffer->GetOffset());
    if (bindTessFactors) {
      binder.GetBufferBindingDesc(
          bindingsDesc, HdTokens->tessFactors, tessFactorsBuffer, tessFactorsBuffer->GetOffset());
  ...
  for (HdStShaderCodeSharedPtr const &shader : shaders) {
    HdStBufferArrayRangeSharedPtr shaderBar = std::static_pointer_cast<HdStBufferArrayRange>(
        shader->GetShaderData());

    binder.GetInterleavedBufferArrayBindingDesc(bindingsDesc, shaderBar, HdTokens->materialParams);
    ...
    HdSt_TextureBinder::GetBindingDescs(binder, bindingsDesc, shader->GetNamedTextureHandles());
  }
}

HgiVertexBufferDescVector _GetVertexBuffersForViewTransformation(_BindingState const &state)
{
  // Bind the dispatchBuffer drawing coordinate resource views
  HdStBufferArrayRangeSharedPtr const dispatchBar = state.dispatchBuffer->GetBufferArrayRange();
  size_t const dispatchBufferStride = state.dispatchBuffer->GetCommandNumUints() *
                                      sizeof(uint32_t);

  HgiVertexAttributeDescVector attrDescVector;

  for (auto const &namedResource : dispatchBar->GetResources()) {
    HdStBinding const binding = state.binder.GetBinding(namedResource.first);
    ...
    if (binding.GetType() == HdStBinding::DRAW_INDEX_INSTANCE) {
      ...
      attrDesc.format = HdStHgiConversions::GetHgiVertexFormat(tupleType.type);
      attrDesc.offset = resource->GetOffset(), attrDesc.shaderBindLocation = binding.GetLocation();
      attrDescVector.push_back(attrDesc);
    }
    else if (binding.GetType() == HdStBinding::DRAW_INDEX_INSTANCE_ARRAY) {
...
HgiVertexBufferDescVector _GetVertexBuffersForDrawing(_BindingState const &state)
{
  // Bind the vertexBar resources
  HgiVertexBufferDescVector vertexBufferDescVector = _GetVertexBuffersForViewTransformation(state);
  ...
  for (auto const &namedResource : state.vertexBar->GetResources()) {
    ...
    if (binding.GetType() == HdStBinding::VERTEX_ATTR) {
      ...
      if (state.geometricShader->GetUseMetalTessellation()) {
        bufferDesc.vertexStepFunction = HgiVertexBufferStepFunctionPerPatchControlPoint;
      }
      else {
        bufferDesc.vertexStepFunction = HgiVertexBufferStepFunctionPerVertex;
      }
...
uint32_t _GetVertexBufferBindingsForViewTransformation(HgiVertexBufferBindingVector *bindings,
                                                       _BindingState const &state)
{
  // Bind the dispatchBuffer drawing coordinate resource views
  HdStBufferResourceSharedPtr resource = state.dispatchBuffer->GetEntireResource();

  bindings->emplace_back(resource->GetHandle(), (uint32_t)resource->GetOffset(), 0);

  return static_cast<uint32_t>(bindings->size());
}

uint32_t _GetVertexBufferBindingsForDrawing(HgiVertexBufferBindingVector *bindings,
                                            _BindingState const &state)
{
  ...
  for (auto const &namedResource : state.vertexBar->GetResources()) {
    ...
    if (binding.GetType() == HdStBinding::VERTEX_ATTR) {
      bindings->emplace_back(
          resource->GetHandle(), static_cast<uint32_t>(resource->GetOffset()), nextBinding);
      nextBinding++;
    }
  }
...
static HgiGraphicsPipelineSharedPtr _GetDrawPipeline(
    HdStRenderPassStateSharedPtr const &renderPassState,
    HdStResourceRegistrySharedPtr const &resourceRegistry,
    _BindingState const &state,
    bool firstDrawBatch)
{
  ...
  HgiShaderProgramHandle const &programHandle = state.glslProgram->GetProgram();

  static const uint64_t salt = ArchHash64(__FUNCTION__, sizeof(__FUNCTION__));
  uint64_t hash = salt;
  hash = TfHash::Combine(hash, programHandle.Get());
  hash = TfHash::Combine(
      hash, renderPassState->GetGraphicsPipelineHash(state.geometricShader, firstDrawBatch));
  ...
  if (pipelineInstance.IsFirstInstance()) {
    ...
    renderPassState->InitGraphicsPipelineDesc(&pipeDesc, state.geometricShader, firstDrawBatch);
...
static HgiGraphicsPipelineSharedPtr _GetPTCSPipeline(
    HdStRenderPassStateSharedPtr const &renderPassState,
    HdStResourceRegistrySharedPtr const &resourceRegistry,
    _BindingState const &state,
    bool firstDrawBatch)
{
  ...
  if (pipelineInstance.IsFirstInstance()) {
    ...
    pipeDesc.shaderProgram = state.glslProgram->GetProgram();
    pipeDesc.vertexBuffers = _GetVertexBuffersForDrawing(state);
    pipeDesc.tessellationState.tessFactorMode = HgiTessellationState::TessControl;
...
void HdSt_PipelineDrawBatch::ExecuteDraw(HgiGraphicsCmds *gfxCmds,
                                         HdStRenderPassStateSharedPtr const &renderPassState,
                                         HdStResourceRegistrySharedPtr const &resourceRegistry,
                                         bool firstDrawBatch)
{
  ...
  if (_tessFactorsBuffer) {
    ...
    _ExecutePTCS(gfxCmds, renderPassState, resourceRegistry, firstDrawBatch);
  ...
  if (_indirectCommands) {
    ...
    _DrawingProgram &program = _GetDrawingProgram(renderPassState, resourceRegistry);
    if (!TF_VERIFY(program.IsValid()))
      return;

    _BindingState state(_drawItemInstances.front()->GetDrawItem(),
                        _dispatchBuffer,
                        program.GetBinder(),
    ...
    HgiGraphicsPipelineSharedPtr pso = _GetDrawPipeline(
        renderPassState, resourceRegistry, state, firstDrawBatch);

    HgiGraphicsPipelineHandle psoHandle = *pso.get();
    gfxCmds->BindPipeline(psoHandle);

    HgiResourceBindingsDesc bindingsDesc;
    state.GetBindingsForDrawing(&bindingsDesc, _tessFactorsBuffer, /*bindTessFactors=*/true);

    HgiResourceBindingsHandle resourceBindings = hgi->CreateResourceBindings(bindingsDesc);
...
void HdSt_PipelineDrawBatch::_ExecuteDrawIndirect(HgiGraphicsCmds *gfxCmds,
                                                  HdStBufferArrayRangeSharedPtr const &indexBar)
{
  TRACE_FUNCTION();

  HdStBufferResourceSharedPtr paramBuffer = _dispatchBuffer->GetBufferArrayRange()->GetResource(
      HdTokens->drawDispatch);
  if (!TF_VERIFY(paramBuffer))
    return;

  if (!_useDrawIndexed) {
    gfxCmds->DrawIndirect(paramBuffer->GetHandle(),
    ...
    HdStBufferResourceSharedPtr indexBuffer = indexBar->GetResource(HdTokens->indices);
    if (!TF_VERIFY(indexBuffer))
      return;

    gfxCmds->DrawIndexedIndirect(indexBuffer->GetHandle(),
...
void HdSt_PipelineDrawBatch::_ExecuteDrawImmediate(HgiGraphicsCmds *gfxCmds,
                                                   HdStBufferArrayRangeSharedPtr const &indexBar)
{
  TRACE_FUNCTION();
  ...
  if (!_useDrawIndexed) {
    for (uint32_t i = 0; i < drawCount; ++i) {
      _DrawNonIndexedCommand const *cmd = reinterpret_cast<_DrawNonIndexedCommand *>(
          &_drawCommandBuffer[i * strideUInt32]);

      if (cmd->common.count && cmd->common.instanceCount) {
        gfxCmds->Draw(cmd->common.count,
    ...
    for (uint32_t i = 0; i < drawCount; ++i) {
      _DrawIndexedCommand const *cmd = reinterpret_cast<_DrawIndexedCommand *>(
          &_drawCommandBuffer[i * strideUInt32]);

      uint32_t const indexBufferByteOffset = static_cast<uint32_t>(cmd->common.baseIndex *
                                                                   sizeof(uint32_t));

      if (cmd->common.count && cmd->common.instanceCount) {
        if (useMetalTessellation) {
          gfxCmds->DrawIndexed(indexBuffer->GetHandle(),
...
static HgiComputePipelineSharedPtr _GetCullPipeline(
    HdStResourceRegistrySharedPtr const &resourceRegistry,
    _BindingState const &state,
    size_t byteSizeUniforms)
{
  ...
  if (pipelineInstance.IsFirstInstance()) {
    ...
    Hgi *hgi = resourceRegistry->GetHgi();
    HgiComputePipelineSharedPtr pipe = std::make_shared<HgiComputePipelineHandle>(
...
void HdSt_PipelineDrawBatch::_PrepareIndirectCommandBuffer(
    HdStRenderPassStateSharedPtr const &renderPassState,
    HdStResourceRegistrySharedPtr const &resourceRegistry,
    bool firstDrawBatch)
{
  ...
  if (!_useDrawIndexed) {
    _indirectCommands = encoder->EncodeDraw(computeCmds,
                                            psoHandle,
                                            resourceBindings,
                                            vertexBindings,
                                            paramBuffer->GetHandle(),
    ...
    HdStBufferResourceSharedPtr indexBuffer = state.indexBar->GetResource(HdTokens->indices);

    _indirectCommands = encoder->EncodeDrawIndexed(computeCmds,
                                                   psoHandle,
                                                   resourceBindings,
                                                   vertexBindings,
...
void HdSt_PipelineDrawBatch::_ExecuteFrustumCull(
    ...
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  ...
  if (!TF_VERIFY(_cullingProgram.IsValid()))
    return;
  ...
  HgiComputePipelineSharedPtr const &pso = _GetCullPipeline(
      resourceRegistry, state, sizeof(Uniforms));
  ...
  if (IsEnabledGPUCountVisibleInstances()) {
    ...
    state.binder.GetBufferBindingDesc(
        &bindingsDesc, _tokens->drawIndirectResult, _resultBuffer, _resultBuffer->GetOffset());
  }

  // bind destination buffer
  // (using entire buffer bind to start from offset=0)
  state.binder.GetBufferBindingDesc(&bindingsDesc,
  ...
  computeCmds->SetConstantValues(psoHandle, 0, sizeof(Uniforms), &cullParams);
...
void HdSt_PipelineDrawBatch::_ExecutePTCS(HgiGraphicsCmds *ptcsGfxCmds,
                                          HdStRenderPassStateSharedPtr const &renderPassState,
                                          HdStResourceRegistrySharedPtr const &resourceRegistry,
                                          bool firstDrawBatch)
{
  ...
  HgiGraphicsPipelineSharedPtr const &psoTess = _GetPTCSPipeline(
      renderPassState, resourceRegistry, state, firstDrawBatch);
  ...
  HgiResourceBindingsDesc bindingsDesc;
  state.GetBindingsForDrawing(&bindingsDesc, _tessFactorsBuffer, /*bindTessFactors=*/false);
...
void HdSt_PipelineDrawBatch::DrawItemInstanceChanged(HdStDrawItemInstance const *instance)
{
  ...
  HdStBufferArrayRangeSharedPtr instanceIndexBar = std::static_pointer_cast<HdStBufferArrayRange>(
      instanceIndexBar_);

  uint32_t const newInstanceCount = _GetInstanceCount(
      instance, instanceIndexBar, instanceIndexWidth);
...
void HdSt_PipelineDrawBatch::_BeginGPUCountVisibleInstances(
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  if (!_resultBuffer) {
...
void HdSt_PipelineDrawBatch::_EndGPUCountVisibleInstances(
    HdStResourceRegistrySharedPtr const &resourceRegistry, size_t *result)
{
...
void HdSt_PipelineDrawBatch::_CreateCullingProgram(
    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  if (!_cullingProgram.GetGLSLProgram() || _dirtyCullingProgram) {
    // Create a culling compute shader key
    HdSt_CullingComputeShaderKey shaderKey(
        _useInstanceCulling, _useTinyPrimCulling, IsEnabledGPUCountVisibleInstances());
    ...
    HdSt_GeometricShaderSharedPtr cullShader = HdSt_GeometricShader::Create(shaderKey,
                                                                            resourceRegistry);
    _cullingProgram.SetDrawingCoordBufferBinding(drawingCoordBufferBinding);
    _cullingProgram.SetGeometricShader(cullShader);

    _cullingProgram.CompileShader(_drawItemInstances.front()->GetDrawItem(), resourceRegistry);

    _dirtyCullingProgram = false;
  }
}

void HdSt_PipelineDrawBatch::_CullingProgram::Initialize(bool useDrawIndexed,
                                                         bool useInstanceCulling,
                                                         size_t bufferArrayHash)
{
  if (useDrawIndexed != _useDrawIndexed || useInstanceCulling != _useInstanceCulling ||
...
/* virtual */
void HdSt_PipelineDrawBatch::_CullingProgram::_GetCustomBindings(
    HdStBindingRequestVector *customBindings, bool *enableInstanceDraw) const
{
  if (!TF_VERIFY(enableInstanceDraw) || !TF_VERIFY(customBindings))
    return;

  customBindings->push_back(HdStBindingRequest(HdStBinding::SSBO, _tokens->drawIndirectResult));
  customBindings->push_back(HdStBindingRequest(HdStBinding::SSBO, _tokens->dispatchBuffer));
  customBindings->push_back(HdStBindingRequest(HdStBinding::UBO, _tokens->ulocCullParams));
  customBindings->push_back(HdStBindingRequest(HdStBinding::SSBO, _tokens->drawCullInput));
