...
//
static HdStTextureIdentifier _GetTextureIdentifier(
    HdStMaterialNetwork::TextureDescriptor const &desc, HdSceneDelegate *const sceneDelegate)
{
...
static size_t _GetTextureHandleHash(HdStTextureHandleSharedPtr const &textureHandle)
{
...
void HdStMaterial::_ProcessTextureDescriptors(
    ...
    HdBufferSourceSharedPtrVector *const sources)
{
  ...
  bool const doublesSupported = resourceRegistry->GetHgi()->GetCapabilities()->IsSet(
      HgiDeviceCapabilitiesBitsShaderDoublePrecision);
  HdSt_TextureBinder::GetBufferSpecs(*texturesFromStorm, specs, doublesSupported);
}

/* virtual */
void HdStMaterial::Sync(HdSceneDelegate *sceneDelegate,
                        ...
                        HdDirtyBits *dirtyBits)
{
  ...
  if (fragmentSource.empty() && geometrySource.empty() && displacementSource.empty()) {
  ...
  std::string const &oldFragmentSource = _materialNetworkShader->GetSource(
      HdShaderTokens->fragmentShader);
  std::string const &oldGeometrySource = _materialNetworkShader->GetSource(
      HdShaderTokens->geometryShader);
  std::string const &oldDisplacementSource = _materialNetworkShader->GetSource(
      HdShaderTokens->displacementShader);
  ...
  for (HdSt_MaterialParam const &param : params) {
    if (param.IsPrimvarRedirect() || param.IsFallback() || param.IsTransform2d()) {
      HdSt_MaterialNetworkShader::AddFallbackValueToSpecsAndSources(param, &specs, &sources);
    }
  ...
  _materialNetworkShader->SetBufferSources(specs, std::move(sources), resourceRegistry);
...
/*virtual*/
void HdStMaterial::Finalize(HdRenderParam *renderParam)
{
  // Flag GC to reclaim resources owned by the surface shader.
  HdStMarkGarbageCollectionNeeded(renderParam);
}

bool HdStMaterial::_GetHasLimitSurfaceEvaluation(VtDictionary const &metadata) const
{
...
// virtual
HdDirtyBits HdStMaterial::GetInitialDirtyBitsMask() const
{
  return AllDirty;
}

HdSt_MaterialNetworkShaderSharedPtr HdStMaterial::GetMaterialNetworkShader() const
{
  return _materialNetworkShader;
}

void HdStMaterial::SetMaterialNetworkShader(
    HdSt_MaterialNetworkShaderSharedPtr &materialNetworkShader)
{
  _materialNetworkShader = materialNetworkShader;
}

void HdStMaterial::_InitFallbackShader()
{
