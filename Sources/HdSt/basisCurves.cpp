...
void HdStBasisCurves::UpdateRenderTag(HdSceneDelegate *delegate, HdRenderParam *renderParam)
{
  HdStUpdateRenderTag(delegate, renderParam, this);
}

void HdStBasisCurves::Sync(HdSceneDelegate *delegate,
...
void HdStBasisCurves::Finalize(HdRenderParam *renderParam)
{
...
void HdStBasisCurves::_UpdateDrawItem(HdSceneDelegate *sceneDelegate,
                                      HdRenderParam *renderParam,
...
void HdStBasisCurves::_UpdateDrawItemGeometricShader(HdSceneDelegate *sceneDelegate,
                                                     HdRenderParam *renderParam,
                                                     HdStDrawItem *drawItem,
                                                     const HdBasisCurvesReprDesc &desc)
{
  ...
  if (!supportsRefinement) {
    ...
    TF_DEBUG(HD_RPRIM_UPDATED)
        .Msg(
            "HdStBasisCurves(%s) - Downcasting curve type to linear because"
  ...
  HdSt_BasisCurvesShaderKey::DrawStyle drawStyle = HdSt_BasisCurvesShaderKey::WIRE;
  HdSt_BasisCurvesShaderKey::NormalStyle normalStyle = HdSt_BasisCurvesShaderKey::HAIR;
  ...
  if (shadingTerminal == HdBasisCurvesReprDescTokens->surfaceShader) {
    TfToken shadingStyle = sceneDelegate->GetShadingStyle(GetId()).GetWithDefault<TfToken>();
  ...
  TF_DEBUG(HD_RPRIM_UPDATED)
      .Msg("HdStBasisCurves(%s) - Shader Key PrimType: %s\n ",
           GetId().GetText(),
           HdSt_PrimTypeToString(shaderKey.primType));
...
HdDirtyBits HdStBasisCurves::_PropagateDirtyBits(HdDirtyBits bits) const
{
  // propagate scene-based dirtyBits into rprim-custom dirtyBits
  if (bits & HdChangeTracker::DirtyTopology) {
    bits |= _customDirtyBitsInUse &
            (DirtyIndices | DirtyHullIndices | DirtyPointsIndices | HdChangeTracker::DirtyPrimvar);
  }

  return bits;
}

void HdStBasisCurves::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits)
{
  ...
  if (isNew) {
    ...
    *dirtyBits |= HdChangeTracker::NewRepr;
    ...
    for (size_t descIdx = 0; descIdx < descs.size(); ++descIdx) {
      ...
      drawingCoord->SetInstancePrimvarBaseIndex(HdStBasisCurves::InstancePrimvar);
    }
  }
}
                                  ...
                                  HdDirtyBits *dirtyBits)
{
  ...
  *dirtyBits &= (_customDirtyBitsInUse | HdChangeTracker::AllSceneDirtyBits |
  ...
  if (TfDebug::IsEnabled(HD_RPRIM_UPDATED)) {
    TfDebug::Helper().Msg("HdStBasisCurves::_UpdateRepr for %s : Repr = %s\n",
                          GetId().GetText(),
                          reprToken.GetText());
...
void HdStBasisCurves::_UpdateShadersForAllReprs(HdSceneDelegate *sceneDelegate,
                                                HdRenderParam *renderParam,
...
void HdStBasisCurves::_UpdateMaterialTagsForAllReprs(HdSceneDelegate *sceneDelegate,
                                                     HdRenderParam *renderParam)
{
  TF_DEBUG(HD_RPRIM_UPDATED)
                                        ...
                                        const HdBasisCurvesReprDesc &desc)
{
  ...
  if (dirtyTopology || HdChangeTracker::IsDisplayStyleDirty(*dirtyBits, id)) {
    ...
    if (dirtyTopology) {
      ...
      HdStProcessTopologyVisibility(srcTopology.GetInvisibleCurves(),
    ...
    if (topologyInstance.IsFirstInstance()) {
      ...
      HdSt_BasisCurvesTopologySharedPtr topology = HdSt_BasisCurvesTopology::New(srcTopology);
  ...
  TfToken indexToken;
  if (drawItem->GetDrawingCoord()->GetTopologyIndex() == HdStBasisCurves::HullTopology) {
    if ((*dirtyBits & DirtyHullIndices) == 0)
      return;
    *dirtyBits &= ~DirtyHullIndices;
    indexToken = HdTokens->hullIndices;
  }
  else if (drawItem->GetDrawingCoord()->GetTopologyIndex() == HdStBasisCurves::PointsTopology) {
...
template<typename T>
void AddVertexOrVaryingPrimvarSource(const SdfPath &id,
                                     const TfToken &name,
                                     ...
                                     T fallbackValue)
{
  ...
  if (!array.empty() || name == HdTokens->points) {
    sources->push_back(std::make_shared<HdSt_BasisCurvesPrimvarInterpolaterComputation<T>>(
        topology,
        array,
        id,
        name,
...
void ProcessVertexOrVaryingPrimvar(const SdfPath &id,
                                   const TfToken &name,
                                   HdInterpolation interpolation,
                                   const VtValue &value,
                                   HdSt_BasisCurvesTopologySharedPtr topology,
                                   HdBufferSourceSharedPtrVector *sources)
{
  if (value.IsHolding<VtHalfArray>()) {
    AddVertexOrVaryingPrimvarSource<GfHalf>(id, name, interpolation, value, topology, sources, 1);
  }
  else if (value.IsHolding<VtFloatArray>()) {
    AddVertexOrVaryingPrimvarSource<float>(id, name, interpolation, value, topology, sources, 1);
  }
    ...
    AddVertexOrVaryingPrimvarSource<double>(id, name, interpolation, value, topology, sources, 1);
  }
    ...
    AddVertexOrVaryingPrimvarSource<int>(id, name, interpolation, value, topology, sources, 1);
  }
    ...
    AddVertexOrVaryingPrimvarSource<int16_t>(id, name, interpolation, value, topology, sources, 1);
  }
  else if (value.IsHolding<VtArray<int32_t>>()) {
    AddVertexOrVaryingPrimvarSource<int32_t>(id, name, interpolation, value, topology, sources, 1);
  }
    ...
    TF_WARN(
        "HdStBasisCurves(%s) - Type of vertex or varying primvar %s"
                                              ...
                                              HdDirtyBits *dirtyBits)
{
  ...
  for (HdPrimvarDescriptor const &primvar : primvars) {
    ...
    if (!value.IsEmpty()) {
      ProcessVertexOrVaryingPrimvar(
          id, primvar.name, HdInterpolationVertex, value, _topology, &sources);
  ...
  HdBufferArrayRangeSharedPtr range = resourceRegistry->UpdateNonUniformBufferArrayRange(
      HdTokens->primvar, bar, bufferSpecs, removedSpecs, HdBufferArrayUsageHintBitsVertex);
...
void HdStBasisCurves::_PopulateVaryingPrimvars(HdSceneDelegate *sceneDelegate,
                                               HdRenderParam *renderParam,
                                               HdStDrawItem *drawItem,
                                               HdDirtyBits *dirtyBits)
{
  ...
  for (HdPrimvarDescriptor const &primvar : primvars) {
    ...
    if (!value.IsEmpty()) {
      ProcessVertexOrVaryingPrimvar(
          id, primvar.name, HdInterpolationVarying, value, _topology, &sources);
                                               ...
                                               HdDirtyBits *dirtyBits)
{
  ...
  HdPrimvarDescriptorVector uniformPrimvars = HdStGetPrimvarDescriptors(
...
static bool HdSt_HasResource(HdStDrawItem *drawItem, const TfToken &resourceToken)
{
  ...
  if (HdBarPtr const &bar = drawItem->GetConstantPrimvarRange()) {
    HdStBufferArrayRangeSharedPtr bar_ = std::static_pointer_cast<HdStBufferArrayRange>(bar);
...
bool HdStBasisCurves::_SupportsRefinement(int refineLevel)
{
  if (!_topology) {
...
bool HdStBasisCurves::_SupportsUserWidths(HdStDrawItem *drawItem)
{
  return HdSt_HasResource(drawItem, HdTokens->widths);
}
bool HdStBasisCurves::_SupportsUserNormals(HdStDrawItem *drawItem)
{
  return HdSt_HasResource(drawItem, HdTokens->normals);
}

HdDirtyBits HdStBasisCurves::GetInitialDirtyBitsMask() const
{
...
/*override*/
TfTokenVector const &HdStBasisCurves::GetBuiltinPrimvarNames() const
{
  ...
  auto _ComputePrimvarNames = [this]() {
    TfTokenVector primvarNames = this->HdBasisCurves::GetBuiltinPrimvarNames();
