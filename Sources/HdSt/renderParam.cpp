...
void HdStRenderParam::MarkDrawBatchesDirty()
{
  ++_drawBatchesVersion;  // uses std::memory_order_seq_cst
}

unsigned int HdStRenderParam::GetDrawBatchesVersion() const
{
...
void HdStRenderParam::MarkMaterialTagsDirty()
{
  ++_materialTagsVersion;  // uses std::memory_order_seq_cst
}

unsigned int HdStRenderParam::GetMaterialTagsVersion() const
{
  // Can use relaxed ordering because render passes are expected to
  // only read the value, and that too in a single threaded fashion.
...
unsigned int HdStRenderParam::GetGeomSubsetDrawItemsVersion() const
{
  // Can use relaxed ordering because render passes are expected to
  // only read the value, and that too in a single threaded fashion.
...
bool HdStRenderParam::HasMaterialTag(const TfToken &materialTag) const
{
  return _HasTag(&_materialTagToCountMutex, &_materialTagToCount, materialTag);
}

void HdStRenderParam::IncreaseMaterialTagCount(const TfToken &materialTag)
{
  _AdjustTagCount(&_materialTagToCountMutex, &_materialTagToCount, materialTag, +1);
}

void HdStRenderParam::DecreaseMaterialTagCount(const TfToken &materialTag)
{
  _AdjustTagCount(&_materialTagToCountMutex, &_materialTagToCount, materialTag, -1);
}
...
bool HdStRenderParam::HasAnyRenderTag(const TfTokenVector &renderTags) const
{
...
void HdStRenderParam::IncreaseRenderTagCount(const TfToken &renderTag)
{
  _AdjustTagCount(&_renderTagToCountMutex, &_renderTagToCount, renderTag, +1);
}

void HdStRenderParam::DecreaseRenderTagCount(const TfToken &renderTag)
{
  _AdjustTagCount(&_renderTagToCountMutex, &_renderTagToCount, renderTag, -1);
}

void HdStRenderParam::_AdjustTagCount(std::shared_timed_mutex *mutex,
...
bool HdStRenderParam::_HasTag(std::shared_timed_mutex *mutex,
