...
HdStInstancer::HdStInstancer(HdSceneDelegate *delegate, SdfPath const &id)
    : HdInstancer(delegate, id), _instancePrimvarNumElements(0)
{
}

void HdStInstancer::Sync(HdSceneDelegate *sceneDelegate,
...
void HdStInstancer::_SyncPrimvars(HdSceneDelegate *sceneDelegate, HdDirtyBits *dirtyBits)
{
  SdfPath const &instancerId = GetId();

  HdPrimvarDescriptorVector primvars = HdStGetInstancerPrimvarDescriptors(this, sceneDelegate);
      ...
      if (primvar.name == HdInstancerTokens->instanceTransforms) {
        if (value.IsHolding<VtArray<GfMatrix4d>>()) {
          ...
          source.reset(new HdVtBufferSource(
              primvar.name, value.UncheckedGet<VtArray<GfMatrix4d>>(), 1, doublesSupported));
        }
        else if (value.IsHolding<VtArray<GfMatrix4f>>()) {
          source.reset(new HdVtBufferSource(primvar.name, value, 1, false));
        }
      }
      else {
  ...
  if (!HdStCanSkipBARAllocationOrUpdate(sources, _instancePrimvarRange, *dirtyBits)) {
    ...
    if (hasDirtyPrimvarDesc) {
      ...
      removedSpecs = HdStGetRemovedOrReplacedPrimvarBufferSpecs(
          _instancePrimvarRange, primvars, internallyGeneratedPrimvars, bufferSpecs, instancerId);
    }
...
void HdStInstancer::_GetInstanceIndices(SdfPath const &prototypeId,
...
VtIntArray HdStInstancer::GetInstanceIndices(SdfPath const &prototypeId)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
