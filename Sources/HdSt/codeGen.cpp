...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    ((_double, "double"))((_float, "float"))((_int, "int"))((_uint, "uint"))((_bool, "bool"))(
        (_atomic_int, "atomic_int"))((_atomic_uint, "atomic_uint"))((_default, "default"))(flat)(noperspective)(sample)(centroid)(patch)(hd_barycentricCoord)(hd_patchID)(hd_tessCoord)(hd_vec3)(hd_vec3_get)(hd_vec3_set)(hd_ivec3)(hd_ivec3_get)(hd_ivec3_set)(hd_dvec3)(hd_dvec3_get)(hd_dvec3_set)(hd_mat3)(hd_mat3_get)(hd_mat3_set)(hd_dmat3)(hd_dmat3_get)(hd_dmat3_set)(hd_vec4_2_10_10_10_get)(hd_vec4_2_10_10_10_set)(hd_half2_get)(hd_half2_set)(hd_half4_get)(hd_half4_set)(PrimvarData)(inPrimvars)(uvec2)(uvec3)(uvec4)(ivec2)(ivec3)(ivec4)(outPrimvars)(vec2)(vec3)(vec4)(dvec2)(dvec3)(dvec4)(mat3)(mat4)(dmat3)(dmat4)(packed_2_10_10_10)(packed_half2)(packed_half4)((ptexTextureSampler,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         "ptexTextureSampler"))(isamplerBuffer)(samplerBuffer)(gl_MaxPatchVertices)(HD_NUM_PATCH_EVAL_VERTS)(HD_NUM_PRIMITIVE_VERTS)(quads)(isolines)(equal_spacing)(fractional_even_spacing)(fractional_odd_spacing)(cw)(ccw)(points)(lines)(lines_adjacency)(triangles)(triangles_adjacency)(line_strip)(triangle_strip)(early_fragment_tests)(OsdPerPatchVertexBezier));

TF_DEFINE_ENV_SETTING(HDST_ENABLE_HGI_RESOURCE_GENERATION,
                      false,
                      "Enable Hgi resource generation for codeGen");

/* static */
bool HdSt_CodeGen::IsEnabledHgiResourceGeneration(Hgi const *hgi)
{
  static bool const isEnabled = TfGetEnvSetting(HDST_ENABLE_HGI_RESOURCE_GENERATION);
      ...
      _hasClipPlanes(false)
{
  ...
  TF_VERIFY(_metaData, "Invalid MetaData ptr passed in as constructor arg.");
}
...
HdSt_CodeGen::ID HdSt_CodeGen::ComputeHash() const
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();

  if (!TF_VERIFY(_metaData, "Metadata not properly initialized by resource binder.")) {
    return {};
  }

  return TfHash::Combine(_geometricShader ? _geometricShader->ComputeHash() : 0,
...
static std::string _GetPtexTextureShaderSource()
{
  static std::string source =
      HioGlslfx(HdStPackagePtexTextureShader()).GetSource(_tokens->ptexTextureSampler);
  return source;
}
...
static void _EmitScalarAccessor(std::stringstream &str, TfToken const &name, TfToken const &type);
/*
...
{
  return "// Alias hgi vec and matrix types to hd.\n"
...
static std::string _GetFlatTypeSwizzleString(TfToken const &token)
{
  if (token == _tokens->ivec2 || token == _tokens->ivec3 || token == _tokens->ivec4 ||
      token == _tokens->uvec2 || token == _tokens->uvec3 || token == _tokens->uvec4 ||
      token == _tokens->vec2 || token == _tokens->vec3 || token == _tokens->vec4 ||
      token == _tokens->dvec2 || token == _tokens->dvec3 || token == _tokens->dvec4 ||
      token == _tokens->packed_2_10_10_10 || token == _tokens->packed_half2 ||
...
static TfToken const &_ConvertBoolType(TfToken const &token)
{
...
class _ResourceGenerator {
 public:
  ...
  void _GenerateHgiResources(Hgi const *hgi,
  ...
  void _GenerateGLSLResources(HgiShaderFunctionDesc *funcDesc,
 ...
 private:
  ...
  int32_t _GetLocation(HioGlslfxResourceLayout::Element const &element,
                       HdSt_ResourceBinder::MetaData const &metaData)
  {
  ...
  TfToken _GetFlattenedName(TfToken const &aggregateName, TfToken const &memberName) const
  {
    return TfToken(aggregateName.GetString() + "_" + memberName.GetString());
  }
...
bool _IsVertexAttribInputStage(TfToken const &shaderStage)
{
...
uint32_t _GetLocationCount(Hgi const *hgi, TfToken const &dataType)
{
...
void _ResourceGenerator::_GenerateHgiResources(
    Hgi const *hgi,
    HgiShaderFunctionDesc *funcDesc,
    TfToken const &shaderStage,
    HioGlslfxResourceLayout::ElementVector const &elements,
    ...
    if (element.kind == Kind::VALUE) {
      if (element.inOut == InOut::STAGE_IN) {
        if (_IsVertexAttribInputStage(shaderStage)) {
          ...
          param.nameInShader = element.name;
          param.type = element.dataType;
          param.interstageSlot = _GetSlot(
              element.name, /*in*/ true, _GetLocationCount(hgi, element.dataType));
        ...
        if (shaderStage == HdShaderTokens->fragmentShader) {
          HgiShaderFunctionAddStageOutput(funcDesc,
                                          /*name=*/element.name,
          ...
          HgiShaderFunctionParamDesc param;
          param.nameInShader = element.name, param.type = element.dataType,
          param.interstageSlot = _GetSlot(
              element.name, /*in*/ false, _GetLocationCount(hgi, element.dataType));
          param.interpolation = _GetInterpolation(element.qualifiers);
          param.sampling = _GetSamplingQualifier(element.qualifiers);
          param.storage = _GetStorageQualifier(element.qualifiers);
          param.arraySize = element.arraySize;
      ...
      TfToken const firstMemberBlockName = _GetFlattenedName(element.aggregateName,
      ...
      paramBlock.interstageSlot = _GetSlot(firstMemberBlockName,
                                           /*in*/ element.inOut == InOut::STAGE_IN,
                                           locationCount);
      ...
      if (shaderStage == HdShaderTokens->tessControlShader) {
        if (element.inOut == InOut::STAGE_OUT) {
          funcDesc->tessellationDescriptor.numVertsPerPatchOut = element.qualifiers.GetString();
        }
      }
               ...
               shaderStage == HdShaderTokens->postTessVertexShader)
      {
        if (element.inOut == InOut::STAGE_IN) {
          ...
          else if (element.qualifiers == _tokens->equal_spacing) {
            funcDesc->tessellationDescriptor.spacing =
                HgiShaderFunctionTessellationDesc::Spacing::Equal;
          }
          else if (element.qualifiers == _tokens->fractional_even_spacing) {
            funcDesc->tessellationDescriptor.spacing =
                HgiShaderFunctionTessellationDesc::Spacing::FractionalEven;
          }
          else if (element.qualifiers == _tokens->fractional_odd_spacing) {
            funcDesc->tessellationDescriptor.spacing =
        ...
        if (element.inOut == InOut::STAGE_IN) {
          ...
          else if (element.qualifiers == _tokens->triangles_adjacency) {
            funcDesc->geometryDescriptor.inPrimitiveType =
          ...
          if (element.qualifiers == _tokens->points) {
            ...
            funcDesc->geometryDescriptor.outMaxVertices = element.qualifiers.GetString();
          }
        }
      }
      ...
      if (TF_VERIFY(element.members.size() == 1)) {
        ...
        uint32_t const arraySize = (element.arraySize.IsEmpty() ?
                                        0 :
                                        static_cast<uint32_t>(std::stoi(element.arraySize)));

        if (arraySize > 0) {
          HgiShaderFunctionAddBuffer(funcDesc,
                                     /*name=*/member.name,
      ...
      for (auto const &member : element.members) {
        HgiShaderFunctionAddConstantParam(funcDesc,
                                          /*name=*/member.name,
                                          /*type=*/_ConvertBoolType(member.dataType));
      }
    }
      ...
      if (TF_VERIFY(element.members.size() == 1)) {
        auto const &member = element.members.front();
        HgiShaderFunctionAddWritableBuffer(funcDesc,
                                           /*name=*/member.name,
                                           /*type=*/_ConvertBoolType(member.dataType),
                                           /*bindIndex=*/_GetLocation(element, metaData));
      }
...
void _ResourceGenerator::_GenerateHgiTextureResources(
    HgiShaderFunctionDesc *funcDesc,
    TfToken const &shaderStage,
    HioGlslfxResourceLayout::TextureElementVector const &textureElements,
    HdSt_ResourceBinder::MetaData const &metaData)
{
  ...
  for (auto const &texture : textureElements) {
    HgiShaderTextureType const textureType = texture.textureType == TextureType::SHADOW_TEXTURE ?
    ...
    HdFormat const hdTextureFormat = HdStHioConversions::GetHdFormat(texture.format);
    if (texture.arraySize > 0) {
      HgiShaderFunctionAddArrayOfTextures(funcDesc,
                                          texture.name,
                                          texture.arraySize,
      ...
      HgiShaderFunctionAddTexture(funcDesc,
                                  texture.name,
                                  texture.bindingIndex,
                                  texture.dim,
                                  HdStHgiConversions::GetHgiFormat(hdTextureFormat),
...
void _ResourceGenerator::_GenerateGLSLResources(
    HgiShaderFunctionDesc *funcDesc,
    std::stringstream &str,
    TfToken const &shaderStage,
    HioGlslfxResourceLayout::ElementVector const &elements,
  ...
  for (auto const &element : elements) {
    switch (element.kind) {
      case HioGlslfxResourceLayout::Kind::VALUE:
        switch (element.inOut) {
          case HioGlslfxResourceLayout::InOut::STAGE_IN:
            if (shaderStage == HdShaderTokens->vertexShader) {
              ;
              str << "layout (location = " << _GetLocation(element, metaData) << ") ";
            }
            str << "in ";
            break;
        ...
        str << element.dataType << " " << element.name;
      ...
      case HioGlslfxResourceLayout::Kind::UNIFORM_VALUE:
        str << "layout(location = " << _GetLocation(element, metaData) << ") uniform "
      ...
      case HioGlslfxResourceLayout::Kind::UNIFORM_BLOCK:
        str << "layout(std140, binding = " << _GetLocation(element, metaData) << ") uniform ubo_"
            << element.name << " {\n"
            << "    " << element.dataType << " " << element.name;
        if (!element.arraySize.IsEmpty()) {
          str << "[" << element.arraySize << "]";
      ...
      case HioGlslfxResourceLayout::Kind::BUFFER_READ_WRITE: {
        ...
        str << "layout(std430, binding = " << location << ") buffer ssbo_" << location << " {\n"
...
static std::string _GetGLSLSamplerTypePrefix(HioFormat hioFormat)
{
...
void _ResourceGenerator::_GenerateGLSLTextureResources(
    std::stringstream &str,
    TfToken const &shaderStage,
    HioGlslfxResourceLayout::TextureElementVector const &textureElements,
    HdSt_ResourceBinder::MetaData const &metaData)
{
  for (auto const &texture : textureElements) {
    bool const isArrayTexture = (texture.textureType == TextureType::ARRAY_TEXTURE);

    bool const isShadowTexture = (texture.textureType == TextureType::SHADOW_TEXTURE);

    bool const isArrayOfTexture = (texture.arraySize > 0);

    const std::string typePrefix = _GetGLSLSamplerTypePrefix(texture.format);

    std::string const samplerType = typePrefix +
                                    (isShadowTexture ?
                                         "sampler" + std::to_string(texture.dim) + "DShadow" :
                                         "sampler" + std::to_string(texture.dim) + "D") +
                                    (isArrayTexture ? "Array" : "");

    std::string const resultType = typePrefix + (isShadowTexture ? "float" : "vec4");

    std::string const resourceName = "sampler" + std::to_string(texture.dim) + "d_" +
                                     texture.name.GetString();

    int const coordDim = (isArrayTexture || isShadowTexture) ? (texture.dim + 1) : texture.dim;

    std::string const intCoordType = coordDim == 1 ? "int" : "ivec" + std::to_string(coordDim);
    std::string const floatCoordType = coordDim == 1 ? "float" : "vec" + std::to_string(coordDim);

    // Resource Declaration
    str << "layout (binding = " << std::to_string(texture.bindingIndex) << ") uniform "
    ...
    // Accessors
    if (isArrayOfTexture) {
      str << "#define HgiGetSampler_" << texture.name << "(index) " << resourceName << "[index]\n";

      str << resultType << " HgiGet_" << texture.name << "(int index, " << floatCoordType
          << " coord) {\n"
          << "  return texture(" << resourceName << "[index], coord)"
          << ";\n"
          << "}\n";

      str << resultType << " HgiTextureLod_" << texture.name << "(int index, " << floatCoordType
          << " coord, float lod) {\n"
          << "  return textureLod(" << resourceName << "[index], coord, lod)"
          << ";\n"
          << "}\n";

      if (!isShadowTexture) {
        str << resultType << " HgiTexelFetch_" << texture.name << "(int index, " << intCoordType
            << " coord) {\n"
            << "  return texelFetch(" << resourceName << "[index], coord, 0)"
            << ";\n"
            << "}\n";
      }
    }
    else {
      str << "#define HgiGetSampler_" << texture.name << "() " << resourceName << "\n";

      str << resultType << " HgiGet_" << texture.name << "(" << floatCoordType << " coord) {\n"
          << "  return texture(" << resourceName << ", coord)"
          << ";\n"
          << "}\n";

      str << resultType << " HgiTextureLod_" << texture.name << "(" << floatCoordType
          << " coord, float lod) {\n"
      ...
      if (!isShadowTexture) {
        str << resultType << " HgiTexelFetch_" << texture.name << "(" << intCoordType
            << " coord) {\n"
...
static void _AddVertexAttribElement(HioGlslfxResourceLayout::ElementVector *elements,
                                    TfToken const &name,
                                    TfToken const &dataType,
                                    int location,
                                    int arraySize = 0)
{
  elements->emplace_back(HioGlslfxResourceLayout::InOut::STAGE_IN,
...
static void _AddInterstageElement(HioGlslfxResourceLayout::ElementVector *elements,
                                  ...
                                  TfToken const &qualifier = TfToken())
{
  elements->emplace_back(
      inOut, HioGlslfxResourceLayout::Kind::VALUE, dataType, name, arraySize, qualifier);
}

static void _AddInterstageBlockElement(HioGlslfxResourceLayout::ElementVector *elements,
                                       HioGlslfxResourceLayout::InOut inOut,
...
static void _AddUniformValueElement(HioGlslfxResourceLayout::ElementVector *elements,
                                    TfToken const &name,
                                    TfToken const &dataType,
                                    int location,
                                    int arraySize = 0)
{
  TfToken const arraySizeArg = ((arraySize > 0) ? TfToken(std::to_string(arraySize)) : TfToken());

  elements->emplace_back(HioGlslfxResourceLayout::InOut::NONE,
...
static void _AddUniformBufferElement(HioGlslfxResourceLayout::ElementVector *elements,
                                     TfToken const &name,
                                     TfToken const &dataType,
                                     int location,
                                     int arraySize = 0)
...
static void _AddBufferElement(HioGlslfxResourceLayout::ElementVector *elements,
                              TfToken const &name,
                              TfToken const &dataType,
                              int location,
                              int arraySize = 0)
...
static void _AddWritableBufferElement(HioGlslfxResourceLayout::ElementVector *elements,
                                      TfToken const &name,
                                      TfToken const &dataType,
                                      int location)
{
...
static void _AddTextureElement(HioGlslfxResourceLayout::TextureElementVector *textureElements,
                               TfToken const &name,
                               int dim,
                               int bindingIndex,
...
{
  textureElements->emplace_back(name, dim, bindingIndex, format, textureType);
}

static void _AddArrayOfTextureElement(
    ...
    int arraySize = 0)
{
  textureElements->emplace_back(name, dim, bindingIndex, format, textureType, arraySize);
}

static bool _IsAtomicBufferShaderResource(HioGlslfxResourceLayout::ElementVector &elements,
                                          TfToken const &name)
{
  for (auto const &element : elements) {
    if (element.name == name && element.kind == HioGlslfxResourceLayout::Kind::BUFFER_READ_WRITE) {
      if (TF_VERIFY(element.members.size() == 1)) {
        TfToken const &dataType = element.members.front().dataType;
        if (dataType == _tokens->_atomic_int || dataType == _tokens->_atomic_uint) {
          return true;
        }
      }
    }
...
void HdSt_CodeGen::_GetShaderResourceLayouts(HdStShaderCodeSharedPtrVector const &shaders)
{
  TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  for (auto const &shader : shaders) {
    ...
    HioGlslfxResourceLayout::ParseLayout(&_resVS, HdShaderTokens->vertexShader, layoutDict);

    HioGlslfxResourceLayout::ParseLayout(&_resTCS, HdShaderTokens->tessControlShader, layoutDict);

    HioGlslfxResourceLayout::ParseLayout(&_resTES, HdShaderTokens->tessEvalShader, layoutDict);

    HioGlslfxResourceLayout::ParseLayout(&_resGS, HdShaderTokens->geometryShader, layoutDict);

    HioGlslfxResourceLayout::ParseLayout(&_resFS, HdShaderTokens->fragmentShader, layoutDict);
    ...
    HioGlslfxResourceLayout::ParseLayout(&_resCS, HdShaderTokens->computeShader, layoutDict);
  }
}

void HdSt_CodeGen::_PlumbInterstageElements(TfToken const &name, TfToken const &dataType)
{
  ...
  TfToken const &qualifier = (dataType == _tokens->_int) ? _tokens->flat : _tokens->_default;

  // Vertex attrib input for VS, PTCS, PTVS
  _resAttrib.emplace_back(
      InOut::STAGE_OUT, Kind::VALUE, dataType, vs_outName, noArraySize, qualifier);

  if (_hasTCS) {
    _resTCS.emplace_back(InOut::STAGE_IN,
                         Kind::VALUE,
                         dataType,
                         vs_outName,
                         _tokens->gl_MaxPatchVertices,
                         qualifier);
    _resTCS.emplace_back(InOut::STAGE_OUT,
                         Kind::VALUE,
                         dataType,
                         tcs_outName,
  ...
  if (_hasTES) {
    _resTES.emplace_back(InOut::STAGE_IN,
                         Kind::VALUE,
                         dataType,
                         tcs_outName,
                         _tokens->gl_MaxPatchVertices,
                         qualifier);
    _resTES.emplace_back(
        InOut::STAGE_OUT, Kind::VALUE, dataType, tes_outName, noArraySize, qualifier);
  }

  // Geometry shader inputs come from previous active stage
  if (_hasGS && _hasTES) {
    _resGS.emplace_back(InOut::STAGE_IN,
                        Kind::VALUE,
                        dataType,
                        tes_outName,
                        ...
                        qualifier);
    _resGS.emplace_back(
        InOut::STAGE_OUT, Kind::VALUE, dataType, gs_outName, noArraySize, qualifier);
  }
  else if (_hasGS) {
    _resGS.emplace_back(InOut::STAGE_IN,
                        Kind::VALUE,
  ...
  if (_hasGS) {
    _resFS.emplace_back(
        InOut::STAGE_IN, Kind::VALUE, dataType, gs_outName, noArraySize, qualifier);
  }
  else if (_hasTES) {
    _resFS.emplace_back(
        InOut::STAGE_IN, Kind::VALUE, dataType, tes_outName, noArraySize, qualifier);
  }
  else {
    _resFS.emplace_back(
        InOut::STAGE_IN, Kind::VALUE, dataType, vs_outName, noArraySize, qualifier);
  }
}

static std::string _GetOSDCommonShaderSource()
{
...
static std::string _GetOSDPatchBasisShaderSource()
{
  std::stringstream ss;
#if defined(__APPLE__)
...
HdStGLSLProgramSharedPtr HdSt_CodeGen::Compile(HdStResourceRegistry *const registry)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  const bool bindlessTextureEnabled = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsBindlessTextures);
  const bool bindlessBuffersEnabled = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsBindlessBuffers);
  const bool shaderDrawParametersEnabled = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsShaderDrawParameters);
  const bool builtinBarycentricsEnabled = capabilities->IsSet(
  ...
  const bool doublePrecisionEnabled = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsShaderDoublePrecision);
  const bool minusOneToOneDepth = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsDepthRangeMinusOnetoOne);

  bool const useHgiResourceGeneration = IsEnabledHgiResourceGeneration(registry->GetHgi());
  ...
  std::string postTessControlShader = _geometricShader->GetSource(
      HdShaderTokens->postTessControlShader);
  std::string postTessVertexShader = _geometricShader->GetSource(
      HdShaderTokens->postTessVertexShader);
  ...
  _genDefines << "#define MAT4 "
              << HdStGLConversions::GetGLSLTypename(HdVtBufferSource::GetDefaultMatrixType())
              << "\n";
  ...
  {
    _genDefines << "#define " << binDecl->name << "_Binding " << binDecl->binding.GetLocation()
                << "\n";
    ...
    _EmitDeclaration(
        &_resCommon, binDecl->name, binDecl->dataType, binDecl->binding, binDecl->isWritable);

    _EmitAccessor(_genAccessors,
                  binDecl->name,
                  binDecl->dataType,
                  binDecl->binding,
                  (binDecl->binding.GetType() == HdStBinding::UNIFORM) ? NULL : "localIndex");
  }
  ...
  TF_FOR_ALL(it, _metaData->customInterleavedBindings)
  {
    ...
    TF_FOR_ALL(dbIt, it->second.entries)
    {
      ...
      if (dbIt->arraySize > 1) {
        _genDefines << "#define HD_NUM_" << dbIt->name << " " << dbIt->arraySize << "\n";
        _genDecl << "[" << dbIt->arraySize << "]";
      }
      _genDecl << ";\n";

      if (it->second.arraySize > 0) {
        _EmitStructAccessor(_genAccessors,
                            varName,
                            dbIt->name,
                            dbIt->dataType,
                            dbIt->arraySize,
  ...
  if (_geometricShader->IsPrimTypePatches()) {
    _genDefines << "#define HD_NUM_PATCH_VERTS " << _geometricShader->GetPrimitiveIndexSize()
                << "\n";
    _genDefines << "#define HD_NUM_PATCH_EVAL_VERTS " << _geometricShader->GetNumPatchEvalVerts()
                << "\n";
  }
  ...
  TF_FOR_ALL(it, _metaData->instanceData)
  {
    ...
    _genDefines << "#define HD_HAS_" << it->second.name << "_" << it->second.level << " 1\n";
  }
  _genDefines << "#define HD_INSTANCER_NUM_LEVELS " << _metaData->instancerNumLevels << "\n"
  ...
  TF_FOR_ALL(it, _metaData->shaderParameterBinding)
  {
    ...
    {
      _genDefines << "#define HD_HAS_COORD_" << it->second.name << " 1\n";
    }
  }

  // Needed for patch-based position and primvar refinement
  if (_geometricShader->IsPrimTypeMesh() && _geometricShader->IsPrimTypePatches()) {
    if (_hasPTCS) {
  ...
  if (_geometricShader->GetFvarPatchType() == HdSt_GeometricShader::FvarPatchType::PATCH_BSPLINE ||
  ...
  _GenerateDrawingCoord(
      shaderDrawParametersEnabled, requiresBasePrimitiveOffset, requiresPrimitiveIdEmulation);
  ...
  TF_FOR_ALL(it, _shaders)
  {
    ...
    if (_hasPTCS) {
      _genPTCS << shader->GetSource(HdShaderTokens->postTessControlShader);
    }
    if (_hasPTVS) {
  ...
  if (_geometricShader->IsPrimTypeRefinedMesh()) {
    ...
    _osd << "vec4 InterpolatePatchCoord(vec2 uv, ivec3 patchParam)\n"
            "{\n"
...
HdStGLSLProgramSharedPtr HdSt_CodeGen::CompileComputeProgram(HdStResourceRegistry *const registry)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdSt_CodeGen::_GenerateComputeParameters(HgiShaderFunctionDesc *const csDesc)
{
  std::stringstream accessors;

  bool const hasComputeData = !_metaData->computeReadWriteData.empty() ||
                              !_metaData->computeReadOnlyData.empty();
  if (hasComputeData) {
    HgiShaderFunctionAddConstantParam(csDesc, "vertexOffset", _tokens->_int);
  }
  ...
  TF_FOR_ALL(it, _metaData->computeReadWriteData)
  {
    ...
    TfToken declDataType = (binding.GetType() == HdStBinding::SSBO ? _GetFlatType(dataType) :
                                                                     dataType);

    HgiShaderFunctionAddConstantParam(csDesc, name.GetString() + "Offset", _tokens->_int);
    HgiShaderFunctionAddConstantParam(csDesc, name.GetString() + "Stride", _tokens->_int);
  ...
  TF_FOR_ALL(it, _metaData->computeReadOnlyData)
  {
    ...
    _EmitDeclaration(&_resCommon, name, declDataType, binding);
    // getter
    {
      std::stringstream indexing;
      // no vertex offset for constant data
  ...
  HgiShaderFunctionAddStageInput(
      csDesc, "hd_GlobalInvocationID", "uvec3", HgiShaderKeywordTokens->hdGlobalInvocationID);
...
HdStGLSLProgramSharedPtr HdSt_CodeGen::_CompileWithGeneratedGLSLResources(
    HdStResourceRegistry *const registry)
{
  ...
  // create GLSL program.
  HdStGLSLProgramSharedPtr glslProgram = std::make_shared<HdStGLSLProgram>(HdTokens->drawingShader,
                                                                           registry);
  ...
  if (_hasVS) {
    ...
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->vertexShader, _resAttrib, _GetMetaData());
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->vertexShader, _resCommon, _GetMetaData());
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->vertexShader, _resVS, _GetMetaData());

    std::string const declarations = _genDefines.str();
    std::string const source = _genDecl.str() + resDecl.str() + _genAccessors.str() + _genVS.str();
    ...
    HgiShaderFunctionAddStageInput(
        &desc, "hd_VertexID", "uint", HgiShaderKeywordTokens->hdVertexID);
    HgiShaderFunctionAddStageInput(
        &desc, "hd_InstanceID", "uint", HgiShaderKeywordTokens->hdInstanceID);
    HgiShaderFunctionAddStageInput(
        &desc, "hd_BaseInstance", "uint", HgiShaderKeywordTokens->hdBaseInstance);

    if (_hasClipPlanes) {
      HgiShaderFunctionAddStageOutput(
          &desc, "gl_ClipDistance", "float", "clip_distance", /*arraySize*/ "HD_NUM_clipPlanes");
    }
  ...
  if (_hasTCS) {
    ...
    std::stringstream resDecl;
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->tessControlShader, _resCommon, _GetMetaData());
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->tessControlShader, _resTCS, _GetMetaData());

    std::string const declarations = _genDefines.str() + _osd.str();
    std::string const source = _genDecl.str() + resDecl.str() + _genAccessors.str() +
                               _genTCS.str();
  ...
  if (_hasTES) {
    ...
    std::stringstream resDecl;
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->tessEvalShader, _resCommon, _GetMetaData());
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->tessEvalShader, _resTES, _GetMetaData());
  ...
  if (_hasGS) {
    ...
    std::stringstream resDecl;
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->geometryShader, _resCommon, _GetMetaData());
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->geometryShader, _resGS, _GetMetaData());

    // material in GS
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->geometryShader, _resMaterial, _GetMetaData());
    resourceGen._GenerateGLSLTextureResources(
        resDecl, HdShaderTokens->geometryShader, _resTextures, _GetMetaData());

    std::string const declarations = _genDefines.str() + _osd.str();
    std::string const source = _genDecl.str() + resDecl.str() + _genAccessors.str() + _genGS.str();
  ...
  if (_hasFS) {
    ...
    std::stringstream resDecl;
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->fragmentShader, _resCommon, _GetMetaData());
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->fragmentShader, _resFS, _GetMetaData());

    // material in FS
    resourceGen._GenerateGLSLResources(
        &desc, resDecl, HdShaderTokens->fragmentShader, _resMaterial, _GetMetaData());
    resourceGen._GenerateGLSLTextureResources(
        resDecl, HdShaderTokens->fragmentShader, _resTextures, _GetMetaData());

    std::string const source = _genDefines.str() + _genDecl.str() + resDecl.str() + _osd.str() +
    ...
    const bool builtinBarycentricsEnabled = registry->GetHgi()->GetCapabilities()->IsSet(
        HgiDeviceCapabilitiesBitsBuiltinBarycentrics);
    if (builtinBarycentricsEnabled) {
      HgiShaderFunctionAddStageInput(
          &desc, "hd_BaryCoordNoPersp", "vec3", HgiShaderKeywordTokens->hdBaryCoordNoPersp);
    }
...
HdStGLSLProgramSharedPtr HdSt_CodeGen::_CompileWithGeneratedHgiResources(
    HdStResourceRegistry *const registry)
{
  // Generator assigns attribute and binding locations
  _ResourceGenerator resourceGen;
  ...
  if (_hasVS) {
    ...
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &vsDesc, HdShaderTokens->vertexShader, _resAttrib, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &vsDesc, HdShaderTokens->vertexShader, _resCommon, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &vsDesc, HdShaderTokens->vertexShader, _resVS, _GetMetaData());
    ...
    HgiShaderFunctionAddStageInput(
        &vsDesc, "hd_VertexID", "uint", HgiShaderKeywordTokens->hdVertexID);
    HgiShaderFunctionAddStageInput(
        &vsDesc, "hd_InstanceID", "uint", HgiShaderKeywordTokens->hdInstanceID);
    HgiShaderFunctionAddStageInput(
        &vsDesc, "hd_BaseInstance", "uint", HgiShaderKeywordTokens->hdBaseInstance);
    HgiShaderFunctionAddStageInput(
        &vsDesc, "gl_BaseVertex", "uint", HgiShaderKeywordTokens->hdBaseVertex);

    if (!_geometricShader->IsFrustumCullingPass()) {
      HgiShaderFunctionAddStageOutput(&vsDesc, "gl_Position", "vec4", "position");

      // For Metal, only set the role for the point size
      // if the primitive is a point list.
      char const *pointRole = (_geometricShader->GetPrimitiveType() ==
                               HdSt_GeometricShader::PrimitiveType::PRIM_POINTS) ?
                                  "point_size" :
                                  "";
      HgiShaderFunctionAddStageOutput(&vsDesc, "gl_PointSize", "float", pointRole);
    }

    if (_hasClipPlanes) {
      HgiShaderFunctionAddStageOutput(
          &vsDesc, "gl_ClipDistance", "float", "clip_distance", /*arraySize*/ "HD_NUM_clipPlanes");
    }
  ...
  if (_hasTCS) {
    ...
    resourceGen._AdvanceShaderStage();
    resourceGen._GenerateHgiResources(registry->GetHgi(),
                                      &tcsDesc,
                                      HdShaderTokens->tessControlShader,
                                      _resCommon,
                                      _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &tcsDesc, HdShaderTokens->tessControlShader, _resTCS, _GetMetaData());
  ...
  if (_hasTES) {
    ...
    resourceGen._AdvanceShaderStage();
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &tesDesc, HdShaderTokens->tessEvalShader, _resCommon, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &tesDesc, HdShaderTokens->tessEvalShader, _resTES, _GetMetaData());
    ...
    if (_hasClipPlanes) {
      HgiShaderFunctionAddStageOutput(&tesDesc,
                                      "gl_ClipDistance",
                                      "float",
                                      "clip_distance",
                                      /*arraySize*/ "HD_NUM_clipPlanes");
    }
  ...
  if (_hasPTCS) {
    ...
    ptcsDesc.tessellationDescriptor.numVertsPerPatchIn = std::to_string(
        _geometricShader->GetPrimitiveIndexSize());
    ptcsDesc.tessellationDescriptor.numVertsPerPatchOut = std::to_string(
    ...
    ptcsDesc.tessellationDescriptor.patchType =
        (_geometricShader->IsPrimTypeTriangles() ||
         _geometricShader->GetPrimitiveType() ==
            ...
            HgiShaderFunctionTessellationDesc::PatchType::Triangles :
            HgiShaderFunctionTessellationDesc::PatchType::Quads;
    if (_geometricShader->GetHgiPrimitiveType() == HgiPrimitiveTypePointList) {
      ptcsDesc.tessellationDescriptor.patchType =
          HgiShaderFunctionTessellationDesc::PatchType::Isolines;
    }
    ...
    resourceGen._GenerateHgiResources(registry->GetHgi(),
                                      ...
                                      _resMaterial,
                                      _GetMetaData());
    resourceGen._GenerateHgiTextureResources(
        &ptcsDesc, HdShaderTokens->postTessControlShader, _resTextures, _GetMetaData());
    ...
    HgiShaderFunctionAddStageInput(
        &ptcsDesc, "hd_BaseInstance", "uint", HgiShaderKeywordTokens->hdBaseInstance);
    HgiShaderFunctionAddStageInput(
        &ptcsDesc, "patch_id", "uint", HgiShaderKeywordTokens->hdPatchID);
    ...
    HgiShaderFunctionAddStageInput(
        &ptcsDesc, "gl_TessCoord", tessCoordType, HgiShaderKeywordTokens->hdPositionInPatch);

    HgiShaderFunctionAddStageInput(
        &ptcsDesc, "hd_InstanceID", "uint", HgiShaderKeywordTokens->hdInstanceID);

    HgiShaderFunctionAddStageOutput(&ptcsDesc, "gl_Position", "vec4", "position");
    ...
    HgiShaderFunctionAddStageOutput(&ptcsDesc, "gl_PointSize", "float", pointRole);
  ...
  if (_hasPTVS) {
    ...
    ptvsDesc.tessellationDescriptor.numVertsPerPatchIn = std::to_string(
        _geometricShader->GetPrimitiveIndexSize());
    ptvsDesc.tessellationDescriptor.numVertsPerPatchOut = std::to_string(
        _geometricShader->GetNumPatchEvalVerts());
    ...
    resourceGen._GenerateHgiResources(registry->GetHgi(),
                                      ...
                                      _GetMetaData());
    resourceGen._GenerateHgiTextureResources(
        &ptvsDesc, HdShaderTokens->postTessVertexShader, _resTextures, _GetMetaData());
    ...
    HgiShaderFunctionAddStageInput(
        &ptvsDesc, "hd_BaseInstance", "uint", HgiShaderKeywordTokens->hdBaseInstance);
    HgiShaderFunctionAddStageInput(
        &ptvsDesc, "patch_id", "uint", HgiShaderKeywordTokens->hdPatchID);
    ...
    HgiShaderFunctionAddStageInput(
        &ptvsDesc, "gl_TessCoord", tessCoordType, HgiShaderKeywordTokens->hdPositionInPatch);

    HgiShaderFunctionAddStageInput(
        &ptvsDesc, "hd_InstanceID", "uint", HgiShaderKeywordTokens->hdInstanceID);

    HgiShaderFunctionAddStageOutput(&ptvsDesc, "gl_Position", "vec4", "position");
    ...
    HgiShaderFunctionAddStageOutput(&ptvsDesc, "gl_PointSize", "float", pointRole);

    if (_hasClipPlanes) {
      HgiShaderFunctionAddStageOutput(&ptvsDesc,
                                      "gl_ClipDistance",
                                      "float",
                                      "clip_distance",
                                      /*arraySize*/ "HD_NUM_clipPlanes");
  ...
  if (_hasGS) {
    ...
    resourceGen._AdvanceShaderStage();
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &gsDesc, HdShaderTokens->geometryShader, _resCommon, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &gsDesc, HdShaderTokens->geometryShader, _resGS, _GetMetaData());

    // material in GS
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &gsDesc, HdShaderTokens->geometryShader, _resMaterial, _GetMetaData());
    resourceGen._GenerateHgiTextureResources(
        &gsDesc, HdShaderTokens->geometryShader, _resTextures, _GetMetaData());
    ...
    if (_hasClipPlanes) {
      HgiShaderFunctionAddStageOutput(
          &gsDesc, "gl_ClipDistance", "float", "clip_distance", /*arraySize*/ "HD_NUM_clipPlanes");
    }
  ...
  if (_hasFS) {
    ...
    resourceGen._AdvanceShaderStage();
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &fsDesc, HdShaderTokens->fragmentShader, _resCommon, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &fsDesc, HdShaderTokens->fragmentShader, _resFS, _GetMetaData());

    // material in FS
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &fsDesc, HdShaderTokens->fragmentShader, _resMaterial, _GetMetaData());
    resourceGen._GenerateHgiTextureResources(
        &fsDesc, HdShaderTokens->fragmentShader, _resTextures, _GetMetaData());
    ...
    // builtins
    HgiShaderFunctionAddStageInput(
        &fsDesc, "gl_PrimitiveID", "uint", HgiShaderKeywordTokens->hdPrimitiveID);
    HgiShaderFunctionAddStageInput(
        &fsDesc, "gl_FrontFacing", "bool", HgiShaderKeywordTokens->hdFrontFacing);
    HgiShaderFunctionAddStageInput(
        &fsDesc, "gl_FragCoord", "vec4", HgiShaderKeywordTokens->hdPosition);
    const bool builtinBarycentricsEnabled = registry->GetHgi()->GetCapabilities()->IsSet(
        HgiDeviceCapabilitiesBitsBuiltinBarycentrics);
    if (builtinBarycentricsEnabled) {
      HgiShaderFunctionAddStageInput(
          &fsDesc, "hd_BaryCoordNoPersp", "vec3", HgiShaderKeywordTokens->hdBaryCoordNoPersp);
    }
  ...
  if (_hasCS) {
    ...
    resourceGen._AdvanceShaderStage();
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &csDesc, HdShaderTokens->computeShader, _resAttrib, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &csDesc, HdShaderTokens->computeShader, _resCommon, _GetMetaData());
    resourceGen._GenerateHgiResources(
        registry->GetHgi(), &csDesc, HdShaderTokens->computeShader, _resCS, _GetMetaData());
                             ...
                             int arraySize)
{
  ...
  if (arraySize > 0) {
    if (!TF_VERIFY(bindingType == HdStBinding::UNIFORM_ARRAY ||
                   ...
                   bindingType == HdStBinding::UBO || bindingType == HdStBinding::SSBO ||
                                ...
                                bool concatenateNames)
{
  ...
  TfToken accessorName = concatenateNames ?
                             TfToken(structName.GetString() + "_" + name.GetString()) :
                             name;
  if (index) {
    if (arraySize > 1) {
      ...
      str << _GetUnpackedType(type, false) << " HdGet_" << accessorName << "(int localIndex) {\n"
          << "  int index = " << index << ";\n"
          << "  return " << _GetPackedTypeAccessor(type, false) << "(" << structName << "[index]."
          << name << ");\n}\n";
    }
    ...
    if (arraySize > 1) {
      ...
      str << _GetUnpackedType(type, false) << " HdGet_" << accessorName
          << "(int localIndex) { return " << _GetPackedTypeAccessor(type, false) << "("
          << structName << "." << name << ");}\n";
    }
  }
  // GLSL spec doesn't allow default parameter. use function overload instead.
  // default to localIndex=0
  if (arraySize > 1) {
    str << _GetUnpackedType(type, false) << " HdGet_" << accessorName << "(int arrayIndex)"
...
{
  return (type == _tokens->_float || type == _tokens->_int || type == _tokens->_uint);
}
                                 ...
                                 const char *index)
{
  if (index) {
    ...
    if (binding.GetType() == HdStBinding::UNIFORM || binding.GetType() == HdStBinding::VERTEX_ATTR)
    {
      str << _GetUnpackedType(type, false) << " HdGet_" << name << "(int localIndex) { return ";
                                ...
                                const char *index)
{
  if (index) {
    ...
    if (binding.GetType() == HdStBinding::SSBO) {
      str << "  int index = " << index << ";\n";
      str << "  " << _GetPackedType(_ConvertBoolType(type), false)
          << " packedValue = " << _GetPackedTypeMutator(_ConvertBoolType(type), false)
          << "(value);\n";
      ...
      str << name << "[localIndex] = " << _GetPackedTypeMutator(_ConvertBoolType(type), true)
          << "(value);\n";
    }
    else {
...
static void _EmitScalarAccessor(std::stringstream &str, TfToken const &name, TfToken const &type)
{
  ...
  if (_GetNumComponents(type) <= 4) {
    str << _GetFlatType(type) << " HdGetScalar_" << name << "(int localIndex)"
        << " { return HdGet_" << name << "(localIndex)" << _GetFlatTypeSwizzleString(type)
        << "; }\n";
    str << _GetFlatType(type) << " HdGetScalar_" << name << "()"
        << " { return HdGet_" << name << "(0)" << _GetFlatTypeSwizzleString(type) << "; }\n";
  }
}
    ...
    bool const isShadowSampler = false)
{
  ...
  std::string const samplerType = isShadowSampler ? "sampler" + std::to_string(dim) + "DShadow" :
                                                    "sampler" + std::to_string(dim) + "D";

  // Forward declare texture scale and bias
  if (hasTextureScaleAndBias) {
    accessors << "#ifdef HD_HAS_" << name << "_" << HdStTokens->storm << "_" << HdStTokens->scale
              << "\n"
              << "FORWARD_DECL(vec4 HdGet_" << name << "_" << HdStTokens->storm << "_"
              << HdStTokens->scale << "());\n"
              << "#endif\n"
  ...
  if (!isBindless) {
    ...
    if (bindlessTextureEnabled) {
      ...
      if (isArray) {
        accessors << "#define HdGetSampler_" << name << "(index) "
  ...
  if (hasTextureTransform) {
    ...
    // to initialize it.
    const std::string eyeToSamplingTransform = "eyeTo" + name.GetString() + "SamplingTransform";
  ...
  if (!isBindless) {
    if (isArray) {
      accessors << _GetUnpackedType(dataType, false) << " HdTextureLod_" << name
                << "(int index, vec" << coordDim << " coord, float lod) {\n"
                << "  return " << _GetPackedTypeAccessor(dataType, false) << "(HgiTextureLod_"
                << name << "(index, coord, lod)" << swizzle << ");\n"
                << "}\n";
    }
    else {
      accessors << _GetUnpackedType(dataType, false) << " HdTextureLod_" << name << "(vec"
                << coordDim << " coord, float lod) {\n"
                << "  return " << _GetPackedTypeAccessor(dataType, false) << "(HgiTextureLod_"
                << name << "(coord, lod)" << swizzle << ");\n"
                << "}\n";
    }
  }
  else {
  ...
  if (isArray) {
    accessors << _GetUnpackedType(dataType, false) << " HdGet_" << name << "(int index, vec"
              << coordDim << " coord) {\n"
              << "  int shaderCoord = GetDrawingCoord().shaderCoord; \n";
  }
  else {
    accessors << _GetUnpackedType(dataType, false) << " HdGet_" << name << "(vec" << coordDim
              << " coord) {\n"
              << "  int shaderCoord = GetDrawingCoord().shaderCoord; \n";
  }
  ...
  if (hasTextureTransform) {
    ...
    accessors << "   vec4 c = " << eyeToSamplingTransform << " * vec4(coord, 1);\n"
              << "   vec3 sampleCoord = c.xyz / c.w;\n";
  }
  else {
    accessors << "  vec" << coordDim << " sampleCoord = coord;\n";
  }

  if (hasTextureScaleAndBias) {
    if (!isBindless) {
      accessors << "  " << _GetUnpackedType(dataType, false)
                << " result = " << _GetPackedTypeAccessor(dataType, false) << "((HgiGet_" << name;
      if (isArray) {
  ...
  if (acc.processTextureFallbackValue) {
    ...
    //
    accessors << "  if (bool(shaderData[shaderCoord]." << name
  ...
  if (!inPrimvars.empty()) {
    accessors << "#if defined(HD_HAS_" << inPrimvars[0] << ")\n"
              << "FORWARD_DECL(vec" << dim << " HdGet_" << inPrimvars[0] << "(int localIndex));\n"
              << "#endif\n";
  }

  // Create accessor for texture coordinates based on texture param name
  // vec2 HdGetCoord_name(int localIndex)
  accessors << "vec" << coordDim << " HdGetCoord_" << name << "(int localIndex) {\n"
            << "  return \n";
  if (!inPrimvars.empty()) {
    accessors << "#if defined(HD_HAS_" << inPrimvars[0] << ")\n"
    ...
    accessors << "  vec" << coordDim << "(0.0)";
  }
  accessors << ";\n}\n";

  // vec2 HdGetCoord_name()
  accessors << "vec" << coordDim << " HdGetCoord_" << name << "() {"
...
static void _EmitFVarAccessor(bool hasGS,
                              std::stringstream &str,
                              TfToken const &name,
                              TfToken const &type,
                              HdStBinding const &binding,
                              ...
                              int fvarChannel)
{
  ...
  if ((fvarPatchType == HdSt_GeometricShader::FvarPatchType::PATCH_COARSE_QUADS) ||
      (fvarPatchType == HdSt_GeometricShader::FvarPatchType::PATCH_COARSE_TRIANGLES))
  {
  ...
  if (fvarPatchType == HdSt_GeometricShader::FvarPatchType::PATCH_BSPLINE) {
    str << "  int patchType = OSD_PATCH_DESCRIPTOR_REGULAR;\n";
  }
  else if (fvarPatchType == HdSt_GeometricShader::FvarPatchType::PATCH_BOXSPLINETRIANGLE) {
...
static void _GetDrawingCoord(std::stringstream &ss,
                             ...
                             char const *inArraySize)
{
  ...
  for (std::string const &param : drawingCoordParams) {
    ss << "  dc." << param << " = " << inputPrefix << param << inArraySize << ";\n";
  }
  for (int i = 0; i < instanceIndexWidth; ++i) {
    ss << "  dc.instanceIndex[" << std::to_string(i) << "]"
       << " = " << inputPrefix << "instanceIndexI" << std::to_string(i) << inArraySize << ";\n";
  }
  for (int i = 0; i < instanceIndexWidth - 1; ++i) {
    ss << "  dc.instanceCoords[" << std::to_string(i) << "]"
       << " = " << inputPrefix << "instanceCoordsI" << std::to_string(i) << inArraySize << ";\n";
  }
...
static void _ProcessDrawingCoord(std::stringstream &ss,
                                 std::vector<std::string> const &drawingCoordParams,
                                 int const instanceIndexWidth,
                                 ...
                                 char const *outArraySize)
{
  ...
  for (std::string const &param : drawingCoordParams) {
    ss << "  " << outputPrefix << param << outArraySize << " = " << "dc." << param << ";\n";
  }
  for (int i = 0; i < instanceIndexWidth; ++i) {
...
void HdSt_CodeGen::_GenerateDrawingCoord(bool const shaderDrawParametersEnabled,
                                         ...
                                         bool const requiresPrimitiveIdEmulation)
{
  ...
  static const std::vector<std::string> drawingCoordParams{"modelCoord",
                                                           "constantCoord",
                                                           "elementCoord",
                                                           "primitiveCoord",
                                                           "fvarCoord",
  ...
  if (_hasCS) {
    ...
    _genCS << "struct hd_DrawIndex {\n"
           ...
           << "  const int drawIndexOffset = " << _metaData->drawingCoordBufferBinding.offset
           << ";\n"

           << "  const int drawIndexStride = " << _metaData->drawingCoordBufferBinding.stride
           << ";\n"

           << "  const int base = "
           << "hd_drawIndex.drawId * drawIndexStride + drawIndexOffset;\n"
           << "  return int(" << _metaData->drawingCoordBufferBinding.bufferName
           << "[base + offset]);\n"
           << "}\n";
  }
  ...
  if (_metaData->instanceIndexArrayBinding.binding.IsValid()) {
    ...
    HdSt_ResourceBinder::MetaData::BindingDeclaration const &bindingDecl =
        _metaData->culledInstanceIndexArrayBinding;
    ...
    if (_geometricShader->IsFrustumCullingPass()) {
      ...
      _EmitAccessor(_genPTCS,
                    _metaData->culledInstanceIndexArrayBinding.name,
                    _metaData->culledInstanceIndexArrayBinding.dataType,
                    _metaData->culledInstanceIndexArrayBinding.binding,
                    "GetInstanceIndexCoord()+localIndex + 1");
      _EmitAccessor(_genPTVS,
                    _metaData->culledInstanceIndexArrayBinding.name,
                    _metaData->culledInstanceIndexArrayBinding.dataType,
                    _metaData->culledInstanceIndexArrayBinding.binding,
                    "GetInstanceIndexCoord()+localIndex + 1");
  ...
  _ProcessDrawingCoord(_procVS, drawingCoordParams, instanceIndexWidth, "vs_dc_", "");
  _ProcessDrawingCoord(_procPTVSOut, drawingCoordParams, instanceIndexWidth, "vs_dc_", "");

  // TCS from VS
  if (_hasTCS) {
    _GetDrawingCoord(_genTCS, drawingCoordParams, instanceIndexWidth, "vs_dc_", "[0]");
    _ProcessDrawingCoord(
        _procTCS, drawingCoordParams, instanceIndexWidth, "tcs_dc_", "[gl_InvocationID]");
  }

  // TES from TCS
  if (_hasTES) {
    _GetDrawingCoord(_genTES, drawingCoordParams, instanceIndexWidth, "tcs_dc_", "[0]");
    _ProcessDrawingCoord(_procTES, drawingCoordParams, instanceIndexWidth, "tes_dc_", "");
  }

  // GS
  if (_hasGS && _hasTES) {
    // from TES
    _GetDrawingCoord(_genGS, drawingCoordParams, instanceIndexWidth, "tes_dc_", "[0]");
  }
  else if (_hasGS) {
    // from VS
    _GetDrawingCoord(_genGS, drawingCoordParams, instanceIndexWidth, "vs_dc_", "[0]");
  }
  _ProcessDrawingCoord(_procGS, drawingCoordParams, instanceIndexWidth, "gs_dc_", "");

  // FS
  if (_hasGS) {
    // from GS
    _GetDrawingCoord(_genFS, drawingCoordParams, instanceIndexWidth, "gs_dc_", "");
  }
  else if (_hasTES) {
    // from TES
    _GetDrawingCoord(_genFS, drawingCoordParams, instanceIndexWidth, "tes_dc_", "");
  }
  else {
    // from VS/PTVS
    _GetDrawingCoord(_genFS, drawingCoordParams, instanceIndexWidth, "vs_dc_", "");
  }
}

void HdSt_CodeGen::_GenerateConstantPrimvar()
{
  /*
  ...
  */
  ...
  TF_FOR_ALL(it, _metaData->constantData)
  {
    ...
    TF_FOR_ALL(dbIt, it->second.entries)
    {
      ...
      _genDecl << "  " << _GetPackedType(dbIt->dataType, false) << " " << dbIt->name;
      if (dbIt->arraySize > 1) {
        _genDecl << "[" << dbIt->arraySize << "]";
      }
...
void HdSt_CodeGen::_GenerateInstancePrimvar()
{
  /*
...
void HdSt_CodeGen::_GenerateElementPrimvar()
{
  ...
  */
    ...
    if (_geometricShader->IsPrimTypeCompute()) {
      ...
      switch (_geometricShader->GetPrimitiveType()) {
        ...
        case HdSt_GeometricShader::PrimitiveType::PRIM_MESH_COARSE_TRIQUADS: {
          ...
          accessors << "#if defined(HD_HAS_coarseFaceIndex)\n"
        ...
        default: {
          TF_CODING_ERROR(
              "HdSt_GeometricShader::PrimitiveType %d is "
  ...
  _genDecl << "FORWARD_DECL(int GetElementID());\n"
  ...
  if (_metaData->coarseFaceIndexBinding.binding.IsValid()) {
    _genDefines << "#define HD_HAS_" << _metaData->coarseFaceIndexBinding.name << " 1\n";
    ...
    _EmitAccessor(accessors,
                  _metaData->coarseFaceIndexBinding.name,
                  _metaData->coarseFaceIndexBinding.dataType,
                  binding,
  ...
  _genDecl << "FORWARD_DECL(int GetPrimitiveEdgeId());\n"
...
void HdSt_CodeGen::_GenerateVertexAndFaceVaryingPrimvar()
{
  if (_geometricShader->IsFrustumCullingPass()) {
    return;
  }
  ...
  */
  ...
  std::stringstream accessorsVS, accessorsTCS, accessorsTES, accessorsPTCS, accessorsPTVS,
  ...
  TF_FOR_ALL(it, _metaData->vertexData)
  {
    ...
    _EmitStageAccessor(accessorsPTCS, name, name.GetString() + "[localIndex]", dataType);

    // Access PTVS vertex primvar from input attributes.
    _EmitStageAccessor(accessorsPTVS, name, name.GetString() + "[localIndex]", dataType);
    ...
    _procPTVSOut << "  outPrimvars." << name << " = InterpolatePrimvar("
                 << "HdGet_" << name << "(i0), "
                 << "HdGet_" << name << "(i1), "
                 << "HdGet_" << name << "(i2), "
  ...
  */
  ...
  HdSt_ResourceBinder::MetaData::BindingDeclaration const &indexBufferBinding =
      _metaData->indexBufferBinding;
  ...
  TF_FOR_ALL(it, _metaData->varyingData)
  {
    ...
    // interstage plumbing
    _procVS << "  outPrimvars." << name << " = " << "HdGet_" << name << "();\n";
    _procTCS << "  outPrimvars[gl_InvocationID]." << name << " = inPrimvars[gl_InvocationID]."
             << name << ";\n";
    _procTES << "  outPrimvars." << name << " = InterpolatePrimvar("
  ...
  */
  ...
  TF_FOR_ALL(it, _metaData->fvarData)
  {
    ...
    if (_hasGS) {
      ...
      interstagePrimvarFVar.emplace_back(_GetPackedType(dataType, false), name);
  ...
  if (!interstagePrimvar.empty()) {
    // VS out
    _AddInterstageBlockElement(&_resVS,
                               HioGlslfxResourceLayout::InOut::STAGE_OUT,
                               _tokens->PrimvarData,
                               _tokens->outPrimvars,
                               interstagePrimvar);
    ...
    // TCS in/out
    _AddInterstageBlockElement(&_resTCS,
                               HioGlslfxResourceLayout::InOut::STAGE_IN,
                               _tokens->PrimvarData,
                               _tokens->inPrimvars,
                               interstagePrimvar,
    ...
    // TES in/out
    _AddInterstageBlockElement(&_resTES,
                               HioGlslfxResourceLayout::InOut::STAGE_IN,
                               _tokens->PrimvarData,
                               _tokens->inPrimvars,
                               interstagePrimvar,
    ...
    // GS in
    _AddInterstageBlockElement(&_resGS,
                               HioGlslfxResourceLayout::InOut::STAGE_IN,
                               _tokens->PrimvarData,
                               _tokens->inPrimvars,
                               interstagePrimvar,
  ...
  if (!interstagePrimvar.empty() || !interstagePrimvarFVar.empty()) {
    ...
    interstagePrimvar.insert(
        interstagePrimvar.end(), interstagePrimvarFVar.begin(), interstagePrimvarFVar.end());

    // PTVS out
    _AddInterstageBlockElement(&_resPTVS,
                               HioGlslfxResourceLayout::InOut::STAGE_OUT,
                               _tokens->PrimvarData,
                               _tokens->outPrimvars,
                               interstagePrimvar);
    ...
    // FS in
    _AddInterstageBlockElement(&_resFS,
                               HioGlslfxResourceLayout::InOut::STAGE_IN,
                               _tokens->PrimvarData,
                               _tokens->inPrimvars,
                               interstagePrimvar);
...
void HdSt_CodeGen::_GenerateShaderParameters(bool bindlessTextureEnabled)
{
  /*
    ------------- Declarations -------------
  ...
  */
  ...
  TF_FOR_ALL(it, _metaData->shaderParameterBinding)
  {
    ...
    std::string fallbackSwizzle = _GetFallbackScalarSwizzleString(it->second.dataType,
                                                                  it->second.name);
    ...
    if (bindingType == HdStBinding::FALLBACK) {
      ...
      // float HdGetScalar_name()
      _EmitScalarAccessor(accessors, it->second.name, it->second.dataType);
    }
      ...
      bool const isShadowTexture = (it->second.name == HdStTokens->shadowCompareTextures);
      ...
      _AddTextureElement(&_resTextures, it->second.name, 2, binding.GetTextureUnit());
      ...
      _AddArrayOfTextureElement(&_resTextures,
                                ...
                                isShadowTexture ? TextureType::SHADOW_TEXTURE :
                                                  TextureType::TEXTURE,
      ...
      _AddTextureElement(&_resTextures, it->second.name, 3, binding.GetTextureUnit());
      ...
      accessors << "#ifdef HD_HAS_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->scale << "\n"
                << "vec4 HdGet_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->scale << "();\n"
                << "#endif\n"
                << "#ifdef HD_HAS_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->bias << "\n"
                << "vec4 HdGet_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->bias << "();\n"
                << "#endif\n";
      ...
      accessors << it->second.dataType << " HdGet_" << it->second.name << "(vec2 coord)" << " {\n"
                << "  int shaderCoord = GetDrawingCoord().shaderCoord;\n"
                << "  uvec2 handle = shaderData[shaderCoord]." << it->second.name
                ...
                << "  if (c.z >= -0.5) {\n"
                << "    uvec2 handleTexels = shaderData[shaderCoord]." << it->second.name << ";\n"
                << "    ret = texture(sampler2DArray(handleTexels), c);\n"
                << "  }\n";

      if (it->second.processTextureFallbackValue) {
        accessors << "  if (!bool(shaderData[shaderCoord]." << it->second.name
                  << HdSt_ResourceBindingSuffixTokens->valid << ")) {\n"
                  << "    return (" << _GetPackedTypeAccessor(it->second.dataType, false)
                  << "(shaderData[shaderCoord]." << it->second.name
                  << HdSt_ResourceBindingSuffixTokens->fallback << fallbackSwizzle << ")\n"
                  << "#ifdef HD_HAS_" << it->second.name << "_" << HdStTokens->storm << "_"
                  << HdStTokens->scale << "\n"
                  << "    * HdGet_" << it->second.name << "_" << HdStTokens->storm << "_"
                  << HdStTokens->scale << "()" << swizzle << "\n"
                  << "#endif\n"
                  << "#ifdef HD_HAS_" << it->second.name << "_" << HdStTokens->storm << "_"
                  << HdStTokens->bias << "\n"
      ...
      accessors << "#ifdef HD_HAS_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->scale << "\n"
                << "FORWARD_DECL(vec4 HdGet_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->scale << "());\n"
                << "#endif\n"
                << "#ifdef HD_HAS_" << it->second.name << "_" << HdStTokens->storm << "_"
                << HdStTokens->bias << "\n"
      ...
      accessors << it->second.dataType << " HdGet_" << it->second.name
                << "(vec2 coord) {\n vec3 c = hd_sample_udim(coord);\n"
                << "  c.z = HgiTexelFetch_" << it->second.name
                << HdSt_ResourceBindingSuffixTokens->layout << "(int(c.z)).x - 1;\n"
                << "  vec4 ret = vec4(0, 0, 0, 0);\n"
                << "  if (c.z >= -0.5) { ret = HgiGet_" << it->second.name << "(c); }\n";
      ...
      _AddTextureElement(&_resTextures, it->second.name, 1, binding.GetTextureUnit());
    }
      ...
      if (_GetNumComponents(it->second.dataType) <= 4) {
        accessors << _GetFlatType(it->second.dataType) << " HdGetScalar_" << it->second.name
                  << "()"
      ...
      accessors << "FORWARD_DECL(float HdGet_" << it->second.name << "_" << HdStTokens->rotation
                << "());\n"
                << "FORWARD_DECL(vec2 HdGet_" << it->second.name << "_" << HdStTokens->scale
                << "());\n"
                << "FORWARD_DECL(vec2 HdGet_" << it->second.name << "_" << HdStTokens->translation
                << "());\n";

      // vec2 HdGet_name(int localIndex)
      accessors << _GetUnpackedType(it->second.dataType, false) << " HdGet_" << it->second.name
                << "(int localIndex) {\n"
                << "  float angleRad = HdGet_" << it->second.name << "_" << HdStTokens->rotation
                << "()"
                << " * 3.1415926f / 180.f;\n"
                << "  mat2 rotMat = mat2(cos(angleRad), sin(angleRad), "
                << "-sin(angleRad), cos(angleRad)); \n";
      ...
      if (!it->second.inPrimvars.empty()) {
        accessors << "#if defined(HD_HAS_" << it->second.inPrimvars[0] << ")\n"
                  << "  return vec2(HdGet_" << it->second.name << "_" << HdStTokens->translation
                  << "() + rotMat * (HdGet_" << it->second.name << "_" << HdStTokens->scale
                  << "() * "
  ...
  accessors << "void ProcessSamplingTransforms("
            << "MAT4 instanceModelViewInverse) {\n";
  ...
  TF_FOR_ALL(it, _metaData->shaderParameterBinding)
  {
        ...
        bindingType == HdStBinding::BINDLESS_TEXTURE_FIELD)
    {
      ...
      const std::string eyeToSamplingTransform = "eyeTo" + it->second.name.GetString() +
                                                 "SamplingTransform";

      accessors << "    Process_" << eyeToSamplingTransform << "(instanceModelViewInverse);\n";
    }
  }
  ...
  TF_FOR_ALL(it, _metaData->shaderParameterBinding)
  {
    ...
    if (bindingType == HdStBinding::FIELD_REDIRECT) {
      ...
      accessors << _GetUnpackedType(it->second.dataType, false) << " HdGet_" << it->second.name
                ...
                << "#if defined(HD_HAS_" << fieldName << HdSt_ResourceBindingSuffixTokens->texture
                << ")\n"
                << "  return HdGet_" << fieldName << HdSt_ResourceBindingSuffixTokens->texture
                << "(coord)" << swizzle << ";\n"
                << "#else\n"
                // Otherwise use default value.
                << "  int shaderCoord = GetDrawingCoord().shaderCoord;\n"
...
void HdSt_CodeGen::_GenerateTopologyVisibilityParameters()
{
...
std::string HdSt_CodeGen::_GetFallbackScalarSwizzleString(TfToken const &returnType,
