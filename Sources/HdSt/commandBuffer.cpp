...
HdStCommandBuffer::HdStCommandBuffer()
    : _visibleSize(0), _visChangeCount(0), _drawBatchesVersion(0)
{
  /*NOTHING*/
}
...
HdSt_DrawBatchSharedPtr _NewDrawBatch(HdStDrawItemInstance *drawItemInstance,
...
TF_DEFINE_ENV_SETTING(HDST_DRAW_BATCH_TEXTURE_AGGREGATION_THRESHOLD,
                      10,
                      "Specifies the draw item instance count threshold "
                      "for aggregating draw items which use distinct textures.");

size_t _GetTextureAggregationThreshold()
{
  static const size_t threshold = TfGetEnvSetting(HDST_DRAW_BATCH_TEXTURE_AGGREGATION_THRESHOLD);
  return threshold;
}
...
struct _SortByTexture {
  bool operator()(HdStDrawItemInstance const *aInstance, HdStDrawItemInstance const *bInstance)
  {
    HdStDrawItem const *a = aInstance->GetDrawItem();
    HdStDrawItem const *b = bInstance->GetDrawItem();

    size_t const textureA = a->GetMaterialNetworkShader()->ComputeTextureSourceHash();
    size_t const textureB = b->GetMaterialNetworkShader()->ComputeTextureSourceHash();
...
struct _BatchMap {
  ...
  HdSt_DrawBatchSharedPtr InsertOrAppend(Key key,
                                         ...
                                         bool allowTextureResourceRebinding)
  {
    ...
    HdSt_DrawBatchSharedPtr newBatch = _NewDrawBatch(
        drawItemInstance, usePipelineDrawBatch, allowTextureResourceRebinding);
 ...
 private:
  void _SetPreviousBatchForKey(Key key, HdSt_DrawBatchSharedPtr const &batch)
  {
    _prevKey = key;
...
void _InsertDrawItemInstance(HdSt_DrawBatchSharedPtrVector *drawBatches,
                             HdStDrawItemInstance *drawItemInstance,
                             _BatchMap *batchMap,
                             bool const usePipelineDrawBatch,
                             bool const allowTextureResourceRebinding)
{
  ...
  size_t key = TfHash::Combine(drawItem->GetGeometricShader()->ComputeHash(),
  ...
  if (!allowTextureResourceRebinding) {
    size_t const textureHash = drawItem->GetMaterialNetworkShader()->ComputeTextureSourceHash();
    key = TfHash::Combine(key, textureHash);
  }

  // Keep track of newly created draw batches.
  if (HdSt_DrawBatchSharedPtr newBatch = batchMap->InsertOrAppend(
          key, drawItemInstance, usePipelineDrawBatch, allowTextureResourceRebinding))
  {
    drawBatches->push_back(newBatch);
  }
}

void _BatchDrawItemInstances(HdSt_DrawBatchSharedPtrVector *drawBatches,
                             std::vector<HdStDrawItemInstance> &instances,
                             bool const usePipelineDrawBatch)
{
  _BatchMap batchMap;
  for (HdStDrawItemInstance &drawItemInstance : instances) {
    _InsertDrawItemInstance(drawBatches,
                            &drawItemInstance,
                            &batchMap,
...
bool _HasTextureResourceBinding(HdStDrawItemInstance const *instance)
{
...
bool _ShouldAttemptToAggregate(std::vector<HdStDrawItemInstance const *> const &instances)
{
  size_t const threshold = _GetTextureAggregationThreshold();
  return (instances.size() <= threshold) && _HasTextureResourceBinding(instances.front());
}

void _AggregateDrawBatches(HdSt_DrawBatchSharedPtrVector *drawBatches,
                           bool const usePipelineDrawBatch)
{
  HD_TRACE_FUNCTION();
  ...
  for (auto const &batch : *drawBatches) {
    std::vector<HdStDrawItemInstance const *> const &instances = batch->GetDrawItemInstances();
    if (_ShouldAttemptToAggregate(instances)) {
      for (auto const *instance : instances) {
        toAggregate.push_back(const_cast<HdStDrawItemInstance *>(instance));
      }
    }
    else {
  ...
  for (HdStDrawItemInstance *drawItemInstance : toAggregate) {
    _InsertDrawItemInstance(&result,
                            drawItemInstance,
                            &batchMap,
                            usePipelineDrawBatch,
...
bool _IsEnabledFrustumCullCPU(Hgi const *hgi)
{
  ...
  const bool multiDrawIndirectEnabled = capabilities->IsSet(
      HgiDeviceCapabilitiesBitsMultiDrawIndirect);

  const bool gpuFrustumCullingEnabled = HdSt_PipelineDrawBatch::IsEnabled(hgi) ?
...
void HdStCommandBuffer::PrepareDraw(HgiGraphicsCmds *gfxCmds,
                                    ...
                                    HdRenderIndex *renderIndex)
{
  ...
  for (auto const &batch : _drawBatches) {
    batch->EncodeDraw(renderPassState,
                      resourceRegistry,
...
void HdStCommandBuffer::ExecuteDraw(HgiGraphicsCmds *gfxCmds,
                                    HdStRenderPassStateSharedPtr const &renderPassState,
                                    HdStResourceRegistrySharedPtr const &resourceRegistry)
{
  HD_TRACE_FUNCTION();
...
void HdStCommandBuffer::SetDrawItems(HdDrawItemConstPtrVectorSharedPtr const &drawItems,
                                     unsigned currentDrawBatchesVersion,
                                     Hgi const *hgi)
{
  if (drawItems == _drawItems && currentDrawBatchesVersion == _drawBatchesVersion) {
    return;
  }
...
void HdStCommandBuffer::RebuildDrawBatchesIfNeeded(unsigned currentBatchesVersion, Hgi const *hgi)
{
  HD_TRACE_FUNCTION();
  ...
  bool rebuildAllDrawBatches = TfDebug::IsEnabled(HDST_FORCE_DRAW_BATCH_REBUILD);
  ...
  if (ARCH_LIKELY(!rebuildAllDrawBatches)) {
    ...
    for (auto const &batch : _drawBatches) {
      const HdSt_DrawBatch::ValidationResult result = batch->Validate(deepValidation);
    ...
    if (!rebuildAllDrawBatches) {
      ...
      for (size_t i = 0; i < numBatches; i++) {
        if (results[i] == HdSt_DrawBatch::ValidationResult::RebuildBatch) {
...
void HdStCommandBuffer::_RebuildDrawBatches(Hgi const *hgi)
{
  HD_TRACE_FUNCTION();
  ...
  for (size_t i = 0; i < drawItems.size(); i++) {
    ...
    if (!TF_VERIFY(drawItem->GetGeometricShader(), "%s", drawItem->GetRprimID().GetText()) ||
        !TF_VERIFY(drawItem->GetMaterialNetworkShader(), "%s", drawItem->GetRprimID().GetText()))
    {
      continue;
    }
  ...
  _BatchDrawItemInstances(&_drawBatches, _drawItemInstances, usePipelineDrawBatch);

  if (!usePipelineDrawBatch && (_GetTextureAggregationThreshold() > 0)) {
    _AggregateDrawBatches(&_drawBatches, usePipelineDrawBatch);
  }
...
void HdStCommandBuffer::SyncDrawItemVisibility(unsigned visChangeCount)
{
  HD_TRACE_FUNCTION();
...
void HdStCommandBuffer::_FrustumCullCPU(GfMatrix4d const &cullMatrix)
{
  HD_TRACE_FUNCTION();

  const bool mtCullingDisabled = TfDebug::IsEnabled(HDST_DISABLE_MULTITHREADED_CULLING) ||
                                 _drawItems->size() < 10000;

  struct _Worker {
    static void cull(std::vector<HdStDrawItemInstance> *drawItemInstances,
                     ...
                     size_t end)
    {
      for (size_t i = begin; i < end; i++) {
        ...
        bool visible = item->GetVisible() && item->IntersectsViewVolume(cullMatrix);
  ...
  if (!mtCullingDisabled) {
    ...
    _Worker::cull(&_drawItemInstances, cullMatrix, 0, _drawItemInstances.size());
  }
...
void HdStCommandBuffer::SetEnableTinyPrimCulling(bool tinyPrimCulling)
{
  for (auto const &batch : _drawBatches) {
