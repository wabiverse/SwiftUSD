...
HdStExtCompCpuComputationSharedPtr HdStExtCompCpuComputation::CreateComputation(
    HdSceneDelegate *sceneDelegate,
    ...
    HdBufferSourceSharedPtrVector *computationSources)
{
  ...
  for (const TfToken &inputName : computation.GetSceneInputNames()) {
    ...
    HdSt_ExtCompInputSourceSharedPtr inputSource = std::make_shared<HdSt_ExtCompSceneInputSource>(
    ...
    if (sourceComp != nullptr) {
      ...
      if (sourceComp->IsInputAggregation()) {
        ...
        HdSt_ExtCompInputSourceSharedPtr inputSource =
            std::make_shared<HdSt_ExtCompSceneInputSource>(compInput.name, inputValue);
        computationSources->push_back(inputSource);
        inputs.push_back(inputSource);
        continue;
      }

      HdStExtCompCpuComputationSharedPtr sourceComputation = CreateComputation(
  ...
  HdStExtCompCpuComputationSharedPtr result = std::make_shared<HdStExtCompCpuComputation>(
...
TfToken const &HdStExtCompCpuComputation::GetName() const
{
  return _id.GetToken();
}

bool HdStExtCompCpuComputation::Resolve()
{
...
size_t HdStExtCompCpuComputation::GetNumElements() const
{
  return _numElements;
}

size_t HdStExtCompCpuComputation::GetOutputIndex(const TfToken &outputName) const
{
  size_t numOutputs = _outputs.size();
  for (size_t outputNum = 0; outputNum < numOutputs; ++outputNum) {
...
const VtValue &HdStExtCompCpuComputation::GetOutputByIndex(size_t index) const
{
  return _outputValues[index];
}

bool HdStExtCompCpuComputation::_CheckValid() const
{
