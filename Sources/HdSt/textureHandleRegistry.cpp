...
class HdSt_TextureHandleRegistry::_TextureToHandlesMap {
 public:
  ...
  bool GarbageCollect(const std::set<HdStTextureObjectSharedPtr> &textures)
  {
    bool result = false;
...
HdSt_TextureHandleRegistry::HdSt_TextureHandleRegistry(HdStResourceRegistry *registry)
    : _textureTypeToMemoryRequestChanged(false),
      _samplerObjectRegistry(std::make_unique<HdSt_SamplerObjectRegistry>(registry)),
      _textureObjectRegistry(std::make_unique<HdSt_TextureObjectRegistry>(registry)),
...
HdStTextureHandleSharedPtr HdSt_TextureHandleRegistry::AllocateTextureHandle(
    ...
    HdStShaderCodePtr const &shaderCode)
{
  ...
  HdStTextureObjectSharedPtr const textureObject = _textureObjectRegistry->AllocateTextureObject(
      textureId, textureType);

  HdStTextureHandleSharedPtr const result = std::make_shared<HdStTextureHandle>(
      textureObject, samplerParams, memoryRequest, shaderCode, this);
...
void HdSt_TextureHandleRegistry::_ComputeMemoryRequest(HdStTextureObjectSharedPtr const &texture)
{
...
void HdSt_TextureHandleRegistry::_ComputeMemoryRequests(
...
void HdSt_TextureHandleRegistry::_ComputeAllMemoryRequests()
{
  TRACE_FUNCTION();
...
bool HdSt_TextureHandleRegistry::_GarbageCollectHandlesAndComputeTargetMemory()
{
  TRACE_FUNCTION();
...
void HdSt_TextureHandleRegistry::_GarbageCollectAndComputeTargetMemory()
{
  TRACE_FUNCTION();
...
std::set<HdStShaderCodeSharedPtr> HdSt_TextureHandleRegistry::_Commit()
{
  TRACE_FUNCTION();
  ...
  const std::set<HdStTextureObjectSharedPtr> committedTextures = _textureObjectRegistry->Commit();
...

  {
    ...
    for (HdStTextureHandleSharedPtr const &handle : dirtyHandles) {
      ...
      if (HdStShaderCodeSharedPtr const shader = handle->GetShaderCode().lock()) {
        result.insert(shader);
      }
    }

    for (HdStShaderCodePtr const &dirtyShader : _dirtyShaders) {
      if (HdStShaderCodeSharedPtr const shader = dirtyShader.lock()) {
        result.insert(shader);
      }
    }
  }
...
void HdSt_TextureHandleRegistry::SetMemoryRequestForTextureType(const HdStTextureType textureType,
...
size_t HdSt_TextureHandleRegistry::GetNumberOfTextureHandles() const
{
