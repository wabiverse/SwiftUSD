...
HdStRenderPassState::HdStRenderPassState(HdStRenderPassShaderSharedPtr const &renderPassShader)
    : HdRenderPassState(),
...
bool HdStRenderPassState::_UseAlphaMask() const
{
  return (_alphaThreshold > 0.0f);
}

unsigned int HdStRenderPassState::_GetFramebufferHeight() const
{
...
static float _CameraAspectRatio(const HdCamera *const camera)
{
  if (!camera) {
    return 1.0f;
  }
...
static GfRange2f _FlipWindow(const GfRange2f &window, const unsigned int framebufferHeight)
{
...
GfRange2f HdStRenderPassState::_ComputeFlippedFilmbackWindow() const
{
  if (_framing.IsValid()) {
    return _FlipWindow(
        _framing.ComputeFilmbackWindow(_CameraAspectRatio(_camera), GetWindowPolicy()),
...
HdStRenderPassState::AxisAlignedAffineTransform HdStRenderPassState::
    ComputeImageToHorizontallyNormalizedFilmback() const
{
  ...
  const float yScale = _framing.IsValid() ? _SafeDiv(xScale, _framing.pixelAspectRatio) : xScale;

  // We need the midpoint of window to go to (0,0) for 2.
  const GfVec2f midPoint = window.GetMidpoint();

  return GfVec4f(xScale, yScale, -midPoint[0] * xScale, -midPoint[1] * yScale);
}
...
static GfVec4i _ComputeViewport(const GfRect2i &dataWindow, unsigned int framebufferHeight)
{
  if (framebufferHeight > 0) {
    return GfVec4i(dataWindow.GetMinX(),
...
GfVec4i HdStRenderPassState::ComputeViewport() const
{
...
void HdStRenderPassState::Prepare(HdResourceRegistrySharedPtr const &resourceRegistry)
{
  HD_TRACE_FUNCTION();
  ...
  const size_t maxClipPlanes =
      (size_t)hdStResourceRegistry->GetHgi()->GetCapabilities()->GetMaxClipDistances();
      ...
      _alphaThresholdCurrent != _alphaThreshold)
  {
    ...
    bufferSpecs.emplace_back(HdShaderTokens->imageToWorldMatrix, HdTupleType{matType, 1});
    bufferSpecs.emplace_back(HdShaderTokens->imageToHorizontallyNormalizedFilmback,
    ...
    bufferSpecs.emplace_back(HdShaderTokens->stepSize, HdTupleType{HdTypeFloat, 1});
    bufferSpecs.emplace_back(HdShaderTokens->stepSizeLighting, HdTupleType{HdTypeFloat, 1});
    ...
    _clipPlanesBufferSize = std::max<size_t>(clipPlanes.size(), s_minNumClipPlanes);
    bufferSpecs.emplace_back(HdShaderTokens->clipPlanes,
                             HdTupleType{HdTypeFloatVec4, _clipPlanesBufferSize});

    bufferSpecs.emplace_back(HdShaderTokens->numClipPlanes, HdTupleType{HdTypeUInt32, 1});

    // allocate interleaved buffer
    _renderPassStateBar = hdStResourceRegistry->AllocateUniformBufferArrayRange(
        HdTokens->drawingShader, bufferSpecs, HdBufferArrayUsageHintBitsUniform);
    ...
    _renderPassShader->AddBufferBinding(HdStBindingRequest(
        HdStBinding::UBO, _tokens->renderPassState, _renderPassStateBar_, /*interleaved=*/true));
  }
  ...
  HgiCapabilities const *capabilities = hdStResourceRegistry->GetHgi()->GetCapabilities();
  ...
  HdBufferSourceSharedPtrVector sources = {
      std::make_shared<HdVtBufferSource>(
          HdShaderTokens->worldToViewMatrix, worldToViewMatrix, doublesSupported),
      std::make_shared<HdVtBufferSource>(HdShaderTokens->worldToViewInverseMatrix,
                                         worldToViewMatrix.GetInverse(),
                                         doublesSupported),
      std::make_shared<HdVtBufferSource>(
          HdShaderTokens->projectionMatrix, projMatrix, doublesSupported),
      std::make_shared<HdVtBufferSource>(
          HdShaderTokens->imageToWorldMatrix, GetImageToWorldMatrix(), doublesSupported),
      std::make_shared<HdVtBufferSource>(HdShaderTokens->imageToHorizontallyNormalizedFilmback,
      ...
      std::make_shared<HdVtBufferSource>(HdShaderTokens->stepSize, VtValue(_stepSize)),
      std::make_shared<HdVtBufferSource>(HdShaderTokens->stepSizeLighting,
  ...
  sources.push_back(std::make_shared<HdVtBufferSource>(HdShaderTokens->numClipPlanes,
...
void HdStRenderPassState::SetResolveAovMultiSample(bool state)
{
  _resolveMultiSampleAov = state;
}

bool HdStRenderPassState::GetResolveAovMultiSample() const
{
  return _resolveMultiSampleAov;
}

void HdStRenderPassState::SetLightingShader(HdStLightingShaderSharedPtr const &lightingShader)
{
  if (lightingShader) {
...
void HdStRenderPassState::SetRenderPassShader(
...
void HdStRenderPassState::SetUseSceneMaterials(bool state)
{
  _useSceneMaterials = state;
}

HdStShaderCodeSharedPtrVector HdStRenderPassState::GetShaders() const
{
...
void _SetGLCullState(HgiCullMode const resolvedCullMode)
{
...
void _SetGLPolygonMode(float rsLineWidth, HdSt_GeometricShaderSharedPtr const &geometricShader)
{
...
void _SetGLColorMask(int drawBufferIndex, HdRenderPassState::ColorMask const &mask)
{
...
HgiColorMask _GetColorMask(HdRenderPassState::ColorMask const &mask)
{
  switch (mask) {
    default:
    case HdStRenderPassState::ColorMaskNone:
    ...
    case HdStRenderPassState::ColorMaskRGB:
      return HgiColorMaskRed | HgiColorMaskGreen | HgiColorMaskBlue;

    case HdStRenderPassState::ColorMaskRGBA:
      return HgiColorMaskRed | HgiColorMaskGreen | HgiColorMaskBlue | HgiColorMaskAlpha;
  }
}

}  // anonymous namespace

void HdStRenderPassState::ApplyStateFromGeometricShader(
    HdSt_ResourceBinder const &binder, HdSt_GeometricShaderSharedPtr const &geometricShader)
{
  _SetGLCullState(geometricShader->ResolveCullMode(_cullStyle));
  _SetGLPolygonMode(_lineWidth, geometricShader);
}

void HdStRenderPassState::ApplyStateFromCamera()
{
  // notify view-transform to the lighting shader to update its uniform block
...
void HdStRenderPassState::Bind(HgiCapabilities const &hgiCapabilities)
{
  GLF_GROUP_FUNCTION();
  ...
  if (GetStencilEnabled()) {
    ...
    glStencilFunc(HdStGLConversions::GetGlStencilFunc(_stencilFunc), _stencilRef, _stencilMask);
  ...
  if (_blendEnabled) {
    glEnable(GL_BLEND);
    glBlendEquationSeparate(HdStGLConversions::GetGlBlendOp(_blendColorOp),
                            HdStGLConversions::GetGlBlendOp(_blendAlphaOp));
    glBlendFuncSeparate(HdStGLConversions::GetGlBlendFactor(_blendColorSrcFactor),
...
void HdStRenderPassState::Unbind(HgiCapabilities const &hgiCapabilities)
{
  GLF_GROUP_FUNCTION();
...
void HdStRenderPassState::SetCameraFramingState(GfMatrix4d const &worldToViewMatrix,
...
size_t HdStRenderPassState::GetShaderHash() const
{
  size_t hash = 0;
  if (_lightingShader) {
  ...
  // used.
  return TfHash::Combine(hash, _clipPlanesBufferSize, _UseAlphaMask());
}
...
void HdStRenderPassState::_InitAttachmentDesc(HgiAttachmentDesc &attachmentDesc,
                                              HdRenderPassAovBinding const &binding,
                                              HdRenderBuffer const *renderBuffer,
                                              int aovIndex) const
{
  if (TF_VERIFY(renderBuffer, "Invalid render buffer")) {
    attachmentDesc.format = HdStHgiConversions::GetHgiFormat(renderBuffer->GetFormat());
  }
  ...
  if (HdAovHasDepthSemantic(binding.aovName)) {
    ...
    attachmentDesc.usage = HgiTextureUsageBitsStencilTarget | HgiTextureUsageBitsDepthTarget;
  }
  else {
  ...
  HgiAttachmentLoadOp loadOp = binding.clearValue.IsEmpty() ? HgiAttachmentLoadOpLoad :
...
HgiGraphicsCmdsDesc HdStRenderPassState::MakeGraphicsCmdsDesc(
    const HdRenderIndex *const renderIndex) const
{
  ...
  for (size_t aovIndex = 0; aovIndex < aovBindings.size(); ++aovIndex) {
    ...
    if (HdAovHasDepthSemantic(aov.aovName) || HdAovHasDepthStencilSemantic(aov.aovName)) {
...
GfMatrix4d HdStRenderPassState::GetWorldToViewMatrix() const
{
  if (_camera) {
...
GfMatrix4d HdStRenderPassState::GetProjectionMatrix() const
{
  if (_camera) {
...
HdRenderPassState::ClipPlanesVector const &HdStRenderPassState::GetClipPlanes() const
{
  if (!_camera) {
    if (_clippingEnabled) {
      return _clipPlanes;
...
void HdStRenderPassState::_InitPrimitiveState(
    HgiGraphicsPipelineDesc *pipeDesc, HdSt_GeometricShaderSharedPtr const &geometricShader) const
{
  pipeDesc->primitiveType = geometricShader->GetHgiPrimitiveType();

  if (pipeDesc->primitiveType == HgiPrimitiveTypePatchList) {
    pipeDesc->tessellationState.primitiveIndexSize = geometricShader->GetPrimitiveIndexSize();

    if (geometricShader->GetUseMetalTessellation()) {
      pipeDesc->tessellationState.patchType = geometricShader->IsPrimTypeTriangles() ?
                                                  HgiTessellationState::PatchType::Triangle :
                                                  HgiTessellationState::PatchType::Quad;
      pipeDesc->tessellationState.tessFactorMode = geometricShader->IsPrimTypePatches() ?
...
void HdStRenderPassState::_InitAttachmentState(HgiGraphicsPipelineDesc *pipeDesc,
                                               bool firstDrawBatch) const
{
  ...
  for (size_t aovIndex = 0; aovIndex < aovBindings.size(); ++aovIndex) {
    ...
    _InitAttachmentDesc(attachment, binding, binding.renderBuffer, aovIndex);
    ...
    if (HdAovHasDepthSemantic(binding.aovName) || HdAovHasDepthStencilSemantic(binding.aovName)) {
  ...
  if (!aovBindings.empty() && GetUseAovMultiSample()) {
    HdStRenderBuffer *firstRenderBuffer = static_cast<HdStRenderBuffer *>(
    ...
    if (firstRenderBuffer->IsMultiSampled()) {
      ...
      if (GetResolveAovMultiSample()) {
        VtValue resolveRes = firstRenderBuffer->GetResource(/*ms*/ false);
        if (TF_VERIFY(resolveRes.IsHolding<HgiTextureHandle>())) {
...
void HdStRenderPassState::_InitDepthStencilState(HgiDepthStencilState *depthState) const
{
  if (GetEnableDepthTest()) {
    depthState->depthTestEnabled = true;
    depthState->depthCompareFn = HdStHgiConversions::GetHgiCompareFunction(_depthFunc);
  ...
  if (GetStencilEnabled()) {
    ...
    depthState->stencilFront.compareFn = HdStHgiConversions::GetHgiCompareFunction(_stencilFunc);
    depthState->stencilFront.referenceValue = _stencilRef;
    depthState->stencilFront.stencilFailOp = HdStHgiConversions::GetHgiStencilOp(_stencilFailOp);
    depthState->stencilFront.depthFailOp = HdStHgiConversions::GetHgiStencilOp(_stencilZFailOp);
    depthState->stencilFront.depthStencilPassOp = HdStHgiConversions::GetHgiStencilOp(
        _stencilZPassOp);
...
void HdStRenderPassState::_InitMultiSampleState(HgiMultiSampleState *multiSampleState) const
{
...
void HdStRenderPassState::_InitRasterizationState(
    ...
    HdSt_GeometricShaderSharedPtr const &geometricShader) const
{
  ...
  rasterizationState->cullMode = geometricShader->ResolveCullMode(_cullStyle);
...
void HdStRenderPassState::InitGraphicsPipelineDesc(
...
uint64_t HdStRenderPassState::GetGraphicsPipelineHash(
    HdSt_GeometricShaderSharedPtr const &geometricShader, bool firstDrawBatch) const
{
  // Hash all of the state that is captured in the pipeline state object.
  uint64_t hash = TfHash::Combine(_depthBiasUseDefault,
                                  _depthBiasEnabled,
  ...
  for (HdRenderPassAovBinding const &binding : GetAovBindings()) {
    HdStRenderBuffer *renderBuffer = static_cast<HdStRenderBuffer *>(binding.renderBuffer);

    const uint32_t msaaCount = renderBuffer->IsMultiSampled() ?
                                   renderBuffer->GetMSAASampleCount() :
                                   1;
    const bool clear = firstDrawBatch ? !binding.clearValue.IsEmpty() : false;

    hash = TfHash::Combine(hash, binding.aovName, renderBuffer->GetFormat(), msaaCount, clear);
  }
