...
HdStRenderBuffer::HdStRenderBuffer(HdStResourceRegistry *const resourceRegistry, SdfPath const &id)
    : HdRenderBuffer(id),
...
void HdStRenderBuffer::Sync(HdSceneDelegate *sceneDelegate,
                            ...
                            HdDirtyBits *dirtyBits)
{
  ...
  if (*dirtyBits & DirtyDescription) {
    VtValue val = sceneDelegate->Get(GetId(), HdStRenderBufferTokens->stormMsaaSampleCount);
...
HdStTextureIdentifier HdStRenderBuffer::GetTextureIdentifier(const bool multiSampled)
{
  ...
  return HdStTextureIdentifier(TfToken(idStr),
...
static std::string _GetDebugName(const HdStDynamicUvTextureObjectSharedPtr &textureObject)
{
  return textureObject->GetTextureIdentifier().GetFilePath().GetString();
}

static void _CreateTexture(HdStDynamicUvTextureObjectSharedPtr const &textureObject,
...
bool HdStRenderBuffer::Allocate(GfVec3i const &dimensions,
                                ...
                                const bool multiSampled)
{
  ...
  if (!_textureObject) {
    ...
    _textureObject = std::dynamic_pointer_cast<HdStDynamicUvTextureObject>(
  ...
  if (multiSampled) {
    if (!_textureMSAAObject) {
      // Allocate texture object if necessary
      _textureMSAAObject = std::dynamic_pointer_cast<HdStDynamicUvTextureObject>(
...
void HdStRenderBuffer::_Deallocate()
{
  _textureObject = nullptr;
  _textureMSAAObject = nullptr;
}

void *HdStRenderBuffer::Map()
{
...
static VtValue _GetResource(const HdStDynamicUvTextureObjectSharedPtr &textureObject)
{
  if (textureObject) {
...
VtValue HdStRenderBuffer::GetResource(const bool multiSampled) const
{
  if (multiSampled) {
...
bool HdStRenderBuffer::IsMultiSampled() const
{
  return bool(_textureMSAAObject);
}

uint32_t HdStRenderBuffer::GetMSAASampleCount() const
{
  return _msaaSampleCount;
}

static const GfVec3i &_GetDimensions(HdStDynamicUvTextureObjectSharedPtr const &textureObject)
{
...
unsigned int HdStRenderBuffer::GetWidth() const
{
  return _GetDimensions(_textureObject)[0];
}

unsigned int HdStRenderBuffer::GetHeight() const
{
  return _GetDimensions(_textureObject)[1];
}

unsigned int HdStRenderBuffer::GetDepth() const
{
