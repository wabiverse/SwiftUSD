...
HdStVolume::HdStVolume(SdfPath const &id) : HdVolume(id) {}
...
HdDirtyBits HdStVolume::GetInitialDirtyBitsMask() const
{
...
void HdStVolume::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits)
{
  ...
  if (!_volumeRepr) {
    ...
    drawingCoord->SetInstancePrimvarBaseIndex(HdStVolume::InstancePrimvar);
...
void HdStVolume::UpdateRenderTag(HdSceneDelegate *delegate, HdRenderParam *renderParam)
{
  HdStUpdateRenderTag(delegate, renderParam, this);
}

void HdStVolume::Sync(HdSceneDelegate *delegate,
                      ...
                      TfToken const &reprToken)
{
  ...
  if (*dirtyBits & HdChangeTracker::DirtyMaterialId) {
    ...
    HdStDrawItem *const drawItem = static_cast<HdStDrawItem *>(_volumeRepr->GetDrawItem(0));
    HdStSetMaterialTag(renderParam, drawItem, HdStMaterialTagTokens->volume);
  }
...
void HdStVolume::Finalize(HdRenderParam *renderParam)
{
                             ...
                             HdDirtyBits *dirtyBits)
{
  ...
  HdStDrawItem *const drawItem = static_cast<HdStDrawItem *>(curRepr->GetDrawItem(0));
...
HdStMaterial::VolumeMaterialData _MakeFallbackVolumeMaterialData()
{
  const HioGlslfx glslfx(HdStPackageFallbackVolumeShader());

  return {glslfx.GetVolumeSource(),
          {HdSt_MaterialParam(HdSt_MaterialParam::ParamTypeFieldRedirect,
...
const HdStMaterial::VolumeMaterialData &_ComputeVolumeMaterialData(
    const HdStMaterial *const material)
{
  // Try to use volume material data from material.
  if (material) {
    const HdStMaterial::VolumeMaterialData &data = material->GetVolumeMaterialData();
  ...
  // ourselves.
  static const HdStMaterial::VolumeMaterialData fallbackData = _MakeFallbackVolumeMaterialData();
  return fallbackData;
}
...
//
HdSt_VolumeShaderSharedPtr _ComputeMaterialNetworkShader(
    ...
    const GfRange3d &authoredExtents)
{
  ...
  HdSt_VolumeShaderSharedPtr const result = std::make_shared<HdSt_VolumeShader>();
  ...
  for (const auto &fieldName : fieldNames) {
    ...
    // the texture.
    const HdSt_MaterialParam param(HdSt_MaterialParam::ParamTypeTexture,
                                   textureName,
                                   VtValue(GfVec4f(0)),
                                   TfTokenVector(),
                                   textureType);
  ...
  const bool doublesSupported = resourceRegistry->GetHgi()->GetCapabilities()->IsSet(
  ...
  HdSt_TextureBinder::GetBufferSpecs(namedTextureHandles, &bufferSpecs, doublesSupported);
  ...
  if (!hasField) {
    HdSt_VolumeShader::GetBufferSourcesForBBoxAndSampleDistance(
        {GfBBox3d(authoredExtents), 1.0f}, &bufferSources, doublesSupported);
  }
...
void HdStVolume::_UpdateDrawItem(HdSceneDelegate *sceneDelegate,
                                 HdRenderParam *renderParam,
                                 ...
                                 HdDirtyBits *dirtyBits)
{
  ...
  if (HdStShouldPopulateConstantPrimvars(dirtyBits, GetId())) {
    ...
    const HdPrimvarDescriptorVector constantPrimvars = HdStGetPrimvarDescriptors(
  ...
  if ((*dirtyBits) & HdChangeTracker::DirtyMaterialId) {
    ...
    drawItem->SetMaterialNetworkShader(
        _ComputeMaterialNetworkShader(sceneDelegate,
                                      GetId(),
  ...
  HdSt_VolumeShaderSharedPtr const materialNetworkShader =
      std::dynamic_pointer_cast<HdSt_VolumeShader>(drawItem->GetMaterialNetworkShader());
  ...
  if (((*dirtyBits) & (HdChangeTracker::DirtyVolumeField)) && !updatedTextureHandles) {
    /* FIELD TEXTURES */
  ...
  if ((*dirtyBits) & HdChangeTracker::NewRepr) {
    ...
    drawItem->SetGeometricShader(HdSt_GeometricShader::Create(shaderKey, resourceRegistry));
    ...
    /* TRIANGLE INDICES */
    {
      ...
      if (!HdStIsValidBAR(drawItem->GetTopologyRange())) {
        ...
        HdBufferArrayRangeSharedPtr const range =
            resourceRegistry->AllocateNonUniformBufferArrayRange(
                HdTokens->primvar, bufferSpecs, HdBufferArrayUsageHintBitsIndex);
