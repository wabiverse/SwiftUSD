...
GlfSimpleLight HdStLight::_ApproximateAreaLight(SdfPath const &id, HdSceneDelegate *sceneDelegate)
{
  ...
  // Color temperature
  VtValue enableColorTemperatureVal = sceneDelegate->GetLightParamValue(
      id, HdLightTokens->enableColorTemperature);
  if (enableColorTemperatureVal.GetWithDefault<bool>(false)) {
    VtValue colorTemperatureVal = sceneDelegate->GetLightParamValue(
  ...
  if (!normalizeVal.GetWithDefault<bool>(false)) {
    ...
    float area = 1.0f;
    if (_lightType == HdPrimTypeTokens->diskLight || _lightType == HdPrimTypeTokens->sphereLight) {
  ...
  float ambientMultiplier =
      sceneDelegate->GetLightParamValue(id, HdLightTokens->ambient).GetWithDefault<float>(0.0f);
  float diffuseMultiplier =
      sceneDelegate->GetLightParamValue(id, HdLightTokens->diffuse).GetWithDefault<float>(1.0f);
  float specularMultiplier =
      sceneDelegate->GetLightParamValue(id, HdLightTokens->specular).GetWithDefault<float>(1.0f);

  // Directional emission shaping
  float shapingConeAngle = sceneDelegate->GetLightParamValue(id, HdLightTokens->shapingConeAngle)
                               .GetWithDefault<float>(90.0f);
  float shapingFocus = sceneDelegate->GetLightParamValue(id, HdLightTokens->shapingFocus)
  ...
  l.SetHasShadow(
      sceneDelegate->GetLightParamValue(id, HdLightTokens->hasShadow).GetWithDefault<bool>(false));

  if (_lightType == HdPrimTypeTokens->rectLight || _lightType == HdPrimTypeTokens->diskLight) {
...
GlfSimpleLight HdStLight::_PrepareDomeLight(SdfPath const &id,
                                            HdSceneDelegate *const sceneDelegate)
{
  ...
  if (!sceneDelegate->GetVisible(id) ||
      (sceneDelegate->GetLightParamValue(id, HdLightTokens->intensity).Get<float>() == 0.0))
  {
    l.SetHasIntensity(false);
    return l;
  }
...
/* virtual */
void HdStLight::Sync(HdSceneDelegate *sceneDelegate,
                     ...
                     HdDirtyBits *dirtyBits)
{
  ...
  if (bits & (DirtyTransform | DirtyParams)) {
    GfMatrix4d transform = Get(HdTokens->transform).GetWithDefault<GfMatrix4d>();
    ...
    if (_lightType == HdPrimTypeTokens->domeLight) {
      ...
      GlfSimpleLight light = Get(HdLightTokens->params).GetWithDefault<GlfSimpleLight>();
  ...
  if (bits & DirtyShadowParams) {
    _params[HdLightTokens->shadowParams] = sceneDelegate->GetLightParamValue(
        id, HdLightTokens->shadowParams);
  }
...
/* virtual */
void HdStLight::Finalize(HdRenderParam *renderParam)
{
  // HdStLight doesn't own resources directly, so we don't need to
  // flag garbage collection.
}

VtValue HdStLight::Get(TfToken const &token) const
{
  VtValue val;
...
/* virtual */
HdDirtyBits HdStLight::GetInitialDirtyBitsMask() const
{
  ...
  if (_lightType == HdPrimTypeTokens->simpleLight || _lightType == HdPrimTypeTokens->distantLight)
  {
    return AllDirty;
  }
  else {
