...
TF_DEFINE_ENV_SETTING(HDST_ENABLE_RESOURCE_INSTANCING,
                      true,
...
static size_t _GetChainedStagingSize(HdBufferSourceSharedPtr const &src)
{
  size_t size = 0;
...
{
  static bool isResourceInstancingEnabled = TfGetEnvSetting(HDST_ENABLE_RESOURCE_INSTANCING);
...
HdStResourceRegistry::HdStResourceRegistry(Hgi *const hgi)
    : _hgi(hgi),
      ...
      ,
      _nonUniformAggregationStrategy(std::make_unique<HdStVBOMemoryManager>(this)),
      _nonUniformImmutableAggregationStrategy(std::make_unique<HdStVBOMemoryManager>(this))
      // default aggregation strategy for uniform on UBO (for globals)
      ,
      _uniformUboAggregationStrategy(std::make_unique<HdStInterleavedUBOMemoryManager>(this))
      // default aggregation strategy for uniform on SSBO (for primvars)
      ,
      _uniformSsboAggregationStrategy(std::make_unique<HdStInterleavedSSBOMemoryManager>(this))
      // default aggregation strategy for single buffers (for nested instancer)
      ,
      _singleAggregationStrategy(std::make_unique<HdStVBOSimpleMemoryManager>(this)),
...
void HdStResourceRegistry::ReloadResource(TfToken const &resourceType, std::string const &path)
{
  ...
  if (resourceType == HdResourceTypeTokens->shaderFile) {
    ...
    HdSt_TextureObjectRegistry *const reg = _textureHandleRegistry->GetTextureObjectRegistry();
    reg->MarkTextureFilePathDirty(TfToken(path));
  }
}

VtDictionary HdStResourceRegistry::GetResourceAllocation() const
{
  VtDictionary result;
...
Hgi *HdStResourceRegistry::GetHgi()
{
  return _hgi;
}
...
/// BAR allocation API
/// ------------------------------------------------------------------------
HdBufferArrayRangeSharedPtr HdStResourceRegistry::AllocateNonUniformBufferArrayRange(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::AllocateNonUniformImmutableBufferArrayRange(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::AllocateUniformBufferArrayRange(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::AllocateShaderStorageBufferArrayRange(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::AllocateSingleBufferArrayRange(
    TfToken const &role, HdBufferSpecVector const &bufferSpecs, HdBufferArrayUsageHint usageHint)
{
...
/// ------------------------------------------------------------------------
HdBufferArrayRangeSharedPtr HdStResourceRegistry::UpdateNonUniformBufferArrayRange(
    TfToken const &role,
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::UpdateShaderStorageBufferArrayRange(
    TfToken const &role,
    HdBufferArrayRangeSharedPtr const &curRange,
    HdBufferSpecVector const &updatedOrAddedSpecs,
    HdBufferSpecVector const &removedSpecs,
...
/// ------------------------------------------------------------------------
void HdStResourceRegistry::AddSources(HdBufferArrayRangeSharedPtr const &range,
...
void HdStResourceRegistry::AddSource(HdBufferArrayRangeSharedPtr const &range,
...
void HdStResourceRegistry::AddSource(HdBufferSourceSharedPtr const &source)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdStResourceRegistry::AddComputation(HdBufferArrayRangeSharedPtr const &range,
...
/// ------------------------------------------------------------------------
HdStDispatchBufferSharedPtr HdStResourceRegistry::RegisterDispatchBuffer(TfToken const &role,
                                                                         int count,
                                                                         int commandNumUints)
{
  HdStDispatchBufferSharedPtr const result = std::make_shared<HdStDispatchBuffer>(
...
HdStBufferResourceSharedPtr HdStResourceRegistry::RegisterBufferResource(
    TfToken const &role, HdTupleType tupleType, HgiBufferUsage bufferUsage)
{
  HdStBufferResourceSharedPtr const result = std::make_shared<HdStBufferResource>(
...
void HdStResourceRegistry::GarbageCollectDispatchBuffers()
{
  HD_TRACE_FUNCTION();
  ...
  _dispatchBufferRegistry.erase(
      std::remove_if(_dispatchBufferRegistry.begin(),
                     ...
                     std::bind(&HdStDispatchBufferSharedPtr::unique, std::placeholders::_1)),
      _dispatchBufferRegistry.end());
}

void HdStResourceRegistry::GarbageCollectBufferResources()
{
  HD_TRACE_FUNCTION();
  ...
  _bufferResourceRegistry.erase(
      std::remove_if(_bufferResourceRegistry.begin(),
                     ...
                     std::bind(&HdStBufferResourceSharedPtr::unique, std::placeholders::_1)),
      _bufferResourceRegistry.end());
}
...
/// Instance Registries
/// ------------------------------------------------------------------------
HdInstance<HdSt_MeshTopologySharedPtr> HdStResourceRegistry::RegisterMeshTopology(
    HdInstance<HdSt_MeshTopologySharedPtr>::ID id)
{
  return _Register(id, _meshTopologyRegistry, HdPerfTokens->instMeshTopology);
}

HdInstance<HdSt_BasisCurvesTopologySharedPtr> HdStResourceRegistry::RegisterBasisCurvesTopology(
...
HdInstance<HdSt_VertexAdjacencyBuilderSharedPtr> HdStResourceRegistry::
    RegisterVertexAdjacencyBuilder(HdInstance<HdSt_VertexAdjacencyBuilderSharedPtr>::ID id)
{
  return _Register(id, _vertexAdjacencyBuilderRegistry, HdPerfTokens->instVertexAdjacency);
}

HdInstance<HdBufferArrayRangeSharedPtr> HdStResourceRegistry::RegisterMeshIndexRange(
...
HdInstance<HdBufferArrayRangeSharedPtr> HdStResourceRegistry::RegisterBasisCurvesIndexRange(
    HdInstance<HdBufferArrayRangeSharedPtr>::ID id, TfToken const &name)
{
  return _Register(id,
...
HdInstance<HdBufferArrayRangeSharedPtr> HdStResourceRegistry::RegisterPrimvarRange(
    HdInstance<HdBufferArrayRangeSharedPtr>::ID id)
{
  return _Register(id, _primvarRangeRegistry, HdPerfTokens->instPrimvarRange);
}

HdInstance<HdBufferArrayRangeSharedPtr> HdStResourceRegistry::RegisterExtComputationDataRange(
    HdInstance<HdBufferArrayRangeSharedPtr>::ID id)
{
  return _Register(
...
HdInstance<HdSt_GeometricShaderSharedPtr> HdStResourceRegistry::RegisterGeometricShader(
    HdInstance<HdSt_GeometricShaderSharedPtr>::ID id)
{
  return _geometricShaderRegistry.GetInstance(id);
}

HdInstance<HdStGLSLProgramSharedPtr> HdStResourceRegistry::RegisterGLSLProgram(
    HdInstance<HdStGLSLProgramSharedPtr>::ID id)
{
  return _glslProgramRegistry.GetInstance(id);
}

HdInstance<HioGlslfxSharedPtr> HdStResourceRegistry::RegisterGLSLFXFile(
...
#ifdef PXR_MATERIALX_SUPPORT_ENABLED
HdInstance<MaterialX::ShaderPtr> HdStResourceRegistry::RegisterMaterialXShader(
...
HdInstance<HgiResourceBindingsSharedPtr> HdStResourceRegistry::RegisterResourceBindings(
    HdInstance<HgiResourceBindingsSharedPtr>::ID id)
{
  return _resourceBindingsRegistry.GetInstance(id);
}

HdInstance<HgiGraphicsPipelineSharedPtr> HdStResourceRegistry::RegisterGraphicsPipeline(
    HdInstance<HgiGraphicsPipelineSharedPtr>::ID id)
{
  return _graphicsPipelineRegistry.GetInstance(id);
}

HdInstance<HgiComputePipelineSharedPtr> HdStResourceRegistry::RegisterComputePipeline(
    HdInstance<HgiComputePipelineSharedPtr>::ID id)
{
  return _computePipelineRegistry.GetInstance(id);
}

HdResourceRegistry *HdStResourceRegistry::FindOrCreateSubResourceRegistry(
...
std::ostream &operator<<(std::ostream &out, const HdStResourceRegistry &self)
{
...
HgiBlitCmds *HdStResourceRegistry::GetGlobalBlitCmds()
{
  if (!_blitCmds) {
...
HgiComputeCmds *HdStResourceRegistry::GetGlobalComputeCmds(HgiComputeDispatch dispatchMethod)
{
  // If the HGI device isn't capable of concurrent dispatch then
  // only specify serial.
  bool const concurrentDispatchSupported = _hgi->GetCapabilities()->IsSet(
...
HdStStagingBuffer *HdStResourceRegistry::GetStagingBuffer()
{
  return _stagingBuffer.get();
}

void HdStResourceRegistry::SubmitBlitWork(HgiSubmitWaitType wait)
{
  if (_blitCmds) {
...
void HdStResourceRegistry::SubmitComputeWork(HgiSubmitWaitType wait)
{
  if (_computeCmds) {
...
void HdStResourceRegistry::_CommitTextures()
{
  HdStShaderCode::ResourceContext ctx(this);

  const std::set<HdStShaderCodeSharedPtr> shaderCodes = _textureHandleRegistry->Commit();
...
void HdStResourceRegistry::_Commit()
{
  ...
  // TODO: requests should be sorted by resource, and range.
  {
    ...
    while (numBufferSourcesResolved < _numBufferSourcesToResolve) {
      ...
      WorkParallelForEach(
          _pendingSources.begin(),
          _pendingSources.end(),
          [&numBufferSourcesResolved, &stagingBufferSize](_PendingSource &req) {
              ...
              if (!source->IsResolved()) {
                if (source->Resolve()) {
                  ...
                  if (req.range && req.range->RequiresStaging()) {
                    const size_t numElements = source->GetNumElements();
                    // Avoid calling functions on
                    // HdNullBufferSources
                    if (numElements > 0) {
                      stagingBufferSize += numElements *
                                           HdDataSizeOfTupleType(source->GetTupleType());
                    }
                    stagingBufferSize += _GetChainedStagingSize(source);
                  }
                }
              }
            }
...
void _DestroyResourceBindings(Hgi *hgi, HgiResourceBindingsHandle *resourceBindings)
{
  hgi->DestroyResourceBindings(resourceBindings);
}

void _DestroyGraphicsPipeline(Hgi *hgi, HgiGraphicsPipelineHandle *graphicsPipeline)
{
  hgi->DestroyGraphicsPipeline(graphicsPipeline);
}

void _DestroyComputePipeline(Hgi *hgi, HgiComputePipelineHandle *computePipeline)
{
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::_AllocateBufferArrayRange(
...
HdBufferArrayRangeSharedPtr HdStResourceRegistry::_UpdateBufferArrayRange(
    HdStAggregationStrategy *strategy,
    HdStBufferArrayRegistry &bufferArrayRegistry,
    TfToken const &role,
    HdBufferArrayRangeSharedPtr const &curRange,
    ...
    HdBufferArrayUsageHint usageHint)
{
  ...
  HdBufferSpecVector newBufferSpecs;
  {
    ...
    const HdBufferSpecVector specsAddX = HdBufferSpec::ComputeDifference(updatedOrAddedSpecs,
                                                                         removedSpecs);
    const HdBufferSpecVector specsRemoveX = HdBufferSpec::ComputeDifference(removedSpecs,
                                                                            updatedOrAddedSpecs);

    newBufferSpecs = HdBufferSpec::ComputeUnion(
        HdBufferSpec::ComputeDifference(curBufferSpecs, specsRemoveX), specsAddX);
  }
...
void HdStResourceRegistry::_TallyResourceAllocation(VtDictionary *result) const
{
...
HdStTextureHandleSharedPtr HdStResourceRegistry::AllocateTextureHandle(
    ...
    HdStShaderCodePtr const &shaderCode)
{
  return _textureHandleRegistry->AllocateTextureHandle(
      textureId, textureType, samplerParams, memoryRequest, shaderCode);
}

HdStTextureObjectSharedPtr HdStResourceRegistry::AllocateTextureObject(
    HdStTextureIdentifier const &textureId, const HdStTextureType textureType)
{
  HdSt_TextureObjectRegistry *const reg = _textureHandleRegistry->GetTextureObjectRegistry();
...
void HdStResourceRegistry::SetMemoryRequestForTextureType(const HdStTextureType textureType,
                                                          const size_t memoryRequest)
{
