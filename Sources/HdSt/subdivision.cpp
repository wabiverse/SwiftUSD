...
TF_DEFINE_PRIVATE_TOKENS(
    _tokens,
    (evalStencils)(stencilData)(sizes)(offsets)(indices)(weights)(baseFaceToRefinedFacesMap)(refinedFaceCounts));
...
class HdSt_OsdIndexComputation final : public HdComputedBufferSource {
 ...
 private:
  ...
  void _PopulateUniformPrimitiveBuffer(HdSt_Subdivision::PatchTable const *patchTable);
  void _PopulatePatchPrimitiveBuffer(HdSt_Subdivision::PatchTable const *patchTable);
...
class HdSt_OsdFvarIndexComputation final : public HdComputedBufferSource {
 ...
 private:
  void _PopulateFvarPatchParamBuffer(HdSt_Subdivision::PatchTable const *patchTable);
...
class HdSt_OsdBaseFaceToRefinedFacesMapComputation final : public HdComputedBufferSource {
 public:
  HdSt_OsdBaseFaceToRefinedFacesMapComputation(HdSt_Subdivision const *subdivision,
                                               HdBufferSourceSharedPtr const &osdTopology);
...
class HdSt_OsdStencilTableBufferSource final : public HdBufferSource {
 public:
  HdSt_OsdStencilTableBufferSource(HdSt_Subdivision const *subdivision,
                                   HdBufferSourceSharedPtr const &osdTopology,
                                   TfToken const &name,
...
bool HdSt_OsdStencilTableBufferSource::Resolve()
{
  ...
  HdSt_Subdivision::StencilTable const *stencilTable = _subdivision->GetStencilTable(
      _interpolation, _fvarChannel);
...
void HdSt_OsdStencilTableBufferSource::GetBufferSpecs(HdBufferSpecVector *) const
{
  // nothing
}
...
HdSt_Subdivision::StencilTable const *HdSt_Subdivision::GetStencilTable(
    HdSt_MeshTopology::Interpolation interpolation, int fvarChannel) const
{
...
HdSt_GpuStencilTableSharedPtr HdSt_Subdivision::_GetGpuStencilTable(
    ...
    int fvarChannel)
{
  ...
  if (interpolation == HdSt_MeshTopology::INTERPOLATE_VERTEX) {
    if (!_gpuVertexStencils) {
      _gpuVertexStencils = _CreateGpuStencilTable(osdTopology, registry, interpolation);
    }
    return _gpuVertexStencils;
  }
  else if (interpolation == HdSt_MeshTopology::INTERPOLATE_VARYING) {
    if (!_gpuVaryingStencils) {
      _gpuVaryingStencils = _CreateGpuStencilTable(osdTopology, registry, interpolation);
    }
...
HdSt_GpuStencilTableSharedPtr HdSt_Subdivision::_CreateGpuStencilTable(
    HdBufferSourceSharedPtr const &osdTopology,
    HdStResourceRegistry *registry,
    HdSt_MeshTopology::Interpolation interpolation,
    int fvarChannel) const
{
  ...
  HdBufferArrayRangeSharedPtr perPointRange = registry->AllocateSingleBufferArrayRange(
      _tokens->stencilData, perPointSpecs, HdBufferArrayUsageHintBitsStorage);
  ...
  HdBufferArrayRangeSharedPtr perIndexRange = registry->AllocateSingleBufferArrayRange(
      _tokens->stencilData, perIndexSpecs, HdBufferArrayUsageHintBitsStorage);

  HdSt_GpuStencilTableSharedPtr gpuStencilTable = std::make_shared<HdSt_GpuStencilTable>(
  ...
  HdBufferSourceSharedPtrVector perPointSources{
      std::make_shared<HdSt_OsdStencilTableBufferSource>(
          this, osdTopology, _tokens->sizes, gpuStencilTable, interpolation, fvarChannel),
      std::make_shared<HdSt_OsdStencilTableBufferSource>(
          this, osdTopology, _tokens->offsets, gpuStencilTable, interpolation, fvarChannel)};
  ...
  HdBufferSourceSharedPtrVector perIndexSources{
      std::make_shared<HdSt_OsdStencilTableBufferSource>(
          this, osdTopology, _tokens->indices, gpuStencilTable, interpolation, fvarChannel),
      std::make_shared<HdSt_OsdStencilTableBufferSource>(
          this, osdTopology, _tokens->weights, gpuStencilTable, interpolation, fvarChannel)};
...
/*virtual*/
void HdSt_OsdIndexComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
/*virtual*/
HdBufferSourceSharedPtrVector HdSt_OsdIndexComputation::GetChainedBuffers() const
{
...
template<class HashState> void TfHashAppend(HashState &h, HdSt_OsdRefineComputationCPU const &bs)
{
...
size_t HdSt_OsdRefineComputationCPU::GetNumElements() const
{
  ...
  size_t const elementStride = HdGetComponentCount(_source->GetTupleType().type);
  return _primvarBuffer.size() / elementStride;
}

HdSt_MeshTopology::Interpolation HdSt_OsdRefineComputationCPU::GetInterpolation() const
{
...
bool HdSt_OsdRefineComputationCPU::Resolve()
{
  ...
  subdivision->RefineCPU(_source, &_primvarBuffer, _interpolation, _fvarChannel);
...
void HdSt_OsdRefineComputationCPU::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
HdBufferSourceSharedPtr HdSt_OsdRefineComputationCPU::GetPreChainedBuffer() const
{
  return _source;
}
...
void HdSt_OsdRefineComputationGPU::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // nothing
  //
...
void HdSt_OsdRefineComputationGPU::Execute(HdBufferArrayRangeSharedPtr const &range,
                                           HdResourceRegistry *resourceRegistry)
{
  ...
  subdivision->RefineGPU(range, _primvarName, _gpuStencilTable, hdStResourceRegistry);

  HD_PERF_COUNTER_INCR(HdPerfTokens->subdivisionRefineGPU);
}

int HdSt_OsdRefineComputationGPU::GetNumOutputElements() const
{
...
HdSt_MeshTopology::Interpolation HdSt_OsdRefineComputationGPU::GetInterpolation() const
{
  return _interpolation;
}
...
HdSt_Subdivision::HdSt_Subdivision(bool adaptive, int refineLevel)
    : _adaptive(adaptive), _refineLevel(refineLevel), _maxNumFaceVarying(0)
{
}
...
void _EvalStencilsCPU(std::vector<float> *primvarBuffer,
...
void _EvalStencilsGPU(HdBufferArrayRangeSharedPtr const &range_,
                      TfToken const &primvarName,
                      int const numCoarsePoints,
                      int const numRefinedPoints,
...
void HdSt_Subdivision::RefineCPU(HdBufferSourceSharedPtr const &source,
                                 ...
                                 int fvarChannel)
{
  ...
  HdSt_Subdivision::StencilTable const *stencilTable = GetStencilTable(interpolation, fvarChannel);
  ...
  // Stride is measured here in components, not bytes.
  size_t const elementStride = HdGetComponentCount(source->GetTupleType().type);

  size_t const numTotalElements = stencilTable->GetNumControlVertices() +
  ...
  std::copy(srcData, srcData + (numSrcElements * elementStride), primvarBuffer->begin());

  _EvalStencilsCPU(primvarBuffer,
                   elementStride,
...
void HdSt_Subdivision::RefineGPU(HdBufferArrayRangeSharedPtr const &primvarRange,
                                 TfToken const &primvarName,
                                 HdSt_GpuStencilTableSharedPtr const &gpuStencilTable,
                                 HdStResourceRegistry *resourceRegistry)
{
  ...
  if (!TF_VERIFY(gpuStencilTable))
    return;

  // if there is no stencil (e.g. torus with adaptive refinement),
  // just return here
  if (gpuStencilTable->numRefinedPoints == 0)
    return;

  _EvalStencilsGPU(primvarRange,
                   primvarName,
...
HdBufferSourceSharedPtr HdSt_Subdivision::CreateTopologyComputation(HdSt_MeshTopology *topology,
                                                                    SdfPath const &id)
{
...
HdBufferSourceSharedPtr HdSt_Subdivision::CreateFvarIndexComputation(
    HdSt_MeshTopology *topology, HdBufferSourceSharedPtr const &osdTopology, int channel)
{
  return std::make_shared<HdSt_OsdFvarIndexComputation>(topology, osdTopology, channel);
}
...
HdStComputationSharedPtr HdSt_Subdivision::CreateRefineComputationGPU(
    HdSt_MeshTopology *topology,
    HdBufferSourceSharedPtr const &osdTopology,
    TfToken const &name,
    HdType dataType,
    ...
    int fvarChannel)
{
  HdSt_GpuStencilTableSharedPtr gpuStencilTable = _GetGpuStencilTable(
      topology, osdTopology, resourceRegistry, interpolation, fvarChannel);

  return std::make_shared<HdSt_OsdRefineComputationGPU>(
      topology, name, dataType, gpuStencilTable, interpolation);
}

HdBufferSourceSharedPtr HdSt_Subdivision::CreateBaseFaceToRefinedFacesMapComputation(
    HdBufferSourceSharedPtr const &osdTopology)
{
  return std::make_shared<HdSt_OsdBaseFaceToRefinedFacesMapComputation>(this, osdTopology);
}

// ---------------------------------------------------------------------------
HdSt_OsdTopologyComputation::HdSt_OsdTopologyComputation(HdSt_MeshTopology *topology,
                                                         SdfPath const &id)
    : _topology(topology), _id(id)
{
}

void HdSt_OsdTopologyComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // nothing
}
...
bool HdSt_OsdTopologyComputation::Resolve()
{
  ...
  std::vector<std::unique_ptr<HdSt_Subdivision::StencilTable const>> faceVaryingStencils(
      numFvarChannels);
  ...
  if (refiner) {
    ...
    }
    {
      ...
      varyingStencils.reset(Far::StencilTableFactory::Create(*refiner, options));
      ...
      for (int i = 0; i < numFvarChannels; ++i) {
        ...
        faceVaryingStencils[i].reset(Far::StencilTableFactory::Create(*refiner, options));
      }
    }
    {
  ...
  if (patchTable && patchTable->GetLocalPointStencilTable()) {
    ...
    if (Far::StencilTable const *vertexStencilsWithLocalPoints =
            Far::StencilTableFactory::AppendLocalPointStencilTable(
                *refiner, vertexStencils.get(), patchTable->GetLocalPointStencilTable()))
    {
  ...
  if (patchTable && patchTable->GetLocalPointVaryingStencilTable()) {
    ...
    if (Far::StencilTable const *varyingStencilsWithLocalPoints =
            Far::StencilTableFactory::AppendLocalPointStencilTableVarying(
                *refiner, varyingStencils.get(), patchTable->GetLocalPointVaryingStencilTable()))
    {
      ...
      if (Far::StencilTable const *faceVaryingStencilsWithLocalPoints =
              Far::StencilTableFactory ::AppendLocalPointStencilTableFaceVarying(
                  *refiner,
                  ...
                  i))
      {
        faceVaryingStencils[i].reset(faceVaryingStencilsWithLocalPoints);
      }
    }
  }
...
HdSt_OsdIndexComputation::HdSt_OsdIndexComputation(HdSt_MeshTopology *topology,
                                                   HdBufferSourceSharedPtr const &osdTopology)
    : _topology(topology), _osdTopology(osdTopology)
{
}
...
bool HdSt_OsdIndexComputation::Resolve()
{
  ...
  HdSt_Subdivision::PatchTable const *patchTable = subdivision->GetPatchTable();
  ...
  if (_topology->RefinesToBSplinePatches() || _topology->RefinesToBoxSplineTrianglePatches()) {
    ...
    memcpy(reinterpret_cast<Far::Index *>(indices.data()), firstIndex, ptableSize * sizeof(int));
    ...
    int const numIndicesPerQuad = _topology->TriangulateQuads() ?
    ...
    HdBufferSourceSharedPtr quadIndices = std::make_shared<HdVtBufferSource>(
...
HdSt_OsdFvarIndexComputation::HdSt_OsdFvarIndexComputation(
    HdSt_MeshTopology *topology, HdBufferSourceSharedPtr const &osdTopology, int channel)
    : _topology(topology), _osdTopology(osdTopology), _channel(channel)
{
  _indicesName = TfToken(HdStTokens->fvarIndices.GetString() + std::to_string(_channel));
  _patchParamName = TfToken(HdStTokens->fvarPatchParam.GetString() + std::to_string(_channel));
}
...
bool HdSt_OsdFvarIndexComputation::Resolve()
{
  ...
  Far::Index const *firstIndex = !fvarIndices.empty() ? fvarIndices.cdata() : nullptr;
  ...
  if (_topology->RefinesToBSplinePatches() || _topology->RefinesToBoxSplineTrianglePatches()) {
    ...
    // Bundle groups of 12 or 16 patch control vertices
    int const arraySize =
        (numPatches > 0) ? patchTable->GetFVarPatchDescriptor(_channel).GetNumControlVertices() :
                           0;
    ...
    memcpy(
        reinterpret_cast<Far::Index *>(indices.data()), firstIndex, 3 * numPatches * sizeof(int));
    ...
    memcpy(
        reinterpret_cast<Far::Index *>(indices.data()), firstIndex, 4 * numPatches * sizeof(int));
...
void HdSt_OsdFvarIndexComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  if (_topology->RefinesToBSplinePatches()) {
    // bi-cubic bspline patches
...
HdBufferSourceSharedPtrVector HdSt_OsdFvarIndexComputation::GetChainedBuffers() const
{
  if (_topology->RefinesToBSplinePatches() || _topology->RefinesToBoxSplineTrianglePatches()) {
...
HdSt_OsdBaseFaceToRefinedFacesMapComputation::HdSt_OsdBaseFaceToRefinedFacesMapComputation(
    HdSt_Subdivision const *subdivision, HdBufferSourceSharedPtr const &osdTopology)
    : _subdivision(subdivision), _osdTopology(osdTopology)
{
}

void HdSt_OsdBaseFaceToRefinedFacesMapComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  specs->emplace_back(_tokens->baseFaceToRefinedFacesMap, HdTupleType{HdTypeInt32, 1});
  specs->emplace_back(_tokens->refinedFaceCounts, HdTupleType{HdTypeInt32, 1});
}
...
bool HdSt_OsdBaseFaceToRefinedFacesMapComputation::Resolve()
{
  ...
  HdSt_Subdivision::PatchTable const *patchTable = _subdivision->GetPatchTable();
  const size_t numPatches = patchTable ? patchTable->GetPatchParamTable().size() : 0;
  ...
  _refinedFaceCounts = std::make_shared<HdVtBufferSource>(_tokens->refinedFaceCounts,
                                                          VtValue(refinedFaceCounts));
...
HdBufferSourceSharedPtrVector HdSt_OsdBaseFaceToRefinedFacesMapComputation::GetChainedBuffers()
    const
{
...
void _EvalStencilsGPU(HdBufferArrayRangeSharedPtr const &primvarRange_,
                      TfToken const &primvarName,
                      int const numCoarsePoints,
                      int const numRefinedPoints,
                      HdBufferArrayRangeSharedPtr const &perPointRange_,
                      ...
                      HdResourceRegistry *resourceRegistry)
{
  ...
  HdStBufferArrayRangeSharedPtr primvarRange = std::static_pointer_cast<HdStBufferArrayRange>(
      primvarRange_);

  HdStBufferResourceSharedPtr primvar = primvarRange->GetResource(primvarName);
  size_t const elementOffset = primvarRange->GetElementOffset();
  size_t const elementStride = HdGetComponentCount(primvar->GetTupleType().type);
  ...
  HdStResourceRegistry *hdStResourceRegistry = static_cast<HdStResourceRegistry *>(
      resourceRegistry);
  std::string const defines = TfStringPrintf("#define EVAL_STENCILS_NUM_ELEMENTS %zd\n",
                                             elementStride);
  HdStGLSLProgramSharedPtr computeProgram = HdStGLSLProgram::GetComputeProgram(
      shaderToken, defines, hdStResourceRegistry, [&](HgiShaderFunctionDesc &computeDesc) {
        ...
        HgiShaderFunctionAddBuffer(
            &computeDesc, "sizes", HdStTokens->_int, BufferBinding_Sizes, HgiBindingTypePointer);
        ...
        HgiShaderFunctionAddWritableBuffer(
            &computeDesc, "primvar", HdStTokens->_float, BufferBinding_Primvar);
        ...
        for (std::string const &param : params) {
          HgiShaderFunctionAddConstantParam(&computeDesc, param, HdStTokens->_int);
        }
  ...
  HdStBufferArrayRangeSharedPtr perPointRange = std::static_pointer_cast<HdStBufferArrayRange>(
      perPointRange_);
  HdStBufferArrayRangeSharedPtr perIndexRange = std::static_pointer_cast<HdStBufferArrayRange>(
      perIndexRange_);
  ...
  HdStBufferResourceSharedPtr sizes = perPointRange->GetResource(_tokens->sizes);
  HdStBufferResourceSharedPtr offsets = perPointRange->GetResource(_tokens->offsets);
  HdStBufferResourceSharedPtr indices = perIndexRange->GetResource(_tokens->indices);
  HdStBufferResourceSharedPtr weights = perIndexRange->GetResource(_tokens->weights);
  ...
  uint64_t const rbHash = (uint64_t)TfHash::Combine(sizes->GetHandle().GetId(),
                                                    offsets->GetHandle().GetId(),
                                                    indices->GetHandle().GetId(),
                                                    weights->GetHandle().GetId(),
                                                    primvar->GetHandle().GetId());

  uint64_t const pHash = (uint64_t)TfHash::Combine(computeProgram->GetProgram().Get(),
                                                   sizeof(uniform));
  ...
  HgiResourceBindingsSharedPtr const &resourceBindingsPtr = resourceBindingsInstance.GetValue();
