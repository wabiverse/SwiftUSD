...
HdSt_VolumeShader::HdSt_VolumeShader() : _fillsPointsBar(false) {}

HdSt_VolumeShader::~HdSt_VolumeShader() = default;

void HdSt_VolumeShader::AddBindings(HdStBindingRequestVector *const customBindings)
{
  HdSt_MaterialNetworkShader::AddBindings(customBindings);
}

void HdSt_VolumeShader::BindResources(const int program, HdSt_ResourceBinder const &binder)
{
  HdSt_MaterialNetworkShader::BindResources(program, binder);
}

void HdSt_VolumeShader::UnbindResources(const int program, HdSt_ResourceBinder const &binder)
{
  HdSt_MaterialNetworkShader::UnbindResources(program, binder);
}

void HdSt_VolumeShader::SetPointsBar(HdBufferArrayRangeSharedPtr const &pointsBar)
{
  _pointsBar = pointsBar;
}

void HdSt_VolumeShader::SetFillsPointsBar(const bool fillsPointsBar)
{
...
{
  return TfToken(token.GetString() + HdSt_ResourceBindingSuffixTokens->fallback.GetString());
}

void HdSt_VolumeShader::GetParamsAndBufferSpecsForBBoxAndSampleDistance(
    HdSt_MaterialParamVector *const params, HdBufferSpecVector *const specs, bool doublesSupported)
{
  ...
  {
    params->emplace_back(
        HdSt_MaterialParam::ParamTypeFallback, _tokens->volumeBBoxInverseTransform, mat4Fallback);

    static const TfToken sourceName(_ConcatFallback(_tokens->volumeBBoxInverseTransform));
    specs->emplace_back(sourceName, HdTupleType{mat4Type, 1});
  }

  {
    params->emplace_back(
        HdSt_MaterialParam::ParamTypeFallback, _tokens->volumeBBoxLocalMin, vec3Fallback);

    static const TfToken sourceName(_ConcatFallback(_tokens->volumeBBoxLocalMin));
    specs->emplace_back(sourceName, HdTupleType{vec3Type, 1});
  }

  {
    params->emplace_back(
        HdSt_MaterialParam::ParamTypeFallback, _tokens->volumeBBoxLocalMax, vec3Fallback);
  ...
  {
    params->emplace_back(
        HdSt_MaterialParam::ParamTypeFallback, _tokens->sampleDistance, VtValue(100000.0f));
...
void HdSt_VolumeShader::GetBufferSourcesForBBoxAndSampleDistance(
    ...
    bool doublesSupported)
{
...

  {
    ...
    sources->push_back(std::make_shared<HdVtBufferSource>(
        sourceName, VtValue(bbox.GetInverseMatrix()), 1, doublesSupported));
  }

  {
    static const TfToken sourceName(_ConcatFallback(_tokens->volumeBBoxLocalMin));
    sources->push_back(std::make_shared<HdVtBufferSource>(
        sourceName, VtValue(GetSafeMin(range)), 1, doublesSupported));
  }

  {
    static const TfToken sourceName(_ConcatFallback(_tokens->volumeBBoxLocalMax));
    sources->push_back(std::make_shared<HdVtBufferSource>(
        sourceName, VtValue(GetSafeMax(range)), 1, doublesSupported));
  }
...
GfVec3d HdSt_VolumeShader::GetSafeMin(const GfRange3d &range)
{
  if (range.IsEmpty()) {
...
GfVec3d HdSt_VolumeShader::GetSafeMax(const GfRange3d &range)
{
  if (range.IsEmpty()) {
    return GfVec3d(0.0, 0.0, 0.0);
  }
    ...
    const HdStShaderCode::NamedTextureHandleVector &textures)
{
  ...
  for (const HdStShaderCode::NamedTextureHandle &texture : textures) {
    HdStTextureObjectSharedPtr const &textureObject = texture.handle->GetTextureObject();

    if (const HdStFieldTextureObject *const fieldTex = dynamic_cast<HdStFieldTextureObject *>(
            textureObject.get()))
    {
...
void HdSt_VolumeShader::AddResourcesFromTextures(ResourceContext &ctx) const
{
...
void HdSt_VolumeShader::SetFieldDescriptors(const HdVolumeFieldDescriptorVector &fieldDescs)
{
  _fieldDescriptors = fieldDescs;
}

void HdSt_VolumeShader::UpdateTextureHandles(HdSceneDelegate *const sceneDelegate)
{
  TRACE_FUNCTION();
  ...
  for (size_t i = 0; i < textureHandles.size(); i++) {
    ...
    textureHandles[i].handle = resourceRegistry->AllocateTextureHandle(
        textureId, textureType, samplerParams, textureMemory, shared_from_this());
  }
