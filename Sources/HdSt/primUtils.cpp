...
TF_DEFINE_ENV_SETTING(HDST_ENABLE_SHARED_VERTEX_PRIMVAR, 1, "Enable sharing of vertex primvar");

TF_MAKE_STATIC_DATA(HdSt_MaterialNetworkShaderSharedPtr, _fallbackWidgetShader)
{
...
void HdStMarkDrawBatchesDirty(HdRenderParam *renderParam)
{
  if (TF_VERIFY(renderParam)) {
    HdStRenderParam *stRenderParam = static_cast<HdStRenderParam *>(renderParam);
...
// -----------------------------------------------------------------------------
static bool _IsEnabledPrimvarFiltering(HdStDrawItem const *drawItem)
{
  HdSt_MaterialNetworkShaderSharedPtr materialNetworkShader = drawItem->GetMaterialNetworkShader();
  return materialNetworkShader && materialNetworkShader->IsEnabledPrimvarFiltering();
}

static TfTokenVector const &_GetFilterNamesForMaterial(HdStDrawItem const *drawItem)
{
  HdSt_MaterialNetworkShaderSharedPtr materialNetworkShader = drawItem->GetMaterialNetworkShader();
                                     ...
                                     HdStInstancer const *instancer = nullptr)
{
  ...
  filterNames.insert(filterNames.end(), matPvNames.begin(), matPvNames.end());
...
HdPrimvarDescriptorVector HdStGetPrimvarDescriptors(HdRprim const *prim,
                                                    HdStDrawItem const *drawItem,
                                                    ...
                                                    size_t numGeomSubsets)
{
  ...
  if (numGeomSubsets != 0 && repr && descGeomStyle != HdMeshGeomStyleInvalid &&
      descGeomStyle != HdMeshGeomStylePoints)
  {
    for (size_t i = 0; i < numGeomSubsets; ++i) {
      HdStDrawItem const *subsetDrawItem = static_cast<HdStDrawItem *>(
      ...
      if (_IsEnabledPrimvarFiltering(subsetDrawItem)) {
        const TfTokenVector matPvNames = _GetFilterNamesForMaterial(subsetDrawItem);
        filterNames.insert(filterNames.end(), matPvNames.begin(), matPvNames.end());
      }
    }
    std::sort(filterNames.begin(), filterNames.end());
    filterNames.erase(std::unique(filterNames.begin(), filterNames.end()), filterNames.end());
  }
...
HdPrimvarDescriptorVector HdStGetInstancerPrimvarDescriptors(HdStInstancer const *instancer,
                                                             HdSceneDelegate *delegate)
{
...
void HdStUpdateRenderTag(HdSceneDelegate *delegate, HdRenderParam *renderParam, HdRprim *rprim)
{
...
void HdStSetMaterialId(HdSceneDelegate *delegate, HdRenderParam *renderParam, HdRprim *rprim)
{
...
void HdStSetMaterialTag(HdRenderParam *const renderParam,
                        HdDrawItem *drawItem,
...
//
static TfToken _ComputeMaterialTag(HdSceneDelegate *const delegate,
...
void HdStSetMaterialTag(HdSceneDelegate *const delegate,
                        ...
                        const bool occludedSelectionShowsThrough)
{
  HdStSetMaterialTag(
      ...
      drawItem,
      _ComputeMaterialTag(
          delegate, materialId, hasDisplayOpacityPrimvar, occludedSelectionShowsThrough));
}

HdSt_MaterialNetworkShaderSharedPtr HdStGetMaterialNetworkShader(HdRprim const *prim,
                                                                 HdSceneDelegate *delegate)
{
  ...
  if (material == nullptr) {
    if (prim->GetRenderTag(delegate) == HdRenderTagTokens->widget) {
      TF_DEBUG(HD_RPRIM_UPDATED)
          .Msg(
              "Using built-in widget material for "
              "%s\n",
              prim->GetId().GetText());
...
bool HdStCanSkipBARAllocationOrUpdate(HdBufferSourceSharedPtrVector const &sources,
                                      ...
                                      HdDirtyBits dirtyBits)
{
  ...
  return noDataSourcesToUpdate && (!HdStIsValidBAR(curRange) || !mayHaveDirtyPrimvarDesc);
}
...
HdBufferSpecVector _GetRemovedPrimvarBufferSpecs(
...
HdBufferSpecVector HdStGetRemovedPrimvarBufferSpecs(
    HdBufferArrayRangeSharedPtr const &curRange,
    HdPrimvarDescriptorVector const &newPrimvarDescs,
    HdExtComputationPrimvarDescriptorVector const &newCompPrimvarDescs,
    TfTokenVector const &internallyGeneratedPrimvarNames,
    SdfPath const &rprimId)
{
  ...
  return _GetRemovedPrimvarBufferSpecs(
      curBarSpecs, newPrimvarDescs, newCompPrimvarDescs, internallyGeneratedPrimvarNames, rprimId);
}
...
HdBufferSpecVector HdStGetRemovedPrimvarBufferSpecs(
    ...
    SdfPath const &rprimId)
{
  return HdStGetRemovedPrimvarBufferSpecs(curRange,
                                          newPrimvarDescs,
...
HdBufferSpecVector HdStGetRemovedOrReplacedPrimvarBufferSpecs(
    HdBufferArrayRangeSharedPtr const &curRange,
    HdPrimvarDescriptorVector const &newPrimvarDescs,
    HdExtComputationPrimvarDescriptorVector const &newCompPrimvarDescs,
    TfTokenVector const &internallyGeneratedPrimvarNames,
    ...
    SdfPath const &rprimId)
{
  ...
  for (const auto &curSpec : curBarSpecs) {
    ...
    if (newSpec != updatedSpecs.end() && curSpec != *newSpec) {
      TF_DEBUG(HD_RPRIM_UPDATED)
          .Msg("%s: Found primvar %s that has been replaced\n",
               rprimId.GetText(),
               curSpec.name.GetText());
...
HdBufferSpecVector HdStGetRemovedOrReplacedPrimvarBufferSpecs(
    ...
    SdfPath const &rprimId)
{
  return HdStGetRemovedOrReplacedPrimvarBufferSpecs(curRange,
                                                    newPrimvarDescs,
                                                    HdExtComputationPrimvarDescriptorVector(),
                                                    internallyGeneratedPrimvarNames,
                                                    updatedSpecs,
...
void HdStUpdateDrawItemBAR(HdBufferArrayRangeSharedPtr const &newRange,
                           int drawCoordIndex,
...
bool HdStIsPrimvarExistentAndValid(HdRprim *prim,
                                   HdSceneDelegate *delegate,
...
// -----------------------------------------------------------------------------
bool HdStShouldPopulateConstantPrimvars(HdDirtyBits const *dirtyBits, SdfPath const &id)
{
...
void HdStPopulateConstantPrimvars(HdRprim *prim,
                                  HdRprimSharedData *sharedData,
                                  HdSceneDelegate *delegate,
                                  HdRenderParam *renderParam,
                                  ...
                                  bool *hasMirroredTransform)
{
  ...
  if (HdChangeTracker::IsTransformDirty(*dirtyBits, id)) {
    ...
    sources.push_back(
        std::make_shared<HdVtBufferSource>(HdTokens->transform, transform, doublesSupported));

    sources.push_back(std::make_shared<HdVtBufferSource>(
        HdTokens->transformInverse, transform.GetInverse(), doublesSupported));
  ...
  if (hasDirtyPrimvarDesc) {
    ...
    removedSpecs = HdStGetRemovedOrReplacedPrimvarBufferSpecs(
        bar, constantPrimvars, internallyGeneratedPrimvars, bufferSpecs, id);
  }

  HdBufferArrayRangeSharedPtr range = hdStResourceRegistry->UpdateShaderStorageBufferArrayRange(
  ...
  if (!sources.empty()) {
    hdStResourceRegistry->AddSources(drawItem->GetConstantPrimvarRange(), std::move(sources));
  }
}
...
void HdStUpdateInstancerData(HdRenderIndex &renderIndex,
                             HdRenderParam *renderParam,
                             HdRprim *prim,
                             HdStDrawItem *drawItem,
                             HdRprimSharedData *sharedData,
                             HdDirtyBits rprimDirtyBits)
{
  ...
  while (!parentId.IsEmpty()) {
    ...
    HdStUpdateDrawItemBAR(static_cast<HdStInstancer *>(instancer)->GetInstancePrimvarRange(),
                          drawCoordIndex,
                          sharedData,
                          renderParam,
                          &changeTracker);
  ...
  if (HdChangeTracker::IsInstanceIndexDirty(rprimDirtyBits, prim->GetId()) || forceIndexRebuild) {
    ...
    if (!parentId.IsEmpty()) {
      ...
      VtIntArray const originalInstanceIndices =
          static_cast<HdStInstancer *>(instancer)->GetInstanceIndices(prim->GetId());
      VtIntArray instanceIndices = VtIntArray(originalInstanceIndices.size() + 1);
      instanceIndices[0] = 0;
      std::copy(originalInstanceIndices.cbegin(),
                originalInstanceIndices.cend(),
                instanceIndices.begin() + 1);

      HdStResourceRegistry *const resourceRegistry = static_cast<HdStResourceRegistry *>(
...
bool HdStIsInstancePrimvarExistentAndValid(HdRenderIndex &renderIndex,
                                           HdRprim *rprim,
                                           TfToken const &primvarName)
{
...
void HdStProcessTopologyVisibility(VtIntArray invisibleElements,
                                   ...
                                   SdfPath const &rprimId)
{
  ...
  if (!tvBAR || barNeedsReallocation) {
    HdBufferArrayRangeSharedPtr range = resourceRegistry->AllocateShaderStorageBufferArrayRange(
        HdTokens->topologyVisibility, bufferSpecs, HdBufferArrayUsageHintBitsStorage);
...
{
  static bool enabled = (TfGetEnvSetting(HDST_ENABLE_SHARED_VERTEX_PRIMVAR) == 1);
  return enabled;
}

uint64_t HdStComputeSharedPrimvarId(uint64_t baseId,
                                    ...
                                    HdStComputationComputeQueuePairVector const &computations)
{
  ...
  for (const auto &computation : computations) {
    if (std::shared_ptr<HdSt_OsdRefineComputationGPU> refinedComputation =
            std::dynamic_pointer_cast<HdSt_OsdRefineComputationGPU>(computation.first))
    {
...
void HdStGetBufferSpecsFromCompuations(HdStComputationComputeQueuePairVector const &computations,
                                       HdBufferSpecVector *bufferSpecs)
{
