...
static object _GetBracketingTimeSamples(const UsdAttribute &self,
                                        double desiredTime) {
  double lower = 0.0, upper = 0.0;
  bool hasTimeSamples = false;

  if (self.GetBracketingTimeSamples(desiredTime, &lower, &upper,
                                    &hasTimeSamples)) {
...
static bool _Set(const UsdAttribute &self, object val,
...
void wrapUsdAttribute() {
  class_<UsdAttribute, bases<UsdProperty>>("Attribute")
      ...
      .def("GetTimeSamplesInInterval", _GetTimeSamplesInInterval,
           arg("interval"), return_value_policy<TfPySequenceToList>())

      .def("GetUnionedTimeSamples", _GetUnionedTimeSamples, arg("attrs"),
           return_value_policy<TfPySequenceToList>())
      .staticmethod("GetUnionedTimeSamples")

      .def("GetUnionedTimeSamplesInInterval", _GetUnionedTimeSamplesInInterval,
      ...
      .def("GetResolveInfo",
           ...
           arg("time"))
      .def("GetResolveInfo", (UsdResolveInfo(UsdAttribute::*)() const) &
                                 UsdAttribute::GetResolveInfo)
