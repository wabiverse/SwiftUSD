...
public:
  Usd_PyPrimRange(UsdPrim root)
      : _rng(root), _startPrim(!_rng.empty() ? *_rng.begin() : UsdPrim()) {}
...
private:
  ...
  explicit Usd_PyPrimRange(const UsdPrimRange &range)
      : _rng(range), _startPrim(!_rng.empty() ? *_rng.begin() : UsdPrim()) {}
  ...
  static void _construct(PyObject *obj_ptr,
    ...
    void *storage =
        ((converter::rvalue_from_python_storage<Usd_PyPrimRange> *)data)
            ->storage.bytes;
...
public:
  explicit Usd_PyPrimRangeIterator(Usd_PyPrimRange const *range)
      : range(range), iter(range->_rng.begin()),
  ...
  UsdPrim next() {
    ...
    _RaiseIfAtEnd();
    if (!curPrim) {
      TfPyThrowRuntimeError(TfStringPrintf("Iterator points to %s",
...
Usd_PyPrimRangeIterator Usd_PyPrimRange::__iter__() const {
  if (!_rng.empty() && !_startPrim) {
    TfPyThrowRuntimeError(TfStringPrintf("Invalid range starting with %s",
...
void wrapUsdPrimRange() {
  {
    scope s = class_<Usd_PyPrimRange>("PrimRange", no_init)
    ...
    class_<Usd_PyPrimRangeIterator>("_Iterator", no_init)
        ...
        .def("__iter__",
             static_cast<void (*)(Usd_PyPrimRangeIterator)>(
