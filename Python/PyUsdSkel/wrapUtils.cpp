...
template<typename Matrix4>
Matrix4 _InterleavedSkinTransform(const TfToken &skinningMethod,
                                  const Matrix4 &geomBindTransform,
                                  TfSpan<const Matrix4> jointXforms,
                                  TfSpan<const GfVec2f> influences)
{
  Matrix4 xform;
  if (!UsdSkelSkinTransform(skinningMethod, geomBindTransform, jointXforms, influences, &xform)) {
    xform = geomBindTransform;
  }
  return xform;
}
...
template<typename Matrix4>
Matrix4 _NonInterleavedSkinTransform(const TfToken &skinningMethod,
                                     const Matrix4 &geomBindTransform,
                                     TfSpan<const Matrix4> jointXforms,
                                     TfSpan<const int> jointIndices,
                                     TfSpan<const float> jointWeights)
...
template<class Matrix3, class Matrix4> void _WrapUtilsT()
{
  ...
  def("SkinPoints",
      static_cast<bool (*)(const TfToken &,
                           const Matrix4 &,
                           TfSpan<const Matrix4>,
                           TfSpan<const int>,
                           TfSpan<const float>,
  ...
  def("SkinTransform",
      &_InterleavedSkinTransform<Matrix4>,
      (arg("skinningMethod"), arg("geomBindTransform"), arg("jointXforms"), arg("influences")));

  def("SkinTransform",
      &_NonInterleavedSkinTransform<Matrix4>,
      (arg("skinningMethod"),
       arg("geomBindTransform"),
       arg("jointXforms"),
       arg("jointIndices"),
