...
struct _VtDictionaryArrayFromPython {
  ...
  {
    converter::registry::insert(&convertible, &construct, type_id<std::vector<VtDictionary>>());
  }
  ...
  static void construct(PyObject *source, converter::rvalue_from_python_stage1_data *data)
  {
    TfAutoMallocTag2 tag("Vt", "_VtDictionaryArrayFromPython::construct");
    void *storage =
        ((converter::rvalue_from_python_storage<std::vector<VtDictionary>> *)data)->storage.bytes;
...
static std::vector<VtDictionary> _DictionaryArrayIdent(std::vector<VtDictionary> const &v)
{
  return v;
}
