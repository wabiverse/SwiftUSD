...
static std::string _Repr(const PcpLayerStackIdentifier &x) {
  return TfStringPrintf(
      "%sLayerStackIdentifier(%s, %s, %s, %s)", TF_PY_REPR_PREFIX.c_str(),
      TfPyRepr(x.rootLayer).c_str(), TfPyRepr(x.sessionLayer).c_str(),
...
void wrapLayerStackIdentifier() {
  ...
  class_<This>("LayerStackIdentifier")
      .def(init<>())
      .def(
          init<const SdfLayerHandle &, const SdfLayerHandle &,
               const ArResolverContext &, const PcpExpressionVariablesSource &>(
              (args("rootLayer"), args("sessionLayer") = SdfLayerHandle(),
