...
static string _GetRepr(const TsSpline &val)
{
  string repr = TF_PY_REPR_PREFIX + "Spline(";
  std::pair<TsExtrapolationType, TsExtrapolationType> extrapolation = val.GetExtrapolation();
  TsLoopParams loopParams = val.GetLoopParams();
  size_t counter = val.size();

  bool empty = (counter == 0) && (extrapolation.first == TsExtrapolationHeld) &&
               (extrapolation.second == TsExtrapolationHeld) && (!loopParams.IsValid());
...
static TsSpline *_ConstructFromKeyFrameDict(boost::python::object &kfDict, TsKnotType knotType)
{
...
static vector<VtValue> _EvalMultipleTimes(const TsSpline &val, const vector<TsTime> &times)
{
...
static vector<VtValue> _GetRange(const TsSpline &val, TsTime startTime, TsTime endTime)
{
...
static boost::python::object _GetClosestKeyFrame(const TsSpline &val, TsTime t)
{
  std::optional<TsKeyFrame> kf = val.GetClosestKeyFrame(t);
  return kf ? object(*kf) : object();
}

static boost::python::object _GetClosestKeyFrameBefore(const TsSpline &val, TsTime t)
{
  std::optional<TsKeyFrame> kf = val.GetClosestKeyFrameBefore(t);
  return kf ? object(*kf) : object();
}

static boost::python::object _GetClosestKeyFrameAfter(const TsSpline &val, TsTime t)
{
...
static void _SliceKeyframes(const boost::python::slice &index,
...
static size_t _GetSize(const TsSpline &val)
{
  return val.GetKeyFrames().size();
}

static vector<TsKeyFrame> _GetValues(const TsSpline &val)
{
  const TsKeyFrameMap &vec = val.GetKeyFrames();
  return vector<TsKeyFrame>(vec.begin(), vec.end());
}

static vector<TsTime> _GetKeys(const TsSpline &val)
{
...
static TsKeyFrame _GetItemByKey(const TsSpline &val, const TsTime &key)
{
  const TsKeyFrameMap &kf = val.GetKeyFrames();
...
static vector<TsKeyFrame> _GetSlice(const TsSpline &val, const boost::python::slice &index)
{
...
static bool _ContainsItemWithKey(const TsSpline &val, const TsTime &key)
{
  return val.GetKeyFrames().find(key) != val.GetKeyFrames().end();
}

static void _DelItemByKey(TsSpline &val, const TsTime &key)
{
  val.RemoveKeyFrame(key);
}

static void _DelSlice(TsSpline &val, const boost::python::slice &index)
{
...
static PyObject *_Iter(const TsSpline &val)
{
  vector<TsKeyFrame> kf = _GetValues(val);

  // Convert our vector to a Python list.
  PyObject *kf_list = TfPySequenceToList::apply<vector<TsKeyFrame>>::type()(kf);
...
static Ts_AnnotatedBoolResult _CanSetKeyFrame(TsSpline &self, const TsKeyFrame &kf)
{
  std::string reason;
...
static void _SetKeyFrame(TsSpline &self, const TsKeyFrame &kf)
{
  // Wrapper to discard optional intervalAffected argument.
  self.SetKeyFrame(kf);
}

static void _SetKeyFrames(TsSpline &self, const std::vector<TsKeyFrame> &kf)
{
  // XXX: pixar-ism
  //
...
// Simple breakdown.
static object _Breakdown1(TsSpline &self,
                          double x,
                          TsKnotType type,
                          bool flatTangents,
...
{
  // Wrapper to discard optional intervalAffected argument.
  std::optional<TsKeyFrame> kf = self.Breakdown(x, type, flatTangents, tangentLength, v);
...
static std::map<TsTime, TsKeyFrame> _Breakdown2(TsSpline &self,
                                                ...
                                                double tangentLength)
{
  // Wrapper to discard optional intervalAffected argument.
  TsKeyFrameMap vec;
  self.Breakdown(times, type, flatTangents, tangentLength, VtValue(), NULL, &vec);
...
static std::map<TsTime, TsKeyFrame> _Breakdown3(TsSpline &self,
                                                ...
                                                std::vector<VtValue> values)
{
  // Wrapper to discard optional intervalAffected argument.
  TsKeyFrameMap vec;
  self.Breakdown(times, type, flatTangents, tangentLength, values, NULL, &vec);
  std::map<TsTime, TsKeyFrame> map;
  TF_FOR_ALL(i, vec)
  {
    map.insert(std::make_pair(i->GetTime(), *i));
...
static std::map<TsTime, TsKeyFrame> _Breakdown4(TsSpline &self,
                                                std::vector<double> times,
                                                ...
                                                std::vector<VtValue> values)
{
  // Wrapper to discard optional intervalAffected argument.
  TsKeyFrameMap vec;
  self.Breakdown(times, types, flatTangents, tangentLength, values, NULL, &vec);
  std::map<TsTime, TsKeyFrame> map;
  TF_FOR_ALL(i, vec)
  {
    map.insert(std::make_pair(i->GetTime(), *i));
...
static void _SetExtrapolation(TsSpline &self,
...
static bool _Eq(const TsSpline &lhs, const TsSpline &rhs)
{
  return (&lhs == &rhs) || (lhs.GetKeyFrames() == rhs.GetKeyFrames() &&
                            lhs.GetExtrapolation() == rhs.GetExtrapolation() &&
                            lhs.GetLoopParams() == rhs.GetLoopParams());
}
static bool _Ne(const TsSpline &lhs, const TsSpline &rhs)
{
  return !_Eq(lhs, rhs);
}

// Functions for inspecting redundancy of key frames.
static bool _IsKeyFrameRedundant(const TsSpline &self,
                                 const TsTime &key,
...
BOOST_PYTHON_FUNCTION_OVERLOADS(_IsKeyFrameRedundant_overloads, _IsKeyFrameRedundant, 2, 3);

static bool _IsKeyFrameRedundant_2(const TsSpline &self,
                                   const TsKeyFrame &kf,
                                   const VtValue &defaultValue = VtValue())
{
...
BOOST_PYTHON_FUNCTION_OVERLOADS(_IsKeyFrameRedundant_2_overloads, _IsKeyFrameRedundant_2, 2, 3);

static bool _IsSegmentFlat(const TsSpline &self, const TsTime &lkey, const TsTime &rkey)
{
  return self.IsSegmentFlat(_GetItemByKey(self, lkey), _GetItemByKey(self, rkey));
}

static bool _IsSegmentFlat_2(const TsSpline &self, const TsKeyFrame &lkf, const TsKeyFrame &rkf)
{
  return self.IsSegmentFlat(lkf, rkf);
}

static bool _IsSegmentValueMonotonic(const TsSpline &self, const TsTime &lkey, const TsTime &rkey)
{
  return self.IsSegmentValueMonotonic(_GetItemByKey(self, lkey), _GetItemByKey(self, rkey));
}

static bool _IsSegmentValueMonotonic_2(const TsSpline &self,
...
void wrapSpline()
{
  ...
  class_<This>("Spline", no_init)
      ...
      .def(init<const vector<TsKeyFrame> &,
                optional<TsExtrapolationType, TsExtrapolationType, const TsLoopParams &>>())
      .def("__init__", make_constructor(&::_ConstructFromKeyFrameDict))

      .def("__repr__", &::_GetRepr)
      .def("IsLinear", &This::IsLinear)
      .def("ClearRedundantKeyFrames",
           &This::ClearRedundantKeyFrames,
           "Clears all redundant key frames from the spline\n\n",
           (arg("defaultValue") = VtValue(),
            arg("intervals") = GfMultiInterval(
                GfInterval(-std::numeric_limits<double>::infinity(),
      ...
      .add_property("extrapolation",
                    make_function(&This::GetExtrapolation, return_value_policy<TfPyPairToTuple>()),
                    make_function(&::_SetExtrapolation))

      .add_property("loopParams",
                    make_function(&This::GetLoopParams, return_value_policy<return_by_value>()),
                    &This::SetLoopParams)
      ...
      .def("Eval", &This::Eval, (arg("time"), arg("side") = TsRight))
      .def("EvalHeld", &This::EvalHeld, (arg("side") = TsRight))
      .def("EvalDerivative", &This::EvalDerivative, (arg("side") = TsRight))
      .def("Eval",
           &::_EvalMultipleTimes,
      ...
      .def("DoSidesDiffer", &This::DoSidesDiffer, (arg("time")))

      .def("Sample", &This::Sample, return_value_policy<TfPySequenceToTuple>())
      ...
      .add_property("frames",
                    make_function(&::_GetKeys, return_value_policy<TfPySequenceToList>()),
                    "A list of the frames for which keyframes exist.")

      .add_property(
          "frameRange", &This::GetFrameRange, "A list with the first and last frames as elements.")

      .def("GetKeyFramesInMultiInterval",
           &This::GetKeyFramesInMultiInterval,
           return_value_policy<TfPySequenceToList>())

      // Keyframes dictionary interface
      .def("__len__", &::_GetSize)
      .def("__getitem__", &::_GetItemByKey)
      .def("__getitem__", &::_GetSlice, return_value_policy<TfPySequenceToList>())
      .def("has_key", &::_ContainsItemWithKey)
      .def("__contains__", &::_ContainsItemWithKey)
      .def("keys", &::_GetKeys, return_value_policy<TfPySequenceToList>())
      .def("values", &::_GetValues, return_value_policy<TfPySequenceToList>())
      ...
      .def("ClosestKeyFrameBefore",
           &::_GetClosestKeyFrameBefore,
      ...
      .def("IsKeyFrameRedundant",
           _IsKeyFrameRedundant_2,
           _IsKeyFrameRedundant_2_overloads(
               "True if the key frame is redundant.\n\n"
               "If a second parameter is provided it is used as a default value"
               "for the spline, so that the last knot on a spline can be marked"
               "redundant if it is equal to the default value."))
      .def("HasRedundantKeyFrames",
           &This::HasRedundantKeyFrames,
      ...
      .def("IsSegmentFlat",
           ...
           "flat.")
      .def("IsSegmentValueMonotonic",
           &::_IsSegmentValueMonotonic,
      ...
      .def("IsVaryingSignificantly",
           &This::IsVaryingSignificantly,
           "True if the value of the spline changes over time, "
