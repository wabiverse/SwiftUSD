...
struct Ar_ResolverContextFromPython {
  ...
  static void _construct(PyObject *obj_ptr,
    ...
    void *storage =
        ((converter::rvalue_from_python_storage<ArResolverContext> *)data)
            ->storage.bytes;
    ...
    if (!obj.is_none()) {
      context =
          ArResolverContext(extract<std::vector<ArResolverContext>>(obj)());
    }
...
static size_t _Hash(const ArResolverContext &ctx) { return hash_value(ctx); }
