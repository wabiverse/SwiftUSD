...
static std::string _Repr(SdfPredicateExpression const &self)
{
  if (!self) {
    return TF_PY_REPR_PREFIX + "PredicateExpression()";
  }
  else {
    return std::string(TF_PY_REPR_PREFIX + "PredicateExpression(") + TfPyRepr(self.GetText()) +
           ")";
  }
}
...
void wrapPredicateExpression()
{
  ...
  scope s = class_<PredExpr>("PredicateExpression")
                ...
                .def(init<std::string, optional<std::string>>(args("exprString", "context")))

                .def(
                    "MakeNot",
                    +[](PredExpr const &r) { return PredExpr::MakeNot(PredExpr(r)); },
                    arg("right"))
                ...
                .def(
                    "__hash__", +[](PredExpr const &e) { return TfHash{}(e); })
                .def(self == self)
                .def(self != self)
...

  {
    scope s = class_<PredExpr::FnCall>("FnCall")
                  ...
                  .def(
                      "__hash__", +[](PredExpr::FnCall const &c) { return TfHash{}(c); })
                  .def(self == self)
                  .def(self != self);
  ...
  class_<PredExpr::FnArg>("FnArg")
      ...
      .add_property(
          "value",
          +[](PredExpr::FnArg const &arg) { return arg.value; },
          +[](PredExpr::FnArg &arg, VtValue const &val) { arg.value = val; })
      .def(
