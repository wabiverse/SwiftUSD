...
static object _WrapGetPrim(UsdviewqHydraObserver &self, const SdfPath &primPath)
{
...
static object _GetPendingNotices(UsdviewqHydraObserver &self)
{
  ...
  for (const UsdviewqHydraObserver::NoticeEntry &noticeEntry : noticeEntries) {
    if (!noticeEntry.added.empty()) {
      list entries;
      for (const HdSceneIndexObserver::AddedPrimEntry &entry : noticeEntry.added) {
    ...
    if (!noticeEntry.removed.empty()) {
      list entries;
      for (const HdSceneIndexObserver::RemovedPrimEntry &entry : noticeEntry.removed) {
    ...
    if (!noticeEntry.dirtied.empty()) {
      list entries;
      for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : noticeEntry.dirtied) {
...
static std::string _WrapLocatorSetAsString(const HdDataSourceLocatorSet &self)
{
...
static list _WrapContainerDataSourceGetNames(const HdContainerDataSourceHandle &self)
{
  list result;
...
static object _CastDataSource(const HdDataSourceBaseHandle &ds)
{
...
static object _WrapContainerDataSourceGet(const HdContainerDataSourceHandle &self,
                                          const TfToken &name)
{
  if (!self) {
    return object();
...
static object _WrapContainerDataSourceGetGetFromLocator(const HdContainerDataSourceHandle &self,
...
size_t _WrapVectorDataSourceGetNumElements(const HdVectorDataSourceHandle &self)
{
  if (!self) {
    return 0;
  }
...
static object _WrapVectorDataSourceGetElement(const HdVectorDataSourceHandle &self, size_t i)
{
  if (!self) {
    return object();
  }
...
static object _WrapSampledDataSourceGetValue(const HdSampledDataSourceHandle &self,
...
static std::string _WrapSampledDataSourceGetTypeString(const HdSampledDataSourceHandle &self)
{
  if (!self) {
    return "";
  }
...
static TfToken _WrapLocatorGetElement(const HdDataSourceLocator &self, size_t i)
{
...
static TfToken _WrapLocatorGetFirstElement(const HdDataSourceLocator &self)
{
...
static TfToken _WrapLocatorGetLastElement(const HdDataSourceLocator &self)
{
  if (self.GetElementCount() == 0) {
    return TfToken();
  }
...
static std::string _WrapHdDataSourceLocatorStr(const HdDataSourceLocator &self)
{
  std::ostringstream buffer;
  buffer << self;
  return buffer.str();
...
static std::string _WrapHdDataSourceLocatorSetStr(const HdDataSourceLocatorSet &self)
{
  std::ostringstream buffer;
  buffer << self;
  return buffer.str();
...
{
  {
    ...
    class_<This>("HydraObserver", init<>())
        .def("GetRegisteredSceneIndexNames", &This::GetRegisteredSceneIndexNames)
...

  {
    ...
    class_<This>("DataSourceLocator", init<>())
        ...
        .def(init<const TfToken &, const TfToken &, const TfToken &, const TfToken &>())
        .def(init<const TfToken &,
                  const TfToken &,
                  const TfToken &,
                  const TfToken &,
