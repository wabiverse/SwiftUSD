...
{
  return TF_PY_REPR_PREFIX + "DualQuath(" + TfPyRepr(self.GetReal()) + ", " +
...
void wrapDualQuath()
{
  object getReal = make_function(&GfDualQuath::GetReal, return_value_policy<return_by_value>());
  object setReal = make_function((void(GfDualQuath::*)(const GfQuath &)) & GfDualQuath::SetReal);

  object getDual = make_function(&GfDualQuath::GetDual, return_value_policy<return_by_value>());
  object setDual = make_function((void(GfDualQuath::*)(const GfQuath &)) & GfDualQuath::SetDual);

  def("Dot", (GfHalf(*)(const GfDualQuath &, const GfDualQuath &))GfDot);

  class_<GfDualQuath> cls("DualQuath", no_init);
  cls.def("__init__", make_constructor(__init__))
      ...
      .def(init<const GfQuath &, const GfQuath &>((arg("real"), arg("dual"))))
      .def(init<const GfQuath &, const GfVec3h &>((arg("rotation"), arg("translation"))))
      ...
      .def("GetNormalized", &GfDualQuath::GetNormalized, (arg("eps") = GF_MIN_VECTOR_LENGTH))
      .def("Normalize",
           &GfDualQuath::Normalize,
           (arg("eps") = GF_MIN_VECTOR_LENGTH),
           return_self<>())

      .def("GetConjugate", &GfDualQuath::GetConjugate)
      .def("GetInverse", &GfDualQuath::GetInverse)
      .def("SetTranslation", (void(GfDualQuath::*)(const GfVec3h &)) & GfDualQuath::SetTranslation)
      .def("GetTranslation", &GfDualQuath::GetTranslation)

      .def("Transform", (GfVec3h(GfDualQuath::*)(const GfVec3h &) const) & GfDualQuath::Transform)
  ...
  to_python_converter<std::vector<GfDualQuath>, TfPySequenceToPython<std::vector<GfDualQuath>>>();
