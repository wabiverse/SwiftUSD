...
static void _ValidateAndPrintNode(std::ostream &out, const PcpNodeRef &node)
{
  TF_AXIOM(node);
...
static void _ValidateAndPrintPrimFromNode(std::ostream &out,
                                          ...
                                          const PcpNodeRef &node)
{
  ...
  TF_AXIOM(node.GetSite().layerStack->GetIdentifier().rootLayer == sdSite.layer);

  out << Pcp_FormatSite(node.GetSite()) << "\t"
      << TfEnum::GetDisplayName(node.GetArcType()).c_str();
}

static std::string _GetRangeType(PcpRangeType type)
{
  return TfEnum::GetDisplayName(type);
}

static void _IterateAndPrintPrimIndexNodes(std::ostream &out,
                                           PcpCache *cache,
                                           const PcpPrimIndex &primIndex,
                                           PcpRangeType type = PcpRangeTypeAll)
{
  out << "Iterating over " << _GetRangeType(type) << " nodes for "
      << TfStringPrintf("<%s>:", primIndex.GetRootNode().GetSite().path.GetText()) << std::endl;
...
static void _IterateAndPrintPrimIndexPrims(std::ostream &out,
                                           PcpCache *cache,
                                           const PcpPrimIndex &primIndex,
                                           PcpRangeType type = PcpRangeTypeAll)
{
...
static void _IterateAndPrintPrimIndex(std::ostream &out,
                                      PcpCache *cache,
...
static void _IterateAndPrintPropertyIndex(std::ostream &out,
                                          PcpCache *cache,
                                          const SdfPath &propPath,
                                          bool localOnly)
{
  PcpErrorVector errors;
  const PcpPropertyIndex &propIndex = cache->ComputePropertyIndex(propPath, &errors);
  PcpRaiseErrors(errors);

  out << "Iterating over " << (localOnly ? "local" : "all") << " property specs for "
      << TfStringPrintf("<%s>:", propPath.GetText()) << std::endl;

  TF_FOR_ALL(it, propIndex.GetPropertyRange(localOnly))
  {
    out << " " << Pcp_FormatSite(PcpSite((*it)->GetLayer(), (*it)->GetPath())) << " from node ";
  ...
  out << "Reverse iterating over " << (localOnly ? "local" : "all") << " property specs for "
      << TfStringPrintf("<%s>:", propPath.GetText()) << std::endl;
...
static void _TestComparisonOperations(IteratorType first, IteratorType last)
{
  TF_AXIOM(first != last);
...
template<class IteratorType>
static void _TestRandomAccessOperations(IteratorType first, IteratorType last)
{
  TF_AXIOM(first != last);
...
int main(int argc, char **argv)
{
  ...
  {
    PcpErrorVector errors;
    const PcpPrimIndex &primIndex = cache->ComputePrimIndex(SdfPath("/Model"), &errors);
    PcpRaiseErrors(errors);
    ...
    const PcpPropertyIndex &propIndex = cache->ComputePropertyIndex(SdfPath("/Model.a"), &errors);
    PcpRaiseErrors(errors);
  ...
  std::cout << "Testing random access operations..." << std::endl;
  {
    ...
    _TestRandomAccessOperations(PcpNodeReverseIterator(nodeRange.second),
                                PcpNodeReverseIterator(nodeRange.first));

    const PcpPrimRange primRange = primIndex.GetPrimRange();
    _TestRandomAccessOperations(primRange.first, primRange.second);
    _TestRandomAccessOperations(PcpPrimReverseIterator(primRange.second),
    ...
    _TestRandomAccessOperations(PcpPropertyReverseIterator(propRange.second),
  ...
  std::cout << "Testing GetNodeIteratorAtNode" << std::endl;
  {
    ...
    const PcpNodeRange nodeRange = primIndex.GetNodeRange();
    for (PcpNodeIterator it = nodeRange.first; it != nodeRange.second; ++it) {
      PcpNodeRef node = *it;
      PcpNodeIterator iteratorAtNode = primIndex.GetNodeIteratorAtNode(node);
      TF_AXIOM(it == iteratorAtNode);
    }

    TF_AXIOM(primIndex.GetNodeIteratorAtNode(PcpNodeRef()) == nodeRange.second);
  }

  std::cout << "Testing iteration (output to file)..." << std::endl;
  {
    std::ofstream outfile("iteration_results.txt");
    for (PcpRangeType type = PcpRangeTypeRoot; type != PcpRangeTypeInvalid;
         type = (PcpRangeType)(type + 1))
    {

      _IterateAndPrintPrimIndex(outfile, cache.get(), SdfPath("/Model"), type);
      outfile << std::endl << "====================" << std::endl << std::endl;
    }

    _IterateAndPrintPropertyIndex(outfile, cache.get(), SdfPath("/Model.a"), /* localOnly */ true);
