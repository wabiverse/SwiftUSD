...
static PcpMapFunction _GetArcFunction(const std::string &source, const std::string &target)
{
...
int main(int argc, char **argv)
{
  ...
  const PcpMapFunction refFunc = _GetArcFunction("/Model", "/World/anim/Model_1");
  ...
  TF_AXIOM(rootIdentityExpr.MapSourceToTarget(SdfPath("/Foo")) == SdfPath("/Foo"));
  ...
  TF_AXIOM(composedExpr.Evaluate() == _GetArcFunction("/Rig", "/World/anim/Model_1/Rig"));
  ...
  // Variable
  {
    ...
    PcpMapExpression::VariableUniquePtr var = PcpMapExpression::NewVariable(PcpMapFunction());
    ...
    // Test changing value
    const PcpMapFunction testValue = _GetArcFunction("/A", "/B");
    ...
    const PcpMapFunction testValue2 = _GetArcFunction("/A2", "/B2");
    ...
    const PcpMapFunction testValue3 = _GetArcFunction("/A3", "/B3");
  ...
  // root identity mapping in the result.
  {
    ...
    const PcpMapExpression exp = PcpMapExpression::Constant(b_to_c).Compose(
        PcpMapExpression::Constant(a_to_b).AddRootIdentity());
    TF_AXIOM(exp.Evaluate() == a_to_c);

    const PcpMapExpression a_to_c_with_id = PcpMapExpression::Constant(a_to_c).AddRootIdentity();
    const PcpMapExpression exp_with_id = exp.AddRootIdentity();
