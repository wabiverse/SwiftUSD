...
#define TEST_PCP_DYNAMIC_FILE_FORMAT_TOKENS \
  ((Id, "Test_PcpDynamicFileFormat"))((Version, "1.0"))((Target, "usd"))( \
      (Extension, "testpcpdynamic"))((Depth, "TestPcp_depth"))((Num, "TestPcp_num"))( \
      (Radius, "TestPcp_radius"))((Height, "TestPcp_height"))((ArgDict, "TestPcp_argDict"))( \
...
TF_DEFINE_PUBLIC_TOKENS(Test_PcpDynamicFileFormatPlugin_FileFormatTokens,
                        TEST_PCP_DYNAMIC_FILE_FORMAT_TOKENS);
...
{
  SDF_DEFINE_FILE_FORMAT(Test_PcpDynamicFileFormatPlugin_FileFormat, SdfFileFormat);
}

Test_PcpDynamicFileFormatPlugin_FileFormat ::Test_PcpDynamicFileFormatPlugin_FileFormat()
    : SdfFileFormat(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Id,
...
bool Test_PcpDynamicFileFormatPlugin_FileFormat::CanRead(const string &filePath) const
{
  return true;
}
...
template<class T>
bool _GetFileFormatArg(SdfFileFormat::FileFormatArguments args,
...
template<class T>
void _SetParamValueInSpec(const SdfPrimSpecHandle &xformSpec,
                          const TfToken &paramName,
                          const T &value)
{
  VtValue val(value);
  if (TfGetEnvSetting(TEST_PCP_DYNAMIC_FILE_FORMAT_TOKENS_USE_ATTRIBUTE_INPUTS)) {
    // Attribute input. Create the attribute spec and set the default value.
    SdfAttributeSpecHandle attr = SdfAttributeSpec::New(
        xformSpec, paramName, SdfGetValueTypeNameForValue(val), SdfVariabilityUniform, true);
...
bool Test_PcpDynamicFileFormatPlugin_FileFormat::Read(SdfLayer *layer,
                                                      ...
                                                      bool metadataOnly) const
{
  ...
  int num = 1;
  _GetFileFormatArg(args, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Num, &num);

  // The number of times to recurse adding "num" transforms at each level
  int depth = 0;
  _GetFileFormatArg(args, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Depth, &depth);
  ...
  _GetFileFormatArg(args, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->PayloadId, &payloadId);

  // At depth 0, we're done recursing. just read in the contents of our file
  // as an sdf text file format into the layer.
  if (depth <= 0) {
    const SdfFileFormatConstPtr fileFormat = SdfFileFormat::FindById(SdfTextFileFormatTokens->Id);
  ...
  SdfPrimSpecHandle geomSpec = SdfPrimSpec::New(rootSpec, std::string("geom"), SdfSpecifierDef);
  ...
  if (depth > 1) {
    ...
    // prims are placed.
    double radius = 3.0;
    _GetFileFormatArg(args, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Radius, &radius);

    // Height is how high the new set of Xform prims are placed above its
    // Root prim.
    double height = 3.0;
    _GetFileFormatArg(args, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Height, &height);
    ...
    for (int i = 0; i < num; ++i) {
      ...
      std::string specName = TfStringPrintf("Xform_%s_%d_%d", payloadId.c_str(), depth - 1, i);
      SdfPrimSpecHandle xformSpec = SdfPrimSpec::New(rootSpec, specName, SdfSpecifierDef, "Xform");
      ...
      _SetParamValueInSpec(xformSpec, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Num, num);
      _SetParamValueInSpec(
          xformSpec, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Height, height);
      // Decrease depth by one. This is the most important as it stops
      // the recursion.
      _SetParamValueInSpec(
          xformSpec, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Depth, depth - 1);
      // Halve the radius
      _SetParamValueInSpec(
          xformSpec, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Radius, radius / 2.0);
      ...
      if (payloadId.empty()) {
        ...
        xformSpec->GetPayloadList().Add(
            SdfPayload(SdfLayer::CreateIdentifier(resolvedPath, newArgs), SdfPath()));
      }
    }
  }
...
bool Test_PcpDynamicFileFormatPlugin_FileFormat::ReadFromString(SdfLayer *layer,
                                                                const std::string &str) const
{
  return true;
}

bool Test_PcpDynamicFileFormatPlugin_FileFormat::WriteToString(const SdfLayer &layer,
                                                               std::string *str,
                                                               const std::string &comment) const
{
  // Write the contents as an sdf text file.
  return SdfFileFormat::FindById(SdfTextFileFormatTokens->Id)->WriteToString(layer, str, comment);
}

bool Test_PcpDynamicFileFormatPlugin_FileFormat::WriteToStream(const SdfSpecHandle &spec,
                                                               std::ostream &out,
                                                               size_t indent) const
{
  // Write the contents as an sdf text file.
  return SdfFileFormat::FindById(SdfTextFileFormatTokens->Id)->WriteToStream(spec, out, indent);
}
                        ...
                        T *outValue)
{
  ...
  if (TfGetEnvSetting(TEST_PCP_DYNAMIC_FILE_FORMAT_TOKENS_USE_ATTRIBUTE_INPUTS)) {
    ...
    if (!context.ComposeAttributeDefaultValue(argName, &val) || !val.IsHolding<T>()) {
      return false;
    }
  }
  else {
    ...
    if (!val.IsHolding<T>()) {
      TF_CODING_ERROR("Expected '%s' value to hold an %s, got '%s'",
                      argName.GetText(),
...
static bool _ExtractPayloadId(const std::string &assetPath, std::string *payloadId)
{
  ...
  return TfMapLookup(args, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->PayloadId, payloadId);
}
...
// particular asset.
static void _ExtractArgDict(const PcpDynamicFileFormatContext &context,
                            ...
                            VtDictionary *dict)
{
  if (TfGetEnvSetting(TEST_PCP_DYNAMIC_FILE_FORMAT_TOKENS_USE_ATTRIBUTE_INPUTS)) {
    ...
    getArgValueForPayloadId(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Depth);
    getArgValueForPayloadId(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Num);
    getArgValueForPayloadId(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Height);
    getArgValueForPayloadId(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Radius);
  }
  else {
    // Compose the "argDict" metadata from the prim field context.
    VtValue value;
    if (!context.ComposeValue(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->ArgDict, &value)) {
      return;
    }
...
void Test_PcpDynamicFileFormatPlugin_FileFormat::ComposeFieldsForFileFormatArguments(
    ...
    VtValue *dependencyContextData) const
{
  ...
  if (_ExtractPayloadId(assetPath, &payloadId)) {
    ...
    customDependencyData[Test_PcpDynamicFileFormatPlugin_FileFormatTokens->PayloadId] = payloadId;
  }
  ...
  int depth = 0;
  if (_ExtractArg(
          Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Depth, context, argDict, &depth))
  {
    if (depth < 1) {
      return;
    }
  ...
  int num = 0;
  if (_ExtractArg(Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Num, context, argDict, &num)) {
    if (num < 1) {
      return;
    }
  }
  (*args)[Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Depth] = TfStringify(depth);
  (*args)[Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Num] = TfStringify(num);

  // Compose the radius and height metadata and add them as well.
  double radius = 10.0;
  if (_ExtractArg(
          Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Radius, context, argDict, &radius))
  {
    (*args)[Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Radius] = TfStringify(radius);
  }
  double height = 0.0;
  if (_ExtractArg(
          Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Height, context, argDict, &height))
  {
    (*args)[Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Height] = TfStringify(height);
  }
}

bool Test_PcpDynamicFileFormatPlugin_FileFormat::CanFieldChangeAffectFileFormatArguments(
    const TfToken &field,
    const VtValue &oldValue,
    const VtValue &newValue,
    const VtValue &dependencyContextData) const
{
  ...
  const VtDictionary &depDataDict = dependencyContextData.UncheckedGet<VtDictionary>();
  ...
  if (field == Test_PcpDynamicFileFormatPlugin_FileFormatTokens->ArgDict) {
    if (!VtDictionaryIsHolding<std::string>(
            depDataDict, Test_PcpDynamicFileFormatPlugin_FileFormatTokens->PayloadId))
    {
      return false;
    }
    ...
    auto getPayloadDict = [&](const VtValue &val) {
      ...
      return VtDictionaryGet<VtDictionary>(
          val.UncheckedGet<VtDictionary>(), payloadId, VtDefault = VtDictionary());
    };
  ...
  if (field == Test_PcpDynamicFileFormatPlugin_FileFormatTokens->Depth) {
    if (oldValue.IsHolding<int>() && newValue.IsHolding<int>()) {
      if (oldValue.UncheckedGet<int>() < 1 && newValue.UncheckedGet<int>() < 1) {
        return false;
      }
    }
...
bool Test_PcpDynamicFileFormatPlugin_FileFormat::
