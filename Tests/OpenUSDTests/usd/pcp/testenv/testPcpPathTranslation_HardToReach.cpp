...
static std::unique_ptr<PcpCache> _CreateCacheForRootLayer(const std::string &rootLayerPath)
{
  ...
  const PcpLayerStackIdentifier layerStackID(rootLayer, SdfLayerRefPtr(), ArResolverContext());
  return std::make_unique<PcpCache>(layerStackID);
}

static void _TestReverseTranslation(const PcpPrimIndex &primIndex,
                                    ...
                                    const SdfPath &expectedReversePath)
{
  std::cout << "Translating " << targetPath << " between "
            ...
            << Pcp_FormatSite(node.GetSite()) << std::endl;

  // Reverse translate targetPath to the given node and compare with the
  // expected result.
  const SdfPath revPath = PcpTranslatePathFromRootToNode(node, targetPath);
  ...
  const SdfPath fwdPath = PcpTranslatePathFromNodeToRoot(node, revPath);
...
static bool _IsExpectedNode(const PcpNodeRef &node,
                            const PcpArcType expectedArcType,
                            const SdfPath &expectedSitePath)
{
  return node.GetArcType() == expectedArcType && node.GetSite().path == expectedSitePath;
}
...
static void TestReverseTranslation_1()
{
  ...
  const PcpPrimIndex &index = pcpCache->ComputePrimIndex(SdfPath("/M_1/B"), &errors);
  TF_AXIOM(errors.empty());

  PcpNodeRange nodeRange = index.GetNodeRange();

  // First node is the root node, which requires no path translation.
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeRoot, SdfPath("/M_1/B")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected */ SdfPath("/M_1/B"));

  // Next node is the propagated relocation.
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeRelocate, SdfPath("/M_1/A")));
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeReference, SdfPath("/M_2/B")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected */ SdfPath("/M_2/B"));

  // Next node is the propagated relocation.
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeRelocate, SdfPath("/M_2/A")));
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeReference, SdfPath("/M_3/B")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected*/ SdfPath("/M_3/B"));
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeRelocate, SdfPath("/M_3/A")));
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeReference, SdfPath("/M_4/A")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected*/ SdfPath("/M_4/A"));
...
static void TestReverseTranslation_2()
{
  std::cout << "========== TestReverseTranslation_2..." << std::endl;
  ...
  PcpErrorVector errors;
  const PcpPrimIndex &index = pcpCache->ComputePrimIndex(SdfPath("/M_1/C"), &errors);
  TF_AXIOM(errors.empty());

  PcpNodeRange nodeRange = index.GetNodeRange();

  // First node is the root node, which requires no path translation.
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeRoot, SdfPath("/M_1/C")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected */ SdfPath("/M_1/B"));
  ...
  // effect on the path translation.
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeReference, SdfPath("/M_2/C")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected */ SdfPath("/M_2/B"));
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeReference, SdfPath("/M_3/C")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected*/ SdfPath("/M_3/B"));
  ...
  // should translate B to A.
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeReference, SdfPath("/M_4/C")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ SdfPath("/M_1/B"),
                          /* expected*/ SdfPath("/M_4/A"));
...
static void TestReverseTranslation_3()
{
  ...
  // First node is the root node, which requires no path translation.
  TF_AXIOM(_IsExpectedNode(*nodeRange.first, PcpArcTypeRoot, primPath));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ targetPath,
                          /* expected */ targetPath);
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(*nodeRange.first,
                           PcpArcTypeInherit,
  ...
  ++nodeRange.first;
  TF_AXIOM(_IsExpectedNode(
      *nodeRange.first, PcpArcTypeReference, SdfPath("/ArmRig/Rig/Some_Internal_Rig_Prim")));
  _TestReverseTranslation(index,
                          *nodeRange.first,
                          /* target */ targetPath,
...
// a coding error.
static void TestErrorsTranslatingInvalidPaths()
{
  std::cout << "========== TestErrorsTranslatingInvalidPaths..." << std::endl;
  ...
  PcpErrorVector errors;
  const PcpPrimIndex &index = pcpCache.ComputePrimIndex(SdfPath("/foo"), &errors);
  TF_AXIOM(errors.empty());
