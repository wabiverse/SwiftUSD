...
UsdStageRefPtr CreateTestData(double timeShift = 0)
{
  // --------------------------------------------------------------------- //
  ...
  TF_VERIFY(
      UsdGeomXform::Define(stage, fooPath), "Failed to create prim at %s", fooPath.GetText());
  ...
  TF_VERIFY(
      UsdGeomXform::Define(stage, barPath), "Failed to create prim at %s", barPath.GetText());

  UsdPrim root(stage->GetPrimAtPath(rootPrimPath));
  TF_VERIFY(root, "Failed to get Prim from %s", rootPrimPath.GetText());

  UsdPrim foo(stage->GetPrimAtPath(fooPath));
  TF_VERIFY(foo, "Failed to get Prim from %s", fooPath.GetText());

  UsdPrim fooBar(stage->GetPrimAtPath(fooBarPath));
  TF_VERIFY(fooBar, "Failed to get Prim from %s", fooBarPath.GetText());

  UsdPrim fooBarBaz(stage->GetPrimAtPath(fooBarBazPath));
  TF_VERIFY(fooBarBaz, "Failed to get Prim from %s", fooBarBazPath.GetText());

  UsdPrim bar(stage->GetPrimAtPath(barPath));
  TF_VERIFY(bar, "Failed to get Prim from %s", barPath.GetText());
...
void VerifyTransforms(UsdStageRefPtr const &stage, UsdGeomXformCache &xfCache, GfMatrix4d xform)
{
  GfMatrix4d ctm;
  ...
  TF_VERIFY(
      ctm == xform, "LocalToWorldTransform value for %s is incorrect.", root.GetPath().GetText());
  ...
  TF_VERIFY(
      ctm == xform, "ParentToWorldTransform value for %s is incorrect.", foo.GetPath().GetText());
  ...
  TF_VERIFY(
      ctm == xform, "ParentToWorldTransform value for %s is incorrect.", bar.GetPath().GetText());
  ...
  ctm = xfCache.ComputeRelativeTransform(root, stage->GetPseudoRoot(), &resetXformStack);
  TF_VERIFY(ctm == xform,
