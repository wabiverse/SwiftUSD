...
static void TestCallback()
{
  ...
  SdfPrimSpecHandle strongPrim = SdfCreatePrimInLayer(layer1, SdfPath("/Root"));
  ...
  SdfPrimSpecHandle weakPrim = SdfCreatePrimInLayer(layer2, SdfPath("/Root"));
  ...
  UsdUtilsStitchValueFn maybeMergeTimeSamples = [layer1, layer2](const TfToken &field,
                                                                 const SdfPath &path,
    ...
    if (field == SdfFieldKeys->TimeSamples) {
      ...
      SdfAttributeSpecHandle attrSpec = weakLayer->GetAttributeAtPath(path);
      ...
      VtDictionary customData = strongLayer->GetAttributeAtPath(path)->GetCustomData();
      int numStitched = VtDictionaryGet<int>(customData, "num_stitched", VtDefault = 0);
  ...
  TF_AXIOM((strongAttr->GetCustomData() == VtDictionary{{"num_stitched", VtValue(1)}}));
  ...
  UsdUtilsStitchLayers(layer1, layer2, maybeMergeTimeSamples);
  TF_AXIOM(layer1->QueryTimeSample(strongAttr->GetPath(), 2.0));
  TF_AXIOM((strongAttr->GetCustomData() == VtDictionary{{"num_stitched", VtValue(2)}}));
}
