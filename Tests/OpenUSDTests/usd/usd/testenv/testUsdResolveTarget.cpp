...
static void _VerifyResolveTargetSite(const PcpNodeRef &node, const PcpSite &expectedSite)
{
...
// Helper for _VerifyResolveTarget
static void _VerifyResolverTargetLayer(const SdfLayerHandle &layer,
...
static void _VerifyResolveTarget(const UsdResolveTarget &resolveTarget,
                                 const std::pair<PcpSite, std::string> &expectedStart,
                                 const std::pair<PcpSite, std::string> *expectedStop = nullptr)
{
  _VerifyResolveTargetSite(resolveTarget.GetStartNode(), expectedStart.first);
  _VerifyResolverTargetLayer(resolveTarget.GetStartLayer(), expectedStart.second);

  if (expectedStop) {
    _VerifyResolveTargetSite(resolveTarget.GetStopNode(), expectedStop->first);
    _VerifyResolverTargetLayer(resolveTarget.GetStopLayer(), expectedStop->second);
  }
  else {
...
// overload.
template<class T>
static void _GetAndVerifyAttributeValue(const UsdAttributeQuery &attrQuery,
...
template<class T> using _ExpectedTimeSamples = std::vector<std::pair<double, T>>;
...
template<class T> using _ExpectedAttrGetValues = std::pair<_ExpectedTimeSamples<T>, VtValue>;
...
template<class T>
static void _VerifyQuery(const UsdAttributeQuery &attrQuery,
                         const _ExpectedAttrGetValues<T> &expectedValues)
{
  // Extract the list of expected time sample values.
  const _ExpectedTimeSamples<T> &expectedTimeSampleValues = expectedValues.first;
  ...
  if (!expectedDefaultVtValue.IsEmpty()) {
    TF_VERIFY(expectedDefaultVtValue.IsHolding<T>(),
              ...
              "templated type.");
    expectedAuthoredDefaultValue = &(expectedDefaultVtValue.UncheckedGet<T>());
  }

  // We expect HasAuthoredValue() to return true if we expect either time
  // samples or a default value.
  const bool expectedHasAuthoredValue = !expectedTimeSampleValues.empty() ||
  ...
  // value.
  _GetAndVerifyAttributeValue(attrQuery, UsdTimeCode::Default(), expectedAuthoredDefaultValue);
  // Verify that calling Get at each expected time sample time returns the
  // expect time sample value.
  for (const auto &timeAndVal : expectedTimeSampleValues) {
    _GetAndVerifyAttributeValue(attrQuery, timeAndVal.first, &(timeAndVal.second));
  }
  ...
  if (expectedTimeSampleValues.empty()) {
    _GetAndVerifyAttributeValue(attrQuery, 1.0, expectedAuthoredDefaultValue);
  }
}
...
template<class T>
static void _MakeAndVerifyQueries(const UsdAttribute &attr,
                                  const std::vector<UsdResolveTarget> &resolveTargets,
                                  const std::vector<_ExpectedAttrGetValues<T>> expectedValues)
{
  std::cout << "\n** Start: Make and verify queries for attribute " << attr.GetPath() << " **"
            << std::endl;
  ...
  std::cout << "** SUCCESS: Make and verify queries for attribute " << attr.GetPath() << " **"
            << std::endl;
}
...
// its prim index.
static void _GetAllResolveTargetsForPrim(const UsdPrim &prim,
                                         ...
                                         std::vector<UsdResolveTarget> *strongThanResolveTargets)
{
  ...
  for (const UsdPrimCompositionQueryArc &arc : arcs) {
    const SdfLayerRefPtrVector &layers = arc.GetTargetNode().GetLayerStack()->GetLayers();
    for (const SdfLayerHandle &layer : layers) {

      upToResolveTargets->push_back(arc.MakeResolveTargetUpTo(layer));
      strongThanResolveTargets->push_back(arc.MakeResolveTargetStrongerThan(layer));
    }
  }
}
...
static void _TestGetAttrValueWithResolveTargets()
{
  ...
  PcpLayerStackIdentifier rootLayerStackId(
      stage->GetRootLayer(), stage->GetSessionLayer(), stage->GetPathResolverContext());
  ...
  PcpLayerStackIdentifier refLayerStackId(refLayer, nullptr, stage->GetPathResolverContext());

  // Get all the possible resolve targets for the child prim.
  std::vector<UsdResolveTarget> upToResolveTargets;
  std::vector<UsdResolveTarget> strongThanResolveTargets;
  _GetAllResolveTargetsForPrim(childPrim, &upToResolveTargets, &strongThanResolveTargets);
  ...
  for (size_t i = 0; i < expectedTargets.size(); ++i) {
    ...
    _VerifyResolveTarget(strongThanResolveTargets[i], expectedTargets[0], &expectedTargets[i]);
  }
  ...
  TF_AXIOM(fooAttr);
  _MakeAndVerifyQueries<float>(fooAttr,
                               upToResolveTargets,
                               {// Node: /Parent/RefChild
                                {{}, VtValue(6.0f)},
                                {{}, VtValue(6.0f)},
  ...
  TF_AXIOM(varAttr);
  _MakeAndVerifyQueries<int>(varAttr,
                             upToResolveTargets,
                             {// Node: /Parent/RefChild
                              {{{1.0, 6}, {6.0, 1}}, VtValue()},
                              {{{1.0, 6}, {6.0, 1}}, VtValue()},
  ...
  TF_AXIOM(barAttr);
  _MakeAndVerifyQueries<int>(barAttr,
                             upToResolveTargets,
                             {// Node: /Parent/RefChild
  ...
  TF_AXIOM(sub1Attr);
  _MakeAndVerifyQueries<TfToken>(
      sub1Attr,
      upToResolveTargets,
      {// Node: /Parent/RefChild
       {{{1.0, TfToken("sub1_1")}, {5.0, TfToken("sub1_5")}}, VtValue(TfToken("sub1_def"))},
       {{{1.0, TfToken("sub1_1")}, {5.0, TfToken("sub1_5")}}, VtValue(TfToken("sub1_def"))},
       {{{1.0, TfToken("sub1_1")}, {5.0, TfToken("sub1_5")}}, VtValue(TfToken("sub1_def"))},
       {{}, VtValue()},
  ...
  _MakeAndVerifyQueries<TfToken>(refSub1Attr,
                                 upToResolveTargets,
                                 {// Node: /Parent/RefChild
  ...
  // Test creating resolve targets from edit targets
  {
    ...
    UsdResolveTarget upToEditTarget = childPrim.MakeResolveTargetUpToEditTarget(editTarget);
    UsdResolveTarget strongerThanEditTarget = childPrim.MakeResolveTargetStrongerThanEditTarget(
        editTarget);
    ...
    _VerifyResolveTarget(strongerThanEditTarget, expectedTargets[0], &expectedTargets[3]);
    ...
    //  ...
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, upToEditTarget), {{}, VtValue(4.0f)});
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, strongerThanEditTarget), {{}, VtValue(6.0f)});
    ...
    // anyway.
    upToEditTarget = childPrim.MakeResolveTargetUpToEditTarget(editTarget);
    strongerThanEditTarget = childPrim.MakeResolveTargetStrongerThanEditTarget(editTarget);
    ...
    //  ...
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, upToEditTarget), {{}, VtValue(10.0f)});
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, strongerThanEditTarget), {{}, VtValue(6.0f)});
  }

  {
    ...
    PcpNodeRef internalRefNode = parentPrim.GetPrimIndex().GetNodeProvidingSpec(
        stage->GetRootLayer(), SdfPath("/InternalRef"));
    UsdEditTarget editTarget(SdfLayer::Find("./resolveTarget/sub2.usda"), internalRefNode);
    ...
    _VerifyResolveTarget(strongerThanEditTarget, expectedTargets[0], &expectedTargets[7]);
    ...
    //  ref_sub1.usda: /RefParent/RefChild -> 2.0
    //  ref_sub2.usda: /RefParent/RefChild -> 1.0
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, upToEditTarget), {{}, VtValue(3.0f)});
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, strongerThanEditTarget), {{}, VtValue(6.0f)});
    ...
    //  ref_sub1.usda: /RefParent/RefChild -> 2.0
    //  ref_sub2.usda: /RefParent/RefChild -> 1.0
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, upToEditTarget), {{}, VtValue(20.0f)});
    _VerifyQuery<float>(UsdAttributeQuery(fooAttr, strongerThanEditTarget), {{}, VtValue(6.0f)});
  }
}
