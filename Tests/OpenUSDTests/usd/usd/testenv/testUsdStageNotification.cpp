...
struct _NoticeTester : public TfWeakBase {
  _NoticeTester(const UsdStageWeakPtr &stage) : _stage(stage), _invokedTestFunctions(false)
  {
    TfNotice::Register(TfCreateWeakPtr(this), &_NoticeTester::_Handle, stage);
  }
  ...
  template<class Fn> void AddTest(const Fn &testFn)
  {
    _testFns.push_back(testFn);
  }

 private:
  void _Handle(const UsdNotice::ObjectsChanged &n, const UsdStageWeakPtr &sender)
  {
    printf("Received notice.\n");

    printf("Resynced paths:\n");
    auto resyncedPaths = n.GetResyncedPaths();
    for (auto it = resyncedPaths.begin(), end = resyncedPaths.end(); it != end; ++it) {
      printf("  - %s\n", it->GetText());
    ...
    for (auto it = changedInfoPaths.begin(), end = changedInfoPaths.end(); it != end; ++it) {
      printf("  - %s\n", it->GetText());
      printf("    changed fields: %s\n", TfStringify(it.GetChangedFields()).c_str());
    }
...
void TestObjectsChanged()
{
  ...
  // Add a new override prim, assert that it's considered a resync.
  {
    ...
    tester.AddTest([stage](Notice const &n) {
      return TF_AXIOM(n.GetChangedFields(SdfPath("/over")).empty()) &&
             TF_AXIOM(!n.HasChangedFields(SdfPath("/over")));
    });
    UsdEditContext context(stage, stage->GetEditTargetForLocalLayer(subLayer));
  ...
  UsdPrim over = stage->GetPrimAtPath(SdfPath("/over"));
  {
    ...
    tester.AddTest([over](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(over)) && TF_AXIOM(n.ChangedInfoOnly(over));
    });
    tester.AddTest([stage](Notice const &n) {
      return TF_AXIOM(n.GetChangedFields(SdfPath("/over")).empty()) &&
             TF_AXIOM(!n.HasChangedFields(SdfPath("/over")));
  ...
  UsdPrim foo = stage->OverridePrim(SdfPath("/foo"));
  {
    ...
    _NoticeTester tester(stage);
    tester.AddTest([foo](Notice const &n) { return TF_AXIOM(n.ResyncedObject(foo)); });
    tester.AddTest([foo](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(foo)) &&
             TF_AXIOM(n.GetChangedFields(foo) == TfTokenVector{SdfFieldKeys->TypeName});
    });
  ...
  UsdPrim bar = stage->OverridePrim(SdfPath("/foo/bar"));
  {
    ...
    tester.AddTest([foo, bar](Notice const &n) {
      return TF_AXIOM(n.ResyncedObject(foo)) && TF_AXIOM(n.ResyncedObject(bar));
    });
    tester.AddTest([foo, bar](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(foo)) &&
             TF_AXIOM(n.GetChangedFields(foo) == TfTokenVector{SdfFieldKeys->TypeName}) &&
             TF_AXIOM(!n.HasChangedFields(bar)) && TF_AXIOM(n.GetChangedFields(bar).empty());
    });
    ...
    tester.AddTest([foo](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(foo)) &&
             TF_AXIOM(n.GetChangedFields(foo) ==
                      (TfTokenVector{SdfFieldKeys->Documentation, SdfFieldKeys->TypeName}));
    });
    {
      SdfChangeBlock block;
      rootLayer->GetPrimAtPath(SdfPath("/foo"))->SetTypeName("Sphere");
      rootLayer->GetPrimAtPath(SdfPath("/foo"))->SetDocumentation("Test docs");
    }
  }
  ...
  // Assert that changing bar doesn't resync foo.
  {
    ...
    tester.AddTest([foo, bar](Notice const &n) {
      return TF_AXIOM(!n.HasChangedFields(foo)) &&
             TF_AXIOM(n.GetChangedFields(foo).empty() && TF_AXIOM(n.HasChangedFields(bar)) &&
                      TF_AXIOM(n.GetChangedFields(bar) == TfTokenVector{SdfFieldKeys->TypeName}));
    });
  ...
  UsdPrim target1 = stage->OverridePrim(SdfPath("/target1")),
          target2 = stage->OverridePrim(SdfPath("/target2"));
  foo.GetReferences().AddReference(rootLayer->GetIdentifier(), target1.GetPath());
  {
    printf(
        "adding reference target1 -> target2 should resync target1 and "
        "foo, but not target2\n");
    _NoticeTester tester(stage);
    tester.AddTest([target1, target2, foo, bar](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(target2)) && TF_AXIOM(n.ResyncedObject(target1)) &&
             TF_AXIOM(n.ResyncedObject(foo)) && TF_AXIOM(n.ResyncedObject(bar));
    });
    tester.AddTest([target1, target2, foo, bar](Notice const &n) {
      return TF_AXIOM(!n.HasChangedFields(target2)) &&
             TF_AXIOM(n.GetChangedFields(target2).empty()) &&
             TF_AXIOM(!n.HasChangedFields(target1)) &&
             TF_AXIOM(n.GetChangedFields(target1).empty()) && TF_AXIOM(!n.HasChangedFields(foo)) &&
    ...
    target1.GetReferences().AddReference(rootLayer->GetIdentifier(), target2.GetPath());
  }
  ...
  bar.GetInherits().AddInherit(cls.GetPath());
  {
    ...
    tester.AddTest([foo, bar, cls](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(foo)) && TF_AXIOM(!n.ResyncedObject(bar)) &&
             TF_AXIOM(!n.ResyncedObject(cls)) && TF_AXIOM(n.AffectedObject(foo)) &&
             TF_AXIOM(n.AffectedObject(bar)) && TF_AXIOM(n.AffectedObject(cls)) &&
             TF_AXIOM(n.ChangedInfoOnly(foo)) && TF_AXIOM(n.ChangedInfoOnly(bar)) &&
             TF_AXIOM(n.ChangedInfoOnly(cls));
    });

    tester.AddTest([foo, bar, cls](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(foo)) &&
             TF_AXIOM(n.GetChangedFields(foo) == TfTokenVector{SdfFieldKeys->Documentation}) &&
             TF_AXIOM(n.HasChangedFields(bar)) &&
             TF_AXIOM(n.GetChangedFields(bar) == TfTokenVector{SdfFieldKeys->Documentation}) &&
             TF_AXIOM(n.HasChangedFields(cls)) &&
             TF_AXIOM(n.GetChangedFields(cls) == TfTokenVector{SdfFieldKeys->Documentation});
    });
  ...
  bar.GetSpecializes().AddSpecialize(specialize.GetPath());
  {
    ...
    tester.AddTest([foo, bar, specialize](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(foo)) && TF_AXIOM(!n.ResyncedObject(bar)) &&
             TF_AXIOM(!n.ResyncedObject(specialize)) && TF_AXIOM(n.AffectedObject(foo)) &&
             TF_AXIOM(n.AffectedObject(bar)) && TF_AXIOM(n.AffectedObject(specialize)) &&
             TF_AXIOM(n.ChangedInfoOnly(foo)) && TF_AXIOM(n.ChangedInfoOnly(bar)) &&
  ...
  // through as info changes rather than resyncs.
  {
    ...
    _NoticeTester tester(stage);
    tester.AddTest([foo](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(foo)) && TF_AXIOM(n.ChangedInfoOnly(foo)) &&
  ...
  {
    printf(
        "Setting prim doc and typename in one go should cause a "
        "resync\n");
    _NoticeTester tester(stage);
    tester.AddTest([foo](Notice const &n) {
      return TF_AXIOM(n.ResyncedObject(foo)) && TF_AXIOM(!n.ChangedInfoOnly(foo)) &&
    ...
    });
    {
      ...
      rootLayer->GetPrimAtPath(SdfPath("/foo"))->SetDocumentation("Cubicle");
    }
  }
  ...
  UsdAttribute attr;
  {
    ...
    tester.AddTest([](Notice const &n) {
      return TF_AXIOM(SdfPathVector(n.GetResyncedPaths()) ==
                      SdfPathVector{SdfPath("/foo.attr")}) &&
             TF_AXIOM(n.GetChangedInfoOnlyPaths().empty());
    });
    tester.AddTest([](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(SdfPath("/foo.attr"))) &&
  ...
  // Assert that changing an attribute value causes info changes.
  {
    ...
    tester.AddTest([attr](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(attr)) && TF_AXIOM(n.ChangedInfoOnly(attr)) &&
             TF_AXIOM(n.AffectedObject(attr)) && TF_AXIOM(n.GetResyncedPaths().empty()) &&
             TF_AXIOM(SdfPathVector(n.GetChangedInfoOnlyPaths()) ==
                      SdfPathVector{SdfPath("/foo.attr")});
    });
    tester.AddTest([attr](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(attr)) &&
             TF_AXIOM(n.GetChangedFields(attr) == TfTokenVector{SdfFieldKeys->Default});
    });
    attr.Set(42);
  }
  ...
  UsdRelationship rel;
  {
    ...
    _NoticeTester tester(stage);
    tester.AddTest([](Notice const &n) {
      return TF_AXIOM(SdfPathVector(n.GetResyncedPaths()) == SdfPathVector{SdfPath("/foo.rel")}) &&
             TF_AXIOM(n.GetChangedInfoOnlyPaths().empty());
    });
    tester.AddTest([](Notice const &n) {
      return TF_AXIOM(n.HasChangedFields(SdfPath("/foo.rel"))) &&
             TF_AXIOM(n.GetChangedFields(SdfPath("/foo.rel")) ==
                      (TfTokenVector{SdfFieldKeys->Custom, SdfFieldKeys->Variability}));
    });
  ...
  // Assert that setting a relationship target causes info changes.
  {
    ...
    tester.AddTest([rel](Notice const &n) {
      return TF_AXIOM(!n.ResyncedObject(rel)) && TF_AXIOM(n.ChangedInfoOnly(rel)) &&
             TF_AXIOM(n.AffectedObject(rel)) && TF_AXIOM(n.GetResyncedPaths().empty()) &&
             TF_AXIOM(SdfPathVector(n.GetChangedInfoOnlyPaths()) ==
                      SdfPathVector{SdfPath("/foo.rel")});
    });
    tester.AddTest([rel](Notice const &n) {
      return TF_AXIOM(!n.HasChangedFields(rel)) && TF_AXIOM(n.GetChangedFields(rel).empty());
      // XXX: Sdf currently does not report affected fields for
      // certain types of changes. Once that's fixed, these test
      // cases can replace the ones above.
      //
  ...
  {
    printf(
        "Changing load rules, population mask, interpolation type "
        "should notify\n");

    {
      _NoticeTester tester(stage);
      tester.AddTest([](Notice const &n) { return TF_AXIOM(!n.GetResyncedPaths().empty()); });
