...
inline static void _Add(std::string *result, const std::string &s)
{
  (*result) += s;
  // XXX You can add a diagnostic print statement here.
}

static void _DumpResults(const UsdStageRefPtr &stage, const SdfPath &path, std::string *result)
{
  if (UsdPrim prim = stage->Load(path)) {
    _Add(result,
         TfStringPrintf("%s: %s\n", path.GetString().c_str(), prim.GetTypeName().GetText()));
    ...
    if (path != SdfPath::AbsoluteRootPath()) {
      ...
      for (const auto &vsetName : vsetNames) {
        ...
        _Add(result, TfStringPrintf("\\tselection: %s\n", vset.GetVariantSelection().c_str()));
      }
    }
...
static _Result _ComputeResult(const std::string &inputAssetPath)
{
  _Result result;
...
static void _AddTestCase(const std::string &assetPath)
{
...
static void _WorkTask(size_t msecsToRun, bool runForever)
{
  TfStopwatch sw;
  int count = 0;
...
int main(int argc, char const **argv)
{
  ...
  app.add_option("--msec", msecsToRun, "Milliseconds to run")->default_val(10 * 1000);
  ...
  WorkWithScopedDispatcher([&](WorkDispatcher &wd) {
    ...
    for (size_t i = 0; i < numThreads; ++i) {
      wd.Run(
          [localMsecsToRun, localRunForever]() { _WorkTask(localMsecsToRun, localRunForever); });
    }
  });
  ...
  // Report.
  printf(
      "Ran %d operations total, paritioned over %zu thread%s in %.3f sec "
      "(%.3f ops/sec)\n",
      (int)_nIters,
      numThreads,
      numThreads > 1 ? "s" : "",
