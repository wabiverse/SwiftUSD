...
void TestInstancing_1()
{
  ...
  const SdfPath &instancePathToUnset = (sourcePrimIndexPath.GetName() == "Instance0") ?
                                           SdfPath("/instancer1/Instance1") :
                                           SdfPath("/instancer1/Instance0");

  SdfLayerRefPtr rootLayerPtr = stage->GetRootLayer();
  SdfLayerRefPtr subLayerPtr = SdfLayer::FindOrOpen(rootLayerPtr->GetSubLayerPaths()[0]);

  SdfPrimSpecHandle instancePrimToUnset = subLayerPtr->GetPrimAtPath(instancePathToUnset);
  ...
  SdfPrimSpecHandle dummpPrim = SdfCreatePrimInLayer(anonymousLayer, SdfPath("/dummy"));
  {
    ...
    rootLayerPtr->SetSubLayerPaths(
        {subLayerPtr->GetIdentifier(), anonymousLayer->GetIdentifier()});
  }

  const UsdPrim &newPrototypePrim = stage->GetPrimAtPath(sourcePrimIndexPath).GetPrototype();
...
void TestInstancing_2()
{
  std::string rootLayer = "./secondRoot.usda";

  // determine which instance to update
  UsdStageRefPtr stage = UsdStage::Open(rootLayer);
  const UsdPrim &prototypePrim = stage->GetPrimAtPath(SdfPath("/Ref1/instance1")).GetPrototype();

  const SdfPath sourcePrimIndexPath =
      Usd_PrimGetSourcePrimIndex(prototypePrim).GetRootNode().GetPath();

  const SdfPath &instancePathToUnset = (sourcePrimIndexPath.GetName() == "instance1") ?
  ...
  SdfPrimSpecHandle ref1PrimSpec = stage->GetRootLayer()->GetPrimAtPath(SdfPath("/Ref1"));
  SdfPrimSpecHandle instancePrimToUnset = stage->GetRootLayer()->GetPrimAtPath(
      instancePathToUnset);

  SdfPrimSpecHandle dummyPrim = SdfCreatePrimInLayer(stage->GetRootLayer(), SdfPath("/dummy"));
...
void TestInstancing_3()
{
  ...
  SdfPrimSpecHandle refPrimSpec = stage->GetRootLayer()->GetPrimAtPath(parentPathToRecompose);
