...
void TestTemplates()
{
  ...
  UsdAttribute attr = prim.CreateAttribute(prop, SdfValueTypeNames->Double3Array);
  ...
  TF_VERIFY(attr.GetMetadata(metaField, &value),
            ...
            propPath.c_str());
  TF_VERIFY(value.IsHolding<bool>(), "Metadata -- VtValue: not holding bool%s", propPath.c_str());
  TF_VERIFY(value.Get<bool>(), "Metadata -- VtValue: value was not true %s", propPath.c_str());
  ...
  TF_VERIFY(attr.GetMetadata(metaField, &valueIn),
            ...
            propPath.c_str());
  TF_VERIFY(valueIn, "Metadata -- bool: value was not true %s", propPath.c_str());
  ...
  TF_VERIFY(
      attr.Set(VtValue(vtVecOut)), "VtValue: Failed to set property at %s", propPath.c_str());
  ...
  // Verify the result.
  TF_VERIFY(attr.Get(&value), "VtValue: Failed to get property value at %s", propPath.c_str());
  TF_VERIFY(
      value.IsHolding<VtVec3dArray>(), "VtValue: not holding VtVec3dArray %s", propPath.c_str());
  ...
  TF_VERIFY(attr.Set(vtVecOut), "Failed to set property at %s", propPath.c_str());
  ...
  // Verify Result.
  TF_VERIFY(attr.Get(&vtVecIn), "VtArray: Failed to get property value at %s", propPath.c_str());
  TF_VERIFY(vtVecIn[0] == vtVecOut[0], "VtArray: VtVec3d[0] does not match %s", propPath.c_str());
  ...
  TF_VERIFY(UsdAttributeQuery(attr).Get(&queryVtVecIn),
            ...
            propPath.c_str());
  TF_VERIFY(
      queryVtVecIn[0] == vtVecOut[0], "VtArray: VtVec3d[0] does not match %s", propPath.c_str());
