...
void TestValueMutation(std::string const &layerTag)
{
  ...
  {
    double d;
    TF_VERIFY(!attr.Get<double>(&d));
  }
  {
    double d;
    TF_VERIFY(!attr.Get<double>(&d, 1.0));
  }
  {
    double d;
    TF_VERIFY(!attr.Get<double>(&d, 2.0));
  }
