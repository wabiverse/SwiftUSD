...
static std::string _GetStringByStreamInsertion(const UsdTimeCode timeCode)
{
...
static UsdTimeCode _GetTimeCodeByStreamExtraction(const std::string &value,
...
int main(int argc, char *argv[])
{
  ...
  const std::string numericValueString = TfStringPrintf("%.f", defaultTimeValue);
  ...
  testTime = _GetTimeCodeByStreamExtraction(UsdTimeCodeTokens->DEFAULT.GetString(), numericTime);
  TF_AXIOM(testTime == defaultTime);

  testTime = _GetTimeCodeByStreamExtraction(UsdTimeCodeTokens->EARLIEST.GetString(), numericTime);
