...
/// \class _TestResolver
///
class _TestResolver : public ArResolver {
 public:
  _TestResolver()
  {
  ...
  static void SetAssetPathsForConfig(const std::string &configName,
                                     const AssetNameToPathMap &assetNamesToPaths)
  {
    _pendingConfigNameToAssetPathMap[configName] = assetNamesToPaths;
  }

  static void SetVersionForConfig(const std::string &configName, const std::string &version)
  {
    _pendingConfigNameToVersionMap[configName] = version;
  }

  static const AssetNameToPathMap &GetAssetPathsForConfig(const std::string &configName)
  {
    static const AssetNameToPathMap empty;
    const AssetNameToPathMap *result = TfMapLookupPtr(_configNameToAssetPathMap, configName);
    return result ? *result : empty;
  }

  static const std::string &GetVersionForConfig(const std::string &configName)
  {
    static const std::string empty;
    const std::string *result = TfMapLookupPtr(_configNameToVersionMap, configName);
    return result ? *result : empty;
  }

 protected:
  std::string _CreateIdentifier(const std::string &assetPathIn,
                                const ArResolvedPath &anchorAssetPath) const final
  {
    std::string assetPath;
    if (const _TestResolverContext *ctx = _GetCurrentContextObject<_TestResolverContext>()) {

      // If this asset path exists in the asset path map, just
      // return it as-is; we'll return the associated path in _Resolve.
      const AssetNameToPathMap &assetPathMap = GetAssetPathsForConfig(ctx->configName);
      if (assetPathMap.find(assetPathIn) != assetPathMap.end()) {
        return assetPathIn;
      }

      // Otherwise replace the {version} string and fall through.
      assetPath = TfStringReplace(assetPathIn, "{version}", GetVersionForConfig(ctx->configName));
    }
    else {
    ...
    if (anchorAssetPath) {
      assetPath = TfStringCatPaths(TfGetPathName(anchorAssetPath), assetPath);
    }
  ...
  ArResolvedPath _Resolve(const std::string &assetPath) const final
  {
    ...
    if (const _TestResolverContext *ctx = _GetCurrentContextObject<_TestResolverContext>()) {
      ...
      if (const std::string *filePath = TfMapLookupPtr(assetPathMap, assetPath)) {
  ...
  void _RefreshContext(const ArResolverContext &context) final
  {
    ...
    auto pendingMapIter = _pendingConfigNameToAssetPathMap.find(ctx->configName);
    if (pendingMapIter != _pendingConfigNameToAssetPathMap.end()) {
      _configNameToAssetPathMap[ctx->configName] = std::move(pendingMapIter->second);
      _pendingConfigNameToAssetPathMap.erase(pendingMapIter);
      didChange = true;
    }

    auto pendingVersionIter = _pendingConfigNameToVersionMap.find(ctx->configName);
    if (pendingVersionIter != _pendingConfigNameToVersionMap.end()) {
      _configNameToVersionMap[ctx->configName] = std::move(pendingVersionIter->second);
  ...
  bool _IsContextDependentPath(const std::string &assetPath) const final
  {
    ...
    return TfStringContains(assetPath, "{version}") || TfGetExtension(assetPath).empty();
  }
 ...
 private:
  using _ConfigNameToAssetPathMap = std::unordered_map<std::string, AssetNameToPathMap>;
  ...
  using _ConfigNameToVersionMap = std::unordered_map<std::string, std::string>;
...
_TestResolver::_ConfigNameToAssetPathMap _TestResolver::_configNameToAssetPathMap;

_TestResolver::_ConfigNameToAssetPathMap _TestResolver::_pendingConfigNameToAssetPathMap;

_TestResolver::_ConfigNameToVersionMap _TestResolver::_configNameToVersionMap;

_TestResolver::_ConfigNameToVersionMap _TestResolver::_pendingConfigNameToVersionMap;

// ------------------------------------------------------------

class _TestResolverPluginImpl : public _TestResolverPluginInterface {
 public:
  ...
  void SetVersionForConfig(const std::string &configName, const std::string &version) final
  {
    _TestResolver::SetVersionForConfig(configName, version);
  }
};

PLUG_REGISTER_INTERFACE_SINGLETON_TYPE(_TestResolverPluginInterface, _TestResolverPluginImpl)
