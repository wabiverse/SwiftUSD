...
void TestTargetSpecs()
{
  for (const std::string &fmt : {"usda", "usdc"}) {
    UsdStageRefPtr stage = UsdStage::CreateInMemory("TestTargetSpecs." + fmt);

    UsdPrim prim = stage->DefinePrim(SdfPath("/Test"));

    UsdAttribute attr = prim.CreateAttribute(TfToken("attr"), SdfValueTypeNames->Int);
...
{
  for (const std::string &fmt : {"usda", "usdc"}) {
    UsdStageRefPtr stage = UsdStage::CreateInMemory("TestGetTargets." + fmt);

    // Add an attribute to test connections first.
    UsdPrim attrPrim = stage->DefinePrim(SdfPath("/TestAttr"));
    UsdAttribute attr = attrPrim.CreateAttribute(TfToken("attr"), SdfValueTypeNames->Int);
    SdfPathVector conns;
    ...
    UsdRelationship forwardingRel = relPrim.CreateRelationship(TfToken("forwardingRel"));
    ...
    UsdRelationship refForwardingRel = refPrim.GetRelationship(TfToken("forwardingRel"));
    ...
    TF_AXIOM(!refForwardingRel.GetForwardedTargets(&targets));
    TF_AXIOM(targets == SdfPathVector({SdfPath("/TestAttr.attr"), SdfPath("/TestAttr.dummy")}));
    ...
    TF_AXIOM(forwardingRel.GetTargets(&targets));
    TF_AXIOM(targets == SdfPathVector({SdfPath("/TestRel.rel"), SdfPath("/TestAttr.dummy")}));
...
static void _CheckNoSpecForOpaqueValues(const std::string &formatExtension)
{
  printf("%s %s\n", TF_FUNC_NAME().c_str(), formatExtension.c_str());

  const UsdStageRefPtr stage = UsdStage::CreateNew("test_opaque_values." + formatExtension);
  const SdfLayerHandle layer = stage->GetRootLayer();

  const UsdPrim prim = stage->DefinePrim(SdfPath("/Prim"));
  const UsdAttribute attr = prim.CreateAttribute(TfToken("attr"), SdfValueTypeNames->Opaque);

  TF_AXIOM(!attr.HasAuthoredValue());

  layer->SetField(SdfPath("/Prim.attr"), SdfFieldKeys->Default, SdfOpaqueValue());
