...
static UsdAttribute _CreateTestAttrOneAttr(UsdContrivedMultipleApplyAPI &self,
                                           object defaultVal,
                                           bool writeSparsely)
{
...
static UsdAttribute _CreateTestAttrTwoAttr(UsdContrivedMultipleApplyAPI &self,
                                           object defaultVal,
                                           bool writeSparsely)
{
...
static bool _WrapIsMultipleApplyAPIPath(const SdfPath &path)
{
  ...
  return UsdContrivedMultipleApplyAPI::IsMultipleApplyAPIPath(path, &collectionName);
}

static std::string _Repr(const UsdContrivedMultipleApplyAPI &self)
{
  std::string primRepr = TfPyRepr(self.GetPrim());
  std::string instanceName = self.GetName();
  return TfStringPrintf(
      "UsdContrived.MultipleApplyAPI(%s, '%s')", primRepr.c_str(), instanceName.c_str());
}

struct UsdContrivedMultipleApplyAPI_CanApplyResult : public TfPyAnnotatedBoolResult<std::string> {
...
static UsdContrivedMultipleApplyAPI_CanApplyResult _WrapCanApply(const UsdPrim &prim,
                                                                 const TfToken &name)
{
  std::string whyNot;
...
void wrapUsdContrivedMultipleApplyAPI()
{
  ...
  class_<This, bases<UsdAPISchemaBase>> cls("MultipleApplyAPI");
  ...
  cls.def(init<UsdPrim, TfToken>())
      ...
      .def("Get",
           (UsdContrivedMultipleApplyAPI(*)(const UsdStagePtr &stage, const SdfPath &path)) &
               This::Get,
           (arg("stage"), arg("path")))
      .def("Get",
           (UsdContrivedMultipleApplyAPI(*)(const UsdPrim &prim, const TfToken &name)) & This::Get,
           (arg("prim"), arg("name")))
      .staticmethod("Get")

      .def("GetAll",
           (std::vector<UsdContrivedMultipleApplyAPI>(*)(const UsdPrim &prim)) & This::GetAll,
           arg("prim"),
