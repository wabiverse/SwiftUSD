...
static void Check(const UsdPrim &parent, char const *prefix, size_t depth)
{
  UsdStagePtr stage = parent.GetStage();
  for (size_t i = 0; i != numSiblings; ++i) {
    UsdPrim p = stage->GetPrimAtPath(
        parent.GetPath().AppendChild(TfToken(TfStringPrintf("%s_%zu", prefix, i))));
    TF_AXIOM(p);
    for (size_t j = 0; j != numAttrs; ++j) {
      UsdAttribute a = p.GetAttribute(TfToken(TfStringPrintf("%s_%zu", prefix, j)));
      TF_AXIOM(a);
      int value;
...
static void Author(const UsdPrim &parent, char const *prefix, size_t depth)
{
  UsdStagePtr stage = parent.GetStage();
  for (size_t i = 0; i != numSiblings; ++i) {
    UsdPrim p = stage->DefinePrim(
        parent.GetPath().AppendChild(TfToken(TfStringPrintf("%s_%zu", prefix, i))));
    for (size_t j = 0; j != numAttrs; ++j) {
      UsdAttribute a = p.CreateAttribute(TfToken(TfStringPrintf("%s_%zu", prefix, j)),
...
static void AddStage(const UsdStageRefPtr &stage)
{
  allStages->push_back(stage);
}

static vector<UsdStageRefPtr> GetAllStages()
{
...
static void TestParallelAuthoring()
{
