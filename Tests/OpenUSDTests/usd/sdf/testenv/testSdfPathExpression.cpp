...
static SdfPath SdfPathExpressionObjectToPath(SdfPath const &path)
{
  return path;
}

static SdfPath SdfPathExpressionPathToObject(SdfPath const &path, SdfPath const *)
{
  return path;
}
...
static void TestBasics()
{
  ...
  {
    auto eval = SdfMakePathExpressionEval(SdfPathExpression("/foo//bar"), predLib);
  ...
  {
    auto eval = SdfMakePathExpressionEval(SdfPathExpression("//foo/bar/baz/qux/quux"), predLib);
  ...
  {
    auto eval = SdfMakePathExpressionEval(SdfPathExpression("/foo*//bar{isPrimPath}"), predLib);
  ...
  {
    auto eval = SdfMakePathExpressionEval(SdfPathExpression("/a /b /c /d/e/f"), predLib);
  ...
  {
    auto eval = SdfMakePathExpressionEval(SdfPathExpression("/a// - /a/b/c"), predLib);
  ...
  {
    auto eval = SdfMakePathExpressionEval(SdfPathExpression("/a//{isPropertyPath} - /a/b.c"),
                                          predLib);
...

  {
    ...
    auto eval = SdfMakePathExpressionEval(composed, predLib);
...

  {
    ...
    auto resolveRefs = [&](SdfPathExpression::ExpressionReference const &ref) {
      ...
      else if (ref.name == "foo") {
        return foo;
      }
      else if (ref.name == "bar") {
        return bar;
      }
    ...
    auto eval = SdfMakePathExpressionEval(resolved, predLib);
...

  {
    ...
    // ReplacePrefix.
    {
      SdfPathExpression home = abs.ReplacePrefix(SdfPath("/World"), SdfPath("/Home"));

      auto eval = SdfMakePathExpressionEval(home, predLib);
