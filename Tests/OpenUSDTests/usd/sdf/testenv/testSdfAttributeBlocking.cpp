...
int main(int argc, char **argv)
{
  ...
  auto defAttr = SdfAttributeSpec::New(prim, visToken, SdfValueTypeNames->Token);
  defAttr->SetDefaultValue(VtValue("visible"));

  auto sampleAttr = SdfAttributeSpec::New(prim, "xformOp:transform", SdfValueTypeNames->Double);
  ...
  for (size_t i = 101; i < 105; ++i) {
    ...
    layer->SetTimeSample<SdfValueBlock>(sampleAttr->GetPath(), sample, SdfValueBlock());
    TF_AXIOM(layer->QueryTimeSample<SdfValueBlock>(sampleAttr->GetPath(), sample, &typedValue));
    TF_AXIOM(value.IsHolding<SdfValueBlock>());

    // Ensure that improperly calling get(mismatched types)
    // both returns false and does not throw any sort of errors.
    TfErrorMark errors;
    TF_AXIOM(!layer->QueryTimeSample<double>(sampleAttr->GetPath(), sample, &illTypedValue));
  ...
  layer->SetField<SdfValueBlock>(defAttr->GetPath(), SdfFieldKeys->Default, typedValue);
  TF_AXIOM(layer->HasField<SdfValueBlock>(defAttr->GetPath(), SdfFieldKeys->Default, &typedValue));
