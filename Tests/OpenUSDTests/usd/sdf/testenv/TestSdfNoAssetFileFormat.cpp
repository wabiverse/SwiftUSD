...
#define TEST_SDF_NO_ASSET_FILE_FORMAT_TOKENS \
  ((Extension, "testsdfnoasset"))((RootName, "rootName"))

TF_DECLARE_PUBLIC_TOKENS(TestSdfNoAssetFileFormat_Tokens, TEST_SDF_NO_ASSET_FILE_FORMAT_TOKENS);

TF_DEFINE_PUBLIC_TOKENS(TestSdfNoAssetFileFormat_Tokens, TEST_SDF_NO_ASSET_FILE_FORMAT_TOKENS);
...
class TestSdfNoAssetFileFormat : public SdfTextFileFormat {
 public:
  ...
  TestSdfNoAssetFileFormat() : SdfTextFileFormat(TestSdfNoAssetFileFormat_Tokens->Extension)
  {
    // Do Nothing.
  }
  ...
  bool Read(SdfLayer *layer, const std::string &resolvedPath, bool metadataOnly) const override
  {
    ...
    if (const std::string *rootName = TfMapLookupPtr(args,
                                                     TestSdfNoAssetFileFormat_Tokens->RootName))
    {
...
{
  SDF_DEFINE_FILE_FORMAT(TestSdfNoAssetFileFormat, SdfTextFileFormat);
}
