...
static void DoUnitTest()
{
  ...
  result = table.insert(make_pair(SdfPath("/foo/anim/chars/AngusGroup/Angus"), "Angus"));
  TF_AXIOM(result.second);
  ...
  // Find subtree range
  {
    ...
    TF_AXIOM(static_cast<size_t>(std::distance(range.first, range.second)) == table.size());
...
static void ReadPaths(string const &fileName, vector<SdfPath> *out)
{
  ...
  sw.Stop();
  printf("building paths took %f sec\n", sw.GetSeconds());
