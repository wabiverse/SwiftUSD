...
static void _TestSdfLayerDictKeyOps()
{
  std::string txt;
  ...
  layer->SetFieldDictValueByKey(
      fooPath, SdfFieldKeys->CustomData, TfToken("hello:world"), std::string("value"));
  ...
  TF_AXIOM(layer->GetFieldDictValueByKey(
               fooPath, SdfFieldKeys->CustomData, TfToken("hello:world")) == VtValue("value"));

  // Erase the key through the Sdf API.
  layer->EraseFieldDictValueByKey(fooPath, SdfFieldKeys->CustomData, TfToken("hello:world"));

  TF_AXIOM(layer->GetField(fooPath, SdfFieldKeys->CustomData).IsEmpty());
}

static void _TestSdfLayerTimeSampleValueType()
{
  SdfLayerRefPtr layer = SdfLayer::CreateAnonymous();
  SdfPrimSpecHandle foo = SdfCreatePrimInLayer(layer, SdfPath("/foo"));
  SdfAttributeSpecHandle attr = SdfAttributeSpec::New(foo, "attr", SdfValueTypeNames->Double);
...
static void _TestSdfLayerTransferContents()
{
  ...
  struct _ChangeListener : public TfWeakBase {
   public:
    _ChangeListener()
    {
      TfNotice::Register(TfCreateWeakPtr(this), &_ChangeListener::OnChangeNotice);
    }
  ...
  auto iter = std::find_if(
      ...
      l.changeListVec.end(),
      [&dstLayer](SdfLayerChangeListVec::value_type const &p) { return p.first == dstLayer; });
...
static void _TestSdfRelationshipTargetSpecEdits()
{
...
static void _TestSdfPathFindLongestPrefix()
{
  std::vector<SdfPath> paths = {SdfPath("/"),
                                SdfPath("/foo"),
                                SdfPath("/foo/bar/baz"),
                                SdfPath("/bar/foo"),
                                SdfPath("/bar/baz"),
  ...
  TF_AXIOM(*SdfPathFindLongestPrefix(paths.begin(), paths.end(), SdfPath("/foo/bar/baz/qux")) ==
           SdfPath("/foo/bar/baz"));
  TF_AXIOM(*SdfPathFindLongestPrefix(paths.begin(), paths.end(), SdfPath("/foo/baz/baz/qux")) ==
           SdfPath("/foo"));
  TF_AXIOM(*SdfPathFindLongestPrefix(paths.begin(), paths.end(), SdfPath("/bar/foo")) ==
           SdfPath("/bar/foo"));
  TF_AXIOM(*SdfPathFindLongestPrefix(paths.begin(), paths.end(), SdfPath("/qux/foo/bar")) ==
           SdfPath("/qux"));
  TF_AXIOM(*SdfPathFindLongestPrefix(paths.begin(), paths.end(), SdfPath("/qix")) == SdfPath("/"));

  TF_AXIOM(
      *SdfPathFindLongestStrictPrefix(paths.begin(), paths.end(), SdfPath("/foo/bar/baz/qux")) ==
      SdfPath("/foo/bar/baz"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(
               paths.begin(), paths.end(), SdfPath("/foo/baz/baz/qux")) == SdfPath("/foo"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(paths.begin(), paths.end(), SdfPath("/bar/foo")) ==
           SdfPath("/"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(paths.begin(), paths.end(), SdfPath("/qux/foo/bar")) ==
           SdfPath("/qux"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(paths.begin(), paths.end(), SdfPath("/qix")) ==
           SdfPath("/"));
  ...
  TF_AXIOM(*SdfPathFindLongestPrefix(pathSet, SdfPath("/foo/bar/baz/qux")) ==
           SdfPath("/foo/bar/baz"));
  TF_AXIOM(*SdfPathFindLongestPrefix(pathSet, SdfPath("/foo/baz/baz/qux")) == SdfPath("/foo"));
  TF_AXIOM(*SdfPathFindLongestPrefix(pathSet, SdfPath("/bar/foo")) == SdfPath("/bar/foo"));
  TF_AXIOM(*SdfPathFindLongestPrefix(pathSet, SdfPath("/qux/foo/bar")) == SdfPath("/qux"));
  TF_AXIOM(*SdfPathFindLongestPrefix(pathSet, SdfPath("/qix")) == SdfPath("/"));

  TF_AXIOM(*SdfPathFindLongestStrictPrefix(pathSet, SdfPath("/foo/bar/baz/qux")) ==
           SdfPath("/foo/bar/baz"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(pathSet, SdfPath("/foo/baz/baz/qux")) ==
           SdfPath("/foo"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(pathSet, SdfPath("/bar/foo")) == SdfPath("/"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(pathSet, SdfPath("/qux/foo/bar")) == SdfPath("/qux"));
  TF_AXIOM(*SdfPathFindLongestStrictPrefix(pathSet, SdfPath("/qix")) == SdfPath("/"));
  ...
  TF_AXIOM(SdfPathFindLongestPrefix(pathMap, SdfPath("/foo/bar/baz/qux"))->first ==
           SdfPath("/foo/bar/baz"));
  TF_AXIOM(SdfPathFindLongestPrefix(pathMap, SdfPath("/foo/baz/baz/qux"))->first ==
           SdfPath("/foo"));
  TF_AXIOM(SdfPathFindLongestPrefix(pathMap, SdfPath("/bar/foo"))->first == SdfPath("/bar/foo"));
  TF_AXIOM(SdfPathFindLongestPrefix(pathMap, SdfPath("/qux/foo/bar"))->first == SdfPath("/qux"));
  TF_AXIOM(SdfPathFindLongestPrefix(pathMap, SdfPath("/qix"))->first == SdfPath("/"));

  TF_AXIOM(SdfPathFindLongestStrictPrefix(pathMap, SdfPath("/foo/bar/baz/qux"))->first ==
           SdfPath("/foo/bar/baz"));
  TF_AXIOM(SdfPathFindLongestStrictPrefix(pathMap, SdfPath("/foo/baz/baz/qux"))->first ==
           SdfPath("/foo"));
  TF_AXIOM(SdfPathFindLongestStrictPrefix(pathMap, SdfPath("/bar/foo"))->first == SdfPath("/"));
  TF_AXIOM(SdfPathFindLongestStrictPrefix(pathMap, SdfPath("/qux/foo/bar"))->first ==
           SdfPath("/qux"));
  TF_AXIOM(SdfPathFindLongestStrictPrefix(pathMap, SdfPath("/qix"))->first == SdfPath("/"));
}

static void _TestSdfFpsAndTcps()
{
  ...
  class _ChangeListener : public TfWeakBase {
   public:
    _ChangeListener(const SdfLayerHandle &layer) : _layer(layer)
    {
      TfNotice::Register(TfCreateWeakPtr(this), &_ChangeListener::OnInfoChange, layer);
      TfNotice::Register(TfCreateWeakPtr(this), &_ChangeListener::OnLayerChange, layer);
    }
    ...
    void ValidateAndClear(const VtValue &oldFps,
                          const VtValue &newFps,
                          const VtValue &oldTcps,
                          const VtValue &newTcps)
    {
      // Verify fields have expected new values.
      TF_AXIOM(_layer->GetFramesPerSecond() == newFps.GetWithDefault(24.0));
      TF_AXIOM(_layer->GetTimeCodesPerSecond() == newTcps.GetWithDefault(24.0));
      ...
      const auto &fpsIt = entry.FindInfoChange(SdfFieldKeys->FramesPerSecond);
      if (newFps != oldFps) {
        TF_AXIOM(fpsIt != entry.infoChanged.end() && fpsIt->second.first == oldFps &&
      ...
      // with expected old and new values.
      const auto &tcpsIt = entry.FindInfoChange(SdfFieldKeys->TimeCodesPerSecond);
      if (newTcps != oldTcps) {
        TF_AXIOM(tcpsIt != entry.infoChanged.end() && tcpsIt->second.first == oldTcps &&
  ...
  listener.ValidateAndClear(VtValue(), VtValue(30.0), VtValue(), VtValue(30.0));

  // Add TCPS, verify fields take on separate values.
  layer->SetTimeCodesPerSecond(1000);
  listener.ValidateAndClear(VtValue(30.0), VtValue(30.0), VtValue(30.0), VtValue(1000.0));

  // Change FPS, verify only FPS changes.
  layer->SetFramesPerSecond(48);
  listener.ValidateAndClear(VtValue(30.0), VtValue(48.0), VtValue(1000.0), VtValue(1000.0));

  // Remove TCPS, verify return to dynamic fallback from FPS.
  layer->ClearTimeCodesPerSecond();
  listener.ValidateAndClear(VtValue(48.0), VtValue(48.0), VtValue(1000.0), VtValue(48.0));

  // Remove FPS, verify return to initial state.
  layer->ClearFramesPerSecond();
  listener.ValidateAndClear(VtValue(48.0), VtValue(), VtValue(48.0), VtValue());
}

static void _TestSdfSchemaPathValidation()
{
  ...
  TF_AXIOM(!schema.IsValidReference(SdfReference("a.sdf", SdfPath("A"))));
  TF_AXIOM(!schema.IsValidReference(SdfReference("a.sdf", SdfPath("/A{x=y}"))));
  TF_AXIOM(!schema.IsValidReference(SdfReference("a.sdf", SdfPath("/A{x=y}B"))));
