...
static TfToken _GetRandomNameToken()
{
  return (*nameTokens)[rand() % nameTokens->size()];
}

static SdfPath _MakeRandomPrimPath()
{
...
static SdfPath _MakeRandomPrimOrPropertyPath()
{
  SdfPath ret = _MakeRandomPrimPath();
  return rand() & 1 ? ret : ret.AppendProperty(_GetRandomNameToken());
}

static SdfPath _MakeRandomPath(SdfPath const &path = SdfPath::AbsoluteRootPath())
{
  SdfPath ret = path;
  ...
  if ((ret.IsPrimPath() || ret.IsPrimVariantSelectionPath())) {
    return (rand() & 1) ? ret : _MakeRandomPath(ret.AppendProperty(_GetRandomNameToken()));
  }
  ...
  if (ret.IsPrimPropertyPath()) {
    ...
    switch (rand() & 3) {
      case 0:
        return _MakeRandomPath(ret.AppendTarget(_MakeRandomPrimOrPropertyPath()));
      case 1:
        return _MakeRandomPath(ret.AppendMapper(_MakeRandomPrimOrPropertyPath()));
      case 2:
  ...
  if (ret.IsTargetPath()) {
    return (rand() & 1) ? ret :
                          _MakeRandomPath(ret.AppendRelationalAttribute(_GetRandomNameToken()));
  }

  // Extend a MapperPath
  if (ret.IsMapperPath()) {
    return (rand() & 1) ? ret : _MakeRandomPath(ret.AppendMapperArg(_GetRandomNameToken()));
  }

  // Extend a RelationalAttributePath
  if (ret.IsRelationalAttributePath()) {
    return (rand() & 1) ? ret : _MakeRandomPath(ret.AppendTarget(_MakeRandomPrimOrPropertyPath()));
  }
...
int main(int argc, char const **argv)
{
  ...
  app.add_option("--seed", randomSeed, "Random seed")->default_val(time(NULL));
  app.add_option("--numThreads", numThreads, "Number of threads to use")
      ->default_val(std::thread::hardware_concurrency());
  app.add_option("--msec", msecsToRun, "Milliseconds to run")->default_val(2000);
  ...
  // Report.
  printf(
      "Ran %d SdfPath operations on %zu thread%s in %.3f sec "
      "(%.3f ops/sec)\n",
      (int)nIters,
      numThreads,
      numThreads > 1 ? "s" : "",
