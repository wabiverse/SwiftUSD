...
template<class Data> class TestContextObject {
 public:
  TestContextObject(const Data &data) : _data(data) {}
...
static void TestSingleContextObject()
{
  ...
  const TestStringContextObject *strObjFromCtx = ctx1.Get<TestStringContextObject>();
  TF_AXIOM(strObjFromCtx);
  TF_AXIOM(strObjFromCtx->GetData() == strContextObj.GetData());

  const TestIntContextObject *intObjFromCtx = ctx1.Get<TestIntContextObject>();
...
static void TestMultipleContextObjects()
{
  // Create an ArResolverContext holding multiple context objects
  // and verify those objects are accessible.
  ArResolverContext context(TestStringContextObject("test string"), TestIntContextObject(42));
  TF_AXIOM(!context.IsEmpty());
  TF_AXIOM(context != ArResolverContext());

  const TestStringContextObject *strObjFromContext = context.Get<TestStringContextObject>();
  TF_AXIOM(strObjFromContext);
  TF_AXIOM(strObjFromContext->GetData() == "test string");

  const TestIntContextObject *intObjFromContext = context.Get<TestIntContextObject>();
  ...
  {
    ArResolverContext testContext(*intObjFromContext, *strObjFromContext);
  ...
  {
    ArResolverContext testContext(*intObjFromContext, ArResolverContext(*strObjFromContext));
    TF_AXIOM(context == testContext);
    TF_AXIOM(!(context < testContext));
    TF_AXIOM(!(testContext < context));
    TF_AXIOM(hash_value(context) == hash_value(testContext));
  ...
  {
    ArResolverContext testContext(ArResolverContext{*intObjFromContext},
  ...
  {
    ArResolverContext testContext(ArResolverContext{strObj}, ArResolverContext{intObj});
    TF_AXIOM(context != testContext);
  }

  {
    ArResolverContext testContext(strObj, ArResolverContext{intObj});
    TF_AXIOM(context != testContext);
  }
}
