...
static void TestOpenAsset()
{
  ...
  // working directory.
  std::string tmpPath;
  int fd = ArchMakeTmpFile(ArchGetCwd(), "testArDefaultResolver_CPP", &tmpPath);
  TF_AXIOM(fd != -1);

  FILE *tmpFile = ArchFdOpen(fd, "w");
  TF_AXIOM(tmpFile);

  const std::string contents = "Test file generated by testArDefaultResolver_CPP";
  fputs(contents.c_str(), tmpFile);
  fclose(tmpFile);

  // Now, try to open it via ArResolver::OpenAsset and check
  // that the contents are what we expect.
  std::shared_ptr<ArAsset> asset = ArGetResolver().OpenAsset(ArResolvedPath(tmpPath));
  TF_AXIOM(asset);
  TF_AXIOM(dynamic_cast<ArFilesystemAsset *>(asset.get()));

  const size_t assetSize = asset->GetSize();
  TF_AXIOM(assetSize == static_cast<size_t>(ArchGetFileLength(tmpPath.c_str())));
