...
static void _CountThreads(size_t begin, size_t end)
{
...
static size_t _ExpectedLimit(const int envVal, const size_t n)
{
  // If envVal is non-zero, it wins over n!
  // envVal may also be a negative number, which means all but that many
  // cores.
  return envVal ?
             (envVal < 0 ? std::max<int>(1, envVal + WorkGetPhysicalConcurrencyLimit()) : envVal) :
             n;
}

static void _TestThreadLimit(const int envVal, const size_t n)
{
...
static void _TestArguments(const int envVal)
{
...
int main(int argc, char **argv)
{
  ...
  if ((argc == 2) && (strcmp(argv[1], "--rawtbb") == 0)) {
    ...
    _uniqueThreads->clear();
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, 100000), _RawTBBCounter(), tbb::simple_partitioner());
    std::cout << "   default TBB used " << _uniqueThreads->size() << " threads\n";

    if (envVal == 0) {
      if (_uniqueThreads->size() < WorkGetPhysicalConcurrencyLimit()) {
        TF_FATAL_ERROR(
            "tbb only used %zu threads when it should be "
      ...
      TF_FATAL_ERROR(
          "tbb used %zu threads, which is greater than the "
  ...
  if (_uniqueThreads->size() > limit) {
    TF_FATAL_ERROR(
        "it appears as though libWork hasn't been initialized "
  ...
  WorkSetMaximumConcurrencyLimit();
  TF_AXIOM(WorkGetConcurrencyLimit() == _ExpectedLimit(envVal, WorkGetPhysicalConcurrencyLimit()));
  ...
  TF_AXIOM(WorkGetConcurrencyLimit() == _ExpectedLimit(envVal, 2));
  _TestThreadLimit(envVal, 2);

  // Test with 4 threads.
  std::cout << "Testing with 4 threads...\n";
  WorkSetConcurrencyLimit(4);
  TF_AXIOM(WorkGetConcurrencyLimit() == _ExpectedLimit(envVal, 4));
