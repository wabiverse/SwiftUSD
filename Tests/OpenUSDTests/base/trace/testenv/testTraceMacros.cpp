...
int main(int argc, char *argv[])
{
  ...
  TraceAggregateNodeRefPtr threadNode = reporter->GetAggregateTreeRoot()->GetChild("Main Thread");
  TF_AXIOM(threadNode);
  ...
  TraceAggregateNodeRefPtr innerScopeNode = scopeNode->GetChild("TestMacros (Inner Scope)");
  TF_AXIOM(innerScopeNode);

  const TraceReporter::CounterMap &counters = reporter->GetCounters();
  TraceReporter::CounterMap::const_iterator it = counters.find(TfToken("Counter A"));
  ...
  const TraceEventTree::MarkerValuesMap &markers = timeline->GetMarkers();
  TraceEventTree::MarkerValuesMap::const_iterator it2 = markers.find(TfToken("Marker A"));
  TF_AXIOM(it2 != markers.end());
  TraceEventTree::MarkerValuesMap::const_iterator it3 = markers.find(TfToken("Dynamic Marker 1"));
