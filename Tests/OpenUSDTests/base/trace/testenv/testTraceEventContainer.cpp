...
static std::shared_ptr<TraceEventList> CreateInnerOnlyTestEvents()
{
  ...
  events->EmplaceBack(
      TraceEvent::Marker, events->CacheKey("Test Marker"), 3 * ms, TraceCategory::Default);
...
static std::shared_ptr<TraceEventList> CreateUseOuterTestEvents()
{
  const TraceEvent::TimeStamp ms = 1;
...
static std::shared_ptr<TraceEventList> CreateAppendedList()
{
  const TraceEvent::TimeStamp ms = 1;
  std::shared_ptr<TraceEventList> events(new TraceEventList);
  ...
  for (int j = 0; j < numAppends; ++j) {
    ...
    for (int i = 0; i < numEvents; i++) {
      otherEvents.EmplaceBack(
          TraceEvent::Timespan,
          otherEvents.CacheKey("Timespan " + std::to_string(j) + ", " + std::to_string(i)),
          i * ms,
          (i + 1) * ms,
          TraceCategory::Default);
    }
...
static void _TestForwardIteration(const std::shared_ptr<TraceEventList> &eventList)
{

  std::cout << "    Forward" << std::endl;
  for (TraceEventList::const_iterator iter = eventList->begin(); iter != eventList->end(); ++iter)
  {
...
static void _TestReverseIteration(const std::shared_ptr<TraceEventList> &eventList)
{
...
int main(int argc, char *argv[])
{
  ...
  std::shared_ptr<TraceEventList> emptyEvents = std::make_shared<TraceEventList>();
