...
static void TestTimelineCounterValues(const TfToken &counterName,
                                      const std::vector<double> desiredValues)
{
  TraceReporterPtr reporter = TraceReporter::GetGlobalReporter();
  TraceEventTreeRefPtr timeline = reporter->GetEventTree();
  TF_AXIOM(timeline);

  const TraceEventTree::CounterValuesMap &counters = timeline->GetCounters();
  TraceEventTree::CounterValuesMap::const_iterator it = counters.find(counterName);
...
static void TestAggregateCounterValue(const TfToken &counterName, const double desiredValue)
{
  TraceReporterPtr reporter = TraceReporter::GetGlobalReporter();
  const TraceReporter::CounterMap &counters = reporter->GetCounters();
  TraceReporter::CounterMap::const_iterator it = counters.find(counterName);
  TF_AXIOM(it != counters.end());
  TF_AXIOM(it->second == desiredValue);
}

static void TestAggregateCounterDelta(const TfToken &counterName, const double desiredValue)
{
  TraceReporterPtr reporter = TraceReporter::GetGlobalReporter();
