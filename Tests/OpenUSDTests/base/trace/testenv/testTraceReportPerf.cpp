...
void WriteStats(FILE *file, const std::string &name, const TfStopwatch &timer)
{
  fprintf(file,
...
std::shared_ptr<TraceCollection> CreateTrace(int N, int R)
{
  ...
  std::shared_ptr<TraceCollection> collection = dataSrc->ConsumeData()[0];
...
int main(int argc, char *argv[])
{
  ...
  for (int R : recrusionSizes) {
    ...
    for (size_t i = 0; i < maxTestSize; ++i) {
      ...
      watch.Stop();
      std::cout << "Create Trace    N: " << size << " time: " << watch.GetSeconds()
                << " scopes/msec: " << float(size) / watch.GetMilliseconds() << std::endl;
      auto reporter = TraceReporter::New("Test",
                                         TraceReporterDataSourceCollection::New(collection));
      ...
      watch.Stop();
      WriteStats(statsFile, TfStringPrintf("trace trees R %d N %d", R, size), watch);
      std::cout << "Trace Trees N: " << size << " time: " << watch.GetSeconds()
                << " scopes/msec: " << float(size) / watch.GetMilliseconds() << std::endl;
    }
  }
  fclose(statsFile);
