...
static void WriteStats(FILE *file, const char *name, const TfStopwatch &timer)
{
  fprintf(file,
...
int main(int argc, char *argv[])
{
  ...
  std::cout << "Cost per scope(ns): " << (traceTime - noTraceTime) / double(N) * 1e9 << std::endl;
  std::cout << "Cost per tickTime(ns): " << (tickTime - noTraceTime) / double(N) * 1e9
            << std::endl;
  std::cout << "Cost per push_back(ns): " << (pushTickTime - noTraceTime) / double(N) * 1e9
            << std::endl;

  double diffTick = (traceTime - tickTime) / tickTime * 100.0;
  double diffDisabled = (traceDisableTime - noTraceTime) / noTraceTime * 100.0;
  std::cout << "trace %diff untimed: " << diffDisabled << std::endl;
  std::cout << "trace %diff tick: " << diffTick << std::endl;
  std::cout << "trace %diff push_back: " << (traceTime - pushTickTime) / pushTickTime * 100.0
            << std::endl;

  fprintf(
      statsFile, "{'profile':'trace_vs_tick','metric':'time','value':%f,'samples':1}\n", diffTick);
  fprintf(statsFile,
