...
constexpr TraceCategoryId TestCategory = TraceCategory::CreateTraceCategoryId("TestCategory");

static std::unique_ptr<TraceEventList> CreateTestEvents(TraceEvent::TimeStamp timeStampOffset)
{
  ...
  {
    TraceEvent counterEvent(TraceEvent::CounterDelta, counterKey, 1, TraceCategory::Default);
    counterEvent.SetTimeStamp(2 * ms + timeStampOffset);
    events->EmplaceBack(std::move(counterEvent));
  }

  events->EmplaceBack(TraceEvent::Begin,
                      events->CacheKey("Inner Scope 2"),
                      3 * ms + timeStampOffset,
                      TestCategory);
  events->EmplaceBack(
      TraceEvent::End, events->CacheKey("Inner Scope 2"), 4 * ms + timeStampOffset, TestCategory);

  {
    bool data = true;
    TraceEvent dataEvent(
        TraceEvent::Data, events->CacheKey("Test Data 0"), data, TraceCategory::Default);
  ...
  }
  {
    int data = -2;
    TraceEvent dataEvent(
        TraceEvent::Data, events->CacheKey("Test Data 1"), data, TraceCategory::Default);
  ...
  }
  {
    uint64_t data = ~0;
    TraceEvent dataEvent(
        TraceEvent::Data, events->CacheKey("Test Data 2"), data, TraceCategory::Default);
  ...
  }
  {
    double data = 1.5;
    TraceEvent dataEvent(
        TraceEvent::Data, events->CacheKey("Test Data 3"), data, TraceCategory::Default);
    dataEvent.SetTimeStamp(8 * ms + timeStampOffset);
    events->EmplaceBack(std::move(dataEvent));
  }
  {
    std::string data = "String Data";
    TraceEvent dataEvent(TraceEvent::Data,
  ...
  events->EmplaceBack(TraceEvent::Timespan,
                      keyInner,
                      ms + timeStampOffset,
  ...
  {
    TraceEvent counterEvent(TraceEvent::CounterValue, counterKey, -1, TraceCategory::Default);
  ...
  events->EmplaceBack(TraceEvent::Marker,
...
static std::unique_ptr<TraceCollection> CreateTestCollection(float startTimeSec = 0.0)
{
...
static void _TestSerialization(const std::vector<std::shared_ptr<TraceCollection>> &testCols,
                               std::string fileName)
{
...
{
  TraceCategory::GetInstance().RegisterCategory(TestCategory, "Test Category");
