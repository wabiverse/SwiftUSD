...
#  define TRACE_FUNCTION_ARGS(...) \
    TRACE_FUNCTION_ARGS_INSTANCE( \
        __LINE__, __ARCH_FUNCTION__, __ARCH_PRETTY_FUNCTION__, __VA_ARGS__)
#endif
...
/// key value pairs.
#  define TRACE_SCOPE_ARGS(name, ...) TRACE_SCOPE_ARGS_INSTANCE(__LINE__, name, __VA_ARGS__)
#endif
...
#define TRACE_DATA(name, value) TraceCollector::GetInstance().StoreData(name, value);

#define TRACE_FUNCTION_ARGS_INSTANCE(instance, name, prettyName, ...) \
  constexpr static TraceStaticKeyData BOOST_PP_CAT(TraceKeyData_, instance)(name, prettyName); \
  ...
  TraceScopeAuto BOOST_PP_CAT(TraceScopeAuto_, instance)( \
      ...
      _TRACE_ARGS_TO_FUNC_PARAMS(BOOST_PP_CAT(TraceKeyData_, instance), __VA_ARGS__));
...
#define _TRACE_ARGS_TO_KEY_SEQ(...) \
  BOOST_PP_SEQ_TRANSFORM(_TRACE_KEY_FROM_TUPLE, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))

#define _TRACE_ARGS_TO_VAL_SEQ(...) \
  BOOST_PP_SEQ_TRANSFORM(_TRACE_VALUE_FROM_TUPLE, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))
...
#define _TRACE_ARGS_TO_STATIC_VARS(varname, ...) \
  BOOST_PP_SEQ_FOR_EACH_I(_TRACE_KEY_KEY_DEF, varname, _TRACE_ARGS_TO_KEY_SEQ(__VA_ARGS__))
...
#define _TRACE_ARGS_TO_FUNC_PARAMS(varname, ...) \
  BOOST_PP_SEQ_FOR_EACH_I(_TRACE_ARG_FUNC_PARAM, varname, _TRACE_ARGS_TO_VAL_SEQ(__VA_ARGS__))
...
TraceEventNodeRefPtr FindNode(TraceEventNodeRefPtr root, const std::string &name)
{
  if (root->GetKey() == name) {
    return root;
  }
...
int main(int argc, char *argv[])
{
  ...
  {
    TraceEventNodeConstRefPtr funcNode = FindNode(reporter->GetEventRoot(), "TestFunc");
    TF_AXIOM(funcNode);
    const TraceEventNode::AttributeMap &attrs = funcNode->GetAttributes();
  ...
  {
    TraceEventNodeConstRefPtr funcNode = FindNode(reporter->GetEventRoot(), "Inner Scope");
    TF_AXIOM(funcNode);
    const TraceEventNode::AttributeMap &attrs = funcNode->GetAttributes();
    TF_AXIOM(attrs.size() == 4);
    TF_AXIOM(attrs.count(TfToken("str")) == 1);
    TF_AXIOM(*attrs.find(TfToken("str"))->second.GetString() == "Test String 1");
    TF_AXIOM(*attrs.find(TfToken("cstr"))->second.GetString() == "Test String 1");
    TF_AXIOM(attrs.count(TfToken("str literal")) == 1);
    TF_AXIOM(*attrs.find(TfToken("str literal"))->second.GetString() == "A String Literal");
    TF_AXIOM(attrs.count(TfToken("sign string")) == 1);
    TF_AXIOM(*attrs.find(TfToken("sign string"))->second.GetString() == "Positive");
  }
...

  {
    ...
    TF_AXIOM(attrs.size() == 4);
    TF_AXIOM(attrs.count(TfToken("str")) == 1);
    TF_AXIOM(*attrs.find(TfToken("str"))->second.GetString() == "Test String -8");
    TF_AXIOM(*attrs.find(TfToken("cstr"))->second.GetString() == "Test String -8");
    TF_AXIOM(attrs.count(TfToken("str literal")) == 1);
    TF_AXIOM(*attrs.find(TfToken("str literal"))->second.GetString() == "A String Literal");
    TF_AXIOM(attrs.count(TfToken("sign string")) == 1);
    TF_AXIOM(*attrs.find(TfToken("sign string"))->second.GetString() == "Not Positive");
  }
}
