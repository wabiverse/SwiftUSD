...
static bool Test_TfFunctionRef()
{
  ...
  // than forming a reference to a reference.
  {
    ...
    const auto error = [] {
      TF_FATAL_ERROR(
          "Constructed new reference to callable instead of "
          "copying");
    };
  ...
  {
    const auto ok = [] {};
    const auto error1 = [] { TF_FATAL_ERROR("Failed to assign reference"); };
    const auto error2 = [] {
      TF_FATAL_ERROR(
          "Assigned new reference to callable instead of "
          "copying");
    };
    TfFunctionRef<void()> ref(ok);
