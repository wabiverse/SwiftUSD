...
static string _Replace(const string &template_,
                       const TfTemplateString::Mapping &mapping = TfTemplateString::Mapping(),
                       bool safe = false)
{
  TfTemplateString ts(template_);
  string result = safe ? ts.SafeSubstitute(mapping) : ts.Substitute(mapping);
  printf("'%s' -> '%s'\n", template_.c_str(), result.c_str());
  return result;
}

static bool TestTemplateString()
{
  ...
  TF_AXIOM("$(var)" == _Replace("$(var)", {{"var", "value"}}));

  TF_AXIOM("value" == _Replace("$var", {{"var", "value"}}));
  ...
  TF_AXIOM("valued" == _Replace("${var}d", {{"var", "value"}}));

  TF_AXIOM("value-value" == _Replace("$var-value", {{"var", "value"}}));

  TF_AXIOM("0000" == _Replace("$var$var$var$var", {{"var", "0"}}));

  TF_AXIOM("0.0.0.0" == _Replace("${var}.${var}.${var}.${var}", {{"var", "0"}}));
  ...
  TF_AXIOM("Please remit the $sum of $19.95" ==
           _Replace("Please remit the $$sum of $$$sum", {{"sum", "19.95"}}));

  {
    TfErrorMark m;
    TF_AXIOM(
        "Unreplaced placeholders ${are} awesome" ==
        _Replace("Unreplaced placeholders ${are} awesome", TfTemplateString::Mapping(), true));
    TF_AXIOM(m.IsClean());
  }
...

  {
    ...
    TfErrorMark m;
    TF_AXIOM("Never stop ${quoting" == _Replace("Never stop ${quoting"));
    TF_AXIOM(!m.IsClean());
    m.Clear();
...
static bool Test_TfTemplateString()
{
