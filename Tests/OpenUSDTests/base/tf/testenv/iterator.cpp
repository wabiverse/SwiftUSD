...
\
  ...
  origMap["c"] = 'c'; \
\
  TfIterator<CONST map<string, char>> mapIter(origMap.begin(), origMap.end()), \
      mapIterCopy = mapIter; \
...
\
  TfIterator<CONST map<string, char>> mapEnd(origMap.end(), origMap.end()); \
...
static bool Test_TfIterator()
{
  return TestNonConst() && TestConst() && TestRefsAndTempsForAll() && TestPointerIterators();
}
