...
enum Condiment { SALT, PEPPER = 13, KETCHUP, NO_NAME };
...
static bool Test_TfEnum()
{
  ...
  for (vector<string>::const_iterator n = lookupNames.begin(); n != lookupNames.end(); ++n)
    printf(
        "type name \"%s\" is %s\n", n->c_str(), TfEnum::IsKnownEnumType(*n) ? "known" : "unknown");
