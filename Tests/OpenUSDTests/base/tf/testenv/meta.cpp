...
#define ASSERT_SAME(x, y) \
  static_assert(std::is_same<TF_PP_EAT_PARENS(x), TF_PP_EAT_PARENS(y)>::value, "")
...
#define TEST_MEM_FN_IMPL(ret, name, thisArg, ...) \
  ...
  ASSERT_SAME((TfMetaApply<TfMetaTail, name##Traits::ArgTypes>), (TfMetaList<__VA_ARGS__>))
...
void testTfMeta()
{
  ...
  ASSERT_SAME((TfMetaApply<TfMetaTail, TestList>), (TfMetaList<float, std::string>));

  using TestRefList = TfMetaList<int &, float const &, std::string>;
  ASSERT_SAME((TfMetaApply<TfMetaDecay, TestRefList>), TestList);

  ASSERT_SAME((TfMetaApply<TfMetaLength, TestList>), (std::integral_constant<size_t, 3>));

  ASSERT_SAME((TfMetaApply<std::tuple, TestList>), (std::tuple<int, float, std::string>));
}
