...
static size_t Tf_CountFileMatches(const string &pattern)
{
...
template<class Fn> static void CheckFail(Fn const &fn)
{
  TfErrorMark m;
...
static void TestErrorCases()
{
  ...
  CheckFail([]() { return TfSafeOutputFile::Update("/var/run/a/testTf_file_"); });
  CheckFail([]() { return TfSafeOutputFile::Replace("/var/run/a/testTf_file_"); });

  // Insufficient permission to create destination file.
  CheckFail([]() { return TfSafeOutputFile::Update("/var/run/testTf_file_"); });
  CheckFail([]() { return TfSafeOutputFile::Replace("/var/run/testTf_file_"); });
...
static void TestReplaceNewFile()
{
...
static void TestReplaceExistingFile()
{
  {
...
static void TestUpdateExistingFile()
{
  {
...
static void TestReplaceSymlink()
{
...
static bool Test_TfSafeOutputFile()
{
  TestErrorCases();
  TestReplaceNewFile();
