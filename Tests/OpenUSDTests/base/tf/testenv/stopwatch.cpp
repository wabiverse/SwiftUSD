...
static bool IsClose(double a, double b, double epsilon = 1e-3)
{
  auto diff = fabs(a - b);
  return diff <= epsilon * abs(a) && diff <= epsilon * abs(b);
}

static bool IsCloselyBounded(double value, double lower, double upper, double epsilon = 1e-3)
{
  return (1 - epsilon) * lower <= value && value <= (1 + epsilon) * upper;
}

static bool Test_TfStopwatch()
{
  bool ok = true;
  ...
  if (watchCopy.GetSeconds() != watch1.GetSeconds()) {
    cout << "expected watchCopy to contain (0.0) but got (\"" << watchCopy.GetSeconds() << ")"
         << endl;
    ok = false;
  }
  ...
  // scheduling issues.
  if (!IsCloselyBounded(watch1.GetSeconds(), minElapsed.count(), maxElapsed.count())) {
  ...
  if (watchCopy.GetSeconds() != 0.0) {
    cout << "watchCopy has non-zero initial time of " << watchCopy.GetSeconds() << " seconds."
         << endl;
    ok = false;
  }
  ...
  if (!IsClose(watchCopy.GetSeconds(), watch1.GetSeconds())) {
    cout << "AddFrom: watchCopy has time of " << watchCopy.GetSeconds() << " instead of "
         << watch1.GetSeconds() << " seconds." << endl;
    ok = false;
  }
  ...
  if (!IsClose(watchCopy.GetSeconds() / watch1.GetSeconds(), 2.0)) {
    cout << "AddFrom: watchCopy has time of " << watchCopy.GetSeconds() << " instead of "
         << 2 * watch1.GetSeconds() << " seconds." << endl;
    ok = false;
  }
  ...
  watchCopy.Reset();
  if (watchCopy.GetSeconds() != 0.0) {
    cout << "Reset: watchCopy has time of " << watchCopy.GetSeconds() << " instead of " << 0.0
         << " seconds." << endl;
    ok = false;
  }
