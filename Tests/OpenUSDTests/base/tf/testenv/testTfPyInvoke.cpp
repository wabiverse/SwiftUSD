...
static void TestInvokeAndExtract_NoArgs()
{
  ...
  const bool ok = TfPyInvokeAndExtract(
      "pxr.Tf.testenv.testTfPyInvoke_callees", "_NoArgs", &result);
  TF_AXIOM(ok == true);
...
static void TestInvokeAndReturn_NoArgs()
{
  ...
  const bool ok = TfPyInvokeAndReturn(
      "pxr.Tf.testenv.testTfPyInvoke_callees", "_ReturnInt", &result);
  TF_AXIOM(ok == true);
...
static void TestInvoke_Simple()
{
  const bool ok = TfPyInvoke(
      "pxr.Tf", "Debug.IsDebugSymbolNameEnabled", "TF_NONEXISTENT_DEBUG_SYMBOL");
  TF_AXIOM(ok == true);
}
...
static void TestInvokeAndExtract_Simple()
{
  bool result = false;
  const bool ok = TfPyInvokeAndExtract(
      "pxr.Tf", "Debug.IsDebugSymbolNameEnabled", &result, "TF_NONEXISTENT_DEBUG_SYMBOL");
  TF_AXIOM(ok == true);
...
static void TestInvokeAndExtract_Complex()
{
  ...
  const bool ok = TfPyInvokeAndExtract("pxr.Tf.testenv.testTfPyInvoke_callees",
                                       "_RepeatStrings",
                                       &result,
                                       input,
                                       /* numRepeats = */ 2);
...
static void TestInvoke_NoneType()
{
  ...
  const bool ok = TfPyInvoke(
      "pxr.Tf.testenv.testTfPyInvoke_callees", "_GetTheeToANonery", nullptr);
  TF_AXIOM(ok == true);
}
...
{
  const bool ok = TfPyInvoke("pxr.NonexistentModule", "NonexistentFunction");
  TF_AXIOM(ok == false);
}

static void TestInvoke_NonexistentFunction()
{
  const bool ok = TfPyInvoke("pxr.Tf", "NonexistentFunction");
  TF_AXIOM(ok == false);
}

static void TestInvoke_NonCallable()
{
  const bool ok = TfPyInvoke("pxr.Tf.testenv.testTfPyInvoke_callees", "_globalVar");
  TF_AXIOM(ok == false);
}
...
static void TestInvoke_BadParamType()
{
  ...
  const bool ok = TfPyInvoke("pxr.Tf", "Debug.IsDebugSymbolNameEnabled", param);
  TF_AXIOM(ok == false);
}

static void TestInvoke_WrongParamType()
{
  const bool ok = TfPyInvoke("pxr.Tf", "Debug.IsDebugSymbolNameEnabled", 5);
  TF_AXIOM(ok == false);
}
...
{
  const bool ok = TfPyInvoke("pxr.Tf.testenv.testTfPyInvoke_callees", "_RaiseException");
  TF_AXIOM(ok == false);
}

static void TestInvoke_TfError()
{
  const bool ok = TfPyInvoke("pxr.Tf.testenv.testTfPyInvoke_callees", "_RaiseTfError");
  TF_AXIOM(ok == false);
}
