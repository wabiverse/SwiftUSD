...
static void testSetDifferences()
{
  ...
  vector<int> result1 = TfOrderedSetDifferenceToContainer<vector<int>>(
      v1.begin(), v1.end(), v2.begin(), v2.end());
  TF_AXIOM(result1 == expected1);

  vector<int> result2 = TfOrderedUniquingSetDifferenceToContainer<vector<int>>(
      v1.begin(), v1.end(), v2.begin(), v2.end());
...
{
  {
    ...
    static_assert(std::is_same<return_type_0, int &>::value, "return type should be int&");

    using return_type_1 = decltype(TfGet<1>()(testPair));
    static_assert(std::is_same<return_type_1, std::string &>::value,
...

  {
    ...
    using return_type_0 = decltype(TfGet<0>()(testPair));
    static_assert(std::is_same<return_type_0, const int &>::value,
                  "return type should be const int&");

    using return_type_1 = decltype(TfGet<1>()(testPair));
    static_assert(std::is_same<return_type_1, const std::string &>::value,
  ...
  {
    auto make_test_pair = []() -> pair<int, std::string> { return std::make_pair(3, "C"); };
  ...
  {
    const std::vector<pair<int, std::string>> pairs = {std::make_pair(1, "A"),
                                                       std::make_pair(2, "B"),
                                                       std::make_pair(3, "C"),
                                                       std::make_pair(4, "D")};
...
static void testGetTuple()
{
...

  {
    ...
    std::transform(tuples.begin(), tuples.end(), intsOnly.begin(), TfGet<0>());

    TF_AXIOM((intsOnly == std::vector<int>{1, 2, 3, 4}));
  }
}

static bool Test_TfStl()
{
  testSetDifferences();
  testGetPair();
  testGetTuple();
