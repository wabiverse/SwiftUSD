...
static NodeRefPtr MakeChain(size_t n)
{
  if (n == 0)
    return TfNullPtr;
  else {
...
static bool Test_TfRefPtr()
{
  ...
  std::cout << "total nodes (should be 15): " << Node::GetTotalNodeCount() << std::endl;
  ...
  std::cout << "total nodes (should be one more than previous): " << Node::GetTotalNodeCount()
            << std::endl;
  ...
  std::cout << "total nodes (should be 10 less than last): " << Node::GetTotalNodeCount()
            << std::endl;
  ...
  std::cout << "total nodes (should be zero): " << Node::GetTotalNodeCount() << std::endl;
