...
static bool Test_TfDebugTestEnv()
{
  bool retVal = true;
  ...
  std::vector<std::string> expSymNames{
      {"FLAM", "FLIM", "FLIMFLAM", "FOO", "FOOFLAM", "FOOFLIMFLAM"}};

  std::vector<std::string> result;
  std::set_intersection(symNames.begin(),
                        symNames.end(),
                        expSymNames.begin(),
                        expSymNames.end(),
  ...
  for (std::map<std::string, std::string>::const_iterator i = expDescriptions.begin();
       i != expDescriptions.end();
       ++i)
  {
    std::string description = TfDebug::GetDebugSymbolDescription(i->first);
    if (description != i->second) {
      printf(
          "Error: unexpected description for symbol '%s'\n"
          "  expected: '%s'\n"
          "    actual: '%s'\n",
          i->first.c_str(),
          i->second.c_str(),
...
static bool Test_TfDebugTestEnvHelp()
{
...
static bool Test_TfDebugFatal_1()
{
  bool retVal = false;
...
static bool Test_TfDebugFatal_2()
{
  bool retVal = false;
...
static bool Test_TfDebugFatal_3()
{
  bool retVal = false;
