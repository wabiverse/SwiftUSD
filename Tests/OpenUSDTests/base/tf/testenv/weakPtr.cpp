...
{
  TF_AXIOM(TfHash()(MonkeyInterfaceWeakPtr()) == TfHash()(MonkeyInterfaceWeakPtr(nullptr)));
  {
    Human h;
...
class ProtectedBase : public TfRefBase, public TfWeakBase {
 public:
  ~ProtectedBase()
  {
    ...
    ProtectedBase_Registry::_RegistryMap::iterator it = reg._registry.find(_id);
  ...
  static ProtectedBaseRefPtr FindOrCreate(int id)
  {
    ...
    ProtectedBase_Registry &reg = ProtectedBase_Registry::GetInstance();
    ProtectedBase_Registry::_RegistryMap::iterator it = reg._registry.find(id);
    if (it != reg._registry.end()) {
      ...
      if (ProtectedBaseRefPtr baseRef = TfCreateRefPtrFromProtectedWeakPtr(it->second)) {
#else
      // This is the *unsafe* way to do this, and will reliably
      // cause this test to fail.
      if (ProtectedBaseRefPtr baseRef = ProtectedBaseRefPtr(it->second)) {
#endif
        return baseRef;
      }
      else {
    ...
    ProtectedBaseRefPtr baseRef = TfCreateRefPtr(new ProtectedBase(id));
...
static ProtectedBaseRefPtr _ThreadFunc()
{
  return ProtectedBase::FindOrCreate(1);  // wait
}

static bool Test_TfCreateRefPtrFromProtectedWeakPtr()
{
  ProtectedBase_Registry &reg = ProtectedBase_Registry::GetInstance();
  ...
  // Now test concurrent destruction during FindOrCreate()
  {
    ...
    std::future<ProtectedBaseRefPtr> t1 = std::async(std::launch::async, _ThreadFunc);
...
inline string F(const TfWeakPtr<Base> &b)
{
  return "base";
}
inline string F(const TfWeakPtr<Unrelated> &b)
{
  return "unrelated";
}
...
static bool Test_TfWeakPtrConversion()
{
  Derived d;
  Unrelated u;
