...
static unsigned int testInterpreter(bool verbose)
{
  ...
  if (!result || !(PyBytes_Check(result.get()) || PyUnicode_Check(result.get())) ||
      (strcmp(PyUnicode_AsUTF8(result.get()), "hello") != 0))
  {
    ...
    else if (!(PyBytes_Check(result.get()) || PyUnicode_Check(result.get()))) {
