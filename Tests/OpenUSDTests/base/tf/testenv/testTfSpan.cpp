...
void Tf_TestSpanMatchesContainer(const Span &span, const Container &cont)
{
...
int main(int argc, char **argv)
{
  ...
  // Test subspans.
  {
    ...
    TF_AXIOM(std::equal(subspan.begin(), subspan.end(), expectedSubspan.begin()));

    // Test with explicit count
    TfSpan<const int> subspan2 = span.subspan(2, 2);
    const std::vector<int> expectedSubspan2({3, 4});
    TF_AXIOM(std::equal(subspan2.begin(), subspan2.end(), expectedSubspan2.begin()));

    // Test first.
    TfSpan<const int> subspan3 = span.first(2);
    const std::vector<int> expectedSubspan3({1, 2});
    TF_AXIOM(std::equal(subspan3.begin(), subspan3.end(), expectedSubspan3.begin()));

    // Test last.
    TfSpan<const int> subspan4 = span.last(2);
    const std::vector<int> expectedSubspan4({4, 5});
    TF_AXIOM(std::equal(subspan4.begin(), subspan4.end(), expectedSubspan4.begin()));
  }
  ...
  // Test span edits.
  {
    ...
    TF_AXIOM(std::equal(data.begin(), data.end(), expected.begin()));
  }
