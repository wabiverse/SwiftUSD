...
template<class HashState> void TfHashAppend(HashState &h, Two two)
{
  h.Append(two.x, two.y);
}

template<class Hasher> static inline void TestTwo(Hasher const &h, Two t, unsigned *counts)
{
...
template<class Hasher> static inline void TestOne(Hasher const &h, uint64_t x, unsigned *counts)
{
  // Hash x, then flip each bit in x and hash again.
  // For each flipped bit in the resulting hash, increment a counter.
...
template<class Hasher> static void _TestStatsOne(Hasher const &h, char const *label)
{
  TfStopwatch sw;
  sw.Start();
...
template<class Hasher> static void _TestStatsTwo(Hasher const &h, char const *label)
{
  TfStopwatch sw;
  sw.Start();
  ...
  while (numTests--) {
    Two t{static_cast<uint32_t>(numTests << 5), static_cast<uint32_t>(numTests >> 5)};
    TestTwo(h, t, counts);
  }
...
template<class T, class = decltype(TfHash()(std::declval<T>()))> constexpr bool _IsHashable(int)
{
  return true;
}
...
template<class HashState> void TfHashAppend(HashState &h, MultipleThings const &mt)
{
  h.Append(mt.ival, mt.fval, mt.sval, mt.vints);
  h.AppendRange(mt.sfloats.begin(), mt.sfloats.end());
}

static bool Test_TfHash()
{
  ...
  enum : char { BarA, BarB, BarC } barEnum;
