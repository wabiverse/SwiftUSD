...
static unsigned int _CheckResultInEnv(const string &envName, const string &envVal)
{
  string result = TfGetenv(envName);
  if (result != envVal) {
    printf("ERROR: Expected '%s', got '%s'.\n", envVal.c_str(), result.c_str());
...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
static unsigned int _CheckResultInOsEnviron(const string &envName, const string &envVal)
{
  static std::once_flag once;
  std::call_once(once, []() { TfPyRunSimpleString("import os\n"); });
...
static unsigned int _CheckResultNotInOsEnviron(const string &envName)
{
  static std::once_flag once;
  std::call_once(once, []() { TfPyRunSimpleString("import os\n"); });
...
static unsigned int _TestPySetenvNoInit()
{
...

  {
    ...
    if (didSet) {
      numErrors += 1;
      printf(
          "ERROR: Calling TfPySetenv with uninitialized Python "
          "should return false.");
    }
...
static unsigned int _TestPySetenvInit()
{
...
static unsigned int _TestSetenvNoInit()
{
...
static unsigned int _TestSetenvInit()
{
...
static bool Test_TfSetenv(int argc, char **argv)
{
  unsigned int numErrors = 0;
