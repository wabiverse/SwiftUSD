...
int main()
{
  ...
  ARCH_AXIOM(ArchGetPrettierFunctionName("Bar", "int Foo<A>::Bar(float) [with A = int]") ==
             "Foo<A>::Bar [with A = int]");

  // Multi-parameter template class
  ARCH_AXIOM(
      ArchGetPrettierFunctionName("Bar", "int Foo<A,B>::Bar(float) [with A = int, B = int]") ==
      "Foo<A,B>::Bar [with A = int, B = int]");

  // Template function
  ARCH_AXIOM(ArchGetPrettierFunctionName(
                 "Bar", "A Foo<A, B>::Bar(C) [with C = double; B = float; A = int]") ==
             "Foo<A, B>::Bar [with A = int, B = float]");

  // Linux-style nested templates
  ARCH_AXIOM(ArchGetPrettierFunctionName(
                 "foo", "int X<A>::Y<B>::foo(A, B, C) [with C = bool; B = float; A = int]") ==
  ...
  ARCH_AXIOM(
      ArchGetPrettierFunctionName("foo",
  ...
  ARCH_AXIOM(
      ArchGetPrettierFunctionName("operator<", "bool Z<W>::operator<(Y) const [with W = int]") ==
