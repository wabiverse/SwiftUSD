...
bool TestSimpleOverlay()
{
  HdContainerDataSourceHandle containers[] = {
      ...
      HdRetainedContainerDataSource::New(TfToken("D"),
                                         HdRetainedContainerDataSource::New(TfToken("E"), I(4)),
                                         TfToken("F"),
                                         I(6),
                                         TfToken("G"),
                                         I(8)),
...
bool TestContainerEditor()
{
  {
    HdContainerDataSourceHandle baseline = HdRetainedContainerDataSource::New(
        TfToken("A"), I(1), TfToken("B"), I(2));

    HdContainerDataSourceHandle test =
        HdContainerDataSourceEditor().Set(L("A"), I(1)).Set(L("B"), I(2)).Finish();
...

  {
    HdContainerDataSourceHandle test = HdContainerDataSourceEditor()
                                           .Set(L("A"),
                                                HdRetainedContainerDataSource::New(TfToken("B"),
                                                                                   I(1)))
                                           .Set(L("A/C"), I(2))
                                           .Set(L("A/D/E"), I(3))
                                           .Finish();
...

  {
    HdContainerDataSourceHandle initialContainer =
        HdContainerDataSourceEditor().Set(L("A/B"), I(1)).Finish();

    HdContainerDataSourceHandle test = HdContainerDataSourceEditor(initialContainer)
                                           .Set(L("A/C"), I(2))
                                           .Set(L("D"), I(3))
                                           .Finish();
    ...
    HdContainerDataSourceHandle baseline = HdRetainedContainerDataSource::New(
        TfToken("A"),
        HdRetainedContainerDataSource::New(TfToken("B"), I(1), TfToken("C"), I(2)),
        TfToken("D"),
        I(3));
...

  {
    ...
    HdContainerDataSourceHandle subcontainer =
        HdContainerDataSourceEditor().Set(L("D"), I(3)).Finish();

    HdContainerDataSourceHandle test =
        HdContainerDataSourceEditor(initialContainer).Overlay(L("A"), subcontainer).Finish();

    HdContainerDataSourceHandle baseline = HdContainerDataSourceEditor()
                                               .Set(L("A/B"), I(1))
                                               .Set(L("A/C"), I(2))
                                               .Set(L("A/D"), I(3))
                                               .Finish();
