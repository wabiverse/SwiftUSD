...
void BufferSpecTest()
{
  ...
  // test set operations
  {
    ...
    spec1.push_back(HdBufferSpec(HdTokens->points, HdTupleType{HdTypeFloatVec3, 1}));
    spec1.push_back(HdBufferSpec(HdTokens->displayColor, HdTupleType{HdTypeFloatVec3, 1}));

    spec2.push_back(HdBufferSpec(HdTokens->points, HdTupleType{HdTypeFloatVec3, 1}));

    TF_VERIFY(HdBufferSpec::IsSubset(spec2, spec1) == true);
    TF_VERIFY(HdBufferSpec::IsSubset(spec1, spec2) == false);

    spec2.push_back(HdBufferSpec(HdTokens->normals, HdTupleType{HdTypeFloatVec4, 1}));
