...
class ExtComputationTestDelegate : public HdUnitTestDelegate {
 public:
  ...
  HdExtComputationPrimvarDescriptorVector virtual GetExtComputationPrimvarDescriptors(
      SdfPath const &id, HdInterpolation interpolationMode) override
  {
    ...
    if (id == pathA && interpolationMode == HdInterpolationConstant) {
      ...
      primvars.emplace_back(primvarName,
  ...
  virtual TfTokenVector GetExtComputationSceneInputNames(SdfPath const &computationId) override
  {
                                    ...
                                    HdExtComputationContext *context) override
  {
    ...
    context->SetOutputValue(compOutputName, VtValue(val1.Get<double>() + val2.Get<double>()));
  }
};
...
const TfTokenVector ExtCompTestRenderDelegate::_sprimTypes = {HdPrimTypeTokens->extComputation};
...
void RunTest()
{
  ...
  auto compPrimvars = delegate.GetExtComputationPrimvarDescriptors(pathA, HdInterpolationConstant);
  ...
  if (pvSamples.count != maxSamples) {
    TF_RUNTIME_ERROR("Unexpected number of samples %d", static_cast<int>(pvSamples.count));
    return;
  }

#define CHECK_SAMPLE(index, time, val) \
  if (pvSamples.times[index] != time) { \
    TF_RUNTIME_ERROR("Unexpected sample time %f vs %f", pvSamples.times[index], time); \
    return; \
  } \
  if (pvSamples.values[index].Get<double>() != val) { \
    TF_RUNTIME_ERROR( \
        "Unexpected sample value %f vs %f", pvSamples.values[index].Get<double>(), val); \
    return; \
  }
