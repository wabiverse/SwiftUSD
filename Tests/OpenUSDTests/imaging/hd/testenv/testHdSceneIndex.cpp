...
static std::ostream &operator<<(std::ostream &out, const SdfPathVector &v)
{
  out << "{" << std::endl;
...
class PrintingSceneIndexObserver : public HdSceneIndexObserver {
 public:
  PrintingSceneIndexObserver(const std::string &prefix) : _prefix(prefix) {}

  void PrimsAdded(const HdSceneIndexBase &sender, const AddedPrimEntries &entries) override
  {
    for (const AddedPrimEntry &entry : entries) {
      std::cout << _prefix << "PrimAdded: " << entry.primPath << ", " << entry.primType
                << std::endl;
    }
  }
  ...
  void PrimsRemoved(const HdSceneIndexBase &sender, const RemovedPrimEntries &entries) override
  {
    for (const RemovedPrimEntry &entry : entries) {
      std::cout << _prefix << "PrimRemoved: " << entry.primPath << ", " << std::endl;
    }
  }
...
class RecordingSceneIndexObserver : public HdSceneIndexObserver {
 public:
  ...
  struct Event {
    ...
    {
      return (eventType == rhs.eventType && primPath == rhs.primPath && primType == rhs.primType &&
              locator == rhs.locator);
    }

    template<class HashState> friend void TfHashAppend(HashState &h, Event const &myObj)
    {
  ...
  {
    for (const AddedPrimEntry &entry : entries) {
      _events.emplace_back(Event{EventType_PrimAdded, entry.primPath, entry.primType});
    }
  }
  ...
  void PrimsDirtied(const HdSceneIndexBase &sender, const DirtiedPrimEntries &entries) override
  {
    for (const DirtiedPrimEntry &entry : entries) {
      for (const HdDataSourceLocator &locator : entry.dirtyLocators) {
        _events.emplace_back(Event{EventType_PrimDirtied, entry.primPath, TfToken(), locator});
      }
    }
  }
...
inline size_t RecordingSceneIndexObserver::Event::Hash() const
{
  return TfHash()(*this);
}

std::ostream &operator<<(std::ostream &out, const RecordingSceneIndexObserver::Event &event)
{
  switch (event.eventType) {
    ...
    case RecordingSceneIndexObserver::EventType_PrimDirtied:
      out << "PrimDirtied: " << event.primPath << ", " << event.locator.GetString();
      break;
    default:
...
std::ostream &operator<<(std::ostream &out, const RecordingSceneIndexObserver::EventVector &events)
{
  out << "{" << std::endl;
...
std::ostream &operator<<(std::ostream &out, const RecordingSceneIndexObserver::EventSet &events)
{
  return out << RecordingSceneIndexObserver::EventVector(events.begin(), events.end());
}

// ----------------------------------------------------------------------------

void PrintContainer(HdContainerDataSourceHandle container,
                    const std::string &prefix = std::string())
{
  ...
  for (const TfToken &name : container->GetNames()) {
    ...
    if (HdContainerDataSourceHandle childContainer = HdContainerDataSource::Cast(childSource)) {
      std::cout << std::endl;
      PrintContainer(childContainer, prefix + "  ");
    }
    else if (HdSampledDataSourceHandle sampledChild = HdSampledDataSource::Cast(childSource)) {
...
void PrintSceneIndexPrim(HdSceneIndexBase &sceneIndex,
...
GfMatrix4d GetPrimTransform(const HdSceneIndexBase &sceneIndex, const SdfPath &primPath)
{
  HdSceneIndexPrim prim = sceneIndex.GetPrim(primPath);
...
template<typename T> bool _CompareValue(const char *msg, const T &v1, const T &v2)
{
  if (v1 == v2) {
...
bool TestFlatteningSceneIndex()
{
  ...
  HdFlatteningSceneIndexRefPtr flatteningSceneIndex_ = HdFlatteningSceneIndex::New(
  ...
  sceneIndex.AddPrims({{SdfPath("/A/B"),
                        TfToken("huh"),
                        HdRetainedContainerDataSource::New(
                            ...
                            HdXformSchema::Builder()
                                ...
                                .Build())}});
  sceneIndex.AddPrims({{SdfPath("/A/B/C"),
                        TfToken("huh"),
                        HdRetainedContainerDataSource::New(
                            HdXformSchemaTokens->xform,
                            HdXformSchema::Builder()
  ...
  if (!_CompareValue("INITIAL LEAF SCENE XFORM",
  ...
  if (!_CompareValue("FLATTENED LEAF SCENE XFORM",
  ...
  if (!_CompareValue("UPDATED INITIAL LEAF SCENE XFORM",
                     GetPrimTransform(sceneIndex, SdfPath("/A/B/C")),
                     GfMatrix4d().SetTranslate({5.0, 0.0, 0.0})))
  {
    return false;
  ...
  if (!_CompareValue("UPDATED FLATTENED LEAF SCENE XFORM",
                     GetPrimTransform(flatteningSceneIndex, SdfPath("/A/B/C")),
  ...
  if (!_CompareValue("FINAL LEAF SCENE XFORM",
                     GetPrimTransform(sceneIndex, SdfPath("/A/B/C")),
                     GfMatrix4d().SetTranslate({5.0, 0.0, 0.0})))
  {
    return false;
  ...
  if (!_CompareValue("FINAL FLATTENED LEAF SCENE XFORM",
                     GetPrimTransform(flatteningSceneIndex, SdfPath("/A/B/C")),
                     GfMatrix4d().SetTranslate({5.0, 0.0, 0.0})))
  {
    return false;
...
HdDataSourceBaseHandle GetDataSourceFromScene(HdSceneIndexBase &sceneIndex,
                                              const SdfPath &primPath,
...
template<typename T>
T GetTypedValueFromScene(HdSceneIndexBase &sceneIndex,
                         const SdfPath &primPath,
                         const HdDataSourceLocator &locator)
{
...
static bool TestPrefixingSceneIndex()
{
  HdRetainedSceneIndexRefPtr sceneIndex_ = HdRetainedSceneIndex::New();
  HdRetainedSceneIndex &sceneIndex = *sceneIndex_;

  HdPrefixingSceneIndexRefPtr prefixingSceneIndex_ = HdPrefixingSceneIndex::New(sceneIndex_,
                                                                                SdfPath("/E/F/G"));
  ...
  sceneIndex.AddPrims(
      {{SdfPath("/A/C"),
        ...
        HdRetainedContainerDataSource::New(
            TfToken("somePath"),
            HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("/A/B")),
            TfToken("someContainer"),
            HdRetainedContainerDataSource::New(
                TfToken("anotherPath"),
                HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("/A/B/C/D")),
                TfToken("relativePath"),
                HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("F/G")),
                TfToken("pathArray"),
  ...
  if (!_CompareValue("COMPARING TOP-LEVEL ABSOLUTE PATH",
                     GetTypedValueFromScene<SdfPath>(prefixingSceneIndex,
                                                     SdfPath("/E/F/G/A/C"),
  ...
  if (!_CompareValue("COMPARING NESTED ABSOLUTE PATH",
                     GetTypedValueFromScene<SdfPath>(
                         prefixingSceneIndex,
                         SdfPath("/E/F/G/A/C"),
                         HdDataSourceLocator(TfToken("someContainer"), TfToken("anotherPath"))),
  ...
  if (!_CompareValue("COMPARING NESTED RELATIVED PATH",
                     GetTypedValueFromScene<SdfPath>(
                         prefixingSceneIndex,
                         SdfPath("/E/F/G/A/C"),
                         HdDataSourceLocator(TfToken("someContainer"), TfToken("relativePath"))),
                     SdfPath("F/G")))
  {
    return false;
  }
  ...
  if (!_CompareValue("COMPARING PATH ARRAY",
                     GetTypedValueFromScene<VtArray<SdfPath>>(
                         prefixingSceneIndex,
                         SdfPath("/E/F/G/A/C"),
                         HdDataSourceLocator(TfToken("someContainer"), TfToken("pathArray"))),
                     VtArray<SdfPath>{SdfPath("/E/F/G/A/B/C/D"), SdfPath("/E/F/G/A/B")}))
  {
    return false;
  }
...
static bool _CompareSceneValue(const std::string &label,
                               HdSceneIndexBaseRefPtr scene,
                               const SdfPath &primPath,
                               const HdDataSourceLocator &locator,
                               const VtValue &value)
{
  if (auto sampledDataSource = HdSampledDataSource::Cast(scene->GetDataSource(primPath, locator)))
  {
    if (sampledDataSource->GetValue(0.0) == value) {
      ...
      std::cerr << label << " doesn't match. Expecting " << value << " got "
                << sampledDataSource->GetValue(0.0) << std::endl;
      return false;
    }
  }
  else {
    std::cerr << label << " value not found. Expecting " << value << std::endl;
    return false;
  }
}
...
bool TestMergingSceneIndex()
{
  ...
  retainedSceneA->AddPrims({
      {SdfPath("/A"),
       TfToken("group"),
       HdRetainedContainerDataSource::New(TfToken("uniqueToA"),
                                          HdRetainedTypedSampledDataSource<int>::New(0),
                                          TfToken("common"),
                                          HdRetainedTypedSampledDataSource<int>::New(0))},
      {SdfPath("/A/AA"),
       TfToken("group"),
       HdRetainedContainerDataSource::New(TfToken("value"),
  ...
  retainedSceneB->AddPrims({
      {SdfPath("/A"),
       TfToken("group"),
       HdRetainedContainerDataSource::New(TfToken("uniqueToB"),
                                          HdRetainedTypedSampledDataSource<int>::New(1),
                                          TfToken("common"),
                                          HdRetainedTypedSampledDataSource<int>::New(1))},
      {SdfPath("/A/BB"),
  ...
  mergingSceneIndex->AddInputScene(retainedSceneA, SdfPath::AbsoluteRootPath());
  mergingSceneIndex->AddInputScene(retainedSceneB, SdfPath::AbsoluteRootPath());

  PrintSceneIndexPrim(*mergingSceneIndex, SdfPath::AbsoluteRootPath(), true);

  if (!_CompareSceneValue("testing common value:",
                          mergingSceneIndex,
                          SdfPath("/A"),
                          HdDataSourceLocator(TfToken("common")),
                          VtValue(0)))
  {
    return false;
  }
  if (!_CompareSceneValue("testing uniqueToA value:",
                          mergingSceneIndex,
                          SdfPath("/A"),
                          HdDataSourceLocator(TfToken("uniqueToA")),
                          VtValue(0)))
  {
    return false;
  }
  if (!_CompareSceneValue("testing uniqueToB value:",
                          mergingSceneIndex,
                          SdfPath("/A"),
                          HdDataSourceLocator(TfToken("uniqueToB")),
                          VtValue(1)))
  {
    return false;
  }
  if (!_CompareSceneValue("testing /A/AA value:",
                          mergingSceneIndex,
                          SdfPath("/A/AA"),
                          HdDataSourceLocator(TfToken("value")),
                          VtValue(1)))
  {
    return false;
  }
  if (!_CompareSceneValue("testing /A/BB value:",
                          mergingSceneIndex,
                          SdfPath("/A/AA"),
                          HdDataSourceLocator(TfToken("value")),
                          VtValue(1)))
...
class _RepopulatingSceneIndex : public HdSingleInputFilteringSceneIndexBase {
 public:
  static _RepopulatingSceneIndexRefPtr New(const HdSceneIndexBaseRefPtr &inputScene)
  {
 ...
 protected:
  _RepopulatingSceneIndex(const HdSceneIndexBaseRefPtr &inputScene)
...
bool TestMergingSceneIndexPrimAddedNotices()
{
  ...
  retainedSceneA->AddPrims({
      ...
      {SdfPath("/A/B"),
       ...
       HdRetainedContainerDataSource::New(TfToken("value"),
                                          HdRetainedTypedSampledDataSource<int>::New(1))},
      {SdfPath("/A/C"),
       TfToken(),  // provides a data source but no type
       HdRetainedContainerDataSource::New(TfToken("value"),
                                          HdRetainedTypedSampledDataSource<int>::New(1))},
  });
  ...
  _RepopulatingSceneIndexRefPtr rpA = _RepopulatingSceneIndex::New(retainedSceneA);

  _RepopulatingSceneIndexRefPtr rpB = _RepopulatingSceneIndex::New(retainedSceneB);
  ...
  auto _Compare = [&mergingSceneIndex, &observer, &expectedTypes]() {
    for (const RecordingSceneIndexObserver::Event event : observer.GetEvents()) {

      if (event.eventType != RecordingSceneIndexObserver::EventType_PrimAdded) {
        std::cerr << "received unexpected event type for " << event.primPath << std::endl;
        return false;
      }

      const auto it = expectedTypes.find(event.primPath);
      if (it == expectedTypes.end()) {
        std::cerr << "expected type is unknown for " << event.primPath << std::endl;
        return false;
      }

      const TfToken &expectedType = it->second;
      if (event.primType != expectedType) {
        std::cerr << "expected '" << expectedType << "' but received '" << event.primType
                  << "' for " << event.primPath << std::endl;
        return false;
      }

      HdSceneIndexPrim prim = mergingSceneIndex->GetPrim(event.primPath);
      if (prim.primType != expectedType) {
        std::cerr << "expected '" << expectedType << "' but received '" << prim.primType
                  << "' for GetPrim(" << event.primPath << ")" << std::endl;
        return false;
      }
    }
...
bool TestDependencyForwardingSceneIndex()
{
  ...
  HdDependencyForwardingSceneIndex &dependencyForwardingScene = *dependencyForwardingScene_;

  retainedScene.AddPrims(
      {{SdfPath("/A"), TfToken("group"), HdRetainedContainerDataSource::New()}});
      ...
      {{SdfPath("/C"),
        ...
        HdRetainedContainerDataSource::New(
            ...
            HdRetainedContainerDataSource::New(
                TfToken("test"),
                HdDependencySchema::BuildRetained(
                    HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("/B")),
                    RDS::New(HdDataSourceLocator(TfToken("chicken"))),
      ...
      {{SdfPath("/D"),
        ...
        HdRetainedContainerDataSource::New(
            ...
            HdRetainedContainerDataSource::New(
                TfToken("test"),
                HdDependencySchema::BuildRetained(
                    HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("/E")),
      ...
      {{SdfPath("/E"),
        ...
        HdRetainedContainerDataSource::New(
            ...
            HdRetainedContainerDataSource::New(
                TfToken("test"),
                HdDependencySchema::BuildRetained(
                    HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("/F")),
      ...
      {{SdfPath("/F"),
        ...
        HdRetainedContainerDataSource::New(
            ...
            HdRetainedContainerDataSource::New(
                TfToken("test"),
                HdDependencySchema::BuildRetained(
                    HdRetainedTypedSampledDataSource<SdfPath>::New(SdfPath("/D")),
                    RDS::New(HdDataSourceLocator(TfToken("attr1"))),
                    RDS::New(HdDataSourceLocator(TfToken("attr3"))))))}});

  RecordingSceneIndexObserver recordingScene;
  dependencyForwardingScene.AddObserver(HdSceneIndexObserverPtr(&recordingScene));
...

  {
    ...
    retainedScene.DirtyPrims({{SdfPath("/A"), HdDataSourceLocator(TfToken("taco"))}});

    auto baseline = EventSet{
        Event{EventType::EventType_PrimDirtied,
              SdfPath("/A"),
              TfToken(),
    ...
    if (!_CompareValue("DIRTYING \"/A @taco\" ->", recordingScene.GetEventsAsSet(), baseline)) {
      return false;
    }
  }
  ...
  {
    recordingScene.Clear();
    retainedScene.DirtyPrims({{SdfPath("/A"), HdDataSourceLocator()}});

    auto baseline = EventSet{
        Event{EventType::EventType_PrimDirtied, SdfPath("/A"), TfToken(), HdDataSourceLocator()},
        Event{EventType::EventType_PrimDirtied,
              SdfPath("/B"),
              TfToken(),
              HdDataSourceLocator(TfToken("chicken"))},
    ...
    if (!_CompareValue(
            "DIRTYING \"/A @(prim level)\" ->", recordingScene.GetEventsAsSet(), baseline))
    {
      return false;
    }
  }
  ...
  // test cycles
  {
    ...
    recordingScene.Clear();
    retainedScene.DirtyPrims({{SdfPath("/D"), HdDataSourceLocator(TfToken("attr1"))}});

    if (!_CompareValue(
            "CYCLE CHECK: DIRTYING \"/D @attr1\" ->", recordingScene.GetEventsAsSet(), baseline))
    {
      return false;
    }

    recordingScene.Clear();
    retainedScene.DirtyPrims({{SdfPath("/E"), HdDataSourceLocator(TfToken("attr2"))}});

    if (!_CompareValue(
            "CYCLE CHECK: DIRTYING \"/E @attr2\" ->", recordingScene.GetEventsAsSet(), baseline))
    {
      return false;
    }

    recordingScene.Clear();
    retainedScene.DirtyPrims({{SdfPath("/F"), HdDataSourceLocator(TfToken("attr3"))}});

    if (!_CompareValue(
            "CYCLE CHECK: DIRTYING \"/E @attr3\" ->", recordingScene.GetEventsAsSet(), baseline))
    {
      return false;
    }
  }
    ...
    HdDependencyForwardingSceneIndexRefPtr *dependencyForwardingSceneP)
{
  ...
  retainedScene->AddPrims(
      {{SdfPath("/C"), TfToken("group"), HdRetainedContainerDataSource::New()}});
...
bool TestDependencyForwardingSceneIndexEviction()
{
  ...
  //---------------------
  {
    TestDependencyForwardingSceneIndexEviction_InitScenes(&retainedScene,
                                                          &dependencyForwardingScene);
    ...
    {
      auto baseline = EventSet{
          Event{EventType::EventType_PrimRemoved, SdfPath("/B"), TfToken(), HdDataSourceLocator()},
      };

      if (!_CompareValue("Removing \"/B\" ->", recordingScene.GetEventsAsSet(), baseline)) {
        return false;
      }
    }
    ...
    // Validate bookkeeping.
    {
      ...
      dependencyForwardingScene->RemoveDeletedEntries(&removedAffectedPrimPaths,
      ...
      if (!_CompareValue(
              "Remove Affected (affected paths): ", removedAffectedPrimPaths, baselineAffected))
      {
        return false;
      }
  ...
  //---------------------
  {
    ...
    {
      auto baseline = EventSet{
          Event{EventType::EventType_PrimRemoved, SdfPath("/A"), TfToken(), HdDataSourceLocator()},
          Event{EventType::EventType_PrimDirtied,
                SdfPath("/B"),
                TfToken(),
                HdDataSourceLocator(TfToken("chicken"))},
      ...
      if (!_CompareValue("Removing \"/A\" ->", recordingScene.GetEventsAsSet(), baseline)) {
        return false;
      }
    }
    ...
    // Validate bookkeeping.
    {
      ...
      if (!_CompareValue(
              "Remove Depended On (affected paths): ", removedAffectedPrimPaths, baselineAffected))
      {
        return false;
      }
  ...
  {
    auto baseline = EventSet{
        Event{EventType::EventType_PrimRemoved, SdfPath("/C"), TfToken(), HdDataSourceLocator()},
    };

    if (!_CompareValue("Removing \"/C\" ->", recordingScene.GetEventsAsSet(), baseline)) {
      return false;
    }
  }
  ...
  // Validate bookkeeping.
  {
    ...
    if (!_CompareValue("Remove Prim Without Dependencies (affected paths): ",
                       removedAffectedPrimPaths,
                       baselineAffected))
    {
      return false;
    }
    if (!_CompareValue("Remove Prim Without Dependencies  (depended on paths): ",
                       removedDependedOnPrimPaths,
                       baselineDependedOn))
    {
      return false;
