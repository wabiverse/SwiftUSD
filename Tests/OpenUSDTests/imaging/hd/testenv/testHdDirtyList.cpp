...
static void _VerifyCounter(HdPerfLog *perfLog, TfToken const &name, size_t count)
{
  size_t value = size_t(perfLog->GetCounter(name));
  TF_VERIFY(value == count, "expected %zu, found %zu", count, value);
}

static HdReprSelector surface(HdReprTokens->refined);
static HdReprSelector wireOnSurf(HdReprTokens->wireOnSurf);
static HdReprSelector wireOnSurfWithPoints(HdReprTokens->wireOnSurf,
...
static bool BasicTest()
{
  ...
  //    So, all the rprims should be in the dirty list.
  {
    ...
    dl.UpdateRenderTagsAndReprSelectors(TfTokenVector(/*empty render tags*/),
  ...
  // 3. Update the render tags.
  {
    ...
    dl.UpdateRenderTagsAndReprSelectors(TfTokenVector({HdRenderTagTokens->geometry}),
                                        HdReprSelectorVector({surface}));
    ...
    dl.UpdateRenderTagsAndReprSelectors(TfTokenVector({HdRenderTagTokens->guide}),
                                        HdReprSelectorVector({surface}));
