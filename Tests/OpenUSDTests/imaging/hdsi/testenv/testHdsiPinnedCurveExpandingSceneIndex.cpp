...
struct _Primvar {
  _Primvar(TfToken const &_name,
      ...
      : name(_name), value(_value), interp(_interp), role(_role), indices(_indices)
  {
  }
...
HdContainerDataSourceHandle _BuildCurveDataSource(_Curve const &curve)
{
  HdDataSourceBaseHandle bcs =
      HdBasisCurvesSchema::Builder()
          .SetTopology(HdBasisCurvesTopologySchema::BuildRetained(
              HdRetainedTypedSampledDataSource<VtIntArray>::New(curve.curveVertexCounts),
  ...
  for (_Primvar const &pv : primvars) {
    ...
    primvarDataSources.push_back(HdPrimvarSchema::BuildRetained(
        pv.indices.empty() ? _GetRetainedDataSource(pv.value) : HdSampledDataSourceHandle(),
        !pv.indices.empty() ? _GetRetainedDataSource(pv.value) : HdSampledDataSourceHandle(),
  ...
  HdDataSourceBaseHandle primvarsDs = HdRetainedContainerDataSource::New(
      primvars.size(), primvarNames.data(), primvarDataSources.data());

  return HdRetainedContainerDataSource::New(
      HdBasisCurvesSchemaTokens->basisCurves, bcs, HdPrimvarsSchemaTokens->primvars, primvarsDs);
}
                                                            ...
                                                            bool hasIndexedPrimvar)
{
  ...
  _Curve c;
  {
    ...
    primvars.push_back({HdTokens->points,
                        VtValue(points),
                        HdPrimvarSchemaTokens->vertex,
                        HdPrimvarSchemaTokens->point});

    primvars.push_back(
        {TfToken("fooConstant"), VtValue(constantPrimvar), HdPrimvarSchemaTokens->constant});

    primvars.push_back(
        {TfToken("fooUniform"), VtValue(uniformPrimvar), HdPrimvarSchemaTokens->uniform});

    if (hasIndexedPrimvar) {
      primvars.push_back({TfToken("fooVertexIndexed"),
                          VtValue(indexedVertexPrimvar),
                          HdPrimvarSchemaTokens->vertex,
                          TfToken("testRole"),
                          vertexPrimvarIndices});

      primvars.push_back({TfToken("fooVaryingIndexed"),
      ...
      primvars.push_back(
          {TfToken("fooVarying"), VtValue(varyingPrimvar), HdPrimvarSchemaTokens->varying});
    }
  }
  ...
  _Curve e;
  {
    ...
    if (basis == HdTokens->bspline) {
      ...
      eVaryingPrimvarIndices = {0, 0, 0, 1, 1, 1, 1, 1, 1, 2, 3, 0, 1,
                                1, 1, 2, 2, 2, 3, 3, 3, 1, 1, 2, 2};
    }
      ...
      eCurveIndices = {0,  0,  1,  2,  3,  3,  4,  4,  5,  6,  7,  8, 9,
                       10, 10, 11, 11, 12, 13, 14, 14, 15, 15, 16, 16};

      // Primvars
      eVertexPrimvarIndices = {0, 0, 1, 2, 3, 3, 1, 1, 2, 3, 0, 1, 2,
                               3, 3, 2, 2, 3, 0, 1, 1, 1, 1, 2, 2};
    ...
    if (hasIndexedPrimvar) {
      ...
      primvars.push_back(
          {TfToken("fooVarying"), VtValue(eVaryingPrimvar), HdPrimvarSchemaTokens->varying});
    }
  }
  return std::make_pair(/* authored */ c, /* expected */ e);
}

bool _Compare(const HdContainerDataSourceHandle &baseline,
...
bool TestPinnedCurves(bool hasCurveIndices, bool hasIndexedPrimvar)
{
  bool success = true;
  {
    // 1. Pinned bspline
    const auto curves = _GetAuthoredAndExpectedTestCurves(
    ...
    success &= _Compare(_BuildCurveDataSource(curves.second),
