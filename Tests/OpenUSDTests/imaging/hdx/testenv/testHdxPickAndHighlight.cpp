...
typedef std::unordered_map<SdfPath, std::vector<VtIntArray>, SdfPath::Hash> InstanceMap;

// helper function that returns prims with selected instances in a map.
static InstanceMap _GetSelectedInstances(HdSelectionSharedPtr const &sel,
                                         HdSelection::HighlightMode const &mode)
{
  ...
  for (const auto &path : selPrimPaths) {
    HdSelection::PrimSelectionState const *primSelState = sel->GetPrimSelectionState(mode, path);
                                          ...
                                          GfMatrix4d const &viewMatrix)
{
  ...
  return HdxUnitTestUtils::TranslateHitsToSelection(p.pickTarget, mode, allHits);
}
...
void My_TestGLDrawing::_InitScene()
{
...

  {
    ...
    delegate.AddTet(SdfPath("/protoBottom"), GfMatrix4d(1), false, SdfPath("/instancerBottom"));
...
{
  return _driver->Pick(
      startPos, endPos, mode, GetWidth(), GetHeight(), GetFrustum(), GetViewMatrix());
}
...
void My_TestGLDrawing::OffscreenTest()
{
  ...
  HdSelectionSharedPtr selection = _Pick(GfVec2i(180, 390), GfVec2i(181, 391), mode);
  ...
  {
    std::vector<VtIntArray> const &indices = selInstances[SdfPath("/protoTop")];
    TF_VERIFY(indices.size() == 2);
    TF_VERIFY(indices[0][0] == 1 || indices[0][0] == 2);
    TF_VERIFY(indices[1][0] == 1 || indices[1][0] == 2);
  }
  {
    std::vector<VtIntArray> const &indices = selInstances[SdfPath("/protoBottom")];
    TF_VERIFY(indices.size() == 2);
    TF_VERIFY(indices[0][0] == 1 || indices[0][0] == 2);
    TF_VERIFY(indices[1][0] == 1 || indices[1][0] == 2);
  }
...
void My_TestGLDrawing::MouseRelease(int button, int x, int y, int modKeys)
{
  ...
  if (!(modKeys & GarchGLDebugWindow::Alt)) {
    HdSelectionSharedPtr selection = _Pick(_startPos, _endPos, HdSelection::HighlightModeSelect);
    _selTracker->SetSelection(selection);
  }
