...
void Hdx_TestDriver::_Init(HdReprSelector const &reprSelector)
{
  ...
  const auto sceneReprSel = HdReprSelector(
      HdReprTokens->wireOnSurf, HdReprTokens->disabled, _tokens->meshPoints);
...
void My_TestGLDrawing::OffscreenTest()
{
  ...
  // 1. Nearest to camera
  {
    HdSelectionSharedPtr selection = _Pick(pickStartPos,
                                           pickEndPos,
    ...
    TF_VERIFY(selection->GetSelectedPrimPaths(mode)[0] == SdfPath("/protoTop"));
  }
  ...
  // 2. Nearest to center (of pick region)
  {
    ...
    TF_VERIFY(allHits.size() == 1);
    TF_VERIFY(selection->GetSelectedPrimPaths(mode).size() == 1);
    TF_VERIFY(selection->GetSelectedPrimPaths(mode)[0] == SdfPath("/protoBottom"));
  }
  ...
  // 3. Unique
  {
    ...
    TfTokenVector pickTargets = {HdxPickTokens->pickPrimsAndInstances,
                                 HdxPickTokens->pickFaces,
                                 HdxPickTokens->pickEdges,
                                 HdxPickTokens->pickPoints};
    size_t expectedHitCount[] = {
        6 /*primsAndInstances*/, 69 /*faces*/, 135 /*edges*/, 41 /*points*/};

    for (size_t i = 0; i < pickTargets.size(); i++) {
      allHits.clear();
      HdSelectionSharedPtr selection = _Pick(
          pickStartPos, pickEndPos, pickTargets[i], HdxPickTokens->resolveUnique, &allHits);
      std::cout << "allHits: " << allHits.size() << " expectedHitCount:  " << expectedHitCount[i]
                << std::endl;
...
void My_TestGLDrawing::MouseRelease(int button, int x, int y, int modKeys)
{
  ...
  if (!(modKeys & GarchGLDebugWindow::Alt)) {
    std::cout << "Pick region: (" << _startPos << ") to (" << _endPos << ")" << std::endl;
    HdxPickHitVector allHits;
    HdSelectionSharedPtr selection = _Pick(_startPos,
                                           _endPos,
                                           HdxPickTokens->pickPrimsAndInstances,
                                           HdxPickTokens->resolveNearestToCenter,
                                           &allHits);
