...
HdSelectionSharedPtr My_TestGLDrawing::_Pick(GfVec2i const &startPos, GfVec2i const &endPos)
{
  return _driver->Pick(startPos, endPos, GetWidth(), GetHeight(), GetFrustum(), GetViewMatrix());
}
...
void My_TestGLDrawing::OffscreenTest()
{
  ...
  // (b)
  {
    ...
    delegate.SetReprName(SdfPath("/cube2"), HdReprTokens->refinedWireOnSurf);
