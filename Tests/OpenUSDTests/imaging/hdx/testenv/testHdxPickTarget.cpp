...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (meshPoints)(pickables));

class Hdx_TestDriver : public HdSt_TestDriverBase<Hdx_UnitTestDelegate> {
 public:
  Hdx_TestDriver();

  void DrawWithSelection(GfVec4d const &viewport, HdxSelectionTrackerSharedPtr selTracker);

  HdSelectionSharedPtr Pick(GfVec2i const &startPos,
  ...
  void SetSceneColReprSelector(HdReprSelector const &reprSelector)
  {
    ...
    GetDelegate().SetTaskParam(renderTask, HdTokens->collection, VtValue(_collection));
  }
...
void Hdx_TestDriver::_Init(HdReprSelector const &reprSelector)
{
  ...
  delegate.SetTaskParam(renderTask, HdTokens->collection, VtValue(_collection));
  ...
  // picking
  _pickablesCol = HdRprimCollection(_tokens->pickables, HdReprSelector(HdReprTokens->refined));
  // We have to unfortunately explictly add collections besides 'geometry'
  // See HdRenderIndex constructor.
  delegate.GetRenderIndex().GetChangeTracker().AddCollection(_tokens->pickables);
}

void Hdx_TestDriver::DrawWithSelection(GfVec4d const &viewport,
                                       HdxSelectionTrackerSharedPtr selTracker)
{
  ...
  HdxRenderTaskParams param =
      GetDelegate().GetTaskParam(renderSetupTask, HdTokens->params).Get<HdxRenderTaskParams>();
  param.viewport = viewport;
  param.aovBindings = _aovBindings;
  GetDelegate().SetTaskParam(renderSetupTask, HdTokens->params, VtValue(param));
  ...
  _GetEngine()->SetTaskContextData(HdxTokens->selectionState, VtValue(selTracker));
  _GetEngine()->Execute(&GetDelegate().GetRenderIndex(), &tasks);
}

HdSelectionSharedPtr Hdx_TestDriver::Pick(GfVec2i const &startPos,
                                          GfVec2i const &endPos,
                                          TfToken const &pickTarget,
                                          int width,
                                          int height,
                                          ...
                                          GfMatrix4d const &viewMatrix)
{
  ...
  p.resolution = HdxUnitTestUtils::CalculatePickResolution(startPos, endPos, GfVec2i(4, 4));
  ...
  HdTaskSharedPtrVector tasks;
  tasks.push_back(GetDelegate().GetRenderIndex().GetTask(SdfPath("/pickTask")));
...
class My_TestGLDrawing : public HdSt_UnitTestGLDrawing {
 ...
 protected:
  ...
  MeshEdges _GetMeshEdges(SdfPath const &meshPath, VtIntArray const &edgeIndices) const;
...
static GfMatrix4d _GetTransform(GfRotation rot, GfVec3d translate)
{
  GfMatrix4d xform;
  xform.SetRotate(rot);
...
My_TestGLDrawing::MeshEdges My_TestGLDrawing::_GetMeshEdges(SdfPath const &meshPath,
                                                            VtIntArray const &edgeIds) const
{
  HdRprim const *rprim = _driver->GetDelegate().GetRenderIndex().GetRprim(meshPath);
...
void My_TestGLDrawing::_InitScene()
{
...
HdSelectionSharedPtr My_TestGLDrawing::_Pick(GfVec2i const &startPos,
                                             GfVec2i const &endPos,
                                             TfToken const &pickTarget)
{
  return _driver->Pick(
      startPos, endPos, pickTarget, GetWidth(), GetHeight(), GetFrustum(), GetViewMatrix());
}
...
void My_TestGLDrawing::OffscreenTest()
{
  ...
  // select tet1
  {
    HdSelectionSharedPtr selection = _Pick(
        GfVec2i(436, 127), GfVec2i(452, 139), HdxPickTokens->pickPrimsAndInstances);
    _selTracker->SetSelection(selection);
    DrawScene();
    _driver->WriteToFile("color", "color10_tet1_pick_prims.png");
    HdSelection::PrimSelectionState const *selState = selection->GetPrimSelectionState(
        mode, SdfPath("/tet1"));
    TF_VERIFY(selState);
  ...
  _driver->SetPickablesColReprSelector(HdReprSelector(HdReprTokens->wireOnSurf));
  ...
  // select edge of tet0
  {
    ...
    HdSelection::PrimSelectionState const *selState = selection->GetPrimSelectionState(mode,
                                                                                       meshPath);
    TF_VERIFY(selState);
  ...
  _driver->SetSceneColReprSelector(
...

  {
    ...
    _driver->SetPickablesColReprSelector(
        HdReprSelector(HdReprTokens->wire, HdReprTokens->disabled, _tokens->meshPoints));
    HdSelectionSharedPtr selection = _Pick(
        GfVec2i(346, 215), GfVec2i(492, 427), HdxPickTokens->pickPoints);
    _selTracker->SetSelection(selection);
    DrawScene();
  ...
  // manually verify if specifying a color for a set of points works.
  {
    ...
    _driver->SetSceneColReprSelector(
        HdReprSelector(HdReprTokens->disabled, HdReprTokens->disabled, _tokens->meshPoints));
...
void My_TestGLDrawing::DrawScene()
{
...
void My_TestGLDrawing::DrawMarquee()
{
...
void My_TestGLDrawing::MouseRelease(int button, int x, int y, int modKeys)
{
  ...
  if (!(modKeys & GarchGLDebugWindow::Alt)) {
    ...
    HdSelectionSharedPtr selection = _Pick(_startPos, _endPos, HdxPickTokens->pickFaces);
    _selTracker->SetSelection(selection);
  }
