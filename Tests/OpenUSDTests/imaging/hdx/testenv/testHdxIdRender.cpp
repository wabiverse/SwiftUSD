...
class Hdx_TestDriver : public HdSt_TestDriverBase<Hdx_UnitTestDelegate> {
 public:
  ...
  template<typename T> HdStTextureUtils::AlignedBuffer<T> ReadAovBuffer(TfToken const &aovName);
...
void Hdx_TestDriver::_Init(HdReprSelector const &reprSelector)
{
  _SetupSceneDelegate();
  ...
  HdxRenderTaskParams param =
      delegate.GetTaskParam(renderSetupTask, HdTokens->params).Get<HdxRenderTaskParams>();
  param.enableLighting = true;  // use default lighting
  delegate.SetTaskParam(renderSetupTask, HdTokens->params, VtValue(param));
  delegate.SetTaskParam(renderTask,
                        HdTokens->collection,
                        VtValue(HdRprimCollection(HdTokens->geometry, reprSelector)));
}

void Hdx_TestDriver::Draw(GfVec4d const &viewport, PickParam const *pickParam)
{
  Hdx_UnitTestDelegate &delegate = GetDelegate();
...
static TfTokenVector _pickableAovOutputs = {
    HdAovTokens->primId, HdAovTokens->instanceId, HdAovTokens->depth};

void Hdx_TestDriver::SetupPickableAovs(int width, int height)
{
  if (_pickableAovBindings.empty()) {
    // Delete old render buffers.
    for (auto const &id : _pickableAovBufferIds) {
      GetDelegate().GetRenderIndex().RemoveBprim(HdPrimTypeTokens->renderBuffer, id);
    }
    ...
    for (size_t i = 0; i < _pickableAovOutputs.size(); i++) {
      ...
      HdAovDescriptor aovDesc = _GetRenderDelegate()->GetDefaultAovDescriptor(
          _pickableAovOutputs[i]);

      HdRenderBufferDescriptor desc = {dimensions,
                                       aovDesc.format,
                                       /*multiSampled*/ false};
      ...
      binding.renderBuffer = dynamic_cast<HdRenderBuffer *>(
          GetDelegate().GetRenderIndex().GetBprim(HdPrimTypeTokens->renderBuffer, aovId));
      binding.clearValue = VtValue(GfVec4f(1));
    }
  }
}

HdRenderBuffer const *Hdx_TestDriver::_FindAovBuffer(TfToken const &aovName) const
{
...
template<typename T>
HdStTextureUtils::AlignedBuffer<T> Hdx_TestDriver::ReadAovBuffer(TfToken const &aovName)
{
  ...
  if (aov.IsHolding<HgiTextureHandle>()) {
    ...
    if (texture) {
      size_t bufferSize = 0;
      return HdStTextureUtils::HgiTextureReadback<T>(_GetHgi(), texture, &bufferSize);
    }
  }
...
void Hdx_TestDriver::UpdatePickableAovDimensions(int width, int height)
{
  const GfVec3i dimensions(width, height, 1);

  for (auto const &id : _pickableAovBufferIds) {
    HdRenderBufferDescriptor desc = GetDelegate().GetRenderBufferDescriptor(id);
...
static GfMatrix4d _GetTranslate(float tx, float ty, float tz)
{
  GfMatrix4d m(1.0f);
...
void My_TestGLDrawing::InitTest()
{
  ...
  GfVec4f red(1, 0, 0, 1), green(0, 1, 0, 1), blue(0, 0, 1, 1), yellow(1, 1, 0, 1),
      magenta(1, 0, 1, 1), cyan(0, 1, 1, 1), white(1, 1, 1, 1), black(0, 0, 0, 1);

  GfVec4f faceColors[] = {red, green, blue, yellow, magenta, cyan};
  VtValue faceColor = VtValue(
      _BuildArray(&faceColors[0], sizeof(faceColors) / sizeof(faceColors[0])));

  GfVec4f vertColors[] = {white, blue, green, yellow, black, blue, magenta, red};
  VtValue vertColor = VtValue(
      _BuildArray(&vertColors[0], sizeof(vertColors) / sizeof(vertColors[0])));

  delegate.AddCube(SdfPath("/cube0"),
                   _GetTranslate(5, 0, 5),
                   /*guide=*/false,
                   /*instancerId=*/SdfPath(),
  ...
  delegate.AddCube(SdfPath("/cube1"),
                   _GetTranslate(-5, 0, 5),
                   /*guide=*/false,
                   /*instancerId=*/SdfPath(),
                   /*scheme=*/PxOsdOpenSubdivTokens->catmullClark,
  ...
  delegate.AddCube(SdfPath("/cube3"),
                   _GetTranslate(5, 0, -5),
                   /*guide=*/false,
                   /*instancerId=*/SdfPath(),
                   /*scheme=*/PxOsdOpenSubdivTokens->catmullClark,
...

  {
    ...
    delegate.AddCube(SdfPath("/protoTop"), GfMatrix4d(1), false, SdfPath("/instancerTop"));
    ...
    delegate.SetInstancerProperties(
        SdfPath("/instancerTop"), prototypeIndex, scale, rotate, translate);
  }

  {
    delegate.AddInstancer(SdfPath("/instancerBottom"));
    delegate.AddCube(SdfPath("/protoBottom"), GfMatrix4d(1), false, SdfPath("/instancerBottom"));
    ...
    delegate.SetInstancerProperties(
        SdfPath("/instancerBottom"), prototypeIndex, scale, rotate, translate);
  }
...
void My_TestGLDrawing::DrawScene(PickParam const *pickParam)
{
...
SdfPath My_TestGLDrawing::PickScene(int pickX, int pickY, int *outInstanceIndex)
{
  int width = 128;
  int height = 128;
  ...
  HdStTextureUtils::AlignedBuffer<unsigned char> primId = _driver->ReadAovBuffer<unsigned char>(
      HdAovTokens->primId);

  HdStTextureUtils::AlignedBuffer<unsigned char> instanceId =
      _driver->ReadAovBuffer<unsigned char>(HdAovTokens->instanceId);

  HdStTextureUtils::AlignedBuffer<float> depths = _driver->ReadAovBuffer<float>(
      HdAovTokens->depth);
  ...
  if (didHit) {
    ...
    if (outInstanceIndex) {
      *outInstanceIndex = HdxPickTask::DecodeIDRenderColor(instanceId.get() + idIndex);
    }
  }
...
void My_TestGLDrawing::Present(uint32_t framebuffer)
{
