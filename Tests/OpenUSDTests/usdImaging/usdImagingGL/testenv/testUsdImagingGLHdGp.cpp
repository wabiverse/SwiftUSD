...
int main(int argc, char *argv[])
{
  ...
  HdSceneIndexBaseRefPtr sceneIndex = HdSceneIndexNameRegistry::GetInstance().GetNamedSceneIndex(
  ...
  std::cout << "Checking initial child count of: " << cubePerMeshProcPrimPath << std::endl;
  TF_AXIOM(sceneIndex->GetChildPrimPaths(cubePerMeshProcPrimPath).size() == (4 + inputChildCount));
  ...
  if (cubePerMeshProcPrim) {
    srcMeshRel = cubePerMeshProcPrim.GetRelationship(TfToken("primvars:sourceMeshPath"));
  }

  //-------------------------------------------------------------------------

  std::cout << "retargeting 'primvars:sourceMeshPath' of " << cubePerMeshProcPrimPath << std::endl;
  ...
  std::cout << "Checking adjusted child count of: " << cubePerMeshProcPrimPath << "..."
            << std::endl;
  TF_AXIOM(sceneIndex->GetChildPrimPaths(cubePerMeshProcPrimPath).size() == (8 + inputChildCount));
  std::cout << "...OK" << std::endl;

  //-------------------------------------------------------------------------
  // confirm dirtied xforms of child prims with myMesh frame change

  auto _CountChildPrimsWithDirtiedXforms = [&observer](const SdfPath &parentPath) {
    std::unordered_set<SdfPath, TfHash> childPrimsWithDirtiedXforms;
    for (const auto &e : observer.GetEvents()) {
      if (e.eventType == _RecordingSceneIndexObserver::EventType_PrimDirtied &&
          e.primPath.HasPrefix(parentPath) && e.primPath != parentPath)
      {
...

  {
    ...
    std::cout << "confirming count of child prims with dirtied xforms..." << std::endl;

    TF_AXIOM(_CountChildPrimsWithDirtiedXforms(cubePerMeshProcPrimPath) == 8);
    std::cout << "...OK" << std::endl;
  }
  //-------------------------------------------------------------------------
  {
    std::cout << "restoring 'primvars:sourceMeshPath' of " << cubePerMeshProcPrimPath << std::endl;
    ...
    std::cout << "Checking restored child count of: " << cubePerMeshProcPrimPath << "..."
              << std::endl;
  ...
  // as myMesh2 does not animate
  {
    ...
    std::cout << "confirming no child prims with dirtied xforms..." << std::endl;

    TF_AXIOM(_CountChildPrimsWithDirtiedXforms(cubePerMeshProcPrimPath) == 0);
    std::cout << "...OK" << std::endl;
  }
  ...
  {
    observer.Clear();
    cubePerMeshProcPrim.GetAttribute(TfToken("primvars:scale")).Set(VtValue(1.25f));
    engine->Render(stage->GetPseudoRoot(), params);

    std::cout << "setting 'primvars:scale' of " << cubePerMeshProcPrimPath << std::endl;

    std::cout << "confirming child prims xform dirtied..." << std::endl;

    TF_AXIOM(_CountChildPrimsWithDirtiedXforms(cubePerMeshProcPrimPath) == 4);
    std::cout << "...OK" << std::endl;
  }
  ...
  {
    auto childPaths = sceneIndex->GetChildPrimPaths(makeSomeStuffProcPrimPath);

    std::cout << "confirming initial child count of " << makeSomeStuffProcPrimPath << "..."
              << std::endl;
    TF_AXIOM(childPaths.size() == 3);
    std::cout << "...OK" << std::endl;

    std::cout << "confirming child prim types of " << makeSomeStuffProcPrimPath << "..."
              << std::endl;
    for (const SdfPath &childPath : childPaths) {
      TF_AXIOM(sceneIndex->GetPrim(childPath).primType == TfToken("stuff"));
    }
    std::cout << "...OK" << std::endl;

    std::cout << "changing 'primvars:myDepth' of  " << makeSomeStuffProcPrimPath << "..."
              << std::endl;
    ...
    std::cout << "confirming child and grandchild types " << makeSomeStuffProcPrimPath << "..."
              << std::endl;
    for (const SdfPath &childPath : sceneIndex->GetChildPrimPaths(makeSomeStuffProcPrimPath)) {
      TF_AXIOM(sceneIndex->GetPrim(childPath).primType == TfToken());

      for (const SdfPath &grandChildPath : sceneIndex->GetChildPrimPaths(childPath)) {
        TF_AXIOM(sceneIndex->GetPrim(grandChildPath).primType == TfToken("stuff"));
      }
    }
    std::cout << "...OK" << std::endl;
  }
