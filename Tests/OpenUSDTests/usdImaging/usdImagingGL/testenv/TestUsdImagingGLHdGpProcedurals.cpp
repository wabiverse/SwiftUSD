...
TF_DEFINE_PRIVATE_TOKENS(_makeSomeStuffTokens, (stuff)(myCount)(myDepth)(addHierarchySparsely));

TF_DEFINE_PRIVATE_TOKENS(_childNameTokens, (A)(B)(C)(D)(E)(F));
...
class _MakeSomeStuffGenerativeProcedural : public HdGpGenerativeProcedural {
 public:
                          ...
                          HdSceneIndexObserver::DirtiedPrimEntries *outputDirtiedPrims) override
  {
    ...
    if (HdSampledDataSourceHandle countDs = HdPrimvarsSchema::GetFromParent(myPrim.dataSource)
    ...
    int depth = 1;
    if (HdSampledDataSourceHandle ds = HdPrimvarsSchema::GetFromParent(myPrim.dataSource)
                                ...
                                const SdfPath &childPrimPath) override
  {
    ...
    if (_addHierarchySparsely && static_cast<int>(relPath.GetPathElementCount()) < _depth) {
      return HdSceneIndexPrim();
    }

    return {_makeSomeStuffTokens->stuff,
            HdRetainedContainerDataSource::New(_makeSomeStuffTokens->stuff,
...
class MakeSomeStuffGenerativeProceduralPlugin : public HdGpGenerativeProceduralPlugin {
 public:
...
TF_DEFINE_PRIVATE_TOKENS(_cubePerMeshTokens, (sourceMeshPath)(scale));
                          ...
                          HdSceneIndexObserver::DirtiedPrimEntries *outputDirtiedPrims) override
  {
    ...
    if (_meshPointsDs) {
      ...
      if (args.sourceMeshPath == _args.sourceMeshPath) {
        ...
        if (dirtiedDependencies.find(args.sourceMeshPath) == dirtiedDependencies.end()) {
    ...
    if (!_meshPointsDs) {
      ...
      if (sourceMeshPrim.primType == HdPrimTypeTokens->mesh) {
        ...
        if (HdSampledDataSourceHandle pointsDs = HdPrimvarsSchema::GetFromParent(
    ...
    for (int i = 0; i < static_cast<int>(points.size()); ++i) {
      ...
      if (!_childIndices.insert({childPath, i}).second) {
        if (outputDirtiedPrims) {
          outputDirtiedPrims->emplace_back(childPath, HdXformSchema::GetDefaultLocator());
        }
      }
      ...
      if (it != _childIndices.end()) {
        ...
        result.dataSource = HdRetainedContainerDataSource::New(
            ...
            HdXformSchema::Builder()
                .SetMatrix(
                    _XformFromMeshPointDataSource::New(_args.scale, it->second, _meshPointsDs))
                .Build(),
 ...
 private:
  ...
  class _XformFromMeshPointDataSource : public HdMatrixDataSource {
   public:
    ...
    {
      VtArray<GfVec3f> p = _pointsDs->GetValue(shutterOffset).UncheckedGet<VtArray<GfVec3f>>();
   ...
   private:
    _XformFromMeshPointDataSource(float scale, int index, HdSampledDataSourceHandle pointsDs)
        : _scale(scale), _index(index), _pointsDs(pointsDs)
    {
    }
  ...
  _Args _GetArgs(const HdSceneIndexBaseRefPtr &inputScene)
  {
    ...
    if (HdSampledDataSourceHandle sourceMeshDs =
            primvars.GetPrimvar(_cubePerMeshTokens->sourceMeshPath).GetPrimvarValue())
    {
    ...
    if (HdSampledDataSourceHandle ds =
            primvars.GetPrimvar(_cubePerMeshTokens->scale).GetPrimvarValue())
    {
      VtValue v = ds->GetValue(0.0f);
      ...
      HdSceneIndexObserver::DirtiedPrimEntries *outputDirtiedPrims)
  {
    if (outputDirtiedPrims) {
      for (const auto &pathTypePair : childTypes) {
        outputDirtiedPrims->emplace_back(pathTypePair.first, HdXformSchema::GetDefaultLocator());
      }
    }
    return childTypes;
  }
  ...
  HdContainerDataSourceHandle _GetChildMeshDs()
  {
    ...
    static const _IntArrayDs::Handle fviDs = _IntArrayDs::New(faceVertexIndices);

    static const HdContainerDataSourceHandle meshDs = HdMeshSchema::Builder()
                                                          .SetTopology(
  ...
  {
    static const VtArray<GfVec3f> points = {{-0.1f, -0.1f, 0.1f},
                                            {0.1f, -0.1f, 0.1f},
                                            {-0.1f, 0.1f, 0.1f},
                                            {0.1f, 0.1f, 0.1f},
                                            {-0.1f, 0.1f, -0.1f},
    ...
    static const HdContainerDataSourceHandle primvarsDs = HdRetainedContainerDataSource::New(
...
class CubePerMeshPointProceduralPlugin : public HdGpGenerativeProceduralPlugin {
 public:
...
{
  HdGpGenerativeProceduralPluginRegistry::Define<MakeSomeStuffGenerativeProceduralPlugin,
                                                 HdGpGenerativeProceduralPlugin>();

  HdGpGenerativeProceduralPluginRegistry::Define<CubePerMeshPointProceduralPlugin,
                                                 HdGpGenerativeProceduralPlugin>();
}
